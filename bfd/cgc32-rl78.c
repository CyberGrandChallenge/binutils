/* Renesas RL78 specific support for 32-bit CGC.
...
#define RL78REL(n,sz,bit,shift,complain,pcrel)				     \
  HOWTO (R_RL78_##n, shift, sz, bit, pcrel, 0, complain_overflow_ ## complain, \
	 bfd_cgc_generic_reloc, "R_RL78_" #n, FALSE, 0, ~0, FALSE)
...
static reloc_howto_type rl78_cgc_howto_table [] =
{
...

/* Map BFD reloc types to RL78 CGC reloc types.  */
			...
			bfd_reloc_code_real_type code)
{
  ...
  if (code == BFD_RELOC_RL78_32_OP)
    return rl78_cgc_howto_table + R_RL78_DIR32;

  for (i = ARRAY_SIZE (rl78_reloc_map); --i;)
    if (rl78_reloc_map [i].bfd_reloc_val == code)
      return rl78_cgc_howto_table + rl78_reloc_map[i].rl78_reloc_val;
...
rl78_reloc_name_lookup (bfd * abfd ATTRIBUTE_UNUSED, const char * r_name)
{
  ...
  for (i = 0; i < ARRAY_SIZE (rl78_cgc_howto_table); i++)
    if (rl78_cgc_howto_table[i].name != NULL
	&& strcasecmp (rl78_cgc_howto_table[i].name, r_name) == 0)
      return rl78_cgc_howto_table + i;

  return NULL;
}

/* Set the howto pointer for an RL78 CGC reloc.  */
			 ...
			 Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = rl78_cgc_howto_table + r_type;
}

static bfd_vma
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = rl78_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
      relocation = 0;
...
static bfd_boolean
rl78_cgc_set_private_flags (bfd * abfd, flagword flags)
{
...
void bfd_cgc32_rl78_set_target_flags (bfd_boolean);

void
bfd_cgc32_rl78_set_target_flags (bfd_boolean user_no_warn_mismatch)
{
...
static bfd_boolean
rl78_cgc_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
  flagword new_flags;
  flagword old_flags;
...

static bfd_boolean
rl78_cgc_print_private_bfd_data (bfd * abfd, void * ptr)
{
  FILE * file = (FILE *) ptr;
  flagword flags;
...
static int
cgc32_rl78_machine (bfd * abfd)
{
  if ((cgc_cgcheader (abfd)->e_flags & EF_RL78_CPU_MASK) == EF_RL78_CPU_RL78)
...
static bfd_boolean
rl78_cgc_object_p (bfd * abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_rl78,
			     cgc32_rl78_machine (abfd));
  return TRUE;
}
 
#ifdef DEBUG
...
rl78_get_reloc (long reloc)
{
  if (0 <= reloc && reloc < R_RL78_max)
    return rl78_cgc_howto_table[reloc].name;
  return "";
}
#endif /* DEBUG */
...
static bfd_boolean
rl78_cgc_finish_dynamic_sections (bfd *abfd ATTRIBUTE_UNUSED,
                                  struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *splt;
...
static bfd_boolean
rl78_cgc_always_size_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
                               struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *splt;
...
/* A subroutine of rl78_cgc_relax_section.  If the global symbol H
...
static bfd_boolean
rl78_relax_plt_check (struct cgc_link_hash_entry *h, void * xdata)
{
...
static bfd_boolean
rl78_relax_plt_realloc (struct cgc_link_hash_entry *h, void * xdata)
{
...
static bfd_boolean
rl78_cgc_relax_plt_section (bfd *dynobj,
                            asection *splt,
                            struct bfd_link_info *info,
                            bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  cgc_link_hash_traverse (cgc_hash_table (info), rl78_relax_plt_check,
			  &relax_plt_data);
...
static bfd_boolean
cgc32_rl78_relax_delete_bytes (bfd *abfd, asection *sec, bfd_vma addr, int count,
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
	...
	irel->r_offset -= count;
	 ...
	 they're for.  */
	  ...
	  && CGC32_R_TYPE (irel->r_info) == R_RL78_RH_RELAX
  ...
  while (again)
      ...
      for (i = 0; i < count - 1; i ++)
	{
	  if (r[i].r_offset > r[i + 1].r_offset)
	    ...
	    swappit = FALSE;
	  else if (CGC32_R_TYPE (r[i + 1].r_info) == R_RL78_RH_RELAX
		   && (r[i + 1].r_addend & RL78_RELAXA_ALIGN))
	    swappit = TRUE;
	  else if (CGC32_R_TYPE (r[i + 1].r_info) == R_RL78_RH_RELAX
		   && (r[i + 1].r_addend & RL78_RELAXA_ELIGN)
		   && !(CGC32_R_TYPE (r[i].r_info) == R_RL78_RH_RELAX
     ...
     bfd_boolean *          again)
{
  ...
  if (abfd == cgc_hash_table (link_info)->dynobj
    ...
    return rl78_cgc_relax_plt_section (abfd, sec, link_info, again);
  ...
  *again = FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      if (CGC32_R_TYPE (irel->r_info) == R_RL78_RH_RELAX
	  && irel->r_addend & RL78_RELAXA_ALIGN)
	{
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	  ...
	  || next_alignment == internal_relocs)
	{
	  ...
	  while (next_alignment < irelend
		 && (CGC32_R_TYPE (next_alignment->r_info) != R_RL78_RH_RELAX
      ...
      if (irel->r_addend & RL78_RELAXA_ALIGN)
	{
	  ...
	  if (CGC32_R_TYPE (erel->r_info) != R_RL78_RH_RELAX)
	    continue;
	  ...
	  cgc32_rl78_relax_delete_bytes (abfd, sec, erel->r_offset-nbytes, nbytes, next_alignment,
...
#define SNIPNR(offset, nbytes) \
	cgc32_rl78_relax_delete_bytes (abfd, sec, (insn - contents) + offset, nbytes, next_alignment, 0);
...
#define CGC_ARCH		bfd_arch_rl78
#define CGC_MACHINE_CODE	EM_RL78
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_LITTLE_SYM	bfd_cgc32_rl78_vec
#define TARGET_LITTLE_NAME	"cgc32-rl78"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			rl78_info_to_howto_rela
#define cgc_backend_object_p			rl78_cgc_object_p
#define cgc_backend_relocate_section		rl78_cgc_relocate_section
#define cgc_symbol_leading_char                 ('_')
#define cgc_backend_can_gc_sections		1

#define bfd_cgc32_bfd_reloc_type_lookup		rl78_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		rl78_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		rl78_cgc_set_private_flags
#define bfd_cgc32_bfd_merge_private_bfd_data	rl78_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	rl78_cgc_print_private_bfd_data

#define bfd_cgc32_bfd_relax_section		rl78_cgc_relax_section
#define cgc_backend_check_relocs                rl78_cgc_check_relocs
#define cgc_backend_always_size_sections \
  rl78_cgc_always_size_sections
#define cgc_backend_finish_dynamic_sections \
  rl78_cgc_finish_dynamic_sections
