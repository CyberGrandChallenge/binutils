...
static bfd_boolean
segment_executable (struct cgc_segment_map *seg)
{
...
static bfd_boolean
segment_eligible_for_headers (struct cgc_segment_map *seg,
...
/* We permute the segment_map to get BFD to do the file layout we want:
   ...
   and contains the CGC file header and phdrs.  */
bfd_boolean
nacl_modify_segment_map (bfd *abfd, struct bfd_link_info *info)
{
  const struct cgc_backend_data *const bed = get_cgc_backend_data (abfd);
  struct cgc_segment_map **m = &cgc_seg_map (abfd);
  struct cgc_segment_map **first_load = NULL;
  struct cgc_segment_map **last_load = NULL;
	  ...
	  if (executable
	      ...
	      if (end % bed->minpagesize != 0)
		{
		  /* This is an executable segment that starts on a page
		     ...
		     assign_file_positions_for_load_sections (cgc.c) so
	  ...
	  /* Now that we've noted the first PT_LOAD, we're looking for
						    ...
						    sizeof_headers))
	    {
		 ...
		 say they include the file header and phdrs.  */
	      struct cgc_segment_map *prevseg;
  ...
  if (first_load != last_load && moved_headers)
    {
	 ...
	 positions in segment_map.  */
      struct cgc_segment_map *first = *first_load;
      struct cgc_segment_map *last = *last_load;
...
/* After nacl_modify_segment_map has done its work, the file layout has
   ...
   proper order for the CGC rule that they must appear in ascending address
...
nacl_modify_program_headers (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (*m != NULL)
    {
      struct cgc_segment_map **first_load_seg = m;
      Cgc_Internal_Phdr *first_load_phdr = p;
      struct cgc_segment_map **next_load_seg = NULL;
      Cgc_Internal_Phdr *next_load_phdr = NULL;
	...
	{
	  Cgc_Internal_Phdr move_phdr;
	  struct cgc_segment_map *first_seg = *first_load_seg;
	  struct cgc_segment_map *next_seg = *next_load_seg;
	  struct cgc_segment_map *first_next = first_seg->next;
	  struct cgc_segment_map *next_next = next_seg->next;
...
{
  struct cgc_segment_map *seg;
  for (seg = cgc_seg_map (abfd); seg != NULL; seg = seg->next)
    if (seg->p_type == PT_LOAD
	&& seg->count > 1
	...
	if (fill == NULL
	    ...
	    || bfd_bwrite (fill, sec->size, abfd) != sec->size)
	  {
	    /* We don't have a proper way to report an error here.  So
	       instead fudge things so that cgc_write_shdrs_and_ehdr will
	       fail.  */
	    cgc_cgcheader (abfd)->e_shoff = (file_ptr) -1;
	  }
