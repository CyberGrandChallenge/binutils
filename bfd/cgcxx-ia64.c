...
/* Perform a relocation.  Not much to do here as all the hard work is
   done in cgcNN_ia64_final_link_relocate.  */
static bfd_reloc_status_type
ia64_cgc_reloc (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc,
...
static unsigned char cgc_code_to_howto_index[R_IA64_MAX_RELOC_CODE + 1];

/* Given a BFD reloc type, return the matching HOWTO structure.  */

reloc_howto_type *
ia64_cgc_lookup_howto (unsigned int rtype)
{
  static int inited = 0;
  int i;
  ...
  if (!inited)
    {
      ...
      memset (cgc_code_to_howto_index, 0xff, sizeof (cgc_code_to_howto_index));
      for (i = 0; i < NELEMS (ia64_howto_table); ++i)
	cgc_code_to_howto_index[ia64_howto_table[i].type] = i;
    }
...
reloc_howto_type*
ia64_cgc_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
                            bfd_reloc_code_real_type bfd_code)
{
  ...
  return ia64_cgc_lookup_howto (rtype);
}

reloc_howto_type *
ia64_cgc_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
                            const char *r_name)
{
  unsigned int i;
...
bfd_boolean
ia64_cgc_relax_br (bfd_byte *contents, bfd_vma off)
{
...
void
ia64_cgc_relax_brl (bfd_byte *contents, bfd_vma off)
{
  int template_val;
  bfd_byte *hit_addr;
...
void
ia64_cgc_relax_ldxmov (bfd_byte *contents, bfd_vma off)
{
  int shift, r1, r3;
  bfd_vma dword, insn;
...
ia64_cgc_install_value (bfd_byte *hit_addr, bfd_vma v, unsigned int r_type)
{
