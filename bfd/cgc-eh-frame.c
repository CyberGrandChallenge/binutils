...
/* Called before calling _bfd_cgc_parse_eh_frame on every input bfd's
   .eh_frame section.  */

void
_bfd_cgc_begin_eh_frame_parsing (struct bfd_link_info *info)
{
  struct eh_frame_hdr_info *hdr_info;

  hdr_info = &cgc_hash_table (info)->eh_info;
...
void
_bfd_cgc_parse_eh_frame (bfd *abfd, struct bfd_link_info *info,
			 asection *sec, struct cgc_reloc_cookie *cookie)
{
#define REQUIRE(COND)					\
  do							\
    if (!(COND))					\
  ...
  ptr_size = (get_cgc_backend_data (abfd)
	      ->cgc_backend_eh_frame_address_size (abfd, sec));
  REQUIRE (ptr_size != 0);
...
#define GET_RELOC(buf)					\
  ...
  buf = ehbuf;
  cie_count = 0;
  gc_mark_hook = get_cgc_backend_data (abfd)->gc_mark_hook;
      ...
      else
	{
	  ...
	  if ((sec->flags & SEC_LINKER_CREATED) == 0 || cookie->rels != NULL)
	      ...
	      if (rsec)
		{
		  ...
		  this_inf->u.fde.next_for_section = cgc_fde_list (rsec);
		  cgc_fde_list (rsec) = this_inf;
		}
	    }
	  ...
	  /* For NULL RSEC (cleared FDE belonging to a discarded section)
	     ...
	     cgc_section_ignore_discarded_relocs.
  ...
  cgc_section_data (sec)->sec_info = sec_info;
...
void
_bfd_cgc_end_eh_frame_parsing (struct bfd_link_info *info)
{
  struct eh_frame_hdr_info *hdr_info;
...
mark_entry (struct bfd_link_info *info, asection *sec,
	    struct eh_cie_fde *ent, cgc_gc_mark_hook_fn gc_mark_hook,
...
bfd_boolean
_bfd_cgc_gc_mark_fdes (struct bfd_link_info *info, asection *sec,
		       asection *eh_frame, cgc_gc_mark_hook_fn gc_mark_hook,
		       struct cgc_reloc_cookie *cookie)
{
  struct eh_cie_fde *fde, *cie;

  for (fde = cgc_fde_list (sec); fde; fde = fde->u.fde.next_for_section)
    {
		 ...
		 struct eh_cie_fde *cie_inf)
{
  ...
  if (cie->per_encoding != DW_EH_PE_omit)
      ...
      else
	{
	  ...
	  sym_sec = bfd_section_from_cgc_index (abfd, sym->st_shndx);
	  if (sym_sec == NULL)
	    return cie_inf;
	  ...
	  && (get_cgc_backend_data (abfd)
	      ->cgc_backend_can_make_relative_eh_frame (abfd, info, sec)))
	{
...
bfd_boolean
_bfd_cgc_discard_section_eh_frame
    ...
    struct cgc_reloc_cookie *cookie)
{
  ...
  sec_info = (struct eh_frame_sec_info *) cgc_section_data (sec)->sec_info;
  if (sec_info == NULL)
    return FALSE;

  ptr_size = (get_cgc_backend_data (sec->owner)
	      ->cgc_backend_eh_frame_address_size (sec->owner, sec));
...
/* This function is called for .eh_frame_hdr section after
   _bfd_cgc_discard_section_eh_frame has been called on all .eh_frame
   input sections.  It finalizes the size of .eh_frame_hdr section.  */

bfd_boolean
_bfd_cgc_discard_section_eh_frame_hdr (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_link_hash_table *htab;
  struct eh_frame_hdr_info *hdr_info;
  asection *sec;
...
bfd_boolean
_bfd_cgc_eh_frame_present (struct bfd_link_info *info)
{
...
bfd_boolean
_bfd_cgc_maybe_strip_eh_frame_hdr (struct bfd_link_info *info)
{
  struct cgc_link_hash_table *htab;
  struct eh_frame_hdr_info *hdr_info;
...
bfd_vma
_bfd_cgc_eh_frame_section_offset (bfd *output_bfd ATTRIBUTE_UNUSED,
...
bfd_boolean
_bfd_cgc_write_section_eh_frame (bfd *abfd,
				 ...
				 bfd_byte *contents)
{
  ...
  /* We don't align the section to its section alignment since the
     ...
     the pointer size. _bfd_cgc_discard_section_eh_frame should
...
/* Write out .eh_frame_hdr section.  This must be called after
   _bfd_cgc_write_section_eh_frame has been called on all input
   .eh_frame sections.
   .eh_frame_hdr format:
...
bfd_boolean
_bfd_cgc_write_section_eh_frame_hdr (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_link_hash_table *htab;
  struct eh_frame_hdr_info *hdr_info;
  asection *sec;
  ...
  if (info->eh_frame_hdr && sec != NULL)
      ...
      if (eh_frame_sec == NULL)
	{
      ...
      contents[0] = 1;
      /* .eh_frame offset.  */
      contents[1] = get_cgc_backend_data (abfd)->cgc_backend_encode_eh_address
...
unsigned int
_bfd_cgc_eh_frame_address_size (bfd *abfd, asection *sec ATTRIBUTE_UNUSED)
{
  return cgc_cgcheader (abfd)->e_ident[EI_CLASS] == CGCCLASS64 ? 8 : 4;
}
...
bfd_boolean
_bfd_cgc_can_make_relative (bfd *input_bfd ATTRIBUTE_UNUSED,
			    struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
bfd_byte
_bfd_cgc_encode_eh_address (bfd *abfd ATTRIBUTE_UNUSED,
			    struct bfd_link_info *info ATTRIBUTE_UNUSED,
