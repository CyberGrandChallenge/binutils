/* DLX specific support for 32-bit CGC
...
#define bfd_cgc32_bfd_reloc_type_lookup cgc32_dlx_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup cgc32_dlx_reloc_name_lookup
#define cgc_info_to_howto               cgc32_dlx_info_to_howto
#define cgc_info_to_howto_rel           cgc32_dlx_info_to_howto_rel
#define cgc_backend_check_relocs        cgc32_dlx_check_relocs
...
static int skip_dlx_cgc_hi16_reloc = 0;
...
{
  skip_dlx_cgc_hi16_reloc = flag;
  return flag;
}

static bfd_reloc_status_type
_bfd_dlx_cgc_hi16_reloc (bfd *abfd,
			 arelent *reloc_entry,
			 asymbol *symbol,
			 void * data,
...
static reloc_howto_type dlx_cgc_howto_table[]=
{
  /* No relocation.  */
...
static reloc_howto_type cgc_dlx_gnu_rel16_s2 =
  HOWTO (R_DLX_RELOC_16_PCREL,  /* Type. */
	 ...
	 cgc32_dlx_relocate16,  /* Special_function.  */
...
static reloc_howto_type cgc_dlx_gnu_rel26_s2 =
  HOWTO (R_DLX_RELOC_26_PCREL,  /* Type. */
	 ...
	 0,                     /* Bitpos.  */
	 complain_overflow_dont,/* Complain_on_overflow.  */
	 cgc32_dlx_relocate26,  /* Special_function.  */
...
static reloc_howto_type cgc_dlx_reloc_16_hi =
  HOWTO (R_DLX_RELOC_16_HI,     /* Type. */
	 ...
	 0,                     /* Bitpos.  */
	 complain_overflow_dont,/* Complain_on_overflow.  */
	 _bfd_dlx_cgc_hi16_reloc,/* Special_function.  */
...
static reloc_howto_type cgc_dlx_reloc_16_lo =
...
/* A mapping from BFD reloc types to DLX CGC reloc types.
   Stolen from cgc32-mips.c.

   More about this table - for dlx cgc relocation we do not really
...
struct cgc_reloc_map
{
  ...
  enum cgc_dlx_reloc_type cgc_reloc_val;
};

static const struct cgc_reloc_map dlx_reloc_map[] =
{
...
static bfd_boolean
cgc32_dlx_check_relocs (bfd *abfd,
...
cgc32_dlx_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			     bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < sizeof (dlx_reloc_map) / sizeof (struct cgc_reloc_map); i++)
    if (dlx_reloc_map[i].bfd_reloc_val == code)
      return &dlx_cgc_howto_table[(int) dlx_reloc_map[i].cgc_reloc_val];
  ...
  switch (code)
    ...
    case BFD_RELOC_16_PCREL_S2:
      return &cgc_dlx_gnu_rel16_s2;
    case BFD_RELOC_DLX_JMP26:
      return &cgc_dlx_gnu_rel26_s2;
    case BFD_RELOC_HI16_S:
      return &cgc_dlx_reloc_16_hi;
    case BFD_RELOC_LO16:
      return &cgc_dlx_reloc_16_lo;
    }
}

static reloc_howto_type *
cgc32_dlx_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			     const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (dlx_cgc_howto_table) / sizeof (dlx_cgc_howto_table[0]);
       i++)
    if (dlx_cgc_howto_table[i].name != NULL
	&& strcasecmp (dlx_cgc_howto_table[i].name, r_name) == 0)
      return &dlx_cgc_howto_table[i];

  if (strcasecmp (cgc_dlx_gnu_rel16_s2.name, r_name) == 0)
    return &cgc_dlx_gnu_rel16_s2;
  if (strcasecmp (cgc_dlx_gnu_rel26_s2.name, r_name) == 0)
    return &cgc_dlx_gnu_rel26_s2;
  if (strcasecmp (cgc_dlx_reloc_16_hi.name, r_name) == 0)
    return &cgc_dlx_reloc_16_hi;
  if (strcasecmp (cgc_dlx_reloc_16_lo.name, r_name) == 0)
    return &cgc_dlx_reloc_16_lo;
...
dlx_rtype_to_howto (unsigned int r_type)
{
  switch (r_type)
    ...
    default:
      ...
      return & dlx_cgc_howto_table[r_type];
    }
}

static void
cgc32_dlx_info_to_howto (bfd * abfd ATTRIBUTE_UNUSED,
...
static void
cgc32_dlx_info_to_howto_rel (bfd *abfd ATTRIBUTE_UNUSED,
			     arelent *cache_ptr,
...
#define TARGET_BIG_SYM          bfd_cgc32_dlx_big_vec
#define TARGET_BIG_NAME         "cgc32-dlx"
#define CGC_ARCH                bfd_arch_dlx
#define CGC_MACHINE_CODE        EM_DLX
