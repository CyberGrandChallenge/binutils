...
#define CGC_DYNAMIC_INTERPRETER "/lib/ld-uClibc.so.0"
...
/* TI C6X CGC linker hash table.  */

struct cgc32_tic6x_link_hash_table
{
  ...
  struct cgc32_tic6x_params params;
...
/* Get the TI C6X CGC linker hash table from a link_info structure.  */

#define cgc32_tic6x_hash_table(p) \
  ((struct cgc32_tic6x_link_hash_table *) ((p)->hash))

/* TI C6X CGC linker hash entry.  */

struct cgc32_tic6x_link_hash_entry
{
...
typedef struct _tic6x_cgc_section_data
{
...
_tic6x_cgc_section_data;

#define cgc32_tic6x_section_data(sec) \
  ((_tic6x_cgc_section_data *) cgc_section_data (sec))

struct cgc32_tic6x_obj_tdata
{
...
#define cgc32_tic6x_tdata(abfd) \
  ((struct cgc32_tic6x_obj_tdata *) (abfd)->tdata.any)

#define is_tic6x_cgc(bfd) \
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour \
   && cgc_tdata (bfd) != NULL \
   && cgc_object_id (bfd) == TIC6X_CGC_DATA)

/* C6X CGC uses two common sections.  One is the usual one, and the
   other is for small objects.  All the small objects are kept
   together, and then referenced via the gp pointer, which yields
   faster assembler code.  This is what we use for the small common
   section.  This approach is copied from ecoff.c.  */
static asection tic6x_cgc_scom_section;
static asymbol  tic6x_cgc_scom_symbol;
static asymbol  *tic6x_cgc_scom_symbol_ptr;

static reloc_howto_type cgc32_tic6x_howto_table[] =
{
...
static reloc_howto_type cgc32_tic6x_howto_table_rel[] =
{
  HOWTO (R_C6000_NONE,		/* type */
	 0,			/* rightshift */
	 0,			/* size (0 = byte, 1 = short, 2 = long) */
...
/* Map BFD relocations to CGC relocations.  */

typedef struct
{
  bfd_reloc_code_real_type bfd_reloc_val;
  enum cgc_tic6x_reloc_type cgc_reloc_val;
} tic6x_reloc_map;

static const tic6x_reloc_map cgc32_tic6x_reloc_map[] =
  {
...
cgc32_tic6x_reloc_type_lookup (bfd *abfd, bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < ARRAY_SIZE (cgc32_tic6x_reloc_map); i++)
    if (cgc32_tic6x_reloc_map[i].bfd_reloc_val == code)
      {
	enum cgc_tic6x_reloc_type cgc_reloc_val;
	reloc_howto_type *howto;

	cgc_reloc_val = cgc32_tic6x_reloc_map[i].cgc_reloc_val;
	if (cgc32_tic6x_tdata (abfd)->use_rela_p)
	  howto = &cgc32_tic6x_howto_table[cgc_reloc_val];
	else
	  howto = &cgc32_tic6x_howto_table_rel[cgc_reloc_val];
...
static reloc_howto_type *
cgc32_tic6x_reloc_name_lookup (bfd *abfd, const char *r_name)
{
  if (cgc32_tic6x_tdata (abfd)->use_rela_p)
    {
      unsigned int i;

      for (i = 0; i < ARRAY_SIZE (cgc32_tic6x_howto_table); i++)
	if (cgc32_tic6x_howto_table[i].name != NULL
	    && strcasecmp (cgc32_tic6x_howto_table[i].name, r_name) == 0)
	  return &cgc32_tic6x_howto_table[i];
    }
  else
    {
      unsigned int i;

      for (i = 0; i < ARRAY_SIZE (cgc32_tic6x_howto_table_rel); i++)
	if (cgc32_tic6x_howto_table_rel[i].name != NULL
	    && strcasecmp (cgc32_tic6x_howto_table_rel[i].name, r_name) == 0)
	  return &cgc32_tic6x_howto_table_rel[i];
    }
...
static void
cgc32_tic6x_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			   Cgc_Internal_Rela *cgc_reloc)
{
  unsigned int r_type;

  r_type = CGC32_R_TYPE (cgc_reloc->r_info);
  if (r_type >= ARRAY_SIZE (cgc32_tic6x_howto_table))
    bfd_reloc->howto = NULL;
  else
    bfd_reloc->howto = &cgc32_tic6x_howto_table[r_type];
}

static void
cgc32_tic6x_info_to_howto_rel (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			       Cgc_Internal_Rela *cgc_reloc)
{
  unsigned int r_type;

  r_type = CGC32_R_TYPE (cgc_reloc->r_info);
  if (r_type >= ARRAY_SIZE (cgc32_tic6x_howto_table_rel))
    bfd_reloc->howto = NULL;
  else
    bfd_reloc->howto = &cgc32_tic6x_howto_table_rel[r_type];
}

void
cgc32_tic6x_set_use_rela_p (bfd *abfd, bfd_boolean use_rela_p)
{
  cgc32_tic6x_tdata (abfd)->use_rela_p = use_rela_p;
}

/* Create an entry in a C6X CGC linker hash table.  */

static struct bfd_hash_entry *
cgc32_tic6x_link_hash_newfunc (struct bfd_hash_entry *entry,
			    ...
			    const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc32_tic6x_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc32_tic6x_link_hash_entry *eh;

      eh = (struct cgc32_tic6x_link_hash_entry *) entry;
      eh->dyn_relocs = NULL;
    }
...
/* Create a C6X CGC linker hash table.  */

static struct bfd_link_hash_table *
cgc32_tic6x_link_hash_table_create (bfd *abfd)
{
  struct cgc32_tic6x_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc32_tic6x_link_hash_table);
  ...
  ret->obfd = abfd;
  ret->cgc.is_relocatable_executable = 1;

  return &ret->cgc.root;
}

static bfd_boolean
cgc32_tic6x_final_link (bfd *abfd, struct bfd_link_info *info)
{
  if (info->shared)
    {
      obj_attribute *out_attr;
      out_attr = cgc_known_obj_attributes_proc (abfd);
...
void
cgc32_tic6x_setup (struct bfd_link_info *info,
		   struct cgc32_tic6x_params *params)
{
  struct cgc32_tic6x_link_hash_table *htab = cgc32_tic6x_hash_table (info);
  htab->params = *params;
}
...
static bfd_boolean
cgc32_tic6x_using_dsbt (bfd *abfd)
{
  return bfd_cgc_get_obj_attr_int (abfd, OBJ_ATTR_PROC,
				   Tag_ABI_DSBT);
}
...
static bfd_boolean
cgc32_tic6x_create_dynamic_sections (bfd *dynobj, struct bfd_link_info *info)
{
  struct cgc32_tic6x_link_hash_table *htab;
  flagword flags;

  htab = cgc32_tic6x_hash_table (info);
  if (htab == NULL)
    return FALSE;
  ...
  if (htab->dsbt == NULL
      ...
      || ! bfd_set_section_alignment (dynobj, htab->cgc.splt, 5))
    return FALSE;
...
static bfd_boolean
cgc32_tic6x_mkobject (bfd *abfd)
{
  bfd_boolean ret;

  ret = bfd_cgc_allocate_object (abfd, sizeof (struct cgc32_tic6x_obj_tdata),
				 TIC6X_CGC_DATA);
  if (ret)
    cgc32_tic6x_set_use_rela_p (abfd, TRUE);
  return ret;
}
...
static void
cgc32_tic6x_install_rela (bfd *output_bfd, asection *srela,
			  Cgc_Internal_Rela *rela)
{
  bfd_byte *loc;
  bfd_vma off = srela->reloc_count++ * sizeof (Cgc32_External_Rela);
...
static void
cgc32_tic6x_make_got_dynreloc (bfd *output_bfd,
			       struct cgc32_tic6x_link_hash_table *htab,
			       asection *sym_sec, bfd_vma offset)
{
  asection *sgot = htab->cgc.sgot;
  ...
  if (sym_sec && sym_sec->output_section
    ...
    {
      dynindx = cgc_section_data (sym_sec->output_section)->dynindx;
  ...
  outrel.r_info = CGC32_R_INFO (dynindx, R_C6000_ABS32);
  cgc32_tic6x_install_rela (output_bfd, htab->cgc.srelgot, &outrel);
}
...
static bfd_boolean
cgc32_tic6x_finish_dynamic_symbol (bfd * output_bfd,
				   ...
				   Cgc_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
	...
	abort ();
	 ...
	 For static executables, we don't reserve anything.  */
      ...
      rela_offset = plt_index * sizeof (Cgc32_External_Rela);
      ...
      rela.r_offset = (gotplt->output_section->vma
		       ...
		       + got_section_offset);
      rela.r_info = CGC32_R_INFO (h->dynindx, R_C6000_JUMP_SLOT);
      rela.r_addend = 0;
  ...
  if (h->got.offset != (bfd_vma) -1)
	      ...
	      || h->dynindx == -1 || h->forced_local) && h->def_regular)
	{
	  ...
	  cgc32_tic6x_make_got_dynreloc (output_bfd, htab, s,
      ...
      else
	{
	  ...
	  outrel.r_offset = (sgot->output_section->vma
			   + sgot->output_offset
			   + (h->got.offset & ~(bfd_vma) 1));
	  outrel.r_info = CGC32_R_INFO (h->dynindx, R_C6000_ABS32);
	  outrel.r_addend = 0;

	  cgc32_tic6x_install_rela (output_bfd, srela, &outrel);
	}
    }
  ...
  if (h->needs_copy)
	...
	abort ();
      ...
      rel.r_info = CGC32_R_INFO (h->dynindx, R_C6000_COPY);
      rel.r_addend = 0;

      cgc32_tic6x_install_rela (output_bfd, htab->srelbss, &rel);
    }
...
static bfd_boolean
cgc32_tic6x_gc_mark_extra_sections (struct bfd_link_info *info,
...
static bfd_boolean
is_tic6x_cgc_unwind_section_name (const char *name)
{
  return (CONST_STRNEQ (name, CGC_STRING_C6000_unwind)
	  || CONST_STRNEQ (name, CGC_STRING_C6000_unwind_once));
}
...
static bfd_boolean
cgc32_tic6x_fake_sections (bfd *abfd ATTRIBUTE_UNUSED,
			   Cgc_Internal_Shdr *hdr, asection *sec)
{
  const char * name;

  name = bfd_get_section_name (abfd, sec);

  if (is_tic6x_cgc_unwind_section_name (name))
    {
...
static bfd_boolean
cgc32_tic6x_gc_sweep_hook (bfd *abfd,
			   struct bfd_link_info *info,
			   asection *sec,
			   const Cgc_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
	  ...
	  eh = (struct cgc32_tic6x_link_hash_entry *) h;
...
static bfd_boolean
cgc32_tic6x_adjust_dynamic_symbol (struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h)
{
  struct cgc32_tic6x_link_hash_table *htab;
  bfd *dynobj;
...
static bfd_boolean
cgc32_tic6x_new_section_hook (bfd *abfd, asection *sec)
{
  bfd_boolean ret;
  ...
  if (!sec->used_by_bfd)
    {
      _tic6x_cgc_section_data *sdata;
      bfd_size_type amt = sizeof (*sdata);

      sdata = (_tic6x_cgc_section_data *) bfd_zalloc (abfd, amt);
      if (sdata == NULL)
	return FALSE;
      sec->used_by_bfd = sdata;
    }

  ret = _bfd_cgc_new_section_hook (abfd, sec);
  sec->use_rela_p = cgc32_tic6x_tdata (abfd)->use_rela_p;
...
static bfd_boolean
cgc32_tic6x_rel_relocation_p (bfd *abfd, asection *sec,
			      const Cgc_Internal_Rela *relocs,
...
static bfd_boolean
cgc32_tic6x_link_omit_section_dynsym (bfd *output_bfd ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc32_tic6x_relocate_section (bfd *output_bfd,
			      struct bfd_link_info *info,
			      bfd *input_bfd,
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      is_rel = cgc32_tic6x_rel_relocation_p (input_bfd, input_section,
					     relocs, rel);

      if (is_rel)
	cgc32_tic6x_info_to_howto_rel (input_bfd, &bfd_reloc, rel);
      else
	cgc32_tic6x_info_to_howto (input_bfd, &bfd_reloc, rel);
	...
	case R_C6000_PCR_S21:
	  ...
	  if (h ? h->root.type == bfd_link_hash_undefweak
	      && (htab->cgc.splt == NULL || h->plt.offset == (bfd_vma) -1)
	...
	case R_C6000_DSBT_INDEX:
	  relocation = cgc32_tic6x_hash_table (info)->params.dsbt_index;
	...
	case R_C6000_ABS_H16:
	  /* When generating a shared object or relocatable executable, these
	     ...
	     run time.  */
	  if ((info->shared || cgc32_tic6x_using_dsbt (output_bfd))
	      ...
	      else
		{
	      ...
	      cgc32_tic6x_install_rela (output_bfd, sreloc, &outrel);
	...
	case R_C6000_EHTYPE:
	  ...
	  if (h != NULL)
		      ...
		      && h->root.type == bfd_link_hash_undefweak))
		{
		  ...
		  if ((off & 1) != 0)
		  ...
		  else
		    {
			  ...
			  && !(CGC_ST_VISIBILITY (h->other)
			       && h->root.type == bfd_link_hash_undefweak))
			cgc32_tic6x_make_got_dynreloc (output_bfd, htab, sec,
						       off);
		    }
		}
	      else
	      ...
	      else
		{
		  ...
		  if (info->shared || cgc32_tic6x_using_dsbt (output_bfd))
		    cgc32_tic6x_make_got_dynreloc (output_bfd, htab, sec, off);
	  ...
	  if (rel->r_addend != 0)
		...
		(*_bfd_error_handler)
		  (_("%B, section %A: relocation %s with non-zero addend %d"
		   ...
		   input_bfd,
		   input_section,
		   cgc32_tic6x_howto_table[r_type].name,
		   rel->r_addend);
	      else
		(*_bfd_error_handler)
		  (_("%B, section %A: relocation %s with non-zero addend %d"
...
static bfd_boolean
cgc32_tic6x_check_relocs (bfd *abfd, struct bfd_link_info *info,
			  asection *sec, const Cgc_Internal_Rela *relocs)
{
  ...
  /* Create dynamic sections for relocatable executables so that we can
     copy relocations.  */
  if ((info->shared || cgc32_tic6x_using_dsbt (abfd))
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_C6000_DSBT_INDEX:
	  /* We'd like to check for nonzero dsbt_index here, but it's
	     ...
	     in cgc32_tic6x_allocate_dynrelocs.  */
	  if (!info->shared)
	    break;
	  ...
	  if ((info->shared || cgc32_tic6x_using_dsbt (abfd))
	      ...
	      if (sreloc == NULL)
		{
		 ...
		 relocations we need for this symbol.  */
	      if (h != NULL)
		{
		  head = &((struct cgc32_tic6x_link_hash_entry *) h)->dyn_relocs;
		}
	      else
		{
	      ...
	      if (p == NULL || p->sec != sec)
		{
		  ...
		  p = bfd_alloc (htab->cgc.dynobj, amt);
		  if (p == NULL)
		    return FALSE;
		  p->next = *head;
		  *head = p;
...
static bfd_boolean
cgc32_tic6x_add_symbol_hook (bfd *abfd,
			     struct bfd_link_info *info ATTRIBUTE_UNUSED,
			     Cgc_Internal_Sym *sym,
...
static void
cgc32_tic6x_symbol_processing (bfd *abfd ATTRIBUTE_UNUSED, asymbol *asym)
{
  ...
  switch (cgcsym->internal_cgc_sym.st_shndx)
    {
    case SHN_TIC6X_SCOMMON:
      if (tic6x_cgc_scom_section.name == NULL)
        {
          /* Initialize the small common section.  */
          tic6x_cgc_scom_section.name = ".scommon";
          tic6x_cgc_scom_section.flags = SEC_IS_COMMON;
          tic6x_cgc_scom_section.output_section = &tic6x_cgc_scom_section;
          tic6x_cgc_scom_section.symbol = &tic6x_cgc_scom_symbol;
          tic6x_cgc_scom_section.symbol_ptr_ptr = &tic6x_cgc_scom_symbol_ptr;
          tic6x_cgc_scom_symbol.name = ".scommon";
          tic6x_cgc_scom_symbol.flags = BSF_SECTION_SYM;
          tic6x_cgc_scom_symbol.section = &tic6x_cgc_scom_section;
          tic6x_cgc_scom_symbol_ptr = &tic6x_cgc_scom_symbol;
        }
      asym->section = &tic6x_cgc_scom_section;
...
static int
cgc32_tic6x_link_output_symbol_hook (struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc32_tic6x_section_from_bfd_section (bfd *abfd ATTRIBUTE_UNUSED,
				      asection *sec,
				      int *retval)
{
...
static bfd_boolean
cgc32_tic6x_allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  struct bfd_link_info *info;
  struct cgc32_tic6x_link_hash_table *htab;
  struct cgc32_tic6x_link_hash_entry *eh;
  ...
  if (htab->cgc.dynamic_sections_created && h->plt.refcount > 0)
    {
	  ...
	  || WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, 0, h))
	{
	  ...
	  htab->cgc.srelplt->size += sizeof (Cgc32_External_Rela);
	}
      else
	{
	  h->plt.offset = (bfd_vma) -1;
  ...
  if (h->got.refcount > 0)
	  ...
	  && !h->forced_local)
	{
      ...
      if (!(CGC_ST_VISIBILITY (h->other)
	    && h->root.type == bfd_link_hash_undefweak))
	htab->cgc.srelgot->size += sizeof (Cgc32_External_Rela);
    }
  else
    h->got.offset = (bfd_vma) -1;
  ...
  /* Discard relocs on undefined weak syms with non-default
     visibility.  */
  if (info->shared || cgc32_tic6x_using_dsbt (htab->obfd))
    {
...
static bfd_boolean
cgc32_tic6x_readonly_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  struct cgc32_tic6x_link_hash_entry *eh;
  struct cgc_dyn_relocs *p;
...
static bfd_boolean
cgc32_tic6x_size_dynamic_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  struct cgc32_tic6x_link_hash_table *htab;
  bfd *dynobj;
  asection *s;
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  cgc_link_hash_traverse (&htab->cgc, cgc32_tic6x_allocate_dynrelocs, info);
  ...
  if (htab->cgc.dynamic_sections_created)
    {
	 ...
	 values later, in cgc32_tic6x_finish_dynamic_sections, but we
      ...
      if (relocs)
	{
	  ...
	  if ((info->flags & DF_TEXTREL) == 0)
	    cgc_link_hash_traverse (&htab->cgc,
				    cgc32_tic6x_readonly_dynrelocs, info);
...
static bfd_boolean
cgc32_tic6x_always_size_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  if (cgc32_tic6x_using_dsbt (output_bfd) && !info->relocatable
...
static bfd_boolean
cgc32_tic6x_finish_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				     struct bfd_link_info *info)
{
  struct cgc32_tic6x_link_hash_table *htab;
  bfd *dynobj;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      Cgc32_External_Dyn * dyncon;
      Cgc32_External_Dyn * dynconend;
	...
	{
	  bfd_vma got_offs = (htab->cgc.sgotplt->output_section->vma
			      + htab->cgc.sgotplt->output_offset
			      - htab->dsbt->output_section->vma
...
static bfd_vma
cgc32_tic6x_plt_sym_val (bfd_vma i, const asection *plt,
...
static int
cgc32_tic6x_obj_attrs_arg_type (int tag)
{
...
static int
cgc32_tic6x_obj_attrs_order (int num)
{
...
static bfd_boolean
cgc32_tic6x_obj_attrs_handle_unknown (bfd *abfd, int tag)
{
  if ((tag & 127) < 64)
    {
      _bfd_error_handler
...
int
cgc32_tic6x_merge_arch_attributes (int arch1, int arch2)
{
...
static int
cgc32_tic6x_tag_to_array_alignment (int tag)
{
  switch (tag)
    {
    case 0:
...
static int
cgc32_tic6x_array_alignment_to_tag (int align)
{
  switch (align)
    {
    case 8:
...
static bfd_boolean
cgc32_tic6x_merge_attributes (bfd *ibfd, bfd *obfd)
{
  ...
  array_align_in = cgc32_tic6x_tag_to_array_alignment
  ...
  array_align_out = cgc32_tic6x_tag_to_array_alignment
  ...
  array_expect_in = cgc32_tic6x_tag_to_array_alignment
  ...
  array_expect_out = cgc32_tic6x_tag_to_array_alignment
  ...
  for (i = LEAST_KNOWN_OBJ_ATTRIBUTE; i < NUM_KNOWN_OBJ_ATTRIBUTES; i++)
	...
	{
	case Tag_ISA:
	  out_attr[i].i = cgc32_tic6x_merge_arch_attributes (in_attr[i].i,
							     out_attr[i].i);
	  break;
	...
	case Tag_ABI_array_object_alignment:
	  if (array_align_out != -1
	    ...
	    out_attr[i].i
	      = cgc32_tic6x_array_alignment_to_tag (array_align_in);
	  break;
	...
	case Tag_ABI_array_object_align_expected:
	  if (array_expect_out != -1
	    ...
	    out_attr[i].i
	      = cgc32_tic6x_array_alignment_to_tag (array_expect_in);
	  break;
	...
	case Tag_ABI_compatibility:
	  /* Merged in _bfd_cgc_merge_object_attributes.  */
	  break;
...
static bfd_boolean
cgc32_tic6x_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  if (!_bfd_generic_verify_endian_match (ibfd, obfd))
    return FALSE;

  if (! is_tic6x_cgc (ibfd) || ! is_tic6x_cgc (obfd))
    return TRUE;

  if (!cgc32_tic6x_merge_attributes (ibfd, obfd))
    return FALSE;
...
static void
cgc32_tic6x_add_unwind_table_edit (tic6x_unwind_table_edit **head,
...
static _tic6x_cgc_section_data *
get_tic6x_cgc_section_data (asection * sec)
{
  if (sec && sec->owner && is_tic6x_cgc (sec->owner))
    return cgc32_tic6x_section_data (sec);
  else
    return NULL;
}
...
static void
cgc32_tic6x_adjust_exidx_size (asection *exidx_sec, int adjust)
{
  asection *out_sec;
...
static void
cgc32_tic6x_insert_cantunwind_after (asection *text_sec, asection *exidx_sec)
{
  struct _tic6x_cgc_section_data *exidx_data;

  exidx_data = get_tic6x_cgc_section_data (exidx_sec);
  cgc32_tic6x_add_unwind_table_edit (
    &exidx_data->u.exidx.unwind_edit_list,
    &exidx_data->u.exidx.unwind_edit_tail,
    INSERT_EXIDX_CANTUNWIND_AT_END, text_sec, UINT_MAX);

  cgc32_tic6x_adjust_exidx_size (exidx_sec, 8);
}
...
/* Scan .cx6abi.exidx tables, and create a list describing edits which
   ...
   (in cgc32_tic6x_write_section).
*/

bfd_boolean
cgc32_tic6x_fix_exidx_coverage (asection **text_section_order,
	  ...
	  if (cgc_sec->linked_to)
	    {
	      ...
	      struct _tic6x_cgc_section_data *linked_sec_tic6x_data
	        = get_tic6x_cgc_section_data (linked_hdr->bfd_section);
  ...
  for (i = 0; i < num_text_sections; i++)
    {
      ...
      asection *exidx_sec;
      struct _tic6x_cgc_section_data *tic6x_data
       	= get_tic6x_cgc_section_data (sec);
      struct _tic6x_cgc_section_data *exidx_data;
      ...
      if (exidx_sec == NULL)
	{
	  ...
	  cgc32_tic6x_insert_cantunwind_after (last_text_sec, last_exidx_sec);
	  last_unwind_type = 0;
	  continue;
	}
      ...
      for (j = 0; j < hdr->sh_size; j += 8)
	{
	  ...
	  if (elide)
	    {
	      cgc32_tic6x_add_unwind_table_edit (&unwind_edit_head,
      ...
      /* Record edits to be applied later (in cgc32_tic6x_write_section).  */
      exidx_data->u.exidx.unwind_edit_list = unwind_edit_head;
      exidx_data->u.exidx.unwind_edit_tail = unwind_edit_tail;

      if (deleted_exidx_bytes > 0)
	cgc32_tic6x_adjust_exidx_size (exidx_sec, -deleted_exidx_bytes);
...
cgc32_tic6x_add_low31 (unsigned long val, bfd_vma addend)
{
...
static void
cgc32_tic6x_copy_exidx_entry (bfd *output_bfd, bfd_byte *to, bfd_byte *from,
			      bfd_vma offset)
{
  ...
  if ((first_word & 0x80000000ul) == 0)
    first_word = cgc32_tic6x_add_low31 (first_word, offset);

  /* If the high bit of the first word is clear, and the bit pattern is not 0x1
     (EXIDX_CANTUNWIND), this is an offset to an .c6xabi.extab entry.  */
  if ((second_word != 0x1) && ((second_word & 0x80000000ul) == 0))
    second_word = cgc32_tic6x_add_low31 (second_word, offset);
...
static bfd_boolean
cgc32_tic6x_write_section (bfd *output_bfd,
			 struct bfd_link_info *link_info,
			 asection *sec,
			 bfd_byte *contents)
{
  _tic6x_cgc_section_data *tic6x_data;
  struct cgc32_tic6x_link_hash_table *globals
    = cgc32_tic6x_hash_table (link_info);
  bfd_vma offset = sec->output_section->vma + sec->output_offset;

  if (globals == NULL)
    return FALSE;

  /* If this section has not been allocated an _tic6x_cgc_section_data
     structure then we cannot record anything.  */
  tic6x_data = get_tic6x_cgc_section_data (sec);
  if (tic6x_data == NULL)
    return FALSE;

  if (tic6x_data->cgc.this_hdr.sh_type != SHT_C6000_UNWIND)
    return FALSE;
  ...
  for (in_index = 0, out_index = 0; in_index * 8 < input_size || edit_node;)
      ...
      if (edit_node)
	{
	  ...
	  if (in_index < edit_index && in_index * 8 < input_size)
	    {
	      cgc32_tic6x_copy_exidx_entry (output_bfd,
...
static void
cgc32_tic6x_set_osabi (bfd *abfd, struct bfd_link_info *link_info)
{
...
#define TARGET_LITTLE_SYM	bfd_cgc32_tic6x_le_vec
#define TARGET_LITTLE_NAME	"cgc32-tic6x-le"
#define TARGET_BIG_SYM		bfd_cgc32_tic6x_be_vec
#define TARGET_BIG_NAME		"cgc32-tic6x-be"
#define CGC_ARCH		bfd_arch_tic6x
#define CGC_TARGET_ID		TIC6X_CGC_DATA
#define CGC_MACHINE_CODE	EM_TI_C6000
#define CGC_MAXPAGESIZE		0x1000
#define bfd_cgc32_bfd_reloc_type_lookup cgc32_tic6x_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup cgc32_tic6x_reloc_name_lookup
#define bfd_cgc32_bfd_merge_private_bfd_data	cgc32_tic6x_merge_private_bfd_data
#define bfd_cgc32_mkobject		cgc32_tic6x_mkobject
#define bfd_cgc32_bfd_link_hash_table_create  cgc32_tic6x_link_hash_table_create
#define bfd_cgc32_new_section_hook	cgc32_tic6x_new_section_hook
...
#define cgc_backend_obj_attrs_arg_type	cgc32_tic6x_obj_attrs_arg_type
#define cgc_backend_obj_attrs_handle_unknown	cgc32_tic6x_obj_attrs_handle_unknown
#define cgc_backend_obj_attrs_order	cgc32_tic6x_obj_attrs_order
#define cgc_backend_obj_attrs_section	".c6xabi.attributes"
#define cgc_backend_obj_attrs_section_type	SHT_C6000_ATTRIBUTES
#define cgc_backend_obj_attrs_vendor	"c6xabi"
...
#define cgc_backend_fake_sections       cgc32_tic6x_fake_sections
#define cgc_backend_gc_sweep_hook	cgc32_tic6x_gc_sweep_hook
#define cgc_backend_gc_mark_extra_sections cgc32_tic6x_gc_mark_extra_sections
#define cgc_backend_create_dynamic_sections \
  cgc32_tic6x_create_dynamic_sections
#define cgc_backend_adjust_dynamic_symbol \
  cgc32_tic6x_adjust_dynamic_symbol
#define cgc_backend_check_relocs        cgc32_tic6x_check_relocs
#define cgc_backend_add_symbol_hook     cgc32_tic6x_add_symbol_hook
#define cgc_backend_symbol_processing   cgc32_tic6x_symbol_processing
#define cgc_backend_link_output_symbol_hook \
  cgc32_tic6x_link_output_symbol_hook
#define cgc_backend_section_from_bfd_section \
  cgc32_tic6x_section_from_bfd_section
#define cgc_backend_relocate_section	cgc32_tic6x_relocate_section
#define cgc_backend_relocs_compatible	_bfd_cgc_relocs_compatible
#define cgc_backend_finish_dynamic_symbol \
  cgc32_tic6x_finish_dynamic_symbol
#define cgc_backend_always_size_sections \
  cgc32_tic6x_always_size_sections
#define cgc_backend_size_dynamic_sections \
  cgc32_tic6x_size_dynamic_sections
#define cgc_backend_finish_dynamic_sections \
  cgc32_tic6x_finish_dynamic_sections
#define bfd_cgc32_bfd_final_link \
	cgc32_tic6x_final_link
#define cgc_backend_write_section	cgc32_tic6x_write_section
#define cgc_info_to_howto		cgc32_tic6x_info_to_howto
#define cgc_info_to_howto_rel		cgc32_tic6x_info_to_howto_rel

#undef cgc_backend_omit_section_dynsym
#define cgc_backend_omit_section_dynsym cgc32_tic6x_link_omit_section_dynsym
#define cgc_backend_plt_sym_val		cgc32_tic6x_plt_sym_val

#include "cgc32-target.h"

#undef cgc32_bed
#define	cgc32_bed		cgc32_tic6x_linux_bed

#undef TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_tic6x_linux_le_vec
#undef TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-tic6x-linux-le"
#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM			bfd_cgc32_tic6x_linux_be_vec
#undef TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-tic6x-linux-be"
#undef CGC_OSABI
#define	CGC_OSABI			CGCOSABI_C6000_LINUX

#undef cgc_backend_post_process_headers
#define cgc_backend_post_process_headers	cgc32_tic6x_set_osabi

#include "cgc32-target.h"

#undef cgc32_bed
#define	cgc32_bed		cgc32_tic6x_cgc_bed

#undef TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_tic6x_cgc_le_vec
#undef TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-tic6x-cgc-le"
#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM			bfd_cgc32_tic6x_cgc_be_vec
#undef TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-tic6x-cgc-be"
#undef CGC_OSABI
#define	CGC_OSABI			CGCOSABI_C6000_CGCABI
