...
static bfd_boolean sh64_address_in_cranges
  (asection *cranges, bfd_vma, sh64_cgc_crange *);
			 ...
			 sh64_cgc_crange *rangep)
{
  ...
  if ((bfd_get_section_flags (cranges->owner, cranges) & SEC_IN_MEMORY)
      && cgc_section_data (cranges)->this_hdr.sh_type == SHT_SH5_CR_SORTED)
...
enum sh64_cgc_cr_type
sh64_get_contents_type (asection *sec, bfd_vma addr, sh64_cgc_crange *rangep)
{
  asection *cranges;

  /* Fill in the range with the boundaries of the section as a default.  */
  if (bfd_get_flavour (sec->owner) == bfd_target_cgc_flavour
      && cgc_cgcheader (sec->owner)->e_type == ET_EXEC)
    {
