/* Renesas / SuperH specific support for Symbian 32-bit CGC files
...
/* Stop cgc32-sh.c from defining any target vectors.  */
#define SH_TARGET_ALREADY_DEFINED
#define sh_find_cgc_flags           sh_symbian_find_cgc_flags
#define sh_cgc_get_flags_from_mach  sh_symbian_cgc_get_flags_from_mach
#include "cgc32-sh.c"
...
typedef struct symbol_rename
{
  ...
  char *                       current_name;
  char *                       new_name;
  struct cgc_link_hash_entry * current_hash;
...
{
  struct cgc_link_hash_entry * new_hash;
  symbol_rename * node;
  ...
  new_hash = cgc_link_hash_lookup (cgc_hash_table (info), node->new_name, TRUE, FALSE, TRUE);
  bfd_cgc_link_record_dynamic_symbol (info, new_hash);
			     ...
			     asection **            local_sections)
{
  ...
  if (!info->relocatable)
      ...
      for (ptr = rename_list; ptr; ptr = ptr->next)
	{
	  ...
	  ptr->current_hash = cgc_link_hash_lookup (hash_table, ptr->current_name, FALSE, FALSE, TRUE);
	  ...
	  new_hash = cgc_link_hash_lookup (hash_table, ptr->new_name,
					   FALSE, FALSE, TRUE);
	  ...
	  if (new_hash == NULL)
	    {
	      ...
	      bfd_boolean collect = get_cgc_backend_data (input_bfd)->collect;
	      if (_bfd_generic_link_add_one_symbol (info, input_bfd,
						    ...
						    &bh))
		{
		  new_hash = (struct cgc_link_hash_entry *) bh;
	  ...
	  if (h == sym_hashes_end)
	    {
	      struct cgc_link_hash_entry **  new_sym_hashes;
	      ...
	      if (new_sym_hashes == NULL)
		{
	      ...
	      cgc_sym_hashes (input_bfd) = sym_hashes = new_sym_hashes;
	      sym_hashes_end = sym_hashes + num_global_syms;
	      symtab_hdr->sh_size  = (num_global_syms + num_local_syms) * sizeof (Cgc32_External_Sym);
	  ...
	  for (ptr = rename_list; ptr; ptr = ptr->next)
		...
		if (SYMBIAN_DEBUG)
		  fprintf (stderr, "convert reloc %lx from using index %ld to using index %ld\n",
			   ...
			   (long) CGC32_R_SYM (rel->r_info), ptr->new_symndx);
		rel->r_info = CGC32_R_INFO (ptr->new_symndx, r_type);
		break;
	      }
	}
    }

  return sh_cgc_relocate_section (output_bfd, info, input_bfd, input_section,
				  contents, relocs, local_syms, local_sections);
}

#define TARGET_LITTLE_SYM	bfd_cgc32_shl_symbian_vec
#define TARGET_LITTLE_NAME      "cgc32-shl-symbian"

#undef  cgc_backend_relocate_section
#define cgc_backend_relocate_section	sh_symbian_relocate_section
#undef  cgc_backend_check_directives
#define cgc_backend_check_directives    sh_symbian_process_directives
