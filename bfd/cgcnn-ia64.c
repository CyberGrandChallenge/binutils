...
struct cgcNN_ia64_dyn_sym_info
{
  ...
  struct cgcNN_ia64_dyn_reloc_entry
  {
    struct cgcNN_ia64_dyn_reloc_entry *next;
    asection *srel;
    int type;
    int count;
...
struct cgcNN_ia64_local_hash_entry
{
  int id;
  unsigned int r_sym;
  /* The number of elements in cgcNN_ia64_dyn_sym_info array.  */
  unsigned int count;
  /* The number of sorted elements in cgcNN_ia64_dyn_sym_info array.  */
  unsigned int sorted_count;
  /* The size of cgcNN_ia64_dyn_sym_info array.  */
  unsigned int size;
  /* The array of cgcNN_ia64_dyn_sym_info.  */
  struct cgcNN_ia64_dyn_sym_info *info;
...
struct cgcNN_ia64_link_hash_entry
{
...
struct cgcNN_ia64_link_hash_table
{
  /* The main hash table.  */
...
struct cgcNN_ia64_allocate_data
{
...
#define cgcNN_ia64_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == IA64_CGC_DATA ? ((struct cgcNN_ia64_link_hash_table *) ((p)->hash)) : NULL)

static struct cgcNN_ia64_dyn_sym_info * get_dyn_sym_info
  (struct cgcNN_ia64_link_hash_table *ia64_info,
   struct cgc_link_hash_entry *h,
   bfd *abfd, const Cgc_Internal_Rela *rel, bfd_boolean create);
static bfd_boolean cgcNN_ia64_dynamic_symbol_p
  (struct cgc_link_hash_entry *h, struct bfd_link_info *info, int);
static bfd_boolean cgcNN_ia64_choose_gp
  (bfd *abfd, struct bfd_link_info *info, bfd_boolean final);
static void cgcNN_ia64_dyn_sym_traverse
  (struct cgcNN_ia64_link_hash_table *ia64_info,
   bfd_boolean (*func) (struct cgcNN_ia64_dyn_sym_info *, void *),
   void * info);
static bfd_boolean allocate_global_data_got
  (struct cgcNN_ia64_dyn_sym_info *dyn_i, void * data);
static bfd_boolean allocate_global_fptr_got
  (struct cgcNN_ia64_dyn_sym_info *dyn_i, void * data);
static bfd_boolean allocate_local_got
  (struct cgcNN_ia64_dyn_sym_info *dyn_i, void * data);
static bfd_boolean cgcNN_ia64_hpux_vec
...
static void
cgcNN_ia64_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED,
			  arelent *bfd_reloc,
			  Cgc_Internal_Rela *cgc_reloc)
{
  bfd_reloc->howto
    = ia64_cgc_lookup_howto ((unsigned int) CGCNN_R_TYPE (cgc_reloc->r_info));
}

...
void
bfd_cgcNN_ia64_after_parse (int itanium)
{
...
static void
cgcNN_ia64_update_short_info (asection *sec, bfd_vma offset,
			      struct cgcNN_ia64_link_hash_table *ia64_info)
{
...
static bfd_boolean
cgcNN_ia64_relax_section (bfd *abfd, asection *sec,
			  ...
			  bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  ia64_info = cgcNN_ia64_hash_table (link_info);
  if (ia64_info == NULL)
    return FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      unsigned long r_type = CGCNN_R_TYPE (irel->r_info);
      bfd_vma symaddr, reladdr, trampoff, toff, roff;
      asection *tsec;
      struct one_fixup *f;
      bfd_size_type amt;
      bfd_boolean is_branch;
      struct cgcNN_ia64_dyn_sym_info *dyn_i;
      char symtype;
	...
	default:
      ...
      if (CGCNN_R_SYM (irel->r_info) < symtab_hdr->sh_info)
	{
	  /* A local symbol.  */
	  ...
	  isym = isymbuf + CGCNN_R_SYM (irel->r_info);
	  ...
	  symtype = CGC_ST_TYPE (isym->st_info);
	}
      else
	{
	  unsigned long indx;
	  ...
	  indx = CGCNN_R_SYM (irel->r_info) - symtab_hdr->sh_info;
	  ...
	  else if (cgcNN_ia64_dynamic_symbol_p (h, link_info, r_type))
	    continue;
      ...
      if (is_branch)
	{
	  ...
	  if ((bfd_signed_vma) (symaddr - reladdr) >= offset
	      ...
	      if (r_type == R_IA64_PCREL60B)
		{
		  ...
		  irel->r_info
		    = CGCNN_R_INFO (CGCNN_R_SYM (irel->r_info),
				    R_IA64_PCREL21B);
	  ...
	  if (f == NULL)
	      ...
	      if (tsec == ia64_info->root.splt)
		{
		  ...
		  irel->r_info = CGCNN_R_INFO (CGCNN_R_SYM (irel->r_info),
					       R_IA64_PLTOFF22);
		...
		continue;
	      ...
	      irel->r_info = CGCNN_R_INFO (0, R_IA64_NONE);
	    }
      ...
      else
	{
	  ...
	  if (gp == 0)
	    {
	      ...
	      if (gp == 0)
		{
		  if (!cgcNN_ia64_choose_gp (obfd, link_info, FALSE))
		    goto error_return;
		  gp = _bfd_get_gp_value (obfd);
		}
	    }
	  ...
	  if (r_type == R_IA64_GPREL22)
	    cgcNN_ia64_update_short_info (tsec->output_section,
  ...
  if (changed_got)
    {
      struct cgcNN_ia64_allocate_data data;
      data.info = link_info;
      data.ofs = 0;
      ia64_info->self_dtpmod_offset = (bfd_vma) -1;

      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_global_data_got, &data);
      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_global_fptr_got, &data);
      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_local_got, &data);
	  ...
	  && ia64_info->root.srelgot != NULL)
	{
	  ...
	  if (link_info->shared
	      && ia64_info->self_dtpmod_offset != (bfd_vma) -1)
	    ia64_info->root.srelgot->size += sizeof (CgcNN_External_Rela);
	  data.only_got = TRUE;
	  cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_dynrel_entries,
				       &data);
	}
    }
...
{
  if (cgcNN_ia64_hpux_vec (abfd->xvec)
      && !strcmp (name, CGC_STRING_ia64_unwind_hdr))
    return FALSE;
...
static bfd_boolean
cgcNN_ia64_section_from_shdr (bfd *abfd,
...
static bfd_boolean
cgcNN_ia64_section_flags (flagword *flags,
...
static bfd_boolean
cgcNN_ia64_fake_sections (bfd *abfd, Cgc_Internal_Shdr *hdr,
			  asection *sec)
{
  const char *name;
  ...
  if (is_unwind_section_name (abfd, name))
    {
	 ...
	 is set later, in cgcNN_ia64_final_write_processing.  */
  ...
  if (cgcNN_ia64_hpux_vec (abfd->xvec) && (sec->flags & SHF_TLS))
    hdr->sh_flags |= SHF_IA_64_HP_TLS;

  return TRUE;
}

/* The final processing done just before writing out an IA-64 CGC
   object file.  */

static void
cgcNN_ia64_final_write_processing (bfd *abfd,
...
static bfd_boolean
cgcNN_ia64_add_symbol_hook (bfd *abfd,
			    struct bfd_link_info *info,
...
static int
cgcNN_ia64_additional_program_headers (bfd *abfd,
				       struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  s = bfd_get_section_by_name (abfd, CGC_STRING_ia64_archext);
...
static bfd_boolean
cgcNN_ia64_modify_segment_map (bfd *abfd,
			       struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
...
static bfd_boolean
cgcNN_ia64_modify_program_headers (bfd *abfd,
				   struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  for (p = tdata->phdr, m = cgc_seg_map (abfd); m != NULL; m = m->next, p++)
    if (m->p_type == PT_LOAD)
      {
	int i;
	for (i = m->count - 1; i >= 0; --i)
		...
		if (order->type == bfd_indirect_link_order)
		  {
		    ...
		    bfd_vma flags = cgc_section_data(is)->this_hdr.sh_flags;
...
static bfd_boolean
cgcNN_ia64_is_local_label_name (bfd *abfd ATTRIBUTE_UNUSED,
				const char *name)
{
  return name[0] == '.';
}
...
static bfd_boolean
cgcNN_ia64_dynamic_symbol_p (struct cgc_link_hash_entry *h,
			     struct bfd_link_info *info, int r_type)
{
  ...
  return _bfd_cgc_dynamic_symbol_p (h, info, ignore_protected);
}

static struct bfd_hash_entry*
cgcNN_ia64_new_cgc_hash_entry (struct bfd_hash_entry *entry,
			       struct bfd_hash_table *table,
			       const char *string)
{
  struct cgcNN_ia64_link_hash_entry *ret;
  ret = (struct cgcNN_ia64_link_hash_entry *) entry;
  ...
  ret = ((struct cgcNN_ia64_link_hash_entry *)
...
static void
cgcNN_ia64_hash_copy_indirect (struct bfd_link_info *info,
			       struct cgc_link_hash_entry *xdir,
			       struct cgc_link_hash_entry *xind)
{
  struct cgcNN_ia64_link_hash_entry *dir, *ind;

  dir = (struct cgcNN_ia64_link_hash_entry *) xdir;
  ind = (struct cgcNN_ia64_link_hash_entry *) xind;
...
static void
cgcNN_ia64_hash_hide_symbol (struct bfd_link_info *info,
			     struct cgc_link_hash_entry *xh,
			     bfd_boolean force_local)
{
  struct cgcNN_ia64_link_hash_entry *h;
  struct cgcNN_ia64_dyn_sym_info *dyn_i;
  unsigned int count;

  h = (struct cgcNN_ia64_link_hash_entry *)xh;
...
static hashval_t
cgcNN_ia64_local_htab_hash (const void *ptr)
{
  struct cgcNN_ia64_local_hash_entry *entry
    = (struct cgcNN_ia64_local_hash_entry *) ptr;
...
static int
cgcNN_ia64_local_htab_eq (const void *ptr1, const void *ptr2)
{
  struct cgcNN_ia64_local_hash_entry *entry1
    = (struct cgcNN_ia64_local_hash_entry *) ptr1;
  struct cgcNN_ia64_local_hash_entry *entry2
    = (struct cgcNN_ia64_local_hash_entry *) ptr2;
...
cgcNN_ia64_hash_table_create (bfd *abfd)
{
  struct cgcNN_ia64_link_hash_table *ret;
  ...
  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      cgcNN_ia64_new_cgc_hash_entry,
				      sizeof (struct cgcNN_ia64_link_hash_entry),
				      IA64_CGC_DATA))
    {
      free (ret);
      return NULL;
  ...
  ret->loc_hash_table = htab_try_create (1024, cgcNN_ia64_local_htab_hash,
					 cgcNN_ia64_local_htab_eq, NULL);
...
/* Free the global cgcNN_ia64_dyn_sym_info array.  */

static bfd_boolean
cgcNN_ia64_global_dyn_info_free (void **xentry,
				 void * unused ATTRIBUTE_UNUSED)
{
  struct cgcNN_ia64_link_hash_entry *entry
    = (struct cgcNN_ia64_link_hash_entry *) xentry;
...
/* Free the local cgcNN_ia64_dyn_sym_info array.  */

static bfd_boolean
cgcNN_ia64_local_dyn_info_free (void **slot,
				void * unused ATTRIBUTE_UNUSED)
{
  struct cgcNN_ia64_local_hash_entry *entry
    = (struct cgcNN_ia64_local_hash_entry *) *slot;
...
static void
cgcNN_ia64_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct cgcNN_ia64_link_hash_table *ia64_info
    = (struct cgcNN_ia64_link_hash_table *) hash;
  if (ia64_info->loc_hash_table)
    {
      htab_traverse (ia64_info->loc_hash_table,
		     cgcNN_ia64_local_dyn_info_free, NULL);
      htab_delete (ia64_info->loc_hash_table);
    }
  if (ia64_info->loc_hash_memory)
    objalloc_free ((struct objalloc *) ia64_info->loc_hash_memory);
  cgc_link_hash_traverse (&ia64_info->root,
			  cgcNN_ia64_global_dyn_info_free, NULL);
  _bfd_cgc_link_hash_table_free (hash);
}

/* Traverse both local and global hash tables.  */

struct cgcNN_ia64_dyn_sym_traverse_data
{
  bfd_boolean (*func) (struct cgcNN_ia64_dyn_sym_info *, void *);
  void * data;
};
...
static bfd_boolean
cgcNN_ia64_global_dyn_sym_thunk (struct bfd_hash_entry *xentry,
				 void * xdata)
{
  struct cgcNN_ia64_link_hash_entry *entry
    = (struct cgcNN_ia64_link_hash_entry *) xentry;
  struct cgcNN_ia64_dyn_sym_traverse_data *data
    = (struct cgcNN_ia64_dyn_sym_traverse_data *) xdata;
  struct cgcNN_ia64_dyn_sym_info *dyn_i;
  unsigned int count;
...
static bfd_boolean
cgcNN_ia64_local_dyn_sym_thunk (void **slot, void * xdata)
{
  struct cgcNN_ia64_local_hash_entry *entry
    = (struct cgcNN_ia64_local_hash_entry *) *slot;
  struct cgcNN_ia64_dyn_sym_traverse_data *data
...
static void
cgcNN_ia64_dyn_sym_traverse (struct cgcNN_ia64_link_hash_table *ia64_info,
			     bfd_boolean (*func) (struct cgcNN_ia64_dyn_sym_info *, void *),
			     void * data)
{
  struct cgcNN_ia64_dyn_sym_traverse_data xdata;
  ...
  cgc_link_hash_traverse (&ia64_info->root,
			  cgcNN_ia64_global_dyn_sym_thunk, &xdata);
  htab_traverse (ia64_info->loc_hash_table,
		 cgcNN_ia64_local_dyn_sym_thunk, &xdata);
}

static bfd_boolean
cgcNN_ia64_create_dynamic_sections (bfd *abfd,
				    struct bfd_link_info *info)
{
  struct cgcNN_ia64_link_hash_table *ia64_info;
  asection *s;
  ...
  ia64_info = cgcNN_ia64_hash_table (info);
  if (ia64_info == NULL)
    return FALSE;
...
static struct cgcNN_ia64_local_hash_entry *
get_local_sym_hash (struct cgcNN_ia64_link_hash_table *ia64_info,
		    bfd *abfd, const Cgc_Internal_Rela *rel,
		    bfd_boolean create)
{
  struct cgcNN_ia64_local_hash_entry e, *ret;
  ...
  e.id = sec->id;
  e.r_sym = CGCNN_R_SYM (rel->r_info);
  ...
  if (*slot)
    return (struct cgcNN_ia64_local_hash_entry *) *slot;

  ret = (struct cgcNN_ia64_local_hash_entry *)
	objalloc_alloc ((struct objalloc *) ia64_info->loc_hash_memory,
			sizeof (struct cgcNN_ia64_local_hash_entry));
  if (ret)
    {
      memset (ret, 0, sizeof (*ret));
      ret->id = sec->id;
      ret->r_sym = CGCNN_R_SYM (rel->r_info);
      *slot = ret;
    }
  return ret;
}

/* Used to sort cgcNN_ia64_dyn_sym_info array.  */

static int
addend_compare (const void *xp, const void *yp)
{
  const struct cgcNN_ia64_dyn_sym_info *x
    = (const struct cgcNN_ia64_dyn_sym_info *) xp;
  const struct cgcNN_ia64_dyn_sym_info *y
    = (const struct cgcNN_ia64_dyn_sym_info *) yp;

  return x->addend < y->addend ? -1 : x->addend > y->addend ? 1 : 0;
}

/* Sort cgcNN_ia64_dyn_sym_info array and remove duplicates.  */

static unsigned int
sort_dyn_sym_info (struct cgcNN_ia64_dyn_sym_info *info,
		   unsigned int count)
{
...
static struct cgcNN_ia64_dyn_sym_info *
get_dyn_sym_info (struct cgcNN_ia64_link_hash_table *ia64_info,
		  struct cgc_link_hash_entry *h, bfd *abfd,
		  const Cgc_Internal_Rela *rel, bfd_boolean create)
{
  struct cgcNN_ia64_dyn_sym_info **info_p, *info, *dyn_i, key;
  ...
  if (h)
    {
      struct cgcNN_ia64_link_hash_entry *global_h;

      global_h = (struct cgcNN_ia64_link_hash_entry *) h;
    ...
    {
      struct cgcNN_ia64_local_hash_entry *loc_h;
...
static bfd_boolean
count_dyn_reloc (bfd *abfd, struct cgcNN_ia64_dyn_sym_info *dyn_i,
		 asection *srel, int type, bfd_boolean reltext)
{
  struct cgcNN_ia64_dyn_reloc_entry *rent;
  ...
  if (!rent)
    {
      rent = ((struct cgcNN_ia64_dyn_reloc_entry *)
...
static bfd_boolean
cgcNN_ia64_check_relocs (bfd *abfd, struct bfd_link_info *info,
			 asection *sec,
			 const Cgc_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	h = NULL;
	 ...
	 this may help reduce memory usage and processing time later.  */
      ...
      need_entry = 0;
      switch (CGCNN_R_TYPE (rel->r_info))
	{
  ...
  for (rel = relocs; rel < relend; ++rel)
      ...
      if (need_entry & NEED_FPTR)
	{
	  ...
	  if (!h && info->shared)
	    {
	      if (! (bfd_cgc_link_record_local_dynamic_symbol
...
static bfd_boolean
allocate_global_data_got (struct cgcNN_ia64_dyn_sym_info *dyn_i,
			  void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;

  if ((dyn_i->want_got || dyn_i->want_gotx)
      && ! dyn_i->want_fptr
      && cgcNN_ia64_dynamic_symbol_p (dyn_i->h, x->info, 0))
     {
  ...
  if (dyn_i->want_dtpmod)
    {
      if (cgcNN_ia64_dynamic_symbol_p (dyn_i->h, x->info, 0))
	{
      ...
      else
	{
	  ...
	  ia64_info = cgcNN_ia64_hash_table (x->info);
	  if (ia64_info == NULL)
	    return FALSE;
...
static bfd_boolean
allocate_global_fptr_got (struct cgcNN_ia64_dyn_sym_info *dyn_i,
			  void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
  ...
  if (dyn_i->want_got
      && dyn_i->want_fptr
      && cgcNN_ia64_dynamic_symbol_p (dyn_i->h, x->info, R_IA64_FPTRNNLSB))
    {
      dyn_i->got_offset = x->ofs;
      x->ofs += 8;
    }
...
static bfd_boolean
allocate_local_got (struct cgcNN_ia64_dyn_sym_info *dyn_i,
		    void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
...
static long
global_sym_index (struct cgc_link_hash_entry *h)
{
  ...
  for (p = cgc_sym_hashes (obj); *p != h; ++p)
    continue;

  return p - cgc_sym_hashes (obj) + cgc_tdata (obj)->symtab_hdr.sh_info;
}
...
static bfd_boolean
allocate_fptr (struct cgcNN_ia64_dyn_sym_info *dyn_i, void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_plt_entries (struct cgcNN_ia64_dyn_sym_info *dyn_i,
		      void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
  ...
  if (dyn_i->want_plt)
	...
	while (h->root.type == bfd_link_hash_indirect
      ...
      if (cgcNN_ia64_dynamic_symbol_p (h, x->info, 0))
	{
...
static bfd_boolean
allocate_plt2_entries (struct cgcNN_ia64_dyn_sym_info *dyn_i,
		       void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_pltoff_entries (struct cgcNN_ia64_dyn_sym_info *dyn_i,
			 void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_dynrel_entries (struct cgcNN_ia64_dyn_sym_info *dyn_i,
			 void * data)
{
  struct cgcNN_ia64_allocate_data *x = (struct cgcNN_ia64_allocate_data *)data;
  struct cgcNN_ia64_link_hash_table *ia64_info;
  ...
  dynamic_symbol = cgcNN_ia64_dynamic_symbol_p (dyn_i->h, x->info, 0);
  ...
  if (ia64_info->rel_fptr_sec && dyn_i->want_fptr)
	...
	ia64_info->rel_fptr_sec->size += sizeof (CgcNN_External_Rela);
    }
  ...
  if (!resolved_zero && dyn_i->want_pltoff)
    {
	 ...
	 main applications get nothing.  */
      if (dynamic_symbol)
	t = sizeof (CgcNN_External_Rela);
      else if (shared)
	t = 2 * sizeof (CgcNN_External_Rela);
  ...
  for (rent = dyn_i->reloc_entries; rent; rent = rent->next)
	...
	default:
      ...
      if (rent->reltext)
	ia64_info->reltext = 1;
      rent->srel->size += sizeof (CgcNN_External_Rela) * count;
    }
...
static bfd_boolean
cgcNN_ia64_adjust_dynamic_symbol (struct bfd_link_info *info ATTRIBUTE_UNUSED,
				  struct cgc_link_hash_entry *h)
{
...
static bfd_boolean
cgcNN_ia64_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *info)
{
  struct cgcNN_ia64_allocate_data data;
  struct cgcNN_ia64_link_hash_table *ia64_info;
  ...
  if (ia64_info->root.dynamic_sections_created
      && info->executable)
    {
      ...
      BFD_ASSERT (sec != NULL);
      sec->contents = (bfd_byte *) CGC_DYNAMIC_INTERPRETER;
      sec->size = strlen (CGC_DYNAMIC_INTERPRETER) + 1;
    }
  ...
  if (ia64_info->fptr_sec)
    {
      data.ofs = 0;
      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_fptr, &data);
  ...
  data.ofs = 0;
  cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_plt_entries, &data);
  ...
  cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_plt2_entries, &data);
  ...
  if (ia64_info->pltoff_sec)
    {
      data.ofs = 0;
      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_pltoff_entries, &data);
  ...
  if (ia64_info->root.dynamic_sections_created)
	...
	ia64_info->root.srelgot->size += sizeof (CgcNN_External_Rela);
      data.only_got = FALSE;
      cgcNN_ia64_dyn_sym_traverse (ia64_info, allocate_dynrel_entries, &data);
    }
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (relplt)
	{
	  ...
	  || !add_dynamic_entry (DT_RELAENT, sizeof (CgcNN_External_Rela)))
	return FALSE;
...
static void
cgcNN_ia64_install_dyn_reloc (bfd *abfd, struct bfd_link_info *info,
			      ...
			      long dynindx, bfd_vma addend)
{
  ...
  outrel.r_info = CGCNN_R_INFO (dynindx, type);
  outrel.r_addend = addend;
  outrel.r_offset = _bfd_cgc_section_offset (abfd, info, sec, offset);
  if (outrel.r_offset >= (bfd_vma) -2)
    {
      /* Run for the hills.  We shouldn't be outputting a relocation
	 for this.  So do what everyone else does and output a no-op.  */
      outrel.r_info = CGCNN_R_INFO (0, R_IA64_NONE);
      outrel.r_addend = 0;
      outrel.r_offset = 0;
    }
  else
  ...
  loc = srel->contents;
  loc += srel->reloc_count++ * sizeof (CgcNN_External_Rela);
  bfd_cgcNN_swap_reloca_out (abfd, &outrel, loc);
  BFD_ASSERT (sizeof (CgcNN_External_Rela) * srel->reloc_count <= srel->size);
}
  ...
  if (! done)
    {
      ...
      if (((info->shared
	    ...
	    && dyn_r_type != R_IA64_DTPREL64LSB)
           || cgcNN_ia64_dynamic_symbol_p (dyn_i->h, info, dyn_r_type)
	   || (dynindx != -1
	      ...
	      || dyn_i->h->root.type != bfd_link_hash_undefweak))
	{
	  ...
	  cgcNN_ia64_install_dyn_reloc (abfd, NULL, got_sec,
  ...
  if (!dyn_i->fptr_done)
      ...
      if (ia64_info->rel_fptr_sec)
	{
	  ...
	  if (bfd_little_endian (abfd))
	    outrel.r_info = CGCNN_R_INFO (0, R_IA64_IPLTLSB);
	  else
	    outrel.r_info = CGCNN_R_INFO (0, R_IA64_IPLTMSB);
		  ...
		  bfd_vma value, bfd_boolean is_plt)
{
  ...
  if ((! dyn_i->want_plt || is_plt)
	      ...
	      || dyn_i->h->root.type != bfd_link_hash_undefweak))
	{
	  ...
	  cgcNN_ia64_install_dyn_reloc (abfd, NULL, pltoff_sec,
...
static bfd_vma
cgcNN_ia64_tprel_base (struct bfd_link_info *info)
{
...
static bfd_vma
cgcNN_ia64_dtprel_base (struct bfd_link_info *info)
{
  return cgc_hash_table (info)->tls_sec->vma;
}

/* Called through qsort to sort the .IA_64.unwind section during a
   non-relocatable link.  Set cgcNN_ia64_unwind_entry_compare_bfd
   to the output bfd so we can do proper endianness frobbing.  */

static bfd *cgcNN_ia64_unwind_entry_compare_bfd;

static int
cgcNN_ia64_unwind_entry_compare (const void * a, const void * b)
{
  bfd_vma av, bv;

  av = bfd_get_64 (cgcNN_ia64_unwind_entry_compare_bfd, a);
  bv = bfd_get_64 (cgcNN_ia64_unwind_entry_compare_bfd, b);
...
static bfd_boolean
cgcNN_ia64_choose_gp (bfd *abfd, struct bfd_link_info *info, bfd_boolean final)
{
...
static bfd_boolean
cgcNN_ia64_final_link (bfd *abfd, struct bfd_link_info *info)
{
  struct cgcNN_ia64_link_hash_table *ia64_info;
  ...
  if (!info->relocatable)
    {
	 ...
	 need to adjust gp for it.  */
      ...
      if (! cgcNN_ia64_choose_gp (abfd, info, TRUE))
	return FALSE;
  ...
  if (unwind_output_sec)
    {
      cgcNN_ia64_unwind_entry_compare_bfd = abfd;
      qsort (unwind_output_sec->contents,
	     (size_t) (unwind_output_sec->size / 24),
	     24,
	     cgcNN_ia64_unwind_entry_compare);
...
static bfd_boolean
cgcNN_ia64_relocate_section (bfd *output_bfd,
			     struct bfd_link_info *info,
			     bfd *input_bfd,
  ...
  for (; rel < relend; ++rel)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (!info->relocatable
		...
		{
		  struct cgcNN_ia64_dyn_sym_info *dynent;
		  unsigned int count;
	...
	continue;
      ...
      value += rel->r_addend;
      dynamic_symbol_p = cgcNN_ia64_dynamic_symbol_p (h, info, r_type);
	...
	case R_IA64_DIR64LSB:
	  ...
	  if ((dynamic_symbol_p || info->shared)
	      ...
	      else
		{
	      ...
	      cgcNN_ia64_install_dyn_reloc (output_bfd, info, input_section,
	  ...
	  if (!dyn_i->want_fptr || info->pie)
	      ...
	      else if (h)
		{
		  if (h->dynindx != -1)
		    ...
		    dynindx = (_bfd_cgc_link_lookup_local_dynindx
	...
	case R_IA64_IPLTLSB:
	  ...
	  if ((dynamic_symbol_p || info->shared)
	      ...
	      if (!dynamic_symbol_p)
		{
		  ...
		  cgcNN_ia64_install_dyn_reloc (output_bfd, info,
						input_section,
						srel, rel->r_offset,
						dyn_r_type, 0, value);
		  cgcNN_ia64_install_dyn_reloc (output_bfd, info,
	...
	case R_IA64_TPREL64I:
	  ...
	  value -= cgcNN_ia64_tprel_base (info);
	  r = ia64_cgc_install_value (hit_addr, value, r_type);
	  break;
	...
	case R_IA64_DTPREL64MSB:
	  if (cgc_hash_table (info)->tls_sec == NULL)
	    goto missing_tls_sec;
	  value -= cgcNN_ia64_dtprel_base (info);
	  r = ia64_cgc_install_value (hit_addr, value, r_type);
	  break;
		...
		if (!dynamic_symbol_p)
		  {
		    ...
		    if (!info->shared)
		      ...
		      {
			r_addend += value - cgcNN_ia64_dtprel_base (info);
			dynindx = 0;
		      }
		  }
...
static bfd_boolean
cgcNN_ia64_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h,
				  Cgc_Internal_Sym *sym)
{
  ...
  if (dyn_i && dyn_i->want_plt)
    {
      ...
      ia64_cgc_install_value (loc, plt_index, R_IA64_IMM22);
      ia64_cgc_install_value (loc+2, -dyn_i->plt_offset, R_IA64_PCREL21B);
	...
	outrel.r_info = CGCNN_R_INFO (h->dynindx, R_IA64_IPLTLSB);
      else
	outrel.r_info = CGCNN_R_INFO (h->dynindx, R_IA64_IPLTMSB);
      outrel.r_addend = 0;
...
static bfd_boolean
cgcNN_ia64_finish_dynamic_sections (bfd *abfd,
				    struct bfd_link_info *info)
{
  struct cgcNN_ia64_link_hash_table *ia64_info;
  bfd *dynobj;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  bfd_cgcNN_swap_dyn_out (abfd, &dyn, dyncon);
	}
      ...
      if (ia64_info->root.splt)
	{
	  ...
	  ia64_cgc_install_value (loc+1, pltres, R_IA64_GPREL22);
	}
    }
...
static bfd_boolean
cgcNN_ia64_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
cgcNN_ia64_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword out_flags;
  flagword in_flags;
  bfd_boolean ok = TRUE;
...
static bfd_boolean
cgcNN_ia64_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
...
cgcNN_ia64_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static const struct bfd_cgc_special_section cgcNN_ia64_special_sections[] =
{
...
static bfd_boolean
cgcNN_ia64_hpux_vec (const bfd_target *vec)
{
  extern const bfd_target bfd_cgcNN_ia64_hpux_big_vec;
  return (vec == & bfd_cgcNN_ia64_hpux_big_vec);
}

static void
cgcNN_hpux_post_process_headers (bfd *abfd,
				 struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
...
static bfd_boolean
cgcNN_hpux_backend_section_from_bfd_section (bfd *abfd ATTRIBUTE_UNUSED,
...
static void
cgcNN_hpux_backend_symbol_processing (bfd *abfd ATTRIBUTE_UNUSED,
				      asymbol *asym)
{
...

#define TARGET_LITTLE_SYM		bfd_cgcNN_ia64_little_vec
#define TARGET_LITTLE_NAME		"cgcNN-ia64-little"
#define TARGET_BIG_SYM			bfd_cgcNN_ia64_big_vec
#define TARGET_BIG_NAME			"cgcNN-ia64-big"
#define CGC_ARCH			bfd_arch_ia64
#define CGC_TARGET_ID			IA64_CGC_DATA
#define CGC_MACHINE_CODE		EM_IA_64
#define CGC_MACHINE_ALT1		1999	/* EAS2.3 */
#define CGC_MACHINE_ALT2		1998	/* EAS2.2 */
#define CGC_MAXPAGESIZE			0x10000	/* 64KB */
#define CGC_COMMONPAGESIZE		0x4000	/* 16KB */

#define cgc_backend_section_from_shdr \
	cgcNN_ia64_section_from_shdr
#define cgc_backend_section_flags \
	cgcNN_ia64_section_flags
#define cgc_backend_fake_sections \
	cgcNN_ia64_fake_sections
#define cgc_backend_final_write_processing \
	cgcNN_ia64_final_write_processing
#define cgc_backend_add_symbol_hook \
	cgcNN_ia64_add_symbol_hook
#define cgc_backend_additional_program_headers \
	cgcNN_ia64_additional_program_headers
#define cgc_backend_modify_segment_map \
	cgcNN_ia64_modify_segment_map
#define cgc_backend_modify_program_headers \
	cgcNN_ia64_modify_program_headers
#define cgc_info_to_howto \
	cgcNN_ia64_info_to_howto
...
#define bfd_cgcNN_bfd_is_local_label_name \
	cgcNN_ia64_is_local_label_name
#define bfd_cgcNN_bfd_relax_section \
	cgcNN_ia64_relax_section
...
#define cgc_backend_copy_indirect_symbol cgcNN_ia64_hash_copy_indirect
#define cgc_backend_hide_symbol		cgcNN_ia64_hash_hide_symbol
#define cgc_backend_fixup_symbol	_bfd_cgc_link_hash_fixup_symbol
#define cgc_backend_reloc_type_class	cgcNN_ia64_reloc_type_class
#define cgc_backend_rela_normal		1
#define cgc_backend_special_sections	cgcNN_ia64_special_sections
...
#define TARGET_BIG_SYM                  bfd_cgcNN_ia64_hpux_big_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME                 "cgcNN-ia64-hpux-big"

/* These are HP-UX specific functions.  */

#undef  cgc_backend_post_process_headers
#define cgc_backend_post_process_headers cgcNN_hpux_post_process_headers

#undef  cgc_backend_section_from_bfd_section
#define cgc_backend_section_from_bfd_section cgcNN_hpux_backend_section_from_bfd_section

#undef cgc_backend_symbol_processing
#define cgc_backend_symbol_processing cgcNN_hpux_backend_symbol_processing

#undef  cgc_backend_want_p_paddr_set_to_zero
#define cgc_backend_want_p_paddr_set_to_zero 1

#undef CGC_COMMONPAGESIZE
#undef CGC_OSABI
#define CGC_OSABI			CGCOSABI_HPUX

#undef  cgcNN_bed
#define cgcNN_bed cgcNN_ia64_hpux_bed
