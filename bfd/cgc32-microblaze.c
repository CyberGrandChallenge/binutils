/* Xilinx MicroBlaze-specific support for 32-bit CGC
...
static reloc_howto_type * microblaze_cgc_howto_table [(int) R_MICROBLAZE_max];

static reloc_howto_type microblaze_cgc_howto_raw[] =
{
   ...
   HOWTO (R_MICROBLAZE_GNU_VTENTRY,   /* Type.  */
          ...
          0,                     /* Bitpos.  */
          complain_overflow_dont,/* Complain on overflow.  */
          _bfd_cgc_rel_vtable_reloc_fn,  /* Special Function.  */
...
#endif

/* Initialize the microblaze_cgc_howto_table, so that linear accesses can be done.  */

static void
microblaze_cgc_howto_init (void)
{
  unsigned int i;

  for (i = NUM_ELEM (microblaze_cgc_howto_raw); i--;)
    {
      unsigned int type;

      type = microblaze_cgc_howto_raw[i].type;

      BFD_ASSERT (type < NUM_ELEM (microblaze_cgc_howto_table));

      microblaze_cgc_howto_table [type] = & microblaze_cgc_howto_raw [i];
    }
}

static reloc_howto_type *
microblaze_cgc_reloc_type_lookup (bfd * abfd ATTRIBUTE_UNUSED,
				  bfd_reloc_code_real_type code)
{
  enum cgc_microblaze_reloc_type microblaze_reloc = R_MICROBLAZE_NONE;
  ...
  if (!microblaze_cgc_howto_table [R_MICROBLAZE_32])
    /* Initialize howto table if needed.  */
    microblaze_cgc_howto_init ();

  return microblaze_cgc_howto_table [(int) microblaze_reloc];
};

static reloc_howto_type *
microblaze_cgc_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				  const char *r_name)
{
  unsigned int i;

  for (i = 0; i < NUM_ELEM (microblaze_cgc_howto_raw); i++)
    if (microblaze_cgc_howto_raw[i].name != NULL
	&& strcasecmp (microblaze_cgc_howto_raw[i].name, r_name) == 0)
      return &microblaze_cgc_howto_raw[i];
...
static void
microblaze_cgc_info_to_howto (bfd * abfd ATTRIBUTE_UNUSED,
			      arelent * cache_ptr,
			      Cgc_Internal_Rela * dst)
{
  ...
  BFD_ASSERT (CGC32_R_TYPE (dst->r_info) < (unsigned int) R_MICROBLAZE_max);

  cache_ptr->howto = microblaze_cgc_howto_table [CGC32_R_TYPE (dst->r_info)];
}

/* Microblaze CGC local labels start with 'L.' or '$L', not '.L'.  */

static bfd_boolean
microblaze_cgc_is_local_label_name (bfd *abfd, const char *name)
{
  if (name[0] == 'L' && name[1] == '.')
    return TRUE;
...
/* The microblaze linker (like many others) needs to keep track of
   ...
   in a field extending the regular CGC linker hash table.  */

struct cgc32_mb_dyn_relocs
{
  struct cgc32_mb_dyn_relocs *next;
...
/* CGC linker hash entry.  */

struct cgc32_mb_link_hash_entry
{
  struct cgc_link_hash_entry cgc;

  /* Track dynamic relocs copied for this symbol.  */
  struct cgc32_mb_dyn_relocs *dyn_relocs;
...
#define cgc32_mb_hash_entry(ent) ((struct cgc32_mb_link_hash_entry *)(ent))

/* CGC linker hash table.  */

struct cgc32_mb_link_hash_table
{
...
#define cgc32_mb_hash_table(p)				\
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == MICROBLAZE_CGC_DATA ? ((struct cgc32_mb_link_hash_table *) ((p)->hash)) : NULL)

/* Create an entry in a microblaze CGC linker hash table.  */
		   ...
		   const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc32_mb_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc32_mb_link_hash_entry *eh;

      eh = (struct cgc32_mb_link_hash_entry *) entry;
      eh->dyn_relocs = NULL;
      eh->tls_mask = 0;
    }
...
/* Create a mb CGC linker hash table.  */

static struct bfd_link_hash_table *
microblaze_cgc_link_hash_table_create (bfd *abfd)
{
  struct cgc32_mb_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc32_mb_link_hash_table);

  ret = (struct cgc32_mb_link_hash_table *) bfd_zmalloc (amt);
  if (ret == NULL)
    return NULL;
...
static void
microblaze_cgc_final_sdp (struct bfd_link_info *info)
{
...
static void
microblaze_cgc_output_dynamic_relocation (bfd *output_bfd,
					  asection *sreloc,
					  ...
					  bfd_vma addend)
{
  ...
  rel.r_info = CGC32_R_INFO (indx, r_type);
  rel.r_offset = offset;
  rel.r_addend = addend;

  bfd_cgc32_swap_reloca_out (output_bfd, &rel,
              (sreloc->contents + reloc_index * sizeof (Cgc32_External_Rela)));
}

/* This code is taken from cgc32-m32r.c
...
static bfd_boolean
microblaze_cgc_relocate_section (bfd *output_bfd,
...
{
  struct cgc32_mb_link_hash_table *htab;
  ...
  if (!microblaze_cgc_howto_table[R_MICROBLAZE_max-1])
    microblaze_cgc_howto_init ();

  htab = cgc32_mb_hash_table (info);
  if (htab == NULL)
    return FALSE;
  ...
  for (; rel < relend; rel++)
      ...
      if (r_type < 0 || r_type >= (int) R_MICROBLAZE_max)
	{
      ...
      howto = microblaze_cgc_howto_table[r_type];
      ...
      if (info->relocatable)
	{
	  ...
	  /* This can't be done for USE_REL because it doesn't mean anything
	     and cgc_link_input_bfd asserts this stays zero.  */
	  /* rel->r_addend = addend; */
#endif
	  ...
	  switch ((int) r_type)
		...
		if (sec)
		  {
			...
			(*_bfd_error_handler) (_("%s: The target (%s) of an %s relocation is in the wrong section (%s)"),
					       ...
					       sym_name,
					       microblaze_cgc_howto_table[(int) r_type]->name,
		...
		if (! ((*offp) & 1) )
		  {
			...
			if (! info->shared)
			  ...
			  {
			    microblaze_cgc_output_dynamic_relocation (output_bfd,
		...
		if ((info->shared
			    ...
			    || h->root.type == bfd_link_hash_undefined)))
		  {
			...
			if (r_type == R_MICROBLAZE_32)
			  {
			    outrel.r_info = CGC32_R_INFO (0, R_MICROBLAZE_REL);
...
static bfd_boolean
microblaze_cgc_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
...
static bfd_boolean
microblaze_cgc_relax_section (bfd *abfd,
			      asection *sec,
			      ...
			      bfd_boolean *again)
{
  ...
  Cgc_Internal_Rela *free_relocs = NULL;
  ...
  Cgc_Internal_Sym *isymbuf, *isymend;
  ...
  *again = FALSE;
  ...
  if (isymbuf == NULL)
    isymbuf = bfd_cgc_get_cgc_syms (abfd, symtab_hdr, symcount,
 			     	    0, NULL, NULL, NULL);
  BFD_ASSERT (isymbuf != NULL);

  internal_relocs = _bfd_cgc_link_read_relocs (abfd, sec, NULL, NULL, link_info->keep_memory);
  ...
  for (irel = internal_relocs; irel < irelend; irel++, rel_count++)
    {
      bfd_vma symval;
      if ((CGC32_R_TYPE (irel->r_info) != (int) R_MICROBLAZE_64_PCREL)
	  && (CGC32_R_TYPE (irel->r_info) != (int) R_MICROBLAZE_64 ))
      ...
      if (CGC32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
	{
	  ...
	  symval = _bfd_cgc_rela_local_sym (abfd, isym, &sym_sec, irel);
	}
      else
	{
	  unsigned long indx;
	  ...
	  || (symval & 0xffff8000) == 0xffff8000)
	{
	  ...
	  /* Rewrite relocation type.  */
          switch ((enum cgc_microblaze_reloc_type) CGC32_R_TYPE (irel->r_info))
	    {
  ...
  if (sec->relax_count > 0)
	  ...
	  nraddr = irel->r_offset - calc_fixup (irel->r_offset, 0, sec);
	    ...
	    case R_MICROBLAZE_32_LO:
		  ...
		  if (isym->st_shndx == shndx
		      && CGC32_ST_TYPE (isym->st_info) == STT_SECTION)
    ...
    microblaze_bfd_write_imm_value_32 (abfd, contents + irel->r_offset
      ...
      for (o = abfd->sections; o != NULL; o = o->next)
        {
          ...
          irelocs = _bfd_cgc_link_read_relocs (abfd, o, NULL, NULL, TRUE);
          if (irelocs == NULL)
            goto error_return;
          ...
          for (irelscan = irelocs; irelscan < irelscanend; irelscan++)
            {
              if (CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_32)
                {
		  ...
		  else if (CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_32_SYM_OP_SYM)
		    {
		      isym = isymbuf + CGC32_R_SYM (irelscan->r_info);
	      ...
	      else if ((CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_32_PCREL_LO)
		       || (CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_32_LO))
		{
		  isym = isymbuf + CGC32_R_SYM (irelscan->r_info);
	      ...
	      if (CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_64)
		{
		  isym = isymbuf + CGC32_R_SYM (irelscan->r_info);
	      ...
	      else if (CGC32_R_TYPE (irelscan->r_info) == (int) R_MICROBLAZE_64_PCREL)
		{
		  isym = isymbuf + CGC32_R_SYM (irelscan->r_info);
      ...
      symcount =  (symtab_hdr->sh_size / sizeof (Cgc32_External_Sym)) - symtab_hdr->sh_info;
...
static asection *
microblaze_cgc_gc_mark_hook (asection *sec,
			     struct bfd_link_info * info,
     			     Cgc_Internal_Rela * rel,
     			     struct cgc_link_hash_entry * h,
     			     Cgc_Internal_Sym * sym)
...
static bfd_boolean
microblaze_cgc_gc_sweep_hook (bfd * abfd ATTRIBUTE_UNUSED,
...
static bfd_boolean
microblaze_cgc_check_relocs (bfd * abfd,
			     struct bfd_link_info * info,
     			     asection * sec,
			     const Cgc_Internal_Rela * relocs)
{
  ...
  struct cgc_link_hash_entry ** sym_hashes_end;
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      else
	{
	  ...
	  /* This relocation requires .got entry.  */
	    ...
	    {
	      h->got.refcount += 1;
	      cgc32_mb_hash_entry (h)->tls_mask |= tls_type;
	    }
          else
	    {
		...
		h->plt.refcount += 1;
		if (CGC32_R_TYPE (rel->r_info) != R_MICROBLAZE_64_PCREL)
			 ...
			 && (! info->symbolic
                ...
                || (!info->shared
                    ...
                    && (h->root.type == bfd_link_hash_defweak
                        || !h->def_regular)))
              {
                struct cgc32_mb_dyn_relocs *p;
                struct cgc32_mb_dyn_relocs **head;
		...
		if (sreloc == NULL)
		  {
		    ...
		    sreloc = _bfd_cgc_make_dynamic_reloc_section (sec, dynobj,
								  2, abfd, 1);
		    if (sreloc == NULL)
		      return FALSE;
		  }
		...
		if (h != NULL)
		  head = &((struct cgc32_mb_link_hash_entry *) h)->dyn_relocs;
		else
		  {
		    ...
		    head = (struct cgc32_mb_dyn_relocs **) vpp;
		  }
...
static bfd_boolean
microblaze_cgc_create_dynamic_sections (bfd *dynobj, struct bfd_link_info *info)
{
  struct cgc32_mb_link_hash_table *htab;
...
static void
microblaze_cgc_copy_indirect_symbol (struct bfd_link_info *info,
				     struct cgc_link_hash_entry *dir,
				     struct cgc_link_hash_entry *ind)
{
  struct cgc32_mb_link_hash_entry *edir, *eind;

  edir = (struct cgc32_mb_link_hash_entry *) dir;
  eind = (struct cgc32_mb_link_hash_entry *) ind;

  if (eind->dyn_relocs != NULL)
    {
      if (edir->dyn_relocs != NULL)
	{
	  struct cgc32_mb_dyn_relocs **pp;
	  struct cgc32_mb_dyn_relocs *p;
...
static bfd_boolean
microblaze_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
				      struct cgc_link_hash_entry *h)
{
  struct cgc32_mb_link_hash_table *htab;
  struct cgc32_mb_link_hash_entry * eh;
  ...
  eh = (struct cgc32_mb_link_hash_entry *) h;
...
static bfd_boolean
allocate_dynrelocs (struct cgc_link_hash_entry *h, void * dat)
{
  struct bfd_link_info *info;
  struct cgc32_mb_link_hash_table *htab;
  struct cgc32_mb_link_hash_entry *eh;
  ...
  if (h->got.refcount > 0)
    {
      ...
      if (need == 0)
      ...
      else
        {
          ...
          h->got.offset = s->size;
          s->size += need;
          htab->srelgot->size += need * (sizeof (Cgc32_External_Rela) / 4);
        }
    }
  else
    h->got.offset = (bfd_vma) -1;
...
static bfd_boolean
microblaze_cgc_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				      struct bfd_link_info *info)
{
  struct cgc32_mb_link_hash_table *htab;
  bfd *dynobj;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = ((struct cgc32_mb_dyn_relocs *)
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in microblaze_cgc_finish_dynamic_sections, but we
...
static bfd_boolean
microblaze_cgc_finish_dynamic_symbol (bfd *output_bfd,
				      struct bfd_link_info *info,
				      struct cgc_link_hash_entry *h,
				      Cgc_Internal_Sym *sym)
{
  struct cgc32_mb_link_hash_table *htab;
  struct cgc32_mb_link_hash_entry *eh = cgc32_mb_hash_entry(h);
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
      ...
      rela.r_offset = (sgotplt->output_section->vma
                       ...
                       + got_offset);
      rela.r_info = CGC32_R_INFO (h->dynindx, R_MICROBLAZE_JUMP_SLOT);
      rela.r_addend = 0;
      loc = srela->contents;
  ...
  if (h->needs_copy)
    {
      ...
      s = bfd_get_linker_section (htab->cgc.dynobj, ".rela.bss");
      BFD_ASSERT (s != NULL);

      rela.r_offset = (h->root.u.def.value
                       + h->root.u.def.section->output_section->vma
                       + h->root.u.def.section->output_offset);
      rela.r_info = CGC32_R_INFO (h->dynindx, R_MICROBLAZE_COPY);
      rela.r_addend = 0;
...
static bfd_boolean
microblaze_cgc_finish_dynamic_sections (bfd *output_bfd,
					struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn, *sgot;
  ...
  if (htab->sgot && htab->sgot->size > 0)
    cgc_section_data (htab->sgot->output_section)->this_hdr.sh_entsize = 4;
...
static bfd_boolean
microblaze_cgc_add_symbol_hook (bfd *abfd,
			        struct bfd_link_info *info,
			        Cgc_Internal_Sym *sym,
...
#define TARGET_LITTLE_SYM      bfd_cgc32_microblazeel_vec
#define TARGET_LITTLE_NAME     "cgc32-microblazeel"

#define TARGET_BIG_SYM          bfd_cgc32_microblaze_vec
#define TARGET_BIG_NAME		"cgc32-microblaze"

#define CGC_ARCH		bfd_arch_microblaze
#define CGC_TARGET_ID		MICROBLAZE_CGC_DATA
#define CGC_MACHINE_CODE	EM_MICROBLAZE
#define CGC_MACHINE_ALT1	EM_MICROBLAZE_OLD
#define CGC_MAXPAGESIZE		0x1000
#define cgc_info_to_howto	microblaze_cgc_info_to_howto
#define cgc_info_to_howto_rel	NULL

#define bfd_cgc32_bfd_reloc_type_lookup		microblaze_cgc_reloc_type_lookup
#define bfd_cgc32_bfd_is_local_label_name       microblaze_cgc_is_local_label_name
#define cgc_backend_relocate_section		microblaze_cgc_relocate_section
#define bfd_cgc32_bfd_relax_section             microblaze_cgc_relax_section
#define bfd_cgc32_bfd_merge_private_bfd_data    microblaze_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_reloc_name_lookup		microblaze_cgc_reloc_name_lookup

#define cgc_backend_gc_mark_hook		microblaze_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook		microblaze_cgc_gc_sweep_hook
#define cgc_backend_check_relocs                microblaze_cgc_check_relocs
#define cgc_backend_copy_indirect_symbol        microblaze_cgc_copy_indirect_symbol
#define bfd_cgc32_bfd_link_hash_table_create    microblaze_cgc_link_hash_table_create
...
#define cgc_backend_adjust_dynamic_symbol       microblaze_cgc_adjust_dynamic_symbol
#define cgc_backend_create_dynamic_sections     microblaze_cgc_create_dynamic_sections
#define cgc_backend_finish_dynamic_sections     microblaze_cgc_finish_dynamic_sections
#define cgc_backend_finish_dynamic_symbol       microblaze_cgc_finish_dynamic_symbol
#define cgc_backend_size_dynamic_sections       microblaze_cgc_size_dynamic_sections
#define cgc_backend_add_symbol_hook		microblaze_cgc_add_symbol_hook
