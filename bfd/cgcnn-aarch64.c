/* AArch64-specific support for NN-bit CGC.
...
/* Notes on implementation:
  ...
  cgcNN_aarch64_allocate_dynrelocs ()
  ...
  cgcNN_aarch64_size_dynamic_sections ()
  ...
  cgcNN_aarch64_relocate_section ()

  Calls cgcNN_aarch64_final_link_relocate ()
  ...
  cgcNN_aarch64_final_link_relocate ()
...
#include "bfdlink.h"
#include "objalloc.h"
#include "cgc/aarch64.h"
#include "cgcxx-aarch64.h"
...
/* Return size of a relocation entry.  HTAB is the bfd's
   cgc_aarch64_link_hash_entry.  */
#define RELOC_SIZE(HTAB) (sizeof (CgcNN_External_Rela))

/* GOT Entry size - 8 bytes in CGC64 and 4 bytes in CGC32.  */
...
/* The first entry in a procedure linkage table looks like this
   ...
   [x16,#-GOT_ENTRY_SIZE].  */
static const bfd_byte cgcNN_aarch64_small_plt0_entry[PLT_ENTRY_SIZE] =
{
...
/* Per function entry in a procedure linkage table looks like this
   if the distance between the PLTGOT and the PLT is < 4GB use
   these PLT entries.  */
static const bfd_byte cgcNN_aarch64_small_plt_entry[PLT_SMALL_ENTRY_SIZE] =
{
...
cgcNN_aarch64_tlsdesc_small_plt_entry[PLT_TLSDESC_ENTRY_SIZE] =
{
...
#define cgc_info_to_howto               cgcNN_aarch64_info_to_howto
#define cgc_info_to_howto_rel           cgcNN_aarch64_info_to_howto

#define AARCH64_CGC_ABI_VERSION		0
...
static reloc_howto_type cgcNN_aarch64_howto_table[] =
{
  EMPTY_HOWTO (0),
...
static reloc_howto_type cgcNN_aarch64_howto_none =
  HOWTO (R_AARCH64_NONE,	/* type */
	 0,			/* rightshift */
	 0,			/* size (0 = byte, 1 = short, 2 = long) */
	 0,			/* bitsize */
...
cgcNN_aarch64_bfd_reloc_from_howto (reloc_howto_type *howto)
{
  const int size
    = (int) ARRAY_SIZE (cgcNN_aarch64_howto_table);
  const ptrdiff_t offset
    = howto - cgcNN_aarch64_howto_table;

  if (offset > 0 && offset < size - 1)
    return BFD_RELOC_AARCH64_RELOC_START + offset;

  if (howto == &cgcNN_aarch64_howto_none)
...
static bfd_reloc_code_real_type
cgcNN_aarch64_bfd_reloc_from_type (unsigned int r_type)
{
  ...
  if (initialized_p == FALSE)
    {
      ...
      for (i = 1; i < ARRAY_SIZE (cgcNN_aarch64_howto_table) - 1; ++i)
	if (cgcNN_aarch64_howto_table[i].type != 0)
	  offsets[cgcNN_aarch64_howto_table[i].type] = i;
...
static const struct cgc_aarch64_reloc_map cgc_aarch64_reloc_map[] =
{
...
cgcNN_aarch64_howto_from_bfd_reloc (bfd_reloc_code_real_type code)
{
  unsigned int i;
  ...
  if (code < BFD_RELOC_AARCH64_RELOC_START
    ...
    for (i = 0; i < ARRAY_SIZE (cgc_aarch64_reloc_map); i++)
      if (cgc_aarch64_reloc_map[i].from == code)
	{
	  code = cgc_aarch64_reloc_map[i].to;
	  break;
	}

  if (code > BFD_RELOC_AARCH64_RELOC_START
      && code < BFD_RELOC_AARCH64_RELOC_END)
    if (cgcNN_aarch64_howto_table[code - BFD_RELOC_AARCH64_RELOC_START].type)
      return &cgcNN_aarch64_howto_table[code - BFD_RELOC_AARCH64_RELOC_START];

  if (code == BFD_RELOC_AARCH64_NONE)
    return &cgcNN_aarch64_howto_none;

  return NULL;
}

static reloc_howto_type *
cgcNN_aarch64_howto_from_type (unsigned int r_type)
{
  ...
  val = cgcNN_aarch64_bfd_reloc_from_type (r_type);
  howto = cgcNN_aarch64_howto_from_bfd_reloc (val);
...
static void
cgcNN_aarch64_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			     Cgc_Internal_Rela *cgc_reloc)
{
  unsigned int r_type;

  r_type = CGCNN_R_TYPE (cgc_reloc->r_info);
  bfd_reloc->howto = cgcNN_aarch64_howto_from_type (r_type);
}

static reloc_howto_type *
cgcNN_aarch64_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				 bfd_reloc_code_real_type code)
{
  reloc_howto_type *howto = cgcNN_aarch64_howto_from_bfd_reloc (code);
...
static reloc_howto_type *
cgcNN_aarch64_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				 const char *r_name)
{
  unsigned int i;

  for (i = 1; i < ARRAY_SIZE (cgcNN_aarch64_howto_table) - 1; ++i)
    if (cgcNN_aarch64_howto_table[i].name != NULL
	&& strcasecmp (cgcNN_aarch64_howto_table[i].name, r_name) == 0)
      return &cgcNN_aarch64_howto_table[i];

  return NULL;
}

#define TARGET_LITTLE_SYM               bfd_cgcNN_littleaarch64_vec
#define TARGET_LITTLE_NAME              "cgcNN-littleaarch64"
#define TARGET_BIG_SYM                  bfd_cgcNN_bigaarch64_vec
#define TARGET_BIG_NAME                 "cgcNN-bigaarch64"
...
/* The name of the dynamic interpreter.  This is put in the .interp
   section.  */
#define CGC_DYNAMIC_INTERPRETER     "/lib/ld.so.1"
...
struct cgc_aarch64_stub_hash_entry
{
  ...
  enum cgc_aarch64_stub_type stub_type;

  /* The symbol table entry, if any, that this was derived from.  */
  struct cgc_aarch64_link_hash_entry *h;
...
typedef struct cgc_cgc_section_map
{
  bfd_vma vma;
  char type;
}
cgc_aarch64_section_map;


typedef struct _aarch64_cgc_section_data
{
  struct bfd_cgc_section_data cgc;
  unsigned int mapcount;
  unsigned int mapsize;
  cgc_aarch64_section_map *map;
}
_aarch64_cgc_section_data;

#define cgc_aarch64_section_data(sec) \
  ((_aarch64_cgc_section_data *) cgc_section_data (sec))

/* The size of the thread control block.  */
#define TCB_SIZE	16

struct cgc_aarch64_local_symbol
{
...
struct cgc_aarch64_obj_tdata
{
  struct cgc_obj_tdata root;

  /* local symbol descriptors */
  struct cgc_aarch64_local_symbol *locals;
...
#define cgc_aarch64_tdata(bfd)				\
  ((struct cgc_aarch64_obj_tdata *) (bfd)->tdata.any)

#define cgc_aarch64_locals(bfd) (cgc_aarch64_tdata (bfd)->locals)

#define is_aarch64_cgc(bfd)				\
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour	\
   && cgc_tdata (bfd) != NULL				\
   && cgc_object_id (bfd) == AARCH64_CGC_DATA)

static bfd_boolean
cgcNN_aarch64_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct cgc_aarch64_obj_tdata),
				  AARCH64_CGC_DATA);
}

#define cgc_aarch64_hash_entry(ent) \
  ((struct cgc_aarch64_link_hash_entry *)(ent))
...
/* AArch64 CGC linker hash entry.  */
struct cgc_aarch64_link_hash_entry
{
  ...
  /* A pointer to the most recently used stub hash entry against this
     symbol.  */
  struct cgc_aarch64_stub_hash_entry *stub_cache;
...
static unsigned int
cgcNN_aarch64_symbol_got_type (struct cgc_link_hash_entry *h,
			       bfd *abfd,
			       unsigned long r_symndx)
{
  if (h)
    return cgc_aarch64_hash_entry (h)->got_type;

  if (! cgc_aarch64_locals (abfd))
    return GOT_UNKNOWN;

  return cgc_aarch64_locals (abfd)[r_symndx].got_type;
}

/* Get the AArch64 cgc linker hash table from a link_info structure.  */
#define cgc_aarch64_hash_table(info)					\
  ((struct cgc_aarch64_link_hash_table *) ((info)->hash))

#define aarch64_stub_hash_lookup(table, string, create, copy)		\
  ((struct cgc_aarch64_stub_hash_entry *)				\
   bfd_hash_lookup ((table), (string), (create), (copy)))

/* AArch64 CGC linker hash table.  */
struct cgc_aarch64_link_hash_table
{
  /* The main hash table.  */
  ...
  /* Assorted information used by cgcNN_aarch64_size_stubs.  */
...
/* Create an entry in an AArch64 CGC linker hash table.  */

static struct bfd_hash_entry *
cgcNN_aarch64_link_hash_newfunc (struct bfd_hash_entry *entry,
				 struct bfd_hash_table *table,
				 const char *string)
{
  struct cgc_aarch64_link_hash_entry *ret =
    (struct cgc_aarch64_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == NULL)
    ret = bfd_hash_allocate (table,
			     sizeof (struct cgc_aarch64_link_hash_entry));
  if (ret == NULL)
    return (struct bfd_hash_entry *) ret;

  /* Call the allocation method of the superclass.  */
  ret = ((struct cgc_aarch64_link_hash_entry *)
		   ...
		   struct bfd_hash_table *table, const char *string)
{
  ...
  if (entry != NULL)
    {
      struct cgc_aarch64_stub_hash_entry *eh;

      /* Initialize the local fields.  */
      eh = (struct cgc_aarch64_stub_hash_entry *) entry;
      eh->stub_sec = NULL;
      eh->stub_offset = 0;
      eh->target_value = 0;
...
static hashval_t
cgcNN_aarch64_local_htab_hash (const void *ptr)
{
...
static int
cgcNN_aarch64_local_htab_eq (const void *ptr1, const void *ptr2)
{
...
cgcNN_aarch64_get_local_sym_hash (struct cgc_aarch64_link_hash_table *htab,
				  bfd *abfd, const Cgc_Internal_Rela *rel,
				  bfd_boolean create)
{
  struct cgc_aarch64_link_hash_entry e, *ret;
  asection *sec = abfd->sections;
  hashval_t h = CGC_LOCAL_SYMBOL_HASH (sec->id,
				       CGCNN_R_SYM (rel->r_info));
  void **slot;

  e.root.indx = sec->id;
  e.root.dynstr_index = CGCNN_R_SYM (rel->r_info);
  ...
  if (*slot)
    {
      ret = (struct cgc_aarch64_link_hash_entry *) *slot;
      return &ret->root;
    }
  ...
  if (ret)
    {
      ...
      ret->root.dynstr_index = CGCNN_R_SYM (rel->r_info);
      ret->root.dynindx = -1;
      *slot = ret;
    }
  return &ret->root;
...
static void
cgcNN_aarch64_copy_indirect_symbol (struct bfd_link_info *info,
				    struct cgc_link_hash_entry *dir,
				    struct cgc_link_hash_entry *ind)
{
  struct cgc_aarch64_link_hash_entry *edir, *eind;

  edir = (struct cgc_aarch64_link_hash_entry *) dir;
  eind = (struct cgc_aarch64_link_hash_entry *) ind;
...
/* Create an AArch64 cgc linker hash table.  */

static struct bfd_link_hash_table *
cgcNN_aarch64_link_hash_table_create (bfd *abfd)
{
  struct cgc_aarch64_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc_aarch64_link_hash_table);

  ret = bfd_zmalloc (amt);
  if (ret == NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init
      (&ret->root, abfd, cgcNN_aarch64_link_hash_newfunc,
       sizeof (struct cgc_aarch64_link_hash_entry), AARCH64_CGC_DATA))
    {
      free (ret);
      return NULL;
    }
  ...
  if (!bfd_hash_table_init (&ret->stub_hash_table, stub_hash_newfunc,
			    sizeof (struct cgc_aarch64_stub_hash_entry)))
    {
      free (ret);
      return NULL;
    }
  ...
  ret->loc_hash_table = htab_try_create (1024,
					 cgcNN_aarch64_local_htab_hash,
					 cgcNN_aarch64_local_htab_eq,
					 NULL);
...
static void
cgcNN_aarch64_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct cgc_aarch64_link_hash_table *ret
    = (struct cgc_aarch64_link_hash_table *) hash;
		  ...
		  bfd_vma offset, bfd_vma value)
{
  ...
  howto = cgcNN_aarch64_howto_from_type (r_type);
  place = (input_section->output_section->vma + input_section->output_offset
	   + offset);

  r_type = cgcNN_aarch64_bfd_reloc_from_type (r_type);
  value = _bfd_aarch64_cgc_resolve_relocation (r_type, place, value, 0, FALSE);
  return _bfd_aarch64_cgc_put_addend (input_bfd,
				      input_section->contents + offset, r_type,
				      howto, value);
}

static enum cgc_aarch64_stub_type
...
aarch64_type_of_stub (struct bfd_link_info *info,
		      ...
		      unsigned char st_type,
		      struct cgc_aarch64_link_hash_entry *hash,
		      bfd_vma destination)
{
  bfd_vma location;
  bfd_signed_vma branch_offset;
  unsigned int r_type;
  struct cgc_aarch64_link_hash_table *globals;
  enum cgc_aarch64_stub_type stub_type = aarch64_stub_none;
  bfd_boolean via_plt_p;

  if (st_type != STT_FUNC)
    return stub_type;

  globals = cgc_aarch64_hash_table (info);
  ...
  r_type = CGCNN_R_TYPE (rel->r_info);
...
static char *
cgcNN_aarch64_stub_name (const asection *input_section,
			 const asection *sym_sec,
			 const struct cgc_aarch64_link_hash_entry *hash,
			 const Cgc_Internal_Rela *rel)
{
  char *stub_name;
  bfd_size_type len;
  ...
  if (hash)
	...
	snprintf (stub_name, len, "%08x_%x:%x+%" BFD_VMA_FMT "x",
		  ...
		  (unsigned int) CGCNN_R_SYM (rel->r_info),
		  rel->r_addend);
    }
...
static struct cgc_aarch64_stub_hash_entry *
cgcNN_aarch64_get_stub_entry (const asection *input_section,
			      const asection *sym_sec,
			      struct cgc_link_hash_entry *hash,
			      const Cgc_Internal_Rela *rel,
			      struct cgc_aarch64_link_hash_table *htab)
{
  struct cgc_aarch64_stub_hash_entry *stub_entry;
  struct cgc_aarch64_link_hash_entry *h =
    (struct cgc_aarch64_link_hash_entry *) hash;
  ...
  if (h != NULL && h->stub_cache != NULL
  ...
  else
    {
      ...
      stub_name = cgcNN_aarch64_stub_name (id_sec, sym_sec, h, rel);
      if (stub_name == NULL)
	return NULL;
...
static struct cgc_aarch64_stub_hash_entry *
cgcNN_aarch64_add_stub (const char *stub_name,
			asection *section,
			struct cgc_aarch64_link_hash_table *htab)
{
  asection *link_sec;
			...
			void *in_arg ATTRIBUTE_UNUSED)
{
  ...
  stub_entry = (struct cgc_aarch64_stub_hash_entry *) gen_entry;
  ...
  switch (stub_entry->stub_type)
	...
	(cgcNN_aarch64_howto_from_type (AARCH64_R (ADD_ABS_LO12_NC)),
	 stub_bfd,
	 stub_sec,
	 stub_sec->contents,
    ...
    case aarch64_stub_long_branch:
      /* We want the value relative to the address 12 bytes back from the
         value itself.  */
      _bfd_final_link_relocate (cgcNN_aarch64_howto_from_type
...
int
cgcNN_aarch64_setup_section_lists (bfd *output_bfd,
...
/* Used by cgcNN_aarch64_next_input_section and group_sections.  */
...
void
cgcNN_aarch64_next_input_section (struct bfd_link_info *info, asection *isec)
{
  struct cgc_aarch64_link_hash_table *htab =
    cgc_aarch64_hash_table (info);
...
static void
group_sections (struct cgc_aarch64_link_hash_table *htab,
...
bfd_boolean
cgcNN_aarch64_size_stubs (bfd *output_bfd,
			  bfd *stub_bfd,
			  struct bfd_link_info *info,
			  ...
			  void (*layout_sections_again) (void))
{
  ...
  struct cgc_aarch64_link_hash_table *htab = cgc_aarch64_hash_table (info);
	   ...
	   input_bfd != NULL; input_bfd = input_bfd->link_next, bfd_indx++)
	{
	  ...
	  for (section = input_bfd->sections;
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  r_type = CGCNN_R_TYPE (irela->r_info);
		  r_indx = CGCNN_R_SYM (irela->r_info);
		  ...
		  if (r_indx < symtab_hdr->sh_info)
		      ...
		      if (local_syms == NULL)
			{
		      ...
		      hdr = cgc_cgcsections (input_bfd)[sym->st_shndx];
		  ...
		  else
		    {
		      ...
		      hash = ((struct cgc_aarch64_link_hash_entry *)
			      cgc_sym_hashes (input_bfd)[e_indx]);
		      ...
		      else
			{
		      ...
		      st_type = CGC_ST_TYPE (hash->root.type);
		  ...
		  stub_name = cgcNN_aarch64_stub_name (id_sec, sym_sec, hash,
						       irela);
		  if (!stub_name)
		    goto error_ret_free_internal;
		  ...
		  stub_entry = cgcNN_aarch64_add_stub (stub_name, section,
						       htab);
		  if (stub_entry == NULL)
		    {
		      free (stub_name);
...
/* Build all the stubs associated with the current output file.  The
   ...
   functions here.  This function is called via aarch64_cgc_finish in the
   linker.  */

bfd_boolean
cgcNN_aarch64_build_stubs (struct bfd_link_info *info)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  struct cgc_aarch64_link_hash_table *htab;

  htab = cgc_aarch64_hash_table (info);
...
static void
cgcNN_aarch64_section_map_add (asection *sec, char type, bfd_vma vma)
{
  struct _aarch64_cgc_section_data *sec_data =
    cgc_aarch64_section_data (sec);
  unsigned int newidx;

  if (sec_data->map == NULL)
    {
      sec_data->map = bfd_malloc (sizeof (cgc_aarch64_section_map));
      sec_data->mapcount = 0;
      sec_data->mapsize = 1;
    }
  ...
  if (sec_data->mapcount > sec_data->mapsize)
	...
	(sec_data->map, sec_data->mapsize * sizeof (cgc_aarch64_section_map));
    }
...
void
bfd_cgcNN_aarch64_init_maps (bfd *abfd)
{
  Cgc_Internal_Sym *isymbuf;
  Cgc_Internal_Shdr *hdr;
  unsigned int i, localsyms;

  /* Make sure that we are dealing with an AArch64 cgc binary.  */
  if (!is_aarch64_cgc (abfd))
    return;
  ...
  isymbuf = bfd_cgc_get_cgc_syms (abfd, hdr, localsyms, 0, NULL, NULL, NULL);
  ...
  for (i = 0; i < localsyms; i++)
    {
      ...
      if (sec != NULL && CGC_ST_BIND (isym->st_info) == STB_LOCAL)
	{
	  ...
	  if (bfd_is_aarch64_special_symbol_name
	    ...
	    cgcNN_aarch64_section_map_add (sec, name[1], isym->st_value);
	}
    }
}
...
void
bfd_cgcNN_aarch64_set_options (struct bfd *output_bfd,
			       ...
			       int no_wchar_warn, int pic_veneer)
{
  ...
  globals = cgc_aarch64_hash_table (link_info);
  globals->pic_veneer = pic_veneer;

  BFD_ASSERT (is_aarch64_cgc (output_bfd));
  cgc_aarch64_tdata (output_bfd)->no_enum_size_warning = no_enum_warn;
  cgc_aarch64_tdata (output_bfd)->no_wchar_size_warning = no_wchar_warn;
}

static bfd_vma
aarch64_calculate_got_entry_vma (struct cgc_link_hash_entry *h,
				 struct cgc_aarch64_link_hash_table
		       ...
		       unsigned long r_symndx)
{
  ...
  symbol_got_type = cgcNN_aarch64_symbol_got_type (h, input_bfd, r_symndx);
			...
			unsigned long r_symndx)
{
  bfd_reloc_code_real_type bfd_r_type
    = cgcNN_aarch64_bfd_reloc_from_type (r_type);
...
dtpoff_base (struct bfd_link_info *info)
{
  ...
  BFD_ASSERT (cgc_hash_table (info)->tls_sec != NULL);
...
static bfd_vma *
symbol_got_offset_ref (bfd *input_bfd, struct cgc_link_hash_entry *h,
		       unsigned long r_symndx)
{
  ...
  if (h != NULL)
    ...
    {
      /* local symbol */
      struct cgc_aarch64_local_symbol *l;

      l = cgc_aarch64_locals (input_bfd);
      return &l[r_symndx].got_offset;
    }
}

static void
symbol_got_offset_mark (bfd *input_bfd, struct cgc_link_hash_entry *h,
			unsigned long r_symndx)
{
  bfd_vma *p;
...
static int
symbol_got_offset_mark_p (bfd *input_bfd, struct cgc_link_hash_entry *h,
			  unsigned long r_symndx)
{
  bfd_vma value;
...
static bfd_vma
symbol_got_offset (bfd *input_bfd, struct cgc_link_hash_entry *h,
		   unsigned long r_symndx)
{
  bfd_vma value;
  value = * symbol_got_offset_ref (input_bfd, h, r_symndx);
...
static bfd_vma *
symbol_tlsdesc_got_offset_ref (bfd *input_bfd, struct cgc_link_hash_entry *h,
			       unsigned long r_symndx)
{
  /* Calculate the address of the GOT entry for symbol
     referred to in h.  */
  if (h != NULL)
    {
      struct cgc_aarch64_link_hash_entry *eh;
      eh = (struct cgc_aarch64_link_hash_entry *) h;
...
static void
symbol_tlsdesc_got_offset_mark (bfd *input_bfd, struct cgc_link_hash_entry *h,
				unsigned long r_symndx)
{
  bfd_vma *p;
...
static bfd_vma
symbol_tlsdesc_got_offset (bfd *input_bfd, struct cgc_link_hash_entry *h,
			  unsigned long r_symndx)
{
  bfd_vma value;
  value = * symbol_tlsdesc_got_offset_ref (input_bfd, h, r_symndx);
...
cgcNN_aarch64_final_link_relocate (reloc_howto_type *howto,
				   bfd *input_bfd,
				   bfd *output_bfd,
				   ...
				   Cgc_Internal_Sym *sym)
{
  ...
  bfd_reloc_code_real_type bfd_r_type
    = cgcNN_aarch64_bfd_reloc_from_howto (howto);
  ...
  BFD_ASSERT (is_aarch64_cgc (input_bfd));

  r_symndx = CGCNN_R_SYM (rel->r_info);
  ...
  if (new_bfd_r_type != bfd_r_type)
    {
      ...
      howto = cgcNN_aarch64_howto_from_bfd_reloc (bfd_r_type);
  ...
  if (h != NULL
	...
	case BFD_RELOC_AARCH64_NN:
	  ...
	  if (info->shared && h->non_got_ref)
		  ...
		  || info->executable)
		{
		  ...
		  outrel.r_info = CGCNN_R_INFO (0, AARCH64_R (IRELATIVE));
		...
		{
		  outrel.r_info = CGCNN_R_INFO (h->dynindx, r_type);
		  outrel.r_addend = 0;
		}
	...
	case BFD_RELOC_AARCH64_CALL26:
	  value = _bfd_aarch64_cgc_resolve_relocation (bfd_r_type, place, value,
						       signed_addend,
						       weak_undef_p);
	  return _bfd_aarch64_cgc_put_addend (input_bfd, hit_data, bfd_r_type,
					      howto, value);
	case BFD_RELOC_AARCH64_LD64_GOT_LO12_NC:
	case BFD_RELOC_AARCH64_LD32_GOT_LO12_NC:
	case BFD_RELOC_AARCH64_ADR_GOT_PAGE:
	case BFD_RELOC_AARCH64_GOT_LD_PREL19:
	  ...
	  value = _bfd_aarch64_cgc_resolve_relocation (bfd_r_type, place, value,
						       0, weak_undef_p);
	  return _bfd_aarch64_cgc_put_addend (input_bfd, hit_data, bfd_r_type, howto, value);
  ...
  switch (bfd_r_type)
	      ...
	      || h->root.type != bfd_link_hash_undefweak))
	{
	  ...
	  *unresolved_reloc_p = FALSE;
	  ...
	  if (skip)
	  ...
	  else
	    {
		 ...
		 so the symbol does not matter.  */
	      symbol = 0;
	      outrel.r_info = CGCNN_R_INFO (symbol, AARCH64_R (RELATIVE));
	  ...
	  bfd_cgcNN_swap_reloca_out (output_bfd, &outrel, loc);
	...
	if (h && h->type == STT_FUNC)
	  {
		...
		struct cgc_aarch64_stub_hash_entry *stub_entry;
		stub_entry = cgcNN_aarch64_get_stub_entry (input_section,
							   sym_sec, h,
							   rel, globals);
		if (stub_entry != NULL)
...
static bfd_reloc_status_type
cgcNN_aarch64_tls_relax (struct cgc_aarch64_link_hash_table *globals,
			 bfd *input_bfd, bfd_byte *contents,
			 Cgc_Internal_Rela *rel, struct cgc_link_hash_entry *h)
{
  bfd_boolean is_local = h == NULL;
  unsigned int r_type = CGCNN_R_TYPE (rel->r_info);
  unsigned long insn;

  BFD_ASSERT (globals && input_bfd && contents && rel);

  switch (cgcNN_aarch64_bfd_reloc_from_type (r_type))
    {
    case BFD_RELOC_AARCH64_TLSGD_ADR_PAGE21:
    case BFD_RELOC_AARCH64_TLSDESC_ADR_PAGE21:
      if (is_local)
      ...
      if (is_local)
	{
	  ...
	  rel[1].r_info = CGCNN_R_INFO (STN_UNDEF, R_AARCH64_NONE);
      ...
      else
	{
	  ...
	  BFD_ASSERT (CGCNN_R_TYPE (rel[1].r_info) == AARCH64_R (CALL26));
...
/* Relocate an AArch64 CGC section.  */

static bfd_boolean
cgcNN_aarch64_relocate_section (bfd *output_bfd,
				struct bfd_link_info *info,
				bfd *input_bfd,
				asection *input_section,
				bfd_byte *contents,
				...
				asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
      ...
      if (howto == NULL)
	{
      ...
      bfd_r_type = cgcNN_aarch64_bfd_reloc_from_howto (howto);
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  sym_type = CGCNN_ST_TYPE (sym->st_info);
	  ...
	  if (!info->relocatable
	    ...
	    {
	      h = cgcNN_aarch64_get_local_sym_hash (globals, input_bfd,
						    rel, FALSE);
	      if (h == NULL)
		abort ();
	  ...
	  && IS_AARCH64_TLS_RELOC (bfd_r_type) != (sym_type == STT_TLS))
	{
      ...
      /* We relax only if we can see that there can be a valid transition
         ...
         We call cgcNN_aarch64_final_link_relocate unless we're completely
      ...
      if (relaxed_bfd_r_type != bfd_r_type)
	{
	  ...
	  BFD_ASSERT (howto != NULL);
	  r_type = howto->type;
	  r = cgcNN_aarch64_tls_relax (globals, input_bfd, contents, rel, h);
	  unresolved_reloc = 0;
	}
      else
	r = bfd_reloc_continue;
	  ...
	  && CGCNN_R_TYPE (rel[1].r_info) != R_AARCH64_NONE
	  && CGCNN_R_TYPE (rel[1].r_info) != R_AARCH64_NULL)
	save_addend = TRUE;
      else
	save_addend = FALSE;

      if (r == bfd_reloc_continue)
	r = cgcNN_aarch64_final_link_relocate (howto, input_bfd, output_bfd,
	...
	case BFD_RELOC_AARCH64_TLSGD_ADD_LO12_NC:
	  if (! symbol_got_offset_mark_p (input_bfd, h, r_symndx))
	      ...
	      if (need_relocs)
		{
		  ...
		  rela.r_info = CGCNN_R_INFO (indx, AARCH64_R (TLS_DTPMOD));
		  rela.r_addend = 0;
		  ...
		  loc += globals->root.srelgot->reloc_count++
		    * RELOC_SIZE (htab);
		  bfd_cgcNN_swap_reloca_out (output_bfd, &rela, loc);
		  ...
		  if (indx == 0)
		  ...
		  else
		    {
			 ...
			 time.  */
		      rela.r_info =
			CGCNN_R_INFO (indx, AARCH64_R (TLS_DTPREL));
		      rela.r_addend = 0;
		      rela.r_offset =
			(globals->root.sgot->output_section->vma
			 + globals->root.sgot->output_offset + off
	...
	case BFD_RELOC_AARCH64_TLSIE_LDNN_GOTTPREL_LO12_NC:
	  if (! symbol_got_offset_mark_p (input_bfd, h, r_symndx))
	      ...
	      if (need_relocs)
		{
		  ...
		  rela.r_info = CGCNN_R_INFO (indx, AARCH64_R (TLS_TPREL));
		  rela.r_offset = globals->root.sgot->output_section->vma +
		    globals->root.sgot->output_offset + off;
	...
	case BFD_RELOC_AARCH64_TLSDESC_LDNN_LO12_NC:
	  if (! symbol_tlsdesc_got_offset_mark_p (input_bfd, h, r_symndx))
	      ...
	      if (need_relocs)
		{
		  bfd_byte *loc;
		  Cgc_Internal_Rela rela;
		  rela.r_info = CGCNN_R_INFO (indx, AARCH64_R (TLSDESC));
...
static bfd_boolean
cgcNN_aarch64_object_p (bfd *abfd)
{
#if ARCH_SIZE == 32
...
/* Function to keep AArch64 specific flags in the CGC header.  */

static bfd_boolean
cgcNN_aarch64_set_private_flags (bfd *abfd, flagword flags)
{
  if (cgc_flags_init (abfd) && cgc_cgcheader (abfd)->e_flags != flags)
    {
    }
  else
    {
...
static bfd_boolean
cgcNN_aarch64_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword in_flags;

  if (!is_aarch64_cgc (ibfd) || !is_aarch64_cgc (obfd))
    return TRUE;
...
static bfd_boolean
cgcNN_aarch64_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword out_flags;
  flagword in_flags;
...
static bfd_boolean
cgcNN_aarch64_print_private_bfd_data (bfd *abfd, void *ptr)
{
  FILE *file = (FILE *) ptr;
  unsigned long flags;
...
static bfd_boolean
cgcNN_aarch64_gc_sweep_hook (bfd *abfd,
			     struct bfd_link_info *info,
			     asection *sec,
			     const Cgc_Internal_Rela * relocs)
{
  ...
  locals = cgc_aarch64_locals (abfd);
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      else
	{
	  ...
	  if (isym != NULL
	    ...
	    {
	      h = cgcNN_aarch64_get_local_sym_hash (htab, abfd, rel, FALSE);
	      if (h == NULL)
		abort ();
	    }
	}
...
static bfd_boolean
cgcNN_aarch64_adjust_dynamic_symbol (struct bfd_link_info *info,
				     struct cgc_link_hash_entry *h)
{
  struct cgc_aarch64_link_hash_table *htab;
  asection *s;
...
static bfd_boolean
cgcNN_aarch64_allocate_local_symbols (bfd *abfd, unsigned number)
{
  struct cgc_aarch64_local_symbol *locals;
  locals = cgc_aarch64_locals (abfd);
  if (locals == NULL)
    {
      locals = (struct cgc_aarch64_local_symbol *)
	bfd_zalloc (abfd, number * sizeof (struct cgc_aarch64_local_symbol));
      if (locals == NULL)
	return FALSE;
      cgc_aarch64_locals (abfd) = locals;
    }
  return TRUE;
}
...
static bfd_boolean
aarch64_cgc_create_got_section (bfd *abfd, struct bfd_link_info *info)
{
...
static bfd_boolean
cgcNN_aarch64_check_relocs (bfd *abfd, struct bfd_link_info *info,
			    asection *sec, const Cgc_Internal_Rela *relocs)
{
  ...
  BFD_ASSERT (is_aarch64_cgc (abfd));
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (CGC_ST_TYPE (isym->st_info) == STT_GNU_IFUNC)
	    {
	      h = cgcNN_aarch64_get_local_sym_hash (htab, abfd, rel,
						    TRUE);
	      if (h == NULL)
		return FALSE;
      ...
      if (h != NULL)
	{
	  ...
	  switch (bfd_r_type)
		...
		htab->root.dynobj = abfd;
	      if (!_bfd_cgc_create_ifunc_sections (htab->root.dynobj, info))
		return FALSE;
	      break;
	    }
	...
	case BFD_RELOC_AARCH64_TLSLE_MOVW_TPREL_G2:
	  {
	    ...
	    if (h)
	      {
		h->got.refcount += 1;
		old_got_type = cgc_aarch64_hash_entry (h)->got_type;
	      }
	    else
	      {
		struct cgc_aarch64_local_symbol *locals;

		if (!cgcNN_aarch64_allocate_local_symbols
	    ...
	    if (old_got_type != got_type)
	      {
		if (h != NULL)
		  cgc_aarch64_hash_entry (h)->got_type = got_type;
		else
		  {
		    struct cgc_aarch64_local_symbol *locals;
		    locals = cgc_aarch64_locals (abfd);
	    ...
	    if (htab->root.dynobj == NULL)
	      htab->root.dynobj = abfd;
	    if (! aarch64_cgc_create_got_section (htab->root.dynobj, info))
	      return FALSE;
	    break;
	  }
...
static bfd_boolean
cgcNN_aarch64_is_target_special_symbol (bfd *abfd ATTRIBUTE_UNUSED,
					asymbol *sym)
{
...
static bfd_boolean
aarch64_cgc_find_function (bfd *abfd ATTRIBUTE_UNUSED,
			   asection *section,
			   asymbol **symbols,
			   bfd_vma offset,
...
/* Find the nearest line to a particular section and offset, for error
   ...
   that it uses aarch64_cgc_find_function.  */

static bfd_boolean
cgcNN_aarch64_find_nearest_line (bfd *abfd,
				 asection *section,
				 asymbol **symbols,
				 bfd_vma offset,
				 const char **filename_ptr,
				 ...
				 unsigned int *line_ptr)
{
  ...
  if (_bfd_dwarf2_find_nearest_line (abfd, dwarf_debug_sections,
    ...
    {
      if (!*functionname_ptr)
	aarch64_cgc_find_function (abfd, section, symbols, offset,
  ...
  if (!aarch64_cgc_find_function (abfd, section, symbols, offset,
				  filename_ptr, functionname_ptr))
    return FALSE;
...
static bfd_boolean
cgcNN_aarch64_find_inliner_info (bfd *abfd,
				 const char **filename_ptr,
				 const char **functionname_ptr,
				 unsigned int *line_ptr)
{
  ...
  found = _bfd_dwarf2_find_inliner_info
    (abfd, filename_ptr,
     functionname_ptr, line_ptr, &cgc_tdata (abfd)->dwarf2_find_line_info);
  return found;
}


static void
cgcNN_aarch64_post_process_headers (bfd *abfd,
				    struct bfd_link_info *link_info)
{
  Cgc_Internal_Ehdr *i_ehdrp;	/* CGC file header, internal form.  */

  i_ehdrp = cgc_cgcheader (abfd);
  i_ehdrp->e_ident[EI_ABIVERSION] = AARCH64_CGC_ABI_VERSION;

  _bfd_cgc_set_osabi (abfd, link_info);
}

static enum cgc_reloc_type_class
cgcNN_aarch64_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
				const asection *rel_sec ATTRIBUTE_UNUSED,
				const Cgc_Internal_Rela *rela)
{
  switch ((int) CGCNN_R_TYPE (rela->r_info))
    {
...
/* Set the right machine number for an AArch64 CGC file.  */

static bfd_boolean
cgcNN_aarch64_section_flags (flagword *flags, const Cgc_Internal_Shdr *hdr)
{
...
static bfd_boolean
cgcNN_aarch64_section_from_shdr (bfd *abfd,
				 Cgc_Internal_Shdr *hdr,
...
/* Unfortunately we need to keep a list of sections for which
   an _aarch64_cgc_section_data structure has been allocated.  This
   is because it is possible for functions like cgcNN_aarch64_write_section
   to be called on a section which has had an cgc_data_structure
   allocated for it (and so the used_by_bfd field is valid) but
   for which the AArch64 extended version of this structure - the
   _aarch64_cgc_section_data structure - has not been allocated.  */
static section_list *sections_with_aarch64_cgc_section_data = NULL;

static void
record_section_with_aarch64_cgc_section_data (asection *sec)
{
  ...
  if (entry == NULL)
    return;
  entry->sec = sec;
  entry->next = sections_with_aarch64_cgc_section_data;
  entry->prev = NULL;
  if (entry->next != NULL)
    entry->next->prev = entry;
  sections_with_aarch64_cgc_section_data = entry;
}

static struct section_list *
find_aarch64_cgc_section_entry (asection *sec)
{
  struct section_list *entry;
  static struct section_list *last_entry = NULL;

  /* This is a short cut for the typical case where the sections are added
     to the sections_with_aarch64_cgc_section_data list in forward order and
     then looked up here in backwards order.  This makes a real difference
     to the ld-srec/sec64k.exp linker test.  */
  entry = sections_with_aarch64_cgc_section_data;
  if (last_entry != NULL)
    {
  ...
  if (entry)
    /* Record the entry prior to this one - it is the entry we are
       ...
       unrecord_section_with_aarch64_cgc_section_data () we will not
...
static void
unrecord_section_with_aarch64_cgc_section_data (asection *sec)
{
  struct section_list *entry;

  entry = find_aarch64_cgc_section_entry (sec);
  ...
  if (entry)
	...
	entry->next->prev = entry->prev;
      if (entry == sections_with_aarch64_cgc_section_data)
	sections_with_aarch64_cgc_section_data = entry->next;
      free (entry);
    }
}
...
static bfd_boolean
cgcNN_aarch64_output_map_sym (output_arch_syminfo *osi,
...
static bfd_boolean
cgcNN_aarch64_output_plt_map (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if (addr == 32)
    {
      if (!cgcNN_aarch64_output_map_sym (osi, AARCH64_MAP_INSN, addr))
	return FALSE;
    }
  return TRUE;
}
...
static bfd_boolean
cgcNN_aarch64_output_stub_sym (output_arch_syminfo *osi, const char *name,
...
aarch64_map_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg)
{
  ...
  switch (stub_entry->stub_type)
    {
    case aarch64_stub_adrp_branch:
      if (!cgcNN_aarch64_output_stub_sym (osi, stub_name, addr,
	...
	return FALSE;
      break;
    case aarch64_stub_long_branch:
      if (!cgcNN_aarch64_output_stub_sym
	  (osi, stub_name, addr, sizeof (aarch64_long_branch_stub)))
	return FALSE;
      if (!cgcNN_aarch64_output_map_sym (osi, AARCH64_MAP_INSN, addr))
	return FALSE;
      if (!cgcNN_aarch64_output_map_sym (osi, AARCH64_MAP_DATA, addr + 16))
	return FALSE;
      break;
    default:
      BFD_FAIL ();
...
static bfd_boolean
cgcNN_aarch64_output_arch_local_syms (bfd *output_bfd,
				      struct bfd_link_info *info,
				      void *finfo,
						   ...
						   *))
{
  ...
  cgc_link_hash_traverse (&htab->root, cgcNN_aarch64_output_plt_map,
			  (void *) &osi);
...
static bfd_boolean
cgcNN_aarch64_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      _aarch64_cgc_section_data *sdata;
  ...
  record_section_with_aarch64_cgc_section_data (sec);
...
{
  unrecord_section_with_aarch64_cgc_section_data (sec);
}

static bfd_boolean
cgcNN_aarch64_close_and_cleanup (bfd *abfd)
{
  if (abfd->sections)
    bfd_map_over_sections (abfd,
			   unrecord_section_via_map_over_sections, NULL);

  return _bfd_cgc_close_and_cleanup (abfd);
}

static bfd_boolean
cgcNN_aarch64_bfd_free_cached_info (bfd *abfd)
{
  if (abfd->sections)
    bfd_map_over_sections (abfd,
			   unrecord_section_via_map_over_sections, NULL);
...
/* Create dynamic sections. This is different from the ARM backend in that
   ...
   standard part of the bfd cgc backend.  */

static bfd_boolean
cgcNN_aarch64_create_dynamic_sections (bfd *dynobj,
				       struct bfd_link_info *info)
{
  struct cgc_aarch64_link_hash_table *htab;

  /* We need to create .got section.  */
  if (!aarch64_cgc_create_got_section (dynobj, info))
    return FALSE;
...
static bfd_boolean
cgcNN_aarch64_allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  struct bfd_link_info *info;
  struct cgc_aarch64_link_hash_table *htab;
  struct cgc_aarch64_link_hash_entry *eh;
  ...
  /* An example of a bfd_link_hash_indirect symbol is versioned
     ...
     the symbol and cgcNN_aarch64_copy_indirect_symbol () will have been
  ...
  if (h->got.refcount > 0)
    {
      bfd_boolean dyn;
      unsigned got_type = cgc_aarch64_hash_entry (h)->got_type;
...
static bfd_boolean
cgcNN_aarch64_allocate_ifunc_dynrelocs (struct cgc_link_hash_entry *h,
					void *inf)
{
  struct bfd_link_info *info;
  struct cgc_aarch64_link_hash_table *htab;
...
static bfd_boolean
cgcNN_aarch64_allocate_local_dynrelocs (void **slot, void *inf)
{
  struct cgc_link_hash_entry *h
  ...
  return cgcNN_aarch64_allocate_dynrelocs (h, inf);
}
...
static bfd_boolean
cgcNN_aarch64_allocate_local_ifunc_dynrelocs (void **slot, void *inf)
{
  struct cgc_link_hash_entry *h
    = (struct cgc_link_hash_entry *) *slot;
  ...
  return cgcNN_aarch64_allocate_ifunc_dynrelocs (h, inf);
}

/* This is the most important function of all . Innocuosly named
   though !  */
static bfd_boolean
cgcNN_aarch64_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				     struct bfd_link_info *info)
{
  struct cgc_aarch64_link_hash_table *htab;
  bfd *dynobj;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
    {
      struct cgc_aarch64_local_symbol *locals = NULL;
      Cgc_Internal_Shdr *symtab_hdr;
      asection *srel;
      unsigned int i;

      if (!is_aarch64_cgc (ibfd))
	continue;
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
      ...
      locals = cgc_aarch64_locals (ibfd);
      if (!locals)
	continue;
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  cgc_link_hash_traverse (&htab->root, cgcNN_aarch64_allocate_dynrelocs,
			  info);

  /* Allocate global ifunc sym .plt and .got entries, and space for global
     ifunc sym dynamic relocs.  */
  cgc_link_hash_traverse (&htab->root, cgcNN_aarch64_allocate_ifunc_dynrelocs,
			  info);
  ...
  htab_traverse (htab->loc_hash_table,
		 cgcNN_aarch64_allocate_local_ifunc_dynrelocs,
		 info);
  ...
  if (htab->root.dynamic_sections_created)
    {
      /* Add some entries to the .dynamic section.  We fill in the
         values later, in cgcNN_aarch64_finish_dynamic_sections, but we
...
static inline void
cgc_aarch64_update_plt_entry (bfd *output_bfd,
			      bfd_reloc_code_real_type r_type,
			      bfd_byte *plt_entry, bfd_vma value)
{
  reloc_howto_type *howto = cgcNN_aarch64_howto_from_bfd_reloc (r_type);

  _bfd_aarch64_cgc_put_addend (output_bfd, plt_entry, r_type, howto, value);
}

static void
cgcNN_aarch64_create_small_pltn_entry (struct cgc_link_hash_entry *h,
				       struct cgc_aarch64_link_hash_table
				       *htab, bfd *output_bfd,
				       struct bfd_link_info *info)
{
  ...
  memcpy (plt_entry, cgcNN_aarch64_small_plt_entry, PLT_SMALL_ENTRY_SIZE);

  /* Fill in the top 21 bits for this: ADRP x16, PLT_GOT + n * 8.
     ADRP:   ((PG(S+A)-PG(P)) >> 12) & 0x1fffff */
  cgc_aarch64_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_ADR_HI21_PCREL,
				plt_entry,
				PG (gotplt_entry_address) -
				PG (plt_entry_address));

  /* Fill in the lo12 bits for the load from the pltgot.  */
  cgc_aarch64_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_LDSTNN_LO12,
				plt_entry + 4,
				PG_OFFSET (gotplt_entry_address));

  /* Fill in the lo12 bits for the add from the pltgot entry.  */
  cgc_aarch64_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_ADD_LO12,
				plt_entry + 8,
				PG_OFFSET (gotplt_entry_address));
  ...
  if (h->dynindx == -1
	  ...
	  && h->type == STT_GNU_IFUNC))
    {
	 ...
	 R_AARCH64_IRELATIVE instead of R_AARCH64_JUMP_SLOT.  */
      rela.r_info = CGCNN_R_INFO (0, AARCH64_R (IRELATIVE));
  ...
  else
    {
      ...
      rela.r_info = CGCNN_R_INFO (h->dynindx, AARCH64_R (JUMP_SLOT));
      rela.r_addend = 0;
    }
...
static bfd_boolean
cgcNN_aarch64_always_size_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  asection *tls_sec;
  ...
  if (tls_sec)
      ...
      if (tlsbase)
	{
	  ...
	  tlsbase = (struct cgc_link_hash_entry *) h;
...
static bfd_boolean
cgcNN_aarch64_finish_dynamic_symbol (bfd *output_bfd,
				     struct bfd_link_info *info,
				     struct cgc_link_hash_entry *h,
				     Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) - 1)
	...
	abort ();
      ...
      cgcNN_aarch64_create_small_pltn_entry (h, htab, output_bfd, info);
      if (!h->def_regular)
	{
  ...
  if (h->got.offset != (bfd_vma) - 1
      && cgc_aarch64_hash_entry (h)->got_type == GOT_NORMAL)
    {
      Cgc_Internal_Rela rela;
      bfd_byte *loc;
      ...
      if (info->shared && SYMBOL_REFERENCES_LOCAL (info, h))
	{
	  ...
	  rela.r_info = CGCNN_R_INFO (0, AARCH64_R (RELATIVE));
	  rela.r_addend = (h->root.u.def.value
			   + h->root.u.def.section->output_section->vma
			   + h->root.u.def.section->output_offset);
	}
      else
	{
	  ...
	  rela.r_info = CGCNN_R_INFO (h->dynindx, AARCH64_R (GLOB_DAT));
	  rela.r_addend = 0;
	}
  ...
  if (h->needs_copy)
	...
	abort ();
      ...
      rela.r_offset = (h->root.u.def.value
		       + h->root.u.def.section->output_section->vma
		       + h->root.u.def.section->output_offset);
      rela.r_info = CGCNN_R_INFO (h->dynindx, AARCH64_R (COPY));
      rela.r_addend = 0;
  ...
  if (sym != NULL
      && (h == cgc_hash_table (info)->hdynamic
...
static bfd_boolean
cgcNN_aarch64_finish_local_dynamic_symbol (void **slot, void *inf)
{
  struct cgc_link_hash_entry *h
    = (struct cgc_link_hash_entry *) *slot;
  struct bfd_link_info *info
  ...
  return cgcNN_aarch64_finish_dynamic_symbol (info->output_bfd,
					      info, h, NULL);
}

static void
cgcNN_aarch64_init_small_plt0_entry (bfd *output_bfd ATTRIBUTE_UNUSED,
				     struct cgc_aarch64_link_hash_table
				     *htab)
{
  ...
  /* PLT0 of the small PLT looks like this in CGC64 -
     ...
     br   x17
     PLT0 will be slightly different in CGC32 due to different got entry
     size.
   */
  ...
  memcpy (htab->root.splt->contents, cgcNN_aarch64_small_plt0_entry,
	  PLT_ENTRY_SIZE);
  cgc_section_data (htab->root.splt->output_section)->this_hdr.sh_entsize =
    PLT_ENTRY_SIZE;
...
static bfd_boolean
cgcNN_aarch64_finish_dynamic_sections (bfd *output_bfd,
				       struct bfd_link_info *info)
{
  struct cgc_aarch64_link_hash_table *htab;
  bfd *dynobj;
  ...
  if (htab->root.dynamic_sections_created)
    {
      CgcNN_External_Dyn *dyncon, *dynconend;

      if (sdyn == NULL || htab->root.sgot == NULL)
	abort ();

      dyncon = (CgcNN_External_Dyn *) sdyn->contents;
      dynconend = (CgcNN_External_Dyn *) (sdyn->contents + sdyn->size);
      for (; dyncon < dynconend; dyncon++)
	{
	  Cgc_Internal_Dyn dyn;
	  asection *s;

	  bfd_cgcNN_swap_dyn_in (dynobj, dyncon, &dyn);
	  ...
	  bfd_cgcNN_swap_dyn_out (output_bfd, &dyn, dyncon);
	}
  ...
  if (htab->root.splt && htab->root.splt->size > 0)
    {
      cgcNN_aarch64_init_small_plt0_entry (output_bfd, htab);

      cgc_section_data (htab->root.splt->output_section)->
      ...
      if (htab->tlsdesc_plt)
	{
	  ...
	  memcpy (htab->root.splt->contents + htab->tlsdesc_plt,
		  cgcNN_aarch64_tlsdesc_small_plt_entry,
		  sizeof (cgcNN_aarch64_tlsdesc_small_plt_entry));

	  {
	    ...
	    cgc_aarch64_update_plt_entry (output_bfd,
  ...
  if (htab->root.sgotplt)
      ...
      if (htab->root.sgot)
	{
      ...
      cgc_section_data (htab->root.sgotplt->output_section)->
	this_hdr.sh_entsize = GOT_ENTRY_SIZE;
    }

  if (htab->root.sgot && htab->root.sgot->size > 0)
    cgc_section_data (htab->root.sgot->output_section)->this_hdr.sh_entsize
      = GOT_ENTRY_SIZE;
...
static bfd_vma
cgcNN_aarch64_plt_sym_val (bfd_vma i, const asection *plt,
...
const struct cgc_size_info cgcNN_aarch64_size_info =
{
  sizeof (CgcNN_External_Ehdr),
  sizeof (CgcNN_External_Phdr),
  sizeof (CgcNN_External_Shdr),
  sizeof (CgcNN_External_Rel),
  sizeof (CgcNN_External_Rela),
  sizeof (CgcNN_External_Sym),
  sizeof (CgcNN_External_Dyn),
  sizeof (Cgc_External_Note),
  4,				/* Hash table entry size.  */
  1,				/* Internal relocs per external relocs.  */
  ARCH_SIZE,			/* Arch size.  */
  LOG_FILE_ALIGN,		/* Log_file_align.  */
  CGCCLASSNN, EV_CURRENT,
  bfd_cgcNN_write_out_phdrs,
  bfd_cgcNN_write_shdrs_and_ehdr,
  bfd_cgcNN_checksum_contents,
  bfd_cgcNN_write_relocs,
  bfd_cgcNN_swap_symbol_in,
  bfd_cgcNN_swap_symbol_out,
  bfd_cgcNN_slurp_reloc_table,
  bfd_cgcNN_slurp_symbol_table,
  bfd_cgcNN_swap_dyn_in,
  bfd_cgcNN_swap_dyn_out,
  bfd_cgcNN_swap_reloc_in,
  bfd_cgcNN_swap_reloc_out,
  bfd_cgcNN_swap_reloca_in,
  bfd_cgcNN_swap_reloca_out
};

#define CGC_ARCH			bfd_arch_aarch64
#define CGC_MACHINE_CODE		EM_AARCH64
#define CGC_MAXPAGESIZE			0x10000
#define CGC_MINPAGESIZE			0x1000
#define CGC_COMMONPAGESIZE		0x1000

#define bfd_cgcNN_close_and_cleanup             \
  cgcNN_aarch64_close_and_cleanup

#define bfd_cgcNN_bfd_copy_private_bfd_data	\
  cgcNN_aarch64_copy_private_bfd_data

#define bfd_cgcNN_bfd_free_cached_info          \
  cgcNN_aarch64_bfd_free_cached_info

#define bfd_cgcNN_bfd_is_target_special_symbol	\
  cgcNN_aarch64_is_target_special_symbol

#define bfd_cgcNN_bfd_link_hash_table_create    \
  cgcNN_aarch64_link_hash_table_create

#define bfd_cgcNN_bfd_link_hash_table_free      \
  cgcNN_aarch64_hash_table_free

#define bfd_cgcNN_bfd_merge_private_bfd_data	\
  cgcNN_aarch64_merge_private_bfd_data

#define bfd_cgcNN_bfd_print_private_bfd_data	\
  cgcNN_aarch64_print_private_bfd_data

#define bfd_cgcNN_bfd_reloc_type_lookup		\
  cgcNN_aarch64_reloc_type_lookup

#define bfd_cgcNN_bfd_reloc_name_lookup		\
  cgcNN_aarch64_reloc_name_lookup

#define bfd_cgcNN_bfd_set_private_flags		\
  cgcNN_aarch64_set_private_flags

#define bfd_cgcNN_find_inliner_info		\
  cgcNN_aarch64_find_inliner_info

#define bfd_cgcNN_find_nearest_line		\
  cgcNN_aarch64_find_nearest_line

#define bfd_cgcNN_mkobject			\
  cgcNN_aarch64_mkobject

#define bfd_cgcNN_new_section_hook		\
  cgcNN_aarch64_new_section_hook

#define cgc_backend_adjust_dynamic_symbol	\
  cgcNN_aarch64_adjust_dynamic_symbol

#define cgc_backend_always_size_sections	\
  cgcNN_aarch64_always_size_sections

#define cgc_backend_check_relocs		\
  cgcNN_aarch64_check_relocs

#define cgc_backend_copy_indirect_symbol	\
  cgcNN_aarch64_copy_indirect_symbol

/* Create .dynbss, and .rela.bss sections in DYNOBJ, and set up shortcuts
   to them in our hash.  */
#define cgc_backend_create_dynamic_sections	\
  cgcNN_aarch64_create_dynamic_sections

#define cgc_backend_init_index_section		\
  _bfd_cgc_init_2_index_sections

#define cgc_backend_finish_dynamic_sections	\
  cgcNN_aarch64_finish_dynamic_sections

#define cgc_backend_finish_dynamic_symbol	\
  cgcNN_aarch64_finish_dynamic_symbol

#define cgc_backend_gc_sweep_hook		\
  cgcNN_aarch64_gc_sweep_hook

#define cgc_backend_object_p			\
  cgcNN_aarch64_object_p

#define cgc_backend_output_arch_local_syms      \
  cgcNN_aarch64_output_arch_local_syms

#define cgc_backend_plt_sym_val			\
  cgcNN_aarch64_plt_sym_val

#define cgc_backend_post_process_headers	\
  cgcNN_aarch64_post_process_headers

#define cgc_backend_relocate_section		\
  cgcNN_aarch64_relocate_section

#define cgc_backend_reloc_type_class		\
  cgcNN_aarch64_reloc_type_class

#define cgc_backend_section_flags		\
  cgcNN_aarch64_section_flags

#define cgc_backend_section_from_shdr		\
  cgcNN_aarch64_section_from_shdr

#define cgc_backend_size_dynamic_sections	\
  cgcNN_aarch64_size_dynamic_sections

#define cgc_backend_size_info			\
  cgcNN_aarch64_size_info
