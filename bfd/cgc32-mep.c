/* MeP-specific support for 32-bit CGC.
...
#define VALID_MEP_RELOC(N) ((N) >= 0 \
  && (N) < ARRAY_SIZE (mep_cgc_howto_table)
     ...
     bfd_reloc_code_real_type code)
{
  ...
  if (mep_cgc_howto_table[type].type != type)
    {
      (*_bfd_error_handler) (_("MeP: howto %d has type %d"),
			     type, mep_cgc_howto_table[type].type);
      abort ();
    }

  return mep_cgc_howto_table + type;
}
...
mep_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (mep_cgc_howto_table) / sizeof (mep_cgc_howto_table[0]);
       i++)
    if (mep_cgc_howto_table[i].name != NULL
	&& strcasecmp (mep_cgc_howto_table[i].name, r_name) == 0)
      return &mep_cgc_howto_table[i];
...

/* Set the howto pointer for a MEP CGC reloc.  */
     ...
     Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = & mep_cgc_howto_table [r_type];
}

/* Relocate a MEP CGC section.
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = mep_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
	...
	continue;
      ...
      if (r_type == R_RELC)
	r = bfd_cgc_perform_complex_relocation (input_bfd, input_section,
...
static bfd_boolean
mep_cgc_set_private_flags (bfd *    abfd,
			   flagword flags)
{
...
static bfd_boolean
mep_cgc_copy_private_bfd_data (bfd * ibfd, bfd * obfd)
{
...
static bfd_boolean
mep_cgc_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
...
static bfd_boolean
mep_cgc_print_private_bfd_data (bfd * abfd, void * ptr)
{
  FILE *   file = (FILE *) ptr;
...
static int
cgc32_mep_machine (bfd * abfd)
{
  switch (cgc_cgcheader (abfd)->e_flags & EF_MEP_CPU_MASK)
    {
    default: break;
...
static bfd_boolean
mep_cgc_object_p (bfd * abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_mep, cgc32_mep_machine (abfd));
  return TRUE;
}

static bfd_boolean
mep_cgc_section_flags (flagword * flags, const Cgc_Internal_Shdr * hdr)
{
...
static bfd_boolean
mep_cgc_fake_sections (bfd *               abfd ATTRIBUTE_UNUSED,
...

#define CGC_ARCH		bfd_arch_mep
#define CGC_MACHINE_CODE	EM_CYGNUS_MEP
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM		bfd_cgc32_mep_vec
#define TARGET_BIG_NAME		"cgc32-mep"

#define TARGET_LITTLE_SYM	bfd_cgc32_mep_little_vec
#define TARGET_LITTLE_NAME	"cgc32-mep-little"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			mep_info_to_howto_rela
#define cgc_backend_relocate_section		mep_cgc_relocate_section
#define cgc_backend_object_p		        mep_cgc_object_p
#define cgc_backend_section_flags		mep_cgc_section_flags
#define cgc_backend_fake_sections		mep_cgc_fake_sections

#define bfd_cgc32_bfd_reloc_type_lookup		mep_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		mep_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		mep_cgc_set_private_flags
#define bfd_cgc32_bfd_copy_private_bfd_data	mep_cgc_copy_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data	mep_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	mep_cgc_print_private_bfd_data
