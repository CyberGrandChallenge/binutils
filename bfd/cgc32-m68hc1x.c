/* Motorola 68HC11/HC12-specific support for 32-bit CGC
...
#define m68hc12_stub_hash_lookup(table, string, create, copy) \
  ((struct cgc32_m68hc11_stub_hash_entry *) \
   bfd_hash_lookup ((table), (string), (create), (copy)))

static struct cgc32_m68hc11_stub_hash_entry* m68hc12_add_stub
...
static void m68hc11_cgc_set_symbol (bfd* abfd, struct bfd_link_info *info,
                                    const char* name, bfd_vma value,
                                    asection* sec);

static bfd_boolean m68hc11_cgc_export_one_stub
...
/* Create a 68HC11/68HC12 CGC linker hash table.  */
...
void
m68hc11_cgc_bfd_link_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct m68hc11_cgc_link_hash_table *ret
    = (struct m68hc11_cgc_link_hash_table *) hash;
                   ...
                   const char *string)
{
  ...
  if (entry != NULL)
    {
      struct cgc32_m68hc11_stub_hash_entry *eh;

      /* Initialize the local fields.  */
      eh = (struct cgc32_m68hc11_stub_hash_entry *) entry;
      eh->stub_sec = NULL;
      eh->stub_offset = 0;
      eh->target_value = 0;
...
static struct cgc32_m68hc11_stub_hash_entry *
...
bfd_boolean
cgc32_m68hc11_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
...
void
cgc32_m68hc11_merge_symbol_attribute (struct cgc_link_hash_entry *h,
...
int
cgc32_m68hc11_setup_section_lists (bfd *output_bfd, struct bfd_link_info *info)
{
  bfd *input_bfd;
  ...
  if (bfd_get_flavour (info->output_bfd) != bfd_target_cgc_flavour)
    return 0;
...
bfd_boolean
cgc32_m68hc11_size_stubs (bfd *output_bfd, bfd *stub_bfd,
                          ...
                          asection * (*add_stub_section) (const char*, asection*))
{
  ...
  bfd_size_type amt;
  asection *stub_sec;
  struct m68hc11_cgc_link_hash_table *htab = m68hc11_cgc_hash_table (info);
  ...
  amt = sizeof (Cgc_Internal_Sym *) * bfd_count;
  all_local_syms = (Cgc_Internal_Sym **) bfd_zmalloc (amt);
       ...
       input_bfd = input_bfd->link_next, bfd_indx++)
    {
      ...
      for (section = input_bfd->sections;
           ...
           section = section->next)
        {
		  ...
		  if (sym->st_shndx >= cgc_numsections (input_bfd))
		    sym_sec = NULL;
		  else
		    {
		      ...
		      sym_sec = hdr->bfd_section;
                  ...
                  stub_name = (bfd_cgc_string_from_cgc_section
              ...
              else
                {
                  ...
                  hash = (struct cgc_link_hash_entry *)
                    (sym_hashes[e_indx]);
...
static bfd_boolean
m68hc11_cgc_export_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg)
{
  struct bfd_link_info *info;
  struct m68hc11_cgc_link_hash_table *htab;
  struct cgc32_m68hc11_stub_hash_entry *stub_entry;
  ...
  m68hc11_cgc_set_symbol (htab->stub_bfd, info, name,
...
static void
m68hc11_cgc_set_symbol (bfd *abfd, struct bfd_link_info *info,
...
/* Build all the stubs associated with the current output file.  The
   ...
   table.  This function is called via m68hc12cgc_finish in the
   linker.  */

bfd_boolean
cgc32_m68hc11_build_stubs (bfd *abfd, struct bfd_link_info *info)
{
  asection *stub_sec;
  ...
  m68hc11_cgc_get_bank_parameters (info);
  htab = m68hc11_cgc_hash_table (info);
  if (htab == NULL)
    return FALSE;
  ...
  bfd_hash_traverse (table, m68hc11_cgc_export_one_stub, info);
  ...
  if (param.use_memory_banks)
    {
      m68hc11_cgc_set_symbol (abfd, info, BFD_M68HC11_BANK_START_NAME,
                              htab->pinfo.bank_physical,
                              bfd_abs_section_ptr);
      m68hc11_cgc_set_symbol (abfd, info, BFD_M68HC11_BANK_VIRTUAL_NAME,
                              htab->pinfo.bank_virtual,
                              bfd_abs_section_ptr);
      m68hc11_cgc_set_symbol (abfd, info, BFD_M68HC11_BANK_SIZE_NAME,
...
void
m68hc11_cgc_get_bank_parameters (struct bfd_link_info *info)
{
  unsigned i;
  struct m68hc11_page_info *pinfo;
...
m68hc11_cgc_ignore_reloc (bfd *abfd ATTRIBUTE_UNUSED,
                          arelent *reloc_entry,
...
bfd_reloc_status_type
m68hc11_cgc_special_reloc (bfd *abfd ATTRIBUTE_UNUSED,
                           arelent *reloc_entry,
                           asymbol *symbol,
                           void *data ATTRIBUTE_UNUSED,
                           asection *input_section,
...
bfd_boolean
cgc32_m68hc11_check_relocs (bfd *abfd, struct bfd_link_info *info,
...
/* Relocate a 68hc11/68hc12 CGC section.  */
bfd_boolean
cgc32_m68hc11_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
                                struct bfd_link_info *info,
                                ...
                                asection **local_sections)
{
  ...
  struct m68hc11_page_info *pinfo;
  const struct cgc_backend_data * const ebd = get_cgc_backend_data (input_bfd);
  struct m68hc11_cgc_link_hash_table *htab;
  unsigned long e_flags;

  symtab_hdr = &cgc_tdata (input_bfd)->symtab_hdr;
  sym_hashes = cgc_sym_hashes (input_bfd);
  e_flags = cgc_cgcheader (input_bfd)->e_flags;
  ...
  for (; rel < relend; rel++)
    {
      ...
      (*ebd->cgc_info_to_howto_rel) (input_bfd, &arel, rel);
      howto = arel.howto;
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  is_section_symbol = CGC_ST_TYPE (sym->st_info) & STT_SECTION;
	}
      else
	{
      ...
      else
	{
      ...
      if (is_far && CGC32_R_TYPE (rel->r_info) == R_M68HC11_16)
	{
	  struct cgc32_m68hc11_stub_hash_entry* stub;
...

/* Set and control CGC flags in CGC header.  */

bfd_boolean
_bfd_m68hc11_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
bfd_boolean
_bfd_m68hc11_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword old_flags;
  flagword new_flags;
  bfd_boolean ok = TRUE;
  ...
  cgc_cgcheader (obfd)->e_flags |= new_flags & EF_M68HC11_ABI;
...
bfd_boolean
_bfd_m68hc11_cgc_print_private_bfd_data (bfd *abfd, void *ptr)
{
  FILE *file = (FILE *) ptr;
  ...
  if (cgc_cgcheader (abfd)->e_flags & E_M68HC11_I32)
    fprintf (file, _("[abi=32-bit int, "));
  else
    fprintf (file, _("[abi=16-bit int, "));

  if (cgc_cgcheader (abfd)->e_flags & E_M68HC11_F64)
    fprintf (file, _("64-bit double, "));
  else
    fprintf (file, _("32-bit double, "));

  if (strcmp (bfd_get_target (abfd), "cgc32-m68hc11") == 0)
    fprintf (file, _("cpu=HC11]"));
  else if (cgc_cgcheader (abfd)->e_flags & EF_M68HCS12_MACH)
    fprintf (file, _("cpu=HCS12]"));
  else
    fprintf (file, _("cpu=HC12]"));

  if (cgc_cgcheader (abfd)->e_flags & E_M68HC12_BANKS)
    fprintf (file, _(" [memory=bank-model]"));
  else
    fprintf (file, _(" [memory=flat]"));

  if (cgc_cgcheader (abfd)->e_flags & E_M68HC11_XGATE_RAMOFFSET)
...
/* Tweak the OSABI field of the cgc header.  */

void
cgc32_m68hc11_post_process_headers (bfd *abfd, struct bfd_link_info *link_info)
{
  struct m68hc11_scan_param param;
  struct m68hc11_cgc_link_hash_table *htab;
  ...
  htab = m68hc11_cgc_hash_table (link_info);
  if (htab == NULL)
    return;

  m68hc11_cgc_get_bank_parameters (link_info);
