/* Motorola 68HC11-specific support for 32-bit CGC
...
static bfd_boolean m68hc11_cgc_size_one_stub
  (struct bfd_hash_entry *gen_entry, void *in_arg);
static bfd_boolean m68hc11_cgc_build_one_stub
  (struct bfd_hash_entry *gen_entry, void *in_arg);
static struct bfd_link_hash_table* m68hc11_cgc_bfd_link_hash_table_create
  (bfd* abfd);

/* Linker relaxation.  */
static bfd_boolean m68hc11_cgc_relax_section
  (bfd *, asection *, struct bfd_link_info *, bfd_boolean *);
static void m68hc11_cgc_relax_delete_bytes
...
static reloc_howto_type cgc_m68hc11_howto_table[] = {
  ...
  HOWTO (R_M68HC11_RL_JUMP,	/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_dont,	/* complain_on_overflow */
	 m68hc11_cgc_ignore_reloc,	/* special_function */
...
/* Map BFD reloc types to M68HC11 CGC reloc types.  */
                                 ...
                                 bfd_reloc_code_real_type code)
{
  ...
  for (i = 0;
	...
	return &cgc_m68hc11_howto_table[m68hc11_reloc_map[i].cgc_reloc_val];
    }
				 ...
				 const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (cgc_m68hc11_howto_table)
	    / sizeof (cgc_m68hc11_howto_table[0]));
       i++)
    if (cgc_m68hc11_howto_table[i].name != NULL
	&& strcasecmp (cgc_m68hc11_howto_table[i].name, r_name) == 0)
      return &cgc_m68hc11_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an M68HC11 CGC reloc.  */
                           ...
                           arelent *cache_ptr, Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = &cgc_m68hc11_howto_table[r_type];
}
...
static bfd_boolean
m68hc11_cgc_build_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg)
{
  struct cgc32_m68hc11_stub_hash_entry *stub_entry;
  struct bfd_link_info *info;
  struct m68hc11_cgc_link_hash_table *htab;
  asection *stub_sec;
  bfd *stub_bfd;
  bfd_byte *loc;
  ...
  stub_entry = (struct cgc32_m68hc11_stub_hash_entry *) gen_entry;
  info = (struct bfd_link_info *) in_arg;

  htab = m68hc11_cgc_hash_table (info);
  if (htab == NULL)
    return FALSE;
...
static bfd_boolean
m68hc11_cgc_size_one_stub (struct bfd_hash_entry *gen_entry,
...
/* Create a 68HC11 CGC linker hash table.  */

static struct bfd_link_hash_table *
m68hc11_cgc_bfd_link_hash_table_create (bfd *abfd)
{
  struct m68hc11_cgc_link_hash_table *ret;

  ret = m68hc11_cgc_hash_table_create (abfd);
  if (ret == (struct m68hc11_cgc_link_hash_table *) NULL)
    return NULL;

  ret->size_one_stub = m68hc11_cgc_size_one_stub;
  ret->build_one_stub = m68hc11_cgc_build_one_stub;
      ...
      if ((new_value & 0xff00) == 0 && bset_use_y == relax_ldy)
        {
          ...
          if (start_offset != offset)
            {
              m68hc11_cgc_relax_delete_bytes (abfd, sec, start_offset,
                                              offset - start_offset);
              end_group--;
            }
        }
  ...
  if (can_delete_ldx)
    {
      ...
      m68hc11_cgc_relax_delete_bytes (abfd, sec, ldx_offset, ldx_size);
    }
}
...
static bfd_boolean
m68hc11_cgc_relax_section (bfd *abfd, asection *sec,
                           struct bfd_link_info *link_info, bfd_boolean *again)
{
  ...
  Cgc32_External_Sym *free_extsyms = NULL;
  Cgc_Internal_Rela *prev_insn_branch = NULL;
  Cgc_Internal_Rela *prev_insn_group = NULL;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
	 ...
	 this reloc.  */
      if (CGC32_R_TYPE (irel->r_info) != (int) R_M68HC11_16
          && CGC32_R_TYPE (irel->r_info) != (int) R_M68HC11_RL_JUMP
          && CGC32_R_TYPE (irel->r_info) != (int) R_M68HC11_RL_GROUP)
        {
          prev_insn_branch = 0;
          prev_insn_group = 0;
          continue;
      ...
      if (contents == NULL)
	{
	 ...
	 but due to other relaxations it can reach at link time.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_M68HC11_RL_JUMP)
	{
	  ...
	  if (nrel == irelend
	      ...
	      || CGC32_R_TYPE (nrel->r_info) != (int) R_M68HC11_16)
	    continue;
      ...
      else
	{
      ...
      if (CGC32_R_TYPE (irel->r_info) == (int) R_M68HC11_RL_GROUP)
	{
          prev_insn_branch = 0;
          prev_insn_group = 0;
          ...
          if (code == 0x7e)
            {
              ...
              irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info),
                                           R_M68HC11_PCREL_8);
              m68hc11_cgc_relax_delete_bytes (abfd, sec,
                                              irel->r_offset + 1, 1);
            }
          else
            {
      ...
      else if (CGC32_R_TYPE (irel->r_info) == (int) R_M68HC11_16
               && (value & 0xff00) == 0)
	{
          unsigned char code;
          ...
          m68hc11_cgc_relax_delete_bytes (abfd, sec, irel->r_offset, 1);
      ...
      else if (CGC32_R_TYPE (irel->r_info) == R_M68HC11_16 && !is_far)
        {
          unsigned char code;
          bfd_vma offset;
...
static void
m68hc11_cgc_relax_delete_bytes (bfd *abfd, asection *sec,
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
    {
	 ...
	 represent addresses, though.  */
      if (CGC32_R_TYPE (irel->r_info) != R_M68HC11_RL_JUMP
          && irel->r_offset >= addr && irel->r_offset < addr + count)
        irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info),
                                     R_M68HC11_NONE);

      if (CGC32_R_TYPE (irel->r_info) == R_M68HC11_NONE)
        continue;
...
/* Specific sections:
   ...
   - The .vectors is the section that represents the interrupt
     vectors.  */
static const struct bfd_cgc_special_section cgc32_m68hc11_special_sections[] =
{
...

#define CGC_ARCH		bfd_arch_m68hc11
#define CGC_TARGET_ID		M68HC11_CGC_DATA
#define CGC_MACHINE_CODE	EM_68HC11
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM          bfd_cgc32_m68hc11_vec
#define TARGET_BIG_NAME		"cgc32-m68hc11"

#define cgc_info_to_howto	0
#define cgc_info_to_howto_rel	m68hc11_info_to_howto_rel
#define bfd_cgc32_bfd_relax_section  m68hc11_cgc_relax_section
#define cgc_backend_check_relocs     cgc32_m68hc11_check_relocs
#define cgc_backend_relocate_section cgc32_m68hc11_relocate_section
#define cgc_backend_add_symbol_hook  cgc32_m68hc11_add_symbol_hook
#define cgc_backend_object_p	0
#define cgc_backend_final_write_processing	0
#define cgc_backend_can_gc_sections		1
#define cgc_backend_special_sections  cgc32_m68hc11_special_sections
#define cgc_backend_merge_symbol_attribute cgc32_m68hc11_merge_symbol_attribute

#define bfd_cgc32_bfd_link_hash_table_create \
                                m68hc11_cgc_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free \
				m68hc11_cgc_bfd_link_hash_table_free
#define bfd_cgc32_bfd_merge_private_bfd_data \
					_bfd_m68hc11_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_set_private_flags	_bfd_m68hc11_cgc_set_private_flags
#define bfd_cgc32_bfd_print_private_bfd_data \
					_bfd_m68hc11_cgc_print_private_bfd_data
