/* IQ2000-specific support for 32-bit CGC.
...
static bfd_reloc_status_type iq2000_cgc_howto_hi16_reloc (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);


static reloc_howto_type iq2000_cgc_howto_table [] =
{
  ...
  HOWTO (R_IQ2000_HI16,		/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_dont, /* complain_on_overflow */
	 iq2000_cgc_howto_hi16_reloc,	/* special_function */
	 "R_IQ2000_HI16",	/* name */
	 FALSE,			/* partial_inplace */
	 0x0000,		/* src_mask */
	 0x7fff,		/* dst_mask */
...
static reloc_howto_type iq2000_cgc_vtinherit_howto =
...
static reloc_howto_type iq2000_cgc_vtentry_howto =
...
iq2000_cgc_howto_hi16_reloc (bfd *abfd ATTRIBUTE_UNUSED,
			     arelent *reloc_entry,
			     asymbol *symbol,
			     void * data,
			     ...
			     char **error_message ATTRIBUTE_UNUSED)
{
  ...
  ret = bfd_cgc_generic_reloc (abfd, reloc_entry, symbol, data,
...
static bfd_reloc_status_type
iq2000_cgc_relocate_hi16 (bfd *input_bfd,
...
static bfd_reloc_status_type
iq2000_cgc_relocate_offset16 (bfd *input_bfd,
...
/* Map BFD reloc types to IQ2000 CGC reloc types.  */

static reloc_howto_type *
iq2000_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			  bfd_reloc_code_real_type code)
{
  /* Note that the iq2000_cgc_howto_table is indxed by the R_
  ...
  switch (code)
    {
    case BFD_RELOC_NONE:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_NONE];
    case BFD_RELOC_16:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_16];
    case BFD_RELOC_32:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_32];
    case BFD_RELOC_MIPS_JMP:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_26];
    case BFD_RELOC_IQ2000_OFFSET_16:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_OFFSET_16];
    case BFD_RELOC_IQ2000_OFFSET_21:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_OFFSET_21];
    case BFD_RELOC_16_PCREL_S2:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_PC16];
    case BFD_RELOC_HI16:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_HI16];
    case BFD_RELOC_IQ2000_UHI16:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_UHI16];
    case BFD_RELOC_LO16:
      return &iq2000_cgc_howto_table[ (int) R_IQ2000_LO16];
    case BFD_RELOC_VTABLE_INHERIT:
      return &iq2000_cgc_vtinherit_howto;
    case BFD_RELOC_VTABLE_ENTRY:
      return &iq2000_cgc_vtentry_howto;
    default:
      /* Pacify gcc -Wall.  */
      return NULL;
    }
...
iq2000_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (iq2000_cgc_howto_table)
	    / sizeof (iq2000_cgc_howto_table[0]));
       i++)
    if (iq2000_cgc_howto_table[i].name != NULL
	&& strcasecmp (iq2000_cgc_howto_table[i].name, r_name) == 0)
      return &iq2000_cgc_howto_table[i];

  if (strcasecmp (iq2000_cgc_vtinherit_howto.name, r_name) == 0)
    return &iq2000_cgc_vtinherit_howto;
  if (strcasecmp (iq2000_cgc_vtentry_howto.name, r_name) == 0)
    return &iq2000_cgc_vtentry_howto;
...

/* Set the howto pointer for a IQ2000 CGC reloc.  */
			   ...
			   Cgc_Internal_Rela * dst)
{
  ...
  switch (r_type)
    ...
    case R_IQ2000_GNU_VTINHERIT:
      cache_ptr->howto = & iq2000_cgc_vtinherit_howto;
      break;

    case R_IQ2000_GNU_VTENTRY:
      cache_ptr->howto = & iq2000_cgc_vtentry_howto;
      break;

    default:
      cache_ptr->howto = & iq2000_cgc_howto_table [r_type];
      break;
    }
}
...
static bfd_boolean
iq2000_cgc_check_relocs (bfd *abfd,
			 ...
			 const Cgc_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_IQ2000_32:
	  ...
	  if (CONST_STRNEQ (sec->name, ".debug")
	    ...
	    {
	      ((Cgc_Internal_Rela *) rel)->r_info
		= CGC32_R_INFO (CGC32_R_SYM (rel->r_info), R_IQ2000_32_DEBUG);
	      changed = TRUE;
	    }
	  break;
	}
  ...
  if (changed)
    ...
    cgc_section_data (sec)->relocs = (Cgc_Internal_Rela *) relocs;

  return TRUE;
}


/* Relocate a IQ2000 CGC section.
...
static bfd_boolean
iq2000_cgc_relocate_section (bfd *		     output_bfd ATTRIBUTE_UNUSED,
			     struct bfd_link_info *  info,
			     bfd *		     input_bfd,
			     asection *		     input_section,
			     bfd_byte *		     contents,
			     ...
			     asection **	     local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = iq2000_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h	     = NULL;
      sym    = NULL;
      sec    = NULL;
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if ((sec->flags & SEC_MERGE)
	    ...
	    rel->r_addend = _bfd_cgc_rel_local_sym (output_bfd, sym, &sec,
						    rel->r_addend);
	...
	{
	case R_IQ2000_HI16:
	  r = iq2000_cgc_relocate_hi16 (input_bfd, rel, contents, relocation);
	  break;

	case R_IQ2000_OFFSET_16:
	  r = iq2000_cgc_relocate_offset16 (input_bfd, rel, contents, relocation,
...
static asection *
iq2000_cgc_gc_mark_hook (asection *sec,
			 struct bfd_link_info *info,
			 Cgc_Internal_Rela *rel,
			 struct cgc_link_hash_entry *h,
			 Cgc_Internal_Sym *sym)
...
static int
cgc32_iq2000_machine (bfd *abfd)
{
  switch (cgc_cgcheader (abfd)->e_flags & EF_IQ2000_CPU_MASK)
    {
...
static bfd_boolean
iq2000_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
iq2000_cgc_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
iq2000_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...

static bfd_boolean
iq2000_cgc_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
  flagword flags;
...
static
bfd_boolean
iq2000_cgc_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_iq2000,
			     cgc32_iq2000_machine (abfd));
  return TRUE;
}


#define CGC_ARCH		bfd_arch_iq2000
#define CGC_MACHINE_CODE	EM_IQ2000
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM		bfd_cgc32_iq2000_vec
#define TARGET_BIG_NAME		"cgc32-iq2000"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			iq2000_info_to_howto_rela
#define cgc_backend_relocate_section		iq2000_cgc_relocate_section
#define cgc_backend_gc_mark_hook		iq2000_cgc_gc_mark_hook
#define cgc_backend_check_relocs		iq2000_cgc_check_relocs
#define cgc_backend_object_p			iq2000_cgc_object_p
#define cgc_backend_rela_normal			1

#define cgc_backend_can_gc_sections		1

#define bfd_cgc32_bfd_reloc_type_lookup		iq2000_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	iq2000_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		iq2000_cgc_set_private_flags
#define bfd_cgc32_bfd_copy_private_bfd_data	iq2000_cgc_copy_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data	iq2000_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	iq2000_cgc_print_private_bfd_data
