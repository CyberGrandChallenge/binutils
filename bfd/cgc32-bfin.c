/* ADI Blackfin BFD support for 32-bit CGC.
...
bfd_boolean cgc32_bfin_code_in_l1 = 0;

/* Set by ld emulation if --data-in-l1.  */
bfd_boolean cgc32_bfin_data_in_l1 = 0;

static void
cgc32_bfin_final_write_processing (bfd *abfd,
				   bfd_boolean linker ATTRIBUTE_UNUSED)
{
  if (cgc32_bfin_code_in_l1)
    cgc_cgcheader (abfd)->e_flags |= EF_BFIN_CODE_IN_L1;
  if (cgc32_bfin_data_in_l1)
    cgc_cgcheader (abfd)->e_flags |= EF_BFIN_DATA_IN_L1;
}
...
bfin_is_local_label_name (bfd *abfd, const char *label)
{
  ...
  return _bfd_cgc_is_local_label_name (abfd, label);
}

...
cgc32_bfin_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...

static bfd_reloc_status_type
bfin_final_link_relocate (Cgc_Internal_Rela *rel, reloc_howto_type *howto,
  ...
  for (; rel < relend; rel++, i++)
	...
	case R_BFIN_GOT:
	  ...
	  if (dynobj == NULL)
	    {
	      /* Create the .got section.  */
	      cgc_hash_table (info)->dynobj = dynobj = output_bfd;
	      if (!_bfd_cgc_create_got_section (dynobj, info))
		return FALSE;
	    }
		...
		if ((off & 1) != 0)
		...
		else
		  {
			...
			outrel.r_info =
			  CGC32_R_INFO (0, R_BFIN_PCREL24);
			outrel.r_addend = relocation;
			loc = s->contents;
			loc +=
			  s->reloc_count++ * sizeof (Cgc32_External_Rela);
...
#define IS_FDPIC(bfd) ((bfd)->xvec == &bfd_cgc32_bfinfdpic_vec)

/* An extension of the cgc hash table data structure,
   containing some additional Blackfin-specific data.  */
struct bfinfdpic_cgc_link_hash_table
{
...
/* Get the Blackfin CGC linker hash table from a link_info structure.  */

#define bfinfdpic_hash_table(info) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((info)->hash)) \
  == BFIN_CGC_DATA ? ((struct bfinfdpic_cgc_link_hash_table *) ((info)->hash)) : NULL)
...
/* Create a Blackfin CGC linker hash table.  */

static struct bfd_link_hash_table *
bfinfdpic_cgc_link_hash_table_create (bfd *abfd)
{
  struct bfinfdpic_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct bfinfdpic_cgc_link_hash_table);
	     ...
	     && BFINFDPIC_SYM_LOCAL (info, entry->d.h)))
	{
	  ...
	  if (entry->symndx == -1
		...
		BFD_ASSERT (entry->privfd);
	      ...
	      idx = cgc_section_data (bfinfdpic_got_section (info)
				      ->output_section)->dynindx;
...

/* Relocate an Blackfin CGC section.
			    ...
			    asection ** local_sections)
{
  ...
  if (IS_FDPIC (output_bfd) && bfinfdpic_got_section (info))
    ...
    got_segment = -1;
  if (IS_FDPIC (output_bfd) && cgc_hash_table (info)->dynamic_sections_created)
	...
	case R_BFIN_FUNCDESC:
	  {
		...
		if (h && ! BFINFDPIC_FUNCDESC_LOCAL (info, h)
		...
		else
		  {
		    ...
		    dynindx = cgc_section_data (bfinfdpic_got_section (info)
						->output_section)->dynindx;
      ...
      if (check_segment[0] != check_segment[1] && IS_FDPIC (output_bfd))
	{
	 ...
	 in the ld testsuite.  */
	  ...
	  if (!silence_segment_error && (info->shared || info->pie))
	    return FALSE;
	  cgc_cgcheader (output_bfd)->e_flags |= EF_BFIN_PIC;
	}
			 ...
			 const Cgc_Internal_Rela *relocs)
{
  ...
  symtab_hdr = &cgc_tdata (abfd)->symtab_hdr;
  sym_hashes = cgc_sym_hashes (abfd);
  sym_hashes_end = sym_hashes + symtab_hdr->sh_size/sizeof(Cgc32_External_Sym);
  ...
  if (bed->want_got_sym)
    {
	 ...
	 a global offset table.  */
      h = _bfd_cgc_define_linkage_sym (abfd, info, s, "__GLOBAL_OFFSET_TABLE_");
	 ...
	 .plt section.  */
      ...
      if (! (_bfd_generic_link_add_one_symbol
	     (info, abfd, "__PROCEDURE_LINKAGE_TABLE_", BSF_GLOBAL, s, 0, NULL,
	      FALSE, get_cgc_backend_data (abfd)->collect, &bh)))
	return FALSE;
...
static bfd_boolean
cgc32_bfinfdpic_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
			 ...
			 struct _bfinfdpic_dynamic_got_plt_info *gpinfop)
{
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    bfinfdpic_pltrel_section (info)->size =
      gpinfop->g.lzplt / LZPLT_NORMAL_SIZE * get_cgc_backend_data (output_bfd)->s->sizeof_rel;
...
static bfd_boolean
cgc32_bfinfdpic_size_dynamic_sections (bfd *output_bfd,
				      struct bfd_link_info *info)
{
...
static bfd_boolean
cgc32_bfinfdpic_always_size_sections (bfd *output_bfd,
				     struct bfd_link_info *info)
{
  if (!info->relocatable
  ...
  for (erel = rel + sec->reloc_count; rel < erel; rel++)
    {
      ...
      if (CGC32_R_TYPE (rel->r_info) != R_BFIN_BYTE4_DATA
	  && CGC32_R_TYPE (rel->r_info) != R_BFIN_FUNCDESC)
	continue;
	...
	return FALSE;
      ...
      *changed = TRUE;
      ...
      if (CGC32_R_TYPE (rel->r_info) == R_BFIN_BYTE4_DATA)
	picrel->relocs32--;
      else /* we know (CGC32_R_TYPE (rel->r_info) == R_BFIN_FUNCDESC) */
	picrel->relocsfd--;
...
static bfd_boolean
bfinfdpic_cgc_discard_info (bfd *ibfd,
...
static bfd_boolean
cgc32_bfinfdpic_finish_dynamic_sections (bfd *output_bfd,
					struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
...
static bfd_boolean
cgc32_bfinfdpic_adjust_dynamic_symbol (struct bfd_link_info *info,
				       struct cgc_link_hash_entry *h)
{
  bfd * dynobj;
...
static bfd_boolean
cgc32_bfinfdpic_finish_dynamic_symbol
(bfd *output_bfd ATTRIBUTE_UNUSED,
 struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
bfinfdpic_cgc_use_relative_eh_frame
...
static bfd_byte
bfinfdpic_cgc_encode_eh_address (bfd *abfd,
				struct bfd_link_info *info,
...
/* Set the right machine number for a Blackfin CGC file.  */

static bfd_boolean
cgc32_bfin_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_bfin, 0);
  return (((cgc_cgcheader (abfd)->e_flags & EF_BFIN_FDPIC) != 0)
	  == (IS_FDPIC (abfd)));
}

static bfd_boolean
cgc32_bfin_set_private_flags (bfd * abfd, flagword flags)
{
...
static bfd_boolean
bfin_cgc_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
cgc32_bfinfdpic_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  if (bfd_get_flavour (ibfd) != bfd_target_cgc_flavour
      || bfd_get_flavour (obfd) != bfd_target_cgc_flavour)
    return TRUE;

  if (! bfin_cgc_copy_private_bfd_data (ibfd, obfd))
    return FALSE;
...
static bfd_boolean
cgc32_bfin_print_private_bfd_data (bfd * abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
  flagword flags;
...
static bfd_boolean
cgc32_bfin_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
/* Create an bfin CGC linker hash table.  */
  ...
  if (h->got.offset != (bfd_vma) - 1)
	      ...
	      || h->dynindx == -1 || h->forced_local) && h->def_regular)
	{
	  (*_bfd_error_handler) (_("*** check this relocation %s"),
				 __FUNCTION__);
	  rela.r_info = CGC32_R_INFO (0, R_BFIN_PCREL24);
      ...
      else
	{
	  ...
	  rela.r_info = CGC32_R_INFO (h->dynindx, R_BFIN_GOT);
	  rela.r_addend = 0;
	}
			    ...
			    struct cgc_link_hash_entry *h)
{
  ...
  /* We need to figure out the alignment required for this symbol.  I
     have no idea how CGC linkers handle this.  */
...
static bfd_boolean
bfin_discard_copies (struct cgc_link_hash_entry *h, void * inf)
{
...
bfd_boolean
bfd_bfin_cgc32_create_embedded_relocs (bfd *abfd,
				       struct bfd_link_info *info,
				       asection *datasec,
				       asection *relsec,
				       char **errmsg)
  ...
  for (irel = internal_relocs; irel < irelend; irel++, p += 12)
    {
      ...
      if (CGC32_R_TYPE (irel->r_info) != (int) R_BFIN_BYTE4_DATA)
	{
...
struct bfd_cgc_special_section const cgc32_bfin_special_sections[] =
{
...

#define TARGET_LITTLE_SYM		bfd_cgc32_bfin_vec
#define TARGET_LITTLE_NAME		"cgc32-bfin"
#define CGC_ARCH			bfd_arch_bfin
#define CGC_TARGET_ID			BFIN_CGC_DATA
#define CGC_MACHINE_CODE		EM_BLACKFIN
#define CGC_MAXPAGESIZE			0x1000
#define cgc_symbol_leading_char		'_'

#define bfd_cgc32_bfd_reloc_type_lookup	bfin_bfd_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup \
					bfin_bfd_reloc_name_lookup
#define cgc_info_to_howto		bfin_info_to_howto
#define cgc_info_to_howto_rel		0
#define cgc_backend_object_p		cgc32_bfin_object_p
...
#define bfd_cgc32_bfd_final_link        bfd_cgc_gc_common_final_link

#define cgc_backend_check_relocs        bfin_check_relocs
...
#define cgc_backend_relocate_section    bfin_relocate_section
...
#define cgc_backend_gc_mark_hook        bfin_gc_mark_hook
#define cgc_backend_gc_sweep_hook       bfin_gc_sweep_hook
#define bfd_cgc32_bfd_merge_private_bfd_data \
                                        cgc32_bfin_merge_private_bfd_data
#define bfd_cgc32_bfd_set_private_flags \
                                        cgc32_bfin_set_private_flags
#define bfd_cgc32_bfd_print_private_bfd_data \
                                        cgc32_bfin_print_private_bfd_data
#define cgc_backend_final_write_processing \
                                        cgc32_bfin_final_write_processing
#define cgc_backend_reloc_type_class    cgc32_bfin_reloc_type_class
#define cgc_backend_stack_align		8
#define cgc_backend_can_gc_sections 1
#define cgc_backend_special_sections	cgc32_bfin_special_sections
...
#define TARGET_LITTLE_SYM          bfd_cgc32_bfinfdpic_vec
#undef TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME		"cgc32-bfinfdpic"
#undef	cgc32_bed
#define	cgc32_bed		cgc32_bfinfdpic_bed

#undef cgc_backend_gc_sweep_hook
#define cgc_backend_gc_sweep_hook       bfinfdpic_gc_sweep_hook

#undef cgc_backend_got_header_size
#define cgc_backend_got_header_size     0

#undef cgc_backend_relocate_section
#define cgc_backend_relocate_section    bfinfdpic_relocate_section
#undef cgc_backend_check_relocs
#define cgc_backend_check_relocs        bfinfdpic_check_relocs

#undef bfd_cgc32_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_create \
		bfinfdpic_cgc_link_hash_table_create
#undef cgc_backend_always_size_sections
#define cgc_backend_always_size_sections \
		cgc32_bfinfdpic_always_size_sections
#undef bfd_cgc32_bfd_copy_private_bfd_data
#define bfd_cgc32_bfd_copy_private_bfd_data \
		cgc32_bfinfdpic_copy_private_bfd_data

#undef cgc_backend_create_dynamic_sections
#define cgc_backend_create_dynamic_sections \
		cgc32_bfinfdpic_create_dynamic_sections
#undef cgc_backend_adjust_dynamic_symbol
#define cgc_backend_adjust_dynamic_symbol \
		cgc32_bfinfdpic_adjust_dynamic_symbol
#undef cgc_backend_size_dynamic_sections
#define cgc_backend_size_dynamic_sections \
		cgc32_bfinfdpic_size_dynamic_sections
#undef cgc_backend_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_symbol \
		cgc32_bfinfdpic_finish_dynamic_symbol
#undef cgc_backend_finish_dynamic_sections
#define cgc_backend_finish_dynamic_sections \
		cgc32_bfinfdpic_finish_dynamic_sections

#undef cgc_backend_discard_info
#define cgc_backend_discard_info \
		bfinfdpic_cgc_discard_info
...
#define cgc_backend_encode_eh_address \
		bfinfdpic_cgc_encode_eh_address
...
#define cgc_backend_omit_section_dynsym _bfinfdpic_link_omit_section_dynsym
