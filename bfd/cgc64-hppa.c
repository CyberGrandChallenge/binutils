...
#define CGC_DYNAMIC_INTERPRETER "/usr/lib/pa20_64/dld.sl"
...
struct cgc64_hppa_link_hash_entry
{
  ...
  struct cgc64_hppa_dyn_reloc_entry
  {
    /* Next relocation in the chain.  */
    struct cgc64_hppa_dyn_reloc_entry *next;
...
struct cgc64_hppa_link_hash_table
{
...
#define hppa_link_hash_table(p) \
  ...
  == HPPA64_CGC_DATA ? ((struct cgc64_hppa_link_hash_table *) ((p)->hash)) : NULL)

#define hppa_cgc_hash_entry(ent) \
  ((struct cgc64_hppa_link_hash_entry *)(ent))
...
static struct bfd_link_hash_table *cgc64_hppa_hash_table_create
  (bfd *abfd);
...
static bfd_boolean cgc64_hppa_object_p
  (bfd *);

static void cgc64_hppa_post_process_headers
  (bfd *, struct bfd_link_info *);

static bfd_boolean cgc64_hppa_create_dynamic_sections
  (bfd *, struct bfd_link_info *);

static bfd_boolean cgc64_hppa_adjust_dynamic_symbol
  (struct bfd_link_info *, struct cgc_link_hash_entry *);

static bfd_boolean cgc64_hppa_mark_milli_and_exported_functions
  (struct cgc_link_hash_entry *, void *);

static bfd_boolean cgc64_hppa_size_dynamic_sections
  (bfd *, struct bfd_link_info *);

static int cgc64_hppa_link_output_symbol_hook
  (struct bfd_link_info *, const char *, Cgc_Internal_Sym *,
   asection *, struct cgc_link_hash_entry *);

static bfd_boolean cgc64_hppa_finish_dynamic_symbol
  (bfd *, struct bfd_link_info *,
   struct cgc_link_hash_entry *, Cgc_Internal_Sym *);

static bfd_boolean cgc64_hppa_finish_dynamic_sections
  (bfd *, struct bfd_link_info *);

static bfd_boolean cgc64_hppa_check_relocs
  (bfd *, struct bfd_link_info *,
   asection *, const Cgc_Internal_Rela *);

static bfd_boolean cgc64_hppa_dynamic_symbol_p
  (struct cgc_link_hash_entry *, struct bfd_link_info *);

static bfd_boolean cgc64_hppa_mark_exported_functions
  (struct cgc_link_hash_entry *, void *);

static bfd_boolean cgc64_hppa_finalize_opd
  (struct cgc_link_hash_entry *, void *);

static bfd_boolean cgc64_hppa_finalize_dlt
  (struct cgc_link_hash_entry *, void *);
...
static bfd_boolean get_reloc_section
  (bfd *, struct cgc64_hppa_link_hash_table *, asection *);

static bfd_boolean count_dyn_reloc
  (bfd *, struct cgc64_hppa_link_hash_entry *,
   int, asection *, int, bfd_vma, bfd_vma);

static bfd_boolean allocate_dynrel_entries
  (struct cgc_link_hash_entry *, void *);

static bfd_boolean cgc64_hppa_finalize_dynreloc
  (struct cgc_link_hash_entry *, void *);

static bfd_boolean get_opd
  (bfd *, struct bfd_link_info *, struct cgc64_hppa_link_hash_table *);
...
static int cgc64_hppa_cgc_get_symbol_type
			  ...
			  const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc64_hppa_link_hash_entry));
      if (entry == NULL)
        return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc64_hppa_link_hash_entry *hh;

      /* Initialize our local data.  All zeros.  */
      hh = hppa_cgc_hash_entry (entry);
      memset (&hh->dlt_offset, 0,
	      (sizeof (struct cgc64_hppa_link_hash_entry)
	       - offsetof (struct cgc64_hppa_link_hash_entry, dlt_offset)));
    }

  return entry;
}

/* Create the derived linker hash table.  The PA64 CGC port uses this
   derived hash table to keep information specific to the PA ElF
   linker (without using static variables).  */

static struct bfd_link_hash_table*
cgc64_hppa_hash_table_create (bfd *abfd)
{
  struct cgc64_hppa_link_hash_table *htab;
  ...
  if (!_bfd_cgc_link_hash_table_init (&htab->root, abfd,
...

/* Return nonzero if ABFD represents a PA2.0 CGC64 file.

   Additionally we set the default architecture and machine.  */
static bfd_boolean
cgc64_hppa_object_p (bfd *abfd)
{
  Cgc_Internal_Ehdr * i_ehdrp;
  unsigned int flags;

  i_ehdrp = cgc_cgcheader (abfd);
  if (strcmp (bfd_get_target (abfd), "cgc64-hppa-linux") == 0)
    {
      /* GCC on hppa-linux produces binaries with OSABI=GNU,
	 but the kernel produces corefiles with OSABI=SysV.  */
      if (i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_GNU
	  && i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_NONE) /* aka SYSV */
	return FALSE;
    }
  else
    {
	 ...
	 but the kernel produces corefiles with OSABI=SysV.  */
      if (i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_HPUX
	  && i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_NONE) /* aka SYSV */
	return FALSE;
    }
  ...
  switch (flags & (EF_PARISC_ARCH | EF_PARISC_WIDE))
    ...
    case EFA_PARISC_2_0:
      if (i_ehdrp->e_ident[EI_CLASS] == CGCCLASS64)
...
/* Given section type (hdr->sh_type), return a boolean indicating
   whether or not the section is an cgc64-hppa specific section.  */
static bfd_boolean
cgc64_hppa_section_from_shdr (bfd *abfd,
...
get_reloc_section (bfd *abfd,
		   struct cgc64_hppa_link_hash_table *hppa_info,
		   asection *sec)
{
...
count_dyn_reloc (bfd *abfd,
		 ...
		 bfd_vma addend)
{
  struct cgc64_hppa_dyn_reloc_entry *rent;

  rent = (struct cgc64_hppa_dyn_reloc_entry *)
...
static bfd_boolean
cgc64_hppa_check_relocs (bfd *abfd,
			 ...
			 const Cgc_Internal_Rela *relocs)
{
  ...
  if (info->shared && hppa_info->section_syms_bfd != abfd)
	...
	{
	  local_syms = (Cgc_Internal_Sym *) symtab_hdr->contents;
	  if (local_syms == NULL)
	    local_syms = bfd_cgc_get_cgc_syms (abfd, symtab_hdr,
  ...
  if (info->shared)
    {
      sec_symndx = _bfd_cgc_section_from_bfd_section (abfd, sec);
  ...
  for (rel = relocs; rel < relend; ++rel)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
	  ...
	  hh = hppa_cgc_hash_entry (cgc_sym_hashes (abfd)[indx]);
	  while (hh->eh.root.type == bfd_link_hash_indirect
		 || hh->eh.root.type == bfd_link_hash_warning)
	    hh = hppa_cgc_hash_entry (hh->eh.root.u.i.link);
	...
	maybe_dynamic = TRUE;
      ...
      howto = cgc_hppa_howto_table + CGC64_R_TYPE (rel->r_info);
      need_entry = 0;
      switch (howto->type)
	{
      ...
      if (need_entry & NEED_DLT)
	{
	  ...
	  if (hh != NULL)
	  ...
	  else
	    {
	      ...
	      local_dlt_refcounts = hppa64_cgc_local_refcounts (abfd);
      ...
      if ((need_entry & NEED_DYNREL) && (sec->flags & SEC_ALLOC))
	{
	  ...
	  if (info->shared && dynrel_type == R_PARISC_FPTR64
	      && ! (bfd_cgc_link_record_local_dynamic_symbol
...
struct cgc64_hppa_allocate_data
{
  struct bfd_link_info *info;
  bfd_size_type ofs;
};
...
static bfd_boolean
cgc64_hppa_mark_exported_functions (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
...
static bfd_boolean
allocate_global_data_dlt (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct cgc64_hppa_allocate_data *x = (struct cgc64_hppa_allocate_data *)data;
...
static bfd_boolean
allocate_global_data_plt (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct cgc64_hppa_allocate_data *x = (struct cgc64_hppa_allocate_data *) data;

  if (hh->want_plt
      && cgc64_hppa_dynamic_symbol_p (eh, x->info)
      && !((eh->root.type == bfd_link_hash_defined
	    || eh->root.type == bfd_link_hash_defweak)
	   && eh->root.u.def.section->output_section != NULL))
    {
...
static bfd_boolean
allocate_global_data_stub (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct cgc64_hppa_allocate_data *x = (struct cgc64_hppa_allocate_data *)data;
...
static bfd_boolean
allocate_global_data_opd (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct cgc64_hppa_allocate_data *x = (struct cgc64_hppa_allocate_data *)data;
	  ...
	  if (x->info->shared && eh)
	    {
	      ...
	      if (! bfd_cgc_link_record_dynamic_symbol (x->info, nh))
		return FALSE;
...
static void
cgc64_hppa_post_process_headers (bfd *abfd,
...
static bfd_boolean
cgc64_hppa_create_dynamic_sections (bfd *abfd,
				    struct bfd_link_info *info)
{
  asection *s;
  struct cgc64_hppa_link_hash_table *hppa_info;
...
static bfd_boolean
allocate_dynrel_entries (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct cgc64_hppa_allocate_data *x = (struct cgc64_hppa_allocate_data *)data;
  struct cgc64_hppa_link_hash_table *hppa_info;
  ...
  dynamic_symbol = cgc64_hppa_dynamic_symbol_p (eh, x->info);
  ...
  for (rent = hh->reloc_entries; rent; rent = rent->next)
	...
	continue;
      ...
      hppa_info->other_rel_sec->size += sizeof (Cgc64_External_Rela);
  ...
  if ((dynamic_symbol || shared) && hh->want_dlt)
    hppa_info->dlt_rel_sec->size += sizeof (Cgc64_External_Rela);
  ...
  if (shared && hh->want_opd)
    hppa_info->opd_rel_sec->size += sizeof (Cgc64_External_Rela);
  ...
  if (hh->want_plt && dynamic_symbol)
    {
	 ...
	 main applications get nothing.  */
      if (dynamic_symbol)
	t = sizeof (Cgc64_External_Rela);
      else if (shared)
	t = 2 * sizeof (Cgc64_External_Rela);
...
static bfd_boolean
cgc64_hppa_adjust_dynamic_symbol (struct bfd_link_info *info ATTRIBUTE_UNUSED,
				  struct cgc_link_hash_entry *eh)
{
...
/* This function is called via cgc_link_hash_traverse to mark millicode
   ...
   cgc64_hppa_mark_exported_functions is called.  */

static bfd_boolean
cgc64_hppa_mark_milli_and_exported_functions (struct cgc_link_hash_entry *eh,
					      void *data)
{
  struct bfd_link_info *info = (struct bfd_link_info *) data;
  ...
  return cgc64_hppa_mark_exported_functions (eh, data);
}
...
static bfd_boolean
cgc64_hppa_size_dynamic_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  struct cgc64_hppa_link_hash_table *hppa_info;
  struct cgc64_hppa_allocate_data data;
  bfd *dynobj;
  bfd *ibfd;
  asection *sec;
  bfd_boolean plt;
  ...
  cgc_link_hash_traverse (cgc_hash_table (info),
			  (cgc_hash_table (info)->dynamic_sections_created
			   ? cgc64_hppa_mark_milli_and_exported_functions
			   : cgc64_hppa_mark_exported_functions),
			  info);
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (info->executable)
	{
	  ...
	  BFD_ASSERT (sec != NULL);
	  sec->size = sizeof CGC_DYNAMIC_INTERPRETER;
	  sec->contents = (unsigned char *) CGC_DYNAMIC_INTERPRETER;
	}
    }
  else
    {
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	...
	{
	  struct cgc64_hppa_dyn_reloc_entry *hdh_p;

	  for (hdh_p = ((struct cgc64_hppa_dyn_reloc_entry *)
		    cgc_section_data (sec)->local_dynrel);
	       hdh_p != NULL;
	       hdh_p = hdh_p->next)
	    {
		...
		{
		  srel = cgc_section_data (hdh_p->sec)->sreloc;
		  srel->size += hdh_p->count * sizeof (Cgc64_External_Rela);
      ...
      local_dlt = cgc_local_got_refcounts (ibfd);
      if (!local_dlt)
	continue;
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
	...
	return FALSE;
	 ...
	 values later, in cgc64_hppa_finish_dynamic_sections, but we
...
static int
cgc64_hppa_link_output_symbol_hook (struct bfd_link_info *info ATTRIBUTE_UNUSED,
				    const char *name,
...
static bfd_boolean
cgc64_hppa_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *eh,
				  Cgc_Internal_Sym *sym)
{
  ...
  /* Incredible.  It is actually necessary to NOT use the symbol's real
     ...
     the original values (in cgc64_hppa_link_output_symbol_hook).  */
  if (hh->want_opd)
    {
  ...
  if (hh->want_plt
      && cgc64_hppa_dynamic_symbol_p (eh, info))
    {
      bfd_vma value;
	...
	value = (eh->root.u.def.value + eh->root.u.def.section->vma);
	 ...
	 address.  */
      ...
      rel.r_info = CGC64_R_INFO (hh->eh.dynindx, R_PARISC_IPLT);
      rel.r_addend = 0;

      loc = spltrel->contents;
      loc += spltrel->reloc_count++ * sizeof (Cgc64_External_Rela);
      bfd_cgc64_swap_reloca_out (splt->output_section->owner, &rel, loc);
    }
...
static bfd_boolean
cgc64_hppa_finalize_opd (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct bfd_link_info *info = (struct bfd_link_info *)data;
  struct cgc64_hppa_link_hash_table *hppa_info;
  ...
  if (info->shared && hh->want_opd)
	...
	dynindx
	  = _bfd_cgc_link_lookup_local_dynindx (info, hh->owner,
						hh->sym_indx);
	 ...
	 handling in cgc64_hppa_finalize_dynreloc.  */
      if (eh)
	{
	  char *new_name;
	  struct cgc_link_hash_entry *nh;
	  ...
	  nh = cgc_link_hash_lookup (cgc_hash_table (info),
      ...
      rel.r_addend = 0;
      rel.r_info = CGC64_R_INFO (dynindx, R_PARISC_EPLT);

      loc = sopdrel->contents;
      loc += sopdrel->reloc_count++ * sizeof (Cgc64_External_Rela);
      bfd_cgc64_swap_reloca_out (sopd->output_section->owner, &rel, loc);
    }
  return TRUE;
}
...
static bfd_boolean
cgc64_hppa_finalize_dlt (struct cgc_link_hash_entry *eh, void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct bfd_link_info *info = (struct bfd_link_info *)data;
  struct cgc64_hppa_link_hash_table *hppa_info;
  ...
  if (hh->want_dlt
      && (cgc64_hppa_dynamic_symbol_p (eh, info) || info->shared))
    {
      Cgc_Internal_Rela rel;
      bfd_byte *loc;
      int dynindx;
	...
	dynindx
	 ...
	 resulting object file.  */
	  ...
	  rel.r_info = CGC64_R_INFO (dynindx, R_PARISC_FPTR64);
      else
	  rel.r_info = CGC64_R_INFO (dynindx, R_PARISC_DIR64);
      rel.r_addend = 0;

      loc = sdltrel->contents;
      loc += sdltrel->reloc_count++ * sizeof (Cgc64_External_Rela);
      bfd_cgc64_swap_reloca_out (sdlt->output_section->owner, &rel, loc);
    }
  return TRUE;
}
...
static bfd_boolean
cgc64_hppa_finalize_dynreloc (struct cgc_link_hash_entry *eh,
			      void *data)
{
  struct cgc64_hppa_link_hash_entry *hh = hppa_cgc_hash_entry (eh);
  struct bfd_link_info *info = (struct bfd_link_info *)data;
  ...
  dynamic_symbol = cgc64_hppa_dynamic_symbol_p (eh, info);
  ...
  if (hh->reloc_entries)
      ...
      for (rent = hh->reloc_entries; rent; rent = rent->next)
	{
	  ...
	  if (info->shared && rent->type == R_PARISC_FPTR64 && hh->want_opd)
	    {
		 ...
		 relocation.  */
	      dynindx
		= _bfd_cgc_link_lookup_local_dynindx (info,
						      rent->sec->owner,
						      rent->sec_symndx);
	    }
	  else
	  ...
	  rel.r_info = CGC64_R_INFO (dynindx, rent->type);

	  loc = hppa_info->other_rel_sec->contents;
	  loc += (hppa_info->other_rel_sec->reloc_count++
		  * sizeof (Cgc64_External_Rela));
	  bfd_cgc64_swap_reloca_out (hppa_info->other_rel_sec->output_section->owner,
				     &rel, loc);
	}
    }
...
cgc64_hppa_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
			     const asection *rel_sec ATTRIBUTE_UNUSED,
			     const Cgc_Internal_Rela *rela)
{
  if (CGC64_R_SYM (rela->r_info) == STN_UNDEF)
...
static bfd_boolean
cgc64_hppa_finish_dynamic_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  cgc_link_hash_traverse (cgc_hash_table (info),
			  cgc64_hppa_finalize_opd,
			  info);

  cgc_link_hash_traverse (cgc_hash_table (info),
			  cgc64_hppa_finalize_dynreloc,
			  info);
...
static bfd_boolean
cgc64_hppa_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  size_t size;
...
static bfd_boolean
cgc64_hppa_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  char * command;
  int n;
  ...
  switch (note->descsz)
    ...
    case 136:		/* Linux/hppa cgc_prpsinfo.  */
  ...
  command = cgc_tdata (abfd)->core->command;
  n = strlen (command);
...
/* Return the number of additional phdrs we will need.
   ...
   The generic CGC code only creates PT_PHDRs for executables.  The HP
...
static int
cgc64_hppa_additional_program_headers (bfd *abfd,
				struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  asection *s;
...
static bfd_boolean
cgc64_hppa_modify_segment_map (bfd *abfd,
			       struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
...
/* Called when writing out an object file to decide the type of a
   symbol.  */
static int
cgc64_hppa_cgc_get_symbol_type (Cgc_Internal_Sym *cgc_sym,
				int type)
{
  if (CGC_ST_TYPE (cgc_sym->st_info) == STT_PARISC_MILLI)
...
static bfd_boolean
cgc64_hppa_section_from_phdr (bfd *abfd, Cgc_Internal_Phdr *hdr, int sec_index,
			      const char *typename)
{
  if (hdr->p_type == PT_HP_CORE_KERNEL)
    {
      ...
      if (!_bfd_cgc_make_section_from_phdr (abfd, hdr, sec_index, typename))
	return FALSE;
  ...
  if (hdr->p_type == PT_HP_CORE_PROC)
	...
	return FALSE;
      ...
      return _bfd_cgccore_make_pseudosection (abfd, ".reg", hdr->p_filesz,
					      hdr->p_offset);
    }
  ...
  return _bfd_cgc_make_section_from_phdr (abfd, hdr, sec_index, typename);
}
...
static bfd_boolean
cgc_hppa_add_symbol_hook (bfd *abfd,
			  struct bfd_link_info *info ATTRIBUTE_UNUSED,
			  Cgc_Internal_Sym *sym,
...
static bfd_boolean
cgc_hppa_unmark_useless_dynamic_symbols (struct cgc_link_hash_entry *h,
					 void *data)
{
  ...
  /* If we are not creating a shared library, and this symbol is
     ...
     Ultimately we should have better controls over the generic CGC BFD
     linker code.  */
  if (! info->relocatable
      && info->unresolved_syms_in_shared_libs != RM_IGNORE
...
static bfd_boolean
cgc_hppa_remark_useless_dynamic_symbols (struct cgc_link_hash_entry *h,
					 void *data)
{
  struct bfd_link_info *info = data;
...
static bfd_boolean
cgc_hppa_is_dynamic_loader_symbol (const char *name)
{
...
static void
cgc_hppa_record_segment_addrs (bfd *abfd,
			       asection *section,
			       void *data)
{
  struct cgc64_hppa_link_hash_table *hppa_info = data;
  ...
  if ((section->flags & (SEC_ALLOC | SEC_LOAD)) == (SEC_ALLOC | SEC_LOAD))
    {
      ...
      p = _bfd_cgc_find_segment_containing_section (abfd, section->output_section);
      BFD_ASSERT (p != NULL);
      value = p->p_vaddr;
...
/* Called after we have seen all the input files/sections, but before
   ...
   fall back to the generic CGC final link routine.  */

static bfd_boolean
cgc_hppa_final_link (bfd *abfd, struct bfd_link_info *info)
{
  bfd_boolean retval;
  struct cgc64_hppa_link_hash_table *hppa_info = hppa_link_hash_table (info);
  ...
  /* HP's shared libraries have references to symbols that are not
     defined anywhere.  The generic CGC BFD linker code will complain
     about such symbols.
     ...
     CGC BFD link code happy and appears to not create any secondary
     problems.  Ultimately we need a way to control the behavior of the
     generic CGC BFD link code better.  */
  cgc_link_hash_traverse (cgc_hash_table (info),
			  cgc_hppa_unmark_useless_dynamic_symbols,
			  info);

  /* Invoke the regular CGC backend linker to do all the work.  */
  retval = bfd_cgc_final_link (abfd, info);

  cgc_link_hash_traverse (cgc_hash_table (info),
			  cgc_hppa_remark_useless_dynamic_symbols,
			  info);

  /* If we're producing a final executable, sort the contents of the
     unwind section. */
  if (retval && !info->relocatable)
    retval = cgc_hppa_sort_unwind (abfd);
...
static int
cgc_hppa_relocate_insn (int insn, int sym_value, unsigned int r_type)
{
  switch (r_type)
    {
...
cgc_hppa_final_link_relocate (Cgc_Internal_Rela *rel,
			      bfd *input_bfd,
			      bfd *output_bfd,
			      ...
			      struct cgc_link_hash_entry *eh)
{
  ...
  reloc_howto_type *howto = cgc_hppa_howto_table + CGC_R_TYPE (rel->r_info);
  unsigned int r_symndx = CGC_R_SYM (rel->r_info);
  ...
  local_offsets = cgc_local_got_offsets (input_bfd);
  ...
  switch (r_type)
	...
	insn = cgc_hppa_relocate_insn (insn, (int) value, r_type);
	break;
      }
	...
	if (hppa_info->text_segment_base == (bfd_vma) -1)
	  bfd_map_over_sections (output_bfd, cgc_hppa_record_segment_addrs,
				 hppa_info);
...
/* Relocate an HPPA CGC section.  */

static bfd_boolean
cgc64_hppa_relocate_section (bfd *output_bfd,
			   struct bfd_link_info *info,
			   bfd *input_bfd,
			   asection *input_section,
			   bfd_byte *contents,
			   ...
			   asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  relocation = _bfd_cgc_rela_local_sym (output_bfd, sym, &sym_sec, rel);
	}
      else
	{
	  ...
	  while (eh->root.type == bfd_link_hash_indirect
	    ...
	    eh = (struct cgc_link_hash_entry *) eh->root.u.i.link;
	  ...
	  if (eh->root.type == bfd_link_hash_defined
	    ...
	    ;
		   ...
		   && CGC_ST_VISIBILITY (eh->other) == STV_DEFAULT)
	    ;
	  else if (!info->relocatable
		   && cgc_hppa_is_dynamic_loader_symbol (eh->root.root.string))
	    continue;
	  else if (!info->relocatable)
	    {
	      bfd_boolean err;
	...
	continue;
      ...
      r = cgc_hppa_final_link_relocate (rel, input_bfd, output_bfd,
      ...
      if (r != bfd_reloc_ok)
	{
	  switch (r)
		...
		if (eh != NULL)
		  ...
		  {
		    sym_name = bfd_cgc_string_from_cgc_section (input_bfd,
								symtab_hdr->sh_link,
								sym->st_name);
		    if (sym_name == NULL)
		      return FALSE;
...
static const struct bfd_cgc_special_section cgc64_hppa_special_sections[] =
{
...
const struct cgc_size_info hppa64_cgc_size_info =
{
...
#define TARGET_BIG_SYM			bfd_cgc64_hppa_vec
#define TARGET_BIG_NAME			"cgc64-hppa"
#define CGC_ARCH			bfd_arch_hppa
#define CGC_TARGET_ID			HPPA64_CGC_DATA
#define CGC_MACHINE_CODE		EM_PARISC
...
#define bfd_cgc64_bfd_reloc_type_lookup cgc_hppa_reloc_type_lookup
#define bfd_cgc64_bfd_reloc_name_lookup cgc_hppa_reloc_name_lookup
#define bfd_cgc64_bfd_is_local_label_name       cgc_hppa_is_local_label_name
#define cgc_info_to_howto		cgc_hppa_info_to_howto
#define cgc_info_to_howto_rel		cgc_hppa_info_to_howto_rel

#define cgc_backend_section_from_shdr	cgc64_hppa_section_from_shdr
#define cgc_backend_object_p		cgc64_hppa_object_p
#define cgc_backend_final_write_processing \
					cgc_hppa_final_write_processing
#define cgc_backend_fake_sections	cgc_hppa_fake_sections
#define cgc_backend_add_symbol_hook	cgc_hppa_add_symbol_hook

#define cgc_backend_relocate_section	cgc_hppa_relocate_section

#define bfd_cgc64_bfd_final_link	cgc_hppa_final_link

#define cgc_backend_create_dynamic_sections \
					cgc64_hppa_create_dynamic_sections
#define cgc_backend_post_process_headers	cgc64_hppa_post_process_headers

#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_adjust_dynamic_symbol \
					cgc64_hppa_adjust_dynamic_symbol

#define cgc_backend_size_dynamic_sections \
					cgc64_hppa_size_dynamic_sections

#define cgc_backend_finish_dynamic_symbol \
					cgc64_hppa_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
					cgc64_hppa_finish_dynamic_sections
#define cgc_backend_grok_prstatus	cgc64_hppa_grok_prstatus
#define cgc_backend_grok_psinfo		cgc64_hppa_grok_psinfo

/* Stuff for the BFD linker: */
#define bfd_cgc64_bfd_link_hash_table_create \
	cgc64_hppa_hash_table_create

#define cgc_backend_check_relocs \
	cgc64_hppa_check_relocs

#define cgc_backend_size_info \
  hppa64_cgc_size_info

#define cgc_backend_additional_program_headers \
	cgc64_hppa_additional_program_headers

#define cgc_backend_modify_segment_map \
	cgc64_hppa_modify_segment_map

#define cgc_backend_link_output_symbol_hook \
	cgc64_hppa_link_output_symbol_hook
...
#define cgc_backend_get_symbol_type	cgc64_hppa_cgc_get_symbol_type
#define cgc_backend_reloc_type_class	cgc64_hppa_reloc_type_class
#define cgc_backend_rela_normal		1
#define cgc_backend_special_sections	cgc64_hppa_special_sections
#define cgc_backend_action_discarded	cgc_hppa_action_discarded
#define cgc_backend_section_from_phdr   cgc64_hppa_section_from_phdr

#define cgc64_bed			cgc64_hppa_hpux_bed

#include "cgc64-target.h"

#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM			bfd_cgc64_hppa_linux_vec
#undef TARGET_BIG_NAME
#define TARGET_BIG_NAME			"cgc64-hppa-linux"
#undef CGC_OSABI
#define CGC_OSABI			CGCOSABI_GNU
#undef cgc_backend_post_process_headers
#define cgc_backend_post_process_headers _bfd_cgc_set_osabi
#undef cgc64_bed
#define cgc64_bed			cgc64_hppa_linux_bed
