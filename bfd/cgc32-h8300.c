/* BFD back-end for Renesas H8/300 CGC binaries.
...
static reloc_howto_type *cgc32_h8_reloc_type_lookup
  (bfd *abfd, bfd_reloc_code_real_type code);
static void cgc32_h8_info_to_howto
  (bfd *, arelent *, Cgc_Internal_Rela *);
static void cgc32_h8_info_to_howto_rel
  (bfd *, arelent *, Cgc_Internal_Rela *);
static unsigned long cgc32_h8_mach (flagword);
static void cgc32_h8_final_write_processing (bfd *, bfd_boolean);
static bfd_boolean cgc32_h8_object_p (bfd *);
static bfd_boolean cgc32_h8_merge_private_bfd_data (bfd *, bfd *);
static bfd_boolean cgc32_h8_relax_section
  (bfd *, asection *, struct bfd_link_info *, bfd_boolean *);
static bfd_boolean cgc32_h8_relax_delete_bytes
  (bfd *, asection *, bfd_vma, int);
static bfd_boolean cgc32_h8_symbol_address_p (bfd *, asection *, bfd_vma);
static bfd_byte *cgc32_h8_get_relocated_section_contents
  (bfd *, struct bfd_link_info *, struct bfd_link_order *,
   bfd_byte *, bfd_boolean, asymbol **);
static bfd_reloc_status_type cgc32_h8_final_link_relocate
  (unsigned long, bfd *, bfd *, asection *,
   bfd_byte *, bfd_vma, bfd_vma, bfd_vma,
   struct bfd_link_info *, asection *, int);
static bfd_boolean cgc32_h8_relocate_section
...
static reloc_howto_type h8_cgc_howto_table[] =
{
...
/* This structure is used to map BFD reloc codes to H8 CGC relocs.  */
...
/* An array mapping BFD reloc codes to H8 CGC relocs.  */

static const struct cgc_reloc_map h8_reloc_map[] = {
...
cgc32_h8_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < sizeof (h8_reloc_map) / sizeof (struct cgc_reloc_map); i++)
    {
      if (h8_reloc_map[i].bfd_reloc_val == code)
	return &h8_cgc_howto_table[(int) h8_reloc_map[i].howto_index];
    }
  return NULL;
}

static reloc_howto_type *
cgc32_h8_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (h8_cgc_howto_table) / sizeof (h8_cgc_howto_table[0]);
       i++)
    if (h8_cgc_howto_table[i].name != NULL
	&& strcasecmp (h8_cgc_howto_table[i].name, r_name) == 0)
      return &h8_cgc_howto_table[i];

  return NULL;
}

static void
cgc32_h8_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			Cgc_Internal_Rela *cgc_reloc)
{
  unsigned int r;
  unsigned int i;

  r = CGC32_R_TYPE (cgc_reloc->r_info);
  for (i = 0; i < sizeof (h8_cgc_howto_table) / sizeof (reloc_howto_type); i++)
    if (h8_cgc_howto_table[i].type == r)
      {
	bfd_reloc->howto = &h8_cgc_howto_table[i];
	return;
      }
  abort ();
}
...
static void
cgc32_h8_info_to_howto_rel (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			    Cgc_Internal_Rela *cgc_reloc ATTRIBUTE_UNUSED)
{
  ...
  abort ();
  r = CGC32_R_TYPE (cgc_reloc->r_info);
  bfd_reloc->howto = &h8_cgc_howto_table[r];
}
...
static bfd_reloc_status_type
cgc32_h8_final_link_relocate (unsigned long r_type, bfd *input_bfd,
...

/* Relocate an H8 CGC section.  */
static bfd_boolean
cgc32_h8_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
  ...
  for (; rel < relend; rel++)
    {
      ...
      cgc32_h8_info_to_howto (input_bfd, &bfd_reloc, rel);
	...
	continue;
      ...
      r = cgc32_h8_final_link_relocate (r_type, input_bfd, output_bfd,
					input_section,
...
/* The final processing done just before writing out a H8 CGC object
   file.  We use this opportunity to encode the BFD machine type
   into the flags field in the object file.  */

static void
cgc32_h8_final_write_processing (bfd *abfd,
				 bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  cgc_cgcheader (abfd)->e_flags &= ~ (EF_H8_MACH);
  cgc_cgcheader (abfd)->e_flags |= val;
}

/* Return nonzero if ABFD represents a valid H8 CGC object file; also
   record the encoded machine type found in the CGC flags.  */

static bfd_boolean
cgc32_h8_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_h8300,
			     cgc32_h8_mach (cgc_cgcheader (abfd)->e_flags));
  return TRUE;
}
...
static bfd_boolean
cgc32_h8_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
cgc32_h8_relax_section (bfd *abfd, asection *sec,
			struct bfd_link_info *link_info, bfd_boolean *again)
{
  ...
  static Cgc_Internal_Rela *last_reloc = NULL;
  ...
  *again = FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	cgc32_h8_info_to_howto (abfd, &bfd_reloc, irel);
      }
	...
	case R_H8_DIR24R8:
	  {
		...
		if (code == 0x5a	/* jmp24.  */
		    ...
		    && (int) gap >= -128
		    && last_reloc
		    && CGC32_R_TYPE (last_reloc->r_info) == R_H8_PCREL8
		    && CGC32_R_SYM (last_reloc->r_info) < symtab_hdr->sh_info)
		  {
		    bfd_vma last_value;
		    asection *last_sym_sec;
		    Cgc_Internal_Sym *last_sym;

		    /* We will need to examine the symbol used by the
		       previous relocation.  */

		    last_sym = isymbuf + CGC32_R_SYM (last_reloc->r_info);
		    last_sym_sec
		      = bfd_section_from_cgc_index (abfd, last_sym->st_shndx);
			...
			if (!cgc32_h8_relax_delete_bytes (abfd, sec,
							  irel->r_offset - 1,
							  4))
			  goto error_return;
		...
		if (irel->r_offset >= 4)
		  {
			...
			cgc32_h8_info_to_howto (abfd, &bfd_reloc, last_reloc);
			h = bfd_reloc.howto;
		    ...
		    if (irel + 1 < irelend)
		      {
			Cgc_Internal_Rela *next_reloc = irel + 1;
			arelent bfd_reloc;
			reloc_howto_type *h;
			bfd_vma next_reloc_size;

			cgc32_h8_info_to_howto (abfd, &bfd_reloc, next_reloc);
			h = bfd_reloc.howto;
		...
		if (relax)
		  {
		    ...
		    /* Fix the relocation's type.  */
		    irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_H8_DIR16);
		    irel->r_offset -= 2;

		    /* Delete four bytes of data.  */
		    if (!cgc32_h8_relax_delete_bytes (abfd, sec, irel->r_offset + 2, 4))
		      goto error_return;
...
static bfd_boolean
cgc32_h8_relax_delete_bytes (bfd *abfd, asection *sec, bfd_vma addr, int count)
{
  Cgc_Internal_Shdr *symtab_hdr;
...
/* Return TRUE if a symbol exists at the given address, else return
   FALSE.  */
static bfd_boolean
cgc32_h8_symbol_address_p (bfd *abfd, asection *sec, bfd_vma addr)
{
  Cgc_Internal_Shdr *symtab_hdr;
  unsigned int sec_shndx;
  Cgc_Internal_Sym *isym;
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses cgc32_h8_relocate_section.  */

static bfd_byte *
cgc32_h8_get_relocated_section_contents (bfd *output_bfd,
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      ...
      for (isym = isymbuf, secpp = sections; isym < isymend; ++isym, ++secpp)
	{
	  ...
	  *secpp = isec;
      ...
      if (! cgc32_h8_relocate_section (output_bfd, link_info, input_bfd,
...
#define TARGET_BIG_SYM			bfd_cgc32_h8300_vec
#define TARGET_BIG_NAME			"cgc32-h8300"
#define CGC_ARCH			bfd_arch_h8300
#define CGC_MACHINE_CODE		EM_H8_300
#define CGC_MAXPAGESIZE			0x1
#define bfd_cgc32_bfd_reloc_type_lookup cgc32_h8_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup cgc32_h8_reloc_name_lookup
#define cgc_info_to_howto		cgc32_h8_info_to_howto
#define cgc_info_to_howto_rel		cgc32_h8_info_to_howto_rel

/* So we can set/examine bits in e_flags to get the specific
   H8 architecture in use.  */
#define cgc_backend_final_write_processing \
  cgc32_h8_final_write_processing
#define cgc_backend_object_p \
  cgc32_h8_object_p
#define bfd_cgc32_bfd_merge_private_bfd_data \
  cgc32_h8_merge_private_bfd_data

/* ??? when cgc_backend_relocate_section is not defined, cgc32-target.h
   defaults to using _bfd_generic_link_hash_table_create, but
   bfd_cgc_size_dynamic_sections uses
   dynobj = cgc_hash_table (info)->dynobj;
   and thus requires an cgc hash table.  */
#define bfd_cgc32_bfd_link_hash_table_create _bfd_cgc_link_hash_table_create

/* Use an H8 specific linker, not the CGC generic linker.  */
#define cgc_backend_relocate_section cgc32_h8_relocate_section
#define cgc_backend_rela_normal		1
#define cgc_backend_can_gc_sections	1

/* And relaxing stuff.  */
#define bfd_cgc32_bfd_relax_section     cgc32_h8_relax_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
                                cgc32_h8_get_relocated_section_contents
