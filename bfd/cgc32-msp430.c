/*  MSP430-specific support for 32-bit CGC
...
static reloc_howto_type cgc_msp430_howto_table[] =
{
...
static reloc_howto_type cgc_msp430x_howto_table[] =
{
  HOWTO (R_MSP430_NONE,		/* type */
	 0,			/* rightshift */
	 2,			/* size (0 = byte, 1 = short, 2 = long) */
...
/* Map BFD reloc types to MSP430 CGC reloc types.  */
...
uses_msp430x_relocs (bfd * abfd)
{
  ...
  return bfd_get_mach (abfd) == bfd_mach_msp430x
    || abfd->xvec == & bfd_cgc32_msp430_ti_vec;
}
				 ...
				 bfd_reloc_code_real_type code)
{
  ...
  if (uses_msp430x_relocs (abfd))
	...
	if (msp430x_reloc_map[i].bfd_reloc_val == code)
	  return cgc_msp430x_howto_table + msp430x_reloc_map[i].cgc_reloc_val;
    }
  else
    {
      for (i = 0; i < ARRAY_SIZE (msp430_reloc_map); i++)
	if (msp430_reloc_map[i].bfd_reloc_val == code)
	  return &cgc_msp430_howto_table[msp430_reloc_map[i].cgc_reloc_val];
    }
				 ...
				 const char *r_name)
{
  ...
  if (uses_msp430x_relocs (abfd))
    {
      for (i = ARRAY_SIZE (cgc_msp430x_howto_table); i--;)
	if (cgc_msp430x_howto_table[i].name != NULL
	    && strcasecmp (cgc_msp430x_howto_table[i].name, r_name) == 0)
	  return cgc_msp430x_howto_table + i;
    }
  else
    {
      for (i = 0;
	   i < (sizeof (cgc_msp430_howto_table)
		/ sizeof (cgc_msp430_howto_table[0]));
	   i++)
	if (cgc_msp430_howto_table[i].name != NULL
	    && strcasecmp (cgc_msp430_howto_table[i].name, r_name) == 0)
	  return &cgc_msp430_howto_table[i];
    }

  return NULL;
}

/* Set the howto pointer for an MSP430 CGC reloc.  */
  ...
  if (uses_msp430x_relocs (abfd))
    {
      ...
      cache_ptr->howto = cgc_msp430x_howto_table + r_type;
      return;
    }

  BFD_ASSERT (r_type < (unsigned int) R_MSP430_max);
  cache_ptr->howto = &cgc_msp430_howto_table[r_type];
}
...
static bfd_boolean
cgc32_msp430_check_relocs (bfd * abfd, struct bfd_link_info * info,
			    ...
			    struct bfd_link_info * info)
{
  ...
  struct bfd_cgc_section_data * esd = cgc_section_data (input_section);
...
/* Relocate an MSP430 CGC section.  */

static bfd_boolean
cgc32_msp430_relocate_section (bfd * output_bfd ATTRIBUTE_UNUSED,
			       struct bfd_link_info * info,
			       bfd * input_bfd,
			       asection * input_section,
			       bfd_byte * contents,
  ...
  for (rel = relocs; rel < relend; rel++)
    {
      ...
      if (uses_msp430x_relocs (input_bfd))
	howto = cgc_msp430x_howto_table + r_type;
      else
	howto = cgc_msp430_howto_table + r_type;
...
/* The final processing done just before writing out a MSP430 CGC object
   file.  This gets the MSP430 architecture right based on the machine
   number.  */

static void
bfd_cgc_msp430_final_write_processing (bfd * abfd,
				       bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  cgc_cgcheader (abfd)->e_machine = EM_MSP430;
  cgc_cgcheader (abfd)->e_flags &= ~EF_MSP430_MACH;
...
static bfd_boolean
cgc32_msp430_object_p (bfd * abfd)
{
  int e_set = bfd_mach_msp14;

  if (cgc_cgcheader (abfd)->e_machine == EM_MSP430
      || cgc_cgcheader (abfd)->e_machine == EM_MSP430_OLD)
    {
      int e_mach = cgc_cgcheader (abfd)->e_flags & EF_MSP430_MACH;
...
static bfd_boolean
msp430_cgc_symbol_address_p (bfd * abfd,
			     asection * sec,
...
static bfd_boolean
msp430_cgc_relax_adjust_locals (bfd * abfd, asection * sec, bfd_vma addr,
				...
				bfd_vma toaddr)
{
  ...
  for (;irel < irelend; irel++)
    {
      int sidx = CGC32_R_SYM(irel->r_info);
      Cgc_Internal_Sym *lsym = isym + sidx;
...
static bfd_boolean
msp430_cgc_relax_delete_bytes (bfd * abfd, asection * sec, bfd_vma addr,
			       int count)
{
  Cgc_Internal_Shdr *symtab_hdr;
  unsigned int sec_shndx;
  ...
  for (p = abfd->sections; p != NULL; p = p->next)
    msp430_cgc_relax_adjust_locals (abfd,p,addr,count,sec_shndx,toaddr);
...
static bfd_byte *
msp430_cgc_relax_add_two_words (bfd * abfd, asection * sec, bfd_vma addr,
				int word1, int word2)
{
  Cgc_Internal_Shdr *symtab_hdr;
  unsigned int sec_shndx;
  ...
  sec_shndx = _bfd_cgc_section_from_bfd_section (abfd, sec);
  for (p = abfd->sections; p != NULL; p = p->next)
    msp430_cgc_relax_adjust_locals (abfd, p, addr, -4,
				    sec_shndx, sec_end);
...
static bfd_boolean
msp430_cgc_relax_section (bfd * abfd, asection * sec,
			  ...
			  bfd_boolean * again)
{
  ...
  *again = FALSE;
  ...
  internal_relocs =
    _bfd_cgc_link_read_relocs (abfd, sec, NULL, NULL, link_info->keep_memory);
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      /* If this isn't something that can be relaxed, then ignore
         this reloc.  */
      if (uses_msp430x_relocs (abfd)
          && CGC32_R_TYPE (irel->r_info) == (int) R_MSP430X_10_PCREL)
	;
      else if (! uses_msp430x_relocs (abfd)
               && CGC32_R_TYPE (irel->r_info) == (int) R_MSP430_10_PCREL)
	;
      else
	continue;
      ...
      if (uses_msp430x_relocs (abfd))
	{
	  ...
	  contents = msp430_cgc_relax_add_two_words
  ...
  if (! uses_msp430x_relocs (abfd))
	...
	/* Try to turn a 16bit pc-relative branch into a 10bit pc-relative
	   branch.  */
	/* Paranoia? paranoia...  */      
	if (CGC32_R_TYPE (irel->r_info) == (int) R_MSP430_RL_PCREL)
	  {
		...
		if (rx->cdx > 0
		    && msp430_cgc_symbol_address_p (abfd, sec, isymbuf,
						    irel->r_offset - 2))
		  continue;
		if (rx->cdx > 1
		    && msp430_cgc_symbol_address_p (abfd, sec, isymbuf,
		...
		/* Delete bytes. */
		if (!msp430_cgc_relax_delete_bytes (abfd, sec,
...
static bfd_boolean
cgc32_msp430_section_from_shdr (bfd *abfd,
...
static bfd_boolean
cgc32_msp430_obj_attrs_handle_unknown (bfd *abfd, int tag)
{
  _bfd_error_handler
...
static int
cgc32_msp430_obj_attrs_arg_type (int tag)
{
...
static bfd_boolean
cgc32_msp430_merge_mspabi_attributes (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
cgc32_msp430_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
...
#undef max
  ...
  return cgc32_msp430_merge_mspabi_attributes (ibfd, obfd);
}

/* Copy backend specific data from one object module to another.  */

static bfd_boolean
cgc32_msp430_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
msp430_cgc_is_target_special_symbol (bfd *abfd, asymbol *sym)
{
...
#define cgc_backend_obj_attrs_vendor		"mspabi"
#undef  cgc_backend_obj_attrs_section
#define cgc_backend_obj_attrs_section		".MSP430.attributes"
#undef  cgc_backend_obj_attrs_section_type
#define cgc_backend_obj_attrs_section_type	SHT_MSP430_ATTRIBUTES
#define cgc_backend_section_from_shdr  		cgc32_msp430_section_from_shdr
#define cgc_backend_obj_attrs_handle_unknown 	cgc32_msp430_obj_attrs_handle_unknown
#undef  cgc_backend_obj_attrs_arg_type
#define cgc_backend_obj_attrs_arg_type		cgc32_msp430_obj_attrs_arg_type
#define bfd_cgc32_bfd_copy_private_bfd_data	cgc32_msp430_copy_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data	cgc32_msp430_merge_private_bfd_data

#define CGC_ARCH		bfd_arch_msp430
#define CGC_MACHINE_CODE	EM_MSP430
#define CGC_MACHINE_ALT1	EM_MSP430_OLD
#define CGC_MAXPAGESIZE		4
#define	CGC_OSABI		CGCOSABI_STANDALONE

#define TARGET_LITTLE_SYM       bfd_cgc32_msp430_vec
#define TARGET_LITTLE_NAME	"cgc32-msp430"

#define cgc_info_to_howto	             msp430_info_to_howto_rela
#define cgc_info_to_howto_rel	             NULL
#define cgc_backend_relocate_section         cgc32_msp430_relocate_section
#define cgc_backend_check_relocs             cgc32_msp430_check_relocs
#define cgc_backend_can_gc_sections          1
#define cgc_backend_final_write_processing   bfd_cgc_msp430_final_write_processing
#define cgc_backend_object_p		     cgc32_msp430_object_p
#define cgc_backend_post_process_headers     _bfd_cgc_set_osabi
#define bfd_cgc32_bfd_relax_section	     msp430_cgc_relax_section
#define bfd_cgc32_bfd_is_target_special_symbol	msp430_cgc_is_target_special_symbol

#undef  cgc32_bed
#define cgc32_bed		cgc32_msp430_bed

#include "cgc32-target.h"

/* The TI compiler sets the OSABI field to CGCOSABI_NONE.  */
#undef  TARGET_LITTLE_SYM
#define TARGET_LITTLE_SYM       bfd_cgc32_msp430_ti_vec

#undef  cgc32_bed
#define cgc32_bed		cgc32_msp430_ti_bed

#undef	CGC_OSABI		
#define	CGC_OSABI		CGCOSABI_NONE

static const struct bfd_cgc_special_section msp430_ti_cgc_special_sections[] =
{
