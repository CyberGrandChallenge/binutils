...
/* ABI version:
    0 - original
    1 - with GOT offset */
#define METAG_CGC_ABI_VERSION 1
...
/* Variable names follow a coding style.
   ...
   cgc_link_hash_table             "etab"
   cgc_link_hash_entry             "eh"

   cgc_metag_link_hash_table       "htab"
   cgc_metag_link_hash_entry       "hh"

   bfd_link_hash_table             "btab"
   bfd_link_hash_entry             "bh"

   bfd_hash_table containing stubs "bstab"
   cgc_metag_stub_hash_entry       "hsh"

   cgc_metag_dyn_reloc_entry       "hdh"

   Always remember to use GNU Coding Style.  */

#define PLT_ENTRY_SIZE sizeof(plt_entry)

static reloc_howto_type cgc_metag_howto_table[] =
{
...
struct cgc_metag_stub_hash_entry
{
  ...
  enum cgc_metag_stub_type stub_type;

  /* The symbol table entry, if any, that this was derived from.  */
  struct cgc_metag_link_hash_entry *hh;
...
struct cgc_metag_link_hash_entry
{
  struct cgc_link_hash_entry eh;

  /* A pointer to the most recently used stub hash entry against this
     symbol.  */
  struct cgc_metag_stub_hash_entry *hsh_cache;

  /* Used to count relocations for delayed sizing of relocation
     sections.  */
  struct cgc_metag_dyn_reloc_entry {

    /* Next relocation in the chain.  */
    struct cgc_metag_dyn_reloc_entry *hdh_next;
...
struct cgc_metag_link_hash_table
{
  /* The main hash table.  */
  struct cgc_link_hash_table etab;
  ...
  /* Assorted information used by cgc_metag_size_stubs.  */
  unsigned int bfd_count;
  int top_index;
  asection **input_list;
  Cgc_Internal_Sym **all_local_syms;
			  ...
			  Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = & cgc_metag_howto_table [r_type];
}
			 ...
			 bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < sizeof (metag_reloc_map) / sizeof (metag_reloc_map[0]); i++)
    if (metag_reloc_map [i].bfd_reloc_val == code)
      return & cgc_metag_howto_table [metag_reloc_map[i].metag_reloc_val];
			 ...
			 const char *r_name)
{
  ...
  for (i = 0; i < sizeof (cgc_metag_howto_table) / sizeof (cgc_metag_howto_table[0]); i++)
    if (cgc_metag_howto_table[i].name != NULL
	&& strcasecmp (cgc_metag_howto_table[i].name, r_name) == 0)
      return &cgc_metag_howto_table[i];
...
#define metag_link_hash_table(p) \
  ...
  == METAG_CGC_DATA ? ((struct cgc_metag_link_hash_table *) ((p)->hash)) : NULL)

#define metag_cgc_hash_entry(ent) \
  ((struct cgc_metag_link_hash_entry *)(ent))

#define metag_stub_hash_entry(ent) \
  ((struct cgc_metag_stub_hash_entry *)(ent))

#define metag_stub_hash_lookup(table, string, create, copy) \
  ((struct cgc_metag_stub_hash_entry *) \
   bfd_hash_lookup ((table), (string), (create), (copy)))

#define metag_cgc_local_got_tls_type(abfd) \
  ((char *)(cgc_local_got_offsets (abfd) + (cgc_tdata (abfd)->symtab_hdr.sh_info)))
		   ...
		   const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc_metag_stub_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc_metag_stub_hash_entry *hsh;

      /* Initialize the local fields.  */
      hsh = (struct cgc_metag_stub_hash_entry *) entry;
      hsh->stub_sec = NULL;
      hsh->stub_offset = 0;
      hsh->target_value = 0;
			 ...
			 const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc_metag_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      ...
      /* Initialize the local fields.  */
      hh = (struct cgc_metag_link_hash_entry *) entry;
      hh->hsh_cache = NULL;
      hh->dyn_relocs = NULL;
...
/* Create the derived linker hash table.  The Meta CGC port uses the derived
   hash table to keep information specific to the Meta CGC linker (without
   using static variables).  */

static struct bfd_link_hash_table *
cgc_metag_link_hash_table_create (bfd *abfd)
{
  struct cgc_metag_link_hash_table *htab;
  ...
  if (!_bfd_cgc_link_hash_table_init (&htab->etab, abfd,
...
static void
cgc_metag_link_hash_table_free (struct bfd_link_hash_table *btab)
{
  struct cgc_metag_link_hash_table *htab
    = (struct cgc_metag_link_hash_table *) btab;

  bfd_hash_table_free (&htab->bstab);
  _bfd_cgc_link_hash_table_free (btab);
}
...
metag_stub_name (const asection *input_section,
		 ...
		 const struct cgc_metag_link_hash_entry *hh,
  ...
  if (hh)
      ...
      if (stub_name != NULL)
	{
	  sprintf (stub_name, "%08x_%x:%x+%x",
		   ...
		   (int) CGC32_R_SYM (rel->r_info) & 0xffffffff,
		   (int) rel->r_addend & 0xffffffff);
	}
    }
  return stub_name;
...
static struct cgc_metag_stub_hash_entry *
			   ...
			   asection *sym_sec)
{
  ...
  struct cgc_metag_stub_hash_entry *hsh = NULL;
  bfd_vma location;
  ...
  switch (howto->type)
    ...
    case R_METAG_TLS_GD:
    case R_METAG_TLS_IE:
      relocation -= cgc_gp (input_section->output_section->owner);
      break;
    default:
      break;
    }
...
static bfd_boolean
cgc_metag_relocate_section (bfd *output_bfd,
			    ...
			    asection **local_sections)
{
  ...
  struct cgc_link_hash_entry **eh_syms;
  struct cgc_metag_link_hash_table *htab;
  Cgc_Internal_Rela *rel;
  ...
  eh_syms = cgc_sym_hashes (input_bfd);
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = cgc_metag_howto_table + CGC32_R_TYPE (rel->r_info);
      hh     = NULL;
      sym    = NULL;
      sec    = NULL;
      ...
      else
	{
	  ...
	  hh = (struct cgc_metag_link_hash_entry *) eh;
	}
	...
	case R_METAG_RELBRANCH:
	  ...
	  if ((info->shared
	      ...
	      if (skip)
		{
		  ...
		  outrel.r_info = CGC32_R_INFO (0, R_METAG_NONE);
		}
	      else if (r_type == R_METAG_RELBRANCH)
		{
		  BFD_ASSERT (hh != NULL && hh->eh.dynindx != -1);
		  outrel.r_info = CGC32_R_INFO (hh->eh.dynindx, r_type);
	      ...
	      else
		{
		  ...
		  if (hh == NULL
		    ...
		    {
		      relocate = TRUE;
		      outrel.r_info = CGC32_R_INFO (0, R_METAG_RELATIVE);
	...
	case R_METAG_TLS_IE:
	  {
		...
		off = local_got_offsets[r_symndx];
		tls_type = metag_cgc_local_got_tls_type (input_bfd) [r_symndx];
	      }
		...
		if ((info->shared || indx != 0)
		    && (hh == NULL
			|| CGC_ST_VISIBILITY (hh->eh.other) == STV_DEFAULT
			|| hh->eh.root.type != bfd_link_hash_undefweak))
		  {
		    need_relocs = TRUE;
		    loc = htab->srelgot->contents;
		    /* FIXME (CAO): Should this be reloc_count++ ? */
		    loc += htab->srelgot->reloc_count * sizeof (Cgc32_External_Rela);
		  }
		...
		if (tls_type & GOT_TLS_GD)
		  {
			...
			outrel.r_offset = (cur_off
					   + htab->sgot->output_section->vma
					   + htab->sgot->output_offset);
			outrel.r_info = CGC32_R_INFO (indx, R_METAG_TLS_DTPMOD);
			outrel.r_addend = 0;
		...
		if (tls_type & GOT_TLS_IE)
		  {
			...
			outrel.r_offset = (cur_off
					   + htab->sgot->output_section->vma
					   + htab->sgot->output_offset);
			outrel.r_info = CGC32_R_INFO (indx, R_METAG_TLS_TPOFF);
	...
	case R_METAG_TLS_LDM:
	  {
		...
		outrel.r_addend = 0;
		outrel.r_info = CGC32_R_INFO (0, R_METAG_TLS_DTPMOD);
		loc = htab->srelgot->contents;
		loc += htab->srelgot->reloc_count++ * sizeof (Cgc32_External_Rela);
		bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
...
static bfd_boolean
cgc_metag_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_metag_link_hash_table *htab;
  struct cgc_link_hash_entry *eh;
  ...
  if (!(_bfd_generic_link_add_one_symbol
    ...
    return FALSE;
  eh = (struct cgc_link_hash_entry *) bh;
  eh->def_regular = 1;
  eh->type = STT_OBJECT;
  eh->other = STV_HIDDEN;

  if (! info->executable
      && ! bfd_cgc_link_record_dynamic_symbol (info, eh))
    return FALSE;

  cgc_hash_table (info)->hgot = eh;
...
static bfd_boolean
cgc_metag_check_relocs (bfd *abfd,
			struct bfd_link_info *info,
			asection *sec,
			const Cgc_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      else
	{
	  ...
	  hh = (struct cgc_metag_link_hash_entry *)
	    eh_syms[r_symndx - symtab_hdr->sh_info];
	  while (hh->eh.root.type == bfd_link_hash_indirect
		 || hh->eh.root.type == bfd_link_hash_warning)
	    hh = (struct cgc_metag_link_hash_entry *) hh->eh.root.u.i.link;
      ...
      if (htab->sgot == NULL)
	{
	  switch (r_type)
		...
		htab->etab.dynobj = dynobj = abfd;
	      if (!cgc_metag_create_dynamic_sections (dynobj, info))
		return FALSE;
	      break;
	...
	case R_METAG_GETSET_GOT:
	  ...
	  if (hh != NULL)
	      ...
	      if (local_got_refcounts == NULL)
		{
		  ...
		  memset (metag_cgc_local_got_tls_type (abfd),
			  GOT_UNKNOWN, symtab_hdr->sh_info);
		}
	      local_got_refcounts[r_symndx] += 1;
	      old_tls_type = metag_cgc_local_got_tls_type (abfd) [r_symndx];
	    }
	  ...
	  if (old_tls_type != tls_type)
		...
		{
		  metag_cgc_local_got_tls_type (abfd) [r_symndx] = tls_type;
		}
	    }
	...
	case R_METAG_LOADDR16:
	  ...
	  if (info->shared
		...
		(_("%B: relocation %s against `%s' can not be used when making a shared object; recompile with -fPIC"),
		 abfd, cgc_metag_howto_table[r_type].name, name);
	...
	case R_METAG_GETSETOFF:
	  ...
	  if ((info->shared
		  ...
		  && (hh->eh.root.type == bfd_link_hash_defweak
		      || !hh->eh.def_regular)))
	    {
	      struct cgc_metag_dyn_reloc_entry *hdh_p;
	      struct cgc_metag_dyn_reloc_entry **hdh_head;
	      ...
	      if (sreloc == NULL)
		{
		  ...
		  cgc_section_data (sec)->sreloc = sreloc;
		}

	      /* If this is a global symbol, we count the number of
		 relocations we need for this symbol.  */
	      if (hh != NULL)
		hdh_head = &((struct cgc_metag_link_hash_entry *) hh)->dyn_relocs;
	      else
		{
		  ...
		  sr = bfd_section_from_cgc_index (abfd, isym->st_shndx);
		  if (sr == NULL)
		    sr = sec;

		  vpp = &cgc_section_data (sr)->local_dynrel;
		  hdh_head = (struct cgc_metag_dyn_reloc_entry **) vpp;
		}

	      hdh_p = *hdh_head;
	      if (hdh_p == NULL || hdh_p->sec != sec)
		{
		  hdh_p = ((struct cgc_metag_dyn_reloc_entry *)
		  ...
		  *hdh_head = hdh_p;
	      ...
	      hdh_p->count += 1;
	      if (CGC32_R_TYPE (rel->r_info) == R_METAG_RELBRANCH)
	...
	case R_METAG_GNU_VTINHERIT:
	  if (!bfd_cgc_gc_record_vtinherit (abfd, sec, &hh->eh,
					    rel->r_offset))
	    return FALSE;
	  break;
	...
	case R_METAG_GNU_VTENTRY:
	  BFD_ASSERT (hh != NULL);
	  if (hh != NULL
	      && !bfd_cgc_gc_record_vtentry (abfd, sec, &hh->eh, rel->r_addend))
	    return FALSE;
	  break;
	}
    }
...
static void
cgc_metag_copy_indirect_symbol (struct bfd_link_info *info,
				struct cgc_link_hash_entry *eh_dir,
				struct cgc_link_hash_entry *eh_ind)
{
  struct cgc_metag_link_hash_entry *hh_dir, *hh_ind;

  hh_dir = metag_cgc_hash_entry (eh_dir);
  hh_ind = metag_cgc_hash_entry (eh_ind);

  if (hh_ind->dyn_relocs != NULL)
    {
      if (hh_dir->dyn_relocs != NULL)
	{
	  struct cgc_metag_dyn_reloc_entry **hdh_pp;
	  struct cgc_metag_dyn_reloc_entry *hdh_p;
	  ...
	  for (hdh_pp = &hh_ind->dyn_relocs; (hdh_p = *hdh_pp) != NULL; )
	    {
	      struct cgc_metag_dyn_reloc_entry *hdh_q;
  ...
  _bfd_cgc_link_hash_copy_indirect (info, eh_dir, eh_ind);
}
...
static bfd_boolean
cgc_metag_adjust_dynamic_symbol (struct bfd_link_info *info,
				 struct cgc_link_hash_entry *eh)
{
  struct cgc_metag_link_hash_table *htab;
  struct cgc_metag_link_hash_entry *hh;
  ...
  return _bfd_cgc_adjust_dynamic_copy (eh, s);
}
...
static bfd_boolean
allocate_dynrelocs (struct cgc_link_hash_entry *eh, void *inf)
{
  struct bfd_link_info *info;
  struct cgc_metag_link_hash_table *htab;
  struct cgc_metag_link_hash_entry *hh;
  ...
  if (htab->etab.dynamic_sections_created
      && eh->plt.refcount > 0)
    {
	 ...
	 Undefined weak syms won't yet be marked as dynamic.  */
	  ...
	  && !eh->forced_local)
	{
	  if (! bfd_cgc_link_record_dynamic_symbol (info, eh))
	    return FALSE;
	}
  ...
  if (eh->got.refcount > 0)
    {
      asection *s;
      bfd_boolean dyn;
      int tls_type = metag_cgc_hash_entry (eh)->tls_type;
  ...
  if (info->shared)
      ...
      if (SYMBOL_CALLS_LOCAL (info, eh))
	{
	 ...
	 visibility.  */
	  ...
	  && eh->root.type == bfd_link_hash_undefweak)
	{
	  if (CGC_ST_VISIBILITY (eh->other) != STV_DEFAULT)
	    hh->dyn_relocs = NULL;
  ...
  for (hdh_p = hh->dyn_relocs; hdh_p != NULL; hdh_p = hdh_p->hdh_next)
    {
      asection *sreloc = cgc_section_data (hdh_p->sec)->sreloc;
      sreloc->size += hdh_p->count * sizeof (Cgc32_External_Rela);
    }
...
static bfd_boolean
readonly_dynrelocs (struct cgc_link_hash_entry *eh, void *inf)
{
  struct cgc_metag_link_hash_entry *hh;
  struct cgc_metag_dyn_reloc_entry *hdh_p;
...
static bfd_boolean
cgc_metag_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				 struct bfd_link_info *info)
{
  struct cgc_metag_link_hash_table *htab;
  bfd *dynobj;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (hdh_p = ((struct cgc_metag_dyn_reloc_entry *)
	      ...
	      else if (hdh_p->count != 0)
		{
		  ...
		  srel->size += hdh_p->count * sizeof (Cgc32_External_Rela);
	...
	continue;
      ...
      local_tls_type = metag_cgc_local_got_tls_type (ibfd);
      s = htab->sgot;
      srel = htab->srelgot;
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  cgc_link_hash_traverse (&htab->etab, allocate_dynrelocs, info);
  ...
  for (s = dynobj->sections; s != NULL; s = s->next)
      ...
      else if (reloc_section)
	{
	  ...
	  Cgc32_External_Rela reloc;

	  /* Fill the reloc section with a R_METAG_NONE type reloc.  */
	  memset(&reloc, 0, sizeof(Cgc32_External_Rela));
	  reloc.r_info[0] = R_METAG_NONE;
	  for (; contents < (s->contents + s->size);
	       contents += sizeof(Cgc32_External_Rela))
	    {
	      memcpy(contents, &reloc, sizeof(Cgc32_External_Rela));
	    }
	}
    }
  ...
  if (htab->etab.dynamic_sections_created)
    {
	 ...
	 values later, in cgc_metag_finish_dynamic_sections, but we
  ...
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (relocs)
	{
	  ...
	  if ((info->flags & DF_TEXTREL) == 0)
	    cgc_link_hash_traverse (&htab->etab, readonly_dynrelocs, info);
...
static bfd_boolean
cgc_metag_finish_dynamic_symbol (bfd *output_bfd,
				 struct bfd_link_info *info,
				 struct cgc_link_hash_entry *eh,
				 Cgc_Internal_Sym *sym)
{
  ...
  if (eh->plt.offset != (bfd_vma) -1)
      ...
      else
	{
      ...
      rel.r_offset = (sgot->output_section->vma
		      + sgot->output_offset
		      + got_offset);
      rel.r_info = CGC32_R_INFO (eh->dynindx, R_METAG_JMP_SLOT);
      rel.r_addend = 0;
  ...
  if (eh->got.offset != (bfd_vma) -1
      && (metag_cgc_hash_entry (eh)->tls_type & GOT_TLS_GD) == 0
      && (metag_cgc_hash_entry (eh)->tls_type & GOT_TLS_IE) == 0)
    {
	 ...
	 relocate_section function.  */
	  ...
	  && eh->def_regular)
	{
	  rel.r_info = CGC32_R_INFO (0, R_METAG_RELATIVE);
      ...
      else
	{
	  ...
	  rel.r_info = CGC32_R_INFO (eh->dynindx, R_METAG_GLOB_DAT);
	  rel.r_addend = 0;
	}
  ...
  if (eh->needs_copy)
	...
	abort ();
      ...
      rel.r_offset = (eh->root.u.def.value
		      ...
		      + eh->root.u.def.section->output_section->vma);
      rel.r_addend = 0;
      rel.r_info = CGC32_R_INFO (eh->dynindx, R_METAG_COPY);
...
/* Set the Meta CGC ABI version.  */

static void
cgc_metag_post_process_headers (bfd * abfd, struct bfd_link_info * link_info ATTRIBUTE_UNUSED)
{
  Cgc_Internal_Ehdr * i_ehdrp;	/* CGC file header, internal form.  */

  i_ehdrp = cgc_cgcheader (abfd);
  i_ehdrp->e_ident[EI_ABIVERSION] = METAG_CGC_ABI_VERSION;
}
...
cgc_metag_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc_metag_finish_dynamic_sections (bfd *output_bfd,
				   struct bfd_link_info *info)
{
  bfd *dynobj;
  struct cgc_metag_link_hash_table *htab;
  ...
  if (htab->sgot != NULL && htab->sgot->size != 0)
    {
	 ...
	 We use it to point to our dynamic section, if we have one.  */
      ...
      /* Set .got entry size.  */
      cgc_section_data (htab->sgot->output_section)
...
static asection *
cgc_metag_gc_mark_hook (asection *sec,
			struct bfd_link_info *info,
...
{
  if (hh != NULL)
    switch ((unsigned int) CGC32_R_TYPE (rela->r_info))
      {
      case R_METAG_GNU_VTINHERIT:
      case R_METAG_GNU_VTENTRY:
	return NULL;
  ...
  return _bfd_cgc_gc_mark_hook (sec, info, rela, hh, sym);
}
...
static bfd_boolean
cgc_metag_gc_sweep_hook (bfd *abfd ATTRIBUTE_UNUSED,
  ...
  for (rel = relocs; rel < relend; rel++)
    {
      unsigned long r_symndx;
      unsigned int r_type;
      struct cgc_link_hash_entry *eh = NULL;
...
static enum cgc_metag_stub_type
...
metag_build_one_stub (struct bfd_hash_entry *gen_entry, void *in_arg ATTRIBUTE_UNUSED)
{
  ...
  hsh = (struct cgc_metag_stub_hash_entry *) gen_entry;
...
int
cgc_metag_setup_section_lists (bfd *output_bfd, struct bfd_link_info *info)
{
  bfd *input_bfd;
  unsigned int bfd_count;
  ...
  bfd_size_type amt;
  struct cgc_metag_link_hash_table *htab = metag_link_hash_table (info);
...
void
cgc_metag_next_input_section (struct bfd_link_info *info, asection *isec)
{
  struct cgc_metag_link_hash_table *htab = metag_link_hash_table (info);
...
static void
group_sections (struct cgc_metag_link_hash_table *htab,
		...
		struct bfd_link_info *info)
{
  ...
  Cgc_Internal_Sym *local_syms, **all_local_syms;
  int stub_changed = 0;
  struct cgc_metag_link_hash_table *htab = metag_link_hash_table (info);
  ...
  bfd_size_type amt = sizeof (Cgc_Internal_Sym *) * htab->bfd_count;
  ...
  for (bfd_indx = 0;
	...
	continue;
      ...
      if (local_syms == NULL)
	{
	  local_syms = bfd_cgc_get_cgc_syms (input_bfd, symtab_hdr,
					     symtab_hdr->sh_info, 0,
					     NULL, NULL, NULL);
	  /* Cache them for cgc_link_input_bfd.  */
...
/* See cgc32-hppa.c and cgc64-ppc.c.  */

bfd_boolean
cgc_metag_size_stubs(bfd *output_bfd, bfd *stub_bfd,
		     struct bfd_link_info *info,
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  if (r_indx < symtab_hdr->sh_info)
			...
			sym_value = sym->st_value;
		      shndx = sym->st_shndx;
		      if (shndx < cgc_numsections (input_bfd))
			{
			  hdr = cgc_cgcsections (input_bfd)[shndx];
...
/* Build all the stubs associated with the current output file.  The
   ...
   table.  This function is called via metagcgc_finish in the linker.  */

bfd_boolean
cgc_metag_build_stubs (struct bfd_link_info *info)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  struct cgc_metag_link_hash_table *htab;
...
static bfd_boolean
cgc_metag_is_local_label_name (bfd *abfd ATTRIBUTE_UNUSED, const char *name)
{
  if (name[0] == '$' && name[1] == 'L')
    return 1;
...
static bfd_vma
cgc_metag_plt_sym_val (bfd_vma i, const asection *plt,
		       const arelent *rel ATTRIBUTE_UNUSED)
{
  return plt->vma + (i + 1) * PLT_ENTRY_SIZE;
}

#define CGC_ARCH		bfd_arch_metag
#define CGC_TARGET_ID		METAG_CGC_DATA
#define CGC_MACHINE_CODE	EM_METAG
#define CGC_MINPAGESIZE	0x1000
#define CGC_MAXPAGESIZE	0x4000
#define CGC_COMMONPAGESIZE	0x1000

#define TARGET_LITTLE_SYM	bfd_cgc32_metag_vec
#define TARGET_LITTLE_NAME	"cgc32-metag"

#define cgc_symbol_leading_char '_'

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			metag_info_to_howto_rela

#define bfd_cgc32_bfd_is_local_label_name	cgc_metag_is_local_label_name
#define bfd_cgc32_bfd_link_hash_table_create \
	cgc_metag_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free	cgc_metag_link_hash_table_free
#define cgc_backend_relocate_section		cgc_metag_relocate_section
#define cgc_backend_gc_mark_hook		cgc_metag_gc_mark_hook
#define cgc_backend_gc_sweep_hook		cgc_metag_gc_sweep_hook
#define cgc_backend_check_relocs		cgc_metag_check_relocs
#define cgc_backend_create_dynamic_sections	cgc_metag_create_dynamic_sections
#define cgc_backend_adjust_dynamic_symbol	cgc_metag_adjust_dynamic_symbol
#define cgc_backend_finish_dynamic_symbol	cgc_metag_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections	cgc_metag_finish_dynamic_sections
#define cgc_backend_size_dynamic_sections	cgc_metag_size_dynamic_sections
#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_post_process_headers	cgc_metag_post_process_headers
#define cgc_backend_reloc_type_class		cgc_metag_reloc_type_class
#define cgc_backend_copy_indirect_symbol	cgc_metag_copy_indirect_symbol
#define cgc_backend_plt_sym_val		cgc_metag_plt_sym_val
...
#define bfd_cgc32_bfd_reloc_type_lookup	metag_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	metag_reloc_name_lookup
