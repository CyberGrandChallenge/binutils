/* TILEPro-specific support for 32-bit CGC.
...
#include "libiberty.h"
#include "cgc32-tilepro.h"

#define TILEPRO_BYTES_PER_WORD 4

static reloc_howto_type tilepro_cgc_howto_table [] =
{
...
#define TILEPRO_IMM_HOWTO(name, size, bitsize) \
  HOWTO (name, 0, size, bitsize, FALSE, 0, \
         complain_overflow_signed, bfd_cgc_generic_reloc, \
         #name, FALSE, 0, -1, FALSE)

#define TILEPRO_UIMM_HOWTO(name, size, bitsize) \
  HOWTO (name, 0, size, bitsize, FALSE, 0, \
         complain_overflow_unsigned, bfd_cgc_generic_reloc, \
         #name, FALSE, 0, -1, FALSE)
...
static reloc_howto_type tilepro_cgc_howto_table2 [] =
{
...

/* Map BFD reloc types to TILEPRO CGC reloc types.  */
...
#define TH_REMAP(bfd, tilepro) \
  { bfd, tilepro, tilepro_cgc_howto_table },
...
#undef TH_REMAP
  ...
  { BFD_RELOC_VTABLE_INHERIT, R_TILEPRO_GNU_VTINHERIT, tilepro_cgc_howto_table2 },
  { BFD_RELOC_VTABLE_ENTRY,   R_TILEPRO_GNU_VTENTRY,   tilepro_cgc_howto_table2 },
};
...
struct tilepro_cgc_dyn_relocs
{
  struct tilepro_cgc_dyn_relocs *next;
...
/* TILEPRO CGC linker hash entry.  */

struct tilepro_cgc_link_hash_entry
{
  struct cgc_link_hash_entry cgc;

  /* Track dynamic relocs copied for this symbol.  */
  struct tilepro_cgc_dyn_relocs *dyn_relocs;
...
#define tilepro_cgc_hash_entry(ent) \
  ((struct tilepro_cgc_link_hash_entry *)(ent))

struct _bfd_tilepro_cgc_obj_tdata
{
...
#define _bfd_tilepro_cgc_tdata(abfd) \
  ((struct _bfd_tilepro_cgc_obj_tdata *) (abfd)->tdata.any)

#define _bfd_tilepro_cgc_local_got_tls_type(abfd) \
  (_bfd_tilepro_cgc_tdata (abfd)->local_got_tls_type)

#define is_tilepro_cgc(bfd)				\
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour	\
   && cgc_tdata (bfd) != NULL				\
   && cgc_object_id (bfd) == TILEPRO_CGC_DATA)

/* Allocate TILEPro CGC private object data.  */

static bfd_boolean
tilepro_cgc_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd,
				  sizeof (struct _bfd_tilepro_cgc_obj_tdata),
				  TILEPRO_CGC_DATA);
}
...
struct tilepro_cgc_link_hash_table
{
...
/* Get the Tilepro CGC linker hash table from a link_info structure.  */
#define tilepro_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == TILEPRO_CGC_DATA \
  ? ((struct tilepro_cgc_link_hash_table *) ((p)->hash)) : NULL)
			   ...
			   const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (tilepro_cgc_howto_table)
            / sizeof (tilepro_cgc_howto_table[0]));
       i++)
    if (tilepro_cgc_howto_table[i].name != NULL
        && strcasecmp (tilepro_cgc_howto_table[i].name, r_name) == 0)
      return &tilepro_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an TILEPro CGC reloc.  */
                            ...
                            Cgc_Internal_Rela * dst)
{
  ...
  if (r_type <= (unsigned int) R_TILEPRO_IMM16_X1_TLS_LE_HA)
    cache_ptr->howto = &tilepro_cgc_howto_table [r_type];
  else if (r_type - R_TILEPRO_GNU_VTINHERIT
	   <= (unsigned int) R_TILEPRO_GNU_VTENTRY)
    cache_ptr->howto
      = &tilepro_cgc_howto_table2 [r_type - R_TILEPRO_GNU_VTINHERIT];
  else
    abort ();
}
...
static bfd_boolean
tilepro_cgc_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  size_t size;
...
static bfd_boolean
tilepro_cgc_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  if (note->descsz != TILEPRO_PRPSINFO_SIZEOF)
    return FALSE;

  cgc_tdata (abfd)->core->program
    = _bfd_cgccore_strndup (abfd,
...
static void
tilepro_cgc_append_rela_32 (bfd *abfd, asection *s, Cgc_Internal_Rela *rel)
{
  Cgc32_External_Rela *loc32;

  loc32 = (Cgc32_External_Rela *) s->contents;
  loc32 += s->reloc_count++;
  bfd_cgc32_swap_reloca_out (abfd, rel, (bfd_byte *) loc32);
}
...
#define TILEPRO_CGC_RELA_BYTES (sizeof(Cgc32_External_Rela))


/* Create an entry in an TILEPro CGC linker hash table.  */
		   ...
		   struct bfd_hash_table *table, const char *string)
{
  ...
  if (entry == NULL)
    {
      entry =
        bfd_hash_allocate (table,
                           sizeof (struct tilepro_cgc_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct tilepro_cgc_link_hash_entry *eh;

      eh = (struct tilepro_cgc_link_hash_entry *) entry;
      eh->dyn_relocs = NULL;
...
/* Create a TILEPRO CGC linker hash table.  */

static struct bfd_link_hash_table *
tilepro_cgc_link_hash_table_create (bfd *abfd)
{
  struct tilepro_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct tilepro_cgc_link_hash_table);

  ret = (struct tilepro_cgc_link_hash_table *) bfd_zmalloc (amt);
  if (ret == NULL)
    return NULL;
...
static bfd_boolean
tilepro_cgc_create_got_section (bfd *abfd, struct bfd_link_info *info)
{
  flagword flags;
  asection *s, *s_got;
  ...
  if (bed->want_got_sym)
    {
	 ...
	 table.  */
      h = _bfd_cgc_define_linkage_sym (abfd, info, s_got,
...
static bfd_boolean
tilepro_cgc_create_dynamic_sections (bfd *dynobj,
				     struct bfd_link_info *info)
{
  struct tilepro_cgc_link_hash_table *htab;

  htab = tilepro_cgc_hash_table (info);
  BFD_ASSERT (htab != NULL);

  if (!tilepro_cgc_create_got_section (dynobj, info))
    return FALSE;
...
static void
tilepro_cgc_copy_indirect_symbol (struct bfd_link_info *info,
				  struct cgc_link_hash_entry *dir,
				  struct cgc_link_hash_entry *ind)
{
  struct tilepro_cgc_link_hash_entry *edir, *eind;

  edir = (struct tilepro_cgc_link_hash_entry *) dir;
  eind = (struct tilepro_cgc_link_hash_entry *) ind;

  if (eind->dyn_relocs != NULL)
    {
      if (edir->dyn_relocs != NULL)
	{
	  struct tilepro_cgc_dyn_relocs **pp;
	  struct tilepro_cgc_dyn_relocs *p;

	  /* Add reloc counts against the indirect sym to the direct sym
	     list.  Merge any entries against the same section.  */
	  for (pp = &eind->dyn_relocs; (p = *pp) != NULL; )
	    {
	      struct tilepro_cgc_dyn_relocs *q;
...
static int
tilepro_cgc_tls_transition (struct bfd_link_info *info, int r_type,
			    int is_local)
{
  if (info->shared)
    return r_type;
...
static bfd_boolean
tilepro_cgc_check_relocs (bfd *abfd, struct bfd_link_info *info,
			  asection *sec, const Cgc_Internal_Rela *relocs)
{
  ...
  BFD_ASSERT (is_tilepro_cgc (abfd) || num_relocs == 0);
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      else
	{
      ...
      r_type = tilepro_cgc_tls_transition (info, r_type, h == NULL);
      switch (r_type)
	{
	  ...
	  /* This symbol requires a global offset table entry.  */
	  {
	    ...
	    if (h != NULL)
	      {
		h->got.refcount += 1;
		old_tls_type = tilepro_cgc_hash_entry(h)->tls_type;
	      }
	    else
	      {
		...
		if (local_got_refcounts == NULL)
		  {
		    ...
		    _bfd_tilepro_cgc_local_got_tls_type (abfd)
		      = (char *) (local_got_refcounts + symtab_hdr->sh_info);
		  }
		local_got_refcounts[r_symndx] += 1;
		old_tls_type =
		  _bfd_tilepro_cgc_local_got_tls_type (abfd) [r_symndx];
	      }
	    ...
	    if (old_tls_type != tls_type)
	      {
		if (h != NULL)
		  tilepro_cgc_hash_entry (h)->tls_type = tls_type;
		else
		  _bfd_tilepro_cgc_local_got_tls_type (abfd) [r_symndx] =
		    tls_type;
	      }
	  ...
	  if (htab->cgc.sgot == NULL)
	    {
	      if (!tilepro_cgc_create_got_section (htab->cgc.dynobj, info))
		return FALSE;
	    }
	  break;
	...
	r_tilepro_plt32:
	  ...
	  if ((info->shared
	       ...
	       && (! tilepro_cgc_howto_table[r_type].pc_relative
		   || (h != NULL
		       && (! info->symbolic
	    ...
	    {
	      struct tilepro_cgc_dyn_relocs *p;
	      struct tilepro_cgc_dyn_relocs **head;
	      ...
	      if (sreloc == NULL)
		{
		 ...
		 relocations we need for this symbol.  */
	      if (h != NULL)
		head =
                  &((struct tilepro_cgc_link_hash_entry *) h)->dyn_relocs;
	      else
		{
		  ...
		  head = (struct tilepro_cgc_dyn_relocs **) vpp;
		}
	      ...
	      if (p == NULL || p->sec != sec)
		{
		  ...
		  p = ((struct tilepro_cgc_dyn_relocs *)
		  ...
		  *head = p;
	      ...
	      p->count += 1;
	      if (tilepro_cgc_howto_table[r_type].pc_relative)
		p->pc_count += 1;
	    }
...

static asection *
tilepro_cgc_gc_mark_hook (asection *sec,
                          struct bfd_link_info *info,
...
static bfd_boolean
tilepro_cgc_gc_sweep_hook (bfd *abfd, struct bfd_link_info *info,
			   asection *sec, const Cgc_Internal_Rela *relocs)
{
  struct tilepro_cgc_link_hash_table *htab;
  Cgc_Internal_Shdr *symtab_hdr;
  ...
  BFD_ASSERT (is_tilepro_cgc (abfd) || sec->reloc_count == 0);
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
	  ...
	  while (h->root.type == bfd_link_hash_indirect
		 || h->root.type == bfd_link_hash_warning)
	    h = (struct cgc_link_hash_entry *) h->root.u.i.link;
	  eh = (struct tilepro_cgc_link_hash_entry *) h;
...
static bfd_boolean
tilepro_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h)
{
  struct tilepro_cgc_link_hash_table *htab;
  struct tilepro_cgc_link_hash_entry * eh;
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0 && h->size != 0)
    {
      htab->srelbss->size += TILEPRO_CGC_RELA_BYTES;
      h->needs_copy = 1;
    }
...
allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if (htab->cgc.dynamic_sections_created
      ...
      if (WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, info->shared, h))
	{
	  ...
	  htab->cgc.srelplt->size += TILEPRO_CGC_RELA_BYTES;
	}
      else
	{
	  h->plt.offset = (bfd_vma) -1;
  ...
  if (h->got.refcount > 0
      && !info->shared
      && h->dynindx == -1
      && tilepro_cgc_hash_entry(h)->tls_type == GOT_TLS_IE)
    h->got.offset = (bfd_vma) -1;
  else if (h->got.refcount > 0)
    {
      asection *s;
      bfd_boolean dyn;
      int tls_type = tilepro_cgc_hash_entry(h)->tls_type;
	...
	htab->cgc.srelgot->size += 2 * TILEPRO_CGC_RELA_BYTES;
      else if (WILL_CALL_FINISH_DYNAMIC_SYMBOL (dyn, info->shared, h))
	htab->cgc.srelgot->size += TILEPRO_CGC_RELA_BYTES;
    }
  else
    h->got.offset = (bfd_vma) -1;
  ...
  for (p = eh->dyn_relocs; p != NULL; p = p->next)
    {
      ...
      sreloc->size += p->count * TILEPRO_CGC_RELA_BYTES;
    }
...
static bfd_boolean
tilepro_cgc_omit_section_dynsym (bfd *output_bfd,
				    struct bfd_link_info *info,
				    asection *p)
{
...
#define CGC32_DYNAMIC_INTERPRETER "/lib/ld.so.1"

static bfd_boolean
tilepro_cgc_size_dynamic_sections (bfd *output_bfd,
				      struct bfd_link_info *info)
{
  (void)output_bfd;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (info->executable)
	{
	  ...
	  BFD_ASSERT (s != NULL);
	  s->size = sizeof CGC32_DYNAMIC_INTERPRETER;
	  s->contents = (unsigned char *) CGC32_DYNAMIC_INTERPRETER;
	}
    }
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = cgc_section_data (s)->local_dynrel; p != NULL; p = p->next)
	      ...
	      else if (p->count != 0)
		{
		  ...
		  srel->size += p->count * TILEPRO_CGC_RELA_BYTES;
	...
	continue;
      ...
      local_tls_type = _bfd_tilepro_cgc_local_got_tls_type (ibfd);
      s = htab->cgc.sgot;
      ...
      for (; local_got < end_local_got; ++local_got, ++local_tls_type)
	{
	  if (*local_got > 0)
		...
		srel->size += TILEPRO_CGC_RELA_BYTES;
	    }
	  else
	    *local_got = (bfd_vma) -1;
	}
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 bit relocations have a greater chance of working. */
      if (htab->cgc.sgot->size >= 0x8000
	  && cgc_hash_table (info)->hgot->root.u.def.value == 0)
	cgc_hash_table (info)->hgot->root.u.def.value = 0x8000;
    }

  if (htab->cgc.sgotplt)
    {
      struct cgc_link_hash_entry *got;
      got = cgc_link_hash_lookup (cgc_hash_table (info),
				  "_GLOBAL_OFFSET_TABLE_",
				  FALSE, FALSE, FALSE);
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in tilepro_cgc_finish_dynamic_sections, but we
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (htab->cgc.srelplt->size != 0)
	{
	  ...
	  || !add_dynamic_entry (DT_RELAENT, TILEPRO_CGC_RELA_BYTES))
	return FALSE;
...
/* Relocate an TILEPRO CGC section.
...
static bfd_boolean
tilepro_cgc_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
                              ...
                              asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	...
	continue;
      ...
      if ((unsigned int)r_type >= NELEMS(tilepro_cgc_howto_table))
	{
      ...
      howto = tilepro_cgc_howto_table + r_type;
	...
	case R_TILEPRO_IMM16_X1_TLS_IE_HA:
	  ...
	  if (h == NULL && local_got_offsets)
	    tls_type =
	      _bfd_tilepro_cgc_local_got_tls_type (input_bfd) [r_symndx];
	  else if (h != NULL)
	    tls_type = tilepro_cgc_hash_entry(h)->tls_type;
      ...
      switch (r_type)
	{
		  ...
		  if (info->shared)
		    {
			 ...
			 for the dynamic linker.  */
		      ...
		      outrel.r_offset = (htab->cgc.sgot->output_section->vma
					 ...
					 + off);
		      outrel.r_info = CGC32_R_INFO (0, R_TILEPRO_RELATIVE);
		      outrel.r_addend = relocation;
		      relocation = 0;
		      tilepro_cgc_append_rela_32 (output_bfd, s, &outrel);
		    }
	  ...
	  if ((info->shared
	      ...
	      else
		{
	      ...
	      tilepro_cgc_append_rela_32 (output_bfd, sreloc, &outrel);
	  ...
	  if (info->shared)
		...
		{
		  outrel.r_info = CGC32_R_INFO (0, r_type);
              ...
              switch (r_type)
                {
                  ...
                  case R_TILEPRO_IMM16_X1_TLS_IE_HA:
		    if (need_relocs) {
	              bfd_put_32 (output_bfd, 0, htab->cgc.sgot->contents + off);
	              outrel.r_offset = (htab->cgc.sgot->output_section->vma
				       + htab->cgc.sgot->output_offset + off);
	              outrel.r_addend = 0;
		      if (indx == 0)
		        outrel.r_addend = relocation - dtpoff_base (info);
		      outrel.r_info = CGC32_R_INFO (indx, R_TILEPRO_TLS_TPOFF32);
		      tilepro_cgc_append_rela_32 (output_bfd, htab->cgc.srelgot,
						  &outrel);
                    } else {
		  ...
		  case R_TILEPRO_IMM16_X1_TLS_GD_HA:
		    if (need_relocs) {
 	              outrel.r_offset = (htab->cgc.sgot->output_section->vma
				       + htab->cgc.sgot->output_offset + off);
	              outrel.r_addend = 0;
	              outrel.r_info = CGC32_R_INFO (indx, R_TILEPRO_TLS_DTPMOD32);
	              bfd_put_32 (output_bfd, 0, htab->cgc.sgot->contents + off);
	              tilepro_cgc_append_rela_32 (output_bfd, htab->cgc.srelgot,
						  &outrel);
		      if (indx == 0)
			  ...
			  outrel.r_offset += TILEPRO_BYTES_PER_WORD;
		          tilepro_cgc_append_rela_32 (output_bfd,
						      htab->cgc.srelgot, &outrel);
		        }
                    }
...
static bfd_boolean
tilepro_cgc_finish_dynamic_symbol (bfd *output_bfd,
				   struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h,
				   Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
	 ...
	 to the beginning of the plt.  */
      ...
      rela.r_offset = (sgotplt->output_section->vma
		       ...
		       + r_offset);
      rela.r_addend = 0;
      rela.r_info = CGC32_R_INFO (h->dynindx, R_TILEPRO_JMP_SLOT);

      loc = srela->contents + rela_index * sizeof (Cgc32_External_Rela);
  ...
  if (h->got.offset != (bfd_vma) -1
      && tilepro_cgc_hash_entry(h)->tls_type != GOT_TLS_GD
      && tilepro_cgc_hash_entry(h)->tls_type != GOT_TLS_IE)
    {
      asection *sgot;
      asection *srela;
	  ...
	  && h->def_regular)
	{
	  ...
	  rela.r_info = CGC32_R_INFO (0, R_TILEPRO_RELATIVE);
	...
	{
	  rela.r_info = CGC32_R_INFO (h->dynindx, R_TILEPRO_GLOB_DAT);
	  rela.r_addend = 0;
	}

      bfd_put_32 (output_bfd, 0,
			  sgot->contents + (h->got.offset & ~(bfd_vma) 1));
      tilepro_cgc_append_rela_32 (output_bfd, srela, &rela);
    }
  ...
  if (h->needs_copy)
    {
      ...
      rela.r_info = CGC32_R_INFO (h->dynindx, R_TILEPRO_COPY);
      rela.r_addend = 0;
      tilepro_cgc_append_rela_32 (output_bfd, s, &rela);
    }
...
static bfd_boolean
tilepro_cgc_finish_dynamic_sections (bfd *output_bfd,
                                     struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  if (htab->cgc.sgotplt)
      ...
      if (htab->cgc.sgotplt->size > 0)
	{
	  ...
	  bfd_put_32 (output_bfd, (bfd_vma) 0,
		      htab->cgc.sgotplt->contents + GOT_ENTRY_SIZE);
	}
  ...
  if (htab->cgc.sgot)
      ...
      if (htab->cgc.sgot->size > 0)
	{
	  ...
	  bfd_vma val = (sdyn ?
			 ...
			 0);
	  bfd_put_32 (output_bfd, val, htab->cgc.sgot->contents);
	}
...
static bfd_vma
tilepro_cgc_plt_sym_val (bfd_vma i, const asection *plt,
...
#define CGC_ARCH		bfd_arch_tilepro
#define CGC_TARGET_ID		TILEPRO_CGC_DATA
#define CGC_MACHINE_CODE	EM_TILEPRO
#define CGC_MAXPAGESIZE		0x10000
#define CGC_COMMONPAGESIZE	0x10000

#define TARGET_LITTLE_SYM	bfd_cgc32_tilepro_vec
#define TARGET_LITTLE_NAME	"cgc32-tilepro"

#define cgc_backend_reloc_type_class	     tilepro_reloc_type_class

#define bfd_cgc32_bfd_reloc_name_lookup      tilepro_reloc_name_lookup
#define bfd_cgc32_bfd_link_hash_table_create tilepro_cgc_link_hash_table_create
#define bfd_cgc32_bfd_reloc_type_lookup	     tilepro_reloc_type_lookup

#define cgc_backend_copy_indirect_symbol     tilepro_cgc_copy_indirect_symbol
#define cgc_backend_create_dynamic_sections  tilepro_cgc_create_dynamic_sections
#define cgc_backend_check_relocs	     tilepro_cgc_check_relocs
#define cgc_backend_adjust_dynamic_symbol    tilepro_cgc_adjust_dynamic_symbol
#define cgc_backend_omit_section_dynsym	     tilepro_cgc_omit_section_dynsym
#define cgc_backend_size_dynamic_sections    tilepro_cgc_size_dynamic_sections
#define cgc_backend_relocate_section	     tilepro_cgc_relocate_section
#define cgc_backend_finish_dynamic_symbol    tilepro_cgc_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections  tilepro_cgc_finish_dynamic_sections
#define cgc_backend_gc_mark_hook	     tilepro_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook            tilepro_cgc_gc_sweep_hook
#define cgc_backend_plt_sym_val		     tilepro_cgc_plt_sym_val
#define cgc_info_to_howto_rel                NULL
#define cgc_info_to_howto                    tilepro_info_to_howto_rela
#define cgc_backend_grok_prstatus            tilepro_cgc_grok_prstatus
#define cgc_backend_grok_psinfo              tilepro_cgc_grok_psinfo
#define cgc_backend_additional_program_headers tilepro_additional_program_headers

#define bfd_cgc32_mkobject		     tilepro_cgc_mkobject
...
#define cgc_backend_got_header_size GOT_ENTRY_SIZE
