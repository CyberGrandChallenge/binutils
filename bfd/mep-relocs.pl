...
# include/cgc/mep.h and scan the comments therein.  It will renumber
# the relocs to be sequential (this is needed so that bfd/cgc32-mep.h
# works) if needed.  It will then update the reloc list in bfd/reloc.c
# and the howto, mapping, and apply routines in bfd/cgc32-mep.c.  You
...
while (! -f "include/cgc/mep.h" && ! -f "bfd/reloc.c") {
    chdir "..";
    $pwd = `pwd`;
    if ($pwd !~ m@/.*/@) {
	print STDERR "Cannot find include/cgc/mep.h or bfd/reloc.h\n";
	exit 1;
    }
}
$pwd = `pwd`;
...
printf "Reading include/cgc/mep.h ...\n";
open(MEPH, "include/cgc/mep.h");
open(MEPHO, "> include/cgc/mep.h.new") || die("mep.h.new create: $!");
$val = 0;
while (<MEPH>) {
...
$autogen = "    /* This section generated from bfd/mep-relocs.pl from include/cgc/mep.h.  */\n";

redo_file ("bfd/cgc32-mep.c",
	   "MEPRELOC:HOWTO",
	   $autogen,
	   "MEPRELOC:END",
	   "",
