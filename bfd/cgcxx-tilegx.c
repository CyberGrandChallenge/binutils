/* TILE-Gx-specific support for CGC.
...
#define TILEGX_CGC_WORD_BYTES(htab) \
  ((htab)->bytes_per_word)

/* The size of an external RELA relocation.  */
#define TILEGX_CGC_RELA_BYTES(htab) \
  ((htab)->bytes_per_rela)

/* Both 32-bit and 64-bit tilegx encode this in an identical manner,
   so just take advantage of that.  */
#define TILEGX_CGC_R_TYPE(r_info) \
  ((r_info) & 0xFF)

#define TILEGX_CGC_R_INFO(htab, in_rel, index, type)	\
  ((htab)->r_info (in_rel, index, type))

#define TILEGX_CGC_R_SYMNDX(htab, r_info) \
  ((htab)->r_symndx(r_info))

#define TILEGX_CGC_DTPOFF_RELOC(htab) \
  ((htab)->dtpoff_reloc)

#define TILEGX_CGC_DTPMOD_RELOC(htab) \
  ((htab)->dtpmod_reloc)

#define TILEGX_CGC_TPOFF_RELOC(htab) \
  ((htab)->tpoff_reloc)

#define TILEGX_CGC_PUT_WORD(htab, bfd, val, ptr) \
  ((htab)->put_word (bfd, val, ptr))

/* The name of the dynamic interpreter.  This is put in the .interp
   section.  */

#define CGC64_DYNAMIC_INTERPRETER "/lib/ld.so.1"
#define CGC32_DYNAMIC_INTERPRETER "/lib32/ld.so.1"


static reloc_howto_type tilegx_cgc_howto_table [] =
{
...
static reloc_howto_type tilegx_cgc_howto_table2 [] =
{
...

/* Map BFD reloc types to TILEGX CGC reloc types.  */
...
#define TH_REMAP(bfd, tilegx) \
  { bfd, tilegx, tilegx_cgc_howto_table },
...
#undef TH_REMAP
  ...
  { BFD_RELOC_VTABLE_INHERIT,       R_TILEGX_GNU_VTINHERIT, tilegx_cgc_howto_table2 },
  { BFD_RELOC_VTABLE_ENTRY,         R_TILEGX_GNU_VTENTRY,   tilegx_cgc_howto_table2 },
};
...
struct tilegx_cgc_dyn_relocs
{
  struct tilegx_cgc_dyn_relocs *next;
...
/* TILEGX CGC linker hash entry.  */

struct tilegx_cgc_link_hash_entry
{
  struct cgc_link_hash_entry cgc;

  /* Track dynamic relocs copied for this symbol.  */
  struct tilegx_cgc_dyn_relocs *dyn_relocs;
...
#define tilegx_cgc_hash_entry(ent) \
  ((struct tilegx_cgc_link_hash_entry *)(ent))

struct _bfd_tilegx_cgc_obj_tdata
{
...
#define _bfd_tilegx_cgc_tdata(abfd) \
  ((struct _bfd_tilegx_cgc_obj_tdata *) (abfd)->tdata.any)

#define _bfd_tilegx_cgc_local_got_tls_type(abfd) \
  (_bfd_tilegx_cgc_tdata (abfd)->local_got_tls_type)

#define is_tilegx_cgc(bfd)				\
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour	\
   && cgc_tdata (bfd) != NULL				\
   && cgc_object_id (bfd) == TILEGX_CGC_DATA)

#include "cgc/common.h"
#include "cgc/internal.h"

struct tilegx_cgc_link_hash_table
{
  ...
  int dtpoff_reloc;
  int tpoff_reloc;
  bfd_vma (*r_info) (Cgc_Internal_Rela *, bfd_vma, bfd_vma);
...
/* Get the Tile CGC linker hash table from a link_info structure.  */
#define tilegx_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == TILEGX_CGC_DATA ? ((struct tilegx_cgc_link_hash_table *) ((p)->hash)) : NULL)

#ifdef BFD64
static bfd_vma
tilegx_cgc_r_info_64 (Cgc_Internal_Rela *in_rel ATTRIBUTE_UNUSED,
		      bfd_vma rel_index,
		      bfd_vma type)
{
  return CGC64_R_INFO (rel_index, type);
}

static bfd_vma
tilegx_cgc_r_symndx_64 (bfd_vma r_info)
{
  return CGC64_R_SYM (r_info);
}
...
static bfd_vma
tilegx_cgc_r_info_32 (Cgc_Internal_Rela *in_rel ATTRIBUTE_UNUSED,
		      bfd_vma rel_index,
		      bfd_vma type)
{
...
static bfd_vma
tilegx_cgc_r_symndx_32 (bfd_vma r_info)
{
			  ...
			  const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (tilegx_cgc_howto_table)
            / sizeof (tilegx_cgc_howto_table[0]));
       i++)
    if (tilegx_cgc_howto_table[i].name != NULL
        && strcasecmp (tilegx_cgc_howto_table[i].name, r_name) == 0)
      return &tilegx_cgc_howto_table[i];
...
{
  unsigned int r_type = TILEGX_CGC_R_TYPE (dst->r_info);

  if (r_type <= (unsigned int) R_TILEGX_IMM8_Y1_TLS_ADD)
    cache_ptr->howto = &tilegx_cgc_howto_table [r_type];
  else if (r_type - R_TILEGX_GNU_VTINHERIT
	   <= (unsigned int) R_TILEGX_GNU_VTENTRY)
    cache_ptr->howto
      = &tilegx_cgc_howto_table2 [r_type - R_TILEGX_GNU_VTINHERIT];
  else
    abort ();
}
...
static void
tilegx_cgc_append_rela (bfd *abfd, asection *s, Cgc_Internal_Rela *rel)
{
...
#define GOT_ENTRY_SIZE(htab) TILEGX_CGC_WORD_BYTES (htab)
...
tilegx_plt_entry_build (bfd *output_bfd,
			struct tilegx_cgc_link_hash_table *htab,
...
/* Create an entry in an TILEGX CGC linker hash table.  */
		   ...
		   struct bfd_hash_table *table, const char *string)
{
  ...
  if (entry == NULL)
    {
      entry =
        bfd_hash_allocate (table,
                           sizeof (struct tilegx_cgc_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct tilegx_cgc_link_hash_entry *eh;

      eh = (struct tilegx_cgc_link_hash_entry *) entry;
      eh->dyn_relocs = NULL;
...
/* Create a TILEGX CGC linker hash table.  */

struct bfd_link_hash_table *
tilegx_cgc_link_hash_table_create (bfd *abfd)
{
  struct tilegx_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct tilegx_cgc_link_hash_table);

  ret = (struct tilegx_cgc_link_hash_table *) bfd_zmalloc (amt);
  if (ret == NULL)
    return NULL;
  ...
  if (ABI_64_P (abfd))
    {
      ...
      ret->r_info = tilegx_cgc_r_info_64;
      ret->r_symndx = tilegx_cgc_r_symndx_64;
...
#endif
    {
      ...
      ret->r_info = tilegx_cgc_r_info_32;
      ret->r_symndx = tilegx_cgc_r_symndx_32;
...
static bfd_boolean
tilegx_cgc_create_got_section (bfd *abfd, struct bfd_link_info *info)
{
  flagword flags;
  asection *s, *s_got;
  ...
  if (bed->want_got_plt)
	...
	return FALSE;
      ...
      s->size += GOTPLT_HEADER_SIZE (tilegx_cgc_hash_table (info));
    }
...
bfd_boolean
tilegx_cgc_create_dynamic_sections (bfd *dynobj,
				    struct bfd_link_info *info)
{
  struct tilegx_cgc_link_hash_table *htab;

  htab = tilegx_cgc_hash_table (info);
  BFD_ASSERT (htab != NULL);

  if (!tilegx_cgc_create_got_section (dynobj, info))
    return FALSE;
...
void
tilegx_cgc_copy_indirect_symbol (struct bfd_link_info *info,
				 struct cgc_link_hash_entry *dir,
				 struct cgc_link_hash_entry *ind)
{
  struct tilegx_cgc_link_hash_entry *edir, *eind;

  edir = (struct tilegx_cgc_link_hash_entry *) dir;
  eind = (struct tilegx_cgc_link_hash_entry *) ind;

  if (eind->dyn_relocs != NULL)
    {
      if (edir->dyn_relocs != NULL)
	{
	  struct tilegx_cgc_dyn_relocs **pp;
	  struct tilegx_cgc_dyn_relocs *p;

	  /* Add reloc counts against the indirect sym to the direct sym
	     list.  Merge any entries against the same section.  */
	  for (pp = &eind->dyn_relocs; (p = *pp) != NULL; )
	    {
	      struct tilegx_cgc_dyn_relocs *q;
...
static int
tilegx_cgc_tls_transition (struct bfd_link_info *info, int r_type,
...
bfd_boolean
tilegx_cgc_check_relocs (bfd *abfd, struct bfd_link_info *info,
			 asection *sec, const Cgc_Internal_Rela *relocs)
{
  ...
  BFD_ASSERT (is_tilegx_cgc (abfd) || num_relocs == 0);
  ...
  for (rel = relocs; rel < rel_end && !has_tls_add; rel++)
    {
      int r_type = TILEGX_CGC_R_TYPE (rel->r_info);
      switch (r_type)
	{
  ...
  for (rel = relocs; rel < rel_end; rel++)
    {
      ...
      r_symndx = TILEGX_CGC_R_SYMNDX (htab, rel->r_info);
      r_type = TILEGX_CGC_R_TYPE (rel->r_info);
      ...
      else
	{
      ...
      r_type = tilegx_cgc_tls_transition (info, r_type, h == NULL,
					  sec->sec_flg0);
      switch (r_type)
	{
	...
	case R_TILEGX_IMM16_X1_HW1_LAST_GOT:
	  ...
	  /* This symbol requires a global offset table entry.  */
	  {
	    ...
	    if (h != NULL)
	      {
		h->got.refcount += 1;
		old_tls_type = tilegx_cgc_hash_entry(h)->tls_type;
	      }
	    else
	      {
		...
		if (local_got_refcounts == NULL)
		  {
		    ...
		    cgc_local_got_refcounts (abfd) = local_got_refcounts;
                    _bfd_tilegx_cgc_local_got_tls_type (abfd)
                      = (char *) (local_got_refcounts + symtab_hdr->sh_info);
		  }
		local_got_refcounts[r_symndx] += 1;
                old_tls_type = _bfd_tilegx_cgc_local_got_tls_type (abfd) [r_symndx];
              }
            ...
            if (old_tls_type != tls_type)
              {
                if (h != NULL)
                  tilegx_cgc_hash_entry (h)->tls_type = tls_type;
                else
                  _bfd_tilegx_cgc_local_got_tls_type (abfd) [r_symndx] = tls_type;
	      }
	  }

	  if (htab->cgc.sgot == NULL)
	    {
	      if (!tilegx_cgc_create_got_section (htab->cgc.dynobj, info))
		return FALSE;
	    }
	  break;
	...
	r_tilegx_plt32:
	  ...
	  if ((info->shared
	       ...
	       && (! tilegx_cgc_howto_table[r_type].pc_relative
		   || (h != NULL
		       && (! info->symbolic
	    ...
	    {
	      struct tilegx_cgc_dyn_relocs *p;
	      struct tilegx_cgc_dyn_relocs **head;
	      ...
	      if (sreloc == NULL)
		{
		  sreloc = _bfd_cgc_make_dynamic_reloc_section
		    (sec, htab->cgc.dynobj, htab->word_align_power, abfd,
		     /*rela?*/ TRUE);
		 ...
		 relocations we need for this symbol.  */
	      if (h != NULL)
		head =
                  &((struct tilegx_cgc_link_hash_entry *) h)->dyn_relocs;
	      else
		{
		  ...
		  head = (struct tilegx_cgc_dyn_relocs **) vpp;
		}
	      ...
	      if (p == NULL || p->sec != sec)
		{
		  ...
		  p = ((struct tilegx_cgc_dyn_relocs *)
		  ...
		  *head = p;
	      ...
	      p->count += 1;
	      if (tilegx_cgc_howto_table[r_type].pc_relative)
		p->pc_count += 1;
	    }
...

asection *
tilegx_cgc_gc_mark_hook (asection *sec,
			 struct bfd_link_info *info,
			 ...
			 Cgc_Internal_Sym *sym)
{
  if (h != NULL)
    {
      switch (TILEGX_CGC_R_TYPE (rel->r_info))
	{
	case R_TILEGX_GNU_VTINHERIT:
	case R_TILEGX_GNU_VTENTRY:
	  return NULL;
...
bfd_boolean
tilegx_cgc_gc_sweep_hook (bfd *abfd, struct bfd_link_info *info,
			  asection *sec, const Cgc_Internal_Rela *relocs)
{
  struct tilegx_cgc_link_hash_table *htab;
  Cgc_Internal_Shdr *symtab_hdr;
  ...
  BFD_ASSERT (is_tilegx_cgc (abfd) || sec->reloc_count == 0);
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
	  ...
	  while (h->root.type == bfd_link_hash_indirect
		 || h->root.type == bfd_link_hash_warning)
	    h = (struct cgc_link_hash_entry *) h->root.u.i.link;
	  eh = (struct tilegx_cgc_link_hash_entry *) h;
...
bfd_boolean
tilegx_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h)
{
  struct tilegx_cgc_link_hash_table *htab;
  struct tilegx_cgc_link_hash_entry * eh;
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0 && h->size != 0)
    {
      htab->srelbss->size += TILEGX_CGC_RELA_BYTES (htab);
      h->needs_copy = 1;
    }
...
allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if (htab->cgc.dynamic_sections_created
      ...
      if (WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, info->shared, h))
	{
	  ...
	  htab->cgc.sgotplt->size += GOT_ENTRY_SIZE (htab);

	  /* We also need to make an entry in the .rela.plt section.  */
	  htab->cgc.srelplt->size += TILEGX_CGC_RELA_BYTES (htab);
	}
      else
	{
	  h->plt.offset = (bfd_vma) -1;
  ...
  if (h->got.refcount > 0
      ...
      && !info->shared
      && h->dynindx == -1
      && tilegx_cgc_hash_entry(h)->tls_type == GOT_TLS_IE)
    h->got.offset = (bfd_vma) -1;
  else if (h->got.refcount > 0)
    {
      asection *s;
      bfd_boolean dyn;
      int tls_type = tilegx_cgc_hash_entry(h)->tls_type;
	  ...
	  && !h->forced_local)
	{
      ...
      s = htab->cgc.sgot;
      h->got.offset = s->size;
      s->size += TILEGX_CGC_WORD_BYTES (htab);
	...
	htab->cgc.srelgot->size += 2 * TILEGX_CGC_RELA_BYTES (htab);
      else if (WILL_CALL_FINISH_DYNAMIC_SYMBOL (dyn, info->shared, h))
	htab->cgc.srelgot->size += TILEGX_CGC_RELA_BYTES (htab);
    }
  else
    h->got.offset = (bfd_vma) -1;
  ...
  for (p = eh->dyn_relocs; p != NULL; p = p->next)
    {
      ...
      sreloc->size += p->count * TILEGX_CGC_RELA_BYTES (htab);
    }
...
bfd_boolean
tilegx_cgc_omit_section_dynsym (bfd *output_bfd,
				struct bfd_link_info *info,
				asection *p)
{
...
bfd_boolean
tilegx_cgc_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *info)
{
  struct tilegx_cgc_link_hash_table *htab;
  bfd *dynobj;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = cgc_section_data (s)->local_dynrel; p != NULL; p = p->next)
	      ...
	      else if (p->count != 0)
		{
		  ...
		  srel->size += p->count * TILEGX_CGC_RELA_BYTES (htab);
	...
	continue;
      ...
      local_tls_type = _bfd_tilegx_cgc_local_got_tls_type (ibfd);
      s = htab->cgc.sgot;
      ...
      for (; local_got < end_local_got; ++local_got, ++local_tls_type)
	{
	  if (*local_got > 0)
		...
		srel->size += TILEGX_CGC_RELA_BYTES (htab);
	    }
	  else
	    *local_got = (bfd_vma) -1;
	}
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in tilegx_cgc_finish_dynamic_sections, but we
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (htab->cgc.srelplt->size != 0)
	{
	  ...
	  || !add_dynamic_entry (DT_RELAENT, TILEGX_CGC_RELA_BYTES (htab)))
	return FALSE;
...
/* Relocate an TILEGX CGC section.
...
bfd_boolean
tilegx_cgc_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
			     ...
			     asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	...
	continue;
      ...
      if ((unsigned int)r_type >= ARRAY_SIZE (tilegx_cgc_howto_table))
	{
      ...
      howto = tilegx_cgc_howto_table + r_type;
	...
	case R_TILEGX_IMM8_Y1_TLS_ADD:
	  ...
	  if (h == NULL && local_got_offsets)
	    tls_type =
	      _bfd_tilegx_cgc_local_got_tls_type (input_bfd) [r_symndx];
	  else if (h != NULL)
	    tls_type = tilegx_cgc_hash_entry(h)->tls_type;
	  ...
	  if (h != NULL)
		      ...
		      && SYMBOL_REFERENCES_LOCAL (info, h)))
		{
		  ...
		  if ((off & 1) != 0)
		    ...
		    {
		      TILEGX_CGC_PUT_WORD (htab, output_bfd, relocation,
		  ...
		  if (info->shared)
		    {
			 ...
			 for the dynamic linker.  */
		      ...
		      outrel.r_offset = (htab->cgc.sgot->output_section->vma
					 ...
					 + off);
		      outrel.r_info =
			TILEGX_CGC_R_INFO (htab, NULL, 0, R_TILEGX_RELATIVE);
		      outrel.r_addend = relocation;
		      relocation = 0;
		      tilegx_cgc_append_rela (output_bfd, s, &outrel);
		    }
	...
	case R_TILEGX_IMM16_X1_HW2_LAST:
	  ...
	  if ((info->shared
			   ...
			   || !h->def_regular))
		{
		  ...
		  outrel.r_info = TILEGX_CGC_R_INFO (htab, rel, h->dynindx, r_type);
	      ...
	      else
		{
		  if (r_type == R_TILEGX_32 || r_type == R_TILEGX_64)
		    {
		      outrel.r_info = TILEGX_CGC_R_INFO (htab, NULL, 0,
							 R_TILEGX_RELATIVE);
		      ...
		      else
			{
		      ...
		      outrel.r_info = TILEGX_CGC_R_INFO (htab, rel, indx,
							 r_type);
		    }
		}

	      tilegx_cgc_append_rela (output_bfd, sreloc, &outrel);
	  ...
	  if (info->shared)
		...
		{
		  outrel.r_info = TILEGX_CGC_R_INFO (htab, NULL, 0, r_type);
	  ...
	  if ((off & 1) != 0)
		...
		if (WILL_CALL_FINISH_DYNAMIC_SYMBOL (dyn, info->shared, h)
		  ...
		  case R_TILEGX_IMM16_X1_HW1_LAST_TLS_IE:
		    if (need_relocs) {
		      TILEGX_CGC_PUT_WORD (htab, output_bfd, 0,
					   htab->cgc.sgot->contents + off);
	              outrel.r_offset = (htab->cgc.sgot->output_section->vma
		      ...
		      if (indx == 0)
		        outrel.r_addend = relocation - dtpoff_base (info);
		      outrel.r_info = TILEGX_CGC_R_INFO (htab, NULL, indx,
							 TILEGX_CGC_TPOFF_RELOC (htab));
		      tilegx_cgc_append_rela (output_bfd, htab->cgc.srelgot, &outrel);
                    } else {
		      TILEGX_CGC_PUT_WORD (htab, output_bfd,
		  ...
		  case R_TILEGX_IMM16_X1_HW1_LAST_TLS_GD:
		    if (need_relocs) {
			  ...
			  TILEGX_CGC_PUT_WORD (htab, output_bfd, 0,
						...
						TILEGX_CGC_WORD_BYTES (htab)));
		          outrel.r_info = TILEGX_CGC_R_INFO (htab, NULL, indx,
							     TILEGX_CGC_DTPOFF_RELOC (htab));
		          outrel.r_offset += TILEGX_CGC_WORD_BYTES (htab);
		          tilegx_cgc_append_rela (output_bfd, htab->cgc.srelgot, &outrel);
		        }
                    }
		      ...
		      TILEGX_CGC_PUT_WORD (htab, output_bfd, 1,
					   htab->cgc.sgot->contents + off );
		      TILEGX_CGC_PUT_WORD (htab, output_bfd,
					   relocation - dtpoff_base (info),
					   htab->cgc.sgot->contents + off +
...
bfd_boolean
tilegx_cgc_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h,
				  Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
	 ...
	 to the beginning of the plt.  */
      ...
      rela.r_offset = (sgotplt->output_section->vma
		       ...
		       + r_offset);
      rela.r_addend = 0;
      rela.r_info = TILEGX_CGC_R_INFO (htab, NULL, h->dynindx, R_TILEGX_JMP_SLOT);

      loc = srela->contents + rela_index * TILEGX_CGC_RELA_BYTES (htab);
  ...
  if (h->got.offset != (bfd_vma) -1
      && tilegx_cgc_hash_entry(h)->tls_type != GOT_TLS_GD
      && tilegx_cgc_hash_entry(h)->tls_type != GOT_TLS_IE)
    {
      asection *sgot;
      asection *srela;
	  ...
	  && h->def_regular)
	{
	  ...
	  rela.r_info = TILEGX_CGC_R_INFO (htab, NULL, 0, R_TILEGX_RELATIVE);
	...
	{
	  rela.r_info = TILEGX_CGC_R_INFO (htab, NULL, h->dynindx, R_TILEGX_GLOB_DAT);
	  rela.r_addend = 0;
	}

      TILEGX_CGC_PUT_WORD (htab, output_bfd, 0,
			   sgot->contents + (h->got.offset & ~(bfd_vma) 1));
      tilegx_cgc_append_rela (output_bfd, srela, &rela);
    }
  ...
  if (h->needs_copy)
    {
      ...
      rela.r_info = TILEGX_CGC_R_INFO (htab, NULL, h->dynindx, R_TILEGX_COPY);
      rela.r_addend = 0;
      tilegx_cgc_append_rela (output_bfd, s, &rela);
    }
...
bfd_boolean
tilegx_cgc_finish_dynamic_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  if (htab->cgc.sgotplt)
      ...
      if (htab->cgc.sgotplt->size > 0)
	{
	  /* Write the first two entries in .got.plt, needed for the dynamic
	     linker.  */
	  TILEGX_CGC_PUT_WORD (htab, output_bfd, (bfd_vma) -1,
			       htab->cgc.sgotplt->contents);
	  TILEGX_CGC_PUT_WORD (htab, output_bfd, (bfd_vma) 0,
			       htab->cgc.sgotplt->contents
			       + GOT_ENTRY_SIZE (htab));
	}
  ...
  if (htab->cgc.sgot)
      ...
      if (htab->cgc.sgot->size > 0)
	{
	  ...
	  bfd_vma val = (sdyn ?
			 ...
			 0);
	  TILEGX_CGC_PUT_WORD (htab, output_bfd, val,
...
bfd_vma
tilegx_cgc_plt_sym_val (bfd_vma i, const asection *plt,
...
{
  switch ((int) TILEGX_CGC_R_TYPE (rela->r_info))
    {
    case R_TILEGX_RELATIVE:
...
bfd_boolean
_bfd_tilegx_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
