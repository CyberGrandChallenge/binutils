/* FRV-specific support for 32-bit CGC.
...
static reloc_howto_type cgc32_frv_howto_table [] =
{
...
static reloc_howto_type cgc32_frv_vtinherit_howto =
...
static reloc_howto_type cgc32_frv_vtentry_howto =
...
static reloc_howto_type cgc32_frv_rel_32_howto =
  HOWTO (R_FRV_32,		/* type */
	 0,			/* rightshift */
	 2,			/* size (0 = byte, 1 = short, 2 = long) */
	 32,			/* bitsize */
...
static reloc_howto_type cgc32_frv_rel_funcdesc_howto =
  HOWTO (R_FRV_FUNCDESC,	/* type */
	 0,			/* rightshift */
	 2,			/* size (0 = byte, 1 = short, 2 = long) */
	 32,			/* bitsize */
...
static reloc_howto_type cgc32_frv_rel_funcdesc_value_howto =
  HOWTO (R_FRV_FUNCDESC_VALUE,	/* type */
	 0,			/* rightshift */
	 2,			/* size (0 = byte, 1 = short, 2 = long) */
	 64,			/* bitsize */
...
static reloc_howto_type cgc32_frv_rel_tlsdesc_value_howto =
...
static reloc_howto_type cgc32_frv_rel_tlsoff_howto =
  /* The 32-bit offset from the thread pointer (not the module base
     address) to a thread-local symbol.  */
  HOWTO (R_FRV_TLSOFF,		/* type */
	 0,			/* rightshift */
...

extern const bfd_target bfd_cgc32_frvfdpic_vec;
#define IS_FDPIC(bfd) ((bfd)->xvec == &bfd_cgc32_frvfdpic_vec)

/* An extension of the cgc hash table data structure, containing some
   additional FRV-specific data.  */
struct frvfdpic_cgc_link_hash_table
{
...
/* Get the FRV CGC linker hash table from a link_info structure.  */

#define frvfdpic_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == FRV_CGC_DATA ? ((struct frvfdpic_cgc_link_hash_table *) ((p)->hash)) : NULL)
...
/* Create an FRV CGC linker hash table.  */

static struct bfd_link_hash_table *
frvfdpic_cgc_link_hash_table_create (bfd *abfd)
{
  struct frvfdpic_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct frvfdpic_cgc_link_hash_table);
...
#define FRVFDPIC_FUNCDESC_LOCAL(INFO, H) \
  ((H)->dynindx == -1 || ! cgc_hash_table (INFO)->dynamic_sections_created)
			 ...
			 struct frvfdpic_relocs_info *entry)
{
  ...
  reloc_offset = sreloc->reloc_count * sizeof (Cgc32_External_Rel);
  BFD_ASSERT (reloc_offset < sreloc->size);
  bfd_cgc32_swap_reloc_out (output_bfd, &outrel,
...
{
  Cgc_Internal_Phdr *p = _bfd_cgc_find_segment_containing_section (output_bfd, osec);
...
_frvfdpic_osec_readonly_p (bfd *output_bfd, asection *osec)
{
  ...
  return ! (cgc_tdata (output_bfd)->phdr[seg].p_flags & PF_W);
}
...
tls_biased_base (struct bfd_link_info *info)
{
  ...
  return cgc_hash_table (info)->tls_sec->vma + FRVFDPIC_TLS_BIAS;
}
...

{
  ...
  if (entry->got_entry)
		  ...
		  || FRVFDPIC_SYM_LOCAL (info, entry->d.h)))
	{
	  ...
	  if (sec->output_section && cgc_section_data (sec->output_section))
	    idx = cgc_section_data (sec->output_section)->dynindx;
	  else
	    idx = 0;
	}
	     ...
	     && FRVFDPIC_SYM_LOCAL (info, entry->d.h)))
	{
	  ...
	  if (entry->symndx == -1
	      ...
	      && !(info->executable && !info->pie))
	    {
	      ...
	      idx = cgc_section_data (entry->d.h->root.u.def.section
		...
		BFD_ASSERT (entry->privfd);
	      reloc = R_FRV_32;
	      idx = cgc_section_data (frvfdpic_got_section (info)
				      ->output_section)->dynindx;
	  ...
	  || FRVFDPIC_SYM_LOCAL (info, entry->d.h))
	{
	  ...
	  if (sec)
		...
		ad += sym->st_value;
		  ...
		  && cgc_section_data (sec->output_section))
		idx = cgc_section_data (sec->output_section)->dynindx;
	      else
		idx = 0;
	    }
	  ...
	  if (idx == 0
	      ...
	      if (! cgc_hash_table (info)->tls_sec)
		{
		  (*info->callbacks->undefined_symbol)
		    (info, "TLS section", cgc_hash_table (info)->dynobj,
		     frvfdpic_got_section (info), entry->tlsoff_entry, TRUE);
		  return FALSE;
		}
	      idx = cgc_section_data (cgc_hash_table (info)->tls_sec)->dynindx;
...
cgc32_frv_relocate_gprel12 (struct bfd_link_info *info,
			    bfd *input_bfd,
			    asection *input_section,
			    Cgc_Internal_Rela *relocation,
			    bfd_byte *contents,
			    bfd_vma value)
{
  bfd_vma insn;
...
static bfd_reloc_status_type
cgc32_frv_relocate_gprelu12 (struct bfd_link_info *info,
			     bfd *input_bfd,
			     asection *input_section,
			     Cgc_Internal_Rela *relocation,
			     bfd_byte *contents,
...
/* Handle an FRV CGC HI16 reloc.  */

static bfd_reloc_status_type
cgc32_frv_relocate_hi16 (bfd *input_bfd,
			 Cgc_Internal_Rela *relhi,
			 bfd_byte *contents,
			 bfd_vma value)
{
  bfd_vma insn;
...
static bfd_reloc_status_type
cgc32_frv_relocate_lo16 (bfd *input_bfd,
			 Cgc_Internal_Rela *rello,
			 bfd_byte *contents,
			 bfd_vma value)
{
  bfd_vma insn;
...
static bfd_reloc_status_type
cgc32_frv_relocate_label24 (bfd *input_bfd,
			    asection *input_section,
			    Cgc_Internal_Rela *rello,
			    bfd_byte *contents,
			    bfd_vma value)
...
static bfd_reloc_status_type
cgc32_frv_relocate_gprelhi (struct bfd_link_info *info,
			    bfd *input_bfd,
			    asection *input_section,
			    Cgc_Internal_Rela *relocation,
			    bfd_byte *contents,
...
static bfd_reloc_status_type
cgc32_frv_relocate_gprello (struct bfd_link_info *info,
			    bfd *input_bfd,
			    asection *input_section,
			    Cgc_Internal_Rela *relocation,
			    bfd_byte *contents,
		       ...
		       bfd_reloc_code_real_type code)
{
  switch (code)
    ...
    case BFD_RELOC_NONE:
      return &cgc32_frv_howto_table[ (int) R_FRV_NONE];

    case BFD_RELOC_32:
      if (cgc_cgcheader (abfd)->e_type == ET_EXEC
	  || cgc_cgcheader (abfd)->e_type == ET_DYN)
	return &cgc32_frv_rel_32_howto;
      /* Fall through.  */
    case BFD_RELOC_CTOR:
      return &cgc32_frv_howto_table[ (int) R_FRV_32];

    case BFD_RELOC_FRV_LABEL16:
      return &cgc32_frv_howto_table[ (int) R_FRV_LABEL16];

    case BFD_RELOC_FRV_LABEL24:
      return &cgc32_frv_howto_table[ (int) R_FRV_LABEL24];

    case BFD_RELOC_FRV_LO16:
      return &cgc32_frv_howto_table[ (int) R_FRV_LO16];

    case BFD_RELOC_FRV_HI16:
      return &cgc32_frv_howto_table[ (int) R_FRV_HI16];

    case BFD_RELOC_FRV_GPREL12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GPREL12];

    case BFD_RELOC_FRV_GPRELU12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GPRELU12];

    case BFD_RELOC_FRV_GPREL32:
      return &cgc32_frv_howto_table[ (int) R_FRV_GPREL32];

    case BFD_RELOC_FRV_GPRELHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_GPRELHI];

    case BFD_RELOC_FRV_GPRELLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_GPRELLO];

    case BFD_RELOC_FRV_GOT12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOT12];

    case BFD_RELOC_FRV_GOTHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTHI];

    case BFD_RELOC_FRV_GOTLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTLO];

    case BFD_RELOC_FRV_FUNCDESC:
      if (cgc_cgcheader (abfd)->e_type == ET_EXEC
	  || cgc_cgcheader (abfd)->e_type == ET_DYN)
	return &cgc32_frv_rel_funcdesc_howto;
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC];

    case BFD_RELOC_FRV_FUNCDESC_GOT12:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOT12];

    case BFD_RELOC_FRV_FUNCDESC_GOTHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOTHI];

    case BFD_RELOC_FRV_FUNCDESC_GOTLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOTLO];

    case BFD_RELOC_FRV_FUNCDESC_VALUE:
      if (cgc_cgcheader (abfd)->e_type == ET_EXEC
	  || cgc_cgcheader (abfd)->e_type == ET_DYN)
	return &cgc32_frv_rel_funcdesc_value_howto;
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_VALUE];

    case BFD_RELOC_FRV_FUNCDESC_GOTOFF12:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOTOFF12];

    case BFD_RELOC_FRV_FUNCDESC_GOTOFFHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOTOFFHI];

    case BFD_RELOC_FRV_FUNCDESC_GOTOFFLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_FUNCDESC_GOTOFFLO];

    case BFD_RELOC_FRV_GOTOFF12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTOFF12];

    case BFD_RELOC_FRV_GOTOFFHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTOFFHI];

    case BFD_RELOC_FRV_GOTOFFLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTOFFLO];

    case BFD_RELOC_FRV_GETTLSOFF:
      return &cgc32_frv_howto_table[ (int) R_FRV_GETTLSOFF];

    case BFD_RELOC_FRV_TLSDESC_VALUE:
      if (cgc_cgcheader (abfd)->e_type == ET_EXEC
	  || cgc_cgcheader (abfd)->e_type == ET_DYN)
	return &cgc32_frv_rel_tlsdesc_value_howto;
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSDESC_VALUE];

    case BFD_RELOC_FRV_GOTTLSDESC12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSDESC12];

    case BFD_RELOC_FRV_GOTTLSDESCHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSDESCHI];

    case BFD_RELOC_FRV_GOTTLSDESCLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSDESCLO];

    case BFD_RELOC_FRV_TLSMOFF12:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSMOFF12];

    case BFD_RELOC_FRV_TLSMOFFHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSMOFFHI];

    case BFD_RELOC_FRV_TLSMOFFLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSMOFFLO];

    case BFD_RELOC_FRV_GOTTLSOFF12:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSOFF12];

    case BFD_RELOC_FRV_GOTTLSOFFHI:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSOFFHI];

    case BFD_RELOC_FRV_GOTTLSOFFLO:
      return &cgc32_frv_howto_table[ (int) R_FRV_GOTTLSOFFLO];

    case BFD_RELOC_FRV_TLSOFF:
      if (cgc_cgcheader (abfd)->e_type == ET_EXEC
	  || cgc_cgcheader (abfd)->e_type == ET_DYN)
	return &cgc32_frv_rel_tlsoff_howto;
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSOFF];

    case BFD_RELOC_FRV_TLSDESC_RELAX:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSDESC_RELAX];

    case BFD_RELOC_FRV_GETTLSOFF_RELAX:
      return &cgc32_frv_howto_table[ (int) R_FRV_GETTLSOFF_RELAX];

    case BFD_RELOC_FRV_TLSOFF_RELAX:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSOFF_RELAX];

    case BFD_RELOC_FRV_TLSMOFF:
      return &cgc32_frv_howto_table[ (int) R_FRV_TLSMOFF];

    case BFD_RELOC_VTABLE_INHERIT:
      return &cgc32_frv_vtinherit_howto;

    case BFD_RELOC_VTABLE_ENTRY:
      return &cgc32_frv_vtentry_howto;
    }
...
frv_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (cgc32_frv_howto_table) / sizeof (cgc32_frv_howto_table[0]);
       i++)
    if (cgc32_frv_howto_table[i].name != NULL
	&& strcasecmp (cgc32_frv_howto_table[i].name, r_name) == 0)
      return &cgc32_frv_howto_table[i];

  if (strcasecmp (cgc32_frv_vtinherit_howto.name, r_name) == 0)
    return &cgc32_frv_vtinherit_howto;
  if (strcasecmp (cgc32_frv_vtentry_howto.name, r_name) == 0)
    return &cgc32_frv_vtentry_howto;

  return NULL;
}

/* Set the howto pointer for an FRV CGC reloc.  */
			...
			Cgc_Internal_Rela *dst)
{
  ...
  switch (r_type)
    ...
    case R_FRV_GNU_VTINHERIT:
      cache_ptr->howto = &cgc32_frv_vtinherit_howto;
      break;

    case R_FRV_GNU_VTENTRY:
      cache_ptr->howto = &cgc32_frv_vtentry_howto;
      break;

    default:
      cache_ptr->howto = & cgc32_frv_howto_table [r_type];
      break;
    }
}

/* Set the howto pointer for an FRV CGC REL reloc.  */
static void
frvfdpic_info_to_howto_rel (bfd *abfd ATTRIBUTE_UNUSED,
			    arelent *cache_ptr, Cgc_Internal_Rela *dst)
{
  unsigned int r_type;
  ...
  switch (r_type)
    {
    case R_FRV_32:
      cache_ptr->howto = &cgc32_frv_rel_32_howto;
      break;

    case R_FRV_FUNCDESC:
      cache_ptr->howto = &cgc32_frv_rel_funcdesc_howto;
      break;

    case R_FRV_FUNCDESC_VALUE:
      cache_ptr->howto = &cgc32_frv_rel_funcdesc_value_howto;
      break;

    case R_FRV_TLSDESC_VALUE:
      cache_ptr->howto = &cgc32_frv_rel_tlsdesc_value_howto;
      break;

    case R_FRV_TLSOFF:
      cache_ptr->howto = &cgc32_frv_rel_tlsoff_howto;
      break;
...
static bfd_boolean
cgc32_frv_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
			    struct bfd_link_info *info,
			    bfd *input_bfd,
			    asection *input_section,
			    bfd_byte *contents,
			    ...
			    asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = cgc32_frv_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
	...
	case R_FRV_TLSMOFF:
	  if (h != NULL)
	    ...
	    /* In order to find the entry we created before, we must
	       ...
	       modified by _bfd_cgc_rela_local_sym().  */
  ...
  (LOCAL_EXEC_P ((info), (picrel)) \
   && TLSMOFF_IN_RANGE_FOR_SETLOS_P((value), (info)))
	  ...
	  case R_FRV_GETTLSOFF:
		...
		howto  = cgc32_frv_howto_table + r_type;
	...
	case R_FRV_FUNCDESC:
	  {
		...
		if (h && ! FRVFDPIC_FUNCDESC_LOCAL (info, h)
		...
		else
		  {
		    ...
		    r_type = R_FRV_32;
		    dynindx = cgc_section_data (frvfdpic_got_section (info)
						->output_section)->dynindx;
		...
		if (info->executable && !info->pie
		    && (!h || FRVFDPIC_FUNCDESC_LOCAL (info, h)))
		  {
			...
			offset = _bfd_cgc_section_offset
			  (output_bfd, info,
	...
	case R_FRV_FUNCDESC_VALUE:
	  {
		...
		if (osec && osec->output_section
		  ...
		  dynindx = cgc_section_data (osec->output_section)->dynindx;
		else
		  dynindx = 0;
	      }
		...
		if (IS_FDPIC (input_bfd)
			...
			& (SEC_ALLOC | SEC_LOAD)) == (SEC_ALLOC | SEC_LOAD))
		  {
		    ...
		    if (!h || h->root.type != bfd_link_hash_undefweak)
		      {
			bfd_vma offset = _bfd_cgc_section_offset
			  (output_bfd, info,
			   input_section, rel->r_offset);
      ...
      if (check_segment[0] != check_segment[1] && IS_FDPIC (output_bfd))
	{
	  ...
	  if (!silence_segment_error && (info->shared || info->pie))
	    return FALSE;
	  cgc_cgcheader (output_bfd)->e_flags |= EF_FRV_PIC;
	}
	...
	default:
     ...
     if (r_type == R_FRV_HI16)
       r = cgc32_frv_relocate_hi16 (input_bfd, rel, contents, relocation);

     else if (r_type == R_FRV_LO16)
       r = cgc32_frv_relocate_lo16 (input_bfd, rel, contents, relocation);

     else if (r_type == R_FRV_LABEL24 || r_type == R_FRV_GETTLSOFF)
       r = cgc32_frv_relocate_label24 (input_bfd, input_section, rel,
				       contents, relocation);

     else if (r_type == R_FRV_GPREL12)
       r = cgc32_frv_relocate_gprel12 (info, input_bfd, input_section, rel,
				       contents, relocation);

     else if (r_type == R_FRV_GPRELU12)
       r = cgc32_frv_relocate_gprelu12 (info, input_bfd, input_section, rel,
					contents, relocation);

     else if (r_type == R_FRV_GPRELLO)
       r = cgc32_frv_relocate_gprello (info, input_bfd, input_section, rel,
				       contents, relocation);

     else if (r_type == R_FRV_GPRELHI)
       r = cgc32_frv_relocate_gprelhi (info, input_bfd, input_section, rel,
				       contents, relocation);
...
static asection *
cgc32_frv_gc_mark_hook (asection *sec,
			struct bfd_link_info *info,
			Cgc_Internal_Rela *rel,
			struct cgc_link_hash_entry *h,
			Cgc_Internal_Sym *sym)
...
static bfd_boolean
cgc32_frv_add_symbol_hook (bfd *abfd,
			   struct bfd_link_info *info,
			   Cgc_Internal_Sym *sym,
...
/* Create  a .got section, as well as its additional info field.  This
   ...
   cgclink.c:_bfd_cgc_create_got_section().  */
...
_frv_create_got_section (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (IS_FDPIC (abfd) && ! bfd_cgc_link_record_dynamic_symbol (info, h))
    return FALSE;
  ...
  /* This is mostly copied from
     cgclink.c:_bfd_cgc_create_dynamic_sections().  */
...
static bfd_boolean
cgc32_frvfdpic_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  /* This is mostly copied from
     cgclink.c:_bfd_cgc_create_dynamic_sections().  */
  flagword flags;
				...
				struct _frvfdpic_dynamic_got_info *dinfo)
{
  ...
  entry->plt = entry->call
    ...
    && cgc_hash_table (dinfo->info)->dynamic_sections_created;
      ...
      && ((entry->symndx == -1
	   ...
	   && (entry->d.h->root.type == bfd_link_hash_undefweak
	       || (bfd_vma)(entry->d.h->root.u.def.value
			    ...
			    + 32768) < (bfd_vma)65536))
	  || (entry->symndx != -1
	      && (cgc_hash_table (dinfo->info)->tls_sec->size
	      ...
	      || FRVFDPIC_SYM_LOCAL (dinfo->g.info, entry->d.h)))
	{
	  if ((bfd_signed_vma)entry->addend >= -(1 << (16 - 1))
		 ...
		 close to 32 KiB of TLS anyway.  */
	      && cgc_hash_table (dinfo->g.info)->tls_sec
	      && (cgc_hash_table (dinfo->g.info)->tls_sec->size
...
_frvfdpic_resolve_final_relocs_info (void **entryp, void *p)
{
  ...
  if (entry->symndx == -1)
    {
      struct cgc_link_hash_entry *h = entry->d.h;
			...
			struct _frvfdpic_dynamic_got_plt_info *gpinfop)
{
  ...
  if (frvfdpic_gotrel_section (info))
    ...
    frvfdpic_gotrel_section (info)->size =
      ...
      * get_cgc_backend_data (output_bfd)->s->sizeof_rel;
  else
  ...
  if (get_cgc_backend_data (output_bfd)->want_got_sym)
    cgc_hash_table (info)->hgot->root.u.def.value
      = frvfdpic_got_initial_offset (info);
...
static bfd_boolean
cgc32_frvfdpic_size_dynamic_sections (bfd *output_bfd,
				      struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *s;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
	...
	if (!_bfd_cgc_add_dynamic_entry (info, DT_PLTRELSZ, 0)
	    || !_bfd_cgc_add_dynamic_entry (info, DT_PLTREL, DT_REL)
	    || !_bfd_cgc_add_dynamic_entry (info, DT_JMPREL, 0))
	  return FALSE;

      if (frvfdpic_gotrel_section (info)->size)
	if (!_bfd_cgc_add_dynamic_entry (info, DT_REL, 0)
	    || !_bfd_cgc_add_dynamic_entry (info, DT_RELSZ, 0)
	    || !_bfd_cgc_add_dynamic_entry (info, DT_RELENT,
...
static bfd_boolean
cgc32_frvfdpic_always_size_sections (bfd *output_bfd,
				     struct bfd_link_info *info)
{
  if (!info->relocatable
				  ...
				  bfd_boolean *changed)
{
  ...
  rel = cgc_section_data (sec)->relocs;
  ...
  for (erel = rel + sec->reloc_count; rel < erel; rel++)
    {
      ...
      if (CGC32_R_TYPE (rel->r_info) != R_FRV_32
	  && CGC32_R_TYPE (rel->r_info) != R_FRV_FUNCDESC)
	continue;

      if (_bfd_cgc_section_offset (sec->output_section->owner,
	...
	return FALSE;
      ...
      *changed = TRUE;
      ...
      if (CGC32_R_TYPE (rel->r_info) == R_FRV_32)
	picrel->relocs32--;
      else /* we know (CGC32_R_TYPE (rel->r_info) == R_FRV_FUNCDESC) */
	picrel->relocsfd--;
...
static bfd_boolean
frvfdpic_cgc_discard_info (bfd *ibfd,
			   struct cgc_reloc_cookie *cookie ATTRIBUTE_UNUSED,
			   struct bfd_link_info *info)
{
...
static bfd_boolean
cgc32_frvfdpic_relax_section (bfd *abfd ATTRIBUTE_UNUSED, asection *sec,
  ...
  if (memcmp (frvfdpic_dynamic_got_plt_info (info),
	      &gpinfo.g, sizeof (gpinfo.g)) != 0)
    {
	 ...
	 be a better way to get to it.  */
      if (!_frvfdpic_size_got_plt (cgc_hash_table (info)->tls_sec->owner,
				   &gpinfo))
	return FALSE;
...
static bfd_boolean
cgc32_frv_finish_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc32_frvfdpic_finish_dynamic_sections (bfd *output_bfd,
					struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  if (frvfdpic_got_section (info))
	...
	{
	  struct cgc_link_hash_entry *hgot = cgc_hash_table (info)->hgot;
...
static bfd_boolean
cgc32_frvfdpic_adjust_dynamic_symbol
(struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc32_frvfdpic_finish_dynamic_symbol
...
static bfd_boolean
frvfdpic_cgc_use_relative_eh_frame
...
static bfd_byte
frvfdpic_cgc_encode_eh_address (bfd *abfd,
				struct bfd_link_info *info,
...
static bfd_boolean
cgc32_frv_check_relocs (bfd *abfd,
			struct bfd_link_info *info,
			asection *sec,
...

/* Return the machine subcode from the CGC e_flags header.  */

static int
cgc32_frv_machine (bfd *abfd)
{
  switch (cgc_cgcheader (abfd)->e_flags & EF_FRV_CPU_MASK)
    {
    default:		    break;
...
/* Set the right machine number for a FRV CGC file.  */

static bfd_boolean
cgc32_frv_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_frv, cgc32_frv_machine (abfd));
  return (((cgc_cgcheader (abfd)->e_flags & EF_FRV_FDPIC) != 0)
	  == (IS_FDPIC (abfd)));
}

/* Function to set the CGC flag bits.  */

static bfd_boolean
frv_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
frv_cgc_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
/* Return true if the architecture described by cgc header flag
   EXTENSION is an extension of the architecture described by BASE.  */

static bfd_boolean
frv_cgc_arch_extension_p (flagword base, flagword extension)
{
  if (base == extension)
    return TRUE;
...
static bfd_boolean
cgc32_frvfdpic_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  if (bfd_get_flavour (ibfd) != bfd_target_cgc_flavour
      || bfd_get_flavour (obfd) != bfd_target_cgc_flavour)
    return TRUE;

  if (! frv_cgc_copy_private_bfd_data (ibfd, obfd))
    return FALSE;

  if (! cgc_tdata (ibfd) || ! cgc_tdata (ibfd)->phdr
      || ! cgc_tdata (obfd) || ! cgc_tdata (obfd)->phdr)
    return TRUE;
...
static bfd_boolean
frv_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  ...
  (*_bfd_error_handler) ("old_flags = 0x%.8lx, new_flags = 0x%.8lx, init = %s, filename = %s",
			 old_flags, new_flags, cgc_flags_init (obfd) ? "yes" : "no",
			 bfd_get_filename (ibfd));
#endif

  if (!cgc_flags_init (obfd))			/* First call, no flags set.  */
    {
      cgc_flags_init (obfd) = TRUE;
      old_flags = new_flags;
    }
      ...
      else
	{
	 ...
	 the generic cpu).  */
      ...
      if (frv_cgc_arch_extension_p (new_partial, old_partial))
	;

      else if (frv_cgc_arch_extension_p (old_partial, new_partial))
  ...
  old_partial = cgc_cgcheader (obfd)->e_flags & EF_FRV_CPU_MASK;
  cgc_cgcheader (obfd)->e_flags = old_flags;
  if (old_partial != (old_flags & EF_FRV_CPU_MASK))
    bfd_default_set_arch_mach (obfd, bfd_arch_frv, cgc32_frv_machine (obfd));
...

static bfd_boolean
frv_cgc_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
  flagword flags;
...
static bfd_boolean
cgc32_frv_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  ...
  switch (note->descsz)
	...
	return FALSE;
      ...
      /* The Linux/FRV cgc_prstatus struct is 268 bytes long.  The other
         hardcoded offsets and sizes listed below (and contained within
	 this lexical block) refer to fields in the target's cgc_prstatus
	 struct.  */
      case 268:
...
static bfd_boolean
cgc32_frv_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  switch (note->descsz)
    {
      default:
	return FALSE;
      ...
      /* The Linux/FRV cgc_prpsinfo struct is 124 bytes long.  */
      case 124:
...
#define CGC_ARCH		bfd_arch_frv
#define CGC_TARGET_ID		FRV_CGC_DATA
#define CGC_MACHINE_CODE	EM_CYGNUS_FRV
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM          bfd_cgc32_frv_vec
#define TARGET_BIG_NAME		"cgc32-frv"

#define cgc_info_to_howto			frv_info_to_howto_rela
#define cgc_backend_relocate_section		cgc32_frv_relocate_section
#define cgc_backend_gc_mark_hook		cgc32_frv_gc_mark_hook
#define cgc_backend_check_relocs                cgc32_frv_check_relocs
#define cgc_backend_object_p			cgc32_frv_object_p
#define cgc_backend_add_symbol_hook             cgc32_frv_add_symbol_hook

#define cgc_backend_stack_align			8
#define cgc_backend_can_gc_sections		1
#define cgc_backend_rela_normal			1

#define bfd_cgc32_bfd_reloc_type_lookup		frv_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	frv_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		frv_cgc_set_private_flags
#define bfd_cgc32_bfd_copy_private_bfd_data	frv_cgc_copy_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data	frv_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	frv_cgc_print_private_bfd_data
...
#define cgc_backend_finish_dynamic_sections \
		cgc32_frv_finish_dynamic_sections

#define cgc_backend_grok_prstatus	cgc32_frv_grok_prstatus
#define cgc_backend_grok_psinfo		cgc32_frv_grok_psinfo

#include "cgc32-target.h"

#undef CGC_MAXPAGESIZE
#define CGC_MAXPAGESIZE		0x4000

#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM          bfd_cgc32_frvfdpic_vec
#undef TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-frvfdpic"
#undef	cgc32_bed
#define	cgc32_bed		cgc32_frvfdpic_bed

#undef cgc_info_to_howto_rel
#define cgc_info_to_howto_rel	frvfdpic_info_to_howto_rel

#undef bfd_cgc32_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_create \
		frvfdpic_cgc_link_hash_table_create
#undef cgc_backend_always_size_sections
#define cgc_backend_always_size_sections \
		cgc32_frvfdpic_always_size_sections
#undef bfd_cgc32_bfd_copy_private_bfd_data
#define bfd_cgc32_bfd_copy_private_bfd_data \
		cgc32_frvfdpic_copy_private_bfd_data

#undef cgc_backend_create_dynamic_sections
#define cgc_backend_create_dynamic_sections \
		cgc32_frvfdpic_create_dynamic_sections
#undef cgc_backend_adjust_dynamic_symbol
#define cgc_backend_adjust_dynamic_symbol \
		cgc32_frvfdpic_adjust_dynamic_symbol
#undef cgc_backend_size_dynamic_sections
#define cgc_backend_size_dynamic_sections \
		cgc32_frvfdpic_size_dynamic_sections
#undef bfd_cgc32_bfd_relax_section
#define bfd_cgc32_bfd_relax_section \
  cgc32_frvfdpic_relax_section
#undef cgc_backend_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_symbol \
		cgc32_frvfdpic_finish_dynamic_symbol
#undef cgc_backend_finish_dynamic_sections
#define cgc_backend_finish_dynamic_sections \
		cgc32_frvfdpic_finish_dynamic_sections

#undef cgc_backend_discard_info
#define cgc_backend_discard_info \
		frvfdpic_cgc_discard_info
#undef cgc_backend_can_make_relative_eh_frame
#define cgc_backend_can_make_relative_eh_frame \
		frvfdpic_cgc_use_relative_eh_frame
#undef cgc_backend_can_make_lsda_relative_eh_frame
#define cgc_backend_can_make_lsda_relative_eh_frame \
		frvfdpic_cgc_use_relative_eh_frame
#undef cgc_backend_encode_eh_address
#define cgc_backend_encode_eh_address \
		frvfdpic_cgc_encode_eh_address
...
#define cgc_backend_omit_section_dynsym _frvfdpic_link_omit_section_dynsym
