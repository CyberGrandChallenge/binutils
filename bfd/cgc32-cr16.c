/* BFD back-end for National Semiconductor's CR16 CGC
...
struct cgc32_cr16_link_hash_entry
{
...
static reloc_howto_type cr16_cgc_howto_table[] =
{
...
static bfd_boolean
_bfd_cr16_cgc_create_got_section (bfd * abfd, struct bfd_link_info * info)
{
  flagword   flags;
  asection * s;
...
cgc_cr16_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
                            bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < R_CR16_MAX; i++)
    if (code == cr16_reloc_map[i].bfd_reloc_enum)
      return &cr16_cgc_howto_table[cr16_reloc_map[i].cr16_reloc_type];
...
static reloc_howto_type *
cgc_cr16_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
                            const char *r_name)
{
  unsigned int i;

  for (i = 0; ARRAY_SIZE (cr16_cgc_howto_table); i++)
    if (cr16_cgc_howto_table[i].name != NULL
        && strcasecmp (cr16_cgc_howto_table[i].name, r_name) == 0)
      return cr16_cgc_howto_table + i;
...
static void
cgc_cr16_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *cache_ptr,
                        Cgc_Internal_Rela *dst)
{
  unsigned int r_type = CGC32_R_TYPE (dst->r_info);

  BFD_ASSERT (r_type < (unsigned int) R_CR16_MAX);
  cache_ptr->howto = cr16_cgc_howto_table + r_type;
}
...
static bfd_boolean
cr16_cgc_check_relocs (bfd *abfd, struct bfd_link_info *info, asection *sec,
      ...
      if (dynobj == NULL)
        {
          switch (CGC32_R_TYPE (rel->r_info))
            ...
            case R_CR16_GOTC_REGREL20:
              ...
              if (! _bfd_cr16_cgc_create_got_section (dynobj, info))
                goto fail;
              break;
...
cr16_cgc_final_link_relocate (reloc_howto_type *howto,
                              bfd *input_bfd,
...
static bfd_boolean
cgc32_cr16_relax_delete_bytes (struct bfd_link_info *link_info, bfd *abfd,
      ...
      if (isym->st_shndx == sec_shndx
          ...
          && isym->st_value < toaddr)
        {
          ...
          for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
            {
              unsigned long r_symndx;
              Cgc_Internal_Sym *rsym;
              bfd_vma addsym, subsym;

              /* Skip if not a SWITCH relocation.  */
              if (CGC32_R_TYPE (irel->r_info) != (int) R_CR16_SWITCH8
                  && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_SWITCH16
                  && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_SWITCH32)
                 continue;

              r_symndx = CGC32_R_SYM (irel->r_info);
              rsym = (Cgc_Internal_Sym *) symtab_hdr->contents + r_symndx;
  ...
  for (; sym_hashes < end_hashes; sym_hashes++)
    {
      ...
      if (link_info->wrap_hash != NULL)
        {
          struct cgc_link_hash_entry **cur_sym_hashes;
...
/* Relocate a CR16 CGC section.  */

static bfd_boolean
cgc32_cr16_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
  ...
  for (; rel < relend; rel++)
    {
      ...
      howto = cr16_cgc_howto_table + (r_type);
	...
	RELOC_AGAINST_DISCARDED_SECTION (info, input_bfd, input_section,
      ...
      r = cr16_cgc_final_link_relocate (howto, input_bfd, output_bfd,
                                        input_section,
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses cgc32_cr16_relocate_section.  */

static bfd_byte *
cgc32_cr16_get_relocated_section_contents (bfd *output_bfd,
                                           ...
                                           asymbol **symbols)
{
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      && input_section->reloc_count > 0)
    {
      ...
      if (! cgc32_cr16_relocate_section (output_bfd, link_info, input_bfd,
...
/* Create an entry in an CR16 CGC linker hash table.  */

static struct bfd_hash_entry *
cgc32_cr16_link_hash_newfunc (struct bfd_hash_entry *entry,
                              struct bfd_hash_table *table,
                              const char *string)
{
  struct cgc32_cr16_link_hash_entry *ret =
    (struct cgc32_cr16_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == (struct cgc32_cr16_link_hash_entry *) NULL)
    ret = ((struct cgc32_cr16_link_hash_entry *)
           bfd_hash_allocate (table,
                              sizeof (struct cgc32_cr16_link_hash_entry)));
  if (ret == (struct cgc32_cr16_link_hash_entry *) NULL)
...
/* Create an cr16 CGC linker hash table.  */

static struct bfd_link_hash_table *
cgc32_cr16_link_hash_table_create (bfd *abfd)
{
...
/* The final processing done just before writing out a CR16 CGC object
   file.  This gets the CR16 architecture right based on the machine
   number.  */

static void
_bfd_cr16_cgc_final_write_processing (bfd *abfd,
...
static bfd_boolean
_bfd_cr16_cgc_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_cr16,
                             cgc_cr16_mach (cgc_cgcheader (abfd)->e_flags));
  return TRUE;
}
...
static bfd_boolean
_bfd_cr16_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
cgc32_cr16_relax_section (bfd *abfd, asection *sec,
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      /* If this isn't something that can be relaxed, then ignore
         this reloc.  */
      if (CGC32_R_TYPE (irel->r_info) != (int) R_CR16_DISP16
          && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_DISP24
          && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_IMM32
          && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_IMM20
          && CGC32_R_TYPE (irel->r_info) != (int) R_CR16_IMM16)
        continue;
      ...
      /* Try to turn a 24  branch/call into a 16bit relative
         branch/call.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CR16_DISP24)
        {
          ...
          if ((long) value < 0x10000 && (long) value > -0x10002)
            {
              ...
              if (!cgc32_cr16_relax_delete_bytes (link_info, abfd, sec,
                                                   irel->r_offset + 2, 2))
                goto error_return;
      ...
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CR16_IMM32)
        {
          bfd_vma value = symval;
      ...
      /* Try to turn a 16bit immediate address into a 4bit
         immediate address.  */
      if ((CGC32_R_TYPE (irel->r_info) == (int) R_CR16_IMM20)
          || (CGC32_R_TYPE (irel->r_info) == (int) R_CR16_IMM16))
        {
          bfd_vma value = symval;
          bfd_vma value1 = 0;
...
static asection *
cgc32_cr16_gc_mark_hook (asection *sec,
...
static bfd_boolean
cgc32_cr16_gc_sweep_hook (bfd *abfd ATTRIBUTE_UNUSED,
                          struct bfd_link_info *info ATTRIBUTE_UNUSED,
                          asection *sec ATTRIBUTE_UNUSED,
...
static bfd_boolean
_bfd_cr16_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  flagword   flags;
  asection * s;
  const struct cgc_backend_data * bed = get_cgc_backend_data (abfd);
  ...
  if (! _bfd_cr16_cgc_create_got_section (abfd, info))
    return FALSE;
...
static bfd_boolean
_bfd_cr16_cgc_adjust_dynamic_symbol (struct bfd_link_info * info,
                                     struct cgc_link_hash_entry * h)
{
  bfd * dynobj;
  asection * s;
...
static bfd_boolean
_bfd_cr16_cgc_size_dynamic_sections (bfd * output_bfd,
                                     struct bfd_link_info * info)
{
  bfd * dynobj;
  asection * s;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      /* Add some entries to the .dynamic section.  We fill in the
         values later, in _bfd_cr16_cgc_finish_dynamic_sections,
...
static bfd_boolean
_bfd_cr16_cgc_finish_dynamic_symbol (bfd * output_bfd,
                                     struct bfd_link_info * info,
                                     struct cgc_link_hash_entry * h,
                                     Cgc_Internal_Sym * sym)
{
  ...
  if (h->got.offset != (bfd_vma) -1)
    {
      ...
      if (info->executable
          ...
          && h->def_regular)
        {
          rel.r_info = CGC32_R_INFO (0, R_CR16_GOT_REGREL20);
      ...
      else
        {
          ...
          rel.r_info = CGC32_R_INFO (h->dynindx, R_CR16_GOT_REGREL20);
          rel.r_addend = 0;
        }
...
static bfd_boolean
_bfd_cr16_cgc_finish_dynamic_sections (bfd * output_bfd,
                                       struct bfd_link_info * info)
{
  bfd *      dynobj;
  asection * sgot;
...
bfd_boolean
bfd_cr16_cgc32_create_embedded_relocs (bfd *abfd,
                                       struct bfd_link_info *info,
                                       asection *datasec,
                                       asection *relsec,
                                       char **errmsg)
  ...
  for (irel = internal_relocs; irel < irelend; irel++, p += 8)
    {
      ...
      if (!((CGC32_R_TYPE (irel->r_info) == (int) R_CR16_NUM32a)
          || (CGC32_R_TYPE (irel->r_info) == (int) R_CR16_NUM32)))
        {
...
_bfd_cr16_cgc_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
#define TARGET_LITTLE_SYM                 bfd_cgc32_cr16_vec
#define TARGET_LITTLE_NAME                "cgc32-cr16"
#define CGC_ARCH                          bfd_arch_cr16
#define CGC_MACHINE_CODE                  EM_CR16
#define CGC_MACHINE_ALT1                  EM_CR16_OLD
#define CGC_MAXPAGESIZE                   0x1
#define cgc_symbol_leading_char           '_'

#define bfd_cgc32_bfd_reloc_type_lookup   cgc_cr16_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup   cgc_cr16_reloc_name_lookup
#define cgc_info_to_howto                 cgc_cr16_info_to_howto
#define cgc_info_to_howto_rel             0
#define cgc_backend_relocate_section      cgc32_cr16_relocate_section
#define bfd_cgc32_bfd_relax_section       cgc32_cr16_relax_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
                                cgc32_cr16_get_relocated_section_contents
#define cgc_backend_gc_mark_hook          cgc32_cr16_gc_mark_hook
#define cgc_backend_gc_sweep_hook         cgc32_cr16_gc_sweep_hook
#define cgc_backend_can_gc_sections       1
#define cgc_backend_rela_normal           1
#define cgc_backend_check_relocs          cr16_cgc_check_relocs
/* So we can set bits in e_flags.  */
#define cgc_backend_final_write_processing \
                                 _bfd_cr16_cgc_final_write_processing
#define cgc_backend_object_p     _bfd_cr16_cgc_object_p

#define bfd_cgc32_bfd_merge_private_bfd_data \
                                 _bfd_cr16_cgc_merge_private_bfd_data


#define bfd_cgc32_bfd_link_hash_table_create \
                                  cgc32_cr16_link_hash_table_create

#define cgc_backend_create_dynamic_sections \
                                  _bfd_cr16_cgc_create_dynamic_sections
#define cgc_backend_adjust_dynamic_symbol \
                                  _bfd_cr16_cgc_adjust_dynamic_symbol
#define cgc_backend_size_dynamic_sections \
                                  _bfd_cr16_cgc_size_dynamic_sections
#define cgc_backend_omit_section_dynsym \
      ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_finish_dynamic_symbol \
                                   _bfd_cr16_cgc_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
                                   _bfd_cr16_cgc_finish_dynamic_sections

#define cgc_backend_reloc_type_class   _bfd_cr16_cgc_reloc_type_class
