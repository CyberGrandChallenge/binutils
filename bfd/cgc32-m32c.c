/* M16C/M32C specific support for 32-bit CGC.
...
static bfd_boolean m32c_cgc_relocate_section
  (bfd *, struct bfd_link_info *, bfd *, asection *, bfd_byte *, Cgc_Internal_Rela *, Cgc_Internal_Sym *, asection **);
static bfd_boolean m32c_cgc_check_relocs
  (bfd *, struct bfd_link_info *, asection *, const Cgc_Internal_Rela *);
static bfd_boolean m32c_cgc_relax_delete_bytes (bfd *, asection *, bfd_vma, int);
#ifdef DEBUG
char * m32c_get_reloc (long reloc);
void dump_symtab (bfd *, void *, void *);
#endif
static bfd_boolean m32c_cgc_relax_section
(bfd *abfd, asection *sec, struct bfd_link_info *link_info, bfd_boolean *again);


static reloc_howto_type m32c_cgc_howto_table [] =
{
...

/* Map BFD reloc types to M32C CGC reloc types.  */
     ...
     bfd_reloc_code_real_type code)
{
  ...
  for (i = ARRAY_SIZE (m32c_reloc_map); --i;)
    if (m32c_reloc_map [i].bfd_reloc_val == code)
      return & m32c_cgc_howto_table [m32c_reloc_map[i].m32c_reloc_val];
...
m32c_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (m32c_cgc_howto_table) / sizeof (m32c_cgc_howto_table[0]);
       i++)
    if (m32c_cgc_howto_table[i].name != NULL
	&& strcasecmp (m32c_cgc_howto_table[i].name, r_name) == 0)
      return &m32c_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an M32C CGC reloc.  */
     ...
     Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = & m32c_cgc_howto_table [r_type];
}



/* Relocate an M32C CGC section.
     ...
     asection **             local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = m32c_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
      relocation = 0;
	...
	case R_M32C_HI16:
      ...
      printf ("relocate %s at %06lx relocation %06lx addend %ld  ",
	      m32c_cgc_howto_table[CGC32_R_TYPE(rel->r_info)].name,
...
static bfd_boolean
m32c_cgc_finish_dynamic_sections (bfd *abfd ATTRIBUTE_UNUSED,
                                  struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *splt;
...
static bfd_boolean
m32c_cgc_always_size_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
                               struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *splt;
...
static bfd_boolean
m32c_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
m32c_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...

static bfd_boolean
m32c_cgc_print_private_bfd_data (bfd *abfd, void *ptr)
{
  FILE *file = (FILE *) ptr;
  flagword flags;
...
static int
cgc32_m32c_machine (bfd *abfd)
{
  switch (cgc_cgcheader (abfd)->e_flags & EF_M32C_CPU_MASK)
    {
...
static bfd_boolean
m32c_cgc_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_m32c,
			     cgc32_m32c_machine (abfd));
  return TRUE;
}
 
...
m32c_get_reloc (long reloc)
{
  if (0 <= reloc && reloc < R_M32C_max)
    return m32c_cgc_howto_table[reloc].name;
  else
    return "";
}
#endif /* DEBUG */
...
/* A subroutine of m32c_cgc_relax_section.  If the global symbol H
...
static bfd_boolean
m32c_relax_plt_check (struct cgc_link_hash_entry *h, void * xdata)
{
...
static bfd_boolean
m32c_relax_plt_realloc (struct cgc_link_hash_entry *h, void * xdata)
{
...
static bfd_boolean
m32c_cgc_relax_plt_section (asection *splt,
                            struct bfd_link_info *info,
                            bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  cgc_link_hash_traverse (cgc_hash_table (info), m32c_relax_plt_check,
			  &relax_plt_data);
...
{
  const Cgc_Internal_Rela *i1 = (const Cgc_Internal_Rela *) e1;
  const Cgc_Internal_Rela *i2 = (const Cgc_Internal_Rela *) e2;
  ...
  if (CGC32_R_SYM (rel->r_info) < symtab_hdr->sh_info)
    {
      ...
      ssec = bfd_section_from_cgc_index (abfd, isym->st_shndx);
     ...
     bfd_boolean *          again)
{
  ...
  Cgc_Internal_Rela *irel, *irelend, *srel;
  ...
  if (abfd == cgc_hash_table (link_info)->dynobj
      && (sec->flags & SEC_LINKER_CREATED) != 0
      && strcmp (sec->name, ".plt") == 0)
    return m32c_cgc_relax_plt_section (sec, link_info, again);

  /* Assume nothing changes.  */
  *again = FALSE;

  machine = cgc32_m32c_machine (abfd);
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      if (CGC32_R_TYPE(irel->r_info) != R_M32C_RL_JUMP
	  && CGC32_R_TYPE(irel->r_info) != R_M32C_RL_1ADDR
	  && CGC32_R_TYPE(irel->r_info) != R_M32C_RL_2ADDR)
	continue;
	 ...
	 shrunk".  */
      gap_size = 0;
      gap = NULL;
      new_type = CGC32_R_TYPE(srel->r_info);
	...
	continue;
      ...
      *again = TRUE;
      ...
      srel->r_info = CGC32_R_INFO (CGC32_R_SYM (srel->r_info), new_type);
      ...
      if (! m32c_cgc_relax_delete_bytes(abfd, sec, gap - contents, gap_size))
	goto error_return;
  ...
  int        count)
{
  ...
  Cgc_Internal_Sym *isymend;
  Cgc_Internal_Sym *intsyms;
  Cgc_External_Sym_Shndx *shndx_buf;
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel ++)
	  ...
	  && irel->r_offset + 7 > addr)
	{
	  ...
	  if (cgc32_m32c_machine (abfd) == bfd_mach_m16c)
	    {
	      if ((*insn & 0xf8) != 0x60)
		continue;
	      disp = (disp & 7);
  ...
  shndx_hdr  = & cgc_tdata (abfd)->symtab_shndx_hdr;
  shndx_buf  = (Cgc_External_Sym_Shndx *) shndx_hdr->contents;
  shndx = shndx_buf;
...
static unsigned int
_bfd_m32c_cgc_eh_frame_address_size (bfd *abfd, asection *sec ATTRIBUTE_UNUSED)
{
  if ((cgc_cgcheader (abfd)->e_flags & EF_M32C_CPU_MASK) == EF_M32C_CPU_M16C)
    return 2;
  return 4;
}
...
#define CGC_ARCH		bfd_arch_m32c
#define CGC_MACHINE_CODE	EM_M32C
#define CGC_MACHINE_ALT1	EM_M32C_OLD
#define CGC_MAXPAGESIZE		0x100

#if 0
#define TARGET_BIG_SYM		bfd_cgc32_m32c_vec
#define TARGET_BIG_NAME		"cgc32-m32c"
#else
#define TARGET_LITTLE_SYM		bfd_cgc32_m32c_vec
#define TARGET_LITTLE_NAME		"cgc32-m32c"
#endif

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			m32c_info_to_howto_rela
#define cgc_backend_object_p			m32c_cgc_object_p
#define cgc_backend_relocate_section		m32c_cgc_relocate_section
#define cgc_backend_check_relocs                m32c_cgc_check_relocs
#define cgc_backend_object_p			m32c_cgc_object_p
#define cgc_symbol_leading_char                 ('_')
#define cgc_backend_always_size_sections \
  m32c_cgc_always_size_sections
#define cgc_backend_finish_dynamic_sections \
  m32c_cgc_finish_dynamic_sections

#define cgc_backend_can_gc_sections		1
#define cgc_backend_eh_frame_address_size _bfd_m32c_cgc_eh_frame_address_size

#define bfd_cgc32_bfd_reloc_type_lookup		m32c_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	m32c_reloc_name_lookup
#define bfd_cgc32_bfd_relax_section		m32c_cgc_relax_section
#define bfd_cgc32_bfd_set_private_flags		m32c_cgc_set_private_flags
#define bfd_cgc32_bfd_merge_private_bfd_data	m32c_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	m32c_cgc_print_private_bfd_data
