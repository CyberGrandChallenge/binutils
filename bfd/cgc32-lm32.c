/* Lattice Mico32-specific support for 32-bit CGC
...
#define IS_FDPIC(bfd) ((bfd)->xvec == &bfd_cgc32_lm32fdpic_vec)

static bfd_reloc_status_type lm32_cgc_gprel_reloc
...
struct cgc_lm32_dyn_relocs
{
  struct cgc_lm32_dyn_relocs *next;
...
/* lm32 CGC linker hash entry.  */

struct cgc_lm32_link_hash_entry
{
  struct cgc_link_hash_entry root;

  /* Track dynamic relocs copied for this symbol.  */
  struct cgc_lm32_dyn_relocs *dyn_relocs;
};

/* lm32 CGC linker hash table.  */

struct cgc_lm32_link_hash_table
{
...
/* Get the lm32 CGC linker hash table from a link_info structure.  */

#define lm32_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == LM32_CGC_DATA ? ((struct cgc_lm32_link_hash_table *) ((p)->hash)) : NULL)

#define lm32fdpic_got_section(info) \
  (lm32_cgc_hash_table (info)->sgot)
#define lm32fdpic_gotrel_section(info) \
  (lm32_cgc_hash_table (info)->srelgot)
#define lm32fdpic_fixup32_section(info) \
  (lm32_cgc_hash_table (info)->sfixup32)
...
/* Create an entry in an lm32 CGC linker hash table.  */

static struct bfd_hash_entry *
lm32_cgc_link_hash_newfunc (struct bfd_hash_entry *entry,
			    struct bfd_hash_table *table,
			    const char *string)
{
  struct cgc_lm32_link_hash_entry *ret =
    (struct cgc_lm32_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == NULL)
    ret = bfd_hash_allocate (table,
			     sizeof (struct cgc_lm32_link_hash_entry));
  if (ret == NULL)
    return NULL;

  /* Call the allocation method of the superclass.  */
  ret = ((struct cgc_lm32_link_hash_entry *)
         _bfd_cgc_link_hash_newfunc ((struct bfd_hash_entry *) ret,
                                     table, string));
  if (ret != NULL)
    {
      struct cgc_lm32_link_hash_entry *eh;

      eh = (struct cgc_lm32_link_hash_entry *) ret;
      eh->dyn_relocs = NULL;
    }
...
/* Create an lm32 CGC linker hash table.  */

static struct bfd_link_hash_table *
lm32_cgc_link_hash_table_create (bfd *abfd)
{
  struct cgc_lm32_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc_lm32_link_hash_table);
...
{
  struct cgc_lm32_link_hash_table *htab;
  asection *s;
...
static reloc_howto_type lm32_cgc_howto_table [] =
{
  ...
  HOWTO (R_LM32_GPREL16,            /* type */
         ...
         0,                         /* bitpos */
         complain_overflow_dont,    /* complain_on_overflow */
         lm32_cgc_gprel_reloc,      /* special_function */
         "R_LM32_GPREL16",          /* name */
         FALSE,                     /* partial_inplace */
         0,                         /* src_mask */
         0xffff,                    /* dst_mask */
...
/* Map BFD reloc types to lm32 CGC reloc types. */
                        ...
                        bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < sizeof (lm32_reloc_map) / sizeof (lm32_reloc_map[0]); i++)
    if (lm32_reloc_map[i].bfd_reloc_val == code)
      return &lm32_cgc_howto_table[lm32_reloc_map[i].cgc_reloc_val];
  return NULL;
}
			...
			const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (lm32_cgc_howto_table) / sizeof (lm32_cgc_howto_table[0]);
       i++)
    if (lm32_cgc_howto_table[i].name != NULL
	&& strcasecmp (lm32_cgc_howto_table[i].name, r_name) == 0)
      return &lm32_cgc_howto_table[i];

  return NULL;
}


/* Set the howto pointer for an Lattice Mico32 CGC reloc.  */
                         ...
                         Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = &lm32_cgc_howto_table[r_type];
}

/* Set the right machine number for an Lattice Mico32 CGC file. */

static bfd_boolean
lm32_cgc_object_p (bfd *abfd)
{
...
static void
lm32_cgc_final_write_processing (bfd *abfd, bfd_boolean linker ATTRIBUTE_UNUSED)
{
  cgc_cgcheader (abfd)->e_machine = EM_LATTICEMICO32;
  cgc_cgcheader (abfd)->e_flags &=~ EF_LM32_MACH;
  switch (bfd_get_mach (abfd))
    {
      case bfd_mach_lm32:
        cgc_cgcheader (abfd)->e_flags |= E_LM32_MACH;
        break;
      default:
        abort ();
    }
...
static bfd_boolean
lm32_cgc_assign_gp (bfd *output_bfd, bfd_vma *pgp)
{
  unsigned int count;
  asymbol **sym;
  unsigned int i;
...
lm32_cgc_final_gp (bfd *output_bfd, asymbol *symbol, bfd_boolean relocatable,
  ...
  if (*pgp == 0 && (!relocatable || (symbol->flags & BSF_SECTION_SYM) != 0))
      ...
      if (relocatable)
	{
	  ...
	  *pgp = symbol->section->output_section->vma + 0x4000;
      ...
      else if (!lm32_cgc_assign_gp (output_bfd, pgp))
	{
	  *error_message =
	    (char *)
...
static bfd_reloc_status_type
lm32_cgc_do_gprel_relocate (bfd *abfd,
		      ...
		      char **msg)
{
  ...
  if ((r =
       lm32_cgc_final_gp (abfd, symbol, FALSE, msg, &gp)) == bfd_reloc_ok)
    {
	...
	{
	  r = lm32_cgc_do_gprel_relocate (abfd, reloc_entry->howto,
					     input_section,
...
_lm32fdpic_osec_to_segment (bfd *output_bfd, asection *osec)
{
  ...
  for (m = cgc_seg_map (output_bfd), p = cgc_tdata (output_bfd)->phdr;
       m != NULL;
       m = m->next, p++)
    {
      int i;
  ...
  return p - cgc_tdata (output_bfd)->phdr;
}
...
static bfd_boolean
lm32_cgc_relocate_section (bfd *output_bfd,
                           ...
                           asection **local_sections)
{
  ...
  struct cgc_lm32_link_hash_table *htab = lm32_cgc_hash_table (info);
  bfd *dynobj;
  ...
  for (; rel < relend; rel++)
    {
      ...
      howto = lm32_cgc_howto_table + r_type;
	  ...
	  r = bfd_reloc_ok;
      ...
      else
        {
          switch (howto->type)
            {
            case R_LM32_GPREL16:
              if (!lm32_cgc_assign_gp (output_bfd, &gp))
                      ...
                      if (info->shared)
                        {
                          ...
                          outrel.r_offset = (sgot->output_section->vma
                                             + sgot->output_offset
                                             + off);
                          outrel.r_info = CGC32_R_INFO (0, R_LM32_RELATIVE);
...
static asection *
lm32_cgc_gc_mark_hook (asection *sec,
                       struct bfd_link_info *info,
...
static bfd_boolean
lm32_cgc_gc_sweep_hook (bfd *abfd,
...
static bfd_boolean
lm32_cgc_check_relocs (bfd *abfd,
                       struct bfd_link_info *info,
                       asection *sec,
...
static bfd_boolean
lm32_cgc_finish_dynamic_sections (bfd *output_bfd,
				  struct bfd_link_info *info)
{
  struct cgc_lm32_link_hash_table *htab;
  bfd *dynobj;
  ...
  if (sgot && sgot->size > 0)
    {
      ...
      if (cgc_section_data (sgot->output_section) != NULL)
...
static bfd_boolean
lm32_cgc_finish_dynamic_symbol (bfd *output_bfd,
				struct bfd_link_info *info,
				struct cgc_link_hash_entry *h,
				Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
      ...
      rela.r_offset = (sgot->output_section->vma
                       + sgot->output_offset
                       + got_offset);
      rela.r_info = CGC32_R_INFO (h->dynindx, R_LM32_JMP_SLOT);
      rela.r_addend = 0;
      loc = srela->contents;
  ...
  if (h->got.offset != (bfd_vma) -1)
    {
      ...
      if (info->shared
          ...
          && h->def_regular)
        {
          rela.r_info = CGC32_R_INFO (0, R_LM32_RELATIVE);
	  ...
	  BFD_ASSERT ((h->got.offset & 1) == 0);
          ...
          rela.r_info = CGC32_R_INFO (h->dynindx, R_LM32_GLOB_DAT);
          rela.r_addend = 0;
        }
  ...
  if (h->needs_copy)
    {
      ...
      rela.r_offset = (h->root.u.def.value
                       + h->root.u.def.section->output_section->vma
                       + h->root.u.def.section->output_offset);
      rela.r_info = CGC32_R_INFO (h->dynindx, R_LM32_COPY);
      rela.r_addend = 0;
      loc = s->contents;
...
lm32_cgc_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
lm32_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
				struct cgc_link_hash_entry *h)
{
  struct cgc_lm32_link_hash_table *htab;
  struct cgc_lm32_link_hash_entry *eh;
  struct cgc_lm32_dyn_relocs *p;
  bfd *dynobj;
  asection *s;
  ...
  eh = (struct cgc_lm32_link_hash_entry *) h;
...
static bfd_boolean
lm32_cgc_size_dynamic_sections (bfd *output_bfd,
                                struct bfd_link_info *info)
{
  struct cgc_lm32_link_hash_table *htab;
  bfd *dynobj;
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
        {
          ...
          for (p = ((struct cgc_lm32_dyn_relocs *)
  ...
  if (htab->root.dynamic_sections_created)
    {
	 ...
	 values later, in lm32_cgc_finish_dynamic_sections, but we
              ...
              if (s->reloc_count)
                {
                  ...
                  Cgc_Internal_Rela *internal_relocs, *end;
                  internal_relocs = cgc_section_data (s)->relocs;
                  if (internal_relocs == NULL)
                    internal_relocs = (_bfd_cgc_link_read_relocs (ibfd, s, NULL, NULL, FALSE));
                  if (internal_relocs != NULL)
                    {
                      ...
                      while (internal_relocs < end)
                        {
                          ...
                          r_symndx = CGC32_R_SYM (internal_relocs->r_info);
                          h = NULL;
                          if (r_symndx < symtab_hdr->sh_info)
                            {
                            }
                          ...
                          if (!h || (h && h->root.type != bfd_link_hash_undefweak))
                            {
                              if (!discarded_section (s) && !((bfd_get_section_flags (ibfd, s) & SEC_ALLOC) == 0))
                                {
                                  switch (CGC32_R_TYPE (internal_relocs->r_info))
                                    {
                                    case R_LM32_32:
                                      r32_count++;
                                      break;
                              ...
                              if (!current && !discarded_section (s) && (bfd_get_section_flags (ibfd, s) & SEC_ALLOC))
                                {
                                  ...
                                  if (CGC32_R_TYPE (internal_relocs->r_info) == R_LM32_16_GOT)
                                    {
                                      /* Create a new entry.  */
...
static bfd_boolean
lm32_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_lm32_link_hash_table *htab;
...
static void
lm32_cgc_copy_indirect_symbol (struct bfd_link_info *info,
                               struct cgc_link_hash_entry *dir,
                               struct cgc_link_hash_entry *ind)
{
  struct cgc_lm32_link_hash_entry * edir;
  struct cgc_lm32_link_hash_entry * eind;

  edir = (struct cgc_lm32_link_hash_entry *) dir;
  eind = (struct cgc_lm32_link_hash_entry *) ind;
...
static bfd_boolean
lm32_cgc_always_size_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  if (!info->relocatable)
    {
      if (!bfd_cgc_stack_segment_size (output_bfd, info,
...
static bfd_boolean
lm32_cgc_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  if (bfd_get_flavour (ibfd) != bfd_target_cgc_flavour
...
static bfd_boolean
lm32_cgc_fdpic_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  unsigned i;
  ...
  if (! lm32_cgc_copy_private_bfd_data (ibfd, obfd))
    return FALSE;
  ...
  /* Copy the stack size.  */
  for (i = 0; i < cgc_cgcheader (ibfd)->e_phnum; i++)
    if (cgc_tdata (ibfd)->phdr[i].p_type == PT_GNU_STACK)
      {
	Cgc_Internal_Phdr *iphdr = &cgc_tdata (ibfd)->phdr[i];

	for (i = 0; i < cgc_cgcheader (obfd)->e_phnum; i++)
	  if (cgc_tdata (obfd)->phdr[i].p_type == PT_GNU_STACK)
	    {
	      memcpy (&cgc_tdata (obfd)->phdr[i], iphdr, sizeof (*iphdr));

	      /* Rewrite the phdrs, since we're only called after they were first written.  */
	      if (bfd_seek (obfd, (bfd_signed_vma) get_cgc_backend_data (obfd)
			    ->s->sizeof_ehdr, SEEK_SET) != 0
		  || get_cgc_backend_data (obfd)->s->write_out_phdrs (obfd, cgc_tdata (obfd)->phdr,
				     cgc_cgcheader (obfd)->e_phnum) != 0)
		return FALSE;
	      break;
	    }
...
#define CGC_ARCH                bfd_arch_lm32
#define CGC_TARGET_ID		LM32_CGC_DATA
#define CGC_MACHINE_CODE        EM_LATTICEMICO32
#define CGC_MAXPAGESIZE         0x1000

#define TARGET_BIG_SYM          bfd_cgc32_lm32_vec
#define TARGET_BIG_NAME         "cgc32-lm32"

#define bfd_cgc32_bfd_reloc_type_lookup         lm32_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup         lm32_reloc_name_lookup
#define cgc_info_to_howto                       lm32_info_to_howto_rela
#define cgc_info_to_howto_rel                   0
#define cgc_backend_rela_normal                 1
#define cgc_backend_object_p                    lm32_cgc_object_p
#define cgc_backend_final_write_processing      lm32_cgc_final_write_processing
#define cgc_backend_stack_align			8
#define cgc_backend_can_gc_sections             1
#define cgc_backend_can_refcount                1
#define cgc_backend_gc_mark_hook                lm32_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook               lm32_cgc_gc_sweep_hook
...
#define bfd_cgc32_bfd_link_hash_table_create    lm32_cgc_link_hash_table_create
#define cgc_backend_check_relocs                lm32_cgc_check_relocs
#define cgc_backend_reloc_type_class            lm32_cgc_reloc_type_class
#define cgc_backend_copy_indirect_symbol        lm32_cgc_copy_indirect_symbol
#define cgc_backend_size_dynamic_sections       lm32_cgc_size_dynamic_sections
#define cgc_backend_omit_section_dynsym         ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_create_dynamic_sections     lm32_cgc_create_dynamic_sections
#define cgc_backend_finish_dynamic_sections     lm32_cgc_finish_dynamic_sections
#define cgc_backend_adjust_dynamic_symbol       lm32_cgc_adjust_dynamic_symbol
#define cgc_backend_finish_dynamic_symbol       lm32_cgc_finish_dynamic_symbol
#define cgc_backend_relocate_section            lm32_cgc_relocate_section
...
#define TARGET_BIG_SYM          bfd_cgc32_lm32fdpic_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-lm32fdpic"
#undef	cgc32_bed
#define	cgc32_bed		cgc32_lm32fdpic_bed

#undef  cgc_backend_always_size_sections
#define cgc_backend_always_size_sections        lm32_cgc_always_size_sections
#undef  bfd_cgc32_bfd_copy_private_bfd_data
#define bfd_cgc32_bfd_copy_private_bfd_data     lm32_cgc_fdpic_copy_private_bfd_data
