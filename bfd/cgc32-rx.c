/* Renesas RX specific support for 32-bit CGC.
...
/* This is a meta-target that's used only with objcopy, to avoid the
   ...
   rx_cgc_object_p().  */
const bfd_target bfd_cgc32_rx_be_ns_vec;
const bfd_target bfd_cgc32_rx_be_vec;
...
#define RXREL(n,sz,bit,shift,complain,pcrel)				     \
  HOWTO (R_RX_##n, shift, sz, bit, pcrel, 0, complain_overflow_ ## complain, \
	 bfd_cgc_generic_reloc, "R_RX_" #n, FALSE, 0, ~0, FALSE)
...
static reloc_howto_type rx_cgc_howto_table [] =
{
...

/* Map BFD reloc types to RX CGC reloc types.  */
		      ...
		      bfd_reloc_code_real_type code)
{
  ...
  if (code == BFD_RELOC_RX_32_OP)
    return rx_cgc_howto_table + R_RX_DIR32;

  for (i = ARRAY_SIZE (rx_reloc_map); --i;)
    if (rx_reloc_map [i].bfd_reloc_val == code)
      return rx_cgc_howto_table + rx_reloc_map[i].rx_reloc_val;
...
rx_reloc_name_lookup (bfd * abfd ATTRIBUTE_UNUSED, const char * r_name)
{
  ...
  for (i = 0; i < ARRAY_SIZE (rx_cgc_howto_table); i++)
    if (rx_cgc_howto_table[i].name != NULL
	&& strcasecmp (rx_cgc_howto_table[i].name, r_name) == 0)
      return rx_cgc_howto_table + i;

  return NULL;
}

/* Set the howto pointer for an RX CGC reloc.  */
		       ...
		       Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = rx_cgc_howto_table + r_type;
}

static bfd_vma
     ...
     asection **             local_sections)
{
  ...
  if (cgc_cgcheader (output_bfd)->e_flags & E_FLAG_RX_PID)
    pid_mode = TRUE;
  else
    pid_mode = FALSE;
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = rx_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
      relocation = 0;
...
static bfd_boolean
cgc32_rx_relax_delete_bytes (bfd *abfd, asection *sec, bfd_vma addr, int count,
			     Cgc_Internal_Rela *alignment_rel, int force_snip)
{
  Cgc_Internal_Shdr * symtab_hdr;
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
	...
	irel->r_offset -= count;
	 ...
	 they're for.  */
	  ...
	  && CGC32_R_TYPE (irel->r_info) == R_RX_RH_RELAX
...
static void
reloc_bubblesort (Cgc_Internal_Rela * r, int count)
{
  int i;
  bfd_boolean again;
  bfd_boolean swappit;
  ...
  while (again)
      ...
      for (i = 0; i < count - 1; i ++)
	{
	  if (r[i].r_offset > r[i + 1].r_offset)
	    ...
	    swappit = FALSE;
	  else if (CGC32_R_TYPE (r[i + 1].r_info) == R_RX_RH_RELAX
		   && (r[i + 1].r_addend & RX_RELAXA_ALIGN))
	    swappit = TRUE;
	  else if (CGC32_R_TYPE (r[i + 1].r_info) == R_RX_RH_RELAX
		   && (r[i + 1].r_addend & RX_RELAXA_ELIGN)
		   && !(CGC32_R_TYPE (r[i].r_info) == R_RX_RH_RELAX
...
rx_offset_for_reloc (bfd *                    abfd,
		     Cgc_Internal_Rela *      rel,
		     Cgc_Internal_Shdr *      symtab_hdr,
		     Cgc_External_Sym_Shndx * shndx_buf ATTRIBUTE_UNUSED,
		     Cgc_Internal_Sym *       intsyms,
		     Cgc_Internal_Rela **     lrel,
		     bfd *                    input_bfd,
		     asection *               input_section,
		     struct bfd_link_info *   info,
		     int *                    scale)
  ...
  while (1)
    {
      ...
      if (CGC32_R_SYM (rel->r_info) < symtab_hdr->sh_info)
	{
	  /* A local symbol.  */
	  Cgc_Internal_Sym *isym;
	  asection *ssec;

	  isym = intsyms + CGC32_R_SYM (rel->r_info);
	  ...
	  if (isym->st_shndx == SHN_UNDEF)
	    ...
	    ssec = bfd_section_from_cgc_index (abfd,
					       isym->st_shndx);
	  ...
	  if (ssec)
		...
		symval = _bfd_merged_section_offset (abfd, & ssec,
						     cgc_section_data (ssec)->sec_info,
						     symval);
	    }
      ...
      else
	{
	  ...
	  /* An external symbol.  */
	  indx = CGC32_R_SYM (rel->r_info) - symtab_hdr->sh_info;
...
static void
move_reloc (Cgc_Internal_Rela * irel, Cgc_Internal_Rela * srel, int delta)
{
...
static bfd_boolean
cgc32_rx_relax_section (bfd *                  abfd,
			asection *             sec,
			...
			bfd_boolean            allow_pcrel3)
{
  ...
  Cgc_Internal_Rela * irel;
  Cgc_Internal_Rela * srel;
  Cgc_Internal_Rela * irelend;
  Cgc_Internal_Rela * next_alignment;
  Cgc_Internal_Rela * prev_alignment;
  bfd_byte *          contents = NULL;
  bfd_byte *          free_contents = NULL;
  Cgc_Internal_Sym *  intsyms = NULL;
  Cgc_Internal_Sym *  free_intsyms = NULL;
  Cgc_External_Sym_Shndx * shndx_buf = NULL;
  bfd_vma pc;
  bfd_vma sec_start;
  bfd_vma symval = 0;
  int pcrel = 0;
  ...
  *again = FALSE;
  ...
  if (symtab_hdr->contents != NULL)
    intsyms = (Cgc_Internal_Sym *) symtab_hdr->contents;
  else
    {
      intsyms = bfd_cgc_get_cgc_syms (abfd, symtab_hdr, symtab_hdr->sh_info, 0, NULL, NULL, NULL);
  ...
  if (shndx_hdr->sh_size != 0)
    {
      ...
      shndx_buf = (Cgc_External_Sym_Shndx *) bfd_malloc (amt);
      if (shndx_buf == NULL)
	goto error_return;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
      if (CGC32_R_TYPE (irel->r_info) == R_RX_RH_RELAX
	  && irel->r_addend & RX_RELAXA_ALIGN)
	{
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	  ...
	  || next_alignment == internal_relocs)
	{
	  ...
	  while (next_alignment < irelend
		 && (CGC32_R_TYPE (next_alignment->r_info) != R_RX_RH_RELAX
      ...
      if (irel->r_addend & RX_RELAXA_ALIGN)
	{
	  ...
	  Cgc_Internal_Rela *erel = irel + 1;
	  unsigned int alignment, nbytes;

	  if (CGC32_R_TYPE (erel->r_info) != R_RX_RH_RELAX)
	    continue;
	  ...
	  cgc32_rx_relax_delete_bytes (abfd, sec, erel->r_offset-nbytes, nbytes, next_alignment,
...
#define SNIPNR(offset, nbytes) \
	cgc32_rx_relax_delete_bytes (abfd, sec, (insn - contents) + offset, nbytes, next_alignment, 0);
#define SNIP(offset, nbytes, newtype) \
        SNIPNR (offset, nbytes);						\
	srel->r_info = CGC32_R_INFO (CGC32_R_SYM (srel->r_info), newtype)
      ...
      if (irel->r_addend & RX_RELAXA_DSP6)
	{
	  ...
	  if (code == 2 && symval/scale <= 255)
	    {
	      unsigned int newrel = CGC32_R_TYPE (srel->r_info);
	      insn[0] &= 0xfc;
	      insn[0] |= 0x01;
	      newrel = next_smaller_reloc (CGC32_R_TYPE (srel->r_info));
	      if (newrel != CGC32_R_TYPE (srel->r_info))
		{
		  SNIP (3, 1, newrel);
		  *again = TRUE;
		}
      ...
      if (irel->r_addend & RX_RELAXA_IMM6)
	{
	  ...
	  /* Special case UIMM8 format: CMP #uimm8,Rdst.  */
		   ...
		   && ((insn[1] & 0xf0) == 0x00))
	    {
	      ...
	      if (STACK_REL_P (CGC32_R_TYPE (srel->r_info)))
		newrel = R_RX_ABS8U;
	      else
		newrel = R_RX_DIR8U;
      ...
      if (irel->r_addend & RX_RELAXA_BRA)
	{
	  ...
	  if (CGC32_R_TYPE(srel[1].r_info) == R_RX_RH_RELAX
	  ...
	  && (insn[0] & 0x03) != 0x03)
	{
	  ...
	  bfd_vma disp_val = 0;
	  long imm_val = 0;
	  Cgc_Internal_Rela * disp_rel = 0;
	  Cgc_Internal_Rela * imm_rel = 0;
	  ...
	  if (0 <= imm_val && imm_val <= 255
	      ...
	      if (disp_rel)
		{
		  ...
		  disp_rel->r_info = CGC32_R_INFO (CGC32_R_SYM (disp_rel->r_info), newrel);
		  move_reloc (irel, disp_rel, -1);
		}
	      if (imm_rel)
		{
		  imm_rel->r_info = CGC32_R_INFO (CGC32_R_SYM (imm_rel->r_info), R_RX_DIR8U);
  ...
  if (!*again && !allow_pcrel3)
    {
      ...
      cgc32_rx_relax_section (abfd, sec, link_info, &ignored, TRUE);
    }
...
static bfd_boolean
cgc32_rx_relax_section_wrapper (bfd *                  abfd,
				asection *             sec,
				struct bfd_link_info * link_info,
				bfd_boolean *          again)
{
  return cgc32_rx_relax_section (abfd, sec, link_info, again, FALSE);
}

/* Function to set the CGC flag bits.  */

static bfd_boolean
rx_cgc_set_private_flags (bfd * abfd, flagword flags)
{
...
void bfd_cgc32_rx_set_target_flags (bfd_boolean, bfd_boolean);

void
bfd_cgc32_rx_set_target_flags (bfd_boolean user_no_warn_mismatch,
...
static bfd_boolean
rx_cgc_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
  flagword old_flags;
  flagword new_flags;
  ...
  if (!cgc_flags_init (obfd))
      ...
      if ((old_flags ^ new_flags) & known_flags)
	{
	  ...
	  if (no_warn_mismatch)
	    {
	      cgc_cgcheader (obfd)->e_flags = (new_flags | old_flags) & known_flags;
	    }
	  else
	    {
	      _bfd_error_handler ("There is a conflict merging the CGC header flags from %s",
	...
	cgc_cgcheader (obfd)->e_flags = new_flags & known_flags;
    }
...

static bfd_boolean
rx_cgc_print_private_bfd_data (bfd * abfd, void * ptr)
{
  FILE * file = (FILE *) ptr;
  flagword flags;
...
static int
cgc32_rx_machine (bfd * abfd ATTRIBUTE_UNUSED)
{
  ...
  if ((cgc_cgcheader (abfd)->e_flags & EF_RX_CPU_MASK) == EF_RX_CPU_RX)
#endif
    return bfd_mach_rx;
...
static bfd_boolean
rx_cgc_object_p (bfd * abfd)
{
  int i;
  unsigned int u;
  Cgc_Internal_Phdr *phdr = cgc_tdata (abfd)->phdr;
  int nphdrs = cgc_cgcheader (abfd)->e_phnum;
  sec_ptr bsec;
  ...
  /* We never want to automatically choose the non-swapping big-endian
     ...
     and objcopy.  */
  if (abfd->xvec == &bfd_cgc32_rx_be_ns_vec
  ...
  if (abfd->xvec == &bfd_cgc32_rx_be_ns_vec
      && saw_be)
    return FALSE;
  if (abfd->xvec == &bfd_cgc32_rx_be_vec)
    saw_be = TRUE;

  bfd_default_set_arch_mach (abfd, bfd_arch_rx,
			     cgc32_rx_machine (abfd));
  ...
  for (i=0; i<nphdrs; i++)
    {
      for (u=0; u<cgc_tdata(abfd)->num_cgc_sections; u++)
	{
	  Cgc_Internal_Shdr *sec = cgc_tdata(abfd)->cgc_sect_ptr[u];
...
rx_dump_symtab (bfd * abfd, void * internal_syms, void * external_syms)
{
  ...
  symtab_hdr = &cgc_tdata (abfd)->symtab_hdr;
  locsymcount = symtab_hdr->sh_size / get_cgc_backend_data (abfd)->s->sizeof_sym;
  if (free_internal)
  ...
  for (isym = isymbuf ; isym < isymend ; isym++)
	...
	default: st_info_stb_str = "";
      ...
      switch (CGC_ST_VISIBILITY (isym->st_other))
	{
	...
	default: st_shndx_str = "";
      ...
      printf ("isym = %p st_value = %lx st_size = %lx st_name = (%lu) %s "
	      ...
	      isym->st_name,
	      bfd_cgc_string_from_cgc_section (abfd, symtab_hdr->sh_link,
					       isym->st_name),
...
rx_get_reloc (long reloc)
{
  if (0 <= reloc && reloc < R_RX_max)
    return rx_cgc_howto_table[reloc].name;
  return "";
}
#endif /* DEBUG */
			 ...
			 bfd_size_type count)
{
  ...
  return _bfd_cgc_set_section_contents (abfd, section, location, offset, count);
}
#define _bfd_cgc_set_section_contents rx2_set_section_contents
#endif
			 ...
			 bfd_size_type count)
{
  ...
  while (count > 0 && caddr > 0 && caddr % 4)
	...
	case 3: faddr = offset - 3; break;
      ...
      rv = _bfd_cgc_set_section_contents (abfd, section, location, faddr, 1);
      if (! rv)
	return rv;
  ...
  if (scount > 0)
      ...
      for (i = 0; i < count; i += 4)
	{
      ...
      rv = _bfd_cgc_set_section_contents (abfd, section, swapped_data, offset, scount);
...
static bfd_boolean
cgc32_rx_modify_program_headers (bfd * abfd ATTRIBUTE_UNUSED,
...
static const struct bfd_cgc_special_section cgc32_rx_special_sections[] =
{
...

#define CGC_ARCH		bfd_arch_rx
#define CGC_MACHINE_CODE	EM_RX
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM		bfd_cgc32_rx_be_vec
#define TARGET_BIG_NAME		"cgc32-rx-be"

#define TARGET_LITTLE_SYM	bfd_cgc32_rx_le_vec
#define TARGET_LITTLE_NAME	"cgc32-rx-le"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			rx_info_to_howto_rela
#define cgc_backend_object_p			rx_cgc_object_p
#define cgc_backend_relocate_section		rx_cgc_relocate_section
#define cgc_symbol_leading_char                 ('_')
#define cgc_backend_can_gc_sections		1
#define cgc_backend_modify_program_headers	cgc32_rx_modify_program_headers

#define bfd_cgc32_bfd_reloc_type_lookup		rx_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		rx_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		rx_cgc_set_private_flags
#define bfd_cgc32_bfd_merge_private_bfd_data	rx_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_print_private_bfd_data	rx_cgc_print_private_bfd_data
#define bfd_cgc32_get_section_contents		rx_get_section_contents
#define bfd_cgc32_set_section_contents		rx_set_section_contents
#define bfd_cgc32_bfd_final_link		rx_final_link
#define bfd_cgc32_bfd_relax_section		cgc32_rx_relax_section_wrapper
#define cgc_backend_special_sections	        cgc32_rx_special_sections
...
#define TARGET_BIG_SYM		bfd_cgc32_rx_be_ns_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-rx-be-ns"
#undef  TARGET_LITTLE_SYM

#undef bfd_cgc32_get_section_contents
#undef bfd_cgc32_set_section_contents

#undef	cgc32_bed
#define cgc32_bed				cgc32_rx_be_ns_bed
