      ...
      if (p[1] == 'A' || p[1] == 'B')
	{
	  ...
	  if (avail == 0)
	      ...
	      else
		{
		  ...
		  abfd = sec->owner;
		  if (abfd != NULL
		      && bfd_get_flavour (abfd) == bfd_target_cgc_flavour
		      && cgc_next_in_group (sec) != NULL
		      && (sec->flags & SEC_GROUP) == 0)
		    group = cgc_group_name (sec);
		  else if (abfd != NULL
...
bfd_get_arch_size (bfd *abfd)
{
  if (abfd->xvec->flavour == bfd_target_cgc_flavour)
    return get_cgc_backend_data (abfd)->s->arch_size;
...
bfd_get_sign_extend_vma (bfd *abfd)
{
  ...
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
    return get_cgc_backend_data (abfd)->sign_extend_vma;
...
bfd_get_gp_size (bfd *abfd)
{
  if (abfd->format == bfd_object)
	...
	return ecoff_data (abfd)->gp_size;
      else if (abfd->xvec->flavour == bfd_target_cgc_flavour)
	return cgc_gp_size (abfd);
    }
  return 0;
}
...
/* Record information about an CGC program header.  */
  ...
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
    {
      ...
      return bed->s->cgcclass == CGCCLASS32;
    }

  /* For non-CGC targets, use architecture information.  */
...
bfd_alt_mach_code (bfd *abfd, int alternative)
{
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
	...
	{
	case 0:
	  code = get_cgc_backend_data (abfd)->cgc_machine_code;
	  break;

	case 1:
	  code = get_cgc_backend_data (abfd)->cgc_machine_alt1;
	  if (code == 0)
	    return FALSE;
	  break;

	case 2:
	  code = get_cgc_backend_data (abfd)->cgc_machine_alt2;
	  if (code == 0)
	    return FALSE;
	  break;
	...
	default:
      ...
      cgc_cgcheader (abfd)->e_machine = code;
...
bfd_emul_get_maxpagesize (const char *emul)
{
  ...
  if (target != NULL
      && target->flavour == bfd_target_cgc_flavour)
    return xvec_get_cgc_backend_data (target)->maxpagesize;

  return 0;
}

static void
bfd_cgc_set_pagesize (const bfd_target *target, bfd_vma size,
		      int offset, const bfd_target *orig_target)
{
  if (target->flavour == bfd_target_cgc_flavour)
    {
      const struct cgc_backend_data *bed;

      bed = xvec_get_cgc_backend_data (target);
      *((bfd_vma *) ((char *) bed + offset)) = size;
    }

  if (target->alternative_target
      && target->alternative_target != orig_target)
    bfd_cgc_set_pagesize (target->alternative_target, size, offset,
			  orig_target);
}
...
bfd_emul_set_maxpagesize (const char *emul, bfd_vma size)
{
  ...
  target = bfd_find_target (emul, NULL);
  if (target)
    bfd_cgc_set_pagesize (target, size,
			  offsetof (struct cgc_backend_data,
				    maxpagesize), target);
}
...
bfd_emul_get_commonpagesize (const char *emul)
{
  ...
  if (target != NULL
      && target->flavour == bfd_target_cgc_flavour)
    return xvec_get_cgc_backend_data (target)->commonpagesize;
...
bfd_demangle (bfd *abfd, const char *name, int options)
{
  ...
  /* This is a hack for better error reporting on XCOFF, PowerPC64-CGC
