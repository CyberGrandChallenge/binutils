/* CRIS-specific support for 32-bit CGC.
   Copyright 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009,
   2010, 2011, 2012 Free Software Foundation, Inc.
   Contributed by Axis Communications AB.
   Written by Hans-Peter Nilsson, based on cgc32-fr30.c
   PIC and shlib bits based primarily on cgc32-m68k.c and cgc32-i386.c.
...
cris_cgc_pcrel_reloc (bfd *, arelent *, asymbol *, void *,
		      asection *, bfd *, char **);
static bfd_boolean
cris_cgc_set_mach_from_flags (bfd *, unsigned long);

/* Forward declarations.  */
static reloc_howto_type cris_cgc_howto_table [] =
{
  ...
  HOWTO (R_CRIS_32,		/* type */
	 ...
	 /* We don't want overflow complaints for 64-bit vma builds
	    ...
	    32-bit CGC) where sym=0xc0001234.
  ...
  HOWTO (R_CRIS_8_PCREL,	/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_bitfield, /* complain_on_overflow */
	 cris_cgc_pcrel_reloc,	/* special_function */
	 "R_CRIS_8_PCREL",	/* name */
	 FALSE,			/* partial_inplace */
	 0x0000,		/* src_mask */
	 0x00ff,		/* dst_mask */
...
#define TLSHOWTO32(name) \
 HOWTO (name, 0, 2, 32, FALSE, 0, complain_overflow_bitfield, \
        bfd_cgc_generic_reloc, #name, FALSE, 0, 0xffffffff, FALSE)
#define TLSHOWTO16X(name, X)	     \
 HOWTO (name, 0, 1, 16, FALSE, 0, complain_overflow_ ## X, \
        bfd_cgc_generic_reloc, #name, FALSE, 0, 0xffff, FALSE)
...

/* Map BFD reloc types to CRIS CGC reloc types.  */
			...
			bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < sizeof (cris_reloc_map) / sizeof (cris_reloc_map[0]); i++)
    if (cris_reloc_map [i].bfd_reloc_val == code)
      return & cris_cgc_howto_table [cris_reloc_map[i].cris_reloc_val];
...
cris_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (cris_cgc_howto_table) / sizeof (cris_cgc_howto_table[0]);
       i++)
    if (cris_cgc_howto_table[i].name != NULL
	&& strcasecmp (cris_cgc_howto_table[i].name, r_name) == 0)
      return &cris_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an CRIS CGC reloc.  */
...
{
  enum cgc_cris_reloc_type r_type;

  r_type = CGC32_R_TYPE (dst->r_info);
  BFD_ASSERT (r_type < (unsigned int) R_CRIS_max);
  cache_ptr->howto = & cris_cgc_howto_table [r_type];
}

bfd_reloc_status_type
cris_cgc_pcrel_reloc (bfd *abfd ATTRIBUTE_UNUSED,
		      arelent *reloc_entry,
		      asymbol *symbol,
...
{
  /* By default (using only bfd_cgc_generic_reloc when linking to
     non-CGC formats) PC-relative relocs are relative to the beginning
  ...
  return
    bfd_cgc_generic_reloc (abfd, reloc_entry, symbol, data,
...
static bfd_boolean
cris_cgc_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  size_t size;
...
static bfd_boolean
cris_cgc_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  if (bfd_get_mach (abfd) == bfd_mach_cris_v32)
    switch (note->descsz)
      {
	...
	return FALSE;
      ...
      case 124:		/* Linux/CRISv32 cgc_prpsinfo */
	...
	return FALSE;
      ...
      case 124:		/* Linux/CRIS cgc_prpsinfo */
	cgc_tdata (abfd)->core->program
	  = _bfd_cgccore_strndup (abfd, note->descdata + 28, 16);
	cgc_tdata (abfd)->core->command
	  = _bfd_cgccore_strndup (abfd, note->descdata + 44, 80);
...
static const bfd_byte cgc_cris_plt0_entry[PLT_ENTRY_SIZE] =
{
  0xfc, 0xe1,
  0x7e, 0x7e,	/* push mof.  */
  0x7f, 0x0d,   /*  (dip [pc+]) */
...
static const bfd_byte cgc_cris_plt0_entry_v32[PLT_ENTRY_SIZE_V32] =
{
  0x84, 0xe2,	/* subq 4,$sp */
...
static const bfd_byte cgc_cris_plt_entry[PLT_ENTRY_SIZE] =
{
  0x7f, 0x0d,   /*  (dip [pc+]) */
...
static const bfd_byte cgc_cris_plt_entry_v32[PLT_ENTRY_SIZE_V32] =
{
  0x6f, 0xfe,	/* move.d 0,$acr */
  0, 0, 0, 0,	/*  Replaced with address of this symbol in .got.  */
  0x6f, 0xfa,   /* move.d [$acr],$acr */
...
static const bfd_byte cgc_cris_pic_plt0_entry[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_cris_pic_plt0_entry_v32[PLT_ENTRY_SIZE_V32] =
{
  0x84, 0xe2,	/* subq 4,$sp */
...
static const bfd_byte cgc_cris_pic_plt_entry[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_cris_pic_plt_entry_v32[PLT_ENTRY_SIZE_V32] =
{
...

/* We copy cgc32-m68k.c and cgc32-i386.c for the basic linker hash bits
...
struct cgc_cris_pcrel_relocs_copied
{
  /* Next section.  */
  struct cgc_cris_pcrel_relocs_copied *next;
...
/* CRIS CGC linker hash entry.  */

struct cgc_cris_link_hash_entry
{
  struct cgc_link_hash_entry root;

  /* Number of PC relative relocs copied for this symbol.  */
  struct cgc_cris_pcrel_relocs_copied *pcrel_relocs_copied;
...
static bfd_boolean
cgc_cris_discard_excess_dso_dynamics (struct cgc_cris_link_hash_entry *,
				      void * );
static bfd_boolean
cgc_cris_discard_excess_program_dynamics (struct cgc_cris_link_hash_entry *,
					  void *);
...
/* CRIS CGC linker hash table.  */

struct cgc_cris_link_hash_table
{
...
/* Traverse a CRIS CGC linker hash table.  */

#define cgc_cris_link_hash_traverse(table, func, info)			\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func),	\
    (info)))

/* Get the CRIS CGC linker hash table from a link_info structure.  */

#define cgc_cris_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == CRIS_CGC_DATA ? ((struct cgc_cris_link_hash_table *) ((p)->hash)) : NULL)

/* Get the CRIS CGC linker hash entry from a regular hash entry (the
   "parent class").  The .root reference is just a simple type
   check on the argument.  */

#define cgc_cris_hash_entry(p) \
 ((struct cgc_cris_link_hash_entry *) (&(p)->root))

/* Create an entry in a CRIS CGC linker hash table.  */

static struct bfd_hash_entry *
cgc_cris_link_hash_newfunc (struct bfd_hash_entry *entry,
			    struct bfd_hash_table *table,
			    const char *string)
{
  struct cgc_cris_link_hash_entry *ret =
    (struct cgc_cris_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == (struct cgc_cris_link_hash_entry *) NULL)
    ret = ((struct cgc_cris_link_hash_entry *)
	   bfd_hash_allocate (table,
			      sizeof (struct cgc_cris_link_hash_entry)));
  if (ret == (struct cgc_cris_link_hash_entry *) NULL)
...
/* Create a CRIS CGC linker hash table.  */

static struct bfd_link_hash_table *
cgc_cris_link_hash_table_create (bfd *abfd)
{
  struct cgc_cris_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc_cris_link_hash_table);

  ret = ((struct cgc_cris_link_hash_table *) bfd_zmalloc (amt));
  if (ret == (struct cgc_cris_link_hash_table *) NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      cgc_cris_link_hash_newfunc,
				      sizeof (struct cgc_cris_link_hash_entry),
				      CRIS_CGC_DATA))
    {
      free (ret);
			  ...
			  bfd_vma             relocation)
{
  ...
  enum cgc_cris_reloc_type r_type = CGC32_R_TYPE (rel->r_info);
...
/* Relocate an CRIS CGC section.  See cgc32-fr30.c, from where this was
   copied, for further comments.  */

static bfd_boolean
cris_cgc_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
...
{
  struct cgc_cris_link_hash_table * htab;
  bfd *dynobj;
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = cris_cgc_howto_table + r_type;
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
      ...
      else
	{
	  ...
	  if (unresolved_reloc
		 ...
		 m68k, but apparently (and according to cgcxx-ia64.c) all
		   ...
		   || h->root.type == bfd_link_hash_defweak)
	    {
		 ...
		 where I copied them from (cgc32-m68k.c and
		 cgc32-i386.c), but let's mention examples of where
		 ...
		 h->cgc_link_hash_flags test, though it's there in
		 other targets.  */
	      if (info->shared
			   ...
			   != (bfd_vma) -1))
		{
		  _bfd_error_handler
		    (_("%B, section %A: unresolvable relocation %s against symbol `%s'"),
		     input_bfd,
		     input_section,
		     cris_cgc_howto_table[r_type].name,
		     symname);
	...
	case R_CRIS_32_GOTPLT:
	  ...
	  if (h != NULL
	      && ((struct cgc_cris_link_hash_entry *) h)->gotplt_offset != 0)
	    {
	      asection *sgotplt
		= bfd_get_linker_section (dynobj, ".got.plt");
	      ...
	      got_offset
		= ((struct cgc_cris_link_hash_entry *) h)->gotplt_offset;
	  ...
	  /* We didn't make a PLT entry for this symbol.  Maybe everything is
	     ...
	     cgc_cris_adjust_dynamic_symbol, and drop through into the
	...
	case R_CRIS_32_GOT:
	  {
		...
		if (!cgc_hash_table (info)->dynamic_sections_created
			...
			&& h->def_regular))
		  {
		    ...
		    BFD_ASSERT (!cgc_hash_table (info)->dynamic_sections_created
				|| info->shared
				|| h->def_regular
				|| h->type == STT_FUNC
				|| h->needs_plt
		...
		if ((off & 1) != 0)
		...
		else
		  {
			...
			outrel.r_offset = (sgot->output_section->vma
					   + sgot->output_offset
					   + off);
			outrel.r_info = CGC32_R_INFO (0, R_CRIS_RELATIVE);
	...
	case R_CRIS_32_GOTREL:
	  ...
	  /* This can happen if we get a link error with the input CGC
	...
	case R_CRIS_32_PLT_PCREL:
	  ...
	  if (h == NULL || CGC_ST_VISIBILITY (h->other) != STV_DEFAULT)
	    break;
	...
	case R_CRIS_32:
	  if (info->shared
	      ...
	      if (sreloc == NULL)
		{
		  ...
		  /* The section should have been created in cris_cgc_check_relocs,
		     but that function will not be called for objects which fail in
		     cris_cgc_merge_private_bfd_data.  */
		  if (sreloc == NULL)
		    {
		      bfd_set_error (bfd_error_bad_value);
		      return FALSE;
	...
	case R_CRIS_32_DTPREL:
	  ...
	  if (htab->dtpmod_refcount > 0
	      ...
	      if (info->shared)
		{
		  ...
		  outrel.r_offset = (sgotplt->output_section->vma
				     ...
				     + 12);
		  outrel.r_info = CGC32_R_INFO (0, R_CRIS_DTPMOD);
		  outrel.r_addend = 0;
		  bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
		}
	      else
	  ...
	  relocation -= cgc_hash_table (info)->tls_sec == NULL
	    ? 0 : cgc_hash_table (info)->tls_sec->vma;
	  break;
	  ...
	  if (info->shared)
	    {
	      ...
	      /* We've already informed in cris_cgc_check_relocs that
		 this is an error.  */
	      return FALSE;
	    }
	  /* Fall through.  */
	...
	case R_CRIS_32_GOT_GD:
	  ...
	  if (!info->shared
	      && (h == NULL || h->def_regular || CGC_COMMON_DEF_P (h)))
	    {
		 ...
		 (1, known_tpoffset) into the GOT.  */
	      relocation -= cgc_hash_table (info)->tls_sec->vma;

	      if (h != NULL)
		{
		  off = cgc_cris_hash_entry (h)->tprel_refcount > 0
	      ...
	      if ((off & 2) == 0)
		{
		  ...
		  if (h != NULL && h->dynindx != -1)
		    {
		      outrel.r_info = CGC32_R_INFO (h->dynindx, R_CRIS_DTP);
		      relocation = 0;
		    }
		  else
		    {
		      outrel.r_info = CGC32_R_INFO (0, R_CRIS_DTP);
	  ...
	  if (!info->shared
	      && (h == NULL || h->def_regular || CGC_COMMON_DEF_P (h)))
	    {
		 ...
		 the -prog_tls_size+known_tpoffset into the GOT.  */
	      relocation -= cgc_hash_table (info)->tls_sec->vma;
	      relocation -= cgc_hash_table (info)->tls_size;
	      ...
	      if ((off & 1) == 0)
		{
		  ...
		  if (h != NULL && h->dynindx != -1)
		    {
		      outrel.r_info = CGC32_R_INFO (h->dynindx, R_CRIS_32_TPREL);
		      relocation = 0;
		    }
		  else
		    {
		      outrel.r_info = CGC32_R_INFO (0, R_CRIS_32_TPREL);
...
static bfd_boolean
cgc_cris_finish_dynamic_symbol (bfd *output_bfd,
				struct bfd_link_info *info,
				struct cgc_link_hash_entry *h,
				Cgc_Internal_Sym *sym)
{
  ...
  const bfd_byte *plt_entry = cgc_cris_plt_entry;
  const bfd_byte *plt_pic_entry = cgc_cris_pic_plt_entry;
  ...
  if (bfd_get_mach (output_bfd) == bfd_mach_cris_v32)
    {
      ...
      plt_entry = cgc_cris_plt_entry_v32;
      plt_pic_entry = cgc_cris_pic_plt_entry_v32;
    }
  ...
  if (h->plt.offset != (bfd_vma) -1)
	...
	= cgc_cris_hash_entry (h)->gotplt_offset;
      ...
      if (has_gotplt)
	{
	  ...
	  bfd_put_32 (output_bfd,
		      rela_plt_index * sizeof (Cgc32_External_Rela),
	  ...
	  rela.r_offset = (sgotplt->output_section->vma
			   + sgotplt->output_offset
			   + got_offset);
	  rela.r_info = CGC32_R_INFO (h->dynindx, R_CRIS_JUMP_SLOT);
	  rela.r_addend = 0;
	  loc = srela->contents + rela_plt_index * sizeof (Cgc32_External_Rela);
      ...
      if (!h->def_regular)
	{
	  ...
	  /* FIXME: From cgc32-sparc.c 2001-02-19 (1.18).  I still don't
  ...
  if (h->got.offset != (bfd_vma) -1
      && (cgc_cris_hash_entry (h)->reg_got_refcount > 0)
      && (info->shared
	  || (h->dynindx != -1
	      ...
	      && h->root.type != bfd_link_hash_undefweak)))
    {
	 ...
	 initialized in the relocate_section function.  */
	  ...
	  || (info->shared
	      ...
	      && h->def_regular))
	{
	  rela.r_info = CGC32_R_INFO (0, R_CRIS_RELATIVE);
	  rela.r_addend = bfd_get_signed_32 (output_bfd, where);
	}
      else
	{
	  bfd_put_32 (output_bfd, (bfd_vma) 0, where);
	  rela.r_info = CGC32_R_INFO (h->dynindx, R_CRIS_GLOB_DAT);
	  rela.r_addend = 0;
	}
  ...
  if (h->needs_copy)
    {
      ...
      rela.r_info = CGC32_R_INFO (h->dynindx, R_CRIS_COPY);
      rela.r_addend = 0;
...
static bfd_boolean
cgc_cris_finish_dynamic_sections (bfd *output_bfd,
				  struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sgot;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (splt->size > 0)
	{
	  if (bfd_get_mach (output_bfd) == bfd_mach_cris_v32)
	    {
	      if (info->shared)
		memcpy (splt->contents, cgc_cris_pic_plt0_entry_v32,
			PLT_ENTRY_SIZE_V32);
	      else
		{
		  memcpy (splt->contents, cgc_cris_plt0_entry_v32,
			  PLT_ENTRY_SIZE_V32);
		  bfd_put_32 (output_bfd,
			      sgot->output_section->vma
	    ...
	    {
	      if (info->shared)
		memcpy (splt->contents, cgc_cris_pic_plt0_entry,
			PLT_ENTRY_SIZE);
	      else
		{
		  memcpy (splt->contents, cgc_cris_plt0_entry,
			  PLT_ENTRY_SIZE);
		  bfd_put_32 (output_bfd,
			      sgot->output_section->vma
			      + sgot->output_offset + 4,
...
static asection *
cris_cgc_gc_mark_hook (asection *sec,
		       struct bfd_link_info *info,
		       Cgc_Internal_Rela *rel,
		       struct cgc_link_hash_entry *h,
		       Cgc_Internal_Sym *sym)
...
static bfd_boolean
cris_cgc_gc_sweep_hook (bfd *abfd,
			struct bfd_link_info *info,
			asection *sec,
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	case R_CRIS_32_GOTPLT:
	  specific_refcount = h != NULL
	    ? &((struct cgc_cris_link_hash_entry *) h)->reg_got_refcount
	...
	case R_CRIS_16_GOT_GD:
	  got_element_size = 8;
	  specific_refcount = h != NULL
	    ? &((struct cgc_cris_link_hash_entry *) h)->dtp_refcount
	...
	case R_CRIS_32_GOT_TPREL:
	  specific_refcount = h != NULL
	    ? &((struct cgc_cris_link_hash_entry *) h)->tprel_refcount
	...
	case R_CRIS_32_GOTPLT:
	  ...
	  if (h == NULL)
	    ...
	    cgc_cris_hash_entry (h)->gotplt_refcount--;
	  /* Fall through.  */
	...
	case R_CRIS_32_PCREL:
	case R_CRIS_32_PLT_PCREL:
	  /* Negate the increment we did in cris_cgc_check_relocs.  */
	  if (h != NULL)
	    {
...
/* The cgc_backend_plt_sym_val hook function.  */

static bfd_vma
cris_cgc_plt_sym_val (bfd_vma i ATTRIBUTE_UNUSED, const asection *plt,
		      const arelent *rel)
{
  ...
  /* Same for CRIS and CRIS v32; see cgc_cris_(|pic_)plt_entry(|_v32)[].  */
  ...
  /* Because we can have merged GOT entries; a single .got entry for
     ...
     fixable by providing our own bfd_cgc32_get_synthetic_symtab.
...
/* Make sure we emit a GOT entry if the symbol was supposed to have a PLT
   ...
   cgc_cris_adjust_dynamic_symbol when we're doing a proper dynamic link,
   or cgc_cris_size_dynamic_sections if no dynamic sections will be
   created (we're only linking static objects).  */

static bfd_boolean
cgc_cris_adjust_gotplt_to_got (struct cgc_cris_link_hash_entry *h, void * p)
{
...
/* Try to fold PLT entries with GOT entries.  There are two cases when we
   ...
   cgc_cris_hide_symbol.  */

static bfd_boolean
cgc_cris_try_fold_plt_to_got (struct cgc_cris_link_hash_entry *h, void * p)
{
  struct bfd_link_info *info = (struct bfd_link_info *) p;
  ...
  if (h->gotplt_refcount == h->root.plt.refcount)
    {
	 ...
	 references.  Convert PLT to GOT references.  */
      if (! cgc_cris_adjust_gotplt_to_got (h, info))
	return FALSE;
...
static void
cgc_cris_hide_symbol (struct bfd_link_info *info,
		      struct cgc_link_hash_entry *h,
		      bfd_boolean force_local)
{
  cgc_cris_adjust_gotplt_to_got ((struct cgc_cris_link_hash_entry *) h, info);
...
static bfd_boolean
cgc_cris_adjust_dynamic_symbol (struct bfd_link_info *info,
				struct cgc_link_hash_entry *h)
{
  struct cgc_cris_link_hash_table * htab;
  bfd *dynobj;
  ...
  if (h->type == STT_FUNC
	  ...
	  && !h->def_dynamic)
	{
	  ...
	  h->plt.offset = (bfd_vma) -1;
	  return
	    cgc_cris_adjust_gotplt_to_got ((struct
					    cgc_cris_link_hash_entry *) h,
					   info);
	}
	 ...
	 entries will point to the PLT there (and be constant).  */
      if (info->shared
	  && !cgc_cris_try_fold_plt_to_got ((struct cgc_cris_link_hash_entry*)
					    h, info))
	return FALSE;
	  ...
	  && !h->def_regular)
	{
	 ...
	 this to cgc_cris_try_fold_plt_to_got.  */
      if (info->shared && h->got.refcount > 0)
	{
	  ...
	  BFD_ASSERT (((struct cgc_cris_link_hash_entry *)
		       h)->gotplt_offset == 0);
...
static void
cgc_cris_copy_indirect_symbol (struct bfd_link_info *info,
			       struct cgc_link_hash_entry *dir,
			       struct cgc_link_hash_entry *ind)
{
  struct cgc_cris_link_hash_entry *edir, *eind;

  edir = (struct cgc_cris_link_hash_entry *) dir;
  eind = (struct cgc_cris_link_hash_entry *) ind;
...
#define XMOVE(F) XMOVOPZ (F, +=, 0)
  if (eind->pcrel_relocs_copied != NULL)
	...
	{
	  struct cgc_cris_pcrel_relocs_copied **pp;
	  struct cgc_cris_pcrel_relocs_copied *p;

	  /* Add reloc counts against the indirect sym to the direct sym
	     list.  Merge any entries against the same section.  */
	  for (pp = &eind->pcrel_relocs_copied; *pp != NULL;)
	    {
	      struct cgc_cris_pcrel_relocs_copied *q;
	      p = *pp;
...
static bfd_boolean
cris_cgc_check_relocs (bfd *abfd,
		       struct bfd_link_info *info,
		       asection *sec,
		       const Cgc_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
	  ...
	  if (dynobj == NULL)
	    {
		 ...
		 output bfd in cgc_cris_adjust_dynamic_symbol.  Failing
	...
	case R_CRIS_32_GOT:
	  ...
	  if (h != NULL)
		...
		case R_CRIS_16_GOT:
		case R_CRIS_32_GOT:
		  if (cgc_cris_hash_entry (h)->reg_got_refcount == 0)
		    {
		      /* Allocate space in the .got section.  */
		      sgot->size += got_element_size;
		      /* Allocate relocation space.  */
		  ...
		  cgc_cris_hash_entry (h)->reg_got_refcount++;
		  break;

		case R_CRIS_32_GD:
		case R_CRIS_16_GOT_GD:
		case R_CRIS_32_GOT_GD:
		  if (cgc_cris_hash_entry (h)->dtp_refcount == 0)
		    {
		      /* Allocate space in the .got section.  */
		      sgot->size += got_element_size;
		      /* Allocate relocation space.  */
		  ...
		  cgc_cris_hash_entry (h)->dtp_refcount++;
		  break;

		case R_CRIS_32_IE:
		case R_CRIS_32_GOT_TPREL:
		case R_CRIS_16_GOT_TPREL:
		  if (cgc_cris_hash_entry (h)->tprel_refcount == 0)
		    {
		      /* Allocate space in the .got section.  */
		      sgot->size += got_element_size;
		      /* Allocate relocation space.  */
		  ...
		  cgc_cris_hash_entry (h)->tprel_refcount++;
		  break;
	...
	case R_CRIS_32_PCREL:
	  ...
	  /* We count the number of PC relative relocations we have
	     ...
	     cgc_cris_link_hash_entry.  */
	  {
	    struct cgc_cris_link_hash_entry *eh;
	    struct cgc_cris_pcrel_relocs_copied *p;

	    eh = cgc_cris_hash_entry (h);
	    ...
	    if (p == NULL)
	      {
		p = ((struct cgc_cris_pcrel_relocs_copied *)
...
static bfd_boolean
cgc_cris_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				struct bfd_link_info *info)
{
  struct cgc_cris_link_hash_table * htab;
  bfd *dynobj;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
  ...
  else
    {
      ...
      cgc_cris_link_hash_traverse (htab, cgc_cris_adjust_gotplt_to_got,
				   info);
  ...
  if (info->shared)
    cgc_cris_link_hash_traverse (htab,
				 cgc_cris_discard_excess_dso_dynamics,
				 info);
  else
    cgc_cris_link_hash_traverse (htab,
				 cgc_cris_discard_excess_program_dynamics,
				 info);
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in cgc_cris_finish_dynamic_sections, but we
...
/* This function is called via cgc_cris_link_hash_traverse if we are
...
static bfd_boolean
cgc_cris_discard_excess_dso_dynamics (struct cgc_cris_link_hash_entry *h,
				      void * inf)
{
  struct cgc_cris_pcrel_relocs_copied *s;
  ...
  if (h->root.def_regular
	...
	{
	  asection *sreloc
	    = _bfd_cgc_get_dynamic_reloc_section (cgc_hash_table (info)
						  ->dynobj,
						  s->section,
						  /*rela?*/ TRUE);
	  sreloc->size -= s->count * sizeof (Cgc32_External_Rela);
	}
      return TRUE;
    }
  ...
  /* If we have accounted for PC-relative relocs for read-only
     ...
     cris_cgc_check_relocs, because we don't know the status of all
  ...
  for (s = h->pcrel_relocs_copied; s != NULL; s = s->next)
	...
	(*_bfd_error_handler)
	  (_("%B, section `%A', to symbol `%s':\n"
	   ...
	   cris_cgc_howto_table[s->r_type].name);
...
/* This function is called via cgc_cris_link_hash_traverse if we are *not*
...
static bfd_boolean
cgc_cris_discard_excess_program_dynamics (struct cgc_cris_link_hash_entry *h,
					  void * inf)
{
  struct bfd_link_info *info = (struct bfd_link_info *) inf;
...
static bfd_boolean
cris_cgc_object_p (bfd *abfd)
{
  if (! cris_cgc_set_mach_from_flags (abfd, cgc_cgcheader (abfd)->e_flags))
    return FALSE;

  if ((cgc_cgcheader (abfd)->e_flags & EF_CRIS_UNDERSCORE))
...
static void
cris_cgc_final_write_processing (bfd *abfd,
...
static bfd_boolean
cris_cgc_set_mach_from_flags (bfd *abfd,
			      unsigned long flags)
{
...
static bfd_boolean
cris_cgc_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
  ...
  if ((cgc_cgcheader (abfd)->e_flags & EF_CRIS_VARIANT_MASK)
...
static bfd_boolean
cris_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  int imach, omach;
  ...
  if (! cgc_flags_init (obfd))
    {
	 ...
	 somehow filter out the OUTPUT_ARCH () setting from cgc.sc.
...
static bfd_boolean
cris_cgc_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  /* Call the base function.  */
  if (!_bfd_cgc_copy_private_bfd_data (ibfd, obfd))
    return FALSE;
...
cgc_cris_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
			   const asection *rel_sec ATTRIBUTE_UNUSED,
			   const Cgc_Internal_Rela *rela)
{
  enum cgc_cris_reloc_type r_type = CGC32_R_TYPE (rela->r_info);
  switch (r_type)
    {
...
/* The cgc_backend_got_elt_size worker.  For one symbol, we can have up to
...
static bfd_vma
cgc_cris_got_elt_size (bfd *abfd ATTRIBUTE_UNUSED,
		       struct bfd_link_info *info ATTRIBUTE_UNUSED,
		       struct cgc_link_hash_entry *hr,
		       bfd *ibfd,
		       unsigned long symndx)
{
  struct cgc_link_hash_entry *h = (struct cgc_link_hash_entry *) hr;
  bfd_vma eltsiz = 0;
  ...
  if (h == NULL)
    {
      Cgc_Internal_Shdr *symtab_hdr = &cgc_tdata (ibfd)->symtab_hdr;
      bfd_signed_vma *local_got_refcounts = cgc_local_got_refcounts (ibfd);
    ...
    {
      struct cgc_cris_link_hash_entry *hh = cgc_cris_hash_entry (h);
...

#define CGC_ARCH		bfd_arch_cris
#define CGC_TARGET_ID		CRIS_CGC_DATA
#define CGC_MACHINE_CODE	EM_CRIS
#define CGC_MAXPAGESIZE		0x2000

#define TARGET_LITTLE_SYM	bfd_cgc32_cris_vec
#define TARGET_LITTLE_NAME	"cgc32-cris"
#define cgc_symbol_leading_char 0

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			cris_info_to_howto_rela
#define cgc_backend_relocate_section		cris_cgc_relocate_section
#define cgc_backend_gc_mark_hook		cris_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook		cris_cgc_gc_sweep_hook
#define cgc_backend_plt_sym_val			cris_cgc_plt_sym_val
#define cgc_backend_check_relocs                cris_cgc_check_relocs
#define cgc_backend_grok_prstatus		cris_cgc_grok_prstatus
#define cgc_backend_grok_psinfo			cris_cgc_grok_psinfo

#define cgc_backend_can_gc_sections		1
#define cgc_backend_can_refcount		1

#define cgc_backend_object_p			cris_cgc_object_p
#define cgc_backend_final_write_processing \
	cris_cgc_final_write_processing
#define bfd_cgc32_bfd_print_private_bfd_data \
	cris_cgc_print_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data \
	cris_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_copy_private_bfd_data \
	cris_cgc_copy_private_bfd_data

#define bfd_cgc32_bfd_reloc_type_lookup		cris_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	cris_reloc_name_lookup

#define bfd_cgc32_bfd_link_hash_table_create \
	cgc_cris_link_hash_table_create
#define cgc_backend_adjust_dynamic_symbol \
	cgc_cris_adjust_dynamic_symbol
#define cgc_backend_copy_indirect_symbol \
	cgc_cris_copy_indirect_symbol
#define cgc_backend_size_dynamic_sections \
	cgc_cris_size_dynamic_sections
#define cgc_backend_init_index_section		_bfd_cgc_init_1_index_section
#define cgc_backend_finish_dynamic_symbol \
	cgc_cris_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
	cgc_cris_finish_dynamic_sections
#define cgc_backend_create_dynamic_sections \
	_bfd_cgc_create_dynamic_sections
#define bfd_cgc32_bfd_final_link \
	bfd_cgc_gc_common_final_link
#define cgc_backend_hide_symbol			cgc_cris_hide_symbol
#define cgc_backend_reloc_type_class		cgc_cris_reloc_type_class
...
#define cgc_backend_got_elt_size cgc_cris_got_elt_size
...
#define TARGET_LITTLE_SYM bfd_cgc32_us_cris_vec
#define TARGET_LITTLE_NAME "cgc32-us-cris"
#define cgc_symbol_leading_char '_'
#undef cgc32_bed
#define cgc32_bed cgc32_us_cris_bed
