/* V850-specific support for 32-bit CGC
...
static reloc_howto_type v850_cgc_howto_table[];
...
static bfd_boolean
v850_cgc_check_relocs (bfd *abfd,
...
#ifdef DEBUG
  _bfd_error_handler ("v850_cgc_check_relocs called for section %A in %B",
		      sec, abfd);
#endif
...
#ifdef DEBUG
	  fprintf (stderr, "v850_cgc_check_relocs, found %s relocation for %s%s\n",
		   v850_cgc_howto_table[ (int)r_type ].name,
...
static bfd_boolean
v850_cgc_perform_lo16_relocation (bfd *abfd, unsigned long *insn,
...
v850_cgc_perform_relocation (bfd *abfd,
			     unsigned int r_type,
			     bfd_vma addend,
			     bfd_byte *address)
{
  ...
  switch (r_type)
    ...
    case R_V850_LO16:
      insn = bfd_get_16 (abfd, address);
      if (! v850_cgc_perform_lo16_relocation (abfd, &insn, addend))
	return bfd_reloc_overflow;
      break;
    ...
    case R_V850_LO16_S1:
      insn = bfd_get_16 (abfd, address);
      result = insn & 0xfffe;
      if (! v850_cgc_perform_lo16_relocation (abfd, &result, addend))
	return bfd_reloc_overflow;
      if (result & 1)
	return bfd_reloc_overflow;
      insn = (result & 0xfffe)
...
static bfd_reloc_status_type
v850_cgc_reloc (bfd *abfd ATTRIBUTE_UNUSED,
		arelent *reloc,
		asymbol *symbol,
...
static bfd_reloc_status_type
v850_cgc_ignore_reloc (bfd *abfd ATTRIBUTE_UNUSED,
		       arelent *reloc_entry,
...
/* Note: It is REQUIRED that the 'type' value of each entry
   ...
   SeeAlso: RELOC_NUBMER in include/cgc/v850.h.  */
static reloc_howto_type v850_cgc_howto_table[] =
{
  ...
  HOWTO (R_V850_9_PCREL,		/* Type.  */
	 ...
	 0,				/* Bitpos.  */
	 complain_overflow_bitfield,	/* Complain_on_overflow.  */
	 v850_cgc_reloc,		/* Special_function.  */
	 "R_V850_9_PCREL",		/* Name.  */
	 FALSE,				/* Partial_inplace.  */
	 0x00ffffff,			/* Src_mask.  */
	 0x00ffffff,			/* Dst_mask.  */
  ...
  HOWTO (R_V850_LONGCALL,     /* Type.  */
	 ...
	 0,                     /* Bitpos.  */
	 complain_overflow_signed, /* Complain_on_overflow.  */
	 v850_cgc_ignore_reloc, /* Special_function.  */
...
/* Map BFD reloc types to V850 CGC reloc types.  */

struct v850_cgc_reloc_map
{
  /* BFD_RELOC_V850_CALLT_16_16_OFFSET is 258, which will not fix in an
     unsigned char.  */
  bfd_reloc_code_real_type bfd_reloc_val;
  unsigned int cgc_reloc_val;
};

static const struct v850_cgc_reloc_map v850_cgc_reloc_map[] =
{
...
#define V800_RELOC(name,sz,bit,shift,complain,pcrel,resolver)		 \
  HOWTO (name, shift, sz, bit, pcrel, 0, complain_overflow_ ## complain, \
	 bfd_cgc_ ## resolver ## _reloc, #name, FALSE, 0, ~0, FALSE)

#define V800_EMPTY(name) EMPTY_HOWTO (name - R_V810_NONE)

#define bfd_cgc_v850_reloc v850_cgc_reloc
...
/* Note: It is REQUIRED that the 'type' value (R_V810_...) of each entry
   ...
   See: bfd_cgc_v850_relocate_section(), v800_cgc_reloc_type_lookup()
   and v800_cgc_info_to_howto().  */

static reloc_howto_type v800_cgc_howto_table[] =
{
...
v850_cgc_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = ARRAY_SIZE (v850_cgc_reloc_map); i --;)
    if (v850_cgc_reloc_map[i].bfd_reloc_val == code)
      {
	unsigned int cgc_reloc_val = v850_cgc_reloc_map[i].cgc_reloc_val;

	BFD_ASSERT (v850_cgc_howto_table[cgc_reloc_val].type == cgc_reloc_val);

	return v850_cgc_howto_table + cgc_reloc_val;
      }
...
static reloc_howto_type *
v850_cgc_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (v850_cgc_howto_table) / sizeof (v850_cgc_howto_table[0]);
       i++)
    if (v850_cgc_howto_table[i].name != NULL
	&& strcasecmp (v850_cgc_howto_table[i].name, r_name) == 0)
      return &v850_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an V850 CGC reloc.  */

static void
v850_cgc_info_to_howto_rel (bfd *abfd ATTRIBUTE_UNUSED,
			    arelent *cache_ptr,
			    Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = &v850_cgc_howto_table[r_type];
}

/* Set the howto pointer for a V850 CGC reloc (type RELA).  */

static void
v850_cgc_info_to_howto_rela (bfd *abfd ATTRIBUTE_UNUSED,
			     arelent * cache_ptr,
			     Cgc_Internal_Rela *dst)
{
  unsigned int r_type;
...

static bfd_boolean
v850_cgc_is_local_label_name (bfd *abfd ATTRIBUTE_UNUSED, const char *name)
{
...
static bfd_boolean
v850_cgc_is_target_special_symbol (bfd *abfd, asymbol *sym)
{
  return v850_cgc_is_local_label_name (abfd, sym->name);
}

...
static bfd_reloc_status_type
v850_cgc_final_link_relocate (reloc_howto_type *howto,
			      bfd *input_bfd,
			      ...
			      int is_local ATTRIBUTE_UNUSED)
{
  ...
  return v850_cgc_perform_relocation (input_bfd, r_type, value + addend, hit_data);
}

/* Relocate an V850 CGC section.  */

static bfd_boolean
v850_cgc_relocate_section (bfd *output_bfd,
			   struct bfd_link_info *info,
			   bfd *input_bfd,
			   asection *input_section,
			   bfd_byte *contents,
			   ...
			   asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	...
	howto = v800_cgc_howto_table + (r_type - R_V810_NONE);
      else
	howto = v850_cgc_howto_table + r_type;
	...
	continue;
      ...
      r = v850_cgc_final_link_relocate (howto, input_bfd, output_bfd,
					input_section,
...
static asection *
v850_cgc_gc_mark_hook (asection *sec,
		       struct bfd_link_info *info,
		       Cgc_Internal_Rela *rel,
		       struct cgc_link_hash_entry *h,
		       Cgc_Internal_Sym *sym)
...
static bfd_boolean
v850_cgc_object_p (bfd *abfd)
{
  enum bfd_architecture arch;
  unsigned long mach;

  switch (cgc_cgcheader (abfd)->e_machine)
    {
    case EM_V800:
      arch = bfd_arch_v850_rh850;
      mach = (cgc_cgcheader (abfd)->e_flags & EF_V800_850E3)
    ...
    case EM_V850:
      arch = bfd_arch_v850;
      switch (cgc_cgcheader (abfd)->e_flags & EF_V850_ARCH)
	{
	default:
...
static void
v850_cgc_final_write_processing (bfd *abfd,
				 bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  switch (bfd_get_arch (abfd))
	...
	case bfd_mach_v850e3v5: val = E_V850E3V5_ARCH; break;
      ...
      cgc_cgcheader (abfd)->e_flags &=~ EF_V850_ARCH;
      cgc_cgcheader (abfd)->e_flags |= val;
      break;
    default:
      break;
...
static bfd_boolean
v850_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
v850_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword out_flags;
  flagword in_flags;
  ...
  if (bfd_get_arch (obfd) == bfd_arch_v850_rh850)
      ...
      if ((in_flags & EF_V800_850E3) != (out_flags & EF_V800_850E3))
	{
	  _bfd_error_handler (_("%B: Architecture mismatch with previous modules"),
			      ibfd);
	  cgc_cgcheader (obfd)->e_flags |= EF_V800_850E3;
	}
      ...
      if ((in_flags & EF_RH850_DATA_ALIGN8) != (out_flags & EF_RH850_DATA_ALIGN8))
	{
	  _bfd_error_handler (_("%B: Alignment mismatch with previous modules"),
			      ibfd);
	  cgc_cgcheader (obfd)->e_flags |= EF_RH850_DATA_ALIGN8;
	}
...
static bfd_boolean
v850_cgc_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE * file = (FILE *) ptr;
  ...
  if (bfd_get_arch (abfd) == bfd_arch_v850_rh850)
    {
      if ((cgc_cgcheader (abfd)->e_flags & EF_RH850_ABI) != EF_RH850_ABI)
	fprintf (file, _("unknown v850 architecture"));
      else if (cgc_cgcheader (abfd)->e_flags & EF_V800_850E3)
	fprintf (file, _("v850 E3 architecture"));
      else
	fprintf (file, _("v850 architecture"));

      if (cgc_cgcheader (abfd)->e_flags & EF_RH850_DATA_ALIGN8)
...
/* V850 CGC uses four common sections.  One is the usual one, and the
   ...
   approach is copied from cgc32-mips.c.  */

static asection  v850_cgc_scom_section;
static asymbol   v850_cgc_scom_symbol;
static asymbol * v850_cgc_scom_symbol_ptr;
static asection  v850_cgc_tcom_section;
static asymbol   v850_cgc_tcom_symbol;
static asymbol * v850_cgc_tcom_symbol_ptr;
static asection  v850_cgc_zcom_section;
static asymbol   v850_cgc_zcom_symbol;
static asymbol * v850_cgc_zcom_symbol_ptr;

/* Given a BFD section, try to locate the
   corresponding CGC section index.  */

static bfd_boolean
v850_cgc_section_from_bfd_section (bfd *abfd ATTRIBUTE_UNUSED,
				   asection *sec,
				   int *retval)
{
...
static void
v850_cgc_symbol_processing (bfd *abfd, asymbol *asym)
{
  cgc_symbol_type * cgcsym = (cgc_symbol_type *) asym;
  unsigned int indx;

  indx = cgcsym->internal_cgc_sym.st_shndx;
  ...
  if (indx < cgc_numsections (abfd))
    switch (cgc_cgcsections (abfd)[indx]->sh_type)
      {
  ...
  switch (indx)
    ...
    case SHN_V850_SCOMMON:
      if (v850_cgc_scom_section.name == NULL)
	{
	  /* Initialize the small common section.  */
	  v850_cgc_scom_section.name           = ".scommon";
	  v850_cgc_scom_section.flags          = SEC_IS_COMMON | SEC_ALLOC | SEC_DATA;
	  v850_cgc_scom_section.output_section = & v850_cgc_scom_section;
	  v850_cgc_scom_section.symbol         = & v850_cgc_scom_symbol;
	  v850_cgc_scom_section.symbol_ptr_ptr = & v850_cgc_scom_symbol_ptr;
	  v850_cgc_scom_symbol.name            = ".scommon";
	  v850_cgc_scom_symbol.flags           = BSF_SECTION_SYM;
	  v850_cgc_scom_symbol.section         = & v850_cgc_scom_section;
	  v850_cgc_scom_symbol_ptr             = & v850_cgc_scom_symbol;
	}
      asym->section = & v850_cgc_scom_section;
      asym->value = cgcsym->internal_cgc_sym.st_size;
      break;

    case SHN_V850_TCOMMON:
      if (v850_cgc_tcom_section.name == NULL)
	{
	  /* Initialize the tcommon section.  */
	  v850_cgc_tcom_section.name           = ".tcommon";
	  v850_cgc_tcom_section.flags          = SEC_IS_COMMON;
	  v850_cgc_tcom_section.output_section = & v850_cgc_tcom_section;
	  v850_cgc_tcom_section.symbol         = & v850_cgc_tcom_symbol;
	  v850_cgc_tcom_section.symbol_ptr_ptr = & v850_cgc_tcom_symbol_ptr;
	  v850_cgc_tcom_symbol.name            = ".tcommon";
	  v850_cgc_tcom_symbol.flags           = BSF_SECTION_SYM;
	  v850_cgc_tcom_symbol.section         = & v850_cgc_tcom_section;
	  v850_cgc_tcom_symbol_ptr             = & v850_cgc_tcom_symbol;
	}
      asym->section = & v850_cgc_tcom_section;
      asym->value = cgcsym->internal_cgc_sym.st_size;
      break;

    case SHN_V850_ZCOMMON:
      if (v850_cgc_zcom_section.name == NULL)
	{
	  /* Initialize the zcommon section.  */
	  v850_cgc_zcom_section.name           = ".zcommon";
	  v850_cgc_zcom_section.flags          = SEC_IS_COMMON;
	  v850_cgc_zcom_section.output_section = & v850_cgc_zcom_section;
	  v850_cgc_zcom_section.symbol         = & v850_cgc_zcom_symbol;
	  v850_cgc_zcom_section.symbol_ptr_ptr = & v850_cgc_zcom_symbol_ptr;
	  v850_cgc_zcom_symbol.name            = ".zcommon";
	  v850_cgc_zcom_symbol.flags           = BSF_SECTION_SYM;
	  v850_cgc_zcom_symbol.section         = & v850_cgc_zcom_section;
	  v850_cgc_zcom_symbol_ptr             = & v850_cgc_zcom_symbol;
	}
      asym->section = & v850_cgc_zcom_section;
      asym->value = cgcsym->internal_cgc_sym.st_size;
      break;
    }
}
...
static bfd_boolean
v850_cgc_add_symbol_hook (bfd *abfd,
...
static int
v850_cgc_link_output_symbol_hook (struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
v850_cgc_section_from_shdr (bfd *abfd,
...
/* Set the correct type for a V850 CGC section.  We do this
   by the section name, which is a hack, but ought to work.  */

static bfd_boolean
v850_cgc_fake_sections (bfd *abfd ATTRIBUTE_UNUSED,
			Cgc_Internal_Shdr *hdr,
			asection *sec)
{
  const char * name;
...
static bfd_boolean
v850_cgc_relax_delete_bytes (bfd *abfd,
			     asection *sec,
			     bfd_vma addr,
			     bfd_vma toaddr,
			     int count)
{
  Cgc_Internal_Shdr *symtab_hdr;
  Cgc32_External_Sym *extsyms;
  Cgc32_External_Sym *esym;
  Cgc32_External_Sym *esymend;
  int sym_index;
  unsigned int sec_shndx;
  bfd_byte *contents;
  Cgc_Internal_Rela *irel;
  Cgc_Internal_Rela *irelend;
  struct cgc_link_hash_entry *sym_hash;
  Cgc_Internal_Shdr *shndx_hdr;
  Cgc_External_Sym_Shndx *shndx;

  symtab_hdr = &cgc_tdata (abfd)->symtab_hdr;
  extsyms = (Cgc32_External_Sym *) symtab_hdr->contents;
  ...
  shndx_hdr = &cgc_tdata (abfd)->symtab_shndx_hdr;
  shndx = (Cgc_External_Sym_Shndx *) shndx_hdr->contents;
  ...
  for (; irel < irelend; irel++)
      ...
      if (raddr >= addr && raddr < addr + count)
	{
      ...
      if (CGC32_R_TYPE (irel->r_info) == (int) R_V850_ALIGN)
	continue;

      bfd_cgc32_swap_symbol_in (abfd,
				extsyms + CGC32_R_SYM (irel->r_info),
				shndx ? shndx + CGC32_R_SYM (irel->r_info) : NULL,
				& isym);
...
#if (DEBUG_RELAX & 2)
	  {
	    char * name = bfd_cgc_string_from_cgc_section
  ...
  for (; esym < esymend; esym++, shndx = (shndx ? shndx + 1 : NULL))
    {
      ...
      bfd_cgc32_swap_symbol_in (abfd, esym, shndx, & isym);
	  ...
	  && isym.st_value < toaddr)
	{
	  ...
	  bfd_cgc32_swap_symbol_out (abfd, & isym, esym, shndx);
	}
  ...
  esymend = extsyms + (symtab_hdr->sh_size / sizeof (Cgc32_External_Sym));
  ...
  for (sym_index = 0; esym < esymend; esym ++, sym_index ++)
    {
      ...
      bfd_cgc32_swap_symbol_in (abfd, esym, shndx, & isym);
      sym_hash = cgc_sym_hashes (abfd) [sym_index];
...
static bfd_boolean
v850_cgc_relax_section (bfd *abfd,
			asection *sec,
			...
			bfd_boolean *again)
{
  ...
  Cgc_Internal_Rela *irel;
  Cgc_Internal_Rela *irelend;
  Cgc_Internal_Rela *irelalign = NULL;
  ...
  *again = FALSE;
  ...
  while (addr < sec->size)
...
#endif
	      if (!v850_cgc_relax_delete_bytes (abfd, sec, alignmoveto,
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  ...
	  int insn[5];
	  int no_match = -1;
	  Cgc_Internal_Rela *hi_ircgcn;
	  Cgc_Internal_Rela *lo_ircgcn;
	  Cgc_Internal_Rela *irelcall;
	  bfd_signed_vma foff;
	  unsigned int r_type;

	  if (! (irel->r_offset >= addr && irel->r_offset < toaddr
		 && (CGC32_R_TYPE (irel->r_info) == (int) R_V850_LONGCALL
		     || CGC32_R_TYPE (irel->r_info) == (int) R_V850_LONGJUMP)))
	    continue;
	  ...
	  if (CGC32_R_TYPE (irel->r_info) == (int) R_V850_LONGCALL)
	    {
	      ...
	      if (no_match >= 0)
		{
	      ...
	      for (hi_ircgcn = internal_relocs; hi_ircgcn < irelend; hi_ircgcn ++)
		{
		  r_type = CGC32_R_TYPE (hi_ircgcn->r_info);

		  if (hi_ircgcn->r_offset == laddr + 2
		      && (r_type == (int) R_V850_HI16_S || r_type == (int) R_V810_WHI1))
		    break;
		}

	      for (lo_ircgcn = internal_relocs; lo_ircgcn < irelend; lo_ircgcn ++)
		{
		  r_type = CGC32_R_TYPE (lo_ircgcn->r_info);

		  if (lo_ircgcn->r_offset == laddr + 6
		...
		{
		  r_type = CGC32_R_TYPE (irelcall->r_info);
		  ...
		  || irelcall  == irelend)
		{
	      ...
	      if (CGC32_R_SYM (irelcall->r_info) < symtab_hdr->sh_info)
		{
		  Cgc_Internal_Sym *  isym;

		  /* A local symbol.  */
		  isym = isymbuf + CGC32_R_SYM (irelcall->r_info);
	      ...
	      else
		{
		  ...
		  /* An external symbol.  */
		  indx = CGC32_R_SYM (irelcall->r_info) - symtab_hdr->sh_info;
		  h = cgc_sym_hashes (abfd)[indx];
		  BFD_ASSERT (h != NULL);
	      ...
	      if (symval + irelcall->r_addend != irelcall->r_offset + 4)
		{
	      ...
	      /* Get the value of the symbol referred to by the reloc.  */
	      if (CGC32_R_SYM (hi_ircgcn->r_info) < symtab_hdr->sh_info)
		{
		  Cgc_Internal_Sym *isym;
		  asection *sym_sec;

		  /* A local symbol.  */
		  isym = isymbuf + CGC32_R_SYM (hi_ircgcn->r_info);
	      ...
	      else
		{
		  ...
		  /* An external symbol.  */
		  indx = CGC32_R_SYM (hi_ircgcn->r_info) - symtab_hdr->sh_info;
		  h = cgc_sym_hashes (abfd)[indx];
		  BFD_ASSERT (h != NULL);
		...
		continue;
	      ...
	      if (bfd_get_arch (abfd) == bfd_arch_v850_rh850)
		irel->r_info = CGC32_R_INFO (CGC32_R_SYM (hi_ircgcn->r_info), R_V850_PCR22);
	      else
		irel->r_info = CGC32_R_INFO (CGC32_R_SYM (hi_ircgcn->r_info), R_V850_22_PCREL);
		...
		bfd_put_32 (abfd, 0x00000780 | (JARL_R2 (insn[2])<<11),
	      ...
	      hi_ircgcn->r_info =
		CGC32_R_INFO (CGC32_R_SYM (hi_ircgcn->r_info), R_V850_NONE);
	      lo_ircgcn->r_info =
		CGC32_R_INFO (CGC32_R_SYM (lo_ircgcn->r_info), R_V850_NONE);
	      irelcall->r_info =
		CGC32_R_INFO (CGC32_R_SYM (irelcall->r_info), R_V850_NONE);

	      if (! v850_cgc_relax_delete_bytes (abfd, sec,
						 irel->r_offset + 4, toaddr, 12))
		goto error_return;

	      align_pad_size += 12;
	    }
	  else if (CGC32_R_TYPE (irel->r_info) == (int) R_V850_LONGJUMP)
	    {
	      ...
	      if (foff < -0x100 || foff >= 0x100)
		{
		  ...
		  if (bfd_get_arch (abfd) == bfd_arch_v850_rh850)
		    irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_V850_PCR22);
		  else
		    irel->r_info =
		      CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_V850_22_PCREL);
	      ...
	      else
		{
		  ...
		  if (bfd_get_arch (abfd) == bfd_arch_v850_rh850)
		    irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_V850_PC9);
		  else
		    irel->r_info =
		      CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_V850_9_PCREL);
 ...
 finish:
  ...
  if (contents != NULL
      && cgc_section_data (sec)->this_hdr.contents != (unsigned char *) contents)
    free (contents);
...
static const struct bfd_cgc_special_section v850_cgc_special_sections[] =
{
...

#define TARGET_LITTLE_SYM			bfd_cgc32_v850_vec
#define TARGET_LITTLE_NAME			"cgc32-v850"
#define CGC_ARCH				bfd_arch_v850
#define CGC_MACHINE_CODE			EM_V850
#define CGC_MACHINE_ALT1			EM_CYGNUS_V850
#define CGC_MAXPAGESIZE				0x1000

#define cgc_info_to_howto			v850_cgc_info_to_howto_rela
#define cgc_info_to_howto_rel			v850_cgc_info_to_howto_rel

#define cgc_backend_check_relocs		v850_cgc_check_relocs
#define cgc_backend_relocate_section    	v850_cgc_relocate_section
#define cgc_backend_object_p			v850_cgc_object_p
#define cgc_backend_final_write_processing 	v850_cgc_final_write_processing
#define cgc_backend_section_from_bfd_section 	v850_cgc_section_from_bfd_section
#define cgc_backend_symbol_processing		v850_cgc_symbol_processing
#define cgc_backend_add_symbol_hook		v850_cgc_add_symbol_hook
#define cgc_backend_link_output_symbol_hook 	v850_cgc_link_output_symbol_hook
#define cgc_backend_section_from_shdr		v850_cgc_section_from_shdr
#define cgc_backend_fake_sections		v850_cgc_fake_sections
#define cgc_backend_gc_mark_hook                v850_cgc_gc_mark_hook
#define cgc_backend_special_sections		v850_cgc_special_sections

#define cgc_backend_can_gc_sections 1
#define cgc_backend_rela_normal 1

#define bfd_cgc32_bfd_is_local_label_name	v850_cgc_is_local_label_name
#define bfd_cgc32_bfd_is_target_special_symbol	v850_cgc_is_target_special_symbol

#define bfd_cgc32_bfd_reloc_type_lookup		v850_cgc_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	        v850_cgc_reloc_name_lookup
#define bfd_cgc32_bfd_merge_private_bfd_data 	v850_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_set_private_flags		v850_cgc_set_private_flags
#define bfd_cgc32_bfd_print_private_bfd_data	v850_cgc_print_private_bfd_data
#define bfd_cgc32_bfd_relax_section		v850_cgc_relax_section

#define cgc_symbol_leading_char			'_'

#undef  cgc32_bed
#define cgc32_bed cgc32_v850_bed

#include "cgc32-target.h"

/* Map BFD reloc types to V800 CGC reloc types.  */

static const struct v850_cgc_reloc_map v800_cgc_reloc_map[] =
{
...
static reloc_howto_type *
v800_cgc_reloc_type_lookup (bfd * abfd, bfd_reloc_code_real_type code)
{
  unsigned int i;

  BFD_ASSERT (bfd_get_arch (abfd) == bfd_arch_v850_rh850);

  for (i = ARRAY_SIZE (v800_cgc_reloc_map); i --;)
    if (v800_cgc_reloc_map[i].bfd_reloc_val == code)
      {
	unsigned int cgc_reloc_val = v800_cgc_reloc_map[i].cgc_reloc_val;
	unsigned int idx = cgc_reloc_val - R_V810_NONE;

	BFD_ASSERT (v800_cgc_howto_table[idx].type == cgc_reloc_val);

	return v800_cgc_howto_table + idx;
      }
...
static reloc_howto_type *
v800_cgc_reloc_name_lookup (bfd * abfd, const char * r_name)
{
  unsigned int i;

  BFD_ASSERT (bfd_get_arch (abfd) == bfd_arch_v850_rh850);

  for (i = ARRAY_SIZE (v800_cgc_howto_table); i--;)
    if (v800_cgc_howto_table[i].name != NULL
	&& strcasecmp (v800_cgc_howto_table[i].name, r_name) == 0)
      return v800_cgc_howto_table + i;

  return NULL;
}


/* Set the howto pointer in CACHE_PTR for a V800 CGC reloc.  */

static void
v800_cgc_info_to_howto (bfd *               abfd,
			arelent *           cache_ptr,
			Cgc_Internal_Rela * dst)
{
  ...
  r_type -= R_V810_NONE;
  BFD_ASSERT (r_type < ARRAY_SIZE (v800_cgc_howto_table));

  cache_ptr->howto = v800_cgc_howto_table + r_type;
}


#undef  TARGET_LITTLE_SYM
#define TARGET_LITTLE_SYM			bfd_cgc32_v850_rh850_vec
#undef  TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME			"cgc32-v850-rh850"
#undef  CGC_ARCH
#define CGC_ARCH				bfd_arch_v850_rh850
#undef  CGC_MACHINE_CODE
#define CGC_MACHINE_CODE			EM_V800
#undef  CGC_MACHINE_ALT1

#undef  cgc32_bed
#define cgc32_bed cgc32_v850_rh850_bed

#undef  cgc_info_to_howto
#define cgc_info_to_howto			v800_cgc_info_to_howto
#undef  cgc_info_to_howto_rel
#define cgc_info_to_howto_rel			NULL
#undef  bfd_cgc32_bfd_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_type_lookup		v800_cgc_reloc_type_lookup
#undef  bfd_cgc32_bfd_reloc_name_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		v800_cgc_reloc_name_lookup
