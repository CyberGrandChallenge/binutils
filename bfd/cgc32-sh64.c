/* SuperH SH64-specific support for 32-bit CGC
...
static bfd_boolean sh64_cgc_new_section_hook
  (bfd *, asection *);
static bfd_boolean sh64_cgc_copy_private_data
  (bfd *, bfd *);
static bfd_boolean sh64_cgc_merge_private_data
  (bfd *, bfd *);
static bfd_boolean sh64_cgc_fake_sections
  (bfd *, Cgc_Internal_Shdr *, asection *);
static bfd_boolean sh64_cgc_set_private_flags
  (bfd *, flagword);
static bfd_boolean sh64_cgc_set_mach_from_flags
  (bfd *);
static bfd_boolean shmedia_prepare_reloc
  (struct bfd_link_info *, bfd *, asection *, bfd_byte *,
   const Cgc_Internal_Rela *, bfd_vma *);
static int sh64_cgc_get_symbol_type
  (Cgc_Internal_Sym *, int);
static bfd_boolean sh64_cgc_add_symbol_hook
  (bfd *, struct bfd_link_info *, Cgc_Internal_Sym *, const char **,
   flagword *, asection **, bfd_vma *);
static int sh64_cgc_link_output_symbol_hook
...
static void sh64_cgc_final_write_processing
  (bfd *, bfd_boolean);
static bfd_boolean sh64_bfd_cgc_copy_private_section_data
  (bfd *, asection *, bfd *, asection *);
static void sh64_find_section_for_address
  (bfd *, asection *, void *);

/* Let cgc32-sh.c handle the "bfd_" definitions, so we only have to
   intrude with an #ifndef around the function definition.  */
#define sh_cgc_copy_private_data		sh64_cgc_copy_private_data
#define sh_cgc_merge_private_data		sh64_cgc_merge_private_data
#define sh_cgc_set_private_flags		sh64_cgc_set_private_flags
/* Typo in cgc32-sh.c (and unlinear name).  */
#define bfd_cgc32_bfd_set_private_flags		sh64_cgc_set_private_flags
#define sh_cgc_set_mach_from_flags		sh64_cgc_set_mach_from_flags

#define cgc_backend_sign_extend_vma		1
#define cgc_backend_fake_sections		sh64_cgc_fake_sections
#define cgc_backend_get_symbol_type		sh64_cgc_get_symbol_type
#define cgc_backend_add_symbol_hook		sh64_cgc_add_symbol_hook
#define cgc_backend_link_output_symbol_hook \
	sh64_cgc_link_output_symbol_hook
#define cgc_backend_merge_symbol_attribute	sh64_cgc_merge_symbol_attribute
#define cgc_backend_final_write_processing 	sh64_cgc_final_write_processing
#define cgc_backend_section_from_shdr		sh64_backend_section_from_shdr
#define cgc_backend_special_sections		sh64_cgc_special_sections
#define cgc_backend_section_flags		sh64_cgc_section_flags

#define bfd_cgc32_new_section_hook		sh64_cgc_new_section_hook

/* For objcopy, we need to set up sh64_cgc_section_data (asection *) from
   incoming section flags.  This is otherwise done in sh64cgc.em when
   linking or tc-sh64.c when assembling.  */
#define bfd_cgc32_bfd_copy_private_section_data \
	sh64_bfd_cgc_copy_private_section_data

/* This COFF-only function (only compiled with COFF support, making
   CGC-only chains problematic) returns TRUE early for SH4, so let's just
   define it TRUE here.  */
...
/* Tack some extra info on struct bfd_cgc_section_data.  */

static bfd_boolean
sh64_cgc_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      struct _sh64_cgc_section_data *sdata;
...
bfd_boolean
sh64_cgc_fake_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
			Cgc_Internal_Shdr *cgc_section_hdr,
			asection *asect)
{
  if (sh64_cgc_section_data (asect)->sh64_info != NULL)
    cgc_section_hdr->sh_flags
      |= sh64_cgc_section_data (asect)->sh64_info->contents_flags;
  ...
  if ((bfd_get_section_flags (output_bfd, asect) & SEC_SORT_ENTRIES) != 0
    ...
    cgc_section_hdr->sh_type = SHT_SH5_CR_SORTED;

  return TRUE;
}

static bfd_boolean
sh64_cgc_set_mach_from_flags (bfd *abfd)
{
...
static bfd_boolean
sh64_cgc_section_flags (flagword *flags,
...
static bfd_boolean
sh64_cgc_copy_private_data (bfd * ibfd, bfd * obfd)
{
...
static bfd_boolean
sh64_cgc_merge_private_data (bfd *ibfd, bfd *obfd)
{
  ...
  return sh64_cgc_set_mach_from_flags (obfd);
}
...
bfd_boolean
sh64_backend_section_from_shdr (bfd *abfd, Cgc_Internal_Shdr *hdr,
				const char *name, int shindex)
{
  ...
  switch (hdr->sh_type)
	...
	return FALSE;
	 ...
	 sh64_cgc_fake_sections, keeping SHT_SH5_CR_SORTED if this object
...
/* In contrast to sh64_backend_section_from_shdr, this is called for all
   ...
   assembling.  We need to set up the sh64_cgc_section_data (asection *)
   structure for the SH64 CGC section flags to be copied correctly.  */

bfd_boolean
sh64_bfd_cgc_copy_private_section_data (bfd *ibfd, asection *isec,
					bfd *obfd, asection *osec)
{
  ...
  if (! _bfd_cgc_copy_private_section_data (ibfd, isec, obfd, osec))
    return FALSE;

  sh64_sec_data = sh64_cgc_section_data (isec)->sh64_info;
  if (sh64_sec_data == NULL)
	...
	= (cgc_section_data (isec)->this_hdr.sh_flags
	   & (SHF_SH5_ISA32 | SHF_SH5_ISA32_MIXED));

      sh64_cgc_section_data (osec)->sh64_info = sh64_sec_data;
    }
...
static bfd_boolean
sh64_cgc_set_private_flags (bfd *abfd, flagword flags)
{
  ...
  return sh64_cgc_set_mach_from_flags (abfd);
}

/* Called when writing out an object file to decide the type of a symbol.  */

static int
sh64_cgc_get_symbol_type (Cgc_Internal_Sym *cgc_sym, int type)
{
...
static bfd_boolean
sh64_cgc_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
...
static int
sh64_cgc_link_output_symbol_hook (struct bfd_link_info *info,
				  const char *cname,
...
shmedia_prepare_reloc (struct bfd_link_info *info, bfd *abfd,
		       ...
		       const Cgc_Internal_Rela *rel, bfd_vma *relocation)
{
  ...
  if (dropped != 0)
	...
	(_("%B: error: unaligned relocation type %d at %08x reloc %p\n"),
	 input_section->owner, CGC32_R_TYPE (rel->r_info),
...
static void
sh64_cgc_final_write_processing (bfd *abfd,
				 bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  /* If no new .cranges were added, the generic CGC linker parts will
  ...
  if (linker
      && cranges != NULL
      && cgc_cgcheader (abfd)->e_type != ET_EXEC
      && (ld_generated_cranges_size
	  = sh64_cgc_section_data (cranges)->sh64_info->cranges_growth) != 0)
    {
  ...
  if (linker && cgc_cgcheader (abfd)->e_type == ET_EXEC)
    {
      struct sh64_find_section_vma_data fsec_data;
      sh64_cgc_crange dummy;
	 ...
	 or a symbol name is kept private in the linker.  */
      fsec_data.addr = cgc_cgcheader (abfd)->e_entry;
      ...
      if (cranges != NULL)
	{
	  ...
	  /* The .cranges may already have been sorted in the process of
	     ...
	     it here.  */
	  if (cgc_section_data (cranges)->this_hdr.sh_type
	      != SHT_SH5_CR_SORTED)
	    {
	      ...
	      cgc_section_data (cranges)->this_hdr.sh_type
		= SHT_SH5_CR_SORTED;
	    }
...
/* Merge non visibility st_other attribute when the symbol comes from
   a dynamic object.  */
static void
sh64_cgc_merge_symbol_attribute (struct cgc_link_hash_entry *h,
...
static const struct bfd_cgc_special_section sh64_cgc_special_sections[] =
{
...
#define	TARGET_BIG_SYM		bfd_cgc32_sh64_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME		"cgc32-sh64"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM	bfd_cgc32_sh64l_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME	"cgc32-sh64l"

#include "cgc32-target.h"

/* NetBSD support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM		bfd_cgc32_sh64nbsd_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME		"cgc32-sh64-nbsd"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM	bfd_cgc32_sh64lnbsd_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME	"cgc32-sh64l-nbsd"
...
#undef	cgc32_bed
#define	cgc32_bed		cgc32_sh64_nbsd_bed

#include "cgc32-target.h"

/* Linux support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM		bfd_cgc32_sh64blin_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME		"cgc32-sh64big-linux"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM	bfd_cgc32_sh64lin_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME	"cgc32-sh64-linux"
#undef	cgc32_bed
#define	cgc32_bed		cgc32_sh64_lin_bed
#undef	CGC_COMMONPAGESIZE
