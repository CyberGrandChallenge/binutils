/* moxie-specific support for 32-bit CGC.
   Copyright 2009, 2010, 2012 Free Software Foundation, Inc.

   Copied from cgc32-fr30.c which is..
...
static reloc_howto_type moxie_cgc_howto_table [] =
{
...

/* Map BFD reloc types to MOXIE CGC reloc types.  */
			 ...
			 bfd_reloc_code_real_type code)
{
  ...
  for (i = sizeof (moxie_reloc_map) / sizeof (moxie_reloc_map[0]);
    ...
    if (moxie_reloc_map [i].bfd_reloc_val == code)
      return & moxie_cgc_howto_table [moxie_reloc_map[i].moxie_reloc_val];
...
moxie_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (moxie_cgc_howto_table) / sizeof (moxie_cgc_howto_table[0]);
       i++)
    if (moxie_cgc_howto_table[i].name != NULL
	&& strcasecmp (moxie_cgc_howto_table[i].name, r_name) == 0)
      return &moxie_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an MOXIE CGC reloc.  */
			  ...
			  Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = & moxie_cgc_howto_table [r_type];
}

...
static bfd_boolean
moxie_cgc_relocate_section (bfd *output_bfd,
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = moxie_cgc_howto_table + r_type;
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
...
static asection *
moxie_cgc_gc_mark_hook (asection *sec,
			struct bfd_link_info *info,
			Cgc_Internal_Rela *rel,
			struct cgc_link_hash_entry *h,
			Cgc_Internal_Sym *sym)
...
static bfd_boolean
moxie_cgc_check_relocs (bfd *abfd,
			struct bfd_link_info *info,
			asection *sec,
...

#define CGC_ARCH		bfd_arch_moxie
#define CGC_MACHINE_CODE	EM_MOXIE
#define CGC_MAXPAGESIZE		0x1

#define TARGET_BIG_SYM          bfd_cgc32_bigmoxie_vec
#define TARGET_BIG_NAME		"cgc32-bigmoxie"
#define TARGET_LITTLE_SYM       bfd_cgc32_littlemoxie_vec
#define TARGET_LITTLE_NAME	"cgc32-littlemoxie"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			moxie_info_to_howto_rela
#define cgc_backend_relocate_section		moxie_cgc_relocate_section
#define cgc_backend_gc_mark_hook		moxie_cgc_gc_mark_hook
#define cgc_backend_check_relocs                moxie_cgc_check_relocs

#define cgc_backend_can_gc_sections		1
#define cgc_backend_rela_normal			1

#define bfd_cgc32_bfd_reloc_type_lookup		moxie_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		moxie_reloc_name_lookup
