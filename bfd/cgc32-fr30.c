/* FR30-specific support for 32-bit CGC.
...
fr30_cgc_i20_reloc (bfd *, arelent *, asymbol *, void * data,
		    asection *, bfd *, char **error_message);
static bfd_reloc_status_type
fr30_cgc_i32_reloc (bfd *, arelent *, asymbol *, void *,
		    asection *, bfd *, char **);

static reloc_howto_type fr30_cgc_howto_table [] =
{
  ...
  HOWTO (R_FR30_20,		/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_bitfield, /* complain_on_overflow */
	 fr30_cgc_i20_reloc,	/* special_function */
	 "R_FR30_20",		/* name */
	 FALSE,			/* partial_inplace */
	 0x00000000,		/* src_mask */
	 0x00f0ffff,		/* dst_mask */
  ...
  HOWTO (R_FR30_48,		/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_bitfield, /* complain_on_overflow */
	 fr30_cgc_i32_reloc,	/* special_function */
	 "R_FR30_48",		/* name */
	 FALSE,			/* partial_inplace */
	 0x00000000,		/* src_mask */
	 0xffffffff,		/* dst_mask */
...

/* Map BFD reloc types to FR30 CGC reloc types.  */
			...
			bfd_reloc_code_real_type code)
{
  ...
  for (i = sizeof (fr30_reloc_map) / sizeof (fr30_reloc_map[0]);
    ...
    if (fr30_reloc_map [i].bfd_reloc_val == code)
      return & fr30_cgc_howto_table [fr30_reloc_map[i].fr30_reloc_val];
...
fr30_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (fr30_cgc_howto_table) / sizeof (fr30_cgc_howto_table[0]);
       i++)
    if (fr30_cgc_howto_table[i].name != NULL
	&& strcasecmp (fr30_cgc_howto_table[i].name, r_name) == 0)
      return &fr30_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an FR30 CGC reloc.  */
			 ...
			 Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = & fr30_cgc_howto_table [r_type];
}

...
static bfd_boolean
fr30_cgc_relocate_section (bfd *output_bfd,
			   ...
			   asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto  = fr30_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
...
static asection *
fr30_cgc_gc_mark_hook (asection *sec,
		       struct bfd_link_info *info,
		       Cgc_Internal_Rela *rel,
		       struct cgc_link_hash_entry *h,
		       Cgc_Internal_Sym *sym)
...
static bfd_boolean
fr30_cgc_check_relocs (bfd *abfd,
		       struct bfd_link_info *info,
		       asection *sec,
...

#define CGC_ARCH		bfd_arch_fr30
#define CGC_MACHINE_CODE	EM_FR30
#define CGC_MACHINE_ALT1	EM_CYGNUS_FR30
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM          bfd_cgc32_fr30_vec
#define TARGET_BIG_NAME		"cgc32-fr30"

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			fr30_info_to_howto_rela
#define cgc_backend_relocate_section		fr30_cgc_relocate_section
#define cgc_backend_gc_mark_hook		fr30_cgc_gc_mark_hook
#define cgc_backend_check_relocs                fr30_cgc_check_relocs

#define cgc_backend_can_gc_sections		1
#define cgc_backend_rela_normal			1

#define bfd_cgc32_bfd_reloc_type_lookup		fr30_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	fr30_reloc_name_lookup
