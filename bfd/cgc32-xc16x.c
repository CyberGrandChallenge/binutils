/* Infineon XC16X-specific support for 16-bit CGC.
...
static reloc_howto_type xc16x_cgc_howto_table [] =
{
...
/* Map BFD reloc types to XC16X CGC reloc types.  */
			 ...
			 bfd_reloc_code_real_type code)
{
  ...
  for (i = ARRAY_SIZE (xc16x_reloc_map); --i;)
    if (xc16x_reloc_map [i].bfd_reloc_val == code)
      return & xc16x_cgc_howto_table [xc16x_reloc_map[i].xc16x_reloc_val];
			 ...
			 const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (xc16x_cgc_howto_table) / sizeof (xc16x_cgc_howto_table[0]);
       i++)
    if (xc16x_cgc_howto_table[i].name != NULL
	&& strcasecmp (xc16x_cgc_howto_table[i].name, r_name) == 0)
      return &xc16x_cgc_howto_table[i];
...
static void
cgc32_xc16x_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *bfd_reloc,
			   Cgc_Internal_Rela *cgc_reloc)
{
  ...
  for (i = 0; i < ARRAY_SIZE (xc16x_cgc_howto_table); i++)
    if (xc16x_cgc_howto_table[i].type == r)
      {
	bfd_reloc->howto = &xc16x_cgc_howto_table[i];
	return;
      }
  abort ();
}
...
cgc32_xc16x_final_link_relocate (unsigned long r_type,
				 bfd *input_bfd,
...
static bfd_boolean
cgc32_xc16x_relocate_section (bfd *output_bfd,
			      ...
			      asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	...
	continue;
      ...
      cgc32_xc16x_final_link_relocate (r_type, input_bfd, output_bfd,
				       input_section,
...
static void
cgc32_xc16x_final_write_processing (bfd *abfd,
...
cgc32_xc16x_mach (flagword flags)
{
  switch (flags)
    {
    case 0x1000:
...
static bfd_boolean
cgc32_xc16x_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_xc16x,
			     cgc32_xc16x_mach (cgc_cgcheader (abfd)->e_flags));
  return TRUE;
}


#define CGC_ARCH		bfd_arch_xc16x
#define CGC_MACHINE_CODE	EM_XC16X
#define CGC_MAXPAGESIZE		0x100

#define TARGET_LITTLE_SYM       bfd_cgc32_xc16x_vec
#define TARGET_LITTLE_NAME	"cgc32-xc16x"
#define cgc_backend_final_write_processing	cgc32_xc16x_final_write_processing
#define cgc_backend_object_p   		cgc32_xc16x_object_p
#define cgc_backend_can_gc_sections	1
#define bfd_cgc32_bfd_reloc_type_lookup	xc16x_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup xc16x_reloc_name_lookup
#define cgc_info_to_howto		cgc32_xc16x_info_to_howto
#define cgc_info_to_howto_rel		cgc32_xc16x_info_to_howto
#define cgc_backend_relocate_section  	cgc32_xc16x_relocate_section
