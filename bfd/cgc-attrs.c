/* CGC attributes support (based on ARM EABI attributes).
...
vendor_obj_attr_name (bfd *abfd, int vendor)
{
  return (vendor == OBJ_ATTR_PROC
	  ? get_cgc_backend_data (abfd)->obj_attrs_vendor
	  : "gnu");
}
...
vendor_obj_attr_size (bfd *abfd, int vendor)
{
  ...
  attr = cgc_known_obj_attributes (abfd)[vendor];
  size = 0;
  for (i = LEAST_KNOWN_OBJ_ATTRIBUTE; i < NUM_KNOWN_OBJ_ATTRIBUTES; i++)
    size += obj_attr_size (i, &attr[i]);

  for (list = cgc_other_obj_attributes (abfd)[vendor];
       list;
       list = list->next)
...
bfd_vma
bfd_cgc_obj_attr_size (bfd *abfd)
{
  bfd_vma size;
			      ...
			      int vendor)
{
  ...
  *(p++) = Tag_File;
  ...
  for (i = LEAST_KNOWN_OBJ_ATTRIBUTE; i < NUM_KNOWN_OBJ_ATTRIBUTES; i++)
    {
      int tag = i;
      if (get_cgc_backend_data (abfd)->obj_attrs_order)
	tag = get_cgc_backend_data (abfd)->obj_attrs_order (i);
...
void
bfd_cgc_set_obj_attr_contents (bfd *abfd, bfd_byte *contents, bfd_vma size)
{
  bfd_byte *p;
  int vendor;
  bfd_vma my_size;
...
cgc_new_obj_attr (bfd *abfd, int vendor, int tag)
{
  obj_attribute *attr;
  obj_attribute_list *list;
  ...
  if (tag < NUM_KNOWN_OBJ_ATTRIBUTES)
    {
      ...
      attr = &cgc_known_obj_attributes (abfd)[vendor][tag];
    }
  else
    {
      /* Create a new tag.  */
	...
	bfd_alloc (abfd, sizeof (obj_attribute_list));
      ...
      lastp = &cgc_other_obj_attributes (abfd)[vendor];
...
int
bfd_cgc_get_obj_attr_int (bfd *abfd, int vendor, int tag)
{
  obj_attribute_list *p;
  ...
  if (tag < NUM_KNOWN_OBJ_ATTRIBUTES)
    {
      /* Known tags are preallocated.  */
      return cgc_known_obj_attributes (abfd)[vendor][tag].i;
    }
  else
    {
      for (p = cgc_other_obj_attributes (abfd)[vendor];
	   p;
	   p = p->next)
	{
	  if (tag == p->tag)
...
void
bfd_cgc_add_obj_attr_int (bfd *abfd, int vendor, int tag, unsigned int i)
{
  obj_attribute *attr;

  attr = cgc_new_obj_attr (abfd, vendor, tag);
  attr->type = _bfd_cgc_obj_attrs_arg_type (abfd, vendor, tag);
  attr->i = i;
}

/* Duplicate an object attribute string value.  */
char *
_bfd_cgc_attr_strdup (bfd *abfd, const char * s)
{
  char * p;
  int len;
...
void
bfd_cgc_add_obj_attr_string (bfd *abfd, int vendor, int tag, const char *s)
{
  obj_attribute *attr;

  attr = cgc_new_obj_attr (abfd, vendor, tag);
  attr->type = _bfd_cgc_obj_attrs_arg_type (abfd, vendor, tag);
  attr->s = _bfd_cgc_attr_strdup (abfd, s);
}

/* Add a int+string object attribute.  */
void
bfd_cgc_add_obj_attr_int_string (bfd *abfd, int vendor, int tag,
...
void
_bfd_cgc_copy_obj_attributes (bfd *ibfd, bfd *obfd)
{
  ...
  for (vendor = OBJ_ATTR_FIRST; vendor <= OBJ_ATTR_LAST; vendor++)
    {
      in_attr
	= &cgc_known_obj_attributes (ibfd)[vendor][LEAST_KNOWN_OBJ_ATTRIBUTE];
      out_attr
	= &cgc_known_obj_attributes (obfd)[vendor][LEAST_KNOWN_OBJ_ATTRIBUTE];
      for (i = LEAST_KNOWN_OBJ_ATTRIBUTE; i < NUM_KNOWN_OBJ_ATTRIBUTES; i++)
	{
	  out_attr->type = in_attr->type;
	  out_attr->i = in_attr->i;
	  if (in_attr->s && *in_attr->s)
	    out_attr->s = _bfd_cgc_attr_strdup (obfd, in_attr->s);
	  in_attr++;
	  out_attr++;
	}

      for (list = cgc_other_obj_attributes (ibfd)[vendor];
	   list;
	   list = list->next)
	{
	  in_attr = &list->attr;
	  switch (in_attr->type & (ATTR_TYPE_FLAG_INT_VAL | ATTR_TYPE_FLAG_STR_VAL))
	    ...
	    case ATTR_TYPE_FLAG_INT_VAL:
	      bfd_cgc_add_obj_attr_int (obfd, vendor, list->tag, in_attr->i);
	      break;
	    case ATTR_TYPE_FLAG_STR_VAL:
	      bfd_cgc_add_obj_attr_string (obfd, vendor, list->tag,
					   in_attr->s);
	      break;
	    case ATTR_TYPE_FLAG_INT_VAL | ATTR_TYPE_FLAG_STR_VAL:
	      bfd_cgc_add_obj_attr_int_string (obfd, vendor, list->tag,
					       in_attr->i, in_attr->s);
	      break;
	    default:
	      abort ();
...
int
_bfd_cgc_obj_attrs_arg_type (bfd *abfd, int vendor, int tag)
{
  switch (vendor)
    {
    case OBJ_ATTR_PROC:
      return get_cgc_backend_data (abfd)->obj_attrs_arg_type (tag);
      break;
    case OBJ_ATTR_GNU:
...
void
_bfd_cgc_parse_attributes (bfd *abfd, Cgc_Internal_Shdr * hdr)
{
  bfd_byte *contents;
  bfd_byte *p;
  bfd_vma len;
  ...
  p = contents;
  std_sec = get_cgc_backend_data (abfd)->obj_attrs_vendor;
  if (*(p++) == 'A')
    {
      len = hdr->sh_size - 1;
      while (len > 0)
		  ...
		  while (p < end)
		    {
		      ...
		      tag = read_unsigned_leb128 (abfd, p, &n);
		      p += n;
		      type = _bfd_cgc_obj_attrs_arg_type (abfd, vendor, tag);
		      switch (type & (ATTR_TYPE_FLAG_INT_VAL | ATTR_TYPE_FLAG_STR_VAL))
			{
			case ATTR_TYPE_FLAG_INT_VAL | ATTR_TYPE_FLAG_STR_VAL:
			  val = read_unsigned_leb128 (abfd, p, &n);
			  p += n;
			  bfd_cgc_add_obj_attr_int_string (abfd, vendor, tag,
							   val, (char *)p);
			  p += strlen ((char *)p) + 1;
			  break;
			case ATTR_TYPE_FLAG_STR_VAL:
			  bfd_cgc_add_obj_attr_string (abfd, vendor, tag,
						       (char *)p);
			  p += strlen ((char *)p) + 1;
			  break;
			case ATTR_TYPE_FLAG_INT_VAL:
			  val = read_unsigned_leb128 (abfd, p, &n);
			  p += n;
			  bfd_cgc_add_obj_attr_int (abfd, vendor, tag, val);
			  break;
			default:
			  abort ();
			}
...
/* Merge common object attributes from IBFD into OBFD.  Raise an error
   ...
   bfd_cgcNN_bfd_merge_private_bfd_data hook for each individual
...
bfd_boolean
_bfd_cgc_merge_object_attributes (bfd *ibfd, bfd *obfd)
{
  obj_attribute *in_attr;
  obj_attribute *out_attr;
  int vendor;
  ...
  for (vendor = OBJ_ATTR_FIRST; vendor <= OBJ_ATTR_LAST; vendor++)
    {
	 ...
	 If the flags are non-zero, then we can only use the string "gnu".  */
      in_attr = &cgc_known_obj_attributes (ibfd)[vendor][Tag_compatibility];
      out_attr = &cgc_known_obj_attributes (obfd)[vendor][Tag_compatibility];
...
bfd_boolean
_bfd_cgc_merge_unknown_attribute_low (bfd *ibfd, bfd *obfd, int tag)
{
  obj_attribute *in_attr;
  obj_attribute *out_attr;
  bfd *err_bfd = NULL;
  ...
  if (err_bfd != NULL)
    result
      = get_cgc_backend_data (err_bfd)->obj_attrs_handle_unknown (err_bfd, tag);
...
bfd_boolean
_bfd_cgc_merge_unknown_attribute_list (bfd *ibfd, bfd *obfd)
{
  ...
  in_list = cgc_other_obj_attributes_proc (ibfd);
  out_listp = &cgc_other_obj_attributes_proc (obfd);
  out_list = *out_listp;
  ...
  for (; in_list || out_list; )
      ...
      else /* The tags are equal.  */
	{
      ...
      if (err_bfd)
	result = result
	  && get_cgc_backend_data (err_bfd)->obj_attrs_handle_unknown (err_bfd,
								       err_tag);
    }
