  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      unsigned short insn;
      struct internal_reloc *ircgcn, *irelscan, *irelcount;
      struct internal_syment sym;
      bfd_signed_vma foff;
	...
	if (ircgcn->r_vaddr == paddr
#ifdef COFF_WITH_PE
	    && (ircgcn->r_type == R_SH_IMM32
		|| ircgcn->r_type == R_SH_IMM32CE
		|| ircgcn->r_type == R_SH_IMAGEBASE)
      ...
      else
	{
	  ...
	  h = obj_coff_sym_hashes (abfd)[ircgcn->r_symndx];
	  BFD_ASSERT (h != NULL);
...
/* Try to align loads and stores within a span of memory.  This is
   called by both the CGC and the COFF sh targets.  ABFD and SEC are
...
/* Some people want versions of the SH COFF target which do not align
   ...
   section alignment in COFF is rather complex.  CGC handles this
   correctly.  */
