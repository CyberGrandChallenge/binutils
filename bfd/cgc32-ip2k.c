/* Ubicom IP2xxx specific support for 32-bit CGC
...
/* Relocation tables.  */
static reloc_howto_type ip2k_cgc_howto_table [] =
{
...
/* Map BFD reloc types to IP2K CGC reloc types.  */

static reloc_howto_type *
ip2k_reloc_type_lookup (bfd * abfd ATTRIBUTE_UNUSED,
			bfd_reloc_code_real_type code)
{
  /* Note that the ip2k_cgc_howto_table is indxed by the R_
  ...
  switch (code)
    {
    case BFD_RELOC_NONE:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_NONE];
    case BFD_RELOC_16:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_16];
    case BFD_RELOC_32:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_32];
    case BFD_RELOC_IP2K_FR9:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_FR9];
    case BFD_RELOC_IP2K_BANK:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_BANK];
    case BFD_RELOC_IP2K_ADDR16CJP:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_ADDR16CJP];
    case BFD_RELOC_IP2K_PAGE3:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_PAGE3];
    case BFD_RELOC_IP2K_LO8DATA:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_LO8DATA];
    case BFD_RELOC_IP2K_HI8DATA:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_HI8DATA];
    case BFD_RELOC_IP2K_LO8INSN:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_LO8INSN];
    case BFD_RELOC_IP2K_HI8INSN:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_HI8INSN];
    case BFD_RELOC_IP2K_PC_SKIP:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_PC_SKIP];
    case BFD_RELOC_IP2K_TEXT:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_TEXT];
    case BFD_RELOC_IP2K_FR_OFFSET:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_FR_OFFSET];
    case BFD_RELOC_IP2K_EX8DATA:
      return &ip2k_cgc_howto_table[ (int) R_IP2K_EX8DATA];
    default:
      /* Pacify gcc -Wall.  */
      return NULL;
    }
...
ip2k_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (ip2k_cgc_howto_table) / sizeof (ip2k_cgc_howto_table[0]);
       i++)
    if (ip2k_cgc_howto_table[i].name != NULL
	&& strcasecmp (ip2k_cgc_howto_table[i].name, r_name) == 0)
      return &ip2k_cgc_howto_table[i];
			...
			int noadj)
{
  ...
  unsigned int shndx;
  Cgc_Internal_Rela *irel, *irelend, *irelbase;
  ...
  irelbase = cgc_section_data (sec)->relocs;
  irelend = irelbase + sec->reloc_count;

  for (irel = irelbase; irel < irelend; irel++)
    {
      if (CGC32_R_TYPE (irel->r_info) != R_IP2K_NONE)
        {
          /* Get the value of the symbol referred to by the reloc.  */
          if (CGC32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
            {
  ...
  if (stab)
    {
      ...
      irelbase = cgc_section_data (stab)->relocs;
      irelend = irelbase + stab->reloc_count;

      /* Pull out the contents of the stab section.  */
      if (cgc_section_data (stab)->this_hdr.contents != NULL)
	stabcontents = cgc_section_data (stab)->this_hdr.contents;
      else
	{
	  ...
	  cgc_section_data (stab)->this_hdr.contents = stabcontents;
	}
...
static bfd_boolean
ip2k_cgc_relax_delete_bytes (bfd *abfd,
			     asection *sec,
			     bfd_vma addr,
			     int count)
{
  bfd_byte *contents = cgc_section_data (sec)->this_hdr.contents;
		       ...
		       struct misc *misc)
{
  ...
  cgc_section_data (sec)->relocs = misc->irelbase;
  cgc_section_data (sec)->this_hdr.contents = misc->contents;
  misc->symtab_hdr->contents = (bfd_byte *) misc->isymbuf;

  /* Fix the relocation's type.  */
  irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info), R_IP2K_NONE);

  /* Delete the PAGE insn.  */
  if (!ip2k_cgc_relax_delete_bytes (abfd, sec, irel->r_offset, 2))
    return FALSE;
...
ip2k_relax_switch_table_128 (bfd *abfd ATTRIBUTE_UNUSED,
			     ...
			     struct misc *misc)
{
  Cgc_Internal_Rela *irelend = misc->irelbase + sec->reloc_count;
  Cgc_Internal_Rela *ireltest = irel;
  bfd_byte code[4];
  bfd_vma addr;
  ...
  if (!ip2k_cgc_relax_delete_bytes (abfd, sec, irel->r_offset - 4, 2))
    return FALSE;
			     ...
			     struct misc *misc)
{
  ...
  if (!ip2k_cgc_relax_delete_bytes (abfd, sec, addr + 0, 6))
    return FALSE;
...
static bfd_boolean
ip2k_cgc_relax_section_page (bfd *abfd,
			     asection *sec,
			     bfd_boolean *again,
			     struct misc *misc,
			     ...
			     unsigned long page_end)
{
  ...
  for (irel = misc->irelbase; irel < irelend; irel++)
    {
      if (CGC32_R_TYPE (irel->r_info) != (int) R_IP2K_PAGE3)
...
static bfd_boolean
ip2k_cgc_relax_section (bfd *abfd,
			asection *sec,
			...
			bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  if ((pass == 1) || (new_pass && !changed))
	...
	{
          if (!ip2k_cgc_relax_section_page (abfd, sec, &changed, &misc, page_start, page_end))
	    return FALSE;
	}
      *again = TRUE;
    }
...
/* Set the howto pointer for a IP2K CGC reloc.  */
			 ...
			 Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = & ip2k_cgc_howto_table [r_type];
}
  ...
  switch (howto->type)
    ...
    case R_IP2K_16:
	 ...
	 symbol, reduce it to a word address.  */
      if ((relocation & IP2K_INSN_MASK) == IP2K_INSN_VALUE)
	howto = &ip2k_cgc_howto_table[ (int) R_IP2K_TEXT];
      break;
...
static bfd_boolean
ip2k_cgc_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = ip2k_cgc_howto_table + r_type;
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
...
#define TARGET_BIG_SYM	 bfd_cgc32_ip2k_vec
#define TARGET_BIG_NAME  "cgc32-ip2k"

#define CGC_ARCH	 bfd_arch_ip2k
#define CGC_MACHINE_CODE EM_IP2K
#define CGC_MACHINE_ALT1 EM_IP2K_OLD
#define CGC_MAXPAGESIZE  1 /* No pages on the IP2K.  */

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			ip2k_info_to_howto_rela

#define cgc_backend_can_gc_sections     	1
#define cgc_backend_rela_normal			1
#define cgc_backend_relocate_section		ip2k_cgc_relocate_section

#define cgc_symbol_leading_char			'_'
#define bfd_cgc32_bfd_reloc_type_lookup		ip2k_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup	ip2k_reloc_name_lookup
#define bfd_cgc32_bfd_relax_section		ip2k_cgc_relax_section
