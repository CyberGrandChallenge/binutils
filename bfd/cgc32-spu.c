/* SPU specific support for 32-bit CGC
...
static void
spu_cgc_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED,
		       arelent *cache_ptr,
		       Cgc_Internal_Rela *dst)
{
  enum cgc_spu_reloc_type r_type;

  r_type = (enum cgc_spu_reloc_type) CGC32_R_TYPE (dst->r_info);
  BFD_ASSERT (r_type < R_SPU_max);
  cache_ptr->howto = &cgc_howto_table[(int) r_type];
}

static reloc_howto_type *
spu_cgc_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   bfd_reloc_code_real_type code)
{
  enum cgc_spu_reloc_type r_type = spu_cgc_bfd_to_reloc_type (code);

  if (r_type == R_SPU_NONE)
    return NULL;

  return cgc_howto_table + r_type;
}

static reloc_howto_type *
spu_cgc_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   const char *r_name)
{
  unsigned int i;
...
spu_cgc_rel9 (bfd *abfd, arelent *reloc_entry, asymbol *symbol,
...
static bfd_boolean
spu_cgc_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      struct _spu_cgc_section_data *sdata;
...
static bfd_boolean
spu_cgc_object_p (bfd *abfd)
{
  if ((abfd->flags & (EXEC_P | DYNAMIC)) != 0)
    {
      unsigned int i, num_ovl, num_buf;
      Cgc_Internal_Phdr *phdr = cgc_tdata (abfd)->phdr;
      Cgc_Internal_Ehdr *ehdr = cgc_cgcheader (abfd);
      Cgc_Internal_Phdr *last_phdr = NULL;
	...
	if (phdr->p_type == PT_LOAD && (phdr->p_flags & PF_OVERLAY) != 0)
	  {
		...
		|| ((last_phdr->p_vaddr ^ phdr->p_vaddr) & 0x3ffff) != 0)
	      ++num_buf;
	    last_phdr = phdr;
	    for (j = 1; j < cgc_numsections (abfd); j++)
	      {
		Cgc_Internal_Shdr *shdr = cgc_cgcsections (abfd)[j];

		if (CGC_SECTION_SIZE (shdr, phdr) != 0
		    && CGC_SECTION_IN_SEGMENT (shdr, phdr))
		  {
		    asection *sec = shdr->bfd_section;
		    spu_cgc_section_data (sec)->u.o.ovl_index = num_ovl;
		    spu_cgc_section_data (sec)->u.o.ovl_buf = num_buf;
		  }
	      }
	  }
    }
...
static void
spu_cgc_backend_symbol_processing (bfd *abfd ATTRIBUTE_UNUSED, asymbol *sym)
{
  if (sym->name != NULL
...
struct spu_link_hash_table
{
  ...
  struct cgc_link_hash_entry *ovly_entry[2];
...
#define spu_hash_table(p) \
  ...
  == SPU_CGC_DATA ? ((struct spu_link_hash_table *) ((p)->hash)) : NULL)
...
/* Create a spu CGC linker hash table.  */

static struct bfd_link_hash_table *
spu_cgc_link_hash_table_create (bfd *abfd)
{
  ...
  if (!_bfd_cgc_link_hash_table_init (&htab->cgc, abfd,
...
void
spu_cgc_setup (struct bfd_link_info *info, struct spu_cgc_params *params)
{
...
bfd_boolean
spu_cgc_create_sections (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  bfd *ibfd;
  ...
  if (htab->params->emit_fixups)
	...
	htab->cgc.dynobj = ibfd;
      ibfd = htab->cgc.dynobj;
...
int
spu_cgc_find_overlays (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  asection **alloc_sec;
      ...
      for (ovl_index = 0, num_buf = 0; i < n; i++)
	{
	  ...
	  if (strncmp (s->name, ".ovl.init", 9) != 0)
	      ...
	      else if (s->size > htab->params->line_size)
		{
	      ...
	      spu_cgc_section_data (s)->u.o.ovl_index
		= (set_id << htab->num_lines_log2) + num_buf;
	      spu_cgc_section_data (s)->u.o.ovl_buf = num_buf;
	    }
	}
	  ...
	  if (s->vma < ovl_end)
	    {
	      ...
	      if (spu_cgc_section_data (s0)->u.o.ovl_index == 0)
		{
		  ++num_buf;
		  if (strncmp (s0->name, ".ovl.init", 9) != 0)
		    {
		      alloc_sec[ovl_index] = s0;
		      spu_cgc_section_data (s0)->u.o.ovl_index = ++ovl_index;
		      spu_cgc_section_data (s0)->u.o.ovl_buf = num_buf;
		    }
		  else
		    ovl_end = s->vma + s->size;
		}
		...
		{
		  alloc_sec[ovl_index] = s;
		  spu_cgc_section_data (s)->u.o.ovl_index = ++ovl_index;
		  spu_cgc_section_data (s)->u.o.ovl_buf = num_buf;
		  if (s0->vma != s->vma)
		    {
...
needs_ovl_stub (struct cgc_link_hash_entry *h,
		Cgc_Internal_Sym *sym,
		asection *sym_sec,
		...
		struct bfd_link_info *info)
{
  ...
  if (sym_sec == NULL
      ...
      || spu_cgc_section_data (sym_sec->output_section) == NULL)
    return ret;
  ...
  if (h != NULL)
    ...
    sym_type = CGC_ST_TYPE (sym->st_info);
      ...
      if (branch || hint)
	{
	  ...
	  if (call
	      ...
	      else
		{
		  ...
		  symtab_hdr = &cgc_tdata (input_section->owner)->symtab_hdr;
		  sym_name = bfd_cgc_sym_name (input_section->owner,
					       symtab_hdr,
					       sym,
					       sym_sec);
		}
  ...
  if (spu_cgc_section_data (sym_sec->output_section)->u.o.ovl_index == 0
      && !htab->params->non_overlay_stubs)
    return ret;

  /* A reference from some other section to a symbol in an overlay
     section needs a stub.  */
  if (spu_cgc_section_data (sym_sec->output_section)->u.o.ovl_index
       != spu_cgc_section_data (input_section->output_section)->u.o.ovl_index)
    {
	    ...
	    const Cgc_Internal_Rela *irela)
{
  ...
  if (stub_type != nonovl_stub)
    ovl = spu_cgc_section_data (isec->output_section)->u.o.ovl_index;

  if (h != NULL)
    head = &h->got.glist;
  else
    {
      if (cgc_local_got_ents (ibfd) == NULL)
	{
	  bfd_size_type amt = (cgc_tdata (ibfd)->symtab_hdr.sh_info
			       * sizeof (*cgc_local_got_ents (ibfd)));
	  cgc_local_got_ents (ibfd) = bfd_zmalloc (amt);
	  if (cgc_local_got_ents (ibfd) == NULL)
	    return FALSE;
	}
      head = cgc_local_got_ents (ibfd) + CGC32_R_SYM (irela->r_info);
    }
...
static unsigned int
ovl_stub_size (struct spu_cgc_params *params)
{
  return 16 << params->ovly_flavour >> params->compact_stub;
}

static unsigned int
ovl_stub_size_log2 (struct spu_cgc_params *params)
{
	    ...
	    asection *dest_sec)
{
  ...
  dest_ovl = spu_cgc_section_data (dest_sec->output_section)->u.o.ovl_index;
  ...
  if (htab->params->ovly_flavour == ovly_normal
      ...
      if (irela == NULL)
	{
      ...
      bfd_put_32 (sec->owner, (lrlive << 29) | (g->br_addr & 0x3ffff),
		  sec->contents + sec->size + 8);
      patt = dest ^ br_dest;
      if (irela != NULL && CGC32_R_TYPE (irela->r_info) == R_SPU_REL16)
  ...
  if (htab->params->emit_stub_syms)
	...
	sprintf (name + 8 + sizeof (".ovl_call.") - 1, "%x:%x",
		 ...
		 (int) CGC32_R_SYM (irela->r_info) & 0xffffffff);
      if (add != 0)
	sprintf (name + len - 9, "+%x", add);

      h = cgc_link_hash_lookup (&htab->cgc, name, TRUE, TRUE, FALSE);
      free (name);
      if (h == NULL)
	return FALSE;
      if (h->root.type == bfd_link_hash_new)
...
/* Called via cgc_link_hash_traverse to allocate stubs for any _SPUEAR_
   symbols.  */

static bfd_boolean
allocate_spuear_stubs (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if ((h->root.type == bfd_link_hash_defined
      ...
      && sym_sec->output_section != bfd_abs_section_ptr
      && spu_cgc_section_data (sym_sec->output_section) != NULL
      && (spu_cgc_section_data (sym_sec->output_section)->u.o.ovl_index != 0
	  || htab->params->non_overlay_stubs))
    {
...
static bfd_boolean
build_spuear_stubs (struct cgc_link_hash_entry *h, void *inf)
{
  /* Symbols starting with _SPUEAR_ need a stub because they may be
     invoked by the PPU.  */
  struct bfd_link_info *info = inf;
...
process_stubs (struct bfd_link_info *info, bfd_boolean build)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
    {
      extern const bfd_target bfd_cgc32_spu_vec;
      Cgc_Internal_Shdr *symtab_hdr;
      asection *isec;
      Cgc_Internal_Sym *local_syms = NULL;

      if (ibfd->xvec != &bfd_cgc32_spu_vec)
	continue;
      ...
      for (isec = ibfd->sections; isec != NULL; isec = isec->next)
	{
	  ...
	  /* Get the relocs.  */
	  internal_relocs = _bfd_cgc_link_read_relocs (ibfd, isec, NULL, NULL,
						       info->keep_memory);
	  ...
	  for (; irela < irelaend; irela++)
		...
		error_ret_free_internal:
		  if (cgc_section_data (isec)->relocs != internal_relocs)
...
int
spu_cgc_size_stubs (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab;
  bfd *ibfd;
  bfd_size_type amt;
  ...
  cgc_link_hash_traverse (&htab->cgc, allocate_spuear_stubs, info);
  if (htab->stub_err)
    return 0;
  ...
  if (htab->stub_count != NULL)
      ...
      for (i = 0; i < htab->num_overlays; ++i)
	{
	  ...
	  unsigned int ovl = spu_cgc_section_data (osec)->u.o.ovl_index;
	  stub = bfd_make_section_anyway_with_flags (ibfd, ".stub", flags);
...
void
spu_cgc_place_overlay_data (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  unsigned int i;
...
bfd_boolean
spu_cgc_open_builtin_lib (bfd **ovl_bfd, const struct _ovl_stream *stream)
{
...
overlay_index (asection *sec)
{
  if (sec == NULL
    ...
    return 0;
  return spu_cgc_section_data (sec->output_section)->u.o.ovl_index;
}
...
static bfd_boolean
spu_cgc_build_stubs (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  struct cgc_link_hash_entry *h;
  bfd_byte *p;
      ...
      for (i = 0; i < 2; i++)
	{
	  ...
	  if (h != NULL
	      ...
	      && h->def_regular)
	    {
	      ...
	      if (spu_cgc_section_data (s)->u.o.ovl_index)
		{
  ...
  if (htab->stub_sec != NULL)
	...
	if (htab->stub_sec[i]->size != 0)
      ...
      if (!htab->stub_err)
	cgc_link_hash_traverse (&htab->cgc, build_spuear_stubs, info);
  ...
  if (htab->params->ovly_flavour == ovly_soft_icache)
	...
	{
	  unsigned int ovl_index = spu_cgc_section_data (s)->u.o.ovl_index;

	  if (ovl_index != 0)
	    {
	      unsigned long off = ovl_index * 16;
	      unsigned int ovl_buf = spu_cgc_section_data (s)->u.o.ovl_buf;

	      bfd_put_32 (htab->ovtab->owner, s->vma, p + off);
	      bfd_put_32 (htab->ovtab->owner, (s->size + 15) & -16,
			  p + off + 4);
	      /* file_off written later in spu_cgc_modify_program_headers.  */
...
asection *
spu_cgc_check_vma (struct bfd_link_info *info)
{
...
static Cgc_Internal_Sym *sort_syms_syms;
static asection **sort_syms_psecs;

static int
sort_syms (const void *a, const void *b)
{
  Cgc_Internal_Sym *const *s1 = a;
  Cgc_Internal_Sym *const *s2 = b;
  asection *sec1,*sec2;
...
/* Allocate a struct spu_cgc_stack_info with MAX_FUN struct function_info
   entries for section SEC.  */

static struct spu_cgc_stack_info *
alloc_stack_info (asection *sec, int max_fun)
{
  struct _spu_cgc_section_data *sec_data = spu_cgc_section_data (sec);
  bfd_size_type amt;

  amt = sizeof (struct spu_cgc_stack_info);
...
{
  struct _spu_cgc_section_data *sec_data = spu_cgc_section_data (sec);
  struct spu_cgc_stack_info *sinfo = sec_data->u.i.stack_info;
  int i;
  bfd_vma off, size;
  ...
  if (!global)
    {
      Cgc_Internal_Sym *sym = sym_h;
      off = sym->st_value;
      size = sym->st_size;
    }
  else
    {
      struct cgc_link_hash_entry *h = sym_h;
  ...
  if (sinfo->num_fun >= sinfo->max_fun)
    {
      bfd_size_type amt = sizeof (struct spu_cgc_stack_info);
...
func_name (struct function_info *fun)
{
  ...
  ibfd = sec->owner;
  symtab_hdr = &cgc_tdata (ibfd)->symtab_hdr;
  return bfd_cgc_sym_name (ibfd, symtab_hdr, fun->u.sym, sec);
}
			   ...
			   int call_tree)
{
  ...
  internal_relocs = _bfd_cgc_link_read_relocs (sec->owner, sec, NULL, NULL,
					       info->keep_memory);
  if (internal_relocs == NULL)
    return FALSE;

  symtab_hdr = &cgc_tdata (sec->owner)->symtab_hdr;
  ...
  for (; irela < irelaend; irela++)
      ...
      if (!call_tree)
	{
	  ...
	  if (irela->r_addend != 0)
	    {
	      Cgc_Internal_Sym *fake = bfd_zmalloc (sizeof (*fake));
	      if (fake == NULL)
		return FALSE;
	      fake->st_value = val;
	      fake->st_shndx
		= _bfd_cgc_section_from_bfd_section (sym_sec->owner, sym_sec);
	      sym = fake;
	    }
	  if (sym)
...
pasted_function (asection *sec)
{
  ...
  struct _spu_cgc_section_data *sec_data;
  struct spu_cgc_stack_info *sinfo;
  Cgc_Internal_Sym *fake;
  ...
  fake->st_shndx
    = _bfd_cgc_section_from_bfd_section (sec->owner, sec);
  ...
  for (l = sec->output_section->map_head.link_order; l != NULL; l = l->next)
      ...
      if (l->u.indirect.section == sec)
	{
	  ...
	  && (sec_data = spu_cgc_section_data (l->u.indirect.section)) != NULL
...
discover_functions (struct bfd_link_info *info)
{
  ...
  for (ibfd = info->input_bfds, bfd_idx = 0;
       ...
       ibfd = ibfd->link_next, bfd_idx++)
    {
      ...
      asection *sec;
      size_t symcount;
      Cgc_Internal_Sym *syms, *sy, **psyms, **psy;
      asection **psecs, **p;
	...
	{
	  /* Don't use cached symbols since the generic CGC linker
	     code only reads local symbols, and we need globals too.  */
	  free (symtab_hdr->contents);
	  symtab_hdr->contents = NULL;
	}
      syms = bfd_cgc_get_cgc_syms (ibfd, symtab_hdr, symcount, 0,
				   NULL, NULL, NULL);
	...
	if (CGC_ST_TYPE (sy->st_info) == STT_NOTYPE
	    || CGC_ST_TYPE (sy->st_info) == STT_FUNC)
	  {
	    asection *s;

	    *p = s = bfd_section_from_cgc_index (ibfd, sy->st_shndx);
	...
	{
	  sy = *psy;
	  if (CGC_ST_TYPE (sy->st_info) == STT_FUNC)
	    {
  ...
  if (gaps)
	   ...
	   ibfd = ibfd->link_next, bfd_idx++)
	{
	  ...
	  symtab_hdr = &cgc_tdata (ibfd)->symtab_hdr;
	  syms = (Cgc_Internal_Sym *) symtab_hdr->contents;
	  ...
	  for (psy = psyms; (sy = *psy) != NULL; ++psy)
	    {
	      ...
	      if (CGC_ST_TYPE (sy->st_info) != STT_FUNC
		  && CGC_ST_BIND (sy->st_info) == STB_GLOBAL)
		{
      ...
      for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	{
	  ...
	  for (sec = ibfd->sections; sec != NULL; sec = sec->next)
		...
		sec_data = spu_cgc_section_data (sec);
	       ...
	       int root_only)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (sec = ibfd->sections; sec != NULL; sec = sec->next)
	{
	  ...
	  if ((sec_data = spu_cgc_section_data (sec)) != NULL
	      && (sinfo = sec_data->u.i.stack_info) != NULL)
	    {
	      int i;
...
/* Set linker_mark and gc_mark on any sections that we will put in
   overlays.  These flags are used by the generic CGC linker, but we
   won't be continuing on to bfd_cgc_final_link so it is OK to use
	  ...
	  if (name != NULL)
	    {
	      ...
	      asection *group_sec = cgc_section_data (fun->sec)->next_in_group;
	      if (group_sec == NULL)
		...
		while (group_sec != NULL && group_sec != fun->sec)
		  {
		    ...
		    group_sec = cgc_section_data (group_sec)->next_in_group;
		  }
	      fun->rodata = rodata;
	      if (fun->rodata)
		{
...
sort_lib (const void *a, const void *b)
{
  ...
  delta = 0;
  if ((sec_data = spu_cgc_section_data (*s1)) != NULL
      && (sinfo = sec_data->u.i.stack_info) != NULL)
    {
      int i;
      for (i = 0; i < sinfo->num_fun; ++i)
  ...
  if ((sec_data = spu_cgc_section_data (*s2)) != NULL
      && (sinfo = sec_data->u.i.stack_info) != NULL)
    {
      int i;
      for (i = 0; i < sinfo->num_fun; ++i)
...
auto_ovl_lib_functions (struct bfd_link_info *info, unsigned int lib_size)
{
  ...
  for (i = 0; i < lib_count; i++)
	...
	tmp += lib_sections[2 * i + 1]->size;
      ...
      stub_size = 0;
      if (tmp < lib_size
	  && (sec_data = spu_cgc_section_data (sec)) != NULL
	  && (sinfo = sec_data->u.i.stack_info) != NULL)
	{
	  int k;
	  struct call_info *call;
  ...
  if (added_fun)
    {
      ...
      if ((sec_data = spu_cgc_section_data (fun->sec)) != NULL
	  && (sinfo = sec_data->u.i.stack_info) != NULL)
	{
	  int i;
	  for (i = 0; i < sinfo->num_fun; ++i)
	   ...
	   void *param)
{
  ...
  if (sum_stack_param->emit_stack_syms)
	...
	return FALSE;
      ...
      if (fun->global || CGC_ST_BIND (fun->u.sym->st_info) == STB_GLOBAL)
...
static void
spu_cgc_auto_overlay (struct bfd_link_info *info)
{
  bfd *ibfd;
  bfd **bfd_arr;
  struct cgc_segment_map *m;
  ...
  lo = (unsigned int) -1;
  hi = 0;
  for (m = cgc_seg_map (info->output_bfd); m != NULL; m = m->next)
    if (m->p_type == PT_LOAD)
      for (i = 0; i < m->count; i++)
	if (m->sections[i]->size != 0)
	  {
  ...
  h = cgc_link_hash_lookup (&htab->cgc, ovly_mgr_entry,
			    FALSE, FALSE, FALSE);
  if (h != NULL
      && (h->root.type == bfd_link_hash_defined
	  || h->root.type == bfd_link_hash_defweak)
    ...
    {
      /* If no user overlay manager, spu_cgc_load_ovl_mgr will add our
	 builtin version to .text, and will adjust .text size.  */
      fixed_size += (*htab->params->spu_cgc_load_ovl_mgr) ();
    }
  ...
  script = htab->params->spu_cgc_open_overlay_script ();
...
static bfd_boolean
spu_cgc_stack_analysis (struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab;
  struct _sum_stack_param sum_stack_param;
...
static bfd_boolean
spu_cgc_final_link (bfd *output_bfd, struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);

  if (htab->params->auto_overlay)
    spu_cgc_auto_overlay (info);

  if ((htab->params->stack_analysis
       || (htab->params->ovly_flavour == ovly_soft_icache
	   && htab->params->lrlive_analysis))
      && !spu_cgc_stack_analysis (info))
    info->callbacks->einfo ("%X%P: stack/lrlive analysis error: %E\n");

  if (!spu_cgc_build_stubs (info))
    info->callbacks->einfo ("%F%P: can not build overlay stubs: %E\n");

  return bfd_cgc_final_link (output_bfd, info);
}
...
static unsigned int
spu_cgc_count_relocs (struct bfd_link_info *info, asection *sec)
{
  Cgc_Internal_Rela *relocs;
  unsigned int count = 0;

  relocs = _bfd_cgc_link_read_relocs (sec->owner, sec, NULL, NULL,
				      info->keep_memory);
  if (relocs != NULL)
    {
      Cgc_Internal_Rela *rel;
      Cgc_Internal_Rela *relend = relocs + sec->reloc_count;
      ...
      for (rel = relocs; rel < relend; rel++)
	{
      ...
      if (cgc_section_data (sec)->relocs != relocs)
	free (relocs);
    }
...
static void
spu_cgc_emit_fixup (bfd * output_bfd, struct bfd_link_info *info,
		    bfd_vma offset)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
...
static int
spu_cgc_relocate_section (bfd *output_bfd,
			  struct bfd_link_info *info,
			  bfd *input_bfd,
			  asection *input_section,
			  bfd_byte *contents,
			  ...
			  asection **local_sections)
{
  ...
  sym_hashes = (struct cgc_link_hash_entry **) (cgc_sym_hashes (input_bfd));
  ...
  for (; rel < relend; rel++)
					  ...
					  contents, info)) != no_stub)
	{
	  ...
	  if (h != NULL)
	    ...
	    head = cgc_local_got_ents (input_bfd) + r_symndx;
	  ...
	  && r_type == R_SPU_ADDR32)
	{
	  ...
	  offset = rel->r_offset + input_section->output_section->vma
		   + input_section->output_offset;
	  spu_cgc_emit_fixup (output_bfd, info, offset);
	}
	  ...
	  if (is_ea_sym)
	    {
		 ...
		 part of an embedded CGC image.  If this reloc is
		 against a symbol defined in ._ea, then transform the
		 reloc into an equivalent one without a symbol
		 relative to the start of the CGC image.  */
	      rel->r_addend += (relocation
				- ea->vma
				+ cgc_section_data (ea)->this_hdr.sh_offset);
	      rel->r_info = CGC32_R_INFO (0, r_type);
	    }
  ...
  if (ret
      ...
      for (; rel < relend; rel++)
	{
      ...
      /* Backflips for _bfd_cgc_link_output_relocs.  */
...
static bfd_boolean
spu_cgc_finish_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				 struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  return TRUE;
}
...
static int
spu_cgc_output_symbol_hook (struct bfd_link_info *info,
...
void
spu_cgc_plugin (int val)
{
  spu_plugin = val;
}

/* Set CGC header e_type for plugins.  */

static void
spu_cgc_post_process_headers (bfd *abfd,
			      struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  if (spu_plugin)
    {
...
static int
spu_cgc_additional_program_headers (bfd *abfd, struct bfd_link_info *info)
{
  int extra = 0;
  asection *sec;
...
static bfd_boolean
spu_cgc_modify_segment_map (bfd *abfd, struct bfd_link_info *info)
{
  asection *toe, *s;
  struct cgc_segment_map *m, *m_overlay;
  struct cgc_segment_map **p, **p_overlay;
  unsigned int i;
  ...
  for (m = cgc_seg_map (abfd); m != NULL; m = m->next)
	...
	if ((s = m->sections[i]) == toe
	    || spu_cgc_section_data (s)->u.o.ovl_index != 0)
	  {
	    struct cgc_segment_map *m2;
	    bfd_vma amt;
  ...
  /* Some SPU CGC loaders ignore the PF_OVERLAY flag and just load all
  ...
  while (*p != NULL)
    {
	  ...
	  && spu_cgc_section_data ((*p)->sections[0])->u.o.ovl_index != 0)
	{
	  m = *p;
	  *p = m->next;
	  *p_overlay = m;
  ...
  *p_overlay = cgc_seg_map (abfd);
  cgc_seg_map (abfd) = m_overlay;
...
static bfd_boolean
spu_cgc_fake_sections (bfd *obfd ATTRIBUTE_UNUSED,
...
static int
spu_cgc_modify_program_headers (bfd *abfd, struct bfd_link_info *info)
{
  const struct cgc_backend_data *bed;
  struct cgc_obj_tdata *tdata;
  Cgc_Internal_Phdr *phdr, *last;
  struct spu_link_hash_table *htab;
  unsigned int count;
  unsigned int i;
  ...
  tdata = cgc_tdata (abfd);
  phdr = tdata->phdr;
  count = cgc_program_header_size (abfd) / bed->s->sizeof_phdr;
  htab = spu_hash_table (info);
  if (htab->num_overlays != 0)
    {
      struct cgc_segment_map *m;
      ...
      for (i = 0, m = cgc_seg_map (abfd); m; ++i, m = m->next)
	if (m->count != 0
	    && (o = spu_cgc_section_data (m->sections[0])->u.o.ovl_index) != 0)
	  {
      ...
      if (htab->init != NULL && htab->init->size != 0)
	{
	  bfd_vma val = cgc_section_data (htab->ovl_sec[0])->this_hdr.sh_offset;
...
bfd_boolean
spu_cgc_size_sections (bfd * output_bfd, struct bfd_link_info *info)
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  if (htab->params->emit_fixups)
    {
      ...
      for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	{
	  ...
	  for (isec = ibfd->sections; isec != NULL; isec = isec->next)
		...
		continue;
	      ...
	      /* Get the relocs.  */
	      internal_relocs =
		_bfd_cgc_link_read_relocs (ibfd, isec, NULL, NULL,
					   info->keep_memory);
	      if (internal_relocs == NULL)
		return FALSE;
	      ...
	      base_end = 0;
	      for (; irela < irelaend; irela++)
		if (CGC32_R_TYPE (irela->r_info) == R_SPU_ADDR32
...
#define TARGET_BIG_SYM		bfd_cgc32_spu_vec
#define TARGET_BIG_NAME		"cgc32-spu"
#define CGC_ARCH		bfd_arch_spu
#define CGC_TARGET_ID		SPU_CGC_DATA
#define CGC_MACHINE_CODE	EM_SPU
/* This matches the alignment need for DMA.  */
#define CGC_MAXPAGESIZE		0x80
#define cgc_backend_rela_normal         1
#define cgc_backend_can_gc_sections	1

#define bfd_cgc32_bfd_reloc_type_lookup		spu_cgc_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		spu_cgc_reloc_name_lookup
#define cgc_info_to_howto			spu_cgc_info_to_howto
#define cgc_backend_count_relocs		spu_cgc_count_relocs
#define cgc_backend_relocate_section		spu_cgc_relocate_section
#define cgc_backend_finish_dynamic_sections	spu_cgc_finish_dynamic_sections
#define cgc_backend_symbol_processing		spu_cgc_backend_symbol_processing
#define cgc_backend_link_output_symbol_hook	spu_cgc_output_symbol_hook
#define cgc_backend_object_p			spu_cgc_object_p
#define bfd_cgc32_new_section_hook		spu_cgc_new_section_hook
#define bfd_cgc32_bfd_link_hash_table_create	spu_cgc_link_hash_table_create

#define cgc_backend_additional_program_headers	spu_cgc_additional_program_headers
#define cgc_backend_modify_segment_map		spu_cgc_modify_segment_map
#define cgc_backend_modify_program_headers	spu_cgc_modify_program_headers
#define cgc_backend_post_process_headers        spu_cgc_post_process_headers
#define cgc_backend_fake_sections		spu_cgc_fake_sections
#define cgc_backend_special_sections		spu_cgc_special_sections
#define bfd_cgc32_bfd_final_link		spu_cgc_final_link
