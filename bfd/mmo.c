...
SECTION
	...
	no advantages feature-wise compared to e.g. CGC.  It is a simple
	non-relocatable object format with no support for archives or
	debugging information, except for symbol value information and
	line numbers (which is not yet implemented in BFD).  See
	@url{http://www-cs-faculty.stanford.edu/~knuth/mmix.html} for more
	information about MMIX.  The CGC format is used for intermediate
...
SUBSECTION
	...
	with e.g.@: CGC.  Memory areas is formed by specifying the
	...
	same as the CGC <<e_machine>> number for MMIX, filling it with
	section information normally found in CGC objects. @xref{mmo
	section mapping}.
...
mmo_object_p (bfd *abfd)
{
  ...
  /* You'll have to tweak this if you want to use this format for other
     ...
     the CGC format for how to make it target-generic.  */
	   ...
	   msp++, ++c)
	{
	  ...
	  if (s->sym_type == mmo_data_sym)
		  ...
		  && c->value <= textsec->vma + textsec->size)
		{
		 ...
		 addresses.  For such needs, use the CGC format.  */
	      else if ((datasec
