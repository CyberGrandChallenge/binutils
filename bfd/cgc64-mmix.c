/* MMIX-specific support for 64-bit CGC.
...
#define mmix_cgc_section_data(sec) \
  ((struct _mmix_cgc_section_data *) cgc_section_data (sec))

/* For each section containing a base-plus-offset (BPO) reloc, we attach
   this struct as mmix_cgc_section_data (section)->bpo, which is otherwise
   NULL.  */
...
/* We attach this as mmix_cgc_section_data (sec)->bpo in the linker-allocated
...
struct bpo_greg_section_info
  {
    ...
    /* The number of linker-allocated GREGs resulting from BPO relocs.
       ...
       and supposedly accurate after mmix_cgc_relax_section is called for
...
extern bfd_boolean mmix_cgc_final_link (bfd *, struct bfd_link_info *);

extern void mmix_cgc_symbol_processing (bfd *, asymbol *);
...
mmix_cgc_reloc (bfd *, arelent *, asymbol *, void *,
		asection *, bfd *, char **);
static bfd_reloc_status_type
...
static reloc_howto_type cgc_mmix_howto_table[] =
 {
  ...
  HOWTO (R_MMIX_GETA,		/* type */
	 ...
	 mmix_cgc_reloc,	/* special_function */
	 "R_MMIX_GETA",		/* name */
	 FALSE,			/* partial_inplace */
	 ~0x0100ffff,		/* src_mask */
	 0x0100ffff,		/* dst_mask */
...
/* Map BFD reloc types to MMIX CGC reloc types.  */

struct mmix_reloc_map
  {
    bfd_reloc_code_real_type bfd_reloc_val;
    enum cgc_mmix_reloc_type cgc_reloc_val;
  };
				 ...
				 bfd_reloc_code_real_type code)
{
  ...
  for (i = 0;
	...
	return &cgc_mmix_howto_table[mmix_reloc_map[i].cgc_reloc_val];
    }
				 ...
				 const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (cgc_mmix_howto_table) / sizeof (cgc_mmix_howto_table[0]);
       i++)
    if (cgc_mmix_howto_table[i].name != NULL
	&& strcasecmp (cgc_mmix_howto_table[i].name, r_name) == 0)
      return &cgc_mmix_howto_table[i];

  return NULL;
}

static bfd_boolean
mmix_cgc_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      struct _mmix_cgc_section_data *sdata;
...
static bfd_reloc_status_type
mmix_cgc_perform_relocation (asection *isec, reloc_howto_type *howto,
			     ...
			     char **error_message)
{
  ...
  switch (howto->type)
      ...
      else
	{
	  ...
	  bfd_byte *stubcontents
	    = ((bfd_byte *) datap
	       ...
	       + size
	       + mmix_cgc_section_data (isec)->pjs.stub_offset);
	  bfd_vma stubaddr;

	  if (mmix_cgc_section_data (isec)->pjs.n_pushj_relocs == 0)
	    {
	      /* This shouldn't happen when linking to CGC or mmo, so
	  ...
	  r = mmix_cgc_perform_relocation (isec,
					   &cgc_mmix_howto_table
					   [R_MMIX_ADDR19],
					   datap,
					   addr,
	  ...
	  if (bfd_check_overflow (complain_overflow_signed,
				  cgc_mmix_howto_table[R_MMIX_ADDR27].bitsize,
				  0,
				  bfd_arch_bits_per_address (abfd),
				  addr + value - stubaddr) == bfd_reloc_ok)
	    {
	      ...
	      r = mmix_cgc_perform_relocation (isec,
					       ...
					       value + addr - stubaddr,
					       error_message);
	      mmix_cgc_section_data (isec)->pjs.stub_offset += 4;

	      if (size + mmix_cgc_section_data (isec)->pjs.stub_offset
		  > isec->size)
		abort ();
	...
	struct bpo_reloc_section_info *bpodata
	  = mmix_cgc_section_data (isec)->bpo.reloc;
	asection *bpo_greg_section;
	...
	if (bpodata == NULL)
	  {
	    /* This shouldn't happen when linking to CGC or mmo, so
	       ...
	       can't access the output bfd, so we can't verify that
	       assumption.  We only know that the critical
	       mmix_cgc_check_common_relocs has not been called, which
	       happens when the output format is different from the
	       input format (and is not mmo).  */
	    if (! mmix_cgc_section_data (isec)->has_warned_bpo)
	      {
		/* For the first such error per input section, produce
		   a verbose message.  */
		*error_message
		  = _("invalid input relocation when producing"
		      ...
		      " \"-mno-base-addresses\".");
		mmix_cgc_section_data (isec)->has_warned_bpo = TRUE;
		return bfd_reloc_dangerous;
	      }
	...
	gregdata = mmix_cgc_section_data (bpo_greg_section)->bpo.greg;
...
/* Set the howto pointer for an MMIX CGC reloc (type RELA).  */
			 ...
			 Cgc_Internal_Rela *dst)
{
  ...
  r_type = CGC64_R_TYPE (dst->r_info);
  BFD_ASSERT (r_type < (unsigned int) R_MMIX_max);
  cache_ptr->howto = &cgc_mmix_howto_table[r_type];
}
...
/* Any MMIX-specific relocation gets here at assembly time or when linking
   ...
   the reloc_table.  We don't get here for final pure CGC linking.  */

static bfd_reloc_status_type
mmix_cgc_reloc (bfd *abfd,
		arelent *reloc_entry,
		asymbol *symbol,
		void * data,
		...
		char **error_message)
{
  ...
  r = bfd_cgc_generic_reloc (abfd, reloc_entry, symbol, data,
...

/* Relocate an MMIX CGC section.  Modified from cgc32-fr30.c; look to it
   for guidance if you're thinking of copying this.  */

static bfd_boolean
mmix_cgc_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
			   ...
			   asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel ++)
	...
	continue;
      ...
      howto = cgc_mmix_howto_table + CGC64_R_TYPE (rel->r_info);
      h = NULL;
      sym = NULL;
      sec = NULL;
	  ...
	  if (r_type == R_MMIX_PUSHJ_STUBBABLE)
	    {
		 ...
		 knowledge.  */
	      if (mmix_cgc_section_data (input_section)->pjs.stub_size[pjsno]
		  != 0)
		{
		  Cgc_Internal_Rela relcpy;

		  if (mmix_cgc_section_data (input_section)
		      ->pjs.stub_size[pjsno] != MAX_PUSHJ_STUB_SIZE)
		    abort ();

		  /* There's already a PUSHJ insn there, so just fill in
		     the offset bits to the stub.  */
		  if (mmix_final_link_relocate (cgc_mmix_howto_table
						+ R_MMIX_ADDR19,
						input_section,
						contents,
						rel->r_offset,
						...
						+ size
						+ mmix_cgc_section_data (input_section)
						->pjs.stub_offset,
		  ...
		  /* Change the reloc to be at the stub, and to a full
		     R_MMIX_JMP reloc.  */
		  rel->r_info = CGC64_R_INFO (r_symndx, R_MMIX_JMP);
		  rel->r_offset
		    = (size
		       + mmix_cgc_section_data (input_section)
		       ->pjs.stub_offset);

		  mmix_cgc_section_data (input_section)->pjs.stub_offset
		    += MAX_PUSHJ_STUB_SIZE;
			  ...
			  char **error_message)
{
  ...
  switch (howto->type)
    ...
    case R_MMIX_JMP:
      ...
      r = mmix_cgc_perform_relocation (input_section, howto, contents,
    ...
    case R_MMIX_REG:
	 ...
	 FIXME: Postpone section->scaling to mmix_cgc_perform_relocation? */
      if (symsec == NULL)
	return bfd_reloc_undefined;
...
static asection *
mmix_cgc_gc_mark_hook (asection *sec,
		       struct bfd_link_info *info,
		       Cgc_Internal_Rela *rel,
		       struct cgc_link_hash_entry *h,
		       Cgc_Internal_Sym *sym)
...
static bfd_boolean
mmix_cgc_gc_sweep_hook (bfd *abfd ATTRIBUTE_UNUSED,
			struct bfd_link_info *info ATTRIBUTE_UNUSED,
			asection *sec,
			const Cgc_Internal_Rela *relocs ATTRIBUTE_UNUSED)
{
  struct bpo_reloc_section_info *bpodata
    = mmix_cgc_section_data (sec)->bpo.reloc;
  ...
  mmix_cgc_section_data (allocated_gregs_section)->bpo.greg->n_bpo_relocs
...
static int
mmix_cgc_sort_relocs (const void * p1, const void * p2)
{
  const Cgc_Internal_Rela *r1 = (const Cgc_Internal_Rela *) p1;
  const Cgc_Internal_Rela *r2 = (const Cgc_Internal_Rela *) p2;
  ...
  r1_is_reg
    = (CGC64_R_TYPE (r1->r_info) == R_MMIX_REG_OR_BYTE
       || CGC64_R_TYPE (r1->r_info) == R_MMIX_REG);
  r2_is_reg
    = (CGC64_R_TYPE (r2->r_info) == R_MMIX_REG_OR_BYTE
       || CGC64_R_TYPE (r2->r_info) == R_MMIX_REG);
...
/* Subset of mmix_cgc_check_relocs, common to CGC and mmo linking.  */

static bfd_boolean
mmix_cgc_check_common_relocs  (bfd *abfd,
			       struct bfd_link_info *info,
			       asection *sec,
			       const Cgc_Internal_Rela *relocs)
{
  ...
  /* We currently have to abuse this COFF-specific member, since there's
     ...
     they're different between CGC and mmo.  */
  ...
  for (rel = relocs; rel < rel_end; rel++)
    {
	  ...
	  /* This relocation causes a GREG allocation.  We need to count
	     ...
	     the CGC dynobj for this, since the CGC bits assume lots of
	...
	case R_MMIX_BASE_PLUS_OFFSET:
	  ...
	  if (allocated_gregs_section == NULL)
		...
		bfd_zalloc (bpo_greg_owner, sizeof (struct bpo_greg_section_info));
	      if (gregdata == NULL)
		return FALSE;
	      mmix_cgc_section_data (allocated_gregs_section)->bpo.greg
		= gregdata;
	    }
	  ...
	  if (bpodata == NULL)
		...
		bfd_alloc (bpo_greg_owner,
	      ...
	      if (bpodata == NULL)
		return FALSE;
	      mmix_cgc_section_data (sec)->bpo.reloc = bpodata;
	...
	case R_MMIX_PUSHJ_STUBBABLE:
	  mmix_cgc_section_data (sec)->pjs.n_pushj_relocs++;
	  break;
	}
    }
  ...
  /* Allocate per-reloc stub storage and initialize it to the max stub
     size.  */
  if (mmix_cgc_section_data (sec)->pjs.n_pushj_relocs != 0)
    {
      size_t i;

      mmix_cgc_section_data (sec)->pjs.stub_size
	= bfd_alloc (abfd, mmix_cgc_section_data (sec)->pjs.n_pushj_relocs
		     * sizeof (mmix_cgc_section_data (sec)
			       ->pjs.stub_size[0]));
      if (mmix_cgc_section_data (sec)->pjs.stub_size == NULL)
	return FALSE;

      for (i = 0; i < mmix_cgc_section_data (sec)->pjs.n_pushj_relocs; i++)
	mmix_cgc_section_data (sec)->pjs.stub_size[i] = MAX_PUSHJ_STUB_SIZE;
    }
...
static bfd_boolean
mmix_cgc_check_relocs (bfd *abfd,
		       struct bfd_link_info *info,
		       asection *sec,
		       const Cgc_Internal_Rela *relocs)
{
  ...
  qsort ((void *) relocs, sec->reloc_count, sizeof (Cgc_Internal_Rela),
	 mmix_cgc_sort_relocs);

  /* Do the common part.  */
  if (!mmix_cgc_check_common_relocs (abfd, info, sec, relocs))
    return FALSE;
...
/* Wrapper for mmix_cgc_check_common_relocs, called when linking to mmo.
   Copied from cgc_link_add_object_symbols.  */
...
_bfd_mmix_check_all_relocs (bfd *abfd, struct bfd_link_info *info)
{
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
	...
	continue;
      ...
      internal_relocs
	= _bfd_cgc_link_read_relocs (abfd, o, NULL,
				     (Cgc_Internal_Rela *) NULL,
				     info->keep_memory);
      if (internal_relocs == NULL)
	return FALSE;

      ok = mmix_cgc_check_common_relocs (abfd, info, o, internal_relocs);
...
static int
mmix_cgc_link_output_symbol_hook (struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
/* We fake a register section that holds values that are register numbers.
   ...
   This section faking is based on a construct in cgc32-mips.c.  */
static asection mmix_cgc_reg_section;
static asymbol mmix_cgc_reg_section_symbol;
static asymbol *mmix_cgc_reg_section_symbol_ptr;

/* Handle the special section numbers that a symbol may use.  */

void
mmix_cgc_symbol_processing (abfd, asym)
     ...
     asymbol *asym;
{
  ...
  switch (cgcsym->internal_cgc_sym.st_shndx)
    {
    case SHN_REGISTER:
      if (mmix_cgc_reg_section.name == NULL)
	{
	  /* Initialize the register section.  */
	  mmix_cgc_reg_section.name = MMIX_REG_SECTION_NAME;
	  mmix_cgc_reg_section.flags = SEC_NO_FLAGS;
	  mmix_cgc_reg_section.output_section = &mmix_cgc_reg_section;
	  mmix_cgc_reg_section.symbol = &mmix_cgc_reg_section_symbol;
	  mmix_cgc_reg_section.symbol_ptr_ptr = &mmix_cgc_reg_section_symbol_ptr;
	  mmix_cgc_reg_section_symbol.name = MMIX_REG_SECTION_NAME;
	  mmix_cgc_reg_section_symbol.flags = BSF_SECTION_SYM;
	  mmix_cgc_reg_section_symbol.section = &mmix_cgc_reg_section;
	  mmix_cgc_reg_section_symbol_ptr = &mmix_cgc_reg_section_symbol;
	}
      asym->section = &mmix_cgc_reg_section;
      break;
...
static bfd_boolean
mmix_cgc_section_from_bfd_section (bfd *       abfd ATTRIBUTE_UNUSED,
				   asection *  sec,
				   int *       retval)
{
...
static bfd_boolean
mmix_cgc_add_symbol_hook (bfd *abfd,
			  struct bfd_link_info *info ATTRIBUTE_UNUSED,
			  Cgc_Internal_Sym *sym,
...
static bfd_boolean
mmix_cgc_is_local_label_name (bfd *abfd, const char *name)
{
  const char *colpos;
  int digits;

  /* Also include the default local-label definition.  */
  if (_bfd_cgc_is_local_label_name (abfd, name))
    return TRUE;
...
bfd_boolean
mmix_cgc_final_link (bfd *abfd, struct bfd_link_info *info)
{
			 ...
			 void *ptr)
{
  ...
  sec->size += (mmix_cgc_section_data (sec)->pjs.n_pushj_relocs
		* MAX_PUSHJ_STUB_SIZE);

  /* For use in relocatable link, we start with a max stubs size.  See
     mmix_cgc_relax_section.  */
  if (info->relocatable && sec->output_section)
    mmix_cgc_section_data (sec->output_section)->pjs.stubs_size_sum
      += (mmix_cgc_section_data (sec)->pjs.n_pushj_relocs
	  * MAX_PUSHJ_STUB_SIZE);
}
				    ...
				    struct bfd_link_info *info)
{
  ...
  /* The bpo_greg_owner bfd is supposed to have been set by
     mmix_cgc_check_relocs when the first R_MMIX_BASE_PLUS_OFFSET is seen.
  ...
  /* We use the target-data handle in the CGC section data.  */
  gregdata = mmix_cgc_section_data (bpo_gregs_section)->bpo.greg;
  if (gregdata == NULL)
    return FALSE;
				   ...
				   struct bfd_link_info *link_info)
{
  ...
  /* The bpo_greg_owner bfd is supposed to have been set by mmix_cgc_check_relocs
...
/* This links all R_MMIX_BASE_PLUS_OFFSET relocs into a special array, and
   ...
   Symbol- and reloc-reading infrastructure copied from cgc-m10200.c.  */

static bfd_boolean
mmix_cgc_relax_section (bfd *abfd,
			asection *sec,
			struct bfd_link_info *link_info,
			bfd_boolean *again)
{
  ...
  mmix_cgc_section_data (sec)->pjs.stubs_size_sum = 0;
  ...
  *again = FALSE;
  ...
  if ((sec->flags & SEC_RELOC) == 0
      ...
      /* If no R_MMIX_BASE_PLUS_OFFSET relocs and no PUSHJ-stub relocs,
         then nothing to do.  */
      || (bpodata == NULL
	  && mmix_cgc_section_data (sec)->pjs.n_pushj_relocs == 0))
    return TRUE;
  ...
  internal_relocs
    = _bfd_cgc_link_read_relocs (abfd, sec, NULL,
				 (Cgc_Internal_Rela *) NULL,
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      if (CGC64_R_TYPE (irel->r_info) != (int) R_MMIX_BASE_PLUS_OFFSET
	  && CGC64_R_TYPE (irel->r_info) != (int) R_MMIX_PUSHJ_STUBBABLE)
	continue;
      ...
      if (link_info->relocatable)
	{
	  /* The only transformation in a relocatable link is to generate
	     ...
	     works.  */
	  if (CGC64_R_TYPE (irel->r_info) == R_MMIX_PUSHJ_STUBBABLE)
	    {
		 ...
		 and thus the order of output stub allocation.  */
	      if (bfd_check_overflow (complain_overflow_signed,
					 ...
					 + MAX_PUSHJ_STUB_SIZE))
		  == bfd_reloc_ok)
		mmix_cgc_section_data (sec)->pjs.stub_size[pjsno] = 0;

	      mmix_cgc_section_data (sec)->pjs.stubs_size_sum
		+= mmix_cgc_section_data (sec)->pjs.stub_size[pjsno];
      ...
      else
	{
	  ...
	  if (h->root.type != bfd_link_hash_defined
	      && h->root.type != bfd_link_hash_defweak)
	    {
		 ...
		 We need to keep BPO reloc accounting consistent, though
	      ...
	      if (CGC64_R_TYPE (irel->r_info) == R_MMIX_BASE_PLUS_OFFSET
		  && gregdata != NULL)
		{
		  gregdata->n_remaining_bpo_relocs_this_relaxation_round--;
		  bpono++;
      ...
      if (CGC64_R_TYPE (irel->r_info) == (int) R_MMIX_PUSHJ_STUBBABLE)
	{
	  ...
	  if ((value & 3) == 0
		...
		== bfd_reloc_ok)
	      ...
	      mmix_cgc_section_data (sec)->pjs.stub_size[pjsno] = 4;
	  else
  ...
  if (sec->size < size + mmix_cgc_section_data (sec)->pjs.stubs_size_sum)
    abort ();

  if (sec->size > size + mmix_cgc_section_data (sec)->pjs.stubs_size_sum)
    {
      sec->size = size + mmix_cgc_section_data (sec)->pjs.stubs_size_sum;
      *again = TRUE;
    }
...

#define CGC_ARCH		bfd_arch_mmix
#define CGC_MACHINE_CODE 	EM_MMIX
...
#define TARGET_BIG_SYM		bfd_cgc64_mmix_vec
#define TARGET_BIG_NAME		"cgc64-mmix"

#define cgc_info_to_howto_rel		NULL
#define cgc_info_to_howto		mmix_info_to_howto_rela
#define cgc_backend_relocate_section	mmix_cgc_relocate_section
#define cgc_backend_gc_mark_hook	mmix_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook	mmix_cgc_gc_sweep_hook

#define cgc_backend_link_output_symbol_hook \
	mmix_cgc_link_output_symbol_hook
#define cgc_backend_add_symbol_hook	mmix_cgc_add_symbol_hook

#define cgc_backend_check_relocs	mmix_cgc_check_relocs
#define cgc_backend_symbol_processing	mmix_cgc_symbol_processing
#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)

#define bfd_cgc64_bfd_is_local_label_name \
	mmix_cgc_is_local_label_name
...
#define cgc_backend_section_from_bfd_section \
	mmix_cgc_section_from_bfd_section

#define bfd_cgc64_new_section_hook	mmix_cgc_new_section_hook
#define bfd_cgc64_bfd_final_link	mmix_cgc_final_link
#define bfd_cgc64_bfd_relax_section	mmix_cgc_relax_section
