  ...
  while (p < endp)
    {
      ...
      else if (bfd_libdata (abfd)->ver == LBR_CGCMAJORID)
        {
          struct vms_cgcidx *ridx = (struct vms_cgcidx *)p;
...
static unsigned int
get_idxlen (struct lib_index *idx, bfd_boolean is_cgcidx)
{
  if (is_cgcidx)
    {
      /* 9 is the size of struct vms_cgcidx without keyname.  */
...
/* Write the index composed by NBR symbols contained in IDX.
   ...
   IS_CGCIDX is true for cgcidx (ie ia64) indexes layout.
   Return TRUE on success.  */

static bfd_boolean
vms_write_index (bfd *abfd,
                 struct lib_index *idx, unsigned int nbr, unsigned int *vbn,
                 unsigned int *topvbn, bfd_boolean is_cgcidx)
{
  ...
  for (i = 0; i < nbr; i++, idx++)
    {
      ...
      idxlen = get_idxlen (idx, is_cgcidx);

      if (is_cgcidx && idx->namlen > MAX_KEYLEN)
        {
              ...
              if (abfd != NULL)
                {
                  ...
                  bfd_putl16
                    ((idx->abfd->proxy_origin % VMS_BLOCK_SIZE)
                     + (is_cgcidx ? 0 : DATA__DATA),
                     rfa->offset);

                  if (is_cgcidx)
                    {
                      /* Use cgcidx format.  */
                      struct vms_cgcidx *en = (struct vms_cgcidx *)rfa;
...
_bfd_vms_lib_write_archive_contents (bfd *arch)
{
  ...
  bfd_boolean is_cgcidx = tdata->kind == vms_lib_ia64;
  unsigned int max_keylen = is_cgcidx ? MAX_EKEYLEN : MAX_KEYLEN;
  ...
  vbn = 0;
  if (!vms_write_index (NULL, modules, nbr_modules, &vbn, NULL, is_cgcidx))
    return FALSE;
  nbr_mod_iblk = vbn;
  ...
  vbn = 0;
  if (!vms_write_index (NULL, symbols, nbr_symbols, &vbn, NULL, is_cgcidx))
    return FALSE;
  nbr_sym_iblk = vbn;
