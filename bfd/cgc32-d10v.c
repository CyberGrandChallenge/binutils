/* D10V-specific support for 32-bit CGC
...
static reloc_howto_type cgc_d10v_howto_table[] =
{
...
/* Map BFD reloc types to D10V CGC reloc types.  */
				 ...
				 bfd_reloc_code_real_type code)
{
  ...
  for (i = 0;
    ...
    if (d10v_reloc_map[i].bfd_reloc_val == code)
      return &cgc_d10v_howto_table[d10v_reloc_map[i].cgc_reloc_val];
				 ...
				 const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (cgc_d10v_howto_table) / sizeof (cgc_d10v_howto_table[0]);
       i++)
    if (cgc_d10v_howto_table[i].name != NULL
	&& strcasecmp (cgc_d10v_howto_table[i].name, r_name) == 0)
      return &cgc_d10v_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an D10V CGC reloc.  */
			...
			Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = &cgc_d10v_howto_table[r_type];
}

static asection *
cgc32_d10v_gc_mark_hook (asection *sec,
...
static bfd_boolean
cgc32_d10v_check_relocs (bfd *abfd,
			 struct bfd_link_info *info,
			 asection *sec,
...
/* Relocate a D10V CGC section.  */

static bfd_boolean
cgc32_d10v_relocate_section (bfd *output_bfd,
			     struct bfd_link_info *info,
			     bfd *input_bfd,
  ...
  for (; rel < relend; rel++)
    {
      ...
      howto = cgc_d10v_howto_table + r_type;
      h = NULL;
      sym = NULL;
      sec = NULL;
      if (r_symndx < symtab_hdr->sh_info)
...
#define CGC_ARCH		bfd_arch_d10v
#define CGC_MACHINE_CODE	EM_D10V
#define CGC_MACHINE_ALT1	EM_CYGNUS_D10V
#define CGC_MAXPAGESIZE		0x1000

#define TARGET_BIG_SYM          bfd_cgc32_d10v_vec
#define TARGET_BIG_NAME		"cgc32-d10v"

#define cgc_info_to_howto	             0
#define cgc_info_to_howto_rel	             d10v_info_to_howto_rel
#define cgc_backend_object_p	             0
#define cgc_backend_final_write_processing   0
#define cgc_backend_gc_mark_hook             cgc32_d10v_gc_mark_hook
#define cgc_backend_check_relocs             cgc32_d10v_check_relocs
#define cgc_backend_relocate_section         cgc32_d10v_relocate_section
