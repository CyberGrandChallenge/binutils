/* Alpha specific support for 64-bit CGC
...
#ifdef USE_SECUREPLT
bfd_boolean cgc64_alpha_use_secureplt = TRUE;
#else
bfd_boolean cgc64_alpha_use_secureplt = FALSE;
#endif
...
#define PLT_HEADER_SIZE \
  (cgc64_alpha_use_secureplt ? NEW_PLT_HEADER_SIZE : OLD_PLT_HEADER_SIZE)
#define PLT_ENTRY_SIZE \
  (cgc64_alpha_use_secureplt ? NEW_PLT_ENTRY_SIZE : OLD_PLT_ENTRY_SIZE)

#define MAX_GOT_SIZE		(64*1024)

#define CGC_DYNAMIC_INTERPRETER "/usr/lib/ld.so"


/* Used to implement multiple .got subsections.  */
struct alpha_cgc_got_entry
{
  struct alpha_cgc_got_entry *next;
...
struct alpha_cgc_reloc_entry
{
  struct alpha_cgc_reloc_entry *next;
...
struct alpha_cgc_link_hash_entry
{
...
#define ALPHA_CGC_LINK_HASH_LU_ADDR	 0x01
#define ALPHA_CGC_LINK_HASH_LU_MEM	 0x02
#define ALPHA_CGC_LINK_HASH_LU_BYTE	 0x04
#define ALPHA_CGC_LINK_HASH_LU_JSR	 0x08
#define ALPHA_CGC_LINK_HASH_LU_TLSGD	 0x10
#define ALPHA_CGC_LINK_HASH_LU_TLSLDM	 0x20
#define ALPHA_CGC_LINK_HASH_LU_JSRDIRECT 0x40
#define ALPHA_CGC_LINK_HASH_LU_PLT	 0x38
#define ALPHA_CGC_LINK_HASH_TLS_IE	 0x80

  /* Used to implement multiple .got subsections.  */
  struct alpha_cgc_got_entry *got_entries;

  /* Used to count non-got, non-plt relocations for delayed sizing
     of relocation sections.  */
  struct alpha_cgc_reloc_entry *reloc_entries;
};

/* Alpha CGC linker hash table.  */

struct alpha_cgc_link_hash_table
{
  struct cgc_link_hash_table root;

  /* The head of a list of .got subsections linked through
     alpha_cgc_tdata(abfd)->got_link_next.  */
  bfd *got_list;
...
/* Look up an entry in a Alpha CGC linker hash table.  */

#define alpha_cgc_link_hash_lookup(table, string, create, copy, follow)	\
  ((struct alpha_cgc_link_hash_entry *)					\
   cgc_link_hash_lookup (&(table)->root, (string), (create),		\
			 (copy), (follow)))

/* Traverse a Alpha CGC linker hash table.  */

#define alpha_cgc_link_hash_traverse(table, func, info)			\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func),	\
    (info)))

/* Get the Alpha CGC linker hash table from a link_info structure.  */

#define alpha_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == ALPHA_CGC_DATA ? ((struct alpha_cgc_link_hash_table *) ((p)->hash)) : NULL)

/* Get the object's symbols as our own entry type.  */

#define alpha_cgc_sym_hashes(abfd) \
  ((struct alpha_cgc_link_hash_entry **)cgc_sym_hashes(abfd))
...
alpha_cgc_dynamic_symbol_p (struct cgc_link_hash_entry *h,
			    struct bfd_link_info *info)
{
  return _bfd_cgc_dynamic_symbol_p (h, info, 0);
}

/* Create an entry in a Alpha CGC linker hash table.  */

static struct bfd_hash_entry *
cgc64_alpha_link_hash_newfunc (struct bfd_hash_entry *entry,
			       struct bfd_hash_table *table,
			       const char *string)
{
  struct alpha_cgc_link_hash_entry *ret =
    (struct alpha_cgc_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == (struct alpha_cgc_link_hash_entry *) NULL)
    ret = ((struct alpha_cgc_link_hash_entry *)
	   bfd_hash_allocate (table,
			      sizeof (struct alpha_cgc_link_hash_entry)));
  if (ret == (struct alpha_cgc_link_hash_entry *) NULL)
...
/* Create a Alpha CGC linker hash table.  */

static struct bfd_link_hash_table *
cgc64_alpha_bfd_link_hash_table_create (bfd *abfd)
{
  struct alpha_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct alpha_cgc_link_hash_table);

  ret = (struct alpha_cgc_link_hash_table *) bfd_zmalloc (amt);
  if (ret == (struct alpha_cgc_link_hash_table *) NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      cgc64_alpha_link_hash_newfunc,
				      sizeof (struct alpha_cgc_link_hash_entry),
				      ALPHA_CGC_DATA))
    {
      free (ret);
...

/* Alpha CGC follows MIPS CGC in using a special find_nearest_line
   routine in order to handle the ECOFF debugging information.  */

struct alpha_cgc_find_line
{
  struct ecoff_debug_info d;
  struct ecoff_find_line i;
};

/* We have some private fields hanging off of the cgc_tdata structure.  */

struct alpha_cgc_obj_tdata
{
  struct cgc_obj_tdata root;

  /* For every input file, these are the got entries for that object's
     local symbols.  */
  struct alpha_cgc_got_entry ** local_got_entries;
  ...
  /* Used by cgc64_alpha_find_nearest_line entry point.  */
  struct alpha_cgc_find_line *find_line_info;

};

#define alpha_cgc_tdata(abfd) \
  ((struct alpha_cgc_obj_tdata *) (abfd)->tdata.any)

#define is_alpha_cgc(bfd) \
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour \
   && cgc_tdata (bfd) != NULL \
   && cgc_object_id (bfd) == ALPHA_CGC_DATA)

static bfd_boolean
cgc64_alpha_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct alpha_cgc_obj_tdata),
				  ALPHA_CGC_DATA);
}

static bfd_boolean
cgc64_alpha_object_p (bfd *abfd)
{
  /* Set the right machine number for an Alpha CGC file.  */
...
cgc64_alpha_reloc_nil (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc,
...
static bfd_reloc_status_type
cgc64_alpha_reloc_bad (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc,
		       asymbol *sym ATTRIBUTE_UNUSED,
		       void * data ATTRIBUTE_UNUSED, asection *sec,
		       bfd *output_bfd, char **error_message ATTRIBUTE_UNUSED)
{
...
static bfd_reloc_status_type
cgc64_alpha_do_reloc_gpdisp (bfd *abfd, bfd_vma gpdisp, bfd_byte *p_ldah,
			     bfd_byte *p_lda)
{
...
static bfd_reloc_status_type
cgc64_alpha_reloc_gpdisp (bfd *abfd, arelent *reloc_entry,
			  ...
			  char **err_msg)
{
  ...
  ret = cgc64_alpha_do_reloc_gpdisp (abfd, gp - relocation, p_ldah, p_lda);
...
#define SKIP_HOWTO(N) \
  HOWTO(N, 0, 0, 0, 0, 0, complain_overflow_dont, cgc64_alpha_reloc_bad, 0, 0, 0, 0, 0)

static reloc_howto_type cgc64_alpha_howto_table[] =
{
  HOWTO (R_ALPHA_NONE,		/* type */
	 ...
	 cgc64_alpha_reloc_nil,	/* special_function */
	 "NONE",		/* name */
  ...
  /* This reloc only appears immediately following an CGC_LITERAL reloc.
  ...
  HOWTO (R_ALPHA_GPDISP,	/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_dont, /* complain_on_overflow */
	 cgc64_alpha_reloc_gpdisp, /* special_function */
	 "GPDISP",		/* name */
	 FALSE,			/* partial_inplace */
	 0xffff,		/* src_mask */
	 0xffff,		/* dst_mask */
  ...
  /* Misc CGC relocations.  */

  /* A dynamic relocation to copy the target into our .dynbss section.  */
  /* Not generated, as all Alpha objects use PIC, so it is not needed.  It
     is present because every other CGC has one, but should not be used
...
/* A mapping from BFD reloc types to Alpha CGC reloc types.  */
...
static const struct cgc_reloc_map cgc64_alpha_reloc_map[] =
{
  ...
  {BFD_RELOC_ALPHA_CGC_LITERAL,		R_ALPHA_LITERAL},
...
cgc64_alpha_bfd_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				   bfd_reloc_code_real_type code)
{
  const struct cgc_reloc_map *i, *e;
  i = e = cgc64_alpha_reloc_map;
  e += sizeof (cgc64_alpha_reloc_map) / sizeof (struct cgc_reloc_map);
  for (; i != e; ++i)
    {
      if (i->bfd_reloc_val == code)
	return &cgc64_alpha_howto_table[i->cgc_reloc_val];
    }
  return 0;
}

static reloc_howto_type *
cgc64_alpha_bfd_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				   const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < (sizeof (cgc64_alpha_howto_table)
	    / sizeof (cgc64_alpha_howto_table[0]));
       i++)
    if (cgc64_alpha_howto_table[i].name != NULL
	&& strcasecmp (cgc64_alpha_howto_table[i].name, r_name) == 0)
      return &cgc64_alpha_howto_table[i];

  return NULL;
}

/* Given an Alpha CGC reloc type, fill in an arelent structure.  */

static void
cgc64_alpha_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *cache_ptr,
			   Cgc_Internal_Rela *dst)
{
  unsigned r_type = CGC64_R_TYPE(dst->r_info);
  BFD_ASSERT (r_type < (unsigned int) R_ALPHA_max);
  cache_ptr->howto = &cgc64_alpha_howto_table[r_type];
}
...
#define alpha_get_dtprel_base(info) \
  (cgc_hash_table (info)->tls_sec->vma)
...
static bfd_boolean
cgc64_alpha_section_from_shdr (bfd *abfd,
...
static bfd_boolean
cgc64_alpha_section_flags (flagword *flags, const Cgc_Internal_Shdr *hdr)
{
...
/* Set the correct type for an Alpha CGC section.  We do this by the
   section name, which is a hack, but ought to work.  */

static bfd_boolean
cgc64_alpha_fake_sections (bfd *abfd, Cgc_Internal_Shdr *hdr, asection *sec)
{
...
static bfd_boolean
cgc64_alpha_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
...
static bfd_boolean
cgc64_alpha_create_got_section (bfd *abfd,
				struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  flagword flags;
  asection *s;

  if (! is_alpha_cgc (abfd))
    return FALSE;
  ...
  alpha_cgc_tdata (abfd)->gotobj = abfd;
...
static bfd_boolean
cgc64_alpha_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  asection *s;
  flagword flags;
  ...
  flags = (SEC_ALLOC | SEC_LOAD | SEC_CODE | SEC_HAS_CONTENTS | SEC_IN_MEMORY
	   | SEC_LINKER_CREATED
	   | (cgc64_alpha_use_secureplt ? SEC_READONLY : 0));
  ...
  if (alpha_cgc_tdata(abfd)->gotobj == NULL)
    {
      if (!cgc64_alpha_create_got_section (abfd, info))
	return FALSE;
    }
  ...
  h = _bfd_cgc_define_linkage_sym (abfd, info, alpha_cgc_tdata(abfd)->got,
...
static bfd_boolean
cgc64_alpha_read_ecoff_info (bfd *abfd, asection *section,
...
/* Alpha CGC local labels start with '$'.  */

static bfd_boolean
cgc64_alpha_is_local_label_name (bfd *abfd ATTRIBUTE_UNUSED, const char *name)
{
  return name[0] == '$';
}

static bfd_boolean
cgc64_alpha_find_nearest_line (bfd *abfd, asection *section, asymbol **symbols,
			       ...
			       unsigned int *line_ptr)
{
  ...
  if (msec != NULL)
    {
      flagword origflags;
      struct alpha_cgc_find_line *fi;
      const struct ecoff_debug_swap * const swap =
	get_cgc_backend_data (abfd)->cgc_backend_ecoff_debug_swap;

      /* If we are called during a link, alpha_cgc_final_link may have
	...
	msec->flags |= SEC_HAS_CONTENTS;
      ...
      fi = alpha_cgc_tdata (abfd)->find_line_info;
      if (fi == NULL)
	{
	  ...
	  char *fraw_end;
	  struct fdr *fdr_ptr;
	  bfd_size_type amt = sizeof (struct alpha_cgc_find_line);

	  fi = (struct alpha_cgc_find_line *) bfd_zalloc (abfd, amt);
	  if (fi == NULL)
	    {
	  ...
	  if (!cgc64_alpha_read_ecoff_info (abfd, msec, &fi->d))
	    {
	      msec->flags = origflags;
	      return FALSE;
	    }
	  ...
	  alpha_cgc_tdata (abfd)->find_line_info = fi;
...

/* Structure used to pass information to alpha_cgc_output_extsym.  */
...
static bfd_boolean
cgc64_alpha_output_extsym (struct alpha_cgc_link_hash_entry *h, void * data)
{
...
static struct alpha_cgc_got_entry *
get_got_entry (bfd *abfd, struct alpha_cgc_link_hash_entry *h,
	       unsigned long r_type, unsigned long r_symndx,
	       bfd_vma r_addend)
{
  struct alpha_cgc_got_entry *gotent;
  struct alpha_cgc_got_entry **slot;
  ...
  if (h)
  ...
  else
    {
      ...
      local_got_entries = alpha_cgc_tdata(abfd)->local_got_entries;
      if (!local_got_entries)
	{
	  ...
	  size *= sizeof (struct alpha_cgc_got_entry *);

	  local_got_entries
	    = (struct alpha_cgc_got_entry **) bfd_zalloc (abfd, size);
	  if (!local_got_entries)
	    return NULL;

	  alpha_cgc_tdata (abfd)->local_got_entries = local_got_entries;
	}
  ...
  if (!gotent)
    {
      ...
      amt = sizeof (struct alpha_cgc_got_entry);
      gotent = (struct alpha_cgc_got_entry *) bfd_alloc (abfd, amt);
      if (!gotent)
	return NULL;
      ...
      *slot = gotent;
      ...
      alpha_cgc_tdata (abfd)->total_got_size += entry_size;
      if (!h)
	alpha_cgc_tdata(abfd)->local_got_size += entry_size;
    }
  else
    gotent->use_count += 1;
...
static bfd_boolean
cgc64_alpha_want_plt (struct alpha_cgc_link_hash_entry *ah)
{
  return ((ah->root.type == STT_FUNC
	  || ah->root.root.type == bfd_link_hash_undefweak
	  || ah->root.root.type == bfd_link_hash_undefined)
	  && (ah->flags & ALPHA_CGC_LINK_HASH_LU_PLT) != 0
	  && (ah->flags & ~ALPHA_CGC_LINK_HASH_LU_PLT) == 0);
}

/* Handle dynamic relocations when doing an Alpha CGC link.  */

static bfd_boolean
cgc64_alpha_check_relocs (bfd *abfd, struct bfd_link_info *info,
			  asection *sec, const Cgc_Internal_Rela *relocs)
{
  bfd *dynobj;
  asection *sreloc;
  Cgc_Internal_Shdr *symtab_hdr;
  struct alpha_cgc_link_hash_entry **sym_hashes;
  ...
  sym_hashes = alpha_cgc_sym_hashes (abfd);
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	NEED_DYNREL = 4
      ...
      struct alpha_cgc_link_hash_entry *h;
      ...
      else
	{
	  ...
	  while (h->root.root.type == bfd_link_hash_indirect
	    ...
	    h = (struct alpha_cgc_link_hash_entry *)h->root.root.u.i.link;
	...
	case R_ALPHA_LITERAL:
	  ...
	  while (++rel < relend && CGC64_R_TYPE (rel->r_info) == R_ALPHA_LITUSE)
	  ...
	  if (gotent_flags == 0)
	    gotent_flags = ALPHA_CGC_LINK_HASH_LU_ADDR;
	  break;
	...
	case R_ALPHA_GOTTPREL:
	  need = NEED_GOT | NEED_GOT_ENTRY;
	  gotent_flags = ALPHA_CGC_LINK_HASH_TLS_IE;
	  if (info->shared)
      ...
      if (need & NEED_GOT_ENTRY)
	{
	  ...
	  if (gotent_flags)
	      ...
	      if (h)
		{
		  ...
		  h->root.needs_plt
		    = (maybe_dynamic && cgc64_alpha_want_plt (h));
		}
	    }
	}
	  ...
	  if (h)
	    {
		 ...
		 expand the relocation sections by the appropriate amount.  */
	      ...
	      struct alpha_cgc_reloc_entry *rent;
		...
		if (rent->rtype == r_type && rent->srel == sreloc)
	      ...
	      if (!rent)
		{
		  amt = sizeof (struct alpha_cgc_reloc_entry);
		  rent = (struct alpha_cgc_reloc_entry *) bfd_alloc (abfd, amt);
		  if (!rent)
		    return FALSE;
	  ...
	  else if (info->shared)
	    {
		 ...
		 loaded into memory, we need a RELATIVE reloc.  */
	      sreloc->size += sizeof (Cgc64_External_Rela);
...
static asection *
cgc64_alpha_gc_mark_hook (asection *sec, struct bfd_link_info *info,
...
static bfd_boolean
cgc64_alpha_gc_sweep_hook (bfd *abfd, struct bfd_link_info *info,
			   asection *sec, const Cgc_Internal_Rela *relocs)
{
  Cgc_Internal_Shdr *symtab_hdr;
  struct alpha_cgc_link_hash_entry **sym_hashes;
  ...
  for (rel = relocs; rel < relend; rel++)
    {
      unsigned long r_symndx, r_type;
      struct alpha_cgc_link_hash_entry *h = NULL;
      struct alpha_cgc_got_entry *gotent;
...
static bfd_boolean
cgc64_alpha_adjust_dynamic_symbol (struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h)
{
  bfd *dynobj;
  asection *s;
  struct alpha_cgc_link_hash_entry *ah;

  dynobj = cgc_hash_table(info)->dynobj;
  ah = (struct alpha_cgc_link_hash_entry *)h;
  ...
  /* Now that we've seen all of the input symbols, finalize our decision
     ...
     of STT_FUNC.  */
  if (alpha_cgc_dynamic_symbol_p (h, info) && cgc64_alpha_want_plt (ah))
    {
      h->needs_plt = TRUE;

      s = bfd_get_linker_section (dynobj, ".plt");
      if (!s && !cgc64_alpha_create_dynamic_sections (dynobj, info))
	return FALSE;
...
static void
cgc64_alpha_merge_symbol_attribute (struct cgc_link_hash_entry *h,
				    const Cgc_Internal_Sym *isym,
				    bfd_boolean definition,
				    bfd_boolean dynamic)
{
  if (!dynamic && definition)
    h->other = ((h->other & CGC_ST_VISIBILITY (-1))
...
static void
cgc64_alpha_copy_indirect_symbol (struct bfd_link_info *info,
				  struct cgc_link_hash_entry *dir,
				  struct cgc_link_hash_entry *ind)
{
  struct alpha_cgc_link_hash_entry *hi
    = (struct alpha_cgc_link_hash_entry *) ind;
  struct alpha_cgc_link_hash_entry *hs
    = (struct alpha_cgc_link_hash_entry *) dir;
  ...
  if (hs->got_entries == NULL)
    ...
    {
      struct alpha_cgc_got_entry *gi, *gs, *gin, *gsh;
  ...
  if (hs->reloc_entries == NULL)
    ...
    {
      struct alpha_cgc_reloc_entry *ri, *rs, *rin, *rsh;
...
static bfd_boolean
cgc64_alpha_can_merge_gots (bfd *a, bfd *b)
{
  int total = alpha_cgc_tdata (a)->total_got_size;
  bfd *bsub;

  /* Trivial quick fallout test.  */
  if (total + alpha_cgc_tdata (b)->total_got_size <= MAX_GOT_SIZE)
    return TRUE;

  /* By their nature, local .got entries cannot be merged.  */
  if ((total += alpha_cgc_tdata (b)->local_got_size) > MAX_GOT_SIZE)
    return FALSE;

  /* Failing the common trivial comparison, we must effectively
     perform the merge.  Not actually performing the merge means that
     we don't have to store undo information in case we fail.  */
  for (bsub = b; bsub ; bsub = alpha_cgc_tdata (bsub)->in_got_link_next)
    {
      struct alpha_cgc_link_hash_entry **hashes = alpha_cgc_sym_hashes (bsub);
      Cgc_Internal_Shdr *symtab_hdr = &cgc_tdata (bsub)->symtab_hdr;
      int i, n;

      n = NUM_SHDR_ENTRIES (symtab_hdr) - symtab_hdr->sh_info;
      for (i = 0; i < n; ++i)
	{
	  struct alpha_cgc_got_entry *ae, *be;
	  struct alpha_cgc_link_hash_entry *h;
...
static void
cgc64_alpha_merge_gots (bfd *a, bfd *b)
{
  int total = alpha_cgc_tdata (a)->total_got_size;
  bfd *bsub;
  ...
  {
    int e = alpha_cgc_tdata (b)->local_got_size;
    total += e;
    alpha_cgc_tdata (a)->local_got_size += e;
  }

  for (bsub = b; bsub ; bsub = alpha_cgc_tdata (bsub)->in_got_link_next)
    {
      struct alpha_cgc_got_entry **local_got_entries;
      struct alpha_cgc_link_hash_entry **hashes;
      Cgc_Internal_Shdr *symtab_hdr;
      int i, n;

      /* Let the local .got entries know they are part of a new subsegment.  */
      local_got_entries = alpha_cgc_tdata (bsub)->local_got_entries;
      if (local_got_entries)
        {
	  n = cgc_tdata (bsub)->symtab_hdr.sh_info;
	  for (i = 0; i < n; ++i)
	    {
	      struct alpha_cgc_got_entry *ent;
      ...
      hashes = alpha_cgc_sym_hashes (bsub);
      symtab_hdr = &cgc_tdata (bsub)->symtab_hdr;

      n = NUM_SHDR_ENTRIES (symtab_hdr) - symtab_hdr->sh_info;
      for (i = 0; i < n; ++i)
        {
	  struct alpha_cgc_got_entry *ae, *be, **pbe, **start;
	  struct alpha_cgc_link_hash_entry *h;
      ...
      alpha_cgc_tdata (bsub)->gotobj = a;
    }
  alpha_cgc_tdata (a)->total_got_size = total;
  ...
  /* Merge the two in_got chains.  */
  {
    ...
    bsub = a;
    while ((next = alpha_cgc_tdata (bsub)->in_got_link_next) != NULL)
      bsub = next;

    alpha_cgc_tdata (bsub)->in_got_link_next = b;
  }
}
...
static bfd_boolean
cgc64_alpha_calc_got_offsets_for_symbol (struct alpha_cgc_link_hash_entry *h,
					 void * arg ATTRIBUTE_UNUSED)
{
  ...
  for (gotent = h->got_entries; gotent; gotent = gotent->next)
    if (gotent->use_count > 0)
      {
	struct alpha_cgc_obj_tdata *td;
	bfd_size_type *plge;

	td = alpha_cgc_tdata (gotent->gotobj);
	plge = &td->got->size;
...
static void
cgc64_alpha_calc_got_offsets (struct bfd_link_info *info)
{
  bfd *i, *got_list;
  struct alpha_cgc_link_hash_table * htab;

  htab = alpha_cgc_hash_table (info);
  if (htab == NULL)
    return;
  ...
  for (i = got_list; i ; i = alpha_cgc_tdata(i)->got_link_next)
    alpha_cgc_tdata(i)->got->size = 0;

  /* Next, fill in the offsets for all the global entries.  */
  alpha_cgc_link_hash_traverse (htab,
				cgc64_alpha_calc_got_offsets_for_symbol,
				NULL);

  /* Finally, fill in the offsets for the local entries.  */
  for (i = got_list; i ; i = alpha_cgc_tdata(i)->got_link_next)
    {
      bfd_size_type got_offset = alpha_cgc_tdata(i)->got->size;
      bfd *j;

      for (j = i; j ; j = alpha_cgc_tdata(j)->in_got_link_next)
	{
	  struct alpha_cgc_got_entry **local_got_entries, *gotent;
	  int k, n;

	  local_got_entries = alpha_cgc_tdata(j)->local_got_entries;
	  if (!local_got_entries)
	    continue;

	  for (k = 0, n = cgc_tdata(j)->symtab_hdr.sh_info; k < n; ++k)
      ...
      alpha_cgc_tdata(i)->got->size = got_offset;
    }
}
...
static bfd_boolean
cgc64_alpha_size_got_sections (struct bfd_link_info *info)
{
  ...
  if (got_list == NULL)
      ...
      for (i = info->input_bfds; i ; i = i->link_next)
	{
	  ...
	  this_got = alpha_cgc_tdata (i)->gotobj;
	  if (this_got == NULL)
	    continue;
	  ...
	  BFD_ASSERT (this_got == i);
          ...
          if (alpha_cgc_tdata (this_got)->total_got_size > MAX_GOT_SIZE)
	    {
	      /* Yikes! A single object file has too many entries.  */
	      (*_bfd_error_handler)
	        (_("%B: .got subsegment exceeds 64K (size %d)"),
	         i, alpha_cgc_tdata (this_got)->total_got_size);
	      return FALSE;
	    }
	  ...
	  if (got_list == NULL)
	    ...
	    alpha_cgc_tdata(cur_got_obj)->got_link_next = this_got;
  ...
  i = alpha_cgc_tdata(cur_got_obj)->got_link_next;
  while (i != NULL)
    {
      if (cgc64_alpha_can_merge_gots (cur_got_obj, i))
	{
	  cgc64_alpha_merge_gots (cur_got_obj, i);

	  alpha_cgc_tdata(i)->got->size = 0;
	  i = alpha_cgc_tdata(i)->got_link_next;
	  alpha_cgc_tdata(cur_got_obj)->got_link_next = i;
	}
      else
	{
	  cur_got_obj = i;
  ...
  /* Once the gots have been merged, fill in the got offsets for
     everything therein.  */
  cgc64_alpha_calc_got_offsets (info);

  return TRUE;
}

static bfd_boolean
cgc64_alpha_size_plt_section_1 (struct alpha_cgc_link_hash_entry *h,
				void * data)
{
...
static void
cgc64_alpha_size_plt_section (struct bfd_link_info *info)
{
  ...
  alpha_cgc_link_hash_traverse (htab,
				cgc64_alpha_size_plt_section_1, splt);
  ...
  spltrel->size = entries * sizeof (Cgc64_External_Rela);
...
static bfd_boolean
cgc64_alpha_always_size_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *info)
{
  bfd *i;
  struct alpha_cgc_link_hash_table * htab;
  ...
  if (!cgc64_alpha_size_got_sections (info))
    return FALSE;

  /* Allocate space for all of the .got subsections.  */
  i = htab->got_list;
  for ( ; i ; i = alpha_cgc_tdata(i)->got_link_next)
    {
      asection *s = alpha_cgc_tdata(i)->got;
      if (s->size > 0)
	{
...
static bfd_boolean
cgc64_alpha_calc_dynrel_sizes (struct alpha_cgc_link_hash_entry *h,
			       struct bfd_link_info *info)
{
  bfd_boolean dynamic;
  struct alpha_cgc_reloc_entry *relent;
  unsigned long entries;

  /* If the symbol was defined as a common symbol in a regular object
     file, and there was no definition in any dynamic object, then the
     linker will have allocated space for the symbol in a common
     section but the CGC_LINK_HASH_DEF_REGULAR flag will not have been
     set.  This is done for dynamic symbols in
     cgc_adjust_dynamic_symbol but this is not done for non-dynamic
     symbols, somehow.  */
  ...
  dynamic = alpha_cgc_dynamic_symbol_p (&h->root, info);
  ...
  for (relent = h->reloc_entries; relent; relent = relent->next)
	...
	{
	  relent->srel->size +=
	    entries * sizeof (Cgc64_External_Rela) * relent->count;
	  if (relent->reltext)
...
/* Subroutine of cgc64_alpha_size_rela_got_section for doing the
   global symbols.  */

static bfd_boolean
cgc64_alpha_size_rela_got_1 (struct alpha_cgc_link_hash_entry *h,
			     struct bfd_link_info *info)
{
  bfd_boolean dynamic;
  struct alpha_cgc_got_entry *gotent;
  ...
  if (entries > 0)
    {
      ...
      srel->size += sizeof (Cgc64_External_Rela) * entries;
    }
...
static void
cgc64_alpha_size_rela_got_section (struct bfd_link_info *info)
{
  unsigned long entries;
  bfd *i, *dynobj;
  asection *srel;
  ...
  entries = 0;
  for (i = htab->got_list;
       i ; i = alpha_cgc_tdata(i)->got_link_next)
    {
      bfd *j;
  ...
  alpha_cgc_link_hash_traverse (htab,
				cgc64_alpha_size_rela_got_1, info);
}

/* Set the sizes of the dynamic sections.  */

static bfd_boolean
cgc64_alpha_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				   struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *s;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (info->executable)
	{
	 ...
	 size the dynamic relocation sections.  */
      alpha_cgc_link_hash_traverse (htab,
				    cgc64_alpha_calc_dynrel_sizes, info);

      cgc64_alpha_size_rela_got_section (info);
      cgc64_alpha_size_plt_section (info);
    }
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in cgc64_alpha_finish_dynamic_sections, but we
...

/* These functions do relaxation for Alpha CGC.
...
struct alpha_relax_info
{
  ...
  bfd_byte *contents;
  Cgc_Internal_Shdr *symtab_hdr;
  Cgc_Internal_Rela *relocs, *relend;
  struct bfd_link_info *link_info;
  bfd_vma gp;
  bfd *gotobj;
  asection *tsec;
  struct alpha_cgc_link_hash_entry *h;
  struct alpha_cgc_got_entry **first_gotent;
  struct alpha_cgc_got_entry *gotent;
...
cgc64_alpha_find_reloc_at_ofs (Cgc_Internal_Rela *rel,
  ...
  while (rel < relend)
    {
	  ...
	  && CGC64_R_TYPE (rel->r_info) == (unsigned int) type)
	return rel;
      ++rel;
    }
  return NULL;
...
static bfd_boolean
cgc64_alpha_relax_got_load (struct alpha_relax_info *info, bfd_vma symval,
			    Cgc_Internal_Rela *irel, unsigned long r_type)
{
  unsigned int insn;
  bfd_signed_vma disp;
  ...
  if (insn >> 26 != OP_LDQ)
    {
      reloc_howto_type *howto = cgc64_alpha_howto_table + r_type;
      ((*_bfd_error_handler)
  ...
  if (alpha_cgc_dynamic_symbol_p (&info->h->root, info->link_info))
    return TRUE;
  ...
  if (r_type == R_ALPHA_LITERAL)
  ...
  else
    {
      ...
      BFD_ASSERT (cgc_hash_table (info->link_info)->tls_sec != NULL);
  ...
  if (--info->gotent->use_count == 0)
    {
      ...
      alpha_cgc_tdata (info->gotobj)->total_got_size -= sz;
      if (!info->h)
	alpha_cgc_tdata (info->gotobj)->local_got_size -= sz;
    }

  /* Smash the existing GOT relocation for its 16-bit immediate pair.  */
  irel->r_info = CGC64_R_INFO (CGC64_R_SYM (irel->r_info), r_type);
...
static bfd_vma
cgc64_alpha_relax_opt_call (struct alpha_relax_info *info, bfd_vma symval)
{
  ...
  /* Otherwise, we may be able to identify a GP load in the first two
    ...
    {
      Cgc_Internal_Rela *tsec_relocs, *tsec_relend, *tsec_free, *gpdisp;
      bfd_vma ofs;
	...
	{
	  tsec_relocs = (_bfd_cgc_link_read_relocs
		         (info->abfd, info->tsec, NULL,
			 (Cgc_Internal_Rela *) NULL,
      ...
      gpdisp = (cgc64_alpha_find_reloc_at_ofs
  ...
  if (info->link_info->output_bfd->xvec != info->tsec->owner->xvec
      || info->gotobj != alpha_cgc_tdata (info->tsec->owner)->gotobj)
    return 0;
...
static bfd_boolean
cgc64_alpha_relax_with_lituse (struct alpha_relax_info *info,
			       bfd_vma symval, Cgc_Internal_Rela *irel)
{
  Cgc_Internal_Rela *urel, *irelend = info->relend;
  int flags, count, i;
  bfd_signed_vma disp;
  bfd_boolean fits16;
  bfd_boolean fits32;
  ...
  for (urel = irel+1, flags = count = 0; urel < irelend; ++urel, ++count)
    {
      if (CGC64_R_TYPE (urel->r_info) != R_ALPHA_LITUSE)
	break;
      if (urel->r_addend <= 6)
  ...
  for (urel = irel+1, i = 0; i < count; ++i, ++urel)
	...
	case LITUSE_ALPHA_BASE:
	  ...
	  if (fits16)
	    {
		 ...
		 register from the literal insn.  Leave the offset alone.  */
	      ...
	      urel->r_info = CGC64_R_INFO (CGC64_R_SYM (irel->r_info),
					   R_ALPHA_GPREL16);
	  ...
	  /* If all mem+byte, we can optimize 32-bit mem displacements.  */
	  else if (fits32 && !(flags & ~6))
	    {
	      ...
	      irel->r_info = CGC64_R_INFO (CGC64_R_SYM (irel->r_info),
					   R_ALPHA_GPRELHIGH);
	...
	case LITUSE_ALPHA_BYTOFF:
	  ...
	  urel->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
	  urel->r_addend = 0;
	  info->changed_relocs = TRUE;
	...
	case LITUSE_ALPHA_JSRDIRECT:
	  {
	    ...
	    optdest = cgc64_alpha_relax_opt_call (info, symval);
		...
		xrel = (cgc64_alpha_find_reloc_at_ofs
			(info->relocs, info->relend, urel->r_offset,
			 R_ALPHA_HINT));
		if (xrel)
		  xrel->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
	    ...
	    if (optdest)
	      {
		Cgc_Internal_Rela *gpdisp
		  = (cgc64_alpha_find_reloc_at_ofs
		...
		if (gpdisp)
		  {
			...
			gpdisp->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
			info->changed_contents = TRUE;
			info->changed_relocs = TRUE;
		      }
		  }
  ...
  if (all_optimized)
      ...
      if (--info->gotent->use_count == 0)
	{
	 ...
	 so just nop it out.  */
      if (!lit_reused)
	{
	  irel->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
	  info->changed_relocs = TRUE;
    ...
    return cgc64_alpha_relax_got_load (info, symval, irel, R_ALPHA_LITERAL);
}

static bfd_boolean
cgc64_alpha_relax_tls_get_addr (struct alpha_relax_info *info, bfd_vma symval,
				Cgc_Internal_Rela *irel, bfd_boolean is_gd)
{
  bfd_byte *pos[5];
  unsigned int insn, tlsgd_reg;
  Cgc_Internal_Rela *gpdisp, *hint;
  bfd_boolean dynamic, use_gottprel;
  unsigned long new_symndx;

  dynamic = alpha_cgc_dynamic_symbol_p (&info->h->root, info->link_info);

  /* If a TLS symbol is accessed using IE at least once, there is no point
     to use dynamic model for it.  */
  if (is_gd && info->h && (info->h->flags & ALPHA_CGC_LINK_HASH_TLS_IE))
    ;
  ...
  if (irel + 2 >= info->relend)
    return TRUE;
  if (CGC64_R_TYPE (irel[1].r_info) != R_ALPHA_LITERAL
      || CGC64_R_TYPE (irel[2].r_info) != R_ALPHA_LITUSE
  ...
  /* There must be a GPDISP relocation positioned immediately after the
     LITUSE relocation.  */
  gpdisp = cgc64_alpha_find_reloc_at_ofs (info->relocs, info->relend,
  ...
  {
    struct alpha_cgc_got_entry *lit_gotent;
    struct alpha_cgc_link_hash_entry *lit_h;
    unsigned long indx;

    BFD_ASSERT (CGC64_R_SYM (irel[1].r_info) >= info->symtab_hdr->sh_info);
    indx = CGC64_R_SYM (irel[1].r_info) - info->symtab_hdr->sh_info;
    lit_h = alpha_cgc_sym_hashes (info->abfd)[indx];

    while (lit_h->root.root.type == bfd_link_hash_indirect
	   || lit_h->root.root.type == bfd_link_hash_warning)
      lit_h = (struct alpha_cgc_link_hash_entry *) lit_h->root.root.u.i.link;
  ...
  use_gottprel = FALSE;
  new_symndx = is_gd ? CGC64_R_SYM (irel->r_info) : STN_UNDEF;
  ...
  switch ((int) (!dynamic && !info->link_info->shared))
	...
	if (disp >= -0x8000 && disp < 0x8000)
	  {
	    ...
	    irel[0].r_info = CGC64_R_INFO (new_symndx, R_ALPHA_TPREL16);
	    irel[1].r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
	    break;
	  }
		 ...
		 && pos[0] + 4 == pos[1])
	  {
	    ...
	    irel[0].r_offset = pos[0] - info->contents;
	    irel[0].r_info = CGC64_R_INFO (new_symndx, R_ALPHA_TPRELHI);
	    irel[1].r_offset = pos[1] - info->contents;
	    irel[1].r_info = CGC64_R_INFO (new_symndx, R_ALPHA_TPRELLO);
	    break;
	  }
      }
      /* FALLTHRU */
    ...
    default:
      ...
      irel[0].r_offset = pos[0] - info->contents;
      irel[0].r_info = CGC64_R_INFO (new_symndx, R_ALPHA_GOTTPREL);
      irel[1].r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
      break;
    }
  ...
  irel[2].r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
  gpdisp->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);

  hint = cgc64_alpha_find_reloc_at_ofs (info->relocs, info->relend,
					irel[2].r_offset, R_ALPHA_HINT);
  if (hint)
    hint->r_info = CGC64_R_INFO (0, R_ALPHA_NONE);
  ...
  if (use_gottprel)
    {
      struct alpha_cgc_got_entry *tprel_gotent;
      ...
      else
	{
	  if (info->gotent->use_count == 0)
	    ...
	    {
	      tprel_gotent = (struct alpha_cgc_got_entry *)
		bfd_alloc (info->abfd, sizeof (struct alpha_cgc_got_entry));
	      if (!tprel_gotent)
		return FALSE;
...
static bfd_boolean
cgc64_alpha_relax_section (bfd *abfd, asection *sec,
			   struct bfd_link_info *link_info, bfd_boolean *again)
{
  ...
  htab = alpha_cgc_hash_table (link_info);
  if (htab == NULL)
    return FALSE;
  ...
  *again = FALSE;
  ...
  if (htab->relax_trip != link_info->relax_trip)
    {
	 ...
	 error is GOT overflow, and relaxation only shrinks the table.  */
      if (!cgc64_alpha_size_got_sections (link_info))
	abort ();
      if (cgc_hash_table (link_info)->dynamic_sections_created)
	{
	  cgc64_alpha_size_plt_section (link_info);
	  cgc64_alpha_size_rela_got_section (link_info);
	}
    }
  ...
  info.gotobj = alpha_cgc_tdata (abfd)->gotobj;
  if (info.gotobj)
    {
      asection *sgot = alpha_cgc_tdata (info.gotobj)->got;
  ...
  if (cgc_section_data (sec)->this_hdr.contents != NULL)
    info.contents = cgc_section_data (sec)->this_hdr.contents;
  else
    {
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      bfd_vma symval;
      struct alpha_cgc_got_entry *gotent;
      unsigned long r_type = CGC64_R_TYPE (irel->r_info);
      unsigned long r_symndx = CGC64_R_SYM (irel->r_info);
	  ...
	  if (r_type == R_ALPHA_TLSLDM)
	  ...
	  else
	    {
	      ...
	      if (isym->st_shndx == SHN_UNDEF)
	        ...
	        info.tsec = bfd_section_from_cgc_index (abfd, isym->st_shndx);
	    }
      ...
      else
	{
	  ...
	  h = alpha_cgc_sym_hashes (abfd)[indx];
	  BFD_ASSERT (h != NULL);
	...
	case R_ALPHA_LITERAL:
	  ...
	  if (irel+1 < irelend
	      && CGC64_R_TYPE (irel[1].r_info) == R_ALPHA_LITUSE)
	    {
	      if (!cgc64_alpha_relax_with_lituse (&info, symval, irel))
		goto error_return;
	    }
	  else
	    {
	      if (!cgc64_alpha_relax_got_load (&info, symval, irel, r_type))
		goto error_return;
	    }
	  break;
	...
	case R_ALPHA_TLSLDM:
	  BFD_ASSERT(info.gotent != NULL);
	  if (!cgc64_alpha_relax_tls_get_addr (&info, symval, irel,
  ...
  if (info.contents != NULL
      && cgc_section_data (sec)->this_hdr.contents != info.contents)
    {
...
static void
cgc64_alpha_emit_dynrel (bfd *abfd, struct bfd_link_info *info,
			 ...
			 long dynindx, long rtype, bfd_vma addend)
{
  ...
  outrel.r_info = CGC64_R_INFO (dynindx, rtype);
  outrel.r_addend = addend;

  offset = _bfd_cgc_section_offset (abfd, info, sec, offset);
  ...
  loc = srel->contents;
  loc += srel->reloc_count++ * sizeof (Cgc64_External_Rela);
  bfd_cgc64_swap_reloca_out (abfd, &outrel, loc);
  BFD_ASSERT (sizeof (Cgc64_External_Rela) * srel->reloc_count <= srel->size);
}

/* Relocate an Alpha CGC section for a relocatable link.
...
static bfd_boolean
cgc64_alpha_relocate_section_r (bfd *output_bfd ATTRIBUTE_UNUSED,
				struct bfd_link_info *info ATTRIBUTE_UNUSED,
				...
				asection **local_sections)
{
  ...
  symtab_hdr_sh_info = cgc_symtab_hdr (input_bfd).sh_info;
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	RELOC_AGAINST_DISCARDED_SECTION (info, input_bfd, input_section,
					 rel, 1, relend,
					 cgc64_alpha_howto_table + r_type, 0,
					 contents);
...
/* Relocate an Alpha CGC section.  */

static bfd_boolean
cgc64_alpha_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
			      bfd *input_bfd, asection *input_section,
			      ...
			      asection **local_sections)
{
  ...
  BFD_ASSERT (is_alpha_cgc (input_bfd));

  /* Handle relocatable links with a smaller loop.  */
  if (info->relocatable)
    return cgc64_alpha_relocate_section_r (output_bfd, info, input_bfd,
  ...
  if (input_section->flags & SEC_ALLOC)
    {
      ...
      section_name = (bfd_cgc_string_from_cgc_section
		      (input_bfd, cgc_cgcheader(input_bfd)->e_shstrndx,
		       _bfd_cgc_single_rel_hdr (input_section)->sh_name));
  ...
  gotobj = alpha_cgc_tdata (input_bfd)->gotobj;
  if (gotobj)
    {
      sgot = alpha_cgc_tdata (gotobj)->got;
  ...
  local_got_entries = alpha_cgc_tdata(input_bfd)->local_got_entries;
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_type >= R_ALPHA_max)
	{
      ...
      howto = cgc64_alpha_howto_table + r_type;
      r_symndx = CGC64_R_SYM(rel->r_info);
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  sec = local_sections[r_symndx];
	  msec = sec;
	  value = _bfd_cgc_rela_local_sym (output_bfd, sym, &msec, rel);
      ...
      else
	{
	  ...
	  h = (struct alpha_cgc_link_hash_entry *) hh;
          dynamic_symbol_p = alpha_cgc_dynamic_symbol_p (&h->root, info);
	...
	case R_ALPHA_GPDISP:
	  {
	    ...
	    r = cgc64_alpha_do_reloc_gpdisp (input_bfd, gp - value,
					     p_ldah, p_lda);
	  }
	  break;
	...
	case R_ALPHA_LITERAL:
	  ...
	  if (!gotent->reloc_done)
		...
		cgc64_alpha_emit_dynrel (output_bfd, info, sgot, srelgot,
					 gotent->got_offset, 0,
	...
	case R_ALPHA_BRSGP:
	  {
	    ...
	    if (h != NULL && sec != NULL
		&& alpha_cgc_tdata (sec->owner)->gotobj
		&& gotobj != alpha_cgc_tdata (sec->owner)->gotobj)
	      {
		(*_bfd_error_handler)
	...
	case R_ALPHA_TPREL64:
	  {
	    ...
	    if (input_section->flags & SEC_ALLOC)
	      cgc64_alpha_emit_dynrel (output_bfd, info, input_section,
...
static bfd_boolean
cgc64_alpha_finish_dynamic_symbol (bfd *output_bfd, struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h,
				   Cgc_Internal_Sym *sym)
{
  struct alpha_cgc_link_hash_entry *ah = (struct alpha_cgc_link_hash_entry *)h;
  bfd *dynobj = cgc_hash_table(info)->dynobj;
  ...
  if (h->needs_plt)
	...
	if (gotent->reloc_type == R_ALPHA_LITERAL
	    && gotent->use_count > 0)
	  {
	    ...
	    sgot = alpha_cgc_tdata (gotent->gotobj)->got;
	    BFD_ASSERT (sgot != NULL);
	    ...
	    outrel.r_info = CGC64_R_INFO(h->dynindx, R_ALPHA_JMP_SLOT);
	    outrel.r_addend = 0;

	    loc = srel->contents + plt_index * sizeof (Cgc64_External_Rela);
  ...
  else if (alpha_cgc_dynamic_symbol_p (h, info))
    {
      ...
      for (gotent = ((struct alpha_cgc_link_hash_entry *) h)->got_entries;
	   gotent != NULL;
	   gotent = gotent->next)
	{
	  asection *sgot;
	  ...
	  cgc64_alpha_emit_dynrel (output_bfd, info, sgot, srel,
...
static bfd_boolean
cgc64_alpha_finish_dynamic_sections (bfd *output_bfd,
				     struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  switch (dyn.d_tag)
	    ...
	    case DT_PLTGOT:
	      dyn.d_un.d_ptr
		= cgc64_alpha_use_secureplt ? gotplt_vma : plt_vma;
	      break;
	    case DT_PLTRELSZ:
	    ...
	    case DT_RELASZ:
	      /* My interpretation of the TIS v1.1 CGC document indicates
      ...
      if (splt->size > 0)
	{
	  ...
	  cgc_section_data (splt->output_section)->this_hdr.sh_entsize = 0;
	}
    }
...
static bfd_boolean
cgc64_alpha_final_link (bfd *abfd, struct bfd_link_info *info)
{
  asection *o;
      ...
      if (strcmp (o->name, ".mdebug") == 0)
	{
	  ...
	  for (p = o->map_head.link_order;
	      ...
	      if (p->type != bfd_indirect_link_order)
		{
	      ...
	      if (! is_alpha_cgc (input_bfd))
		/* I don't know what a non ALPHA CGC bfd would be
		...
		continue;
		 ...
		 ecoff_debug_info structure, so we do that now.  */
	      if (!cgc64_alpha_read_ecoff_info (input_bfd, input_section,
						&input_debug))
		return FALSE;
		   ...
		   eraw_src += input_swap->external_ext_size)
		{
		  ...
		  h = alpha_cgc_link_hash_lookup (htab, name, FALSE, FALSE, TRUE);
  ...
  /* The .got subsections...  */
  {
    bfd *i, *dynobj = cgc_hash_table(info)->dynobj;
    for (i = htab->got_list;
	 i != NULL;
	 i = alpha_cgc_tdata(i)->got_link_next)
      {
	...
	/* cgc_bfd_final_link already did everything in dynobj.  */
	if (i == dynobj)
	  continue;

	sgot = alpha_cgc_tdata(i)->got;
...
cgc64_alpha_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...

static const struct bfd_cgc_special_section cgc64_alpha_special_sections[] =
{
...
cgc64_alpha_ecoff_debug_swap =
{
  ...
  cgc64_alpha_read_ecoff_info
};

/* Use a non-standard hash bucket size of 8.  */

static const struct cgc_size_info alpha_cgc_size_info =
{
...
#define TARGET_LITTLE_SYM	bfd_cgc64_alpha_vec
#define TARGET_LITTLE_NAME	"cgc64-alpha"
#define CGC_ARCH		bfd_arch_alpha
#define CGC_TARGET_ID		ALPHA_CGC_DATA
#define CGC_MACHINE_CODE	EM_ALPHA
#define CGC_MAXPAGESIZE	0x10000
#define CGC_COMMONPAGESIZE	0x2000

#define bfd_cgc64_bfd_link_hash_table_create \
  cgc64_alpha_bfd_link_hash_table_create

#define bfd_cgc64_bfd_reloc_type_lookup \
  cgc64_alpha_bfd_reloc_type_lookup
#define bfd_cgc64_bfd_reloc_name_lookup \
  cgc64_alpha_bfd_reloc_name_lookup
#define cgc_info_to_howto \
  cgc64_alpha_info_to_howto

#define bfd_cgc64_mkobject \
  cgc64_alpha_mkobject
#define cgc_backend_object_p \
  cgc64_alpha_object_p

#define cgc_backend_section_from_shdr \
  cgc64_alpha_section_from_shdr
#define cgc_backend_section_flags \
  cgc64_alpha_section_flags
#define cgc_backend_fake_sections \
  cgc64_alpha_fake_sections

#define bfd_cgc64_bfd_is_local_label_name \
  cgc64_alpha_is_local_label_name
#define bfd_cgc64_find_nearest_line \
  cgc64_alpha_find_nearest_line
#define bfd_cgc64_bfd_relax_section \
  cgc64_alpha_relax_section

#define cgc_backend_add_symbol_hook \
  cgc64_alpha_add_symbol_hook
#define cgc_backend_relocs_compatible \
  _bfd_cgc_relocs_compatible
#define cgc_backend_check_relocs \
  cgc64_alpha_check_relocs
#define cgc_backend_create_dynamic_sections \
  cgc64_alpha_create_dynamic_sections
#define cgc_backend_adjust_dynamic_symbol \
  cgc64_alpha_adjust_dynamic_symbol
#define cgc_backend_merge_symbol_attribute \
  cgc64_alpha_merge_symbol_attribute
#define cgc_backend_copy_indirect_symbol \
  cgc64_alpha_copy_indirect_symbol
#define cgc_backend_always_size_sections \
  cgc64_alpha_always_size_sections
#define cgc_backend_size_dynamic_sections \
  cgc64_alpha_size_dynamic_sections
#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_relocate_section \
  cgc64_alpha_relocate_section
#define cgc_backend_finish_dynamic_symbol \
  cgc64_alpha_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
  cgc64_alpha_finish_dynamic_sections
#define bfd_cgc64_bfd_final_link \
  cgc64_alpha_final_link
#define cgc_backend_reloc_type_class \
  cgc64_alpha_reloc_type_class

#define cgc_backend_can_gc_sections	1
#define cgc_backend_gc_mark_hook	cgc64_alpha_gc_mark_hook
#define cgc_backend_gc_sweep_hook	cgc64_alpha_gc_sweep_hook

#define cgc_backend_ecoff_debug_swap \
  &cgc64_alpha_ecoff_debug_swap
...
#define TARGET_LITTLE_SYM	bfd_cgc64_alpha_freebsd_vec
#undef TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME	"cgc64-alpha-freebsd"
#undef	CGC_OSABI
...
static void
cgc64_alpha_fbsd_post_process_headers (bfd * abfd,
...
#define cgc_backend_post_process_headers \
  cgc64_alpha_fbsd_post_process_headers

#undef  cgc64_bed
#define cgc64_bed cgc64_alpha_fbsd_bed
