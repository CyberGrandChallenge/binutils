/* Renesas / SuperH SH specific support for 32-bit CGC
...
static bfd_reloc_status_type sh_cgc_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);
static bfd_reloc_status_type sh_cgc_ignore_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);
static bfd_boolean sh_cgc_relax_delete_bytes
  (bfd *, asection *, bfd_vma, int);
static bfd_boolean sh_cgc_align_loads
  (bfd *, asection *, Cgc_Internal_Rela *, bfd_byte *, bfd_boolean *);
#ifndef SH64_CGC
static bfd_boolean sh_cgc_swap_insns
  (bfd *, asection *, void *, bfd_byte *, bfd_vma);
#endif
static int sh_cgc_optimized_tls_reloc
...
#define SH_CGC_RELOC sh_cgc_reloc
static reloc_howto_type sh_cgc_howto_table[] =
{
#include "cgc32-sh-relocs.h"
};

#define SH_PARTIAL32 FALSE
#define SH_SRC_MASK32 0
#define SH_CGC_RELOC bfd_cgc_generic_reloc
...
vxworks_object_p (bfd *abfd ATTRIBUTE_UNUSED)
{
  ...
  extern const bfd_target bfd_cgc32_shlvxworks_vec;
  extern const bfd_target bfd_cgc32_shvxworks_vec;

  return (abfd->xvec == &bfd_cgc32_shlvxworks_vec
	  || abfd->xvec == &bfd_cgc32_shvxworks_vec);
#else
  return FALSE;
#endif
}
...
{
#if !defined INCLUDE_SHMEDIA && !defined SH_TARGET_ALREADY_DEFINED
  extern const bfd_target bfd_cgc32_shfd_vec;
  extern const bfd_target bfd_cgc32_shbfd_vec;

  return (abfd->xvec == &bfd_cgc32_shfd_vec
	  || abfd->xvec == &bfd_cgc32_shbfd_vec);
#else
  return FALSE;
#endif
}
...
get_howto_table (bfd *abfd)
{
  ...
  return sh_cgc_howto_table;
}

static bfd_reloc_status_type
sh_cgc_reloc_loop (int r_type ATTRIBUTE_UNUSED, bfd *input_bfd,
		   ...
		   bfd_vma start, bfd_vma end)
{
  ...
  if (symbol_section != input_section)
    {
      if (cgc_section_data (symbol_section)->this_hdr.contents != NULL)
	contents = cgc_section_data (symbol_section)->this_hdr.contents;
      else
	{
...
#define IS_PPI(PTR) ((bfd_get_16 (input_bfd, (PTR)) & 0xfc00) == 0xf800)
  ...
  if (contents != NULL
      && cgc_section_data (symbol_section)->this_hdr.contents != contents)
    free (contents);
...
/* This function is used for normal relocs.  This used to be like the COFF
   function, and is almost certainly incorrect for other CGC targets.  */

static bfd_reloc_status_type
sh_cgc_reloc (bfd *abfd, arelent *reloc_entry, asymbol *symbol_in,
	      void *data, asection *input_section, bfd *output_bfd,
	      char **error_message ATTRIBUTE_UNUSED)
{
  unsigned long insn;
  bfd_vma sym_value;
  enum cgc_sh_reloc_type r_type;
  bfd_vma addr = reloc_entry->address;
  bfd_byte *hit_data = addr + (bfd_byte *) data;

  r_type = (enum cgc_sh_reloc_type) reloc_entry->howto->type;
...
static bfd_reloc_status_type
sh_cgc_ignore_reloc (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc_entry,
...
/* This structure is used to map BFD reloc codes to SH CGC relocs.  */
...
/* An array mapping BFD reloc codes to SH CGC relocs.  */

static const struct cgc_reloc_map sh_reloc_map[] =
{
...
/* Given a BFD reloc code, return the howto structure for the
   corresponding SH CGC reloc.  */

static reloc_howto_type *
sh_cgc_reloc_type_lookup (bfd *abfd, bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < sizeof (sh_reloc_map) / sizeof (struct cgc_reloc_map); i++)
    {
      if (sh_reloc_map[i].bfd_reloc_val == code)
	return get_howto_table (abfd) + (int) sh_reloc_map[i].cgc_reloc_val;
    }
...
static reloc_howto_type *
sh_cgc_reloc_name_lookup (bfd *abfd, const char *r_name)
{
  unsigned int i;
  ...
  if (vxworks_object_p (abfd))
    ...
    {
      for (i = 0;
	   i < (sizeof (sh_cgc_howto_table)
		/ sizeof (sh_cgc_howto_table[0]));
	   i++)
	if (sh_cgc_howto_table[i].name != NULL
	    && strcasecmp (sh_cgc_howto_table[i].name, r_name) == 0)
	  return &sh_cgc_howto_table[i];
    }

  return NULL;
}

/* Given an CGC reloc, fill in the howto field of a relent.  */

static void
sh_cgc_info_to_howto (bfd *abfd, arelent *cache_ptr, Cgc_Internal_Rela *dst)
{
  unsigned int r;

  r = CGC32_R_TYPE (dst->r_info);
...

/* This function handles relaxing for SH CGC.  See the corresponding
   ...
   they come from enum cgc_sh_reloc_type in include/cgc/sh.h.  */

static bfd_boolean
sh_cgc_relax_section (bfd *abfd, asection *sec,
		      struct bfd_link_info *link_info, bfd_boolean *again)
{
  ...
  *again = FALSE;
...
#ifdef INCLUDE_SHMEDIA
  if (cgc_section_data (sec)->this_hdr.sh_flags
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      unsigned short insn;
      Cgc_Internal_Rela *ircgcn, *irelscan, *irelcount;
      bfd_signed_vma foff;

      if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_CODE)
	have_code = TRUE;

      if (CGC32_R_TYPE (irel->r_info) != (int) R_SH_USES)
	continue;
      ...
      if (paddr >= sec->size)
	{
	 ...
	 actually being called.  */
      for (ircgcn = internal_relocs; ircgcn < irelend; ircgcn++)
	if (ircgcn->r_offset == paddr
	    && CGC32_R_TYPE (ircgcn->r_info) == (int) R_SH_DIR32)
	  break;
      if (ircgcn >= irelend)
	{
	  ((*_bfd_error_handler)
      ...
      if (isymbuf == NULL && symtab_hdr->sh_info != 0)
	{
      ...
      if (CGC32_R_SYM (ircgcn->r_info) < symtab_hdr->sh_info)
	{
	  /* A local symbol.  */
	  Cgc_Internal_Sym *isym;

	  isym = isymbuf + CGC32_R_SYM (ircgcn->r_info);
	  if (isym->st_shndx
	      != (unsigned int) _bfd_cgc_section_from_bfd_section (abfd, sec))
	    {
	      ((*_bfd_error_handler)
      ...
      else
	{
	  ...
	  indx = CGC32_R_SYM (ircgcn->r_info) - symtab_hdr->sh_info;
      ...
      if (foff < -0x1000 || foff >= 0x1000 - 8)
	{
	 ...
	 replace the jsr with a bsr.  */
      irel->r_info = CGC32_R_INFO (CGC32_R_SYM (ircgcn->r_info), R_SH_IND12W);
      /* We used to test (CGC32_R_SYM (ircgcn->r_info) < symtab_hdr->sh_info)
	...
	if (CGC32_R_TYPE (irelscan->r_info) == (int) R_SH_USES
	...
	if (irelcount->r_offset == paddr
	    && CGC32_R_TYPE (irelcount->r_info) == (int) R_SH_COUNT)
	  break;

      /* Delete the register load.  */
      if (! sh_cgc_relax_delete_bytes (abfd, sec, laddr, 2))
	goto error_return;
      ...
      if (irelcount->r_addend == 0)
	{
	 ...
	 the address from ircgcn, in case it was changed by the
	 previous call to sh_cgc_relax_delete_bytes.  */
      if (irelcount->r_addend == 0)
	{
	  if (! sh_cgc_relax_delete_bytes (abfd, sec, ircgcn->r_offset, 4))
	    goto error_return;
	}
  ...
  /* Look for load and store instructions that we can align on four
     byte boundaries.  */
  if ((cgc_cgcheader (abfd)->e_flags & EF_SH_MACH_MASK) != EF_SH4
      && have_code)
    {
      bfd_boolean swapped;
      ...
      if (contents == NULL)
	{
      ...
      if (! sh_cgc_align_loads (abfd, sec, internal_relocs, contents,
				&swapped))
	goto error_return;
...
static bfd_boolean
sh_cgc_relax_delete_bytes (bfd *abfd, asection *sec, bfd_vma addr,
			   int count)
{
  Cgc_Internal_Shdr *symtab_hdr;
  ...
  bfd_byte *contents;
  Cgc_Internal_Rela *irel, *irelend;
  Cgc_Internal_Rela *irelalign;
  bfd_vma toaddr;
  Cgc_Internal_Sym *isymbuf, *isym, *isymend;
  ...
  for (; irel < irelend; irel++)
    {
      if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_ALIGN
	  && irel->r_offset > addr
...
#define NOP_OPCODE (0x0009)
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
    {
	  ...
	  || (CGC32_R_TYPE (irel->r_info) == (int) R_SH_ALIGN
	      && irel->r_offset == toaddr))
	nraddr -= count;
	 ...
	 represent addresses, though.  */
	  ...
	  && CGC32_R_TYPE (irel->r_info) != (int) R_SH_ALIGN
	  && CGC32_R_TYPE (irel->r_info) != (int) R_SH_CODE
	  && CGC32_R_TYPE (irel->r_info) != (int) R_SH_DATA
	  && CGC32_R_TYPE (irel->r_info) != (int) R_SH_LABEL)
	irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info),
				     (int) R_SH_NONE);

      /* If this is a PC relative reloc, see if the range it covers
	 includes the bytes we have deleted.  */
      switch ((enum cgc_sh_reloc_type) CGC32_R_TYPE (irel->r_info))
	{
	default:
	  break;
	...
	case R_SH_SWITCH32:
	  /* These relocs types represent
	     ...
	     N.B. in gas / coff bfd, the cgc bfd r_addend is called r_offset,
	     and the cgc bfd r_offset is called r_vaddr.  */
	  ...
	  if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_SWITCH16)
	    voff = bfd_get_signed_16 (abfd, contents + nraddr);
	  else if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_SWITCH8)
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
    {
      Cgc_Internal_Rela *internal_relocs;
      Cgc_Internal_Rela *irelscan, *irelscanend;
      bfd_byte *ocontents;
	...
	continue;
	 ...
	 leave sh_coff_relax_section.  */
      internal_relocs = (_bfd_cgc_link_read_relocs
			 (abfd, o, NULL, (Cgc_Internal_Rela *) NULL, TRUE));
      if (internal_relocs == NULL)
	return FALSE;
      ...
      for (irelscan = internal_relocs; irelscan < irelscanend; irelscan++)
	{
	  ...
	  if (CGC32_R_TYPE (irelscan->r_info) == (int) R_SH_SWITCH32)
	    {
	      bfd_vma start, stop;
	      bfd_signed_vma voff;
	      ...
	      if (ocontents == NULL)
		{
		  if (cgc_section_data (o)->this_hdr.contents != NULL)
		    ocontents = cgc_section_data (o)->this_hdr.contents;
		  else
		    {
		      ...
		      if (!bfd_malloc_and_get_section (abfd, o, &ocontents))
			{
		      ...
		      cgc_section_data (o)->this_hdr.contents = ocontents;
		    }
		}
	  ...
	  if (CGC32_R_TYPE (irelscan->r_info) != (int) R_SH_DIR32)
	    continue;

	  if (CGC32_R_SYM (irelscan->r_info) >= symtab_hdr->sh_info)
	    continue;


	  isym = isymbuf + CGC32_R_SYM (irelscan->r_info);
	  if (isym->st_shndx == sec_shndx
	      && (isym->st_value <= addr
		  || isym->st_value >= toaddr))
	    {
  ...
  if (irelalign != NULL)
	...
	{
	  /* Tail recursion.  */
	  return sh_cgc_relax_delete_bytes (abfd, sec, alignaddr,
...
static bfd_boolean
sh_cgc_align_loads (bfd *abfd ATTRIBUTE_UNUSED, asection *sec,
		    Cgc_Internal_Rela *internal_relocs,
		    ...
		    bfd_boolean *pswapped)
{
  ...
  *pswapped = FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_LABEL)
	{
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	if (CGC32_R_TYPE (irel->r_info) == (int) R_SH_DATA)
	  break;
      if (irel < irelend)
	stop = irel->r_offset;
      else
	stop = sec->size;
      ...
      if (! _bfd_sh_align_load_span (abfd, sec, contents, sh_cgc_swap_insns,
...
static bfd_boolean
sh_cgc_swap_insns (bfd *abfd, asection *sec, void *relocs,
		   bfd_byte *contents, bfd_vma addr)
{
  Cgc_Internal_Rela *internal_relocs = (Cgc_Internal_Rela *) relocs;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      enum cgc_sh_reloc_type type;
      int add;

      /* There are a few special types of relocs that we don't want to
	 adjust.  These relocs do not apply to the instruction itself,
	 but are only associated with the address.  */
      type = (enum cgc_sh_reloc_type) CGC32_R_TYPE (irel->r_info);
      if (type == R_SH_ALIGN
	  || type == R_SH_CODE
	  || type == R_SH_DATA
	  || type == R_SH_LABEL)
...
#endif /* defined SH64_CGC */

/* Describes one of the various PLT styles.  */

struct cgc_sh_plt_info
{
  ...
  /* A different PLT layout which can be used for the first
     ...
     other cases.  */
  const struct cgc_sh_plt_info *short_plt;
};
...
#define CGC_PLT_ENTRY_SIZE 64

/* First entry in an absolute procedure linkage table look like this.  */

static const bfd_byte cgc_sh_plt0_entry_be[CGC_PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh_plt0_entry_le[CGC_PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh_plt_entry_be[CGC_PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh_plt_entry_le[CGC_PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh_pic_plt_entry_be[CGC_PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh_pic_plt_entry_le[CGC_PLT_ENTRY_SIZE] =
{
...
static const struct cgc_sh_plt_info cgc_sh_plts[2][2] = {
  {
    {
      /* Big-endian non-PIC.  */
      cgc_sh_plt0_entry_be,
      CGC_PLT_ENTRY_SIZE,
...
#define cgc_sh_plt0_gotplt_offset(info) 0
...
static const struct cgc_sh_plt_info *
get_plt_info (bfd *abfd ATTRIBUTE_UNUSED, bfd_boolean pic_p)
{
  return &cgc_sh_plts[pic_p][!bfd_big_endian (abfd)];
}
#else
/* The size in bytes of an entry in the procedure linkage table.  */

#define CGC_PLT_ENTRY_SIZE 28
...
static const struct cgc_sh_plt_info vxworks_sh_plts[2][2] = {
  {
    {
      /* Big-endian non-PIC.  */
...
static const struct cgc_sh_plt_info fdpic_sh_plts[2] = {
  {
    /* Big-endian PIC.  */
    NULL,
    0,
...
static const struct cgc_sh_plt_info fdpic_sh2a_short_plt_be = {
...
static const struct cgc_sh_plt_info fdpic_sh2a_short_plt_le = {
...
static const struct cgc_sh_plt_info fdpic_sh2a_plts[2] = {
  {
    /* Big-endian PIC.  */
    NULL,
    0,
...
static bfd_vma
get_plt_index (const struct cgc_sh_plt_info *info, bfd_vma offset)
{
  bfd_vma plt_index = 0;
...
static bfd_vma
get_plt_offset (const struct cgc_sh_plt_info *info, bfd_vma plt_index)
{
  bfd_vma offset = 0;
...
struct cgc_sh_dyn_relocs
{
  struct cgc_sh_dyn_relocs *next;
...
/* sh CGC linker hash entry.  */

struct cgc_sh_link_hash_entry
{
  ...
  struct cgc_sh_dyn_relocs *dyn_relocs;
...
#define sh_cgc_hash_entry(ent) ((struct cgc_sh_link_hash_entry *)(ent))

struct sh_cgc_obj_tdata
{
...
#define sh_cgc_tdata(abfd) \
  ((struct sh_cgc_obj_tdata *) (abfd)->tdata.any)

#define sh_cgc_local_got_type(abfd) \
  (sh_cgc_tdata (abfd)->local_got_type)

#define sh_cgc_local_funcdesc(abfd) \
  (sh_cgc_tdata (abfd)->local_funcdesc)

#define is_sh_cgc(bfd) \
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour \
   && cgc_tdata (bfd) != NULL \
   && cgc_object_id (bfd) == SH_CGC_DATA)

/* Override the generic function because we need to store sh_cgc_obj_tdata
   as the specific tdata.  */

static bfd_boolean
sh_cgc_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct sh_cgc_obj_tdata),
				  SH_CGC_DATA);
}

/* sh CGC linker hash table.  */

struct cgc_sh_link_hash_table
{
  ...
  const struct cgc_sh_plt_info *plt_info;
...
/* Traverse an sh CGC linker hash table.  */

#define sh_cgc_link_hash_traverse(table, func, info)			\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func), \
    (info)))

/* Get the sh CGC linker hash table from a link_info structure.  */

#define sh_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == SH_CGC_DATA ? ((struct cgc_sh_link_hash_table *) ((p)->hash)) : NULL)

/* Create an entry in an sh CGC linker hash table.  */

static struct bfd_hash_entry *
sh_cgc_link_hash_newfunc (struct bfd_hash_entry *entry,
			  struct bfd_hash_table *table,
			  const char *string)
{
  struct cgc_sh_link_hash_entry *ret =
    (struct cgc_sh_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == (struct cgc_sh_link_hash_entry *) NULL)
    ret = ((struct cgc_sh_link_hash_entry *)
	   bfd_hash_allocate (table,
			      sizeof (struct cgc_sh_link_hash_entry)));
  if (ret == (struct cgc_sh_link_hash_entry *) NULL)
...
/* Create an sh CGC linker hash table.  */

static struct bfd_link_hash_table *
sh_cgc_link_hash_table_create (bfd *abfd)
{
  struct cgc_sh_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc_sh_link_hash_table);

  ret = (struct cgc_sh_link_hash_table *) bfd_zmalloc (amt);
  if (ret == (struct cgc_sh_link_hash_table *) NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      sh_cgc_link_hash_newfunc,
				      sizeof (struct cgc_sh_link_hash_entry),
				      SH_CGC_DATA))
    {
      free (ret);
...
static bfd_boolean
sh_cgc_omit_section_dynsym (bfd *output_bfd ATTRIBUTE_UNUSED,
			    struct bfd_link_info *info, asection *p)
{
  struct cgc_sh_link_hash_table *htab = sh_cgc_hash_table (info);
...
{
  struct cgc_sh_link_hash_table *htab;

  if (! _bfd_cgc_create_got_section (dynobj, info))
    return FALSE;

  htab = sh_cgc_hash_table (info);
  if (htab == NULL)
    return FALSE;
  ...
  htab->srcgcuncdesc = bfd_make_section_anyway_with_flags (dynobj,
							   ".rela.got.funcdesc",
							   (SEC_ALLOC | SEC_LOAD
							    | SEC_HAS_CONTENTS
							    | SEC_IN_MEMORY
							    | SEC_LINKER_CREATED
							    | SEC_READONLY));
  if (htab->srcgcuncdesc == NULL
      || ! bfd_set_section_alignment (dynobj, htab->srcgcuncdesc, 2))
    return FALSE;
...
static bfd_boolean
sh_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_sh_link_hash_table *htab;
  ...
  if (htab->vxworks_p)
    {
      if (!cgc_vxworks_create_dynamic_sections (abfd, info, &htab->srelplt2))
	return FALSE;
    }
...
static bfd_boolean
sh_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
			      struct cgc_link_hash_entry *h)
{
  struct cgc_sh_link_hash_table *htab;
  struct cgc_sh_link_hash_entry *eh;
  struct cgc_sh_dyn_relocs *p;
  asection *s;
  ...
  eh = (struct cgc_sh_link_hash_entry *) h;
...
allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if (htab->root.dynamic_sections_created
	  ...
	  || WILL_CALL_FINISH_DYNAMIC_SYMBOL (1, 0, h))
	{
	  ...
	  htab->srelplt->size += sizeof (Cgc32_External_Rela);
	  ...
	  if (htab->vxworks_p && !info->shared)
		...
		htab->srelplt2->size += sizeof (Cgc32_External_Rela);
  ...
  if (h->got.refcount > 0)
    {
      asection *s;
      bfd_boolean dyn;
      enum got_type got_type = sh_cgc_hash_entry (h)->got_type;
	...
	;
	 ...
	 R_SH_TLS_GD needs one if local symbol and two if global.  */
	       ...
	       || got_type == GOT_TLS_IE)
	htab->srelgot->size += sizeof (Cgc32_External_Rela);
      else if (got_type == GOT_TLS_GD)
	htab->srelgot->size += 2 * sizeof (Cgc32_External_Rela);
  ...
  if (eh->abs_funcdesc_refcount > 0
	...
	htab->srelgot->size
	  += eh->abs_funcdesc_refcount * sizeof (Cgc32_External_Rela);
    }
  ...
  if ((eh->funcdesc.refcount > 0
	...
	htab->srcgcuncdesc->size += sizeof (Cgc32_External_Rela);
    }
...
static bfd_boolean
sh_cgc_always_size_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  sh_cgc_hash_table (info)->plt_info = get_plt_info (output_bfd, info->shared);

  if (sh_cgc_hash_table (info)->fdpic_p && !info->relocatable
      && !bfd_cgc_stack_segment_size (output_bfd, info,
...
static bfd_boolean
sh_cgc_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
			      struct bfd_link_info *info)
{
  struct cgc_sh_link_hash_table *htab;
  bfd *dynobj;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = ((struct cgc_sh_dyn_relocs *)
	      ...
	      else if (p->count != 0)
		{
		  ...
		  srel->size += p->count * sizeof (Cgc32_External_Rela);
      ...
      if (local_got)
	{
	  ...
	  local_got_type = sh_cgc_local_got_type (ibfd);
	  local_funcdesc = sh_cgc_local_funcdesc (ibfd);
		  ...
		  *local_got = s->size;
		  ...
		  if (*local_got_type == GOT_FUNCDESC)
		      ...
		      if (local_funcdesc == NULL)
			{
			  ...
			  if (local_funcdesc == NULL)
			    return FALSE;
			  sh_cgc_local_funcdesc (ibfd) = local_funcdesc;
			  local_funcdesc += (local_got
					     - cgc_local_got_refcounts (ibfd));
			}
  ...
  if (htab->root.dynamic_sections_created)
    {
	 ...
	 values later, in sh_cgc_finish_dynamic_sections, but we
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (htab->splt->size != 0)
	{
      ...
      else if ((cgc_cgcheader (output_bfd)->e_flags & EF_SH_FDPIC)
	       && htab->sgot->size != 0)
	{
	  if (! add_dynamic_entry (DT_PLTGOT, 0))
	    return FALSE;
      ...
      if (relocs)
	{
	  ...
	  if ((info->flags & DF_TEXTREL) == 0)
	    cgc_link_hash_traverse (&htab->root, readonly_dynrelocs, info);
...
sh_cgc_add_dyn_reloc (bfd *output_bfd, asection *sreloc, bfd_vma offset,
		      int reloc_type, long dynindx, bfd_vma addend)
{
  ...
  outrel.r_info = CGC32_R_INFO (dynindx, reloc_type);
  outrel.r_addend = addend;

  reloc_offset = sreloc->reloc_count * sizeof (Cgc32_External_Rela);
  BFD_ASSERT (reloc_offset < sreloc->size);
  bfd_cgc32_swap_reloca_out (output_bfd, &outrel,
...
inline static void
sh_cgc_add_rofixup (bfd *output_bfd, asection *srofixup, bfd_vma offset)
{
...
sh_cgc_got_offset (struct cgc_sh_link_hash_table *htab)
{
...
static unsigned
sh_cgc_osec_to_segment (bfd *output_bfd, asection *osec)
{
  Cgc_Internal_Phdr *p = NULL;

  if (output_bfd->xvec->flavour == bfd_target_cgc_flavour)
    p = _bfd_cgc_find_segment_containing_section (output_bfd, osec);
  ...
  return (p != NULL) ? p - cgc_tdata (output_bfd)->phdr : -1;
}

static bfd_boolean
sh_cgc_osec_readonly_p (bfd *output_bfd, asection *osec)
{
  unsigned seg = sh_cgc_osec_to_segment (output_bfd, osec);

  return (seg != (unsigned) -1
	  && ! (cgc_tdata (output_bfd)->phdr[seg].p_flags & PF_W));
}

/* Generate the initial contents of a local function descriptor, along
   with any relocations or fixups required.  */
static bfd_boolean
sh_cgc_initialize_funcdesc (bfd *output_bfd,
			    struct bfd_link_info *info,
			    struct cgc_link_hash_entry *h,
			    bfd_vma offset,
			    asection *section,
			    bfd_vma value)
{
  ...
  if (h == NULL || SYMBOL_CALLS_LOCAL (info, h))
    {
      dynindx = cgc_section_data (section->output_section)->dynindx;
      addr = value + section->output_offset;
      seg = sh_cgc_osec_to_segment (output_bfd, section->output_section);
    }
  else
    {
  ...
  if (!info->shared && SYMBOL_CALLS_LOCAL (info, h))
	...
	{
	  sh_cgc_add_rofixup (output_bfd, htab->srofixup,
			      offset
    ...
    sh_cgc_add_dyn_reloc (output_bfd, htab->srcgcuncdesc,
			  offset
			  + htab->sfuncdesc->output_section->vma
			  + htab->sfuncdesc->output_offset,
...
/* Relocate an SH CGC section.  */

static bfd_boolean
sh_cgc_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
			 bfd *input_bfd, asection *input_section,
			 ...
			 asection **local_sections)
{
  ...
  BFD_ASSERT (is_sh_cgc (input_bfd));
  ...
  isec_segment = sh_cgc_osec_to_segment (output_bfd,
					 input_section->output_section);
  if (fdpic_p && sgot)
    got_segment = sh_cgc_osec_to_segment (output_bfd,
					  sgot->output_section);
  else
    got_segment = -1;
  if (fdpic_p && splt)
    plt_segment = sh_cgc_osec_to_segment (output_bfd,
					  splt->output_section);
  else
    plt_segment = -1;
  ...
  for (; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  symname = bfd_cgc_string_from_cgc_section
	      ...
	      || h->root.type == bfd_link_hash_defweak)
	    {
		 ...
		 sec->output_section will be NULL.  */
		  ...
		  || (sec->output_section == NULL
			  ...
			  && h->def_dynamic))
		  || (sec->output_section == NULL
		      && (sh_cgc_hash_entry (h)->got_type == GOT_TLS_IE
			  || sh_cgc_hash_entry (h)->got_type == GOT_TLS_GD)))
		;
	...
	continue;
	 ...
	 the target symbol, but there are some exceptions below.  */
      ...
      if (sec != NULL)
	check_segment[1] = sh_cgc_osec_to_segment (output_bfd,
						   sec->output_section);
      else
	check_segment[1] = -1;
	  ...
	  if (info->shared
		  ...
		  || !SYMBOL_CALLS_LOCAL (info, h)))
	    {
		 ...
		 time.  */
	      ...
	      if (sreloc == NULL)
		{
		  sreloc = _bfd_cgc_get_dynamic_reloc_section
	      ...
	      else if (r_type == R_SH_REL32)
		{
		  ...
		  outrel.r_info = CGC32_R_INFO (h->dynindx, R_SH_REL32);
		  outrel.r_addend
			       ...
			       && h->def_regular)))
		{
		  ...
		  dynindx = cgc_section_data (sec->output_section)->dynindx;
		  outrel.r_info = CGC32_R_INFO (dynindx, R_SH_DIR32);
	      ...
	      else
		{
		  ...
		  if (h == NULL
			  ...
			  && h->def_regular))
		    {
		      ...
		      outrel.r_info = CGC32_R_INFO (0, R_SH_RELATIVE);
		    }
		  else
		    {
		      BFD_ASSERT (h->dynindx != -1);
		      outrel.r_info = CGC32_R_INFO (h->dynindx, R_SH_DIR32);
		    }
		  outrel.r_addend = relocation;
		  outrel.r_addend
		    += (howto->partial_inplace
		...
		if (sh_cgc_osec_readonly_p (output_bfd,
					    input_section->output_section))
		  {
		    (*_bfd_error_handler)
	...
	case R_SH_GOT10BY8:
	  ...
	  if (h != NULL)
		...
		{
		  struct cgc_sh_link_hash_entry *hsh;

		  hsh = (struct cgc_sh_link_hash_entry *)h;
		      ...
		      && h->root.type == bfd_link_hash_undefweak))
		{
		  ...
		  if ((off & 1) != 0)
			...
			h->got.offset |= 1;
			 ...
			 symbol address, also add a fixup.  */
			  ...
			  && sh_cgc_hash_entry (h)->got_type == GOT_NORMAL
			  && (CGC_ST_VISIBILITY (h->other) == STV_DEFAULT
			      || h->root.type != bfd_link_hash_undefweak))
			sh_cgc_add_rofixup (output_bfd, htab->srofixup,
	      ...
	      relocation = sh_cgc_got_offset (htab) + off;
	    }
	  else
	    {
#ifdef INCLUDE_SHMEDIA
	      ...
	      else
		{
		  ...
		  if (info->shared)
			...
			{
			  int dynindx
			    = cgc_section_data (sec->output_section)->dynindx;
			  outrel.r_info = CGC32_R_INFO (dynindx, R_SH_DIR32);
			  outrel.r_addend = relocation;
			  outrel.r_addend -= sec->output_section->vma;
			}
		      else
			{
		      ...
		      loc += srelgot->reloc_count++ * sizeof (Cgc32_External_Rela);
		      bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
		    }
		  else if (fdpic_p
			   && (sh_cgc_local_got_type (input_bfd) [r_symndx]
			       == GOT_NORMAL))
		    sh_cgc_add_rofixup (output_bfd, htab->srofixup,
					sgot->output_section->vma
					+ sgot->output_offset
	...
	case R_SH_FUNCDESC:
	  {
		...
		dynindx = cgc_section_data (h->root.u.def.section
		...
		dynindx = cgc_section_data (htab->sfuncdesc
					    ->output_section)->dynindx;

		if (h)
		  {
		    offset = sh_cgc_hash_entry (h)->funcdesc.offset;
		    BFD_ASSERT (offset != MINUS_ONE);
		    if ((offset & 1) == 0)
		      {
			if (!sh_cgc_initialize_funcdesc (output_bfd, info, h,
							 offset, NULL, 0))
			  return FALSE;
			sh_cgc_hash_entry (h)->funcdesc.offset |= 1;
		      }
		  }
		else
		  {
		    ...
		    local_funcdesc = sh_cgc_local_funcdesc (input_bfd);
		    offset = local_funcdesc[r_symndx].offset;
		    BFD_ASSERT (offset != MINUS_ONE);
		    if ((offset & 1) == 0)
		      {
			if (!sh_cgc_initialize_funcdesc (output_bfd, info, NULL,
							 offset, sec,
							 sym->st_value))
			  return FALSE;
		...
		if (offset != (bfd_vma)-1)
		  sh_cgc_add_dyn_reloc (output_bfd, srelgot,
					offset
					+ reloc_section->output_section->vma
					+ reloc_section->output_offset,
	  ...
	  funcdesc_leave_zero:
		...
		relocation = sh_cgc_got_offset (htab) + reloc_offset;
#ifdef GOT_BIAS
		relocation -= GOT_BIAS;
#endif
	      }
	...
	case R_SH_LOOP_START:
	  {
	    ...
	    r = sh_cgc_reloc_loop (r_type, input_bfd, input_section, contents,
	...
	case R_SH_TLS_IE_32:
	  BFD_ASSERT (htab);
	  check_segment[0] = check_segment[1] = -1;
	  r_type = sh_cgc_optimized_tls_reloc (info, r_type, h == NULL);
	  got_type = GOT_UNKNOWN;
	  if (h == NULL && local_got_offsets)
	    got_type = sh_cgc_local_got_type (input_bfd) [r_symndx];
	  else if (h != NULL)
	    {
	      got_type = sh_cgc_hash_entry (h)->got_type;
	      if (! info->shared
		  && (h->dynindx == -1
		      || h->def_regular))
	  ...
	  if (r_type == R_SH_TLS_LE_32)
	    {
	      ...
	      if (CGC32_R_TYPE (rel->r_info) == R_SH_TLS_GD_32)
		{
		  /* GD->LE transition:
	  ...
	  if (r_type == R_SH_TLS_IE_32
	      && ! htab->root.dynamic_sections_created)
	    {
	      ...
	      bfd_put_32 (output_bfd, tpoff (info, relocation),
			  sgot->contents + off);
	      bfd_put_32 (output_bfd, sh_cgc_got_offset (htab) + off,
			  contents + rel->r_offset);
	      continue;
	    }
	  ...
	  if (r_type == (int) CGC32_R_TYPE (rel->r_info))
	    relocation = sh_cgc_got_offset (htab) + off;
	  else
	    {
	      bfd_vma offset;
	  ...
	  if (off & 1)
		...
		abort ();
	      ...
	      outrel.r_offset = (sgot->output_section->vma
				 + sgot->output_offset + off);
	      outrel.r_addend = 0;
	      outrel.r_info = CGC32_R_INFO (0, R_SH_TLS_DTPMOD32);
	      loc = srelgot->contents;
	      loc += srelgot->reloc_count++ * sizeof (Cgc32_External_Rela);
	      bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
	      htab->tls_ldm_got.offset |= 1;
	...
	case R_SH_TLS_LE_32:
	  {
	    ...
	    outrel.r_offset = (input_section->output_section->vma
			       + input_section->output_offset
			       + rel->r_offset);
	    outrel.r_info = CGC32_R_INFO (indx, R_SH_TLS_TPOFF32);
	    if (indx == 0)
	      outrel.r_addend = relocation - dtpoff_base (info);
	    else
	      outrel.r_addend = 0;
	  ...
	  && check_segment[0] != check_segment[1])
	{
	  ...
	  cgc_cgcheader (output_bfd)->e_flags &= ~EF_SH_PIC;
	}
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses sh_cgc_relocate_section.  */

static bfd_byte *
sh_cgc_get_relocated_section_contents (bfd *output_bfd,
				       ...
				       asymbol **symbols)
{
  ...
  if (relocatable
      || cgc_section_data (input_section)->this_hdr.contents == NULL)
  ...
  memcpy (data, cgc_section_data (input_section)->this_hdr.contents,
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      ...
      if (symtab_hdr->sh_info != 0)
	{
	  isymbuf = (Cgc_Internal_Sym *) symtab_hdr->contents;
	  if (isymbuf == NULL)
	    isymbuf = bfd_cgc_get_cgc_syms (input_bfd, symtab_hdr,
					    symtab_hdr->sh_info, 0,
					    NULL, NULL, NULL);
	  if (isymbuf == NULL)
	    goto error_return;
      ...
      for (isym = isymbuf, secpp = sections; isym < isymend; ++isym, ++secpp)
	{
	  ...
	  *secpp = isec;
      ...
      if (! sh_cgc_relocate_section (output_bfd, link_info, input_bfd,
	...
	free (sections);
	  ...
	  && symtab_hdr->contents != (unsigned char *) isymbuf)
	free (isymbuf);
      if (cgc_section_data (input_section)->relocs != internal_relocs)
	free (internal_relocs);
    }
 ...
 error_return:
  ...
  if (isymbuf != NULL
    ...
    free (isymbuf);
  if (internal_relocs != NULL
      && cgc_section_data (input_section)->relocs != internal_relocs)
    free (internal_relocs);
  return NULL;
}
...
tpoff (struct bfd_link_info *info, bfd_vma address)
{
  ...
  return (address - cgc_hash_table (info)->tls_sec->vma
	  + align_power ((bfd_vma) 8,
			 cgc_hash_table (info)->tls_sec->alignment_power));
}

static asection *
sh_cgc_gc_mark_hook (asection *sec,
		     struct bfd_link_info *info,
...
static bfd_boolean
sh_cgc_gc_sweep_hook (bfd *abfd, struct bfd_link_info *info,
		      asection *sec, const Cgc_Internal_Rela *relocs)
{
  ...
  local_funcdesc = sh_cgc_local_funcdesc (abfd);
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
      ...
      r_type = CGC32_R_TYPE (rel->r_info);
      switch (sh_cgc_optimized_tls_reloc (info, r_type, h != NULL))
	{
	case R_SH_TLS_LD_32:
	  if (sh_cgc_hash_table (info)->tls_ldm_got.refcount > 0)
	    sh_cgc_hash_table (info)->tls_ldm_got.refcount -= 1;
	  break;
	...
	case R_SH_FUNCDESC:
	  if (h != NULL)
	    sh_cgc_hash_entry (h)->abs_funcdesc_refcount -= 1;
	  else if (sh_cgc_hash_table (info)->fdpic_p && !info->shared)
	    sh_cgc_hash_table (info)->srofixup->size -= 4;
	...
	case R_SH_GOTOFFFUNCDESC20:
	  if (h != NULL)
	    sh_cgc_hash_entry (h)->funcdesc.refcount -= 1;
	  else
	    local_funcdesc[r_symndx].refcount -= 1;
	  break;

	case R_SH_DIR32:
	  if (sh_cgc_hash_table (info)->fdpic_p && !info->shared
...
static void
sh_cgc_copy_indirect_symbol (struct bfd_link_info *info,
			     struct cgc_link_hash_entry *dir,
			     struct cgc_link_hash_entry *ind)
{
  struct cgc_sh_link_hash_entry *edir, *eind;

  edir = (struct cgc_sh_link_hash_entry *) dir;
  eind = (struct cgc_sh_link_hash_entry *) ind;
...
static int
sh_cgc_optimized_tls_reloc (struct bfd_link_info *info, int r_type,
			    int is_local)
{
  if (info->shared)
    return r_type;
...
static bfd_boolean
sh_cgc_check_relocs (bfd *abfd, struct bfd_link_info *info, asection *sec,
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_SH_GOTFUNCDESC20:
	  ...
	  if (h != NULL)
		...
		{
		  struct cgc_sh_link_hash_entry *eh
		    = (struct cgc_sh_link_hash_entry *) h;
...
#endif
		h->got.refcount += 1;
	      old_got_type = sh_cgc_hash_entry (h)->got_type;
	    }
	  else
	    {
	      bfd_signed_vma *local_got_refcounts;
...
#endif
		local_got_refcounts[r_symndx] += 1;
	      old_got_type = sh_cgc_local_got_type (abfd) [r_symndx];
	    }
	  ...
	  if (old_got_type != got_type)
	    {
	      if (h != NULL)
		sh_cgc_hash_entry (h)->got_type = got_type;
	      else
		sh_cgc_local_got_type (abfd) [r_symndx] = got_type;
	    }
	...
	case R_SH_GOTOFFFUNCDESC20:
	  ...
	  if (h == NULL)
	      ...
	      if (local_funcdesc == NULL)
		{
		  ...
		  if (local_funcdesc == NULL)
		    return FALSE;
		  sh_cgc_local_funcdesc (abfd) = local_funcdesc;
		}
	      local_funcdesc[r_symndx].refcount += 1;
	    ...
	    {
	      sh_cgc_hash_entry (h)->funcdesc.refcount++;
	      if (r_type == R_SH_FUNCDESC)
		sh_cgc_hash_entry (h)->abs_funcdesc_refcount++;
	...
	case R_SH_GOTPLT10BY8:
	  ...
	  h->needs_plt = 1;
	  h->plt.refcount += 1;
	  ((struct cgc_sh_link_hash_entry *) h)->gotplt_refcount += 1;
	  ...
	  if ((info->shared
	      ...
	      if (sreloc == NULL)
		{
		 ...
		 relocations we need for this symbol.  */
	      if (h != NULL)
		head = &((struct cgc_sh_link_hash_entry *) h)->dyn_relocs;
	      else
		{
		  ...
		  head = (struct cgc_sh_dyn_relocs **) vpp;
		}
...
#ifndef sh_cgc_set_mach_from_flags
static unsigned int sh_ef_bfd_table[] = { EF_SH_BFD_TABLE };

static bfd_boolean
sh_cgc_set_mach_from_flags (bfd *abfd)
{
  flagword flags = cgc_cgcheader (abfd)->e_flags & EF_SH_MACH_MASK;
...
/* Reverse table lookup for sh_ef_bfd_table[].
   ...
   return the equivalent CGC flags from the table.
   Return -1 if no match is found.  */

int
sh_cgc_get_flags_from_mach (unsigned long mach)
{
...
#endif /* not sh_cgc_set_mach_from_flags */

#ifndef sh_cgc_set_private_flags
/* Function to keep SH specific file flags.  */

static bfd_boolean
sh_cgc_set_private_flags (bfd *abfd, flagword flags)
{
  ...
  return sh_cgc_set_mach_from_flags (abfd);
}
#endif /* not sh_cgc_set_private_flags */

#ifndef sh_cgc_copy_private_data
/* Copy backend specific data from one object module to another */

static bfd_boolean
sh_cgc_copy_private_data (bfd * ibfd, bfd * obfd)
{
  /* Copy object attributes.  */
  _bfd_cgc_copy_obj_attributes (ibfd, obfd);

  if (! is_sh_cgc (ibfd) || ! is_sh_cgc (obfd))
    return TRUE;

  return sh_cgc_set_private_flags (obfd, cgc_cgcheader (ibfd)->e_flags);
}
#endif /* not sh_cgc_copy_private_data */

#ifndef sh_cgc_merge_private_data

/* This function returns the CGC architecture number that
   corresponds to the given arch_sh* flags.  */

int
sh_find_cgc_flags (unsigned int arch_set)
{
  ...
  return sh_cgc_get_flags_from_mach (bfd_mach);
}

/* This routine initialises the cgc flags when required and
   calls sh_merge_bfd_arch() to check dsp/fpu compatibility.  */

static bfd_boolean
sh_cgc_merge_private_data (bfd *ibfd, bfd *obfd)
{
  ...
  if (! cgc_flags_init (obfd))
    {
      ...
      sh_cgc_set_mach_from_flags (obfd);
      if (cgc_cgcheader (obfd)->e_flags & EF_SH_FDPIC)
	cgc_cgcheader (obfd)->e_flags |= EF_SH_PIC;
    }
  ...
  cgc_cgcheader (obfd)->e_flags &= ~EF_SH_MACH_MASK;
  cgc_cgcheader (obfd)->e_flags |=
    sh_cgc_get_flags_from_mach (bfd_get_mach (obfd));
...
#endif /* not sh_cgc_merge_private_data */
...
static bfd_boolean
sh_cgc_object_p (bfd *abfd)
{
  if (! sh_cgc_set_mach_from_flags (abfd))
    return FALSE;

  return (((cgc_cgcheader (abfd)->e_flags & EF_SH_FDPIC) != 0)
	  == fdpic_object_p (abfd));
}
...
static bfd_boolean
sh_cgc_finish_dynamic_symbol (bfd *output_bfd, struct bfd_link_info *info,
			      struct cgc_link_hash_entry *h,
			      Cgc_Internal_Sym *sym)
{
  struct cgc_sh_link_hash_table *htab;
  ...
  if (h->plt.offset != (bfd_vma) -1)
	...
	install_plt_field (output_bfd, FALSE,
			   plt_index * sizeof (Cgc32_External_Rela),
			   (splt->contents
			    + h->plt.offset
      ...
      if (htab->fdpic_p)
	bfd_put_32 (output_bfd,
		    sh_cgc_osec_to_segment (output_bfd,
      ...
      rel.r_offset = (sgotplt->output_section->vma
		      ...
		      + got_offset);
      if (htab->fdpic_p)
	rel.r_info = CGC32_R_INFO (h->dynindx, R_SH_FUNCDESC_VALUE);
      else
	rel.r_info = CGC32_R_INFO (h->dynindx, R_SH_JMP_SLOT);
      rel.r_addend = 0;
#ifdef GOT_BIAS
      rel.r_addend = GOT_BIAS;
#endif
      loc = srelplt->contents + plt_index * sizeof (Cgc32_External_Rela);
      ...
      if (htab->vxworks_p && !info->shared)
	{
	  ...
	  loc = (htab->srelplt2->contents
		 + (plt_index * 2 + 1) * sizeof (Cgc32_External_Rela));
	  ...
	  rel.r_offset = (htab->splt->output_section->vma
			  ...
			  + h->plt.offset
			  + plt_info->symbol_fields.got_entry);
	  rel.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_SH_DIR32);
	  ...
	  rel.r_offset = (sgotplt->output_section->vma
			  + sgotplt->output_offset
			  + got_offset);
	  rel.r_info = CGC32_R_INFO (htab->root.hplt->indx, R_SH_DIR32);
	  rel.r_addend = 0;
  ...
  if (h->got.offset != (bfd_vma) -1
      && sh_cgc_hash_entry (h)->got_type != GOT_TLS_GD
      && sh_cgc_hash_entry (h)->got_type != GOT_TLS_IE
      && sh_cgc_hash_entry (h)->got_type != GOT_FUNCDESC)
    {
      asection *sgot;
      asection *srelgot;
      Cgc_Internal_Rela rel;
	  ...
	  && SYMBOL_REFERENCES_LOCAL (info, h))
	{
	  if (htab->fdpic_p)
		...
		= cgc_section_data (sec->output_section)->dynindx;
	      ...
	      rel.r_info = CGC32_R_INFO (dynindx, R_SH_DIR32);
	      rel.r_addend = (h->root.u.def.value
			      + h->root.u.def.section->output_offset);
	    }
	  else
	    {
	      rel.r_info = CGC32_R_INFO (0, R_SH_RELATIVE);
	      rel.r_addend = (h->root.u.def.value
			      + h->root.u.def.section->output_section->vma
			      + h->root.u.def.section->output_offset);
	    }
      ...
      else
	{
	  ...
	  rel.r_info = CGC32_R_INFO (h->dynindx, R_SH_GLOB_DAT);
	  rel.r_addend = 0;
	}
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_offset = (h->root.u.def.value
		      + h->root.u.def.section->output_section->vma
		      + h->root.u.def.section->output_offset);
      rel.r_info = CGC32_R_INFO (h->dynindx, R_SH_COPY);
      rel.r_addend = 0;
...
static bfd_boolean
sh_cgc_finish_dynamic_sections (bfd *output_bfd, struct bfd_link_info *info)
{
  struct cgc_sh_link_hash_table *htab;
  asection *sgotplt;
  asection *sdyn;
  ...
  if (htab->root.dynamic_sections_created)
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  bfd_cgc32_swap_dyn_in (htab->root.dynobj, dyncon, &dyn);
	  ...
	  switch (dyn.d_tag)
	      ...
	      if (dyn.d_un.d_val != 0)
		{
		  ...
		  h = cgc_link_hash_lookup (&htab->root, name,
					    FALSE, FALSE, TRUE);
      ...
      if (splt && splt->size > 0 && htab->plt_info->plt0_entry)
	{
	  ...
	  if (htab->vxworks_p)
	      ...
	      while (loc < htab->srelplt2->contents + htab->srelplt2->size)
		{
		  ...
		  rel.r_info = CGC32_R_INFO (htab->root.hgot->indx,
					     R_SH_DIR32);
		  bfd_cgc32_swap_reloc_out (output_bfd, &rel, loc);
		  loc += sizeof (Cgc32_External_Rela);
		  ...
		  bfd_cgc32_swap_reloc_in (output_bfd, loc, &rel);
		  rel.r_info = CGC32_R_INFO (htab->root.hplt->indx,
					     R_SH_DIR32);
		  bfd_cgc32_swap_reloc_out (output_bfd, &rel, loc);
		  loc += sizeof (Cgc32_External_Rela);
		}
  ...
  if (sgotplt && sgotplt->size > 0)
    cgc_section_data (sgotplt->output_section)->this_hdr.sh_entsize = 4;

  /* At the very end of the .rofixup section is a pointer to the GOT.  */
  if (htab->fdpic_p && htab->srofixup != NULL)
    {
      struct cgc_link_hash_entry *hgot = htab->root.hgot;
      bfd_vma got_value = hgot->root.u.def.value
	+ hgot->root.u.def.section->output_section->vma
	+ hgot->root.u.def.section->output_offset;

      sh_cgc_add_rofixup (output_bfd, htab->srofixup, got_value);

      /* Make sure we allocated and generated the same number of fixups.  */
      BFD_ASSERT (htab->srofixup->reloc_count * 4 == htab->srofixup->size);
    }

  if (htab->srcgcuncdesc)
    BFD_ASSERT (htab->srcgcuncdesc->reloc_count * sizeof (Cgc32_External_Rela)
		== htab->srcgcuncdesc->size);

  if (htab->srelgot)
    BFD_ASSERT (htab->srelgot->reloc_count * sizeof (Cgc32_External_Rela)
		== htab->srelgot->size);
...
sh_cgc_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static bfd_boolean
cgc32_shlin_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  unsigned int size;
...
static bfd_boolean
cgc32_shlin_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  switch (note->descsz)
    {
      default:
	return FALSE;
      ...
      case 124:		/* Linux/SH cgc_prpsinfo */
...
static bfd_vma
sh_cgc_plt_sym_val (bfd_vma i, const asection *plt,
...
static bfd_boolean
sh_cgc_use_relative_eh_frame (bfd *input_bfd ATTRIBUTE_UNUSED,
			      struct bfd_link_info *info,
...
static bfd_byte
sh_cgc_encode_eh_address (bfd *abfd,
			  struct bfd_link_info *info,
			  ...
			  bfd_vma *encoded)
{
  ...
  if (!htab->fdpic_p)
    return _bfd_cgc_encode_eh_address (abfd, info, osec, offset, loc_sec,
				       loc_offset, encoded);

  h = htab->root.hgot;
  BFD_ASSERT (h && h->root.type == bfd_link_hash_defined);

  if (! h || (sh_cgc_osec_to_segment (abfd, osec)
	      == sh_cgc_osec_to_segment (abfd, loc_sec->output_section)))
    return _bfd_cgc_encode_eh_address (abfd, info, osec, offset,
				       loc_sec, loc_offset, encoded);

  BFD_ASSERT (sh_cgc_osec_to_segment (abfd, osec)
	      == (sh_cgc_osec_to_segment
...
#if !defined SH_TARGET_ALREADY_DEFINED
#define TARGET_BIG_SYM		bfd_cgc32_sh_vec
#define TARGET_BIG_NAME		"cgc32-sh"
#define TARGET_LITTLE_SYM	bfd_cgc32_shl_vec
#define TARGET_LITTLE_NAME	"cgc32-shl"
#endif

#define CGC_ARCH		bfd_arch_sh
#define CGC_TARGET_ID		SH_CGC_DATA
#define CGC_MACHINE_CODE	EM_SH
#ifdef __QNXTARGET__
#define CGC_MAXPAGESIZE		0x1000
#else
#define CGC_MAXPAGESIZE		0x80
#endif

#define cgc_symbol_leading_char '_'

#define bfd_cgc32_bfd_reloc_type_lookup	sh_cgc_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup \
					sh_cgc_reloc_name_lookup
#define cgc_info_to_howto		sh_cgc_info_to_howto
#define bfd_cgc32_bfd_relax_section	sh_cgc_relax_section
#define cgc_backend_relocate_section	sh_cgc_relocate_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
					sh_cgc_get_relocated_section_contents
#define bfd_cgc32_mkobject		sh_cgc_mkobject
#define cgc_backend_object_p		sh_cgc_object_p
#define bfd_cgc32_bfd_set_private_bfd_flags \
					sh_cgc_set_private_flags
#define bfd_cgc32_bfd_copy_private_bfd_data \
					sh_cgc_copy_private_data
#define bfd_cgc32_bfd_merge_private_bfd_data \
					sh_cgc_merge_private_data

#define cgc_backend_gc_mark_hook	sh_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook	sh_cgc_gc_sweep_hook
#define cgc_backend_check_relocs	sh_cgc_check_relocs
#define cgc_backend_copy_indirect_symbol \
					sh_cgc_copy_indirect_symbol
#define cgc_backend_create_dynamic_sections \
					sh_cgc_create_dynamic_sections
#define bfd_cgc32_bfd_link_hash_table_create \
					sh_cgc_link_hash_table_create
#define cgc_backend_adjust_dynamic_symbol \
					sh_cgc_adjust_dynamic_symbol
#define cgc_backend_always_size_sections \
					sh_cgc_always_size_sections
#define cgc_backend_size_dynamic_sections \
					sh_cgc_size_dynamic_sections
#define cgc_backend_omit_section_dynsym	sh_cgc_omit_section_dynsym
#define cgc_backend_finish_dynamic_symbol \
					sh_cgc_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
					sh_cgc_finish_dynamic_sections
#define cgc_backend_reloc_type_class	sh_cgc_reloc_type_class
#define cgc_backend_plt_sym_val		sh_cgc_plt_sym_val
#define cgc_backend_can_make_relative_eh_frame \
					sh_cgc_use_relative_eh_frame
#define cgc_backend_can_make_lsda_relative_eh_frame \
					sh_cgc_use_relative_eh_frame
#define cgc_backend_encode_eh_address \
					sh_cgc_encode_eh_address

#define cgc_backend_stack_align		8
...
#define	TARGET_BIG_SYM			bfd_cgc32_shnbsd_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-sh-nbsd"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_shlnbsd_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-shl-nbsd"
#undef	CGC_MAXPAGESIZE
#define	CGC_MAXPAGESIZE			0x10000
#undef	CGC_COMMONPAGESIZE
#undef	cgc_symbol_leading_char
#define	cgc_symbol_leading_char		0
#undef	cgc32_bed
#define	cgc32_bed			cgc32_sh_nbsd_bed
...
/* Linux support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM			bfd_cgc32_shblin_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-shbig-linux"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_shlin_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-sh-linux"
#undef	CGC_COMMONPAGESIZE
#define	CGC_COMMONPAGESIZE		0x1000

#undef	cgc_backend_grok_prstatus
#define	cgc_backend_grok_prstatus	cgc32_shlin_grok_prstatus
#undef	cgc_backend_grok_psinfo
#define	cgc_backend_grok_psinfo		cgc32_shlin_grok_psinfo
#undef	cgc32_bed
#define	cgc32_bed			cgc32_sh_lin_bed
...
/* FDPIC support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM			bfd_cgc32_shbfd_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-shbig-fdpic"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_shfd_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-sh-fdpic"

#undef	cgc32_bed
#define	cgc32_bed			cgc32_sh_fd_bed
...
/* VxWorks support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM			bfd_cgc32_shvxworks_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc32-sh-vxworks"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc32_shlvxworks_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc32-shl-vxworks"
#undef	cgc32_bed
#define	cgc32_bed			cgc32_sh_vxworks_bed

#undef	cgc_backend_want_plt_sym
#define	cgc_backend_want_plt_sym	1
#undef	cgc_symbol_leading_char
#define	cgc_symbol_leading_char		'_'
#define	cgc_backend_want_got_underscore 1
#undef	cgc_backend_grok_prstatus
#undef	cgc_backend_grok_psinfo
#undef	cgc_backend_add_symbol_hook
#define	cgc_backend_add_symbol_hook	cgc_vxworks_add_symbol_hook
