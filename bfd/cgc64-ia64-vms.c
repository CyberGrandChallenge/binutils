...
struct cgc64_ia64_dyn_sym_info
{
  ...
  struct cgc64_ia64_dyn_reloc_entry
  {
    struct cgc64_ia64_dyn_reloc_entry *next;
    asection *srel;
    int type;
    int count;
  } *reloc_entries;
...
struct cgc64_ia64_local_hash_entry
{
  int id;
  unsigned int r_sym;
  /* The number of elements in cgc64_ia64_dyn_sym_info array.  */
  unsigned int count;
  /* The number of sorted elements in cgc64_ia64_dyn_sym_info array.  */
  unsigned int sorted_count;
  /* The size of cgc64_ia64_dyn_sym_info array.  */
  unsigned int size;
  /* The array of cgc64_ia64_dyn_sym_info.  */
  struct cgc64_ia64_dyn_sym_info *info;
...
struct cgc64_ia64_link_hash_entry
{
...
struct cgc64_ia64_link_hash_table
{
  /* The main hash table.  */
...
struct cgc64_ia64_allocate_data
{
  struct bfd_link_info *info;
  bfd_size_type ofs;
};

#define cgc64_ia64_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == IA64_CGC_DATA ? ((struct cgc64_ia64_link_hash_table *) ((p)->hash)) : NULL)

struct cgc64_ia64_vms_obj_tdata
{
...
#define cgc_ia64_vms_tdata(abfd) \
  ((struct cgc64_ia64_vms_obj_tdata *)((abfd)->tdata.any))
#define cgc_ia64_vms_ident(abfd) (cgc_ia64_vms_tdata(abfd)->ident)

struct cgc64_vms_transfer
{
...
typedef struct
{
  Cgc64_External_Ehdr ehdr;
  unsigned char vms_needed_count[8];
} Cgc64_External_VMS_Ehdr;

static struct cgc64_ia64_dyn_sym_info * get_dyn_sym_info
  (struct cgc64_ia64_link_hash_table *,
   struct cgc_link_hash_entry *,
   bfd *, const Cgc_Internal_Rela *, bfd_boolean);
static bfd_boolean cgc64_ia64_dynamic_symbol_p
  (struct cgc_link_hash_entry *);
static bfd_boolean cgc64_ia64_choose_gp
  (bfd *, struct bfd_link_info *, bfd_boolean);
static void cgc64_ia64_dyn_sym_traverse
  (struct cgc64_ia64_link_hash_table *,
   bfd_boolean (*) (struct cgc64_ia64_dyn_sym_info *, void *),
   void *);
static bfd_boolean allocate_global_data_got
  (struct cgc64_ia64_dyn_sym_info *, void *);
...
static asection *get_pltoff
  (bfd *, struct cgc64_ia64_link_hash_table *);
static asection *get_got
  (bfd *, struct cgc64_ia64_link_hash_table *);


/* Given a CGC reloc, return the matching HOWTO structure.  */

static void
cgc64_ia64_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED,
			  arelent *bfd_reloc,
			  Cgc_Internal_Rela *cgc_reloc)
{
  bfd_reloc->howto
    = ia64_cgc_lookup_howto ((unsigned int) CGC64_R_TYPE (cgc_reloc->r_info));
}
...
/* These functions do relaxation for IA-64 CGC.  */
...
static void
cgc64_ia64_update_short_info (asection *sec, bfd_vma offset,
			      struct cgc64_ia64_link_hash_table *ia64_info)
{
...
static bfd_boolean
cgc64_ia64_relax_section (bfd *abfd, asection *sec,
			  ...
			  bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  /* Don't even try to relax for non-CGC outputs.  */
  ...
  ia64_info = cgc64_ia64_hash_table (link_info);
  if (ia64_info == NULL)
    return FALSE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      struct cgc64_ia64_dyn_sym_info *dyn_i;
	...
	default:
      ...
      if (CGC64_R_SYM (irel->r_info) < symtab_hdr->sh_info)
	{
	  /* A local symbol.  */
	  ...
	  isym = isymbuf + CGC64_R_SYM (irel->r_info);
      ...
      else
	{
	  ...
	  indx = CGC64_R_SYM (irel->r_info) - symtab_hdr->sh_info;
	  ...
	  else if (cgc64_ia64_dynamic_symbol_p (h))
	    continue;
      ...
      if (is_branch)
	{
	  ...
	  if ((bfd_signed_vma) (symaddr - reladdr) >= offset
		...
		{
		  ia64_cgc_relax_brl (contents, roff);
	    ...
	    continue;
	  else if (ia64_cgc_relax_br (contents, roff))
	    {
	      irel->r_info = CGC64_R_INFO (CGC64_R_SYM (irel->r_info),
                                           R_IA64_PCREL60B);
	  ...
	  if (f == NULL)
		...
		continue;
	      ...
	      irel->r_info = CGC64_R_INFO (0, R_IA64_NONE);
	    }

	  /* Fix up the existing branch to hit the trampoline.  */
	  if (ia64_cgc_install_value (contents + roff, offset, r_type)
	      != bfd_reloc_ok)
	    goto error_return;
      ...
      else
	{
	  ...
	  if (gp == 0)
	    {
	      ...
	      if (gp == 0)
		{
		  if (!cgc64_ia64_choose_gp (obfd, link_info, FALSE))
		    goto error_return;
		  gp = _bfd_get_gp_value (obfd);
		}
	    }
	  ...
	  if (r_type == R_IA64_GPREL22)
	    cgc64_ia64_update_short_info (tsec->output_section,
	    ...
	    {
	      ia64_cgc_relax_ldxmov (contents, roff);
	      irel->r_info = CGC64_R_INFO (0, R_IA64_NONE);
	      changed_contents = TRUE;
	      changed_relocs = TRUE;
	    }
...
{
  return ((CONST_STRNEQ (name, CGC_STRING_ia64_unwind)
	   && ! CONST_STRNEQ (name, CGC_STRING_ia64_unwind_info))
	  || CONST_STRNEQ (name, CGC_STRING_ia64_unwind_once));
}
...
static bfd_boolean
cgc64_ia64_section_flags (flagword *flags,
			  const Cgc_Internal_Shdr *hdr)
{
...
/* Set the correct type for an IA-64 CGC section.  We do this by the
   section name, which is a hack, but ought to work.  */

static bfd_boolean
cgc64_ia64_fake_sections (bfd *abfd, Cgc_Internal_Shdr *hdr,
			  asection *sec)
{
  const char *name;
  ...
  if (is_unwind_section_name (abfd, name))
    {
	 ...
	 is set later, in cgc64_ia64_final_write_processing.  */
      hdr->sh_type = SHT_IA_64_UNWIND;
      hdr->sh_flags |= SHF_LINK_ORDER;
    }
  else if (strcmp (name, CGC_STRING_ia64_archext) == 0)
...
static bfd_boolean
cgc64_ia64_add_symbol_hook (bfd *abfd,
			    struct bfd_link_info *info,
...
static bfd_boolean
cgc64_ia64_is_local_label_name (bfd *abfd ATTRIBUTE_UNUSED,
				const char *name)
{
  return name[0] == '.';
}
...
static bfd_boolean
cgc64_ia64_dynamic_symbol_p (struct cgc_link_hash_entry *h)
{
  return h != NULL && h->def_dynamic;
}

static struct bfd_hash_entry*
cgc64_ia64_new_cgc_hash_entry (struct bfd_hash_entry *entry,
			       struct bfd_hash_table *table,
			       const char *string)
{
  struct cgc64_ia64_link_hash_entry *ret;
  ret = (struct cgc64_ia64_link_hash_entry *) entry;
  ...
  ret = ((struct cgc64_ia64_link_hash_entry *)
...
static void
cgc64_ia64_hash_hide_symbol (struct bfd_link_info *info,
			     struct cgc_link_hash_entry *xh,
			     bfd_boolean force_local)
{
  struct cgc64_ia64_link_hash_entry *h;
  struct cgc64_ia64_dyn_sym_info *dyn_i;
  unsigned int count;

  h = (struct cgc64_ia64_link_hash_entry *)xh;
...
static hashval_t
cgc64_ia64_local_htab_hash (const void *ptr)
{
  struct cgc64_ia64_local_hash_entry *entry
    = (struct cgc64_ia64_local_hash_entry *) ptr;

  return CGC_LOCAL_SYMBOL_HASH (entry->id, entry->r_sym);
}

/* Compare local hash entries.  */

static int
cgc64_ia64_local_htab_eq (const void *ptr1, const void *ptr2)
{
  struct cgc64_ia64_local_hash_entry *entry1
    = (struct cgc64_ia64_local_hash_entry *) ptr1;
  struct cgc64_ia64_local_hash_entry *entry2
    = (struct cgc64_ia64_local_hash_entry *) ptr2;

  return entry1->id == entry2->id && entry1->r_sym == entry2->r_sym;
}

/* Create the derived linker hash table.  The IA-64 CGC port uses this
   derived hash table to keep information specific to the IA-64 ElF
   linker (without using static variables).  */

static struct bfd_link_hash_table *
cgc64_ia64_hash_table_create (bfd *abfd)
{
  struct cgc64_ia64_link_hash_table *ret;
  ...
  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      cgc64_ia64_new_cgc_hash_entry,
				      sizeof (struct cgc64_ia64_link_hash_entry),
				      IA64_CGC_DATA))
    {
      free (ret);
      return NULL;
  ...
  ret->loc_hash_table = htab_try_create (1024, cgc64_ia64_local_htab_hash,
					 cgc64_ia64_local_htab_eq, NULL);
...
/* Free the global cgc64_ia64_dyn_sym_info array.  */

static bfd_boolean
cgc64_ia64_global_dyn_info_free (void **xentry,
				 void * unused ATTRIBUTE_UNUSED)
{
  struct cgc64_ia64_link_hash_entry *entry
    = (struct cgc64_ia64_link_hash_entry *) xentry;

  if (entry->root.root.type == bfd_link_hash_warning)
    entry = (struct cgc64_ia64_link_hash_entry *) entry->root.root.u.i.link;
...
/* Free the local cgc64_ia64_dyn_sym_info array.  */

static bfd_boolean
cgc64_ia64_local_dyn_info_free (void **slot,
				void * unused ATTRIBUTE_UNUSED)
{
  struct cgc64_ia64_local_hash_entry *entry
    = (struct cgc64_ia64_local_hash_entry *) *slot;
...
static void
cgc64_ia64_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct cgc64_ia64_link_hash_table *ia64_info
    = (struct cgc64_ia64_link_hash_table *) hash;
  if (ia64_info->loc_hash_table)
    {
      htab_traverse (ia64_info->loc_hash_table,
		     cgc64_ia64_local_dyn_info_free, NULL);
      htab_delete (ia64_info->loc_hash_table);
    }
  if (ia64_info->loc_hash_memory)
    objalloc_free ((struct objalloc *) ia64_info->loc_hash_memory);
  cgc_link_hash_traverse (&ia64_info->root,
			  cgc64_ia64_global_dyn_info_free, NULL);
  _bfd_cgc_link_hash_table_free (hash);
}

/* Traverse both local and global hash tables.  */

struct cgc64_ia64_dyn_sym_traverse_data
{
  bfd_boolean (*func) (struct cgc64_ia64_dyn_sym_info *, void *);
  void * data;
};

static bfd_boolean
cgc64_ia64_global_dyn_sym_thunk (struct bfd_hash_entry *xentry,
				 void * xdata)
{
  struct cgc64_ia64_link_hash_entry *entry
    = (struct cgc64_ia64_link_hash_entry *) xentry;
  struct cgc64_ia64_dyn_sym_traverse_data *data
    = (struct cgc64_ia64_dyn_sym_traverse_data *) xdata;
  struct cgc64_ia64_dyn_sym_info *dyn_i;
  unsigned int count;
...
static bfd_boolean
cgc64_ia64_local_dyn_sym_thunk (void **slot, void * xdata)
{
  struct cgc64_ia64_local_hash_entry *entry
    = (struct cgc64_ia64_local_hash_entry *) *slot;
  struct cgc64_ia64_dyn_sym_traverse_data *data
...
static void
cgc64_ia64_dyn_sym_traverse (struct cgc64_ia64_link_hash_table *ia64_info,
			     bfd_boolean (*func) (struct cgc64_ia64_dyn_sym_info *, void *),
			     void * data)
{
  struct cgc64_ia64_dyn_sym_traverse_data xdata;
  ...
  cgc_link_hash_traverse (&ia64_info->root,
			  cgc64_ia64_global_dyn_sym_thunk, &xdata);
  htab_traverse (ia64_info->loc_hash_table,
		 cgc64_ia64_local_dyn_sym_thunk, &xdata);
}
...
{
#define NBR_NOTES 7
  Cgc_Internal_Note notes[NBR_NOTES];
  char *module_name;
  int module_name_len;
  unsigned char cur_time[8];
  Cgc64_External_VMS_ORIG_DYN_Note *orig_dyn;
  ...
  ia64_info = cgc64_ia64_hash_table (info);
...
#define IMG_ID "V1.0"
  ...
  bfd_putl32 (EF_IA_64_ABI64, orig_dyn->cgc_flags);
  ...
  for (i = 0; i < NBR_NOTES; i++)
    note_size += sizeof (Cgc64_External_VMS_Note) - 1
  ...
  for (i = 0; i < NBR_NOTES; i++)
    {
      Cgc64_External_VMS_Note *enote = (Cgc64_External_VMS_Note *) noteptr;
...
static bfd_boolean
cgc64_ia64_create_dynamic_sections (bfd *abfd,
				    struct bfd_link_info *info)
{
  struct cgc64_ia64_link_hash_table *ia64_info;
  asection *s;
  ...
  if (s == NULL
      || !bfd_set_section_alignment (abfd, s, 3))
    return FALSE;
  s->size = sizeof (struct cgc64_vms_transfer);
...
static struct cgc64_ia64_local_hash_entry *
get_local_sym_hash (struct cgc64_ia64_link_hash_table *ia64_info,
		    bfd *abfd, const Cgc_Internal_Rela *rel,
		    bfd_boolean create)
{
  struct cgc64_ia64_local_hash_entry e, *ret;
  asection *sec = abfd->sections;
  hashval_t h = CGC_LOCAL_SYMBOL_HASH (sec->id,
				       CGC64_R_SYM (rel->r_info));
  void **slot;

  e.id = sec->id;
  e.r_sym = CGC64_R_SYM (rel->r_info);
  ...
  if (*slot)
    return (struct cgc64_ia64_local_hash_entry *) *slot;

  ret = (struct cgc64_ia64_local_hash_entry *)
	objalloc_alloc ((struct objalloc *) ia64_info->loc_hash_memory,
			sizeof (struct cgc64_ia64_local_hash_entry));
  if (ret)
    {
      memset (ret, 0, sizeof (*ret));
      ret->id = sec->id;
      ret->r_sym = CGC64_R_SYM (rel->r_info);
      *slot = ret;
    }
  return ret;
}

/* Used to sort cgc64_ia64_dyn_sym_info array.  */

static int
addend_compare (const void *xp, const void *yp)
{
  const struct cgc64_ia64_dyn_sym_info *x
    = (const struct cgc64_ia64_dyn_sym_info *) xp;
  const struct cgc64_ia64_dyn_sym_info *y
    = (const struct cgc64_ia64_dyn_sym_info *) yp;

  return x->addend < y->addend ? -1 : x->addend > y->addend ? 1 : 0;
}

/* Sort cgc64_ia64_dyn_sym_info array and remove duplicates.  */

static unsigned int
sort_dyn_sym_info (struct cgc64_ia64_dyn_sym_info *info,
		   unsigned int count)
{
...
static struct cgc64_ia64_dyn_sym_info *
get_dyn_sym_info (struct cgc64_ia64_link_hash_table *ia64_info,
		  struct cgc_link_hash_entry *h, bfd *abfd,
		  const Cgc_Internal_Rela *rel, bfd_boolean create)
{
  struct cgc64_ia64_dyn_sym_info **info_p, *info, *dyn_i, key;
  ...
  if (h)
    {
      struct cgc64_ia64_link_hash_entry *global_h;

      global_h = (struct cgc64_ia64_link_hash_entry *) h;
    ...
    {
      struct cgc64_ia64_local_hash_entry *loc_h;
...
static asection *
get_got (bfd *abfd, struct cgc64_ia64_link_hash_table *ia64_info)
{
  asection *got;
  bfd *dynobj;
  ...
  if (!got)
	...
	ia64_info->root.dynobj = dynobj = abfd;
      ...
      flags = get_cgc_backend_data (dynobj)->dynamic_sec_flags;
...
static asection *
get_pltoff (bfd *abfd, struct cgc64_ia64_link_hash_table *ia64_info)
{
  asection *pltoff;
  bfd *dynobj;
		   ...
		   asection *sec, bfd_boolean create)
{
  ...
  srel_name = (bfd_cgc_string_from_cgc_section
	       (abfd, cgc_cgcheader(abfd)->e_shstrndx,
		_bfd_cgc_single_rel_hdr (sec)->sh_name));
  if (srel_name == NULL)
    return NULL;
...
static bfd_boolean
count_dyn_reloc (bfd *abfd, struct cgc64_ia64_dyn_sym_info *dyn_i,
		 asection *srel, int type)
{
  struct cgc64_ia64_dyn_reloc_entry *rent;
  ...
  if (!rent)
    {
      rent = ((struct cgc64_ia64_dyn_reloc_entry *)
...
static bfd_boolean
cgc64_ia64_check_relocs (bfd *abfd, struct bfd_link_info *info,
			 asection *sec,
...
static bfd_boolean
allocate_global_data_got (struct cgc64_ia64_dyn_sym_info *dyn_i,
			  void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *)data;

  if ((dyn_i->want_got || dyn_i->want_gotx)
      && ! dyn_i->want_fptr
      && cgc64_ia64_dynamic_symbol_p (dyn_i->h))
     {
...
static bfd_boolean
allocate_global_fptr_got (struct cgc64_ia64_dyn_sym_info *dyn_i,
			  void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_local_got (struct cgc64_ia64_dyn_sym_info *dyn_i,
		    void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *) data;
...
static bfd_boolean
allocate_fptr (struct cgc64_ia64_dyn_sym_info *dyn_i, void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *) data;

  if (dyn_i->want_fptr)
    {
      struct cgc_link_hash_entry *h = dyn_i->h;
...
static bfd_boolean
allocate_plt_entries (struct cgc64_ia64_dyn_sym_info *dyn_i,
		      void * data ATTRIBUTE_UNUSED)
{
  if (dyn_i->want_plt)
	...
	while (h->root.type == bfd_link_hash_indirect
      ...
      if (cgc64_ia64_dynamic_symbol_p (h))
	{
	  dyn_i->want_pltoff = 1;
	}
      else
...
static bfd_boolean
allocate_plt2_entries (struct cgc64_ia64_dyn_sym_info *dyn_i,
		       void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_pltoff_entries (struct cgc64_ia64_dyn_sym_info *dyn_i,
			 void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *)data;
...
static bfd_boolean
allocate_dynrel_entries (struct cgc64_ia64_dyn_sym_info *dyn_i,
			 void * data)
{
  struct cgc64_ia64_allocate_data *x = (struct cgc64_ia64_allocate_data *)data;
  struct cgc64_ia64_link_hash_table *ia64_info;
  ...
  struct cgc64_ia64_link_hash_entry *h_ia64;

  ia64_info = cgc64_ia64_hash_table (x->info);
  if (ia64_info == NULL)
    return FALSE;

  /* Note that this can't be used in relation to FPTR relocs below.  */
  dynamic_symbol = cgc64_ia64_dynamic_symbol_p (dyn_i->h);

  shared = x->info->shared;
  resolved_zero = (dyn_i->h
		   && CGC_ST_VISIBILITY (dyn_i->h->other)
  ...
  if ((!resolved_zero
	  ...
	  && dyn_i->h->def_dynamic))
    {
      ...
      if (dyn_i->h != NULL && dyn_i->h->def_dynamic)
        {
          h_ia64 = (struct cgc64_ia64_link_hash_entry *) dyn_i->h;
          cgc_ia64_vms_tdata (h_ia64->shl)->fixups_off +=
            sizeof (Cgc64_External_VMS_IMAGE_FIXUP);
  ...
  if (ia64_info->rel_fptr_sec && dyn_i->want_fptr)
	...
	ia64_info->rel_fptr_sec->size += sizeof (Cgc64_External_Rela);
    }
...
static bfd_boolean
cgc64_ia64_adjust_dynamic_symbol (struct bfd_link_info *info ATTRIBUTE_UNUSED,
				  struct cgc_link_hash_entry *h)
{
...
static bfd_boolean
cgc64_ia64_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *info)
{
  struct cgc64_ia64_allocate_data data;
  struct cgc64_ia64_link_hash_table *ia64_info;
  asection *sec;
  bfd *dynobj;
  ...
  if (ia64_info->root.sgot)
    {
      data.ofs = 0;
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_global_data_got, &data);
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_global_fptr_got, &data);
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_local_got, &data);
  ...
  if (ia64_info->fptr_sec)
    {
      data.ofs = 0;
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_fptr, &data);
  ...
  data.ofs = 0;
  cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_plt_entries, &data);

  /* Align the pointer for the plt2 entries.  */
  data.ofs = (data.ofs + 31) & (bfd_vma) -32;

  cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_plt2_entries, &data);
  ...
  if (ia64_info->pltoff_sec)
    {
      data.ofs = 0;
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_pltoff_entries, &data);
  ...
  if (ia64_info->root.dynamic_sections_created)
    {
	 ...
	 required.  */
      cgc64_ia64_dyn_sym_traverse (ia64_info, allocate_dynrel_entries, &data);
    }
  ...
  for (sec = dynobj->sections; sec != NULL; sec = sec->next)
	...
	continue;
	 ...
	 bfd_cgc_size_dynamic_sections is called, and it is that
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      ...
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_IDENT, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_LINKTIME,
      ...
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_STRTAB_OFFSET, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_STRSZ, 0))
        return FALSE;

      /* PLTGOT  */
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_PLTGOT_SEG, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_PLTGOT_OFFSET, 0))
        return FALSE;

      /* Misc.  */
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_FPMODE, 0x9800000))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_LNKFLAGS,
      ...
      for (abfd = info->input_bfds; abfd; abfd = abfd->link_next)
        {
          ...
          if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_NEEDED_IDENT,
                                           cgc_ia64_vms_ident (abfd)))
            return FALSE;
          ...
          if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_FIXUP_NEEDED,
                                           shl_num))
            return FALSE;
          shl_num++;
          ...
          fixups_shl_off = cgc_ia64_vms_tdata (abfd)->fixups_off;
          cgc_ia64_vms_tdata (abfd)->fixups_off = fixups_off;

          if (!_bfd_cgc_add_dynamic_entry
              (info, DT_IA_64_VMS_FIXUP_RELA_CNT,
               fixups_shl_off / sizeof (Cgc64_External_VMS_IMAGE_FIXUP)))
            return FALSE;
          if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_FIXUP_RELA_OFF,
                                           fixups_off))
            return FALSE;
      ...
      /* Unwind.  */
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_UNWINDSZ, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_UNWIND_CODSEG, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_UNWIND_INFOSEG, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_UNWIND_OFFSET, 0))
        return FALSE;
      if (!_bfd_cgc_add_dynamic_entry (info, DT_IA_64_VMS_UNWIND_SEG, 0))
        return FALSE;

      if (!_bfd_cgc_add_dynamic_entry (info, DT_NULL, 0xdead))
            return FALSE;
      ...
      /* Note: one 'spare' (ie DT_NULL) entry is added by
         bfd_cgc_size_dynsym_hash_dynstr.  */
      dyn.d_tag = DT_IA_64_VMS_STRTAB_OFFSET;
      dyn.d_un.d_val = dynsec->size /* + sizeof (Cgc64_External_Dyn) */;
      ...
      cgc_ia64_vms_tdata (output_bfd)->needed_count = shl_num;
...
static void
cgc64_ia64_install_fixup (bfd *output_bfd,
                          struct cgc64_ia64_link_hash_table *ia64_info,
                          struct cgc_link_hash_entry *h,
...
{
  asection *relsec;
  Cgc64_External_VMS_IMAGE_FIXUP *fixup;
  struct cgc64_ia64_link_hash_entry *h_ia64;
  bfd_vma fixoff;
  ...
  h_ia64 = (struct cgc64_ia64_link_hash_entry *) h;
  fixoff = cgc_ia64_vms_tdata (h_ia64->shl)->fixups_off;
  cgc_ia64_vms_tdata (h_ia64->shl)->fixups_off +=
    sizeof (Cgc64_External_VMS_IMAGE_FIXUP);
  relsec = ia64_info->fixups_sec;

  fixup = (Cgc64_External_VMS_IMAGE_FIXUP *)(relsec->contents + fixoff);
  offset += sec->output_section->vma + sec->output_offset;

  /* FIXME: this is slow.  We should cache the last one used, or create a
     map.  */
  phdr = _bfd_cgc_find_segment_containing_section
    (output_bfd, sec->output_section);
  BFD_ASSERT (phdr != NULL);

  bfd_putl64 (offset - phdr->p_vaddr, fixup->fixup_offset);
  bfd_putl32 (type, fixup->type);
  bfd_putl32 (phdr - cgc_tdata (output_bfd)->phdr, fixup->fixup_seg);
  ...
  if (! done)
    {
      ...
      if (((info->shared
	    && (!dyn_i->h
		|| CGC_ST_VISIBILITY (dyn_i->h->other) == STV_DEFAULT
		|| dyn_i->h->root.type != bfd_link_hash_undefweak))
           || cgc64_ia64_dynamic_symbol_p (dyn_i->h))
	  && (!dyn_i->want_ltoff_fptr
	      || !info->pie
...
/* Called through qsort to sort the .IA_64.unwind section during a
   non-relocatable link.  Set cgc64_ia64_unwind_entry_compare_bfd
   to the output bfd so we can do proper endianness frobbing.  */

static bfd *cgc64_ia64_unwind_entry_compare_bfd;

static int
cgc64_ia64_unwind_entry_compare (const void * a, const void * b)
{
  bfd_vma av, bv;

  av = bfd_get_64 (cgc64_ia64_unwind_entry_compare_bfd, a);
  bv = bfd_get_64 (cgc64_ia64_unwind_entry_compare_bfd, b);
...
static bfd_boolean
cgc64_ia64_choose_gp (bfd *abfd, struct bfd_link_info *info, bfd_boolean final)
{
  bfd_vma min_vma = (bfd_vma) -1, max_vma = 0;
  bfd_vma min_short_vma = min_vma, max_short_vma = 0;
  struct cgc_link_hash_entry *gp;
  bfd_vma gp_val;
  asection *os;
  struct cgc64_ia64_link_hash_table *ia64_info;
  ...
  for (os = abfd->sections; os ; os = os->next)
	...
	continue;
      ...
      lo = os->vma;
      /* When this function is called from cgcNN_ia64_final_link
	 the correct value to use is os->size.  When called from
	 cgcNN_ia64_relax_section we are in the middle of section
  ...
  gp = cgc_link_hash_lookup (cgc_hash_table (info), "__gp", FALSE,
			     FALSE, FALSE);
...
static bfd_boolean
cgc64_ia64_final_link (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc64_ia64_link_hash_table *ia64_info;
  ...
  if (!info->relocatable)
    {
	 ...
	 need to adjust gp for it.  */
      ...
      if (! cgc64_ia64_choose_gp (abfd, info, TRUE))
	return FALSE;
  ...
  if (!info->relocatable)
    {
      asection *s = bfd_get_section_by_name (abfd, CGC_STRING_ia64_unwind);
      if (s)
	{
  ...
  if (unwind_output_sec)
    {
      cgc64_ia64_unwind_entry_compare_bfd = abfd;
      qsort (unwind_output_sec->contents,
	     (size_t) (unwind_output_sec->size / 24),
	     24,
	     cgc64_ia64_unwind_entry_compare);
...
static bfd_boolean
cgc64_ia64_relocate_section (bfd *output_bfd,
			     struct bfd_link_info *info,
			     bfd *input_bfd,
			     ...
			     asection **local_sections)
{
  ...
  if (info->relocatable)
    {
      ...
      flags = cgc_section_data(input_section)->this_hdr.sh_flags;
      flags &= SHF_IA_64_NORECOV;

      cgc_section_data(input_section->output_section)
  ...
  for (; rel < relend; ++rel)
      ...
      if (r_type > R_IA64_MAX_RELOC_CODE)
	{
      ...
      howto = ia64_cgc_lookup_howto (r_type);
      r_symndx = CGC64_R_SYM (rel->r_info);
      h = NULL;
      sym = NULL;
      sym_sec = NULL;
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (!info->relocatable
		...
		{
		  struct cgc64_ia64_dyn_sym_info *dynent;
		  unsigned int count;
	...
	continue;
      ...
      value += rel->r_addend;
      dynamic_symbol_p = cgc64_ia64_dynamic_symbol_p (h);
		  ...
		  (*_bfd_error_handler)
		    (_("%B: non-pic code with imm relocation against dynamic symbol `%s'"),
		     ...
		     h ? h->root.root.string
		       : bfd_cgc_sym_name (input_bfd, symtab_hdr, sym,
					   sym_sec));
		  ret_val = FALSE;
		  continue;
	...
	case R_IA64_LTV64LSB:
	  r = ia64_cgc_install_value (hit_addr, value, r_type);
	  break;
	...
	case R_IA64_SEGREL64LSB:
	    {
	      ...
	      Cgc_Internal_Phdr *p = _bfd_cgc_find_segment_containing_section
	...
	case R_IA64_IPLTLSB:
	  ...
	  ia64_cgc_install_value (hit_addr, value, r_type);
	  r = ia64_cgc_install_value (hit_addr + 8, gp_val, r_type);
	  break;
	...
	default:
	  {
		...
		if (is_cgc_hash_table (info->hash))
		  {
		    /* Relaxtion is always performed for CGC output.
...
static bfd_boolean
cgc64_ia64_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h,
				  Cgc_Internal_Sym *sym)
{
  ...
  if (dyn_i && dyn_i->want_plt)
      ...
      if (dyn_i->want_plt2)
	{
	  ...
	  ia64_cgc_install_value (loc, pltoff_addr - gp_val, R_IA64_IMM22);

	  /* Mark the symbol as undefined, rather than as defined in the
	     plt section.  Leave the value alone.  */
	  /* ??? We didn't redefine it in adjust_dynamic_symbol in the
	     first place.  But perhaps cgclink.c did some for us.  */
	  if (!h->def_regular)
...
static bfd_boolean
cgc64_ia64_finish_dynamic_sections (bfd *abfd,
				    struct bfd_link_info *info)
{
  struct cgc64_ia64_link_hash_table *ia64_info;
  bfd *dynobj;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      ...
      BFD_ASSERT (phdr != NULL);
      base_phdr = cgc_tdata (info->output_bfd)->phdr;
      gp_seg = phdr - base_phdr;
      gp_off = gp_val - phdr->p_vaddr;

      unwind_sec = bfd_get_section_by_name (abfd, CGC_STRING_ia64_unwind);
      if (unwind_sec != NULL)
        {
          asection *code_sec;

          phdr = _bfd_cgc_find_segment_containing_section (abfd, unwind_sec);
          BFD_ASSERT (phdr != NULL);
          unwind_seg = phdr - base_phdr;

          code_sec = bfd_get_section_by_name (abfd, "$CODE$");
          phdr = _bfd_cgc_find_segment_containing_section (abfd, code_sec);
          BFD_ASSERT (phdr != NULL);
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  bfd_cgc64_swap_dyn_out (abfd, &dyn, dyncon);
	}
    }
  ...
  /* Handle transfer addresses.  */
  {
    ...
    struct cgc64_vms_transfer *tfr;
    struct cgc_link_hash_entry *tfr3;

    tfr = (struct cgc64_vms_transfer *)tfr_sec->contents;
    bfd_putl32 (6 * 8, tfr->size);
    bfd_putl64 (tfr_sec->output_section->vma
                + tfr_sec->output_offset
                + 6 * 8, tfr->tfradr3);

    tfr3 = cgc_link_hash_lookup (cgc_hash_table (info), "CGC$TFRADR", FALSE,
                                 FALSE, FALSE);
...
/* CGC file flag handling:  */

/* Function to keep IA-64 specific file flags.  */
static bfd_boolean
cgc64_ia64_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
cgc64_ia64_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword out_flags;
  flagword in_flags;
  bfd_boolean ok = TRUE;
  ...
  if (!(in_flags & EF_IA_64_REDUCEDFP) && (out_flags & EF_IA_64_REDUCEDFP))
    cgc_cgcheader (obfd)->e_flags &= ~EF_IA_64_REDUCEDFP;
...
static bfd_boolean
cgc64_ia64_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE *file = (FILE *) ptr;
  flagword flags = cgc_cgcheader (abfd)->e_flags;
...
cgc64_ia64_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static const struct bfd_cgc_special_section cgc64_ia64_special_sections[] =
{
...
cgc64_ia64_object_p (bfd *abfd)
{
  ...
  for (sec = abfd->sections; sec != NULL; sec = sec->next)
	  ...
	  && CONST_STRNEQ (sec->name, ".gnu.linkonce.t."))
	{
	  ...
	  cgc_next_in_group (group) = sec;

	  cgc_group_name (sec) = name;
	  cgc_next_in_group (sec) = sec;
	  cgc_sec_group (sec) = group;

	  if (unwi)
	    {
	      cgc_group_name (unwi) = name;
	      cgc_next_in_group (unwi) = sec;
	      cgc_next_in_group (sec) = unwi;
	      cgc_sec_group (unwi) = group;
	    }

	   if (unw)
	     {
	       cgc_group_name (unw) = name;
	       if (unwi)
		 {
		   cgc_next_in_group (unw) = cgc_next_in_group (unwi);
		   cgc_next_in_group (unwi) = unw;
		 }
	       else
		 {
		   cgc_next_in_group (unw) = sec;
		   cgc_next_in_group (sec) = unw;
		 }
	       cgc_sec_group (unw) = group;
	     }

	   /* Fake SHT_GROUP section header.  */
	  cgc_section_data (group)->this_hdr.bfd_section = group;
	  cgc_section_data (group)->this_hdr.sh_type = SHT_GROUP;
	}
    }
  return TRUE;
}
...
static bfd_boolean
cgc64_vms_section_from_shdr (bfd *abfd,
			     ...
			     int shindex)
{
  ...
  switch (hdr->sh_type)
    ...
    case SHT_IA_64_EXT:
      if (strcmp (name, CGC_STRING_ia64_archext) != 0)
	return FALSE;
      break;
...
static bfd_boolean
cgc64_vms_object_p (bfd *abfd)
{
  Cgc_Internal_Ehdr *i_ehdrp = cgc_cgcheader (abfd);
  Cgc_Internal_Phdr *i_phdr = cgc_tdata (abfd)->phdr;
  unsigned int i;
...
static void
cgc64_vms_post_process_headers (bfd *abfd,
				struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  Cgc_Internal_Ehdr *i_ehdrp = cgc_cgcheader (abfd);

  i_ehdrp->e_ident[EI_OSABI] = CGCOSABI_OPENVMS;
  i_ehdrp->e_ident[EI_ABIVERSION] = 2;
}

static bfd_boolean
cgc64_vms_section_processing (bfd *abfd ATTRIBUTE_UNUSED,
			      Cgc_Internal_Shdr *hdr)
{
...
/* The final processing done just before writing out a VMS IA-64 CGC
   object file.  */

static void
cgc64_vms_final_write_processing (bfd *abfd,
				  bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  for (s = abfd->sections; s; s = s->next)
    {
      hdr = &cgc_section_data (s)->this_hdr;

      if (strcmp (bfd_get_section_name (abfd, hdr->bfd_section),
		  ".IA_64.unwind_info") == 0)
	unwind_info_sect_idx = cgc_section_data (s)->this_idx;
  ...
  if (! cgc_flags_init (abfd))
	...
	flags |= EF_IA_64_BE;
      if (bfd_get_mach (abfd) == bfd_mach_ia64_cgc64)
...
static bfd_boolean
cgc64_vms_write_shdrs_and_ehdr (bfd *abfd)
{
  unsigned char needed_count[8];

  if (!bfd_cgc64_write_shdrs_and_ehdr (abfd))
    return FALSE;

  bfd_putl64 (cgc_ia64_vms_tdata (abfd)->needed_count, needed_count);

  if (bfd_seek (abfd, sizeof (Cgc64_External_Ehdr), SEEK_SET) != 0
...
static bfd_boolean
cgc64_vms_close_and_cleanup (bfd *abfd)
{
...
static bfd_boolean
cgc64_vms_link_add_object_symbols (bfd *abfd, struct bfd_link_info *info)
{
  Cgc_Internal_Shdr *hdr;
  ...
  if (! dynamic)
	...
	{
	  if (! cgc64_ia64_create_dynamic_sections (abfd, info))
	    goto error_return;
	}
    }
	...
	abort ();
      ...
      if (! cgc64_ia64_create_dynamic_sections (htab->dynobj, info))
        goto error_return;
      ...
      if (s == NULL)
        {
          ...
          if (phdr == NULL)
            goto error_return;
          phnum = cgc_cgcheader (abfd)->e_phnum;
...
error_free_dyn:
  ...
  if (extsymcount != 0)
	...
	goto error_return;
	 ...
	 symbol.  */
      ...
      sym_hash = (struct cgc_link_hash_entry **) bfd_alloc (abfd, amt);
      if (sym_hash == NULL)
	goto error_free_sym;
  ...
  for (isym = isymbuf, isymend = isymbuf + extsymcount;
	...
	definition = TRUE;
      ...
      if (! bfd_is_und_section (sec))
        h = cgc_link_hash_lookup (htab, name, TRUE, FALSE, FALSE);
      else
        h = ((struct cgc_link_hash_entry *) bfd_wrapped_link_hash_lookup
	  ...
	  if (! dynamic)
	  ...
	  else
	    {
	      BFD_ASSERT (definition);
              h->def_dynamic = 1;
              h->dynindx = -2;
              ((struct cgc64_ia64_link_hash_entry *)h)->shl = abfd;
	    }
	}
    }
...
static bfd_boolean
cgc64_vms_link_add_archive_symbols (bfd *abfd, struct bfd_link_info *info)
{
  int pass;
  ...
  for (pundef = &info->hash->undefs; *pundef != NULL; pundef = next_pundef)
	...
	return FALSE;
      if (! cgc64_vms_link_add_object_symbols (element, info))
	return FALSE;
...
static bfd_boolean
cgc64_vms_bfd_link_add_symbols (bfd *abfd, struct bfd_link_info *info)
{
  switch (bfd_get_format (abfd))
    {
    case bfd_object:
      return cgc64_vms_link_add_object_symbols (abfd, info);
      break;
    case bfd_archive:
      return cgc64_vms_link_add_archive_symbols (abfd, info);
      break;
    default:
      bfd_set_error (bfd_error_wrong_format);
      return FALSE;
...
static bfd_boolean
cgc64_ia64_vms_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object
    (abfd, sizeof (struct cgc64_ia64_vms_obj_tdata), IA64_CGC_DATA);
}


/* Size-dependent data and functions.  */
static const struct cgc_size_info cgc64_ia64_vms_size_info = {
  sizeof (Cgc64_External_VMS_Ehdr),
  ...
  cgc64_vms_write_shdrs_and_ehdr,
...
#define CGC_ARCH			bfd_arch_ia64
#define CGC_MACHINE_CODE		EM_IA_64
#define CGC_MAXPAGESIZE			0x10000	/* 64KB */
#define CGC_COMMONPAGESIZE		0x200	/* 16KB */

#define cgc_backend_section_from_shdr \
	cgc64_ia64_section_from_shdr
#define cgc_backend_section_flags \
	cgc64_ia64_section_flags
#define cgc_backend_fake_sections \
	cgc64_ia64_fake_sections
#define cgc_backend_final_write_processing \
	cgc64_ia64_final_write_processing
#define cgc_backend_add_symbol_hook \
	cgc64_ia64_add_symbol_hook
#define cgc_info_to_howto \
	cgc64_ia64_info_to_howto

#define bfd_cgc64_bfd_reloc_type_lookup \
	ia64_cgc_reloc_type_lookup
#define bfd_cgc64_bfd_reloc_name_lookup \
	ia64_cgc_reloc_name_lookup
#define bfd_cgc64_bfd_is_local_label_name \
	cgc64_ia64_is_local_label_name
#define bfd_cgc64_bfd_relax_section \
	cgc64_ia64_relax_section
...
#define bfd_cgc64_bfd_link_hash_table_create \
	cgc64_ia64_hash_table_create
#define bfd_cgc64_bfd_link_hash_table_free \
	cgc64_ia64_hash_table_free
#define cgc_backend_create_dynamic_sections \
	cgc64_ia64_create_dynamic_sections
#define cgc_backend_check_relocs \
	cgc64_ia64_check_relocs
#define cgc_backend_adjust_dynamic_symbol \
	cgc64_ia64_adjust_dynamic_symbol
#define cgc_backend_size_dynamic_sections \
	cgc64_ia64_size_dynamic_sections
#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_relocate_section \
	cgc64_ia64_relocate_section
#define cgc_backend_finish_dynamic_symbol \
	cgc64_ia64_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
	cgc64_ia64_finish_dynamic_sections
#define bfd_cgc64_bfd_final_link \
	cgc64_ia64_final_link

#define bfd_cgc64_bfd_merge_private_bfd_data \
	cgc64_ia64_merge_private_bfd_data
#define bfd_cgc64_bfd_set_private_flags \
	cgc64_ia64_set_private_flags
#define bfd_cgc64_bfd_print_private_bfd_data \
	cgc64_ia64_print_private_bfd_data
...
#define cgc_backend_hide_symbol		cgc64_ia64_hash_hide_symbol
#define cgc_backend_fixup_symbol	_bfd_cgc_link_hash_fixup_symbol
#define cgc_backend_reloc_type_class	cgc64_ia64_reloc_type_class
#define cgc_backend_rela_normal		1
#define cgc_backend_special_sections	cgc64_ia64_special_sections
...
/* FIXME: PR 290: The Intel C compiler generates SHT_IA_64_UNWIND with
   ...
   compiler is fixed.   */
#define cgc_backend_link_order_error_handler NULL

/* VMS-specific vectors.  */

#undef  TARGET_LITTLE_SYM
#define TARGET_LITTLE_SYM		bfd_cgc64_ia64_vms_vec
#undef  TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME		"cgc64-ia64-vms"
...
#define cgc_backend_object_p cgc64_vms_object_p

#undef  cgc_backend_section_from_shdr
#define cgc_backend_section_from_shdr cgc64_vms_section_from_shdr

#undef  cgc_backend_post_process_headers
#define cgc_backend_post_process_headers cgc64_vms_post_process_headers

#undef  cgc_backend_section_processing
#define cgc_backend_section_processing cgc64_vms_section_processing

#undef  cgc_backend_final_write_processing
#define cgc_backend_final_write_processing cgc64_vms_final_write_processing

#undef  bfd_cgc64_close_and_cleanup
#define bfd_cgc64_close_and_cleanup cgc64_vms_close_and_cleanup

#undef  cgc_backend_section_from_bfd_section

#undef  cgc_backend_symbol_processing

#undef  cgc_backend_want_p_paddr_set_to_zero

#undef  CGC_OSABI
#define CGC_OSABI			CGCOSABI_OPENVMS

#undef  CGC_MAXPAGESIZE
#define CGC_MAXPAGESIZE			0x10000	/* 64KB */

#undef  cgc64_bed
#define cgc64_bed cgc64_ia64_vms_bed

#define cgc_backend_size_info cgc64_ia64_vms_size_info

/* Use VMS-style archives (in particular, don't use the standard coff
   archive format).  */
#define bfd_cgc64_archive_functions

#undef bfd_cgc64_archive_p
#define bfd_cgc64_archive_p _bfd_vms_lib_ia64_archive_p
#undef bfd_cgc64_write_archive_contents
#define bfd_cgc64_write_archive_contents _bfd_vms_lib_write_archive_contents
#undef bfd_cgc64_mkarchive
#define bfd_cgc64_mkarchive _bfd_vms_lib_ia64_mkarchive

#define bfd_cgc64_archive_slurp_armap \
  _bfd_vms_lib_slurp_armap
#define bfd_cgc64_archive_slurp_extended_name_table \
  _bfd_vms_lib_slurp_extended_name_table
#define bfd_cgc64_archive_construct_extended_name_table \
  _bfd_vms_lib_construct_extended_name_table
#define bfd_cgc64_archive_truncate_arname \
  _bfd_vms_lib_truncate_arname
#define bfd_cgc64_archive_write_armap \
  _bfd_vms_lib_write_armap
#define bfd_cgc64_archive_read_ar_hdr \
  _bfd_vms_lib_read_ar_hdr
#define bfd_cgc64_archive_write_ar_hdr \
  _bfd_vms_lib_write_ar_hdr
#define bfd_cgc64_archive_openr_next_archived_file \
  _bfd_vms_lib_openr_next_archived_file
#define bfd_cgc64_archive_get_elt_at_index \
  _bfd_vms_lib_get_elt_at_index
#define bfd_cgc64_archive_generic_stat_arch_elt \
  _bfd_vms_lib_generic_stat_arch_elt
#define bfd_cgc64_archive_update_armap_timestamp \
  _bfd_vms_lib_update_armap_timestamp

/* VMS link methods.  */
#undef  bfd_cgc64_bfd_link_add_symbols
#define bfd_cgc64_bfd_link_add_symbols 	cgc64_vms_bfd_link_add_symbols

#undef  cgc_backend_want_got_sym
#define cgc_backend_want_got_sym 	0

#undef  bfd_cgc64_mkobject
#define bfd_cgc64_mkobject		cgc64_ia64_vms_mkobject

/* Redefine to align segments on block size.  */
#undef  CGC_MAXPAGESIZE
#define CGC_MAXPAGESIZE			0x200 /* 512B  */
