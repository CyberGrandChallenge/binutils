/* BFD back-end for National Semiconductor's CRX CGC
...
static reloc_howto_type *cgc_crx_reloc_type_lookup
  (bfd *, bfd_reloc_code_real_type);
static void cgc_crx_info_to_howto
  (bfd *, arelent *, Cgc_Internal_Rela *);
static bfd_boolean cgc32_crx_relax_delete_bytes
  (struct bfd_link_info *, bfd *, asection *, bfd_vma, int);
static bfd_reloc_status_type crx_cgc_final_link_relocate
  (reloc_howto_type *, bfd *, bfd *, asection *,
   bfd_byte *, bfd_vma, bfd_vma, bfd_vma,
   struct bfd_link_info *, asection *, int);
static bfd_boolean cgc32_crx_relocate_section
  (bfd *, struct bfd_link_info *, bfd *, asection *, bfd_byte *,
   Cgc_Internal_Rela *, Cgc_Internal_Sym *, asection **);
static bfd_boolean cgc32_crx_relax_section
  (bfd *, asection *, struct bfd_link_info *, bfd_boolean *);
static bfd_byte * cgc32_crx_get_relocated_section_contents
...
static reloc_howto_type crx_cgc_howto_table[] =
{
...
cgc_crx_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   bfd_reloc_code_real_type code)
{
  ...
  for (i = 0; i < R_CRX_MAX; i++)
    if (code == crx_reloc_map[i].bfd_reloc_enum)
      return &crx_cgc_howto_table[crx_reloc_map[i].crx_reloc_type];
...
static reloc_howto_type *
cgc_crx_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (crx_cgc_howto_table) / sizeof (crx_cgc_howto_table[0]);
       i++)
    if (crx_cgc_howto_table[i].name != NULL
	&& strcasecmp (crx_cgc_howto_table[i].name, r_name) == 0)
      return &crx_cgc_howto_table[i];
...
static void
cgc_crx_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *cache_ptr,
		       Cgc_Internal_Rela *dst)
{
  unsigned int r_type = CGC32_R_TYPE (dst->r_info);
  BFD_ASSERT (r_type < (unsigned int) R_CRX_MAX);
  cache_ptr->howto = &crx_cgc_howto_table[r_type];
}

/* Perform a relocation as part of a final link.  */

static bfd_reloc_status_type
crx_cgc_final_link_relocate (reloc_howto_type *howto, bfd *input_bfd,
...
static bfd_boolean
cgc32_crx_relax_delete_bytes (struct bfd_link_info *link_info, bfd *abfd,
	  ...
	  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
	    {
	      ...
	      if (CGC32_R_TYPE (irel->r_info) != (int) R_CRX_SWITCH8
		  && CGC32_R_TYPE (irel->r_info) != (int) R_CRX_SWITCH16
		  && CGC32_R_TYPE (irel->r_info) != (int) R_CRX_SWITCH32)
		  continue;
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses cgc32_crx_relocate_section.  */

static bfd_byte *
cgc32_crx_get_relocated_section_contents (bfd *output_bfd,
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      ...
      for (isym = isymbuf, secpp = sections; isym < isymend; ++isym, ++secpp)
	{
	  ...
	  *secpp = isec;
      ...
      if (! cgc32_crx_relocate_section (output_bfd, link_info, input_bfd,
...
/* Relocate a CRX CGC section.  */

static bfd_boolean
cgc32_crx_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
  ...
  for (; rel < relend; rel++)
    {
      ...
      howto = crx_cgc_howto_table + (r_type);
	...
	continue;
      ...
      r = crx_cgc_final_link_relocate (howto, input_bfd, output_bfd,
					input_section,
...
static bfd_boolean
cgc32_crx_relax_section (bfd *abfd, asection *sec,
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
	 ...
	 this reloc.  */
      if (CGC32_R_TYPE (irel->r_info) != (int) R_CRX_REL32
	  && CGC32_R_TYPE (irel->r_info) != (int) R_CRX_REL16
	  && CGC32_R_TYPE (irel->r_info) != (int) R_CRX_REL24
	  && CGC32_R_TYPE (irel->r_info) != (int) R_CRX_IMM32)
	continue;
      ...
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CRX_REL32)
	{
	  ...
	  if ((long) value < 0x10000 && (long) value > -0x10002)
		...
		continue;
	      ...
	      if (!cgc32_crx_relax_delete_bytes (link_info, abfd, sec,
						   irel->r_offset + 2, 2))
		goto error_return;
	 ...
	 8bit pc-relative branch.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CRX_REL16)
	{
	  bfd_vma value = symval;
	 ...
	 an 8bit pc-relative cmp&branch.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CRX_REL24)
	{
	  bfd_vma value = symval;
	 ...
	 a 16bit immediate address.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_CRX_IMM32)
	{
	  bfd_vma value = symval;
...
#define TARGET_LITTLE_SYM		bfd_cgc32_crx_vec
#define TARGET_LITTLE_NAME		"cgc32-crx"
#define CGC_ARCH			bfd_arch_crx
#define CGC_MACHINE_CODE		EM_CRX
#define CGC_MAXPAGESIZE			0x1
#define cgc_symbol_leading_char		'_'

#define bfd_cgc32_bfd_reloc_type_lookup	cgc_crx_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup \
					cgc_crx_reloc_name_lookup
#define cgc_info_to_howto		cgc_crx_info_to_howto
#define cgc_info_to_howto_rel		0
#define cgc_backend_relocate_section	cgc32_crx_relocate_section
#define bfd_cgc32_bfd_relax_section	cgc32_crx_relax_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
				cgc32_crx_get_relocated_section_contents
