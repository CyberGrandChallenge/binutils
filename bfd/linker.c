			     ...
			     bfd_boolean generic_linker)
{
  ...
  if ((output_section->flags & (SEC_GROUP | SEC_LINKER_CREATED)) == SEC_GROUP
    ...
    {
      /* Group section contents are set by bfd_cgc_set_group_contents.  */
      if (!output_bfd->output_has_begun)
	{
	  /* FIXME: This hack ensures bfd_cgc_set_group_contents is called.  */
...
/* This is used on non-CGC inputs.  */
...
DESCRIPTION
	Search an cgc version script tree for symbol versioning
...
bfd_find_version_for_sym (struct bfd_cgc_version_tree *verdefs,
			  const char *sym_name,
			  bfd_boolean *hide)
{
  struct bfd_cgc_version_tree *t;
  struct bfd_cgc_version_tree *local_ver, *global_ver, *exist_ver;
  struct bfd_cgc_version_tree *star_local_ver, *star_global_ver;
  ...
  for (t = verdefs; t != NULL; t = t->next)
	...
	{
	  struct bfd_cgc_version_expr *d = NULL;
...
bfd_boolean
bfd_hide_sym_by_version (struct bfd_cgc_version_tree *verdefs,
			 const char *sym_name)
{
