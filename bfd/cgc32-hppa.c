/* BFD back-end for HP PA-RISC CGC files.
...
/* In order to gain some understanding of code in this file without
   ...
   Functions named cgc32_hppa_* are called by external routines, other
   functions are only called locally.  cgc32_hppa_* functions appear
   in this file more or less in the order in which they are called
   from external routines.  eg. cgc32_hppa_check_relocs is called
   early in the link process, cgc32_hppa_finish_dynamic_sections is
   one of the last functions.  */

/* We use two hash tables to hold information for linking PA CGC objects.

   The first is the cgc32_hppa_link_hash_table which is derived
   from the standard CGC linker hash table.  We use this as a place to
...
/* Variable names follow a coding style.
   ...
   cgc32_hppa_link_hash_table		"htab"
   cgc32_hppa_link_hash_entry		"hh"

   bfd_hash_table			"btab"
   bfd_hash_entry			"bh"

   bfd_hash_table containing stubs	"bstab"
   cgc32_hppa_stub_hash_entry		"hsh"

   cgc32_hppa_dyn_reloc_entry		"hdh"
...
struct cgc32_hppa_stub_hash_entry
{
  ...
  enum cgc32_hppa_stub_type stub_type;

  /* The symbol table entry, if any, that this was derived from.  */
  struct cgc32_hppa_link_hash_entry *hh;
...
struct cgc32_hppa_link_hash_entry
{
  struct cgc_link_hash_entry eh;

  /* A pointer to the most recently used stub hash entry against this
     symbol.  */
  struct cgc32_hppa_stub_hash_entry *hsh_cache;

  /* Used to count relocations for delayed sizing of relocation
     sections.  */
  struct cgc32_hppa_dyn_reloc_entry
  {
    /* Next relocation in the chain.  */
    struct cgc32_hppa_dyn_reloc_entry *hdh_next;
...
struct cgc32_hppa_link_hash_table
{
  /* The main hash table.  */
...
#define hppa_link_hash_table(p) \
  ...
  == HPPA32_CGC_DATA ? ((struct cgc32_hppa_link_hash_table *) ((p)->hash)) : NULL)

#define hppa_cgc_hash_entry(ent) \
  ((struct cgc32_hppa_link_hash_entry *)(ent))

#define hppa_stub_hash_entry(ent) \
  ((struct cgc32_hppa_stub_hash_entry *)(ent))

#define hppa_stub_hash_lookup(table, string, create, copy) \
  ((struct cgc32_hppa_stub_hash_entry *) \
   bfd_hash_lookup ((table), (string), (create), (copy)))

#define hppa_cgc_local_got_tls_type(abfd) \
  ((char *)(cgc_local_got_offsets (abfd) + (cgc_tdata (abfd)->symtab_hdr.sh_info * 2)))
		   ...
		   const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc32_hppa_stub_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc32_hppa_stub_hash_entry *hsh;
			...
			const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct cgc32_hppa_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
...
/* Create the derived linker hash table.  The PA CGC port uses the derived
   hash table to keep information specific to the PA CGC linker (without
   using static variables).  */

static struct bfd_link_hash_table *
cgc32_hppa_link_hash_table_create (bfd *abfd)
{
  struct cgc32_hppa_link_hash_table *htab;
  ...
  if (!_bfd_cgc_link_hash_table_init (&htab->etab, abfd, hppa_link_hash_newfunc,
				      sizeof (struct cgc32_hppa_link_hash_entry),
				      HPPA32_CGC_DATA))
    {
      free (htab);
...
static void
cgc32_hppa_link_hash_table_free (struct bfd_link_hash_table *btab)
{
  struct cgc32_hppa_link_hash_table *htab
    = (struct cgc32_hppa_link_hash_table *) btab;
...
hppa_stub_name (const asection *input_section,
		...
		const struct cgc32_hppa_link_hash_entry *hh,
		const Cgc_Internal_Rela *rela)
{
  ...
  if (hh)
	...
	sprintf (stub_name, "%08x_%x:%x+%x",
		 ...
		 (int) CGC32_R_SYM (rela->r_info) & 0xffffffff,
		 (int) rela->r_addend & 0xffffffff);
    }
  return stub_name;
}
...
static struct cgc32_hppa_stub_hash_entry *
hppa_get_stub_entry (const asection *input_section,
		     ...
		     struct cgc32_hppa_link_hash_table *htab)
{
  struct cgc32_hppa_stub_hash_entry *hsh_entry;
...
static enum cgc32_hppa_stub_type
		   ...
		   struct bfd_link_info *info)
{
  ...
  r_type = CGC32_R_TYPE (rela->r_info);
  ...
  switch (hsh->stub_type)
	...
	abort ();
      ...
      sym_value = (off
		   ...
		   - cgc_gp (htab->splt->output_section->owner));
...
/* Return nonzero if ABFD represents an HPPA CGC32 file.
   Additionally we set the default architecture and machine.  */

static bfd_boolean
cgc32_hppa_object_p (bfd *abfd)
{
  Cgc_Internal_Ehdr * i_ehdrp;
  unsigned int flags;

  i_ehdrp = cgc_cgcheader (abfd);
  if (strcmp (bfd_get_target (abfd), "cgc32-hppa-linux") == 0)
    {
  ...
  else if (strcmp (bfd_get_target (abfd), "cgc32-hppa-netbsd") == 0)
    {
      /* GCC on hppa-netbsd produces binaries with OSABI=NetBSD,
	 but the kernel produces corefiles with OSABI=SysV.  */
      if (i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_NETBSD &&
	  i_ehdrp->e_ident[EI_OSABI] != CGCOSABI_NONE) /* aka SYSV */
	return FALSE;
    }
  else
...
static bfd_boolean
cgc32_hppa_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc32_hppa_link_hash_table *htab;
  struct cgc_link_hash_entry *eh;
  ...
  eh = cgc_hash_table (info)->hgot;
  eh->forced_local = 0;
  eh->other = STV_DEFAULT;
  return bfd_cgc_link_record_dynamic_symbol (info, eh);
}

/* Copy the extra info we tack onto an cgc_link_hash_entry.  */

static void
cgc32_hppa_copy_indirect_symbol (struct bfd_link_info *info,
				 struct cgc_link_hash_entry *eh_dir,
				 struct cgc_link_hash_entry *eh_ind)
{
  struct cgc32_hppa_link_hash_entry *hh_dir, *hh_ind;

  hh_dir = hppa_cgc_hash_entry (eh_dir);
  hh_ind = hppa_cgc_hash_entry (eh_ind);

  if (hh_ind->dyn_relocs != NULL)
    {
      if (hh_dir->dyn_relocs != NULL)
	{
	  struct cgc32_hppa_dyn_reloc_entry **hdh_pp;
	  struct cgc32_hppa_dyn_reloc_entry *hdh_p;

	  /* Add reloc counts against the indirect sym to the direct sym
	     list.  Merge any entries against the same section.  */
	  for (hdh_pp = &hh_ind->dyn_relocs; (hdh_p = *hdh_pp) != NULL; )
	    {
	      struct cgc32_hppa_dyn_reloc_entry *hdh_q;
...
static int
cgc32_hppa_optimized_tls_reloc (struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
hppa32_cgc_local_refcounts (bfd *abfd)
{
  ...
  if (local_refcounts == NULL)
	...
	return NULL;
      ...
      memset (hppa_cgc_local_got_tls_type (abfd), GOT_UNKNOWN,
	      symtab_hdr->sh_info);
    }
...
static bfd_boolean
cgc32_hppa_check_relocs (bfd *abfd,
			 struct bfd_link_info *info,
			 asection *sec,
			 const Cgc_Internal_Rela *relocs)
{
  ...
  const Cgc_Internal_Rela *rela;
  const Cgc_Internal_Rela *rela_end;
  struct cgc32_hppa_link_hash_table *htab;
  asection *sreloc;
  ...
  for (rela = relocs; rela < rela_end; rela++)
	...
	PLT_PLABEL = 8
      ...
      r_symndx = CGC32_R_SYM (rela->r_info);

      if (r_symndx < symtab_hdr->sh_info)
	hh = NULL;
      else
	{
	  hh =  hppa_cgc_hash_entry (eh_syms[r_symndx - symtab_hdr->sh_info]);
      ...
      r_type = CGC32_R_TYPE (rela->r_info);
      r_type = cgc32_hppa_optimized_tls_reloc (info, r_type, hh == NULL);
	...
	case R_PARISC_DPREL21L:
	  if (info->shared)
		...
		(_("%B: relocation %s can not be used when making a shared object; recompile with -fPIC"),
		 abfd,
		 cgc_hppa_howto_table[r_type].name);
	      bfd_set_error (bfd_error_bad_value);
	      return FALSE;
	    }
	  /* Fall through.  */
	...
	case R_PARISC_GNU_VTINHERIT:
	  if (!bfd_cgc_gc_record_vtinherit (abfd, sec, &hh->eh, rela->r_offset))
	    return FALSE;
	  continue;
	...
	case R_PARISC_GNU_VTENTRY:
	  BFD_ASSERT (hh != NULL);
	  if (hh != NULL
	      && !bfd_cgc_gc_record_vtentry (abfd, sec, &hh->eh, rela->r_addend))
	    return FALSE;
	  continue;
      ...
      if (need_entry & NEED_GOT)
	{
	  ...
	  if (htab->sgot == NULL)
		...
		htab->etab.dynobj = abfd;
	      if (!cgc32_hppa_create_dynamic_sections (htab->etab.dynobj, info))
		return FALSE;
	    }
	  ...
	  else
	    {
	      if (hh != NULL)
	      ...
	      else
	        {
	          ...
	          local_got_refcounts = hppa32_cgc_local_refcounts (abfd);
	          if (local_got_refcounts == NULL)
		    return FALSE;
	          local_got_refcounts[r_symndx] += 1;

	          old_tls_type = hppa_cgc_local_got_tls_type (abfd) [r_symndx];
	        }
	      ...
	      if (old_tls_type != tls_type)
		    ...
		    hppa_cgc_local_got_tls_type (abfd) [r_symndx] = tls_type;
	        }
      ...
      if (need_entry & NEED_DYNREL)
	{
	  ...
	  if ((info->shared
	    ...
	    {
	      struct cgc32_hppa_dyn_reloc_entry *hdh_p;
	      struct cgc32_hppa_dyn_reloc_entry **hdh_head;
	      ...
	      else
		{
		  ...
		  hdh_head = (struct cgc32_hppa_dyn_reloc_entry **) vpp;
		}
...
static asection *
cgc32_hppa_gc_mark_hook (asection *sec,
			 struct bfd_link_info *info,
...
static bfd_boolean
cgc32_hppa_gc_sweep_hook (bfd *abfd,
			  ...
			  const Cgc_Internal_Rela *relocs)
{
  ...
  bfd_signed_vma *local_got_refcounts;
  bfd_signed_vma *local_plt_refcounts;
  const Cgc_Internal_Rela *rela, *relend;
  struct cgc32_hppa_link_hash_table *htab;
  ...
  for (rela = relocs; rela < relend; rela++)
      ...
      if (r_symndx >= symtab_hdr->sh_info)
	{
      ...
      r_type = CGC32_R_TYPE (rela->r_info);
      r_type = cgc32_hppa_optimized_tls_reloc (info, r_type, eh != NULL);
...
static bfd_boolean
cgc32_hppa_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  size_t size;
...
static bfd_boolean
cgc32_hppa_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  switch (note->descsz)
    {
      default:
	return FALSE;
      ...
      case 124:		/* Linux/hppa cgc_prpsinfo.  */
...
static void
cgc32_hppa_hide_symbol (struct bfd_link_info *info,
			struct cgc_link_hash_entry *eh,
			bfd_boolean force_local)
{
  ...
  if (! hppa_cgc_hash_entry (eh)->plabel
      && eh->type != STT_GNU_IFUNC)
    {
      eh->needs_plt = 0;
      eh->plt = cgc_hash_table (info)->init_plt_offset;
    }
}
...
static bfd_boolean
cgc32_hppa_adjust_dynamic_symbol (struct bfd_link_info *info,
				  struct cgc_link_hash_entry *eh)
{
  struct cgc32_hppa_link_hash_table *htab;
  asection *sec;
  ...
  return _bfd_cgc_adjust_dynamic_copy (eh, sec);
}
...
static bfd_boolean
allocate_plt_static (struct cgc_link_hash_entry *eh, void *inf)
{
  struct bfd_link_info *info;
  struct cgc32_hppa_link_hash_table *htab;
  struct cgc32_hppa_link_hash_entry *hh;
...
/* This function is called via cgc_link_hash_traverse to force
   ...
   cgc_adjust_dynamic_symbol.  */

static bfd_boolean
clobber_millicode_symbols (struct cgc_link_hash_entry *eh,
			   struct bfd_link_info *info)
{
  if (eh->type == STT_PARISC_MILLI
      && !eh->forced_local)
    {
      cgc32_hppa_hide_symbol (info, eh, TRUE);
    }
  return TRUE;
}
...
static bfd_boolean
cgc32_hppa_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *info)
{
  struct cgc32_hppa_link_hash_table *htab;
  bfd *dynobj;
  ...
  if (htab->etab.dynamic_sections_created)
      ...
      if (info->executable)
	{
      ...
      cgc_link_hash_traverse (&htab->etab,
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (sec = ibfd->sections; sec != NULL; sec = sec->next)
	{
	  ...
	  for (hdh_p = ((struct cgc32_hppa_dyn_reloc_entry *)
	...
	continue;
      ...
      local_tls_type = hppa_cgc_local_got_tls_type (ibfd);
      sec = htab->sgot;
      srel = htab->srelgot;
      for (; local_got < end_local_got; ++local_got)
	{
	  if (*local_got > 0)
		...
		sec->size += GOT_ENTRY_SIZE;
		  ...
		  if ((*local_tls_type & (GOT_TLS_GD | GOT_TLS_IE)) == (GOT_TLS_GD | GOT_TLS_IE))
		    srel->size += 2 * sizeof (Cgc32_External_Rela);
		  else if ((*local_tls_type & GOT_TLS_GD) == GOT_TLS_GD)
		    srel->size += sizeof (Cgc32_External_Rela);
	        }
	    }
  ...
  cgc_link_hash_traverse (&htab->etab, allocate_plt_static, info);
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
	...
	return FALSE;
	 ...
	 values later, in cgc32_hppa_finish_dynamic_sections, but we
...
int
cgc32_hppa_setup_section_lists (bfd *output_bfd, struct bfd_link_info *info)
{
  bfd *input_bfd;
  unsigned int bfd_count;
  ...
  bfd_size_type amt;
  struct cgc32_hppa_link_hash_table *htab = hppa_link_hash_table (info);
...
void
cgc32_hppa_next_input_section (struct bfd_link_info *info, asection *isec)
{
  struct cgc32_hppa_link_hash_table *htab = hppa_link_hash_table (info);
...
static void
group_sections (struct cgc32_hppa_link_hash_table *htab,
...
get_local_syms (bfd *output_bfd, bfd *input_bfd, struct bfd_link_info *info)
{
  ...
  for (bfd_indx = 0;
	...
	{
	  struct cgc_link_hash_entry **eh_syms;
	  struct cgc_link_hash_entry **eh_symend;
	  unsigned int symcount;

	  symcount = (symtab_hdr->sh_size / sizeof (Cgc32_External_Sym)
		      - symtab_hdr->sh_info);
	  eh_syms = (struct cgc_link_hash_entry **) cgc_sym_hashes (input_bfd);
	  eh_symend = (struct cgc_link_hash_entry **) (eh_syms + symcount);
	  ...
	  for (; eh_syms < eh_symend; eh_syms++)
	    {
	      ...
	      hh = hppa_cgc_hash_entry (*eh_syms);
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  if (r_indx < symtab_hdr->sh_info)
		  ...
		  else
		    {
		      ...
		      hh = hppa_cgc_hash_entry (cgc_sym_hashes (input_bfd)[e_indx]);
...
bfd_boolean
cgc32_hppa_set_gp (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (h != NULL
  ...
  else
    {
	 ...
	 the .plt section.  */
      sec = strcmp (bfd_get_target (abfd), "cgc32-hppa-netbsd") == 0
	  ? NULL : splt;
      if (sec != NULL)
	{
	  gp_val = sec->size;
      ...
      else
	{
	  ...
	  if (sec != NULL)
	    {
	      if (strcmp (bfd_get_target (abfd), "cgc32-hppa-netbsd") != 0)
		{
...
bfd_boolean
cgc32_hppa_build_stubs (struct bfd_link_info *info)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  struct cgc32_hppa_link_hash_table *htab;
...
static bfd_boolean
cgc32_hppa_final_link (bfd *abfd, struct bfd_link_info *info)
{
  /* Invoke the regular CGC linker to do all the work.  */
  ...
  return cgc_hppa_sort_unwind (abfd);
}
...
hppa_record_segment_addr (bfd *abfd, asection *section, void *data)
{
  ...
  htab = (struct cgc32_hppa_link_hash_table*) data;
  if (htab == NULL)
    return;
...
{
  int insn;
  unsigned int r_type = CGC32_R_TYPE (rela->r_info);
  unsigned int orig_r_type = r_type;
  reloc_howto_type *howto = cgc_hppa_howto_table + r_type;
  ...
  bfd_vma location;
  struct cgc32_hppa_stub_hash_entry *hsh = NULL;
  int val;
  ...
  switch (r_type)
    ...
    case R_PARISC_TLS_LDM14R:
    case R_PARISC_TLS_IE14R:
      value -= cgc_gp (input_section->output_section->owner);
      break;
...
static bfd_boolean
cgc32_hppa_relocate_section (bfd *output_bfd,
			     struct bfd_link_info *info,
			     bfd *input_bfd,
			     ...
			     asection **local_sections)
{
  ...
  for (; rela < relend; rela++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  relocation = _bfd_cgc_rela_local_sym (output_bfd, sym, &sym_sec, rela);
	}
      else
	{
	  struct cgc_link_hash_entry *eh;
		...
		if (info->shared)
		  {
		    ...
		    outrel.r_info = CGC32_R_INFO (0, R_PARISC_DIR32);
		    outrel.r_addend = relocation;
		    loc = sec->contents;
		    loc += sec->reloc_count++ * sizeof (Cgc32_External_Rela);
		  ...
		  if (info->shared)
		    {
			 ...
			 PLT entry.  */
		      ...
		      outrel.r_offset = (off
					 + htab->splt->output_offset
					 + htab->splt->output_section->vma);
		      outrel.r_info = CGC32_R_INFO (0, R_PARISC_IPLT);
		      outrel.r_addend = relocation;
		      loc = s->contents;
	...
	case R_PARISC_TLS_LDM14R:
	  {
		...
		outrel.r_offset = (off
				   ...
				   + htab->sgot->output_offset);
		outrel.r_addend = 0;
		outrel.r_info = CGC32_R_INFO (0, R_PARISC_TLS_DTPMOD32);
	...
	case R_PARISC_TLS_IE14R:
	  {
		...
		tls_type = hh->tls_type;
	      ...
	      {
		off = local_got_offsets[r_symndx];
		tls_type = hppa_cgc_local_got_tls_type (input_bfd)[r_symndx];
	      }
		...
		if (tls_type & GOT_TLS_GD)
		  {
			...
			outrel.r_offset = (cur_off
					   + htab->sgot->output_section->vma
					   + htab->sgot->output_offset);
			outrel.r_info = CGC32_R_INFO (indx,R_PARISC_TLS_DTPMOD32);
			outrel.r_addend = 0;
			...
			if (indx == 0)
			...
			else
			  {
			    bfd_put_32 (output_bfd, 0,
					htab->sgot->contents + cur_off + 4);
			    outrel.r_info = CGC32_R_INFO (indx, R_PARISC_TLS_DTPOFF32);
			    outrel.r_offset += 4;
			    bfd_cgc32_swap_reloca_out (output_bfd, &outrel,loc);
			    htab->srelgot->reloc_count++;
			    loc += sizeof (Cgc32_External_Rela);
		...
		if (tls_type & GOT_TLS_IE)
		  {
			...
			outrel.r_offset = (cur_off
					   + htab->sgot->output_section->vma
					   + htab->sgot->output_offset);
			outrel.r_info = CGC32_R_INFO (indx, R_PARISC_TLS_TPREL32);
      ...
      else
	{
      ...
      howto = cgc_hppa_howto_table + r_type;
...
static bfd_boolean
cgc32_hppa_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *eh,
				  Cgc_Internal_Sym *sym)
{
  ...
  if (eh->plt.offset != (bfd_vma) -1)
	  ...
	  || eh->root.type == bfd_link_hash_defweak)
	{
      ...
      if (eh->dynindx != -1)
	{
	  rela.r_info = CGC32_R_INFO (eh->dynindx, R_PARISC_IPLT);
	  rela.r_addend = 0;
	}
      else
	{
	  ...
	  rela.r_info = CGC32_R_INFO (0, R_PARISC_IPLT);
	  rela.r_addend = value;
	}

      loc = htab->srelplt->contents;
      loc += htab->srelplt->reloc_count++ * sizeof (Cgc32_External_Rela);
      bfd_cgc32_swap_reloca_out (htab->splt->output_section->owner, &rela, loc);
  ...
  if (eh->got.offset != (bfd_vma) -1
      && (hppa_cgc_hash_entry (eh)->tls_type & GOT_TLS_GD) == 0
      && (hppa_cgc_hash_entry (eh)->tls_type & GOT_TLS_IE) == 0)
    {
	 ...
	 relocate_section function.  */
	  ...
	  && eh->def_regular)
	{
	  rela.r_info = CGC32_R_INFO (0, R_PARISC_DIR32);
      ...
      else
	{
	  ...
	  rela.r_info = CGC32_R_INFO (eh->dynindx, R_PARISC_DIR32);
	  rela.r_addend = 0;
	}
  ...
  if (eh->needs_copy)
	...
	abort ();
      ...
      rela.r_offset = (eh->root.u.def.value
		      ...
		      + eh->root.u.def.section->output_section->vma);
      rela.r_addend = 0;
      rela.r_info = CGC32_R_INFO (eh->dynindx, R_PARISC_COPY);
      loc = sec->contents + sec->reloc_count++ * sizeof (Cgc32_External_Rela);
      bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
    }
...
cgc32_hppa_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
			     const asection *rel_sec ATTRIBUTE_UNUSED,
			     const Cgc_Internal_Rela *rela)
{
  /* Handle TLS relocs first; we don't want them to be marked
     relative by the "if (CGC32_R_SYM (rela->r_info) == STN_UNDEF)"
     check below.  */
  ...
  if (CGC32_R_SYM (rela->r_info) == STN_UNDEF)
...
static bfd_boolean
cgc32_hppa_finish_dynamic_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  bfd *dynobj;
  struct cgc32_hppa_link_hash_table *htab;
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  switch (dyn.d_tag)
	    ...
	    case DT_PLTGOT:
	      ...
	      dyn.d_un.d_ptr = cgc_gp (output_bfd);
	      break;
  ...
  if (sgot != NULL && sgot->size != 0)
    {
	 ...
	 We use it to point to our dynamic section, if we have one.  */
      ...
      /* Set .got entry size.  */
      cgc_section_data (sgot->output_section)
  ...
  if (htab->splt != NULL && htab->splt->size != 0)
    {
      /* Set plt entry size.  */
      cgc_section_data (htab->splt->output_section)
...
/* Called when writing out an object file to decide the type of a
   symbol.  */
static int
cgc32_hppa_cgc_get_symbol_type (Cgc_Internal_Sym *cgc_sym, int type)
{
...
#define bfd_cgc32_bfd_is_local_label_name    cgc_hppa_is_local_label_name
#define bfd_cgc32_bfd_reloc_type_lookup	     cgc_hppa_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup      cgc_hppa_reloc_name_lookup
#define cgc_info_to_howto		     cgc_hppa_info_to_howto
#define cgc_info_to_howto_rel		     cgc_hppa_info_to_howto_rel

/* Stuff for the BFD linker.  */
#define bfd_cgc32_bfd_final_link	     cgc32_hppa_final_link
#define bfd_cgc32_bfd_link_hash_table_create cgc32_hppa_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free   cgc32_hppa_link_hash_table_free
#define cgc_backend_adjust_dynamic_symbol    cgc32_hppa_adjust_dynamic_symbol
#define cgc_backend_copy_indirect_symbol     cgc32_hppa_copy_indirect_symbol
#define cgc_backend_check_relocs	     cgc32_hppa_check_relocs
#define cgc_backend_create_dynamic_sections  cgc32_hppa_create_dynamic_sections
#define cgc_backend_fake_sections	     cgc_hppa_fake_sections
#define cgc_backend_relocate_section	     cgc32_hppa_relocate_section
#define cgc_backend_hide_symbol		     cgc32_hppa_hide_symbol
#define cgc_backend_finish_dynamic_symbol    cgc32_hppa_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections  cgc32_hppa_finish_dynamic_sections
#define cgc_backend_size_dynamic_sections    cgc32_hppa_size_dynamic_sections
#define cgc_backend_init_index_section	     _bfd_cgc_init_1_index_section
#define cgc_backend_gc_mark_hook	     cgc32_hppa_gc_mark_hook
#define cgc_backend_gc_sweep_hook	     cgc32_hppa_gc_sweep_hook
#define cgc_backend_grok_prstatus	     cgc32_hppa_grok_prstatus
#define cgc_backend_grok_psinfo		     cgc32_hppa_grok_psinfo
#define cgc_backend_object_p		     cgc32_hppa_object_p
#define cgc_backend_final_write_processing   cgc_hppa_final_write_processing
#define cgc_backend_post_process_headers     _bfd_cgc_set_osabi
#define cgc_backend_get_symbol_type	     cgc32_hppa_cgc_get_symbol_type
#define cgc_backend_reloc_type_class	     cgc32_hppa_reloc_type_class
...
#define TARGET_BIG_SYM		bfd_cgc32_hppa_vec
#define TARGET_BIG_NAME		"cgc32-hppa"
#define CGC_ARCH		bfd_arch_hppa
#define CGC_TARGET_ID		HPPA32_CGC_DATA
#define CGC_MACHINE_CODE	EM_PARISC
#define CGC_MAXPAGESIZE		0x1000
#define CGC_OSABI		CGCOSABI_HPUX
#define cgc32_bed		cgc32_hppa_hpux_bed

#include "cgc32-target.h"

#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM		bfd_cgc32_hppa_linux_vec
#undef TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-hppa-linux"
#undef CGC_OSABI
#define CGC_OSABI		CGCOSABI_GNU
#undef cgc32_bed
#define cgc32_bed		cgc32_hppa_linux_bed

#include "cgc32-target.h"

#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM		bfd_cgc32_hppa_nbsd_vec
#undef TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-hppa-netbsd"
#undef CGC_OSABI
#define CGC_OSABI		CGCOSABI_NETBSD
#undef cgc32_bed
#define cgc32_bed		cgc32_hppa_netbsd_bed
