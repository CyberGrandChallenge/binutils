/* AVR-specific support for 32-bit CGC
...
/* We use two hash tables to hold information for linking avr objects.
   ...
   The first is the cgc32_avr_link_hash_table which is derived from the
   stanard CGC linker hash table.  We use this as a place to attach the other
...
struct cgc32_avr_stub_hash_entry
{
...
struct cgc32_avr_link_hash_table
{
  /* The main hash table.  */
  ...
  /* Usually 0, unless we are generating code for a bootloader.  Will
     be initialized by cgc32_avr_size_stubs to the vma offset of the
     output section associated with the stub section.  */
  bfd_vma vector_base;

  /* Assorted information used by cgc32_avr_size_stubs.  */
...
#define avr_link_hash_table(p) \
  ...
  == AVR_CGC_DATA ? ((struct cgc32_avr_link_hash_table *) ((p)->hash)) : NULL)

#define avr_stub_hash_entry(ent) \
  ((struct cgc32_avr_stub_hash_entry *)(ent))

#define avr_stub_hash_lookup(table, string, create, copy) \
  ((struct cgc32_avr_stub_hash_entry *) \
   bfd_hash_lookup ((table), (string), (create), (copy)))

static reloc_howto_type cgc_avr_howto_table[] =
{
...
/* Map BFD reloc types to AVR CGC reloc types.  */
                   ...
                   const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
                                 sizeof (struct cgc32_avr_stub_hash_entry));
      if (entry == NULL)
        return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc32_avr_stub_hash_entry *hsh;
...
static struct bfd_hash_entry *
cgc32_avr_link_hash_newfunc (struct bfd_hash_entry * entry,
			     struct bfd_hash_table * table,
			     const char * string)
{
  return _bfd_cgc_link_hash_newfunc (entry, table, string);
}

/* Create the derived linker hash table.  The AVR CGC port uses the derived
   hash table to keep information specific to the AVR CGC linker (without
   using static variables).  */

static struct bfd_link_hash_table *
cgc32_avr_link_hash_table_create (bfd *abfd)
{
  struct cgc32_avr_link_hash_table *htab;
...
static void
cgc32_avr_link_hash_table_free (struct bfd_link_hash_table *btab)
{
  struct cgc32_avr_link_hash_table *htab
    = (struct cgc32_avr_link_hash_table *) btab;
				 ...
				 bfd_reloc_code_real_type code)
{
  ...
  for (i = 0;
    ...
    if (avr_reloc_map[i].bfd_reloc_val == code)
      return &cgc_avr_howto_table[avr_reloc_map[i].cgc_reloc_val];
				 ...
				 const char *r_name)
{
  ...
  for (i = 0;
       i < sizeof (cgc_avr_howto_table) / sizeof (cgc_avr_howto_table[0]);
       i++)
    if (cgc_avr_howto_table[i].name != NULL
	&& strcasecmp (cgc_avr_howto_table[i].name, r_name) == 0)
      return &cgc_avr_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an AVR CGC reloc.  */
			...
			Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = &cgc_avr_howto_table[r_type];
}
...
/* Relocate an AVR CGC section.  */

static bfd_boolean
cgc32_avr_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
			    ...
			    asection **local_sections)
{
  ...
  struct cgc32_avr_link_hash_table * htab = avr_link_hash_table (info);
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = cgc_avr_howto_table + r_type;
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
...
/* The final processing done just before writing out a AVR CGC object
   file.  This gets the AVR architecture right based on the machine
   number.  */

static void
bfd_cgc_avr_final_write_processing (bfd *abfd,
				    bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  cgc_cgcheader (abfd)->e_machine = EM_AVR;
  cgc_cgcheader (abfd)->e_flags &= ~ EF_AVR_MACH;
  cgc_cgcheader (abfd)->e_flags |= val;
  cgc_cgcheader (abfd)->e_flags |= EF_AVR_LINKRELAX_PREPARED;
}

/* Set the right machine number.  */

static bfd_boolean
cgc32_avr_object_p (bfd *abfd)
{
  unsigned int e_set = bfd_mach_avr2;

  if (cgc_cgcheader (abfd)->e_machine == EM_AVR
      || cgc_cgcheader (abfd)->e_machine == EM_AVR_OLD)
    {
      int e_mach = cgc_cgcheader (abfd)->e_flags & EF_AVR_MACH;
...
static bfd_boolean
cgc32_avr_relax_delete_bytes (bfd *abfd,
                              asection *sec,
                              bfd_vma addr,
                              int count)
{
...

  {
    ...
    for (isec = abfd->sections; isec; isec = isec->next)
	 ...
	 continue;
       ...
       irel = cgc_section_data (isec)->relocs;
       /* PR 12161: Read in the relocs for this section if necessary.  */
       if (irel == NULL)
         irel = _bfd_cgc_link_read_relocs (abfd, isec, NULL, NULL, TRUE);
...
static bfd_boolean
cgc32_avr_relax_section (bfd *abfd,
			 asection *sec,
                         ...
                         bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  if ((!htab->no_stubs) && (sec == htab->stub_sec))
    {
	 ...
	 Let's calculate the size needed again.  */
      ...
      cgc32_avr_size_stubs (htab->stub_sec->output_section->owner,
                            link_info, FALSE);
  ...
  if (!(cgc_cgcheader (abfd)->e_flags & EF_AVR_LINKRELAX_PREPARED))
    return TRUE;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      if (   CGC32_R_TYPE (irel->r_info) != R_AVR_13_PCREL
	     && CGC32_R_TYPE (irel->r_info) != R_AVR_7_PCREL
	     && CGC32_R_TYPE (irel->r_info) != R_AVR_CALL)
        continue;
      ...
      /* For simplicity of coding, we are going to modify the section
         ...
         the linker is run.  */
      switch (CGC32_R_TYPE (irel->r_info))
        {
	...
	case R_AVR_CALL:
            ...
            if (distance_short_enough)
              {
                ...
                if (!shrinkable)
                ...
                else
                  {
                    ...
                    if (!cgc32_avr_relax_delete_bytes (abfd, sec,
                                                       irel->r_offset + 2, 2))
                      goto error_return;
		...
		if ((0x95 == next_insn_msb) && (0x08 == next_insn_lsb))
			...
			struct bfd_section *isec;
                        ...
                        sec_shndx =
			  _bfd_cgc_section_from_bfd_section (abfd, sec);
			...
			for (isec = abfd->sections; isec && deleting_ret_is_safe; isec = isec->next)
			  {
			    ...
			    rel = cgc_section_data (isec)->relocs;
			    if (rel == NULL)
			      rel = _bfd_cgc_link_read_relocs (abfd, isec, NULL, NULL, TRUE);
				...
				if (isymbuf == NULL && symtab_hdr->sh_info != 0)
				  {
				    isymbuf = (Cgc_Internal_Sym *)
				      symtab_hdr->contents;
				    if (isymbuf == NULL)
				      isymbuf = bfd_cgc_get_cgc_syms
					(abfd,
					 symtab_hdr,
					 symtab_hdr->sh_info, 0,
					 NULL, NULL, NULL);
				...
				if (CGC32_R_SYM (rel->r_info)
				    < symtab_hdr->sh_info)
				  {
				    ...
				    sym_sec = bfd_section_from_cgc_index
				      (abfd, isym->st_shndx);
				    symval = isym->st_value;
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses cgc32_avr_relocate_section.
...
static bfd_byte *
cgc32_avr_get_relocated_section_contents (bfd *output_bfd,
                                          struct bfd_link_info *link_info,
                                          ...
                                          asymbol **symbols)
{
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      && input_section->reloc_count > 0)
    {
      ...
      if (! cgc32_avr_relocate_section (output_bfd, link_info, input_bfd,
...
static struct cgc32_avr_stub_hash_entry *
...
void
cgc32_avr_setup_params (struct bfd_link_info *info,
                        bfd *avr_stub_bfd,
...
int
cgc32_avr_setup_section_lists (bfd *output_bfd,
                               struct bfd_link_info *info)
{
  bfd *input_bfd;
  unsigned int bfd_count;
...
get_local_syms (bfd *input_bfd, struct bfd_link_info *info)
{
  ...
  amt = sizeof (Cgc_Internal_Sym *) * htab->bfd_count;
...
bfd_boolean
cgc32_avr_size_stubs (bfd *output_bfd,
                      struct bfd_link_info *info,
                      bfd_boolean is_prealloc_run)
{
  ...
  if (ADD_DUMMY_STUBS_FOR_DEBUGGING)
    {
      struct cgc32_avr_stub_hash_entry *test;
...
/* Build all the stubs associated with the current output file.  The
   ...
   functions here.  This function is called via hppacgc_finish in the
   linker.  */

bfd_boolean
cgc32_avr_build_stubs (struct bfd_link_info *info)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  struct cgc32_avr_link_hash_table *htab;
...
#define CGC_ARCH		bfd_arch_avr
#define CGC_TARGET_ID		AVR_CGC_DATA
#define CGC_MACHINE_CODE	EM_AVR
#define CGC_MACHINE_ALT1	EM_AVR_OLD
#define CGC_MAXPAGESIZE		1

#define TARGET_LITTLE_SYM       bfd_cgc32_avr_vec
#define TARGET_LITTLE_NAME	"cgc32-avr"

#define bfd_cgc32_bfd_link_hash_table_create cgc32_avr_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free   cgc32_avr_link_hash_table_free

#define cgc_info_to_howto	             avr_info_to_howto_rela
#define cgc_info_to_howto_rel	             NULL
#define cgc_backend_relocate_section         cgc32_avr_relocate_section
#define cgc_backend_can_gc_sections          1
#define cgc_backend_rela_normal		     1
#define cgc_backend_final_write_processing \
					bfd_cgc_avr_final_write_processing
#define cgc_backend_object_p		cgc32_avr_object_p

#define bfd_cgc32_bfd_relax_section cgc32_avr_relax_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
                                        cgc32_avr_get_relocated_section_contents
