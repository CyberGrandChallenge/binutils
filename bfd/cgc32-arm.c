...
#define SWAP_RELOC_IN(HTAB) \
  ((HTAB)->use_rel \
   ? bfd_cgc32_swap_reloc_in \
   : bfd_cgc32_swap_reloca_in)
...
#define cgc_info_to_howto_rel           cgc32_arm_info_to_howto

#define ARM_CGC_ABI_VERSION		0
#define ARM_CGC_OS_ABI_VERSION		CGCOSABI_ARM

/* The Adjusted Place, as defined by AACGC.  */
#define Pa(X) ((X) & 0xfffffffc)

static bfd_boolean cgc32_arm_write_section (bfd *output_bfd,
					    struct bfd_link_info *link_info,
					    asection *sec,
					    bfd_byte *contents);

/* Note: code such as cgc32_arm_reloc_type_lookup expect to use e.g.
   R_ARM_PC24 as an index into this, and find the R_ARM_PC24 HOWTO
   in that slot.  */

static reloc_howto_type cgc32_arm_howto_table_1[] =
{
  /* No relocation.  */
  ...
  /* 8 bit absolute - R_ARM_LDR_PC_G0 in AACGC */
...
/* 160 onwards: */
static reloc_howto_type cgc32_arm_howto_table_2[1] =
{
...
static reloc_howto_type cgc32_arm_howto_table_3[4] =
{
...
cgc32_arm_howto_from_type (unsigned int r_type)
{
  if (r_type < ARRAY_SIZE (cgc32_arm_howto_table_1))
    return &cgc32_arm_howto_table_1[r_type];

  if (r_type == R_ARM_IRELATIVE)
    return &cgc32_arm_howto_table_2[r_type - R_ARM_IRELATIVE];

  if (r_type >= R_ARM_RREL32
      && r_type < R_ARM_RREL32 + ARRAY_SIZE (cgc32_arm_howto_table_3))
    return &cgc32_arm_howto_table_3[r_type - R_ARM_RREL32];

  return NULL;
}

static void
cgc32_arm_info_to_howto (bfd * abfd ATTRIBUTE_UNUSED, arelent * bfd_reloc,
			 Cgc_Internal_Rela * cgc_reloc)
{
  unsigned int r_type;

  r_type = CGC32_R_TYPE (cgc_reloc->r_info);
  bfd_reloc->howto = cgc32_arm_howto_from_type (r_type);
}

struct cgc32_arm_reloc_map
  {
    bfd_reloc_code_real_type  bfd_reloc_val;
    unsigned char             cgc_reloc_val;
  };

/* All entries in this list must also be present in cgc32_arm_howto_table.  */
static const struct cgc32_arm_reloc_map cgc32_arm_reloc_map[] =
  {
...
static reloc_howto_type *
cgc32_arm_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			     bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < ARRAY_SIZE (cgc32_arm_reloc_map); i ++)
    if (cgc32_arm_reloc_map[i].bfd_reloc_val == code)
      return cgc32_arm_howto_from_type (cgc32_arm_reloc_map[i].cgc_reloc_val);

  return NULL;
}

static reloc_howto_type *
cgc32_arm_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			     const char *r_name)
{
  unsigned int i;

  for (i = 0; i < ARRAY_SIZE (cgc32_arm_howto_table_1); i++)
    if (cgc32_arm_howto_table_1[i].name != NULL
	&& strcasecmp (cgc32_arm_howto_table_1[i].name, r_name) == 0)
      return &cgc32_arm_howto_table_1[i];

  for (i = 0; i < ARRAY_SIZE (cgc32_arm_howto_table_2); i++)
    if (cgc32_arm_howto_table_2[i].name != NULL
	&& strcasecmp (cgc32_arm_howto_table_2[i].name, r_name) == 0)
      return &cgc32_arm_howto_table_2[i];

  for (i = 0; i < ARRAY_SIZE (cgc32_arm_howto_table_3); i++)
    if (cgc32_arm_howto_table_3[i].name != NULL
	&& strcasecmp (cgc32_arm_howto_table_3[i].name, r_name) == 0)
      return &cgc32_arm_howto_table_3[i];
...
static bfd_boolean
cgc32_arm_nabi_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  size_t size;
...
static bfd_boolean
cgc32_arm_nabi_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  switch (note->descsz)
    {
      default:
	return FALSE;
      ...
      case 124:		/* Linux/ARM cgc_prpsinfo.  */
...
static char *
cgc32_arm_nabi_write_core_note (bfd *abfd, char *buf, int *bufsiz,
				int note_type, ...)
{
  switch (note_type)
    {
...
#define TARGET_LITTLE_SYM               bfd_cgc32_littlearm_vec
#define TARGET_LITTLE_NAME              "cgc32-littlearm"
#define TARGET_BIG_SYM                  bfd_cgc32_bigarm_vec
#define TARGET_BIG_NAME                 "cgc32-bigarm"

#define cgc_backend_grok_prstatus	cgc32_arm_nabi_grok_prstatus
#define cgc_backend_grok_psinfo		cgc32_arm_nabi_grok_psinfo
#define cgc_backend_write_core_note	cgc32_arm_nabi_write_core_note
...
#define INTERWORK_FLAG(abfd)  \
  (EF_ARM_EABI_VERSION (cgc_cgcheader (abfd)->e_flags) >= EF_ARM_EABI_VER4 \
  || (cgc_cgcheader (abfd)->e_flags & EF_ARM_INTERWORK) \
...
/* The name of the dynamic interpreter.  This is put in the .interp
   section.  */
#define CGC_DYNAMIC_INTERPRETER     "/usr/lib/ld.so.1"
...
/* The first entry in a procedure linkage table looks like
   ...
   linker first.  */
static const bfd_vma cgc32_arm_plt0_entry [] =
{
...
/* Subsequent entries in a procedure linkage table look like
   this.  */
static const bfd_vma cgc32_arm_plt_entry [] =
{
...
static const bfd_vma cgc32_arm_vxworks_exec_plt0_entry[] =
{
...
static const bfd_vma cgc32_arm_vxworks_exec_plt_entry[] =
{
  0xe59fc000,         /* ldr    ip,[pc]			*/
  ...
  0xe59fc000,         /* ldr    ip,[pc]			*/
  0xea000000,         /* b      _PLT				*/
  0x00000000,         /* .long  @pltindex*sizeof(Cgc32_Rela)	*/
};

/* The format of entries in a VxWorks shared library.  */
static const bfd_vma cgc32_arm_vxworks_shared_plt_entry[] =
{
  0xe59fc000,         /* ldr    ip,[pc]			*/
...
static const bfd_vma cgc32_arm_plt_thumb_stub [] =
{
  0x4778,		/* bx pc */
  0x46c0		/* nop   */
};
...
/* The entries in a PLT when using a DLL-based target with multiple
   address spaces.  */
static const bfd_vma cgc32_arm_symbian_plt_entry [] =
{
...
/* The first entry in a procedure linkage table looks like
   ...
   called before the relocation has been set up calls the dynamic
   linker first.  */
static const bfd_vma cgc32_arm_nacl_plt0_entry [] =
{
  /* First bundle: */
...
static const bfd_vma cgc32_arm_nacl_plt_entry [] =
{
...
static const insn_sequence cgc32_arm_stub_long_branch_any_any[] =
{
...
/* V4T Arm -> Thumb long branch stub. Used on V4T where blx is not
   available.  */
static const insn_sequence cgc32_arm_stub_long_branch_v4t_arm_thumb[] =
{
...
static const insn_sequence cgc32_arm_stub_long_branch_thumb_only[] =
{
...
/* V4T Thumb -> Thumb long branch stub. Using the stack is not
   allowed.  */
static const insn_sequence cgc32_arm_stub_long_branch_v4t_thumb_thumb[] =
{
...
/* V4T Thumb -> ARM long branch stub. Used on V4T where blx is not
   available.  */
static const insn_sequence cgc32_arm_stub_long_branch_v4t_thumb_arm[] =
{
  THUMB16_INSN (0x4778),             /* bx   pc */
  THUMB16_INSN (0x46c0),             /* nop   */
  ARM_INSN (0xe51ff004),             /* ldr   pc, [pc, #-4] */
...
static const insn_sequence cgc32_arm_stub_short_branch_v4t_thumb_arm[] =
{
  THUMB16_INSN (0x4778),             /* bx   pc */
  THUMB16_INSN (0x46c0),             /* nop   */
...
static const insn_sequence cgc32_arm_stub_long_branch_any_arm_pic[] =
{
...
/* ARM/Thumb -> Thumb long branch stub, PIC.  On V5T and above, use
   ...
   ARMv7).  */
static const insn_sequence cgc32_arm_stub_long_branch_any_thumb_pic[] =
{
...
static const insn_sequence cgc32_arm_stub_long_branch_v4t_arm_thumb_pic[] =
{
  ARM_INSN (0xe59fc004),             /* ldr   ip, [pc, #4] */
  ARM_INSN (0xe08fc00c),             /* add   ip, pc, ip */
  ARM_INSN (0xe12fff1c),             /* bx    ip */
...
static const insn_sequence cgc32_arm_stub_long_branch_v4t_thumb_arm_pic[] =
{
  THUMB16_INSN (0x4778),             /* bx   pc */
  THUMB16_INSN (0x46c0),             /* nop  */
  ARM_INSN (0xe59fc000),             /* ldr  ip, [pc, #0] */
...
/* Thumb -> Thumb long branch stub, PIC. Used on M-profile
   architectures.  */
static const insn_sequence cgc32_arm_stub_long_branch_thumb_only_pic[] =
{
  THUMB16_INSN (0xb401),             /* push {r0} */
  THUMB16_INSN (0x4802),             /* ldr  r0, [pc, #8] */
...
/* V4T Thumb -> Thumb long branch stub, PIC. Using the stack is not
   allowed.  */
static const insn_sequence cgc32_arm_stub_long_branch_v4t_thumb_thumb_pic[] =
{
  THUMB16_INSN (0x4778),             /* bx   pc */
  THUMB16_INSN (0x46c0),             /* nop */
  ARM_INSN (0xe59fc004),             /* ldr  ip, [pc, #4] */
...
static const insn_sequence cgc32_arm_stub_long_branch_any_tls_pic[] =
{
...
/* V4T Thumb -> TLS trampoline.  lowest common denominator, which is a
   long PIC stub.  We can use r1 as a scratch -- and cannot use ip.  */
static const insn_sequence cgc32_arm_stub_long_branch_v4t_thumb_tls_pic[] =
{
  THUMB16_INSN (0x4778),             /* bx   pc */
  THUMB16_INSN (0x46c0),             /* nop */
...
static const insn_sequence cgc32_arm_stub_long_branch_arm_nacl[] =
{
...
static const insn_sequence cgc32_arm_stub_long_branch_arm_nacl_pic[] =
{
  ARM_INSN (0xe59fc00c),		/* ldr	ip, [pc, #12] */
...
static const insn_sequence cgc32_arm_stub_a8_veneer_b_cond[] =
{
...
static const insn_sequence cgc32_arm_stub_a8_veneer_b[] =
{
  THUMB32_B_INSN (0xf000b800, -4)	/* b.w original_branch_dest.  */
};

static const insn_sequence cgc32_arm_stub_a8_veneer_bl[] =
{
  THUMB32_B_INSN (0xf000b800, -4)	/* b.w original_branch_dest.  */
};
...
static const insn_sequence cgc32_arm_stub_a8_veneer_blx[] =
{
...
#define DEF_STUB(x) {cgc32_arm_stub_##x, ARRAY_SIZE(cgc32_arm_stub_##x)},
...
struct cgc32_arm_stub_hash_entry
{
  ...
  /* The stub type.  */
  enum cgc32_arm_stub_type stub_type;
  ...
  struct cgc32_arm_link_hash_entry *h;
...
typedef struct cgc32_cgc_section_map
{
  bfd_vma vma;
  char type;
}
cgc32_arm_section_map;
...
cgc32_vfp11_erratum_type;

typedef struct cgc32_vfp11_erratum_list
{
  struct cgc32_vfp11_erratum_list *next;
  bfd_vma vma;
  union
  {
    struct
    {
      struct cgc32_vfp11_erratum_list *veneer;
      unsigned int vfp_insn;
    } b;
    struct
    {
      struct cgc32_vfp11_erratum_list *branch;
      unsigned int id;
    } v;
  } u;
  cgc32_vfp11_erratum_type type;
}
cgc32_vfp11_erratum_list;
...
_arm_cgc_section_data;

#define cgc32_arm_section_data(sec) \
  ((_arm_cgc_section_data *) cgc_section_data (sec))

/* A fix which might be required for Cortex-A8 Thumb-2 branch/TLB erratum.
   These fixes are subject to a relaxation procedure (in cgc32_arm_size_stubs),
...
struct a8_erratum_reloc
{
  bfd_vma from;
  bfd_vma destination;
  struct cgc32_arm_link_hash_entry *hash;
  const char *sym_name;
  unsigned int r_type;
  enum arm_st_branch_type branch_type;
...
{
  /* The information that is usually found in the generic CGC part of
...
#define cgc_arm_tdata(bfd) \
  ((struct cgc_arm_obj_tdata *) (bfd)->tdata.any)

#define cgc32_arm_local_got_tls_type(bfd) \
  (cgc_arm_tdata (bfd)->local_got_tls_type)

#define cgc32_arm_local_tlsdesc_gotent(bfd) \
  (cgc_arm_tdata (bfd)->local_tlsdesc_gotent)

#define cgc32_arm_local_iplt(bfd) \
  (cgc_arm_tdata (bfd)->local_iplt)

#define is_arm_cgc(bfd) \
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour \
   && cgc_tdata (bfd) != NULL \
   && cgc_object_id (bfd) == ARM_CGC_DATA)

static bfd_boolean
cgc32_arm_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct cgc_arm_obj_tdata),
				  ARM_CGC_DATA);
}

#define cgc32_arm_hash_entry(ent) ((struct cgc32_arm_link_hash_entry *)(ent))

/* Arm CGC linker hash entry.  */
struct cgc32_arm_link_hash_entry
{
...
#define GOT_TLS_GD_ANY_P(type)	((type & GOT_TLS_GD) || (type & GOT_TLS_GDESC))
  ...
  struct cgc_link_hash_entry *export_glue;

  /* A pointer to the most recently used stub hash entry against this
     symbol.  */
  struct cgc32_arm_stub_hash_entry *stub_cache;
};

/* Traverse an arm CGC linker hash table.  */
#define cgc32_arm_link_hash_traverse(table, func, info)			\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func),	\
    (info)))

/* Get the ARM cgc linker hash table from a link_info structure.  */
#define cgc32_arm_hash_table(info) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((info)->hash)) \
  == ARM_CGC_DATA ? ((struct cgc32_arm_link_hash_table *) ((info)->hash)) : NULL)
...
#define cgc32_arm_compute_jump_table_size(htab) \
  ((htab)->next_tls_desc_index * 4)

/* ARM CGC linker hash table.  */
struct cgc32_arm_link_hash_table
{
  /* The main hash table.  */
  ...
  /* A table of fix locations for Cortex-A8 Thumb-2 branch/TLB erratum.  This
     holds Cortex-A8 erratum fix locations between cgc32_arm_size_stubs() and
     cgc32_arm_write_section().  */
  ...
  /* Assorted information used by cgc32_arm_size_stubs.  */
  unsigned int bfd_count;
  int top_index;
  asection **input_list;
};

/* Create an entry in an ARM CGC linker hash table.  */

static struct bfd_hash_entry *
cgc32_arm_link_hash_newfunc (struct bfd_hash_entry * entry,
			     struct bfd_hash_table * table,
			     const char * string)
{
  struct cgc32_arm_link_hash_entry * ret =
    (struct cgc32_arm_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == NULL)
    ret = (struct cgc32_arm_link_hash_entry *)
	bfd_hash_allocate (table, sizeof (struct cgc32_arm_link_hash_entry));
  if (ret == NULL)
...
static bfd_boolean
cgc32_arm_allocate_local_sym_info (bfd *abfd)
{
  if (cgc_local_got_refcounts (abfd) == NULL)
    {
      bfd_size_type num_syms;
      bfd_size_type size;
      char *data;

      num_syms = cgc_tdata (abfd)->symtab_hdr.sh_info;
	...
	return FALSE;
      ...
      cgc_local_got_refcounts (abfd) = (bfd_signed_vma *) data;
      data += num_syms * sizeof (bfd_signed_vma);

      cgc32_arm_local_iplt (abfd) = (struct arm_local_iplt_info **) data;
      data += num_syms * sizeof (struct arm_local_iplt_info *);

      cgc32_arm_local_tlsdesc_gotent (abfd) = (bfd_vma *) data;
      data += num_syms * sizeof (bfd_vma);

      cgc32_arm_local_got_tls_type (abfd) = data;
    }
  return TRUE;
}
...
cgc32_arm_create_local_iplt (bfd *abfd, unsigned long r_symndx)
{
  struct arm_local_iplt_info **ptr;

  if (!cgc32_arm_allocate_local_sym_info (abfd))
    return NULL;

  BFD_ASSERT (r_symndx < cgc_tdata (abfd)->symtab_hdr.sh_info);
  ptr = &cgc32_arm_local_iplt (abfd)[r_symndx];
  if (*ptr == NULL)
...
static bfd_boolean
cgc32_arm_get_plt_info (bfd *abfd, struct cgc32_arm_link_hash_entry *h,
			...
			struct arm_plt_info **arm_plt)
{
  ...
  if (cgc32_arm_local_iplt (abfd) == NULL)
    return FALSE;

  local_iplt = cgc32_arm_local_iplt (abfd)[r_symndx];
  if (local_iplt == NULL)
    return FALSE;
...
static bfd_boolean
cgc32_arm_plt_needs_thumb_stub_p (struct bfd_link_info *info,
				  struct arm_plt_info *arm_plt)
{
  struct cgc32_arm_link_hash_table *htab;

  htab = cgc32_arm_hash_table (info);
...
static struct cgc_dyn_relocs **
cgc32_arm_get_local_dynreloc_list (bfd *abfd, unsigned long r_symndx,
				   Cgc_Internal_Sym *isym)
{
  if (CGC32_ST_TYPE (isym->st_info) == STT_GNU_IFUNC)
    {
      struct arm_local_iplt_info *local_iplt;

      local_iplt = cgc32_arm_create_local_iplt (abfd, r_symndx);
      if (local_iplt == NULL)
	return NULL;
	...
	abort ();
      ...
      return (struct cgc_dyn_relocs **) vpp;
    }
}
		   ...
		   const char *string)
{
  ...
  if (entry == NULL)
    {
	  ...
	  bfd_hash_allocate (table, sizeof (struct cgc32_arm_stub_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      struct cgc32_arm_stub_hash_entry *eh;

      /* Initialize the local fields.  */
      eh = (struct cgc32_arm_stub_hash_entry *) entry;
      eh->stub_sec = NULL;
      eh->stub_offset = 0;
      eh->target_value = 0;
...
static bfd_boolean
cgc32_arm_create_dynamic_sections (bfd *dynobj, struct bfd_link_info *info)
{
  struct cgc32_arm_link_hash_table *htab;
  ...
  if (htab->vxworks_p)
    {
      if (!cgc_vxworks_create_dynamic_sections (dynobj, info, &htab->srelplt2))
	return FALSE;
      ...
      if (info->shared)
	{
	  ...
	  htab->plt_entry_size
	    = 4 * ARRAY_SIZE (cgc32_arm_vxworks_shared_plt_entry);
	}
      else
	{
	  htab->plt_header_size
	    = 4 * ARRAY_SIZE (cgc32_arm_vxworks_exec_plt0_entry);
	  htab->plt_entry_size
	    = 4 * ARRAY_SIZE (cgc32_arm_vxworks_exec_plt_entry);
	}
    }
...
static void
cgc32_arm_copy_indirect_symbol (struct bfd_link_info *info,
				struct cgc_link_hash_entry *dir,
				struct cgc_link_hash_entry *ind)
{
  struct cgc32_arm_link_hash_entry *edir, *eind;

  edir = (struct cgc32_arm_link_hash_entry *) dir;
  eind = (struct cgc32_arm_link_hash_entry *) ind;
...
/* Create an ARM cgc linker hash table.  */

static struct bfd_link_hash_table *
cgc32_arm_link_hash_table_create (bfd *abfd)
{
  struct cgc32_arm_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct cgc32_arm_link_hash_table);

  ret = (struct cgc32_arm_link_hash_table *) bfd_zmalloc (amt);
  if (ret == NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (& ret->root, abfd,
				      cgc32_arm_link_hash_newfunc,
				      sizeof (struct cgc32_arm_link_hash_entry),
				      ARM_CGC_DATA))
    {
      free (ret);
      return NULL;
  ...
  if (!bfd_hash_table_init (&ret->stub_hash_table, stub_hash_newfunc,
			    sizeof (struct cgc32_arm_stub_hash_entry)))
    {
      free (ret);
      return NULL;
    }
...
static void
cgc32_arm_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct cgc32_arm_link_hash_table *ret
    = (struct cgc32_arm_link_hash_table *) hash;

  bfd_hash_table_free (&ret->stub_hash_table);
  _bfd_cgc_link_hash_table_free (hash);
}

/* Determine if we're dealing with a Thumb only architecture.  */

static bfd_boolean
using_thumb_only (struct cgc32_arm_link_hash_table *globals)
{
  int arch = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
				       Tag_CPU_arch);
  int profile;
  ...
  profile = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
...
static bfd_boolean
using_thumb2 (struct cgc32_arm_link_hash_table *globals)
{
  int arch = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
				       Tag_CPU_arch);
...
static bfd_boolean
arch_has_arm_nop (struct cgc32_arm_link_hash_table *globals)
{
  const int arch = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
					     Tag_CPU_arch);
...
static bfd_boolean
arch_has_thumb2_nop (struct cgc32_arm_link_hash_table *globals)
{
  const int arch = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
					     Tag_CPU_arch);
  return (arch == TAG_CPU_ARCH_V6T2 || arch == TAG_CPU_ARCH_V7
...
static bfd_boolean
arm_stub_is_thumb (enum cgc32_arm_stub_type stub_type)
{
  switch (stub_type)
    {
...
static enum cgc32_arm_stub_type
		  ...
		  const char *name)
{
  ...
  unsigned int r_type;
  struct cgc32_arm_link_hash_table * globals;
  int thumb2;
  int thumb_only;
  enum cgc32_arm_stub_type stub_type = arm_stub_none;
  int use_plt = 0;
  ...
  globals = cgc32_arm_hash_table (info);
  if (globals == NULL)
    return stub_type;
  ...
  if (r_type != R_ARM_TLS_CALL
      ...
      && cgc32_arm_get_plt_info (input_bfd, hash, CGC32_R_SYM (rel->r_info),
				 &root_plt, &arm_plt)
...
static char *
cgc32_arm_stub_name (const asection *input_section,
		     const asection *sym_sec,
		     const struct cgc32_arm_link_hash_entry *hash,
		     const Cgc_Internal_Rela *rel,
		     enum cgc32_arm_stub_type stub_type)
{
  char *stub_name;
  ...
  if (hash)
	...
	sprintf (stub_name, "%08x_%x:%x+%x_%d",
		 ...
		 CGC32_R_TYPE (rel->r_info) == R_ARM_TLS_CALL
		 || CGC32_R_TYPE (rel->r_info) == R_ARM_THM_TLS_CALL
		 ? 0 : (int) CGC32_R_SYM (rel->r_info) & 0xffffffff,
		 (int) rel->r_addend & 0xffffffff,
		 (int) stub_type);
    }
...
static struct cgc32_arm_stub_hash_entry *
cgc32_arm_get_stub_entry (const asection *input_section,
			  const asection *sym_sec,
			  ...
			  enum cgc32_arm_stub_type stub_type)
{
  struct cgc32_arm_stub_hash_entry *stub_entry;
  struct cgc32_arm_link_hash_entry *h = (struct cgc32_arm_link_hash_entry *) hash;
  ...
  if (h != NULL && h->stub_cache != NULL
  ...
  else
    {
      ...
      stub_name = cgc32_arm_stub_name (id_sec, sym_sec, h, rel, stub_type);
      if (stub_name == NULL)
	return NULL;
...
static asection *
cgc32_arm_create_or_find_stub_sec (asection **link_sec_p, asection *section,
				   struct cgc32_arm_link_hash_table *htab)
{
  asection *link_sec;
  asection *stub_sec;
...
static struct cgc32_arm_stub_hash_entry *
cgc32_arm_add_stub (const char *stub_name,
		    asection *section,
		    struct cgc32_arm_link_hash_table *htab)
{
  asection *link_sec;
  ...
  stub_sec = cgc32_arm_create_or_find_stub_sec (&link_sec, section, htab);
  if (stub_sec == NULL)
    return NULL;
...
static void
put_arm_insn (struct cgc32_arm_link_hash_table * htab,
...
static void
put_thumb_insn (struct cgc32_arm_link_hash_table * htab,
		bfd * output_bfd, bfd_vma val, void * ptr)
{
  if (htab->byteswap_code != bfd_little_endian (output_bfd))
    bfd_putl16 (val, ptr);
...
static unsigned
cgc32_arm_tls_transition (struct bfd_link_info *info, int r_type,
...
static bfd_reloc_status_type cgc32_arm_final_link_relocate
  (reloc_howto_type *, bfd *, bfd *, asection *, bfd_byte *,
   Cgc_Internal_Rela *, bfd_vma, struct bfd_link_info *, asection *,
   const char *, unsigned char, enum arm_st_branch_type,
   struct cgc_link_hash_entry *, bfd_boolean *, char **);

static unsigned int
arm_stub_required_alignment (enum cgc32_arm_stub_type stub_type)
{
  switch (stub_type)
    {
...
#define MAXRELOCS 3
  ...
  stub_entry = (struct cgc32_arm_stub_hash_entry *) gen_entry;
  ...
  for (i = 0; i < nrelocs; i++)
	...
	rel.r_info = CGC32_R_INFO (0,
				   template_sequence[stub_reloc_idx[i]].r_type);
	rel.r_addend = template_sequence[stub_reloc_idx[i]].reloc_addend;

	if (stub_entry->stub_type == arm_stub_a8_veneer_b_cond && i == 0)
	  /* The first relocation in the cgc32_arm_stub_a8_veneer_b_cond[]
	...
	cgc32_arm_final_link_relocate (cgc32_arm_howto_from_type
	    (template_sequence[stub_reloc_idx[i]].r_type),
	  stub_bfd, info->output_bfd, stub_sec, stub_sec->contents, &rel,
	  points_to, info, stub_entry->target_section, "", STT_FUNC,
	  branch_type, (struct cgc_link_hash_entry *) stub_entry->h,
	...
	cgc32_arm_final_link_relocate (cgc32_arm_howto_from_type
	  ...
	  (struct cgc_link_hash_entry *) stub_entry->h, &unresolved_reloc,
	  &error_message);
      }
...
static unsigned int
find_stub_size_and_template (enum cgc32_arm_stub_type stub_type,
			     const insn_sequence **stub_template,
			     int *stub_template_size)
{
...
int
cgc32_arm_setup_section_lists (bfd *output_bfd,
			       struct bfd_link_info *info)
{
  bfd *input_bfd;
  unsigned int bfd_count;
  ...
  bfd_size_type amt;
  struct cgc32_arm_link_hash_table *htab = cgc32_arm_hash_table (info);
...
void
cgc32_arm_next_input_section (struct bfd_link_info *info,
			      asection *isec)
{
  struct cgc32_arm_link_hash_table *htab = cgc32_arm_hash_table (info);
...
static void
group_sections (struct cgc32_arm_link_hash_table *htab,
...
static struct cgc_link_hash_entry *find_thumb_glue (struct bfd_link_info *,
						    const char *, char **);
			...
			bfd_boolean *stub_changed_p)
{
  ...
  for (section = input_bfd->sections;
       ...
       section = section->next)
    {
      ...
      struct _arm_cgc_section_data *sec_data;
      unsigned int span;
      bfd_vma base_vma;

      if (cgc_section_type (section) != SHT_PROGBITS
	  || (cgc_section_flags (section) & SHF_EXECINSTR) == 0
	...
	continue;
      ...
      if (cgc_section_data (section)->this_hdr.contents != NULL)
	contents = cgc_section_data (section)->this_hdr.contents;
      else if (! bfd_malloc_and_get_section (input_bfd, section, &contents))
	return TRUE;

      sec_data = cgc32_arm_section_data (section);
		  ...
		  if (found)
		    {
		      ...
		      struct cgc_link_hash_entry *entry;
...
bfd_boolean
cgc32_arm_size_stubs (bfd *output_bfd,
		      bfd *stub_bfd,
		      struct bfd_link_info *info,
		      ...
		      void (*layout_sections_again) (void))
{
  ...
  while (1)
	   ...
	   input_bfd = input_bfd->link_next, bfd_indx++)
	{
	  ...
	  asection *section;
	  Cgc_Internal_Sym *local_syms = NULL;

	  if (!is_arm_cgc (input_bfd))
	    continue;
	  ...
	  for (section = input_bfd->sections;
	       ...
	       section = section->next)
	    {
	      Cgc_Internal_Rela *internal_relocs, *irelaend, *irela;
		...
		continue;
	      ...
	      /* Get the relocs.  */
	      internal_relocs
		= _bfd_cgc_link_read_relocs (input_bfd, section, NULL,
	      ...
	      for (; irela < irelaend; irela++)
		{
		  ...
		  r_type = CGC32_R_TYPE (irela->r_info);
		  r_indx = CGC32_R_SYM (irela->r_info);

		  if (r_type >= (unsigned int) R_ARM_max)
		    {
		      bfd_set_error (bfd_error_bad_value);
		    error_ret_free_internal:
		      if (cgc_section_data (section)->relocs == NULL)
		  ...
		  if (r_indx >= symtab_hdr->sh_info)
		    hash = cgc32_arm_hash_entry
		      (cgc_sym_hashes (input_bfd)
		  ...
		  if ((r_type != (unsigned int) R_ARM_CALL)
		      ...
		      && !((r_type == (unsigned int) R_ARM_TLS_CALL
			   ...
			   && r_type == cgc32_arm_tls_transition
			       (info, r_type, &hash->root)
			   && ((hash ? hash->tls_type
				: (cgc32_arm_local_got_tls_type
				   (input_bfd)[r_indx]))
			       & GOT_TLS_GDESC) != 0))
		    continue;
		  ...
		  if (r_type == (unsigned int) R_ARM_TLS_CALL
			...
			{
			  local_syms
			    = (Cgc_Internal_Sym *) symtab_hdr->contents;
			  if (local_syms == NULL)
			    local_syms
			      = bfd_cgc_get_cgc_syms (input_bfd, symtab_hdr,
			...
			sym_sec =
			  bfd_section_from_cgc_index (input_bfd, sym->st_shndx);
			...
			sym_value = sym->st_value;
		      ...
		      st_type = CGC_ST_TYPE (sym->st_info);
		      branch_type = ARM_SYM_BRANCH_TYPE (sym);
		      sym_name
			= bfd_cgc_string_from_cgc_section (input_bfd,
							   symtab_hdr->sh_link,
							   sym->st_name);
		    }
		  else
			...
			hash = ((struct cgc32_arm_link_hash_entry *)
		  ...
		  do
			...
			break;
		      ...
		      stub_name = cgc32_arm_stub_name (id_sec, sym_sec, hash,
						       irela, stub_type);
		      if (!stub_name)
			goto error_ret_free_internal;
		      ...
		      if (stub_entry != NULL)
			{
		      ...
		      stub_entry = cgc32_arm_add_stub (stub_name, section,
						       htab);
		      if (stub_entry == NULL)
			{
			  free (stub_name);
	...
	for (i = 0; i < num_a8_fixes; i++)
	  {
	    stub_sec = cgc32_arm_create_or_find_stub_sec (NULL,
...
/* Build all the stubs associated with the current output file.  The
   ...
   functions here.  This function is called via arm_cgc_finish in the
   linker.  */

bfd_boolean
cgc32_arm_build_stubs (struct bfd_link_info *info)
{
  asection *stub_sec;
  struct bfd_hash_table *table;
  struct cgc32_arm_link_hash_table *htab;
...
static struct cgc_link_hash_entry *
		 ...
		 char **error_message)
{
  char *tmp_name;
  struct cgc_link_hash_entry *hash;
  struct cgc32_arm_link_hash_table *hash_table;

  /* We need a pointer to the armcgc specific hash table.  */
  hash_table = cgc32_arm_hash_table (link_info);
  if (hash_table == NULL)
    return NULL;
...
{
  char *tmp_name;
  struct cgc_link_hash_entry *myh;
  struct cgc32_arm_link_hash_table *hash_table;

  /* We need a pointer to the cgcarm specific hash table.  */
  hash_table = cgc32_arm_hash_table (link_info);
  if (hash_table == NULL)
    return NULL;
...
bfd_boolean
bfd_cgc32_arm_allocate_interworking_sections (struct bfd_link_info * info)
{
  struct cgc32_arm_link_hash_table * globals;
			  ...
			  struct cgc_link_hash_entry * h)
{
  ...
  globals = cgc32_arm_hash_table (link_info);
  BFD_ASSERT (globals != NULL);
  ...
  myh = (struct cgc_link_hash_entry *) bh;
  myh->type = CGC_ST_INFO (STB_LOCAL, STT_FUNC);
  myh->forced_local = 1;
...
static void
cgc32_arm_section_map_add (asection *sec, char type, bfd_vma vma)
{
  struct _arm_cgc_section_data *sec_data = cgc32_arm_section_data (sec);
  unsigned int newidx;

  if (sec_data->map == NULL)
    {
      sec_data->map = (cgc32_arm_section_map *)
	  bfd_malloc (sizeof (cgc32_arm_section_map));
      sec_data->mapcount = 0;
      sec_data->mapsize = 1;
    }
...
record_vfp11_erratum_veneer (struct bfd_link_info *link_info,
			     cgc32_vfp11_erratum_list *branch,
			     bfd *branch_bfd,
			     asection *branch_sec,
			     unsigned int offset)
{
  ...
  bfd_vma val;
  struct _arm_cgc_section_data *sec_data;
  cgc32_vfp11_erratum_list *newerr;
  ...
  sec_data = cgc32_arm_section_data (s);
  ...
  newerr = (cgc32_vfp11_erratum_list *)
      bfd_zmalloc (sizeof (cgc32_vfp11_erratum_list));
  ...
  if (hash_table->vfp11_erratum_glue_size == 0)
    {
	 ...
	 ever requires this erratum fix.  */
      ...
      myh = (struct cgc_link_hash_entry *) bh;
      myh->type = CGC_ST_INFO (STB_LOCAL, STT_NOTYPE);
      myh->forced_local = 1;

      /* The cgc32_arm_init_maps function only cares about symbols from input
	 BFDs.  We must make a note of this generated mapping symbol
	 ourselves so that code byteswapping works properly in
	 cgc32_arm_write_section.  */
      cgc32_arm_section_map_add (s, 'a', 0);
    }
...
/* Add the glue sections to ABFD.  This function is called from the
   linker scripts in ld/emultempl/{armcgc}.em.  */

bfd_boolean
bfd_cgc32_arm_add_glue_sections_to_bfd (bfd *abfd,
					struct bfd_link_info *info)
{
...
bfd_boolean
bfd_cgc32_arm_get_bfd_for_interworking (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc32_arm_link_hash_table *globals;
...
static void
check_use_blx (struct cgc32_arm_link_hash_table *globals)
{
  int cpu_arch;

  cpu_arch = bfd_cgc_get_obj_attr_int (globals->obfd, OBJ_ATTR_PROC,
				       Tag_CPU_arch);
...
bfd_boolean
bfd_cgc32_arm_process_before_allocation (bfd *abfd,
					 struct bfd_link_info *link_info)
{
  Cgc_Internal_Shdr *symtab_hdr;
  Cgc_Internal_Rela *internal_relocs = NULL;
  ...
  for (; sec != NULL; sec = sec->next)
	...
	continue;
      ...
      /* Load the relocs.  */
      internal_relocs
	= _bfd_cgc_link_read_relocs (abfd, sec, NULL, NULL, FALSE);
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	{
	  ...
	  r_type = CGC32_R_TYPE (irel->r_info);
	  r_index = CGC32_R_SYM (irel->r_info);
...
void
bfd_cgc32_arm_init_maps (bfd *abfd)
{
  Cgc_Internal_Sym *isymbuf;
  Cgc_Internal_Shdr *hdr;
  unsigned int i, localsyms;

  /* PR 7093: Make sure that we are dealing with an arm cgc binary.  */
  if (! is_arm_cgc (abfd))
    return;
  ...
  hdr = & cgc_symtab_hdr (abfd);
  localsyms = hdr->sh_info;

  /* Obtain a buffer full of symbols for this BFD. The hdr->sh_info field
     should contain the number of local symbols, which should come before any
     global symbols.  Mapping symbols are always local.  */
  isymbuf = bfd_cgc_get_cgc_syms (abfd, hdr, localsyms, 0, NULL, NULL,
				  NULL);
  ...
  for (i = 0; i < localsyms; i++)
    {
      Cgc_Internal_Sym *isym = &isymbuf[i];
      asection *sec = bfd_section_from_cgc_index (abfd, isym->st_shndx);
      const char *name;

      if (sec != NULL
	  && CGC_ST_BIND (isym->st_info) == STB_LOCAL)
	{
	  name = bfd_cgc_string_from_cgc_section (abfd,
	    hdr->sh_link, isym->st_name);

	  if (bfd_is_arm_special_symbol_name (name,
					      BFD_ARM_SPECIAL_SYM_TYPE_MAP))
	    cgc32_arm_section_map_add (sec, name[1], isym->st_value);
	}
    }
}
...
void
bfd_cgc32_arm_set_cortex_a8_fix (bfd *obfd, struct bfd_link_info *link_info)
{
  struct cgc32_arm_link_hash_table *globals = cgc32_arm_hash_table (link_info);
  obj_attribute *out_attr = cgc_known_obj_attributes_proc (obfd);
...
void
bfd_cgc32_arm_set_vfp11_fix (bfd *obfd, struct bfd_link_info *link_info)
{
  struct cgc32_arm_link_hash_table *globals = cgc32_arm_hash_table (link_info);
  obj_attribute *out_attr = cgc_known_obj_attributes_proc (obfd);
...
static int cgc32_arm_compare_mapping (const void * a, const void * b);
...
bfd_boolean
bfd_cgc32_arm_vfp11_erratum_scan (bfd *abfd, struct bfd_link_info *link_info)
{
  asection *sec;
  bfd_byte *contents = NULL;
  int state = 0;
  ...
  /* Skip if this bfd does not correspond to an CGC image.  */
  if (! is_arm_cgc (abfd))
    return TRUE;
  ...
  for (sec = abfd->sections; sec != NULL; sec = sec->next)
    {
	 ...
	 section.  Also skip if section is to be excluded.  */
      if (cgc_section_type (sec) != SHT_PROGBITS
	...
	continue;
      ...
      sec_data = cgc32_arm_section_data (sec);
	...
	goto error_return;
      ...
      qsort (sec_data->map, sec_data->mapcount, sizeof (cgc32_arm_section_map),
	     cgc32_arm_compare_mapping);
      ...
      for (span = 0; span < sec_data->mapcount; span++)
	{
	  ...
	  for (i = span_start; i < span_end;)
		...
		case 3:
	      ...
	      if (state == 3)
		{
		  cgc32_vfp11_erratum_list *newerr =(cgc32_vfp11_erratum_list *)
		      bfd_zmalloc (sizeof (cgc32_vfp11_erratum_list));

		  cgc32_arm_section_data (sec)->erratumcount += 1;
...
void
bfd_cgc32_arm_vfp11_fix_veneer_locations (bfd *abfd,
					  struct bfd_link_info *link_info)
{
  asection *sec;
  struct cgc32_arm_link_hash_table *globals;
  ...
  for (sec = abfd->sections; sec != NULL; sec = sec->next)
    {
      struct _arm_cgc_section_data *sec_data = cgc32_arm_section_data (sec);
      cgc32_vfp11_erratum_list *errnode = sec_data->erratumlist;
...
void
bfd_cgc32_arm_set_target_relocs (struct bfd *output_bfd,
				 struct bfd_link_info *link_info,
				 int target1_is_rel,
				 char * target2_type,
				 int fix_v4bx,
				 ...
				 int fix_arm1176)
{
  ...
  BFD_ASSERT (is_arm_cgc (output_bfd));
  cgc_arm_tdata (output_bfd)->no_enum_size_warning = no_enum_warn;
  cgc_arm_tdata (output_bfd)->no_wchar_size_warning = no_wchar_warn;
}
...
static int
cgc32_thumb_to_arm_stub (struct bfd_link_info * info,
			 const char *           name,
			 bfd *                  input_bfd,
			 bfd *                  output_bfd,
...
static struct cgc_link_hash_entry *
cgc32_arm_create_thumb_stub (struct bfd_link_info * info,
			     const char *           name,
			     bfd *                  input_bfd,
			     bfd *                  output_bfd,
			     asection *             sym_sec,
...
static int
cgc32_arm_to_thumb_stub (struct bfd_link_info * info,
			 const char *           name,
			 bfd *                  input_bfd,
			 bfd *                  output_bfd,
			 asection *             input_section,
			 ...
			 char **error_message)
{
  ...
  myh = cgc32_arm_create_thumb_stub (info, name, input_bfd, output_bfd,
...
static bfd_boolean
cgc32_arm_to_thumb_export_stub (struct cgc_link_hash_entry *h, void * inf)
{
  struct bfd_link_info * info = (struct bfd_link_info *) inf;
  asection * s;
  struct cgc_link_hash_entry * myh;
  struct cgc32_arm_link_hash_entry *eh;
  struct cgc32_arm_link_hash_table * globals;
  asection *sec;
  bfd_vma val;
  char *error_message;

  eh = cgc32_arm_hash_entry (h);
  ...
  myh = cgc32_arm_create_thumb_stub (info, h->root.root.string,
...
static bfd_vma
cgc32_arm_bx_glue (struct bfd_link_info * info, int reg)
{
  bfd_byte *p;
  bfd_vma glue_addr;
  asection *s;
...
static void
cgc32_arm_begin_write_processing (bfd *abfd ATTRIBUTE_UNUSED,
				  struct bfd_link_info *link_info)
{
  struct cgc32_arm_link_hash_table * globals;

  if (link_info == NULL)
    /* Ignore this if we are not called by the CGC backend linker.  */
    return;
  ...
  cgc_link_hash_traverse (&globals->root, cgc32_arm_to_thumb_export_stub,
			  link_info);
}
...
static void
cgc32_arm_allocate_dynrelocs (struct bfd_link_info *info, asection *sreloc,
			      bfd_size_type count)
{
  struct cgc32_arm_link_hash_table *htab;
...
static void
cgc32_arm_allocate_irelocs (struct bfd_link_info *info, asection *sreloc,
			    bfd_size_type count)
{
  struct cgc32_arm_link_hash_table *htab;
...
static void
cgc32_arm_add_dynreloc (bfd *output_bfd, struct bfd_link_info *info,
			asection *sreloc, Cgc_Internal_Rela *rel)
{
  bfd_byte *loc;
  struct cgc32_arm_link_hash_table *htab;
  ...
  htab = cgc32_arm_hash_table (info);
  if (!htab->root.dynamic_sections_created
      && CGC32_R_TYPE (rel->r_info) == R_ARM_IRELATIVE)
...
static void
cgc32_arm_allocate_plt_entry (struct bfd_link_info *info,
			      ...
			      struct arm_plt_info *arm_plt)
{
  ...
  if (is_iplt_entry)
	...
	splt->size += htab->plt_header_size;
      ...
      cgc32_arm_allocate_irelocs (info, htab->root.irelplt, 1);
    }
  else
    {
      splt = htab->root.splt;
      ...
      cgc32_arm_allocate_dynrelocs (info, htab->root.srelplt, 1);
  ...
  if (cgc32_arm_plt_needs_thumb_stub_p (info, arm_plt))
...
static void
cgc32_arm_populate_plt_entry (bfd *output_bfd, struct bfd_link_info *info,
			      union gotplt_union *root_plt,
			      struct arm_plt_info *arm_plt,
			      int dynindx, bfd_vma sym_value)
{
  ...
  if (dynindx == -1)
  ...
  else
    {
      ...
      got_header_size = get_cgc_backend_data (output_bfd)->got_header_size;
  ...
  if (htab->symbian_p)
    {
      ...
      put_arm_insn (htab, output_bfd,
		    cgc32_arm_symbian_plt_entry[0],
		    splt->contents + root_plt->offset);
      bfd_put_32 (output_bfd,
		  cgc32_arm_symbian_plt_entry[1],
      ...
      rel.r_offset = (splt->output_section->vma
		      + splt->output_offset
		      + root_plt->offset + 4);
      rel.r_info = CGC32_R_INFO (dynindx, R_ARM_GLOB_DAT);
      ...
      if (htab->vxworks_p && info->shared)
	{
	  ...
	  for (i = 0; i != htab->plt_entry_size / 4; i++, ptr += 4)
	    {
	      val = cgc32_arm_vxworks_shared_plt_entry[i];
	      if (i == 2)
      ...
      else if (htab->vxworks_p)
	{
	  ...
	  for (i = 0; i != htab->plt_entry_size / 4; i++, ptr += 4)
	    {
	      val = cgc32_arm_vxworks_exec_plt_entry[i];
	      if (i == 2)
		val |= got_address;
	      if (i == 4)
	  ...
	  rel.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_ARM_ABS32);
	  ...
	  rel.r_info = CGC32_R_INFO (htab->root.hplt->indx, R_ARM_ABS32);
	  rel.r_addend = 0;
	  SWAP_RELOC_OUT (htab) (output_bfd, &rel, loc);
	}
      else if (htab->nacl_p)
	{
	  ...
	  BFD_ASSERT (!cgc32_arm_plt_needs_thumb_stub_p (info, arm_plt));

	  put_arm_insn (htab, output_bfd,
			cgc32_arm_nacl_plt_entry[0]
			| arm_movw_immediate (got_displacement),
			ptr + 0);
	  put_arm_insn (htab, output_bfd,
			cgc32_arm_nacl_plt_entry[1]
			| arm_movt_immediate (got_displacement),
			ptr + 4);
	  put_arm_insn (htab, output_bfd,
			cgc32_arm_nacl_plt_entry[2],
			ptr + 8);
	  put_arm_insn (htab, output_bfd,
			cgc32_arm_nacl_plt_entry[3]
	  ...
	  if (cgc32_arm_plt_needs_thumb_stub_p (info, arm_plt))
	    {
	      put_thumb_insn (htab, output_bfd,
			      cgc32_arm_plt_thumb_stub[0], ptr - 4);
	      put_thumb_insn (htab, output_bfd,
			      cgc32_arm_plt_thumb_stub[1], ptr - 2);
	    }

	  put_arm_insn (htab, output_bfd,
			cgc32_arm_plt_entry[0]
			| ((got_displacement & 0x0ff00000) >> 20),
			ptr + 0);
	  put_arm_insn (htab, output_bfd,
			cgc32_arm_plt_entry[1]
			| ((got_displacement & 0x000ff000) >> 12),
			ptr+ 4);
	  put_arm_insn (htab, output_bfd,
			cgc32_arm_plt_entry[2]
			| (got_displacement & 0x00000fff),
			ptr + 8);
#ifdef FOUR_WORD_PLT
	  bfd_put_32 (output_bfd, cgc32_arm_plt_entry[3], ptr + 12);
#endif
	}
      ...
      if (dynindx == -1)
	{
	  ...
	  rel.r_info = CGC32_R_INFO (0, R_ARM_IRELATIVE);
	  initial_got_entry = sym_value;
	}
      else
	{
	  rel.r_info = CGC32_R_INFO (dynindx, R_ARM_JUMP_SLOT);
  ...
  if (dynindx == -1)
    cgc32_arm_add_dynreloc (output_bfd, info, srel, &rel);
  else
    {
...
static int
arm_real_reloc_type (struct cgc32_arm_link_hash_table * globals,
		     int r_type)
{
  switch (r_type)
    {
...
cgc32_arm_abs12_reloc (bfd *abfd, void *data, bfd_vma value)
{
  if (value > 0xfff)
...
static bfd_reloc_status_type
cgc32_arm_tls_relax (struct cgc32_arm_link_hash_table *globals,
		     bfd *input_bfd, asection *input_sec, bfd_byte *contents,
		     Cgc_Internal_Rela *rel, unsigned long is_local)
{
  unsigned long insn;
...
static bfd_reloc_status_type
cgc32_arm_final_link_relocate (reloc_howto_type *           howto,
			       bfd *                        input_bfd,
			       bfd *                        output_bfd,
			       asection *                   input_section,
			       bfd_byte *                   contents,
			       ...
			       char **                      error_message)
{
  ...
  BFD_ASSERT (is_arm_cgc (input_bfd));
  ...
  r_type = cgc32_arm_tls_transition (info, r_type, h);

  if (r_type != howto->type)
    howto = cgc32_arm_howto_from_type (r_type);
  ...
  if (bfd_get_start_address (output_bfd) != 0)
    cgc_cgcheader (output_bfd)->e_flags |= EF_ARM_HASENTRY;

  eh = (struct cgc32_arm_link_hash_entry *) h;
  sgot = globals->root.sgot;
  local_got_offsets = cgc_local_got_offsets (input_bfd);
  local_tlsdesc_gotents = cgc32_arm_local_tlsdesc_gotent (input_bfd);
  ...
  if (cgc32_arm_get_plt_info (input_bfd, eh, r_symndx, &root_plt, &arm_plt)
      && root_plt->offset != (bfd_vma) -1)
    {
      ...
      if (h == NULL || eh->is_iplt)
	{
	  ...
	  if (plt_offset & 1)
	    ...
	    {
	      cgc32_arm_populate_plt_entry (output_bfd, info, root_plt, arm_plt,
					    -1, dynreloc_value);
	      root_plt->offset |= 1;
	    }
  ...
  switch (r_type)
	...
	return cgc32_arm_abs12_reloc (input_bfd, hit_data, value + addend);
	  ...
	  && r_type != R_ARM_PLT32)
	{
	  ...
	  *unresolved_reloc_p = FALSE;
	  ...
	  if (sreloc == NULL && globals->root.dynamic_sections_created)
	    {
	      sreloc = _bfd_cgc_get_dynamic_reloc_section (input_bfd, input_section,
							   ! globals->use_rel);
	  ...
	  if (skip)
	    memset (&outrel, 0, sizeof outrel);
		   ...
		   && (!info->shared
		       || !info->symbolic
		       || !h->def_regular))
	    outrel.r_info = CGC32_R_INFO (h->dynindx, r_type);
	  else
	    {
	      int symbol;
	      ...
	      if (globals->symbian_p)
		{
		  ...
		  symbol = cgc_section_data (osec)->dynindx;
		  if (symbol == 0)
		    {
		      struct cgc_link_hash_table *htab = cgc_hash_table (info);
		...
		outrel.r_info = CGC32_R_INFO (symbol, R_ARM_IRELATIVE);
	      else
		outrel.r_info = CGC32_R_INFO (symbol, R_ARM_RELATIVE);
	      if (globals->use_rel)
		relocate = TRUE;
	      else
	  ...
	  cgc32_arm_add_dynreloc (output_bfd, info, sreloc, &outrel);
	...
	case R_ARM_PLT32:
	  {
	  struct cgc32_arm_stub_hash_entry *stub_entry = NULL;
	  ...
	  if (r_type == R_ARM_XPC25)
	  ...
	  else if (r_type == R_ARM_PC24)
	    {
	      ...
	      if (branch_type == ST_BRANCH_TO_THUMB)
		{
		  if (cgc32_arm_to_thumb_stub (info, sym_name, input_bfd,
	  ...
	  if (   r_type == R_ARM_CALL
	      ...
	      || r_type == R_ARM_PLT32)
	    {
	      ...
	      hash = (struct cgc32_arm_link_hash_entry *) h;
	      ...
	      if (stub_type != arm_stub_none)
		{
		  ...
		  stub_entry = cgc32_arm_get_stub_entry (input_section,
							 sym_sec, h,
							 rel, globals,
							 stub_type);
		  {
	  ...
	  /* The ARM CGC ABI says that this reloc is computed as: S - P + A
	     where:
	  ...
	  if ((off & 1) != 0)
		...
		*unresolved_reloc_p = FALSE;
	      ...
	      if (h->dynindx != -1 && !SYMBOL_REFERENCES_LOCAL (info, h))
		{
		  ...
		  if (globals->root.dynamic_sections_created)
		    {
		      outrel.r_info = CGC32_R_INFO (h->dynindx, R_ARM_GLOB_DAT);
		      *unresolved_reloc_p = FALSE;
		    }
		  else
		    outrel.r_info = 0;
		...
		{
		  if (dynreloc_st_type == STT_GNU_IFUNC)
		    outrel.r_info = CGC32_R_INFO (0, R_ARM_IRELATIVE);
		  else if (info->shared &&
			   (CGC_ST_VISIBILITY (h->other) == STV_DEFAULT
			    || h->root.type != bfd_link_hash_undefweak))
		    outrel.r_info = CGC32_R_INFO (0, R_ARM_RELATIVE);
		  else
		    outrel.r_info = 0;
		  outrel.r_addend = dynreloc_value;
		}
	      ...
	      if (outrel.r_info != 0)
		{
		  outrel.r_offset = (sgot->output_section->vma
				     + sgot->output_offset
				     + off);
		  cgc32_arm_add_dynreloc (output_bfd, info, srelgot, &outrel);
		}
	      h->got.offset |= 1;
	    }
	...
	if ((off & 1) != 0)
	...
	else
	  {
		...
		outrel.r_info = CGC32_R_INFO (0, R_ARM_TLS_DTPMOD32);
	...
	if (h != NULL)
	  {
		...
		*unresolved_reloc_p = FALSE;
	    ...
	    off = h->got.offset;
	    offplt = cgc32_arm_hash_entry (h)->tlsdesc_got;
	    tls_type = ((struct cgc32_arm_link_hash_entry *) h)->tls_type;
	  }
	else
	  {
	    BFD_ASSERT (local_got_offsets != NULL);
	...
	if ((off & 1) != 0)
	...
	else
	  {
		...
		outrel.r_info = CGC32_R_INFO (indx, R_ARM_TLS_DESC);
		...
		bfd_put_32 (output_bfd,
			    !h ? value - cgc_hash_table (info)->tls_sec->vma
			    : info->flags & DF_BIND_NOW ? 0
			    : 0x80000000 | CGC32_R_SYM (outrel.r_info),
		...
		if (need_relocs)
		  {
		    ...
		    outrel.r_offset = (sgot->output_section->vma
				       + sgot->output_offset
				       + cur_off);
		    outrel.r_info = CGC32_R_INFO (indx, R_ARM_TLS_DTPMOD32);
		    ...
		    if (indx == 0)
		      ...
		      {
			outrel.r_addend = 0;
			outrel.r_info = CGC32_R_INFO (indx,
						      R_ARM_TLS_DTPOFF32);
			outrel.r_offset += 4;
			...
			cgc32_arm_add_dynreloc (output_bfd, info,
						srelgot, &outrel);
		      }
		  }
		else
		...
		if (need_relocs)
		  {
		    ...
		    outrel.r_offset = (sgot->output_section->vma
				       + sgot->output_offset
				       + cur_off);
		    outrel.r_info = CGC32_R_INFO (indx, R_ARM_TLS_TPOFF32);
	...
	if (CGC32_R_TYPE(rel->r_info) == R_ARM_TLS_CALL
	    || CGC32_R_TYPE(rel->r_info) == R_ARM_THM_TLS_CALL)
	  {
	  ...
	  if (globals->fix_v4bx == 2 && (insn & 0xf) != 0xf)
	    {
	      /* Branch to veneer.  */
	      bfd_vma glue_addr;
	      glue_addr = cgc32_arm_bx_glue (info, insn & 0xf);
...
/* Relocate an ARM CGC section.  */

static bfd_boolean
cgc32_arm_relocate_section (bfd *                  output_bfd,
			    struct bfd_link_info * info,
			    bfd *                  input_bfd,
			    asection *             input_section,
			    bfd_byte *             contents,
			    ...
			    asection **            local_sections)
{
  ...
  for (; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  sym_type = CGC32_ST_TYPE (sym->st_info);
	  ...
	  if (r_type != R_ARM_V4BX
	      ...
	      && CGC_ST_BIND (sym->st_info) != STB_WEAK)
	    {
		  ...
		  (info, bfd_cgc_string_from_cgc_section
	  ...
	  if (globals->use_rel)
		  ...
		  && CGC_ST_TYPE (sym->st_info) == STT_SECTION)
		{
		  ...
		  msec = sec;
		  addend =
		    _bfd_cgc_rel_local_sym (output_bfd, sym, &msec, addend)
		    - relocation;
      ...
      if (info->relocatable)
	{
	  ...
	  if (sym != NULL && CGC_ST_TYPE (sym->st_info) == STT_SECTION)
	    {
	      if (globals->use_rel)
	  ...
	  && IS_ARM_TLS_RELOC (r_type) != (sym_type == STT_TLS))
	{
      ...
      /* We call cgc32_arm_final_link_relocate unless we're completely
	 done, i.e., the relaxation produced the final output we want,
	 and we won't let anybody mess with it. Also, we have to do
	 addend adjustments in case of a R_ARM_TLS_GOTDESC relocation
	 both in relaxed and non-relaxed cases */
     if ((cgc32_arm_tls_transition (info, r_type, h) != (unsigned)r_type)
	 || (IS_ARM_TLS_GNU_RELOC (r_type)
	     && !((h ? cgc32_arm_hash_entry (h)->tls_type :
		   cgc32_arm_local_got_tls_type (input_bfd)[r_symndx])
		  & GOT_TLS_GDESC)))
       {
	 r = cgc32_arm_tls_relax (globals, input_bfd, input_section,
				  contents, rel, h == NULL);
     ...
     if (r == bfd_reloc_continue)
       r = cgc32_arm_final_link_relocate (howto, input_bfd, output_bfd,
...
static _arm_cgc_section_data *get_arm_cgc_section_data (asection *);
...
{
  struct _arm_cgc_section_data *exidx_arm_data;

  exidx_arm_data = get_arm_cgc_section_data (exidx_sec);
...
/* Scan .ARM.exidx tables, and create a list describing edits which should be
   ...
   (in cgc32_arm_write_section).  */

bfd_boolean
cgc32_arm_fix_exidx_coverage (asection **text_section_order,
			      ...
			      bfd_boolean merge_exidx_entries)
{
  ...
  for (inp = info->input_bfds; inp != NULL; inp = inp->link_next)
	...
	{
	  struct bfd_cgc_section_data *cgc_sec = cgc_section_data (sec);
	  Cgc_Internal_Shdr *hdr = &cgc_sec->this_hdr;

	  if (!hdr || hdr->sh_type != SHT_ARM_EXIDX)
	    continue;

	  if (cgc_sec->linked_to)
	    {
	      Cgc_Internal_Shdr *linked_hdr
		= &cgc_section_data (cgc_sec->linked_to)->this_hdr;
	      struct _arm_cgc_section_data *linked_sec_arm_data
		= get_arm_cgc_section_data (linked_hdr->bfd_section);
  ...
  for (i = 0; i < num_text_sections; i++)
    {
      ...
      asection *exidx_sec;
      struct _arm_cgc_section_data *arm_data = get_arm_cgc_section_data (sec);
      struct _arm_cgc_section_data *exidx_arm_data;
      bfd_byte *contents = NULL;
	...
	continue;
      ...
      hdr = &cgc_section_data (exidx_sec)->this_hdr;
	...
	free (contents);
      ...
      /* Record edits to be applied later (in cgc32_arm_write_section).  */
...
static bfd_boolean
cgc32_arm_output_glue_section (struct bfd_link_info *info, bfd *obfd,
			       bfd *ibfd, const char *name)
{
  ...
  if (cgc32_arm_write_section (obfd, info, sec, sec->contents))
    return TRUE;
...
static bfd_boolean
cgc32_arm_final_link (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc32_arm_link_hash_table *globals = cgc32_arm_hash_table (info);
  asection *sec, *osec;
  ...
  for (i=0; i<htab->top_id; i++)
	...
	{
	  osec = sec->output_section;
	  cgc32_arm_write_section (abfd, info, sec, sec->contents);
  ...
  if (globals->bfd_of_glue_owner != NULL)
    {
      if (! cgc32_arm_output_glue_section (info, abfd,
...
{
  int arch = bfd_cgc_get_obj_attr_int (abfd, OBJ_ATTR_PROC, Tag_CPU_arch);
  ...
  switch (arch)
	...
	name = cgc_known_obj_attributes (abfd) [OBJ_ATTR_PROC][Tag_CPU_name].s;
	...
	if (name)
	  {
		...
		wmmx = cgc_known_obj_attributes (abfd) [OBJ_ATTR_PROC][Tag_WMMX_arch].i;
		switch (wmmx)
		  {
...
cgc32_arm_object_p (bfd *abfd)
{
  ...
  if (mach == bfd_mach_arm_unknown)
    {
      if (cgc_cgcheader (abfd)->e_flags & EF_ARM_MAVERICK_FLOAT)
...
/* Function to keep ARM specific flags in the CGC header.  */

static bfd_boolean
cgc32_arm_set_private_flags (bfd *abfd, flagword flags)
{
  if (cgc_flags_init (abfd)
...
static bfd_boolean
cgc32_arm_copy_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword in_flags;
  flagword out_flags;

  if (! is_arm_cgc (ibfd) || ! is_arm_cgc (obfd))
    return TRUE;

  in_flags  = cgc_cgcheader (ibfd)->e_flags;
  out_flags = cgc_cgcheader (obfd)->e_flags;
  ...
  cgc_cgcheader (obfd)->e_flags = in_flags;
  cgc_flags_init (obfd) = TRUE;

  /* Also copy the EI_OSABI field.  */
  cgc_cgcheader (obfd)->e_ident[EI_OSABI] =
    cgc_cgcheader (ibfd)->e_ident[EI_OSABI];
...
static int
cgc32_arm_obj_attrs_arg_type (int tag)
{
...
/* The ABI defines that Tag_conformance should be emitted first, and that
   ...
   compensate.  */
static int
cgc32_arm_obj_attrs_order (int num)
{
...
static bfd_boolean
cgc32_arm_obj_attrs_handle_unknown (bfd *abfd, int tag)
{
  if ((tag & 127) < 64)
    {
      _bfd_error_handler
...
{
  obj_attribute *attr =
    &cgc_known_obj_attributes_proc (abfd)[Tag_also_compatible_with];
...
static bfd_boolean
cgc32_arm_attributes_accept_div (const obj_attribute *attr)
{
...
/* Query attributes object to see if integer divide instructions are
   ...
   cgc32_arm_attributes_accept_div.  */
static bfd_boolean
cgc32_arm_attributes_forbid_div (const obj_attribute *attr)
{
...
static bfd_boolean
cgc32_arm_merge_eabi_attributes (bfd *ibfd, bfd *obfd)
{
  ...
  if (!cgc_known_obj_attributes_proc (obfd)[0].i)
    {
      ...
      out_attr = cgc_known_obj_attributes_proc (obfd);
  ...
  in_attr = cgc_known_obj_attributes_proc (ibfd);
  out_attr = cgc_known_obj_attributes_proc (obfd);
	...
	case Tag_CPU_arch:
	  {
		...
		out_attr[Tag_CPU_name].s = in_attr[Tag_CPU_name].s
		  ? _bfd_cgc_attr_strdup (obfd, in_attr[Tag_CPU_name].s)
		  : NULL;
		out_attr[Tag_CPU_raw_name].s = in_attr[Tag_CPU_raw_name].s
		  ? _bfd_cgc_attr_strdup (obfd, in_attr[Tag_CPU_raw_name].s)
		  : NULL;
	      }
	    else
	      {
		...
		_bfd_cgc_attr_strdup (obfd, name_table[out_attr[i].i]);
	  }
	  break;
...
#define VFP_VERSION_COUNT 8
	...
	case Tag_ABI_PCS_wchar_t:
	  if (out_attr[i].i && in_attr[i].i && out_attr[i].i != in_attr[i].i
	      && !cgc_arm_tdata (obfd)->no_wchar_size_warning)
	    {
	      _bfd_error_handler
	  ...
	  if (in_attr[i].i != AEABI_enum_unused)
		  ...
		  || out_attr[i].i == AEABI_enum_forced_wide)
		{
		  ...
		  out_attr[i].i = in_attr[i].i;
		       ...
		       && out_attr[i].i != in_attr[i].i
		       && !cgc_arm_tdata (obfd)->no_enum_size_warning)
		{
	...
	case Tag_DIV_use:
	  ...
	  if (in_attr[i].i == out_attr[i].i)
	    /* Do nothing.  */ ;
	  else if (cgc32_arm_attributes_forbid_div (in_attr)
		   && !cgc32_arm_attributes_accept_div (out_attr))
	    out_attr[i].i = 1;
	  else if (cgc32_arm_attributes_forbid_div (out_attr)
		   && cgc32_arm_attributes_accept_div (in_attr))
	    out_attr[i].i = in_attr[i].i;
	...
	default:
	  result
	    = result && _bfd_cgc_merge_unknown_attribute_low (ibfd, obfd, i);
	}
  ...
  if (!_bfd_cgc_merge_object_attributes (ibfd, obfd))
    return FALSE;

  /* Check for any attributes not known on ARM.  */
  result &= _bfd_cgc_merge_unknown_attribute_list (ibfd, obfd);
...
static bfd_boolean
cgc32_arm_versions_compatible (unsigned iver, unsigned over)
{
...
static bfd_boolean
cgc32_arm_merge_private_bfd_data (bfd * ibfd, bfd * obfd);

/* Display the flags field.  */

static bfd_boolean
cgc32_arm_print_private_bfd_data (bfd *abfd, void * ptr)
{
  FILE * file = (FILE *) ptr;
  unsigned long flags;
  ...
  switch (EF_ARM_EABI_VERSION (flags))
    ...
    case EF_ARM_EABI_UNKNOWN:
	 ...
	 official ARM CGC extended ABI.  Hence they are only decoded if
...
static int
cgc32_arm_get_symbol_type (Cgc_Internal_Sym * cgc_sym, int type)
{
  switch (CGC_ST_TYPE (cgc_sym->st_info))
    {
    case STT_ARM_TFUNC:
      return CGC_ST_TYPE (cgc_sym->st_info);
...
static asection *
cgc32_arm_gc_mark_hook (asection *sec,
			struct bfd_link_info *info,
			Cgc_Internal_Rela *rel,
			struct cgc_link_hash_entry *h,
			Cgc_Internal_Sym *sym)
...
static bfd_boolean
cgc32_arm_gc_sweep_hook (bfd *                     abfd,
			 struct bfd_link_info *    info,
			 asection *                sec,
			 const Cgc_Internal_Rela * relocs)
{
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	default:
	  ...
	  && cgc32_arm_get_plt_info (abfd, eh, r_symndx, &root_plt, &arm_plt))
	{
      ...
      if (may_become_dynamic_p)
	{
	  ...
	  if (h != NULL)
	  ...
	  else
	    {
	      ...
	      if (isym == NULL)
		return FALSE;
	      pp = cgc32_arm_get_local_dynreloc_list (abfd, r_symndx, isym);
	      if (pp == NULL)
		return FALSE;
	    }
...
static bfd_boolean
cgc32_arm_check_relocs (bfd *abfd, struct bfd_link_info *info,
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      switch (r_type)
	{
	  ...
	  case R_ARM_THM_TLS_CALL:
		...
		{
		  h->got.refcount++;
		  old_tls_type = cgc32_arm_hash_entry (h)->tls_type;
		}
	      else
		{
		  ...
		  if (!cgc32_arm_allocate_local_sym_info (abfd))
		    return FALSE;
		  cgc_local_got_refcounts (abfd)[r_symndx] += 1;
		  old_tls_type = cgc32_arm_local_got_tls_type (abfd) [r_symndx];
		}
		...
		{
		  if (h != NULL)
		    cgc32_arm_hash_entry (h)->tls_type = tls_type;
		  else
		    cgc32_arm_local_got_tls_type (abfd) [r_symndx] = tls_type;
		}
	    }
	    /* Fall through.  */
	  ...
	  case R_ARM_THM_MOVT_ABS:
		...
		(*_bfd_error_handler)
		  (_("%B: relocation %s against `%s' can not be used when making a shared object; recompile with -fPIC"),
		   abfd, cgc32_arm_howto_table_1[r_type].name,
      ...
      if (h != NULL)
	{
      ...
      if (may_need_local_target_p
	  && (h != NULL || CGC32_ST_TYPE (isym->st_info) == STT_GNU_IFUNC))
	{
	  union gotplt_union *root_plt;
	  struct arm_plt_info *arm_plt;
	  struct arm_local_iplt_info *local_iplt;
      ...
      if (may_become_dynamic_p)
	{
	  ...
	  if (h != NULL)
	    head = &((struct cgc32_arm_link_hash_entry *) h)->dyn_relocs;
	  else
	    {
	      head = cgc32_arm_get_local_dynreloc_list (abfd, r_symndx, isym);
	      if (head == NULL)
		return FALSE;
	    }
	  ...
	  if (p == NULL || p->sec != sec)
	    {
	      ...
	      p = (struct cgc_dyn_relocs *) bfd_alloc (htab->root.dynobj, amt);
	      if (p == NULL)
		return FALSE;
	      p->next = *head;
	      *head = p;
...
static bfd_boolean
cgc32_arm_gc_mark_extra_sections (struct bfd_link_info *info,
				  cgc_gc_mark_hook_fn gc_mark_hook)
{
  ...
  _bfd_cgc_gc_mark_extra_sections (info, gc_mark_hook);
  ...
  while (again)
      ...
      for (sub = info->input_bfds; sub != NULL; sub = sub->link_next)
	{
	  ...
	  cgc_shdrp = cgc_cgcsections (sub);
	  for (o = sub->sections; o != NULL; o = o->next)
	    {
	      Cgc_Internal_Shdr *hdr;

	      hdr = &cgc_section_data (o)->this_hdr;
	      if (hdr->sh_type == SHT_ARM_EXIDX
		  && hdr->sh_link
		  && hdr->sh_link < cgc_numsections (sub)
		  && !o->gc_mark
		  && cgc_shdrp[hdr->sh_link]->bfd_section->gc_mark)
		{
		  again = TRUE;
...
static bfd_boolean
cgc32_arm_is_target_special_symbol (bfd * abfd ATTRIBUTE_UNUSED, asymbol * sym)
{
  return bfd_is_arm_special_symbol_name (sym->name,
					 BFD_ARM_SPECIAL_SYM_TYPE_ANY);
}

/* This is a copy of cgc_find_function() from cgc.c except that
   ARM mapping symbols are ignored when looking for function names
   and STT_ARM_TFUNC is considered to a function type.  */

static bfd_boolean
arm_cgc_find_function (bfd *         abfd ATTRIBUTE_UNUSED,
		       asection *    section,
		       asymbol **    symbols,
		       bfd_vma       offset,
  ...
  for (p = symbols; *p != NULL; p++)
    {
      ...
      switch (CGC_ST_TYPE (q->internal_cgc_sym.st_info))
	{
	default:
	  break;
	case STT_FILE:
...
/* Find the nearest line to a particular section and offset, for error
   reporting.   This code is a duplicate of the code in cgc.c, except
   that it uses arm_cgc_find_function.  */

static bfd_boolean
cgc32_arm_find_nearest_line (bfd *          abfd,
			     asection *     section,
			     asymbol **     symbols,
			     bfd_vma        offset,
			     const char **  filename_ptr,
			     ...
			     unsigned int * line_ptr)
{
  ...
  if (_bfd_dwarf2_find_nearest_line (abfd, dwarf_debug_sections,
    ...
    {
      if (!*functionname_ptr)
	arm_cgc_find_function (abfd, section, symbols, offset,
  ...
  if (! arm_cgc_find_function (abfd, section, symbols, offset,
			       filename_ptr, functionname_ptr))
    return FALSE;
...
static bfd_boolean
cgc32_arm_find_inliner_info (bfd *          abfd,
			     const char **  filename_ptr,
			     const char **  functionname_ptr,
			     unsigned int * line_ptr)
{
...
static bfd_boolean
cgc32_arm_adjust_dynamic_symbol (struct bfd_link_info * info,
				 struct cgc_link_hash_entry * h)
{
  bfd * dynobj;
  asection * s;
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0 && h->size != 0)
    {
      ...
      cgc32_arm_allocate_dynrelocs (info, srel, 1);
      h->needs_copy = 1;
    }
...
static bfd_boolean
allocate_dynrelocs_for_symbol (struct cgc_link_hash_entry *h, void * inf)
{
  struct bfd_link_info *info;
  struct cgc32_arm_link_hash_table *htab;
  struct cgc32_arm_link_hash_entry *eh;
  ...
  if ((htab->root.dynamic_sections_created || h->type == STT_GNU_IFUNC)
	...
	{
	  cgc32_arm_allocate_plt_entry (info, eh->is_iplt, &h->plt, &eh->plt);
  ...
  if (h->got.refcount > 0)
    {
      asection *s;
      bfd_boolean dyn;
      int tls_type = cgc32_arm_hash_entry (h)->tls_type;
      int indx;
      ...
      if (!htab->symbian_p)
	{
	      ...
	      if (tls_type & GOT_TLS_GDESC)
		{
		  ...
		  eh->tlsdesc_got
		    = (htab->root.sgotplt->size
		       - cgc32_arm_compute_jump_table_size (htab));
	  ...
	  if (tls_type != GOT_NORMAL
	    ...
	    {
	      if (tls_type & GOT_TLS_IE)
		cgc32_arm_allocate_dynrelocs (info, htab->root.srelgot, 1);
	    ...
	    cgc32_arm_allocate_irelocs (info, htab->root.srelgot, 1);
	  else if (info->shared && (CGC_ST_VISIBILITY (h->other) == STV_DEFAULT
				    || h->root.type != bfd_link_hash_undefweak))
  ...
  if (!htab->use_blx && h->dynindx != -1
      ...
      && CGC_ST_VISIBILITY (h->other) == STV_DEFAULT)
    {
      ...
      h->type = CGC_ST_INFO (CGC_ST_BIND (h->type), STT_FUNC);
      h->target_internal = ST_BRANCH_TO_ARM;
  ...
  for (p = eh->dyn_relocs; p != NULL; p = p->next)
    {
	  ...
	  && eh->plt.noncall_refcount == 0
	  && SYMBOL_REFERENCES_LOCAL (info, h))
	cgc32_arm_allocate_irelocs (info, sreloc, p->count);
      else
	cgc32_arm_allocate_dynrelocs (info, sreloc, p->count);
    }
...
static bfd_boolean
cgc32_arm_readonly_dynrelocs (struct cgc_link_hash_entry * h, void * inf)
{
  struct cgc32_arm_link_hash_entry * eh;
  struct cgc_dyn_relocs * p;
...
void
bfd_cgc32_arm_set_byteswap_code (struct bfd_link_info *info,
				 int byteswap_code)
{
  struct cgc32_arm_link_hash_table *globals;
...
static bfd_boolean
cgc32_arm_size_dynamic_sections (bfd * output_bfd ATTRIBUTE_UNUSED,
				 struct bfd_link_info * info)
{
  bfd * dynobj;
  asection * s;
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = (struct cgc_dyn_relocs *)
		   cgc_section_data (s)->local_dynrel; p != NULL; p = p->next)
	    {
		...
		{
		  srel = cgc_section_data (p->sec)->sreloc;
		  cgc32_arm_allocate_dynrelocs (info, srel, p->count);
	...
	continue;
      ...
      local_iplt_ptr = cgc32_arm_local_iplt (ibfd);
      local_tls_type = cgc32_arm_local_got_tls_type (ibfd);
      local_tlsdesc_gotent = cgc32_arm_local_tlsdesc_gotent (ibfd);
      symndx = 0;
      s = htab->root.sgot;
	   ...
	   ++local_tlsdesc_gotent, ++symndx)
	{
	  *local_tlsdesc_gotent = (bfd_vma) -1;
	  ...
	  if (local_iplt != NULL)
		...
		{
		  cgc32_arm_allocate_plt_entry (info, TRUE,
						&local_iplt->root,
						&local_iplt->arm);
	      ...
	      for (p = local_iplt->dyn_relocs; p != NULL; p = p->next)
		{
		  ...
		  psrel = cgc_section_data (p->sec)->sreloc;
		  if (local_iplt->arm.noncall_refcount == 0)
		    cgc32_arm_allocate_irelocs (info, psrel, p->count);
		  else
		    cgc32_arm_allocate_dynrelocs (info, psrel, p->count);
		}
	    }
	  if (*local_got > 0)
	    {
		...
		cgc32_arm_allocate_irelocs (info, srel, 1);
	      else if (info->shared || output_bfd->flags & DYNAMIC)
		{
		  ...
		  if (info->shared && *local_tls_type & GOT_TLS_GDESC)
		    {
		      cgc32_arm_allocate_dynrelocs (info,
						    htab->root.srelplt, 1);
		      htab->tls_trampoline = -1;
		    }
		}
  ...
  /* Allocate global sym .plt and .got entries, and space for global
     sym dynamic relocs.  */
  cgc_link_hash_traverse (& htab->root, allocate_dynrelocs_for_symbol, info);
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
	...
	continue;
      ...
      bfd_cgc32_arm_init_maps (ibfd);

      if (!bfd_cgc32_arm_process_before_allocation (ibfd, info)
	  || !bfd_cgc32_arm_vfp11_erratum_scan (ibfd, info))
	/* xgettext:c-format */
  ...
  bfd_cgc32_arm_allocate_interworking_sections (info);
  ...
  if (htab->root.srelplt)
    htab->sgotplt_jump_table_size = cgc32_arm_compute_jump_table_size(htab);
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in cgc32_arm_finish_dynamic_sections, but we
...
#define add_dynamic_entry(TAG, VAL) \
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
	...
	cgc_link_hash_traverse (& htab->root, cgc32_arm_readonly_dynrelocs,
				info);
...
static bfd_boolean
cgc32_arm_always_size_sections (bfd *output_bfd,
				struct bfd_link_info *info)
{
  asection *tls_sec;
  ...
  tls_sec = cgc_hash_table (info)->tls_sec;

  if (tls_sec)
    {
      struct cgc_link_hash_entry *tlsbase;

      tlsbase = cgc_link_hash_lookup
	(cgc_hash_table (info), "_TLS_MODULE_BASE_", TRUE, TRUE, FALSE);

      if (tlsbase)
	{
	  struct bfd_link_hash_entry *bh = NULL;
	  const struct cgc_backend_data *bed
	    = get_cgc_backend_data (output_bfd);
	  ...
	  tlsbase = (struct cgc_link_hash_entry *)bh;
	  tlsbase->def_regular = 1;
	  tlsbase->other = STV_HIDDEN;
	  (*bed->cgc_backend_hide_symbol) (info, tlsbase, TRUE);
	}
    }
  return TRUE;
}
...
static bfd_boolean
cgc32_arm_finish_dynamic_symbol (bfd * output_bfd,
				 struct bfd_link_info * info,
				 struct cgc_link_hash_entry * h,
				 Cgc_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
      ...
      if (!eh->is_iplt)
	{
	  ...
	  cgc32_arm_populate_plt_entry (output_bfd, info, &h->plt, &eh->plt,
					h->dynindx, 0);
	}
      ...
      else if (eh->is_iplt && eh->plt.noncall_refcount != 0)
	{
	  ...
	  sym->st_info = CGC_ST_INFO (CGC_ST_BIND (sym->st_info), STT_FUNC);
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_info = CGC32_R_INFO (h->dynindx, R_ARM_COPY);
      cgc32_arm_add_dynreloc (output_bfd, info, s, &rel);
    }
...
static void
arm_put_trampoline (struct cgc32_arm_link_hash_table *htab, bfd *output_bfd,
		    void *contents,
...
/* Install the special first PLT entry for cgc32-arm-nacl.  Unlike
   ...
   of .Lplt_tail.  */
static void
arm_nacl_put_plt0 (struct cgc32_arm_link_hash_table *htab, bfd *output_bfd,
		   asection *plt, bfd_vma got_displacement)
{
  unsigned int i;

  put_arm_insn (htab, output_bfd,
		cgc32_arm_nacl_plt0_entry[0]
		| arm_movw_immediate (got_displacement),
		plt->contents + 0);
  put_arm_insn (htab, output_bfd,
		cgc32_arm_nacl_plt0_entry[1]
		| arm_movt_immediate (got_displacement),
		plt->contents + 4);

  for (i = 2; i < ARRAY_SIZE (cgc32_arm_nacl_plt0_entry); ++i)
    put_arm_insn (htab, output_bfd,
		  cgc32_arm_nacl_plt0_entry[i],
		  plt->contents + (i * 4));
}
...
static bfd_boolean
cgc32_arm_finish_dynamic_sections (bfd * output_bfd, struct bfd_link_info * info)
{
  bfd * dynobj;
  asection * sgot;
  asection * sdyn;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      asection *splt;
      Cgc32_External_Dyn *dyncon, *dynconend;

      splt = htab->root.splt;
      BFD_ASSERT (splt != NULL && sdyn != NULL);
      BFD_ASSERT (htab->symbian_p || sgot != NULL);

      dyncon = (Cgc32_External_Dyn *) sdyn->contents;
      dynconend = (Cgc32_External_Dyn *) (sdyn->contents + sdyn->size);
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  bfd_cgc32_swap_dyn_in (dynobj, dyncon, &dyn);
	  ...
	  switch (dyn.d_tag)
		...
		bfd_cgc32_swap_dyn_out (output_bfd, &dyn, dyncon);
	      break;
	    ...
	    case DT_RELA:
		 ...
		 cgclink.c, but do not check for SHF_ALLOC on the
	      ...
	      if (htab->symbian_p)
		{
		  ...
		  type = ((dyn.d_tag == DT_REL || dyn.d_tag == DT_RELSZ)
			  ? SHT_REL : SHT_RELA);
		  dyn.d_un.d_val = 0;
		  for (i = 1; i < cgc_numsections (output_bfd); i++)
		    {
		      Cgc_Internal_Shdr *hdr
			= cgc_cgcsections (output_bfd)[i];
		      if (hdr->sh_type == type)
			{
	    ...
	    get_sym:
	      /* If it wasn't set by cgc_bfd_final_link
		 then there is nothing to adjust.  */
	      if (dyn.d_un.d_val != 0)
		{
		  struct cgc_link_hash_entry * eh;

		  eh = cgc_link_hash_lookup (cgc_hash_table (info), name,
					     FALSE, FALSE, TRUE);
	  ...
	  if (htab->vxworks_p)
	    {
		 ...
		 computing the values now.  */
	      ...
	      plt0_entry = cgc32_arm_vxworks_exec_plt0_entry;
	  ...
	  else
	    {
	      ...
	      plt0_entry = cgc32_arm_plt0_entry;
	      put_arm_insn (htab, output_bfd, plt0_entry[0],
			    splt->contents + 0);
	      put_arm_insn (htab, output_bfd, plt0_entry[1],
			    splt->contents + 4);
	...
	cgc_section_data (splt->output_section)->this_hdr.sh_entsize = 4;
  ...
  if (sgot)
      ...
      if (sgot->size > 0)
	{
      ...
      cgc_section_data (sgot->output_section)->this_hdr.sh_entsize = 4;
    }
...
static void
cgc32_arm_post_process_headers (bfd * abfd, struct bfd_link_info * link_info ATTRIBUTE_UNUSED)
{
  ...
  if (EF_ARM_EABI_VERSION (i_ehdrp->e_flags) == EF_ARM_EABI_UNKNOWN)
    i_ehdrp->e_ident[EI_OSABI] = CGCOSABI_ARM;
  else
    i_ehdrp->e_ident[EI_OSABI] = 0;
  i_ehdrp->e_ident[EI_ABIVERSION] = ARM_CGC_ABI_VERSION;
  ...
  if (EF_ARM_EABI_VERSION (i_ehdrp->e_flags) == EF_ARM_EABI_VER5
    ...
    {
      int abi = bfd_cgc_get_obj_attr_int (abfd, OBJ_ATTR_PROC, Tag_ABI_VFP_args);
      if (abi)
...
cgc32_arm_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
static void
cgc32_arm_final_write_processing (bfd *abfd, bfd_boolean linker ATTRIBUTE_UNUSED)
{
...
static bfd_boolean
is_arm_cgc_unwind_section_name (bfd * abfd ATTRIBUTE_UNUSED, const char * name)
{
  return (CONST_STRNEQ (name, CGC_STRING_ARM_unwind)
	  || CONST_STRNEQ (name, CGC_STRING_ARM_unwind_once));
}
...
static bfd_boolean
cgc32_arm_fake_sections (bfd * abfd, Cgc_Internal_Shdr * hdr, asection * sec)
{
  const char * name;

  name = bfd_get_section_name (abfd, sec);

  if (is_arm_cgc_unwind_section_name (abfd, name))
    {
...
static bfd_boolean
cgc32_arm_section_from_shdr (bfd *abfd,
			     Cgc_Internal_Shdr * hdr,
			     const char *name,
			     int shindex)
{
...
static _arm_cgc_section_data *
get_arm_cgc_section_data (asection * sec)
{
  if (sec && sec->owner && is_arm_cgc (sec->owner))
    return cgc32_arm_section_data (sec);
  else
    return NULL;
}
...
typedef struct
{
  ...
  asection *sec;
  int sec_shndx;
  int (*func) (void *, const char *, Cgc_Internal_Sym *,
...
static bfd_boolean
cgc32_arm_output_map_sym (output_arch_syminfo *osi,
			  ...
			  bfd_vma offset)
{
  ...
  sym.st_size = 0;
  sym.st_other = 0;
  sym.st_info = CGC_ST_INFO (STB_LOCAL, STT_NOTYPE);
  sym.st_shndx = osi->sec_shndx;
  sym.st_target_internal = 0;
  cgc32_arm_section_map_add (osi->sec, names[type][1], offset);
...
static bfd_boolean
cgc32_arm_output_plt_map_1 (output_arch_syminfo *osi,
			    ...
			    struct arm_plt_info *arm_plt)
{
  ...
  htab = cgc32_arm_hash_table (osi->info);
  if (htab == NULL)
    return FALSE;
  ...
  osi->sec_shndx = (_bfd_cgc_section_from_bfd_section
		    (osi->info->output_bfd, osi->sec->output_section));

  addr = root_plt->offset & -2;
  if (htab->symbian_p)
    {
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_ARM, addr))
	return FALSE;
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_DATA, addr + 4))
	return FALSE;
    }
  else if (htab->vxworks_p)
    {
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_ARM, addr))
	return FALSE;
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_DATA, addr + 8))
	return FALSE;
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_ARM, addr + 12))
	return FALSE;
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_DATA, addr + 20))
	return FALSE;
    }
  else if (htab->nacl_p)
    {
  ...
  else
    {
      ...
      thumb_stub_p = cgc32_arm_plt_needs_thumb_stub_p (osi->info, arm_plt);
      if (thumb_stub_p)
	{
	  if (!cgc32_arm_output_map_sym (osi, ARM_MAP_THUMB, addr - 4))
	    return FALSE;
	}
#ifdef FOUR_WORD_PLT
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_ARM, addr))
	return FALSE;
      if (!cgc32_arm_output_map_sym (osi, ARM_MAP_DATA, addr + 12))
	return FALSE;
#else
...
static bfd_boolean
cgc32_arm_output_plt_map (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  eh = (struct cgc32_arm_link_hash_entry *) h;
  return cgc32_arm_output_plt_map_1 (osi, SYMBOL_CALLS_LOCAL (osi->info, h),
				     &h->plt, &eh->plt);
}
...
static bfd_boolean
cgc32_arm_output_stub_sym (output_arch_syminfo *osi, const char *name,
			   bfd_vma offset, bfd_vma size)
{
  ...
  sym.st_size = size;
  sym.st_other = 0;
  sym.st_info = CGC_ST_INFO (STB_LOCAL, STT_FUNC);
  sym.st_shndx = osi->sec_shndx;
  sym.st_target_internal = 0;
		  ...
		  void * in_arg)
{
  ...
  switch (template_sequence[0].type)
    {
    case ARM_TYPE:
      if (!cgc32_arm_output_stub_sym (osi, stub_name, addr, stub_entry->stub_size))
	return FALSE;
      break;
    case THUMB16_TYPE:
    case THUMB32_TYPE:
      if (!cgc32_arm_output_stub_sym (osi, stub_name, addr | 1,
  ...
  for (i = 0; i < stub_entry->stub_template_size; i++)
      ...
      if (template_sequence[i].type != prev_type)
	{
	  ...
	  if (!cgc32_arm_output_map_sym (osi, sym_type, addr + size))
	    return FALSE;
	}
...
static bfd_boolean
cgc32_arm_output_arch_local_syms (bfd *output_bfd,
				  struct bfd_link_info *info,
				  void *flaginfo,
					       ...
					       struct cgc_link_hash_entry *))
{
	...
	for (osi.sec = input_bfd->sections;
	     ...
	     osi.sec = osi.sec->next)
	  {
		...
		&& (osi.sec->flags & (SEC_HAS_CONTENTS | SEC_LINKER_CREATED))
		   == SEC_HAS_CONTENTS
		&& get_arm_cgc_section_data (osi.sec) != NULL
		&& get_arm_cgc_section_data (osi.sec)->mapcount == 0
		&& osi.sec->size > 0
		...
		if (osi.sec_shndx != (int)SHN_BAD)
		  cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA, 0);
	      }
	  }
    }
  ...
  if (htab->arm_glue_size > 0)
	...
	{
	  cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, offset);
	  cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA, offset + size - 4);
	}
    }
  ...
  if (htab->thumb_glue_size > 0)
	...
	{
	  cgc32_arm_output_map_sym (&osi, ARM_MAP_THUMB, offset);
	  cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, offset + 4);
	}
    }
  ...
  if (htab->bx_glue_size > 0)
    {
      ...
      cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, 0);
    }
  ...
  if (htab->root.splt && htab->root.splt->size > 0)
      ...
      if (htab->vxworks_p)
	{
	  ...
	  if (!info->shared)
	    {
	      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, 0))
		return FALSE;
	      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA, 12))
		return FALSE;
	    }
	}
      else if (htab->nacl_p)
      ...
      else if (!htab->symbian_p)
	{
	  ...
	  if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA, 16))
	    return FALSE;
#endif
	}
    }
  ...
  if ((htab->root.splt && htab->root.splt->size > 0)
    ...
    {
      cgc_link_hash_traverse (&htab->root, cgc32_arm_output_plt_map, &osi);
      for (input_bfd = info->input_bfds;
	   input_bfd != NULL;
	   input_bfd = input_bfd->link_next)
	{
	  ...
	  local_iplt = cgc32_arm_local_iplt (input_bfd);
	  if (local_iplt != NULL)
	    {
	      num_syms = cgc_symtab_hdr (input_bfd).sh_info;
	      for (i = 0; i < num_syms; i++)
		if (local_iplt[i] != NULL
		    && !cgc32_arm_output_plt_map_1 (&osi, TRUE,
						    &local_iplt[i]->root,
						    &local_iplt[i]->arm))
		  return FALSE;
	    }
  ...
  if (htab->dt_tlsdesc_plt != 0)
    {
      ...
      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, htab->dt_tlsdesc_plt))
	return FALSE;

      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA,
  ...
  if (htab->tls_trampoline != 0)
    {
      ...
      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_ARM, htab->tls_trampoline))
	return FALSE;
#ifdef FOUR_WORD_PLT
      if (!cgc32_arm_output_map_sym (&osi, ARM_MAP_DATA,
...
static bfd_boolean
cgc32_arm_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      _arm_cgc_section_data *sdata;
      bfd_size_type amt = sizeof (*sdata);

      sdata = (_arm_cgc_section_data *) bfd_zalloc (abfd, amt);
      if (sdata == NULL)
	return FALSE;
...
static int
cgc32_arm_compare_mapping (const void * a, const void * b)
{
  const cgc32_arm_section_map *amap = (const cgc32_arm_section_map *) a;
  const cgc32_arm_section_map *bmap = (const cgc32_arm_section_map *) b;
		       ...
		       void *in_arg)
{
  ...
  /* We attempt to avoid this condition by setting stubs_always_after_branch
     in cgc32_arm_size_stubs if we've enabled the Cortex-A8 erratum workaround.
...
static bfd_boolean
cgc32_arm_write_section (bfd *output_bfd,
			 struct bfd_link_info *link_info,
			 asection *sec,
			 bfd_byte *contents)
{
  unsigned int mapcount, errcount;
  _arm_cgc_section_data *arm_data;
  struct cgc32_arm_link_hash_table *globals = cgc32_arm_hash_table (link_info);
  cgc32_arm_section_map *map;
  cgc32_vfp11_erratum_list *errnode;
  bfd_vma ptr;
  bfd_vma end;
  ...
  /* If this section has not been allocated an _arm_cgc_section_data
     structure then we cannot record anything.  */
  arm_data = get_arm_cgc_section_data (sec);
  if (arm_data == NULL)
    return FALSE;
  ...
  if (arm_data->cgc.this_hdr.sh_type == SHT_ARM_EXIDX)
    {
  ...
  if (globals->byteswap_code)
    {
      qsort (map, mapcount, sizeof (* map), cgc32_arm_compare_mapping);
...
static bfd_boolean
cgc32_arm_swap_symbol_in (bfd * abfd,
			  const void *psrc,
			  const void *pshn,
			  Cgc_Internal_Sym *dst)
{
  if (!bfd_cgc32_swap_symbol_in (abfd, psrc, pshn, dst))
    return FALSE;

  /* New EABI objects mark thumb function symbols by setting the low bit of
     the address.  */
  if (CGC_ST_TYPE (dst->st_info) == STT_FUNC
      || CGC_ST_TYPE (dst->st_info) == STT_GNU_IFUNC)
    {
      if (dst->st_value & 1)
	{
  ...
  else if (CGC_ST_TYPE (dst->st_info) == STT_ARM_TFUNC)
    {
      dst->st_info = CGC_ST_INFO (CGC_ST_BIND (dst->st_info), STT_FUNC);
      dst->st_target_internal = ST_BRANCH_TO_THUMB;
    }
  else if (CGC_ST_TYPE (dst->st_info) == STT_SECTION)
...
static void
cgc32_arm_swap_symbol_out (bfd *abfd,
			   const Cgc_Internal_Sym *src,
			   void *cdst,
			   void *shndx)
{
  Cgc_Internal_Sym newsym;

  /* We convert STT_ARM_TFUNC symbols into STT_FUNC with the low bit
     of the address set, as per the new EABI.  We do this unconditionally
     because objcopy does not set the cgc header flags until after
     it writes out the symbol table.  */
  if (src->st_target_internal == ST_BRANCH_TO_THUMB)
    {
      newsym = *src;
      if (CGC_ST_TYPE (src->st_info) != STT_GNU_IFUNC)
	newsym.st_info = CGC_ST_INFO (CGC_ST_BIND (src->st_info), STT_FUNC);
  ...
  bfd_cgc32_swap_symbol_out (abfd, src, cdst, shndx);
}

/* Add the PT_ARM_EXIDX program header.  */

static bfd_boolean
cgc32_arm_modify_segment_map (bfd *abfd,
			      struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  ...
  if (sec != NULL && (sec->flags & SEC_LOAD) != 0)
	...
	m = m->next;
      if (!m)
	{
	  m = (struct cgc_segment_map *)
	      bfd_zalloc (abfd, sizeof (struct cgc_segment_map));
	  if (m == NULL)
	    return FALSE;
...
static int
cgc32_arm_additional_program_headers (bfd *abfd,
				      struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  asection *sec;
...
static bfd_boolean
cgc32_arm_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
			   ...
			   flagword *flagsp, asection **secp, bfd_vma *valp)
{
  ...
  if (cgc32_arm_hash_table (info)->vxworks_p
      && !cgc_vxworks_add_symbol_hook (abfd, info, sym, namep,
				       flagsp, secp, valp))
    return FALSE;
...
const struct cgc_size_info cgc32_arm_size_info =
{
  sizeof (Cgc32_External_Ehdr),
  sizeof (Cgc32_External_Phdr),
  sizeof (Cgc32_External_Shdr),
  sizeof (Cgc32_External_Rel),
  sizeof (Cgc32_External_Rela),
  sizeof (Cgc32_External_Sym),
  sizeof (Cgc32_External_Dyn),
  sizeof (Cgc_External_Note),
  4,
  1,
  32, 2,
  CGCCLASS32, EV_CURRENT,
  bfd_cgc32_write_out_phdrs,
  bfd_cgc32_write_shdrs_and_ehdr,
  bfd_cgc32_checksum_contents,
  bfd_cgc32_write_relocs,
  cgc32_arm_swap_symbol_in,
  cgc32_arm_swap_symbol_out,
  bfd_cgc32_slurp_reloc_table,
  bfd_cgc32_slurp_symbol_table,
  bfd_cgc32_swap_dyn_in,
  bfd_cgc32_swap_dyn_out,
  bfd_cgc32_swap_reloc_in,
  bfd_cgc32_swap_reloc_out,
  bfd_cgc32_swap_reloca_in,
  bfd_cgc32_swap_reloca_out
...
#define CGC_ARCH			bfd_arch_arm
#define CGC_TARGET_ID			ARM_CGC_DATA
#define CGC_MACHINE_CODE		EM_ARM
#ifdef __QNXTARGET__
#define CGC_MAXPAGESIZE			0x1000
#else
#define CGC_MAXPAGESIZE			0x8000
#endif
#define CGC_MINPAGESIZE			0x1000
#define CGC_COMMONPAGESIZE		0x1000

#define bfd_cgc32_mkobject		        cgc32_arm_mkobject

#define bfd_cgc32_bfd_copy_private_bfd_data	cgc32_arm_copy_private_bfd_data
#define bfd_cgc32_bfd_merge_private_bfd_data	cgc32_arm_merge_private_bfd_data
#define bfd_cgc32_bfd_set_private_flags		cgc32_arm_set_private_flags
#define bfd_cgc32_bfd_print_private_bfd_data	cgc32_arm_print_private_bfd_data
#define bfd_cgc32_bfd_link_hash_table_create    cgc32_arm_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free      cgc32_arm_hash_table_free
#define bfd_cgc32_bfd_reloc_type_lookup		cgc32_arm_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		cgc32_arm_reloc_name_lookup
#define bfd_cgc32_find_nearest_line	        cgc32_arm_find_nearest_line
#define bfd_cgc32_find_inliner_info	        cgc32_arm_find_inliner_info
#define bfd_cgc32_new_section_hook		cgc32_arm_new_section_hook
#define bfd_cgc32_bfd_is_target_special_symbol	cgc32_arm_is_target_special_symbol
#define bfd_cgc32_bfd_final_link		cgc32_arm_final_link

#define cgc_backend_get_symbol_type             cgc32_arm_get_symbol_type
#define cgc_backend_gc_mark_hook                cgc32_arm_gc_mark_hook
#define cgc_backend_gc_mark_extra_sections	cgc32_arm_gc_mark_extra_sections
#define cgc_backend_gc_sweep_hook               cgc32_arm_gc_sweep_hook
#define cgc_backend_check_relocs                cgc32_arm_check_relocs
#define cgc_backend_relocate_section		cgc32_arm_relocate_section
#define cgc_backend_write_section		cgc32_arm_write_section
#define cgc_backend_adjust_dynamic_symbol	cgc32_arm_adjust_dynamic_symbol
#define cgc_backend_create_dynamic_sections     cgc32_arm_create_dynamic_sections
#define cgc_backend_finish_dynamic_symbol	cgc32_arm_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections	cgc32_arm_finish_dynamic_sections
#define cgc_backend_size_dynamic_sections	cgc32_arm_size_dynamic_sections
#define cgc_backend_always_size_sections	cgc32_arm_always_size_sections
#define cgc_backend_init_index_section		_bfd_cgc_init_2_index_sections
#define cgc_backend_post_process_headers	cgc32_arm_post_process_headers
#define cgc_backend_reloc_type_class		cgc32_arm_reloc_type_class
#define cgc_backend_object_p			cgc32_arm_object_p
#define cgc_backend_fake_sections  		cgc32_arm_fake_sections
#define cgc_backend_section_from_shdr  		cgc32_arm_section_from_shdr
#define cgc_backend_final_write_processing      cgc32_arm_final_write_processing
#define cgc_backend_copy_indirect_symbol        cgc32_arm_copy_indirect_symbol
#define cgc_backend_size_info			cgc32_arm_size_info
#define cgc_backend_modify_segment_map		cgc32_arm_modify_segment_map
#define cgc_backend_additional_program_headers  cgc32_arm_additional_program_headers
#define cgc_backend_output_arch_local_syms      cgc32_arm_output_arch_local_syms
#define cgc_backend_begin_write_processing      cgc32_arm_begin_write_processing
#define cgc_backend_add_symbol_hook		cgc32_arm_add_symbol_hook
...
#define cgc_backend_may_use_rel_p      1
#define cgc_backend_may_use_rela_p     0
#define cgc_backend_default_use_rela_p 0

#define cgc_backend_got_header_size	12

#undef  cgc_backend_obj_attrs_vendor
#define cgc_backend_obj_attrs_vendor		"aeabi"
#undef  cgc_backend_obj_attrs_section
#define cgc_backend_obj_attrs_section		".ARM.attributes"
#undef  cgc_backend_obj_attrs_arg_type
#define cgc_backend_obj_attrs_arg_type		cgc32_arm_obj_attrs_arg_type
#undef  cgc_backend_obj_attrs_section_type
#define cgc_backend_obj_attrs_section_type	SHT_ARM_ATTRIBUTES
#define cgc_backend_obj_attrs_order		cgc32_arm_obj_attrs_order
#define cgc_backend_obj_attrs_handle_unknown 	cgc32_arm_obj_attrs_handle_unknown
...
#define TARGET_LITTLE_SYM		bfd_cgc32_littlearm_nacl_vec
#undef	TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME		"cgc32-littlearm-nacl"
#undef	TARGET_BIG_SYM
#define TARGET_BIG_SYM			bfd_cgc32_bigarm_nacl_vec
#undef	TARGET_BIG_NAME
#define TARGET_BIG_NAME			"cgc32-bigarm-nacl"

/* Like cgc32_arm_link_hash_table_create -- but overrides
   appropriately for NaCl.  */

static struct bfd_link_hash_table *
cgc32_arm_nacl_link_hash_table_create (bfd *abfd)
{
  struct bfd_link_hash_table *ret;

  ret = cgc32_arm_link_hash_table_create (abfd);
  if (ret)
    {
      struct cgc32_arm_link_hash_table *htab
	= (struct cgc32_arm_link_hash_table *) ret;
      ...
      htab->plt_header_size = 4 * ARRAY_SIZE (cgc32_arm_nacl_plt0_entry);
      htab->plt_entry_size = 4 * ARRAY_SIZE (cgc32_arm_nacl_plt_entry);
    }
  return ret;
}

/* Since NaCl doesn't use the ARM-specific unwind format, we don't
   really need to use cgc32_arm_modify_segment_map.  But we do it
   anyway just to reduce gratuitous differences with the stock ARM backend.  */

static bfd_boolean
cgc32_arm_nacl_modify_segment_map (bfd *abfd, struct bfd_link_info *info)
{
  return (cgc32_arm_modify_segment_map (abfd, info)
	  && nacl_modify_segment_map (abfd, info));
}

static void
cgc32_arm_nacl_final_write_processing (bfd *abfd, bfd_boolean linker)
{
  cgc32_arm_final_write_processing (abfd, linker);
  nacl_final_write_processing (abfd, linker);
}


#undef	cgc32_bed
#define cgc32_bed			cgc32_arm_nacl_bed
#undef  bfd_cgc32_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_create	\
  cgc32_arm_nacl_link_hash_table_create
#undef	cgc_backend_plt_alignment
#define cgc_backend_plt_alignment	4
#undef	cgc_backend_modify_segment_map
#define	cgc_backend_modify_segment_map		cgc32_arm_nacl_modify_segment_map
#undef	cgc_backend_modify_program_headers
#define	cgc_backend_modify_program_headers	nacl_modify_program_headers
#undef  cgc_backend_final_write_processing
#define cgc_backend_final_write_processing	cgc32_arm_nacl_final_write_processing

#undef	CGC_MAXPAGESIZE
#define CGC_MAXPAGESIZE			0x10000
#undef	CGC_MINPAGESIZE
#undef	CGC_COMMONPAGESIZE
...
#undef  TARGET_LITTLE_SYM
#define TARGET_LITTLE_SYM               bfd_cgc32_littlearm_vxworks_vec
#undef  TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME              "cgc32-littlearm-vxworks"
#undef  TARGET_BIG_SYM
#define TARGET_BIG_SYM                  bfd_cgc32_bigarm_vxworks_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME                 "cgc32-bigarm-vxworks"

/* Like cgc32_arm_link_hash_table_create -- but overrides
   appropriately for VxWorks.  */

static struct bfd_link_hash_table *
cgc32_arm_vxworks_link_hash_table_create (bfd *abfd)
{
  struct bfd_link_hash_table *ret;
...
static void
cgc32_arm_vxworks_final_write_processing (bfd *abfd, bfd_boolean linker)
{
  cgc32_arm_final_write_processing (abfd, linker);
...
#undef  cgc32_bed
#define cgc32_bed cgc32_arm_vxworks_bed

#undef  bfd_cgc32_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_create	cgc32_arm_vxworks_link_hash_table_create
#undef  cgc_backend_final_write_processing
#define cgc_backend_final_write_processing	cgc32_arm_vxworks_final_write_processing
#undef  cgc_backend_emit_relocs
#define cgc_backend_emit_relocs			cgc_vxworks_emit_relocs

#undef  cgc_backend_may_use_rel_p
#define cgc_backend_may_use_rel_p	0
#undef  cgc_backend_may_use_rela_p
#define cgc_backend_may_use_rela_p	1
#undef  cgc_backend_default_use_rela_p
#define cgc_backend_default_use_rela_p	1
#undef  cgc_backend_want_plt_sym
...
cgc32_arm_merge_private_bfd_data (bfd * ibfd, bfd * obfd)
{
  ...
  if (!cgc32_arm_merge_eabi_attributes (ibfd, obfd))
    return FALSE;

  /* The input BFD must have had its flags initialised.  */
  /* The following seems bogus to me -- The flags are initialized in
     the assembler but I don't think an cgc_flags_init field is
     written into the object.  */
  /* BFD_ASSERT (cgc_flags_init (ibfd)); */
  ...
  if (!cgc32_arm_versions_compatible (EF_ARM_EABI_VERSION (in_flags),
  ...
  if (get_cgc_backend_data (obfd) != &cgc32_arm_vxworks_bed
      && get_cgc_backend_data (ibfd) != &cgc32_arm_vxworks_bed
...
#undef  TARGET_LITTLE_SYM
#define TARGET_LITTLE_SYM               bfd_cgc32_littlearm_symbian_vec
#undef  TARGET_LITTLE_NAME
#define TARGET_LITTLE_NAME              "cgc32-littlearm-symbian"
#undef  TARGET_BIG_SYM
#define TARGET_BIG_SYM                  bfd_cgc32_bigarm_symbian_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME                 "cgc32-bigarm-symbian"

/* Like cgc32_arm_link_hash_table_create -- but overrides
   appropriately for Symbian OS.  */

static struct bfd_link_hash_table *
cgc32_arm_symbian_link_hash_table_create (bfd *abfd)
{
  struct bfd_link_hash_table *ret;
  ...
  if (ret)
	...
	= (struct cgc32_arm_link_hash_table *)ret;
      ...
      htab->plt_entry_size = 4 * ARRAY_SIZE (cgc32_arm_symbian_plt_entry);
      htab->symbian_p = 1;
...
static const struct bfd_cgc_special_section
cgc32_arm_symbian_special_sections[] =
{
...
static void
cgc32_arm_symbian_begin_write_processing (bfd *abfd,
					  struct bfd_link_info *link_info)
{
  /* BPABI objects are never loaded directly by an OS kernel; they are
     ...
     loadable segment.  */
  abfd->flags &= ~D_PAGED;
  cgc32_arm_begin_write_processing (abfd, link_info);
}

static bfd_boolean
cgc32_arm_symbian_modify_segment_map (bfd *abfd,
				      struct bfd_link_info *info)
{
  struct cgc_segment_map *m;
  asection *dynsec;
  ...
  /* BPABI shared libraries and executables should have a PT_DYNAMIC
     ...
     with SEC_LOAD, the generic CGC code will not create such a
     segment.  */
...
static bfd_vma
cgc32_arm_symbian_plt_sym_val (bfd_vma i, const asection *plt,
			       const arelent *rel ATTRIBUTE_UNUSED)
{
  return plt->vma + 4 * ARRAY_SIZE (cgc32_arm_symbian_plt_entry) * i;
}


#undef  cgc32_bed
#define cgc32_bed cgc32_arm_symbian_bed

/* The dynamic sections are not allocated on SymbianOS; the postlinker
   will process them and then discard them.  */
#undef  CGC_DYNAMIC_SEC_FLAGS
...
#undef  bfd_cgc32_bfd_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_create	cgc32_arm_symbian_link_hash_table_create
#undef  cgc_backend_special_sections
#define cgc_backend_special_sections 		cgc32_arm_symbian_special_sections
#undef  cgc_backend_begin_write_processing
#define cgc_backend_begin_write_processing	cgc32_arm_symbian_begin_write_processing
#undef  cgc_backend_final_write_processing
#define cgc_backend_final_write_processing	cgc32_arm_final_write_processing

#undef  cgc_backend_modify_segment_map
#define cgc_backend_modify_segment_map cgc32_arm_symbian_modify_segment_map
...
#undef  cgc_backend_plt_sym_val
#define cgc_backend_plt_sym_val		cgc32_arm_symbian_plt_sym_val
