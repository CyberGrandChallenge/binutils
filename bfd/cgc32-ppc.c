/* PowerPC-specific support for 32-bit CGC
...
/* This file is based on a preliminary PowerPC CGC ABI.  The
   information may not match the final PowerPC CGC ABI.  It includes
...
static bfd_reloc_status_type ppc_cgc_addr16_ha_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);
static bfd_reloc_status_type ppc_cgc_unhandled_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);
static void ppc_cgc_vle_split16
...
static const bfd_vma ppc_cgc_vxworks_plt_entry
...
static const bfd_vma ppc_cgc_vxworks_pic_plt_entry
    [VXWORKS_PLT_ENTRY_SIZE / 4] =
  {
...
static const bfd_vma ppc_cgc_vxworks_plt0_entry
...
static const bfd_vma ppc_cgc_vxworks_pic_plt0_entry
    [VXWORKS_PLT_INITIAL_ENTRY_SIZE / 4] =
  {
...

static reloc_howto_type *ppc_cgc_howto_table[R_PPC_max];

static reloc_howto_type ppc_cgc_howto_raw[] = {
  ...
  HOWTO (R_PPC_ADDR16_HA,	/* type */
	 ...
	 0,			/* bitpos */
	 complain_overflow_dont, /* complain_on_overflow */
	 ppc_cgc_addr16_ha_reloc, /* special_function */
  ...
  HOWTO (R_PPC_DTPMOD32,
	 ...
	 0,			/* bitpos */
	 complain_overflow_dont, /* complain_on_overflow */
	 ppc_cgc_unhandled_reloc, /* special_function */
	 "R_PPC_DTPMOD32",	/* name */
	 FALSE,			/* partial_inplace */
	 0,			/* src_mask */
	 0xffffffff,		/* dst_mask */
  ...
  /* The remaining relocs are from the Embedded CGC ABI, and are not
     in the SVR4 CGC ABI.  */
...
struct cgc_external_ppc_linux_prpsinfo32
  {
...
/* Helper macro to swap (properly handling endianess) things from the
   `cgc_internal_prpsinfo' structure to the `cgc_external_ppc_prpsinfo32'
   structure.
...

/* Initialize the ppc_cgc_howto_table, so that linear accesses can be done.  */

static void
ppc_cgc_howto_init (void)
{
  unsigned int i, type;

  for (i = 0;
       i < sizeof (ppc_cgc_howto_raw) / sizeof (ppc_cgc_howto_raw[0]);
       i++)
    {
      type = ppc_cgc_howto_raw[i].type;
      if (type >= (sizeof (ppc_cgc_howto_table)
		   / sizeof (ppc_cgc_howto_table[0])))
	abort ();
      ppc_cgc_howto_table[type] = &ppc_cgc_howto_raw[i];
    }
}

static reloc_howto_type *
ppc_cgc_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   bfd_reloc_code_real_type code)
{
  enum cgc_ppc_reloc_type r;

  /* Initialize howto table if not already done.  */
  if (!ppc_cgc_howto_table[R_PPC_ADDR32])
    ppc_cgc_howto_init ();
  ...
  return ppc_cgc_howto_table[r];
};

static reloc_howto_type *
ppc_cgc_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			   const char *r_name)
{
  unsigned int i;
  ...
  for (i = 0;
       i < sizeof (ppc_cgc_howto_raw) / sizeof (ppc_cgc_howto_raw[0]);
       i++)
    if (ppc_cgc_howto_raw[i].name != NULL
	&& strcasecmp (ppc_cgc_howto_raw[i].name, r_name) == 0)
      return &ppc_cgc_howto_raw[i];
...
static void
ppc_cgc_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED,
		       arelent *cache_ptr,
		       Cgc_Internal_Rela *dst)
{
  ...
  BFD_ASSERT (CGC32_R_TYPE (dst->r_info) < (unsigned int) R_PPC_max);
  cache_ptr->howto = ppc_cgc_howto_table[CGC32_R_TYPE (dst->r_info)];

  /* Just because the above assert didn't trigger doesn't mean that
     CGC32_R_TYPE (dst->r_info) is necessarily a valid relocation.  */
  if (!cache_ptr->howto)
    {
      (*_bfd_error_handler) (_("%B: invalid relocation type %d"),
                             abfd, CGC32_R_TYPE (dst->r_info));
      bfd_set_error (bfd_error_bad_value);

      cache_ptr->howto = ppc_cgc_howto_table[R_PPC_NONE];
    }
}
...
ppc_cgc_addr16_ha_reloc (bfd *abfd ATTRIBUTE_UNUSED,
			 arelent *reloc_entry,
			 asymbol *symbol,
...
static bfd_reloc_status_type
ppc_cgc_unhandled_reloc (bfd *abfd,
			 arelent *reloc_entry,
			 asymbol *symbol,
			 void *data,
			 asection *input_section,
...
typedef struct cgc_linker_section
{
  ...
  const char *sym_name;
  /* Associated symbol.  */
  struct cgc_link_hash_entry *sym;
} cgc_linker_section_t;

/* Linked list of allocated pointer entries.  This hangs off of the
   symbol lists, and provides allows us to return different pointers,
   based on different addend's.  */

typedef struct cgc_linker_section_pointers
{
  /* next allocated pointer for this symbol */
  struct cgc_linker_section_pointers *next;
  /* offset of pointer from beginning of section */
  bfd_vma offset;
  /* addend used */
  bfd_vma addend;
  /* which linker section this is */
  cgc_linker_section_t *lsect;
} cgc_linker_section_pointers_t;

struct ppc_cgc_obj_tdata
{
  struct cgc_obj_tdata cgc;

  /* A mapping from local symbols to offsets into the various linker
     sections added.  This is index by the symbol index.  */
  cgc_linker_section_pointers_t **linker_section_pointers;
...
#define ppc_cgc_tdata(bfd) \
  ((struct ppc_cgc_obj_tdata *) (bfd)->tdata.any)

#define cgc_local_ptr_offsets(bfd) \
  (ppc_cgc_tdata (bfd)->linker_section_pointers)

#define is_ppc_cgc(bfd) \
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour \
   && cgc_object_id (bfd) == PPC32_CGC_DATA)

/* Override the generic function because we store some extras.  */

static bfd_boolean
ppc_cgc_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct ppc_cgc_obj_tdata),
				  PPC32_CGC_DATA);
}
...
static bfd_boolean
ppc_cgc_object_p (bfd *abfd)
{
  if (abfd->arch_info->the_default && abfd->arch_info->bits_per_word == 64)
    {
      Cgc_Internal_Ehdr *i_ehdr = cgc_cgcheader (abfd);

      if (i_ehdr->e_ident[EI_CLASS] == CGCCLASS32)
	{
...
static bfd_boolean
ppc_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
static bfd_boolean
ppc_cgc_grok_prstatus (bfd *abfd, Cgc_Internal_Note *note)
{
  int offset;
  unsigned int size;
...
static bfd_boolean
ppc_cgc_grok_psinfo (bfd *abfd, Cgc_Internal_Note *note)
{
  switch (note->descsz)
    {
    default:
    ...
    case 128:		/* Linux/PPC cgc_prpsinfo.  */
      cgc_tdata (abfd)->core->pid
	= bfd_get_32 (abfd, note->descdata + 16);
      cgc_tdata (abfd)->core->program
	= _bfd_cgccore_strndup (abfd, note->descdata + 32, 16);
      cgc_tdata (abfd)->core->command
	= _bfd_cgccore_strndup (abfd, note->descdata + 48, 80);
    }
...
char *
cgccore_write_ppc_linux_prpsinfo32 (bfd *abfd, char *buf, int *bufsiz,
				      const struct cgc_internal_linux_prpsinfo *prpsinfo)
{
  struct cgc_external_ppc_linux_prpsinfo32 data;
...
static char *
ppc_cgc_write_core_note (bfd *abfd, char *buf, int *bufsiz, int note_type, ...)
{
  switch (note_type)
    {
    default:
...
static flagword
ppc_cgc_lookup_section_flags (char *flag_name)
{
...
bfd_boolean
ppc_cgc_section_processing (bfd *abfd, Cgc_Internal_Shdr *shdr)
{
...
static bfd_vma
ppc_cgc_plt_sym_val (bfd_vma i ATTRIBUTE_UNUSED,
...
static bfd_boolean
ppc_cgc_section_from_shdr (bfd *abfd,
...
static bfd_boolean
ppc_cgc_fake_sections (bfd *abfd ATTRIBUTE_UNUSED,
...
static int
ppc_cgc_additional_program_headers (bfd *abfd,
...
bfd_boolean
ppc_cgc_modify_segment_map (bfd *abfd,
			    struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  struct cgc_segment_map *m, *n;
  bfd_size_type amt;
  unsigned int j, k;
  ...
  for (m = cgc_seg_map (abfd); m != NULL; m = m->next)
	...
	continue;
      ...
      sect0_vle = (cgc_section_flags (m->sections[0]) & SHF_PPC_VLE) != 0;
      for (j = 1; j < m->count; ++j)
	{
	  sectj_vle = (cgc_section_flags (m->sections[j]) & SHF_PPC_VLE) != 0;
	...
	continue;
	 ...
	 The scan resumes with the new segment.  */
      ...
      n = (struct cgc_segment_map *) bfd_zalloc (abfd, amt);
      if (n == NULL)
        return FALSE;
...
static const struct bfd_cgc_special_section ppc_cgc_special_sections[] =
{
...
static struct bfd_cgc_special_section ppc_alt_plt =
  { STRING_COMMA_LEN (".plt"),             0, SHT_PROGBITS, SHF_ALLOC };

static const struct bfd_cgc_special_section *
ppc_cgc_get_sec_type_attr (bfd *abfd ATTRIBUTE_UNUSED, asection *sec)
{
  ...
  ssect = _bfd_cgc_get_special_section (sec->name, ppc_cgc_special_sections,
					sec->use_rela_p);
  if (ssect != NULL)
    {
      if (ssect == ppc_cgc_special_sections && (sec->flags & SEC_LOAD) != 0)
	ssect = &ppc_alt_plt;
      return ssect;
    }

  return _bfd_cgc_get_sec_type_attr (abfd, sec);
}

...
static void
ppc_cgc_begin_write_processing (bfd *abfd, struct bfd_link_info *link_info)
{
  bfd *ibfd;
  asection *asec;
  char *buffer = NULL;
...
static bfd_boolean
ppc_cgc_write_section (bfd *abfd ATTRIBUTE_UNUSED,
...
static void
ppc_cgc_final_write_processing (bfd *abfd, bfd_boolean linker ATTRIBUTE_UNUSED)
{
  bfd_byte *buffer;
  asection *asec;
  unsigned i;
...
static long
ppc_cgc_get_synthetic_symtab (bfd *abfd, long symcount, asymbol **syms,
			      ...
			      asymbol **ret)
{
  ...
  *ret = NULL;
  ...
  if (cgc_section_flags (plt) & SHF_EXECINSTR)
    return _bfd_cgc_get_synthetic_symtab (abfd, symcount, syms,
  ...
  count = relplt->size / sizeof (Cgc32_External_Rela);
...
/* The following functions are specific to the CGC linker, while
   ...
   ppc_cgc_check_relocs is called early in the link process,
   ppc_cgc_finish_dynamic_sections is one of the last functions
   called.  */
...
must_be_dyn_reloc (struct bfd_link_info *info,
		   enum cgc_ppc_reloc_type r_type)
{
  switch (r_type)
    {
    default:
...
/* PPC CGC linker hash entry.  */

struct ppc_cgc_link_hash_entry
{
  ...
  cgc_linker_section_pointers_t *linker_section_pointer;
...
#define ppc_cgc_hash_entry(ent) ((struct ppc_cgc_link_hash_entry *) (ent))

/* PPC CGC linker hash table.  */

struct ppc_cgc_link_hash_table
{
  ...
  struct cgc_link_hash_entry *tls_get_addr;
  ...
  enum ppc_cgc_plt_type plt_type;
...
/* Rename some of the generic section flags to better document how they
   are used for ppc32.  The flags are only valid for ppc32 cgc objects.  */
...
/* Get the PPC CGC linker hash table from a link_info structure.  */

#define ppc_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == PPC32_CGC_DATA ? ((struct ppc_cgc_link_hash_table *) ((p)->hash)) : NULL)

/* Create an entry in a PPC CGC linker hash table.  */

static struct bfd_hash_entry *
ppc_cgc_link_hash_newfunc (struct bfd_hash_entry *entry,
			   ...
			   const char *string)
{
  ...
  if (entry == NULL)
    {
      entry = bfd_hash_allocate (table,
				 sizeof (struct ppc_cgc_link_hash_entry));
      if (entry == NULL)
	return entry;
    }
  ...
  if (entry != NULL)
    {
      ppc_cgc_hash_entry (entry)->linker_section_pointer = NULL;
      ppc_cgc_hash_entry (entry)->dyn_relocs = NULL;
      ppc_cgc_hash_entry (entry)->tls_mask = 0;
      ppc_cgc_hash_entry (entry)->has_sda_refs = 0;
    }

  return entry;
}

/* Create a PPC CGC linker hash table.  */

static struct bfd_link_hash_table *
ppc_cgc_link_hash_table_create (bfd *abfd)
{
  struct ppc_cgc_link_hash_table *ret;

  ret = bfd_zmalloc (sizeof (struct ppc_cgc_link_hash_table));
  if (ret == NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (&ret->cgc, abfd,
				      ppc_cgc_link_hash_newfunc,
				      sizeof (struct ppc_cgc_link_hash_entry),
				      PPC32_CGC_DATA))
    {
      free (ret);
  ...
  ret->cgc.init_plt_refcount.refcount = 0;
  ret->cgc.init_plt_refcount.glist = NULL;
  ret->cgc.init_plt_offset.offset = 0;
  ret->cgc.init_plt_offset.glist = NULL;
...
static bfd_boolean
ppc_cgc_create_got (bfd *abfd, struct bfd_link_info *info)
{
  struct ppc_cgc_link_hash_table *htab;
  asection *s;
  flagword flags;
...
static bfd_boolean
ppc_cgc_create_glink (bfd *abfd, struct bfd_link_info *info)
{
  struct ppc_cgc_link_hash_table *htab = ppc_cgc_hash_table (info);
  asection *s;
  flagword flags;
...
/* We have to create .dynsbss and .rela.sbss here so that they get mapped
   to output sections (just like _bfd_cgc_create_dynamic_sections has
   to create .dynbss and .rela.bss).  */

static bfd_boolean
ppc_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct ppc_cgc_link_hash_table *htab;
  asection *s;
  flagword flags;
  ...
  if (htab->glink == NULL
      && !ppc_cgc_create_glink (abfd, info))
    return FALSE;
...
static void
ppc_cgc_copy_indirect_symbol (struct bfd_link_info *info,
			      struct cgc_link_hash_entry *dir,
			      struct cgc_link_hash_entry *ind)
{
  struct ppc_cgc_link_hash_entry *edir, *eind;

  edir = (struct ppc_cgc_link_hash_entry *) dir;
  eind = (struct ppc_cgc_link_hash_entry *) ind;
  ...
  /* If we were called to copy over info for a weak sym, that's all.
     ...
     code in ppc_cgc_adjust_dynamic_symbol needs to check for
  ...
  edir->cgc.got.refcount += eind->cgc.got.refcount;
  eind->cgc.got.refcount = 0;

  /* And plt entries.  */
  if (eind->cgc.plt.plist != NULL)
    {
      if (edir->cgc.plt.plist != NULL)
	{
	  struct plt_entry **entp;
	  struct plt_entry *ent;

	  for (entp = &eind->cgc.plt.plist; (ent = *entp) != NULL; )
	    {
	      struct plt_entry *dent;

	      for (dent = edir->cgc.plt.plist; dent != NULL; dent = dent->next)
	  ...
	  *entp = edir->cgc.plt.plist;
	}

      edir->cgc.plt.plist = eind->cgc.plt.plist;
      eind->cgc.plt.plist = NULL;
    }
...
static bfd_boolean
ppc_cgc_add_symbol_hook (bfd *abfd,
			 ...
			 bfd_vma *valp)
{
  if (sym->st_shndx == SHN_COMMON
      && !info->relocatable
      && is_ppc_cgc (info->output_bfd)
      && sym->st_size <= cgc_gp_size (abfd))
    {
      ...
      if (htab->sbss == NULL)
	{
	  ...
	  htab->sbss = bfd_make_section_anyway_with_flags (htab->cgc.dynobj,
							   ".sbss",
							   flags);
	  if (htab->sbss == NULL)
	    return FALSE;
...

static bfd_boolean
create_sdata_sym (struct bfd_link_info *info, cgc_linker_section_t *lsect)
{
  struct ppc_cgc_link_hash_table *htab = ppc_cgc_hash_table (info);

  lsect->sym = cgc_link_hash_lookup (&htab->cgc, lsect->sym_name,
				     TRUE, FALSE, TRUE);
  if (lsect->sym == NULL)
    return FALSE;
  ...
  _bfd_cgc_link_hash_hide_symbol (info, lsect->sym, TRUE);
  return TRUE;
}
...
static bfd_boolean
ppc_cgc_create_linker_section (bfd *abfd,
			       struct bfd_link_info *info,
			       flagword flags,
			       cgc_linker_section_t *lsect)
{
  ...
  s = bfd_make_section_anyway_with_flags (htab->cgc.dynobj,
					  lsect->name,
					  flags);
  if (s == NULL
      || !bfd_set_section_alignment (htab->cgc.dynobj, s, 2))
    return FALSE;
  lsect->section = s;
...
static cgc_linker_section_pointers_t *
cgc_find_pointer_linker_section
  (cgc_linker_section_pointers_t *linker_pointers,
   bfd_vma addend,
   cgc_linker_section_t *lsect)
{
...
static bfd_boolean
cgc_create_pointer_linker_section (bfd *abfd,
				   cgc_linker_section_t *lsect,
				   struct cgc_link_hash_entry *h,
				   const Cgc_Internal_Rela *rel)
{
  cgc_linker_section_pointers_t **ptr_linker_section_ptr = NULL;
  cgc_linker_section_pointers_t *linker_section_ptr;
  unsigned long r_symndx = CGC32_R_SYM (rel->r_info);
  bfd_size_type amt;
  ...
  if (h != NULL)
    {
      struct ppc_cgc_link_hash_entry *eh;

      /* Has this symbol already been allocated?  If so, our work is done.  */
      eh = (struct ppc_cgc_link_hash_entry *) h;
      if (cgc_find_pointer_linker_section (eh->linker_section_pointer,
					   rel->r_addend,
					   lsect))
	return TRUE;
    ...
    {
      BFD_ASSERT (is_ppc_cgc (abfd));

      /* Allocation of a pointer to a local symbol.  */
      cgc_linker_section_pointers_t **ptr = cgc_local_ptr_offsets (abfd);

      /* Allocate a table to hold the local symbols if first time.  */
      if (!ptr)
	{
	  unsigned int num_symbols = cgc_symtab_hdr (abfd).sh_info;

	  amt = num_symbols;
	  amt *= sizeof (cgc_linker_section_pointers_t *);
	  ptr = bfd_zalloc (abfd, amt);

	  if (!ptr)
	    return FALSE;

	  cgc_local_ptr_offsets (abfd) = ptr;
	}

      /* Has this symbol already been allocated?  If so, our work is done.  */
      if (cgc_find_pointer_linker_section (ptr[r_symndx],
					   rel->r_addend,
					   lsect))
	return TRUE;
...
update_local_sym_info (bfd *abfd,
		       ...
		       int tls_type)
{
  bfd_signed_vma *local_got_refcounts = cgc_local_got_refcounts (abfd);
...
static bfd_boolean
is_branch_reloc (enum cgc_ppc_reloc_type r_type)
{
...
static void
bad_shared_reloc (bfd *abfd, enum cgc_ppc_reloc_type r_type)
{
  (*_bfd_error_handler)
    (_("%B: relocation %s cannot be used when making a shared object"),
     abfd,
     ppc_cgc_howto_table[r_type]->name);
  bfd_set_error (bfd_error_bad_value);
}
		      ...
		      const Cgc_Internal_Rela *relocs)
{
  ...
  struct cgc_link_hash_entry *tga;
...
#ifdef DEBUG
  _bfd_error_handler ("ppc_cgc_check_relocs called for section %A in %B",
		      sec, abfd);
#endif
  ...
  if (htab->glink == NULL)
	...
	htab->cgc.dynobj = abfd;
      if (!ppc_cgc_create_glink (htab->cgc.dynobj, info))
	return FALSE;
    }
  ...
  for (rel = relocs; rel < rel_end; rel++)
	  ...
	  && strcmp (h->root.root.string, "_GLOBAL_OFFSET_TABLE_") == 0)
	{
	  if (htab->cgc.dynobj == NULL)
	    htab->cgc.dynobj = abfd;
	  if (!ppc_cgc_create_got (htab->cgc.dynobj, info))
	    return FALSE;
	  BFD_ASSERT (h == htab->cgc.hgot);
	}
	  ...
	  if (CGC_ST_TYPE (isym->st_info) == STT_GNU_IFUNC)
		  ...
		  || is_branch_reloc (r_type))
		{
		  ...
		  if (r_type == R_PPC_PLTREL24)
		    {
		      ppc_cgc_tdata (abfd)->makes_plt_call = 1;
		      if (info->shared)
			addend = rel->r_addend;
		    }
	...
	{
	  if (rel != relocs
	      && (CGC32_R_TYPE (rel[-1].r_info) == R_PPC_TLSGD
		  || CGC32_R_TYPE (rel[-1].r_info) == R_PPC_TLSLD))
	...
	case R_PPC_GOT16_HA:
	  ...
	  if (h != NULL)
	    {
	      h->got.refcount += 1;
	      ppc_cgc_hash_entry (h)->tls_mask |= tls_type;
	    }
	  else
	...
	case R_PPC_EMB_SDAI16:
	  ...
	  if (htab->sdata[0].section == NULL
	      && !ppc_cgc_create_linker_section (abfd, info, 0,
						 &htab->sdata[0]))
	    return FALSE;
	  if (!cgc_create_pointer_linker_section (abfd, &htab->sdata[0],
						  h, rel))
	    return FALSE;
	  if (h != NULL)
	    {
	      ppc_cgc_hash_entry (h)->has_sda_refs = TRUE;
	      h->non_got_ref = TRUE;
	    }
	  break;
	...
	case R_PPC_EMB_SDA2I16:
	  ...
	  if (htab->sdata[1].section == NULL
	      && !ppc_cgc_create_linker_section (abfd, info, SEC_READONLY,
						 &htab->sdata[1]))
	    return FALSE;
	  if (!cgc_create_pointer_linker_section (abfd, &htab->sdata[1],
						  h, rel))
	    return FALSE;
	  if (h != NULL)
	    {
	...
	case R_PPC_REL16_HI:
	case R_PPC_REL16_HA:
	  ppc_cgc_tdata (abfd)->has_rel16 = 1;
	  break;
	...
	case R_PPC_LOCAL24PC:
	  if (h != NULL && h == htab->cgc.hgot && htab->plt_type == PLT_UNSET)
	    {
	  ...
	  if ((info->shared
	      ...
	      if (sreloc == NULL)
		{
		  ...
		  sreloc = _bfd_cgc_make_dynamic_reloc_section
		    (sec, htab->cgc.dynobj, 2, abfd, /*rela?*/ TRUE);
		...
		{
		  struct cgc_dyn_relocs *p;
		  struct cgc_dyn_relocs **rel_head;

		  rel_head = &ppc_cgc_hash_entry (h)->dyn_relocs;
		  p = *rel_head;
		  if (p == NULL || p->sec != sec)
		    {
...
static bfd_boolean
ppc_cgc_merge_obj_attributes (bfd *ibfd, bfd *obfd)
{
...
static bfd_boolean
ppc_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword old_flags;
  flagword new_flags;
  bfd_boolean error;

  if (!is_ppc_cgc (ibfd) || !is_ppc_cgc (obfd))
    return TRUE;
  ...
  if (!ppc_cgc_merge_obj_attributes (ibfd, obfd))
    return FALSE;
  ...
  /* Incompatible flags.  */
	...
	cgc_cgcheader (obfd)->e_flags &= ~EF_PPC_RELOCATABLE_LIB;

      /* The output is -mrelocatable iff it can't be -mrelocatable-lib,
	 but each input file is either -mrelocatable or -mrelocatable-lib.  */
      if (! (cgc_cgcheader (obfd)->e_flags & EF_PPC_RELOCATABLE_LIB)
	  && (new_flags & (EF_PPC_RELOCATABLE_LIB | EF_PPC_RELOCATABLE))
	  && (old_flags & (EF_PPC_RELOCATABLE_LIB | EF_PPC_RELOCATABLE)))
	cgc_cgcheader (obfd)->e_flags |= EF_PPC_RELOCATABLE;

      /* Do not warn about eabi vs. V.4 mismatch, just or in the bit if
	 any module uses it.  */
      cgc_cgcheader (obfd)->e_flags |= (new_flags & EF_PPC_EMB);
...
static void
ppc_cgc_vle_split16 (bfd *output_bfd, bfd_byte *contents,
...
int
ppc_cgc_select_plt_layout (bfd *output_bfd ATTRIBUTE_UNUSED,
			   struct bfd_link_info *info,
			   enum ppc_cgc_plt_type plt_style,
			   int emit_stub_syms)
{
  struct ppc_cgc_link_hash_table *htab;
  flagword flags;
  ...
  if (htab->plt_type == PLT_UNSET)
	...
	htab->plt_type = PLT_OLD;
	       ...
	       && (h = cgc_link_hash_lookup (&htab->cgc, "_mcount",
	...
	{
	  bfd *ibfd;
	  enum ppc_cgc_plt_type plt_type = plt_style;

	  /* Look through the reloc flags left by ppc_cgc_check_relocs.
	  ...
	  for (ibfd = info->input_bfds; ibfd; ibfd = ibfd->link_next)
	    if (is_ppc_cgc (ibfd))
	      {
		if (ppc_cgc_tdata (ibfd)->has_rel16)
		  plt_type = PLT_NEW;
		else if (ppc_cgc_tdata (ibfd)->makes_plt_call)
		  {
		    plt_type = PLT_OLD;
		    htab->old_bfd = ibfd;
		    break;
  ...
  if (htab->plt_type == PLT_NEW)
    {
      ...
      if (htab->plt != NULL
	  && !bfd_set_section_flags (htab->cgc.dynobj, htab->plt, flags))
	return -1;

      /* The new GOT is not executable.  */
      if (htab->got != NULL
	  && !bfd_set_section_flags (htab->cgc.dynobj, htab->got, flags))
	return -1;
    }
  else
    {
      ...
      if (htab->glink != NULL
	  && !bfd_set_section_alignment (htab->cgc.dynobj, htab->glink, 0))
	return -1;
    }
...
static asection *
ppc_cgc_gc_mark_hook (asection *sec,
		      struct bfd_link_info *info,
...
static bfd_boolean
ppc_cgc_gc_sweep_hook (bfd *abfd,
		       struct bfd_link_info *info,
		       asection *sec,
		       const Cgc_Internal_Rela *relocs)
{
...
/* Set plt output section type, htab->tls_get_addr, and call the
   generic CGC tls_setup function.  */

asection *
ppc_cgc_tls_setup (bfd *obfd,
		   struct bfd_link_info *info,
		   int no_tls_get_addr_opt)
{
  ...
  htab = ppc_cgc_hash_table (info);
  htab->tls_get_addr = cgc_link_hash_lookup (&htab->cgc, "__tls_get_addr",
					     FALSE, FALSE, TRUE);
  if (!no_tls_get_addr_opt)
    {
      struct cgc_link_hash_entry *opt, *tga;
	      ...
	      || opt->root.type == bfd_link_hash_defweak))
	{
	  ...
	  if (htab->cgc.dynamic_sections_created
	      ...
	      && !(SYMBOL_CALLS_LOCAL (info, tga)
		   || (CGC_ST_VISIBILITY (tga->other) != STV_DEFAULT
	      ...
	      if (ent != NULL)
		{
		  ...
		  ppc_cgc_copy_indirect_symbol (info, opt, tga);
		  if (opt->dynindx != -1)
		    {
		      ...
		      _bfd_cgc_strtab_delref (cgc_hash_table (info)->dynstr,
					      opt->dynstr_index);
		      if (!bfd_cgc_link_record_dynamic_symbol (info, opt))
			return FALSE;
		    }
  ...
  if (htab->plt_type == PLT_NEW
    ...
    {
      cgc_section_type (htab->plt->output_section) = SHT_PROGBITS;
      cgc_section_flags (htab->plt->output_section) = SHF_ALLOC + SHF_WRITE;
    }

  return _bfd_cgc_tls_setup (obfd, info);
}
...
branch_reloc_hash_match (const bfd *ibfd,
			 const Cgc_Internal_Rela *rel,
			 const struct cgc_link_hash_entry *hash)
{
  Cgc_Internal_Shdr *symtab_hdr = &cgc_symtab_hdr (ibfd);
  enum cgc_ppc_reloc_type r_type = CGC32_R_TYPE (rel->r_info);
  unsigned int r_symndx = CGC32_R_SYM (rel->r_info);
...
bfd_boolean
ppc_cgc_tls_optimize (bfd *obfd ATTRIBUTE_UNUSED,
		      struct bfd_link_info *info)
{
  bfd *ibfd;
  asection *sec;
	...
	for (sec = ibfd->sections; sec != NULL; sec = sec->next)
	  if (sec->has_tls_reloc && !bfd_is_abs_section (sec->output_section))
	      ...
	      for (rel = relstart; rel < relend; rel++)
		{
		  ...
		  if (r_symndx >= symtab_hdr->sh_info)
		    {
		      ...
		      sym_hashes = cgc_sym_hashes (ibfd);
		      h = sym_hashes[r_symndx - symtab_hdr->sh_info];
		      while (h->root.type == bfd_link_hash_indirect
			     || h->root.type == bfd_link_hash_warning)
			h = (struct cgc_link_hash_entry *) h->root.u.i.link;
		  ...
		  if (expecting_tls_get_addr)
		    {
		      ...
		      if (info->shared
			  && CGC32_R_TYPE (rel[1].r_info) == R_PPC_PLTREL24)
		  ...
		  if (h != NULL)
		    {
		      tls_mask = &ppc_cgc_hash_entry (h)->tls_mask;
		      ...
		      if (locsyms == NULL)
			{
		      ...
		      lgot_refs = cgc_local_got_refcounts (ibfd);
		      if (lgot_refs == NULL)
			abort ();
...
readonly_dynrelocs (struct cgc_link_hash_entry *h)
{
  ...
  for (p = ppc_cgc_hash_entry (h)->dyn_relocs; p != NULL; p = p->next)
    {
...
static bfd_boolean
ppc_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
			       struct cgc_link_hash_entry *h)
{
  struct ppc_cgc_link_hash_table *htab;
  asection *s;
...
#ifdef DEBUG
  fprintf (stderr, "ppc_cgc_adjust_dynamic_symbol called for %s\n",
	   h->root.root.string);
#endif
  ...
  if (h->type == STT_FUNC
		      ...
		      && h->root.type == bfd_link_hash_undefweak))))
	{
	  /* A PLT entry is not required/allowed when:
	     ...
	     ppc_cgc_adjust_dynamic_symbol won't even be called.
      ...
      else
	{
	  ...
	  if (!h->ref_regular_nonweak
	      ...
	      && h->type != STT_GNU_IFUNC
	      && !htab->is_vxworks
	      && !ppc_cgc_hash_entry (h)->has_sda_refs
  ...
  if (ppc_cgc_hash_entry (h)->has_sda_refs)
    s = htab->dynsbss;
  else
    s = htab->dynbss;
  BFD_ASSERT (s != NULL);
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0 && h->size != 0)
	...
	srel = htab->relbss;
      ...
      srel->size += sizeof (Cgc32_External_Rela);
      h->needs_copy = 1;
    }
	      ...
	      struct bfd_link_info *info)
{
  ...
  sh = cgc_link_hash_lookup (&htab->cgc, name, TRUE, FALSE, FALSE);
  if (sh == NULL)
    return FALSE;
...
static bfd_vma
allocate_got (struct ppc_cgc_link_hash_table *htab, unsigned int need)
{
  bfd_vma where;
		...
		if (!doneone)
		  {
		    if (!htab->cgc.dynamic_sections_created
			|| h->dynindx == -1)
		      htab->reliplt->size += sizeof (Cgc32_External_Rela);
		    else
		      {
			htab->relplt->size += sizeof (Cgc32_External_Rela);
  ...
  if (eh->cgc.got.refcount > 0)
    {
      ...
      if (eh->cgc.dynindx == -1
	  && !eh->cgc.forced_local
	  && eh->cgc.type != STT_GNU_IFUNC
	  && htab->cgc.dynamic_sections_created)
	{
	  if (!bfd_cgc_link_record_dynamic_symbol (info, &eh->cgc))
	    return FALSE;
	}
	...
	need += 4;
      if (need == 0)
	eh->cgc.got.offset = (bfd_vma) -1;
      else
	{
	  eh->cgc.got.offset = allocate_got (htab, need);
	  dyn = htab->cgc.dynamic_sections_created;
	  if ((info->shared
	       || WILL_CALL_FINISH_DYNAMIC_SYMBOL (dyn, 0, &eh->cgc))
	      && (CGC_ST_VISIBILITY (eh->cgc.other) == STV_DEFAULT
		...
		rsec = htab->reliplt;
		 ...
		 Except LD only needs one.  */
		  ...
		  && eh->cgc.def_dynamic)
		need -= 4;
	      rsec->size += need * (sizeof (Cgc32_External_Rela) / 4);
	    }
	}
    }
  else
  ...
  for (p = eh->dyn_relocs; p != NULL; p = p->next)
    {
      ...
      if (eh->cgc.type == STT_GNU_IFUNC)
	sreloc = htab->reliplt;
      sreloc->size += p->count * sizeof (Cgc32_External_Rela);
    }
...
static bfd_boolean
ppc_cgc_size_dynamic_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
			       struct bfd_link_info *info)
{
  struct ppc_cgc_link_hash_table *htab;
  asection *s;
...
#ifdef DEBUG
  fprintf (stderr, "ppc_cgc_size_dynamic_sections called\n");
#endif
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      ...
      if (info->executable)
	{
	  s = bfd_get_linker_section (htab->cgc.dynobj, ".interp");
	  BFD_ASSERT (s != NULL);
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link_next)
      ...
      for (s = ibfd->sections; s != NULL; s = s->next)
	{
	  ...
	  for (p = ((struct ppc_dyn_relocs *)
		    cgc_section_data (s)->local_dynrel);
	       p != NULL;
	       p = p->next)
	    {
		...
		if (info->shared)
		  {
		    ...
		    if ((*lgot_masks & PLT_IFUNC) != 0)
		      srel = htab->reliplt;
		    srel->size += need * (sizeof (Cgc32_External_Rela) / 4);
		  }
	      }
	  }
	else
  ...
  cgc_link_hash_traverse (cgc_hash_table (info), allocate_dynrelocs, info);

  if (htab->tlsld_got.refcount > 0)
    {
      htab->tlsld_got.offset = allocate_got (htab, 8);
      if (info->shared)
	htab->relgot->size += sizeof (Cgc32_External_Rela);
    }
  else
  ...
  if (htab->got != NULL && htab->plt_type != PLT_VXWORKS)
      ...
      if (htab->got->size <= 32768)
	{
      ...
      htab->cgc.hgot->root.u.def.value = g_o_t;
    }
  if (info->shared)
    {
      struct cgc_link_hash_entry *sda = htab->sdata[0].sym;
      if (sda != NULL
	  && !(sda->root.type == bfd_link_hash_defined
	       || sda->root.type == bfd_link_hash_defweak))
	{
	  sda->root.type = bfd_link_hash_defined;
	  sda->root.u.def.section = htab->cgc.hgot->root.u.def.section;
	  sda->root.u.def.value = htab->cgc.hgot->root.u.def.value;
	}
    }
  ...
  if (htab->glink != NULL
	...
	{
	  struct cgc_link_hash_entry *sh;
	  sh = cgc_link_hash_lookup (&htab->cgc, "__glink",
				     TRUE, FALSE, FALSE);
	  if (sh == NULL)
	    return FALSE;
	  if (sh->root.type == bfd_link_hash_new)
	  ...
	  sh = cgc_link_hash_lookup (&htab->cgc, "__glink_PLTresolve",
				     TRUE, FALSE, FALSE);
	  if (sh == NULL)
	    return FALSE;
	  if (sh->root.type == bfd_link_hash_new)
  ...
  if (htab->glink != NULL
      ...
      && _bfd_cgc_eh_frame_present (info))
    {
  ...
  relocs = FALSE;
  for (s = htab->cgc.dynobj->sections; s != NULL; s = s->next)
    {
	       ...
	       || s == htab->sdata[1].section)
	{
      ...
      else if (CONST_STRNEQ (bfd_get_section_name (htab->cgc.dynobj, s),
			     ".rela"))
	{
	  if (s->size != 0)
	    {
	...
	continue;
      ...
      s->contents = bfd_zalloc (htab->cgc.dynobj, s->size);
      if (s->contents == NULL)
	return FALSE;
    }
  ...
  if (htab->cgc.dynamic_sections_created)
    {
	 ...
	 values later, in ppc_cgc_finish_dynamic_sections, but we
  ...
  _bfd_cgc_add_dynamic_entry (info, TAG, VAL)
      ...
      if (relocs)
	{
	  if (!add_dynamic_entry (DT_RELA, 0)
	      ...
	      || !add_dynamic_entry (DT_RELAENT, sizeof (Cgc32_External_Rela)))
	    return FALSE;
	}
	...
	cgc_link_hash_traverse (cgc_hash_table (info), maybe_set_textrel,
				info);
...
#undef add_dynamic_entry
  ...
  if (htab->glink_eh_frame != NULL
      && htab->glink_eh_frame->contents != NULL)
    {
      ...
      bfd_put_32 (htab->cgc.dynobj, sizeof (glink_eh_frame_cie) - 4, p);
      ...
      p += 4;
      /* .glink size.  */
      bfd_put_32 (htab->cgc.dynobj, htab->glink->size, p);
      p += 4;
      /* Augmentation.  */
      p += 1;
	    ...
	    *p++ = DW_CFA_advance_loc + adv;
	  ...
	  else if (adv < 65536)
	    {
	      *p++ = DW_CFA_advance_loc2;
	      bfd_put_16 (htab->cgc.dynobj, adv, p);
	      p += 2;
	    }
	  else
	    {
	      *p++ = DW_CFA_advance_loc4;
	      bfd_put_32 (htab->cgc.dynobj, adv, p);
	      p += 4;
	    }
	  *p++ = DW_CFA_register;
	  *p++ = 65;
...
static bfd_boolean
ppc_cgc_hash_symbol (struct cgc_link_hash_entry *h)
{
  if (h->plt.plist != NULL
      && !h->def_regular
...
static bfd_boolean
ppc_cgc_relax_section (bfd *abfd,
		       asection *isec,
		       ...
		       bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  internal_relocs = _bfd_cgc_link_read_relocs (abfd, isec, NULL, NULL,
					       link_info->keep_memory);
  if (internal_relocs == NULL)
    goto error_return;

  htab = ppc_cgc_hash_table (link_info);
  got2 = bfd_get_section_by_name (abfd, ".got2");

  irelend = internal_relocs + isec->reloc_count;
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      unsigned long r_type = CGC32_R_TYPE (irel->r_info);
      bfd_vma toff, roff;
      asection *tsec;
      struct one_fixup *f;
	...
	default:
      ...
      h = NULL;
      if (CGC32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
	{
	  /* A local symbol.  */
	  Cgc_Internal_Sym *isym;
	  ...
	  isym = isymbuf + CGC32_R_SYM (irel->r_info);
	  if (isym->st_shndx == SHN_UNDEF)
	    ...
	    tsec = bfd_section_from_cgc_index (abfd, isym->st_shndx);

	  toff = isym->st_value;
	  sym_type = CGC_ST_TYPE (isym->st_info);
	}
      else
	{
	  ...
	  indx = CGC32_R_SYM (irel->r_info) - symtab_hdr->sh_info;
      ...
      if (h != NULL)
	{
	  ...
	  if (h->type == STT_GNU_IFUNC
	    ...
	    plist = &h->plt.plist;
	       ...
	       && cgc_local_got_offsets (abfd) != NULL)
	{
	  bfd_vma *local_got_offsets = cgc_local_got_offsets (abfd);
	  struct plt_entry **local_plt = (struct plt_entry **)
	    (local_got_offsets + symtab_hdr->sh_info);
	  plist = local_plt + CGC32_R_SYM (irel->r_info);
	}
      if (plist != NULL)
	{
	  bfd_vma addend = 0;
      ...
      if (0 && tsec->sec_info_type == SEC_INFO_TYPE_MERGE)
	{
	  ...
	  toff = _bfd_merged_section_offset (abfd, &tsec,
					     cgc_section_data (tsec)->sec_info,
					     toff);
      ...
      if (f == NULL)
	{
	  ...
	  irel->r_info = CGC32_R_INFO (CGC32_R_SYM (irel->r_info),
				       stub_rtype);
      ...
      else
	{
	  ...
	  irel->r_info = CGC32_R_INFO (0, R_PPC_NONE);
	}
      ...
      if (contents == NULL)
	{
	  ...
	  if (cgc_section_data (isec)->this_hdr.contents != NULL)
	    contents = cgc_section_data (isec)->this_hdr.contents;
	  else
	    {
  ...
  if (contents != NULL
      && cgc_section_data (isec)->this_hdr.contents != contents)
    {
  ...
  if (changes != 0)
    {
	 ...
	 information for the trampolines.  */
      ...
      Cgc_Internal_Rela *new_relocs = bfd_malloc ((changes + isec->reloc_count)
						  * sizeof (*new_relocs));
      unsigned ix;
      ...
      for (ix = changes; ix--;)
	{
      ...
      if (internal_relocs != cgc_section_data (isec)->relocs)
	free (internal_relocs);
      cgc_section_data (isec)->relocs = new_relocs;
      isec->reloc_count += changes;
      rel_hdr = _bfd_cgc_single_rel_hdr (isec);
      rel_hdr->sh_size += changes * rel_hdr->sh_entsize;
    }
  else if (cgc_section_data (isec)->relocs != internal_relocs)
    free (internal_relocs);
  ...
  if (!*again && link_info->relocatable)
    {
      ...
      for (irel = internal_relocs; irel < irelend; irel++)
	if (CGC32_R_TYPE (irel->r_info) == R_PPC_RELAX)
	  {
	    unsigned long r_symndx = CGC32_R_SYM (irel->r_info);

	    /* Rewrite the reloc and convert one of the trailing nop
	       relocs to describe this relocation.  */
	    BFD_ASSERT (CGC32_R_TYPE (irelend[-1].r_info) == R_PPC_NONE);
	    /* The relocs are at the bottom 2 bytes */
	    irel[0].r_offset += 2;
	    memmove (irel + 1, irel, (irelend - irel - 1) * sizeof (*irel));
	    irel[0].r_info = CGC32_R_INFO (r_symndx, R_PPC_ADDR16_HA);
	    irel[1].r_offset += 4;
	    irel[1].r_info = CGC32_R_INFO (r_symndx, R_PPC_ADDR16_LO);
	    irel++;
	  }
    }
 ...
 error_return:
  ...
  if (contents != NULL
    ...
    free (contents);
  if (internal_relocs != NULL
      && cgc_section_data (isec)->relocs != internal_relocs)
    free (internal_relocs);
  return FALSE;
}

...
static unsigned int
ppc_cgc_action_discarded (asection *sec)
{
...
static bfd_vma
cgc_finish_pointer_linker_section (bfd *input_bfd,
				   cgc_linker_section_t *lsect,
				   struct cgc_link_hash_entry *h,
				   bfd_vma relocation,
				   const Cgc_Internal_Rela *rel)
{
  ...
  if (h != NULL)
  ...
  else
    {
      ...
      BFD_ASSERT (is_ppc_cgc (input_bfd));
      BFD_ASSERT (cgc_local_ptr_offsets (input_bfd) != NULL);
      linker_section_ptr = cgc_local_ptr_offsets (input_bfd)[r_symndx];
    }

  linker_section_ptr = cgc_find_pointer_linker_section (linker_section_ptr,
							rel->r_addend,
							lsect);
...
unsigned int
_bfd_cgc_ppc_at_tls_transform (unsigned int insn, unsigned int reg)
{
  unsigned int rtra;
...
unsigned int
_bfd_cgc_ppc_at_tprel_transform (unsigned int insn, unsigned int reg)
{
...
static bfd_boolean
ppc_cgc_relocate_section (bfd *output_bfd,
			  struct bfd_link_info *info,
			  bfd *input_bfd,
			  asection *input_section,
			  bfd_byte *contents,
			  ...
			  asection **local_sections)
{
...
#ifdef DEBUG
  _bfd_error_handler ("ppc_cgc_relocate_section called for %B section %A, "
		      "%ld relocations%s",
  ...
  for (; rel < relend; rel++)
      ...
      if (sec != NULL && discarded_section (sec))
	{
	  ...
	  howto = NULL;
	  if (r_type < R_PPC_max)
	    howto = ppc_cgc_howto_table[r_type];
      ...
      if (info->relocatable)
	{
	 ...
	 for the final instruction stream.  */
      ...
      tls_gd = 0;
      if (h != NULL)
	tls_mask = ((struct ppc_cgc_link_hash_entry *) h)->tls_mask;
	...
	case R_PPC_GOT_TPREL16_LO:
	  if ((tls_mask & TLS_TLS) != 0
	      && (tls_mask & TLS_TPREL) == 0)
	    {
	      ...
	      rel->r_info = CGC32_R_INFO (r_symndx, r_type);
	    }
	  break;
	...
	case R_PPC_TLS:
	  if ((tls_mask & TLS_TLS) != 0
	      && (tls_mask & TLS_TPREL) == 0)
	    {
	      ...
	      insn = _bfd_cgc_ppc_at_tls_transform (insn, 2);
	      if (insn == 0)
		abort ();
	...
	case R_PPC_GOT_TLSLD16_LO:
	  if ((tls_mask & TLS_TLS) != 0 && (tls_mask & TLS_LD) == 0)
	      ...
	      if ((tls_mask & tls_gd) != 0)
		{
		  ...
		  if (offset != (bfd_vma) -1)
		    {
		      rel[1].r_info = CGC32_R_INFO (STN_UNDEF, R_PPC_NONE);
	      ...
	      else
		{
		  ...
		  if (offset != (bfd_vma) -1)
		    {
		      rel[1].r_info = CGC32_R_INFO (r_symndx, R_PPC_TPREL16_LO);
	...
	case R_PPC_TLSLD:
	  if ((tls_mask & TLS_TLS) != 0 && (tls_mask & TLS_LD) == 0)
		...
		rel->r_addend -= (local_syms[r_symndx].st_value
	      ...
	      rel->r_info = CGC32_R_INFO (r_symndx, R_PPC_TPREL16_LO);
	      rel->r_offset += d_offset;
	      insn2 = 0x38630000;	/* addi 3,3,0 */
	  ...
	  if (ent != NULL)
	      ...
	      if (h == NULL && (ent->plt.offset & 1) == 0)
		{
		  ...
		  rela.r_offset = (htab->iplt->output_section->vma
				   ...
				   + ent->plt.offset);
		  rela.r_info = CGC32_R_INFO (0, R_PPC_IRELATIVE);
		...
		do
		  {
			...
			if (tls_ty & (TLS_LD | TLS_GD))
			  {
			    outrel.r_info = CGC32_R_INFO (indx, R_PPC_DTPMOD32);
			    if (tls_ty == (TLS_TLS | TLS_GD))
			      {
				loc = rsec->contents;
				loc += (rsec->reloc_count++
					* sizeof (Cgc32_External_Rela));
				bfd_cgc32_swap_reloca_out (output_bfd,
							   &outrel, loc);
				outrel.r_offset += 4;
				outrel.r_info
				  = CGC32_R_INFO (indx, R_PPC_DTPREL32);
			      }
			  }
			else if (tls_ty == (TLS_TLS | TLS_DTPREL))
			  outrel.r_info = CGC32_R_INFO (indx, R_PPC_DTPREL32);
			else if (tls_ty == (TLS_TLS | TLS_TPREL))
			  outrel.r_info = CGC32_R_INFO (indx, R_PPC_TPREL32);
			else if (indx != 0)
			  outrel.r_info = CGC32_R_INFO (indx, R_PPC_GLOB_DAT);
			else if (ifunc != NULL)
			  outrel.r_info = CGC32_R_INFO (0, R_PPC_IRELATIVE);
			else
			  outrel.r_info = CGC32_R_INFO (0, R_PPC_RELATIVE);
			...
			loc += (rsec->reloc_count++
				* sizeof (Cgc32_External_Rela));
			bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
		      }
			...
			if (tls_ty == (TLS_TLS | TLS_LD))
			  ...
			  {
			    value -= htab->cgc.tls_sec->vma + DTP_OFFSET;
	  ...
	  if (h != NULL
	      ...
	      && h->dynindx == -1)
	    {
		 ...
		 defined before using them.  */
	      ...
	      insn = _bfd_cgc_ppc_at_tprel_transform (insn, 2);
	      if (insn != 0)
	...
	dodyn:
	  ...
	  if ((info->shared
		  ...
		  && !h->def_regular))
	    {
	      ...
	      asection *sreloc;
#ifdef DEBUG
	      fprintf (stderr, "ppc_cgc_relocate_section needs to "
		       "create relocation for %s\n",
		       (h && h->root.root.string
			? h->root.root.string : "<unknown>"));
#endif
		...
		return FALSE;
	      ...
	      skip = 0;
	      outrel.r_offset = _bfd_cgc_section_offset (output_bfd, info,
							 input_section,
							 rel->r_offset);
	      ...
	      else
		{
		  ...
		  if (r_type != R_PPC_ADDR32)
		      ...
		      else
			{
		      ...
		      outrel.r_info = CGC32_R_INFO (indx, r_type);
		    }
		  else if (ifunc != NULL)
		    outrel.r_info = CGC32_R_INFO (0, R_PPC_IRELATIVE);
		  else
	      ...
	      loc = sreloc->contents;
	      loc += sreloc->reloc_count++ * sizeof (Cgc32_External_Rela);
	      bfd_cgc32_swap_reloca_out (output_bfd, &outrel, loc);
		...
		continue;
		 ...
		 so that it contains predictable value.  */
		  ...
		  && ((input_section->flags & SEC_ALLOC) != 0
		      || CGC32_R_TYPE (outrel.r_info) != R_PPC_RELATIVE))
		{
	...
	case R_PPC_RELAX:
...

	  {
	    ...
	    /* Rewrite the reloc and convert one of the trailing nop
	       relocs to describe this relocation.  */
	    BFD_ASSERT (CGC32_R_TYPE (relend[-1].r_info) == R_PPC_NONE);
	    /* The relocs are at the bottom 2 bytes */
	    rel[0].r_offset += 2;
	    memmove (rel + 1, rel, (relend - rel - 1) * sizeof (*rel));
	    rel[0].r_info = CGC32_R_INFO (r_symndx, R_PPC_ADDR16_HA);
	    rel[1].r_offset += 4;
	    rel[1].r_info = CGC32_R_INFO (r_symndx, R_PPC_ADDR16_LO);
	    rel++;
	  }
	  continue;
	...
	case R_PPC_EMB_SDAI16:
	  ...
	  relocation
	    = cgc_finish_pointer_linker_section (input_bfd, &htab->sdata[0],
						 h, relocation, rel);
	  addend = 0;
	  break;
	...
	case R_PPC_EMB_SDA2I16:
	  ...
	  relocation
	    = cgc_finish_pointer_linker_section (input_bfd, &htab->sdata[1],
						 h, relocation, rel);
	  addend = 0;
	  break;
	  ...
	  /* Handle the TOC16 reloc.  We want to use the offset within the .got
	     ...
	     an embedded CGC object, for which the .got section acts like the
	     AIX .toc section.  */
	...
	case R_PPC_EMB_SDA2REL:
	  {
	    const char *name;
	    struct cgc_link_hash_entry *sda = htab->sdata[1].sym;
	...
	case R_PPC_VLE_LO16A:
	  ...
	  ppc_cgc_vle_split16 (output_bfd, contents, rel->r_offset,
	...
	case R_PPC_VLE_SDA21_LO:
	  {
	    const char *name;
	    int reg;
	    struct cgc_link_hash_entry *sda = NULL;
...
static bfd_boolean
ppc_cgc_finish_dynamic_symbol (bfd *output_bfd,
			       struct bfd_link_info *info,
			       struct cgc_link_hash_entry *h,
			       Cgc_Internal_Sym *sym)
{
...
#ifdef DEBUG
  fprintf (stderr, "ppc_cgc_finish_dynamic_symbol called for %s",
	   h->root.root.string);
#endif
	...
	if (!doneone)
	  {
		...
		/* Use the right PLT. */
		plt_entry = info->shared ? ppc_cgc_vxworks_pic_plt_entry
			    : ppc_cgc_vxworks_plt_entry;
		...
		if (info->shared)
		  ...
		  {
		    bfd_vma got_loc = got_offset + SYM_VAL (htab->cgc.hgot);
		...
		if (!info->shared)
		  {
		    ...
		    rela.r_offset = (htab->plt->output_section->vma
				     + htab->plt->output_offset
				     + ent->plt.offset + 2);
		    rela.r_info = CGC32_R_INFO (htab->cgc.hgot->indx,
						R_PPC_ADDR16_HA);
		    ...
		    rela.r_offset = (htab->sgotplt->output_section->vma
				     ...
				     + got_offset);
		    rela.r_info = CGC32_R_INFO (htab->cgc.hplt->indx,
						R_PPC_ADDR32);
		...
		rela.r_addend = SYM_VAL (h);
	      ...
	      rela.r_info = CGC32_R_INFO (h->dynindx, R_PPC_JMP_SLOT);
		...
		|| h->dynindx == -1)
	      ...
	      loc = (htab->relplt->contents
		     + reloc_index * sizeof (Cgc32_External_Rela));
	    bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
  ...
  if (h->needs_copy)
	...
	s = htab->relbss;
      ...
      rela.r_info = CGC32_R_INFO (h->dynindx, R_PPC_COPY);
      rela.r_addend = 0;
...
ppc_cgc_reloc_type_class (const struct bfd_link_info *info,
			  ...
			  const Cgc_Internal_Rela *rela)
{
  ...
  switch (CGC32_R_TYPE (rela->r_info))
    {
    case R_PPC_RELATIVE:
...
static bfd_boolean
ppc_cgc_finish_dynamic_sections (bfd *output_bfd,
				 struct bfd_link_info *info)
{
  asection *sdyn;
  asection *splt;
...
#ifdef DEBUG
  fprintf (stderr, "ppc_cgc_finish_dynamic_sections called\n");
#endif
  ...
  if (htab->got != NULL)
    {
      if (htab->cgc.hgot->root.u.def.section == htab->got
	  || htab->cgc.hgot->root.u.def.section == htab->sgotplt)
	{
	  unsigned char *p = htab->cgc.hgot->root.u.def.section->contents;

	  p += htab->cgc.hgot->root.u.def.value;
	  if (htab->plt_type == PLT_OLD)
	    {
	      /* Add a blrl instruction at _GLOBAL_OFFSET_TABLE_-4
		 so that a function can easily find the address of
		 _GLOBAL_OFFSET_TABLE_.  */
	      BFD_ASSERT (htab->cgc.hgot->root.u.def.value - 4
			  < htab->cgc.hgot->root.u.def.section->size);
	  ...
	  if (sdyn != NULL)
	    {
	      ...
	      BFD_ASSERT (htab->cgc.hgot->root.u.def.value
			  < htab->cgc.hgot->root.u.def.section->size);
      ...
      else
	{
	  info->callbacks->einfo (_("%P: %s not defined in linker created %s\n"),
				  htab->cgc.hgot->root.root.string,
				  (htab->sgotplt != NULL
      ...
      cgc_section_data (htab->got->output_section)->this_hdr.sh_entsize = 4;
    }
  ...
  if (splt && splt->size > 0)
    {
      ...
      const bfd_vma *plt_entry = (info->shared
				  ? ppc_cgc_vxworks_pic_plt0_entry
				  : ppc_cgc_vxworks_plt0_entry);

      if (!info->shared)
	{
	  bfd_vma got_value = SYM_VAL (htab->cgc.hgot);
      ...
      if (! info->shared)
	{
	  ...
	  rela.r_offset = (htab->plt->output_section->vma
			   + htab->plt->output_offset
			   + 2);
	  rela.r_info = CGC32_R_INFO (htab->cgc.hgot->indx, R_PPC_ADDR16_HA);
	  rela.r_addend = 0;
	  bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
	  loc += sizeof (Cgc32_External_Rela);
	  ...
	  rela.r_offset = (htab->plt->output_section->vma
			   + htab->plt->output_offset
			   + 6);
	  rela.r_info = CGC32_R_INFO (htab->cgc.hgot->indx, R_PPC_ADDR16_LO);
	  rela.r_addend = 0;
	  bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
	  loc += sizeof (Cgc32_External_Rela);
	  ...
	  while (loc < htab->srelplt2->contents + htab->srelplt2->size)
	    {
	      ...
	      rel.r_info = CGC32_R_INFO (htab->cgc.hgot->indx, R_PPC_ADDR16_HA);
	      bfd_cgc32_swap_reloc_out (output_bfd, &rel, loc);
	      loc += sizeof (Cgc32_External_Rela);

	      bfd_cgc32_swap_reloc_in (output_bfd, loc, &rel);
	      rel.r_info = CGC32_R_INFO (htab->cgc.hgot->indx, R_PPC_ADDR16_LO);
	      bfd_cgc32_swap_reloc_out (output_bfd, &rel, loc);
	      loc += sizeof (Cgc32_External_Rela);

	      bfd_cgc32_swap_reloc_in (output_bfd, loc, &rel);
	      rel.r_info = CGC32_R_INFO (htab->cgc.hplt->indx, R_PPC_ADDR32);
	      bfd_cgc32_swap_reloc_out (output_bfd, &rel, loc);
	      loc += sizeof (Cgc32_External_Rela);
	    }
	}
...

#define TARGET_LITTLE_SYM	bfd_cgc32_powerpcle_vec
#define TARGET_LITTLE_NAME	"cgc32-powerpcle"
#define TARGET_BIG_SYM		bfd_cgc32_powerpc_vec
#define TARGET_BIG_NAME		"cgc32-powerpc"
#define CGC_ARCH		bfd_arch_powerpc
#define CGC_TARGET_ID		PPC32_CGC_DATA
#define CGC_MACHINE_CODE	EM_PPC
#ifdef __QNXTARGET__
...
#define cgc_info_to_howto	ppc_cgc_info_to_howto

#ifdef  EM_CYGNUS_POWERPC
#define CGC_MACHINE_ALT1	EM_CYGNUS_POWERPC
#endif

#ifdef EM_PPC_OLD
#define CGC_MACHINE_ALT2	EM_PPC_OLD
#endif
...
#define bfd_cgc32_mkobject			ppc_cgc_mkobject
#define bfd_cgc32_bfd_merge_private_bfd_data	ppc_cgc_merge_private_bfd_data
#define bfd_cgc32_bfd_relax_section		ppc_cgc_relax_section
#define bfd_cgc32_bfd_reloc_type_lookup		ppc_cgc_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		ppc_cgc_reloc_name_lookup
#define bfd_cgc32_bfd_set_private_flags		ppc_cgc_set_private_flags
#define bfd_cgc32_bfd_link_hash_table_create	ppc_cgc_link_hash_table_create
#define bfd_cgc32_get_synthetic_symtab		ppc_cgc_get_synthetic_symtab

#define cgc_backend_object_p			ppc_cgc_object_p
#define cgc_backend_gc_mark_hook		ppc_cgc_gc_mark_hook
#define cgc_backend_gc_sweep_hook		ppc_cgc_gc_sweep_hook
#define cgc_backend_section_from_shdr		ppc_cgc_section_from_shdr
#define cgc_backend_relocate_section		ppc_cgc_relocate_section
#define cgc_backend_create_dynamic_sections	ppc_cgc_create_dynamic_sections
#define cgc_backend_check_relocs		ppc_cgc_check_relocs
#define cgc_backend_copy_indirect_symbol	ppc_cgc_copy_indirect_symbol
#define cgc_backend_adjust_dynamic_symbol	ppc_cgc_adjust_dynamic_symbol
#define cgc_backend_add_symbol_hook		ppc_cgc_add_symbol_hook
#define cgc_backend_size_dynamic_sections	ppc_cgc_size_dynamic_sections
#define cgc_backend_hash_symbol			ppc_cgc_hash_symbol
#define cgc_backend_finish_dynamic_symbol	ppc_cgc_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections	ppc_cgc_finish_dynamic_sections
#define cgc_backend_fake_sections		ppc_cgc_fake_sections
#define cgc_backend_additional_program_headers	ppc_cgc_additional_program_headers
#define cgc_backend_modify_segment_map     	ppc_cgc_modify_segment_map
#define cgc_backend_grok_prstatus		ppc_cgc_grok_prstatus
#define cgc_backend_grok_psinfo			ppc_cgc_grok_psinfo
#define cgc_backend_write_core_note		ppc_cgc_write_core_note
#define cgc_backend_reloc_type_class		ppc_cgc_reloc_type_class
#define cgc_backend_begin_write_processing	ppc_cgc_begin_write_processing
#define cgc_backend_final_write_processing	ppc_cgc_final_write_processing
#define cgc_backend_write_section		ppc_cgc_write_section
#define cgc_backend_get_sec_type_attr		ppc_cgc_get_sec_type_attr
#define cgc_backend_plt_sym_val			ppc_cgc_plt_sym_val
#define cgc_backend_action_discarded		ppc_cgc_action_discarded
...
#define cgc_backend_lookup_section_flags_hook	ppc_cgc_lookup_section_flags
#define cgc_backend_section_processing		ppc_cgc_section_processing
...
#define TARGET_BIG_SYM  bfd_cgc32_powerpc_freebsd_vec
#undef  TARGET_BIG_NAME
#define TARGET_BIG_NAME "cgc32-powerpc-freebsd"

#undef  CGC_OSABI
#define CGC_OSABI	CGCOSABI_FREEBSD

#undef  cgc32_bed
#define cgc32_bed	cgc32_powerpc_fbsd_bed
...
#undef TARGET_BIG_SYM
#define TARGET_BIG_SYM		bfd_cgc32_powerpc_vxworks_vec
#undef TARGET_BIG_NAME
#define TARGET_BIG_NAME		"cgc32-powerpc-vxworks"

#undef  CGC_OSABI

/* VxWorks uses the cgc default section flags for .plt.  */
static const struct bfd_cgc_special_section *
ppc_cgc_vxworks_get_sec_type_attr (bfd *abfd ATTRIBUTE_UNUSED, asection *sec)
{
  if (sec->name == NULL)
    return NULL;
  ...
  return ppc_cgc_get_sec_type_attr (abfd, sec);
}

/* Like ppc_cgc_link_hash_table_create, but overrides
   appropriately for VxWorks.  */
static struct bfd_link_hash_table *
ppc_cgc_vxworks_link_hash_table_create (bfd *abfd)
{
  struct bfd_link_hash_table *ret;

  ret = ppc_cgc_link_hash_table_create (abfd);
  if (ret)
    {
      struct ppc_cgc_link_hash_table *htab
        = (struct ppc_cgc_link_hash_table *)ret;
...
static bfd_boolean
ppc_cgc_vxworks_add_symbol_hook (bfd *abfd,
				 struct bfd_link_info *info,
				 Cgc_Internal_Sym *sym,
				 const char **namep ATTRIBUTE_UNUSED,
				 flagword *flagsp ATTRIBUTE_UNUSED,
				 ...
				 bfd_vma *valp)
{
  if (!cgc_vxworks_add_symbol_hook(abfd, info, sym,namep, flagsp, secp,
				   valp))
    return FALSE;

  return ppc_cgc_add_symbol_hook(abfd, info, sym,namep, flagsp, secp, valp);
}

static void
ppc_cgc_vxworks_final_write_processing (bfd *abfd, bfd_boolean linker)
{
  ppc_cgc_final_write_processing(abfd, linker);
...
#undef cgc_backend_got_symbol_offset
#define cgc_backend_got_symbol_offset		0
#undef cgc_backend_plt_not_loaded
...
#define cgc_backend_final_write_processing \
  ppc_cgc_vxworks_final_write_processing
#undef cgc_backend_get_sec_type_attr
#define cgc_backend_get_sec_type_attr \
  ppc_cgc_vxworks_get_sec_type_attr
#undef cgc_backend_emit_relocs
#define cgc_backend_emit_relocs \
  cgc_vxworks_emit_relocs

#undef cgc32_bed
#define cgc32_bed				ppc_cgc_vxworks_bed
#undef cgc_backend_post_process_headers
