/* Adapteva epiphany specific support for 32-bit CGC
...
/* Relocation tables.  */
static reloc_howto_type epiphany_cgc_howto_table [] =
{
#define AHOW(t,rs,s,bs,pr,bp,co,name,sm,dm)	\
  ...
  HOWTO ( R_EPIPHANY_8_PCREL,  0, 0,  8, TRUE, 0, complain_overflow_bitfield, bfd_cgc_generic_reloc, "R_EPIPHANY_8_PCREL", FALSE, 0x000000ff, 0x000000ff, FALSE),
  /* 16 bit relative relocation */
  HOWTO ( R_EPIPHANY_16_PCREL, 0, 1, 16, TRUE, 0, complain_overflow_bitfield, bfd_cgc_generic_reloc, "R_EPIPHANY_8_PCREL", FALSE, 0x000000ff, 0x000000ff, FALSE),
  /* 32 bit relative relocation */
  HOWTO ( R_EPIPHANY_32_PCREL, 0, 2, 32, TRUE, 0, complain_overflow_bitfield, bfd_cgc_generic_reloc, "R_EPIPHANY_8_PCREL", FALSE, 0x000000ff, 0x000000ff, FALSE),
...
/* Map BFD reloc types to EPIPHANY CGC reloc types.  */

static reloc_howto_type *
epiphany_reloc_type_lookup (bfd * abfd ATTRIBUTE_UNUSED,
			    bfd_reloc_code_real_type code)
{
  /* Note that the epiphany_cgc_howto_table is indxed by the R_
  ...
  switch (code)
    {
    case BFD_RELOC_NONE:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_NONE];

    case BFD_RELOC_EPIPHANY_SIMM8:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_SIMM8];
    case BFD_RELOC_EPIPHANY_SIMM24:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_SIMM24];

    case BFD_RELOC_8_PCREL:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_8_PCREL];
    case BFD_RELOC_16_PCREL:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_16_PCREL];
    case BFD_RELOC_32_PCREL:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_32_PCREL];

    case BFD_RELOC_8:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_8];
    case BFD_RELOC_16:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_16];
    case BFD_RELOC_32:
      return &epiphany_cgc_howto_table[ (int) R_EPIPHANY_32];

    case BFD_RELOC_EPIPHANY_HIGH:
      return & epiphany_cgc_howto_table[ (int) R_EPIPHANY_HIGH];
    case BFD_RELOC_EPIPHANY_LOW:
      return & epiphany_cgc_howto_table[ (int) R_EPIPHANY_LOW];

    case BFD_RELOC_EPIPHANY_SIMM11:
      return & epiphany_cgc_howto_table[ (int) R_EPIPHANY_SIMM11];
    case BFD_RELOC_EPIPHANY_IMM11:
      return & epiphany_cgc_howto_table[ (int) R_EPIPHANY_IMM11];

    case BFD_RELOC_EPIPHANY_IMM8:
      return & epiphany_cgc_howto_table[ (int) R_EPIPHANY_IMM8];
...
epiphany_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED, const char *r_name)
{
  ...
  for (i = 0; i < ARRAY_SIZE (epiphany_cgc_howto_table); i++)
    if (epiphany_cgc_howto_table[i].name != NULL
	&& strcasecmp (epiphany_cgc_howto_table[i].name, r_name) == 0)
      return &epiphany_cgc_howto_table[i];
...
static bfd_boolean
epiphany_cgc_relax_section (bfd *abfd, asection *sec,
			    ...
			    bfd_boolean *again)
{
  ...
  *again = FALSE;
  ...
  if (stab)
    {
      /* So stab does exits.  */
      Cgc_Internal_Rela * irelbase ATTRIBUTE_UNUSED;

      irelbase = _bfd_cgc_link_read_relocs (abfd, stab, NULL, NULL,
					    link_info->keep_memory);
    }
  ...
  if ((pass == 1) || (new_pass && !changed))
	...
	{
#if 0
	  if (!epiphany_cgc_relax_section_page (abfd, sec, &changed, &misc,
						page_start, page_end))
#endif
	    return FALSE;
	}
...
/* Set the howto pointer for a EPIPHANY CGC reloc.  */
			     ...
			     Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = & epiphany_cgc_howto_table [r_type];
}
...
/* Relocate an EPIPHANY CGC section.
...
static bfd_boolean
epiphany_cgc_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
  ...
  for (rel = relocs; rel < relend; rel ++)
    {
      ...
      howto  = epiphany_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h      = NULL;
      sym    = NULL;
      sec    = NULL;
...
#define TARGET_LITTLE_SYM	 bfd_cgc32_epiphany_vec
#define TARGET_LITTLE_NAME  "cgc32-epiphany"

#define CGC_ARCH	 bfd_arch_epiphany
#define CGC_MACHINE_CODE EM_ADAPTEVA_EPIPHANY

#define CGC_MAXPAGESIZE  0x8000 /* No pages on the EPIPHANY.  */

#define cgc_info_to_howto_rel			NULL
#define cgc_info_to_howto			epiphany_info_to_howto_rela

#define cgc_backend_can_gc_sections     	1
#define cgc_backend_rela_normal			1
#define cgc_backend_relocate_section		epiphany_cgc_relocate_section

#define cgc_symbol_leading_char			'_'
#define bfd_cgc32_bfd_reloc_type_lookup		epiphany_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup		epiphany_reloc_name_lookup
#define bfd_cgc32_bfd_relax_section		epiphany_cgc_relax_section
