/* SuperH SH64-specific support for 64-bit CGC
...
#define cgc_sh64_sizeof_plt(info) PLT_ENTRY_SIZE

/* Return offset of the PLT0 address in an absolute PLT entry.  */
#define cgc_sh64_plt_plt0_offset(info) 32

/* Return offset of the linker in PLT0 entry.  */
#define cgc_sh64_plt0_gotplt_offset(info) 0

/* Return offset of the trampoline in PLT entry */
#define cgc_sh64_plt_temp_offset(info) 33 /* Add one because it's SHmedia.  */

/* Return offset of the symbol in PLT entry.  */
#define cgc_sh64_plt_symbol_offset(info) 0

/* Return offset of the relocation in PLT entry.  */
#define cgc_sh64_plt_reloc_offset(info) (info->shared ? 52 : 44)
...
struct cgc_sh64_pcrel_relocs_copied
{
  /* Next section.  */
  struct cgc_sh64_pcrel_relocs_copied *next;
  /* A section in dynobj.  */
  asection *section;
...
struct cgc_sh64_link_hash_entry
{
  ...
  struct cgc_sh64_pcrel_relocs_copied *pcrel_relocs_copied;
};

/* Traverse an sh CGC linker hash table.  */

#define sh64_cgc64_link_hash_traverse(table, func, info)	\
  (cgc_link_hash_traverse					\
   ((table),							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func), \
    (info)))

static bfd_reloc_status_type sh_cgc64_ignore_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);
static bfd_reloc_status_type sh_cgc64_reloc
  (bfd *, arelent *, asymbol *, void *, asection *, bfd *, char **);

static reloc_howto_type sh_cgc64_howto_table[] = {
  /* No relocation.  */
  HOWTO (R_SH_NONE,		/* type */
	 ...
	 sh_cgc64_ignore_reloc,	/* special_function */
	 "R_SH_NONE",		/* name */
  ...
  HOWTO (R_SH_DIR32,		/* type */
	 ...
	 sh_cgc64_reloc,		/* special_function */
	 "R_SH_DIR32",		/* name */
...
sh_cgc64_ignore_reloc (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc_entry,
...
/* This function is used for normal relocs.  This used to be like the COFF
   ...
   See sh_cgc_reloc in cgc32-sh.c for the original.  */

static bfd_reloc_status_type
sh_cgc64_reloc (bfd *abfd, arelent *reloc_entry, asymbol *symbol_in,
...
static const struct cgc_reloc_map sh64_reloc_map[] =
{
...
sh_cgc64_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    bfd_reloc_code_real_type code)
{
  unsigned int i;

  for (i = 0; i < sizeof (sh64_reloc_map) / sizeof (struct cgc_reloc_map); i++)
    {
      if (sh64_reloc_map[i].bfd_reloc_val == code)
	return &sh_cgc64_howto_table[(int) sh64_reloc_map[i].cgc_reloc_val];
    }
...
static reloc_howto_type *
sh_cgc64_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
			    const char *r_name)
{
  unsigned int i;

  for (i = 0;
       i < sizeof (sh_cgc64_howto_table) / sizeof (sh_cgc64_howto_table[0]);
       i++)
    if (sh_cgc64_howto_table[i].name != NULL
	&& strcasecmp (sh_cgc64_howto_table[i].name, r_name) == 0)
      return &sh_cgc64_howto_table[i];
...
/* Given an CGC reloc, fill in the howto field of a relent.
   ...
   See sh_cgc_info_to_howto in cgc32-sh.c for the original.  */

static void
sh_cgc64_info_to_howto (bfd *abfd ATTRIBUTE_UNUSED, arelent *cache_ptr,
			Cgc_Internal_Rela *dst)
{
  unsigned int r;

  r = CGC64_R_TYPE (dst->r_info);
  ...
  cache_ptr->howto = &sh_cgc64_howto_table[r];
}
...
static bfd_boolean
sh_cgc64_relocate_section (bfd *output_bfd ATTRIBUTE_UNUSED,
			   ...
			   asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	      ...
	      && r_type <= (int) R_SH_LAST_INVALID_RELOC_4))
	{
      ...
      howto = sh_cgc64_howto_table + r_type;
	...
	case R_SH_64_PCREL:
	  if (info->shared
	      ...
	      else if (r_type == R_SH_64_PCREL)
		{
		  ...
		  outrel.r_info = CGC64_R_INFO (h->dynindx, R_SH_64_PCREL);
	      ...
	      else
		{
		  ...
		  if (h == NULL
		    ...
		    {
		      relocate = TRUE;
		      outrel.r_info = CGC64_R_INFO (0, R_SH_RELATIVE64);
		      outrel.r_addend = relocation + rel->r_addend;
		    }
		  else
		    {
		      BFD_ASSERT (h->dynindx != -1);
		      outrel.r_info = CGC64_R_INFO (h->dynindx, R_SH_64);
		      outrel.r_addend = relocation + rel->r_addend;
		    }
		}
	...
	case R_SH_GOTPLT10BY8:
	  ...
	  relocation = (sgotplt->output_offset
			+ ((h->plt.offset / cgc_sh64_sizeof_plt (info)
			    - 1 + 3) * 8));
	...
	case R_SH_GOT10BY8:
	  ...
	  if (h != NULL)
		...
		{
		  struct cgc_sh64_link_hash_entry *hsh;

		  hsh = (struct cgc_sh64_link_hash_entry *)h;
...
/* This is a version of bfd_generic_get_relocated_section_contents
   that uses sh_cgc64_relocate_section.

   See sh_cgc_relocate_section in cgc32-sh.c for the original.  */

static bfd_byte *
sh_cgc64_get_relocated_section_contents (bfd *output_bfd,
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      ...
      for (isymp = isymbuf; isymp < isymend; ++isymp, ++secpp)
	{
	  ...
	  if (isymp->st_shndx == SHN_UNDEF)
	    ...
	    isec = bfd_section_from_cgc_index (input_bfd, isymp->st_shndx);

	  *secpp = isec;
	}

      if (! sh_cgc64_relocate_section (output_bfd, link_info, input_bfd,
...
static bfd_boolean
sh64_cgc64_fake_sections (bfd *output_bfd ATTRIBUTE_UNUSED,
			  Cgc_Internal_Shdr *cgc_section_hdr,
			  asection *asect)
{
  /* Code sections can only contain SH64 code, so mark them as such.  */
  if (bfd_get_section_flags (output_bfd, asect) & SEC_CODE)
    cgc_section_hdr->sh_flags |= SHF_SH5_ISA32;

  return TRUE;
}

static bfd_boolean
sh_cgc64_set_mach_from_flags (bfd *abfd)
{
...
/* Function to keep SH64 specific file flags.
   ...
   See sh64_cgc_set_private_flags in cgc32-sh64.c for the original.  */

static bfd_boolean
sh_cgc64_set_private_flags (bfd *abfd, flagword flags)
{
  ...
  return sh_cgc64_set_mach_from_flags (abfd);
}

/* Copy the SHF_SH5_ISA32 attribute that we keep on all sections with
   code, to keep attributes the same as for SHmedia in 32-bit CGC.  */

static bfd_boolean
sh_cgc64_copy_private_data_internal (bfd *ibfd, bfd *obfd)
{
  Cgc_Internal_Shdr **o_shdrp;
  asection *isec;
  asection *osec;
  ...
  o_shdrp = cgc_cgcsections (obfd);
  for (osec = obfd->sections; osec; osec = osec->next)
    {
      int oIndex = ((struct bfd_cgc_section_data *) cgc_section_data (osec))->this_idx;
	  ...
	  if (strcmp (osec->name, isec->name) == 0)
	    {
	      ...
	      if ((cgc_section_data (isec)->this_hdr.sh_flags
		   & SHF_SH5_ISA32) != 0)
  ...
  return sh_cgc64_set_private_flags (obfd, cgc_cgcheader (ibfd)->e_flags);
}

static bfd_boolean
sh_cgc64_copy_private_data (bfd *ibfd, bfd *obfd)
{
  return sh_cgc64_copy_private_data_internal (ibfd, obfd);
}

static bfd_boolean
sh_cgc64_merge_private_data (bfd *ibfd, bfd *obfd)
{
  ...
  if (! cgc_flags_init (obfd))
    {
      ...
      cgc_cgcheader (obfd)->e_flags = old_flags = new_flags;
    }
  /* We don't allow linking in anything else than SH64 code, and since
     this is a 64-bit CGC, we assume the 64-bit ABI is used.  Add code
  ...
  sh_cgc64_copy_private_data_internal (ibfd, obfd);
  ...
  return sh_cgc64_set_mach_from_flags (obfd);
}
...
static asection *
sh_cgc64_gc_mark_hook (asection *sec,
...
static bfd_boolean
sh_cgc64_check_relocs (bfd *abfd, struct bfd_link_info *info,
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_SH_GOT10BY8:
	  ...
	  if (h != NULL)
	      ...
	      if (h->dynindx == -1)
		{
	      ...
	      srelgot->size += sizeof (Cgc64_External_Rela);
	    }
	  else
	    {
	...
	case R_SH_64_PCREL:
	  ...
	  if (info->shared
	      ...
	      && (CGC32_R_TYPE (rel->r_info) != R_SH_64_PCREL
		  || (h != NULL
		      && (! info->symbolic
			  || !h->def_regular))))
	    {
	      ...
	      if (sreloc == NULL)
		{
		 ...
		 is only called if we are using an cgc_sh linker
		 hash table, which means that h is really a pointer to
		 an cgc_sh_link_hash_entry.  */
	      if (h != NULL && info->symbolic
		  && CGC64_R_TYPE (rel->r_info) == R_SH_64_PCREL)
		{
		  struct cgc_sh64_link_hash_entry *eh;
		  struct cgc_sh64_pcrel_relocs_copied *p;

		  eh = (struct cgc_sh64_link_hash_entry *) h;
		  ...
		  if (p == NULL)
		    {
		      p = ((struct cgc_sh64_pcrel_relocs_copied *)
...
static int
sh64_cgc64_get_symbol_type (Cgc_Internal_Sym * cgc_sym, int type)
{
  if (CGC_ST_TYPE (cgc_sym->st_info) == STT_DATALABEL)
    return STT_DATALABEL;

  return type;
}

/* FIXME: This is a copy of sh64_cgc_add_symbol_hook in cgc32-sh64.c.
   Either file can presumably exist without the other, but do not differ
   in cgc-size-ness.  How to share?
   ...
   into adding an option to cgc_add_symbol_hook (preferably) for the index
   or the hash entry, alternatively adding the index to Cgc_Internal_Sym
   (not so good).  */

static bfd_boolean
sh64_cgc64_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
			    Cgc_Internal_Sym *sym, const char **namep,
			    flagword *flagsp ATTRIBUTE_UNUSED,
			    asection **secp, bfd_vma *valp)
{
  /* We want to do this for relocatable as well as final linking.  */
  if (CGC_ST_TYPE (sym->st_info) == STT_DATALABEL
	...
	= bfd_malloc (strlen (*namep) + sizeof (DATALABEL_SUFFIX));
      struct cgc_link_hash_entry ** sym_hash = cgc_sym_hashes (abfd);
...
static int
sh64_cgc64_link_output_symbol_hook (struct bfd_link_info *info,
				    const char *cname,
				    Cgc_Internal_Sym *sym,
...
static void
sh64_cgc64_final_write_processing (bfd *abfd,
				   bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  if (cgc_cgcheader (abfd)->e_type == ET_EXEC)
    cgc_cgcheader (abfd)->e_entry |= 1;
}

/* First entry in an absolute procedure linkage table look like this.  */

static const bfd_byte cgc_sh64_plt0_entry_be[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh64_plt0_entry_le[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh64_plt_entry_be[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh64_plt_entry_le[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh64_pic_plt_entry_be[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_sh64_pic_plt_entry_le[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte *cgc_sh64_plt0_entry;
static const bfd_byte *cgc_sh64_plt_entry;
static const bfd_byte *cgc_sh64_pic_plt_entry;

/* Create an entry in an sh CGC linker hash table.  */

static struct bfd_hash_entry *
sh64_cgc64_link_hash_newfunc (struct bfd_hash_entry *entry,
			      struct bfd_hash_table *table,
			      const char *string)
{
  struct cgc_sh64_link_hash_entry *ret =
    (struct cgc_sh64_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == (struct cgc_sh64_link_hash_entry *) NULL)
    ret = ((struct cgc_sh64_link_hash_entry *)
	   bfd_hash_allocate (table,
			      sizeof (struct cgc_sh64_link_hash_entry)));
  if (ret == (struct cgc_sh64_link_hash_entry *) NULL)
...
/* Create an sh64 CGC linker hash table.  */

static struct bfd_link_hash_table *
sh64_cgc64_link_hash_table_create (bfd *abfd)
{
  struct cgc_link_hash_table *ret;

  ret = (struct cgc_link_hash_table *) bfd_zmalloc (sizeof (* ret));
  if (ret == (struct cgc_link_hash_table *) NULL)
    return NULL;

  if (!_bfd_cgc_link_hash_table_init (ret, abfd,
				      sh64_cgc64_link_hash_newfunc,
				      sizeof (struct cgc_sh64_link_hash_entry),
				      GENERIC_CGC_DATA))
    {
      free (ret);
...
static bfd_boolean
sh64_cgc64_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
...
static bfd_boolean
sh64_cgc64_adjust_dynamic_symbol (struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h)
{
  bfd *dynobj;
  asection *s;
  ...
  if (h->type == STT_FUNC
	  ...
	  && !h->def_regular)
	{
      ...
      s->size += cgc_sh64_sizeof_plt (info);
...
/* This function is called via sh_cgc_link_hash_traverse if we are
...
static bfd_boolean
sh64_cgc64_discard_copies (struct cgc_sh64_link_hash_entry *h,
			   void *ignore ATTRIBUTE_UNUSED)
{
  struct cgc_sh64_pcrel_relocs_copied *s;
  ...
  for (s = h->pcrel_relocs_copied; s != NULL; s = s->next)
    s->section->size -= s->count * sizeof (Cgc64_External_Rela);
...
static bfd_boolean
sh64_cgc64_size_dynamic_sections (bfd *output_bfd,
				  struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *s;
  ...
  if (info->shared && info->symbolic)
    sh64_cgc64_link_hash_traverse (cgc_hash_table (info),
				   sh64_cgc64_discard_copies, NULL);
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in sh64_cgc64_finish_dynamic_sections, but we
...
static bfd_boolean
sh64_cgc64_finish_dynamic_symbol (bfd *output_bfd,
				  struct bfd_link_info *info,
				  struct cgc_link_hash_entry *h,
				  Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
	 ...
	 first entry in the procedure linkage table is reserved.  */
      plt_index = h->plt.offset / cgc_sh64_sizeof_plt (info) - 1;
	...
	got_offset -= GOT_BIAS;
      ...
      if (! info->shared)
	{
	  if (cgc_sh64_plt_entry == NULL)
	    {
	      cgc_sh64_plt_entry = (bfd_big_endian (output_bfd) ?
				  cgc_sh64_plt_entry_be : cgc_sh64_plt_entry_le);
	    }
	  memcpy (splt->contents + h->plt.offset, cgc_sh64_plt_entry,
		  cgc_sh64_sizeof_plt (info));
	  movi_3shori_putval (output_bfd,
			      (sgot->output_section->vma
			       + sgot->output_offset
			       + got_offset),
			      (splt->contents + h->plt.offset
			       + cgc_sh64_plt_symbol_offset (info)));

	  /* Set bottom bit because its for a branch to SHmedia */
	  movi_shori_putval (output_bfd,
			     -(h->plt.offset
			      + cgc_sh64_plt_plt0_offset (info) + 8)
			     | 1,
			     (splt->contents + h->plt.offset
			      + cgc_sh64_plt_plt0_offset (info)));
	}
      else
	{
	  if (cgc_sh64_pic_plt_entry == NULL)
	    {
	      cgc_sh64_pic_plt_entry = (bfd_big_endian (output_bfd) ?
				      cgc_sh64_pic_plt_entry_be :
				      cgc_sh64_pic_plt_entry_le);
	    }
	  memcpy (splt->contents + h->plt.offset, cgc_sh64_pic_plt_entry,
		  cgc_sh64_sizeof_plt (info));
	...
	got_offset += GOT_BIAS;
      ...
      movi_shori_putval (output_bfd,
			 ...
			 (splt->contents + h->plt.offset
			  + cgc_sh64_plt_reloc_offset (info)));
      ...
      rel.r_offset = (sgot->output_section->vma
		      + sgot->output_offset
		      + got_offset);
      rel.r_info = CGC64_R_INFO (h->dynindx, R_SH_JMP_SLOT64);
      rel.r_addend = 0;
      rel.r_addend = GOT_BIAS;
      loc = srel->contents + plt_index * sizeof (Cgc64_External_Rela);
      bfd_cgc64_swap_reloca_out (output_bfd, &rel, loc);
  ...
  if (h->got.offset != (bfd_vma) -1)
    {
	 ...
	 initialized in the relocate_section function.  */
	  ...
	  && h->def_regular)
	{
	  rel.r_info = CGC64_R_INFO (0, R_SH_RELATIVE64);
      ...
      else
	{
	  ...
	  rel.r_info = CGC64_R_INFO (h->dynindx, R_SH_GLOB_DAT64);
	  rel.r_addend = 0;
	}
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_offset = (h->root.u.def.value
		      + h->root.u.def.section->output_section->vma
		      + h->root.u.def.section->output_offset);
      rel.r_info = CGC64_R_INFO (h->dynindx, R_SH_COPY64);
      rel.r_addend = 0;
      loc = s->contents;
      loc += s->reloc_count++ * sizeof (Cgc64_External_Rela);
      bfd_cgc64_swap_reloca_out (output_bfd, &rel, loc);
...
static bfd_boolean
sh64_cgc64_finish_dynamic_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sgot;
      ...
      if (splt && splt->size > 0)
	{
	  if (info->shared)
	      ...
	      if (cgc_sh64_pic_plt_entry == NULL)
		{
	      ...
	      memcpy (splt->contents, cgc_sh64_pic_plt_entry,
		      cgc_sh64_sizeof_plt (info));
	    }
	  else
	    {
	      if (cgc_sh64_plt0_entry == NULL)
		{
		  cgc_sh64_plt0_entry = (bfd_big_endian (output_bfd) ?
				       cgc_sh64_plt0_entry_be :
				       cgc_sh64_plt0_entry_le);
		}
	      memcpy (splt->contents, cgc_sh64_plt0_entry, PLT_ENTRY_SIZE);
	      movi_3shori_putval (output_bfd,
				  sgot->output_section->vma
				  + sgot->output_offset,
				  splt->contents
				  + cgc_sh64_plt0_gotplt_offset (info));
	    }

	  /* UnixWare sets the entsize of .plt to 8, although that doesn't
	     really seem like the right value.  */
	  cgc_section_data (splt->output_section)->this_hdr.sh_entsize = 8;
	}
    }
  ...
  cgc_section_data (sgot->output_section)->this_hdr.sh_entsize = 8;
...
/* Merge non visibility st_other attribute when the symbol comes from
   a dynamic object.  */
static void
sh64_cgc64_merge_symbol_attribute (struct cgc_link_hash_entry *h,
...
static const struct bfd_cgc_special_section sh64_cgc64_special_sections[]=
{
  { STRING_COMMA_LEN (".cranges"), 0, SHT_PROGBITS, 0 },
  { NULL,                       0, 0, 0,            0 }
};

#define TARGET_BIG_SYM		bfd_cgc64_sh64_vec
#define TARGET_BIG_NAME		"cgc64-sh64"
#define TARGET_LITTLE_SYM	bfd_cgc64_sh64l_vec
#define TARGET_LITTLE_NAME	"cgc64-sh64l"
#define CGC_ARCH		bfd_arch_sh
#define CGC_MACHINE_CODE	EM_SH
#define CGC_MAXPAGESIZE		128

#define cgc_symbol_leading_char '_'

#define bfd_cgc64_bfd_reloc_type_lookup	sh_cgc64_reloc_type_lookup
#define bfd_cgc64_bfd_reloc_name_lookup \
					sh_cgc64_reloc_name_lookup
#define cgc_info_to_howto		sh_cgc64_info_to_howto

/* Note: there's no relaxation at present.  */

#define cgc_backend_relocate_section	sh_cgc64_relocate_section
#define bfd_cgc64_bfd_get_relocated_section_contents \
					sh_cgc64_get_relocated_section_contents
#define cgc_backend_object_p		sh_cgc64_set_mach_from_flags
#define bfd_cgc64_bfd_set_private_flags \
					sh_cgc64_set_private_flags
#define bfd_cgc64_bfd_copy_private_bfd_data \
					sh_cgc64_copy_private_data
#define bfd_cgc64_bfd_merge_private_bfd_data \
					sh_cgc64_merge_private_data
#define cgc_backend_fake_sections	sh64_cgc64_fake_sections

#define cgc_backend_gc_mark_hook        sh_cgc64_gc_mark_hook
#define cgc_backend_check_relocs        sh_cgc64_check_relocs

#define cgc_backend_can_gc_sections	1

#define cgc_backend_get_symbol_type	sh64_cgc64_get_symbol_type

#define cgc_backend_add_symbol_hook	sh64_cgc64_add_symbol_hook

#define cgc_backend_link_output_symbol_hook \
	sh64_cgc64_link_output_symbol_hook

#define	cgc_backend_merge_symbol_attribute \
	sh64_cgc64_merge_symbol_attribute

#define cgc_backend_final_write_processing \
 	sh64_cgc64_final_write_processing

#define cgc_backend_create_dynamic_sections \
					sh64_cgc64_create_dynamic_sections
#define bfd_cgc64_bfd_link_hash_table_create \
					sh64_cgc64_link_hash_table_create
#define cgc_backend_adjust_dynamic_symbol \
					sh64_cgc64_adjust_dynamic_symbol
#define cgc_backend_size_dynamic_sections \
					sh64_cgc64_size_dynamic_sections
#define cgc_backend_omit_section_dynsym \
  ((bfd_boolean (*) (bfd *, struct bfd_link_info *, asection *)) bfd_true)
#define cgc_backend_finish_dynamic_symbol \
					sh64_cgc64_finish_dynamic_symbol
#define cgc_backend_finish_dynamic_sections \
					sh64_cgc64_finish_dynamic_sections
#define cgc_backend_special_sections	sh64_cgc64_special_sections
...
#define	TARGET_BIG_SYM			bfd_cgc64_sh64nbsd_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc64-sh64-nbsd"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc64_sh64lnbsd_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc64-sh64l-nbsd"
#undef	CGC_MAXPAGESIZE
#define	CGC_MAXPAGESIZE			0x10000
#undef	cgc_symbol_leading_char
#define	cgc_symbol_leading_char		0

#define	cgc64_bed			cgc64_sh64_nbsd_bed

#include "cgc64-target.h"

/* Linux support.  */
#undef	TARGET_BIG_SYM
#define	TARGET_BIG_SYM			bfd_cgc64_sh64blin_vec
#undef	TARGET_BIG_NAME
#define	TARGET_BIG_NAME			"cgc64-sh64big-linux"
#undef	TARGET_LITTLE_SYM
#define	TARGET_LITTLE_SYM		bfd_cgc64_sh64lin_vec
#undef	TARGET_LITTLE_NAME
#define	TARGET_LITTLE_NAME		"cgc64-sh64-linux"
#undef cgc64_bed
#define	cgc64_bed			cgc64_sh64_linux_bed
