/* OpenRISC-specific support for 32-bit CGC.
...
#include "libbfd.h"
#include "cgc-bfd.h"
#include "cgc/openrisc.h"
#include "libiberty.h"

static reloc_howto_type openrisc_cgc_howto_table[] =
{
...
/* Map BFD reloc types to OpenRISC CGC reloc types.  */
			    ...
			    bfd_reloc_code_real_type code)
{
  ...
  for (i = ARRAY_SIZE (openrisc_reloc_map); --i;)
    if (openrisc_reloc_map[i].bfd_reloc_val == code)
      return & openrisc_cgc_howto_table[openrisc_reloc_map[i].
				       openrisc_reloc_val];
			    ...
			    const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (openrisc_cgc_howto_table)
	    / sizeof (openrisc_cgc_howto_table[0]));
       i++)
    if (openrisc_cgc_howto_table[i].name != NULL
	&& strcasecmp (openrisc_cgc_howto_table[i].name, r_name) == 0)
      return &openrisc_cgc_howto_table[i];

  return NULL;
}

/* Set the howto pointer for an OpenRISC CGC reloc.  */
			     ...
			     Cgc_Internal_Rela * dst)
{
  ...
  cache_ptr->howto = & openrisc_cgc_howto_table[r_type];
}
...
/* Relocate an OpenRISC CGC section.
...
static bfd_boolean
openrisc_cgc_relocate_section (bfd *output_bfd,
			       ...
			       asection **local_sections)
{
  ...
  for (rel = relocs; rel < relend; rel++)
	...
	continue;
	  ...
	  (sizeof openrisc_cgc_howto_table / sizeof (reloc_howto_type)))
	abort ();

      howto = openrisc_cgc_howto_table + CGC32_R_TYPE (rel->r_info);
      h = NULL;
      sym = NULL;
      sec = NULL;
...
static asection *
openrisc_cgc_gc_mark_hook (asection *sec,
			   struct bfd_link_info *info,
			   Cgc_Internal_Rela *rel,
			   struct cgc_link_hash_entry *h,
			   Cgc_Internal_Sym *sym)
...
static bfd_boolean
openrisc_cgc_check_relocs (bfd *abfd,
			   struct bfd_link_info *info,
			   asection *sec,
...
static bfd_boolean
openrisc_cgc_object_p (bfd *abfd)
{
...
static void
openrisc_cgc_final_write_processing (bfd *abfd,
				     bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  cgc_cgcheader (abfd)->e_flags &= ~0xf;
  cgc_cgcheader (abfd)->e_flags |= val;
}


#define CGC_ARCH			bfd_arch_openrisc
#define CGC_MACHINE_CODE		EM_OPENRISC
#define CGC_MACHINE_ALT1		EM_OPENRISC_OLD
#define CGC_MAXPAGESIZE			0x1000

#define TARGET_BIG_SYM			bfd_cgc32_openrisc_vec
#define TARGET_BIG_NAME			"cgc32-openrisc"

#define cgc_info_to_howto_rel		NULL
#define cgc_info_to_howto		openrisc_info_to_howto_rela
#define cgc_backend_relocate_section	openrisc_cgc_relocate_section
#define cgc_backend_gc_mark_hook	openrisc_cgc_gc_mark_hook
#define cgc_backend_check_relocs	openrisc_cgc_check_relocs

#define cgc_backend_can_gc_sections	1
#define cgc_backend_rela_normal		1

#define bfd_cgc32_bfd_reloc_type_lookup openrisc_reloc_type_lookup
#define bfd_cgc32_bfd_reloc_name_lookup openrisc_reloc_name_lookup

#define cgc_backend_object_p                openrisc_cgc_object_p
#define cgc_backend_final_write_processing  openrisc_cgc_final_write_processing
