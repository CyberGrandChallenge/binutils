/* Matsushita 10300 specific support for 32-bit CGC
...
/* The mn10300 linker needs to keep track of the number of relocs that
   ...
   extending the regular CGC linker hash table.  */

struct cgc32_mn10300_link_hash_entry
{
  /* The basic cgc link hash table entry.  */
...
/* We derive a hash table from the main cgc linker hash table so
   we can store state variables and a secondary hash table without
   resorting to global variables.  */
struct cgc32_mn10300_link_hash_table
{
  /* The main hash table.  */
  struct cgc_link_hash_table root;

  /* A hash table for static functions.  We could derive a new hash table
     instead of using the full cgc32_mn10300_link_hash_table if we wanted
     to save some memory.  */
  struct cgc32_mn10300_link_hash_table *static_hash_table;
...
#define cgc_mn10300_hash_entry(ent) ((struct cgc32_mn10300_link_hash_entry *)(ent))

struct cgc_mn10300_obj_tdata
{
...
#define cgc_mn10300_tdata(abfd) \
  ((struct cgc_mn10300_obj_tdata *) (abfd)->tdata.any)

#define cgc_mn10300_local_got_tls_type(abfd) \
  (cgc_mn10300_tdata (abfd)->local_got_tls_type)
...
/* Get the MN10300 CGC linker hash table from a link_info structure.  */

#define cgc32_mn10300_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == MN10300_CGC_DATA ? ((struct cgc32_mn10300_link_hash_table *) ((p)->hash)) : NULL)

#define cgc32_mn10300_link_hash_traverse(table, func, info)		\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func),	\
    (info)))

static reloc_howto_type cgc_mn10300_howto_table[] =
{
...
static bfd_boolean
_bfd_mn10300_cgc_create_got_section (bfd * abfd,
				     struct bfd_link_info * info)
{
  ...
  h = _bfd_cgc_define_linkage_sym (abfd, info, s, "_GLOBAL_OFFSET_TABLE_");
  htab->hgot = h;
  if (h == NULL)
    return FALSE;
...
bfd_cgc32_bfd_reloc_type_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				 bfd_reloc_code_real_type code)
{
  ...
  for (i = ARRAY_SIZE (mn10300_reloc_map); i--;)
    if (mn10300_reloc_map[i].bfd_reloc_val == code)
      return &cgc_mn10300_howto_table[mn10300_reloc_map[i].cgc_reloc_val];

  return NULL;
}

static reloc_howto_type *
bfd_cgc32_bfd_reloc_name_lookup (bfd *abfd ATTRIBUTE_UNUSED,
				 const char *r_name)
{
  unsigned int i;

  for (i = ARRAY_SIZE (cgc_mn10300_howto_table); i--;)
    if (cgc_mn10300_howto_table[i].name != NULL
	&& strcasecmp (cgc_mn10300_howto_table[i].name, r_name) == 0)
      return cgc_mn10300_howto_table + i;

  return NULL;
}

/* Set the howto pointer for an MN10300 CGC reloc.  */
		       ...
		       Cgc_Internal_Rela *dst)
{
  ...
  cache_ptr->howto = cgc_mn10300_howto_table + r_type;
}

static int
cgc_mn10300_tls_transition (struct bfd_link_info *        info,
			    int                           r_type,
			    struct cgc_link_hash_entry *  h,
			    asection *                    sec,
			    bfd_boolean                   counting)
{
  ...
  if (r_type == R_MN10300_TLS_GD
      && h != NULL
      && cgc_mn10300_hash_entry (h)->tls_type == GOT_TLS_IE)
  ...
  if (! counting && h != NULL && ! cgc_hash_table (info)->dynamic_sections_created)
    is_local = TRUE;
  else
...
#define TLS_PAIR(r1,r2) ((r1) * R_MN10300_MAX + (r2))
  ...
  switch (TLS_PAIR (r_type, tls_r_type))
	...
	(_("%s: Unsupported transition from %s to %s"),
	 ...
	 cgc_mn10300_howto_table[r_type].name,
	 cgc_mn10300_howto_table[tls_r_type].name);
      break;
    }
#undef TLS_PAIR
  return 0;
...
static bfd_boolean
mn10300_cgc_check_relocs (bfd *abfd,
			  struct bfd_link_info *info,
			  asection *sec,
			  const Cgc_Internal_Rela *relocs)
{
  struct cgc32_mn10300_link_hash_table * htab = cgc32_mn10300_hash_table (info);
  ...
  local_got_offsets = cgc_local_got_offsets (abfd);
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      else
	{
      ...
      r_type = cgc_mn10300_tls_transition (info, r_type, h, sec, TRUE);
      ...
      if (dynobj == NULL)
	{
	  switch (r_type)
	    ...
	    case R_MN10300_TLS_IE:
	      ...
	      if (! _bfd_mn10300_cgc_create_got_section (dynobj, info))
		goto fail;
	      break;
	...
	create_got:
	  ...
	  if (r_type == R_MN10300_TLS_LD)
	    ...
	    {
	      if (cgc_mn10300_hash_entry (h)->tls_type != tls_type
		  && cgc_mn10300_hash_entry (h)->tls_type != GOT_UNKNOWN)
		{
		  if (tls_type == GOT_TLS_IE
		      && cgc_mn10300_hash_entry (h)->tls_type == GOT_TLS_GD)
	      ...
	      cgc_mn10300_hash_entry (h)->tls_type = tls_type;
		  ...
		  && h->dynindx == -1)
		{
	      ...
	      srelgot->size += sizeof (Cgc32_External_Rela);
	      if (r_type == R_MN10300_TLS_GD)
		srelgot->size += sizeof (Cgc32_External_Rela);
	    }
	  else
	      ...
	      if (local_got_offsets == NULL)
		{
		  ...
		  cgc_local_got_offsets (abfd) = local_got_offsets;
		  cgc_mn10300_local_got_tls_type (abfd)
	      ...
	      if (info->shared)
		{
	      ...
	      cgc_mn10300_local_got_tls_type (abfd) [r_symndx] = tls_type;
	    }
	...
	need_shared_relocs:
      ...
      if (CGC32_R_TYPE (rel->r_info) != R_MN10300_SYM_DIFF)
	sym_diff_reloc_seen = FALSE;
    }
...
static asection *
mn10300_cgc_gc_mark_hook (asection *sec,
			  struct bfd_link_info *info,
...
mn10300_cgc_final_link_relocate (reloc_howto_type *howto,
				 bfd *input_bfd,
				 ...
				 int is_local ATTRIBUTE_UNUSED)
{
  ...
  switch (r_type)
	  ...
	  && (input_section->flags & SEC_ALLOC) != 0)
	{
	  ...
	  outrel.r_offset = _bfd_cgc_section_offset (input_bfd, info,
	  ...
	  if (skip)
		...
		{
		  relocate = TRUE;
		  outrel.r_info = CGC32_R_INFO (0, R_MN10300_RELATIVE);
	      ...
	      else
		{
		  ...
		  relocate = FALSE;
		  outrel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_32);
		  outrel.r_addend = value + addend;
		}
	    }

	  bfd_cgc32_swap_reloca_out (output_bfd, &outrel,
				     (bfd_byte *) (((Cgc32_External_Rela *) sreloc->contents)
						   + sreloc->reloc_count));
	  ++sreloc->reloc_count;
      ...
      if (!htab->tls_ldm_got.rel_emitted)
	{
	  ...
	  rel.r_info = CGC32_R_INFO (0, R_MN10300_TLS_DTPMOD);
	  rel.r_addend = 0;
	  bfd_cgc32_swap_reloca_out (output_bfd, & rel,
				     (bfd_byte *) ((Cgc32_External_Rela *) srelgot->contents
						   + srelgot->reloc_count));
	  ++ srelgot->reloc_count;
	}
      ...
      else
	{
	  ...
	  off = cgc_local_got_offsets (input_bfd)[symndx];
	  ...
	  if (off & 1)
	      ...
	      if (info->shared)
		{
		  ...
		  switch (r_type)
		    {
		    case R_MN10300_TLS_GD:
		      outrel.r_info = CGC32_R_INFO (0, R_MN10300_TLS_DTPOFF);
		      outrel.r_offset = (sgot->output_section->vma
					 + sgot->output_offset
					 + off + 4);
		      bfd_cgc32_swap_reloca_out (output_bfd, & outrel,
						 (bfd_byte *) (((Cgc32_External_Rela *)
								srelgot->contents)
							       + srelgot->reloc_count));
		      ++ srelgot->reloc_count;
		      outrel.r_info = CGC32_R_INFO (0, R_MN10300_TLS_DTPMOD);
		      break;
		    case R_MN10300_TLS_GOTIE:
		    case R_MN10300_TLS_IE:
		      outrel.r_info = CGC32_R_INFO (0, R_MN10300_TLS_TPOFF);
		      break;
		    default:
		      outrel.r_info = CGC32_R_INFO (0, R_MN10300_RELATIVE);
		      break;
		  ...
		  bfd_cgc32_swap_reloca_out (output_bfd, &outrel,
					     (bfd_byte *) (((Cgc32_External_Rela *)
							   ...
							   + srelgot->reloc_count));
		  ++ srelgot->reloc_count;
		  cgc_local_got_offsets (input_bfd)[symndx] |= 1;
		}
...

/* Relocate an MN10300 CGC section.  */

static bfd_boolean
mn10300_cgc_relocate_section (bfd *output_bfd,
			      struct bfd_link_info *info,
			      bfd *input_bfd,
			      asection *input_section,
			      bfd_byte *contents,
  ...
  for (; rel < relend; rel++)
    {
      ...
      Cgc_Internal_Sym *sym;
      asection *sec;
      struct cgc32_mn10300_link_hash_entry *h;
      bfd_vma relocation;
      ...
      r_symndx = CGC32_R_SYM (rel->r_info);
      r_type = CGC32_R_TYPE (rel->r_info);
      howto = cgc_mn10300_howto_table + r_type;
      ...
      else
	{
      ...
      h = cgc_mn10300_hash_entry (hh);

      tls_r_type = cgc_mn10300_tls_transition (info, r_type, hh, input_section, 0);
      if (tls_r_type != r_type)
	{
	  ...
	  if (had_plt)
	    for (trel = rel+1; trel < relend; trel++)
	      if ((CGC32_R_TYPE (trel->r_info) == R_MN10300_PLT32
		   || CGC32_R_TYPE (trel->r_info) == R_MN10300_PCREL32)
		  && rel->r_offset + had_plt == trel->r_offset)
		trel->r_info = CGC32_R_INFO (0, R_MN10300_NONE);
	}
      ...
      else
	{
	  if ((h->root.root.type == bfd_link_hash_defined
		  ...
		  || ((   r_type == R_MN10300_PLT32
		      ...
		      && CGC_ST_VISIBILITY (h->root.other) != STV_INTERNAL
		      && CGC_ST_VISIBILITY (h->root.other) != STV_HIDDEN
	...
	continue;
      ...
      r = mn10300_cgc_final_link_relocate (howto, input_bfd, output_bfd,
					   input_section,
					   contents, rel->r_offset,
...
static bfd_boolean
cgc32_mn10300_finish_hash_table_entry (struct bfd_hash_entry *gen_entry,
				       void * in_args)
{
  struct cgc32_mn10300_link_hash_entry *entry;
  struct bfd_link_info *link_info = (struct bfd_link_info *) in_args;
  unsigned int byte_count = 0;

  entry = (struct cgc32_mn10300_link_hash_entry *) gen_entry;
  ...
  if (entry->direct_calls == 0
	  ...
	  && CGC_ST_VISIBILITY (entry->root.other) != STV_INTERNAL
	  && CGC_ST_VISIBILITY (entry->root.other) != STV_HIDDEN))
    {
...
static bfd_boolean
cgc32_mn10300_count_hash_table_entries (struct bfd_hash_entry *gen_entry ATTRIBUTE_UNUSED,
					void * in_args)
{
...
static bfd_boolean
cgc32_mn10300_list_hash_table_entries (struct bfd_hash_entry *gen_entry,
				       void * in_args)
{
...
{
  struct cgc32_mn10300_link_hash_entry *a
    = *(struct cgc32_mn10300_link_hash_entry **) va;
  struct cgc32_mn10300_link_hash_entry *b
    = *(struct cgc32_mn10300_link_hash_entry **) vb;
...
compute_function_info (bfd *abfd,
		       struct cgc32_mn10300_link_hash_entry *hash,
		       bfd_vma addr,
...
static bfd_boolean
mn10300_cgc_relax_delete_bytes (bfd *abfd,
				asection *sec,
				bfd_vma addr,
				int count)
{
  ...
  if (sec->reloc_count > 0)
    {
	 ...
	 serve to keep the section artifically inflated.  */
      if (CGC32_R_TYPE ((irelend - 1)->r_info) == (int) R_MN10300_ALIGN)
	--irelend;
      ...
      for (; irel < irelend; irel++)
	{
	  ...
	  if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_ALIGN
	      && irel->r_offset > addr
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
    {
      ...
      if ((irel->r_offset > addr
	   && irel->r_offset < toaddr)
	  || (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_ALIGN
	      && irel->r_offset == toaddr))
	irel->r_offset -= count;
    }
  ...
  for (isymend = isym + symtab_hdr->sh_info; isym < isymend; isym++)
	  ...
	  && isym->st_value < toaddr)
	{
      ...
      /* Adjust the function symbol's size as well.  */
	       ...
	       && CGC_ST_TYPE (isym->st_info) == STT_FUNC
  ...
  if (irelalign != NULL)
      ...
      if ((int) irelalign->r_addend > 0)
	{
	  ...
	  if (alignaddr < alignto)
	    /* Tail recursion.  */
	    return mn10300_cgc_relax_delete_bytes (abfd, sec, alignaddr,
...
static bfd_boolean
mn10300_cgc_symbol_address_p (bfd *abfd,
			      asection *sec,
			      Cgc_Internal_Sym *isym,
			      bfd_vma addr)
{
...
static bfd_boolean
mn10300_cgc_relax_section (bfd *abfd,
			   asection *sec,
			   ...
			   bfd_boolean *again)
{
  ...
  Cgc_Internal_Sym *isymbuf = NULL;
  struct cgc32_mn10300_link_hash_table *hash_table;
  ...
  *again = FALSE;
  ...
  hash_table = cgc32_mn10300_hash_table (link_info);
  if (hash_table == NULL)
    return FALSE;
  ...
  if ((hash_table->flags & MN10300_HASH_ENTRIES_INITIALIZED) == 0)
	   ...
	   input_bfd = input_bfd->link_next)
	{
	  ...
	  for (section = input_bfd->sections;
		  ...
		  && section->reloc_count != 0)
		{
		  ...
		  internal_relocs = _bfd_cgc_link_read_relocs (input_bfd, section,
							       NULL, NULL,
		  ...
		  for (; irel < irelend; irel++)
		      ...
		      if (r_index < symtab_hdr->sh_info)
			{
			  /* A local symbol.  */
			  Cgc_Internal_Sym *isym;
			  struct cgc_link_hash_table *cgctab;
			  bfd_size_type amt;
			  ...
			  if (isym->st_shndx == SHN_UNDEF)
			    ...
			    sym_sec
			      = bfd_section_from_cgc_index (input_bfd,
							    isym->st_shndx);
			  ...
			  /* If it isn't a function, then we don't care
			     about it.  */
			  if (CGC_ST_TYPE (isym->st_info) != STT_FUNC)
			    continue;
			  ...
			  cgctab = &hash_table->static_hash_table->root;
			  hash = ((struct cgc32_mn10300_link_hash_entry *)
				  cgc_link_hash_lookup (cgctab, sym_name,
							TRUE, TRUE, FALSE));
			  free (new_name);
			}
		      else
			{
			  ...
			  hash = (struct cgc32_mn10300_link_hash_entry *)
				   cgc_sym_hashes (input_bfd)[r_index];
			}
	      ...
	      if ((section->flags & SEC_CODE) != 0)
		{
		  ...
		  sec_shndx = _bfd_cgc_section_from_bfd_section (input_bfd,
								 section);

		  symcount = (symtab_hdr->sh_size / sizeof (Cgc32_External_Sym)
			      - symtab_hdr->sh_info);
		  hashes = cgc_sym_hashes (input_bfd);
		  ...
		  for (isym = isymbuf; isym < isymend; isym++)
			  ...
			  && CGC_ST_TYPE (isym->st_info) == STT_FUNC)
			{
			  struct cgc_link_hash_table *cgctab;
			  bfd_size_type amt;
			  struct cgc_link_hash_entry **lhashes = hashes;

			  /* Skip a local symbol if it aliases a
			     global one.  */
			  for (; lhashes < end_hashes; lhashes++)
			    {
			      hash = (struct cgc32_mn10300_link_hash_entry *) *lhashes;
			  ...
			  sym_name = (bfd_cgc_string_from_cgc_section
		  ...
		  for (; hashes < end_hashes; hashes++)
		    {
		      hash = (struct cgc32_mn10300_link_hash_entry *) *hashes;
		      if ((hash->root.root.type == bfd_link_hash_defined
			   || hash->root.root.type == bfd_link_hash_defweak)
			  && hash->root.root.u.def.section == section
			  && hash->root.type == STT_FUNC)
	      ...
	      if (internal_relocs != NULL
		  && cgc_section_data (section)->relocs != internal_relocs)
		free (internal_relocs);
	      internal_relocs = NULL;

	      /* Cache or free any memory we allocated for the contents.  */
	      if (contents != NULL
		  && cgc_section_data (section)->this_hdr.contents != contents)
		{
	 ...
	 the final initialization steps on each.  */
      cgc32_mn10300_link_hash_traverse (hash_table,
					cgc32_mn10300_finish_hash_table_entry,
					link_info);
      cgc32_mn10300_link_hash_traverse (hash_table->static_hash_table,
					cgc32_mn10300_finish_hash_table_entry,
					link_info);
	...
	struct cgc32_mn10300_link_hash_entry **entries;
	struct cgc32_mn10300_link_hash_entry **ptr;

	cgc32_mn10300_link_hash_traverse (hash_table->static_hash_table,
					  cgc32_mn10300_count_hash_table_entries,
					  &static_count);
	...
	ptr = entries;
	cgc32_mn10300_link_hash_traverse (hash_table->static_hash_table,
					  cgc32_mn10300_list_hash_table_entries,
					  & ptr);
	  ...
	  for (section = input_bfd->sections;
	      ...
	      else
		{
		 ...
		 insns deleted from its prologue.  */
	      ...
	      for (isym = isymbuf; isym < isymend; isym++)
		{
		  struct cgc32_mn10300_link_hash_entry *sym_hash;
		  asection *sym_sec = NULL;
		  const char *sym_name;
		  char *new_name;
		  struct cgc_link_hash_table *cgctab;
		  ...
		  if (isym->st_shndx == SHN_UNDEF)
		    ...
		    sym_sec
		      = bfd_section_from_cgc_index (input_bfd, isym->st_shndx);
		  ...
		  cgctab = & hash_table->static_hash_table->root;
		  sym_hash = (struct cgc32_mn10300_link_hash_entry *)
		    cgc_link_hash_lookup (cgctab, sym_name,
					  FALSE, FALSE, FALSE);
		  ...
		  if (! (sym_hash->flags & MN10300_CONVERT_CALL_TO_CALLS)
		      ...
		      if (sym_hash->stack_size > 0)
			{
			 ...
			 function.  */
		      ...
		      /* Actually delete the bytes.  */
		      if (!mn10300_cgc_relax_delete_bytes (input_bfd,
							   section,
							   isym->st_value,
							   bytes))
			goto error_return;
	      ...
	      for (; hashes < end_hashes; hashes++)
		{
		  ...
		  sym_hash = (struct cgc32_mn10300_link_hash_entry *) *hashes;
		  if ((sym_hash->root.root.type == bfd_link_hash_defined
			...
			goto error_return;
			 ...
			 prologue bytes deleted as well.  */
		      for (hh = cgc_sym_hashes (input_bfd); hh < end_hashes; hh++)
			{
			  struct cgc32_mn10300_link_hash_entry *h;

			  h = (struct cgc32_mn10300_link_hash_entry *) * hh;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      ...
      asection *sym_sec = NULL;
      struct cgc32_mn10300_link_hash_entry *h = NULL;

      /* If this isn't something that can be relaxed, then ignore
	 this reloc.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_NONE
	  || CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_8
	  || CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_MAX)
	continue;
      ...
      if (CGC32_R_SYM (irel->r_info) < symtab_hdr->sh_info)
	{
	  ...
	  sym_name = bfd_cgc_string_from_cgc_section (abfd,
						      symtab_hdr->sh_link,
						      isym->st_name);
	  ...
	  if ((sym_sec->flags & SEC_MERGE)
		...
		symval += irel->r_addend;
	      ...
	      symval = _bfd_merged_section_offset (abfd, & sym_sec,
						   cgc_section_data (sym_sec)->sec_info,
						   symval);
	  ...
	  h = (struct cgc32_mn10300_link_hash_entry *)
		cgc_link_hash_lookup (&hash_table->static_hash_table->root,
      ...
      else
	{
	 ...
	 insertion of prologue data into "call" instructions.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_PCREL32
	  || CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_PLT32)
	{
	  bfd_vma value = symval;

	  if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_PLT32
	      && h != NULL
	      && CGC_ST_VISIBILITY (h->root.other) != STV_INTERNAL
	      && CGC_ST_VISIBILITY (h->root.other) != STV_HIDDEN
	      && h->root.plt.offset != (bfd_vma) -1)
	  ...
	  if (h && (h->flags & MN10300_CONVERT_CALL_TO_CALLS))
	      ...
	      if (code == 0xdd)
		{
		  ...
		  if (!mn10300_cgc_relax_delete_bytes (abfd, sec,
						       irel->r_offset + 3, 1))
		    goto error_return;
	 ...
	 branch.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_PCREL16)
	{
	  bfd_vma value = symval;
	 ...
	 but due to other relaxations it can reach at link time.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_PCREL8)
	{
	  Cgc_Internal_Rela *nrel;
	  bfd_vma value = symval;
	  unsigned char code;
	  ...
	  if (nrel == irelend
	      ...
	      || CGC32_R_TYPE (nrel->r_info) != (int) R_MN10300_PCREL8)
	    continue;
	  ...
	  if (mn10300_cgc_symbol_address_p (abfd, sec, isymbuf,
					    irel->r_offset + 1))
	    continue;
	  ...
	  nrel->r_info = CGC32_R_INFO (CGC32_R_SYM (nrel->r_info),
				       R_MN10300_NONE);
	 ...
	 into a 8 immediate, displacement or absolute address.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_24)
	{
	  bfd_vma value = symval;
	  value += irel->r_addend;
	      ...
	      if (code == 0xfd)
		{
		  ...
		  if (code != 0x6b && code != 0x7b
		      ...
		      if ((value & 0x80) == 0)
			{
			  ...
			  /* Fix the relocation's type.  */
			  irel->r_info =
			    CGC32_R_INFO (CGC32_R_SYM (irel->r_info),
					  R_MN10300_8);
	 ...
	 into a 16bit immediate, displacement or absolute address.  */
      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_32
	  || CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_GOT32
	  || CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_GOTOFF32)
	{
	  bfd_vma value = symval;
	  ...
	  if (CGC32_R_TYPE (irel->r_info) != (int) R_MN10300_32)
	    {
	      ...
	      if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_GOT32)
		{
		  ...
		  if (h)
		    ...
		    value += (cgc_local_got_offsets
			      (abfd)[CGC32_R_SYM (irel->r_info)]);
		}
	      else if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_GOTOFF32)
		value -= sgot->output_section->vma;
	      else if (CGC32_R_TYPE (irel->r_info) == (int) R_MN10300_GOTPC32)
		value = (sgot->output_section->vma
			 - (sec->output_section->vma
			    + sec->output_offset
			    + irel->r_offset));
...
/* This is a version of bfd_generic_get_relocated_section_contents
   which uses mn10300_cgc_relocate_section.  */

static bfd_byte *
mn10300_cgc_get_relocated_section_contents (bfd *output_bfd,
					    struct bfd_link_info *link_info,
					    ...
					    asymbol **symbols)
{
  ...
  if ((input_section->flags & SEC_RELOC) != 0
      && input_section->reloc_count > 0)
    {
      ...
      internal_relocs = _bfd_cgc_link_read_relocs (input_bfd, input_section,
						   NULL, NULL, FALSE);
      if (internal_relocs == NULL)
	goto error_return;
      ...
      for (isym = isymbuf, secpp = sections; isym < isymend; ++isym, ++secpp)
	{
	  ...
	  *secpp = isec;
      ...
      if (! mn10300_cgc_relocate_section (output_bfd, link_info, input_bfd,
	...
	free (isymbuf);
      if (internal_relocs != cgc_section_data (input_section)->relocs)
	free (internal_relocs);
    }
 ...
 error_return:
  ...
  if (isymbuf != NULL && symtab_hdr->contents != (unsigned char *) isymbuf)
    free (isymbuf);
  if (internal_relocs != NULL
      && internal_relocs != cgc_section_data (input_section)->relocs)
    free (internal_relocs);
  return NULL;
}
...
/* Create an entry in an MN10300 CGC linker hash table.  */

static struct bfd_hash_entry *
cgc32_mn10300_link_hash_newfunc (struct bfd_hash_entry *entry,
				 struct bfd_hash_table *table,
				 const char *string)
{
  struct cgc32_mn10300_link_hash_entry *ret =
    (struct cgc32_mn10300_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == NULL)
    ret = (struct cgc32_mn10300_link_hash_entry *)
...
{
  struct cgc32_mn10300_link_hash_entry * edir;
  struct cgc32_mn10300_link_hash_entry * eind;

  edir = cgc_mn10300_hash_entry (dir);
  eind = cgc_mn10300_hash_entry (ind);
...
/* Create an mn10300 CGC linker hash table.  */

static struct bfd_link_hash_table *
cgc32_mn10300_link_hash_table_create (bfd *abfd)
{
  struct cgc32_mn10300_link_hash_table *ret;
  ...
  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      cgc32_mn10300_link_hash_newfunc,
				      sizeof (struct cgc32_mn10300_link_hash_entry),
				      MN10300_CGC_DATA))
    {
      free (ret);
      return NULL;
  ...
  amt = sizeof (struct cgc_link_hash_table);
  ...
  if (!_bfd_cgc_link_hash_table_init (&ret->static_hash_table->root, abfd,
				      cgc32_mn10300_link_hash_newfunc,
				      sizeof (struct cgc32_mn10300_link_hash_entry),
				      MN10300_CGC_DATA))
    {
...
/* Free an mn10300 CGC linker hash table.  */

static void
cgc32_mn10300_link_hash_table_free (struct bfd_link_hash_table *hash)
{
  struct cgc32_mn10300_link_hash_table *ret
    = (struct cgc32_mn10300_link_hash_table *) hash;
...
cgc_mn10300_mach (flagword flags)
{
...
/* The final processing done just before writing out a MN10300 CGC object
   file.  This gets the MN10300 architecture right based on the machine
   number.  */

static void
_bfd_mn10300_cgc_final_write_processing (bfd *abfd,
					 bfd_boolean linker ATTRIBUTE_UNUSED)
{
  ...
  cgc_cgcheader (abfd)->e_flags &= ~ (EF_MN10300_MACH);
  cgc_cgcheader (abfd)->e_flags |= val;
}

static bfd_boolean
_bfd_mn10300_cgc_object_p (bfd *abfd)
{
  bfd_default_set_arch_mach (abfd, bfd_arch_mn10300,
			     cgc_mn10300_mach (cgc_cgcheader (abfd)->e_flags));
  return TRUE;
}
...
static bfd_boolean
_bfd_mn10300_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
...
static const bfd_byte cgc_mn10300_plt0_entry[PLT0_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_mn10300_plt_entry[PLT_ENTRY_SIZE] =
{
...
static const bfd_byte cgc_mn10300_pic_plt_entry[PIC_PLT_ENTRY_SIZE] =
{
...
#define cgc_mn10300_sizeof_plt0(info) \
  (info->shared ? PIC_PLT_ENTRY_SIZE : PLT0_ENTRY_SIZE)

/* Return size of a PLT entry.  */
#define cgc_mn10300_sizeof_plt(info) \
  (info->shared ? PIC_PLT_ENTRY_SIZE : PLT_ENTRY_SIZE)

/* Return offset of the PLT0 address in an absolute PLT entry.  */
#define cgc_mn10300_plt_plt0_offset(info) 16

/* Return offset of the linker in PLT0 entry.  */
#define cgc_mn10300_plt0_linker_offset(info) 2

/* Return offset of the GOT id in PLT0 entry.  */
#define cgc_mn10300_plt0_gotid_offset(info) 9

/* Return offset of the temporary in PLT entry.  */
#define cgc_mn10300_plt_temp_offset(info) 8

/* Return offset of the symbol in PLT entry.  */
#define cgc_mn10300_plt_symbol_offset(info) 2

/* Return offset of the relocation in PLT entry.  */
#define cgc_mn10300_plt_reloc_offset(info) 11
...
static bfd_boolean
_bfd_mn10300_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  flagword   flags;
  asection * s;
  const struct cgc_backend_data * bed = get_cgc_backend_data (abfd);
  ...
  if (! _bfd_mn10300_cgc_create_got_section (abfd, info))
    return FALSE;
...
static bfd_boolean
_bfd_mn10300_cgc_adjust_dynamic_symbol (struct bfd_link_info * info,
					struct cgc_link_hash_entry * h)
{
  struct cgc32_mn10300_link_hash_table *htab = cgc32_mn10300_hash_table (info);
  bfd * dynobj;
  ...
  if (h->type == STT_FUNC
      ...
      if (h->dynindx == -1)
	{
	 ...
	 first entry.  */
      if (s->size == 0)
	s->size += cgc_mn10300_sizeof_plt0 (info);
	  ...
	  && !h->def_regular)
	{
      ...
      s->size += cgc_mn10300_sizeof_plt (info);
...
static bfd_boolean
_bfd_mn10300_cgc_size_dynamic_sections (bfd * output_bfd,
					struct bfd_link_info * info)
{
  struct cgc32_mn10300_link_hash_table *htab = cgc32_mn10300_hash_table (info);
  bfd * dynobj;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in _bfd_mn10300_cgc_finish_dynamic_sections,
	 but we must add the entries now so that we get the correct
	 size for the .dynamic section.  The DT_DEBUG entry is filled
	 in by the dynamic linker and used by the debugger.  */
      if (! info->shared)
	{
	  if (!_bfd_cgc_add_dynamic_entry (info, DT_DEBUG, 0))
	    return FALSE;
	}

      if (plt)
	{
	  if (!_bfd_cgc_add_dynamic_entry (info, DT_PLTGOT, 0)
	      || !_bfd_cgc_add_dynamic_entry (info, DT_PLTRELSZ, 0)
	      || !_bfd_cgc_add_dynamic_entry (info, DT_PLTREL, DT_RELA)
	      || !_bfd_cgc_add_dynamic_entry (info, DT_JMPREL, 0))
	    return FALSE;
	}

      if (relocs)
	{
	  if (!_bfd_cgc_add_dynamic_entry (info, DT_RELA, 0)
	      || !_bfd_cgc_add_dynamic_entry (info, DT_RELASZ, 0)
	      || !_bfd_cgc_add_dynamic_entry (info, DT_RELAENT,
      ...
      if (reltext)
	{
	  if (!_bfd_cgc_add_dynamic_entry (info, DT_TEXTREL, 0))
	    return FALSE;
	}
    }
...
static bfd_boolean
_bfd_mn10300_cgc_finish_dynamic_symbol (bfd * output_bfd,
					struct bfd_link_info * info,
					struct cgc_link_hash_entry * h,
					Cgc_Internal_Sym * sym)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    {
	 ...
	 first entry in the procedure linkage table is reserved.  */
      plt_index = ((h->plt.offset - cgc_mn10300_sizeof_plt0 (info))
		   / cgc_mn10300_sizeof_plt (info));
	 ...
	 The first three are reserved.  */
      ...
      if (! info->shared)
	{
	  memcpy (splt->contents + h->plt.offset, cgc_mn10300_plt_entry,
		  cgc_mn10300_sizeof_plt (info));
	  bfd_put_32 (output_bfd,
		      ...
		      (splt->contents + h->plt.offset
		       + cgc_mn10300_plt_symbol_offset (info)));

	  bfd_put_32 (output_bfd,
		      (1 - h->plt.offset - cgc_mn10300_plt_plt0_offset (info)),
		      (splt->contents + h->plt.offset
		       + cgc_mn10300_plt_plt0_offset (info)));
	}
      else
	{
	  memcpy (splt->contents + h->plt.offset, cgc_mn10300_pic_plt_entry,
		  cgc_mn10300_sizeof_plt (info));
      ...
      bfd_put_32 (output_bfd, plt_index * sizeof (Cgc32_External_Rela),
		  (splt->contents + h->plt.offset
		   + cgc_mn10300_plt_reloc_offset (info)));
      ...
      bfd_put_32 (output_bfd,
		  (splt->output_section->vma
		   + splt->output_offset
		   + h->plt.offset
		   + cgc_mn10300_plt_temp_offset (info)),
      ...
      rel.r_offset = (sgot->output_section->vma
		      + sgot->output_offset
		      + got_offset);
      rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_JMP_SLOT);
      rel.r_addend = 0;
      bfd_cgc32_swap_reloca_out (output_bfd, &rel,
				 (bfd_byte *) ((Cgc32_External_Rela *) srel->contents
					       + plt_index));
  ...
  if (h->got.offset != (bfd_vma) -1)
    {
      ...
      switch (cgc_mn10300_hash_entry (h)->tls_type)
	{
	case GOT_TLS_GD:
	  bfd_put_32 (output_bfd, (bfd_vma) 0, sgot->contents + h->got.offset);
	  bfd_put_32 (output_bfd, (bfd_vma) 0, sgot->contents + h->got.offset + 4);
	  rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_TLS_DTPMOD);
	  rel.r_addend = 0;
	  bfd_cgc32_swap_reloca_out (output_bfd, & rel,
				     (bfd_byte *) ((Cgc32_External_Rela *) srel->contents
						   + srel->reloc_count));
	  ++ srel->reloc_count;
	  rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_TLS_DTPOFF);
	  rel.r_offset += 4;
	  rel.r_addend = 0;
	  break;
	...
	case GOT_TLS_IE:
	  ...
	  bfd_put_32 (output_bfd, (bfd_vma) 0, sgot->contents + h->got.offset);
	  if (h->dynindx == -1)
	    rel.r_info = CGC32_R_INFO (0, R_MN10300_TLS_TPOFF);
	  else
	    rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_TLS_TPOFF);
	  break;
	...
	default:
	  ...
	  if (info->shared
	      ...
	      && h->def_regular)
	    {
	      rel.r_info = CGC32_R_INFO (0, R_MN10300_RELATIVE);
	    ...
	    {
	      bfd_put_32 (output_bfd, (bfd_vma) 0, sgot->contents + h->got.offset);
	      rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_GLOB_DAT);
	      rel.r_addend = 0;
	    }
	}

      if (CGC32_R_TYPE (rel.r_info) != R_MN10300_NONE)
	{
	  bfd_cgc32_swap_reloca_out (output_bfd, &rel,
				     (bfd_byte *) ((Cgc32_External_Rela *) srel->contents
						   + srel->reloc_count));
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_offset = (h->root.u.def.value
		      + h->root.u.def.section->output_section->vma
		      + h->root.u.def.section->output_offset);
      rel.r_info = CGC32_R_INFO (h->dynindx, R_MN10300_COPY);
      rel.r_addend = 0;
      bfd_cgc32_swap_reloca_out (output_bfd, & rel,
				 (bfd_byte *) ((Cgc32_External_Rela *) s->contents
					       + s->reloc_count));
      ++ s->reloc_count;
    }
...
static bfd_boolean
_bfd_mn10300_cgc_finish_dynamic_sections (bfd * output_bfd,
					  struct bfd_link_info * info)
{
  bfd *      dynobj;
  asection * sgot;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (splt && splt->size > 0)
	{
	  if (info->shared)
	    {
	      memcpy (splt->contents, cgc_mn10300_pic_plt_entry,
		      cgc_mn10300_sizeof_plt (info));
	    }
	  else
	    {
	      memcpy (splt->contents, cgc_mn10300_plt0_entry, PLT0_ENTRY_SIZE);
	      bfd_put_32 (output_bfd,
			  sgot->output_section->vma + sgot->output_offset + 4,
			  splt->contents + cgc_mn10300_plt0_gotid_offset (info));
	      bfd_put_32 (output_bfd,
			  sgot->output_section->vma + sgot->output_offset + 8,
			  splt->contents + cgc_mn10300_plt0_linker_offset (info));
	    }
	  ...
	  /* UnixWare sets the entsize of .plt to 4, but this is incorrect
	     ...
	     a multiple of the sh_entsize.  Some CGC tools flag this as an
	  ...
	  cgc_section_data (splt->output_section)->this_hdr.sh_entsize = 1;
	}
    }
...
_bfd_mn10300_cgc_reloc_type_class (const struct bfd_link_info *info ATTRIBUTE_UNUSED,
...
/* Allocate space for an MN10300 extension to the bfd cgc data structure.  */

static bfd_boolean
mn10300_cgc_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct cgc_mn10300_obj_tdata),
				  MN10300_CGC_DATA);
}

#define bfd_cgc32_mkobject	mn10300_cgc_mkobject

#ifndef CGC_ARCH
#define TARGET_LITTLE_SYM	bfd_cgc32_mn10300_vec
#define TARGET_LITTLE_NAME	"cgc32-mn10300"
#define CGC_ARCH		bfd_arch_mn10300
#define CGC_TARGET_ID		MN10300_CGC_DATA
#define CGC_MACHINE_CODE	EM_MN10300
#define CGC_MACHINE_ALT1	EM_CYGNUS_MN10300
#define CGC_MAXPAGESIZE		0x1000
#endif

#define cgc_info_to_howto		mn10300_info_to_howto
#define cgc_info_to_howto_rel		0
#define cgc_backend_can_gc_sections	1
#define cgc_backend_rela_normal		1
#define cgc_backend_check_relocs	mn10300_cgc_check_relocs
#define cgc_backend_gc_mark_hook	mn10300_cgc_gc_mark_hook
#define cgc_backend_relocate_section	mn10300_cgc_relocate_section
#define bfd_cgc32_bfd_relax_section	mn10300_cgc_relax_section
#define bfd_cgc32_bfd_get_relocated_section_contents \
				mn10300_cgc_get_relocated_section_contents
#define bfd_cgc32_bfd_link_hash_table_create \
				cgc32_mn10300_link_hash_table_create
#define bfd_cgc32_bfd_link_hash_table_free \
				cgc32_mn10300_link_hash_table_free
