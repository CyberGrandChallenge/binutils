...
struct mips_got_entry
{
  ...
  union
  {
    ...
    struct mips_cgc_link_hash_entry *h;
  } d;
...
struct mips_cgc_got_per_bfd_arg
{
  /* The output bfd.  */
  bfd *obfd;
...
struct mips_cgc_traverse_got_arg
{
  struct bfd_link_info *info;
...
struct _mips_cgc_section_data
{
  struct bfd_cgc_section_data cgc;
  union
  {
    bfd_byte *tdata;
  } u;
...
#define mips_cgc_section_data(sec) \
  ((struct _mips_cgc_section_data *) cgc_section_data (sec))

#define is_mips_cgc(bfd)				\
  (bfd_get_flavour (bfd) == bfd_target_cgc_flavour	\
   && cgc_tdata (bfd) != NULL				\
   && cgc_object_id (bfd) == MIPS_CGC_DATA)
...
/* Macros for populating a mips_cgc_la25_stub.  */
...
/* This structure is passed to mips_cgc_sort_hash_table_f when sorting
   the dynamic symbols.  */

struct mips_cgc_hash_sort_data
{
  /* The symbol in the global GOT with the lowest dynamic symbol table
     index.  */
  struct cgc_link_hash_entry *low;
...
/* The MIPS CGC linker needs additional information for each symbol in
   the global hash table.  */

struct mips_cgc_link_hash_entry
{
  struct cgc_link_hash_entry root;

  /* External symbol information.  */
  EXTR esym;

  /* The la25 stub we have created for ths symbol, if any.  */
  struct mips_cgc_la25_stub *la25_stub;
...
/* MIPS CGC linker hash table.  */

struct mips_cgc_link_hash_table
{
  struct cgc_link_hash_table root;
  ...
  struct cgc_link_hash_entry *rld_symbol;
  ...
  /* The global symbol in the GOT with the lowest index in the dynamic
     symbol table.  */
  struct cgc_link_hash_entry *global_gotsym;
  ...
  /* The section used for mips_cgc_la25_stub trampolines.
     See the comment above that structure for details.  */
  asection *strampoline;

  /* A table of mips_cgc_la25_stubs, indexed by (input_section, offset)
     pairs.  */
  htab_t la25_stubs;
...
/* Get the MIPS CGC linker hash table from a link_info structure.  */

#define mips_cgc_hash_table(p) \
  (cgc_hash_table_id ((struct cgc_link_hash_table *) ((p)->hash)) \
  == MIPS_CGC_DATA ? ((struct mips_cgc_link_hash_table *) ((p)->hash)) : NULL)
...
/* MIPS CGC private object data.  */

struct mips_cgc_obj_tdata
{
  /* Generic CGC private object data.  */
  ...
  /* Used by _bfd_mips_cgc_find_nearest_line.  The structure could be
     included directly in this one, but there's no point to wasting
     the memory just for the infrequently called find_nearest_line.  */
  struct mips_cgc_find_line *find_line_info;
  ...
  asymbol *cgc_data_symbol;
  asymbol *cgc_text_symbol;
  asection *cgc_data_section;
  asection *cgc_text_section;
};

/* Get MIPS CGC private object data from BFD's tdata.  */

#define mips_cgc_tdata(bfd) \
  ((struct mips_cgc_obj_tdata *) (bfd)->tdata.any)
...
/* Structure used to pass information to mips_cgc_output_extsym.  */
...
static const char * const mips_cgc_dynsym_rtproc_names[] =
{
  "_procedure_table",
...
typedef struct
{
  ...
  bfd_byte konst[4];
  bfd_byte vaddr[4];
} Cgc32_External_crinfo;

typedef struct
{
  bfd_byte info[4];
  bfd_byte konst[4];
} Cgc32_External_crinfo2;
...
#define mips_cgc_set_cr_format(x,format)	((x).ctype = (format))
#define mips_cgc_set_cr_type(x,type)		((x).rtype = (type))
#define mips_cgc_set_cr_dist2to(x,v)		((x).dist2to = (v))
#define mips_cgc_set_cr_relvaddr(x,d)		((x).relvaddr = (d)<<2)

...

static struct mips_got_entry *mips_cgc_create_local_got_entry
  (bfd *, struct bfd_link_info *, bfd *, bfd_vma, unsigned long,
   struct mips_cgc_link_hash_entry *, int);
static bfd_boolean mips_cgc_sort_hash_table_f
  (struct mips_cgc_link_hash_entry *, void *);
static bfd_vma mips_cgc_high
  (bfd_vma);
static bfd_boolean mips_cgc_create_dynamic_relocation
  (bfd *, struct bfd_link_info *, const Cgc_Internal_Rela *,
   struct mips_cgc_link_hash_entry *, asection *, bfd_vma,
   bfd_vma *, asection *);
static bfd_vma mips_cgc_adjust_gp
...
#define LOAD_INTERLOCKS_P(abfd) \
  (   ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) != E_MIPS_ARCH_1) \
   || ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_MACH) == E_MIPS_MACH_3900))
...
#define JAL_TO_BAL_P(abfd) \
  ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_MACH) == E_MIPS_MACH_9000)
...
#define PIC_OBJECT_P(abfd) \
  ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_PIC) != 0)

/* Nonzero if ABFD is using the N32 ABI.  */
#define ABI_N32_P(abfd) \
  ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI2) != 0)

/* Nonzero if ABFD is using the N64 ABI.  */
#define ABI_64_P(abfd) \
  (get_cgc_backend_data (abfd)->s->cgcclass == CGCCLASS64)
...
#define MICROMIPS_P(abfd) \
  ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH_ASE_MICROMIPS) != 0)

/* The IRIX compatibility level we are striving for.  */
#define IRIX_COMPAT(abfd) \
  (get_cgc_backend_data (abfd)->cgc_backend_mips_irix_compat (abfd))
...
#define MIPS_CGC_OPTIONS_SECTION_NAME(abfd) \
  (NEWABI_P (abfd) ? ".MIPS.options" : ".options")

/* True if NAME is the recognized name of any SHT_MIPS_OPTIONS section.
   Some IRIX system files do not use MIPS_CGC_OPTIONS_SECTION_NAME.  */
#define MIPS_CGC_OPTIONS_SECTION_NAME_P(NAME) \
  (strcmp (NAME, ".MIPS.options") == 0 || strcmp (NAME, ".options") == 0)

/* Whether the section is readonly.  */
#define MIPS_CGC_READONLY_SECTION(sec) \
  ((sec->flags & (SEC_ALLOC | SEC_LOAD | SEC_READONLY))		\
   == (SEC_ALLOC | SEC_LOAD | SEC_READONLY))

/* The name of the stub section.  */
#define MIPS_CGC_STUB_SECTION_NAME(abfd) ".MIPS.stubs"

/* The size of an external REL relocation.  */
#define MIPS_CGC_REL_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->sizeof_rel)

/* The size of an external RELA relocation.  */
#define MIPS_CGC_RELA_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->sizeof_rela)

/* The size of an external dynamic table entry.  */
#define MIPS_CGC_DYN_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->sizeof_dyn)

/* The size of a GOT entry.  */
#define MIPS_CGC_GOT_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->arch_size / 8)

/* The size of the .rld_map section. */
#define MIPS_CGC_RLD_MAP_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->arch_size / 8)

/* The size of a symbol-table entry.  */
#define MIPS_CGC_SYM_SIZE(abfd) \
  (get_cgc_backend_data (abfd)->s->sizeof_sym)

/* The default alignment for sections, as a power of two.  */
#define MIPS_CGC_LOG_FILE_ALIGN(abfd)				\
  (get_cgc_backend_data (abfd)->s->log_file_align)

/* Get word-sized data.  */
#define MIPS_CGC_GET_WORD(abfd, ptr) \
  (ABI_64_P (abfd) ? bfd_get_64 (abfd, ptr) : bfd_get_32 (abfd, ptr))

/* Put out word-sized data.  */
#define MIPS_CGC_PUT_WORD(abfd, val, ptr)	\
  (ABI_64_P (abfd) 				\
   ? bfd_put_64 (abfd, val, ptr) 		\
   : bfd_put_32 (abfd, val, ptr))

/* The opcode for word-sized loads (LW or LD).  */
#define MIPS_CGC_LOAD_WORD(abfd) \
  (ABI_64_P (abfd) ? 0xdc000000 : 0x8c000000)

/* Add a dynamic symbol table-entry.  */
#define MIPS_CGC_ADD_DYNAMIC_ENTRY(info, tag, val)	\
  _bfd_cgc_add_dynamic_entry (info, tag, val)

#define MIPS_CGC_RTYPE_TO_HOWTO(abfd, rtype, rela)			\
  (get_cgc_backend_data (abfd)->cgc_backend_mips_rtype_to_howto (rtype, rela))

/* The name of the dynamic relocation section.  */
#define MIPS_CGC_REL_DYN_NAME(INFO) \
  (mips_cgc_hash_table (INFO)->is_vxworks ? ".rela.dyn" : ".rel.dyn")
...
/* The value to write into got[1] for SVR4 targets, to identify it is
   ...
   module pointer.  */
#define MIPS_CGC_GNU_GOT1_MASK(abfd) \
  ((bfd_vma) 1 << (ABI_64_P (abfd) ? 63 : 31))

/* The offset of $gp from the beginning of the .got section.  */
#define CGC_MIPS_GP_OFFSET(INFO) \
  (mips_cgc_hash_table (INFO)->is_vxworks ? 0x0 : 0x7ff0)

/* The maximum size of the GOT for it to be addressable using 16-bit
   offsets from $gp.  */
#define MIPS_CGC_GOT_MAX_SIZE(INFO) (CGC_MIPS_GP_OFFSET (INFO) + 0x7fff)
...
#define CGC_DYNAMIC_INTERPRETER(abfd) 		\
...
#define CGC_R_SYM(bfd, i)					\
  (ABI_64_P (bfd) ? CGC64_R_SYM (i) : CGC32_R_SYM (i))
#define CGC_R_TYPE(bfd, i)					\
  (ABI_64_P (bfd) ? CGC64_MIPS_R_TYPE (i) : CGC32_R_TYPE (i))
#define CGC_R_INFO(bfd, s, t)					\
  (ABI_64_P (bfd) ? CGC64_R_INFO (s, t) : CGC32_R_INFO (s, t))
#else
...

/* Look up an entry in a MIPS CGC linker hash table.  */

#define mips_cgc_link_hash_lookup(table, string, create, copy, follow)	\
  ((struct mips_cgc_link_hash_entry *)					\
   cgc_link_hash_lookup (&(table)->root, (string), (create),		\
			 (copy), (follow)))

/* Traverse a MIPS CGC linker hash table.  */

#define mips_cgc_link_hash_traverse(table, func, info)			\
  (cgc_link_hash_traverse						\
   (&(table)->root,							\
    (bfd_boolean (*) (struct cgc_link_hash_entry *, void *)) (func),	\
    (info)))
...
dtprel_base (struct bfd_link_info *info)
{
  ...
  if (cgc_hash_table (info)->tls_sec == NULL)
    return 0;
  return cgc_hash_table (info)->tls_sec->vma + DTP_OFFSET;
}
...
tprel_base (struct bfd_link_info *info)
{
  ...
  if (cgc_hash_table (info)->tls_sec == NULL)
    return 0;
  return cgc_hash_table (info)->tls_sec->vma + TP_OFFSET;
}

/* Create an entry in a MIPS CGC linker hash table.  */

static struct bfd_hash_entry *
mips_cgc_link_hash_newfunc (struct bfd_hash_entry *entry,
			    struct bfd_hash_table *table, const char *string)
{
  struct mips_cgc_link_hash_entry *ret =
    (struct mips_cgc_link_hash_entry *) entry;

  /* Allocate the structure if it has not already been allocated by a
     subclass.  */
  if (ret == NULL)
    ret = bfd_hash_allocate (table, sizeof (struct mips_cgc_link_hash_entry));
  if (ret == NULL)
    return (struct bfd_hash_entry *) ret;

  /* Call the allocation method of the superclass.  */
  ret = ((struct mips_cgc_link_hash_entry *)
	 _bfd_cgc_link_hash_newfunc ((struct bfd_hash_entry *) ret,
				     table, string));
  if (ret != NULL)
    {
      /* Set local fields.  */
...
/* Allocate MIPS CGC private object data.  */

bfd_boolean
_bfd_mips_cgc_mkobject (bfd *abfd)
{
  return bfd_cgc_allocate_object (abfd, sizeof (struct mips_cgc_obj_tdata),
				  MIPS_CGC_DATA);
}

bfd_boolean
_bfd_mips_cgc_new_section_hook (bfd *abfd, asection *sec)
{
  if (!sec->used_by_bfd)
    {
      struct _mips_cgc_section_data *sdata;
  ...
  return _bfd_cgc_new_section_hook (abfd, sec);
}

...
bfd_boolean
_bfd_mips_cgc_read_ecoff_info (bfd *abfd, asection *section,
			       struct ecoff_debug_info *debug)
{
  HDRR *symhdr;
  const struct ecoff_debug_swap *swap;
  ...
  swap = get_cgc_backend_data (abfd)->cgc_backend_ecoff_debug_swap;
...
static bfd_boolean
mips_cgc_create_procedure_table (void *handle, bfd *abfd,
				 ...
				 struct ecoff_debug_info *debug)
{
  ...
  if (rtproc == NULL)
    {
      mips_cgc_hash_table (info)->procedure_count = 0;
      goto error_return;
    }

  mips_cgc_hash_table (info)->procedure_count = count + 2;
...
static bfd_boolean
mips_cgc_create_stub_symbol (struct bfd_link_info *info,
			     struct mips_cgc_link_hash_entry *h,
			     const char *prefix, asection *s, bfd_vma value,
			     bfd_vma size)
{
  struct bfd_link_hash_entry *bh;
  struct cgc_link_hash_entry *cgch;
  const char *name;

  if (CGC_ST_IS_MICROMIPS (h->root.other))
    value |= 1;
  ...
  cgch = (struct cgc_link_hash_entry *) bh;
  cgch->type = CGC_ST_INFO (STB_LOCAL, STT_FUNC);
  cgch->size = size;
  cgch->forced_local = 1;
  return TRUE;
}
...
static bfd_boolean
mips_cgc_create_shadow_symbol (struct bfd_link_info *info,
			       struct mips_cgc_link_hash_entry *h,
			       const char *prefix)
{
  struct bfd_link_hash_entry *bh;
  ...
  cgch = (struct cgc_link_hash_entry *) bh;
  cgch->type = CGC_ST_INFO (STB_LOCAL, CGC_ST_TYPE (h->root.type));
  cgch->other = h->root.other;
  cgch->size = h->root.size;
  cgch->forced_local = 1;
  return TRUE;
}
...
mips16_stub_symndx (const struct cgc_backend_data *bed,
		    asection *sec ATTRIBUTE_UNUSED,
		    const Cgc_Internal_Rela *relocs,
		    const Cgc_Internal_Rela *relend)
{
  ...
  for (rel = relocs; rel < relend; rel += int_rels_per_ext_rel)
    if (CGC_R_TYPE (sec->owner, rel->r_info) == R_MIPS_NONE)
      return CGC_R_SYM (sec->owner, rel->r_info);

  /* Otherwise trust the first relocation, whatever its kind.  This is
     the traditional behavior.  */
  if (relocs < relend)
    return CGC_R_SYM (sec->owner, relocs->r_info);
...
static void
mips_cgc_check_mips16_stubs (struct bfd_link_info *info,
			     struct mips_cgc_link_hash_entry *h)
{
  ...
  if (h->fn_stub != NULL
      && h->root.dynindx != -1)
    {
      mips_cgc_create_shadow_symbol (info, h, ".mips16.");
      h->need_fn_stub = TRUE;
    }
...
/* Hashtable callbacks for mips_cgc_la25_stubs.  */

static hashval_t
mips_cgc_la25_stub_hash (const void *entry_)
{
  const struct mips_cgc_la25_stub *entry;

  entry = (struct mips_cgc_la25_stub *) entry_;
  return entry->h->root.root.u.def.section->id
    + entry->h->root.root.u.def.value;
}

static int
mips_cgc_la25_stub_eq (const void *entry1_, const void *entry2_)
{
  const struct mips_cgc_la25_stub *entry1, *entry2;

  entry1 = (struct mips_cgc_la25_stub *) entry1_;
  entry2 = (struct mips_cgc_la25_stub *) entry2_;
...
bfd_boolean
_bfd_mips_cgc_init_stubs (struct bfd_link_info *info,
			  asection *(*fn) (const char *, asection *,
					   asection *))
{
  struct mips_cgc_link_hash_table *htab;

  htab = mips_cgc_hash_table (info);
  if (htab == NULL)
    return FALSE;

  htab->add_stub_section = fn;
  htab->la25_stubs = htab_try_create (1, mips_cgc_la25_stub_hash,
				      mips_cgc_la25_stub_eq, NULL);
...
static bfd_boolean
mips_cgc_local_pic_function_p (struct mips_cgc_link_hash_entry *h)
{
  return ((h->root.root.type == bfd_link_hash_defined
	  ...
	  && (PIC_OBJECT_P (h->root.root.u.def.section->owner)
	      || CGC_ST_IS_MIPS_PIC (h->root.other)));
}
...
static bfd_vma
mips_cgc_get_la25_target (struct mips_cgc_la25_stub *stub,
			  asection **sec)
{
  if (CGC_ST_IS_MIPS16 (stub->h->root.other))
    {
...
static bfd_boolean
mips_cgc_add_la25_intro (struct mips_cgc_la25_stub *stub,
			 struct bfd_link_info *info)
{
  struct mips_cgc_link_hash_table *htab;
  char *name;
  ...
  /* Create the section.  */
  mips_cgc_get_la25_target (stub, &input_section);
  ...
  mips_cgc_create_stub_symbol (info, stub->h, ".pic.", s, s->size, 8);
  stub->stub_section = s;
  stub->offset = s->size;
...
static bfd_boolean
mips_cgc_add_la25_trampoline (struct mips_cgc_la25_stub *stub,
			      struct bfd_link_info *info)
{
  struct mips_cgc_link_hash_table *htab;
  asection *s;
  ...
  /* Create a symbol for the stub.  */
  mips_cgc_create_stub_symbol (info, stub->h, ".pic.", s, s->size, 16);
  stub->stub_section = s;
  stub->offset = s->size;
...
static bfd_boolean
mips_cgc_add_la25_stub (struct bfd_link_info *info,
			struct mips_cgc_link_hash_entry *h)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_cgc_la25_stub search, *stub;
  ...
  stub = (struct mips_cgc_la25_stub *) *slot;
  if (stub != NULL)
    {
  ...
  *slot = stub;
  ...
  value = mips_cgc_get_la25_target (stub, &s);
  use_trampoline_p = (value != 0 || s->alignment_power > 4);

  h->la25_stub = stub;
  return (use_trampoline_p
	  ? mips_cgc_add_la25_trampoline (stub, info)
	  : mips_cgc_add_la25_intro (stub, info));
}

/* A mips_cgc_link_hash_traverse callback that is called before sizing
   sections.  DATA points to a mips_htab_traverse_info structure.  */

static bfd_boolean
mips_cgc_check_symbols (struct mips_cgc_link_hash_entry *h, void *data)
{
  struct mips_htab_traverse_info *hti;

  hti = (struct mips_htab_traverse_info *) data;
  if (!hti->info->relocatable)
    mips_cgc_check_mips16_stubs (hti->info, h);

  if (mips_cgc_local_pic_function_p (h))
    {
      ...
      if (hti->info->relocatable)
	{
	  if (!PIC_OBJECT_P (hti->output_bfd))
	    h->root.other = CGC_ST_SET_MIPS_PIC (h->root.other);
	}
      else if (h->has_nonpic_branches && !mips_cgc_add_la25_stub (hti->info, h))
	{
	  hti->error = TRUE;
	  return FALSE;
	}
...
void
_bfd_mips_cgc_reloc_unshuffle (bfd *abfd, int r_type,
...
void
_bfd_mips_cgc_reloc_shuffle (bfd *abfd, int r_type,
			     bfd_boolean jal_shuffle, bfd_byte *data)
{
  bfd_vma first, second, val;
...
_bfd_mips_cgc_gprel16_with_gp (bfd *abfd, asymbol *symbol,
			       ...
			       bfd_boolean relocatable, void *data, bfd_vma gp)
{
  ...
  _bfd_mips_cgc_sign_extend (val, 16);
...
bfd_reloc_status_type
_bfd_mips_cgc_hi16_reloc (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc_entry,
...
bfd_reloc_status_type
_bfd_mips_cgc_got16_reloc (bfd *abfd, arelent *reloc_entry, asymbol *symbol,
			   ...
			   bfd *output_bfd, char **error_message)
{
  if ((symbol->flags & (BSF_GLOBAL | BSF_WEAK)) != 0
    ...
    return _bfd_mips_cgc_generic_reloc (abfd, reloc_entry, symbol, data,
					input_section, output_bfd,
					error_message);

  return _bfd_mips_cgc_hi16_reloc (abfd, reloc_entry, symbol, data,
...
bfd_reloc_status_type
_bfd_mips_cgc_lo16_reloc (bfd *abfd, arelent *reloc_entry, asymbol *symbol,
			  void *data, asection *input_section,
			  bfd *output_bfd, char **error_message)
{
  bfd_vma vallo;
  ...
  _bfd_mips_cgc_reloc_unshuffle (abfd, reloc_entry->howto->type, FALSE,
				 location);
  vallo = bfd_get_32 (abfd, location);
  _bfd_mips_cgc_reloc_shuffle (abfd, reloc_entry->howto->type, FALSE,
			       location);
  ...
  while (mips_hi16_list != NULL)
	...
	hi->rel.howto = MIPS_CGC_RTYPE_TO_HOWTO (abfd, R_MIPS_HI16, FALSE);
      else if (hi->rel.howto->type == R_MIPS16_GOT16)
	hi->rel.howto = MIPS_CGC_RTYPE_TO_HOWTO (abfd, R_MIPS16_HI16, FALSE);
      else if (hi->rel.howto->type == R_MICROMIPS_GOT16)
	hi->rel.howto = MIPS_CGC_RTYPE_TO_HOWTO (abfd, R_MICROMIPS_HI16, FALSE);

      /* VALLO is a signed 16-bit number.  Bias it by 0x8000 so that any
	 carry or borrow will induce a change of +1 or -1 in the high part.  */
      hi->rel.addend += (vallo + 0x8000) & 0xffff;

      ret = _bfd_mips_cgc_generic_reloc (abfd, &hi->rel, symbol, hi->data,
...
bfd_reloc_status_type
_bfd_mips_cgc_generic_reloc (bfd *abfd ATTRIBUTE_UNUSED, arelent *reloc_entry,
...
static void
bfd_mips_cgc32_swap_gptab_in (bfd *abfd, const Cgc32_External_gptab *ex,
			      Cgc32_gptab *in)
{
...
static void
bfd_mips_cgc32_swap_gptab_out (bfd *abfd, const Cgc32_gptab *in,
			       Cgc32_External_gptab *ex)
{
...
static void
bfd_cgc32_swap_compact_rel_out (bfd *abfd, const Cgc32_compact_rel *in,
				Cgc32_External_compact_rel *ex)
{
...
static void
bfd_cgc32_swap_crinfo_out (bfd *abfd, const Cgc32_crinfo *in,
			   Cgc32_External_crinfo *ex)
{
  unsigned long l;
...

/* A .reginfo section holds a single Cgc32_RegInfo structure.  These
   routines swap this structure in and out.  They are used outside of
   BFD, so they are globally visible.  */

void
bfd_mips_cgc32_swap_reginfo_in (bfd *abfd, const Cgc32_External_RegInfo *ex,
				Cgc32_RegInfo *in)
{
...
void
bfd_mips_cgc32_swap_reginfo_out (bfd *abfd, const Cgc32_RegInfo *in,
				 Cgc32_External_RegInfo *ex)
{
...
/* In the 64 bit ABI, the .MIPS.options section holds register
   information in an Cgc64_Reginfo structure.  These routines swap
   them in and out.  They are globally visible because they are used
   outside of BFD.  These routines are here so that gas can call them
   without worrying about whether the 64 bit ABI has been included.  */

void
bfd_mips_cgc64_swap_reginfo_in (bfd *abfd, const Cgc64_External_RegInfo *ex,
				Cgc64_Internal_RegInfo *in)
{
  in->ri_gprmask = H_GET_32 (abfd, ex->ri_gprmask);
...
void
bfd_mips_cgc64_swap_reginfo_out (bfd *abfd, const Cgc64_Internal_RegInfo *in,
				 Cgc64_External_RegInfo *ex)
{
  H_PUT_32 (abfd, in->ri_gprmask, ex->ri_gprmask);
...
void
bfd_mips_cgc_swap_options_in (bfd *abfd, const Cgc_External_Options *ex,
			      Cgc_Internal_Options *in)
{
...
void
bfd_mips_cgc_swap_options_out (bfd *abfd, const Cgc_Internal_Options *in,
			       Cgc_External_Options *ex)
{
...
{
  Cgc_Internal_Rela int_reloc1;
  Cgc_Internal_Rela int_reloc2;
  int diff;

  bfd_cgc32_swap_reloc_in (reldyn_sorting_bfd, arg1, &int_reloc1);
  bfd_cgc32_swap_reloc_in (reldyn_sorting_bfd, arg2, &int_reloc2);

  diff = CGC32_R_SYM (int_reloc1.r_info) - CGC32_R_SYM (int_reloc2.r_info);
  if (diff != 0)
    return diff;
...
/* Like sort_dynamic_relocs, but used for cgc64 relocations.  */
...
{
#ifdef BFD64
  Cgc_Internal_Rela int_reloc1[3];
  Cgc_Internal_Rela int_reloc2[3];

  (*get_cgc_backend_data (reldyn_sorting_bfd)->s->swap_reloc_in)
    (reldyn_sorting_bfd, arg1, int_reloc1);
  (*get_cgc_backend_data (reldyn_sorting_bfd)->s->swap_reloc_in)
    (reldyn_sorting_bfd, arg2, int_reloc2);

  if (CGC64_R_SYM (int_reloc1[0].r_info) < CGC64_R_SYM (int_reloc2[0].r_info))
    return -1;
  if (CGC64_R_SYM (int_reloc1[0].r_info) > CGC64_R_SYM (int_reloc2[0].r_info))
    return 1;
...
/* This routine is used to write out ECOFF debugging external symbol
   information.  It is called via mips_cgc_link_hash_traverse.  The
   ECOFF external symbol information must match the CGC external
...
static bfd_boolean
mips_cgc_output_extsym (struct mips_cgc_link_hash_entry *h, void *data)
{
  ...
  if (h->esym.ifd == -2)
	  ...
	  || h->root.root.type == bfd_link_hash_undefweak)
	{
	  ...
	  if (strcmp (name, mips_cgc_dynsym_rtproc_names[0]) == 0
	      || strcmp (name, mips_cgc_dynsym_rtproc_names[1]) == 0)
	    {
	      h->esym.asym.sc = scData;
	      h->esym.asym.st = stLabel;
	      h->esym.asym.value = 0;
	    }
	  else if (strcmp (name, mips_cgc_dynsym_rtproc_names[2]) == 0)
	    {
	      h->esym.asym.sc = scAbs;
	      h->esym.asym.st = stLabel;
	      h->esym.asym.value =
		mips_cgc_hash_table (einfo->info)->procedure_count;
	    }
	  else if (strcmp (name, "_gp_disp") == 0 && ! NEWABI_P (einfo->abfd))
	    {
	      h->esym.asym.sc = scAbs;
	      h->esym.asym.st = stLabel;
	      h->esym.asym.value = cgc_gp (einfo->abfd);
	    }
	  else
  ...
  if (h->root.root.type == bfd_link_hash_common)
    ...
    {
      struct mips_cgc_link_hash_entry *hd = h;

      while (hd->root.root.type == bfd_link_hash_indirect)
	hd = (struct mips_cgc_link_hash_entry *)h->root.root.u.i.link;
...
{
  const Cgc32_gptab *a1 = p1;
  const Cgc32_gptab *a2 = p2;
...
mips_cgc_hash_bfd_vma (bfd_vma addr)
{
#ifdef BFD64
  return addr + (addr >> 32);
#else
...
static hashval_t
mips_cgc_got_entry_hash (const void *entry_)
{
  ...
  return (entry->symndx
	  ...
	  + (entry->tls_type == GOT_TLS_LDM ? 0
	     : !entry->abfd ? mips_cgc_hash_bfd_vma (entry->d.address)
	     : entry->symndx >= 0 ? (entry->abfd->id
				     + mips_cgc_hash_bfd_vma (entry->d.addend))
	     : entry->d.h->root.root.root.hash));
}

static int
mips_cgc_got_entry_eq (const void *entry1, const void *entry2)
{
...
mips_got_page_ref_hash (const void *ref_)
{
  ...
  return ((ref->symndx >= 0
	  ...
	  + mips_cgc_hash_bfd_vma (ref->addend));
}
...
mips_cgc_create_got_info (bfd *abfd)
{
  struct mips_got_info *g;
  ...
  g->got_entries = htab_try_create (1, mips_cgc_got_entry_hash,
				    mips_cgc_got_entry_eq, NULL);
...
static struct mips_got_info *
mips_cgc_bfd_got (bfd *abfd, bfd_boolean create_p)
{
  struct mips_cgc_obj_tdata *tdata;

  if (!is_mips_cgc (abfd))
    return NULL;

  tdata = mips_cgc_tdata (abfd);
  if (!tdata->got && create_p)
    tdata->got = mips_cgc_create_got_info (abfd);
  return tdata->got;
}
...
static void
mips_cgc_replace_bfd_got (bfd *abfd, struct mips_got_info *g)
{
  struct mips_cgc_obj_tdata *tdata;

  BFD_ASSERT (is_mips_cgc (abfd));
  tdata = mips_cgc_tdata (abfd);
  if (tdata->got)
    {
...
static asection *
mips_cgc_rel_dyn_section (struct bfd_link_info *info, bfd_boolean create_p)
{
  const char *dname;
  asection *sreloc;
  bfd *dynobj;

  dname = MIPS_CGC_REL_DYN_NAME (info);
  ...
  if (sreloc == NULL && create_p)
    {
	  ...
	  || ! bfd_set_section_alignment (dynobj, sreloc,
					  MIPS_CGC_LOG_FILE_ALIGN (dynobj)))
	return NULL;
    }
  return sreloc;
}
...
static int
mips_cgc_reloc_tls_type (unsigned int r_type)
{
		     ...
		     struct cgc_link_hash_entry *h)
{
  ...
  bfd_boolean dyn = cgc_hash_table (info)->dynamic_sections_created;
...
static void
mips_cgc_count_got_entry (struct bfd_link_info *info,
			  struct mips_got_info *g,
...
static void
mips_cgc_output_dynamic_relocation (bfd *output_bfd,
				    asection *sreloc,
				    ...
				    bfd_vma offset)
{
  Cgc_Internal_Rela rel[3];

  memset (rel, 0, sizeof (rel));

  rel[0].r_info = CGC_R_INFO (output_bfd, indx, r_type);
  rel[0].r_offset = rel[1].r_offset = rel[2].r_offset = offset;

  if (ABI_64_P (output_bfd))
    {
      (*get_cgc_backend_data (output_bfd)->s->swap_reloc_out)
	(output_bfd, &rel[0],
	 (sreloc->contents
	  + reloc_index * sizeof (Cgc64_Mips_External_Rel)));
    }
  else
    bfd_cgc32_swap_reloc_out
      (output_bfd, &rel[0],
       (sreloc->contents
	+ reloc_index * sizeof (Cgc32_External_Rel)));
}

/* Initialize a set of TLS GOT entries for one symbol.  */

static void
mips_cgc_initialize_tls_slots (bfd *abfd, struct bfd_link_info *info,
			       struct mips_got_entry *entry,
			       struct mips_cgc_link_hash_entry *h,
			       bfd_vma value)
{
  ...
  if ((info->shared || indx != 0)
      && (h == NULL
	  || CGC_ST_VISIBILITY (h->root.other) == STV_DEFAULT
	  || h->root.type != bfd_link_hash_undefweak))
    need_relocs = TRUE;
  ...
  sreloc = mips_cgc_rel_dyn_section (info, FALSE);
  ...
  switch (entry->tls_type)
    ...
    case GOT_TLS_GD:
      /* General Dynamic.  */
      got_offset2 = got_offset + MIPS_CGC_GOT_SIZE (abfd);

      if (need_relocs)
	{
	  mips_cgc_output_dynamic_relocation
	  ...
	  if (indx)
	    ...
	    MIPS_CGC_PUT_WORD (abfd, value - dtprel_base (info),
			       sgot->contents + got_offset2);
	}
      else
	{
	  MIPS_CGC_PUT_WORD (abfd, 1,
	...
	{
	  if (indx == 0)
	    MIPS_CGC_PUT_WORD (abfd, value - cgc_hash_table (info)->tls_sec->vma,
			       sgot->contents + got_offset);
	  else
	    MIPS_CGC_PUT_WORD (abfd, 0,
			       sgot->contents + got_offset);
	...
	MIPS_CGC_PUT_WORD (abfd, value - tprel_base (info),
			   sgot->contents + got_offset);
      break;
...
static bfd_vma
mips_cgc_gotplt_index (struct bfd_link_info *info,
		       struct cgc_link_hash_entry *h)
{
  ...
  got_address = (htab->sgotplt->output_section->vma
		 ...
		 + (h->plt.plist->gotplt_index
		    * MIPS_CGC_GOT_SIZE (info->output_bfd)));
...
static bfd_vma
mips_cgc_local_got_index (bfd *abfd, bfd *ibfd, struct bfd_link_info *info,
			  bfd_vma value, unsigned long r_symndx,
			  struct mips_cgc_link_hash_entry *h, int r_type)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_got_entry *entry;
  ...
  entry = mips_cgc_create_local_got_entry (abfd, info, ibfd, value,
					   r_symndx, h, r_type);
  if (!entry)
    return MINUS_ONE;

  if (entry->tls_type)
    mips_cgc_initialize_tls_slots (abfd, info, entry, h, value);
  return entry->gotidx;
}
...
static bfd_vma
mips_cgc_primary_global_got_index (bfd *obfd, struct bfd_link_info *info,
				   struct cgc_link_hash_entry *h)
{
  struct mips_cgc_link_hash_table *htab;
...
static bfd_vma
mips_cgc_global_got_index (bfd *obfd, struct bfd_link_info *info, bfd *ibfd,
			   struct cgc_link_hash_entry *h, int r_type)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_got_info *g;
  ...
  g = mips_cgc_bfd_got (ibfd, FALSE);
  BFD_ASSERT (g);

  lookup.tls_type = mips_cgc_reloc_tls_type (r_type);
  if (!lookup.tls_type && g == mips_cgc_bfd_got (obfd, FALSE))
    return mips_cgc_primary_global_got_index (obfd, info, h);

  lookup.abfd = ibfd;
  lookup.symndx = -1;
  lookup.d.h = (struct mips_cgc_link_hash_entry *) h;
  ...
  if (lookup.tls_type)
	...
	value = (h->root.u.def.value
      ...
      mips_cgc_initialize_tls_slots (obfd, info, entry, lookup.d.h, value);
    }
  return gotidx;
}
...
static bfd_vma
mips_cgc_got_page (bfd *abfd, bfd *ibfd, struct bfd_link_info *info,
		   bfd_vma value, bfd_vma *offsetp)
{
  ...
  entry = mips_cgc_create_local_got_entry (abfd, info, ibfd, page, 0,
...
static bfd_vma
mips_cgc_got16_entry (bfd *abfd, bfd *ibfd, struct bfd_link_info *info,
		      bfd_vma value, bfd_boolean external)
{
  ...
  /* It doesn't matter whether the original relocation was R_MIPS_GOT16,
     ...
     same in all cases.  */
  entry = mips_cgc_create_local_got_entry (abfd, info, ibfd, value, 0,
					   NULL, R_MIPS_GOT16);
  if (entry)
    return entry->gotidx;
  else
...
static bfd_vma
mips_cgc_got_offset_from_index (struct bfd_link_info *info, bfd *output_bfd,
				bfd *input_bfd, bfd_vma got_index)
{
  ...
  gp = _bfd_get_gp_value (output_bfd)
    + mips_cgc_adjust_gp (output_bfd, htab->got_info, input_bfd);
...
mips_cgc_create_local_got_entry (bfd *abfd, struct bfd_link_info *info,
				 ...
				 int r_type)
{
  ...
  lookup.gotidx = MIPS_CGC_GOT_SIZE (abfd) * g->assigned_gotno++;
  *entry = lookup;
  *loc = entry;

  MIPS_CGC_PUT_WORD (abfd, value, htab->sgot->contents + entry->gotidx);
  ...
  if (htab->is_vxworks)
    {
      ...
      s = mips_cgc_rel_dyn_section (info, FALSE);
      got_address = (htab->sgot->output_section->vma
		     + htab->sgot->output_offset
		     + entry->gotidx);

      rloc = s->contents + (s->reloc_count++ * sizeof (Cgc32_External_Rela));
      outrel.r_offset = got_address;
      outrel.r_info = CGC32_R_INFO (STN_UNDEF, R_MIPS_32);
      outrel.r_addend = value;
      bfd_cgc32_swap_reloca_out (abfd, &outrel, rloc);
    }
...
/* Return the number of dynamic section symbols required by OUTPUT_BFD.
   ...
   much information is available to cgc_backend_omit_section_dynsym at
...
count_section_dynsyms (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  count = 0;
  if (info->shared || cgc_hash_table (info)->is_relocatable_executable)
    {
      asection *p;
	...
	if ((p->flags & SEC_EXCLUDE) == 0
	    && (p->flags & SEC_ALLOC) != 0
	    && !(*bed->cgc_backend_omit_section_dynsym) (output_bfd, info, p))
	  ++count;
    }
  return count;
}
...
static bfd_boolean
mips_cgc_sort_hash_table (bfd *abfd, struct bfd_link_info *info)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_cgc_hash_sort_data hsd;
  struct mips_got_info *g;

  if (cgc_hash_table (info)->dynsymcount == 0)
    return TRUE;
  ...
  hsd.max_unref_got_dynindx
    = hsd.min_got_dynindx
    = (cgc_hash_table (info)->dynsymcount - g->reloc_only_gotno);
  hsd.max_non_got_dynindx = count_section_dynsyms (abfd, info) + 1;
  mips_cgc_link_hash_traverse (((struct mips_cgc_link_hash_table *)
				cgc_hash_table (info)),
			       mips_cgc_sort_hash_table_f,
			       &hsd);
  ...
  BFD_ASSERT ((unsigned long) hsd.max_unref_got_dynindx
	      == cgc_hash_table (info)->dynsymcount);
  BFD_ASSERT (cgc_hash_table (info)->dynsymcount - hsd.min_got_dynindx
	      == g->global_gotno);
...
static bfd_boolean
mips_cgc_sort_hash_table_f (struct mips_cgc_link_hash_entry *h, void *data)
{
  struct mips_cgc_hash_sort_data *hsd = data;
  ...
  switch (h->global_got_area)
    ...
    case GGA_NORMAL:
      ...
      hsd->low = (struct cgc_link_hash_entry *) h;
      break;
...
static bfd_boolean
mips_cgc_record_got_entry (struct bfd_link_info *info, bfd *abfd,
...
static bfd_boolean
mips_cgc_record_global_got_symbol (struct cgc_link_hash_entry *h,
				   bfd *abfd, struct bfd_link_info *info,
				   bfd_boolean for_call, int r_type)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_cgc_link_hash_entry *hmips;
  struct mips_got_entry entry;
  unsigned char tls_type;
  ...
  hmips = (struct mips_cgc_link_hash_entry *) h;
  if (!for_call)
  ...
  if (h->dynindx == -1)
    {
      switch (CGC_ST_VISIBILITY (h->other))
	{
	case STV_INTERNAL:
	case STV_HIDDEN:
	  _bfd_cgc_link_hash_hide_symbol (info, h, TRUE);
	  break;
	}
  ...
  tls_type = mips_cgc_reloc_tls_type (r_type);
  if (tls_type == GOT_TLS_NONE && hmips->global_got_area > GGA_NORMAL)
    hmips->global_got_area = GGA_NORMAL;

  entry.abfd = abfd;
  entry.symndx = -1;
  entry.d.h = (struct mips_cgc_link_hash_entry *) h;
  entry.tls_type = tls_type;
  return mips_cgc_record_got_entry (info, abfd, &entry);
}
...
static bfd_boolean
mips_cgc_record_local_got_symbol (bfd *abfd, long symndx, bfd_vma addend,
				  struct bfd_link_info *info, int r_type)
{
  ...
  entry.tls_type = mips_cgc_reloc_tls_type (r_type);
  return mips_cgc_record_got_entry (info, abfd, &entry);
}
...
static bfd_boolean
mips_cgc_record_got_page_ref (struct bfd_link_info *info, bfd *abfd,
			      long symndx, struct cgc_link_hash_entry *h,
			      bfd_signed_vma addend)
{
  ...
  if (h)
    {
      lookup.symndx = -1;
      lookup.u.h = (struct mips_cgc_link_hash_entry *) h;
    }
  else
    {
...
static void
mips_cgc_allocate_dynamic_relocations (bfd *abfd, struct bfd_link_info *info,
				       unsigned int n)
{
  asection *s;
  struct mips_cgc_link_hash_table *htab;
  ...
  if (htab->is_vxworks)
    s->size += n * MIPS_CGC_RELA_SIZE (abfd);
  else
    {
      if (s->size == 0)
	{
	  /* Make room for a null element.  */
	  s->size += MIPS_CGC_REL_SIZE (abfd);
	  ++s->reloc_count;
	}
      s->size += n * MIPS_CGC_REL_SIZE (abfd);
    }
}

/* A htab_traverse callback for GOT entries, with DATA pointing to a
   mips_cgc_traverse_got_arg structure.  Count the number of GOT
   entries and TLS relocs.  Set DATA->value to true if we need
   to resolve indirect or warning symbols and then recreate the GOT.  */

static int
mips_cgc_check_recreate_got (void **entryp, void *data)
{
  struct mips_got_entry *entry;
  struct mips_cgc_traverse_got_arg *arg;

  entry = (struct mips_got_entry *) *entryp;
  arg = (struct mips_cgc_traverse_got_arg *) data;
  ...
  mips_cgc_count_got_entry (arg->info, arg->g, entry);
  return 1;
}

/* A htab_traverse callback for GOT entries, with DATA pointing to a
   mips_cgc_traverse_got_arg structure.  Add all entries to DATA->g,
   converting entries for indirect and warning symbols into entries
   for the target symbol.  Set DATA->g to null on error.  */

static int
mips_cgc_recreate_got (void **entryp, void *data)
{
  ...
  if (entry->abfd != NULL
      ...
      do
	{
	  ...
	  h = (struct mips_cgc_link_hash_entry *) h->root.root.u.i.link;
	}
...
static bfd_vma
mips_cgc_pages_for_range (const struct mips_got_page_range *range)
{
...
static bfd_boolean
mips_cgc_record_got_page_entry (struct mips_got_info *g,
				asection *sec, bfd_signed_vma addend)
{
  ...
  old_pages = mips_cgc_pages_for_range (range);
  ...
  if (addend < range->min_addend)
	...
	{
	  old_pages += mips_cgc_pages_for_range (range->next);
  ...
  new_pages = mips_cgc_pages_for_range (range);
...
/* A htab_traverse callback for which *REFP points to a mips_got_page_ref
   and for which DATA points to a mips_cgc_traverse_got_arg.  Work out
...
static bfd_boolean
mips_cgc_resolve_got_page_ref (void **refp, void *data)
{
  ...
  arg = (struct mips_cgc_traverse_got_arg *) data;
  htab = mips_cgc_hash_table (arg->info);
  ...
  if (ref->symndx < 0)
      ...
      if (isym == NULL)
	{
      ...
      sec = bfd_section_from_cgc_index (ref->u.abfd, isym->st_shndx);
      if (sec == NULL)
	{
	  arg->g = NULL;
	  return 0;
      ...
      if (sec->flags & SEC_MERGE)
	{
	  ...
	  secinfo = cgc_section_data (sec)->sec_info;
	  if (CGC_ST_TYPE (isym->st_info) == STT_SECTION)
  ...
  if (!mips_cgc_record_got_page_entry (arg->g, sec, addend))
    {
      arg->g = NULL;
      return 0;
    }
...
static bfd_boolean
mips_cgc_resolve_final_got_entries (struct bfd_link_info *info,
				    struct mips_got_info *g)
{
  struct mips_cgc_traverse_got_arg tga;
  ...
  tga.g = g;
  tga.value = FALSE;
  htab_traverse (g->got_entries, mips_cgc_check_recreate_got, &tga);
  if (tga.value)
    {
      *g = oldg;
      g->got_entries = htab_create (htab_size (oldg.got_entries),
				    mips_cgc_got_entry_hash,
				    mips_cgc_got_entry_eq, NULL);
      if (!g->got_entries)
	return FALSE;

      htab_traverse (oldg.got_entries, mips_cgc_recreate_got, &tga);
      if (!tga.g)
	return FALSE;
  ...
  tga.info = info;
  tga.g = g;
  htab_traverse (g->got_page_refs, mips_cgc_resolve_got_page_ref, &tga);

  return TRUE;
}

/* A mips_cgc_link_hash_traverse callback for which DATA points to the
...
static int
mips_cgc_count_got_symbols (struct mips_cgc_link_hash_entry *h, void *data)
{
  struct bfd_link_info *info;
  struct mips_cgc_link_hash_table *htab;
  struct mips_got_info *g;
  ...
  if (h->global_got_area != GGA_NONE)
	...
	/* On VxWorks, calls can refer directly to the .got.plt entry;
	   ...
	   will be allocated by _bfd_mips_cgc_adjust_dynamic_symbol.  */
	h->global_got_area = GGA_NONE;
...
/* A htab_traverse callback for GOT entries.  Add each one to the GOT
   given in mips_cgc_traverse_got_arg DATA.  Clear DATA->G on error.  */

static int
mips_cgc_add_got_entry (void **entryp, void *data)
{
  struct mips_got_entry *entry;
  struct mips_cgc_traverse_got_arg *arg;
  void **slot;
...
static int
mips_cgc_add_got_page_entry (void **entryp, void *data)
{
...
static int
mips_cgc_merge_got_with (bfd *abfd, struct mips_got_info *from,
			 struct mips_got_info *to,
			 struct mips_cgc_got_per_bfd_arg *arg)
{
  struct mips_cgc_traverse_got_arg tga;
  ...
  tga.info = arg->info;
  tga.g = to;
  htab_traverse (from->got_entries, mips_cgc_add_got_entry, &tga);
  if (!tga.g)
    return 0;

  htab_traverse (from->got_page_entries, mips_cgc_add_got_page_entry, &tga);
  if (!tga.g)
    return 0;

  mips_cgc_replace_bfd_got (abfd, to);
  return 1;
}
...
static bfd_boolean
mips_cgc_merge_got (bfd *abfd, struct mips_got_info *g,
		    struct mips_cgc_got_per_bfd_arg *arg)
{
  unsigned int estimate;
  int result;

  if (!mips_cgc_resolve_final_got_entries (arg->info, g))
    return FALSE;
  ...
  if (estimate <= arg->max_count)
      ...
      if (!arg->primary)
	{
      ...
      result = mips_cgc_merge_got_with (abfd, g, arg->primary, arg);
      if (result >= 0)
	return result;
    }
  ...
  if (arg->current)
    {
      result = mips_cgc_merge_got_with (abfd, g, arg->current, arg);
      if (result >= 0)
	return result;
    }
...
static bfd_boolean
mips_cgc_set_gotidx (void **entryp, long gotidx)
{
  struct mips_got_entry *entry;
...
/* Set the TLS GOT index for the GOT entry in ENTRYP.  DATA points to a
   mips_cgc_traverse_got_arg in which DATA->value is the size of one
   GOT entry.  Set DATA->g to null on failure.  */

static int
mips_cgc_initialize_tls_index (void **entryp, void *data)
{
  struct mips_got_entry *entry;
  struct mips_cgc_traverse_got_arg *arg;
  ...
  arg = (struct mips_cgc_traverse_got_arg *) data;
  if (!mips_cgc_set_gotidx (entryp, arg->value * arg->g->tls_assigned_gotno))
    {
      arg->g = NULL;
      return 0;
    }
...
/* A htab_traverse callback for GOT entries, where DATA points to a
   mips_cgc_traverse_got_arg.  Set the global_got_area of each global
   symbol to DATA->value.  */

static int
mips_cgc_set_global_got_area (void **entryp, void *data)
{
  struct mips_got_entry *entry;
  struct mips_cgc_traverse_got_arg *arg;
...
mips_cgc_set_global_gotidx (void **entryp, void *data)
{
  ...
  if (entry->abfd != NULL
      ...
      && entry->d.h->global_got_area != GGA_NONE)
    {
      if (!mips_cgc_set_gotidx (entryp, arg->value * arg->g->assigned_gotno))
	{
	  arg->g = NULL;
	  return 0;
	}
      ...
      if (arg->info->shared
	  || (cgc_hash_table (arg->info)->dynamic_sections_created
...
static int
mips_cgc_forbid_lazy_stubs (void **entryp, void *data)
{
  struct bfd_link_info *info;
  struct mips_cgc_link_hash_table *htab;
  struct mips_got_entry *entry;
...
/* Return the offset of an input bfd IBFD's GOT from the beginning of
   the primary GOT.  */
static bfd_vma
mips_cgc_adjust_gp (bfd *abfd, struct mips_got_info *g, bfd *ibfd)
{
  if (!g->next)
    return 0;
...
static bfd_boolean
mips_cgc_multi_got (bfd *abfd, struct bfd_link_info *info,
		    asection *got, bfd_size_type pages)
{
  struct mips_cgc_link_hash_table *htab;
  struct mips_cgc_got_per_bfd_arg got_per_bfd_arg;
  struct mips_cgc_traverse_got_arg tga;
  ...
  got_per_bfd_arg.max_count = ((MIPS_CGC_GOT_MAX_SIZE (info)
				/ MIPS_CGC_GOT_SIZE (abfd))
			       - htab->reserved_gotno);
  got_per_bfd_arg.max_pages = pages;
  /* The number of globals that will be included in the primary GOT.
     See the calls to mips_cgc_set_global_got_area below for more
     information.  */
  ...
  for (ibfd = info->input_bfds; ibfd; ibfd = ibfd->link_next)
    {
      gg = mips_cgc_bfd_got (ibfd, FALSE);
      if (gg && !mips_cgc_merge_got (ibfd, gg, &got_per_bfd_arg))
	return FALSE;
    }

  /* If we do not find any suitable primary GOT, create an empty one.  */
  if (got_per_bfd_arg.primary == NULL)
    g->next = mips_cgc_create_got_info (abfd);
  else
  ...
  /* Map the output bfd to the primary got.  That's what we're going
     ...
     list.  */
  mips_cgc_replace_bfd_got (abfd, g);
  ...
  htab_traverse (gg->got_entries, mips_cgc_set_global_got_area, &tga);
  tga.value = GGA_NORMAL;
  htab_traverse (g->got_entries, mips_cgc_set_global_got_area, &tga);
  ...
  do
    {
	 ...
	 this operation, as required by mips_cgc_initialize_tls_index. */
      gn = g->next;
      g->next = gg->next;
      gg->next = g;
	 ...
	 all non-TLS entries.  */
      ...
      tga.g = g;
      tga.value = MIPS_CGC_GOT_SIZE (abfd);
      htab_traverse (g->got_entries, mips_cgc_initialize_tls_index, &tga);
      if (!tga.g)
	return FALSE;
	 ...
	 lazy-binding stubs.  */
      if (g)
	htab_traverse (g->got_entries, mips_cgc_forbid_lazy_stubs, info);
    }
  while (g);

  got->size = assign * MIPS_CGC_GOT_SIZE (abfd);
  ...
  for (g = gg->next; g && g->next != gg; g = g->next)
    {
	 ...
	 relocations they need.  */
      ...
      tga.value = MIPS_CGC_GOT_SIZE (abfd);
      tga.g = g;
      htab_traverse (g->got_entries, mips_cgc_set_global_gotidx, &tga);
      if (!tga.g)
	return FALSE;
  ...
  if (needed_relocs)
    mips_cgc_allocate_dynamic_relocations (dynobj, info,
					   needed_relocs);
...
static const Cgc_Internal_Rela *
mips_cgc_next_relocation (bfd *abfd ATTRIBUTE_UNUSED, unsigned int r_type,
			  const Cgc_Internal_Rela *relocation,
			  const Cgc_Internal_Rela *relend)
{
  unsigned long r_symndx = CGC_R_SYM (abfd, relocation->r_info);

  while (relocation < relend)
    {
      if (CGC_R_TYPE (abfd, relocation->r_info) == r_type
	  && CGC_R_SYM (abfd, relocation->r_info) == r_symndx)
	return relocation;
...
static bfd_boolean
mips_cgc_local_relocation_p (bfd *input_bfd,
			     const Cgc_Internal_Rela *relocation,
			     asection **local_sections)
{
  ...
  r_symndx = CGC_R_SYM (input_bfd, relocation->r_info);
  symtab_hdr = &cgc_tdata (input_bfd)->symtab_hdr;
  extsymoff = (cgc_bad_symtab (input_bfd)) ? 0 : symtab_hdr->sh_info;

  if (r_symndx < extsymoff)
    return TRUE;
  if (cgc_bad_symtab (input_bfd) && local_sections[r_symndx] != NULL)
    return TRUE;
...
bfd_vma
_bfd_mips_cgc_sign_extend (bfd_vma value, int bits)
{
...
static bfd_boolean
mips_cgc_overflow_p (bfd_vma value, int bits)
{
...
static bfd_vma
mips_cgc_higher (bfd_vma value ATTRIBUTE_UNUSED)
{
#ifdef BFD64
...
static bfd_vma
mips_cgc_highest (bfd_vma value ATTRIBUTE_UNUSED)
{
#ifdef BFD64
...
static bfd_boolean
mips_cgc_create_compact_rel_section
  ...
  if (bfd_get_linker_section (abfd, ".compact_rel") == NULL)
    {
	  ...
	  || ! bfd_set_section_alignment (abfd, s,
					  MIPS_CGC_LOG_FILE_ALIGN (abfd)))
	return FALSE;

      s->size = sizeof (Cgc32_External_compact_rel);
    }
...
static bfd_boolean
mips_cgc_create_got_section (bfd *abfd, struct bfd_link_info *info)
{
  flagword flags;
  register asection *s;
  struct cgc_link_hash_entry *h;
  ...
  if (! (_bfd_generic_link_add_one_symbol
	 (info, abfd, "_GLOBAL_OFFSET_TABLE_", BSF_GLOBAL, s,
	  0, NULL, FALSE, get_cgc_backend_data (abfd)->collect, &bh)))
    return FALSE;

  h = (struct cgc_link_hash_entry *) bh;
  h->non_cgc = 0;
  h->def_regular = 1;
  h->type = STT_OBJECT;
  h->other = (h->other & ~CGC_ST_VISIBILITY (-1)) | STV_HIDDEN;
  cgc_hash_table (info)->hgot = h;

  if (info->shared
      && ! bfd_cgc_link_record_dynamic_symbol (info, h))
    return FALSE;

  htab->got_info = mips_cgc_create_got_info (abfd);
  mips_cgc_section_data (s)->cgc.this_hdr.sh_flags
...
static bfd_boolean
is_gott_symbol (struct bfd_link_info *info, struct cgc_link_hash_entry *h)
{
  return (mips_cgc_hash_table (info)->is_vxworks
	  && info->shared
...
/* Return TRUE if a relocation of type R_TYPE from INPUT_BFD might
   require an la25 stub.  See also mips_cgc_local_pic_function_p,
   which determines whether the destination function ever requires a
   stub.  */

static bfd_boolean
mips_cgc_relocation_needs_la25_stub (bfd *input_bfd, int r_type,
...
mips_cgc_calculate_relocation (bfd *abfd, bfd *input_bfd,
			       asection *input_section,
			       struct bfd_link_info *info,
			       const Cgc_Internal_Rela *relocation,
			       ...
			       bfd_boolean save_addend)
{
  ...
  /* The section in which the symbol referenced by the relocation is
     located.  */
  asection *sec = NULL;
  struct mips_cgc_link_hash_entry *h = NULL;
  ...
  r_symndx = CGC_R_SYM (input_bfd, relocation->r_info);
  r_type = CGC_R_TYPE (input_bfd, relocation->r_info);
  ...
  symtab_hdr = &cgc_tdata (input_bfd)->symtab_hdr;
  local_p = mips_cgc_local_relocation_p (input_bfd, relocation,
					 local_sections);
  was_local_p = local_p;
  if (! cgc_bad_symtab (input_bfd))
  ...
  if (local_p)
    {
      ...
      if (CGC_ST_TYPE (sym->st_info) != STT_SECTION
	  || (sec->flags & SEC_MERGE))
	symbol += sym->st_value;
      if ((sec->flags & SEC_MERGE)
	  && CGC_ST_TYPE (sym->st_info) == STT_SECTION)
	{
	  addend = _bfd_cgc_rel_local_sym (abfd, sym, &sec, addend);
	  addend -= symbol;
	  addend += sec->output_section->vma + sec->output_offset;
	}

      /* MIPS16/microMIPS text labels should be treated as odd.  */
      if (CGC_ST_IS_COMPRESSED (sym->st_other))
	++symbol;

      /* Record the name of this symbol, for our caller.  */
      *namep = bfd_cgc_string_from_cgc_section (input_bfd,
						symtab_hdr->sh_link,
						sym->st_name);
      if (*namep == '\0')
	*namep = bfd_section_name (input_bfd, sec);

      target_is_16_bit_code_p = CGC_ST_IS_MIPS16 (sym->st_other);
      target_is_micromips_code_p = CGC_ST_IS_MICROMIPS (sym->st_other);
    }
  else
    {
      ...
      h = ((struct mips_cgc_link_hash_entry *)
	   cgc_sym_hashes (input_bfd) [r_symndx - extsymoff]);
		       ...
		       ? "_DYNAMIC_LINK" : "_DYNAMIC_LINKING") == 0)
	{
	  /* If this is a dynamic link, we should have created a
	     ...
	     in in _bfd_mips_cgc_create_dynamic_sections.
      ...
      else if (CGC_MIPS_IS_OPTIONAL (h->root.other))
	{
	  ...
	  symbol = 0;
		...
		(info->unresolved_syms_in_objects == RM_GENERATE_ERROR)
		 || CGC_ST_VISIBILITY (h->root.other)))
	{
      ...
      else
	{
      ...
      target_is_16_bit_code_p = CGC_ST_IS_MIPS16 (h->root.other);
      target_is_micromips_code_p = CGC_ST_IS_MICROMIPS (h->root.other);
    }
  ...
  if (r_type != R_MIPS16_26
	  ...
	  || (local_p
	      && mips_cgc_tdata (input_bfd)->local_stubs != NULL
	      && mips_cgc_tdata (input_bfd)->local_stubs[r_symndx] != NULL))
      && !section_allows_mips16_refs_p (input_section))
    {
	 ...
	 stub.  */
      if (local_p)
	{
	  sec = mips_cgc_tdata (input_bfd)->local_stubs[r_symndx];
	  value = 0;
	}
      else
	{
  ...
  /* If this is a MIPS16 call with a stub, that is made through the PLT or
     ...
     indirect calls should use an indirect stub instead.  */
	   ...
	   && ((h != NULL && (h->call_stub != NULL || h->call_fp_stub != NULL))
	       || (local_p
		   && mips_cgc_tdata (input_bfd)->local_call_stubs != NULL
		   && mips_cgc_tdata (input_bfd)->local_call_stubs[r_symndx] != NULL))
	   && ((h != NULL && h->use_plt_entry) || !target_is_16_bit_code_p))
    {
      if (local_p)
	sec = mips_cgc_tdata (input_bfd)->local_call_stubs[r_symndx];
      else
	{
  ...
  /* If this is a direct call to a PIC function, redirect to the
     non-PIC stub.  */
	   ...
	   && mips_cgc_relocation_needs_la25_stub (input_bfd, r_type,
  ...
  /* For direct MIPS16 and microMIPS calls make sure the compressed PLT
     ...
     case the symbol will have been set by mips_cgc_set_plt_sym_value
  ...
  *cross_mode_jump_p = (!info->relocatable
  ...
  if (htab->got_info)
    gp += mips_cgc_adjust_gp (abfd, htab->got_info, input_bfd);
  ...
  switch (r_type)
    ...
    case R_MICROMIPS_TLS_LDM:
      ...
      if (tls_ldm_reloc_p (r_type))
	{
	  g = mips_cgc_local_got_index (abfd, input_bfd, info,
					0, 0, NULL, r_type);
	  if (g == MINUS_ONE)
	    return bfd_reloc_outofrange;
	}
      else if (!local_p)
	{
	  ...
	  if (htab->is_vxworks
		  ...
		  || call16_reloc_p (r_type)))
	    {
	      ...
	      g = mips_cgc_gotplt_index (info, &h->root);
	    }
	  else
	    {
	      BFD_ASSERT (addend == 0);
	      g = mips_cgc_global_got_index (abfd, info, input_bfd,
					     &h->root, r_type);
	      if (!TLS_RELOC_P (r_type)
		  && !cgc_hash_table (info)->dynamic_sections_created)
		/* This is a static link.  We must initialize the GOT entry.  */
		MIPS_CGC_PUT_WORD (dynobj, symbol, htab->sgot->contents + g);
	    }
	}
      else if (!htab->is_vxworks
      ...
      else
	{
      ...
      g = mips_cgc_got_offset_from_index (info, abfd, input_bfd, g);
      break;
    }
  ...
  if (h != NULL && is_gott_symbol (info, &h->root))
    {
      ...
      s = mips_cgc_rel_dyn_section (info, FALSE);
      loc = s->contents + s->reloc_count++ * sizeof (Cgc32_External_Rela);

      outrel.r_offset = (input_section->output_section->vma
			 + input_section->output_offset
			 + relocation->r_offset);
      outrel.r_info = CGC32_R_INFO (h->root.dynindx, r_type);
      outrel.r_addend = addend;
      bfd_cgc32_swap_reloca_out (abfd, &outrel, loc);

      /* If we've written this relocation for a readonly section,
	 we need to set DF_TEXTREL again, so that we do not delete the
	 DT_TEXTREL tag.  */
      if (MIPS_CGC_READONLY_SECTION (input_section))
  ...
  switch (r_type)
    ...
    case R_MIPS_16:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 16);
      overflowed_p = mips_cgc_overflow_p (value, 16);
      break;
	  ...
	  && (input_section->flags & SEC_ALLOC) != 0)
	{
	  ...
	  value = addend;
	  if (!mips_cgc_create_dynamic_relocation (abfd,
						   info,
						   relocation,
						   h,
						   sec,
    ...
    case R_MIPS16_26:
	 ...
	 mips_cgc_perform_relocation.  So, we just fall through to the
	 R_MIPS_26 case here.  */
    case R_MIPS_26:
    case R_MICROMIPS_26_S1:
      {
	...
	if (was_local_p)
	  ...
	  value = _bfd_mips_cgc_sign_extend (addend, 26 + shift);
    ...
    case R_MICROMIPS_TLS_DTPREL_HI16:
      value = (mips_cgc_high (addend + symbol - dtprel_base (info))
	       & howto->dst_mask);
      break;
    ...
    case R_MICROMIPS_TLS_TPREL_HI16:
      value = (mips_cgc_high (addend + symbol - tprel_base (info))
	       & howto->dst_mask);
      break;
    ...
    case R_MICROMIPS_HI16:
      if (!gp_disp_p)
	{
	  value = mips_cgc_high (addend + symbol);
	  value &= howto->dst_mask;
	}
      else
	{
	  ...
	  if (r_type == R_MIPS16_HI16)
	    value = mips_cgc_high (addend + gp - ((p + 4) & ~(bfd_vma) 0x3));
	  /* The microMIPS .cpload sequence uses the same assembly
	     instructions as the traditional psABI version, but the
	     incoming $t9 has the low bit set.  */
	  else if (r_type == R_MICROMIPS_HI16)
	    value = mips_cgc_high (addend + gp - p - 1);
	  else
	    value = mips_cgc_high (addend + gp - p);
	  overflowed_p = mips_cgc_overflow_p (value, 16);
	}
      break;
      ...
      else
	{
	  ...
	  /* The MIPS ABI requires checking the R_MIPS_LO16 relocation
	     ...
	     as required by the MIPS CGC ABI.  The R_MIPS_LO16
    ...
    case R_MIPS16_GPREL:
	 ...
	 differences are handled in mips_cgc_perform_relocation.  */
    case R_MIPS_GPREL16:
	...
	addend = _bfd_mips_cgc_sign_extend (addend, 16);
	...
	{
	  value = mips_cgc_got16_entry (abfd, input_bfd, info,
					symbol + addend, !was_local_p);
	  if (value == MINUS_ONE)
	    return bfd_reloc_outofrange;
	  value
	    = mips_cgc_got_offset_from_index (info, abfd, input_bfd, value);
	  overflowed_p = mips_cgc_overflow_p (value, 16);
	  break;
	}
    ...
    case R_MIPS_GNU_REL16_S2:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 18) - p;
      overflowed_p = mips_cgc_overflow_p (value, 18);
      value >>= howto->rightshift;
      value &= howto->dst_mask;
      break;

    case R_MICROMIPS_PC7_S1:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 8) - p;
      overflowed_p = mips_cgc_overflow_p (value, 8);
      value >>= howto->rightshift;
      value &= howto->dst_mask;
      break;

    case R_MICROMIPS_PC10_S1:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 11) - p;
      overflowed_p = mips_cgc_overflow_p (value, 11);
      value >>= howto->rightshift;
      value &= howto->dst_mask;
      break;

    case R_MICROMIPS_PC16_S1:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 17) - p;
      overflowed_p = mips_cgc_overflow_p (value, 17);
      value >>= howto->rightshift;
      value &= howto->dst_mask;
      break;

    case R_MICROMIPS_PC23_S2:
      value = symbol + _bfd_mips_cgc_sign_extend (addend, 25) - ((p | 3) ^ 3);
      overflowed_p = mips_cgc_overflow_p (value, 25);
      value >>= howto->rightshift;
      value &= howto->dst_mask;
      break;
    ...
    case R_MICROMIPS_GOT_PAGE:
      value = mips_cgc_got_page (abfd, input_bfd, info, symbol + addend, NULL);
      if (value == MINUS_ONE)
	return bfd_reloc_outofrange;
      value = mips_cgc_got_offset_from_index (info, abfd, input_bfd, value);
      overflowed_p = mips_cgc_overflow_p (value, 16);
      break;
    ...
    case R_MICROMIPS_GOT_OFST:
      if (local_p)
	mips_cgc_got_page (abfd, input_bfd, info, symbol + addend, &value);
      else
	value = addend;
      overflowed_p = mips_cgc_overflow_p (value, 16);
      break;
    ...
    case R_MICROMIPS_HIGHER:
      value = mips_cgc_higher (addend + symbol);
      value &= howto->dst_mask;
      break;

    case R_MIPS_HIGHEST:
    case R_MICROMIPS_HIGHEST:
      value = mips_cgc_highest (addend + symbol);
      value &= howto->dst_mask;
      break;
...
static bfd_vma
mips_cgc_obtain_contents (reloc_howto_type *howto,
			  const Cgc_Internal_Rela *relocation,
...
static bfd_boolean
mips_cgc_perform_relocation (struct bfd_link_info *info,
			     ...
			     bfd_boolean cross_mode_jump_p)
{
  bfd_vma x;
  bfd_byte *location;
  int r_type = CGC_R_TYPE (input_bfd, relocation->r_info);

  /* Figure out where the relocation is occurring.  */
  location = contents + relocation->r_offset;

  _bfd_mips_cgc_reloc_unshuffle (input_bfd, r_type, FALSE, location);

  /* Obtain the current value.  */
  x = mips_cgc_obtain_contents (howto, relocation, input_bfd, contents);
  ...
  _bfd_mips_cgc_reloc_shuffle (input_bfd, r_type, !info->relocatable,
			       location);
...
static bfd_boolean
mips_cgc_create_dynamic_relocation (bfd *output_bfd,
				    struct bfd_link_info *info,
				    const Cgc_Internal_Rela *rel,
				    struct mips_cgc_link_hash_entry *h,
				    ...
				    bfd_vma *addendp, asection *input_section)
{
  ...
  r_type = CGC_R_TYPE (output_bfd, rel->r_info);
  dynobj = cgc_hash_table (info)->dynobj;
  sreloc = mips_cgc_rel_dyn_section (info, FALSE);
  BFD_ASSERT (sreloc != NULL);
  BFD_ASSERT (sreloc->contents != NULL);
  BFD_ASSERT (sreloc->reloc_count * MIPS_CGC_REL_SIZE (output_bfd)
	      < sreloc->size);

  outrel[0].r_offset =
    _bfd_cgc_section_offset (output_bfd, info, input_section, rel[0].r_offset);
  if (ABI_64_P (output_bfd))
    {
      outrel[1].r_offset =
	_bfd_cgc_section_offset (output_bfd, info, input_section, rel[1].r_offset);
      outrel[2].r_offset =
	_bfd_cgc_section_offset (output_bfd, info, input_section, rel[2].r_offset);
    }
  ...
  if (outrel[0].r_offset == MINUS_TWO)
    {
	 ...
	 some sort.  Functions like _bfd_cgc_write_section_eh_frame expect
  ...
  if (h != NULL && ! SYMBOL_REFERENCES_LOCAL (info, &h->root))
	...
	{
	  indx = cgc_section_data (sec->output_section)->dynindx;
	  if (indx == 0)
	    {
	      asection *osec = htab->root.text_index_section;
	      indx = cgc_section_data (osec)->dynindx;
	    }
	  if (indx == 0)
	    abort ();
	}
  ...
  if (htab->is_vxworks)
    ...
    outrel[0].r_info = CGC32_R_INFO (indx, R_MIPS_32);
  else
    /* The relocation is always an REL32 relocation because we don't
       know where the shared library will wind up at load-time.  */
    outrel[0].r_info = CGC_R_INFO (output_bfd, (unsigned long) indx,
				   R_MIPS_REL32);
  ...
  /* For strict adherence to the ABI specification, we should
     ...
     However, since none of the existing CGC64 MIPS dynamic
     loaders seems to care, we don't waste space with these
     artificial relocations.  If this turns out to not be true,
     mips_cgc_allocate_dynamic_relocation() should be tweaked so
     as to make room for a pair of dynamic relocations per
     invocation if ABI_64_P, and here we should generate an
     additional relocation record with R_MIPS_64 by itself for a
     NULL symbol before this relocation record.  */
  outrel[1].r_info = CGC_R_INFO (output_bfd, 0,
				 ABI_64_P (output_bfd)
				 ? R_MIPS_64
				 : R_MIPS_NONE);
  outrel[2].r_info = CGC_R_INFO (output_bfd, 0, R_MIPS_NONE);
  ...
  if (ABI_64_P (output_bfd))
	...
	(output_bfd, &outrel[0],
	 (sreloc->contents
	  + sreloc->reloc_count * sizeof (Cgc64_Mips_External_Rel)));
    }
  else if (htab->is_vxworks)
    {
      /* VxWorks uses RELA rather than REL dynamic relocations.  */
      outrel[0].r_addend = *addendp;
      bfd_cgc32_swap_reloca_out
	(output_bfd, &outrel[0],
	 (sreloc->contents
	  + sreloc->reloc_count * sizeof (Cgc32_External_Rela)));
    }
  else
    bfd_cgc32_swap_reloc_out
      (output_bfd, &outrel[0],
       (sreloc->contents + sreloc->reloc_count * sizeof (Cgc32_External_Rel)));
  ...
  cgc_section_data (input_section->output_section)->this_hdr.sh_flags
    |= SHF_WRITE;
  ...
  if (IRIX_COMPAT (output_bfd) == ict_irix5)
      ...
      if (scpt)
	{
	  ...
	  mips_cgc_set_cr_format (cptrel, CRF_MIPS_LONG);
	  cptrel.vaddr = (rel->r_offset
			  + input_section->output_section->vma
			  + input_section->output_offset);
	  if (r_type == R_MIPS_REL32)
	    mips_cgc_set_cr_type (cptrel, CRT_MIPS_REL32);
	  else
	    mips_cgc_set_cr_type (cptrel, CRT_MIPS_WORD);
	  mips_cgc_set_cr_dist2to (cptrel, 0);
	  cptrel.konst = *addendp;

	  cr = (scpt->contents
		+ sizeof (Cgc32_External_compact_rel));
	  mips_cgc_set_cr_relvaddr (cptrel, 0);
	  bfd_cgc32_swap_crinfo_out (output_bfd, &cptrel,
				     ((Cgc32_External_crinfo *) cr
				      + scpt->reloc_count));
	  ++scpt->reloc_count;
	}
    }
...
unsigned long
_bfd_cgc_mips_mach (flagword flags)
{
...
static INLINE char *
cgc_mips_abi_name (bfd *abfd)
{
  flagword flags;

  flags = cgc_cgcheader (abfd)->e_flags;
...

/* MIPS CGC uses two common sections.  One is the usual one, and the
   other is for small objects.  All the small objects are kept
   together, and then referenced via the gp pointer, which yields
   faster assembler code.  This is what we use for the small common
   section.  This approach is copied from ecoff.c.  */
static asection mips_cgc_scom_section;
static asymbol mips_cgc_scom_symbol;
static asymbol *mips_cgc_scom_symbol_ptr;

/* MIPS CGC also uses an acommon section, which represents an
   allocated common symbol which may be overridden by a
   definition in a shared library.  */
static asection mips_cgc_acom_section;
static asymbol mips_cgc_acom_symbol;
static asymbol *mips_cgc_acom_symbol_ptr;

/* This is used for both the 32-bit and the 64-bit ABI.  */

void
_bfd_mips_cgc_symbol_processing (bfd *abfd, asymbol *asym)
{
  cgc_symbol_type *cgcsym;

  /* Handle the special MIPS section numbers that a symbol may use.  */
  cgcsym = (cgc_symbol_type *) asym;
  switch (cgcsym->internal_cgc_sym.st_shndx)
    {
    case SHN_MIPS_ACOMMON:
	 ...
	 we can consider these symbols to be in a new section.  */
      if (mips_cgc_acom_section.name == NULL)
	{
	  /* Initialize the acommon section.  */
	  mips_cgc_acom_section.name = ".acommon";
	  mips_cgc_acom_section.flags = SEC_ALLOC;
	  mips_cgc_acom_section.output_section = &mips_cgc_acom_section;
	  mips_cgc_acom_section.symbol = &mips_cgc_acom_symbol;
	  mips_cgc_acom_section.symbol_ptr_ptr = &mips_cgc_acom_symbol_ptr;
	  mips_cgc_acom_symbol.name = ".acommon";
	  mips_cgc_acom_symbol.flags = BSF_SECTION_SYM;
	  mips_cgc_acom_symbol.section = &mips_cgc_acom_section;
	  mips_cgc_acom_symbol_ptr = &mips_cgc_acom_symbol;
	}
      asym->section = &mips_cgc_acom_section;
      break;

    case SHN_COMMON:
      /* Common symbols less than the GP size are automatically
	 treated as SHN_MIPS_SCOMMON symbols on IRIX5.  */
      if (asym->value > cgc_gp_size (abfd)
	  || CGC_ST_TYPE (cgcsym->internal_cgc_sym.st_info) == STT_TLS
	  || IRIX_COMPAT (abfd) == ict_irix6)
	break;
      /* Fall through.  */
    case SHN_MIPS_SCOMMON:
      if (mips_cgc_scom_section.name == NULL)
	{
	  /* Initialize the small common section.  */
	  mips_cgc_scom_section.name = ".scommon";
	  mips_cgc_scom_section.flags = SEC_IS_COMMON;
	  mips_cgc_scom_section.output_section = &mips_cgc_scom_section;
	  mips_cgc_scom_section.symbol = &mips_cgc_scom_symbol;
	  mips_cgc_scom_section.symbol_ptr_ptr = &mips_cgc_scom_symbol_ptr;
	  mips_cgc_scom_symbol.name = ".scommon";
	  mips_cgc_scom_symbol.flags = BSF_SECTION_SYM;
	  mips_cgc_scom_symbol.section = &mips_cgc_scom_section;
	  mips_cgc_scom_symbol_ptr = &mips_cgc_scom_symbol;
	}
      asym->section = &mips_cgc_scom_section;
      asym->value = cgcsym->internal_cgc_sym.st_size;
      break;
  ...
  /* If this is an odd-valued function symbol, assume it's a MIPS16
     or microMIPS one.  */
  if (CGC_ST_TYPE (cgcsym->internal_cgc_sym.st_info) == STT_FUNC
      && (asym->value & 1) != 0)
    {
      asym->value--;
      if (MICROMIPS_P (abfd))
	cgcsym->internal_cgc_sym.st_other
	  = CGC_ST_SET_MICROMIPS (cgcsym->internal_cgc_sym.st_other);
      else
	cgcsym->internal_cgc_sym.st_other
	  = CGC_ST_SET_MIPS16 (cgcsym->internal_cgc_sym.st_other);
    }
}

/* Implement cgc_backend_eh_frame_address_size.  This differs from
   ...
   Both forms use cgc32 and have pointer-sized FDE addresses.
...
unsigned int
_bfd_mips_cgc_eh_frame_address_size (bfd *abfd, asection *sec)
{
  if (cgc_cgcheader (abfd)->e_ident[EI_CLASS] == CGCCLASS64)
    return 8;
  if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI) == E_MIPS_ABI_EABI64)
    {
	...
	return 8;
	  ...
	  && cgc_section_data (sec)->relocs != NULL
	  && (CGC32_R_TYPE (cgc_section_data (sec)->relocs[0].r_info)
	      == R_MIPS_64))
	return 8;
...
bfd_boolean
_bfd_mips_cgc_name_local_section_symbols (bfd *abfd)
{
...
bfd_boolean
_bfd_mips_cgc_section_processing (bfd *abfd, Cgc_Internal_Shdr *hdr)
{
  if (hdr->sh_type == SHT_MIPS_REGINFO
      && hdr->sh_size > 0)
    {
      ...
      BFD_ASSERT (hdr->sh_size == sizeof (Cgc32_External_RegInfo));
      BFD_ASSERT (hdr->contents == NULL);

      if (bfd_seek (abfd,
		    hdr->sh_offset + sizeof (Cgc32_External_RegInfo) - 4,
		    SEEK_SET) != 0)
	return FALSE;
      H_PUT_32 (abfd, cgc_gp (abfd), buf);
  ...
  if (hdr->sh_type == SHT_MIPS_OPTIONS
      ...
      && mips_cgc_section_data (hdr->bfd_section) != NULL
      && mips_cgc_section_data (hdr->bfd_section)->u.tdata != NULL)
    {
	 ...
	 At this point we know that cgc_gp is set, so we can look
	 through the section contents to see if there is an
	 ODK_REGINFO structure.  */

      contents = mips_cgc_section_data (hdr->bfd_section)->u.tdata;
      l = contents;
      lend = contents + hdr->sh_size;
      while (l + sizeof (Cgc_External_Options) <= lend)
	{
	  Cgc_Internal_Options intopt;

	  bfd_mips_cgc_swap_options_in (abfd, (Cgc_External_Options *) l,
					&intopt);
	  if (intopt.size < sizeof (Cgc_External_Options))
	    {
	      (*_bfd_error_handler)
		(_("%B: Warning: bad `%s' option size %u smaller than its header"),
		abfd, MIPS_CGC_OPTIONS_SECTION_NAME (abfd), intopt.size);
	      break;
	    }
	  if (ABI_64_P (abfd) && intopt.kind == ODK_REGINFO)
	    {
	      ...
	      if (bfd_seek (abfd,
			    (hdr->sh_offset
			     + (l - contents)
			     + sizeof (Cgc_External_Options)
			     + (sizeof (Cgc64_External_RegInfo) - 8)),
			     SEEK_SET) != 0)
		return FALSE;
	      H_PUT_64 (abfd, cgc_gp (abfd), buf);
	  ...
	  else if (intopt.kind == ODK_REGINFO)
	    {
	      ...
	      if (bfd_seek (abfd,
			    (hdr->sh_offset
			     + (l - contents)
			     + sizeof (Cgc_External_Options)
			     + (sizeof (Cgc32_External_RegInfo) - 4)),
			    SEEK_SET) != 0)
		return FALSE;
	      H_PUT_32 (abfd, cgc_gp (abfd), buf);
	      if (bfd_bwrite (buf, 4, abfd) != 4)
  ...
  if (hdr->bfd_section != NULL)
    {
	 ...
	 _bfd_mips_cgc_special_sections will ensure the correct flags
...
/* Handle a MIPS specific section when reading an object file.  This
   is called when cgccode.h finds a section with an unknown type.
   This routine supports both the 32-bit and 64-bit CGC ABI.

   FIXME: We need to handle the SHF_MIPS_GPREL flag, but I'm not sure
   how to.  */

bfd_boolean
_bfd_mips_cgc_section_from_shdr (bfd *abfd,
				 Cgc_Internal_Shdr *hdr,
				 const char *name,
				 int shindex)
{
  flagword flags = 0;

  /* There ought to be a place to keep CGC backend specific flags, but
  ...
  switch (hdr->sh_type)
    ...
    case SHT_MIPS_REGINFO:
	  ...
	  || hdr->sh_size != sizeof (Cgc32_External_RegInfo))
	return FALSE;
    ...
    case SHT_MIPS_OPTIONS:
      if (!MIPS_CGC_OPTIONS_SECTION_NAME_P (name))
	return FALSE;
      break;
    case SHT_MIPS_DWARF:
  ...
  if (! _bfd_cgc_make_section_from_shdr (abfd, hdr, name, shindex))
    return FALSE;
  ...
  /* For a .reginfo section, set the gp value in the tdata information
     ...
     is not used in the 64-bit MIPS CGC ABI.  */
  if (hdr->sh_type == SHT_MIPS_REGINFO)
    {
      Cgc32_External_RegInfo ext;
      Cgc32_RegInfo s;

      if (! bfd_get_section_contents (abfd, hdr->bfd_section,
				      &ext, 0, sizeof ext))
	return FALSE;
      bfd_mips_cgc32_swap_reginfo_in (abfd, &ext, &s);
      cgc_gp (abfd) = s.ri_gp_value;
    }
  ...
  if (hdr->sh_type == SHT_MIPS_OPTIONS)
      ...
      while (l + sizeof (Cgc_External_Options) <= lend)
	{
	  ...
	  if (ABI_64_P (abfd) && intopt.kind == ODK_REGINFO)
	    {
	      Cgc64_Internal_RegInfo intreg;

	      bfd_mips_cgc64_swap_reginfo_in
		(abfd,
		 ((Cgc64_External_RegInfo *)
		  (l + sizeof (Cgc_External_Options))),
		 &intreg);
	      cgc_gp (abfd) = intreg.ri_gp_value;
	    }
	  else if (intopt.kind == ODK_REGINFO)
	    {
	      Cgc32_RegInfo intreg;

	      bfd_mips_cgc32_swap_reginfo_in
		(abfd,
		 ((Cgc32_External_RegInfo *)
		  (l + sizeof (Cgc_External_Options))),
		 &intreg);
	      cgc_gp (abfd) = intreg.ri_gp_value;
	    }
...
/* Set the correct type for a MIPS CGC section.  We do this by the
   section name, which is a hack, but ought to work.  This routine is
   used by both the 32-bit and the 64-bit ABI.  */

bfd_boolean
_bfd_mips_cgc_fake_sections (bfd *abfd, Cgc_Internal_Shdr *hdr, asection *sec)
{
  ...
  if (strcmp (name, ".liblist") == 0)
    {
      ...
      hdr->sh_info = sec->size / sizeof (Cgc32_Lib);
  ...
  else if (CONST_STRNEQ (name, ".gptab."))
    {
      ...
      hdr->sh_entsize = sizeof (Cgc32_External_gptab);
      ...
      if (SGI_COMPAT (abfd))
	{
	  if ((abfd->flags & DYNAMIC) != 0)
	    hdr->sh_entsize = sizeof (Cgc32_External_RegInfo);
	  else
	    hdr->sh_entsize = 1;
	}
      else
  ...
  else if (MIPS_CGC_OPTIONS_SECTION_NAME_P (name))
    {
  ...
  /* The generic cgc_fake_sections will set up REL_HDR using the default
...
/* Given a BFD section, try to locate the corresponding CGC section
...
bfd_boolean
_bfd_mips_cgc_section_from_bfd_section (bfd *abfd ATTRIBUTE_UNUSED,
...
bfd_boolean
_bfd_mips_cgc_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
			       Cgc_Internal_Sym *sym, const char **namep,
			       ...
			       asection **secp, bfd_vma *valp)
{
  ...
  switch (sym->st_shndx)
    ...
    case SHN_COMMON:
	 ...
	 treated as SHN_MIPS_SCOMMON symbols.  */
      if (sym->st_size > cgc_gp_size (abfd)
	  || CGC_ST_TYPE (sym->st_info) == STT_TLS
	  || IRIX_COMPAT (abfd) == ict_irix6)
	break;
      /* Fall through.  */
    case SHN_MIPS_SCOMMON:
    ...
    case SHN_MIPS_TEXT:
      ...
      if (mips_cgc_tdata (abfd)->cgc_text_section == NULL)
	{
	  asymbol *cgc_text_symbol;
	  asection *cgc_text_section;
	  bfd_size_type amt = sizeof (asection);

	  cgc_text_section = bfd_zalloc (abfd, amt);
	  if (cgc_text_section == NULL)
	    return FALSE;

	  amt = sizeof (asymbol);
	  cgc_text_symbol = bfd_zalloc (abfd, amt);
	  if (cgc_text_symbol == NULL)
	    return FALSE;

	  /* Initialize the section.  */

	  mips_cgc_tdata (abfd)->cgc_text_section = cgc_text_section;
	  mips_cgc_tdata (abfd)->cgc_text_symbol = cgc_text_symbol;

	  cgc_text_section->symbol = cgc_text_symbol;
	  cgc_text_section->symbol_ptr_ptr = &mips_cgc_tdata (abfd)->cgc_text_symbol;

	  cgc_text_section->name = ".text";
	  cgc_text_section->flags = SEC_NO_FLAGS;
	  cgc_text_section->output_section = NULL;
	  cgc_text_section->owner = abfd;
	  cgc_text_symbol->name = ".text";
	  cgc_text_symbol->flags = BSF_SECTION_SYM | BSF_DYNAMIC;
	  cgc_text_symbol->section = cgc_text_section;
	}
      /* This code used to do *secp = bfd_und_section_ptr if
         info->shared.  I don't know why, and that doesn't make sense,
         so I took it out.  */
      *secp = mips_cgc_tdata (abfd)->cgc_text_section;
      break;
    ...
    case SHN_MIPS_DATA:
      /* This section is used in a shared object.  */
      if (mips_cgc_tdata (abfd)->cgc_data_section == NULL)
	{
	  asymbol *cgc_data_symbol;
	  asection *cgc_data_section;
	  bfd_size_type amt = sizeof (asection);

	  cgc_data_section = bfd_zalloc (abfd, amt);
	  if (cgc_data_section == NULL)
	    return FALSE;

	  amt = sizeof (asymbol);
	  cgc_data_symbol = bfd_zalloc (abfd, amt);
	  if (cgc_data_symbol == NULL)
	    return FALSE;

	  /* Initialize the section.  */

	  mips_cgc_tdata (abfd)->cgc_data_section = cgc_data_section;
	  mips_cgc_tdata (abfd)->cgc_data_symbol = cgc_data_symbol;

	  cgc_data_section->symbol = cgc_data_symbol;
	  cgc_data_section->symbol_ptr_ptr = &mips_cgc_tdata (abfd)->cgc_data_symbol;

	  cgc_data_section->name = ".data";
	  cgc_data_section->flags = SEC_NO_FLAGS;
	  cgc_data_section->output_section = NULL;
	  cgc_data_section->owner = abfd;
	  cgc_data_symbol->name = ".data";
	  cgc_data_symbol->flags = BSF_SECTION_SYM | BSF_DYNAMIC;
	  cgc_data_symbol->section = cgc_data_section;
	}
      /* This code used to do *secp = bfd_und_section_ptr if
         info->shared.  I don't know why, and that doesn't make sense,
         so I took it out.  */
      *secp = mips_cgc_tdata (abfd)->cgc_data_section;
      break;
  ...
  if (SGI_COMPAT (abfd)
      ...
      && strcmp (*namep, "__rld_obj_head") == 0)
    {
      ...
      if (! (_bfd_generic_link_add_one_symbol
	     (info, abfd, *namep, BSF_GLOBAL, *secp, *valp, NULL, FALSE,
	      get_cgc_backend_data (abfd)->collect, &bh)))
	return FALSE;
	...
	return FALSE;
      ...
      mips_cgc_hash_table (info)->use_rld_obj_head = TRUE;
      mips_cgc_hash_table (info)->rld_symbol = h;
    }
...
int
_bfd_mips_cgc_link_output_symbol_hook
  (struct bfd_link_info *info ATTRIBUTE_UNUSED,
   const char *name ATTRIBUTE_UNUSED, Cgc_Internal_Sym *sym,
   asection *input_sec, struct cgc_link_hash_entry *h ATTRIBUTE_UNUSED)
{
...
bfd_boolean
_bfd_mips_cgc_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  struct cgc_link_hash_entry *h;
  struct bfd_link_hash_entry *bh;
  flagword flags;
  ...
  if (!mips_cgc_create_got_section (abfd, info))
    return FALSE;

  if (! mips_cgc_rel_dyn_section (info, TRUE))
    return FALSE;

  /* Create .stub section.  */
  s = bfd_make_section_anyway_with_flags (abfd,
					  MIPS_CGC_STUB_SECTION_NAME (abfd),
					  flags | SEC_CODE);
  if (s == NULL
      || ! bfd_set_section_alignment (abfd, s,
				      MIPS_CGC_LOG_FILE_ALIGN (abfd)))
  ...
  if (!mips_cgc_hash_table (info)->use_rld_obj_head
      && !info->shared
  ...
  if (IRIX_COMPAT (abfd) == ict_irix5)
    {
      for (namep = mips_cgc_dynsym_rtproc_names; *namep != NULL; namep++)
	{
	  bh = NULL;
	  if (! (_bfd_generic_link_add_one_symbol
		 (info, abfd, *namep, BSF_GLOBAL, bfd_und_section_ptr, 0,
		  NULL, FALSE, get_cgc_backend_data (abfd)->collect, &bh)))
	    return FALSE;
      ...
      if (SGI_COMPAT (abfd))
	{
	  if (!mips_cgc_create_compact_rel_section (abfd, info))
	    return FALSE;
	}
      ...
      if (s != NULL)
	(void) bfd_set_section_alignment (abfd, s, MIPS_CGC_LOG_FILE_ALIGN (abfd));
  ...
  if (!info->shared)
      ...
      if (! mips_cgc_hash_table (info)->use_rld_obj_head)
	{
	  /* __rld_map is a four byte word located in the .data section
	     ...
	     _bfd_mips_cgc_finish_dynamic_symbol.  */
  ...
  if (!_bfd_cgc_create_dynamic_sections (abfd, info))
    return FALSE;
  ...
  if (htab->is_vxworks
      && !cgc_vxworks_create_dynamic_sections (abfd, info, &htab->srelplt2))
    return FALSE;
...
static bfd_boolean
mips_cgc_rel_relocation_p (bfd *abfd, asection *sec,
			   const Cgc_Internal_Rela *relocs,
			   const Cgc_Internal_Rela *rel)
{
  Cgc_Internal_Shdr *rel_hdr;
  const struct cgc_backend_data *bed;
  ...
  rel_hdr = cgc_section_data (sec)->rel.hdr;
  if (rel_hdr == NULL)
    return FALSE;
  bed = get_cgc_backend_data (abfd);
...
static bfd_vma
mips_cgc_read_rel_addend (bfd *abfd, const Cgc_Internal_Rela *rel,
			  reloc_howto_type *howto, bfd_byte *contents)
{
  ...
  r_type = CGC_R_TYPE (abfd, rel->r_info);
  location = contents + rel->r_offset;

  /* Get the addend, which is stored in the input file.  */
  _bfd_mips_cgc_reloc_unshuffle (abfd, r_type, FALSE, location);
  addend = mips_cgc_obtain_contents (howto, rel, abfd, contents);
  _bfd_mips_cgc_reloc_shuffle (abfd, r_type, FALSE, location);
...
static bfd_boolean
mips_cgc_add_lo16_rel_addend (bfd *abfd,
			      const Cgc_Internal_Rela *rel,
			      const Cgc_Internal_Rela *relend,
			      bfd_byte *contents, bfd_vma *addend)
{
  unsigned int r_type, lo16_type;
  const Cgc_Internal_Rela *lo16_relocation;
  ...
  /* The combined value is the sum of the HI16 addend, left-shifted by
     ...
     According to the MIPS CGC ABI, the R_MIPS_LO16 relocation must
  ...
  lo16_relocation = mips_cgc_next_relocation (abfd, lo16_type, rel, relend);
  if (lo16_relocation == NULL)
    return FALSE;

  /* Obtain the addend kept there.  */
  lo16_howto = MIPS_CGC_RTYPE_TO_HOWTO (abfd, lo16_type, FALSE);
  l = mips_cgc_read_rel_addend (abfd, lo16_relocation, lo16_howto, contents);

  l <<= lo16_howto->rightshift;
  l = _bfd_mips_cgc_sign_extend (l, 16);
...
static bfd_boolean
mips_cgc_get_section_contents (bfd *abfd, asection *sec, bfd_byte **contents)
{
  if (*contents)
    return TRUE;

  /* Get cached copy if it exists.  */
  if (cgc_section_data (sec)->this_hdr.contents != NULL)
    {
      *contents = cgc_section_data (sec)->this_hdr.contents;
      return TRUE;
    }
...
mips_cgc_make_plt_record (bfd *abfd)
{
...
bfd_boolean
_bfd_mips_cgc_check_relocs (bfd *abfd, struct bfd_link_info *info,
			    asection *sec, const Cgc_Internal_Rela *relocs)
{
  const char *name;
  bfd *dynobj;
  Cgc_Internal_Shdr *symtab_hdr;
  ...
  extsymoff = (cgc_bad_symtab (abfd)) ? 0 : symtab_hdr->sh_info;
  ...
  if (FN_STUB_P (name))
	  ...
	  || sym_hashes[r_symndx - extsymoff] == NULL)
	{
	  ...
	  for (o = abfd->sections; o != NULL; o = o->next)
	    {
	      Cgc_Internal_Rela *sec_relocs;
	      const Cgc_Internal_Rela *r, *rend;
		...
		continue;
	      ...
	      sec_relocs
		= _bfd_cgc_link_read_relocs (abfd, o, NULL, NULL,
					     info->keep_memory);
	      if (sec_relocs == NULL)
		return FALSE;
		...
		if (CGC_R_SYM (abfd, r->r_info) == r_symndx
		    && !mips16_call_reloc_p (CGC_R_TYPE (abfd, r->r_info)))
		  break;

	      if (cgc_section_data (o)->relocs != sec_relocs)
		free (sec_relocs);
	  ...
	  /* Record this stub in an array of local symbol stubs for
             this BFD.  */
	  if (mips_cgc_tdata (abfd)->local_stubs == NULL)
	    {
	      unsigned long symcount;
	      asection **n;
	      bfd_size_type amt;

	      if (cgc_bad_symtab (abfd))
		...
		symcount = symtab_hdr->sh_info;
	      ...
	      if (n == NULL)
		return FALSE;
	      mips_cgc_tdata (abfd)->local_stubs = n;
	    }

	  sec->flags |= SEC_KEEP;
	  mips_cgc_tdata (abfd)->local_stubs[r_symndx] = sec;
      ...
      else
	{
	  ...
	  sec->flags |= SEC_KEEP;
	  h->fn_stub = sec;
	  mips_cgc_hash_table (info)->mips16_stubs_seen = TRUE;
	}
    }
	  ...
	  || sym_hashes[r_symndx - extsymoff] == NULL)
	{
	  ...
	  for (o = abfd->sections; o != NULL; o = o->next)
		...
		return FALSE;
	      ...
	      for (r = sec_relocs; r < rend; r++)
		if (CGC_R_SYM (abfd, r->r_info) == r_symndx
		    && CGC_R_TYPE (abfd, r->r_info) == R_MIPS16_26)
		    break;
	  ...
	  /* Record this stub in an array of local symbol call_stubs for
             this BFD.  */
	  if (mips_cgc_tdata (abfd)->local_call_stubs == NULL)
	    {
	      unsigned long symcount;
	      asection **n;
	      bfd_size_type amt;
		...
		symcount = symtab_hdr->sh_info;
	      ...
	      if (n == NULL)
		return FALSE;
	      mips_cgc_tdata (abfd)->local_call_stubs = n;
	    }

	  sec->flags |= SEC_KEEP;
	  mips_cgc_tdata (abfd)->local_call_stubs[r_symndx] = sec;
  ...
  for (rel = relocs; rel < rel_end; ++rel)
    {
      ...
      r_symndx = CGC_R_SYM (abfd, rel->r_info);
      r_type = CGC_R_TYPE (abfd, rel->r_info);
	...
	case R_MICROMIPS_TLS_LDM:
	  ...
	  if (!mips_cgc_create_got_section (dynobj, info))
	    return FALSE;
	...
	case R_MICROMIPS_PC23_S2:
	  if (h)
	    ((struct mips_cgc_link_hash_entry *) h)->has_static_relocs = TRUE;
	  break;
	}
	  ...
	  if (is_gott_symbol (info, h))
	    {
	      if (sreloc == NULL)
		{
		  sreloc = mips_cgc_rel_dyn_section (info, TRUE);
		  if (sreloc == NULL)
		    return FALSE;
		}
	      mips_cgc_allocate_dynamic_relocations (dynobj, info, 1);
	      if (MIPS_CGC_READONLY_SECTION (sec))
	       ...
	       || (got16_reloc_p (r_type) && htab->is_vxworks))
	{
	  ...
	  if (!mips_cgc_record_local_got_symbol (abfd, r_symndx,
						 rel->r_addend, info, r_type))
	    return FALSE;
	}

      if (h != NULL
	  && mips_cgc_relocation_needs_la25_stub (abfd, r_type,
						  CGC_ST_IS_MIPS16 (h->other)))
	((struct mips_cgc_link_hash_entry *) h)->has_nonpic_branches = TRUE;
	  ...
	  if (h != NULL)
	    {
		 ...
		 a .got.plt entry later; see mips_cgc_count_got_symbols.  */
	      if (!mips_cgc_record_global_got_symbol (h, abfd, info, TRUE,
						      r_type))
		return FALSE;

	      /* We need a stub, not a plt entry for the undefined
		 function.  But we record it as if it needs plt.  See
		 _bfd_cgc_adjust_dynamic_symbol.  */
	      h->needs_plt = 1;
	      h->type = STT_FUNC;
	    }
	  break;
	  ...
	  if (!h || got_page_reloc_p (r_type))
	    {
		 ...
		 preemptible.  */
	      if (mips_cgc_rel_relocation_p (abfd, sec, relocs, rel))
		{
		  if (!mips_cgc_get_section_contents (abfd, sec, &contents))
		    return FALSE;
		  howto = MIPS_CGC_RTYPE_TO_HOWTO (abfd, r_type, FALSE);
		  addend = mips_cgc_read_rel_addend (abfd, rel,
						     howto, contents);
		  if (got16_reloc_p (r_type))
		    mips_cgc_add_lo16_rel_addend (abfd, rel, rel_end,
						  contents, &addend);
		  else
		...
		addend = rel->r_addend;
	      if (!mips_cgc_record_got_page_ref (info, abfd, r_symndx,
						 h, addend))
		return FALSE;
	...
	case R_MIPS_GOT_DISP:
	case R_MICROMIPS_GOT_DISP:
	  if (h && !mips_cgc_record_global_got_symbol (h, abfd, info,
						       FALSE, r_type))
	    return FALSE;
	  break;
	...
	case R_MICROMIPS_TLS_GD:
	  ...
	  if (h != NULL)
	    {
	      if (!mips_cgc_record_global_got_symbol (h, abfd, info,
						      FALSE, r_type))
		return FALSE;
	    }
	  else
	...
	case R_MIPS_64:
	  ...
	  if (SGI_COMPAT (abfd))
	    mips_cgc_hash_table (info)->compact_rel_size +=
	      sizeof (Cgc32_External_crinfo);
	  break;
	...
	case R_MIPS_GNU_VTENTRY:
	  BFD_ASSERT (h != NULL);
	  if (h != NULL
	      && !bfd_cgc_gc_record_vtentry (abfd, sec, h, rel->r_offset))
	    return FALSE;
	  break;
	...
	{
	  if (h->plt.plist == NULL)
	    h->plt.plist = mips_cgc_make_plt_record (abfd);
	  if (h->plt.plist == NULL)
	    return FALSE;
	...
	switch (r_type)
	  {
	  default:
	    ((struct mips_cgc_link_hash_entry *) h)->no_fn_stub = TRUE;
	    break;
	  case R_MIPS16_CALL16:
	  case R_MIPS_CALL16:
	  case R_MIPS_CALL_HI16:
	...
	{
	  struct mips_cgc_link_hash_entry *mh;

	  mh = (struct mips_cgc_link_hash_entry *) h;
			 ...
			 bfd_boolean *again)
{
  ...
  Cgc_Internal_Rela *irel, *irelend;
  Cgc_Internal_Shdr *symtab_hdr;
  ...
  *again = FALSE;
  ...
  internal_relocs = _bfd_cgc_link_read_relocs (abfd, sec, NULL, NULL,
					       link_info->keep_memory);
  if (internal_relocs == NULL)
    return TRUE;

  irelend = internal_relocs + sec->reloc_count
    * get_cgc_backend_data (abfd)->s->int_rels_per_ext_rel;
  symtab_hdr = &cgc_tdata (abfd)->symtab_hdr;
  extsymoff = (cgc_bad_symtab (abfd)) ? 0 : symtab_hdr->sh_info;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
	 ...
	 This saves some pipeline bubbles.  */
      r_type = CGC_R_TYPE (abfd, irel->r_info);
      if (r_type != R_MIPS_JALR)
	continue;

      r_symndx = CGC_R_SYM (abfd, irel->r_info);
      /* Compute the address of the jump target.  */
      if (r_symndx >= extsymoff)
	{
	  struct mips_cgc_link_hash_entry *h
	    = ((struct mips_cgc_link_hash_entry *)
	       cgc_sym_hashes (abfd) [r_symndx - extsymoff]);
      ...
      else
	{
	  ...
	  if (isymbuf == NULL && symtab_hdr->sh_info != 0)
	    {
	      ...
	      if (isymbuf == NULL)
		isymbuf = bfd_cgc_get_cgc_syms (abfd, symtab_hdr,
	  ...
	  if (isym->st_shndx == SHN_UNDEF)
	    ...
	    sym_sec
	      = bfd_section_from_cgc_index (abfd, isym->st_shndx);
  ...
  if (contents != NULL
      && cgc_section_data (sec)->this_hdr.contents != contents)
    {
      if (!changed_contents && !link_info->keep_memory)
        free (contents);
      else
        {
          /* Cache the section contents for cgc_link_input_bfd.  */
          cgc_section_data (sec)->this_hdr.contents = contents;
        }
    }
  return TRUE;
...
static bfd_boolean
allocate_dynrelocs (struct cgc_link_hash_entry *h, void *inf)
{
  ...
  if (! info->relocatable
      ...
      && (h->root.type == bfd_link_hash_defweak
	  || (!h->def_regular && !CGC_COMMON_DEF_P (h))
	  || info->shared))
    {
      ...
      if (do_copy)
	{
	  ...
	  mips_cgc_allocate_dynamic_relocations
...
bfd_boolean
_bfd_mips_cgc_adjust_dynamic_symbol (struct bfd_link_info *info,
				     struct cgc_link_hash_entry *h)
{
  bfd *dynobj;
  struct mips_cgc_link_hash_entry *hmips;
  ...
  if (!htab->is_vxworks && h->needs_plt && !hmips->no_fn_stub)
    {
      if (! cgc_hash_table (info)->dynamic_sections_created)
	return TRUE;
  ...
  /* As above, VxWorks requires PLT entries for externally-defined
      ...
      if (htab->plt_mips_offset + htab->plt_comp_offset == 0)
	{
	  ...
	  if (!htab->is_vxworks)
	    htab->plt_got_index
	      += (get_cgc_backend_data (dynobj)->got_header_size
		  / MIPS_CGC_GOT_SIZE (dynobj));

	  /* On VxWorks, also allocate room for the header's
	     .rela.plt.unloaded entries.  */
	  if (htab->is_vxworks && !info->shared)
	    htab->srelplt2->size += 2 * sizeof (Cgc32_External_Rela);
      ...
      if (h->plt.plist == NULL)
	h->plt.plist = mips_cgc_make_plt_record (dynobj);
      if (h->plt.plist == NULL)
	return FALSE;
	...
	hmips->use_plt_entry = TRUE;
      ...
      htab->srelplt->size += (htab->is_vxworks
			      ? MIPS_CGC_RELA_SIZE (dynobj)
			      : MIPS_CGC_REL_SIZE (dynobj));

      /* Make room for the .rela.plt.unloaded relocations.  */
      if (htab->is_vxworks && !info->shared)
	htab->srelplt2->size += 3 * sizeof (Cgc32_External_Rela);
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0)
    {
      if (htab->is_vxworks)
	htab->srelbss->size += sizeof (Cgc32_External_Rela);
      else
	mips_cgc_allocate_dynamic_relocations (dynobj, info, 1);
      h->needs_copy = 1;
    }
  ...
  return _bfd_cgc_adjust_dynamic_copy (h, htab->sdynbss);
}

...
bfd_boolean
_bfd_mips_cgc_always_size_sections (bfd *output_bfd,
				    struct bfd_link_info *info)
{
  asection *ri;
  struct mips_cgc_link_hash_table *htab;
  ...
  if (ri != NULL)
    bfd_set_section_size (output_bfd, ri, sizeof (Cgc32_External_RegInfo));

  hti.info = info;
  hti.output_bfd = output_bfd;
  hti.error = FALSE;
  mips_cgc_link_hash_traverse (mips_cgc_hash_table (info),
			       mips_cgc_check_symbols, &hti);
  if (hti.error)
    return FALSE;
...
static bfd_boolean
mips_cgc_lay_out_got (bfd *output_bfd, struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *s;
  struct mips_got_info *g;
  ...
  mips_cgc_link_hash_traverse (htab, mips_cgc_count_got_symbols, info);

  if (!mips_cgc_resolve_final_got_entries (info, g))
    return FALSE;
  ...
  s->size += g->local_gotno * MIPS_CGC_GOT_SIZE (output_bfd);
  s->size += g->global_gotno * MIPS_CGC_GOT_SIZE (output_bfd);
  s->size += g->tls_gotno * MIPS_CGC_GOT_SIZE (output_bfd);

  /* VxWorks does not support multiple GOTs.  It initializes $gp to
     __GOTT_BASE__[__GOTT_INDEX__], the value of which is set by the
     dynamic loader.  */
  if (!htab->is_vxworks && s->size > MIPS_CGC_GOT_MAX_SIZE (info))
    {
      if (!mips_cgc_multi_got (output_bfd, info, s, page_gotno))
	return FALSE;
    }
  else
    {
	 ...
	 the per-bfd GOTs, which we no longer need.  */
      for (ibfd = info->input_bfds; ibfd; ibfd = ibfd->link_next)
	if (mips_cgc_bfd_got (ibfd, FALSE))
	  mips_cgc_replace_bfd_got (ibfd, g);
      mips_cgc_replace_bfd_got (output_bfd, g);

      /* Set up TLS entries.  */
      g->tls_assigned_gotno = g->global_gotno + g->local_gotno;
      tga.info = info;
      tga.g = g;
      tga.value = MIPS_CGC_GOT_SIZE (output_bfd);
      htab_traverse (g->got_entries, mips_cgc_initialize_tls_index, &tga);
      if (!tga.g)
	return FALSE;
	...
	g->relocs += g->global_gotno + g->local_gotno - htab->reserved_gotno;
      ...
      if (g->relocs)
	mips_cgc_allocate_dynamic_relocations (dynobj, info, g->relocs);
    }
...
static void
mips_cgc_estimate_stub_size (bfd *output_bfd, struct bfd_link_info *info)
{
  struct mips_cgc_link_hash_table *htab;
  ...
  /* Get a worst-case estimate of the number of dynamic symbols needed.
     ...
     be needed.  */
  dynsymcount = (cgc_hash_table (info)->dynsymcount
...
/* A mips_cgc_link_hash_traverse callback for which DATA points to a
   mips_htab_traverse_info.  If H needs a traditional MIPS lazy-binding
   stub, allocate an entry in the stubs section.  */

static bfd_boolean
mips_cgc_allocate_lazy_stub (struct mips_cgc_link_hash_entry *h, void *data)
{
  ...
  if (h->needs_lazy_stub)
	...
	h->root.plt.plist = mips_cgc_make_plt_record (htab->sstubs->owner);
      if (h->root.plt.plist == NULL)
	{
	  hti->error = TRUE;
	  return FALSE;
...
static bfd_boolean
mips_cgc_lay_out_lazy_stubs (struct bfd_link_info *info)
{
  ...
  hti.output_bfd = output_bfd;
  hti.error = FALSE;
  mips_cgc_link_hash_traverse (htab, mips_cgc_allocate_lazy_stub, &hti);
  if (hti.error)
    return FALSE;
  htab->sstubs->size += htab->function_stub_size;
  ...
  dynobj = cgc_hash_table (info)->dynobj;
  BFD_ASSERT (dynobj != NULL);
  h = _bfd_cgc_define_linkage_sym (dynobj, info, htab->sstubs, "_MIPS_STUBS_");
  if (h == NULL)
    return FALSE;
...
static bfd_boolean
mips_cgc_set_plt_sym_value (struct mips_cgc_link_hash_entry *h, void *data)
{
...
bfd_boolean
_bfd_mips_cgc_size_dynamic_sections (bfd *output_bfd,
				     struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *s, *sreldyn;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
      ...
      if (info->executable)
	{
	  ...
	  BFD_ASSERT (s != NULL);
	  s->size
	    = strlen (CGC_DYNAMIC_INTERPRETER (output_bfd)) + 1;
	  s->contents
	    = (bfd_byte *) CGC_DYNAMIC_INTERPRETER (output_bfd);
	}
      ...
      /* Figure out the size of the PLT header if we know that we
         ...
         haven't already in _bfd_cgc_create_dynamic_sections.  */
      if (htab->splt && htab->plt_mips_offset + htab->plt_comp_offset != 0)
	{
	  ...
	  mips_cgc_link_hash_traverse (htab, mips_cgc_set_plt_sym_value, info);

	  if (htab->root.hplt == NULL)
	    {
	      h = _bfd_cgc_define_linkage_sym (dynobj, info, htab->splt,
  ...
  /* Allocate space for global sym dynamic relocs.  */
  cgc_link_hash_traverse (&htab->root, allocate_dynrelocs, info);

  mips_cgc_estimate_stub_size (output_bfd, info);

  if (!mips_cgc_lay_out_got (output_bfd, info))
    return FALSE;

  mips_cgc_lay_out_lazy_stubs (info);
	  ...
	  if (s->size != 0)
	    {
		  ...
		  || strcmp (outname, MIPS_CGC_REL_DYN_NAME (info)) == 0)
		reltext = TRUE;

	      /* We use the reloc_count field as a counter if we need
		 to copy relocs into the output file.  */
	      if (strcmp (name, MIPS_CGC_REL_DYN_NAME (info)) != 0)
		s->reloc_count = 0;

	      /* If combreloc is enabled, cgc_link_sort_relocs() will
		 ...
		 broken, so don't let it run.  */
	      info->combreloc = 0;
	       ...
	       && ! mips_cgc_hash_table (info)->use_rld_obj_head
	       && CONST_STRNEQ (name, ".rld_map"))
	{
	  /* We add a room for __rld_map.  It will be filled in by the
	     rtld to contain a pointer to the _r_debug structure.  */
	  s->size += MIPS_CGC_RLD_MAP_SIZE (output_bfd);
	}
      else if (SGI_COMPAT (output_bfd)
	       && CONST_STRNEQ (name, ".compact_rel"))
	s->size += mips_cgc_hash_table (info)->compact_rel_size;
      else if (s == htab->splt)
	{
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
	 ...
	 values later, in _bfd_mips_cgc_finish_dynamic_sections, but we
	 ...
	 may only look at the first one they see.  */
      if (!info->shared
	  && !MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_RLD_MAP, 0))
	return FALSE;
	 ...
	 used by the debugger.  */
	  ...
	  && !MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_DEBUG, 0))
	return FALSE;
	...
	{
	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_TEXTREL, 0))
	    return FALSE;
      ...
      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_PLTGOT, 0))
	return FALSE;

      sreldyn = mips_cgc_rel_dyn_section (info, FALSE);
      if (htab->is_vxworks)
	{
	  ...
	  if (sreldyn && sreldyn->size > 0)
	    {
	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_RELA, 0))
		return FALSE;

	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_RELASZ, 0))
		return FALSE;

	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_RELAENT, 0))
		return FALSE;
	    }
	}
      else
	{
	  if (sreldyn && sreldyn->size > 0)
	    {
	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_REL, 0))
		return FALSE;

	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_RELSZ, 0))
		return FALSE;

	      if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_RELENT, 0))
		return FALSE;
	    }

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_RLD_VERSION, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_FLAGS, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_BASE_ADDRESS, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_LOCAL_GOTNO, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_SYMTABNO, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_UNREFEXTNO, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_GOTSYM, 0))
	    return FALSE;

	  if (IRIX_COMPAT (dynobj) == ict_irix5
	      && ! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_HIPAGENO, 0))
	    return FALSE;

	  if (IRIX_COMPAT (dynobj) == ict_irix6
	      && (bfd_get_section_by_name
		  (output_bfd, MIPS_CGC_OPTIONS_SECTION_NAME (dynobj)))
	      && !MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_OPTIONS, 0))
	    return FALSE;
	}
      if (htab->splt->size > 0)
	{
	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_PLTREL, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_JMPREL, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_PLTRELSZ, 0))
	    return FALSE;

	  if (! MIPS_CGC_ADD_DYNAMIC_ENTRY (info, DT_MIPS_PLTGOT, 0))
	    return FALSE;
	}
      if (htab->is_vxworks
	  && !cgc_vxworks_add_dynamic_entries (output_bfd, info))
	return FALSE;
    }
...
static void
mips_cgc_adjust_addend (bfd *output_bfd, struct bfd_link_info *info,
			bfd *input_bfd, Cgc_Internal_Sym *local_syms,
			asection **local_sections, Cgc_Internal_Rela *rel)
{
  unsigned int r_type, r_symndx;
  Cgc_Internal_Sym *sym;
  asection *sec;

  if (mips_cgc_local_relocation_p (input_bfd, rel, local_sections))
    {
      r_type = CGC_R_TYPE (output_bfd, rel->r_info);
	  ...
	  || literal_reloc_p (r_type))
	{
      ...
      r_symndx = CGC_R_SYM (output_bfd, rel->r_info);
      sym = local_syms + r_symndx;
	...
	{
	  sec = local_sections[r_symndx];
	  _bfd_cgc_rela_local_sym (output_bfd, sym, &sec, rel);
	}

      /* This would normally be done by the rela_normal code in cgclink.c.  */
      if (CGC_ST_TYPE (sym->st_info) == STT_SECTION)
...
/* Handle relocations against symbols from removed linkonce sections,
   ...
   on 64-bit CGC targets.  In this case for any relocation handled, which
...
mips_reloc_against_discarded_section (bfd *output_bfd,
				      ...
				      bfd_byte *contents)
{
  const struct cgc_backend_data *bed = get_cgc_backend_data (output_bfd);
  ...
  for (i = count - 1; i > 0; i--)
    {
      r_type = CGC_R_TYPE (output_bfd, (*rel)[i].r_info);
      if (r_type != R_MIPS_NONE)
	{
	  howto = MIPS_CGC_RTYPE_TO_HOWTO (input_bfd, r_type, !rel_reloc);
	  break;
	}
    }
  do
...
/* Relocate a MIPS CGC section.  */

bfd_boolean
_bfd_mips_cgc_relocate_section (bfd *output_bfd, struct bfd_link_info *info,
				bfd *input_bfd, asection *input_section,
				bfd_byte *contents, Cgc_Internal_Rela *relocs,
				Cgc_Internal_Sym *local_syms,
				asection **local_sections)
{
  Cgc_Internal_Rela *rel;
  ...
  for (rel = relocs; rel < relend; ++rel)
    {
      ...
      unsigned int r_type = CGC_R_TYPE (output_bfd, rel->r_info);
      const char *msg;
      unsigned long r_symndx;
      asection *sec;
      Cgc_Internal_Shdr *symtab_hdr;
      ...
      rel_reloc = (NEWABI_P (input_bfd)
		   && mips_cgc_rel_relocation_p (input_bfd, input_section,
						 relocs, rel));
      /* Find the relocation howto for this relocation.  */
      howto = MIPS_CGC_RTYPE_TO_HOWTO (input_bfd, r_type, !rel_reloc);

      r_symndx = CGC_R_SYM (input_bfd, rel->r_info);
      symtab_hdr = &cgc_tdata (input_bfd)->symtab_hdr;
      if (mips_cgc_local_relocation_p (input_bfd, rel, local_sections))
	{
	  sec = local_sections[r_symndx];
      ...
      else
	{
	  ...
	  if (!cgc_bad_symtab (input_bfd))
	    extsymoff = symtab_hdr->sh_info;
	  h = cgc_sym_hashes (input_bfd) [r_symndx - extsymoff];
	  while (h->root.type == bfd_link_hash_indirect
		 || h->root.type == bfd_link_hash_warning)
	    h = (struct cgc_link_hash_entry *) h->root.u.i.link;
      ...
      if (r_type == R_MIPS_64 && ! NEWABI_P (input_bfd))
	{
	  ...
	  howto = MIPS_CGC_RTYPE_TO_HOWTO (input_bfd, R_MIPS_32, FALSE);
      ...
      if (!use_saved_addend_p)
	{
	  /* If these relocations were originally of the REL variety,
	     ...
	     RELA relocation.  */
	  if (mips_cgc_rel_relocation_p (input_bfd, input_section,
					 relocs, rel))
	    {
	      rela_relocation_p = FALSE;
	      addend = mips_cgc_read_rel_addend (input_bfd, rel,
						 howto, contents);
	      if (hi16_reloc_p (r_type)
		  || (got16_reloc_p (r_type)
		      && mips_cgc_local_relocation_p (input_bfd, rel,
						      local_sections)))
		{
		  if (!mips_cgc_add_lo16_rel_addend (input_bfd, rel, relend,
						     contents, &addend))
		    {
		      if (h)
			name = h->root.root.string;
		      else
			name = bfd_cgc_sym_name (input_bfd, symtab_hdr,
						 local_syms + r_symndx,
						 sec);
		      (*_bfd_error_handler)
	    ...
	    addend = rel->r_addend;
	  mips_cgc_adjust_addend (output_bfd, info, input_bfd,
      ...
      if (info->relocatable)
	{
	  ...
	  if (!rela_relocation_p && rel->r_addend)
		...
		addend = mips_cgc_high (addend);
	      else if (r_type == R_MIPS_HIGHER)
		addend = mips_cgc_higher (addend);
	      else if (r_type == R_MIPS_HIGHEST)
		addend = mips_cgc_highest (addend);
	      else
		addend >>= howto->rightshift;
		   ...
		   but for endianness.  */
		{
	      ...
	      if (! mips_cgc_perform_relocation (info, howto, rel, addend,
	 ...
	 for the next.  */
	  ...
	  && CGC_R_TYPE (input_bfd, rel[1].r_info) != R_MIPS_NONE)
	use_saved_addend_p = TRUE;
      else
	use_saved_addend_p = FALSE;

      /* Figure out what value we are supposed to relocate.  */
      switch (mips_cgc_calculate_relocation (output_bfd, input_bfd,
	...
	case bfd_reloc_undefined:
	  /* mips_cgc_calculate_relocation already called the
	   ...
	   only a 32-bit VMA.  */
	{
      ...
      if (! mips_cgc_perform_relocation (info, howto, rel, value,
					 input_bfd, input_section,
...
static int
mips_cgc_create_la25_stub (void **slot, void *data)
{
  struct mips_htab_traverse_info *hti;
  struct mips_cgc_link_hash_table *htab;
  struct mips_cgc_la25_stub *stub;
  asection *s;
  bfd_byte *loc;
  ...
  stub = (struct mips_cgc_la25_stub *) *slot;
  hti = (struct mips_htab_traverse_info *) data;
  htab = mips_cgc_hash_table (hti->info);
  BFD_ASSERT (htab != NULL);
  ...
  target = mips_cgc_get_la25_target (stub, &s);
  ...
  if (stub->stub_section != htab->strampoline)
    {
	 ...
	 of the section and write the two instructions at the end.  */
      memset (loc, 0, offset);
      loc += offset;
      if (CGC_ST_IS_MICROMIPS (stub->h->root.other))
	{
...
static void
mips_cgc_irix6_finish_dynamic_symbol (bfd *abfd ATTRIBUTE_UNUSED,
				      const char *name, Cgc_Internal_Sym *sym)
{
  ...
  for (i = 0; i < 2; ++i)
      ...
      if (strcmp (*p, name) == 0)
	{
	  /* All of these symbols are given type STT_SECTION by the
	     IRIX6 linker.  */
	  sym->st_info = CGC_ST_INFO (STB_GLOBAL, STT_SECTION);
...
bfd_boolean
_bfd_mips_cgc_finish_dynamic_symbol (bfd *output_bfd,
				     struct bfd_link_info *info,
				     struct cgc_link_hash_entry *h,
				     Cgc_Internal_Sym *sym)
{
  ...
  if (h->plt.plist != NULL
	  ...
	  || h->plt.plist->comp_offset != MINUS_ONE))
    {
      ...
      got_address = (htab->sgotplt->output_section->vma
		     + htab->sgotplt->output_offset
		     + got_index * MIPS_CGC_GOT_SIZE (dynobj));

      got_address_high = ((got_address + 0x8000) >> 16) & 0xffff;
      got_address_low = got_address & 0xffff;

      /* Initially point the .got.plt entry at the PLT header.  */
      loc = (htab->sgotplt->contents + got_index * MIPS_CGC_GOT_SIZE (dynobj));
      if (ABI_64_P (output_bfd))
      ...
      if (h->plt.plist->mips_offset != MINUS_ONE)
	{
	  ...
	  load = MIPS_CGC_LOAD_WORD (output_bfd);
      ...
      if (h->plt.plist->comp_offset != MINUS_ONE)
	{
      ...
      mips_cgc_output_dynamic_relocation (output_bfd, htab->srelplt,
	...
	sym->st_other = CGC_ST_SET_MIPS_PLT (sym->st_other);
      else
	{
	  sym->st_value = 0;
	  sym->st_other = 0;
  ...
  if (h->dynindx != -1 && hmips->fn_stub != NULL)
    {
      ...
      sym->st_other = CGC_ST_VISIBILITY (sym->st_other);
    }
  ...
  if (hmips->global_got_area != GGA_NONE)
    {
      ...
      value = sym->st_value;
      offset = mips_cgc_primary_global_got_index (output_bfd, info, h);
      MIPS_CGC_PUT_WORD (output_bfd, value, sgot->contents + offset);
    }
	  ...
	  if (g->got_entries
		      ...
		      && !p->d.h->root.def_regular))
		{
		  /* Create an R_MIPS_REL32 relocation for this entry.  Due to
		     ...
		     mips_cgc_create_dynamic_relocation to calculate the
		     appropriate addend.  */
		  Cgc_Internal_Rela rel[3];

		  memset (rel, 0, sizeof (rel));
		  if (ABI_64_P (output_bfd))
		    rel[0].r_info = CGC_R_INFO (output_bfd, 0, R_MIPS_64);
		  else
		    rel[0].r_info = CGC_R_INFO (output_bfd, 0, R_MIPS_32);
		  rel[0].r_offset = rel[1].r_offset = rel[2].r_offset = offset;

		  entry = 0;
		  if (! (mips_cgc_create_dynamic_relocation
			 (output_bfd, info, rel,
		...
		entry = sym->st_value;
	      MIPS_CGC_PUT_WORD (output_bfd, entry, sgot->contents + offset);
	    }
	}
    }
  ...
  name = h->root.root.string;
  if (h == cgc_hash_table (info)->hdynamic
      || h == cgc_hash_table (info)->hgot)
    sym->st_shndx = SHN_ABS;
  ...
  else if (strcmp (name, "_gp_disp") == 0 && ! NEWABI_P (output_bfd))
    {
      sym->st_shndx = SHN_ABS;
      sym->st_info = CGC_ST_INFO (STB_GLOBAL, STT_SECTION);
      sym->st_value = cgc_gp (output_bfd);
    }
  else if (SGI_COMPAT (output_bfd))
    {
      if (strcmp (name, mips_cgc_dynsym_rtproc_names[0]) == 0
      ...
      else if (strcmp (name, mips_cgc_dynsym_rtproc_names[2]) == 0)
	{
	  sym->st_info = CGC_ST_INFO (STB_GLOBAL, STT_SECTION);
	  sym->st_other = STO_PROTECTED;
	  sym->st_value = mips_cgc_hash_table (info)->procedure_count;
	  sym->st_shndx = SHN_ABS;
	}
  ...
  if (h->plt.plist != NULL && h->plt.plist->mips_offset != MINUS_ONE)
    {
      ...
      got_address = (htab->sgotplt->output_section->vma
		     + htab->sgotplt->output_offset
		     + gotplt_index * MIPS_CGC_GOT_SIZE (output_bfd));

      /* Calculate the offset of the .got.plt entry from
	 _GLOBAL_OFFSET_TABLE_.  */
      got_offset = mips_cgc_gotplt_index (info, h);
      ...
      else
	{
	  ...
	  loc = (htab->srelplt2->contents
		 + (gotplt_index * 3 + 2) * sizeof (Cgc32_External_Rela));

	  /* Emit a relocation for the .got.plt entry.  */
	  rel.r_offset = got_address;
	  rel.r_info = CGC32_R_INFO (htab->root.hplt->indx, R_MIPS_32);
	  rel.r_addend = plt_offset;
	  bfd_cgc32_swap_reloca_out (output_bfd, &rel, loc);

	  /* Emit a relocation for the lui of %hi(<.got.plt slot>).  */
	  loc += sizeof (Cgc32_External_Rela);
	  rel.r_offset = plt_address + 8;
	  rel.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_MIPS_HI16);
	  ...
	  loc += sizeof (Cgc32_External_Rela);
	  rel.r_offset += 4;
	  rel.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_MIPS_LO16);
	  bfd_cgc32_swap_reloca_out (output_bfd, &rel, loc);
	}

      /* Emit an R_MIPS_JUMP_SLOT relocation against the .got.plt entry.  */
      loc = (htab->srelplt->contents
	     + gotplt_index * sizeof (Cgc32_External_Rela));
      rel.r_offset = got_address;
      rel.r_info = CGC32_R_INFO (h->dynindx, R_MIPS_JUMP_SLOT);
      rel.r_addend = 0;
      bfd_cgc32_swap_reloca_out (output_bfd, &rel, loc);
  ...
  if (hmips->global_got_area != GGA_NONE)
    {
      ...
      offset = mips_cgc_primary_global_got_index (output_bfd, info, h);
      MIPS_CGC_PUT_WORD (output_bfd, sym->st_value, sgot->contents + offset);
      ...
      outrel.r_offset = (sgot->output_section->vma
			 + sgot->output_offset
			 + offset);
      outrel.r_info = CGC32_R_INFO (h->dynindx, R_MIPS_32);
      outrel.r_addend = 0;
      bfd_cgc32_swap_reloca_out (dynobj, &outrel, loc);
    }
  ...
  if (h->needs_copy)
    {
      ...
      rel.r_offset = (h->root.u.def.section->output_section->vma
		      + h->root.u.def.section->output_offset
		      + h->root.u.def.value);
      rel.r_info = CGC32_R_INFO (h->dynindx, R_MIPS_COPY);
      rel.r_addend = 0;
      bfd_cgc32_swap_reloca_out (output_bfd, &rel,
				 htab->srelbss->contents
				 + (htab->srelbss->reloc_count
				    * sizeof (Cgc32_External_Rela)));
      ++htab->srelbss->reloc_count;
    }
...
mips_vxworks_finish_exec_plt (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  rela.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_MIPS_HI16);
  rela.r_addend = 0;
  bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
  loc += sizeof (Cgc32_External_Rela);

  /* Output the relocation for the following addiu of
     %lo(_GLOBAL_OFFSET_TABLE_).  */
  rela.r_offset += 4;
  rela.r_info = CGC32_R_INFO (htab->root.hgot->indx, R_MIPS_LO16);
  bfd_cgc32_swap_reloca_out (output_bfd, &rela, loc);
  loc += sizeof (Cgc32_External_Rela);
  ...
  while (loc < htab->srelplt2->contents + htab->srelplt2->size)
    {
      ...
      bfd_cgc32_swap_reloca_in (output_bfd, loc, &rel);
      rel.r_info = CGC32_R_INFO (htab->root.hplt->indx, R_MIPS_32);
      bfd_cgc32_swap_reloca_out (output_bfd, &rel, loc);
      loc += sizeof (Cgc32_External_Rela);
...
bfd_boolean
_bfd_mips_cgc_finish_dynamic_sections (bfd *output_bfd,
				       struct bfd_link_info *info)
{
  bfd *dynobj;
  asection *sdyn;
  ...
  if (cgc_hash_table (info)->dynamic_sections_created)
    {
      ...
      g = mips_cgc_bfd_got (output_bfd, FALSE);
      BFD_ASSERT (g != NULL);

      for (b = sdyn->contents;
	   b < sdyn->contents + sdyn->size;
	   b += MIPS_CGC_DYN_SIZE (dynobj))
	{
	  ...
	  (*get_cgc_backend_data (dynobj)->s->swap_dyn_in) (dynobj, b, &dyn);
	  ...
	  switch (dyn.d_tag)
	    {
	    case DT_RELENT:
	      dyn.d_un.d_val = MIPS_CGC_REL_SIZE (dynobj);
	      break;

	    case DT_RELAENT:
	      BFD_ASSERT (htab->is_vxworks);
	      dyn.d_un.d_val = MIPS_CGC_RELA_SIZE (dynobj);
	      break;

	    case DT_STRSZ:
	      /* Rewrite DT_STRSZ.  */
	      dyn.d_un.d_val =
		_bfd_cgc_strtab_size (cgc_hash_table (info)->dynstr);
	      break;
	    ...
	    case DT_MIPS_SYMTABNO:
	      name = ".dynsym";
	      elemsize = MIPS_CGC_SYM_SIZE (output_bfd);
	    ...
	    case DT_MIPS_RLD_MAP:
	      {
		struct cgc_link_hash_entry *h;
		h = mips_cgc_hash_table (info)->rld_symbol;
		if (!h)
		  {
		    dyn_to_skip = MIPS_CGC_DYN_SIZE (dynobj);
		    swap_out_p = FALSE;
		    break;
		  }
	    ...
	    case DT_MIPS_OPTIONS:
	      s = (bfd_get_section_by_name
		   (output_bfd, MIPS_CGC_OPTIONS_SECTION_NAME (output_bfd)));
	    ...
	    default:
	      swap_out_p = FALSE;
	      if (htab->is_vxworks
		  && cgc_vxworks_finish_dynamic_entry (output_bfd, &dyn))
		swap_out_p = TRUE;
	      break;
	    }

	  if (swap_out_p || dyn_skipped)
	    (*get_cgc_backend_data (dynobj)->s->swap_dyn_out)
  ...
  if (sgot != NULL && sgot->size > 0
      ...
      if (htab->is_vxworks)
	{
	  ...
	  MIPS_CGC_PUT_WORD (output_bfd,
			     sdyn->output_offset + sdyn->output_section->vma,
			     sgot->contents);
	  MIPS_CGC_PUT_WORD (output_bfd, 0,
			     sgot->contents + MIPS_CGC_GOT_SIZE (output_bfd));
	  MIPS_CGC_PUT_WORD (output_bfd, 0,
			     sgot->contents
			     + 2 * MIPS_CGC_GOT_SIZE (output_bfd));
	}
      else
	{
	  ...
	  MIPS_CGC_PUT_WORD (output_bfd, (bfd_vma) 0, sgot->contents);
	  MIPS_CGC_PUT_WORD (output_bfd, MIPS_CGC_GNU_GOT1_MASK (output_bfd),
			     sgot->contents + MIPS_CGC_GOT_SIZE (output_bfd));
	}

      cgc_section_data (sgot->output_section)->this_hdr.sh_entsize
	 = MIPS_CGC_GOT_SIZE (output_bfd);
    }
  ...
  if (gg != NULL && gg->next)
    {
      ...
      memset (rel, 0, sizeof (rel));
      rel[0].r_info = CGC_R_INFO (output_bfd, 0, R_MIPS_REL32);
      ...
      for (g = gg->next; g->next != gg; g = g->next)
	{
	  ...
	  MIPS_CGC_PUT_WORD (output_bfd, 0, sgot->contents
			     + got_index++ * MIPS_CGC_GOT_SIZE (output_bfd));
	  MIPS_CGC_PUT_WORD (output_bfd, MIPS_CGC_GNU_GOT1_MASK (output_bfd),
			     sgot->contents
			     + got_index++ * MIPS_CGC_GOT_SIZE (output_bfd));
	    ...
	    case DT_RELSZ:
		 ...
		 are trailing null entries.  */
	      ...
	      dyn.d_un.d_val = (s->reloc_count
				* (ABI_64_P (output_bfd)
				   ? sizeof (Cgc64_Mips_External_Rel)
				   : sizeof (Cgc32_External_Rel)));
	      /* Adjust the section size too.  Tools like the prelinker
		 can reasonably expect the values to the same.  */
	      cgc_section_data (s->output_section)->this_hdr.sh_size
		= dyn.d_un.d_val;
	      break;
...

  {
	...
	if (s != NULL)
	  {
	    ...
	    cpt.reserved0 = 0;
	    cpt.reserved1 = 0;
	    bfd_cgc32_swap_compact_rel_out (output_bfd, &cpt,
					    ((Cgc32_External_compact_rel *)
					     s->contents));
	...
	s = mips_cgc_rel_dyn_section (info, FALSE);
	if (s != NULL
	    && s->size > (bfd_vma)2 * MIPS_CGC_REL_SIZE (output_bfd))
	  {
	    reldyn_sorting_bfd = output_bfd;

	    if (ABI_64_P (output_bfd))
	      qsort ((Cgc64_External_Rel *) s->contents + 1,
		     s->reloc_count - 1, sizeof (Cgc64_Mips_External_Rel),
		     sort_dynamic_relocs_64);
	    else
	      qsort ((Cgc32_External_Rel *) s->contents + 1,
		     s->reloc_count - 1, sizeof (Cgc32_External_Rel),
		     sort_dynamic_relocs);
	  }
      }
  }
...
mips_set_isa_flags (bfd *abfd)
{
  ...
  cgc_cgcheader (abfd)->e_flags &= ~(EF_MIPS_ARCH | EF_MIPS_MACH);
  cgc_cgcheader (abfd)->e_flags |= val;

}


/* The final processing done just before writing out a MIPS CGC object
   file.  This gets the MIPS architecture right based on the machine
   number.  This is used by both the 32-bit and the 64-bit ABI.  */

void
_bfd_mips_cgc_final_write_processing (bfd *abfd,
				      bfd_boolean linker ATTRIBUTE_UNUSED)
{
  unsigned int i;
  Cgc_Internal_Shdr **hdrpp;
  const char *name;
  asection *sec;
  ...
  if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_MACH) == 0)
    mips_set_isa_flags (abfd);

  /* Set the sh_info field for .gptab sections and other appropriate
     info for each special section.  */
  for (i = 1, hdrpp = cgc_cgcsections (abfd) + 1;
       i < cgc_numsections (abfd);
       i++, hdrpp++)
    {
	...
	case SHT_MIPS_LIBLIST:
	  ...
	  if (sec != NULL)
	    (*hdrpp)->sh_link = cgc_section_data (sec)->this_idx;
	  break;
	...
	case SHT_MIPS_GPTAB:
	  ...
	  BFD_ASSERT (sec != NULL);
	  (*hdrpp)->sh_info = cgc_section_data (sec)->this_idx;
	  break;
...
int
_bfd_mips_cgc_additional_program_headers (bfd *abfd,
					  struct bfd_link_info *info ATTRIBUTE_UNUSED)
{
  asection *s;
  int ret = 0;
  ...
  if (IRIX_COMPAT (abfd) == ict_irix6
      && bfd_get_section_by_name (abfd,
				  MIPS_CGC_OPTIONS_SECTION_NAME (abfd)))
    ++ret;
  ...
  /* Allocate a PT_NULL header in dynamic objects.  See
     _bfd_mips_cgc_modify_segment_map for details.  */
  if (!SGI_COMPAT (abfd)
      && bfd_get_section_by_name (abfd, ".dynamic"))
    ++ret;
...
bfd_boolean
_bfd_mips_cgc_modify_segment_map (bfd *abfd,
				  struct bfd_link_info *info)
{
  asection *s;
  struct cgc_segment_map *m, **pm;
  bfd_size_type amt;
  ...
  if (s != NULL && (s->flags & SEC_LOAD) != 0)
    {
      for (m = cgc_seg_map (abfd); m != NULL; m = m->next)
  ...
  if (NEWABI_P (abfd)
	...
	if (cgc_section_data (s)->this_hdr.sh_type == SHT_MIPS_OPTIONS)
	  break;

      if (s)
	{
	  struct cgc_segment_map *options_segment;
	  ...
	  if (*pm == NULL || (*pm)->p_type != PT_MIPS_OPTIONS)
	    {
	      amt = sizeof (struct cgc_segment_map);
      ...
      if (IRIX_COMPAT (abfd) == ict_irix5)
	{
	 ...
	 between.  */
      for (pm = &cgc_seg_map (abfd); *pm != NULL;
	   pm = &(*pm)->next)
      ...
      if (m != NULL && IRIX_COMPAT (abfd) == ict_none)
	{
	  /* For a normal mips executable the permissions for the PT_DYNAMIC
	     ...
	     the code in cgc.c sets only the read permission. This matters
  ...
  /* Allocate a spare program header in dynamic objects so that tools
     ...
     start within sizeof (CgcNN_Phdr) bytes of the last program header.
  ...
  if (info != NULL
      ...
      && bfd_get_section_by_name (abfd, ".dynamic"))
    {
      for (pm = &cgc_seg_map (abfd); *pm != NULL; pm = &(*pm)->next)
...
asection *
_bfd_mips_cgc_gc_mark_hook (asection *sec,
			    struct bfd_link_info *info,
			    Cgc_Internal_Rela *rel,
			    struct cgc_link_hash_entry *h,
			    Cgc_Internal_Sym *sym)
{
  ...
  if (h != NULL)
    switch (CGC_R_TYPE (sec->owner, rel->r_info))
      {
      case R_MIPS_GNU_VTINHERIT:
      case R_MIPS_GNU_VTENTRY:
	return NULL;
...
bfd_boolean
_bfd_mips_cgc_gc_sweep_hook (bfd *abfd ATTRIBUTE_UNUSED,
			     struct bfd_link_info *info ATTRIBUTE_UNUSED,
			     asection *sec ATTRIBUTE_UNUSED,
			     const Cgc_Internal_Rela *relocs ATTRIBUTE_UNUSED)
{
  ...
  for (rel = relocs; rel < relend; rel++)
    switch (CGC_R_TYPE (abfd, rel->r_info))
      {
      case R_MIPS16_GOT16:
      case R_MIPS16_CALL16:
      case R_MIPS_GOT16:
...

/* Copy data from a MIPS CGC indirect symbol to its direct symbol,
   hiding the old indirect symbol.  Process additional relocation
   information.  Also called for weakdefs, in which case we just let
   _bfd_cgc_link_hash_copy_indirect copy the flags for us.  */

void
_bfd_mips_cgc_copy_indirect_symbol (struct bfd_link_info *info,
				    struct cgc_link_hash_entry *dir,
				    struct cgc_link_hash_entry *ind)
{
  struct mips_cgc_link_hash_entry *dirmips, *indmips;

  _bfd_cgc_link_hash_copy_indirect (info, dir, ind);

  dirmips = (struct mips_cgc_link_hash_entry *) dir;
  indmips = (struct mips_cgc_link_hash_entry *) ind;
...
bfd_boolean
_bfd_mips_cgc_discard_info (bfd *abfd, struct cgc_reloc_cookie *cookie,
			    struct bfd_link_info *info)
{
  asection *o;
  ...
  cookie->rels = _bfd_cgc_link_read_relocs (abfd, o, NULL, NULL,
					    info->keep_memory);
  if (!cookie->rels)
    {
      free (tdata);
  ...
  for (i = 0, skip = 0; i < o->size / PDR_SIZE; i ++)
    {
      if (bfd_cgc_reloc_symbol_deleted_p (i * PDR_SIZE, cookie))
	{
	  tdata[i] = 1;
	  skip ++;
	}
  ...
  if (skip != 0)
    {
      mips_cgc_section_data (o)->u.tdata = tdata;
...
bfd_boolean
_bfd_mips_cgc_ignore_discarded_relocs (asection *sec)
{
...
bfd_boolean
_bfd_mips_cgc_write_section (bfd *output_bfd,
                             ...
                             asection *sec, bfd_byte *contents)
{
  ...
  if (mips_cgc_section_data (sec)->u.tdata == NULL)
    return FALSE;
  ...
  for (from = contents, i = 0;
       ...
       from += PDR_SIZE, i++)
    {
      if ((mips_cgc_section_data (sec)->u.tdata)[i] == 1)
	continue;
      if (to != from)
...
bfd_boolean
_bfd_mips_cgc_is_target_special_symbol (bfd *abfd, asymbol *sym)
{
  return _bfd_cgc_is_local_label_name (abfd, sym->name);
}

/* MIPS CGC uses a special find_nearest_line routine in order the
   handle the ECOFF debugging information.  */

struct mips_cgc_find_line
{
...
bfd_boolean
_bfd_mips_cgc_find_nearest_line (bfd *abfd, asection *section,
				 ...
				 unsigned int *line_ptr)
{
  ...
  if (_bfd_dwarf2_find_nearest_line (abfd, dwarf_debug_sections,
				     ...
				     &cgc_tdata (abfd)->dwarf2_find_line_info))
    return TRUE;
  ...
  if (msec != NULL)
    {
      flagword origflags;
      struct mips_cgc_find_line *fi;
      const struct ecoff_debug_swap * const swap =
	get_cgc_backend_data (abfd)->cgc_backend_ecoff_debug_swap;

      /* If we are called during a link, mips_cgc_final_link may have
	 cleared the SEC_HAS_CONTENTS field.  We force it back on here
	 if appropriate (which it normally will be).  */
      origflags = msec->flags;
      if (cgc_section_data (msec)->this_hdr.sh_type != SHT_NOBITS)
	msec->flags |= SEC_HAS_CONTENTS;

      fi = mips_cgc_tdata (abfd)->find_line_info;
      if (fi == NULL)
	{
	  ...
	  char *fraw_end;
	  struct fdr *fdr_ptr;
	  bfd_size_type amt = sizeof (struct mips_cgc_find_line);
	  ...
	  if (! _bfd_mips_cgc_read_ecoff_info (abfd, msec, &fi->d))
	    {
	      msec->flags = origflags;
	      return FALSE;
	    }
	  ...
	  mips_cgc_tdata (abfd)->find_line_info = fi;
  ...
  /* Fall back on the generic CGC find_nearest_line routine.  */

  return _bfd_cgc_find_nearest_line (abfd, section, symbols, offset,
				     filename_ptr, functionname_ptr,
				     line_ptr);
}

bfd_boolean
_bfd_mips_cgc_find_inliner_info (bfd *abfd,
				 const char **filename_ptr,
				 const char **functionname_ptr,
				 unsigned int *line_ptr)
{
...
bfd_boolean
_bfd_mips_cgc_set_section_contents (bfd *abfd, sec_ptr section,
				    const void *location,
				    file_ptr offset, bfd_size_type count)
{
  if (MIPS_CGC_OPTIONS_SECTION_NAME_P (section->name))
    {
      bfd_byte *c;

      if (cgc_section_data (section) == NULL)
	{
	  bfd_size_type amt = sizeof (struct bfd_cgc_section_data);
	  section->used_by_bfd = bfd_zalloc (abfd, amt);
	  if (cgc_section_data (section) == NULL)
	    return FALSE;
	}
      c = mips_cgc_section_data (section)->u.tdata;
      if (c == NULL)
	{
	  c = bfd_zalloc (abfd, section->size);
	  if (c == NULL)
	    return FALSE;
	  mips_cgc_section_data (section)->u.tdata = c;
	}

      memcpy (c + offset, location, count);
    }

  return _bfd_cgc_set_section_contents (abfd, section, location, offset,
					count);
}
...
bfd_byte *
_bfd_cgc_mips_get_relocated_section_contents
  (bfd *abfd,
   struct bfd_link_info *link_info,
  ...
  if (reloc_count > 0)
      ...
      for (parent = reloc_vector; *parent != NULL; parent++)
	{
	  ...
	  if (gp_found
	      ...
	      == _bfd_mips_cgc32_gprel16_reloc)
	    r = _bfd_mips_cgc_gprel16_with_gp (input_bfd, sym, *parent,
...

static bfd_boolean
mips_cgc_relax_delete_bytes (bfd *abfd,
			     asection *sec, bfd_vma addr, int count)
{
  ...
  Cgc_Internal_Rela *irel, *irelend;
  Cgc_Internal_Sym *isym;
  Cgc_Internal_Sym *isymend;
  struct cgc_link_hash_entry **sym_hashes;
  struct cgc_link_hash_entry **end_hashes;
  struct cgc_link_hash_entry **start_hashes;
  unsigned int symcount;

  sec_shndx = _bfd_cgc_section_from_bfd_section (abfd, sec);
  contents = cgc_section_data (sec)->this_hdr.contents;

  irel = cgc_section_data (sec)->relocs;
  ...
  for (irel = cgc_section_data (sec)->relocs; irel < irelend; irel++)
    {
  ...
  symtab_hdr = &cgc_tdata (abfd)->symtab_hdr;
  isym = (Cgc_Internal_Sym *) symtab_hdr->contents;
  for (isymend = isym + symtab_hdr->sh_info; isym < isymend; isym++)
    if (isym->st_shndx == sec_shndx && isym->st_value > addr)
      isym->st_value -= count;

  /* Now adjust the global symbols defined in this section.  */
  symcount = (symtab_hdr->sh_size / sizeof (Cgc32_External_Sym)
	      - symtab_hdr->sh_info);
  sym_hashes = start_hashes = cgc_sym_hashes (abfd);
  end_hashes = sym_hashes + symcount;

  for (; sym_hashes < end_hashes; sym_hashes++)
    {
      struct cgc_link_hash_entry *sym_hash = *sym_hashes;
	  ...
	  && sym_hash->root.u.def.section == sec)
	{
	  ...
	  if (CGC_ST_IS_MICROMIPS (sym_hash->other))
	    value &= MINUS_TWO;
	  if (value > addr)
...
check_relocated_bzc (bfd *abfd, const bfd_byte *ptr, bfd_vma offset,
		     const Cgc_Internal_Rela *internal_relocs,
		     const Cgc_Internal_Rela *irelend)
{
  const Cgc_Internal_Rela *irel;
  unsigned long opcode;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	&& CGC32_R_TYPE (irel->r_info) == R_MICROMIPS_PC16_S1)
      return TRUE;
...

bfd_boolean
_bfd_mips_cgc_relax_section (bfd *abfd, asection *sec,
			     struct bfd_link_info *link_info,
			     bfd_boolean *again)
{
  bfd_boolean insn32 = mips_cgc_hash_table (link_info)->insn32;
  Cgc_Internal_Shdr *symtab_hdr;
  Cgc_Internal_Rela *internal_relocs;
  Cgc_Internal_Rela *irel, *irelend;
  bfd_byte *contents = NULL;
  ...
  *again = FALSE;
  ...
  internal_relocs = (_bfd_cgc_link_read_relocs
		     (abfd, sec, NULL, (Cgc_Internal_Rela *) NULL,
		      link_info->keep_memory));
  if (internal_relocs == NULL)
    goto error_return;
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
    {
      unsigned long r_symndx = CGC32_R_SYM (irel->r_info);
      unsigned int r_type = CGC32_R_TYPE (irel->r_info);
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (isym->st_shndx == SHN_UNDEF)
	    ...
	    sym_sec = bfd_section_from_cgc_index (abfd, isym->st_shndx);
	  symval = (isym->st_value
		    + sym_sec->output_section->vma
		    + sym_sec->output_offset);
	  target_is_micromips_code_p = CGC_ST_IS_MICROMIPS (isym->st_other);
	}
      else
	{
	  unsigned long indx;
      ...
      if (r_type == R_MICROMIPS_HI16 && MATCH (opcode, lui_insn))
	{
	  ...
	  if (irel > internal_relocs
	      && CGC32_R_TYPE (irel[-1].r_info) == R_MICROMIPS_HI16
	      && CGC32_R_SYM (irel[-1].r_info) == r_symndx)
	    continue;

	  /* Or if the next reloc is not a LO16 against this symbol.  */
	  if (irel + 1 >= irelend
	      || CGC32_R_TYPE (irel[1].r_info) != R_MICROMIPS_LO16
	      || CGC32_R_SYM (irel[1].r_info) != r_symndx)
	    continue;

	  /* Or if the second next reloc is a LO16 against this symbol too.  */
	  if (irel + 2 >= irelend
	      && CGC32_R_TYPE (irel[2].r_info) == R_MICROMIPS_LO16
	      && CGC32_R_SYM (irel[2].r_info) == r_symndx)
	    continue;
	  ...
	  if (IS_BITSIZE (symval, 16))
	    {
	      ...
	      irel[1].r_info = CGC32_R_INFO (r_symndx, R_MICROMIPS_HI0_LO16);
	  ...
	  /* R_MICROMIPS_LO16 / ADDIU relaxation to R_MICROMIPS_PC23_S2.
	    ...
	    {
	      /* Fix the relocation's type.  */
	      irel[1].r_info = CGC32_R_INFO (r_symndx, R_MICROMIPS_PC23_S2);
	  ...
	  /* Fix the relocation's type.  */
	  irel->r_info = CGC32_R_INFO (r_symndx, R_MIPS_NONE);
	...
	{
	  /* Fix the relocation's type.  */
	  irel->r_info = CGC32_R_INFO (r_symndx, R_MICROMIPS_PC10_S1);
		       ...
		       && OP16_VALID_REG (OP32_TREG (opcode)))))
	{
	  ...
	  /* Fix the relocation's type.  */
	  irel->r_info = CGC32_R_INFO (r_symndx, R_MICROMIPS_PC7_S1);
      ...
      if (delcnt != 0)
	{
	  ...
	  cgc_section_data (sec)->relocs = internal_relocs;
	  cgc_section_data (sec)->this_hdr.contents = contents;
	  symtab_hdr->contents = (unsigned char *) isymbuf;

	  /* Delete bytes depending on the delcnt and deloff.  */
	  if (!mips_cgc_relax_delete_bytes (abfd, sec,
...

/* Create a MIPS CGC linker hash table.  */

struct bfd_link_hash_table *
_bfd_mips_cgc_link_hash_table_create (bfd *abfd)
{
  struct mips_cgc_link_hash_table *ret;
  bfd_size_type amt = sizeof (struct mips_cgc_link_hash_table);
  ...
  if (!_bfd_cgc_link_hash_table_init (&ret->root, abfd,
				      mips_cgc_link_hash_newfunc,
				      sizeof (struct mips_cgc_link_hash_entry),
				      MIPS_CGC_DATA))
    {
      free (ret);
      return NULL;
...
_bfd_mips_vxworks_link_hash_table_create (bfd *abfd)
{
  ...
  ret = _bfd_mips_cgc_link_hash_table_create (abfd);
  if (ret)
    {
      struct mips_cgc_link_hash_table *htab;

      htab = (struct mips_cgc_link_hash_table *) ret;
...
void
_bfd_mips_cgc_use_plts_and_copy_relocs (struct bfd_link_info *info)
{
  mips_cgc_hash_table (info)->use_plts_and_copy_relocs = TRUE;
}
...
void
_bfd_mips_cgc_insn32 (struct bfd_link_info *info, bfd_boolean on)
{
  mips_cgc_hash_table (info)->insn32 = on;
}

...
bfd_boolean
_bfd_mips_cgc_final_link (bfd *abfd, struct bfd_link_info *info)
{
  asection *o;
  struct bfd_link_order *p;
  asection *reginfo_sec, *mdebug_sec, *gptab_data_sec, *gptab_bss_sec;
  asection *rtproc_sec;
  Cgc32_RegInfo reginfo;
  struct ecoff_debug_info debug;
  struct mips_htab_traverse_info hti;
  const struct cgc_backend_data *bed = get_cgc_backend_data (abfd);
  const struct ecoff_debug_swap *swap = bed->cgc_backend_ecoff_debug_swap;
  ...
  if (!mips_cgc_sort_hash_table (abfd, info))
    return FALSE;
  ...
  hti.output_bfd = abfd;
  hti.error = FALSE;
  htab_traverse (htab->la25_stubs, mips_cgc_create_la25_stub, &hti);
  if (hti.error)
    return FALSE;
  ...
  if (cgc_gp (abfd) == 0)
	...
	cgc_gp (abfd) = (h->u.def.section->output_section->vma
			 + h->u.def.section->output_offset
			 + h->u.def.value);
      else if (info->relocatable)
	{
	  ...
	  for (o = abfd->sections; o != NULL; o = o->next)
	    if (o->vma < lo
		&& (cgc_section_data (o)->this_hdr.sh_flags & SHF_MIPS_GPREL))
	      lo = o->vma;

	  /* And calculate GP relative to that.  */
	  cgc_gp (abfd) = lo + CGC_MIPS_GP_OFFSET (info);
	}
      else
	{
  ...
  for (o = abfd->sections; o != NULL; o = o->next)
      ...
      if (strcmp (o->name, ".reginfo") == 0)
	{
	  ...
	  for (p = o->map_head.link_order; p != NULL; p = p->next)
		...
		return FALSE;
	      ...
	      bfd_mips_cgc32_swap_reginfo_in (input_bfd, &ext, &sub);
		 ...
		 mips_cgc32_section_processing when the section is
		 finally written out.  */

	      /* Hack: reset the SEC_HAS_CONTENTS flag so that
		 cgc_link_input_bfd ignores this section.  */
	      input_section->flags &= ~SEC_HAS_CONTENTS;
	    }

	  /* Size has been set in _bfd_mips_cgc_always_size_sections.  */
	  BFD_ASSERT(o->size == sizeof (Cgc32_External_RegInfo));
      ...
      if (strcmp (o->name, ".mdebug") == 0)
	{
	  ...
	  for (p = o->map_head.link_order; p != NULL; p = p->next)
	      ...
	      if (p->type != bfd_indirect_link_order)
		{
	      ...
	      if (!is_mips_cgc (input_bfd))
		{
		  /* I don't know what a non MIPS CGC bfd would be
		     doing with a .mdebug section, but I don't really
		     want to deal with it.  */
		  continue;
		}

	      input_swap = (get_cgc_backend_data (input_bfd)
		 ...
		 ecoff_debug_info structure, so we do that now.  */
	      if (! _bfd_mips_cgc_read_ecoff_info (input_bfd, input_section,
						   &input_debug))
		return FALSE;
		   ...
		   eraw_src += input_swap->external_ext_size)
		{
		  ...
		  h = mips_cgc_link_hash_lookup (mips_cgc_hash_table (info),
	  ...
	  if (SGI_COMPAT (abfd) && info->shared)
	      ...
	      if (rtproc_sec == NULL)
		{
	      ...
	      if (! mips_cgc_create_procedure_table (mdebug_handle, abfd,
						     info, rtproc_sec,
						     &debug))
		return FALSE;
	    }
      ...
      if (CONST_STRNEQ (o->name, ".gptab."))
	{
	  ...
	  c = 1;
	  amt = c * sizeof (Cgc32_gptab);
	  tab = bfd_malloc (amt);
	  if (tab == NULL)
	    return FALSE;
	  tab[0].gt_header.gt_current_g_value = cgc_gp_size (abfd);
	  ...
	  for (p = o->map_head.link_order; p != NULL; p = p->next)
	      ...
	      if (p->type != bfd_indirect_link_order)
		{
		 ...
		 sorted by ascending -G value.  */
	      ...
	      last = 0;
	      for (gpentry = sizeof (Cgc32_External_gptab);
		   gpentry < size;
		   gpentry += sizeof (Cgc32_External_gptab))
		{
		  Cgc32_External_gptab ext_gptab;
		  Cgc32_gptab int_gptab;
		  unsigned long val;
		  unsigned long add;
		  bfd_boolean exact;
		  unsigned int look;
		  ...
		  if (! (bfd_get_section_contents
			 (input_bfd, input_section, &ext_gptab, gpentry,
			  sizeof (Cgc32_External_gptab))))
		    {
		      free (tab);
		      return FALSE;
		    }

		  bfd_mips_cgc32_swap_gptab_in (input_bfd, &ext_gptab,
						&int_gptab);
		  ...
		  if (! exact)
		    {
		      ...
		      amt = (bfd_size_type) (c + 1) * sizeof (Cgc32_gptab);
	  ...
	  /* Swap out the table.  */
	  amt = (bfd_size_type) c * sizeof (Cgc32_External_gptab);
	  ...
	  for (j = 0; j < c; j++)
	    bfd_mips_cgc32_swap_gptab_out (abfd, tab + j, ext_tab + j);
	  free (tab);

	  o->size = c * sizeof (Cgc32_External_gptab);
  ...
  /* Invoke the regular CGC backend linker to do all the work.  */
  if (!bfd_cgc_final_link (abfd, info))
    return FALSE;
  ...
  if (reginfo_sec != NULL)
    {
      ...
      bfd_mips_cgc32_swap_reginfo_out (abfd, &reginfo, &ext);
...
/* Return true if the given CGC header flags describe a 32-bit binary.  */
...
static bfd_boolean
mips_cgc_merge_obj_attributes (bfd *ibfd, bfd *obfd)
{
  obj_attribute *in_attr;
  obj_attribute *out_attr;
  bfd *abi_fp_bfd;

  abi_fp_bfd = mips_cgc_tdata (obfd)->abi_fp_bfd;
  in_attr = cgc_known_obj_attributes (ibfd)[OBJ_ATTR_GNU];
  if (!abi_fp_bfd && in_attr[Tag_GNU_MIPS_ABI_FP].i != Val_GNU_MIPS_ABI_FP_ANY)
    mips_cgc_tdata (obfd)->abi_fp_bfd = ibfd;

  if (!cgc_known_obj_attributes_proc (obfd)[0].i)
    {
      /* This is the first object.  Copy the attributes.  */
      _bfd_cgc_copy_obj_attributes (ibfd, obfd);

      /* Use the Tag_null value to indicate the attributes have been
	 initialized.  */
      cgc_known_obj_attributes_proc (obfd)[0].i = 1;
  ...
  out_attr = cgc_known_obj_attributes (obfd)[OBJ_ATTR_GNU];
  ...
  _bfd_cgc_merge_object_attributes (ibfd, obfd);
...
bfd_boolean
_bfd_mips_cgc_merge_private_bfd_data (bfd *ibfd, bfd *obfd)
{
  flagword old_flags;
  flagword new_flags;
  bfd_boolean ok;
  ...
  if (!is_mips_cgc (ibfd) || !is_mips_cgc (obfd))
    return TRUE;
  ...
  if (!mips_cgc_merge_obj_attributes (ibfd, obfd))
    return FALSE;

  new_flags = cgc_cgcheader (ibfd)->e_flags;
  cgc_cgcheader (obfd)->e_flags |= new_flags & EF_MIPS_NOREORDER;
  ...
  if (! cgc_flags_init (obfd))
    {
      ...
      cgc_cgcheader (obfd)->e_ident[EI_CLASS]
	= cgc_cgcheader (ibfd)->e_ident[EI_CLASS];
  ...
  if (new_flags & (EF_MIPS_PIC | EF_MIPS_CPIC))
    cgc_cgcheader (obfd)->e_flags |= EF_MIPS_CPIC;
  if (! (new_flags & EF_MIPS_PIC))
    cgc_cgcheader (obfd)->e_flags &= ~EF_MIPS_PIC;
  ...
  if (mips_32bit_flags_p (old_flags) != mips_32bit_flags_p (new_flags))
      ...
      if (mips_mach_extends_p (bfd_get_mach (obfd), bfd_get_mach (ibfd)))
	{
	  ...
	  cgc_cgcheader (obfd)->e_flags &= ~(EF_MIPS_ARCH | EF_MIPS_MACH);
	  cgc_cgcheader (obfd)->e_flags
	  ...
	  if ((old_flags & EF_MIPS_ABI) == 0
	    ...
	    cgc_cgcheader (obfd)->e_flags |= new_flags & EF_MIPS_ABI;
	}
      else
	{
	      ...
	      != cgc_cgcheader (obfd)->e_ident[EI_CLASS]))
	{
	  (*_bfd_error_handler)
	    (_("%B: ABI mismatch: linking %s module with previous %s modules"),
	     ibfd,
	     cgc_mips_abi_name (ibfd),
	     cgc_mips_abi_name (obfd));
	  ok = FALSE;
	}
  ...
  if ((new_flags & EF_MIPS_ARCH_ASE) != (old_flags & EF_MIPS_ARCH_ASE))
      ...
      if (m16_mis || micro_mis)
	{
      ...
      cgc_cgcheader (obfd)->e_flags |= new_flags & EF_MIPS_ARCH_ASE;
...
bfd_boolean
_bfd_mips_cgc_set_private_flags (bfd *abfd, flagword flags)
{
...
char *
_bfd_mips_cgc_get_target_dtag (bfd_vma dtag)
{
  switch (dtag)
    {
    default: return "";
...
bfd_boolean
_bfd_mips_cgc_print_private_bfd_data (bfd *abfd, void *ptr)
{
  FILE *file = ptr;

  BFD_ASSERT (abfd != NULL && ptr != NULL);

  /* Print normal CGC private data.  */
  _bfd_cgc_print_private_bfd_data (abfd, ptr);

  /* xgettext:c-format */
  fprintf (file, _("private flags = %lx:"), cgc_cgcheader (abfd)->e_flags);

  if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI) == E_MIPS_ABI_O32)
    fprintf (file, _(" [abi=O32]"));
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI) == E_MIPS_ABI_O64)
    fprintf (file, _(" [abi=O64]"));
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI) == E_MIPS_ABI_EABI32)
    fprintf (file, _(" [abi=EABI32]"));
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI) == E_MIPS_ABI_EABI64)
    fprintf (file, _(" [abi=EABI64]"));
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ABI))
  ...
  if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_1)
    fprintf (file, " [mips1]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_2)
    fprintf (file, " [mips2]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_3)
    fprintf (file, " [mips3]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_4)
    fprintf (file, " [mips4]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_5)
    fprintf (file, " [mips5]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_32)
    fprintf (file, " [mips32]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_64)
    fprintf (file, " [mips64]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_32R2)
    fprintf (file, " [mips32r2]");
  else if ((cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH) == E_MIPS_ARCH_64R2)
    fprintf (file, " [mips64r2]");
  else
    fprintf (file, _(" [unknown ISA]"));

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH_ASE_MDMX)
    fprintf (file, " [mdmx]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH_ASE_M16)
    fprintf (file, " [mips16]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_ARCH_ASE_MICROMIPS)
    fprintf (file, " [micromips]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_NAN2008)
    fprintf (file, " [nan2008]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_FP64)
    fprintf (file, " [fp64]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_32BITMODE)
    fprintf (file, " [32bitmode]");
  else
    fprintf (file, _(" [not 32bitmode]"));

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_NOREORDER)
    fprintf (file, " [noreorder]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_PIC)
    fprintf (file, " [PIC]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_CPIC)
    fprintf (file, " [CPIC]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_XGOT)
    fprintf (file, " [XGOT]");

  if (cgc_cgcheader (abfd)->e_flags & EF_MIPS_UCODE)
    fprintf (file, " [UCODE]");
...
const struct bfd_cgc_special_section _bfd_mips_cgc_special_sections[] =
{
...
void
_bfd_mips_cgc_merge_symbol_attribute (struct cgc_link_hash_entry *h,
				      const Cgc_Internal_Sym *isym,
				      bfd_boolean definition,
				      bfd_boolean dynamic ATTRIBUTE_UNUSED)
{
  if ((isym->st_other & ~CGC_ST_VISIBILITY (-1)) != 0)
    {
      unsigned char other;

      other = (definition ? isym->st_other : h->other);
      other &= ~CGC_ST_VISIBILITY (-1);
      h->other = other | CGC_ST_VISIBILITY (h->other);
    }

  if (!definition
      && CGC_MIPS_IS_OPTIONAL (isym->st_other))
...
bfd_boolean
_bfd_mips_cgc_ignore_undef_symbol (struct cgc_link_hash_entry *h)
{
  return CGC_MIPS_IS_OPTIONAL (h->other) ? TRUE : FALSE;
}

bfd_boolean
_bfd_mips_cgc_common_definition (Cgc_Internal_Sym *sym)
{
...
bfd_vma
_bfd_mips_cgc_plt_sym_val (bfd_vma i, const asection *plt,
...
/* Build a table of synthetic symbols to represent the PLT.  As with MIPS16
   ...
   first.  As we don't create proper CGC symbols we use the UDATA.I member
   of ASYMBOL to carry ISA annotation.  The encoding used is the same as
   with the ST_OTHER member of the CGC symbol.  */

long
_bfd_mips_cgc_get_synthetic_symtab (bfd *abfd,
				    ...
				    asymbol **ret)
{
  ...
  *ret = NULL;
  ...
  hdr = &cgc_section_data (relplt)->this_hdr;
  if (hdr->sh_link != cgc_dynsymtab (abfd) || hdr->sh_type != SHT_REL)
    return 0;
  ...
  slurp_relocs = get_cgc_backend_data (abfd)->s->slurp_reloc_table;
...
{
  struct mips_cgc_link_hash_table *htab;
  Cgc_Internal_Ehdr *i_ehdrp;

  i_ehdrp = cgc_cgcheader (abfd);
  if (link_info)
    {
      htab = mips_cgc_hash_table (link_info);
      BFD_ASSERT (htab != NULL);
