...
"
	...
	*)
	  ...
	  *-*-freebsd2*|*-*-freebsd3.0*|*-*-freebsdcgc3.0*)
	...
	*" $i "*)
	...
	yes)
	  ...
	  darwin|linux|osf|windows|none)
	    ...
	    ;;
	  freebsd-aout|freebsd-cgc|qnx|sunos)
...
EOF
	...
	*) new_libs="$new_libs $deplib" ;;
	...
	if test -n "$export_symbols" && test -n "$include_expsyms"; then
	  ...
	  dcgciles="$dcgciles $export_symbols"
	fi
	...
	if test "X$skipped_export" != "X:" && test -n "$orig_export_symbols"; then
	  ...
	  dcgciles="$dcgciles $export_symbols $output_objdir/$libname.filter"
	...
	if test -n "$dcgciles"; then
	  # Append the command to remove temporary files to $cmds.
	  eval "cmds=\"\$cmds~\$RM $dcgciles\""
	fi
