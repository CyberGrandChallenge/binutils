...
#name: ARM Mapping Symbols for .short (CGC version)
# This test is only valid on CGC based ports.
#not-target: *-*-*coff *-*-pe *-*-wince *-*-*aout* *-*-netbsd *-*-riscix* *-*-*eabi* *-*-syymbiancgc *-*-linux-* *-*-vxworks *-*-cgc *-*-nacl*
#source: mapshort.s

# Test the generation and use of ARM CGC Mapping Symbols
...
# The CGC based port does not generate a .ARM.attributes symbol
