...
#ifndef OBJ_CGC
#error This file assumes object output is in the CGC format
#endif
...
static void shmedia_check_limits  (offsetT *, bfd_reloc_code_real_type,
				   fixS *);
static void sh64_set_contents_type (enum sh64_cgc_cr_type);
...
static void sh64_emit_crange (symbolS *, symbolS *, enum sh64_cgc_cr_type);
...
shmedia_frob_section_type (asection *sec)
{
  ...
  /* This and cgc32-sh64.c:sh64_cgc_fake_sections are the only places
     where we use anything else than CGC header flags to communicate the
     section as containing SHmedia or other contents.  BFD SEC_* section
     flags are running out and should not be overloaded with
     target-specific semantics.  This target is CGC only (semantics not
     defined for other formats), so we use the target-specific pointer
     field of the CGC section data.  */
  if (seginfo && sh64_abi == sh64_abi_32)
    {
      struct sh64_section_data *sec_cgc_data;
      flagword sec_type = 0;
	...
	sec_type = SHF_SH5_ISA32;
      ...
      sec_cgc_data = sh64_cgc_section_data (sec)->sh64_info;
      if (sec_cgc_data == NULL)
	{
	  sec_cgc_data = xcalloc (1, sizeof (*sec_cgc_data));
	  sh64_cgc_section_data (sec)->sh64_info = sec_cgc_data;
	}

      sec_cgc_data->contents_flags = sec_type;
    }
}
  ...
  for (symp = symbol_rootP; symp; symp = symbol_next (symp))
      ...
      if (main_symbol)
	{
	  ...
	  /* All datalabels not used in relocs should be gone by now.
	     ...
	     symbol, and we set the CGC type of the symbol of the reloc to
	     STT_DATALABEL.  */
	  sym_name[strlen (sym_name) - strlen (DATALABEL_SUFFIX)] = 0;
	  cgc_symbol (symbol_get_bfdsym (symp))->internal_cgc_sym.st_info
	    = STT_DATALABEL;
				       ...
				       segT segment_type ATTRIBUTE_UNUSED)
{
  ...
  /* For CGC, we can't relax externally visible symbols; see tc-i386.c.  */
...
#ifdef TE_NetBSD
  if (sh64_abi == sh64_abi_64)
    return (target_big_endian ? "cgc64-sh64-nbsd" : "cgc64-sh64l-nbsd");
  else
    return (target_big_endian ? "cgc32-sh64-nbsd" : "cgc32-sh64l-nbsd");
#elif defined (TE_LINUX)
  if (sh64_abi == sh64_abi_64)
    return (target_big_endian ? "cgc64-sh64big-linux" : "cgc64-sh64-linux");
  else
    return (target_big_endian ? "cgc32-sh64big-linux" : "cgc32-sh64-linux");
#else
  /* When the ISA is not one of SHmedia or SHcompact, use the old SH
     object format.  */
  if (sh64_isa_mode == sh64_isa_unspecified)
    return (target_big_endian ? "cgc32-sh" : "cgc32-shl");
  else if (sh64_abi == sh64_abi_64)
    return (target_big_endian ? "cgc64-sh64" : "cgc64-sh64l");
  else
    return (target_big_endian ? "cgc32-sh64" : "cgc32-sh64l");
#endif
}
...
static void
sh64_set_contents_type (enum sh64_cgc_cr_type new_contents_type)
{
  segment_info_type *seginfo;
  ...
  if (seginfo)
    {
      ...
      enum sh64_cgc_cr_type contents_type
