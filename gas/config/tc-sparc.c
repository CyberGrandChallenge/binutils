...
#ifdef TE_VXWORKS
  return "cgc32-sparc-vxworks";
#endif

#ifdef OBJ_CGC
  return sparc_arch_size == 64 ? CGC64_TARGET_FORMAT : CGC_TARGET_FORMAT;
#endif
	...
	for (l = list; *l != NULL; l++)
	  {
	    if (sparc_arch_size == 32)
	      {
		if (CONST_STRNEQ (*l, "cgc32-sparc"))
		  break;
	      }
	    else
	      {
		if (CONST_STRNEQ (*l, "cgc64-sparc"))
		  break;
	      }
	  }
	if (*l == NULL)
...
#if defined(OBJ_CGC) && !defined(TE_SOLARIS)
  if (hwcap_seen)
    bfd_cgc_add_obj_attr_int (stdoutput, OBJ_ATTR_GNU, Tag_GNU_Sparc_HWCAPS, hwcap_seen);
#endif
}

...
#ifdef OBJ_CGC
  /* SPARC CGC relocations don't use an addend in the data field.  */
...
#if defined (OBJ_CGC) || defined (OBJ_AOUT)
...
#define GOTT_INDEX "__GOTT_INDEX__"
  ...
  /* This code must be parallel to the OBJ_CGC tc_fix_adjustable.  */
...
#endif /* defined (OBJ_CGC) || defined (OBJ_AOUT)  */
...
{
#ifndef OBJ_CGC
  /* This is not right for CGC; a.out wants it, and COFF will force
...
#ifdef OBJ_CGC
/* Handle the SPARC CGC .register pseudo-op.  This sets the binding of a
   global register.
   The syntax is:
	  ...
	  if (regname == NULL)
		...
		flags |= BSF_GLOBAL;
		 ...
		 We fix this up in sparc_adjust_symtab.  */
	      ...
	      cgc_symbol (symbol_get_bfdsym (globals[reg]))
		->internal_cgc_sym.st_info =
		  CGC_ST_INFO(STB_GLOBAL, STT_REGISTER);
	      cgc_symbol (symbol_get_bfdsym (globals[reg]))
		->internal_cgc_sym.st_shndx = SHN_UNDEF;
	    }
	}
    }
...
sparc_adjust_symtab (void)
{
  ...
  for (sym = symbol_rootP; sym != NULL; sym = symbol_next (sym))
    {
      if (CGC_ST_TYPE (cgc_symbol (symbol_get_bfdsym (sym))
		       ->internal_cgc_sym.st_info) != STT_REGISTER)
	continue;
...
#ifdef OBJ_CGC
/* Some special processing for a Sparc CGC file.  */

void
sparc_cgc_final_processing (void)
{
  /* Set the Sparc CGC flag bits.  FIXME: There should probably be some
  ...
  if (sparc_arch_size == 64)
	...
	{
	case MM_RMO:
	  cgc_cgcheader (stdoutput)->e_flags |= EF_SPARCV9_RMO;
	  break;
	case MM_PSO:
	  cgc_cgcheader (stdoutput)->e_flags |= EF_SPARCV9_PSO;
	  break;
	default:
	  break;
	}
    ...
    cgc_cgcheader (stdoutput)->e_flags |= EF_SPARC_32PLUS;
  if (current_architecture == SPARC_OPCODE_ARCH_V9A)
    cgc_cgcheader (stdoutput)->e_flags |= EF_SPARC_SUN_US1;
  else if (current_architecture == SPARC_OPCODE_ARCH_V9B)
    cgc_cgcheader (stdoutput)->e_flags |= EF_SPARC_SUN_US1|EF_SPARC_SUN_US3;
}
