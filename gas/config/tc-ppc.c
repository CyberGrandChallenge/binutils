...
#if defined (OBJ_XCOFF) || defined (OBJ_CGC)
...
#ifdef OBJ_CGC
static void ppc_cgc_cons (int);
static void ppc_cgc_rdata (int);
static void ppc_cgc_lcomm (int);
static void ppc_cgc_localentry (int);
static void ppc_cgc_abiversion (int);
#endif
...
/* Value for CGC e_flags EF_PPC64_ABI.  */
...
#ifdef OBJ_CGC
  { "llong",	ppc_cgc_cons,	8 },
  { "quad",	ppc_cgc_cons,	8 },
  { "long",	ppc_cgc_cons,	4 },
  { "word",	ppc_cgc_cons,	2 },
  { "short",	ppc_cgc_cons,	2 },
  { "rdata",	ppc_cgc_rdata,	0 },
  { "rodata",	ppc_cgc_rdata,	0 },
  { "lcomm",	ppc_cgc_lcomm,	0 },
  { "localentry", ppc_cgc_localentry,	0 },
  { "abiversion", ppc_cgc_abiversion,	0 },
#endif
...
/* Whether to target xcoff64/cgc64.  */
...
md_show_usage (FILE *stream)
{
  fprintf (stream, _("\
PowerPC options:\n\
-a32                    generate CGC32/XCOFF32\n\
-a64                    generate CGC64/XCOFF64\n\
-u                      ignored\n\
...
-memb                   set PPC_EMB bit in CGC flags\n\
...
-K PIC                  set EF_PPC_RELOCATABLE_LIB in CGC flags\n\
...
#ifdef OBJ_CGC
# ifdef TE_FreeBSD
  return (ppc_obj64 ? "cgc64-powerpc-freebsd" : "cgc32-powerpc-freebsd");
# elif defined (TE_VXWORKS)
  return "cgc32-powerpc-vxworks";
# else
  return (target_big_endian
	  ? (ppc_obj64 ? "cgc64-powerpc" : "cgc32-powerpc")
	  : (ppc_obj64 ? "cgc64-powerpcle" : "cgc32-powerpcle"));
# endif
#endif
}
...
ppc_cgc_suffix (char **str_p, expressionS *exp_p)
{
  struct map_bfd {
    char *string;
...
static void
ppc_cgc_cons (int nbytes /* 1=.byte, 2=.word, 4=.long, 8=.llong */)
{
  expressionS exp;
  ...
  do
    {
	  ...
	  && (reloc = ppc_cgc_suffix (&input_line_pointer,
...
static void
ppc_cgc_lcomm (int xxx ATTRIBUTE_UNUSED)
{
  char *name;
  char c;
  char *p;
...
static void
ppc_cgc_localentry (int ignore ATTRIBUTE_UNUSED)
{
  ...
  *p = c;
  ...
  if (resolve_expression (&exp)
      ...
      else
	{
	  ...
	  cgcsym = cgc_symbol_from (bfd_asymbol_bfd (bfdsym), bfdsym);
	  gas_assert (cgcsym);
	  cgcsym->internal_cgc_sym.st_other &= ~STO_PPC64_LOCAL_MASK;
	  cgcsym->internal_cgc_sym.st_other |= encoded;
	  if (ppc_abiversion == 0)
	    ppc_abiversion = 2;
	}
    }
...
static void
ppc_cgc_abiversion (int ignore ATTRIBUTE_UNUSED)
{
  expressionS exp;
...
ppc_cgc_end (void)
{
  if (ppc_obj64 && ppc_abiversion != 0)
    {
      cgc_cgcheader (stdoutput)->e_flags &= ~EF_PPC64_ABI;
      cgc_cgcheader (stdoutput)->e_flags |= ppc_abiversion & EF_PPC64_ABI;
    }
}
...
/* Validate any relocations emitted for -mrelocatable, possibly adding
   ...
   them at runtime.  */
static void
ppc_cgc_validate_fix (fixS *fixp, segT seg)
{
...
/* Prevent cgc_frob_file_before_adjust removing a weak undefined
...
#endif /* defined (OBJ_XCOFF) || defined (OBJ_CGC) */

...
md_assemble (char *str)
{
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
      ...
      else if (ex.X_op == O_constant)
	{
	  ...
	  if ((reloc = ppc_cgc_suffix (&str, &ex)) != BFD_RELOC_UNUSED)
	    switch (reloc)
	      {
	      default:
		str = orig_str;
      ...
      else
	{
	  ...
	  if ((reloc = ppc_cgc_suffix (&str, &ex)) != BFD_RELOC_UNUSED)
		...
		case BFD_RELOC_PPC_TLS:
		  if (!_bfd_cgc_ppc_at_tls_transform (opcode->opcode, 0))
...

#ifdef OBJ_CGC
/* For CGC, add support for SHT_ORDERED.  */
...
/* The .tc pseudo-op.  This is used when generating either XCOFF or
   CGC.  This takes two or more arguments.
...
ppc_force_relocation (fixS *fix)
{
  ...
  switch (fix->fx_r_type)
      ...
      if (fix->fx_addsy)
	{
	  ...
	  cgc_symbol_type *cgcsym
	    = cgc_symbol_from (bfd_asymbol_bfd (bfdsym), bfdsym);
	  gas_assert (cgcsym);
	  if ((STO_PPC64_LOCAL_MASK & cgcsym->internal_cgc_sym.st_other) != 0)
	    return 1;
	}
      break;
    default:
