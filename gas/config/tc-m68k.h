...
#ifdef OBJ_CGC
#define TARGET_FORMAT "cgc32-m68k"
#endif
...
#if defined (M68KCOFF) || defined (OBJ_CGC)
...
/* Target *-*-cgc implies an embedded target.  No shared libs.
   *-*-uclinux also requires special casing to prevent GAS from
   generating unsupported R_68K_PC16 relocs.  */
#define EXTERN_FORCE_RELOC \
  ((strcmp (TARGET_OS, "cgc") != 0) && (strcmp (TARGET_OS, "uclinux") != 0))

/* Values passed to md_apply_fix don't include symbol values.  */
#define MD_APPLY_SYM_VALUE(FIX) 0

#define cgc_tc_final_processing m68k_cgc_final_processing
extern void m68k_cgc_final_processing (void);
#endif
...
#ifdef TE_UCLINUX
/* cgc2flt does not honor PT_LOAD's from the executable.
