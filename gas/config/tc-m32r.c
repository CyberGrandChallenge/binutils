...
m32r_target_format (void)
{
  ...
  if (target_big_endian)
    return "cgc32-m32r-linux";
  else
    return "cgc32-m32rle-linux";
#else
  if (target_big_endian)
    return "cgc32-m32r";
  else
...
void
m32r_cgc_section_change_hook (void)
{
...
void
m32r_cgc_final_processing (void)
{
  if (use_parallel)
    m32r_flags |= E_M32R_HAS_PARALLEL;
  cgc_cgcheader (stdoutput)->e_flags |= m32r_flags;
}
