  ...
  for (i = 0; i < ARRAY_SIZE (tic6x_arches); i++)
	...
	if (tic6x_seen_insns)
	  tic6x_arch_attribute
	    = cgc32_tic6x_merge_arch_attributes (tic6x_arch_attribute,
						 tic6x_arches[i].attr);
	else
...
tic6x_frob_label (symbolS *sym)
{
  ...
  /* Defining tc_frob_label overrides the CGC definition of
...
{
  cgc32_tic6x_set_use_rela_p (stdoutput, tic6x_generate_rela);
}
...
tic6x_start_unwind_section (const segT text_seg, int idx)
{
  ...
  if (idx)
    {
      prefix = CGC_STRING_C6000_unwind;
      prefix_once = CGC_STRING_C6000_unwind_once;
      type = SHT_C6000_UNWIND;
    }
  else
    {
      prefix = CGC_STRING_C6000_unwind_info;
      prefix_once = CGC_STRING_C6000_unwind_info_once;
      type = SHT_PROGBITS;
    }
