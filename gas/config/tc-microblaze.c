...
{
#ifdef OBJ_CGC
  obj_cgc_change_section (".data", SHT_PROGBITS, SHF_ALLOC+SHF_WRITE, 0, 0, 0, 0);
#else
  s_data (ignore);
#endif
}
...
{
#ifdef OBJ_CGC
  obj_cgc_change_section (".sdata", SHT_PROGBITS, SHF_ALLOC+SHF_WRITE, 0, 0, 0, 0);
#else
  s_data (ignore);
#endif
}
...
microblaze_s_rdata (int localvar)
{
  ...
  if (localvar == 0)
    {
      /* rodata.  */
      obj_cgc_change_section (".rodata", SHT_PROGBITS, SHF_ALLOC, 0, 0, 0, 0);
      if (rodata_segment == 0)
    ...
    {
      /* 1 .sdata2.  */
      obj_cgc_change_section (".sdata2", SHT_PROGBITS, SHF_ALLOC, 0, 0, 0, 0);
    }
#else
  s_data (ignore);
#endif
...
#ifdef OBJ_CGC
  if (localvar == 0) /* bss.  */
    obj_cgc_change_section (".bss", SHT_NOBITS, SHF_ALLOC+SHF_WRITE, 0, 0, 0, 0);
  else if (localvar == 1)
    {
      /* sbss.  */
      obj_cgc_change_section (".sbss", SHT_NOBITS, SHF_ALLOC+SHF_WRITE, 0, 0, 0, 0);
      if (sbss_segment == 0)
...
/* If the pseudo-op is not found in this table, it searches in the obj-cgc.c,
	      ...
	      segT     segment)
{
  ...
  if (fixP->fx_addsy != NULL
          ...
          || (S_GET_SEGMENT (fixP->fx_addsy) != segment)))
    {
      fixP->fx_done = 0;
#ifdef OBJ_CGC
      /* For CGC we can just return and let the reloc that will be generated
