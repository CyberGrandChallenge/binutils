...
static int cgc_flags = E_FLAG_RX_ABI;
...
md_parse_option (int c ATTRIBUTE_UNUSED, char * arg ATTRIBUTE_UNUSED)
{
  switch (c)
    ...
    case OPTION_32BIT_DOUBLES:
      cgc_flags &= ~ E_FLAG_RX_64BIT_DOUBLES;
      return 1;
...
{
  if (cgc_flags & E_FLAG_RX_64BIT_DOUBLES)
    return float_cons ('d');
  return float_cons ('f');
}
...
parse_rx_section (char * name)
{
  ...
  if ((sec = bfd_get_section_by_name (stdoutput, name)) == NULL)
	...
	type = SHT_NOBITS;
      ...
      obj_cgc_change_section (name, type, attr, 0, NULL, FALSE, FALSE);
    }
  else /* Try not to redefine a section, especially B_1.  */
    {
      int flags = sec->flags;

      type = cgc_section_type (sec);
