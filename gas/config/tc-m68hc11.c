...
/* Pseudo op to control the CGC flags.  */
...
/* CGC flags to set in the output file header.  */
static int cgc_flags = E_M68HC11_F64;
...
m68hc11_arch_format (void)
{
  ...
  if (current_architecture & cpu6811)
    return "cgc32-m68hc11";
  else
    return "cgc32-m68hc12";
}
...
get_default_target (void)
{
  ...
  if (target && target->name)
    {
      if (strcmp (target->name, "cgc32-m68hc12") == 0)
	{
	  current_architecture = cpu6812;
	  default_cpu = "m68hc12";
	}
      else if (strcmp (target->name, "cgc32-m68hc11") == 0)
	{
...
md_parse_option (int c, char *arg)
{
  ...
  switch (c)
    ...
    case OPTION_XGATE_RAMOFFSET:
      cgc_flags |= E_M68HC11_XGATE_RAMOFFSET;
      break;
  ...
  if (*p == '*' && (opmode & (M6811_OP_DIRECT | M6811_OP_IND16)))
      ...
      else if (strncmp (p, "%page", 5) == 0)
	{
      ...
      /* %addr modifier is used to obtain the physical address part
         ...
         in 16-bit).  See bfd/cgc32-m68hc12.c for the translation.  */
  ...
  do
    {
      ...
      *input_line_pointer = c;
      ...
      cgcsym->internal_cgc_sym.st_other |= mark;
...
/* On an CGC system, we can't relax a weak symbol.  The weak symbol
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  /* Patch the instruction with the resolved operand.  Cgc relocation
...
/* Set the CGC specific flags.  */
void
m68hc11_cgc_final_processing (void)
{
  if (current_architecture & cpu6812s)
    cgc_flags |= EF_M68HCS12_MACH;
  cgc_cgcheader (stdoutput)->e_flags &= ~EF_M68HC11_ABI;
  cgc_cgcheader (stdoutput)->e_flags |= cgc_flags;
}
...
s_m68hc11_parse_pseudo_instruction (int pseudo_insn)
{
  switch (pseudo_insn)
    ...
    case E_M68HC11_NO_BANK_WARNING:
      cgc_flags |= E_M68HC11_NO_BANK_WARNING;
      break;
    default:
