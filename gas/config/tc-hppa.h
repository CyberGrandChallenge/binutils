...
#include "bfd/cgc64-hppa.h"
#if defined (TE_LINUX) || defined (TE_NetBSD)
#define TARGET_FORMAT "cgc64-hppa-linux"
#else
#define TARGET_FORMAT "cgc64-hppa"
#endif
#else /* TARGET_ARCH_SIZE == 32 */
#include "bfd/cgc32-hppa.h"
#if defined (TE_LINUX)
#define TARGET_FORMAT "cgc32-hppa-linux"
#else
#if defined (TE_NetBSD)
#define TARGET_FORMAT "cgc32-hppa-netbsd"
#else
#define TARGET_FORMAT "cgc32-hppa"
#endif
#endif
#endif
#endif
...
/* Handle .type psuedo.  Given a type string of `millicode', set the
   internal cgc symbol type to STT_PARISC_MILLI, and return
   BSF_FUNCTION for the BFD symbol type.  */
#define md_cgc_symbol_type(name, sym, cgc)				\
  ((strcmp ((name), "millicode") == 0					\
    || strcmp ((name), "STT_PARISC_MILLI") == 0)			\
   ? (((cgc)->internal_cgc_sym.st_info = CGC_ST_INFO			\
       (CGC_ST_BIND ((cgc)->internal_cgc_sym.st_info), STT_PARISC_MILLI)\
       ), BSF_FUNCTION)							\
   : -1)
...
#define cgc_tc_final_processing	cgc_hppa_final_processing
void cgc_hppa_final_processing (void);
#endif /* OBJ_CGC */
