...
#if defined (OBJ_CGC) && defined (OBJ_SOM)
error only one of OBJ_CGC and OBJ_SOM can be defined
#endif

/* If we are using CGC, then we probably can support dwarf2 debug
...
typedef enum cgc_hppa_reloc_type reloc_type;

/* Object file formats specify BFD symbol types.  */
typedef cgc_symbol_type obj_symbol_type;
#define symbol_arg_reloc_info(sym)\
  (((obj_symbol_type *) symbol_get_bfdsym (sym))->tc_data.hppa_arg_reloc)

#if TARGET_ARCH_SIZE == 64
/* How to generate a relocation.  */
#define hppa_gen_reloc_type _bfd_cgc64_hppa_gen_reloc_type
#define cgc_hppa_reloc_final_type cgc64_hppa_reloc_final_type
#else
#define hppa_gen_reloc_type _bfd_cgc32_hppa_gen_reloc_type
#define cgc_hppa_reloc_final_type cgc32_hppa_reloc_final_type
#endif

/* CGC objects can have versions, but apparently do not have anywhere
   to store a copyright string.  */
#define obj_version obj_cgc_version
#define obj_copyright obj_cgc_version
...
#if !(defined (OBJ_CGC) && (defined (TE_LINUX) || defined (TE_NetBSD)))
  ...
  switch (fixp->fx_r_type)
	...
	case R_PARISC_PLABEL14R:
	  /* For plabel relocations, the addend of the
	     ...
	     bfd/cgc32-hppa.c:cgc32_hppa_relocate_section.
...
#if defined (OBJ_SOM) || defined (CGC_ARG_RELOC)
...
pa_build_unwind_subspace (struct call_info *call_info)
{
  ...
  /* Relocation info. for end offset of the function.
     ...
     Because we allow reductions of 32bit relocations for CGC, this will be
...
#endif /* !(defined (OBJ_CGC) && (defined (TE_LINUX) || defined (TE_NetBSD))) */
...
pa_entry (int unused ATTRIBUTE_UNUSED)
{
  ...
  /* SOM defers building of unwind descriptors until the link phase.
     ...
     FIXME.  CGC should be using the same conventions!  The problem
...
/* Mark the end of a function so that it's possible to compute
   the size of the function in cgc_hppa_final_processing.  */

static void
hppa_cgc_mark_end_of_function (void)
{
  /* CGC does not have EXIT relocations.  All we do is create a
...
#ifdef OBJ_CGC
      {
	cgc_symbol_type *cgcsym = (cgc_symbol_type *) bfdsym;
	cgcsym->internal_cgc_sym.st_info =
	  CGC_ST_INFO (CGC_ST_BIND (cgcsym->internal_cgc_sym.st_info),
		       STT_PARISC_MILLI);
      }
#endif
...
#ifdef OBJ_CGC
  /* CGC needs to mark the end of each function so that it can compute
...
pa_subspace (int create_new)
{
  ...
  if (within_procedure)
	...
	flags &= ~(SEC_LOAD | SEC_HAS_CONTENTS);
	 ...
	 FIXME NOW!  CGC BFD doesn't appear to be ready to deal with
	 lots of sections.  It might be a problem in the PA CGC
	 code, I do not know yet.  For now avoid creating anything
	 but the "standard" sections for CGC.  */
      if (create_new)
...
hppa_fix_adjustable (fixS *fixp)
{
  ...
  code = cgc_hppa_reloc_final_type (stdoutput, fixp->fx_r_type,
...
/* Now for some CGC specific code.  FIXME.  */
#ifdef OBJ_CGC
/* For CGC, this function serves one purpose:  to setup the st_size
...
void
cgc_hppa_final_processing (void)
{
  ...
  for (call_info_pointer = call_info_root;
    ...
    {
      cgc_symbol_type *esym
	= ((cgc_symbol_type *)
	   symbol_get_bfdsym (call_info_pointer->start_symbol));
      esym->internal_cgc_sym.st_size =
...
pa_vtable_entry (int ignore ATTRIBUTE_UNUSED)
{
  ...
  new_fix = obj_cgc_vtable_entry (0);
...
pa_vtable_inherit (int ignore ATTRIBUTE_UNUSED)
{
  ...
  new_fix = obj_cgc_vtable_inherit (0);
