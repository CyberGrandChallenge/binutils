/* tc-cris.c -- Assembler code for the CRIS CPU core.
   ...
   Updates, BFDizing, GNUifying and CGC support by Hans-Peter Nilsson.
...
/* If we target cris-axis-linux-gnu (as opposed to generic cris-axis-cgc),
...
/* Report output format.  Small changes in output format (like cgc
...
#define HANDLE_RELAXABLE(state)						\
  ...
  switch (fragP->fr_subtype)
    ...
    case ENCODE_RELAX (STATE_BASE_PLUS_DISP_PREFIX, STATE_UNDF):
	 ...
	 (like the GOT for CGC shared libraries), but that's no use, it
...
tc_gen_reloc (asection *section ATTRIBUTE_UNUSED, fixS *fixP)
{
  ...
  *relP->sym_ptr_ptr = symbol_get_bfdsym (fixP->fx_addsy);
  ...
  /* This is the standard place for KLUDGEs to work around bugs in
     ...
     That happens for a.out but not for CGC, since for CGC,
...
md_pcrel_from (fixS *fixP)
{
  ...
  /* FIXME:  We get here only at the end of assembly, when X in ".-X" is
     ...
     loader.  For CGC, allow straight-forward PC-relative relocations,
...
{
  if (OUTPUT_FLAVOR != bfd_target_cgc_flavour)
    as_bad (_("Pseudodirective .file is only valid when generating CGC"));
  else
...
{
  if (OUTPUT_FLAVOR != bfd_target_cgc_flavour)
    as_bad (_("Pseudodirective .loc is only valid when generating CGC"));
  else
