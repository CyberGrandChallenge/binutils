...
/* Like obj_cgc_section, but issues a warning for new
   sections which do not have an attribute specification.  */

static void
h8300_cgc_section (int push)
{
...
#ifdef OBJ_CGC
  {"section",   h8300_cgc_section, 0},
  {"section.s", h8300_cgc_section, 0},
  {"sect",      h8300_cgc_section, 0},
  {"sect.s",    h8300_cgc_section, 0},
#endif
  ...
  for (i = 0; i < this_try->noperands; i++)
      ...
      else if (x_mode == PCREL)
	{
	  ...
	  /* The COFF port has always been off by one, changing it
	     ...
	     We don't want to do this for CGC as we want to be
	     compatible with the proposed CGC format from Hitachi.  */
...
#ifdef OBJ_CGC
	  /* To be compatible with the proposed H8 CGC format, we
