...
static void
sh_cgc_cons (register int nbytes)
{
  expressionS exp;
...
sh_cgc_final_processing (void)
{
  ...
  if (sh64_isa_mode != sh64_isa_unspecified)
    ...
    if (1)
      {
	extern int sh_symbian_find_cgc_flags (unsigned int);

	val = sh_symbian_find_cgc_flags (valid_arch);
      }
    else
#endif /* HAVE_SH64 */
    val = sh_find_cgc_flags (valid_arch);

  cgc_cgcheader (stdoutput)->e_flags &= ~EF_SH_MACH_MASK;
  cgc_cgcheader (stdoutput)->e_flags |= val;

  if (sh_fdpic)
    cgc_cgcheader (stdoutput)->e_flags |= EF_SH_FDPIC;
}
#endif
...
{
  if (sh_fdpic)
    return (!target_big_endian ? "cgc32-sh-fdpic" : "cgc32-shbig-fdpic");
  else
    return (!target_big_endian ? "cgc32-shl" : "cgc32-sh");
}
#endif
...
/* Apply fixup FIXP to SIZE-byte field BUF given that VAL is its
   ...
   it should be in an CGC r_addend field.  */
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  /* The function adjust_reloc_syms won't convert a reloc against a weak
     ...
     For ordinary relocs, this does not happen for CGC, since for CGC,
     ...
     described in cgc32-sh.c as they use bfd_cgc_generic_reloc, but it
