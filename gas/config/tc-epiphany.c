...
static void
epiphany_cgc_section_rtn (int i)
{
  obj_cgc_section (i);
...
static void
epiphany_cgc_section_text (int i)
{
...
{
    { "text",   epiphany_cgc_section_text,  0 },
    { "sect",   epiphany_cgc_section_rtn,   0 },
...
int
epiphany_cgc_section_flags (int flags,
			    int attr ATTRIBUTE_UNUSED,
			    int type ATTRIBUTE_UNUSED)
{
  /* This is used to detect when the section changes to an executable section.
     This function is called by the cgc section processing.  When we note an
...
void
epiphany_cgc_final_processing (void)
{
  cgc_cgcheader (stdoutput)->e_flags |= epiphany_flags;
}
