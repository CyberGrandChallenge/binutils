...
static int cgc_flags = E_XGATE_F64;
...
get_default_target (void)
{
  ...
  if (target && target->name)
    {
      if (strcmp (target->name, "cgc32-xgate") == 0)
	{
...
void
xgate_cgc_final_processing (void)
{
  cgc_flags |= EF_XGATE_MACH;
  cgc_cgcheader (stdoutput)->e_flags &= ~EF_XGATE_ABI;
...
xgate_frob_symbol (symbolS *sym)
{
  ...
  cgcsym->internal_cgc_sym.st_target_internal = 1;
