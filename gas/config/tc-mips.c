...
/* Clean up namespace so we can include obj-cgc.h too.  */
...
#define CPU_HAS_MIPS16(cpu)						\
   (strncmp (TARGET_CPU, "mips16", sizeof ("mips16") - 1) == 0		\
    || strncmp (TARGET_CANONICAL, "mips-lsi-cgc", sizeof ("mips-lsi-cgc") - 1) == 0)
...
struct option md_longopts[] =
{
  ...
  /* Strictly speaking this next option is CGC specific,
     but we allow it for other ports as well in order to
     make testing easier.  */
  {"32", no_argument, NULL, OPTION_32},

  /* CGC-specific options.  */
...
#define CGC_TARGET(PREFIX, ENDIAN) PREFIX "trad" ENDIAN "mips-freebsd"
#elif defined (TE_TMIPS)
#define CGC_TARGET(PREFIX, ENDIAN) PREFIX "trad" ENDIAN "mips"
#else
#define CGC_TARGET(PREFIX, ENDIAN) PREFIX ENDIAN "mips"
#endif
...
mips_target_format (void)
{
  switch (OUTPUT_FLAVOR)
    {
    case bfd_target_cgc_flavour:
#ifdef TE_VXWORKS
      if (!HAVE_64BIT_OBJECTS && !HAVE_NEWABI)
	return (target_big_endian
		? "cgc32-bigmips-vxworks"
		: "cgc32-littlemips-vxworks");
#endif
      return (target_big_endian
	      ? (HAVE_64BIT_OBJECTS
		 ? CGC_TARGET ("cgc64-", "big")
		 : (HAVE_NEWABI
		    ? CGC_TARGET ("cgc32-n", "big")
		    : CGC_TARGET ("cgc32-", "big")))
	      : (HAVE_64BIT_OBJECTS
		 ? CGC_TARGET ("cgc64-", "little")
		 : (HAVE_NEWABI
		    ? CGC_TARGET ("cgc32-n", "little")
		    : CGC_TARGET ("cgc32-", "little"))));
    default:
      abort ();
      return NULL;
    }
...
md_begin (void)
{
  ...
  /* On a native system other than VxWorks, sections must be aligned
     to 16 byte boundaries.  When configured for an embedded CGC
     target, we don't bother.  */
  if (strncmp (TARGET_OS, "cgc", 3) != 0
     ...
     section for debugging information.  */
  {
	...
	mips_regmask_frag = frag_more (sizeof (Cgc32_External_RegInfo));
      }
    else
      {
	...
	{
	  Cgc_Internal_Options opthdr;
	  char *f;

	  opthdr.kind = ODK_REGINFO;
	  opthdr.size = (sizeof (Cgc_External_Options)
			 + sizeof (Cgc64_External_RegInfo));
	  opthdr.section = 0;
	  opthdr.info = 0;
	  f = frag_more (sizeof (Cgc_External_Options));
	  bfd_mips_cgc_swap_options_out (stdoutput, &opthdr,
					 (Cgc_External_Options *) f);

	  mips_regmask_frag = frag_more (sizeof (Cgc64_External_RegInfo));
	}
      }
...
s_is_linkonce (symbolS *sym, segT from_seg)
{
  ...
  if (symseg != from_seg && !S_IS_LOCAL (sym))
	...
	linkonce = TRUE;
      /* The GNU toolchain uses an extension for CGC: a section
...
mips_compressed_mark_label (symbolS *label)
{
  ...
  if (mips_opts.mips16)
    S_SET_OTHER (label, CGC_ST_SET_MIPS16 (S_GET_OTHER (label)));
  else
    S_SET_OTHER (label, CGC_ST_SET_MICROMIPS (S_GET_OTHER (label)));
...
micromips_add_label (void)
{
  ...
  micromips_label_inc ();
  S_SET_OTHER (s, CGC_ST_SET_MICROMIPS (S_GET_OTHER (s)));
}
...
static int support_64bit_objects(void)
{
  ...
  for (l = list; *l != NULL; l++)
    if (strcmp (*l, CGC_TARGET ("cgc64-", "big")) == 0
	|| strcmp (*l, CGC_TARGET ("cgc64-", "little")) == 0)
      break;
  yes = (*l != NULL);
  free (list);
  return yes;
...
md_parse_option (int c, char *arg)
{
  ...
  switch (c)
    ...
    case OPTION_MNO_SYM32:
      ...
      /* When generating CGC code, we permit -KPIC and -call_shared to
...
s_change_sec (int sec)
{
  ...
  /* The CGC backend needs to know that we are changing sections, so
     that .previous works correctly.  We could do something like check
     for an obj_section_change_hook macro, but that might be confusing
     as it would not be appropriate to use it in the section changing
     functions in read.c, since obj-cgc.c intercepts those.  FIXME:
     This should be cleaner, somehow.  */
  obj_cgc_section_change_hook ();
...
s_change_section (int ignore ATTRIBUTE_UNUSED)
{
  ...
  /* When using the generic form of .section (as implemented by obj-cgc.c),
  ...
  obj_cgc_change_section (section_name, section_type, section_flag,
...
s_cpload (int ignore ATTRIBUTE_UNUSED)
{
  ...
  /* In CGC, this symbol is implicitly an STT_OBJECT symbol.  */
...
md_section_align (asection *seg, valueT addr)
{
  ...
  /* We don't need to align CGC sections to the full alignment.
...
mips_fix_adjustable (fixS *fixp)
{
  ...
  /* R_MIPS16_26 relocations against non-MIPS16 functions might resolve
     ...
     relocations against global symbols will never be reduced on CGC
  ...
  if (fixp->fx_subsy == NULL
      && (CGC_ST_IS_MIPS16 (S_GET_OTHER (fixp->fx_addsy))
	  || *symbol_get_tc (fixp->fx_addsy)
	  || (HAVE_IN_PLACE_ADDENDS
	      && CGC_ST_IS_MICROMIPS (S_GET_OTHER (fixp->fx_addsy))
...
tc_gen_reloc (asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  /* Since the old MIPS CGC ABI uses Rel instead of Rela, encode the vtable
...
md_convert_frag (bfd *abfd ATTRIBUTE_UNUSED, segT asec, fragS *fragp)
{
  ...
  if (RELAX_MICROMIPS_P (fragp->fr_subtype))
      ...
      if (!RELAX_MICROMIPS_UNCOND (fragp->fr_subtype))
	{
	  ...
	  micromips_label_inc ();
	  S_SET_OTHER (l, CGC_ST_SET_MICROMIPS (S_GET_OTHER (l)));
...
mips_frob_file_after_relocs (void)
{
  ...
  for (i = 0; i < count; i++, syms++)
    if (CGC_ST_IS_COMPRESSED (cgc_symbol (*syms)->internal_cgc_sym.st_other)
	&& ((*syms)->value & 1) != 0)
      {
	(*syms)->value &= ~1;
	/* If the symbol has an odd size, it was probably computed
	   incorrectly, so adjust that as well.  */
	if ((cgc_symbol (*syms)->internal_cgc_sym.st_size & 1) != 0)
	  ++cgc_symbol (*syms)->internal_cgc_sym.st_size;
      }
}
...

/* Some special processing for a MIPS CGC file.  */

void
mips_cgc_final_processing (void)
{
  ...
  if (mips_abi != N64_ABI)
    {
      ...
      /* The gp_value field is set by the MIPS CGC backend.  */

      bfd_mips_cgc32_swap_reginfo_out (stdoutput, &s,
				       ((Cgc32_External_RegInfo *)
					mips_regmask_frag));
    }
  else
    {
      Cgc64_Internal_RegInfo s;
      ...
      bfd_mips_cgc64_swap_reginfo_out (stdoutput, &s,
				       ((Cgc64_External_RegInfo *)
					mips_regmask_frag));
    }

  /* Set the MIPS CGC flag bits.  FIXME: There should probably be some
     sort of BFD interface for this.  */
  if (mips_any_noreorder)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_NOREORDER;
  if (mips_pic != NO_PIC)
    {
      cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_PIC;
      cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_CPIC;
    }
  if (mips_abicalls)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_CPIC;

  /* Set MIPS CGC flags for ASEs.  Note that not all ASEs have flags
     defined at present; this might need to change in future.  */
  if (file_ase_mips16)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_ARCH_ASE_M16;
  if (file_ase_micromips)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_ARCH_ASE_MICROMIPS;
  if (file_ase & ASE_MDMX)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_ARCH_ASE_MDMX;

  /* Set the MIPS CGC ABI flags.  */
  if (mips_abi == O32_ABI && USE_E_MIPS_ABI_O32)
    cgc_cgcheader (stdoutput)->e_flags |= E_MIPS_ABI_O32;
  else if (mips_abi == O64_ABI)
    cgc_cgcheader (stdoutput)->e_flags |= E_MIPS_ABI_O64;
  else if (mips_abi == EABI_ABI)
    {
      if (!file_mips_gp32)
	cgc_cgcheader (stdoutput)->e_flags |= E_MIPS_ABI_EABI64;
      else
	cgc_cgcheader (stdoutput)->e_flags |= E_MIPS_ABI_EABI32;
    }
  else if (mips_abi == N32_ABI)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_ABI2;

  /* Nothing to do for N64_ABI.  */

  if (mips_32bitmode)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_32BITMODE;

  if (mips_flag_nan2008)
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_NAN2008;

  /* 32 bit code with 64 bit FP registers.  */
  if (!file_mips_fp32 && ABI_NEEDS_32BIT_REGS (mips_abi))
    cgc_cgcheader (stdoutput)->e_flags |= EF_MIPS_FP64;
}

typedef struct proc {
  symbolS *func_sym;
