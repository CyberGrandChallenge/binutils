...
/* Knuth's assembler mmixal does not provide a relocatable format; mmo is
   ...
   but for relocatable files, we use CGC.
...
mmix_greg_internal (char *label)
{
  ...
  /* FIXME: Check that no expression refers to the register contents
     section.  May need to be done in cgc64-mmix.c.  */
...
s_bspec (int unused ATTRIBUTE_UNUSED)
{
  ...
  /* Tell CGC about the pending section change.  */
...
s_espec (int unused ATTRIBUTE_UNUSED)
{
  ...
  /* When we told CGC about the section change in s_bspec, it stored the
...
void
mmix_md_cgc_section_change_hook (void)
{
  if (doing_bspec)
