...
#endif
#if defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC)
...
const char extra_symbol_chars[] = "*%-([{"
	...
	;
     ...
     || ((defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC))	\
	 && !defined (TE_GNU)				\
	 && !defined (TE_LINUX)				\
	 && !defined (TE_NACL)				\
	 && !defined (TE_NETWARE)			\
...
static enum x86_cgc_abi x86_cgc_abi = I386_ABI;
#endif
			   ...
			   i386_cpu_flags new_flag ATTRIBUTE_UNUSED)
{
  ...
  /* Intel LIOM is only supported on CGC.  */
  if (!IS_CGC)
    return;
  ...
  if (get_cgc_backend_data (stdoutput)->cgc_machine_code != EM_L1OM
      || new_flag.bitfield.cpul1om)
    return;

  /* If we are targeting Intel K1OM, we must enable it.  */
  if (get_cgc_backend_data (stdoutput)->cgc_machine_code != EM_K1OM
  ...
  if (cpu_arch_isa == PROCESSOR_L1OM)
    {
	  ...
	  || flag_code != CODE_64BIT)
	as_fatal (_("Intel L1OM is 64bit CGC only"));
  ...
  else if (cpu_arch_isa == PROCESSOR_K1OM)
    {
      if (OUTPUT_FLAVOR != bfd_target_cgc_flavour
	  || flag_code != CODE_64BIT)
	as_fatal (_("Intel K1OM is 64bit CGC only"));
...
md_assemble (char *line)
{
  ...
  /* CGC DECREE does not support certain mnemonics */

  if (( strcmp (mnemonic, "rdpmc") == 0)
      || ( strcmp (mnemonic, "rdrand") == 0)
      || ( strcmp (mnemonic, "rdtsc") == 0)
      || ( strcmp (mnemonic, "rdtscp") == 0)
      || ( strcmp (mnemonic, "sysenter") == 0))
    {
      as_warn(_("unsupported instruction: `%s'"), mnemonic);
    }
...
#if !(defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC) || defined (OBJ_MACH_O)) \
    || defined (LEX_AT)
...
/* Parse operands of the form
   ...
   This function is copied from the CGC version above adjusted for PE targets.  */
...
md_estimate_size_before_relax (fragS *fragP, segT segment)
{
  /* We've already got fragP->fr_subtype right;  all we have to do is
     check for un-relaxable symbols.  On an CGC system, we can't relax
...
#endif /* defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC)  */
  *valP = value;
...
md_parse_option (int c, char *arg)
{
  ...
  switch (c)
	...
	for (l = list; *l != NULL; l++)
	  if (CONST_STRNEQ (*l, "cgc64-x86-64")
      ...
      if (IS_CGC)
	{
	  ...
	  list = bfd_target_list ();
	  for (l = list; *l != NULL; l++)
	    if (CONST_STRNEQ (*l, "cgc32-x86-64"))
	      {
	...
	as_fatal (_("32bit x86_64 is only supported for CGC"));
      break;
#endif
  ...
  if (!strncmp (default_arch, "x86_64", 6))
    {
      ...
      if (default_arch[6] == '\0')
	x86_cgc_abi = X86_64_ABI;
      else
	x86_cgc_abi = X86_64_X32_ABI;
    }
  else if (!strcmp (default_arch, "i386"))
...
# endif
#endif
#if defined (OBJ_MAYBE_CGC) || defined (OBJ_CGC)
	...
	switch (x86_cgc_abi)
	  {
	  default:
	    format = CGC_TARGET_FORMAT;
	    break;
	  case X86_64_ABI:
	    use_rela_relocations = 1;
	    object_64bit = 1;
	    format = CGC_TARGET_FORMAT64;
	    break;
	  case X86_64_X32_ABI:
	    use_rela_relocations = 1;
	    object_64bit = 1;
	    disallow_64bit_reloc = 1;
	    format = CGC_TARGET_FORMAT32;
	    break;
	  }
	if (cpu_arch_isa == PROCESSOR_L1OM)
	  {
	    if (x86_cgc_abi != X86_64_ABI)
	      as_fatal (_("Intel L1OM is 64bit only"));
	    return CGC_TARGET_L1OM_FORMAT;
	  }
	if (cpu_arch_isa == PROCESSOR_K1OM)
	  {
	    if (x86_cgc_abi != X86_64_ABI)
	      as_fatal (_("Intel K1OM is 64bit only"));
	    return CGC_TARGET_K1OM_FORMAT;
	  }
	else
	  return format;
      }
...
#if (defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC))
void
i386_cgc_emit_arch_note (void)
{
  if (IS_CGC && cpu_arch_name != NULL)
    {
      char *p;
      asection *seg = now_seg;
      subsegT subseg = now_subseg;
      Cgc_Internal_Note i_note;
      Cgc_External_Note e_note;
      asection *note_secp;
      int len;
...
tc_gen_reloc (asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
  ...
  *rel->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  if (!use_rela_relocations)
    {
      /* HACK: Since i386 CGC uses Rel instead of Rela, encode the
...
int
i386_cgc_section_type (const char *str, size_t len)
{
  if (flag_code == CODE_64BIT
      && len == sizeof ("unwind") - 1
...
{
  if (flag_code == CODE_64BIT)
    cgc_section_type (sec) = SHT_X86_64_UNWIND;
}
#endif
...
#if defined (OBJ_CGC) || defined (OBJ_MAYBE_CGC)
/* For CGC on x86-64, add support for SHF_X86_64_LARGE.  */
...
handle_large_common (int small ATTRIBUTE_UNUSED)
{
  if (flag_code != CODE_64BIT)
      ...
      if (lbss_section == NULL)
	{
      ...
      cgc_com_section_ptr = &_bfd_cgc_large_com_section;
      bss_section = lbss_section;

      s_comm_internal (0, cgc_common_parse);

      cgc_com_section_ptr = saved_com_section_ptr;
      bss_section = saved_bss_section;
    }
}
#endif /* OBJ_CGC || OBJ_MAYBE_CGC */
