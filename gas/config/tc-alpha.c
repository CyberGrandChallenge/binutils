/* tc-alpha.c - Processor-specific code for the DEC Alpha AXP CPU.
   ...
   Modified by Richard Henderson for CGC support.
...
#if defined(OBJ_CGC) || defined (OBJ_EVAX)
...
alpha_reloc_op[] =
{
  DEF (literal, BFD_RELOC_ALPHA_CGC_LITERAL, 0, 1),
		     ...
		     void * ptr ATTRIBUTE_UNUSED)
{
  ...
  for (fixp = seginfo->fix_root; fixp; fixp = next)
	...
	case BFD_RELOC_ALPHA_CGC_LITERAL:
		 ...
		 const char *opname)
{
  ...
  switch (exp->X_op)
	...
	gas_assert (insn.nfixups == 1);
	insn.fixups[0].reloc = BFD_RELOC_ALPHA_CGC_LITERAL;
	insn.sequence = emit_lituse = next_sequence_num--;
#endif /* OBJ_CGC */
#ifdef OBJ_EVAX
	     ...
	     int force ATTRIBUTE_UNUSED)
{
  ...
  /* ??? If alpha_flag_relax && force && cgc, record the requested alignment
...
FIXME
#endif
#if defined(OBJ_ECOFF) || defined(OBJ_CGC)
...
struct alpha_cgc_frame_data
{
  ...
  struct alpha_cgc_frame_data *next;
};

static struct alpha_cgc_frame_data *all_frame_data;
static struct alpha_cgc_frame_data **plast_frame_data = &all_frame_data;
static struct alpha_cgc_frame_data *cur_frame_data;
...
s_alpha_ent (int dummy ATTRIBUTE_UNUSED)
{
  if (ECOFF_DEBUGGING)
      ...
      else
	{
	  ...
	  cur_frame_data = (struct alpha_cgc_frame_data *)
...
void
alpha_cgc_md_end (void)
{
  struct alpha_cgc_frame_data *p;
...
bfd_vma
alpha_cgc_section_letter (int letter, char **ptr_msg)
{
  if (letter == 's')
...
flagword
alpha_cgc_section_flags (flagword flags, bfd_vma attr, int type ATTRIBUTE_UNUSED)
{
...
/* Attempt to simplify or even eliminate a fixup.  The return value is
   ...
   For CGC, here it is that we transform the GPDISP_HI16 reloc we used
