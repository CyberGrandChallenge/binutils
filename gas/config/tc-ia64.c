...
static struct
  {
    ...
    *dynreg[DYNREG_NUM_TYPES], in, loc, out, rot;
    ...
    flagword flags;			/* CGC-header flags */
...
pseudo_func[] =
  {
    ...
    { "svr4",	PSEUDO_FUNC_CONST,	{ CGCOSABI_NONE } },
    { "hpux",	PSEUDO_FUNC_CONST,	{ CGCOSABI_HPUX } },
    { "nt",	PSEUDO_FUNC_CONST,	{ 2 } },		/* conflicts w/CGCOSABI_NETBSD */
    { "linux",	PSEUDO_FUNC_CONST,	{ CGCOSABI_GNU } },
    { "freebsd", PSEUDO_FUNC_CONST,	{ CGCOSABI_FREEBSD } },
    { "openvms", PSEUDO_FUNC_CONST,	{ CGCOSABI_OPENVMS } },
    { "nsk",	PSEUDO_FUNC_CONST,	{ CGCOSABI_NSK } },
...
/* Switch to section NAME and create section if necessary.  It's
   ...
   changing obj-cgc.c (which may be the Right Thing, in the end).  */
static void
...
bfd_vma
ia64_cgc_section_letter (int letter, char **ptr_msg)
{
  if (letter == 's')
...
flagword
ia64_cgc_section_flags (flagword flags,
			bfd_vma attr,
...
int
ia64_cgc_section_type (const char *str, size_t len)
{
#define STREQ(s) ((len == sizeof (s) - 1) && (strncmp (str, s, sizeof (s) - 1) == 0))

  if (STREQ (CGC_STRING_ia64_unwind_info))
    return SHT_PROGBITS;

  if (STREQ (CGC_STRING_ia64_unwind_info_once))
    return SHT_PROGBITS;

  if (STREQ (CGC_STRING_ia64_unwind))
    return SHT_IA_64_UNWIND;

  if (STREQ (CGC_STRING_ia64_unwind_once))
    return SHT_IA_64_UNWIND;
...
static void
obj_cgc_vms_common (int ignore ATTRIBUTE_UNUSED)
{
  char *sec_name;
  char *sym_name;
  char c;
...
#ifdef md_cgc_section_change_hook
  md_cgc_section_change_hook ();
#endif
}
...
start_unwind_section (const segT text_seg, int sec_index)
{
  ...
  */
  ...
  if ((text_seg->flags & SEC_LINK_ONCE) != 0
      && (cgc_section_flags (text_seg) & SHF_GROUP) != 0)
    {
      char *section;
      size_t len, group_name_len;
      const char *group_name = cgc_group_name (text_seg);
  ...
  cgc_linked_to_section (now_seg) = text_seg;
}
...
const pseudo_typeS md_pseudo_table[] =
  {
    ...
    /* ??? These are needed to make gas/testsuite/gas/cgc/ehopt.s work.
...
#ifdef TE_VMS
    {"vms_common", obj_cgc_vms_common, 0},
#endif
...
{
  const struct ia64_operand *odesc = &cgc64_ia64_operands[opnd];
  unsigned int bits = 0;
  int i;
...
#undef FORCE_CONST
  ...
  if (!idesc)
	...
	as_bad (_("Operand %u of `%s' should be %s"),
		...
		cgc64_ia64_operands[expected_operand].desc);
...
build_insn (struct slot *slot, bfd_vma *insnp)
{
  ...
  for (i = 0; i < NELEMS (idesc->operands) && idesc->operands[i]; ++i)
	...
	default:
      ...
      odesc = cgc64_ia64_operands + idesc->operands[i];
      ...
      if (idesc->flags & IA64_OPCODE_PSEUDO)
	{
	  if ((idesc->flags & IA64_OPCODE_F2_EQ_F3)
	      && odesc == cgc64_ia64_operands + IA64_OPND_F3)
	    {
	      o2desc = cgc64_ia64_operands + IA64_OPND_F2;
	      (*o2desc->insert) (o2desc, val, &insn);
	    }
	  if ((idesc->flags & IA64_OPCODE_LEN_EQ_64MCNT)
	      && (odesc == cgc64_ia64_operands + IA64_OPND_CPOS6a
		  || odesc == cgc64_ia64_operands + IA64_OPND_POS6))
	    {
	      o2desc = cgc64_ia64_operands + IA64_OPND_LEN6;
...
IA-64 options:\n\
  --mconstant-gp	  mark output file as using the constant-GP model\n\
			  (sets CGC header flag EF_IA_64_CONS_GP)\n\
  --mauto-pic		  mark output file as using the constant-GP model\n\
			  without function descriptors (sets CGC header flag\n\
...
md_begin (void)
{
 ...
 if (md.tune != itanium1)
  ...
  /* Set the architecture and machine depending on defaults and command line
     options.  */
  if (md.flags & EF_IA_64_ABI64)
    ok = bfd_set_arch_mach (stdoutput, bfd_arch_ia64, bfd_mach_ia64_cgc64);
  else
    ok = bfd_set_arch_mach (stdoutput, bfd_arch_ia64, bfd_mach_ia64_cgc32);
...
{
  if (OUTPUT_FLAVOR == bfd_target_cgc_flavour)
    {
      if (md.flags & EF_IA_64_BE)
	{
	  if (md.flags & EF_IA_64_ABI64)
#if defined(TE_AIX50)
	    return "cgc64-ia64-aix-big";
#elif defined(TE_HPUX)
	    return "cgc64-ia64-hpux-big";
#else
	    return "cgc64-ia64-big";
#endif
	  else
#if defined(TE_AIX50)
	    return "cgc32-ia64-aix-big";
#elif defined(TE_HPUX)
	    return "cgc32-ia64-hpux-big";
#else
	    return "cgc32-ia64-big";
#endif
	}
      else
	{
	  if (md.flags & EF_IA_64_ABI64)
#if defined (TE_AIX50)
	    return "cgc64-ia64-aix-little";
#elif defined (TE_VMS)
	  {
	    md.flags |= EF_IA_64_ARCHVER_1;
	    return "cgc64-ia64-vms";
	  }
#else
	    return "cgc64-ia64-little";
#endif
	  else
#ifdef TE_AIX50
	    return "cgc32-ia64-aix-little";
#else
	    return "cgc32-ia64-little";
#endif
	}
    }
  else
...
ia64_frob_symbol (struct symbol *sym)
{
  if ((S_GET_SEGMENT (sym) == bfd_und_section_ptr && ! symbol_used_p (sym) &&
       CGC_ST_VISIBILITY (S_GET_OTHER (sym)) == STV_DEFAULT)
...
fix_insn (fixS *fix, const struct ia64_operand *odesc, valueT value)
{
  ...
  err = NULL;
  if (odesc - cgc64_ia64_operands == IA64_OPND_IMMU64)
    {
  ...
  else if (odesc - cgc64_ia64_operands == IA64_OPND_IMMU62)
    {
  ...
  else if (odesc - cgc64_ia64_operands == IA64_OPND_TGT64)
    {
      value >>= 4;
  ...
  if (fix->fx_addsy)
	...
	case BFD_RELOC_UNUSED:
	  ...
	  as_bad_where (fix->fx_file, fix->fx_line,
			...
			cgc64_ia64_operands[fix->tc_fix_data.opnd].desc);
	  fix->fx_done = 1;
	  return;
    ...
    {
      fix_insn (fix, cgc64_ia64_operands + fix->tc_fix_data.opnd, value);
      fix->fx_done = 1;
    }
}
...
void
ia64_cgc_section_change_hook (void)
{
  if (cgc_section_type (now_seg) == SHT_IA_64_UNWIND
      && cgc_linked_to_section (now_seg) == NULL)
    cgc_linked_to_section (now_seg) = text_section;
  dot_byteorder (-1);
}
...
/* Integrity VMS 8.x identifies it's CGC modules with a standard CGC
   .note section.  */
