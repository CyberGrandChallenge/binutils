...
/* Are we trying to generate PIC code?  If so, absolute references
   ...
   references.  Not implemented.  For CGC there are other means
...
const pseudo_typeS md_pseudo_table[] =
{
  ...
  {"skip", s_space, 0},
  {"proc", s_proc, 0},
#if defined (TE_SUN3) || defined (OBJ_CGC)
  {"align", s_align_bytes, 0},
#endif
#ifdef OBJ_CGC
  {"swbeg", s_ignore, 0},
  {"long", m68k_cgc_cons, 4},
#endif
  {"extend", float_cons, 'x'},
...
/* On an CGC system, we can't relax an externally visible symbol,
   because it may be overridden by a shared library.  However, if
   TARGET_OS is "cgc", then we presume that we are assembling for an
...
#ifdef OBJ_CGC
void
m68k_cgc_final_processing (void)
{
  unsigned flags = 0;
  ...
  cgc_cgcheader (stdoutput)->e_flags |= flags;
}

/* Parse @TLSLDO and return the desired relocation.  */
static bfd_reloc_code_real_type
m68k_cgc_suffix (char **str_p, expressionS *exp_p)
{
  char ident[20];
  char *str = *str_p;
  char *str2;
...
/* Handles .long <tls_symbol>+0x8000 debug info.
   ...
   Adapted from tc-ppc.c:ppc_cgc_cons.  */
static void
m68k_cgc_cons (int nbytes /* 4=.long */)
{
  ...
  do
    {
	  ...
	  && *input_line_pointer == '@'
	  && (reloc = m68k_cgc_suffix (&input_line_pointer,
