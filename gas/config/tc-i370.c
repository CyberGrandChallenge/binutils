...
/* This assembler implements a very hacked version of an cgc-like thing
...
i370_cgc_suffix (char **str_p, expressionS *exp_p)
{
  struct map_bfd
  {
    char *string;
...
static void
i370_cgc_cons (int nbytes)   /* 1=.byte, 2=.word, 4=.long.  */
{
  expressionS exp;
  bfd_reloc_code_real_type reloc;
  ...
  do
    {
      ...
      if (exp.X_op == O_symbol
          && *input_line_pointer == '@'
          && (reloc = i370_cgc_suffix (&input_line_pointer, &exp)) != BFD_RELOC_UNUSED)
        {
...
/* DC Define Const  is only partially supported.
   For samplecode on what to do, look at i370_cgc_cons() above.
...
static void
i370_cgc_lcomm (int unused ATTRIBUTE_UNUSED)
{
  char *name;
  char c;
  char *p;
...
static void
i370_cgc_validate_fix (fixS *fixp, segT seg)
{
  ...
  for (opindex_ptr = opcode->operands; *opindex_ptr != 0; opindex_ptr++)
	...
	insn = i370_insert_operand (insn, operand, ex.X_add_number);
      else if (ex.X_op == O_constant)
        {
          ...
          if ((reloc = i370_cgc_suffix (&str, &ex)) != BFD_RELOC_UNUSED)
            switch (reloc)
              {
              default:
        	str = orig_str;
...
#ifdef OBJ_CGC
      else if ((reloc = i370_cgc_suffix (&str, &ex)) != BFD_RELOC_UNUSED)
        {
...

/* The .tc pseudo-op.  This is used when generating XCOFF and CGC.
  ...
  if ((int) fixP->fx_r_type >= (int) BFD_RELOC_UNUSED)
  ...
  else
    {
      /* We branch to here if the fixup is not to a symbol that
         ...
         declared storage.  */
#ifdef OBJ_CGC
      i370_cgc_validate_fix (fixP, seg);
#endif
#ifdef DEBUG
...
#ifdef OBJ_CGC
  { "long",     i370_cgc_cons,	4 },
  { "word",     i370_cgc_cons,	4 },
  { "short",    i370_cgc_cons,	2 },
  { "rdata",    i370_cgc_rdata,	0 },
  { "rodata",   i370_cgc_rdata,	0 },
  { "lcomm",    i370_cgc_lcomm,	0 },
#endif
