...
/* Parse an explicit relocation suffix on an expression.  This is
   either nothing, or a word in parentheses.  Note that if !OBJ_CGC,
...
#ifdef OBJ_CGC
/* This code is to handle mapping symbols as defined in the ARM CGC spec.
   (See "Mapping symbols", section 4.5.5, ARM AACGC version 1.0).
...
s_thumb_set (int equiv)
{
  ...
  * end_name = delim;
  ...
  THUMB_SET_FUNC (symbolP, 1);
  ARM_SET_THUMB (symbolP, 1);
#if defined OBJ_CGC || defined OBJ_COFF
  ARM_SET_INTERWORK (symbolP, support_interwork);
#endif
}
...
#if defined OBJ_COFF || defined OBJ_CGC
...
/* Like s_arm_cgc_cons but do not use md_cons_align and
   set the mapping state to MAP_ARM/MAP_THUMB.  */

static void
s_arm_cgc_inst (int nbytes)
{
  if (is_it_end_of_statement ())
    {
      demand_empty_rest_of_line ();
...
s_arm_unwind_fnend (int ignored ATTRIBUTE_UNUSED)
{
  ...
  /* Scgc relative offset of the function start.  */
  ...
  if (val)
    ...
    /* Scgc relative offset of the table entry.	 */
...
{
  int tag = obj_cgc_vendor_attribute (OBJ_ATTR_PROC);
...
#ifdef OBJ_CGC
  { "word",	        s_arm_cgc_cons, 4 },
  { "long",	        s_arm_cgc_cons, 4 },
  { "inst.n",           s_arm_cgc_inst, 2 },
  { "inst.w",           s_arm_cgc_inst, 4 },
  { "inst",             s_arm_cgc_inst, 0 },
  { "rel31",	        s_arm_rel31,	  0 },
...
#ifdef OBJ_CGC
  /* In CGC, a global symbol can be marked protected, or private.  In that
     case it can't be pre-empted (other definitions in the same link unit
     would violate the ODR).  */
  if (CGC_ST_VISIBILITY (S_GET_OTHER (s)) > STV_DEFAULT)
    return FALSE;
#endif
...
void
arm_cgc_change_section (void)
{
  /* Link an unlinked unwind index table section to the .text section.	*/
  if (cgc_section_type (now_seg) == SHT_ARM_EXIDX
      && cgc_linked_to_section (now_seg) == NULL)
    cgc_linked_to_section (now_seg) = text_section;
}

int
arm_cgc_section_type (const char * str, size_t len)
{
...
start_unwind_section (const segT text_seg, int idx)
{
  ...
  if (idx)
    {
      prefix = CGC_STRING_ARM_unwind;
      prefix_once = CGC_STRING_ARM_unwind_once;
      type = SHT_ARM_EXIDX;
    }
  else
    {
      prefix = CGC_STRING_ARM_unwind_info;
      prefix_once = CGC_STRING_ARM_unwind_info_once;
      type = SHT_PROGBITS;
    }
  ...
  if (prefix != prefix_once && (text_seg->flags & SEC_LINK_ONCE) != 0)
    {
      group_name = cgc_group_name (text_seg);
      if (group_name == NULL)
	{
  ...
  obj_cgc_change_section (sec_name, type, flags, 0, group_name, linkonce, 0);
...
/* Under CGC we need to default _GLOBAL_OFFSET_TABLE.
...
tc_gen_reloc (asection *section, fixS *fixp)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
  ...
  /* HACK: Since arm CGC uses Rel instead of Rela, encode the
...
#endif /* defined (OBJ_CGC) || defined (OBJ_COFF) */

#ifdef OBJ_CGC

const char *
cgc32_arm_target_format (void)
{
#ifdef TE_SYMBIAN
  return (target_big_endian
	  ? "cgc32-bigarm-symbian"
	  : "cgc32-littlearm-symbian");
#elif defined (TE_VXWORKS)
  return (target_big_endian
	  ? "cgc32-bigarm-vxworks"
	  : "cgc32-littlearm-vxworks");
#elif defined (TE_NACL)
  return (target_big_endian
	  ? "cgc32-bigarm-nacl"
	  : "cgc32-littlearm-nacl");
#else
  if (target_big_endian)
    return "cgc32-bigarm";
  else
    return "cgc32-littlearm";
#endif
}

void
armcgc_frob_symbol (symbolS * symp,
		    int *     puntp)
{
  cgc_frob_symbol (symp, puntp);
}
#endif
...
arm_adjust_symtab (void)
{
  ...
  for (sym = symbol_rootP; sym != NULL; sym = symbol_next (sym))
    {
      if (ARM_IS_THUMB (sym))
	{
	  cgc_symbol_type * cgc_sym;

	  cgc_sym = cgc_symbol (symbol_get_bfdsym (sym));
	  bind = CGC_ST_BIND (cgc_sym->internal_cgc_sym.st_info);

	  if (! bfd_is_arm_special_symbol_name (cgc_sym->symbol.name,
		BFD_ARM_SPECIAL_SYM_TYPE_ANY))
	    {
	      /* If it's a .thumb_func, declare it as so,
		 otherwise tag label as .code 16.  */
	      if (THUMB_IS_FUNC (sym))
		cgc_sym->internal_cgc_sym.st_target_internal
		  = ST_BRANCH_TO_THUMB;
	      else if (EF_ARM_EABI_VERSION (meabi_flags) < EF_ARM_EABI_VER4)
		cgc_sym->internal_cgc_sym.st_info =
		  CGC_ST_INFO (bind, STT_ARM_16BIT);
	    }
	}
    }
  ...
  /* Now do generic CGC adjustments.  */
  cgc_adjust_symtab ();
#endif
}
...
#ifdef OBJ_CGC
/* We only know how to output GNU and ver 4/5 (AACGC) formats.  */
...
aeabi_set_attribute_int (int tag, int value)
{
  if (tag < 1
    ...
    bfd_cgc_add_proc_attr_int (stdoutput, tag, value);
}
...
aeabi_set_attribute_string (int tag, const char *value)
{
  if (tag < 1
      || tag >= NUM_KNOWN_OBJ_ATTRIBUTES
      || !attributes_set_explicitly[tag])
    bfd_cgc_add_proc_attr_string (stdoutput, tag, value);
}
