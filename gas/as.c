...
extern struct emulation mipsbcgc, mipslcgc, mipself;
extern struct emulation i386coff, i386cgc, i386aout;
...
Options:\n\
  ...
  --size-check=[error|warning]\n\
			  CGC .size directive check (default --size-check=error)\n"));
#endif
  fprintf (stream, _("\
...
create_obj_attrs_section (void)
{
  ...
  size = bfd_cgc_obj_attr_size (stdoutput);
  if (size)
    {
      name = get_cgc_backend_data (stdoutput)->obj_attrs_section;
      if (!name)
	name = ".gnu.attributes";
      s = subseg_new (name, 0);
      cgc_section_type (s)
	= get_cgc_backend_data (stdoutput)->obj_attrs_section_type;
      bfd_set_section_flags (stdoutput, s, SEC_READONLY | SEC_DATA);
      frag_now_fix ();
      p = frag_more (size);
      bfd_cgc_set_obj_attr_contents (stdoutput, (bfd_byte *)p, size);
    }
}
#endif

