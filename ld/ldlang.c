...
static void lang_finalize_version_expr_head
  (struct bfd_cgc_version_expr_head *);
		  ...
		  lang_output_section_statement_type *output)
{
  ...
  switch (output->sectype)
    ...
    case noload_section:
	 ...
	 meanings to NOLOAD in scripts.  CGC gets a .bss style noload,
	 alloc, no contents section.  All others get a noload, noalloc
	 section.  */
      if (bfd_get_flavour (link_info.output_bfd) == bfd_target_cgc_flavour)
...
closest_target_match (const bfd_target *target, void *data)
{
  ...
  /* Ignore generic big and little endian cgc vectors.  */
  if (strcmp (target->name, "cgc32-big") == 0
      || strcmp (target->name, "cgc64-big") == 0
      || strcmp (target->name, "cgc32-little") == 0
      || strcmp (target->name, "cgc64-little") == 0)
    return 0;
...
/* Callback routine that is used in _bfd_cgc_map_sections_to_segments.
...
lang_process (void)
{
  ...
  /* Do anything special before sizing sections.  This is where CGC
...
/* Record the program header information in the output BFD.  FIXME: We
   should not be calling an CGC specific function here.  */
...

/* Version handling.  This is only useful for CGC.  */

/* If PREV is NULL, return first version pattern matching particular symbol.
   If PREV is non-NULL, return first version pattern matching particular
   symbol after PREV (previously returned by lang_vers_match).  */

static struct bfd_cgc_version_expr *
lang_vers_match (struct bfd_cgc_version_expr_head *head,
		 struct bfd_cgc_version_expr *prev,
		 const char *sym)
{
  const char *c_sym;
  const char *cxx_sym = sym;
  const char *java_sym = sym;
  struct bfd_cgc_version_expr *expr = NULL;
  ...
  if (head->mask & BFD_CGC_VERSION_CXX_TYPE)
    {
  ...
  if (head->mask & BFD_CGC_VERSION_JAVA_TYPE)
    {
  ...
  if (head->htab && (prev == NULL || prev->literal))
    {
      struct bfd_cgc_version_expr e;

      switch (prev ? prev->mask : 0)
	{
	case 0:
	  if (head->mask & BFD_CGC_VERSION_C_TYPE)
	    {
	      e.pattern = c_sym;
	      expr = (struct bfd_cgc_version_expr *)
		  htab_find ((htab_t) head->htab, &e);
	      while (expr && strcmp (expr->pattern, c_sym) == 0)
		if (expr->mask == BFD_CGC_VERSION_C_TYPE)
		  goto out_ret;
		else
		  expr = expr->next;
	    }
	  /* Fallthrough */
	case BFD_CGC_VERSION_C_TYPE:
	  if (head->mask & BFD_CGC_VERSION_CXX_TYPE)
	    {
	      e.pattern = cxx_sym;
	      expr = (struct bfd_cgc_version_expr *)
		...
		if (expr->mask == BFD_CGC_VERSION_CXX_TYPE)
		  goto out_ret;
		else
		  expr = expr->next;
	    }
	  /* Fallthrough */
	case BFD_CGC_VERSION_CXX_TYPE:
	  if (head->mask & BFD_CGC_VERSION_JAVA_TYPE)
	    {
	      e.pattern = java_sym;
	      expr = (struct bfd_cgc_version_expr *)
		...
		if (expr->mask == BFD_CGC_VERSION_JAVA_TYPE)
		  goto out_ret;
		else
		  expr = expr->next;
	    }
  ...
  for (; expr; expr = expr->next)
	...
	break;
      ...
      if (expr->mask == BFD_CGC_VERSION_JAVA_TYPE)
	s = java_sym;
      else if (expr->mask == BFD_CGC_VERSION_CXX_TYPE)
	s = cxx_sym;
      else
	s = c_sym;
...
struct bfd_cgc_version_expr *
lang_new_vers_pattern (struct bfd_cgc_version_expr *orig,
		       const char *new_name,
		       const char *lang,
		       bfd_boolean literal_p)
{
  struct bfd_cgc_version_expr *ret;

  ret = (struct bfd_cgc_version_expr *) xmalloc (sizeof *ret);
  ret->next = orig;
  ret->symver = 0;
  ret->script = 0;
  ret->literal = TRUE;
  ...
  if (lang == NULL || strcasecmp (lang, "C") == 0)
    ret->mask = BFD_CGC_VERSION_C_TYPE;
  else if (strcasecmp (lang, "C++") == 0)
    ret->mask = BFD_CGC_VERSION_CXX_TYPE;
  else if (strcasecmp (lang, "Java") == 0)
    ret->mask = BFD_CGC_VERSION_JAVA_TYPE;
  else
    {
...
lang_new_vers_node (struct bfd_cgc_version_expr *globals,
		    struct bfd_cgc_version_expr *locals)
{
  struct bfd_cgc_version_tree *ret;

  ret = (struct bfd_cgc_version_tree *) xcalloc (1, sizeof *ret);
...
{
  const struct bfd_cgc_version_expr *e =
      (const struct bfd_cgc_version_expr *) p;
...
{
  const struct bfd_cgc_version_expr *e1 =
      (const struct bfd_cgc_version_expr *) p1;
  const struct bfd_cgc_version_expr *e2 =
      (const struct bfd_cgc_version_expr *) p2;

  return strcmp (e1->pattern, e2->pattern) == 0;
}

static void
lang_finalize_version_expr_head (struct bfd_cgc_version_expr_head *head)
{
  size_t count = 0;
  struct bfd_cgc_version_expr *e, *next;
  struct bfd_cgc_version_expr **list_loc, **remaining_loc;
      ...
      for (e = head->list; e; e = next)
	{
	  ...
	  if (!e->literal)
	  ...
	  else
	    {
	      ...
	      if (*loc)
		{
		  struct bfd_cgc_version_expr *e1, *last;

		  e1 = (struct bfd_cgc_version_expr *) *loc;
		  last = NULL;
		  do
		    {
		      if (e1->mask == e->mask)
...
lang_register_vers_node (const char *name,
			 struct bfd_cgc_version_tree *version,
			 struct bfd_cgc_version_deps *deps)
{
  struct bfd_cgc_version_tree *t, **pp;
  struct bfd_cgc_version_expr *e1;
  ...
  for (e1 = version->globals.list; e1 != NULL; e1 = e1->next)
    {
      for (t = link_info.version_info; t != NULL; t = t->next)
	{
	  struct bfd_cgc_version_expr *e2;

	  if (t->locals.htab && e1->literal)
	    {
	      e2 = (struct bfd_cgc_version_expr *)
...
struct bfd_cgc_version_deps *
lang_add_vers_depend (struct bfd_cgc_version_deps *list, const char *name)
{
  struct bfd_cgc_version_deps *ret;
  struct bfd_cgc_version_tree *t;

  ret = (struct bfd_cgc_version_deps *) xmalloc (sizeof *ret);
  ret->next = list;
...
{
  struct bfd_cgc_version_expr *greg = NULL, *lreg;
...
void
lang_append_dynamic_list (struct bfd_cgc_version_expr *dynamic)
{
  if (link_info.dynamic_list)
    {
      struct bfd_cgc_version_expr *tail;
    ...
    {
      struct bfd_cgc_dynamic_list *d;

      d = (struct bfd_cgc_dynamic_list *) xcalloc (1, sizeof *d);
      d->head.list = dynamic;
...
lang_append_dynamic_list_cpp_typeinfo (void)
{
  ...
  struct bfd_cgc_version_expr *dynamic = NULL;
  unsigned int i;
