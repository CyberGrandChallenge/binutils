			    ...
			    const struct ld_plugin_symbol *ldsym)
{
  ...
  switch (ldsym->def)
    ...
    case LDPK_COMMON:
      ...
      /* For CGC targets, set alignment of common symbol to 1.  */
      if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
	{
	  ((cgc_symbol_type *) asym)->internal_cgc_sym.st_shndx = SHN_COMMON;
	  ((cgc_symbol_type *) asym)->internal_cgc_sym.st_value = 1;
	}
      break;
  ...
  /* Visibility only applies on CGC targets.  */
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
    {
      cgc_symbol_type *cgcsym = cgc_symbol_from (abfd, asym);
      unsigned char visibility;

      if (!cgcsym)
	einfo (_("%P%F: %s: non-CGC symbol in CGC BFD!\n"), asym->name);
      switch (ldsym->visibility)
	{
	default:
	  einfo (_("%P%F: unknown CGC symbol visibility: %d!\n"),
		 ldsym->visibility);
	case LDPV_DEFAULT:
	...
	= (visibility | (cgcsym->internal_cgc_sym.st_other
			 & ~CGC_ST_VISIBILITY (-1)));
    }
  ...
  if (link_info.export_dynamic || !link_info.executable)
    {
      ...
      if (bfd_hide_sym_by_version (link_info.version_info,
				   blhe->root.string))
	return FALSE;
      /* Only CGC symbols really have visibility.  */
      if (bfd_get_flavour (link_info.output_bfd) == bfd_target_cgc_flavour)
	{
	  struct cgc_link_hash_entry *el = (struct cgc_link_hash_entry *)blhe;
	  int vis = CGC_ST_VISIBILITY (el->other);
	  return vis == STV_DEFAULT || vis == STV_PROTECTED;
	}
      /* On non-CGC targets, we can safely make inferences by considering
	 what visibility the plugin would have liked to apply when it first
	 sent us the symbol.  During CGC symbol processing, visibility only
