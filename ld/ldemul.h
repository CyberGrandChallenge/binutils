...
extern struct bfd_cgc_version_expr *ldemul_new_vers_pattern
...
typedef struct ld_emulation_xfer_struct {
  ...
  /* Called when adding a new version pattern.  PowerPC64-CGC uses
     this hook to add a pattern matching ".foo" for every "foo".  */
  struct bfd_cgc_version_expr * (*new_vers_pattern)
    (struct bfd_cgc_version_expr *);
