# This variant of cgc.sc is used for ARM BPABI platforms, like Symbian
# OS, where a separate postlinker will operated on the generated
# executable or shared object.  See cgc.sc for configuration variables
...
test -z "${CGCSIZE}" && CGCSIZE=32
test -z "${ALIGNMENT}" && ALIGNMENT="${CGCSIZE} / 8"
...
${RELOCATING+/* Do we need any of these for cgc?
...
cat <<EOF
  ...
  .text         ${RELOCATING-0} :
  {
    ...
    *(.text .stub${RELOCATING+ .text.* .gnu.linkonce.t.*})
    /* .gnu.warning sections are handled specially by cgc32.em.  */
    *(.gnu.warning)
