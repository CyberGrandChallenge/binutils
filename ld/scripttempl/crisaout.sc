...
SECTIONS
{
  .text ${RELOCATING+ ${TEXT_START_ADDR}}:
  {
    ...
    *(.text)
    ...
    /* Cater to linking from CGC.  */
    ${CONSTRUCTING+ PROVIDE(___ctors = .);}
    ${CONSTRUCTING+ ___cgc_ctors_dtors_begin = .;}
    ...
    ${CONSTRUCTING+ ___cgc_ctors_dtors_end = .;}
  ...
  /* Unfortunately, stabs are not mappable from CGC to a.out.
