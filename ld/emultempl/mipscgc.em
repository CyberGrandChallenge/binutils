...
mips_create_output_section_statements (void)
{
  ...
  htab = cgc_hash_table (&link_info);
  if (is_cgc_hash_table (htab) && is_mips_cgc (link_info.output_bfd))
    _bfd_mips_cgc_insn32 (&link_info, insn32);

  if (is_mips_cgc (link_info.output_bfd))
    _bfd_mips_cgc_init_stubs (&link_info, mips_add_stub_section);
}
...
mips_before_allocation (void)
{
  ...
  flags = cgc_cgcheader (link_info.output_bfd)->e_flags;
  if (!link_info.shared
      && !link_info.nocopyreloc
      && (flags & (EF_MIPS_PIC | EF_MIPS_CPIC)) == EF_MIPS_CPIC)
    _bfd_mips_cgc_use_plts_and_copy_relocs (&link_info);
