...
# This file is sourced from cgc32.em, and defines extra arm-cgc
# specific routines.
#
...
static void
arm_cgc_before_allocation (void)
{
  bfd_cgc32_arm_set_byteswap_code (&link_info, byteswap_code);

  /* Choose type of VFP11 erratum fix, or warn if specified fix is unnecessary
     due to architecture version.  */
  bfd_cgc32_arm_set_vfp11_fix (link_info.output_bfd, &link_info);

  /* Auto-select Cortex-A8 erratum fix if it wasn't explicitly specified.  */
  bfd_cgc32_arm_set_cortex_a8_fix (link_info.output_bfd, &link_info);

  /* We should be able to set the size of the interworking stub section.  We
     can't do it until later if we have dynamic sections, though.  */
  if (cgc_hash_table (&link_info)->dynobj == NULL)
    {
      /* Here we rummage through the found bfds to collect glue information.  */
      LANG_FOR_EACH_INPUT_STATEMENT (is)
	{
          /* Initialise mapping tables for code/data.  */
          bfd_cgc32_arm_init_maps (is->the_bfd);

	  if (!bfd_cgc32_arm_process_before_allocation (is->the_bfd,
							&link_info)
	      || !bfd_cgc32_arm_vfp11_erratum_scan (is->the_bfd, &link_info))
	    /* xgettext:c-format */
	    einfo (_("Errors encountered processing file %s"), is->filename);
	}

      /* We have seen it all.  Allocate it, and carry on.  */
      bfd_cgc32_arm_allocate_interworking_sections (& link_info);
    }

  /* Call the standard cgc routine.  */
...
/* Call-back for cgc32_arm_size_stubs.  */

/* Create a new stub section, and arrange for it to be linked
   immediately after INPUT_SECTION.  */

static asection *
cgc32_arm_add_stub_section (const char * stub_sec_name,
			    asection *   input_section,
...
/* Another call-back for cgc_arm_size_stubs.  */
...
build_section_lists (lang_statement_union_type *statement)
{
  if (statement->header.type == lang_input_section_enum)
	...
	cgc32_arm_next_input_section (& link_info, i);
    }
}
...
gld${EMULATION_NAME}_after_allocation (void)
{
  if (!link_info.relocatable)
      ...
      LANG_FOR_EACH_INPUT_STATEMENT (is)
	{
      ...
      if (cgc32_arm_fix_exidx_coverage (sec_list, sec_count, &link_info,
					   merge_exidx_entries))
	need_laying_out = 1;
  ...
  if (stub_file != NULL && !link_info.relocatable)
    {
      int  ret = cgc32_arm_setup_section_lists (link_info.output_bfd, & link_info);
      ...
      if (ret != 0)
	{
	  ...
	  if (! cgc32_arm_size_stubs (link_info.output_bfd,
				      stub_file->the_bfd,
				      & link_info,
				      group_size,
				      & cgc32_arm_add_stub_section,
...
gld${EMULATION_NAME}_finish (void)
{
    ...
    LANG_FOR_EACH_INPUT_STATEMENT (is)
      {
        ...
        bfd_cgc32_arm_vfp11_fix_veneer_locations (is->the_bfd, &link_info);
      }
  }
  ...
  if (! link_info.relocatable)
	...
	{
	  if (! cgc32_arm_build_stubs (& link_info))
  ...
  if (thumb_entry_symbol)
	...
	return;
      ...
      h = bfd_link_hash_lookup (link_info.hash, entry_symbol.name,
				FALSE, FALSE, TRUE);
      eh = (struct cgc_link_hash_entry *)h;
...
static void
arm_cgc_create_output_section_statements (void)
{
  ...
  bfd_cgc32_arm_set_target_relocs (link_info.output_bfd, &link_info,
				   target1_is_rel,
  ...
  bfd_cgc32_arm_add_glue_sections_to_bfd (stub_file->the_bfd, &link_info);
  bfd_cgc32_arm_get_bfd_for_interworking (stub_file->the_bfd, &link_info);
}
...
# Define some shell vars to insert bits of code into the standard cgc
...
LDEMUL_BEFORE_ALLOCATION=arm_cgc_before_allocation
LDEMUL_AFTER_ALLOCATION=gld${EMULATION_NAME}_after_allocation
LDEMUL_CREATE_OUTPUT_SECTION_STATEMENTS=arm_cgc_create_output_section_statements

# Replace the cgc before_parse function with our own.
LDEMUL_BEFORE_PARSE=gld"${EMULATION_NAME}"_before_parse

# Call the extra arm-cgc function
