...
# This file is sourced from cgc32.em, and defines extra sh64
# specific routines.
#

LDEMUL_AFTER_ALLOCATION=sh64_cgc_${EMULATION_NAME}_after_allocation
LDEMUL_BEFORE_ALLOCATION=sh64_cgc_${EMULATION_NAME}_before_allocation
...
/* Check if we need a .cranges section and create it if it's not in any
   ...
   Putting it here instead of as a cgc_backend_always_size_sections hook
   in cgc32-sh64.c, means that we have access to linker command line
   options here, and we can access input sections in the order in which
   they will be linked.  */

static void
sh64_cgc_${EMULATION_NAME}_before_allocation (void)
{
  asection *cranges;
  asection *osec;
  ...
  if (RELAXATION_ENABLED)
    {
      LANG_FOR_EACH_INPUT_STATEMENT (f)
	{
	  if (bfd_get_flavour (f->the_bfd) == bfd_target_cgc_flavour)
	    {
	      asection *isec;
 ...
 done_scanning_shmedia_sections:
  ...
  for (osec = link_info.output_bfd->sections;
	...
	einfo (_("%FError: non-CGC output formats are not supported by this target's linker.\n"));

      sh64_sec_data = sh64_cgc_section_data (osec)->sh64_info;
			...
			if (cranges == NULL)
			  {
			    /* This section will be *appended* to
			       ...
			       sh64_cgc_section_data; no need to set it
			       specifically here.  */
			    cranges
...
static void
sh64_cgc_${EMULATION_NAME}_after_allocation (void)
{
		...
		if (isec->output_section == osec
			...
			& SEC_EXCLUDE) == 0)
		  {
		    ...
		    /* If flags don't agree, set the target-specific data
		       ...
		       bother setting CGC section flags in output section;
			...
			BFD_ASSERT (sh64_cgc_section_data (osec)->sh64_info);

			sh64_cgc_section_data (osec)->sh64_info->contents_flags
			  = SHF_SH5_ISA32_MIXED;
  ...
  BFD_ASSERT (sh64_cgc_section_data (cranges)->sh64_info != NULL);
  ...
  for (osec = link_info.output_bfd->sections;
       ...
       osec = osec->next)
    {
      ...
      enum sh64_cgc_cr_type last_cr_type = CRT_NONE;
  ...
  /* The .cranges section will have this size, no larger or smaller.
     ...
     Sorting before writing is done by sh64_cgc_final_write_processing.  */

  sh64_cgc_section_data (cranges)->sh64_info->cranges_growth
    = crangesp - cranges->contents - cranges->size;
