...
# This file is sourced from cgc32.em, and defines extra powerpc64-cgc
# specific routines.
#
fragment <<EOF
...
ppc_create_output_section_statements (void)
{
  if (!(bfd_get_flavour (link_info.output_bfd) == bfd_target_cgc_flavour
	&& cgc_object_id (link_info.output_bfd) == PPC64_CGC_DATA))
    return;
  ...
  if (!ppc64_cgc_init_stub_bfd (stub_file->the_bfd, &link_info))
		   ...
		   lang_statement_union_type **small)
{
  ...
  while ((s = *u) != NULL)
	...
	case lang_input_section_enum:
	  ...
	  if (ppc64_cgc_has_small_toc_reloc (i))
	    {
...
ppc_before_allocation (void)
{
  if (stub_file != NULL)
    {
      if (!no_opd_opt
	  && !ppc64_cgc_edit_opd (&link_info, non_overlapping_opd))
	einfo ("%X%P: can not edit %s: %E\n", "opd");

      if (ppc64_cgc_tls_setup (&link_info, no_tls_get_addr_opt, &no_multi_toc)
	  && !no_tls_opt)
	{
	  ...
	  if (!ppc64_cgc_tls_optimize (&link_info))
	  ...
	  && !link_info.relocatable)
	{
	  ...
	  if (!ppc64_cgc_edit_toc (&link_info))
	    einfo ("%X%P: can not edit %s: %E\n", "toc");
	}
...
/* Call-back for ppc64_cgc_size_stubs.  */
...
/* Another call-back for ppc64_cgc_size_stubs.  */
...
ppc_layout_sections_again (void)
{
  ...
  if (!link_info.relocatable)
    ppc64_cgc_set_toc (&link_info, link_info.output_bfd);
...
build_toc_list (lang_statement_union_type *statement)
{
  if (statement->header.type == lang_input_section_enum)
	...
	{
	  if (!ppc64_cgc_next_toc_section (&link_info, i))
...
build_section_lists (lang_statement_union_type *statement)
{
  if (statement->header.type == lang_input_section_enum)
	...
	{
	  if (!ppc64_cgc_next_input_section (&link_info, i))
...
{
  /* bfd_cgc_discard_info just plays with data and debugging sections,
  ...
  if (stub_file != NULL && !link_info.relocatable)
    {
      int ret = ppc64_cgc_setup_section_lists (&link_info,
	...
	{
	  ppc64_cgc_start_multitoc_partition (&link_info);
	  ...
	  if (ppc64_cgc_layout_multitoc (&link_info)
	      && !no_multi_toc
	      && toc_section != NULL)
	    lang_for_each_statement (build_toc_list);

	  ppc64_cgc_finish_multitoc_partition (&link_info);

	  lang_for_each_statement (build_section_lists);

	  if (!ppc64_cgc_check_init_fini (&link_info))
	    einfo ("%P: .init/.fini fragments use differing TOC pointers\n");

	  /* Call into the BFD backend to do the real work.  */
	  if (!ppc64_cgc_size_stubs (&link_info, group_size,
...
gld${EMULATION_NAME}_finish (void)
{
  ...
  if (stub_added)
    {
      ...
      if (emit_stub_syms < 0)
	emit_stub_syms = 1;
      if (!ppc64_cgc_build_stubs (emit_stub_syms, &link_info,
  ...
  ppc64_cgc_restore_symbols (&link_info);
  finish_default ();
}
...
gld${EMULATION_NAME}_new_vers_pattern (struct bfd_cgc_version_expr *entry)
{
  struct bfd_cgc_version_expr *dot_entry;
  unsigned int len;
  char *dot_pat;
...
# Put these extra ppc64cgc routines in ld_${EMULATION_NAME}_emulation
#
