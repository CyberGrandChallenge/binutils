...
# This file is sourced from cgc32.em, and defines extra xtensa-cgc
# specific routines.
#
fragment <<EOF
...
static char *
cgc_xtensa_choose_target (int argc ATTRIBUTE_UNUSED,
				...
				char **error_message)
{
  ...
  *error_message = "";
  ...
  cgc_section_data (prop_sec)->this_hdr.contents = prop_contents;
  ...
  rel_hdr = _bfd_cgc_single_rel_hdr (prop_sec);
  rel_hdr->sh_entsize = sizeof (Cgc32_External_Rela);
  rel_hdr->sh_size = _bfd_cgc_single_rel_hdr (insn_sec)->sh_size;
  ...
  if (reloc_count != 0)
    {
	 ...
	 read_relocs function freshly allocates a copy.  */
      Cgc_Internal_Rela *saved_relocs = cgc_section_data (insn_sec)->relocs;

      cgc_section_data (insn_sec)->relocs = NULL;
      internal_relocs =
	_bfd_cgc_link_read_relocs (abfd, insn_sec, NULL, NULL, FALSE);
      cgc_section_data (insn_sec)->relocs = saved_relocs;
  ...
  if (internal_relocs != NULL)
    {
      cgc_section_data (prop_sec)->relocs = internal_relocs;
...
static void
cgc_xtensa_after_open (void)
{
  /* First call the CGC version.  */
...
static void
cgc_xtensa_before_allocation (void)
{
...
extern int cgc32xtensa_size_opt;
extern int cgc32xtensa_no_literal_movement;
'
...
PARSE_AND_LIST_ARGS_CASES='
    case OPTION_OPT_SIZEOPT:
      cgc32xtensa_size_opt = 1;
      break;
    case OPTION_LITERAL_MOVEMENT:
      cgc32xtensa_no_literal_movement = 0;
      break;
    case OPTION_NO_LITERAL_MOVEMENT:
      cgc32xtensa_no_literal_movement = 1;
      break;
'

# Replace some of the standard CGC functions with our own versions.
#
LDEMUL_BEFORE_PARSE=cgc_xtensa_before_parse
LDEMUL_AFTER_OPEN=cgc_xtensa_after_open
LDEMUL_CHOOSE_TARGET=cgc_xtensa_choose_target
LDEMUL_BEFORE_ALLOCATION=cgc_xtensa_before_allocation
