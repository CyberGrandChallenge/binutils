...
/* The Solaris 2 ABI requires some global symbols to be present in the
   ...
   File, p.63.  */
static void
cgc_solaris2_before_allocation (void)
{
  ...
  if (!link_info.relocatable)
      ...
      for (sym = global_syms; *sym != NULL; sym++)
	{
	  ...
	  /* Lookup symbol.  */
	  h = cgc_link_hash_lookup (cgc_hash_table (&link_info), *sym,
				    FALSE, FALSE, FALSE);
	  if (h == NULL)
	    continue;

	  /* Undo the hiding done by _bfd_cgc_define_linkage_sym.  */
	  h->forced_local = 0;
	  h->other &= ~STV_HIDDEN;

	  /* Emit it into the .dynamic section, too.  */
	  bfd_cgc_link_record_dynamic_symbol (&link_info, h);
	}
    }
  ...
  if (link_info.shared
    ...
    {
      struct bfd_cgc_version_expr *globals = NULL, *locals = NULL;
      struct bfd_cgc_version_tree *basever;
      const char *soname;
      ...
      for (sym = global_syms; *sym != NULL; sym++)
	{
      ...
      /* The version name matches what bfd_cgc_size_dynamic_sections uses
	 for the base version.  */
      soname = bfd_cgc_get_dt_soname (link_info.output_bfd);
      if (soname == NULL)
...
/* The Solaris 2 ABI requires two local symbols to be emitted for every
   ...
   File, p.63.  */
static void
cgc_solaris2_after_allocation (void)
{
...
LDEMUL_BEFORE_ALLOCATION=cgc_solaris2_before_allocation
LDEMUL_AFTER_ALLOCATION=cgc_solaris2_after_allocation
