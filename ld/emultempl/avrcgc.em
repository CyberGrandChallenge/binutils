...
# This file is sourced from cgc32.em, and defines extra avr-cgc specific
...
static void
avr_cgc_set_global_bfd_parameters (void)
{
  cgc32_avr_setup_params (& link_info,
                          stub_file->the_bfd,
                          avr_stub_section,
                          avr_no_stubs,
                          avr_debug_stubs,
...
static void
avr_cgc_${EMULATION_NAME}_before_allocation (void)
{
  int ret;
  ...
  avr_cgc_set_global_bfd_parameters ();
  ...
  ret = cgc32_avr_setup_section_lists (link_info.output_bfd, &link_info);
  ...
  if (! cgc32_avr_size_stubs (link_info.output_bfd, &link_info, TRUE))
...
static void
avr_cgc_create_output_section_statements (void)
{
  flagword flags;
...
static void
avr_cgc_after_allocation (void)
{
  if (!avr_no_stubs && ! RELAXATION_ENABLED)
    {
      /* If relaxing, cgc32_avr_size_stubs will be called from
	 cgc32_avr_relax_section.  */
      if (!cgc32_avr_size_stubs (link_info.output_bfd, &link_info, TRUE))
	einfo ("%X%P: can not size stub section: %E\n");
    }
  ...
  if (!avr_no_stubs)
    {
      if (!cgc32_avr_build_stubs (&link_info))
	einfo ("%X%P: can not build stubs: %E\n");
    }
}

static void
avr_cgc_before_parse (void)
{
...
#
# Put these extra avr-cgc routines in ld_${EMULATION_NAME}_emulation
#
LDEMUL_BEFORE_PARSE=avr_cgc_before_parse
LDEMUL_BEFORE_ALLOCATION=avr_cgc_${EMULATION_NAME}_before_allocation
LDEMUL_AFTER_ALLOCATION=avr_cgc_after_allocation
LDEMUL_CREATE_OUTPUT_SECTION_STATEMENTS=avr_cgc_create_output_section_statements
