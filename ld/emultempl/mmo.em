...
fragment <<EOF
/* Need to have this macro defined before mmix-cgcnmmo, which uses the
   name for the before_allocation function, defined in ldemul.c (for
   the mmo "emulation") or in cgc32.em (for the cgc64mmix
   "emulation").  */
#define gldmmo_before_allocation before_allocation_default

/* We include this header *not* because we expect to handle CGC here
   but because we re-use the map_segments function in cgc-generic.em,
   a file which is rightly somewhat CGC-centric.  But this is only to
   get a weird testcase right; ld-mmix/bpo-22, forcing CGC to be
   output from the mmo emulation: -m mmo --oformat cgc64-mmix!  */
#include "cgc-bfd.h"

static void gld${EMULATION_NAME}_after_allocation (void);
EOF

source_em ${srcdir}/emultempl/cgc-generic.em
source_em ${srcdir}/emultempl/mmix-cgcnmmo.em
		  ...
		  int constraint ATTRIBUTE_UNUSED)
{
  ...
  /* If there's an output section by *this* name, we'll use it, regardless
     of actual section flags, in contrast to what's done in cgc32.em.  */
...
/* Remove the spurious settings of SEC_RELOC that make it to the output at
   link time.  We are as confused as cgclink.h:cgc_bfd_final_link, and
...
/* To get on-demand global register allocation right, we need to parse the
   relocs, like what happens when linking to CGC.  It needs to be done
   before all input sections are supposed to be present.  When linking to
   CGC, it's done when reading symbols.  When linking to mmo, we do it
...
mmo_after_open (void)
{
  /* When there's a mismatch between the output format and the emulation
     (using weird combinations like "-m mmo --oformat cgc64-mmix" for
     example), we'd count relocs twice because they'd also be counted
     along the usual route for CGC-only linking, which would lead to an
     internal accounting error.  */
  if (bfd_get_flavour (link_info.output_bfd) != bfd_target_cgc_flavour)
    {
      LANG_FOR_EACH_INPUT_STATEMENT (is)
	{
	  if (bfd_get_flavour (is->the_bfd) == bfd_target_cgc_flavour
