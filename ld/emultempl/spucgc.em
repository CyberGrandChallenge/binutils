...
# This file is sourced from cgc32.em, and defines extra spu specific
# features.
#
fragment <<EOF
#include "ldctor.h"
...
static bfd_size_type spu_cgc_load_ovl_mgr (void);
static FILE *spu_cgc_open_overlay_script (void);
static void spu_cgc_relink (void);

static struct spu_cgc_params params =
{
  &spu_place_special_section,
  &spu_cgc_load_ovl_mgr,
  &spu_cgc_open_overlay_script,
  &spu_cgc_relink,
...
is_spu_target (void)
{
  ...
  return link_info.output_bfd->xvec == &bfd_cgc32_spu_vec;
}
...
spu_after_open (void)
{
  if (is_spu_target ())
	...
	params.auto_overlay = 0;
      ...
      spu_cgc_setup (&link_info, &params);
	...
	lang_add_unique (".text.ia.*");
	  ...
	  && !spu_cgc_create_sections (&link_info))
...
spu_cgc_load_ovl_mgr (void)
{
  ...
  h = cgc_link_hash_lookup (cgc_hash_table (&link_info),
  ...
  if (h != NULL
  ...
  else
    {
      ...
      if (!spu_cgc_open_builtin_lib (&ovl_is->the_bfd, mgr_stream))
...
spu_before_allocation (void)
{
  if (is_spu_target ()
      ...
      && !no_overlays)
    {
	 ...
	 rough layout so that overlays can be found.  */
      ...
      ret = spu_cgc_find_overlays (&link_info);
      if (ret == 0)
      ...
      else if (ret == 2)
	{
	  ...
	  for (os = &lang_output_section_statement.head->output_section_statement;
		...
		&& spu_cgc_section_data (os->bfd_section) != NULL
		&& spu_cgc_section_data (os->bfd_section)->u.o.ovl_index != 0)
	      {
	  ...
	  ret = spu_cgc_size_stubs (&link_info);
	  if (ret == 0)
	    einfo ("%X%P: can not size overlay stubs: %E\n");
	  else if (ret == 2)
	    spu_cgc_load_ovl_mgr ();

	  spu_cgc_place_overlay_data (&link_info);
	}
  ...
  if (is_spu_target ()
      && !link_info.relocatable)
    spu_cgc_size_sections (link_info.output_bfd, &link_info);
...
static FILE *
spu_cgc_open_overlay_script (void)
{
  FILE *script = NULL;
...
if grep -q 'ld_cgc.*ppc.*_emulation' ldemul-list.h; then
  fragment <<EOF
#include <errno.h>
#include "filenames.h"
#include "libiberty.h"
...
embedded_spu_file (lang_input_statement_type *entry, const char *flags)
{
  ...
  if (entry->the_bfd->format != bfd_object
      || strcmp (entry->the_bfd->xvec->name, "cgc32-spu") != 0
      || (entry->the_bfd->tdata.cgc_obj_data->cgc_header->e_type != ET_EXEC
	  && entry->the_bfd->tdata.cgc_obj_data->cgc_header->e_type != ET_DYN))
    return FALSE;
