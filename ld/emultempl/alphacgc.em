...
# This file is sourced from cgc32.em, and defines extra alpha
# specific routines.
#
fragment <<EOF
...
extern bfd_boolean cgc64_alpha_use_secureplt;
...
alpha_after_open (void)
{
  if (bfd_get_flavour (link_info.output_bfd) == bfd_target_cgc_flavour
      && cgc_object_id (link_info.output_bfd) == ALPHA_CGC_DATA)
    {
	   ...
	   os = os->next)
	{
      ...
      if (num_plt == 2)
	{
	  plt_os[0]->constraint = cgc64_alpha_use_secureplt ? 0 : -1;
	  plt_os[1]->constraint = cgc64_alpha_use_secureplt ? -1 : 0;
	}
    }
...
{
  if (limit_32bit)
    cgc_cgcheader (link_info.output_bfd)->e_flags |= EF_ALPHA_32BIT;
...
PARSE_AND_LIST_ARGS_CASES='
    ...
    case OPTION_SECUREPLT:
      cgc64_alpha_use_secureplt = TRUE;
      break;
    case OPTION_NO_SECUREPLT:
      cgc64_alpha_use_secureplt = FALSE;
      break;
'
