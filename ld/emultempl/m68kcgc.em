...
#   Written by Michael Sokolov <msokolov@ivan.Harhan.ORG>, based on armcgc.em
#
...
# This file is sourced from cgc32.em, and defines some extra routines for m68k
# embedded systems using CGC and for some other systems using m68k CGC.  While
# it is sourced from cgc32.em for all m68k CGC configurations, here we include
  ...
  if (command_line.embedded_relocs
      ...
      for (abfd = link_info.input_bfds; abfd != NULL; abfd = abfd->link_next)
	{
	  ...
	  /* As first-order business, make sure that each input BFD is either
	     COFF or CGC.  We need to call a special BFD backend function to
	     generate the embedded relocs, and we have such functions only for
	     COFF and CGC.  */
	  if (bfd_get_flavour (abfd) != bfd_target_coff_flavour
	      && bfd_get_flavour (abfd) != bfd_target_cgc_flavour)
	    einfo ("%F%B: all input objects must be COFF or CGC for --embedded-relocs\n");
...
static void
m68k_cgc_after_allocation (void)
{
  /* Call the standard cgc routine.  */
  ...
  if (command_line.embedded_relocs
      ...
      for (abfd = link_info.input_bfds; abfd != NULL; abfd = abfd->link_next)
	{
	  ...
	  else if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
	    {
	      if (! bfd_m68k_cgc32_create_embedded_relocs (abfd, &link_info,
							   datasec, relsec,
							   &errmsg))
		{
		  if (errmsg == NULL)
...
static void
cgc_m68k_create_output_section_statements (void)
{
  bfd_cgc_m68k_set_target_options (&link_info, got_handling);
}
...
LDEMUL_AFTER_OPEN=m68k_cgc_after_open
LDEMUL_AFTER_ALLOCATION=m68k_cgc_after_allocation
LDEMUL_CREATE_OUTPUT_SECTION_STATEMENTS=cgc_m68k_create_output_section_statements
