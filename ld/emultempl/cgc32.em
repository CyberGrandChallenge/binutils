...
/* ${CGCSIZE} bit CGC emulation code for ${EMULATION_NAME}
   Copyright 1991, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001,
   2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013
   Free Software Foundation, Inc.
   Written by Steve Chamberlain <sac@cygnus.com>
   CGC support by Ian Lance Taylor <ian@cygnus.com>
...
gld${EMULATION_NAME}_load_symbols (lang_input_statement_type *entry)
{
  ...
  /* Tell the CGC linker that we don't want the output file to have a
  ...
  bfd_cgc_set_dyn_lib_class (entry->the_bfd,
...
gld${EMULATION_NAME}_vercheck (lang_input_statement_type *s)
{
  ...
  soname = bfd_cgc_get_dt_soname (s->the_bfd);
  if (soname == NULL)
...
gld${EMULATION_NAME}_stat_needed (lang_input_statement_type *s)
{
  ...
  if ((bfd_cgc_get_dyn_lib_class (s->the_bfd) & DYN_AS_NEEDED) != 0)
    return;
  ...
  if (! force)
    {
      ...
      if (! bfd_cgc_get_bfd_needed_list (abfd, &needs))
	einfo ("%F%P:%B: bfd_cgc_get_bfd_needed_list failed: %E\n", abfd);
...
fragment <<EOF
  ...
  bfd_cgc_set_dt_needed_name (abfd, soname);
  ...
  if (needed->by != NULL
      && (bfd_cgc_get_dyn_lib_class (needed->by) & DYN_NO_ADD_NEEDED) != 0)
    link_class |= DYN_NO_NEEDED | DYN_NO_ADD_NEEDED;

  bfd_cgc_set_dyn_lib_class (abfd, (enum dynamic_lib_link_class) link_class);
...
#ifdef HAVE_CGC_HINTS_H
#include <cgc-hints.h>
#else
#include "cgc-hints-local.h"
#endif

static bfd_boolean
gld${EMULATION_NAME}_check_ld_cgc_hints (const struct bfd_link_needed_list *l,
					 int force)
{
  static bfd_boolean initialized;
  static char *ld_cgc_hints;
  struct dt_needed needed;
  ...
  if (!initialized)
    {
      ...
      tmppath = concat (ld_sysroot, _PATH_CGC_HINTS, (const char *) NULL);
      f = fopen (tmppath, FOPEN_RB);
      free (tmppath);
      if (f != NULL)
	{
	  struct cgchints_hdr hdr;

	  if (fread (&hdr, 1, sizeof (hdr), f) == sizeof (hdr)
	      && hdr.magic == CGCHINTS_MAGIC
	      && hdr.version == 1)
	    {
	      if (fseek (f, hdr.strtab + hdr.dirlist, SEEK_SET) != -1)
		{
		  ...
		  if (fread (b, 1, hdr.dirlistlen + 1, f) ==
		      hdr.dirlistlen + 1)
		    ld_cgc_hints = gld${EMULATION_NAME}_add_sysroot (b);
  ...
  if (ld_cgc_hints == NULL)
    return FALSE;

  needed.by = l->by;
  needed.name = l->name;
  return gld${EMULATION_NAME}_search_needed (ld_cgc_hints, &needed, force);
}
EOF
    # FreeBSD
    ;;
...
gld${EMULATION_NAME}_check_needed (lang_input_statement_type *s)
{
  ...
  if (global_found != NULL
      && (bfd_cgc_get_dyn_lib_class (global_found->the_bfd)
	  & DYN_AS_NEEDED) == 0)
    return;
  ...
  if (global_found != NULL
      && (bfd_cgc_get_dyn_lib_class (s->the_bfd) & DYN_AS_NEEDED) != 0)
    return;
...
id_note_section_size (bfd *abfd ATTRIBUTE_UNUSED)
{
  ...
  size = offsetof (Cgc_External_Note, name[sizeof "GNU"]);
...
write_build_id (bfd *abfd)
{
  ...
  i_shdr = &cgc_section_data (asec->output_section)->this_hdr;
  ...
  e_note = (Cgc_External_Note *) contents;
  size = offsetof (Cgc_External_Note, name[sizeof "GNU"]);
  size = (size + 3) & -(bfd_size_type) 4;
...
/* Make .note.gnu.build-id section, and set up cgc_tdata->build_id.  */
...
setup_build_id (bfd *ibfd)
{
  ...
  if (s != NULL && bfd_set_section_alignment (ibfd, s, 2))
    {
      struct cgc_obj_tdata *t = cgc_tdata (link_info.output_bfd);
...
gld${EMULATION_NAME}_after_open (void)
{
  ...
  if (emit_note_gnu_build_id != NULL)
	...
	if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
      ...
      /* PR 10555: If there are no CGC input files do not try to
  ...
  if (link_info.eh_frame_hdr
      ...
      if (cgcbfd)
	{
	  ...
	  bed = get_cgc_backend_data (cgcbfd);
	  s = bfd_make_section_with_flags (cgcbfd, ".eh_frame_hdr",
					   bed->dynamic_sec_flags
					   | SEC_READONLY);
	  if (s != NULL
	      && bfd_set_section_alignment (cgcbfd, s, 2))
	    {
  ...
  /* Get the list of files which appear in DT_NEEDED entries in
     ...
     loop.  */
  needed = bfd_cgc_get_needed_list (link_info.output_bfd, &link_info);
  for (l = needed; l != NULL; l = l->next)
    {
	 ...
	 found to be needed, then this lib isn't needed either.  */
      if (l->by != NULL
	  && (bfd_cgc_get_dyn_lib_class (l->by) & DYN_AS_NEEDED) != 0)
	continue;
	 ...
	 --allow-shlib-undefined is in effect.  */
	  ...
	  && (bfd_cgc_get_dyn_lib_class (l->by) & DYN_NO_ADD_NEEDED) != 0)
	continue;

      /* If we've already seen this file, skip it.  */
      for (ll = needed; ll != l; ll = ll->next)
	if ((ll->by == NULL
	     || (bfd_cgc_get_dyn_lib_class (ll->by) & DYN_AS_NEEDED) == 0)
...
fragment <<EOF
	  found = 0;
	  rp = bfd_cgc_get_runpath_list (link_info.output_bfd, &link_info);
...
if [ "x${USE_LIBPATH}" = xyes ] ; then
  case ${target} in
    *-*-freebsd* | *-*-dragonfly*)
      fragment <<EOF
	  if (gld${EMULATION_NAME}_check_ld_cgc_hints (l, force))
	    break;
EOF
    # FreeBSD
    ;;
...
gld${EMULATION_NAME}_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
	...
	{
	  if (!bfd_cgc_record_link_assignment (link_info.output_bfd,
					       &link_info,
...
/* This is called by the before_allocation routine via
   ...
   tells the CGC backend about them, in case they are assignments to
...
if test x"$LDEMUL_BEFORE_ALLOCATION" != xgld"$EMULATION_NAME"_before_allocation; then
  if test x"${CGC_INTERPRETER_NAME+set}" = xset; then
    CGC_INTERPRETER_SET_DEFAULT="
  if (sinterp != NULL)
    {
      sinterp->contents = (unsigned char *) ${CGC_INTERPRETER_NAME};
...
gld${EMULATION_NAME}_before_allocation (void)
{
  ...
  if (is_cgc_hash_table (link_info.hash))
    {
      _bfd_cgc_tls_setup (link_info.output_bfd, &link_info);
      ...
      if (!link_info.relocatable)
       {
         ...
         if (h != NULL
           ...
           {
             _bfd_cgc_link_hash_hide_symbol (&link_info, h, TRUE);
	 ...
	 let the CGC backend know about them in case the variables are
	 referred to by dynamic objects.  */
      lang_for_each_statement (gld${EMULATION_NAME}_find_statement_assignment);
    }

  /* Let the CGC backend work out the sizes of any sections required
     by dynamic linking.  */
  ...
  for (abfd = link_info.input_bfds; abfd; abfd = abfd->link_next)
    if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
      {
	const char *audit_libs = cgc_dt_audit (abfd);
  ...
  if (! (bfd_cgc_size_dynamic_sections
...
${CGC_INTERPRETER_SET_DEFAULT}
  ...
  if (!bfd_cgc_size_dynsym_hash_dynstr (link_info.output_bfd, &link_info))
    einfo ("%P%F: failed to set dynamic section sizes: %E\n");
}
...
/* Try to open a dynamic archive.  This is where we know that CGC
  ...
  (const char *arch, search_dirs_type *search, lang_input_statement_type *entry)
{
  ...
  /* We have found a dynamic object to include in the link.  The CGC
     backend linker will create a DT_NEEDED entry in the .dynamic
     section naming this file.  If this file includes a DT_SONAME
     entry, it will be used.  Otherwise, the CGC linker will just use
     ...
     FIXME: This approach--using bfd_cgc_set_dt_needed_name--is not
  ...
  if (bfd_check_format (entry->the_bfd, bfd_object)
      && (entry->the_bfd->flags & DYNAMIC) != 0)
    {
	 ...
	 filename we recorded earlier.  */
      ...
      bfd_cgc_set_dt_needed_name (entry->the_bfd, filename);
    }
				   ...
				   int constraint)
{
  ...
  int isdyn = 0;
  int iself = s->owner->xvec->flavour == bfd_target_cgc_flavour;
  unsigned int sh_type = iself ? cgc_section_type (s) : SHT_NULL;
  ...
  if (constraint == 0)
	...
	if (os->bfd_section != NULL
		...
		|| (_bfd_cgc_match_sections_by_type (link_info.output_bfd,
						     os->bfd_section,
						     s->owner, s)
  ...
  if (place != NULL)
	...
	after = lang_output_section_find_by_flags
	  (s, &place->os, _bfd_cgc_match_sections_by_type);
      if (after == NULL)
