...
# This file is sourced from cgc32.em, and defines extra powerpc32-cgc
# specific routines.
#
fragment <<EOF
...
static enum ppc_cgc_plt_type plt_style = PLT_UNSET;
static int old_got = 0;

static void
ppc_after_open (void)
{
  if (is_ppc_cgc (link_info.output_bfd))
    {
      int new_plt;
      int keep_new;
	...
	emit_stub_syms = link_info.emitrelocations || link_info.shared;
      new_plt = ppc_cgc_select_plt_layout (link_info.output_bfd, &link_info,
...
ppc_before_allocation (void)
{
  if (is_ppc_cgc (link_info.output_bfd))
    {
      if (ppc_cgc_tls_setup (link_info.output_bfd, &link_info,
			     no_tls_get_addr_opt)
	  && !notlsopt)
	{
	  if (!ppc_cgc_tls_optimize (link_info.output_bfd, &link_info))
	    {
...
# Put these extra ppc32cgc routines in ld_${EMULATION_NAME}_emulation
#
