...
if test "$OUTPUT_FORMAT" = "cgc64-ia64-vms"; then
...
gld${EMULATION_NAME}_before_allocation (void)
{
  ...
  bed = get_cgc_backend_data (link_info.output_bfd);

  /* The backend must work out the sizes of all the other dynamic
     sections.  */
  if (cgc_hash_table (&link_info)->dynamic_sections_created
      && bed->cgc_backend_size_dynamic_sections
      && ! (*bed->cgc_backend_size_dynamic_sections) (link_info.output_bfd,
                                                      &link_info))
...
{
  bfd_boolean need_layout = bfd_cgc_discard_info (link_info.output_bfd,
						  &link_info);
