// cgccpp.h -- main header file for cgccpp    -*- C++ -*-
...
// This is the external interface for cgccpp.

#ifndef CGCCPP_H
#define CGCCPP_H

#include "cgccpp_swap.h"
...
typedef uint16_t Cgc_Half;
typedef uint32_t Cgc_Word;
typedef int32_t Cgc_Sword;
typedef uint64_t Cgc_Xword;
typedef int64_t Cgc_Sxword;

// These types vary in size depending on the CGC file class.  The
...
struct Cgc_types<32>
{
  typedef uint32_t Cgc_Addr;
  typedef uint32_t Cgc_Off;
  typedef uint32_t Cgc_WXword;
  typedef int32_t Cgc_Swxword;
};

template<>
struct Cgc_types<64>
{
  typedef uint64_t Cgc_Addr;
  typedef uint64_t Cgc_Off;
  typedef uint64_t Cgc_WXword;
  typedef int64_t Cgc_Swxword;
};
...
enum CGCOSABI
{
  ...
  CGCOSABI_NETBSD = 2,
  CGCOSABI_GNU = 3,
  // CGCOSABI_LINUX is an alias for CGCOSABI_GNU.
  CGCOSABI_LINUX = 3,
  CGCOSABI_SOLARIS = 6,
  CGCOSABI_AIX = 7,
  CGCOSABI_IRIX = 8,
...
enum STT
{
  ...
  // ARM: a THUMB function.  This is not defined in ARM CGC Specification but
  // used by the GNU tool-chain.
  STT_ARM_TFUNC = 13
};

inline STB
cgc_st_bind(unsigned char info)
{
  return static_cast<STB>(info >> 4);
}

inline STT
cgc_st_type(unsigned char info)
{
  return static_cast<STT>(info & 0xf);
}

inline unsigned char
cgc_st_info(STB bind, STT type)
{
...
inline STV
cgc_st_visibility(unsigned char other)
{
  return static_cast<STV>(other & 0x3);
}

inline unsigned char
cgc_st_nonvis(unsigned char other)
{
  return static_cast<STV>(other >> 2);
}

inline unsigned char
cgc_st_other(STV vis, unsigned char nonvis)
{
  return ((nonvis << 2)
...
template<int size>
unsigned int
cgc_r_sym(typename Cgc_types<size>::Cgc_WXword);

template<>
inline unsigned int
cgc_r_sym<32>(Cgc_Word v)
{
  return v >> 8;
}
...
template<int size>
unsigned int
cgc_r_type(typename Cgc_types<size>::Cgc_WXword);

template<>
inline unsigned int
cgc_r_type<32>(Cgc_Word v)
{
  return v & 0xff;
}
...
cgc_r_info(unsigned int s, unsigned int t);

template<>
inline Cgc_Word
cgc_r_info<32>(unsigned int s, unsigned int t)
{
  return (s << 8) + (t & 0xff);
}
...
template<>
inline Cgc_Xword
cgc_r_info<64>(unsigned int s, unsigned int t)
{
  return (static_cast<Cgc_Xword>(s) << 32) + (t & 0xffffffff);
}
...
enum
{
  CGC_NOTE_OS_LINUX = 0,
  CGC_NOTE_OS_GNU = 1,
  CGC_NOTE_OS_SOLARIS2 = 2,
  CGC_NOTE_OS_FREEBSD = 3,
  CGC_NOTE_OS_NETBSD = 4,
  CGC_NOTE_OS_SYLLABLE = 5
};

} // End namespace cgccpp.

// Include internal details after defining the types.
#include "cgccpp_internal.h"

namespace cgccpp
{

// The offset of the CGC file header in the CGC file.
...
// Accessor class for the CGC file header.
...
class Ehdr
{
 public:
  ...
  typename Cgc_types<size>::Cgc_Off
  get_e_phoff() const
...
// Write class for the CGC file header.
...
// Accessor class for an CGC section header.
...
// Write class for an CGC section header.
...
// Accessor class for an CGC segment header.
...
// Write class for an CGC segment header.
...
// Accessor class for an CGC symbol table entry.
...
class Sym
{
 public:
  ...
  STB
  get_st_bind() const
  { return cgc_st_bind(this->get_st_info()); }

  STT
  get_st_type() const
  { return cgc_st_type(this->get_st_info()); }
  ...
  { return cgc_st_visibility(this->get_st_other()); }

  unsigned char
  get_st_nonvis() const
  { return cgc_st_nonvis(this->get_st_other()); }
...
// Writer class for an CGC symbol table entry.
...
class Sym_write
{
 public:
  ...
  void
  put_st_name(Cgc_Word v)
  { this->p_->st_name = Convert<32, big_endian>::convert_host(v); }

  void
  put_st_value(typename Cgc_types<size>::Cgc_Addr v)
  { this->p_->st_value = Convert<size, big_endian>::convert_host(v); }

  void
  put_st_size(typename Cgc_types<size>::Cgc_WXword v)
  ...
  { this->p_->st_info = cgc_st_info(bind, type); }
  ...
  { this->p_->st_other = cgc_st_other(vis, nonvis); }

  void
  put_st_shndx(Cgc_Half v)
...
// Accessor classes for an CGC REL relocation entry.
...
// Writer class for an CGC Rel relocation.
...
class Rel_write
{
 public:
  ...
  void
  put_r_offset(typename Cgc_types<size>::Cgc_Addr v)
  { this->p_->r_offset = Convert<size, big_endian>::convert_host(v); }

  void
  put_r_info(typename Cgc_types<size>::Cgc_WXword v)
...
// Accessor class for an CGC Rela relocation.
...
class Rela
{
 public:
  ...
  typename Cgc_types<size>::Cgc_Swxword
  get_r_addend() const
...
// Writer class for an CGC Rela relocation.
...
class Rela_write
{
 public:
  ...
  void
  put_r_addend(typename Cgc_types<size>::Cgc_Swxword v)
...
// Accessor classes for entries in the CGC SHT_DYNAMIC section aka
// PT_DYNAMIC segment.
...
class Dyn_write
{
 public:
  ...
  void
  put_d_tag(typename Cgc_types<size>::Cgc_Swxword v)
  { this->p_->d_tag = Convert<size, big_endian>::convert_host(v); }

  void
  put_d_val(typename Cgc_types<size>::Cgc_WXword v)
  { this->p_->d_val = Convert<size, big_endian>::convert_host(v); }

  void
  put_d_ptr(typename Cgc_types<size>::Cgc_Addr v)
  { this->p_->d_val = Convert<size, big_endian>::convert_host(v); }
...
// Accessor classes for entries in the CGC SHT_GNU_verdef section.
...
// Accessor classes for auxiliary entries in the CGC SHT_GNU_verdef
// section.
...
// Accessor classes for entries in the CGC SHT_GNU_verneed section.
...
// Accessor classes for auxiliary entries in the CGC SHT_GNU_verneed
// section.
