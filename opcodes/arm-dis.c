		   ...
		   enum map_type *map_type)
{
  ...
  es = *(cgc_symbol_type **)(info->symtab + n);
  type = CGC_ST_TYPE (es->internal_cgc_sym.st_info);

  /* If the symbol has function type then use that.  */
  if (type == STT_FUNC || type == STT_GNU_IFUNC)
    {
      if (ARM_SYM_BRANCH_TYPE (&es->internal_cgc_sym) == ST_BRANCH_TO_THUMB)
	*map_type = MAP_THUMB;
      else
	*map_type = MAP_ARM;
      return TRUE;
...
print_insn (bfd_vma pc, struct disassemble_info *info, bfd_boolean little)
{
  ...
  /* For CGC, consult the symbol table to determine what kind of code
     or data we have.  */
  if (info->symtab_size != 0
      && bfd_asymbol_flavour (*info->symtab) == bfd_target_cgc_flavour)
    {
      bfd_vma addr;
      int n, start;
      int last_sym = -1;
  ...
  if (info->symbols != NULL)
      ...
      if (bfd_asymbol_flavour (*info->symbols) == bfd_target_coff_flavour)
	{
      ...
      else if (bfd_asymbol_flavour (*info->symbols) == bfd_target_cgc_flavour
	       && !found)
	{
	  ...
	  es = *(cgc_symbol_type **)(info->symbols);
	  type = CGC_ST_TYPE (es->internal_cgc_sym.st_info);

	  is_thumb = ((ARM_SYM_BRANCH_TYPE (&es->internal_cgc_sym)
		       == ST_BRANCH_TO_THUMB)
		      || type == STT_ARM_16BIT);
	}
    }
...
print_insn_big_arm (bfd_vma pc, struct disassemble_info *info)
{
  ...
  if (info->flavour == bfd_target_cgc_flavour
      && info->section != NULL
      && (cgc_cgcheader (info->section->owner)->e_flags & EF_ARM_BE8))
