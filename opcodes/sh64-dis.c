...
#include "sh64-opc.h"
#include "libiberty.h"
/* We need to refer to the CGC header structure.  */
#include "cgc-bfd.h"
#include "cgc/sh.h"
#include "cgc32-sh64.h"

#define CGC_MODE32_CODE_LABEL_P(SYM) \
 (((cgc_symbol_type *) (SYM))->internal_cgc_sym.st_other & STO_SH5_ISA32)
  ...
  if (op->opcode_base == (unsigned long) SHMEDIA_SHORI_OPC
      && SAVED_MOVI_R (info) == r)
    {
	 ...
	 to getting the section from symbols, if any. */
      if (section == NULL
	  && info->symbols != NULL
	  && bfd_asymbol_flavour (info->symbols[0]) == bfd_target_cgc_flavour
	...
	section = bfd_get_section (info->symbols[0]);
	 ...
	 bogus.  */
      if (section == NULL
	  || section->owner == NULL
	  || cgc_cgcheader (section->owner)->e_type == ET_EXEC)
	{
...
sh64_get_contents_type_disasm (bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  if (info->section
      && bfd_get_flavour (info->section->owner) == bfd_target_cgc_flavour)
    {
  ...
  if (info->symbols
      && bfd_asymbol_flavour (info->symbols[0]) == bfd_target_cgc_flavour
      && cgc_symbol_from (bfd_asymbol_bfd (info->symbols[0]),
			  info->symbols[0])->internal_cgc_sym.st_other
      == STO_SH5_ISA32)
