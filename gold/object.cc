...
Xindex::initialize_symtab_xindex(Object* object, unsigned int symtab_shndx)
{
  ...
  while (i > 0)
    {
      --i;
      if (object->section_type(i) == cgccpp::SHT_SYMTAB_SHNDX
			   ...
			   const unsigned char* pshdrs)
{
  ...
  if (pshdrs == NULL)
  ...
  else
    {
      ...
      typename cgccpp::Shdr<size, big_endian> shdr(p);
  ...
  for (section_size_type i = 0; i < bytecount; i += 4)
    {
      unsigned int shndx = cgccpp::Swap<32, big_endian>::readval(contents + i);
  ...
  if (symndx >= this->symtab_xindex_.size())
    {
      object->error(_("symbol %u out of range for SHT_SYMTAB_SHNDX section"),
		    symndx);
      return cgccpp::SHN_UNDEF;
    }
  unsigned int shndx = this->symtab_xindex_[symndx];
  if (shndx < cgccpp::SHN_LORESERVE || shndx >= object->shnum())
    {
...
// cgccpp::Cgc_file interface, and also called by the Object code
// itself.
...
template<int size, bool big_endian>
void
Object::read_section_data(cgccpp::Cgc_file<size, big_endian, Object>* cgc_file,
			  Read_symbols_data* sd)
{
  const int shdr_size = cgccpp::Cgc_sizes<size>::shdr_size;

  // Read the section headers.
  const off_t shoff = cgc_file->shoff();
  ...
  const unsigned char* pshdrnames = pshdrs + cgc_file->shstrndx() * shdr_size;
  typename cgccpp::Shdr<size, big_endian> shdrnames(pshdrnames);

  if (shdrnames.get_sh_type() != cgccpp::SHT_STRTAB)
...
{
  const unsigned int shnum = this->cgc_file_.shnum();
  this->set_shnum(shnum);
}

// Find the SHT_SYMTAB section, given the section headers.  The CGC
  ...
  if (shnum > 0)
      ...
      while (i > 0)
	{
	  ...
	  if (shdr.get_sh_type() == cgccpp::SHT_SYMTAB)
		...
		{
		  Xindex* xindex =
		    new Xindex(this->cgc_file_.large_shndx_offset());
	  ...
	  // section.
	  if (shdr.get_sh_type() == cgccpp::SHT_SYMTAB_SHNDX)
	    {
	      xindex_shndx = i;
...
Sized_relobj_file<size, big_endian>::do_initialize_xindex()
{
  ...
  Xindex* xindex = new Xindex(this->cgc_file_.large_shndx_offset());
...
Sized_relobj_file<size, big_endian>::check_eh_frame_flags(
    const cgccpp::Shdr<size, big_endian>* shdr) const
{
  cgccpp::Cgc_Word sh_type = shdr->get_sh_type();
  return ((sh_type == cgccpp::SHT_PROGBITS
	   || sh_type == cgccpp::SHT_X86_64_UNWIND)
    ...
    const unsigned char* hdr) const
{
  ...
  while (1)
      ...
      if (hdr)
	{
	  ...
	  typename cgccpp::Shdr<size, big_endian> shdr(hdr);
      ...
      else
	{
	  ...
	  // by the CGC spec and not all CGC object file producers may
	  // be so clever.
    ...
    Sized_relobj_file<size, big_endian>* obj)
{
  ...
  const unsigned int shdr_size = cgccpp::Cgc_sizes<size>::shdr_size;
...
{
  this->read_section_data(&this->cgc_file_, sd);
							      ...
							      bool* is_ordinary)
{
  ...
  cgccpp::Sym<size, big_endian> cgcsym(symbols + sym * This::sym_size);
  *value = cgcsym.get_st_value();

  return this->adjust_sym_shndx(sym, cgcsym.get_st_shndx(), is_ordinary);
}
    ...
    std::vector<bool>* omit)
{
  ...
  const cgccpp::Cgc_Word* pword =
    reinterpret_cast<const cgccpp::Cgc_Word*>(pcon);
  ...
  cgccpp::Cgc_Word flags = cgccpp::Swap<32, big_endian>::readval(pword);

  // Look up the group signature, which is the name of a symbol.  CGC
  ...
  typename This::Shdr symshdr(this, this->cgc_file_.section_header(link));
  ...
  const unsigned char* psym = this->get_view(symoff, This::sym_size, true,
					     false);
  cgccpp::Sym<size, big_endian> sym(psym);
  ...
  if (signature[0] == '\0' && sym.get_st_type() == cgccpp::STT_SECTION)
    {
      bool is_ordinary;
  ...
  size_t count = shdr.get_sh_size() / sizeof(cgccpp::Cgc_Word);
  ...
  for (size_t i = 1; i < count; ++i)
    {
      cgccpp::Cgc_Word shndx =
	this->adjust_shndx(cgccpp::Swap<32, big_endian>::readval(pword + i));
					       ...
					       Read_symbols_data* sd)
{
  ...
  std::vector<unsigned int> reloc_type(shnum, cgccpp::SHT_NULL);
  ...
  for (unsigned int i = 1; i < shnum; ++i, pshdrs += This::shdr_size)
    {
      ...
      if (should_defer_layout && (shdr.get_sh_flags() & cgccpp::SHF_ALLOC))
  ...
  for (unsigned int i = 1; i < shnum; ++i, pshdrs += This::shdr_size)
      ...
      if (!is_pass_two)
	{
	  ...
	  if (!discard)
	    {
	      if (shdr.get_sh_type() == cgccpp::SHT_GROUP)
		{
	      ...
	      else if ((shdr.get_sh_flags() & cgccpp::SHF_GROUP) == 0
      ...
      if (is_pass_one && parameters->options().gc_sections())
	{
	  if (this->is_section_name_included(name)
	      ...
	      || shdr.get_sh_type() == cgccpp::SHT_INIT_ARRAY
	      || shdr.get_sh_type() == cgccpp::SHT_FINI_ARRAY)
	    {
      ...
      if (emit_relocs
	  && (shdr.get_sh_type() == cgccpp::SHT_REL
	      || shdr.get_sh_type() == cgccpp::SHT_RELA))
	{
	  reloc_sections.push_back(i);
	  continue;
	}

      if (relocatable && shdr.get_sh_type() == cgccpp::SHT_GROUP)
	continue;
      ...
      if (is_pass_two && parameters->options().gc_sections())
	{
	  ...
	  if (((shdr.get_sh_flags() & cgccpp::SHF_ALLOC) != 0)
      ...
      else
	{
	  ...
	  if (!relocatable
	      && parameters->options().gdb_index()
	      && !(shdr.get_sh_flags() & cgccpp::SHF_ALLOC))
	    {
  ...
  for (std::vector<unsigned int>::const_iterator p = reloc_sections.begin();
      ...
      if (data_section == reinterpret_cast<Output_section*>(2))
	{
	  ...
	  this->deferred_layout_relocs_.push_back(
	      Deferred_layout(i, name, pshdr, 0, cgccpp::SHT_NULL));
	  out_sections[i] = reinterpret_cast<Output_section*>(2);
	  out_section_offsets[i] = invalid_address;
	  continue;
	}
    ...
    std::string* why)
{
  ...
  for (size_t i = 0; i < symcount; ++i, p += sym_size)
    {
      cgccpp::Sym<size, big_endian> sym(p);
      unsigned int st_shndx = sym.get_st_shndx();
      if (st_shndx == cgccpp::SHN_UNDEF)
	continue;
    ...
    Library_base::Symbol_visitor_base* v)
{
  ...
  for (size_t i = 0; i < symcount; ++i, p += sym_size)
    {
      cgccpp::Sym<size, big_endian> sym(p);
      if (sym.get_st_shndx() != cgccpp::SHN_UNDEF)
							    ...
							    Stringpool* dynpool)
{
  ...
  typename This::Shdr symtabshdr(this,
				 this->cgc_file_.section_header(symtab_shndx));
  gold_assert(symtabshdr.get_sh_type() == cgccpp::SHT_SYMTAB);
  ...
  for (unsigned int i = 1; i < loccount; ++i, psyms += sym_size)
	...
	lv.set_is_section_symbol();
      else if (sym.get_st_type() == cgccpp::STT_TLS)
	lv.set_is_tls_symbol();
      else if (sym.get_st_type() == cgccpp::STT_GNU_IFUNC)
	  ...
	  || (discard_all && lv.may_be_discarded_from_output_symtab()))
	{
      ...
      // prefixes, typically .L for CGC system.  We want to be compatible
      ...
      if (discard_locals
	  && sym.get_st_type() != cgccpp::STT_FILE
    ...
    const Symbol_table* symtab)
{
  ...
  if (!is_ordinary)
    {
      if (shndx == cgccpp::SHN_ABS || Symbol::is_common_shndx(shndx))
...
Sized_relobj_file<size, big_endian>::do_section_flags(unsigned int shndx)
{
  ...
  return this->cgc_file_.section_flags(shndx);
}
    ...
    off_t symtab_off)
{
  ...
  for (unsigned int i = 1; i < loccount; ++i, psyms += sym_size)
    {
      cgccpp::Sym<size, big_endian> isym(psyms);
      ...
      if (is_ordinary)
	{
      ...
      if (lv.has_output_symtab_entry())
	{
	  cgccpp::Sym_write<size, big_endian> osym(ov);
      ...
      if (lv.has_output_dynsym_entry())
	{
	  ...
	  cgccpp::Sym_write<size, big_endian> osym(dyn_ov);
  ...
  for (size_t i = 0; i < count; ++i, p += sym_size)
    {
      ...
      if (sym.get_st_type() == cgccpp::STT_FILE)
	{
...
Relocate_info<size, big_endian>::location(size_t, off_t offset) const
{
  ...
  if (this->object->get_symbol_location_info(this->data_shndx, offset, &info))
      ...
      if (!info.source_file.empty())
	{
      ...
      ret += ":";
      if (info.enclosing_symbol_type == cgccpp::STT_FUNC)
	ret += _("function ");
...
// Read an CGC file with the header and return the appropriate
// instance of Object.

template<int size, bool big_endian>
Object*
make_cgc_sized_object(const std::string& name, Input_file* input_file,
		      ...
		      bool* punconfigured)
{
  Target* target = select_target(input_file, offset,
				 ...
				 ehdr.get_e_ident()[cgccpp::EI_OSABI],
				 ehdr.get_e_ident()[cgccpp::EI_ABIVERSION]);
  if (target == NULL)
    gold_fatal(_("%s: unsupported CGC machine number %d"),
  ...
  return target->make_cgc_object<size, big_endian>(name, input_file, offset,
						   ehdr);
}
...
// Return whether INPUT_FILE is an CGC object.

bool
is_cgc_object(Input_file* input_file, off_t offset,
	      const unsigned char** start, int* read_size)
{
  off_t filesize = input_file->file().filesize();
  int want = cgccpp::Cgc_recognizer::max_header_size;
  ...
  *read_size = want;
  ...
  return cgccpp::Cgc_recognizer::is_cgc_file(p, want);
}

// Read an CGC file and return the appropriate instance of Object.

Object*
make_cgc_object(const std::string& name, Input_file* input_file, off_t offset,
		...
		bool* punconfigured)
{
  ...
  int size = 0;
  if (!cgccpp::Cgc_recognizer::is_valid_header(p, bytes, &size,
					       &big_endian, &error))
    {
  ...
  if (size == 32)
      ...
      if (big_endian)
	{
	  ...
	  cgccpp::Ehdr<32, true> ehdr(p);
	  return make_cgc_sized_object<32, true>(name, input_file,
      ...
      else
	{
	  ...
	  cgccpp::Ehdr<32, false> ehdr(p);
	  return make_cgc_sized_object<32, false>(name, input_file,
						  offset, ehdr, punconfigured);
#else
	  if (punconfigured != NULL)
	    *punconfigured = true;
      ...
      if (big_endian)
	{
	  ...
	  cgccpp::Ehdr<64, true> ehdr(p);
	  return make_cgc_sized_object<64, true>(name, input_file,
						 offset, ehdr, punconfigured);
#else
	  if (punconfigured != NULL)
	    *punconfigured = true;
      ...
      else
	{
	  ...
	  cgccpp::Ehdr<64, false> ehdr(p);
	  return make_cgc_sized_object<64, false>(name, input_file,
						  offset, ehdr, punconfigured);
#else
	  if (punconfigured != NULL)
	    *punconfigured = true;
...
template
void
Object::read_section_data<32, false>(cgccpp::Cgc_file<32, false, Object>*,
				     Read_symbols_data*);
template
const unsigned char*
...
template
void
Object::read_section_data<32, true>(cgccpp::Cgc_file<32, true, Object>*,
				    Read_symbols_data*);
template
const unsigned char*
...
template
void
Object::read_section_data<64, false>(cgccpp::Cgc_file<64, false, Object>*,
				     Read_symbols_data*);
template
const unsigned char*
...
template
void
Object::read_section_data<64, true>(cgccpp::Cgc_file<64, true, Object>*,
				    Read_symbols_data*);
template
const unsigned char*
