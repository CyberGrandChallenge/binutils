...
Eh_frame_hdr::do_sized_write(Output_file* of)
{
  ...
  // .eh_frame section.
  oview[1] = cgccpp::DW_EH_PE_pcrel | cgccpp::DW_EH_PE_sdata4;
  ...
  cgccpp::Swap<32, big_endian>::writeval(oview + 4, eh_frame_offset);
  ...
  if (this->any_unrecognized_eh_frame_sections_
      || this->fde_offsets_.empty())
    {
      ...
      // sorted table.
      oview[2] = cgccpp::DW_EH_PE_omit;
      oview[3] = cgccpp::DW_EH_PE_omit;

      gold_assert(oview_size == 8);
    }
  else
    {
      oview[2] = cgccpp::DW_EH_PE_udata4;
      oview[3] = cgccpp::DW_EH_PE_datarel | cgccpp::DW_EH_PE_sdata4;

      cgccpp::Swap<32, big_endian>::writeval(oview + 8,
      ...
      typename cgccpp::Cgc_types<size>::Cgc_Addr output_address;
      ...
      for (typename Fde_addresses<size>::iterator p = fde_addresses.begin();
	   ...
	   ++p)
	{
	  cgccpp::Swap<32, big_endian>::writeval(pfde,
						 p->first - output_address);
	  cgccpp::Swap<32, big_endian>::writeval(pfde + 4,
...
Eh_frame_hdr::get_fde_pc(
    typename cgccpp::Cgc_types<size>::Cgc_Addr eh_frame_address,
    ...
    unsigned char fde_encoding)
{
  ...
  typename cgccpp::Cgc_types<size>::Cgc_Addr pc;
  bool is_signed = (fde_encoding & cgccpp::DW_EH_PE_signed) != 0;
  int pc_size = fde_encoding & 7;
  if (pc_size == cgccpp::DW_EH_PE_absptr)
    {
      if (size == 32)
	pc_size = cgccpp::DW_EH_PE_udata4;
      else if (size == 64)
	pc_size = cgccpp::DW_EH_PE_udata8;
      else
	gold_unreachable();
    }
  ...
  switch (pc_size)
    {
    case cgccpp::DW_EH_PE_udata2:
      pc = cgccpp::Swap<16, big_endian>::readval(p);
      if (is_signed)
	pc = (pc ^ 0x8000) - 0x8000;
      break;

    case cgccpp::DW_EH_PE_udata4:
      pc = cgccpp::Swap<32, big_endian>::readval(p);
      if (size > 32 && is_signed)
	pc = (pc ^ 0x80000000) - 0x80000000;
      break;

    case cgccpp::DW_EH_PE_udata8:
      gold_assert(size == 64);
      pc = cgccpp::Swap_unaligned<64, big_endian>::readval(p);
      break;
  ...
  switch (fde_encoding & 0x70)
    ...
    case cgccpp::DW_EH_PE_pcrel:
      pc += eh_frame_address + fde_offset + 8;
      break;

    case cgccpp::DW_EH_PE_datarel:
  ...
  gold_assert((fde_encoding & cgccpp::DW_EH_PE_indirect) == 0);
				...
				Fde_addresses<size>* fde_addresses)
{
  ...
  for (Fde_offsets::const_iterator p = fde_offsets->begin();
       ...
       ++p)
    {
      typename cgccpp::Cgc_types<size>::Cgc_Addr fde_pc;
	   ...
	   Eh_frame_hdr* eh_frame_hdr)
{
  ...
  cgccpp::Swap<32, big_endian>::writeval(oview + offset,
  ...
  // CIE address.
  cgccpp::Swap<32, big_endian>::writeval(oview + offset + 4,
					 offset + 4 - cie_offset);
  ...
  if (this->object_ == NULL)
	...
	gold_warning(_("overflow in PLT unwind data; "
      ...
      cgccpp::Swap<32, big_endian>::writeval(oview + offset + 8, spoffset);
      cgccpp::Swap<32, big_endian>::writeval(oview + offset + 12, upsize);
    }
	   ...
	   Eh_frame_hdr* eh_frame_hdr, Post_fdes* post_fdes)
{
  ...
  cgccpp::Swap<32, big_endian>::writeval(oview + offset + 4, 0);
    ...
    unsigned int reloc_type)
{
  ...
  if (contents_len == 4
      && cgccpp::Swap<32, big_endian>::readval(pcontents) == 0)
    return false;
    ...
    New_cies* new_cies)
{
  ...
  while (p < pend)
	...
	return false;
      ...
      unsigned int len = cgccpp::Swap<32, big_endian>::readval(p);
      p += 4;
      if (len == 0)
	{
	...
	return false;
      ...
      unsigned int id = cgccpp::Swap<32, big_endian>::readval(p);
      p += 4;
		   ...
		   New_cies* new_cies)
{
  ...
  unsigned char fde_encoding = cgccpp::DW_EH_PE_absptr;
  int per_offset = -1;
  while (*paug != '\0')
    {
      switch (*paug)
	...
	case 'P':
	  // Personality encoding.
	  {
	    ...
	    if ((per_encoding & 0xf0) == cgccpp::DW_EH_PE_aligned)
	      {
  ...
  if (per_offset != -1)
      ...
      else
	{
	  ...
	  if (personality_symndx >= symbols_size / sym_size)
	    return false;
	  cgccpp::Sym<size, big_endian> sym(symbols
		   ...
		   Offsets_to_cie* cies)
{
  ...
  if (symndx >= symbols_size / sym_size)
    return false;
  cgccpp::Sym<size, big_endian> sym(symbols + symndx * sym_size);
  bool is_ordinary;
  fde_shndx = object->adjust_sym_shndx(symndx, sym.get_st_shndx(),
				       &is_ordinary);

  if (is_ordinary
      && fde_shndx != cgccpp::SHN_UNDEF
...
Eh_frame::add_ehframe_for_plt(Output_data* plt, const unsigned char* cie_data,
			      ...
			      size_t fde_length)
{
  Cie cie(NULL, 0, 0, cgccpp::DW_EH_PE_pcrel | cgccpp::DW_EH_PE_sdata4, "",
	  cie_data, cie_length);
  Cie_offsets::iterator find_cie = this->cie_offsets_.find(&cie);
  Cie* pcie;
  if (find_cie != this->cie_offsets_.end())
