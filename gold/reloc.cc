...
Sized_relobj_file<size, big_endian>::do_read_relocs(Read_relocs_data* rd)
{
  ...
  const unsigned char* pshdrs = this->get_view(this->cgc_file_.shoff(),
  ...
  for (unsigned int i = 1; i < shnum; ++i, ps += This::shdr_size)
	...
	continue;
      ...
      bool is_section_allocated = ((secshdr.get_sh_flags() & cgccpp::SHF_ALLOC)
				   != 0);
	...
	reloc_size = cgccpp::Cgc_sizes<size>::rel_size;
      else
	reloc_size = cgccpp::Cgc_sizes<size>::rela_size;
    ...
    const Read_relocs_data::Relocs_list::iterator& p)
{
  ...
  if (p->sh_type == cgccpp::SHT_REL)
    this->emit_relocs_scan_reltype<cgccpp::SHT_REL>(symtab, layout,
						    plocal_syms, p, rr);
  else
    {
      gold_assert(p->sh_type == cgccpp::SHT_RELA);
      this->emit_relocs_scan_reltype<cgccpp::SHT_RELA>(symtab, layout,
						       plocal_syms, p, rr);
    }
}
...
{
  if (p->sh_type == cgccpp::SHT_REL)
    this->incremental_relocs_scan_reltype<cgccpp::SHT_REL>(p);
  else
    {
      gold_assert(p->sh_type == cgccpp::SHT_RELA);
      this->incremental_relocs_scan_reltype<cgccpp::SHT_RELA>(p);
    }
}
    ...
    const Read_relocs_data::Relocs_list::iterator& p)
{
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
	...
	continue;
	  ...
	  reloc.get_r_info();
      const unsigned int r_sym = cgccpp::cgc_r_sym<size>(r_info);
						    ...
						    Views* pviews)
{
  ...
  for (unsigned int i = 1; i < shnum; ++i, p += This::shdr_size)
	...
	continue;
      ...
      if (shdr.get_sh_type() == cgccpp::SHT_NOBITS)
	continue;
    ...
    Views* pviews)
{
  ...
  for (unsigned int i = 1; i < shnum; ++i, p += This::shdr_size)
      ...
      if (this->uses_split_stack() && output_offset != invalid_address)
	{
	  ...
	  if ((data_shdr.get_sh_flags() & cgccpp::SHF_EXECINSTR) != 0)
...
{
  if (sh_type == cgccpp::SHT_REL)
    this->incremental_relocs_write_reltype<cgccpp::SHT_REL>(
	relinfo,
	prelocs,
	reloc_count,
	output_section,
  ...
  else
    {
      ...
      this->incremental_relocs_write_reltype<cgccpp::SHT_RELA>(
	  relinfo,
	  prelocs,
	  reloc_count,
	  output_section,
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
    {
      ...
      const unsigned int r_sym = cgccpp::cgc_r_sym<size>(r_info);
      const unsigned int r_type = cgccpp::cgc_r_type<size>(r_info);
	...
	addend =
      ...
      cgccpp::Swap<32, big_endian>::writeval(pov, r_type);
      cgccpp::Swap<32, big_endian>::writeval(pov + 4, out_shndx);
      cgccpp::Swap<size, big_endian>::writeval(pov + 8, offset);
      cgccpp::Swap<size, big_endian>::writeval(pov + 8 + sizeof_addr, addend);
...
{
  if (sh_type == cgccpp::SHT_REL)
    this->split_stack_adjust_reltype<cgccpp::SHT_REL>(symtab, pshdrs, shndx,
						      prelocs, reloc_count,
						      view, view_size,
						      reloc_map);
  else
    {
      gold_assert(sh_type == cgccpp::SHT_RELA);
      this->split_stack_adjust_reltype<cgccpp::SHT_RELA>(symtab, pshdrs, shndx,
							 prelocs, reloc_count,
							 view, view_size,
							 reloc_map);
    }
    ...
    Reloc_symbol_changes** reloc_map)
{
  ...
  for (size_t i = 0; i < reloc_count; ++i, pr += reloc_size)
	...
	{
	  unsigned int r_type = cgccpp::cgc_r_type<size>(reloc.get_r_info());
    ...
    Sized_relobj_file<size, big_endian>::Function_offsets* function_offsets)
{
  ...
  typename cgccpp::Cgc_types<size>::Cgc_WXword sh_size =
...
Sized_relobj_file<size, big_endian>::reverse_words(unsigned char* view,
						   section_size_type view_size)
{
  typedef typename cgccpp::Swap<size, big_endian>::Valtype Valtype;
...
Merged_symbol_value<size>::value_from_output_section(
    const Relobj* object,
    unsigned int input_shndx,
    typename cgccpp::Cgc_types<size>::Cgc_Addr input_offset) const
{
    ...
    unsigned int reloc_type)
{
  ...
  if (reloc_type == cgccpp::SHT_REL)
    this->reloc_size_ = cgccpp::Cgc_sizes<size>::rel_size;
  else if (reloc_type == cgccpp::SHT_RELA)
    this->reloc_size_ = cgccpp::Cgc_sizes<size>::rela_size;
  else
    gold_unreachable();
...
Track_relocs<size, big_endian>::next_offset() const
{
  ...
  // the r_offset value.
  cgccpp::Rel<size, big_endian> rel(this->prelocs_ + this->pos_);
...
Track_relocs<size, big_endian>::next_symndx() const
{
  ...
  // symbol index.
  cgccpp::Rel<size, big_endian> rel(this->prelocs_ + this->pos_);
  return cgccpp::cgc_r_sym<size>(rel.get_r_info());
}
...
Track_relocs<size, big_endian>::next_addend() const
{
  if (this->pos_ >= this->len_)
    return 0;
  if (this->reloc_size_ == cgccpp::Cgc_sizes<size>::rel_size)
    return 0;
  cgccpp::Rela<size, big_endian> rela(this->prelocs_ + this->pos_);
