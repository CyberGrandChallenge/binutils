...
Gdb_index_info_reader::visit_top_die(Dwarf_die* die)
{
  ...
  switch (die->tag())
    {
      case cgccpp::DW_TAG_compile_unit:
      case cgccpp::DW_TAG_type_unit:
	this->cu_language_ = die->int_attribute(cgccpp::DW_AT_language);
	// Check for languages that require specialized knowledge to
	// construct fully-qualified names, that we don't yet support.
	if (this->cu_language_ == cgccpp::DW_LANG_Ada83
	    || this->cu_language_ == cgccpp::DW_LANG_Fortran77
	    || this->cu_language_ == cgccpp::DW_LANG_Fortran90
	    || this->cu_language_ == cgccpp::DW_LANG_Java
	    || this->cu_language_ == cgccpp::DW_LANG_Ada95
	    || this->cu_language_ == cgccpp::DW_LANG_Fortran95)
	  {
	...
	if (die->tag() == cgccpp::DW_TAG_compile_unit)
...
Gdb_index_info_reader::visit_die(Dwarf_die* die, Dwarf_die* context)
{
  switch (die->tag())
    {
      case cgccpp::DW_TAG_subprogram:
      case cgccpp::DW_TAG_constant:
      case cgccpp::DW_TAG_variable:
      case cgccpp::DW_TAG_enumerator:
      case cgccpp::DW_TAG_base_type:
	...
	break;
      case cgccpp::DW_TAG_typedef:
      case cgccpp::DW_TAG_union_type:
      case cgccpp::DW_TAG_class_type:
      case cgccpp::DW_TAG_interface_type:
      case cgccpp::DW_TAG_structure_type:
      case cgccpp::DW_TAG_enumeration_type:
      case cgccpp::DW_TAG_subrange_type:
      case cgccpp::DW_TAG_namespace:
	{
	  ...
	  if (context == NULL
	      && (die->tag() == cgccpp::DW_TAG_class_type
		  || die->tag() == cgccpp::DW_TAG_structure_type
		  || die->tag() == cgccpp::DW_TAG_union_type))
	  ...
	  // a declaration.
	  if (die->tag() == cgccpp::DW_TAG_namespace
	      || !die->is_declaration())
	    {
	      if (full_name.empty())
	  ...
	  if (die->tag() == cgccpp::DW_TAG_namespace
	      || die->tag() == cgccpp::DW_TAG_enumeration_type)
...
Gdb_index_info_reader::guess_full_class_name(Dwarf_die* die)
{
  ...
  for (off_t child_offset = die->child_offset();
       ...
       child_offset = next_offset)
    {
      ...
      if (child.tag() == 0)
	break;
      if (child.tag() == cgccpp::DW_TAG_subprogram)
        {
  ...
  if (name == NULL)
    {
      ...
      else if (die->tag() == cgccpp::DW_TAG_union_type)
...
Gdb_index_info_reader::get_qualified_name(Dwarf_die* die, Dwarf_die* context)
{
  ...
  if (name == NULL && die->tag() == cgccpp::DW_TAG_namespace)
    name = "(anonymous namespace)";
  else if (name == NULL)
    return full_name;
  ...
  if (die->tag() == cgccpp::DW_TAG_enumerator)
    {
...
Gdb_index_info_reader::record_cu_ranges(Dwarf_die* die)
{
  ...
  off_t ranges_offset = die->ref_attribute(cgccpp::DW_AT_ranges, &shndx);
  if (ranges_offset != -1)
    {
  ...
  off_t low_pc = die->address_attribute(cgccpp::DW_AT_low_pc, &shndx);
  off_t high_pc = die->address_attribute(cgccpp::DW_AT_high_pc, &shndx2);
  if (high_pc == -1)
    {
      high_pc = die->uint_attribute(cgccpp::DW_AT_high_pc);
      high_pc += low_pc;
      shndx2 = shndx;
    }
...
Gdb_index_info_reader::read_pubnames_and_pubtypes(Dwarf_die* die)
{
  ...
  unsigned int shndx;
  off_t stmt_list_off = die->ref_attribute (cgccpp::DW_AT_stmt_list,
                                            &shndx);
  // Look for the attr as either a flag or a ref.
  off_t offset = die->ref_attribute(cgccpp::DW_AT_GNU_pubnames, &shndx);

  // Newer versions of GCC generate CUs, but not TUs, with
  // DW_AT_FORM_flag_present.
  unsigned int flag = die->uint_attribute(cgccpp::DW_AT_GNU_pubnames);
  if (offset == -1 && flag == 0)
    {
      // Didn't find the attribute.
      if (die->tag() == cgccpp::DW_TAG_type_unit)
        {
                                ...
                                off_t symbols_size)
{
  ...
  if (attr == cgccpp::DW_AT_GNU_pubnames)
    {
                                          ...
                                          off_t symbols_size)
{
  ...
  this->pubnames_table_
      = this->map_pubtable_to_dies(cgccpp::DW_AT_GNU_pubnames, dwinfo,
                                   object, symbols, symbols_size);
  delete this->pubtypes_table_;
  this->pubtypes_table_
      = this->map_pubtable_to_dies(cgccpp::DW_AT_GNU_pubtypes, dwinfo,
                                   object, symbols, symbols_size);
}
...
Gdb_index::do_write(Output_file* of)
{
  ...
  // (1) Version number.
  cgccpp::Swap<32, false>::writeval(pov, gdb_index_version);
  pov += 4;
  // (2) Offset of the CU list.
  cgccpp::Swap<32, false>::writeval(pov, gdb_index_hdr_size);
  pov += 4;
  // (3) Offset of the types CU list.
  cgccpp::Swap<32, false>::writeval(pov, this->tu_offset_);
  pov += 4;
  // (4) Offset of the address area.
  cgccpp::Swap<32, false>::writeval(pov, this->addr_offset_);
  pov += 4;
  // (5) Offset of the symbol table.
  cgccpp::Swap<32, false>::writeval(pov, this->symtab_offset_);
  pov += 4;
  // (6) Offset of the constant pool.
  cgccpp::Swap<32, false>::writeval(pov, this->cu_pool_offset_);
  pov += 4;
  ...
  for (unsigned int i = 0; i < comp_units_count; ++i)
    {
      ...
      cgccpp::Swap<64, false>::writeval(pov, cu.cu_offset);
      cgccpp::Swap<64, false>::writeval(pov + 8, cu.cu_length);
      pov += 16;
    }
  ...
  for (unsigned int i = 0; i < this->type_units_.size(); ++i)
    {
      ...
      cgccpp::Swap<64, false>::writeval(pov, tu.tu_offset);
      cgccpp::Swap<64, false>::writeval(pov + 8, tu.type_offset);
      cgccpp::Swap<64, false>::writeval(pov + 16, tu.type_signature);
      pov += 24;
    }
  ...
  for (unsigned int i = 0; i < this->ranges_.size(); ++i)
    {
	  ...
	  cgccpp::Swap_aligned32<64, false>::writeval(pov, base + range.start);
	  cgccpp::Swap_aligned32<64, false>::writeval(pov + 8,
						      base + range.end);
	  cgccpp::Swap<32, false>::writeval(pov + 16, cu_index);
	  pov += 20;
	}
    }
  ...
  for (unsigned int i = 0; i < this->gdb_symtab_->capacity(); ++i)
      ...
      if (sym != NULL)
	{
      ...
      cgccpp::Swap<32, false>::writeval(pov, name_offset);
      cgccpp::Swap<32, false>::writeval(pov + 4, cu_vector_offset);
      pov += 8;
    }
  ...
  for (unsigned int i = 0; i < this->cu_vector_list_.size(); ++i)
    {
      Cu_vector* cu_vec = this->cu_vector_list_[i];
      cgccpp::Swap<32, false>::writeval(pov, cu_vec->size());
      pov += 4;
      for (unsigned int j = 0; j < cu_vec->size(); ++j)
	{
	  ...
	  if (cu_index < 0)
	    cu_index = comp_units_count + (-1 - cu_index);
	  cgccpp::Swap<32, false>::writeval(pov, cu_index);
	  pov += 4;
	}
    }
