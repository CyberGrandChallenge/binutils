...
// For convenience.
typedef cgccpp::Cgc_types<32>::Cgc_Addr Arm_address;
...
class Insn_template
{
 public:
  ...
  { return Insn_template(data, THUMB16_TYPE, cgccpp::R_ARM_NONE, 0); }
  ...
  { return Insn_template(data, THUMB16_SPECIAL_TYPE, cgccpp::R_ARM_NONE, 1); }

  static const Insn_template
  thumb32_insn(uint32_t data)
  { return Insn_template(data, THUMB32_TYPE, cgccpp::R_ARM_NONE, 0); }

  static const Insn_template
  thumb32_b_insn(uint32_t data, int reloc_addend)
  {
    return Insn_template(data, THUMB32_TYPE, cgccpp::R_ARM_THM_JUMP24,
			 reloc_addend);
  }

  static const Insn_template
  arm_insn(uint32_t data)
  { return Insn_template(data, ARM_TYPE, cgccpp::R_ARM_NONE, 0); }

  static const Insn_template
  arm_rel_insn(unsigned data, int reloc_addend)
  { return Insn_template(data, ARM_TYPE, cgccpp::R_ARM_JUMP24, reloc_addend); }
...
class Arm_v4bx_stub : public Stub
{
 ...
 private:
  ...
  do_fixed_endian_v4bx_write(unsigned char* view, section_size_type)
  {
    ...
    cgccpp::Swap<32, big_endian>::writeval(view,
					   (insns[0].data()
					   + (this->reg_ << 16)));
    view += insns[0].size();
    cgccpp::Swap<32, big_endian>::writeval(view,
...
class Arm_output_section : public Output_section
{
 public:
  ...
  Arm_output_section(const char* name, cgccpp::Cgc_Word type,
		     cgccpp::Cgc_Xword flags)
    : Output_section(name, type,
		     (type == cgccpp::SHT_ARM_EXIDX
		      ? flags | cgccpp::SHF_LINK_ORDER
		      : flags))
  {
    if (type == cgccpp::SHT_ARM_EXIDX)
...
class Arm_relobj : public Sized_relobj_file<32, big_endian>
{
 public:
  ...
  Arm_relobj(const std::string& name, Input_file* input_file, off_t offset,
	     const typename cgccpp::Ehdr<32, big_endian>& ehdr)
  ...
  // Processor-specific flags in CGC file header.  This is valid only after
  // reading symbols.
  cgccpp::Cgc_Word
 ...
 private:
  ...
  bool
  section_needs_reloc_stub_scanning(const cgccpp::Shdr<32, big_endian>&,
				    const Relobj::Output_sections&,
				    const Symbol_table*, const unsigned char*);

  // Whether a section is a scannable text section.
  bool
  section_is_scannable(const cgccpp::Shdr<32, big_endian>&, unsigned int,
		       const Output_section*, const Symbol_table*);

  // Whether a section needs to be scanned for the Cortex-A8 erratum.
  bool
  section_needs_cortex_a8_stub_scanning(const cgccpp::Shdr<32, big_endian>&,
					unsigned int, Output_section*,
					const Symbol_table*);

  // Scan a section for the Cortex-A8 erratum.
  void
  scan_section_for_cortex_a8_erratum(const cgccpp::Shdr<32, big_endian>&,
				     unsigned int, Output_section*,
				     Target_arm<big_endian>*);
  ...
  make_exidx_input_section(unsigned int shndx,
			   const cgccpp::Shdr<32, big_endian>& shdr,
			   unsigned int text_shndx,
			   const cgccpp::Shdr<32, big_endian>& text_shdr);
  ...
  // processor-specific flags in CGC file header.
  cgccpp::Cgc_Word processor_specific_flags_;
...
class Arm_dynobj : public Sized_dynobj<32, big_endian>
{
 public:
  Arm_dynobj(const std::string& name, Input_file* input_file, off_t offset,
	     const cgccpp::Ehdr<32, big_endian>& ehdr)
...
struct Stub_addend_reader
{
  ...
  cgccpp::Cgc_types<32>::Cgc_Swxword
  operator()(
    unsigned int /* r_type */,
...
template<bool big_endian>
struct Stub_addend_reader<cgccpp::SHT_REL, big_endian>
{
  cgccpp::Cgc_types<32>::Cgc_Swxword
  operator()(
    unsigned int,
    const unsigned char*,
    const typename Reloc_types<cgccpp::SHT_REL, 32, big_endian>::Reloc&) const;
};
...
template<bool big_endian>
struct Stub_addend_reader<cgccpp::SHT_RELA, big_endian>
{
  cgccpp::Cgc_types<32>::Cgc_Swxword
  operator()(
    unsigned int,
    const unsigned char*,
    const typename Reloc_types<cgccpp::SHT_RELA, 32,
  ...
  public Default_scan_relocatable_relocs<sh_type, Classify_reloc>
{
 public:
  ...
  {
    if (sh_type == cgccpp::SHT_RELA)
      return Relocatable_relocs::RELOC_ADJUST_FOR_SECTION_RELA;
    else
      {
	if (r_type == cgccpp::R_ARM_TARGET1
	    || r_type == cgccpp::R_ARM_TARGET2)
	  {
	...
	switch(r_type)
	  ...
	  // and R_ARM_TARGET2.
	  case cgccpp::R_ARM_NONE:
	  case cgccpp::R_ARM_V4BX:
	  case cgccpp::R_ARM_TLS_GOTDESC:
	  case cgccpp::R_ARM_TLS_CALL:
	  case cgccpp::R_ARM_TLS_DESCSEQ:
	  case cgccpp::R_ARM_THM_TLS_CALL:
	  case cgccpp::R_ARM_GOTRELAX:
	  case cgccpp::R_ARM_GNU_VTENTRY:
	  case cgccpp::R_ARM_GNU_VTINHERIT:
	  case cgccpp::R_ARM_THM_TLS_DESCSEQ16:
	  case cgccpp::R_ARM_THM_TLS_DESCSEQ32:
	    return Relocatable_relocs::RELOC_ADJUST_FOR_SECTION_0;
	  // These should have been converted to something else above.
	  case cgccpp::R_ARM_TARGET1:
	  case cgccpp::R_ARM_TARGET2:
	    gold_unreachable();
	  // Relocations that write full 32 bits and
	  // have alignment of 1.
	  case cgccpp::R_ARM_ABS32:
	  case cgccpp::R_ARM_REL32:
	  case cgccpp::R_ARM_SBREL32:
	  case cgccpp::R_ARM_GOTOFF32:
	  case cgccpp::R_ARM_BASE_PREL:
	  case cgccpp::R_ARM_GOT_BREL:
	  case cgccpp::R_ARM_BASE_ABS:
	  case cgccpp::R_ARM_ABS32_NOI:
	  case cgccpp::R_ARM_REL32_NOI:
	  case cgccpp::R_ARM_PLT32_ABS:
	  case cgccpp::R_ARM_GOT_ABS:
	  case cgccpp::R_ARM_GOT_PREL:
	  case cgccpp::R_ARM_TLS_GD32:
	  case cgccpp::R_ARM_TLS_LDM32:
	  case cgccpp::R_ARM_TLS_LDO32:
	  case cgccpp::R_ARM_TLS_IE32:
	  case cgccpp::R_ARM_TLS_LE32:
...
{
 public:
  typedef Output_data_reloc<cgccpp::SHT_REL, true, 32, big_endian>
    Reloc_section;
  ...
  Target_arm(const Target::Target_info* info = &arm_info)
    : Sized_target<32, big_endian>(info),
      ...
      copy_relocs_(cgccpp::R_ARM_COPY),
  ...
  using_thumb2() const
  {
    Object_attribute* attr =
      this->get_aeabi_object_attribute(cgccpp::Tag_CPU_arch);
    int arch = attr->int_value();
    return arch == cgccpp::TAG_CPU_ARCH_V6T2 || arch >= cgccpp::TAG_CPU_ARCH_V7;
  }
  ...
  using_thumb_only() const
  {
    ...
    if (attr->int_value() == cgccpp::TAG_CPU_ARCH_V6_M
	|| attr->int_value() == cgccpp::TAG_CPU_ARCH_V6S_M)
      return true;
    if (attr->int_value() != cgccpp::TAG_CPU_ARCH_V7
	&& attr->int_value() != cgccpp::TAG_CPU_ARCH_V7E_M)
      return false;
    attr = this->get_aeabi_object_attribute(cgccpp::Tag_CPU_arch_profile);
  ...
  may_use_arm_nop() const
  {
    Object_attribute* attr =
      this->get_aeabi_object_attribute(cgccpp::Tag_CPU_arch);
    int arch = attr->int_value();
    return (arch == cgccpp::TAG_CPU_ARCH_V6T2
	    || arch == cgccpp::TAG_CPU_ARCH_V6K
	    || arch == cgccpp::TAG_CPU_ARCH_V7
	    || arch == cgccpp::TAG_CPU_ARCH_V7E_M);
  }
  ...
  may_use_v4t_interworking() const
  {
    Object_attribute* attr =
      this->get_aeabi_object_attribute(cgccpp::Tag_CPU_arch);
    int arch = attr->int_value();
    return (arch != cgccpp::TAG_CPU_ARCH_PRE_V4
	    && arch != cgccpp::TAG_CPU_ARCH_V4);
  }
  ...
  may_use_v5t_interworking() const
  {
    ...
    if (parameters->options().fix_arm1176())
      return (arch == cgccpp::TAG_CPU_ARCH_V6T2
	      || arch == cgccpp::TAG_CPU_ARCH_V7
	      || arch == cgccpp::TAG_CPU_ARCH_V6_M
	      || arch == cgccpp::TAG_CPU_ARCH_V6S_M
	      || arch == cgccpp::TAG_CPU_ARCH_V7E_M);
    else
      return (arch != cgccpp::TAG_CPU_ARCH_PRE_V4
	      && arch != cgccpp::TAG_CPU_ARCH_V4
	      && arch != cgccpp::TAG_CPU_ARCH_V4T);
  }
  ...
  relocate_relocs(const Relocate_info<32, big_endian>*,
		  ...
		  size_t reloc_count,
		  Output_section* output_section,
		  typename cgccpp::Cgc_types<32>::Cgc_Off
  ...
  relocate_special_relocatable(const Relocate_info<32, big_endian>* relinfo,
			       ...
			       typename cgccpp::Cgc_types<32>::Cgc_Off
                                 offset_in_output_section,
			       unsigned char* view,
			       typename cgccpp::Cgc_types<32>::Cgc_Addr
				 view_address,
			       section_size_type view_size,
  ...
  default_target()
  {
    gold_assert(parameters->target().machine_code() == cgccpp::EM_ARM
 ...
 protected:
  ...
  // Make an CGC object.
  Object*
  do_make_cgc_object(const std::string&, Input_file*, off_t,
		     const cgccpp::Ehdr<32, big_endian>& ehdr);

  Object*
  do_make_cgc_object(const std::string&, Input_file*, off_t,
		     const cgccpp::Ehdr<32, !big_endian>&)
  { gold_unreachable(); }

  Object*
  do_make_cgc_object(const std::string&, Input_file*, off_t,
		      const cgccpp::Ehdr<64, false>&)
  { gold_unreachable(); }

  Object*
  do_make_cgc_object(const std::string&, Input_file*, off_t,
		     const cgccpp::Ehdr<64, true>&)
  { gold_unreachable(); }

  // Make an output section.
  Output_section*
  do_make_output_section(const char* name, cgccpp::Cgc_Word type,
			 cgccpp::Cgc_Xword flags)
  { return new Arm_output_section<big_endian>(name, type, flags); }

  void
  do_adjust_cgc_header(unsigned char* view, int len);
 ...
 private:
  ...
  class Scan
  {
   public:
    ...
    local(Symbol_table* symtab, Layout* layout, Target_arm* target,
	  ...
	  unsigned int data_shndx,
	  Output_section* output_section,
	  const cgccpp::Rel<32, big_endian>& reloc, unsigned int r_type,
	  const cgccpp::Sym<32, big_endian>& lsym,
	  bool is_discarded);
    ...
    local_reloc_may_be_function_pointer(Symbol_table* , Layout* , Target_arm* ,
					...
					unsigned int ,
					Output_section* ,
					const cgccpp::Rel<32, big_endian>& ,
					unsigned int ,
					const cgccpp::Sym<32, big_endian>&);
   ...
   private:
	...
	return false;
      ...
      return (!parameters->doing_static_link()
	      && (sym->type() == cgccpp::STT_FUNC
		  || sym->type() == cgccpp::STT_ARM_TFUNC)
	      && (sym->is_from_dynobj()
		  || sym->is_undefined()
		  || sym->is_preemptible()));
    }
  ...
  class Relocate
  {
   public:
	...
	case cgccpp::R_ARM_GOT_ABS:
	case cgccpp::R_ARM_GOT_PREL:
	case cgccpp::R_ARM_GOT_BREL12:
	case cgccpp::R_ARM_PLT32_ABS:
	case cgccpp::R_ARM_TLS_GD32:
	case cgccpp::R_ARM_TLS_LDM32:
	case cgccpp::R_ARM_TLS_IE32:
	case cgccpp::R_ARM_TLS_IE12GP:

	// These relocate types may use PLT entries.
	case cgccpp::R_ARM_CALL:
	case cgccpp::R_ARM_THM_CALL:
	case cgccpp::R_ARM_JUMP24:
	case cgccpp::R_ARM_THM_JUMP24:
	case cgccpp::R_ARM_THM_JUMP19:
	case cgccpp::R_ARM_PLT32:
	case cgccpp::R_ARM_THM_XPC22:
	case cgccpp::R_ARM_PREL31:
	case cgccpp::R_ARM_SBREL31:
	  return false;
   ...
   private:
    ...
    relocate_tls(const Relocate_info<32, big_endian>*, Target_arm<big_endian>*,
		 size_t, const cgccpp::Rel<32, big_endian>&, unsigned int,
		 const Sized_symbol<32>*, const Symbol_value<32>*,
		 unsigned char*, cgccpp::Cgc_types<32>::Cgc_Addr,
		 section_size_type);
  ...
  {
    return (gsym->type() != cgccpp::STT_ARM_TFUNC
  ...
  copy_reloc(Symbol_table* symtab, Layout* layout,
	     ...
	     Symbol* sym, const cgccpp::Rel<32, big_endian>& reloc)
  {
  ...
  static bool
  are_eabi_versions_compatible(cgccpp::Cgc_Word v1, cgccpp::Cgc_Word v2);

  // Merge processor-specific flags from input object and those in the CGC
  // header of the output.
  void
  merge_processor_specific_flags(const std::string&, cgccpp::Cgc_Word);
  ...
  scan_reloc_for_stub(const Relocate_info<32, big_endian>*, unsigned int,
		      ...
		      const Symbol_value<32>*,
		      cgccpp::Cgc_types<32>::Cgc_Swxword, Arm_address);
  ...
  scan_reloc_section_for_stubs(
      ...
      bool needs_special_offset_handling,
      const unsigned char* view,
      cgccpp::Cgc_types<32>::Cgc_Addr view_address,
      section_size_type);
  ...
  Copy_relocs<cgccpp::SHT_REL, 32, big_endian> copy_relocs_;
...
const Target::Target_info Target_arm<big_endian>::arm_info =
{
  ...
  0,                    // rosegment_gap
  cgccpp::SHN_UNDEF,	// small_common_shndx
  cgccpp::SHN_UNDEF,	// large_common_shndx
...
class Arm_relocate_functions : public Relocate_functions<32, big_endian>
{
 ...
 private:
  ...
  static inline typename cgccpp::Swap<32, big_endian>::Valtype
  extract_arm_movw_movt_addend(
      typename cgccpp::Swap<32, big_endian>::Valtype val)
  {
    // According to the Cgc ABI for ARM Architecture the immediate
  ...
  insert_val_arm_movw_movt(
      typename cgccpp::Swap<32, big_endian>::Valtype val,
      typename cgccpp::Swap<32, big_endian>::Valtype x)
  {
    val &= 0xfff0f000;
    val |= x & 0x0fff;
    val |= (x & 0xf000) << 4;
  ...
  // (see (AACGC 4.6.1.4 Static ARM relocations, Group Relocations, p.32)
  static uint32_t
  calc_grp_kn(typename cgccpp::Swap<32, big_endian>::Valtype residual)
  {
    int32_t msb;
  ...
  // (see (AACGC 4.6.1.4 Static ARM relocations, Group Relocations, p.32)
  static typename cgccpp::Swap<32, big_endian>::Valtype
  calc_grp_residual(typename cgccpp::Swap<32, big_endian>::Valtype residual,
		    const int group)
  {
  ...
  // (see (AACGC 4.6.1.4 Static ARM relocations, Group Relocations, p.32)
  static typename cgccpp::Swap<32, big_endian>::Valtype
  calc_grp_gn(typename cgccpp::Swap<32, big_endian>::Valtype residual,
	      const int group)
  {
    typename cgccpp::Swap<32, big_endian>::Valtype gn = 0;
    uint32_t shift = 0;
 ...
 public:
  ...
  {
    typedef typename cgccpp::Swap<8, big_endian>::Valtype Valtype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Valtype val = cgccpp::Swap<8, big_endian>::readval(wv);
    int32_t addend = Bits<8>::sign_extend32(val);
    Arm_address x = psymval->value(object, addend);
    val = Bits<32>::bit_select32(val, x, 0xffU);
    cgccpp::Swap<8, big_endian>::writeval(wv, val);
  ...
  thm_abs5(unsigned char* view,
       ...
       const Symbol_value<32>* psymval)
  {
    typedef typename cgccpp::Swap<16, big_endian>::Valtype Valtype;
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Reltype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Valtype val = cgccpp::Swap<16, big_endian>::readval(wv);
    Reltype addend = (val & 0x7e0U) >> 6;
    Reltype x = psymval->value(object, addend);
    val = Bits<32>::bit_select32(val, x << 6, 0x7e0U);
    cgccpp::Swap<16, big_endian>::writeval(wv, val);
  ...
  abs12(unsigned char* view,
	...
	const Symbol_value<32>* psymval)
  {
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Valtype;
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Reltype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Valtype val = cgccpp::Swap<32, big_endian>::readval(wv);
    Reltype addend = val & 0x0fffU;
    Reltype x = psymval->value(object, addend);
    val = Bits<32>::bit_select32(val, x, 0x0fffU);
    cgccpp::Swap<32, big_endian>::writeval(wv, val);
  ...
  abs16(unsigned char* view,
	...
	const Symbol_value<32>* psymval)
  {
    typedef typename cgccpp::Swap_unaligned<16, big_endian>::Valtype Valtype;
    Valtype val = cgccpp::Swap_unaligned<16, big_endian>::readval(view);
    int32_t addend = Bits<16>::sign_extend32(val);
    Arm_address x = psymval->value(object, addend);
    val = Bits<32>::bit_select32(val, x, 0xffffU);
    cgccpp::Swap_unaligned<16, big_endian>::writeval(view, val);
  ...
  {
    typedef typename cgccpp::Swap_unaligned<32, big_endian>::Valtype Valtype;
    Valtype addend = cgccpp::Swap_unaligned<32, big_endian>::readval(view);
    Valtype x = psymval->value(object, addend) | thumb_bit;
    cgccpp::Swap_unaligned<32, big_endian>::writeval(view, x);
  ...
  {
    typedef typename cgccpp::Swap<16, big_endian>::Valtype Valtype;
    typedef typename cgccpp::Swap<16, big_endian>::Valtype Reltype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Valtype val = cgccpp::Swap<16, big_endian>::readval(wv);
	    ...
	    Arm_address address)
  {
    ...
    cgccpp::Swap<16, big_endian>::writeval(wv, ((val & 0xff00)
						| ((x & 0x01fe) >> 1)));
	    ...
	    Arm_address address)
  {
    ...
    int32_t x = (psymval->value(object, addend) - address);
    cgccpp::Swap<16, big_endian>::writeval(wv, ((val & 0xf800)
						| ((x & 0x0ffe) >> 1)));
  ...
  got_brel(unsigned char* view,
	   typename cgccpp::Swap<32, big_endian>::Valtype got_offset)
  {
  ...
  {
    typedef typename cgccpp::Swap_unaligned<32, big_endian>::Valtype Valtype;
    Valtype val = cgccpp::Swap_unaligned<32, big_endian>::readval(view);
    Valtype addend = Bits<31>::sign_extend32(val);
    Valtype x = (psymval->value(object, addend) | thumb_bit) - address;
    val = Bits<32>::bit_select32(val, x, 0x7fffffffU);
    cgccpp::Swap_unaligned<32, big_endian>::writeval(view, val);
	   ...
	   bool check_overflow)
  {
    ...
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Reltype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Reltype val = (cgccpp::Swap<16, big_endian>::readval(wv) << 16)
		  | cgccpp::Swap<16, big_endian>::readval(wv + 1);
    ...
    cgccpp::Swap<16, big_endian>::writeval(wv, val >> 16);
    cgccpp::Swap<16, big_endian>::writeval(wv + 1, val & 0xffff);
    return ((check_overflow && Bits<16>::has_overflow32(x))
	    ? This::STATUS_OVERFLOW
	    : This::STATUS_OKAY);
  }
	    ...
	    Arm_address thumb_bit)
  {
    ...
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Reltype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Reltype insn = (cgccpp::Swap<16, big_endian>::readval(wv) << 16)
		   | cgccpp::Swap<16, big_endian>::readval(wv + 1);
    ...
    cgccpp::Swap<16, big_endian>::writeval(wv, insn >> 16);
    cgccpp::Swap<16, big_endian>::writeval(wv + 1, insn & 0xffff);
    return ((val > 0xfff) ?
	...
	bool check_overflow)
  {
    ...
    typedef typename cgccpp::Swap<32, big_endian>::Valtype Valtype;
    Valtype* wv = reinterpret_cast<Valtype*>(view);
    Valtype insn = cgccpp::Swap<32, big_endian>::readval(wv);
    ...
    cgccpp::Swap<32, big_endian>::writeval(wv, insn);
    return This::STATUS_OKAY;
  }
    ...
    bool is_weakly_undefined_without_plt)
{
  ...
  if (r_type == cgccpp::R_ARM_CALL)
    {
      if (!insn_is_uncond_bl && !insn_is_blx)
	return This::STATUS_BAD_RELOC;
    }
  else if (r_type == cgccpp::R_ARM_JUMP24)
    {
      if (!insn_is_b && !insn_is_cond_bl)
	return This::STATUS_BAD_RELOC;
    }
  else if (r_type == cgccpp::R_ARM_PLT32)
    {
      if (!insn_is_any_branch)
	return This::STATUS_BAD_RELOC;
    }
  else if (r_type == cgccpp::R_ARM_XPC25)
    {
      // FIXME: AACGC document IH0044C does not say much about it other
  ...
  if (!parameters->options().relocatable()
	  ...
	  || ((thumb_bit != 0)
	      && !(may_use_blx && r_type == cgccpp::R_ARM_CALL))))
    {
  ...
  if (thumb_bit != 0)
    {
      // Turn BL to BLX.
      gold_assert(may_use_blx && r_type == cgccpp::R_ARM_CALL);
    ...
    bool is_weakly_undefined_without_plt)
{
  typedef typename cgccpp::Swap<16, big_endian>::Valtype Valtype;
  Valtype* wv = reinterpret_cast<Valtype*>(view);
  uint32_t upper_insn = cgccpp::Swap<16, big_endian>::readval(wv);
  uint32_t lower_insn = cgccpp::Swap<16, big_endian>::readval(wv + 1);
  ...
  // Check that the instruction is valid.
  if (r_type == cgccpp::R_ARM_THM_CALL)
    {
      if (!is_bl_insn && !is_blx_insn)
	return This::STATUS_BAD_RELOC;
    }
  else if (r_type == cgccpp::R_ARM_THM_JUMP24)
    {
      // This cannot be a BLX.
      if (!is_bl_insn)
	return This::STATUS_BAD_RELOC;
    }
  else if (r_type == cgccpp::R_ARM_THM_XPC22)
    {
  ...
  if (is_weakly_undefined_without_plt)
	...
	{
	  cgccpp::Swap<16, big_endian>::writeval(wv, 0xf3af);
	  cgccpp::Swap<16, big_endian>::writeval(wv + 1, 0x8000);
	}
      else
	{
	  cgccpp::Swap<16, big_endian>::writeval(wv, 0xe000);
	  cgccpp::Swap<16, big_endian>::writeval(wv + 1, 0xbf00);
	}
      return This::STATUS_OKAY;
    }
  ...
  if (!parameters->options().relocatable()
	  ...
	  || ((thumb_bit == 0)
	      && (((r_type == cgccpp::R_ARM_THM_CALL) && !may_use_blx)
		  || r_type == cgccpp::R_ARM_THM_JUMP24))))
    {
  ...
  if (thumb_bit == 0)
    {
      gold_assert(may_use_blx
		  && (r_type == cgccpp::R_ARM_THM_CALL
		      || r_type == cgccpp::R_ARM_THM_XPC22));
  ...
  cgccpp::Swap<16, big_endian>::writeval(wv, upper_insn);
  cgccpp::Swap<16, big_endian>::writeval(wv + 1, lower_insn);
  ...
  if (this->got_ == NULL)
    {
      ...
      layout->add_output_section_data(".got", cgccpp::SHT_PROGBITS,
				      (cgccpp::SHF_ALLOC | cgccpp::SHF_WRITE),
  ...
  if (this->rel_dyn_ == NULL)
    {
      ...
      layout->add_output_section_data(".rel.dyn", cgccpp::SHT_REL,
				      cgccpp::SHF_ALLOC, this->rel_dyn_,
    ...
    entry_in_thumb_mode_(false), relocs_()
{
  ...
  for (size_t i = 0; i < insn_count; i++)
	...
	case Insn_template::THUMB32_TYPE:
	  if (insns[i].r_type() != cgccpp::R_ARM_NONE)
	...
	case Insn_template::ARM_TYPE:
	  ...
	  // instruction.
	  if (insns[i].r_type() == cgccpp::R_ARM_JUMP24)
	    this->relocs_.push_back(Reloc(i, offset));
	  break;
...
Stub::do_fixed_endian_write(unsigned char* view, section_size_type view_size)
{
  ...
  for (size_t i = 0; i < stub_template->insn_count(); i++)
	...
	{
	case Insn_template::THUMB16_TYPE:
	  cgccpp::Swap<16, big_endian>::writeval(pov, insns[i].data() & 0xffff);
	  break;
	case Insn_template::THUMB16_SPECIAL_TYPE:
	  cgccpp::Swap<16, big_endian>::writeval(
	      pov,
	...
	case Insn_template::THUMB32_TYPE:
	  {
	    ...
	    cgccpp::Swap<16, big_endian>::writeval(pov, hi);
	    cgccpp::Swap<16, big_endian>::writeval(pov + 2, lo);
	  }
	  break;
	case Insn_template::ARM_TYPE:
	case Insn_template::DATA_TYPE:
	  cgccpp::Swap<32, big_endian>::writeval(pov, insns[i].data());
	  break;
	default:
	  gold_unreachable();
	}
...
// bfd/cgc32-arm.c.  We have changed the interface a little to keep the Stub
// class simple.
   ...
   bool target_is_thumb)
{
  ...
  if (r_type == cgccpp::R_ARM_THM_CALL || r_type == cgccpp::R_ARM_THM_JUMP24)
    {
      // For THUMB BLX instruction, bit 1 of target comes from bit 1 of the
      // base address (instruction address + 4).
      if ((r_type == cgccpp::R_ARM_THM_CALL) && may_use_blx && !target_is_thumb)
  ...
  else if (r_type == cgccpp::R_ARM_CALL
	   || r_type == cgccpp::R_ARM_JUMP24
	   || r_type == cgccpp::R_ARM_PLT32)
    {
      branch_offset = static_cast<int64_t>(destination) - location;
      if (target_is_thumb)
	{
	  ...
	  if (branch_offset > (ARM_MAX_FWD_BRANCH_OFFSET + 2)
	      ...
	      || ((r_type == cgccpp::R_ARM_CALL) && !may_use_blx)
	      || (r_type == cgccpp::R_ARM_JUMP24)
	      || (r_type == cgccpp::R_ARM_PLT32))
	    {
	      stub_type = (output_is_position_independent
...
Stub_factory::Stub_factory()
{
  ...
  static const Insn_template cgc32_arm_stub_long_branch_any_any[] =
    {
      Insn_template::arm_insn(0xe51ff004),	// ldr   pc, [pc, #-4]
      Insn_template::data_word(0, cgccpp::R_ARM_ABS32, 0),
						// dcd   R_ARM_ABS32(X)
    };

  // V4T Arm -> Thumb long branch stub. Used on V4T where blx is not
  // available.
  static const Insn_template cgc32_arm_stub_long_branch_v4t_arm_thumb[] =
    {
  ...
  static const Insn_template cgc32_arm_stub_long_branch_thumb_only[] =
    {
  ...
  // allowed.
  static const Insn_template cgc32_arm_stub_long_branch_v4t_thumb_thumb[] =
    {
  ...
  // available.
  static const Insn_template cgc32_arm_stub_long_branch_v4t_thumb_arm[] =
    {
      Insn_template::thumb16_insn(0x4778),	// bx   pc
      Insn_template::thumb16_insn(0x46c0),	// nop
      Insn_template::arm_insn(0xe51ff004),	// ldr   pc, [pc, #-4]
  ...
  static const Insn_template cgc32_arm_stub_short_branch_v4t_thumb_arm[] =
    {
      Insn_template::thumb16_insn(0x4778),		// bx   pc
      Insn_template::thumb16_insn(0x46c0),		// nop
  ...
  static const Insn_template cgc32_arm_stub_long_branch_any_arm_pic[] =
    {
      Insn_template::arm_insn(0xe59fc000),	// ldr   r12, [pc]
      Insn_template::arm_insn(0xe08ff00c),	// add   pc, pc, ip
      Insn_template::data_word(0, cgccpp::R_ARM_REL32, -4),
						// dcd   R_ARM_REL32(X-4)
    };
  ...
  // ARMv7).
  static const Insn_template cgc32_arm_stub_long_branch_any_thumb_pic[] =
    {
      Insn_template::arm_insn(0xe59fc004),	// ldr   r12, [pc, #4]
      Insn_template::arm_insn(0xe08fc00c),	// add   ip, pc, ip
      Insn_template::arm_insn(0xe12fff1c),	// bx    ip
      Insn_template::data_word(0, cgccpp::R_ARM_REL32, 0),
						// dcd   R_ARM_REL32(X)
    };

  // V4T ARM -> ARM long branch stub, PIC.
  static const Insn_template cgc32_arm_stub_long_branch_v4t_arm_thumb_pic[] =
    {
  ...
  static const Insn_template cgc32_arm_stub_long_branch_v4t_thumb_arm_pic[] =
    {
      Insn_template::thumb16_insn(0x4778),	// bx   pc
      Insn_template::thumb16_insn(0x46c0),	// nop
      Insn_template::arm_insn(0xe59fc000),	// ldr  ip, [pc, #0]
  ...
  // architectures.
  static const Insn_template cgc32_arm_stub_long_branch_thumb_only_pic[] =
    {
      Insn_template::thumb16_insn(0xb401),	// push {r0}
      Insn_template::thumb16_insn(0x4802),	// ldr  r0, [pc, #8]
  ...
  // allowed.
  static const Insn_template cgc32_arm_stub_long_branch_v4t_thumb_thumb_pic[] =
    {
      Insn_template::thumb16_insn(0x4778),	// bx   pc
      Insn_template::thumb16_insn(0x46c0),	// nop
      Insn_template::arm_insn(0xe59fc004),	// ldr  ip, [pc, #4]
  ...
  // after:
  //
  static const Insn_template cgc32_arm_stub_a8_veneer_b_cond[] =
    {
  ...
  static const Insn_template cgc32_arm_stub_a8_veneer_b[] =
    {
      Insn_template::thumb32_b_insn(0xf000b800, -4)	// b.w dest
    };

  static const Insn_template cgc32_arm_stub_a8_veneer_bl[] =
    {
      Insn_template::thumb32_b_insn(0xf000b800, -4)	// b.w dest
    };
  ...
  static const Insn_template cgc32_arm_stub_a8_veneer_blx[] =
    {
  ...
  // (bx r[n] instruction).
  static const Insn_template cgc32_arm_stub_v4_veneer_bx[] =
    {
...
#define DEF_STUB(x)	\
  do \
    { \
      size_t array_size \
	= sizeof(cgc32_arm_stub_##x) / sizeof(cgc32_arm_stub_##x[0]); \
      Stub_type type = arm_stub_##x; \
      this->stub_templates_[type] = \
	new Stub_template(type, cgc32_arm_stub_##x, array_size); \
    } \
  while (0);
...
Arm_exidx_cantunwind::do_fixed_endian_write(Output_file* of)
{
  ...
  cgccpp::Swap_unaligned<32, big_endian>::writeval(oview,
						   prel31_offset & 0x7fffffffU);
  cgccpp::Swap_unaligned<32, big_endian>::writeval(oview + 4,
						   cgccpp::EXIDX_CANTUNWIND);
...
{
  bool delete_entry;
  if (second_word == cgccpp::EXIDX_CANTUNWIND)
    {
  ...
  for (section_size_type i = 0; i < section_size; i += 8)
    {
      ...
      uint32_t second_word = cgccpp::Swap<32, big_endian>::readval(wv);
  ...
  const Task* task)
{
  ...
  gold_assert(this->type() != cgccpp::SHT_ARM_EXIDX);
...
//
// This is similar to the group_sections() function in cgc32-arm.c but is
...
{
  gold_assert((this->flags() & cgccpp::SHF_ALLOC) != 0);
    ...
    const Symbol_table* symtab)
{
  ...
  if (shdr.get_sh_size() == 0
      || (shdr.get_sh_flags() & cgccpp::SHF_ALLOC) == 0
      || shdr.get_sh_type() != cgccpp::SHT_PROGBITS)
    return false;
    ...
    const unsigned char* pshdrs)
{
  ...
  if (sh_type != cgccpp::SHT_REL && sh_type != cgccpp::SHT_RELA)
    return false;
  ...
  if (sh_type == cgccpp::SHT_REL)
    reloc_size = cgccpp::Cgc_sizes<32>::rel_size;
  else
    reloc_size = cgccpp::Cgc_sizes<32>::rela_size;
  ...
  const unsigned int shdr_size = cgccpp::Cgc_sizes<32>::shdr_size;
  const cgccpp::Shdr<32, big_endian> text_shdr(pshdrs + index * shdr_size);
    ...
    Target_arm<big_endian>* arm_target)
{
  ...
  if (p == this->mapping_symbols_info_.end() || p->first.first != shndx)
    {
      if ((this->section_flags(shndx) & cgccpp::SHF_EXECINSTR) != 0)
    ...
    const Layout* layout)
{
  ...
  const unsigned char* pshdrs = this->get_view(this->cgc_file()->shoff(),
					       shnum * shdr_size,
					       true, true);
  ...
  for (unsigned int i = 1; i < shnum; ++i, p += shdr_size)
    {
      const cgccpp::Shdr<32, big_endian> shdr(p);
...
// the Symbol object keeps the CGC symbol type.  For local symbol it is
    ...
    Stringpool_template<char>* dynpool)
{
  ...
  cgccpp::Shdr<32, big_endian>
      symtabshdr(this, this->cgc_file()->section_header(symtab_shndx));
  gold_assert(symtabshdr.get_sh_type() == cgccpp::SHT_SYMTAB);

  // Read the local symbols.
  const int sym_size =cgccpp::Cgc_sizes<32>::sym_size;
  ...
  cgccpp::Shdr<32, big_endian>
    strtabshdr(this, this->cgc_file()->section_header(strtab_shndx));
  if (strtabshdr.get_sh_type() != cgccpp::SHT_STRTAB)
    {
  ...
  for (unsigned int i = 1; i < loccount; ++i, psyms += sym_size)
    {
      cgccpp::Sym<32, big_endian> sym(psyms);
      cgccpp::STT st_type = sym.get_st_type();
      ...
      if (Target_arm<big_endian>::is_mapping_symbol_name(sym_name))
	{
      ...
      if (st_type == cgccpp::STT_ARM_TFUNC
	  || (st_type == cgccpp::STT_FUNC && ((input_value & 1) != 0)))
	{
    ...
    typename Sized_relobj_file<32, big_endian>::Views* pviews)
{
  ...
  for (unsigned int i = 1; i < shnum; ++i)
	  ...
	  && !arm_input_section->stub_table()->empty())
	{
	  ...
	  relinfo.reloc_shndx = cgccpp::SHN_UNDEF;
	  relinfo.reloc_shdr = NULL;
	  relinfo.data_shndx = i;
	  relinfo.data_shdr = pshdrs + i * cgccpp::Cgc_sizes<32>::shdr_size;
...
Arm_relobj<big_endian>::find_linked_text_section(
    ...
    unsigned int* pshndx)
{
  cgccpp::Shdr<32, big_endian> shdr(pshdr);

  // If there is no relocation, we cannot find the linked text section.
  size_t reloc_size;
  if (shdr.get_sh_type() == cgccpp::SHT_REL)
      reloc_size = cgccpp::Cgc_sizes<32>::rel_size;
  else
      reloc_size = cgccpp::Cgc_sizes<32>::rela_size;
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
    {
      ...
      typename cgccpp::Cgc_types<32>::Cgc_WXword r_info;
      if (shdr.get_sh_type() == cgccpp::SHT_REL)
	{
	  typename cgccpp::Rel<32, big_endian> reloc(prelocs);
	  r_info = reloc.get_r_info();
	  r_offset = reloc.get_r_offset();
	}
      else
	{
	  typename cgccpp::Rela<32, big_endian> reloc(prelocs);
	  r_info = reloc.get_r_info();
	  r_offset = reloc.get_r_offset();
	}

      unsigned int r_type = cgccpp::cgc_r_type<32>(r_info);
      if (r_type != cgccpp::R_ARM_PREL31 && r_type != cgccpp::R_ARM_SBREL31)
	continue;

      unsigned int r_sym = cgccpp::cgc_r_sym<32>(r_info);
      if (r_sym == 0
	...
	continue;
      ...
      const int sym_size = cgccpp::Cgc_sizes<32>::sym_size;
      cgccpp::Sym<32, big_endian> sym(psyms + r_sym * sym_size);
      if (sym.get_st_type() == cgccpp::STT_SECTION)
	{
	  bool is_ordinary;
	  *pshndx =
    ...
    const cgccpp::Shdr<32, big_endian>& text_shdr)
{
  ...
  if (text_shndx == cgccpp::SHN_UNDEF || text_shndx >= this->shnum())
    {
  ...
  if ((text_shdr.get_sh_flags() & cgccpp::SHF_ALLOC) == 0)
    {
  ...
  else if ((text_shdr.get_sh_flags() & cgccpp::SHF_EXECINSTR) == 0)
...
Arm_relobj<big_endian>::do_read_symbols(Read_symbols_data* sd)
{
  ...
  if (format != Input_file::FORMAT_CGC)
    {
  ...
  // Read processor-specific flags in CGC file header.
  const unsigned char* pehdr = this->get_view(cgccpp::file_header_offset,
					      cgccpp::Cgc_sizes<32>::ehdr_size,
					      true, false);
  cgccpp::Ehdr<32, big_endian> ehdr(pehdr);
  ...
  const size_t shdr_size = cgccpp::Cgc_sizes<32>::shdr_size;
  const unsigned char* pshdrs = sd->section_headers->data();
  const unsigned char* ps = pshdrs + shdr_size;
  bool must_merge_flags_and_attributes = false;
  for (unsigned int i = 1; i < this->shnum(); ++i, ps += shdr_size)
    {
      cgccpp::Shdr<32, big_endian> shdr(ps);
      ...
      if (shdr.get_sh_type() == cgccpp::SHT_SYMTAB)
	{
	  // Symbol table is not empty.
	  const cgccpp::Cgc_types<32>::Cgc_WXword sym_size =
	     cgccpp::Cgc_sizes<32>::sym_size;
	  if (shdr.get_sh_size() > sym_size)
	    must_merge_flags_and_attributes = true;
	}
      else if (shdr.get_sh_type() != cgccpp::SHT_STRTAB)
	// If this is neither an empty symbol table nor a string table,
	// be conservative.
	must_merge_flags_and_attributes = true;

      if (shdr.get_sh_type() == cgccpp::SHT_ARM_ATTRIBUTES)
	{
      ...
      else if (shdr.get_sh_type() == cgccpp::SHT_ARM_EXIDX)
	{
	  unsigned int text_shndx = this->adjust_shndx(shdr.get_sh_link());
	  if (text_shndx == cgccpp::SHN_UNDEF)
	    deferred_exidx_sections.push_back(i);
	  else
	    {
	      cgccpp::Shdr<32, big_endian> text_shdr(pshdrs
	  ...
	  if ((shdr.get_sh_flags() & cgccpp::SHF_LINK_ORDER) == 0)
  ...
  if (!deferred_exidx_sections.empty())
	...
	{
	  cgccpp::Shdr<32, big_endian> shdr(ps);
	  cgccpp::Cgc_Word sh_type = shdr.get_sh_type();
	  if (sh_type == cgccpp::SHT_REL || sh_type == cgccpp::SHT_RELA)
	    {
      ...
      for (unsigned int i = 0; i < deferred_exidx_sections.size(); ++i)
	{
	  ...
	  cgccpp::Shdr<32, big_endian> shdr(pshdrs + shndx * shdr_size);
	  unsigned int text_shndx = cgccpp::SHN_UNDEF;
					     ...
					     Read_relocs_data* rd)
{
  ...
  for (unsigned int i = 1; i < shnum; ++i, ps += shdr_size)
    {
      cgccpp::Shdr<32, big_endian> shdr(ps);
      if (shdr.get_sh_type() == cgccpp::SHT_ARM_EXIDX)
	{
...
template<bool big_endian>
cgccpp::Cgc_types<32>::Cgc_Swxword
Stub_addend_reader<cgccpp::SHT_REL, big_endian>::operator()(
    unsigned int r_type,
    const unsigned char* view,
    const typename Reloc_types<cgccpp::SHT_REL, 32, big_endian>::Reloc&) const
{
  ...
  switch (r_type)
	...
	typedef typename cgccpp::Swap<16, big_endian>::Valtype Valtype;
	const Valtype* wv = reinterpret_cast<const Valtype*>(view);
	Valtype upper_insn = cgccpp::Swap<16, big_endian>::readval(wv);
	Valtype lower_insn = cgccpp::Swap<16, big_endian>::readval(wv + 1);
...
Arm_output_data_got<big_endian>::do_write(Output_file* of)
{
  ...
  for (size_t i = 0; i < this->static_relocs_.size(); ++i)
      ...
      if (!reloc.symbol_is_global())
	{
	  ...
	  if ((shndx == cgccpp::SHN_UNDEF)
	      || (is_ordinary
		  && shndx != cgccpp::SHN_UNDEF
      ...
      else
	{
      ...
      switch (reloc.r_type())
	{
	case cgccpp::R_ARM_TLS_DTPOFF32:
	  x = value;
	  break;
	case cgccpp::R_ARM_TLS_TPOFF32:
	  x = value + aligned_tcb_size;
	  break;
	default:
	  gold_unreachable();
	}
      cgccpp::Swap<32, big_endian>::writeval(wv, x);
    }
  ...
  : Output_section_data(addralign), got_plt_(got_plt), count_(0)
{
  ...
  layout->add_output_section_data(".rel.plt", cgccpp::SHT_REL,
				  cgccpp::SHF_ALLOC, this->rel_,
...
Output_data_plt_arm<big_endian>::add_entry(Symbol* gsym)
{
  ...
  this->rel_->add_global(gsym, cgccpp::R_ARM_JUMP_SLOT, this->got_plt_,
			 got_offset);
    ...
    Arm_address plt_address)
{
  ...
  for (size_t i = 0; i < num_first_plt_words - 1; i++)
    cgccpp::Swap<32, big_endian>::writeval(pov + i * 4, first_plt_entry[i]);
  // Last word in first PLT entry is &GOT[0] - .
  cgccpp::Swap<32, big_endian>::writeval(pov + 16,
    ...
    unsigned int plt_offset)
{
  ...
  cgccpp::Swap<32, big_endian>::writeval(pov, plt_insn0);
  uint32_t plt_insn1 = plt_entry[1] | ((offset >> 12) & 0xff);
  cgccpp::Swap<32, big_endian>::writeval(pov + 4, plt_insn1);
  uint32_t plt_insn2 = plt_entry[2] | (offset & 0xfff);
  cgccpp::Swap<32, big_endian>::writeval(pov + 8, plt_insn2);
}

// Write out the PLT.  This uses the hand-coded instructions above,
// and adjusts them as needed.  This is all specified by the arm CGC
...
Output_data_plt_arm<big_endian>::do_write(Output_file* of)
{
  ...
  for (unsigned int i = 0;
	 ...
	 got_offset += 4)
    {
      ...
      cgccpp::Swap<32, big_endian>::writeval(got_pov, plt_address);
    }
  ...
  if (this->plt_ == NULL)
    {
      ...
      layout->add_output_section_data(".plt", cgccpp::SHT_PROGBITS,
				      (cgccpp::SHF_ALLOC
				       | cgccpp::SHF_EXECINSTR),
    ...
    Sized_relobj_file<32, big_endian>* object)
{
  if (this->got_mod_index_offset_ == -1U)
      ...
      if (!parameters->doing_static_link())
	{
	  ...
	  rel_dyn->add_local(object, 0, cgccpp::R_ARM_TLS_DTPMOD32, got,
			     got_offset);
	}
      else
	{
...
Target_arm<big_endian>::Scan::get_reference_flags(unsigned int r_type)
{
  switch (r_type)
    ...
    case cgccpp::R_ARM_ABS32:
    case cgccpp::R_ARM_ABS16:
    case cgccpp::R_ARM_ABS12:
    case cgccpp::R_ARM_THM_ABS5:
    case cgccpp::R_ARM_ABS8:
    case cgccpp::R_ARM_BASE_ABS:
    case cgccpp::R_ARM_MOVW_ABS_NC:
    case cgccpp::R_ARM_MOVT_ABS:
    case cgccpp::R_ARM_THM_MOVW_ABS_NC:
    case cgccpp::R_ARM_THM_MOVT_ABS:
    case cgccpp::R_ARM_ABS32_NOI:
      return Symbol::ABSOLUTE_REF;

    case cgccpp::R_ARM_REL32:
    case cgccpp::R_ARM_LDR_PC_G0:
    case cgccpp::R_ARM_SBREL32:
    case cgccpp::R_ARM_THM_PC8:
    case cgccpp::R_ARM_BASE_PREL:
    case cgccpp::R_ARM_MOVW_PREL_NC:
    case cgccpp::R_ARM_MOVT_PREL:
    case cgccpp::R_ARM_THM_MOVW_PREL_NC:
    case cgccpp::R_ARM_THM_MOVT_PREL:
    case cgccpp::R_ARM_THM_ALU_PREL_11_0:
    case cgccpp::R_ARM_THM_PC12:
    case cgccpp::R_ARM_REL32_NOI:
    case cgccpp::R_ARM_ALU_PC_G0_NC:
    case cgccpp::R_ARM_ALU_PC_G0:
    case cgccpp::R_ARM_ALU_PC_G1_NC:
    case cgccpp::R_ARM_ALU_PC_G1:
    case cgccpp::R_ARM_ALU_PC_G2:
    case cgccpp::R_ARM_LDR_PC_G1:
    case cgccpp::R_ARM_LDR_PC_G2:
    case cgccpp::R_ARM_LDRS_PC_G0:
    case cgccpp::R_ARM_LDRS_PC_G1:
    case cgccpp::R_ARM_LDRS_PC_G2:
    case cgccpp::R_ARM_LDC_PC_G0:
    case cgccpp::R_ARM_LDC_PC_G1:
    case cgccpp::R_ARM_LDC_PC_G2:
    case cgccpp::R_ARM_ALU_SB_G0_NC:
    case cgccpp::R_ARM_ALU_SB_G0:
    case cgccpp::R_ARM_ALU_SB_G1_NC:
    case cgccpp::R_ARM_ALU_SB_G1:
    case cgccpp::R_ARM_ALU_SB_G2:
    case cgccpp::R_ARM_LDR_SB_G0:
    case cgccpp::R_ARM_LDR_SB_G1:
    case cgccpp::R_ARM_LDR_SB_G2:
    case cgccpp::R_ARM_LDRS_SB_G0:
    case cgccpp::R_ARM_LDRS_SB_G1:
    case cgccpp::R_ARM_LDRS_SB_G2:
    case cgccpp::R_ARM_LDC_SB_G0:
    case cgccpp::R_ARM_LDC_SB_G1:
    case cgccpp::R_ARM_LDC_SB_G2:
    case cgccpp::R_ARM_MOVW_BREL_NC:
    case cgccpp::R_ARM_MOVT_BREL:
    case cgccpp::R_ARM_MOVW_BREL:
    case cgccpp::R_ARM_THM_MOVW_BREL_NC:
    case cgccpp::R_ARM_THM_MOVT_BREL:
    case cgccpp::R_ARM_THM_MOVW_BREL:
    case cgccpp::R_ARM_GOTOFF32:
    case cgccpp::R_ARM_GOTOFF12:
    case cgccpp::R_ARM_SBREL31:
    ...
    case cgccpp::R_ARM_THM_JUMP24:
    case cgccpp::R_ARM_THM_JUMP19:
    case cgccpp::R_ARM_THM_JUMP6:
    case cgccpp::R_ARM_THM_JUMP11:
    case cgccpp::R_ARM_THM_JUMP8:
    ...
    case cgccpp::R_ARM_TLS_GD32:	// Global-dynamic
    case cgccpp::R_ARM_TLS_LDM32:	// Local-dynamic
    case cgccpp::R_ARM_TLS_LDO32:	// Alternate local-dynamic
    case cgccpp::R_ARM_TLS_IE32:	// Initial-exec
    case cgccpp::R_ARM_TLS_LE32:	// Local-exec
      return Symbol::TLS_REF;

    case cgccpp::R_ARM_TARGET1:
    case cgccpp::R_ARM_TARGET2:
    case cgccpp::R_ARM_COPY:
    case cgccpp::R_ARM_GLOB_DAT:
    case cgccpp::R_ARM_JUMP_SLOT:
    case cgccpp::R_ARM_RELATIVE:
    case cgccpp::R_ARM_PC24:
    case cgccpp::R_ARM_LDR_SBREL_11_0_NC:
    case cgccpp::R_ARM_ALU_SBREL_19_12_NC:
    case cgccpp::R_ARM_ALU_SBREL_27_20_CK:
    default:
					    ...
					    unsigned int r_type)
{
  switch (r_type)
    ...
    // linker.
    case cgccpp::R_ARM_TLS_DTPMOD32:
    case cgccpp::R_ARM_TLS_DTPOFF32:
    case cgccpp::R_ARM_TLS_TPOFF32:
      return;
...
Target_arm<big_endian>::Scan::local(Symbol_table* symtab,
				    ...
				    unsigned int data_shndx,
				    Output_section* output_section,
				    const cgccpp::Rel<32, big_endian>& reloc,
				    unsigned int r_type,
				    const cgccpp::Sym<32, big_endian>& lsym,
				    bool is_discarded)
{
  ...
  switch (r_type)
      ...
      if (parameters->options().output_is_position_independent())
	{
	  ...
	  unsigned int r_sym = cgccpp::cgc_r_sym<32>(reloc.get_r_info());
	  // If we are to add more other reloc types than R_ARM_ABS32,
	  // we need to add check_non_pic(object, r_type) here.
	  rel_dyn->add_local_relative(object, r_sym, cgccpp::R_ARM_RELATIVE,
      ...
      if (parameters->options().output_is_position_independent())
	{
	  ...
	  Reloc_section* rel_dyn = target->rel_dyn_section(layout);
	  unsigned int r_sym = cgccpp::cgc_r_sym<32>(reloc.get_r_info());
	  if (lsym.get_st_type() != cgccpp::STT_SECTION)
	...
	if (got->add_local(object, r_sym, GOT_TYPE_STANDARD))
	  {
		...
		rel_dyn->add_local_relative(
		    object, r_sym, cgccpp::R_ARM_RELATIVE, got,
	...
	switch (r_type)
	  ...
	  case cgccpp::R_ARM_TLS_IE32:		// Initial-exec
		...
		Arm_output_data_got<big_endian>* got
		  = target->got_section(symtab, layout);
		unsigned int r_sym =
		   cgccpp::cgc_r_sym<32>(reloc.get_r_info());
		if (!parameters->doing_static_link())
		    got->add_local_with_rel(object, r_sym, GOT_TYPE_TLS_OFFSET,
					    target->rel_dyn_section(layout),
					    cgccpp::R_ARM_TLS_TPOFF32);
						       ...
						       GOT_TYPE_TLS_OFFSET))
		  {
		    ...
		    got->add_static_reloc(got_offset,
					  cgccpp::R_ARM_TLS_TPOFF32, object,
					  r_sym);
		  }
	      }
	    else
	  ...
	  case cgccpp::R_ARM_TLS_LE32:		// Local-exec
		...
		gold_assert(lsym.get_st_type() != cgccpp::STT_SECTION);
		unsigned int r_sym = cgccpp::cgc_r_sym<32>(reloc.get_r_info());
		Reloc_section* rel_dyn = target->rel_dyn_section(layout);
		rel_dyn->add_local(object, r_sym, cgccpp::R_ARM_TLS_TPOFF32,
				   output_section, data_shndx,
				   reloc.get_r_offset());
	      }
	    break;
  ...
  switch (r_type)
	...
	if (gsym->needs_dynamic_reloc(Scan::get_reference_flags(r_type)))
	  {
	    ...
	    else if ((r_type == cgccpp::R_ARM_ABS32
		      || r_type == cgccpp::R_ARM_ABS32_NOI)
		     && gsym->can_use_relative_reloc(false))
	      {
		Reloc_section* rel_dyn = target->rel_dyn_section(layout);
		rel_dyn->add_global_relative(gsym, cgccpp::R_ARM_RELATIVE,
	...
	if (gsym->final_value_is_known())
	...
	else
	  {
		...
		|| gsym->is_undefined()
		|| gsym->is_preemptible()
		|| (gsym->visibility() == cgccpp::STV_PROTECTED
		    && parameters->options().shared()))
	      got->add_global_with_rel(gsym, GOT_TYPE_STANDARD,
				       rel_dyn, cgccpp::R_ARM_GLOB_DAT);
	    else
	      {
		if (got->add_global(gsym, GOT_TYPE_STANDARD))
		  rel_dyn->add_global_relative(
		      gsym, cgccpp::R_ARM_RELATIVE, got,
	...
	switch (r_type)
	  ...
	  case cgccpp::R_ARM_TLS_LE32:	// Local-exec
		...
		// We need to create a dynamic relocation.
		Reloc_section* rel_dyn = target->rel_dyn_section(layout);
		rel_dyn->add_global(gsym, cgccpp::R_ARM_TLS_TPOFF32,
				    output_section, object,
				    data_shndx, reloc.get_r_offset());
	      }
	    break;
    ...
    const unsigned char* plocal_symbols)
{
  ...
  gold::gc_process_relocs<32, big_endian, Arm, cgccpp::SHT_REL, Scan,
				    ...
				    const unsigned char* plocal_symbols)
{
  ...
  gold::scan_relocs<32, big_endian, Target_arm, cgccpp::SHT_REL, Scan>(
    symtab,
    layout,
    this,
    object,
    ...
    Symbol_table*)
{
  ...
  if (parameters->options().user_set_fix_cortex_a8())
	...
	this->get_aeabi_object_attribute(cgccpp::Tag_CPU_arch_profile);
      this->fix_cortex_a8_ =
	(cpu_arch_attr->int_value() == cgccpp::TAG_CPU_ARCH_V7
  ...
  if (!parameters->options().relocatable())
    {
	  ...
	  && exidx_section->type() == cgccpp::SHT_ARM_EXIDX)
	{
	  // For the ARM target, we need to add a PT_ARM_EXIDX segment for
	  // the .ARM.exidx section.
	  if (!layout->script_options()->saw_phdrs_clause())
	    {
	      gold_assert(layout->find_output_segment(cgccpp::PT_ARM_EXIDX, 0,
						      0)
			  == NULL);
	      Output_segment*  exidx_segment =
		layout->make_output_segment(cgccpp::PT_ARM_EXIDX, cgccpp::PF_R);
  ...
  for (Layout::Section_list::const_iterator p = layout->section_list().begin();
       ...
       ++p)
    if ((*p)->type() == cgccpp::SHT_ARM_EXIDX)
      {
    ...
    Output_section* output_section)
{
  ...
  // the reloc here.
  if ((output_section->flags() & cgccpp::SHF_ALLOC) == 0)
      return true;
...
Target_arm<big_endian>::Relocate::relocate(
    ...
    Output_section* output_section,
    size_t relnum,
    const cgccpp::Rel<32, big_endian>& rel,
    unsigned int r_type,
    const Sized_symbol<32>* gsym,
    const Symbol_value<32>* psymval,
    unsigned char* view,
  ...
  if (reloc_property->uses_got_entry())
	...
	case cgccpp::R_ARM_GOT_PREL:
	  if (gsym != NULL)
	    ...
	    {
	      unsigned int r_sym = cgccpp::cgc_r_sym<32>(rel.get_r_info());
      ...
      if (gsym != NULL)
	{
	  ...
	  if (gsym->use_plt_offset(Scan::get_reference_flags(r_type)))
	  ...
	  else
	    {
	      ...
	      thumb_bit =
		(((gsym->type() == cgccpp::STT_ARM_TFUNC)
		 || (gsym->type() == cgccpp::STT_FUNC
		     && !gsym->is_undefined()
      ...
      else
	{
	  ...
	  cgccpp::Cgc_types<32>::Cgc_WXword r_info = rel.get_r_info();
	  unsigned int r_sym = cgccpp::cgc_r_sym<32>(r_info);
...
Target_arm<big_endian>::Relocate::relocate_tls(
    ...
    const Symbol_value<32>* psymval,
    unsigned char* view,
    cgccpp::Cgc_types<32>::Cgc_Addr address,
    section_size_type /*view_size*/ )
{
  ...
  cgccpp::Cgc_types<32>::Cgc_Addr value = psymval->value(object, 0);
    ...
    const Reloc_symbol_changes* reloc_symbol_changes)
{
  ...
  gold_assert(sh_type == cgccpp::SHT_REL);
  ...
  gold::relocate_section<32, big_endian, Target_arm, cgccpp::SHT_REL,
    ...
    Relocatable_relocs* rr)
{
  ...
  typedef Arm_scan_relocatable_relocs<big_endian, cgccpp::SHT_REL,
    Relocatable_size_for_reloc> Scan_relocatable_relocs;

  gold::scan_relocatable_relocs<32, big_endian, cgccpp::SHT_REL,
...
Target_arm<big_endian>::relocate_relocs(
    ...
    size_t reloc_count,
    Output_section* output_section,
    typename cgccpp::Cgc_types<32>::Cgc_Off offset_in_output_section,
    ...
    section_size_type reloc_view_size)
{
  ...
  gold::relocate_relocs<32, big_endian, cgccpp::SHT_REL>(
    relinfo,
    prelocs,
    reloc_count,
    output_section,
    ...
    unsigned char* preloc_out)
{
  ...
  typedef typename Reloc_types<cgccpp::SHT_REL, 32, big_endian>::Reloc Reltype;
  typedef typename Reloc_types<cgccpp::SHT_REL, 32, big_endian>::Reloc_write
    Reltype_write;
  ...
  cgccpp::Cgc_types<32>::Cgc_WXword r_info = reloc.get_r_info();
  const unsigned int r_sym = cgccpp::cgc_r_sym<32>(r_info);
  const unsigned int r_type = cgccpp::cgc_r_type<32>(r_info);
  ...
  reloc_write.put_r_info(cgccpp::cgc_r_info<32>(new_symndx, r_type));
    ...
    cgccpp::Cgc_Word v2)
{
  ...
  if ((v1 == cgccpp::EF_ARM_EABI_UNKNOWN || v2 == cgccpp::EF_ARM_EABI_UNKNOWN)
      || (v1 == cgccpp::EF_ARM_EABI_VER4 && v2 == cgccpp::EF_ARM_EABI_VER5)
      || (v1 == cgccpp::EF_ARM_EABI_VER5 && v2 == cgccpp::EF_ARM_EABI_VER4))
    return true;
...
// flags in the CGC header of the output.  Much of this is adapted from the
// processor-specific flags merging code in cgc32_arm_merge_private_bfd_data
// in bfd/cgc32-arm.c.
...
Target_arm<big_endian>::merge_processor_specific_flags(
    ...
    cgccpp::Cgc_Word flags)
{
  if (this->are_processor_specific_flags_set())
    {
      cgccpp::Cgc_Word out_flags = this->processor_specific_flags();
	...
	return;
      ...
      cgccpp::Cgc_Word version1 = cgccpp::arm_eabi_version(flags);
      cgccpp::Cgc_Word version2 = cgccpp::arm_eabi_version(out_flags);
	...
	gold_error(_("Source object %s has EABI version %d but output has "
		     "EABI version %d."),
		   name.c_str(),
		   (flags & cgccpp::EF_ARM_EABIMASK) >> 24,
		   (out_flags & cgccpp::EF_ARM_EABIMASK) >> 24);
    }
  else
    {
	...
	return;
      ...
      this->set_processor_specific_flags(flags & cgccpp::EF_ARM_EABIMASK);
    }
}

// Adjust CGC file header.
template<bool big_endian>
void
Target_arm<big_endian>::do_adjust_cgc_header(
    unsigned char* view,
    int len)
{
  gold_assert(len == cgccpp::Cgc_sizes<32>::ehdr_size);

  cgccpp::Ehdr<32, big_endian> ehdr(view);
  cgccpp::Cgc_Word flags = this->processor_specific_flags();
  unsigned char e_ident[cgccpp::EI_NIDENT];
  memcpy(e_ident, ehdr.get_e_ident(), cgccpp::EI_NIDENT);

  if (cgccpp::arm_eabi_version(flags)
      == cgccpp::EF_ARM_EABI_UNKNOWN)
    e_ident[cgccpp::EI_OSABI] = cgccpp::CGCOSABI_ARM;
  else
    e_ident[cgccpp::EI_OSABI] = 0;
  e_ident[cgccpp::EI_ABIVERSION] = 0;

  // FIXME: Do EF_ARM_BE8 adjustment.

  // If we're working in EABI_VER5, set the hard/soft float ABI flags
  // as appropriate.
  if (cgccpp::arm_eabi_version(flags) == cgccpp::EF_ARM_EABI_VER5)
  {
    cgccpp::Cgc_Half type = ehdr.get_e_type();
    if (type == cgccpp::ET_EXEC || type == cgccpp::ET_DYN)
      {
	Object_attribute* attr = this->get_aeabi_object_attribute(cgccpp::Tag_ABI_VFP_args);
	if (attr->int_value())
	  flags |= cgccpp::EF_ARM_ABI_FLOAT_HARD;
	else
	  flags |= cgccpp::EF_ARM_ABI_FLOAT_SOFT;
	this->set_processor_specific_flags(flags);
      }
  }
  cgccpp::Ehdr_write<32, big_endian> oehdr(view);
  oehdr.put_e_ident(e_ident);
}

// do_make_cgc_object to override the same function in the base class.
// We need to use a target-specific sub-class of
// Sized_relobj_file<32, big_endian> to store ARM specific information.
// Hence we need to have our own CGC object creation.

template<bool big_endian>
Object*
Target_arm<big_endian>::do_make_cgc_object(
    const std::string& name,
    Input_file* input_file,
    off_t offset, const cgccpp::Ehdr<32, big_endian>& ehdr)
{
  int et = ehdr.get_e_type();
  // ET_EXEC files are valid input for --just-symbols/-R,
  // and we treat them as relocatable objects.
  if (et == cgccpp::ET_REL
      || (et == cgccpp::ET_EXEC && input_file->just_symbols()))
    {
    ...
    {
      gold_error(_("%s: unsupported CGC file type %d"),
		 name.c_str(), et);
      return NULL;
    }
}
...
// This is adapted from get_secondary_compatible_arch() in bfd/cgc32-arm.c.
    ...
    const Attributes_section_data* pasd)
{
  ...
  const std::string& sv =
    known_attributes[cgccpp::Tag_also_compatible_with].string_value();
  if (sv.size() == 2
      && sv.data()[0] == cgccpp::Tag_CPU_arch
      && (sv.data()[1] & 128) != 128)
   return sv.data()[1];
...
// This is adapted from set_secondary_compatible_arch() in bfd/cgc32-arm.c.
    ...
    int arch)
{
  ...
  if (arch == -1)
    {
      known_attributes[cgccpp::Tag_also_compatible_with].set_string_value("");
      return;
    }
  ...
  // currently-defined values fit in one byte for each.
  char sv[3];
  sv[0] = cgccpp::Tag_CPU_arch;
  gold_assert(arch != 0);
  sv[1] = arch;
  sv[2] = '\0';

  known_attributes[cgccpp::Tag_also_compatible_with].set_string_value(sv);
}

// Combine two values for Tag_CPU_arch, taking secondary compatibility tags
// into account.
// This is adapted from tag_cpu_arch_combine() in bfd/cgc32-arm.c.
...
{
#define T(X) cgccpp::TAG_CPU_ARCH_##X
  ...
  if (oldtag > cgccpp::MAX_TAG_CPU_ARCH || newtag > cgccpp::MAX_TAG_CPU_ARCH)
    {
  ...
  if (tagh <= cgccpp::TAG_CPU_ARCH_V6KZ)
    return tagh;
    ...
    const Object_attribute* div_attr)
{
  switch (div_attr->int_value())
    ...
    case 0:
      ...
      // archetecture.
      if (arch == cgccpp::TAG_CPU_ARCH_V7 && (profile == 'R' || profile == 'M'))
        return true;
      else if (arch >= cgccpp::TAG_CPU_ARCH_V7E_M)
        return true;
      else
        return false;
    ...
    const Attributes_section_data* pasd)
{
  ...
  if (this->attributes_section_data_ == NULL)
	...
	this->attributes_section_data_->known_attributes(vendor);
      ...
      if (out_attr[cgccpp::Tag_MPextension_use_legacy].int_value() != 0)
	{
	  if (out_attr[cgccpp::Tag_MPextension_use].int_value() != 0
	      && out_attr[cgccpp::Tag_MPextension_use_legacy].int_value()
		!= out_attr[cgccpp::Tag_MPextension_use].int_value())
	    {
	  ...
	  out_attr[cgccpp::Tag_MPextension_use] =
	    out_attr[cgccpp::Tag_MPextension_use_legacy];
	  out_attr[cgccpp::Tag_MPextension_use_legacy].set_type(0);
	  out_attr[cgccpp::Tag_MPextension_use_legacy].set_int_value(0);
	}
  ...
  if (in_attr[cgccpp::Tag_ABI_VFP_args].int_value()
      != out_attr[cgccpp::Tag_ABI_VFP_args].int_value())
    {
      // Ignore mismatches if the object doesn't use floating point.  */
      if (out_attr[cgccpp::Tag_ABI_FP_number_model].int_value() == 0)
	out_attr[cgccpp::Tag_ABI_VFP_args].set_int_value(
	    in_attr[cgccpp::Tag_ABI_VFP_args].int_value());
      else if (in_attr[cgccpp::Tag_ABI_FP_number_model].int_value() != 0
	       && parameters->options().warn_mismatch())
  ...
  for (int i = 4; i < Vendor_object_attributes::NUM_KNOWN_ATTRIBUTES; ++i)
    {
      ...
      switch (i)
	{
	case cgccpp::Tag_CPU_raw_name:
	case cgccpp::Tag_CPU_name:
	  // These are merged after Tag_CPU_arch.
	  break;

	case cgccpp::Tag_ABI_optimization_goals:
	case cgccpp::Tag_ABI_FP_optimization_goals:
	  // Use the first value seen.
	  break;

	case cgccpp::Tag_CPU_arch:
	  {
		...
		out_attr[cgccpp::Tag_CPU_name].set_string_value(
		    in_attr[cgccpp::Tag_CPU_name].string_value());
		out_attr[cgccpp::Tag_CPU_raw_name].set_string_value(
		    in_attr[cgccpp::Tag_CPU_raw_name].string_value());
	      }
	    else
	      {
		out_attr[cgccpp::Tag_CPU_name].set_string_value("");
	    ...
	    if (out_attr[cgccpp::Tag_CPU_name].string_value() == "")
	      {
		...
		out_attr[cgccpp::Tag_CPU_name].set_string_value(cpu_name);
	      }
	  }
	  break;

	case cgccpp::Tag_ARM_ISA_use:
	case cgccpp::Tag_THUMB_ISA_use:
	case cgccpp::Tag_WMMX_arch:
	case cgccpp::Tag_Advanced_SIMD_arch:
	  // ??? Do Advanced_SIMD (NEON) and WMMX conflict?
	case cgccpp::Tag_ABI_FP_rounding:
	case cgccpp::Tag_ABI_FP_exceptions:
	case cgccpp::Tag_ABI_FP_user_exceptions:
	case cgccpp::Tag_ABI_FP_number_model:
	case cgccpp::Tag_VFP_HP_extension:
	case cgccpp::Tag_CPU_unaligned_access:
	case cgccpp::Tag_T2EE_use:
	case cgccpp::Tag_Virtualization_use:
	case cgccpp::Tag_MPextension_use:
	  // Use the largest value specified.
	  if (in_attr[i].int_value() > out_attr[i].int_value())
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;

	case cgccpp::Tag_ABI_align8_preserved:
	case cgccpp::Tag_ABI_PCS_RO_data:
	  // Use the smallest value specified.
	  if (in_attr[i].int_value() < out_attr[i].int_value())
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;

	case cgccpp::Tag_ABI_align8_needed:
	  if ((in_attr[i].int_value() > 0 || out_attr[i].int_value() > 0)
	      && (in_attr[cgccpp::Tag_ABI_align8_preserved].int_value() == 0
		  || (out_attr[cgccpp::Tag_ABI_align8_preserved].int_value()
		      == 0)))
	    {
	  ...
	  // Fall through.
	case cgccpp::Tag_ABI_FP_denormal:
	case cgccpp::Tag_ABI_PCS_GOT_use:
	  {
	...
	case cgccpp::Tag_CPU_arch_profile:
	...
	case cgccpp::Tag_PCS_config:
	  ...
	  break;
	case cgccpp::Tag_ABI_PCS_R9_use:
	  if (in_attr[i].int_value() != out_attr[i].int_value()
	      && out_attr[i].int_value() != cgccpp::AEABI_R9_unused
	      && in_attr[i].int_value() != cgccpp::AEABI_R9_unused
	      && parameters->options().warn_mismatch())
	    {
	      gold_error(_("%s: conflicting use of R9"), name);
	    }
	  if (out_attr[i].int_value() == cgccpp::AEABI_R9_unused)
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;
	case cgccpp::Tag_ABI_PCS_RW_data:
	  if (in_attr[i].int_value() == cgccpp::AEABI_PCS_RW_data_SBrel
	      && (in_attr[cgccpp::Tag_ABI_PCS_R9_use].int_value()
		  != cgccpp::AEABI_R9_SB)
	      && (out_attr[cgccpp::Tag_ABI_PCS_R9_use].int_value()
		  != cgccpp::AEABI_R9_unused)
	      && parameters->options().warn_mismatch())
	    {
	  ...
	  if (in_attr[i].int_value() < out_attr[i].int_value())
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;
	case cgccpp::Tag_ABI_PCS_wchar_t:
	  if (out_attr[i].int_value()
	    ...
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;
	case cgccpp::Tag_ABI_enum_size:
	  if (in_attr[i].int_value() != cgccpp::AEABI_enum_unused)
	    {
	      if (out_attr[i].int_value() == cgccpp::AEABI_enum_unused
		  || out_attr[i].int_value() == cgccpp::AEABI_enum_forced_wide)
		{
		  // The existing object is compatible with anything.
		  // Use whatever requirements the new object has.
		  out_attr[i].set_int_value(in_attr[i].int_value());
		}
	      else if (in_attr[i].int_value() != cgccpp::AEABI_enum_forced_wide
		       && out_attr[i].int_value() != in_attr[i].int_value()
		       && parameters->options().warn_mismatch()
	  ...
	  break;
	case cgccpp::Tag_ABI_VFP_args:
	  // Already done.
	  break;
	case cgccpp::Tag_ABI_WMMX_args:
	  if (in_attr[i].int_value() != out_attr[i].int_value()
	      && parameters->options().warn_mismatch())
	    {
	...
	case Object_attribute::Tag_compatibility:
	  ...
	  break;
	case cgccpp::Tag_ABI_HardFP_use:
	  ...
	  if ((in_attr[i].int_value() == 1 && out_attr[i].int_value() == 2)
	    ...
	    out_attr[i].set_int_value(in_attr[i].int_value());
	  break;
	case cgccpp::Tag_ABI_FP_16bit_format:
	  if (in_attr[i].int_value() != 0 && out_attr[i].int_value() != 0)
	    {
	      if (in_attr[i].int_value() != out_attr[i].int_value()
		  && parameters->options().warn_mismatch())
	...
	case cgccpp::Tag_DIV_use:
	  {
	    ...
	    int arch = this->
	      get_aeabi_object_attribute(cgccpp::Tag_CPU_arch)->
	      int_value();
	    int profile = this->
	      get_aeabi_object_attribute(cgccpp::Tag_CPU_arch_profile)->
	      int_value();
	    if (in_attr[i].int_value() == out_attr[i].int_value())
	      {
		// Do nothing.
	...
	case cgccpp::Tag_MPextension_use_legacy:
	  // We don't output objects with Tag_MPextension_use_legacy - we
	  // move the value to Tag_MPextension_use.
	  if (in_attr[i].int_value() != 0
	      && in_attr[cgccpp::Tag_MPextension_use].int_value() != 0)
	    {
	      if (in_attr[cgccpp::Tag_MPextension_use].int_value()
		  != in_attr[i].int_value())
		{
	  ...
	  if (in_attr[i].int_value()
	      > out_attr[cgccpp::Tag_MPextension_use].int_value())
	    out_attr[cgccpp::Tag_MPextension_use] = in_attr[i];

	  break;

	case cgccpp::Tag_nodefaults:
	  // This tag is set if it exists, but the value is unused (and is
	  // typically zero).  We don't actually need to do anything here -
	  // the merge happens automatically when the type flags are merged
	  // below.
	  break;
	case cgccpp::Tag_also_compatible_with:
	  // Already done in Tag_CPU_arch.
	  break;
	case cgccpp::Tag_conformance:
...
Target_arm<big_endian>::scan_reloc_for_stub(
    ...
    unsigned int r_sym,
    const Symbol_value<32>* psymval,
    cgccpp::Cgc_types<32>::Cgc_Swxword addend,
    Arm_address address)
{
  ...
  if (this->fix_cortex_a8_
      && (r_type == cgccpp::R_ARM_THM_JUMP24
	  || r_type == cgccpp::R_ARM_THM_JUMP19
	  || r_type == cgccpp::R_ARM_THM_CALL
	  || r_type == cgccpp::R_ARM_THM_XPC22)
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
    {
      ...
      typename cgccpp::Cgc_types<32>::Cgc_WXword r_info = reloc.get_r_info();
      unsigned int r_sym = cgccpp::cgc_r_sym<32>(r_info);
      unsigned int r_type = cgccpp::cgc_r_type<32>(r_info);
      ...
      if (r_type == cgccpp::R_ARM_V4BX)
	{
	  if (this->fix_v4bx() == General_options::FIX_V4BX_INTERWORKING)
		...
		reinterpret_cast<const Valtype*>(view + offset);
	      cgccpp::Cgc_types<32>::Cgc_Swxword insn =
		cgccpp::Swap<32, big_endian>::readval(wv);
      ...
      else
	{
	  ...
	  if (gsym->type() == cgccpp::STT_TLS)
	    symval.set_is_tls_symbol();
	  else if (gsym->type() == cgccpp::STT_GNU_IFUNC)
	  ...
	  if (comdat_behavior == CB_PRETEND)
	    {
	      ...
	      // script.
	      bool found;
	      typename cgccpp::Cgc_types<32>::Cgc_Addr value =
...
{
  if (sh_type == cgccpp::SHT_REL)
    this->scan_reloc_section_for_stubs<cgccpp::SHT_REL>(
	relinfo,
	prelocs,
	reloc_count,
	output_section,
	...
	view_address,
	view_size);
  else if (sh_type == cgccpp::SHT_RELA)
    // We do not support RELA type relocations yet.  This is provided for
    // completeness.
    this->scan_reloc_section_for_stubs<cgccpp::SHT_RELA>(
	relinfo,
	prelocs,
	reloc_count,
	output_section,
  ...
  for (size_t i = 0; i < stub_template->reloc_count(); i++)
    {
      ...
      // we use 0.
      unsigned char reloc_buffer[cgccpp::Cgc_sizes<32>::rel_size];
      memset(reloc_buffer, 0, sizeof(reloc_buffer));
      cgccpp::Rel_write<32, big_endian> reloc_write(reloc_buffer);
      reloc_write.put_r_offset(reloc_offset);
      reloc_write.put_r_info(cgccpp::cgc_r_info<32>(0, r_type));
      cgccpp::Rel<32, big_endian> rel(reloc_buffer);
...
Target_arm<big_endian>::do_attribute_arg_type(int tag) const
{
  ...
  else if (tag == cgccpp::Tag_nodefaults)
    return (Object_attribute::ATTR_TYPE_FLAG_INT_VAL
	    | Object_attribute::ATTR_TYPE_FLAG_NO_DEFAULT);
  else if (tag == cgccpp::Tag_CPU_raw_name || tag == cgccpp::Tag_CPU_name)
...
Target_arm<big_endian>::do_attributes_order(int num) const
{
  ...
  if (num == 4)
    return cgccpp::Tag_conformance;
  if (num == 5)
    return cgccpp::Tag_nodefaults;
  if ((num - 2) < cgccpp::Tag_nodefaults)
    return num - 2;
  if ((num - 1) < cgccpp::Tag_conformance)
    return num - 1;
  return num;
}
  ...
  while (i < span_end)
    {
      ...
      uint32_t insn = cgccpp::Swap<16, big_endian>::readval(wv);
      ...
      if (insn_32bit)
	{
	  ...
	  insn = (insn << 16) | cgccpp::Swap<16, big_endian>::readval(wv + 1);
	  ...
	  if (p != this->cortex_a8_relocs_info_.end())
	    {
	      ...
	      if (cortex_a8_reloc->r_type() == cgccpp::R_ARM_THM_CALL
		  && !target_is_thumb)
		force_target_arm = true;
	      else if (cortex_a8_reloc->r_type() == cgccpp::R_ARM_THM_CALL
		       && target_is_thumb)
...
class Target_selector_arm : public Target_selector
{
 public:
  Target_selector_arm()
    : Target_selector(cgccpp::EM_ARM, 32, big_endian,
		      (big_endian ? "cgc32-bigarm" : "cgc32-littlearm"),
		      (big_endian ? "armcgcb" : "armcgc"))
  { }
	  ...
	  if (!exidx_input_section->has_errors())
	    {
	      unsigned int text_shndx = exidx_input_section->link();
	      Output_section* os = arm_relobj->output_section(text_shndx);
	      if (os != NULL && (os->flags() & cgccpp::SHF_ALLOC) != 0)
  ...
  if (exidx_section != NULL)
  ...
  else
    {
      ...
      symtab->define_as_constant("__exidx_start", NULL,
				 Symbol_table::PREDEFINED,
				 0, 0, cgccpp::STT_OBJECT,
				 cgccpp::STB_GLOBAL, cgccpp::STV_HIDDEN, 0,
				 true, false);
    ...
    Arm_address plt_address)
{
  ...
  cgccpp::Swap<32, big_endian>::writeval
 ...
 public:
  Target_selector_arm_nacl()
    : Target_selector_nacl<Target_selector_arm<big_endian>,
			   Target_arm_nacl<big_endian> >(
	  "arm",
	  big_endian ? "cgc32-bigarm-nacl" : "cgc32-littlearm-nacl",
	  big_endian ? "armcgcb_nacl" : "armcgc_nacl")
  { }
};
