...
Symbol::init_fields(const char* name, const char* version,
		    cgccpp::STT type, cgccpp::STB binding,
		    cgccpp::STV visibility, unsigned char nonvis)
{
  this->name_ = name;
...
Symbol::init_base_object(const char* name, const char* version, Object* object,
			 const cgccpp::Sym<size, big_endian>& sym,
			 unsigned int st_shndx, bool is_ordinary)
{
  ...
  this->in_real_cgc_ = object->pluginobj() == NULL;
}
...
Symbol::init_base_output_data(const char* name, const char* version,
			      Output_data* od, cgccpp::STT type,
			      cgccpp::STB binding, cgccpp::STV visibility,
...
Symbol::init_base_output_segment(const char* name, const char* version,
				 Output_segment* os, cgccpp::STT type,
				 cgccpp::STB binding, cgccpp::STV visibility,
...
Sized_symbol<size>::init_output_data(const char* name, const char* version,
				     ...
				     Size_type symsize, cgccpp::STT type,
				     cgccpp::STB binding,
				     cgccpp::STV visibility,
				     unsigned char nonvis,
...
{
  return (shndx == cgccpp::SHN_COMMON
...
Symbol::should_add_dynsym_entry(Symbol_table* symtab) const
{
  ...
  if (parameters->options().gc_sections() 
      ...
      && !this->object()->is_dynamic())
    {
      ...
      if (is_ordinary && shndx != cgccpp::SHN_UNDEF
...
Symbol::output_section() const
{
  switch (this->source_)
	...
	if (shndx != cgccpp::SHN_UNDEF && this->is_ordinary_shndx_)
	  {
...
// this the slow way, by converting back to an CGC symbol.

template<int size, bool big_endian>
void
Symbol_table::resolve(Sized_symbol<size>* to, const Sized_symbol<size>* from)
{
  unsigned char buf[cgccpp::Cgc_sizes<size>::sym_size];
  cgccpp::Sym_write<size, big_endian> esym(buf);
				     ...
				     Symbol_table_type::iterator pdef)
{
  if (default_is_new)
	...
	gold_assert(pdef->second->version() != sym->version());
      else if (sym->visibility() != cgccpp::STV_DEFAULT
	       && pdef->second->is_from_dynobj())
	;
      else if (pdef->second->visibility() != cgccpp::STV_DEFAULT
	       && sym->is_from_dynobj())
	;
      else
	{
			      ...
			      unsigned int orig_st_shndx)
{
  ...
  if (parameters->options().is_trace_symbol(name))
    {
      if (orig_st_shndx == cgccpp::SHN_UNDEF)
  ...
  if ((ret->visibility() == cgccpp::STV_HIDDEN
       || ret->visibility() == cgccpp::STV_INTERNAL)
      && (ret->binding() == cgccpp::STB_GLOBAL
	  || ret->binding() == cgccpp::STB_GNU_UNIQUE
	  || ret->binding() == cgccpp::STB_WEAK)
      && !parameters->options().relocatable())
  ...
  *defined = 0;
  ...
  for (size_t i = 0; i < count; ++i, p += sym_size)
      ...
      if (st_name >= sym_name_size)
	{
      ...
      if (!is_ordinary)
	orig_st_shndx = cgccpp::SHN_UNDEF;
      ...
      else
	{
      ...
      cgccpp::Sym<size, big_endian>* psym = &sym;
      unsigned char symbuf[sym_size];
      cgccpp::Sym<size, big_endian> sym2(symbuf);
      if (just_symbols)
	{
	  memcpy(symbuf, p, sym_size);
	  cgccpp::Sym_write<size, big_endian> sw(symbuf);
	  if (orig_st_shndx != cgccpp::SHN_UNDEF
	      && is_ordinary
	      && relobj->e_type() == cgccpp::ET_REL)
	    {
      ...
      if (relobj->no_export()
	  && (orig_st_shndx != cgccpp::SHN_UNDEF || !is_ordinary))
        {
	  ...
	  cgccpp::STV visibility = sym2.get_st_visibility();
	  if (visibility == cgccpp::STV_DEFAULT
	      || visibility == cgccpp::STV_PROTECTED)
	    {
	      cgccpp::Sym_write<size, big_endian> sw(symbuf);
	      unsigned char nonvis = sym2.get_st_nonvis();
	      sw.put_st_other(cgccpp::STV_HIDDEN, nonvis);
	    }
        }
...
Symbol_table::add_from_pluginobj(
    ...
    const char* name,
    const char* ver,
    cgccpp::Sym<size, big_endian>* sym)
{
  unsigned int st_shndx = sym->get_st_shndx();
  bool is_ordinary = st_shndx < cgccpp::SHN_LORESERVE;
  ...
  *defined = 0;
  ...
  for (size_t i = 0; i < count; ++i, p += sym_size, vs += 2)
	...
	(*sympointers)[i] = NULL;
      ...
      if (sym.get_st_bind() == cgccpp::STB_LOCAL
          || sym.get_st_visibility() == cgccpp::STV_INTERNAL
          || sym.get_st_visibility() == cgccpp::STV_HIDDEN)
	continue;
      ...
      unsigned char symbuf[sym_size];
      cgccpp::Sym<size, big_endian> sym2(symbuf);
      if (sym.get_st_visibility() == cgccpp::STV_PROTECTED)
	{
	  memcpy(symbuf, p, sym_size);
	  cgccpp::Sym_write<size, big_endian> sw(symbuf);
	  sw.put_st_other(cgccpp::STV_DEFAULT, sym.get_st_nonvis());
	  psym = &sym2;
	}
      ...
      else
	{
	  ...
	  unsigned int v = cgccpp::Swap<16, big_endian>::readval(vs);

	  bool hidden = (v & cgccpp::VERSYM_HIDDEN) != 0;
	  v &= cgccpp::VERSYM_VERSION;
	  ...
	  if (v == static_cast<unsigned int>(cgccpp::VER_NDX_LOCAL)
	      && st_shndx != cgccpp::SHN_UNDEF)
	    {
	  ...
	  if (v == static_cast<unsigned int>(cgccpp::VER_NDX_LOCAL)
	      || v == static_cast<unsigned int>(cgccpp::VER_NDX_GLOBAL))
	    {
	      ...
	      else
		{
		  const bool is_default_version =
		    !hidden && st_shndx != cgccpp::SHN_UNDEF;
      ...
      if (st_shndx != cgccpp::SHN_UNDEF
	  && is_ordinary
	  && psym->get_st_type() == cgccpp::STT_OBJECT
				    ...
				    const Sized_symbol<size>* s2) const
{
  ...
  if (s1->binding() != s2->binding())
    {
      if (s1->binding() == cgccpp::STB_WEAK)
	return true;
      if (s2->binding() == cgccpp::STB_WEAK)
	return false;
    }
...
Symbol_table::do_define_in_output_data(
    ...
    typename cgccpp::Cgc_types<size>::Cgc_WXword symsize,
    cgccpp::STT type,
    cgccpp::STB binding,
    cgccpp::STV visibility,
    unsigned char nonvis,
    ...
    typename cgccpp::Cgc_types<size>::Cgc_Addr value)
{
  ...
  // library.
  cgccpp::STB binding = csym->binding();
  if (binding == cgccpp::STB_WEAK)
    binding = cgccpp::STB_GLOBAL;
...
Symbol_table::add_undefined_symbol_from_command_line(const char* name)
{
  ...
  sym->init_undefined(name, version, cgccpp::STT_NOTYPE, cgccpp::STB_GLOBAL,
		      cgccpp::STV_DEFAULT, 0);
  ++this->saw_undefined_;
}
				  ...
				  unsigned int* pindex, off_t* poff)
{
  ...
  ++*pindex;
  *poff += cgccpp::Cgc_sizes<size>::sym_size;
}
    ...
    Compute_final_value_status* pstatus) const
{
  ...
  switch (sym->source())
	...
	else if (shndx == cgccpp::SHN_UNDEF)
	  value = 0;
	else if (!is_ordinary
		 && (shndx == cgccpp::SHN_ABS
		     || Symbol::is_common_shndx(shndx)))
				  ...
				  Output_file* of) const
{
  ...
  for (Symbol_table_type::const_iterator p = this->table_.begin();
      ...
      if (sym_index == -1U && dynsym_index == -1U)
	{
      ...
      unsigned int shndx;
      typename cgccpp::Cgc_types<size>::Cgc_Addr sym_value = sym->value();
      typename cgccpp::Cgc_types<size>::Cgc_Addr dynsym_value = sym_value;
      cgccpp::STB binding = sym->binding();

      // If --no-gnu-unique is set, change STB_GNU_UNIQUE to STB_GLOBAL.
      if (binding == cgccpp::STB_GNU_UNIQUE
		...
		if (symobj->is_dynamic())
		  {
		    ...
		    if (sym->is_undef_binding_weak())
		      binding = cgccpp::STB_WEAK;
		    else
		      binding = cgccpp::STB_GLOBAL;
		  }
		else if (symobj->pluginobj() != NULL)
		  shndx = cgccpp::SHN_UNDEF;
		else if (in_shndx == cgccpp::SHN_UNDEF
			 || (!is_ordinary
			     && (in_shndx == cgccpp::SHN_ABS
				 || Symbol::is_common_shndx(in_shndx))))
		  shndx = in_shndx;
...
{
  cgccpp::Sym_write<size, big_endian> osym(p);
  if (sym->version() == NULL || !parameters->options().relocatable())
  ...
  if (shndx == cgccpp::SHN_UNDEF && sym->is_from_dynobj())
    osym.put_st_size(0);
  else
    osym.put_st_size(sym->symsize());
  cgccpp::STT type = sym->type();
  // Turn IFUNC symbols from shared libraries into normal FUNC symbols.
  if (type == cgccpp::STT_GNU_IFUNC
      && sym->is_from_dynobj())
    type = cgccpp::STT_FUNC;
  // A version script may have overridden the default binding.
  if (sym->is_forced_local())
    osym.put_st_info(cgccpp::cgc_st_info(cgccpp::STB_LOCAL, type));
  else
    osym.put_st_info(cgccpp::cgc_st_info(binding, type));
  osym.put_st_other(cgccpp::cgc_st_other(sym->visibility(), sym->nonvis()));
...
Symbol_table::warn_about_undefined_dynobj_symbol(Symbol* sym) const
{
  ...
  if (sym->source() == Symbol::FROM_OBJECT
      && sym->object()->is_dynamic()
      && sym->shndx(&dummy) == cgccpp::SHN_UNDEF
      && sym->binding() != cgccpp::STB_WEAK
					 ...
					 off_t offset) const
{
  ...
  cgccpp::Sym_write<size, big_endian> osym(pov);
  osym.put_st_name(0);
  if (parameters->options().relocatable())
    osym.put_st_value(0);
  else
  ...
  osym.put_st_size(0);
  osym.put_st_info(cgccpp::cgc_st_info(cgccpp::STB_LOCAL,
				       cgccpp::STT_SECTION));
  osym.put_st_other(cgccpp::cgc_st_other(cgccpp::STV_DEFAULT, 0));
...
Symbol_table::define_with_copy_reloc<64>(
    Sized_symbol<64>* sym,
    Output_data* posd,
    cgccpp::Cgc_types<64>::Cgc_Addr value);
#endif
