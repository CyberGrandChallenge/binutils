...
// possible, e.g. FILE is not an CGC file or has an unsupported target.
...
class Incremental_inputs_reader
{
 ...
 public:
  ...
  Incremental_inputs_reader(const unsigned char* p,
			    const cgccpp::Cgc_strtab& strtab)
    : p_(p), strtab_(strtab)
 ...
 private:
  // Lookup a string in the CGC string table.
  ...
  const unsigned char* p_;
  // The associated CGC string table.
  cgccpp::Cgc_strtab strtab_;
...
class Incremental_symtab_reader
{
 public:
  ...
  { return cgccpp::Swap<32, big_endian>::readval(this->p_ + 4 * n); }
...
class Incremental_got_plt_reader
{
 public:
  ...
  {
    this->got_count_ = cgccpp::Swap<32, big_endian>::readval(p);
  ...
  {
    return cgccpp::Swap<32, big_endian>::readval(this->p_ + 4);
  }
  ...
  {
    return cgccpp::Swap<32, big_endian>::readval(this->got_desc_p_ + n * 8);
  }
  ...
  {
    return cgccpp::Swap<32, big_endian>::readval(this->got_desc_p_ + n * 8 + 4);
  }
  ...
  {
    return cgccpp::Swap<32, big_endian>::readval(this->plt_desc_p_ + n * 4);
  }
...
// An object representing the CGC file we edit during an incremental build.
...
class Sized_incremental_binary : public Incremental_binary
{
 public:
  Sized_incremental_binary(Output_file* output,
                           ...
                           Target* target)
    : Incremental_binary(output, target), cgc_file_(this, ehdr),
 ...
 private:
  ...
  // Output as an CGC file.
  cgccpp::Cgc_file<size, big_endian, Incremental_binary> cgc_file_;
