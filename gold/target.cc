...
// in _bfd_cgc_is_local_label_name().
...
// Implementations of methods Target::do_make_cgc_object are almost identical
// except for the address sizes and endianities.  So we extract this
// into a template.

template<int size, bool big_endian>
inline Object*
Target::do_make_cgc_object_implementation(
...
// Make an CGC object called NAME by reading INPUT_FILE at OFFSET.  EHDR
// is the CGC header of the object.  There are four versions of this
// for different address sizes and endianities.

#ifdef HAVE_TARGET_32_LITTLE
Object*
Target::do_make_cgc_object(const std::string& name, Input_file* input_file,
			   off_t offset, const cgccpp::Ehdr<32, false>& ehdr)
{
  return this->do_make_cgc_object_implementation<32, false>(name, input_file,
							    offset, ehdr);
}
#endif
...
Object*
Target::do_make_cgc_object(const std::string& name, Input_file* input_file,
			   off_t offset, const cgccpp::Ehdr<32, true>& ehdr)
{
  return this->do_make_cgc_object_implementation<32, true>(name, input_file,
							   offset, ehdr);
}
#endif
...
Object*
Target::do_make_cgc_object(const std::string& name, Input_file* input_file,
			   off_t offset, const cgccpp::Ehdr<64, false>& ehdr)
{
  return this->do_make_cgc_object_implementation<64, false>(name, input_file,
							    offset, ehdr);
}
#endif
...
{
  return this->do_make_cgc_object_implementation<64, true>(name, input_file,
							   offset, ehdr);
}
#endif

Output_section*
Target::do_make_output_section(const char* name, cgccpp::Cgc_Word type,
...
// Set the EI_OSABI field of the CGC header if requested.

template<int size, bool big_endian>
void
Sized_target<size, big_endian>::do_adjust_cgc_header(unsigned char* view,
						     int len)
{
  cgccpp::CGCOSABI osabi = this->osabi();
  if (osabi != cgccpp::CGCOSABI_NONE)
    {
      gold_assert(len == cgccpp::Cgc_sizes<size>::ehdr_size);

      cgccpp::Ehdr<size, big_endian> ehdr(view);
      unsigned char e_ident[cgccpp::EI_NIDENT];
      memcpy(e_ident, ehdr.get_e_ident(), cgccpp::EI_NIDENT);

      e_ident[cgccpp::EI_OSABI] = osabi;
