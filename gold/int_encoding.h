...
void insert_into_vector(std::vector<unsigned char>* destination,
                        typename cgccpp::Valtype_base<valsize>::Valtype value)
{
  unsigned char buffer[valsize / 8];
  if (parameters->target().is_big_endian())
    cgccpp::Swap_unaligned<valsize, true>::writeval(buffer, value);
  else
    cgccpp::Swap_unaligned<valsize, false>::writeval(buffer, value);
...
template <int valsize>
typename cgccpp::Valtype_base<valsize>::Valtype
