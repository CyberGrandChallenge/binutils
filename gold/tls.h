...
check_range(const Relocate_info<size, big_endian>* relinfo,
            size_t relnum,
            typename cgccpp::Cgc_types<size>::Cgc_Addr rel_offset,
            section_size_type view_size, int off)
{
  typename cgccpp::Cgc_types<size>::Cgc_Addr offset = rel_offset + off;
  // Cgc_Addr is unsigned, so this also tests for signed offset < 0.
  if (offset > view_size)
