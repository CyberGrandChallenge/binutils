...
Input_file::open(const Dirsearch& dirpath, const Task* task, int* pindex)
{
  ...
  bool ok;
  if (format == General_options::OBJECT_FORMAT_CGC)
    {
      ok = this->file_.open(task, name);
      this->format_ = FORMAT_CGC;
    }
  else
    {
...
Input_file::open_binary(const Task* task, const std::string& name)
{
  ...
  Binary_to_cgc binary_to_cgc(target.machine_code(),
			      target.get_size(),
			      target.is_big_endian(),
			      name);
  if (!binary_to_cgc.convert(task))
    return false;
  return this->file_.open(task, name, binary_to_cgc.converted_data_leak(),
			  binary_to_cgc.converted_size());
}
