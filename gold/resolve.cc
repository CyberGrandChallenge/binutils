...
inline void
Symbol::override_visibility(cgccpp::STV visibility)
{
...
void
Symbol::override_base(const cgccpp::Sym<size, big_endian>& sym,
...
void
Sized_symbol<size>::override(const cgccpp::Sym<size, big_endian>& sym,
...
static unsigned int
symbol_to_bits(cgccpp::STB binding, bool is_dynamic,
	       unsigned int shndx, bool is_ordinary, cgccpp::STT type)
{
  unsigned int bits;

  switch (binding)
    {
    case cgccpp::STB_GLOBAL:
    case cgccpp::STB_GNU_UNIQUE:
      bits = global_flag;
      break;

    case cgccpp::STB_WEAK:
      bits = weak_flag;
      break;

    case cgccpp::STB_LOCAL:
  ...
  switch (shndx)
    {
    case cgccpp::SHN_UNDEF:
      bits |= undef_flag;
      break;

    case cgccpp::SHN_COMMON:
      if (!is_ordinary)
	bits |= common_flag;
      break;

    default:
      if (type == cgccpp::STT_COMMON)
	bits |= common_flag;
		      ...
		      Object* object, const char* version)
{
  ...
  else if (st_shndx == cgccpp::SHN_UNDEF
           && (to->visibility() == cgccpp::STV_HIDDEN
               || to->visibility() == cgccpp::STV_INTERNAL))
    {
  ...
  // Record if we've seen this symbol in a real CGC object (i.e., the
  ...
  if (parameters->options().detect_odr_violations()
      && (sym.get_st_bind() == cgccpp::STB_WEAK
	  || to->binding() == cgccpp::STB_WEAK)
      && orig_st_shndx != cgccpp::SHN_UNDEF
      && to->shndx(&to_is_ordinary) != cgccpp::SHN_UNDEF
      && to_is_ordinary
  ...
  cgccpp::STT fromtype = (object->pluginobj() != NULL
			  ? to->type()
			  : sym.get_st_type());
  ...
  if (Symbol_table::should_override(to, frombits, fromtype, OBJECT,
				    ...
				    &adjust_dyndef))
    {
      cgccpp::STB tobinding = to->binding();
      ...
      if (adjust_dyndef)
	{
      ...
      // The CGC ABI says that even for a reference to a symbol we
...
Symbol_table::should_override(const Symbol* to, unsigned int frombits,
			      cgccpp::STT fromtype, Defined defined,
			      ...
			      bool* adjust_dyndef)
{
  ...
  *adjust_dyndef = false;
  ...
  if (to->source() == Symbol::IS_UNDEFINED)
    tobits = symbol_to_bits(to->binding(), false, cgccpp::SHN_UNDEF, true,
			    to->type());
  else if (to->source() != Symbol::FROM_OBJECT)
    tobits = symbol_to_bits(to->binding(), false, cgccpp::SHN_ABS, false,
			    to->type());
  else
    {
      bool is_ordinary;
  ...
  if ((to->type() == cgccpp::STT_TLS) ^ (fromtype == cgccpp::STT_TLS)
      && !to->is_placeholder())
				    ...
				    const Sized_symbol<size>* fromsym)
{
  ...
  if (tosym->binding() == cgccpp::STB_LOCAL
      || ((tosym->visibility() == cgccpp::STV_HIDDEN
	   || tosym->visibility() == cgccpp::STV_INTERNAL)
	  && (tosym->binding() == cgccpp::STB_GLOBAL
	      || tosym->binding() == cgccpp::STB_GNU_UNIQUE
	      || tosym->binding() == cgccpp::STB_WEAK)
