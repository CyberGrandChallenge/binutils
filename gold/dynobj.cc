					     ...
					     off_t strtab_size)
{
  ...
  gold_assert(dynamicshdr.get_sh_type() == cgccpp::SHT_DYNAMIC);
...
Sized_dynobj<size, big_endian>::do_read_symbols(Read_symbols_data* sd)
{
  ...
  if (this->dynsym_shndx_ != -1U)
	...
	convert_to_section_size_type(strtabshdr.get_sh_size());
      ...
      unsigned int dummy;
      this->read_dynsym_section(pshdrs, versym_shndx, cgccpp::SHT_GNU_versym,
				this->dynsym_shndx_,
      ...
      this->read_dynsym_section(pshdrs, verdef_shndx, cgccpp::SHT_GNU_verdef,
				strtab_shndx, &sd->verdef, &sd->verdef_size,
				&sd->verdef_info);

      this->read_dynsym_section(pshdrs, verneed_shndx, cgccpp::SHT_GNU_verneed,
    ...
    Version_map* version_map) const
{
  ...
  for (unsigned int i = 0; i < count; ++i)
    {
      cgccpp::Verdef<size, big_endian> verdef(p);

      if (verdef.get_vd_version() != cgccpp::VER_DEF_CURRENT)
	{
      ...
      if ((p - pverdef) + vd_aux >= verdef_size)
	{
      ...
      cgccpp::Verdaux<size, big_endian> verdaux(pvda);
    ...
    Version_map* version_map) const
{
  ...
  for (unsigned int i = 0; i < count; ++i)
    {
      cgccpp::Verneed<size, big_endian> verneed(p);

      if (verneed.get_vn_version() != cgccpp::VER_NEED_CURRENT)
	{
	...
	{
	  cgccpp::Vernaux<size, big_endian> vernaux(pvna);
  ...
  for (size_t i = 0; i < symcount; ++i, p += sym_size)
    {
	  ...
	  && sym.get_st_bind() != cgccpp::STB_LOCAL)
...
// The standard CGC hash function.  This hash function must not
// change, as the dynamic linker uses it also.

uint32_t
Dynobj::cgc_hash(const char* name)
{
  ...
  while ((c = *nameu++) != '\0')
	...
	{
	  h ^= g >> 24;
	  // The CGC ABI says h &= ~g, but using xor is equivalent in
...
// Create a standard CGC hash table, setting *PPHASH and *PHASHLEN.
// DYNSYMS is a vector with all the global dynamic symbols.
// LOCAL_DYNSYM_COUNT is the number of local symbols in the dynamic
// symbol table.

void
Dynobj::create_cgc_hash_table(const std::vector<Symbol*>& dynsyms,
			      ...
			      unsigned int* phashlen)
{
  ...
  for (unsigned int i = 0; i < dynsym_count; ++i)
    dynsym_hashvals[i] = Dynobj::cgc_hash(dynsyms[i]->name());
  ...
  if (parameters->target().is_big_endian())
    {
      ...
      Dynobj::sized_create_cgc_hash_table<true>(bucket, chain, phash,
						hashlen);
#else
      gold_unreachable();
#endif
  ...
  else
    {
      ...
      Dynobj::sized_create_cgc_hash_table<false>(bucket, chain, phash,
						 hashlen);
#else
      gold_unreachable();
#endif
...
// Fill in an CGC hash table.

template<bool big_endian>
void
Dynobj::sized_create_cgc_hash_table(const std::vector<uint32_t>& bucket,
				    ...
				    unsigned int hashlen)
{
  ...
  cgccpp::Swap<32, big_endian>::writeval(p, bucketcount);
  p += 4;
  cgccpp::Swap<32, big_endian>::writeval(p, chaincount);
  p += 4;

  for (unsigned int i = 0; i < bucketcount; ++i)
    {
      cgccpp::Swap<32, big_endian>::writeval(p, bucket[i]);
      p += 4;
    }

  for (unsigned int i = 0; i < chaincount; ++i)
    {
      cgccpp::Swap<32, big_endian>::writeval(p, chain[i]);
      p += 4;
    }
...
// tables are an extension to CGC which are recognized by the GNU
    ...
    unsigned int* phashlen)
{
  if (hashed_dynsyms.empty())
    {
      ...
      unsigned char* phash = new unsigned char[hashlen];
      // One empty bucket.
      cgccpp::Swap<32, big_endian>::writeval(phash, 1);
      // Symbol index above unhashed symbols.
      cgccpp::Swap<32, big_endian>::writeval(phash + 4, unhashed_dynsym_count);
      // One word for bitmask.
      cgccpp::Swap<32, big_endian>::writeval(phash + 8, 1);
      // Only bloom filter.
      cgccpp::Swap<32, big_endian>::writeval(phash + 12, 0);
      // No valid hashes.
      cgccpp::Swap<size, big_endian>::writeval(phash + 16, 0);
      // No hashes in only bucket.
      cgccpp::Swap<32, big_endian>::writeval(phash + 16 + size / 8, 0);
  ...
  typedef typename cgccpp::Cgc_types<size>::Cgc_WXword Word;
  ...
  cgccpp::Swap<32, big_endian>::writeval(phash, bucketcount);
  cgccpp::Swap<32, big_endian>::writeval(phash + 4, symindx);
  cgccpp::Swap<32, big_endian>::writeval(phash + 8, maskwords);
  cgccpp::Swap<32, big_endian>::writeval(phash + 12, shift2);

  unsigned char* p = phash + 16 + maskbits / 8;
  for (unsigned int i = 0; i < bucketcount; ++i)
    {
      if (counts[i] == 0)
	cgccpp::Swap<32, big_endian>::writeval(p, 0);
      else
	cgccpp::Swap<32, big_endian>::writeval(p, indx[i]);
      p += 4;
    }
  ...
  for (unsigned int i = 0; i < nsyms; ++i)
      ...
      if (counts[bucket] == 1)
	{
      ...
      cgccpp::Swap<32, big_endian>::writeval(p + (indx[bucket] - symindx) * 4,
					     val);
      --counts[bucket];
  ...
  for (unsigned int i = 0; i < maskwords; ++i)
    {
      cgccpp::Swap<size, big_endian>::writeval(p, bitmask[i]);
      p += size / 8;
    }
...
{
  const int verdef_size = cgccpp::Cgc_sizes<size>::verdef_size;
  const int verdaux_size = cgccpp::Cgc_sizes<size>::verdaux_size;

  cgccpp::Verdef_write<size, big_endian> vd(pb);
  vd.set_vd_version(cgccpp::VER_DEF_CURRENT);
  vd.set_vd_flags((this->is_base_ ? cgccpp::VER_FLG_BASE : 0)
		  | (this->is_weak_ ? cgccpp::VER_FLG_WEAK : 0)
		  | (this->is_info_ ? cgccpp::VER_FLG_INFO : 0));
  vd.set_vd_ndx(this->index());
  vd.set_vd_cnt(1 + this->deps_.size());
  vd.set_vd_hash(Dynobj::cgc_hash(this->name()));
  ...
  cgccpp::Verdaux_write<size, big_endian> vda(pb);
...
{
  const int verneed_size = cgccpp::Cgc_sizes<size>::verneed_size;
  const int vernaux_size = cgccpp::Cgc_sizes<size>::vernaux_size;

  cgccpp::Verneed_write<size, big_endian> vn(pb);
  vn.set_vn_version(cgccpp::VER_NEED_CURRENT);
  ...
  for (p = this->need_versions_.begin(), i = 0;
       ...
       ++p, ++i)
    {
      cgccpp::Vernaux_write<size, big_endian> vna(pb);
      vna.set_vna_hash(Dynobj::cgc_hash((*p)->version()));
				  ...
				  unsigned int* psize) const
{
  ...
  for (unsigned int i = 0; i < local_symcount; ++i)
    cgccpp::Swap<16, big_endian>::writeval(pbuf + i * 2,
					   cgccpp::VER_NDX_LOCAL);
  ...
  for (std::vector<Symbol*>::const_iterator p = syms.begin();
      ...
      else
	{
	  if ((*p)->is_defined() && !(*p)->is_from_dynobj())
	    version_index = cgccpp::VER_NDX_GLOBAL;
	  else
	    version_index = cgccpp::VER_NDX_LOCAL;
	}
      // If the symbol was defined as foo@V1 instead of foo@@V1, add
      // the hidden bit.
      if ((*p)->version() != NULL && !(*p)->is_default())
        version_index |= cgccpp::VERSYM_HIDDEN;
      cgccpp::Swap<16, big_endian>::writeval(pbuf + (*p)->dynsym_index() * 2,
                                             version_index);
    }
