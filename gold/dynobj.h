...
class Dynobj : public Object
{
 public:
  ...
  // Compute the CGC hash code for a string.
  static uint32_t
  cgc_hash(const char*);
  ...
  static void
  create_cgc_hash_table(const std::vector<Symbol*>& dynsyms,
 ...
 private:
  ...
  // Sized version of create_cgc_hash_table.
  template<bool big_endian>
  static void
  sized_create_cgc_hash_table(const std::vector<uint32_t>& bucket,
...
class Sized_dynobj : public Dynobj
{
 public:
  ...
  do_section_contents(unsigned int shndx, section_size_type* plen,
		      bool cache)
  {
    Location loc(this->cgc_file_.section_contents(shndx));
 ...
 private:
  ...
  static const int dyn_size = cgccpp::Cgc_sizes<size>::dyn_size;
  typedef cgccpp::Shdr<size, big_endian> Shdr;
  typedef cgccpp::Dyn<size, big_endian> Dyn;
  ...
  read_dynsym_section(const unsigned char* pshdrs, unsigned int shndx,
		      cgccpp::SHT type, unsigned int link,
