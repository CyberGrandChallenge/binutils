...
Output_section_headers::do_size() const
{
  ...
  int shdr_size;
  if (size == 32)
    shdr_size = cgccpp::Cgc_sizes<32>::shdr_size;
  else if (size == 64)
    shdr_size = cgccpp::Cgc_sizes<64>::shdr_size;
  else
    gold_unreachable();
...
Output_section_headers::do_sized_write(Output_file* of)
{
  ...
  {
    typename cgccpp::Shdr_write<size, big_endian> oshdr(v);
    oshdr.put_sh_name(0);
    oshdr.put_sh_type(cgccpp::SHT_NULL);
    oshdr.put_sh_flags(0);
    oshdr.put_sh_addr(0);
    oshdr.put_sh_offset(0);

    size_t section_count = (this->data_size()
			    / cgccpp::Cgc_sizes<size>::shdr_size);
    if (section_count < cgccpp::SHN_LORESERVE)
      oshdr.put_sh_size(0);
    else
      oshdr.put_sh_size(section_count);

    unsigned int shstrndx = this->shstrtab_section_->out_shndx();
    if (shstrndx < cgccpp::SHN_LORESERVE)
      oshdr.put_sh_link(0);
    else
      oshdr.put_sh_link(shstrndx);

    size_t segment_count = this->segment_list_->size();
    oshdr.put_sh_info(segment_count >= cgccpp::PN_XNUM ? segment_count : 0);
  ...
  if (!parameters->options().relocatable())
	   ...
	   ++p)
	{
	  ...
	  cgccpp::Shdr_write<size, big_endian> oshdr(v);
...
{
  const int phdr_size = cgccpp::Cgc_sizes<size>::phdr_size;
  ...
  for (Layout::Segment_list::const_iterator p = this->segment_list_.begin();
       ...
       ++p)
    {
      cgccpp::Phdr_write<size, big_endian> ophdr(v);
...
Output_segment_headers::do_size() const
{
  ...
  int phdr_size;
  if (size == 32)
    phdr_size = cgccpp::Cgc_sizes<32>::phdr_size;
  else if (size == 64)
    phdr_size = cgccpp::Cgc_sizes<64>::phdr_size;
  else
    gold_unreachable();
...
Output_file_header::do_sized_write(Output_file* of)
{
  ...
  int ehdr_size = cgccpp::Cgc_sizes<size>::ehdr_size;
  ...
  memset(e_ident, 0, cgccpp::EI_NIDENT);
  e_ident[cgccpp::EI_MAG0] = cgccpp::CGCMAG0;
  e_ident[cgccpp::EI_MAG1] = cgccpp::CGCMAG1;
  e_ident[cgccpp::EI_MAG2] = cgccpp::CGCMAG2;
  e_ident[cgccpp::EI_MAG3] = cgccpp::CGCMAG3;
  if (size == 32)
    e_ident[cgccpp::EI_CLASS] = cgccpp::CGCCLASS32;
  else if (size == 64)
    e_ident[cgccpp::EI_CLASS] = cgccpp::CGCCLASS64;
  else
    gold_unreachable();
  e_ident[cgccpp::EI_DATA] = (big_endian
			      ? cgccpp::CGCDATA2MSB
			      : cgccpp::CGCDATA2LSB);
  e_ident[cgccpp::EI_VERSION] = cgccpp::EV_CURRENT;
  oehdr.put_e_ident(e_ident);

  cgccpp::ET e_type;
  if (parameters->options().relocatable())
    e_type = cgccpp::ET_REL;
  else if (parameters->options().output_is_position_independent())
    e_type = cgccpp::ET_DYN;
  else
    e_type = cgccpp::ET_EXEC;
  oehdr.put_e_type(e_type);

  oehdr.put_e_machine(this->target_->machine_code());
  oehdr.put_e_version(cgccpp::EV_CURRENT);
  ...
  oehdr.put_e_ehsize(cgccpp::Cgc_sizes<size>::ehdr_size);
  ...
  if (this->segment_header_ == NULL)
    ...
    {
      oehdr.put_e_phentsize(cgccpp::Cgc_sizes<size>::phdr_size);
      size_t phnum = (this->segment_header_->data_size()
		      / cgccpp::Cgc_sizes<size>::phdr_size);
      if (phnum > cgccpp::PN_XNUM)
	phnum = cgccpp::PN_XNUM;
      oehdr.put_e_phnum(phnum);
    }

  oehdr.put_e_shentsize(cgccpp::Cgc_sizes<size>::shdr_size);
  ...
  if (shstrndx < cgccpp::SHN_LORESERVE)
    ...
    oehdr.put_e_shstrndx(cgccpp::SHN_XINDEX);

  // Let the target adjust the CGC header, e.g., to set EI_OSABI in
  // the e_ident field.
  this->target_->adjust_cgc_header(view, ehdr_size);
...
Output_file_header::do_size() const
{
  const int size = parameters->target().get_size();
  if (size == 32)
    return cgccpp::Cgc_sizes<32>::ehdr_size;
  else if (size == 64)
    return cgccpp::Cgc_sizes<64>::ehdr_size;
  else
    gold_unreachable();
}
...
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::Output_reloc(
    Symbol* gsym,
    unsigned int type,
    Output_data* od,
    Address address,
...
template<bool dynamic, int size, bool big_endian>
void
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::
...
template<bool dynamic, int size, bool big_endian>
unsigned int
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::get_symbol_index()
  const
{
  unsigned int index;
  if (this->is_symbolless_)
...
template<bool dynamic, int size, bool big_endian>
typename cgccpp::Cgc_types<size>::Cgc_Addr
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::get_address() const
{
...
void
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::write_rel(
    Write_rel* wr) const
{
  wr->put_r_offset(this->get_address());
  unsigned int sym_index = this->get_symbol_index();
  wr->put_r_info(cgccpp::cgc_r_info<size>(sym_index, this->type_));
}
...
template<bool dynamic, int size, bool big_endian>
void
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::write(
    unsigned char* pov) const
{
  cgccpp::Rel_write<size, big_endian> orel(pov);
  this->write_rel(&orel);
}
...
template<bool dynamic, int size, bool big_endian>
typename cgccpp::Cgc_types<size>::Cgc_Addr
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::symbol_value(
    Addend addend) const
{
...
int
Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>::
  compare(const Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>& r2)
    const
{
...
template<bool dynamic, int size, bool big_endian>
void
Output_reloc<cgccpp::SHT_RELA, dynamic, size, big_endian>::write(
    unsigned char* pov) const
{
  cgccpp::Rela_write<size, big_endian> orel(pov);
...
Output_data_group<size, big_endian>::do_write(Output_file* of)
{
  ...
  cgccpp::Cgc_Word* contents = reinterpret_cast<cgccpp::Cgc_Word*>(oview);
  cgccpp::Swap<32, big_endian>::writeval(contents, this->flags_);
  ++contents;
  ...
  for (std::vector<unsigned int>::const_iterator p =
      ...
      else
	{
      ...
      cgccpp::Swap<32, big_endian>::writeval(contents, output_shndx);
    }
    ...
    unsigned char* pov) const
{
  ...
  cgccpp::Swap<got_size, big_endian>::writeval(pov, val);
}
...
{
  typename cgccpp::Cgc_types<size>::Cgc_WXword val;
  switch (this->offset_)
    {
    case DYNAMIC_NUMBER:
      val = this->u_.val;
  ...
  cgccpp::Dyn_write<size, big_endian> dw(pov);
...
Output_data_dynamic::do_adjust_output_section(Output_section* os)
{
  if (parameters->target().get_size() == 32)
    os->set_entsize(cgccpp::Cgc_sizes<32>::dyn_size);
  else if (parameters->target().get_size() == 64)
    os->set_entsize(cgccpp::Cgc_sizes<64>::dyn_size);
  else
    gold_unreachable();
}
...
Output_data_dynamic::set_final_data_size()
{
  ...
  if (this->entries_.empty() || this->entries_.back().tag() != cgccpp::DT_NULL)
    {
      int extra = parameters->options().spare_dynamic_tags();
      for (int i = 0; i < extra; ++i)
	this->add_constant(cgccpp::DT_NULL, 0);
      this->add_constant(cgccpp::DT_NULL, 0);
    }

  int dyn_size;
  if (parameters->target().get_size() == 32)
    dyn_size = cgccpp::Cgc_sizes<32>::dyn_size;
  else if (parameters->target().get_size() == 64)
    dyn_size = cgccpp::Cgc_sizes<64>::dyn_size;
  else
    gold_unreachable();
...
{
  const int dyn_size = cgccpp::Cgc_sizes<size>::dyn_size;
...
Output_symtab_xindex::endian_do_write(unsigned char* const oview)
{
  for (Xindex_entries::const_iterator p = this->entries_.begin();
       ...
       ++p)
    {
      ...
      cgccpp::Swap<32, big_endian>::writeval(oview + symndx * 4, p->second);
    }
}
...
Output_fill_debug_info::do_write(Output_file* of, off_t off, size_t len) const
{
  ...
  if (this->is_big_endian())
    {
      cgccpp::Swap_unaligned<32, true>::writeval(pov, len - 4);
      cgccpp::Swap_unaligned<16, true>::writeval(pov + 4, this->version);
      cgccpp::Swap_unaligned<32, true>::writeval(pov + 6, 0);
    }
  else
    {
      cgccpp::Swap_unaligned<32, false>::writeval(pov, len - 4);
      cgccpp::Swap_unaligned<16, false>::writeval(pov + 4, this->version);
      cgccpp::Swap_unaligned<32, false>::writeval(pov + 6, 0);
    }
  pov += 4 + 2 + 4;
  *pov++ = 4;
...
Output_fill_debug_line::do_write(Output_file* of, off_t off, size_t len) const
{
  ...
  if (this->is_big_endian())
    {
      cgccpp::Swap_unaligned<32, true>::writeval(pov, len - 4);
      cgccpp::Swap_unaligned<16, true>::writeval(pov + 4, this->version);
      cgccpp::Swap_unaligned<32, true>::writeval(pov + 6, len - (4 + 2 + 4));
    }
  else
    {
      cgccpp::Swap_unaligned<32, false>::writeval(pov, len - 4);
      cgccpp::Swap_unaligned<16, false>::writeval(pov + 4, this->version);
      cgccpp::Swap_unaligned<32, false>::writeval(pov + 6, len - (4 + 2 + 4));
    }
  pov += 4 + 2 + 4;
  ...
  *pov++ = 0;	// filenames (empty)
  ...
  if (pov < oview + len)
    memset(pov, cgccpp::DW_LNS_set_basic_block, oview + len - pov);
...
Output_section::Output_section(const char* name, cgccpp::Cgc_Word type,
...
{
  cgccpp::Cgc_Xword addralign = shdr.get_sh_addralign();
  ...
  if (strcmp(secname, ".debug_str") == 0)
    {
      sh_flags |= (cgccpp::SHF_MERGE | cgccpp::SHF_STRINGS);
      entsize = 1;
    }
  ...
  if ((sh_flags & cgccpp::SHF_MERGE) != 0
      && reloc_shndx == 0
      && shdr.get_sh_size() > 0
  ...
  if (!this->generate_code_fills_at_write_
      && !have_sections_script
      && (sh_flags & cgccpp::SHF_EXECINSTR) != 0
...
{
  bool is_string = (flags & cgccpp::SHF_STRINGS) != 0;
...
Output_section::update_flags_for_input_section(cgccpp::Cgc_Xword flags)
{
  ...
  if ((flags & cgccpp::SHF_MERGE) == 0)
    this->flags_ &=~ cgccpp::SHF_MERGE;
  else
    {
  ...
  if ((flags & cgccpp::SHF_STRINGS) == 0)
    this->flags_ &=~ cgccpp::SHF_STRINGS;
  else
    {
      if (this->current_data_size_for_child() == 0)
	this->flags_ |= cgccpp::SHF_STRINGS;
...
Output_section::do_reset_address_and_file_offset()
{
  ...
  if (((this->flags_ & cgccpp::SHF_ALLOC) == 0) && !this->is_noload_)
     this->set_address(0);
...
Output_section::sort_attached_input_sections()
{
  ...
  if (this->must_sort_attached_input_sections())
    {
      if (this->type() == cgccpp::SHT_PREINIT_ARRAY
	  || this->type() == cgccpp::SHT_INIT_ARRAY
	  || this->type() == cgccpp::SHT_FINI_ARRAY)
...
Output_segment::Output_segment(cgccpp::Cgc_Word type, cgccpp::Cgc_Word flags)
  : vaddr_(0),
    paddr_(0),
    memsz_(0),
    max_align_(0),
    ...
    is_unique_segment_(false)
{
  // The CGC ABI specifies that a PT_TLS segment always has PF_R as
  // the flags.
  if (type == cgccpp::PT_TLS)
    this->flags_ = cgccpp::PF_R;
}
...
Output_segment::add_output_section_to_load(Layout* layout,
					   Output_section* os,
					   cgccpp::Cgc_Word seg_flags)
{
  gold_assert(this->type() == cgccpp::PT_LOAD);
...
Output_segment::has_any_data_sections() const
{
  for (int i = 0; i < static_cast<int>(ORDER_MAX); ++i)
	   ...
	   ++p)
	{
	  if (!(*p)->is_section())
	    return true;
	  if ((*p)->output_section()->type() != cgccpp::SHT_NOBITS)
	    return true;
	}
    }
  return false;
...
Output_segment::set_section_addresses(const Target* target,
				      ...
				      unsigned int* pshndx)
{
  gold_assert(this->type_ == cgccpp::PT_LOAD);
      ...
      for (int i = 0; i <= static_cast<int>(ORDER_RELRO_LAST); ++i)
	{
	  ...
	  for (p = pdl->begin(); p != pdl->end(); ++p)
		...
		break;
	      ...
	      if (align > max_align)
		max_align = align;
	      if ((*p)->is_section_flag_set(cgccpp::SHF_TLS))
		in_tls = true;
	      else if (in_tls)
		{
	      ...
	      if ((*p)->is_section_flag_set(cgccpp::SHF_TLS)
		  && (*p)->is_section_type(cgccpp::SHT_NOBITS))
		continue;
  ...
  *poff = off;
  ...
  if (target->isolate_execinstr() && (this->flags() & cgccpp::PF_X) != 0)
    {
...
Output_segment::set_offset(unsigned int increase)
{
  ...
  for (Output_data_list::const_iterator p = pdl->begin();
	...
	first = *p;
      ...
      if ((*p)->is_section()
	  && (*p)->output_section()->type() == cgccpp::SHT_NOBITS)
	plast = &last_bss;
      else
	plast = &last_data;
      if (*plast == NULL
  ...
  // page boundary.
  if (this->type_ == cgccpp::PT_GNU_RELRO)
    {
      uint64_t page_align = parameters->target().abi_pagesize();
...
{
  gold_assert(this->type_ == cgccpp::PT_TLS);
...
template<int size, bool big_endian>
void
Output_segment::write_header(cgccpp::Phdr_write<size, big_endian>* ophdr)
{
...
#ifdef HAVE_TARGET_32_LITTLE
template
class Output_reloc<cgccpp::SHT_REL, false, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_reloc<cgccpp::SHT_REL, false, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_reloc<cgccpp::SHT_REL, false, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_reloc<cgccpp::SHT_REL, false, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_reloc<cgccpp::SHT_REL, true, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_reloc<cgccpp::SHT_REL, true, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_reloc<cgccpp::SHT_REL, true, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_reloc<cgccpp::SHT_REL, true, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_reloc<cgccpp::SHT_RELA, false, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_reloc<cgccpp::SHT_RELA, false, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_reloc<cgccpp::SHT_RELA, false, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_reloc<cgccpp::SHT_RELA, false, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_reloc<cgccpp::SHT_RELA, true, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_reloc<cgccpp::SHT_RELA, true, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_reloc<cgccpp::SHT_RELA, true, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_reloc<cgccpp::SHT_RELA, true, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_data_reloc<cgccpp::SHT_REL, false, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_data_reloc<cgccpp::SHT_REL, false, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_data_reloc<cgccpp::SHT_REL, false, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_data_reloc<cgccpp::SHT_REL, false, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_data_reloc<cgccpp::SHT_REL, true, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_data_reloc<cgccpp::SHT_REL, true, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_data_reloc<cgccpp::SHT_REL, true, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_data_reloc<cgccpp::SHT_REL, true, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_data_reloc<cgccpp::SHT_RELA, false, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_data_reloc<cgccpp::SHT_RELA, false, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_data_reloc<cgccpp::SHT_RELA, false, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_data_reloc<cgccpp::SHT_RELA, false, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_data_reloc<cgccpp::SHT_RELA, true, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_data_reloc<cgccpp::SHT_RELA, true, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_data_reloc<cgccpp::SHT_RELA, true, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_data_reloc<cgccpp::SHT_RELA, true, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_relocatable_relocs<cgccpp::SHT_REL, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_relocatable_relocs<cgccpp::SHT_REL, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_relocatable_relocs<cgccpp::SHT_REL, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_relocatable_relocs<cgccpp::SHT_REL, 64, true>;
#endif

#ifdef HAVE_TARGET_32_LITTLE
template
class Output_relocatable_relocs<cgccpp::SHT_RELA, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Output_relocatable_relocs<cgccpp::SHT_RELA, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Output_relocatable_relocs<cgccpp::SHT_RELA, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Output_relocatable_relocs<cgccpp::SHT_RELA, 64, true>;
#endif
