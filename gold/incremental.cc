...
class Output_section_incremental_inputs : public Output_section_data
{
 ...
 private:
  ...
  typedef cgccpp::Swap<size, big_endian> Swap;
  typedef cgccpp::Swap<16, big_endian> Swap16;
  typedef cgccpp::Swap<32, big_endian> Swap32;
  typedef cgccpp::Swap<64, big_endian> Swap64;
...
can_incremental_update(unsigned int sh_type)
{
  return (sh_type == cgccpp::SHT_PROGBITS
	  || sh_type == cgccpp::SHT_NOBITS
	  || sh_type == cgccpp::SHT_INIT_ARRAY
	  || sh_type == cgccpp::SHT_FINI_ARRAY
	  || sh_type == cgccpp::SHT_PREINIT_ARRAY
	  || sh_type == cgccpp::SHT_NOTE);
}
    ...
    unsigned int* p_strtab_shndx)
{
  unsigned int inputs_shndx =
      this->cgc_file_.find_section_by_type(cgccpp::SHT_GNU_INCREMENTAL_INPUTS);
  if (inputs_shndx == cgccpp::SHN_UNDEF)  // Not found.
    return false;

  unsigned int symtab_shndx =
      this->cgc_file_.find_section_by_type(cgccpp::SHT_GNU_INCREMENTAL_SYMTAB);
  if (symtab_shndx == cgccpp::SHN_UNDEF)  // Not found.
    return false;
  if (this->cgc_file_.section_link(symtab_shndx) != inputs_shndx)
    return false;

  unsigned int relocs_shndx =
      this->cgc_file_.find_section_by_type(cgccpp::SHT_GNU_INCREMENTAL_RELOCS);
  if (relocs_shndx == cgccpp::SHN_UNDEF)  // Not found.
    return false;
  if (this->cgc_file_.section_link(relocs_shndx) != inputs_shndx)
    return false;

  unsigned int got_plt_shndx =
      this->cgc_file_.find_section_by_type(cgccpp::SHT_GNU_INCREMENTAL_GOT_PLT);
  if (got_plt_shndx == cgccpp::SHN_UNDEF)  // Not found.
    return false;
  if (this->cgc_file_.section_link(got_plt_shndx) != inputs_shndx)
    return false;

  unsigned int strtab_shndx = this->cgc_file_.section_link(inputs_shndx);
  if (strtab_shndx == cgccpp::SHN_UNDEF
      || strtab_shndx > this->cgc_file_.shnum()
      || this->cgc_file_.section_type(strtab_shndx) != cgccpp::SHT_STRTAB)
    return false;
...
Sized_incremental_binary<size, big_endian>::setup_readers()
{
  ...
  Location inputs_location(this->cgc_file_.section_contents(inputs_shndx));
  Location symtab_location(this->cgc_file_.section_contents(symtab_shndx));
  Location relocs_location(this->cgc_file_.section_contents(relocs_shndx));
  Location got_plt_location(this->cgc_file_.section_contents(got_plt_shndx));
  Location strtab_location(this->cgc_file_.section_contents(strtab_shndx));
  ...
  cgccpp::Cgc_strtab strtab(strtab_view.data(), strtab_location.data_size);
  ...
  unsigned int main_symtab_shndx =
      this->cgc_file_.find_section_by_type(cgccpp::SHT_SYMTAB);
  gold_assert(main_symtab_shndx != cgccpp::SHN_UNDEF);
  this->main_symtab_loc_ = this->cgc_file_.section_contents(main_symtab_shndx);

  // Find the main symbol string table.
  unsigned int main_strtab_shndx =
      this->cgc_file_.section_link(main_symtab_shndx);
  gold_assert(main_strtab_shndx != cgccpp::SHN_UNDEF
	      && main_strtab_shndx < this->cgc_file_.shnum());
  this->main_strtab_loc_ = this->cgc_file_.section_contents(main_strtab_shndx);
  ...
  if (input_file.type() == INCREMENTAL_INPUT_SHARED_LIBRARY)
    {
      ...
      cgccpp::Cgc_strtab strtab(NULL, 0);
	  ...
	  if (is_copy)
	    {
	      ...
	      cgccpp::Sym<size, big_endian> gsym(sym_p);
    ...
    Layout* layout)
{
  ...
  // main symbol table.
  unsigned int symtab_count =
      this->main_symtab_loc_.data_size / cgccpp::Cgc_sizes<size>::sym_size;
    ...
    Output_file* of)
{
  ...
  typedef typename cgccpp::Cgc_types<size>::Cgc_Swxword Addend;
...
Sized_incremental_binary<size, big_endian>::get_symtab_view(
    View* symtab_view,
    unsigned int* nsyms,
    cgccpp::Cgc_strtab* strtab)
{
  *symtab_view = this->view(this->main_symtab_loc_);
  *nsyms = this->main_symtab_loc_.data_size / cgccpp::Cgc_sizes<size>::sym_size;

  View strtab_view(this->view(this->main_strtab_loc_));
  *strtab = cgccpp::Cgc_strtab(strtab_view.data(),
			      ...
			      const cgccpp::Ehdr<size, big_endian>& ehdr)
{
  ...
  if (target == NULL)
    {
      explain_no_incremental(_("unsupported CGC machine number %d"),
	       ehdr.get_e_machine());
      return NULL;
    }
...
// possible, e.g. FILE is not an CGC file or has an unsupported target.  FILE
// should be opened.
...
open_incremental_binary(Output_file* file)
{
  ...
  if (!cgccpp::Cgc_recognizer::is_cgc_file(p, want))
    {
      explain_no_incremental(_("output is not an CGC file."));
      return NULL;
    }
  ...
  std::string error;
  if (!cgccpp::Cgc_recognizer::is_valid_header(p, want, &size, &big_endian,
					       &error))
    {
	...
	case INCREMENTAL_INPUT_SHARED_LIBRARY:
	  {
		...
		if (sym->visibility() == cgccpp::STV_HIDDEN
		    || sym->visibility() == cgccpp::STV_INTERNAL)
...
class Local_got_offset_visitor : public Got_offset_list::Visitor
{
 public:
  ...
  visit(unsigned int got_type, unsigned int got_offset)
  {
    ...
    cgccpp::Swap<32, big_endian>::writeval(pov, this->info_.sym_index);
    cgccpp::Swap<32, big_endian>::writeval(pov + 4, this->info_.input_index);
  }
...
class Global_got_offset_visitor : public Got_offset_list::Visitor
{
 public:
  ...
  visit(unsigned int got_type, unsigned int got_offset)
  {
    ...
    unsigned char* pov = this->info_.got_desc_p + got_index * 8;
    cgccpp::Swap<32, big_endian>::writeval(pov, this->info_.sym_index);
    cgccpp::Swap<32, big_endian>::writeval(pov + 4, 0);
  }
...
class Global_symbol_visitor_got_plt
{
 public:
  ...
  operator()(const Sized_symbol<size>* sym)
  {
	...
	cgccpp::Swap<32, big_endian>::writeval(pov, sym->symtab_index());
      }
  }
    ...
    Layout*)
{
  ...
  cgccpp::Sym<size, big_endian> sym(symbuf);
  cgccpp::Sym_write<size, big_endian> osym(symbuf);

  typedef typename cgccpp::Cgc_types<size>::Cgc_WXword Cgc_size_type;
  ...
  for (unsigned int i = 0; i < nsyms; ++i)
	...
	name = "";
      ...
      typename cgccpp::Cgc_types<size>::Cgc_Addr v = gsym.get_st_value();
      unsigned int shndx = gsym.get_st_shndx();
      cgccpp::STB st_bind = gsym.get_st_bind();
      cgccpp::STT st_type = gsym.get_st_type();

      // Local hidden symbols start out as globals, but get converted to
      // to local during output.
      if (st_bind == cgccpp::STB_LOCAL)
	st_bind = cgccpp::STB_GLOBAL;
      ...
      if (input_shndx == -1U && !res->is_defined())
	{
	  ...
	  v = gsym.get_st_value();
	  Cgc_size_type symsize = gsym.get_st_size();
	  if (shndx == cgccpp::SHN_ABS)
	    {
    ...
    Stringpool_template<char>*)
{
  ...
  for (unsigned int i = 0; i < nlocals; ++i, symp += sym_size)
    {
      cgccpp::Sym<size, big_endian> sym(symp);
      const char* name;
						 ...
						 Output_file* of)
{
  ...
  for (unsigned int i = 0; i < this->incr_reloc_count_; ++i)
    {
      ...
      unsigned int shndx = cgccpp::Swap<32, big_endian>::readval(pov + 4);
      Output_section* os = this->ibase_->output_section(shndx);
      gold_assert(os != NULL);
      shndx = os->out_shndx();
      cgccpp::Swap<32, big_endian>::writeval(pov + 4, shndx);
    }
  ...
  for (unsigned int i = 0; i < this->local_symbol_count_; ++i)
	...
	osym.put_st_info(cgccpp::STB_LOCAL,
			 static_cast<cgccpp::STT>(lsym.st_type));
	osym.put_st_other(0);
