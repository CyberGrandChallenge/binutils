  ...
  if (rc == Z_OK)
    {
      ...
      cgccpp::Swap_unaligned<64, true>::writeval(*compressed_data + 4,
						 uncompressed_size);
		      ...
		      section_size_type compressed_size)
{
  ...
  if (compressed_size >= zlib_header_size
      && strncmp(reinterpret_cast<const char*>(compressed_data),
		 "ZLIB", 4) == 0)
    return cgccpp::Swap_unaligned<64, true>::readval(compressed_data + 4);
  return -1ULL;
}
			 ...
			 unsigned long uncompressed_size)
{
  ...
  if (compressed_size >= zlib_header_size
		 ...
		 "ZLIB", 4) == 0)
    {
	  ...
	  cgccpp::Swap_unaligned<64, true>::readval(compressed_data + 4);
