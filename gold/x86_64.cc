...
{
 public:
  typedef Output_data_reloc<cgccpp::SHT_RELA, true, size, false> Reloc_section;
 ...
 protected:
  ...
  fill_first_plt_entry(unsigned char* pov,
		       ...
		       typename cgccpp::Cgc_types<size>::Cgc_Addr plt_address)
  ...
  fill_tlsdesc_entry(unsigned char* pov,
		     typename cgccpp::Cgc_types<size>::Cgc_Addr got_address,
		     typename cgccpp::Cgc_types<size>::Cgc_Addr plt_address,
		     typename cgccpp::Cgc_types<size>::Cgc_Addr got_base,
  ...
  do_fill_first_plt_entry(unsigned char* pov,
			  typename cgccpp::Cgc_types<size>::Cgc_Addr got_addr,
			  typename cgccpp::Cgc_types<size>::Cgc_Addr plt_addr)
    = 0;
...
class Target_x86_64 : public Sized_target<size, false>
{
 public:
  ...
  // uses only Cgc64_Rela relocation entries with explicit addends."
  typedef Output_data_reloc<cgccpp::SHT_RELA, true, size, false> Reloc_section;
  ...
  Target_x86_64(const Target::Target_info* info = &x86_64_info)
    : Sized_target<size, false>(info),
      ...
      rela_irelative_(NULL), copy_relocs_(cgccpp::R_X86_64_COPY),
  ...
  {
    gold_assert(r_type == cgccpp::R_X86_64_TLSDESC);
    return 0;
  }
 ...
 private:
  ...
  public:
    ...
    local(Symbol_table* symtab, Layout* layout, Target_x86_64* target,
	  ...
	  unsigned int data_shndx,
	  Output_section* output_section,
	  const cgccpp::Rela<size, false>& reloc, unsigned int r_type,
	  const cgccpp::Sym<size, false>& lsym,
	  bool is_discarded);
    ...
    local_reloc_may_be_function_pointer(Symbol_table* symtab, Layout* layout,
					...
					unsigned int data_shndx,
					Output_section* output_section,
					const cgccpp::Rela<size, false>& reloc,
					unsigned int r_type,
					const cgccpp::Sym<size, false>& lsym);
  ...
  class Relocate
  {
   public:
    ...
    relocate(const Relocate_info<size, false>*, Target_x86_64*,
	     Output_section*,
	     size_t relnum, const cgccpp::Rela<size, false>&,
   ...
   private:
    ...
    tls_gd_to_ie(const Relocate_info<size, false>*, size_t relnum,
		 ...
		 const cgccpp::Rela<size, false>&, unsigned int r_type,
  ...
  copy_reloc(Symbol_table* symtab, Layout* layout,
	     ...
	     Symbol* sym, const cgccpp::Rela<size, false>& reloc)
  {
  ...
  Copy_relocs<cgccpp::SHT_RELA, size, false> copy_relocs_;
...
const Target::Target_info Target_x86_64<64>::x86_64_info =
{
  ...
  cgccpp::SHN_X86_64_LCOMMON,	// large_common_shndx
  0,			// small_common_section_flags
  cgccpp::SHF_X86_64_LARGE,	// large_common_section_flags
...
{
  if ((os->flags() & cgccpp::SHF_X86_64_LARGE) != 0)
					     ...
					     unsigned int got_offset)
{
  if (gsym->type() == cgccpp::STT_GNU_IFUNC
    ...
    {
      Reloc_section* rela = this->rela_irelative(symtab, layout);
      rela->add_symbolless_global_addend(gsym, cgccpp::R_X86_64_IRELATIVE,
					 this->got_irelative_, got_offset, 0);
    }
  else
    {
      gsym->set_needs_dynsym_entry();
      this->rel_->add_global(gsym, cgccpp::R_X86_64_JUMP_SLOT, this->got_plt_,
			     got_offset, 0);
    }
}
  ...
  if (this->tlsdesc_rel_ == NULL)
    {
      ...
      layout->add_output_section_data(".rela.plt", cgccpp::SHT_RELA,
				      cgccpp::SHF_ALLOC, this->tlsdesc_rel_,
				      ORDER_DYNAMIC_PLT_RELOCS, false);
    ...
    typename cgccpp::Cgc_types<size>::Cgc_Addr plt_address)
{
  ...
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 2,
					      (got_address + 8
					       - (plt_address + 6)));
  cgccpp::Swap<32, false>::writeval(pov + 8,
				    (got_address + 16
				     - (plt_address + 12)));
}
    ...
    unsigned int plt_index)
{
  ...
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 7, plt_index);
  cgccpp::Swap<32, false>::writeval(pov + 12,
    ...
    unsigned int plt_offset)
{
  ...
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 2,
					      (got_address + 8
					       - (plt_address + plt_offset
						  + 6)));
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 8,
					      (got_base
					       + tlsdesc_got_offset
					       - (plt_address + plt_offset
						  + 12)));
...
Output_data_plt_x86_64<size>::plt_eh_frame_cie[plt_eh_frame_cie_size] =
{
  ...
  1,				// Augmentation size.
  (cgccpp::DW_EH_PE_pcrel	// FDE encoding.
   | cgccpp::DW_EH_PE_sdata4),
  cgccpp::DW_CFA_def_cfa, 7, 8,	// DW_CFA_def_cfa: r7 (rsp) ofs 8.
  cgccpp::DW_CFA_offset + 16, 1,// DW_CFA_offset: r16 (rip) at cfa-8.
  cgccpp::DW_CFA_nop,		// Align to 16 bytes.
  cgccpp::DW_CFA_nop
};
...
Output_data_plt_x86_64_standard<size>::plt_eh_frame_fde[plt_eh_frame_fde_size] =
{
  ...
  0,					// Augmentation size.
  cgccpp::DW_CFA_def_cfa_offset, 16,	// DW_CFA_def_cfa_offset: 16.
  cgccpp::DW_CFA_advance_loc + 6,	// Advance 6 to __PLT__ + 6.
  cgccpp::DW_CFA_def_cfa_offset, 24,	// DW_CFA_def_cfa_offset: 24.
  cgccpp::DW_CFA_advance_loc + 10,	// Advance 10 to __PLT__ + 16.
  cgccpp::DW_CFA_def_cfa_expression,	// DW_CFA_def_cfa_expression.
  11,					// Block length.
  cgccpp::DW_OP_breg7, 8,		// Push %rsp + 8.
  cgccpp::DW_OP_breg16, 0,		// Push %rip.
  cgccpp::DW_OP_lit15,			// Push 0xf.
  cgccpp::DW_OP_and,			// & (%rip & 0xf).
  cgccpp::DW_OP_lit11,			// Push 0xb.
  cgccpp::DW_OP_ge,			// >= ((%rip & 0xf) >= 0xb)
  cgccpp::DW_OP_lit3,			// Push 3.
  cgccpp::DW_OP_shl,			// << (((%rip & 0xf) >= 0xb) << 3)
  cgccpp::DW_OP_plus,			// + ((((%rip&0xf)>=0xb)<<3)+%rsp+8
  cgccpp::DW_CFA_nop,			// Align to 32 bytes.
  cgccpp::DW_CFA_nop,
  cgccpp::DW_CFA_nop,
  cgccpp::DW_CFA_nop
};
...
Output_data_plt_x86_64<size>::do_write(Output_file* of)
{
  ...
  typename cgccpp::Cgc_types<size>::Cgc_Addr got_base = this->got_->address();
  ...
  cgccpp::Swap<64, false>::writeval(got_pov, dynamic_addr);
  got_pov += 8;
  memset(got_pov, 0, 16);
  got_pov += 16;
  ...
  for (unsigned int plt_index = 0;
	 ...
	 got_offset += 8)
    {
      ...
      cgccpp::Swap<64, false>::writeval(got_pov,
    ...
    unsigned int got_type)
{
  ...
  switch (got_type)
	...
	rela_dyn->add_local_relative(obj, r_sym, cgccpp::R_X86_64_RELATIVE,
				     this->got_, got_offset, 0, false);
      break;
    case GOT_TYPE_TLS_OFFSET:
      rela_dyn->add_local(obj, r_sym, cgccpp::R_X86_64_TPOFF64,
			  this->got_, got_offset, 0);
      break;
    case GOT_TYPE_TLS_PAIR:
      this->got_->reserve_slot(got_index + 1);
      rela_dyn->add_local(obj, r_sym, cgccpp::R_X86_64_DTPMOD64,
			  this->got_, got_offset, 0);
      break;
    case GOT_TYPE_TLS_DESC:
      gold_fatal(_("TLS_DESC not yet supported for incremental linking"));
      // this->got_->reserve_slot(got_index + 1);
      // rela_dyn->add_target_specific(cgccpp::R_X86_64_TLSDESC, arg,
      //			       this->got_, got_offset, 0);
      break;
    default:
      gold_unreachable();
  ...
  switch (got_type)
      ...
      if (!gsym->final_value_is_known())
	{
	  if (gsym->is_from_dynobj()
	    ...
	    rela_dyn->add_global(gsym, cgccpp::R_X86_64_GLOB_DAT,
				 this->got_, got_offset, 0);
	  else
	    rela_dyn->add_global_relative(gsym, cgccpp::R_X86_64_RELATIVE,
					  this->got_, got_offset, 0, false);
	}
      break;
    case GOT_TYPE_TLS_OFFSET:
      rela_dyn->add_global_relative(gsym, cgccpp::R_X86_64_TPOFF64,
				    this->got_, got_offset, 0, false);
      break;
    case GOT_TYPE_TLS_PAIR:
      this->got_->reserve_slot(got_index + 1);
      rela_dyn->add_global_relative(gsym, cgccpp::R_X86_64_DTPMOD64,
				    this->got_, got_offset, 0, false);
      rela_dyn->add_global_relative(gsym, cgccpp::R_X86_64_DTPOFF64,
				    this->got_, got_offset + 8, 0, false);
      break;
    case GOT_TYPE_TLS_DESC:
      this->got_->reserve_slot(got_index + 1);
      rela_dyn->add_global_relative(gsym, cgccpp::R_X86_64_TLSDESC,
				    this->got_, got_offset, 0, false);
      break;
    default:
      gold_unreachable();
  ...
  if (this->got_mod_index_offset_ == -1U)
    {
      ...
      Output_data_got<64, false>* got = this->got_section(symtab, layout);
      unsigned int got_offset = got->add_constant(0);
      rela_dyn->add_local(object, 0, cgccpp::R_X86_64_DTPMOD64, got,
			  got_offset, 0);
      got->add_constant(0);
...
Target_x86_64<size>::optimize_tls_reloc(bool is_final, int r_type)
{
  ...
  switch (r_type)
    {
    case cgccpp::R_X86_64_TLSGD:
    case cgccpp::R_X86_64_GOTPC32_TLSDESC:
    case cgccpp::R_X86_64_TLSDESC_CALL:
    ...
    case cgccpp::R_X86_64_TLSLD:
      // This is Local-Dynamic, which refers to a local symbol in the
      // dynamic TLS block.  Since we know that we generating an
      // executable, we can switch to Local-Exec.
      return tls::TLSOPT_TO_LE;

    case cgccpp::R_X86_64_DTPOFF32:
    case cgccpp::R_X86_64_DTPOFF64:
      // Another Local-Dynamic reloc.
      return tls::TLSOPT_TO_LE;

    case cgccpp::R_X86_64_GOTTPOFF:
...
Target_x86_64<size>::Scan::get_reference_flags(unsigned int r_type)
{
  switch (r_type)
    {
    case cgccpp::R_X86_64_NONE:
    case cgccpp::R_X86_64_GNU_VTINHERIT:
    case cgccpp::R_X86_64_GNU_VTENTRY:
    case cgccpp::R_X86_64_GOTPC32:
    case cgccpp::R_X86_64_GOTPC64:
      // No symbol reference.
      return 0;

    case cgccpp::R_X86_64_64:
    case cgccpp::R_X86_64_32:
    case cgccpp::R_X86_64_32S:
    case cgccpp::R_X86_64_16:
    case cgccpp::R_X86_64_8:
      return Symbol::ABSOLUTE_REF;

    case cgccpp::R_X86_64_PC64:
    case cgccpp::R_X86_64_PC32:
    case cgccpp::R_X86_64_PC32_BND:
    case cgccpp::R_X86_64_PC16:
    case cgccpp::R_X86_64_PC8:
    case cgccpp::R_X86_64_GOTOFF64:
      return Symbol::RELATIVE_REF;

    case cgccpp::R_X86_64_PLT32:
    case cgccpp::R_X86_64_PLT32_BND:
    case cgccpp::R_X86_64_PLTOFF64:
      return Symbol::FUNCTION_CALL | Symbol::RELATIVE_REF;

    case cgccpp::R_X86_64_GOT64:
    case cgccpp::R_X86_64_GOT32:
    case cgccpp::R_X86_64_GOTPCREL64:
    case cgccpp::R_X86_64_GOTPCREL:
    case cgccpp::R_X86_64_GOTPLT64:
      // Absolute in GOT.
      return Symbol::ABSOLUTE_REF;

    case cgccpp::R_X86_64_TLSGD:            // Global-dynamic
    case cgccpp::R_X86_64_GOTPC32_TLSDESC:  // Global-dynamic (from ~oliva url)
    case cgccpp::R_X86_64_TLSDESC_CALL:
    case cgccpp::R_X86_64_TLSLD:            // Local-dynamic
    case cgccpp::R_X86_64_DTPOFF32:
    case cgccpp::R_X86_64_DTPOFF64:
    case cgccpp::R_X86_64_GOTTPOFF:         // Initial-exec
    case cgccpp::R_X86_64_TPOFF32:          // Local-exec
      return Symbol::TLS_REF;

    case cgccpp::R_X86_64_COPY:
    case cgccpp::R_X86_64_GLOB_DAT:
    case cgccpp::R_X86_64_JUMP_SLOT:
    case cgccpp::R_X86_64_RELATIVE:
    case cgccpp::R_X86_64_IRELATIVE:
    case cgccpp::R_X86_64_TPOFF64:
    case cgccpp::R_X86_64_DTPMOD64:
    case cgccpp::R_X86_64_TLSDESC:
    case cgccpp::R_X86_64_SIZE32:
    case cgccpp::R_X86_64_SIZE64:
    default:
					 ...
					 Symbol* gsym)
{
  switch (r_type)
    ...
    case cgccpp::R_X86_64_32:
      ...
      if (size == 32 && r_type == cgccpp::R_X86_64_32)
	return;
  ...
  switch (r_type)
      ...
      if (parameters->options().output_is_position_independent())
	{
	  ...
	  rela_dyn->add_local_relative(object, r_sym,
				       (size == 32
					? cgccpp::R_X86_64_RELATIVE64
					: cgccpp::R_X86_64_RELATIVE),
	...
	if (is_new)
	  {
		...
		if (r_type != cgccpp::R_X86_64_GOT32)
		  {
		    unsigned int got_offset =
	...
	switch (r_type)
	  {
	  case cgccpp::R_X86_64_TLSGD:       // General-dynamic
		...
		if (!object->local_has_got_offset(r_sym, GOT_TYPE_TLS_DESC))
		  {
		    ...
		    rt->add_target_specific(cgccpp::R_X86_64_TLSDESC, arg,
					    got, got_offset, 0);
		  }
	      }
	    else if (optimized_type != tls::TLSOPT_TO_LE)
...
Target_x86_64<size>::Scan::local_reloc_may_be_function_pointer(
  ...
  unsigned int ,
  Output_section* ,
  const cgccpp::Rela<size, false>& ,
  unsigned int r_type,
  const cgccpp::Sym<size, false>&)
{
			    ...
			    Symbol* gsym)
{
  ...
  switch (r_type)
	...
	if (gsym->needs_dynamic_reloc(Scan::get_reference_flags(r_type)))
	  {
	    ...
	    else if (((size == 64 && r_type == cgccpp::R_X86_64_64)
		      || (size == 32 && r_type == cgccpp::R_X86_64_32))
		     && gsym->type() == cgccpp::STT_GNU_IFUNC
		     && gsym->can_use_relative_reloc(false)
		     && !gsym->is_from_dynobj()
		     && !gsym->is_undefined()
		...
		unsigned int r_type = cgccpp::R_X86_64_IRELATIVE;
	...
	if (r_type == cgccpp::R_X86_64_GOTPLT64
    ...
    case cgccpp::R_X86_64_PLTOFF64:
      ...
      // symbol is not fully resolved).
      if (r_type == cgccpp::R_X86_64_PLTOFF64
	  && !gsym->final_value_is_known())
	target->make_plt_entry(symtab, layout, gsym);
      break;
				       ...
				       const unsigned char* plocal_symbols)
{
  ...
  if (sh_type == cgccpp::SHT_REL)
   ...
   gold::gc_process_relocs<size, false, Target_x86_64<size>, cgccpp::SHT_RELA,
				 ...
				 const unsigned char* plocal_symbols)
{
  ...
  gold::scan_relocs<size, false, Target_x86_64<size>, cgccpp::SHT_RELA,
      typename Target_x86_64<size>::Scan>(
    symtab,
    layout,
    this,
    ...
    Symbol_table* symtab)
{
  ...
  if (odyn != NULL)
	  ...
	  && this->plt_->has_tlsdesc_entry())
	{
	  ...
	  odyn->add_section_plus_offset(cgccpp::DT_TLSDESC_PLT,
					this->plt_, plt_offset);
	  odyn->add_section_plus_offset(cgccpp::DT_TLSDESC_GOT,
...
Target_x86_64<size>::Relocate::relocate(
    ...
    Output_section*,
    size_t relnum,
    const cgccpp::Rela<size, false>& rela,
    unsigned int r_type,
    ...
    section_size_type view_size)
{
  if (this->skip_call_tls_get_addr_)
    {
      if ((r_type != cgccpp::R_X86_64_PLT32
	   && r_type != cgccpp::R_X86_64_PLT32_BND
	   && r_type != cgccpp::R_X86_64_PC32_BND
	   && r_type != cgccpp::R_X86_64_PC32)
	  || gsym == NULL
    ...
    section_size_type view_size)
{
  ...
  const Sized_relobj_file<size, false>* object = relinfo->object;
  const cgccpp::Cgc_Xword addend = rela.get_r_addend();
  cgccpp::Shdr<size, false> data_shdr(relinfo->data_shdr);
  bool is_executable = (data_shdr.get_sh_flags() & cgccpp::SHF_EXECINSTR) != 0;

  typename cgccpp::Cgc_types<size>::Cgc_Addr value = psymval->value(relinfo->object, 0);
  ...
  switch (r_type)
      ...
      else
	{
	  unsigned int got_type = (optimized_type == tls::TLSOPT_TO_IE
				   ...
				   : GOT_TYPE_TLS_DESC);
	  unsigned int got_offset = 0;
	  if (r_type == cgccpp::R_X86_64_GOTPC32_TLSDESC
	      && optimized_type == tls::TLSOPT_NONE)
	    {
    ...
    section_size_type view_size)
{
  if (r_type == cgccpp::R_X86_64_GOTPC32_TLSDESC)
  ...
  else
    {
      ...
      // ==> nop; nop
      gold_assert(r_type == cgccpp::R_X86_64_TLSDESC_CALL);
    ...
    const Reloc_symbol_changes* reloc_symbol_changes)
{
  ...
  gold::relocate_section<size, false, Target_x86_64<size>, cgccpp::SHT_RELA,
    ...
    Relocatable_relocs* rr)
{
  ...
  gold::scan_relocatable_relocs<size, false, cgccpp::SHT_RELA,
    ...
    section_size_type reloc_view_size)
{
  ...
  gold::relocate_relocs<size, false, cgccpp::SHT_RELA>(
    relinfo,
    prelocs,
    reloc_count,
    output_section,
  ...
  if (length >= 16)
    {
      ...
      unsigned char jmp[5];
      jmp[0] = 0xe9;
      cgccpp::Swap_unaligned<32, false>::writeval(jmp + 1, length - 5);
					...
					std::string* to) const
{
  ...
  // lea NN(%rsp),%r11
	   ...
	   && fnsize > 8)
    {
      ...
      uint32_t val = cgccpp::Swap_unaligned<32, false>::readval(pval);
      val -= parameters->options().split_stack_adjust_size();
      cgccpp::Swap_unaligned<32, false>::writeval(pval, val);
    }
  else
    {
...
public:
  Target_selector_x86_64()
    : Target_selector_freebsd(cgccpp::EM_X86_64, size, false,
			      (size == 64
			       ? "cgc64-x86-64" : "cgc32-x86-64"),
			      (size == 64
			       ? "cgc64-x86-64-freebsd"
			       : "cgc32-x86-64-freebsd"),
			      (size == 64 ? "cgc_x86_64" : "cgc32_x86_64"))
  { }
    ...
    typename cgccpp::Cgc_types<size>::Cgc_Addr plt_address)
{
  ...
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 2,
					      (got_address + 8
					       - (plt_address + 2 + 4)));
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 9,
					      (got_address + 16
					       - (plt_address + 9 + 4)));
}
...
{
  memcpy(pov, plt_entry, plt_entry_size);
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 3,
					      (got_address + got_offset
					       - (plt_address + plt_offset
						  + 3 + 4)));

  cgccpp::Swap_unaligned<32, false>::writeval(pov + 33, plt_index);
  cgccpp::Swap_unaligned<32, false>::writeval(pov + 38,
					      - (plt_offset + 38 + 4));
...
Output_data_plt_x86_64_nacl<size>::plt_eh_frame_fde[plt_eh_frame_fde_size] =
{
  ...
  cgccpp::DW_CFA_advance_loc + 6,	// Advance 6 to __PLT__ + 6.
  cgccpp::DW_CFA_def_cfa_offset, 24,	// DW_CFA_def_cfa_offset: 24.
  cgccpp::DW_CFA_advance_loc + 58,	// Advance 58 to __PLT__ + 64.
  cgccpp::DW_CFA_def_cfa_expression,	// DW_CFA_def_cfa_expression.
  13,					// Block length.
  cgccpp::DW_OP_breg7, 8,		// Push %rsp + 8.
  cgccpp::DW_OP_breg16, 0,		// Push %rip.
  cgccpp::DW_OP_const1u, 63,		// Push 0x3f.
  cgccpp::DW_OP_and,			// & (%rip & 0x3f).
  cgccpp::DW_OP_const1u, 37,            // Push 0x25.
  cgccpp::DW_OP_ge,			// >= ((%rip & 0x3f) >= 0x25)
  cgccpp::DW_OP_lit3,			// Push 3.
  cgccpp::DW_OP_shl,			// << (((%rip & 0x3f) >= 0x25) << 3)
  cgccpp::DW_OP_plus,			// + ((((%rip&0x3f)>=0x25)<<3)+%rsp+8
  cgccpp::DW_CFA_nop,			// Align to 32 bytes.
  cgccpp::DW_CFA_nop
};
