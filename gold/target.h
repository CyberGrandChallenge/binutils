...
class Target
{
 public:
  ...
  // Machine code to store in e_machine field of CGC header.
  cgccpp::EM
  machine_code() const
  { return this->pti_->machine_code; }

  // Processor specific flags to store in e_flags field of CGC header.
  cgccpp::Cgc_Word
  ...
  {
    gold_assert(this->pti_->small_common_shndx != cgccpp::SHN_UNDEF);
  ...
  {
    gold_assert(this->pti_->large_common_shndx != cgccpp::SHN_UNDEF);
  ...
  void
  adjust_cgc_header(unsigned char* view, int len)
  { return this->do_adjust_cgc_header(view, len); }
  ...
  template<int size, bool big_endian>
  Object*
  make_cgc_object(const std::string& name, Input_file* input_file,
		  off_t offset, const cgccpp::Ehdr<size, big_endian>& ehdr)
  { return this->do_make_cgc_object(name, input_file, offset, ehdr); }

  // Make an output section.
  Output_section*
  make_output_section(const char* name, cgccpp::Cgc_Word type,
  ...
  // Return the value to store in the EI_OSABI field in the CGC
  // header.
  cgccpp::CGCOSABI
  osabi() const
  { return this->osabi_; }

  // Set the value to store in the EI_OSABI field in the CGC header.
  void
  set_osabi(cgccpp::CGCOSABI osabi)
  { this->osabi_ = osabi; }
 ...
 protected:
  ...
  struct Target_info
  {
    ...
    bool is_big_endian;
    // The code to store in the e_machine field of the CGC header.
    cgccpp::EM machine_code;
    // Whether this target has a specific make_symbol function.
    ...
    // if none.
    cgccpp::Cgc_Half small_common_shndx;
    // The special section index for large common symbols; SHN_UNDEF
    // if none.
    cgccpp::Cgc_Half large_common_shndx;
    // Section flags for small common section.
    cgccpp::Cgc_Xword small_common_section_flags;
    // Section flags for large common section.
    cgccpp::Cgc_Xword large_common_section_flags;
  ...
  Target(const Target_info* pti)
    : pti_(pti), processor_specific_flags_(0),
      are_processor_specific_flags_set_(false), osabi_(cgccpp::CGCOSABI_NONE)
  { }
  ...
  // will be one of the values of cgccpp::Cgc_sizes<size>::ehdr_size.
  // By default, we set the EI_OSABI field if requested (in
  // Sized_target).
  virtual void
  do_adjust_cgc_header(unsigned char*, int) = 0;
  ...
  // make_cgc_object hooks.  There are four versions of these for
  // different address sizes and endianness.

  // Set processor specific flags.
  void
  set_processor_specific_flags(cgccpp::Cgc_Word flags)
  {
  ...
  virtual Object*
  do_make_cgc_object(const std::string&, Input_file*, off_t,
		     const cgccpp::Ehdr<64, false>& ehdr);
#endif

#ifdef HAVE_TARGET_64_BIG
  // Virtual functions which may be overridden by the child class.
  virtual Object*
  do_make_cgc_object(const std::string& name, Input_file* input_file,
		     off_t offset, const cgccpp::Ehdr<64, true>& ehdr);
#endif
 ...
 private:
  // The implementations of the four do_make_cgc_object virtual functions are
  // almost identical except for their sizes and endianness.  We use a template.
  // for their implementations.
  template<int size, bool big_endian>
  inline Object*
  do_make_cgc_object_implementation(const std::string&, Input_file*, off_t,
  ...
  // If not CGCOSABI_NONE, the value to put in the EI_OSABI field of
  // the CGC header.  This is handled at this level because it is
  // OS-specific rather than processor-specific.
  cgccpp::CGCOSABI osabi_;
};
...
class Sized_target : public Target
{
 public:
  ...
  virtual void
  resolve(Symbol*, const cgccpp::Sym<size, big_endian>&, Object*,
	  const char*)
  { gold_unreachable(); }
  ...
  gc_add_reference(Symbol_table* symtab,
		   ...
		   typename cgccpp::Cgc_types<size>::Cgc_Addr dst_off) const
  {
