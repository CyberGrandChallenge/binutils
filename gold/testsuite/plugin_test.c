...
static void parse_readcgc_line(char*, struct sym_info*);
...
claim_file_hook (const struct ld_plugin_input_file* file, int* claimed)
{
  ...
  /* Look for the beginning of output from readcgc -s.  */
  ...
  /* Parse the output from readcgc. The columns are:
...
static void
parse_readcgc_line(char* p, struct sym_info* info)
{
  int len;
