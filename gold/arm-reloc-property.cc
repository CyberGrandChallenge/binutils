    ...
    uses_symbol_(false)
{
  ...
  if (rtype == RT_STATIC)
	...
	case RC_DATA:
	  ...
	  if (code == cgccpp::R_ARM_ABS8)
	    this->size_ = 1;
	  else if (code == cgccpp::R_ARM_ABS16)
	    this->size_ = 2;
	  else
	    this->size_ = 4;
	  this->align_ = 1;
...
#define RD(name, type, deprecated, class, operation, is_implemented, \
  ...
  do \
    { \
      unsigned int code = cgccpp::R_ARM_##name; \
      gold_assert(code < Property_table_size); \
      this->table_[code] = \
	new Arm_reloc_property(cgccpp::R_ARM_##name, "R_ARM_" #name, \
