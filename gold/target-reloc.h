    ...
    const unsigned char* plocal_syms)
{
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
      ...
      if (r_sym < local_count)
	{
	  ...
	  typename cgccpp::Sym<size, big_endian> lsym(plocal_syms
						      + r_sym * sym_size);
...
visibility_error(const Symbol* sym)
{
  ...
  switch (sym->visibility())
    {
    case cgccpp::STV_INTERNAL:
      v = _("internal");
      break;
    case cgccpp::STV_HIDDEN:
      v = _("hidden");
      break;
    case cgccpp::STV_PROTECTED:
      v = _("protected");
      break;
    default:
      gold_unreachable();
...
issue_undefined_symbol_error(const Symbol* sym)
{
  ...
  if (sym->binding() == cgccpp::STB_WEAK)
    return false;
...
// SIZE is the CGC size: 32 or 64.  BIG_ENDIAN is the endianness of
      ...
      if (issue_undefined_symbol_error(sym))
	{
	  ...
	  if (sym->is_cxx_vtable())
	    gold_info(_("%s: the vtable symbol may be undefined because "
	       ...
	       && sym->visibility() != cgccpp::STV_DEFAULT
...
apply_relocation(const Relocate_info<size, big_endian>* relinfo,
		 ...
		 section_size_type view_size)
{
  // Construct the CGC relocation in a temporary buffer.
  const int reloc_size = cgccpp::Cgc_sizes<size>::rela_size;
  unsigned char relbuf[reloc_size];
  cgccpp::Rela<size, big_endian> rel(relbuf);
  cgccpp::Rela_write<size, big_endian> orel(relbuf);
  orel.put_r_offset(r_offset);
  orel.put_r_info(cgccpp::cgc_r_info<size>(0, r_type));
      ...
      else
	{
	  ...
	  if (r_sym >= local_symbol_count)
		  ...
		  && !object->is_section_included(shndx))
		{
	      ...
	      else if (lsym.get_st_type() != cgccpp::STT_SECTION)
    ...
    section_size_type reloc_view_size)
{
  ...
  for (size_t i = 0; i < reloc_count; ++i, prelocs += reloc_size)
      ...
      if (!parameters->options().relocatable())
	{
      ...
      reloc_write.put_r_info(cgccpp::cgc_r_info<size>(new_symndx, r_type));
