...
struct Symbol_location_info
{
  ...
  cgccpp::STT enclosing_symbol_type;
};
...
class Object
{
 public:
  ...
  adjust_sym_shndx(unsigned int symndx, unsigned int shndx, bool* is_ordinary)
  {
    if (shndx < cgccpp::SHN_LORESERVE)
      *is_ordinary = true;
    else if (shndx == cgccpp::SHN_XINDEX)
      {
  ...
  // Functions and types for the cgccpp::Cgc_file interface.  This
 ...
 protected:
  ...
  template<int size, bool big_endian>
  void
  read_section_data(cgccpp::Cgc_file<size, big_endian, Object>*,
		    Read_symbols_data*);
...
{
 public:
  typedef typename cgccpp::Cgc_types<size>::Cgc_Addr Value;
...
class Sized_relobj_file : public Sized_relobj<size, big_endian>
{
 public:
  ...
  Sized_relobj_file(const std::string& name,
		    ...
		    off_t offset,
		    const typename cgccpp::Ehdr<size, big_endian>&);
 ...
 protected:
  ...
  { return this->cgc_file_.section_addr(shndx); }
  ...
  { return this->cgc_file_.section_link(shndx); }

  // Return the section info field.
  unsigned int
  do_section_info(unsigned int shndx)
  { return this->cgc_file_.section_info(shndx); }

  // Return the section alignment.
  uint64_t
  do_section_addralign(unsigned int shndx)
  { return this->cgc_file_.section_addralign(shndx); }
  ...
  {
    if (shndx >= cgccpp::SHN_LORESERVE)
      shndx += this->cgc_file_.large_shndx_offset();
    return shndx;
  }
  ...
  // Allow a child class to access the CGC file.
  cgccpp::Cgc_file<size, big_endian, Object>*
  cgc_file()
  { return &this->cgc_file_; }
 ...
 private:
  ...
  static const int ehdr_size = cgccpp::Cgc_sizes<size>::ehdr_size;
  static const int shdr_size = cgccpp::Cgc_sizes<size>::shdr_size;
  static const int sym_size = cgccpp::Cgc_sizes<size>::sym_size;
  ...
  // frame section.
  bool
  check_eh_frame_flags(const cgccpp::Shdr<size, big_endian>* shdr) const;
  ...
  include_linkonce_section(Layout*, unsigned int, const char*,
			   const cgccpp::Shdr<size, big_endian>&);
  ...
  // Type of CGC file (ET_REL or ET_EXEC).  ET_EXEC files are allowed
...
// Return whether INPUT_FILE contains an CGC object start at file
...
// and holds the CGC header.  If PUNCONFIGURED is not NULL, then if
...
extern Object*
make_cgc_object(const std::string& name, Input_file*,
