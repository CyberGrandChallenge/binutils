...
// FreeBSD 4.1 and later wants the EI_OSABI field in the CGC header to
// be set to CGCOSABI_FREEBSD.  This is a target selector for targets
...
class Target_selector_freebsd : public Target_selector
{
 ...
 protected:
  ...
  do_recognize(Input_file*, off_t, int, int osabi, int)
  {
    ...
    if (osabi == cgccpp::CGCOSABI_FREEBSD)
      ret->set_osabi(static_cast<cgccpp::CGCOSABI>(osabi));
    return ret;
  }
  ...
  do_recognize_by_bfd_name(const char* name)
  {
    if (strcmp(name, this->bfd_name_) == 0)
      ...
      {
	Target* ret = this->instantiate_target();
	ret->set_osabi(cgccpp::CGCOSABI_FREEBSD);
	return ret;
      }
    else
      return NULL;
  ...
  do_target_bfd_name(const Target* target)
  {
    if (!this->is_our_target(target))
      return NULL;
    return (target->osabi() == cgccpp::CGCOSABI_FREEBSD
