...
Read_symbols::do_read_symbols(Workqueue* workqueue)
{
  ...
  // Read enough of the file to pick up the entire CGC header.
  ...
  int read_size;
  bool is_cgc = is_cgc_object(input_file, 0, &ehdr, &read_size);
  ...
  if (is_cgc)
    {
      // This is an CGC object.

      unconfigured = false;
      punconfigured = (input_file->will_search_for()
		       ? &unconfigured
		       : NULL);
      cgc_obj = make_cgc_object(input_file->filename(),
  ...
  if (parameters->options().has_plugins())
    {
      ...
      if (obj != NULL)
        {
	  // Delete the cgc_obj, this file has been claimed.
	  if (cgc_obj != NULL)
	    delete cgc_obj;
