...
class Memory_region
{
 public:
  ...
  bool
  attributes_compatible(cgccpp::Cgc_Xword flags, cgccpp::Cgc_Xword type) const;
...
bool
Memory_region::attributes_compatible(cgccpp::Cgc_Xword flags,
				     cgccpp::Cgc_Xword type) const
{
					      ...
					      Elements_iterator location)
{
  ...
  if (!this->places_[PLACE_REL].have_location
      && os != NULL
      && (os->type() == cgccpp::SHT_REL || os->type() == cgccpp::SHT_RELA)
      && (os->flags() & cgccpp::SHF_ALLOC) != 0)
    {
				     ...
				     Elements_iterator** pwhere)
{
  ...
  if ((flags & cgccpp::SHF_ALLOC) == 0
    ...
    index = PLACE_LAST;
  else if (type == cgccpp::SHT_NOTE)
    index = PLACE_INTERP;
  else if ((flags & cgccpp::SHF_TLS) != 0)
    {
      if (type == cgccpp::SHT_NOBITS)
	index = PLACE_TLS_BSS;
      else
	index = PLACE_TLS;
    }
  else if (type == cgccpp::SHT_NOBITS)
    index = PLACE_BSS;
  else if ((flags & cgccpp::SHF_WRITE) != 0)
    index = PLACE_DATA;
  else if (type == cgccpp::SHT_REL || type == cgccpp::SHT_RELA)
    index = PLACE_REL;
  else if ((flags & cgccpp::SHF_EXECINSTR) == 0)
    index = PLACE_RODATA;
  else
    index = PLACE_TEXT;
					       ...
					       unsigned char* buf)
{
  switch (this->data_size())
    {
    case 1:
      cgccpp::Swap_unaligned<8, big_endian>::writeval(buf, val);
      break;
    case 2:
      cgccpp::Swap_unaligned<16, big_endian>::writeval(buf, val);
      break;
    case 4:
      cgccpp::Swap_unaligned<32, big_endian>::writeval(buf, val);
      break;
    case 8:
      if (parameters->target().get_size() == 32)
	{
      ...
      cgccpp::Swap_unaligned<64, big_endian>::writeval(buf, val);
      break;
    default:
      gold_unreachable();
    }
...
class Output_section_element_fill : public Output_section_element
{
 public:
			...
			std::string* fill, Input_section_list*)
  {
    ...
    cgccpp::Swap_unaligned<32, true>::writeval(fill_buff, fill_val);
    ...
    Input_section_list* input_sections)
{
  ...
  if (output_section == NULL
      || (output_section->flags() & cgccpp::SHF_TLS) == 0
      || output_section->type() != cgccpp::SHT_NOBITS)
    *dot_value = dot;
                                                 ...
                                                 uint64_t* load_address)
{
  ...
  *dot_value = address;
  ...
  if (this->output_section_ != NULL
      && ((this->output_section_->flags() & cgccpp::SHF_ALLOC) != 0
...
Output_section_definition::check_constraint(Output_section_definition** posd)
{
  switch (this->constraint_)
    ...
    case CONSTRAINT_ONLY_IF_RO:
      if (this->output_section_ != NULL
	  && (this->output_section_->flags() & cgccpp::SHF_WRITE) != 0)
	{
	  *posd = this;
					       ...
					       bool* orphan)
{
  ...
  if (this->output_section_ == NULL)
    return NULL;
  if ((this->output_section_->flags() & cgccpp::SHF_ALLOC) == 0)
    return NULL;
  *orphan = false;
  return this->output_section_;
}
                                             ...
                                             uint64_t* load_address)
{
  ...
  if ((this->os_->flags() & cgccpp::SHF_ALLOC) != 0)
    {
  ...
  if (parameters->options().relocatable())
    {
      ...
      *load_address = 0;
	   ...
	   || (this->os_->flags() & cgccpp::SHF_TLS) == 0
	   || this->os_->type() != cgccpp::SHT_NOBITS)
    {
...
Script_sections::set_section_addresses(Symbol_table* symtab, Layout* layout)
{
  ...
  for (Sections_elements::const_iterator p = this->sections_elements_->begin();
       ...
       ++p)
    {
      ...
      if (os != NULL && (os->flags() & cgccpp::SHF_TLS) != 0)
	{
	  if (first_tls == NULL)
	    first_tls = os;
				 ...
				 const Output_section* os2) const
{
  ...
  bool nobits1 = os1->type() == cgccpp::SHT_NOBITS;
  bool nobits2 = os2->type() == cgccpp::SHT_NOBITS;
  if (nobits1 != nobits2)
    return nobits2;

  // Sort PROGBITS TLS sections to the end, NOBITS TLS sections to the
  // beginning.
  bool tls1 = (os1->flags() & cgccpp::SHF_TLS) != 0;
  bool tls2 = (os2->flags() & cgccpp::SHF_TLS) != 0;
  if (tls1 != tls2)
...
{
  return (os->type() == cgccpp::SHT_NOBITS
	  && (os->flags() & cgccpp::SHF_TLS) == 0);
}
...
Script_sections::total_header_size(Layout* layout) const
{
  ...
  if (parameters->target().get_size() == 32)
    {
      file_header_size = cgccpp::Cgc_sizes<32>::ehdr_size;
      segment_headers_size = segment_count * cgccpp::Cgc_sizes<32>::phdr_size;
    }
  else if (parameters->target().get_size() == 64)
    {
      file_header_size = cgccpp::Cgc_sizes<64>::ehdr_size;
      segment_headers_size = segment_count * cgccpp::Cgc_sizes<64>::phdr_size;
    }
  else
    gold_unreachable();
...
Script_sections::create_segments(Layout* layout, uint64_t dot_alignment)
{
  ...
  for (Layout::Section_list::iterator p = sections.begin();
	...
	Layout::section_flags_to_segment((*p)->flags());
      ...
      if (need_new_segment)
	{
	  current_seg = layout->make_output_segment(cgccpp::PT_LOAD,
						    seg_flags);
  ...
  // An CGC program should work even if the program headers are not in
  ...
  Output_segment* load_seg = layout->make_output_segment(cgccpp::PT_LOAD,
							 cgccpp::PF_R);
    ...
    const Layout::Section_list* sections)
{
  ...
  for (Layout::Section_list::const_iterator p = sections->begin();
       ...
       ++p)
    {
      if ((*p)->type() == cgccpp::SHT_NOTE)
	{
	  cgccpp::Cgc_Word seg_flags =
	    Layout::section_flags_to_segment((*p)->flags());
	  Output_segment* oseg = layout->make_output_segment(cgccpp::PT_NOTE,
							     seg_flags);
      ...
      if (((*p)->flags() & cgccpp::SHF_TLS) != 0)
	{
	  ...
	  cgccpp::Cgc_Word seg_flags =
	    Layout::section_flags_to_segment((*p)->flags());
	  Output_segment* oseg = layout->make_output_segment(cgccpp::PT_TLS,
							     seg_flags);
	  oseg->add_output_section_to_nonload(*p, seg_flags);

	  Layout::Section_list::const_iterator pnext = p + 1;
	  while (pnext != sections->end()
		 && ((*pnext)->flags() & cgccpp::SHF_TLS) != 0)
	    {
	      seg_flags = Layout::section_flags_to_segment((*pnext)->flags());
	      oseg->add_output_section_to_nonload(*pnext, seg_flags);
	      p = pnext;
      ...
      if (strcmp((*p)->name(), ".interp") == 0)
	{
	  cgccpp::Cgc_Word seg_flags =
	    Layout::section_flags_to_segment((*p)->flags());
	  Output_segment* oseg = layout->make_output_segment(cgccpp::PT_INTERP,
							     seg_flags);
	  oseg->add_output_section_to_nonload(*p, seg_flags);
	}
    }
...
Script_sections::expected_segment_count(const Layout* layout) const
{
  ...
  for (Layout::Section_list::const_iterator p = sections.begin();
      ...
      if ((*p)->type() == cgccpp::SHT_NOTE)
	{
      ...
      else if (((*p)->flags() & cgccpp::SHF_TLS) != 0)
	{
	   ...
	   ++q)
	{
	  ...
	  if (r == name_to_segment.end())
	  ...
	  else
	    {
		  ...
		  && r->second->type() != cgccpp::PT_LOAD)
		continue;

	      if (r->second->type() != cgccpp::PT_LOAD)
