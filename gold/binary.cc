...
// We implement this by creating an CGC file in memory.
...
Binary_to_cgc::Binary_to_cgc(cgccpp::EM machine, int size, bool big_endian,
			     const std::string& filename)
  : cgc_machine_(machine), size_(size), big_endian_(big_endian),
    filename_(filename), data_(NULL), filesize_(0)
{
}

Binary_to_cgc::~Binary_to_cgc()
{
  if (this->data_ != NULL)
    delete[] this->data_;
}

// Given FILENAME, create a buffer which looks like an CGC file with
...
bool
Binary_to_cgc::convert(const Task* task)
{
  if (this->size_ == 32)
    {
      if (!this->big_endian_)
...
bool
Binary_to_cgc::sized_convert(const Task* task)
{
  // Read the input file.
  ...
  size_t output_size = (cgccpp::Cgc_sizes<size>::ehdr_size
			+ 5 * cgccpp::Cgc_sizes<size>::shdr_size);
  ...
  this->write_section_header<size, big_endian>("", &shstrtab, cgccpp::SHT_NULL,
					       0, 0, 0, 0, 0,
					       0, 0, &pout);
...
template<int size, bool big_endian>
void
Binary_to_cgc::write_file_header(unsigned char** ppout)
{
  cgccpp::Ehdr_write<size, big_endian> oehdr(*ppout);
  ...
  oehdr.put_e_type(cgccpp::ET_REL);
  oehdr.put_e_machine(this->cgc_machine_);
  oehdr.put_e_version(cgccpp::EV_CURRENT);
  oehdr.put_e_entry(0);
  oehdr.put_e_phoff(0);
  oehdr.put_e_shoff(cgccpp::Cgc_sizes<size>::ehdr_size);
  oehdr.put_e_flags(0);
  ...
  *ppout += cgccpp::Cgc_sizes<size>::ehdr_size;
}
...
template<int size, bool big_endian>
void
Binary_to_cgc::write_section_header(
    const char* name,
    ...
    unsigned char** ppout)
{
  cgccpp::Shdr_write<size, big_endian> oshdr(*ppout);
  ...
  *ppout += cgccpp::Cgc_sizes<size>::shdr_size;
}
    ...
    unsigned char** ppout)
{
  ...
  cgccpp::Sym_write<size, big_endian> osym(pout);
  osym.put_st_name(name.empty() ? 0 : strtab->get_offset(name.c_str()));
  osym.put_st_value(value);
  osym.put_st_size(0);
  osym.put_st_info(name.empty() ? cgccpp::STB_LOCAL : cgccpp::STB_GLOBAL,
		   cgccpp::STT_NOTYPE);
  osym.put_st_other(cgccpp::STV_DEFAULT, 0);
  osym.put_st_shndx(shndx);

  *ppout += cgccpp::Cgc_sizes<size>::sym_size;
}
