    ...
    unsigned int shndx) const
{
  ...
  if ((object->section_flags(shndx) & cgccpp::SHF_WRITE) == 0)
    return true;
    ...
    Output_data_reloc<sh_type, true, size, big_endian>* reloc_section)
{
  ...
  typename cgccpp::Cgc_types<size>::Cgc_WXword symsize = sym->symsize();
  ...
  typename cgccpp::Cgc_types<size>::Cgc_WXword addralign;
...
Copy_relocs<sh_type, size, big_endian>::save(
    ...
    const Reloc& rel)
{
  unsigned int reloc_type = cgccpp::cgc_r_type<size>(rel.get_r_info());
...
template
class Copy_relocs<cgccpp::SHT_REL, 32, false>;

template
class Copy_relocs<cgccpp::SHT_RELA, 32, false>;
#endif

#ifdef HAVE_TARGET_32_BIG
template
class Copy_relocs<cgccpp::SHT_REL, 32, true>;

template
class Copy_relocs<cgccpp::SHT_RELA, 32, true>;
#endif

#ifdef HAVE_TARGET_64_LITTLE
template
class Copy_relocs<cgccpp::SHT_REL, 64, false>;

template
class Copy_relocs<cgccpp::SHT_RELA, 64, false>;
#endif

#ifdef HAVE_TARGET_64_BIG
template
class Copy_relocs<cgccpp::SHT_REL, 64, true>;

template
class Copy_relocs<cgccpp::SHT_RELA, 64, true>;
#endif
