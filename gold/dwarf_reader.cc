...
// Class Sized_cgc_reloc_mapper

// Initialize the relocation tracker for section RELOC_SHNDX.

template<int size, bool big_endian>
bool
Sized_cgc_reloc_mapper<size, big_endian>::do_initialize(
...
template<int size, bool big_endian>
unsigned int
Sized_cgc_reloc_mapper<size, big_endian>::symbol_section(
    unsigned int symndx, Address* value, bool* is_ordinary)
{
  const int symsize = cgccpp::Cgc_sizes<size>::sym_size;
  gold_assert(static_cast<off_t>((symndx + 1) * symsize) <= this->symtab_size_);
  cgccpp::Sym<size, big_endian> cgcsym(this->symtab_ + symndx * symsize);
  *value = cgcsym.get_st_value();
  return this->object_->adjust_sym_shndx(symndx, cgcsym.get_st_shndx(),
					 is_ordinary);
}
...
template<int size, bool big_endian>
unsigned int
Sized_cgc_reloc_mapper<size, big_endian>::do_get_reloc_target(
    off_t reloc_offset, off_t* target_offset)
{
  ...
  if (!is_ordinary)
    return 0;
  if (this->reloc_type_ == cgccpp::SHT_RELA)
    value += this->track_relocs_.next_addend();
  *target_offset = value;
  return target_shndx;
}

static inline Cgc_reloc_mapper*
make_cgc_reloc_mapper(Relobj* object, const unsigned char* symtab,
		      off_t symtab_size)
{
  if (object->cgcsize() == 32)
    {
      if (object->is_big_endian())
        {
#ifdef HAVE_TARGET_32_BIG
	  return new Sized_cgc_reloc_mapper<32, true>(object, symtab,
						      symtab_size);
#else
	  gold_unreachable();
#endif
	  ...
	  return new Sized_cgc_reloc_mapper<32, false>(object, symtab,
						       symtab_size);
#else
	  gold_unreachable();
#endif
  ...
  else if (object->cgcsize() == 64)
    {
      if (object->is_big_endian())
        {
#ifdef HAVE_TARGET_64_BIG
	  return new Sized_cgc_reloc_mapper<64, true>(object, symtab,
						      symtab_size);
#else
	  gold_unreachable();
#endif
	  ...
	  return new Sized_cgc_reloc_mapper<64, false>(object, symtab,
						       symtab_size);
#else
	  gold_unreachable();
#endif
...
Dwarf_abbrev_table::do_get_abbrev(unsigned int code)
{
  ...
  for (;;)
	...
	return NULL;
      ...
      if (this->buffer_pos_ >= this->buffer_end_)
	return NULL;
      bool has_children = *this->buffer_pos_ == cgccpp::DW_CHILDREN_yes;
      this->buffer_pos_ += 1;
    ...
    unsigned int ranges_shndx)
{
  ...
  for (unsigned int i = 0; i < object->shnum(); ++i)
    {
      ...
      if ((reloc_type == cgccpp::SHT_REL
	   || reloc_type == cgccpp::SHT_RELA)
  ...
  this->ranges_reloc_mapper_ = make_cgc_reloc_mapper(object, symtab,
						     symtab_size);
...
Dwarf_ranges_table::lookup_reloc(off_t off, off_t* target_off)
{
  ...
  if (shndx == 0)
    return 0;
  if (this->reloc_type_ == cgccpp::SHT_REL)
    *target_off += value;
  else
    *target_off = value;
  return shndx;
                                   ...
                                   off_t symtab_size)
{
  ...
  this->reloc_mapper_ = make_cgc_reloc_mapper(object, symtab, symtab_size);
  ...
  for (unsigned int i = 0; i < nattr; ++i)
    {
      ...
      if (form == cgccpp::DW_FORM_indirect)
        {
      ...
      switch(form)
	{
	  case cgccpp::DW_FORM_flag_present:
	    attr_value.val.intval = 1;
	    break;
	  case cgccpp::DW_FORM_strp:
	    {
	      off_t str_off;
	  ...
	  case cgccpp::DW_FORM_sec_offset:
	    {
	      off_t sec_off;
	      if (this->dwinfo_->offset_size() == 4)
	  ...
	  case cgccpp::DW_FORM_addr:
	  case cgccpp::DW_FORM_ref_addr:
	    {
	      off_t sec_off;
	  ...
	  case cgccpp::DW_FORM_exprloc:
	    ...
	    break;
	  case cgccpp::DW_FORM_data1:
	  case cgccpp::DW_FORM_flag:
	    attr_value.val.intval = *pattr++;
	    break;
	  case cgccpp::DW_FORM_ref1:
	    attr_value.val.refval = *pattr++;
	    ref_form = true;
	    break;
	  case cgccpp::DW_FORM_data2:
	    attr_value.val.intval =
		this->dwinfo_->read_from_pointer<16>(&pattr);
	    break;
	  case cgccpp::DW_FORM_ref2:
	    attr_value.val.refval =
		this->dwinfo_->read_from_pointer<16>(&pattr);
	    ref_form = true;
	    break;
	  case cgccpp::DW_FORM_data4:
	    {
	      off_t sec_off;
	      sec_off = this->dwinfo_->read_from_pointer<32>(&pattr);
	      unsigned int shndx =
	  ...
	  case cgccpp::DW_FORM_ref_udata:
	    attr_value.val.refval = read_unsigned_LEB_128(pattr, &len);
	    ref_form = true;
	    pattr += len;
	    break;
	  case cgccpp::DW_FORM_udata:
	  case cgccpp::DW_FORM_GNU_addr_index:
	  case cgccpp::DW_FORM_GNU_str_index:
	    attr_value.val.uintval = read_unsigned_LEB_128(pattr, &len);
	    pattr += len;
	    break;
	  case cgccpp::DW_FORM_sdata:
	    attr_value.val.intval = read_signed_LEB_128(pattr, &len);
	    pattr += len;
	    break;
	  case cgccpp::DW_FORM_string:
      ...
      switch (attr)
	{
	  case cgccpp::DW_AT_name:
	    if (form == cgccpp::DW_FORM_string)
	      this->name_ = attr_value.val.stringval;
	    else if (form == cgccpp::DW_FORM_strp)
	      {
	    ...
	    break;
	  case cgccpp::DW_AT_linkage_name:
	  case cgccpp::DW_AT_MIPS_linkage_name:
	    if (form == cgccpp::DW_FORM_string)
	    ...
	    break;
	  case cgccpp::DW_AT_specification:
	    if (ref_form)
	      this->specification_ = attr_value.val.refval;
	    break;
	  case cgccpp::DW_AT_abstract_origin:
	    if (ref_form)
	      this->abstract_origin_ = attr_value.val.refval;
	    break;
	  case cgccpp::DW_AT_sibling:
...
{
  const Attribute_value* attr_val = this->attribute(attr);
  if (attr_val == NULL || attr_val->form != cgccpp::DW_FORM_addr)
    return -1;
...
Dwarf_info_reader::do_parse()
{
  ...
  this->reloc_mapper_ = make_cgc_reloc_mapper(this->object_, this->symtab_,
					      this->symtab_size_);
  ...
  while (pinfo < this->buffer_end_)
	...
	break;
      uint32_t unit_length =
          cgccpp::Swap_unaligned<32, big_endian>::readval(pinfo);
      pinfo += 4;
      if (unit_length == 0xffffffff)
	{
	  if (!this->check_buffer(pinfo + 8))
	    break;
	  unit_length = cgccpp::Swap_unaligned<64, big_endian>::readval(pinfo);
	  pinfo += 8;
	  this->offset_size_ = 8;
	}
      else
	...
	break;
      ...
      this->cu_version_ =
	  cgccpp::Swap_unaligned<16, big_endian>::readval(pinfo);
      pinfo += 2;

      // Read debug_abbrev_offset (4 or 8 bytes).
      if (this->offset_size_ == 4)
	abbrev_offset = cgccpp::Swap_unaligned<32, big_endian>::readval(pinfo);
      else
	abbrev_offset = cgccpp::Swap_unaligned<64, big_endian>::readval(pinfo);
      if (this->reloc_shndx_ > 0)
	{
	  ...
	  signature = cgccpp::Swap_unaligned<64, big_endian>::readval(pinfo);
	  pinfo += 8;
	  ...
	  if (this->offset_size_ == 4)
	    ...
	    type_offset =
		cgccpp::Swap_unaligned<64, big_endian>::readval(pinfo);
	  pinfo += this->offset_size_;
	}
...
inline typename cgccpp::Valtype_base<valsize>::Valtype
Dwarf_info_reader::read_from_pointer(const unsigned char* source)
{
  typename cgccpp::Valtype_base<valsize>::Valtype return_value;
  if (this->object_->is_big_endian())
    return_value = cgccpp::Swap_unaligned<valsize, true>::readval(source);
  else
    return_value = cgccpp::Swap_unaligned<valsize, false>::readval(source);
  return return_value;
}
    ...
    current_header_index_(-1)
{
  ...
  for (unsigned int i = 0; i < object->shnum(); ++i)
    {
      ...
      if ((reloc_sh_type == cgccpp::SHT_REL
	   || reloc_sh_type == cgccpp::SHT_RELA)
  ...
  if (reloc_shndx != 0)
    {
      ...
      for (symtab_shndx = 0; symtab_shndx < object->shnum(); ++symtab_shndx)
        if (object->section_type(symtab_shndx) == cgccpp::SHT_SYMTAB)
          {
  ...
  this->reloc_mapper_ =
      new Sized_cgc_reloc_mapper<size, big_endian>(object,
						   this->symtab_buffer_,
...
{
  uint32_t initial_length = cgccpp::Swap_unaligned<32, big_endian>::readval(lineptr);
  lineptr += 4;
  ...
  if (initial_length == 0xffffffff)
    {
      ...
      initial_length = cgccpp::Swap_unaligned<64, big_endian>::readval(lineptr);
      lineptr += 8;
    }
  else
  ...
  header_.version = cgccpp::Swap_unaligned<16, big_endian>::readval(lineptr);
  lineptr += 2;

  if (header_.offset_size == 4)
    header_.prologue_length = cgccpp::Swap_unaligned<32, big_endian>::readval(lineptr);
  else
    header_.prologue_length = cgccpp::Swap_unaligned<64, big_endian>::readval(lineptr);
    ...
    const unsigned char* start, struct LineStateMachine* lsm, size_t* len)
{
  ...
  switch (opcode)
    {
    case cgccpp::DW_LNS_copy:
      lsm->basic_block = false;
      *len = oplen;
      return true;

    case cgccpp::DW_LNS_advance_pc:
      {
    ...
    case cgccpp::DW_LNS_advance_line:
      {
    ...
    case cgccpp::DW_LNS_negate_stmt:
      lsm->is_stmt = !lsm->is_stmt;
      break;

    case cgccpp::DW_LNS_set_basic_block:
      lsm->basic_block = true;
      break;

    case cgccpp::DW_LNS_fixed_advance_pc:
      {
        int advance_address;
        advance_address = cgccpp::Swap_unaligned<16, big_endian>::readval(start);
        oplen += 2;
        lsm->address += advance_address;
      }
      break;

    case cgccpp::DW_LNS_const_add_pc:
      {
    ...
    case cgccpp::DW_LNS_extended_op:
      {
        ...
        switch (extended_op)
          {
          case cgccpp::DW_LNE_end_sequence:
          ...
          case cgccpp::DW_LNE_set_address:
            {
              lsm->address =
		cgccpp::Swap_unaligned<size, big_endian>::readval(start);
		  ...
		  // symbol value.
		  if (this->track_relocs_type_ == cgccpp::SHT_RELA)
		    lsm->address = 0;
          ...
          case cgccpp::DW_LNE_define_file:
            {
