...
// minimally compatible.  In practice for an CGC target this would be
// the same target as the input files; that name always start with
// "cgc".  Non-CGC targets would be "srec", "symbolsrec", "tekhex",
// "binary", "ihex".

General_options::Object_format
General_options::string_to_object_format(const char* arg)
{
  if (strncmp(arg, "cgc", 3) == 0 || strcmp(arg, "default") == 0)
    return gold::General_options::OBJECT_FORMAT_CGC;
  else if (strcmp(arg, "binary") == 0)
    return gold::General_options::OBJECT_FORMAT_BINARY;
  else
    {
      gold::gold_error(_("format '%s' not supported; treating as cgc "
			 "(supported formats: cgc, binary)"),
		       arg);
      return gold::General_options::OBJECT_FORMAT_CGC;
    }
}
...
#define NATIVE_LINKER 0
  ...
  if (this->oformat_enum() != General_options::OBJECT_FORMAT_CGC
      && (this->shared()
	  || this->pie()
	  || this->relocatable()))
