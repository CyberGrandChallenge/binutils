...
// readcgc. Because of that, the use of gold's data structures and functions
			...
			Sized_incremental_binary<size, big_endian>* inc)
{
  ...
  cgccpp::Cgc_file<size, big_endian, Incremental_binary> cgc_file(inc);

  unsigned int symtab_shndx = cgc_file.find_section_by_type(cgccpp::SHT_SYMTAB);
  if (symtab_shndx == cgccpp::SHN_UNDEF)  // Not found.
    {
      fprintf(stderr, "%s: %s: no symbol table section\n", argv0, filename);
      exit(1);
    }
  Location symtab_location(cgc_file.section_contents(symtab_shndx));
  View symtab_view(inc->view(symtab_location));

  // Get a view of the .strtab section.

  unsigned int strtab_shndx = cgc_file.section_link(symtab_shndx);
  if (strtab_shndx == cgccpp::SHN_UNDEF
      || strtab_shndx > cgc_file.shnum()
      || cgc_file.section_type(strtab_shndx) != cgccpp::SHT_STRTAB)
    {
      fprintf(stderr, "%s: %s: no string table section\n", argv0, filename);
      exit(1);
    }
  Location strtab_location(cgc_file.section_contents(strtab_shndx));
  ...
  unsigned int sym_size = cgccpp::Cgc_sizes<size>::sym_size;
	  ...
	  for (unsigned int symndx = 0; symndx < nsyms; ++symndx)
	    {
	      ...
	      cgccpp::Sym<size, big_endian> sym(sym_p);
	      const char* symname;
