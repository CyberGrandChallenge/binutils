  ...
  if (layout->incremental_base() == NULL)
    {
      ...
      if (this->options_.oformat_enum() != General_options::OBJECT_FORMAT_CGC)
	of->set_is_temporary();
      of->open(file_size);
    }
  else
...
Layout::include_section(Sized_relobj_file<size, big_endian>*, const char* name,
			const cgccpp::Shdr<size, big_endian>& shdr)
{
  if (!parameters->options().relocatable()
      && (shdr.get_sh_flags() & cgccpp::SHF_EXCLUDE))
    return false;

  switch (shdr.get_sh_type())
    {
    case cgccpp::SHT_NULL:
    case cgccpp::SHT_SYMTAB:
    case cgccpp::SHT_DYNSYM:
    case cgccpp::SHT_HASH:
    case cgccpp::SHT_DYNAMIC:
    case cgccpp::SHT_SYMTAB_SHNDX:
      return false;

    case cgccpp::SHT_STRTAB:
      // Discard the sections which have special meanings in the CGC
    ...
    case cgccpp::SHT_RELA:
    case cgccpp::SHT_REL:
    case cgccpp::SHT_GROUP:
      // If we are emitting relocations these should be handled
      // elsewhere.
      gold_assert(!parameters->options().relocatable());
      return false;

    case cgccpp::SHT_PROGBITS:
...
Output_segment*
Layout::find_output_segment(cgccpp::PT type, cgccpp::Cgc_Word set,
			    cgccpp::Cgc_Word clear) const
{
  for (Segment_list::const_iterator p = this->segment_list_.begin();
       p != this->segment_list_.end();
       ++p)
    if (static_cast<cgccpp::PT>((*p)->type()) == type
	&& ((*p)->flags() & set) == set
	&& ((*p)->flags() & clear) == 0)
      return *p;
  return NULL;
...
Layout::get_output_section(const char* name, Stringpool::Key name_key,
			   cgccpp::Cgc_Word type, cgccpp::Cgc_Xword flags,
			   Output_section_order order, bool is_relro)
{
  cgccpp::Cgc_Word lookup_type = type;
  ...
  if (lookup_type == cgccpp::SHT_INIT_ARRAY
      || lookup_type == cgccpp::SHT_FINI_ARRAY
      || lookup_type == cgccpp::SHT_PREINIT_ARRAY)
    lookup_type = cgccpp::SHT_PROGBITS;

  cgccpp::Cgc_Xword lookup_flags = flags;

  // Ignoring SHF_WRITE and SHF_EXECINSTR here means that we combine
  // read-write with read-only sections.  Some other CGC linkers do
  // not do this.  FIXME: Perhaps there should be an option
  // controlling this.
  lookup_flags &= ~(cgccpp::SHF_WRITE | cgccpp::SHF_EXECINSTR);
  ...
  if (!ins.second)
  ...
  else
    {
      ...
      if (lookup_type == cgccpp::SHT_PROGBITS)
	{
	  if (flags == 0)
	    {
	      Output_section* same_name = this->find_output_section(name);
	      if (same_name != NULL
		  && (same_name->type() == cgccpp::SHT_PROGBITS
		      || same_name->type() == cgccpp::SHT_INIT_ARRAY
		      || same_name->type() == cgccpp::SHT_FINI_ARRAY
		      || same_name->type() == cgccpp::SHT_PREINIT_ARRAY)
		  && (same_name->flags() & cgccpp::SHF_TLS) == 0)
		os = same_name;
	    }
	  else if ((flags & cgccpp::SHF_TLS) == 0)
	    {
	      cgccpp::Cgc_Xword zero_flags = 0;
...
cgccpp::Cgc_Xword
Layout::get_output_section_flags(cgccpp::Cgc_Xword input_section_flags)
{
  // Some flags in the input section should not be automatically
  // copied to the output section.
  input_section_flags &= ~ (cgccpp::SHF_INFO_LINK
			    | cgccpp::SHF_GROUP
			    | cgccpp::SHF_MERGE
			    | cgccpp::SHF_STRINGS);
  ...
  if (!parameters->options().relocatable())
    input_section_flags &= ~cgccpp::SHF_LINK_ORDER;
	  ...
	  if (script_section_type == Script_sections::ST_NOLOAD)
	    {
	      ...
	      if ((os->flags() & cgccpp::SHF_ALLOC) == 0
		  && os->is_address_valid())
		{
...
Layout::init_fixed_output_section(const char* name,
				  cgccpp::Shdr<size, big_endian>& shdr)
{
  ...
  // it from scratch.
  if (parameters->options().gdb_index()
      && sh_type == cgccpp::SHT_PROGBITS
      && strcmp(name, ".gdb_index") == 0)
    return NULL;

  typename cgccpp::Cgc_types<size>::Cgc_Addr sh_addr = shdr.get_sh_addr();
  typename cgccpp::Cgc_types<size>::Cgc_Off sh_offset = shdr.get_sh_offset();
  typename cgccpp::Cgc_types<size>::Cgc_WXword sh_size = shdr.get_sh_size();
  typename cgccpp::Cgc_types<size>::Cgc_WXword sh_flags = shdr.get_sh_flags();
  typename cgccpp::Cgc_types<size>::Cgc_WXword sh_addralign =
...
Layout::layout(Sized_relobj_file<size, big_endian>* object, unsigned int shndx,
	       const char* name, const cgccpp::Shdr<size, big_endian>& shdr,
	       unsigned int reloc_shndx, unsigned int, off_t* off)
{
  *off = 0;
  ...
  Output_section* os;
  if (parameters->options().relocatable()
      && (shdr.get_sh_flags() & cgccpp::SHF_GROUP) != 0)
    {
  ...
  cgccpp::Cgc_Xword orig_flags = os->flags();

  *off = os->add_input_section(this, object, shndx, name, shdr, reloc_shndx,
			       this->script_options_->saw_sections_clause());

  // If the flags changed, we may have to change the order.
  if ((orig_flags & cgccpp::SHF_ALLOC) != 0)
    {
      orig_flags &= (cgccpp::SHF_WRITE | cgccpp::SHF_EXECINSTR);
      cgccpp::Cgc_Xword new_flags =
	os->flags() & (cgccpp::SHF_WRITE | cgccpp::SHF_EXECINSTR);
		     ...
		     Relocatable_relocs* rr)
{
  ...
  Output_section* os;
  if (!parameters->options().relocatable()
      || (data_section->flags() & cgccpp::SHF_GROUP) == 0)
  ...
  if (sh_type == cgccpp::SHT_REL)
    {
      os->set_entsize(cgccpp::Cgc_sizes<size>::rel_size);
      posd = new Output_relocatable_relocs<cgccpp::SHT_REL,
					   size,
					   big_endian>(rr);
    }
  else if (sh_type == cgccpp::SHT_RELA)
    {
      os->set_entsize(cgccpp::Cgc_sizes<size>::rela_size);
      posd = new Output_relocatable_relocs<cgccpp::SHT_RELA,
					   size,
					   big_endian>(rr);
    }
  else
...
{
  gold_assert(parameters->options().relocatable());
  gold_assert(shdr.get_sh_type() == cgccpp::SHT_GROUP);
...
Layout::layout_eh_frame(Sized_relobj_file<size, big_endian>* object,
			...
			off_t* off)
{
  gold_assert(shdr.get_sh_type() == cgccpp::SHT_PROGBITS
	      || shdr.get_sh_type() == cgccpp::SHT_X86_64_UNWIND);
  gold_assert((shdr.get_sh_flags() & cgccpp::SHF_ALLOC) != 0);
  ...
  if (!parameters->incremental()
							 ...
							 reloc_type))
    {
      ...
      if ((orig_flags & (cgccpp::SHF_WRITE | cgccpp::SHF_EXECINSTR))
	  != (os->flags() & (cgccpp::SHF_WRITE | cgccpp::SHF_EXECINSTR)))
	{
	  os->set_is_relro();
      ...
      if (parameters->options().eh_frame_hdr() && !parameters->incremental())
	{
	  ...
	  if (hdr_os != NULL)
	      ...
	      if (!this->script_options_->saw_phdrs_clause())
		{
		  ...
		  hdr_oseg = this->make_output_segment(cgccpp::PT_GNU_EH_FRAME,
						       cgccpp::PF_R);
...
Output_section*
Layout::add_output_section_data(const char* name, cgccpp::Cgc_Word type,
				cgccpp::Cgc_Xword flags,
				Output_section_data* posd,
				Output_section_order order, bool is_relro)
{
...
cgccpp::Cgc_Word
Layout::section_flags_to_segment(cgccpp::Cgc_Xword flags)
{
  cgccpp::Cgc_Word ret = cgccpp::PF_R;
  if ((flags & cgccpp::SHF_WRITE) != 0)
    ret |= cgccpp::PF_W;
  if ((flags & cgccpp::SHF_EXECINSTR) != 0)
    ret |= cgccpp::PF_X;
  return ret;
}
...
Output_section*
Layout::make_output_section(const char* name, cgccpp::Cgc_Word type,
			    cgccpp::Cgc_Xword flags,
			    Output_section_order order, bool is_relro)
{
  Output_section* os;
  if ((flags & cgccpp::SHF_ALLOC) == 0
    ...
    os = new Output_compressed_section(&parameters->options(), name, type,
				       flags);
  else if ((flags & cgccpp::SHF_ALLOC) == 0
  ...
  else
    {
      ...
      if (type == cgccpp::SHT_PROGBITS)
	{
	  if (is_prefix_of(".init_array", name))
	    type = cgccpp::SHT_INIT_ARRAY;
	  else if (is_prefix_of(".preinit_array", name))
	    type = cgccpp::SHT_PREINIT_ARRAY;
	  else if (is_prefix_of(".fini_array", name))
	    type = cgccpp::SHT_FINI_ARRAY;
	}
  ...
  if (!this->script_options_->saw_sections_clause()
      ...
      && (flags & cgccpp::SHF_ALLOC) != 0
      && (flags & cgccpp::SHF_WRITE) != 0)
    {
      if (type == cgccpp::SHT_PROGBITS)
	{
	  if ((flags & cgccpp::SHF_TLS) != 0)
	    is_relro = true;
      ...
      else if (type == cgccpp::SHT_INIT_ARRAY
	       || type == cgccpp::SHT_FINI_ARRAY
	       || type == cgccpp::SHT_PREINIT_ARRAY)
	is_relro = true;
    }
  ...
  if (order == ORDER_INVALID && (flags & cgccpp::SHF_ALLOC) != 0)
  ...
  // arbitrarily padded.
  if ((type == cgccpp::SHT_PROGBITS || type == cgccpp::SHT_NOBITS)
      && order != ORDER_INTERP
      && order != ORDER_INIT
      && order != ORDER_PLT
      && order != ORDER_FINI
...
// ld/scripttempl/cgc.sc in the GNU linker.  Note that the order of a
...
{
  gold_assert((os->flags() & cgccpp::SHF_ALLOC) != 0);
  bool is_write = (os->flags() & cgccpp::SHF_WRITE) != 0;
  bool is_execinstr = (os->flags() & cgccpp::SHF_EXECINSTR) != 0;
  bool is_bss = false;
  ...
  switch (os->type())
    ...
    case cgccpp::SHT_HASH:
    case cgccpp::SHT_DYNAMIC:
    case cgccpp::SHT_SHLIB:
    case cgccpp::SHT_DYNSYM:
    case cgccpp::SHT_GNU_HASH:
    case cgccpp::SHT_GNU_verdef:
    case cgccpp::SHT_GNU_verneed:
    case cgccpp::SHT_GNU_versym:
      if (!is_write)
	return ORDER_DYNAMIC_LINKER;
      break;
    case cgccpp::SHT_NOTE:
      return is_write ? ORDER_RW_NOTE : ORDER_RO_NOTE;
    }

  if ((os->flags() & cgccpp::SHF_TLS) != 0)
...
Layout::attach_allocated_section_to_segment(const Target* target,
					    Output_section* os)
{
  cgccpp::Cgc_Xword flags = os->flags();
  gold_assert((flags & cgccpp::SHF_ALLOC) != 0);
  ...
  cgccpp::Cgc_Word seg_flags = Layout::section_flags_to_segment(flags);
  ...
  if (!os->is_unique_segment())
    {
      for (p = this->segment_list_.begin();
	   ...
	   ++p)
	{
	  if ((*p)->type() != cgccpp::PT_LOAD)
	    continue;
	  if ((*p)->is_unique_segment())
	    continue;
	  if (!parameters->options().omagic()
	      && ((*p)->flags() & cgccpp::PF_W) != (seg_flags & cgccpp::PF_W))
	    continue;
	  if ((target->isolate_execinstr() || parameters->options().rosegment())
	      && ((*p)->flags() & cgccpp::PF_X) != (seg_flags & cgccpp::PF_X))
	    continue;
	  // If -Tbss was specified, we need to separate the data and BSS
	  // segments.
	  if (parameters->options().user_set_Tbss())
	    {
	      if ((os->type() == cgccpp::SHT_NOBITS)
  ...
  if (p == this->segment_list_.end()
    ...
    {
      Output_segment* oseg = this->make_output_segment(cgccpp::PT_LOAD,
						       seg_flags);
  ...
  if (os->type() == cgccpp::SHT_NOTE)
    {
      ...
      for (p = this->segment_list_.begin();
	   ...
	   ++p)
	{
	  if ((*p)->type() == cgccpp::PT_NOTE
	      && (((*p)->flags() & cgccpp::PF_W)
		  == (seg_flags & cgccpp::PF_W)))
	    {
      ...
      if (p == this->segment_list_.end())
	{
	  Output_segment* oseg = this->make_output_segment(cgccpp::PT_NOTE,
							   seg_flags);
  ...
  if ((flags & cgccpp::SHF_TLS) != 0)
	...
	this->make_output_segment(cgccpp::PT_TLS, seg_flags);
  ...
  if (os->is_relro() && parameters->options().relro())
    {
      gold_assert(seg_flags == (cgccpp::PF_R | cgccpp::PF_W));
      if (this->relro_segment_ == NULL)
	this->make_output_segment(cgccpp::PT_GNU_RELRO, seg_flags);
  ...
  if (strcmp(os->name(), ".interp") == 0
	...
	this->make_output_segment(cgccpp::PT_INTERP, seg_flags);
      else
...
{
  name = this->namepool_.add(name, false, NULL);
  cgccpp::Cgc_Xword sh_flags = cgccpp::SHF_ALLOC;
  if (section_type == Script_sections::ST_NOLOAD)
    sh_flags = 0;
  Output_section* os = this->make_output_section(name, cgccpp::SHT_PROGBITS,
			 ...
			 const Object* obj)
{
  if (!seen_gnu_stack)
  ...
  else
    {
      ...
      if ((gnu_stack_flags & cgccpp::SHF_EXECINSTR) != 0)
	{
...
Layout::create_initial_dynamic_sections(Symbol_table* symtab)
{
  ...
  if (this->dynamic_section_ != NULL)
	...
	symtab->define_in_output_data("_DYNAMIC", NULL,
				      ...
				      cgccpp::STT_OBJECT, cgccpp::STB_LOCAL,
...
Layout::find_first_load_seg(const Target* target)
{
  ...
  for (Segment_list::const_iterator p = this->segment_list_.begin();
    ...
    {
      if ((*p)->type() == cgccpp::PT_LOAD
	  && ((*p)->flags() & cgccpp::PF_R) != 0
	  && (parameters->options().omagic()
	      || ((*p)->flags() & cgccpp::PF_W) == 0)
	  && (!target->isolate_execinstr()
	      || ((*p)->flags() & cgccpp::PF_X) == 0))
	{
  ...
  Output_segment* load_seg = this->make_output_segment(cgccpp::PT_LOAD,
						       cgccpp::PF_R);
  return load_seg;
}
  ...
  while (list_iter != this->segment_list_.end())
      ...
      if (states_iter != segment_states->end())
	{
	  ...
	  *segment = *copy;
	  ...
	  if (segment->type() == cgccpp::PT_TLS)
	    this->tls_segment_ = segment;
	  else if (segment->type() == cgccpp::PT_GNU_RELRO)
    ...
    unsigned int* pshndx)
{
  ...
  if (parameters->options().oformat_enum()
      != General_options::OBJECT_FORMAT_CGC)
    load_seg = NULL;
...
// 9) Finalize the CGC file header.
  ...
  if (!parameters->options().relocatable() && !parameters->doing_static_link())
    {
      ...
      // headers.
      if (!this->script_options_->saw_phdrs_clause())
	phdr_seg = this->make_output_segment(cgccpp::PT_PHDR, cgccpp::PF_R);
...
// Create a note header following the format defined in the CGC ABI.
		    ...
		    bool allocate, size_t* trailing_padding)
{
  ...
  //    http://www.netbsd.org/docs/kernel/cgc-notes.html#note-format
  // GNU ld and GNU readcgc both support the latter (at least as of
  ...
  if (size == 32)
    {
      if (!is_big_endian)
	{
	  cgccpp::Swap<32, false>::writeval(buffer, namesz);
	  cgccpp::Swap<32, false>::writeval(buffer + 4, descsz);
	  cgccpp::Swap<32, false>::writeval(buffer + 8, note_type);
	}
      else
	{
	  cgccpp::Swap<32, true>::writeval(buffer, namesz);
	  cgccpp::Swap<32, true>::writeval(buffer + 4, descsz);
	  cgccpp::Swap<32, true>::writeval(buffer + 8, note_type);
	}
    }
  else if (size == 64)
    {
      if (!is_big_endian)
	{
	  cgccpp::Swap<64, false>::writeval(buffer, namesz);
	  cgccpp::Swap<64, false>::writeval(buffer + 8, descsz);
	  cgccpp::Swap<64, false>::writeval(buffer + 16, note_type);
	}
      else
	{
	  cgccpp::Swap<64, true>::writeval(buffer, namesz);
	  cgccpp::Swap<64, true>::writeval(buffer + 8, descsz);
	  cgccpp::Swap<64, true>::writeval(buffer + 16, note_type);
	}
    }
  else
    gold_unreachable();
...
Layout::create_gold_note()
{
  ...
  Output_section* os = this->create_note("GNU", cgccpp::NT_GNU_GOLD_VERSION,
...
Layout::create_executable_stack_info()
{
  ...
  if (parameters->options().relocatable())
	...
	flags |= cgccpp::SHF_EXECINSTR;
      this->make_output_section(name, cgccpp::SHT_PROGBITS, flags,
				ORDER_INVALID, false);
    }
  else
    {
      if (this->script_options_->saw_phdrs_clause())
	return;
      int flags = cgccpp::PF_R | cgccpp::PF_W;
      if (is_stack_executable)
	flags |= cgccpp::PF_X;
      this->make_output_segment(cgccpp::PT_GNU_STACK, flags);
    }
}
...
Layout::create_build_id()
{
  ...
  // Create the note.
  size_t trailing_padding;
  Output_section* os = this->create_note("GNU", cgccpp::NT_GNU_BUILD_ID,
...
Layout::create_incremental_info_sections(Symbol_table* symtab)
{
  ...
  Output_section* incremental_inputs_os =
    this->make_output_section(incremental_inputs_name,
			      cgccpp::SHT_GNU_INCREMENTAL_INPUTS, 0,
			      ORDER_INVALID, false);
  ...
  Output_section* incremental_symtab_os =
    this->make_output_section(incremental_symtab_name,
			      cgccpp::SHT_GNU_INCREMENTAL_SYMTAB, 0,
			      ORDER_INVALID, false);
  ...
  Output_section* incremental_relocs_os =
    this->make_output_section(incremental_relocs_name,
			      cgccpp::SHT_GNU_INCREMENTAL_RELOCS, 0,
			      ORDER_INVALID, false);
  ...
  Output_section* incremental_got_plt_os =
    this->make_output_section(incremental_got_plt_name,
			      cgccpp::SHT_GNU_INCREMENTAL_GOT_PLT, 0,
			      ORDER_INVALID, false);
...
{
  cgccpp::Cgc_Word type1 = seg1->type();
  cgccpp::Cgc_Word type2 = seg2->type();

  // The single PT_PHDR segment is required to precede any loadable
  // segment.  We simply make it always first.
  if (type1 == cgccpp::PT_PHDR)
    {
      gold_assert(type2 != cgccpp::PT_PHDR);
      return true;
    }
  if (type2 == cgccpp::PT_PHDR)
    return false;

  // The single PT_INTERP segment is required to precede any loadable
  // segment.  We simply make it always second.
  if (type1 == cgccpp::PT_INTERP)
    {
      gold_assert(type2 != cgccpp::PT_INTERP);
      return true;
    }
  if (type2 == cgccpp::PT_INTERP)
    return false;

  // We then put PT_LOAD segments before any other segments.
  if (type1 == cgccpp::PT_LOAD && type2 != cgccpp::PT_LOAD)
    return true;
  if (type2 == cgccpp::PT_LOAD && type1 != cgccpp::PT_LOAD)
    return false;
  ...
  // correctly).
  if (type1 == cgccpp::PT_TLS
      && type2 != cgccpp::PT_TLS
      && type2 != cgccpp::PT_GNU_RELRO)
    return false;
  if (type2 == cgccpp::PT_TLS
      && type1 != cgccpp::PT_TLS
      && type1 != cgccpp::PT_GNU_RELRO)
    return true;

  // We put the PT_GNU_RELRO segment last, because that is where the
  // dynamic linker expects to find it (as with PT_TLS, this is just
  // for efficiency).
  if (type1 == cgccpp::PT_GNU_RELRO && type2 != cgccpp::PT_GNU_RELRO)
    return false;
  if (type2 == cgccpp::PT_GNU_RELRO && type1 != cgccpp::PT_GNU_RELRO)
    return true;

  const cgccpp::Cgc_Word flags1 = seg1->flags();
  const cgccpp::Cgc_Word flags2 = seg2->flags();

  // The order of non-PT_LOAD segments is unimportant.  We simply sort
  // by the numeric segment type and flags values.  There should not
  // be more than one segment with the same type and flags, except
  // when a linker script specifies such.
  if (type1 != cgccpp::PT_LOAD)
    {
      if (type1 != type2)
	return type1 < type2;
  ...
  // address.
  if ((flags1 & cgccpp::PF_W) != (flags2 & cgccpp::PF_W))
    return (flags1 & cgccpp::PF_W) == 0;
  if ((flags1 & cgccpp::PF_W) != 0
      && seg1->has_any_data_sections() != seg2->has_any_data_sections())
    return seg1->has_any_data_sections();
  if ((flags1 & cgccpp::PF_X) != (flags2 & cgccpp::PF_X))
    return (flags1 & cgccpp::PF_X) != 0;
  if ((flags1 & cgccpp::PF_R) != (flags2 & cgccpp::PF_R))
    return (flags1 & cgccpp::PF_R) == 0;
...
{
  cgccpp::Cgc_Xword flags = seg->flags();
  if ((flags & cgccpp::PF_W) != 0)
    return false;
  if ((flags & cgccpp::PF_X) == 0)
			    ...
			    unsigned int* pshndx)
{
	  ...
	  if (are_addresses_set)
		       ...
		       || is_text_segment(target, *p)))
	    {
	      are_addresses_set = true;
		   ...
		   && ((*p)->flags() & (cgccpp::PF_W | cgccpp::PF_X)) == 0)
	    {
  ...
  for (Segment_list::iterator p = this->segment_list_.begin();
    ...
    {
      if ((*p)->type() != cgccpp::PT_LOAD)
	(*p)->set_offset((*p)->type() == cgccpp::PT_GNU_RELRO
			 ? increase_relro
			 : 0);
    }
					...
					unsigned int* pshndx)
{
  ...
  for (Section_list::iterator p = this->section_list_.begin();
       ...
       ++p)
    {
      ...
      // have to come first.
      if (((*p)->flags() & cgccpp::SHF_ALLOC) == 0
	  && (*p)->type() != cgccpp::SHT_GROUP)
	continue;
			       ...
			       off_t* poff)
{
  ...
  if (parameters->target().get_size() == 32)
    {
      symsize = cgccpp::Cgc_sizes<32>::sym_size;
      align = 4;
    }
  else if (parameters->target().get_size() == 64)
    {
      symsize = cgccpp::Cgc_sizes<64>::sym_size;
      align = 8;
    }
  else
    gold_unreachable();
...
Layout::create_shstrtab()
{
  ...
  Output_section* os = this->make_output_section(name, cgccpp::SHT_STRTAB, 0,
						 ORDER_INVALID, false);
			      ...
			      Versions* pversions)
{
  ...
  *plocal_dynamic_count = local_symcount;
  ...
  if (odyn != NULL)
    {
      odyn->add_section_address(cgccpp::DT_SYMTAB, dynsym);
      odyn->add_constant(cgccpp::DT_SYMENT, symsize);
    }
  ...
  if (this->allocated_output_section_count() >= cgccpp::SHN_LORESERVE)
    {
      Output_section* dynsym_xindex =
	this->choose_output_section(NULL, ".dynsym_shndx",
				    cgccpp::SHT_SYMTAB_SHNDX,
				    cgccpp::SHF_ALLOC,
  ...
  *pdynstr = dynstr;
  if (dynstr != NULL)
	...
	this->dynamic_section_->set_link_section(dynstr);
      ...
      if (odyn != NULL)
	{
	  odyn->add_section_address(cgccpp::DT_STRTAB, dynstr);
	  odyn->add_section_size(cgccpp::DT_STRSZ, dynstr);
	}
    }
  ...
  if (strcmp(parameters->options().hash_style(), "sysv") == 0
      || strcmp(parameters->options().hash_style(), "both") == 0)
    {
      ...
      Dynobj::create_cgc_hash_table(*pdynamic_symbols, local_symcount,
				    &phash, &hashlen);

      Output_section* hashsec =
	this->choose_output_section(NULL, ".hash", cgccpp::SHT_HASH,
				    cgccpp::SHF_ALLOC, false,
      ...
      if (hashsec != NULL)
	{
      ...
      if (odyn != NULL)
	odyn->add_section_address(cgccpp::DT_HASH, hashsec);
    }
  ...
  if (strcmp(parameters->options().hash_style(), "gnu") == 0
      || strcmp(parameters->options().hash_style(), "both") == 0)
    {
      ...
      Output_section* hashsec =
	this->choose_output_section(NULL, ".gnu.hash", cgccpp::SHT_GNU_HASH,
				    cgccpp::SHF_ALLOC, false,
				    ORDER_DYNAMIC_LINKER, false);
      ...
      if (hashsec != NULL)
	{
	  ...
	  if (odyn != NULL)
	    odyn->add_section_address(cgccpp::DT_GNU_HASH, hashsec);
	}
    }
}
    ...
    const Output_section* dynstr)
{
  ...
  if (odyn != NULL && vsec != NULL)
    odyn->add_section_address(cgccpp::DT_VERSYM, vsec);

  if (versions->any_defs())
    {
      Output_section* vdsec;
      vdsec = this->choose_output_section(NULL, ".gnu.version_d",
					  cgccpp::SHT_GNU_verdef,
					  cgccpp::SHF_ALLOC,
					  false, ORDER_DYNAMIC_LINKER, false);
      ...
      if (vdsec != NULL)
	{
	  ...
	  if (odyn != NULL)
	    {
	      odyn->add_section_address(cgccpp::DT_VERDEF, vdsec);
	      odyn->add_constant(cgccpp::DT_VERDEFNUM, vdentries);
	    }
	}
    }
  ...
  if (versions->any_needs())
      ...
      if (vnsec != NULL)
	{
	  ...
	  if (odyn != NULL)
	    {
	      odyn->add_section_address(cgccpp::DT_VERNEED, vnsec);
	      odyn->add_constant(cgccpp::DT_VERNEEDNUM, vnentries);
	    }
	}
    }
}
				...
				bool add_debug, bool dynrel_includes_plt)
{
  ...
  if (plt_got != NULL && plt_got->output_section() != NULL)
    odyn->add_section_address(cgccpp::DT_PLTGOT, plt_got);

  if (plt_rel != NULL && plt_rel->output_section() != NULL)
    {
      odyn->add_section_size(cgccpp::DT_PLTRELSZ, plt_rel->output_section());
      odyn->add_section_address(cgccpp::DT_JMPREL, plt_rel->output_section());
      odyn->add_constant(cgccpp::DT_PLTREL,
			 use_rel ? cgccpp::DT_REL : cgccpp::DT_RELA);
    }
  ...
  if ((dyn_rel != NULL && dyn_rel->output_section() != NULL)
	  ...
	  && plt_rel->output_section() != NULL))
    {
      ...
      odyn->add_section_address(use_rel ? cgccpp::DT_REL : cgccpp::DT_RELA,
				(have_dyn_rel
				 ? dyn_rel->output_section()
				 : plt_rel->output_section()));
      cgccpp::DT size_tag = use_rel ? cgccpp::DT_RELSZ : cgccpp::DT_RELASZ;
	...
	odyn->add_section_size(size_tag, plt_rel->output_section());
      ...
      if (use_rel)
	{
	  rel_tag = cgccpp::DT_RELENT;
	  if (size == 32)
	    rel_size = Reloc_types<cgccpp::SHT_REL, 32, false>::reloc_size;
	  else if (size == 64)
	    rel_size = Reloc_types<cgccpp::SHT_REL, 64, false>::reloc_size;
	  else
	    gold_unreachable();
	}
      else
	{
	  rel_tag = cgccpp::DT_RELAENT;
	  if (size == 32)
	    rel_size = Reloc_types<cgccpp::SHT_RELA, 32, false>::reloc_size;
	  else if (size == 64)
	    rel_size = Reloc_types<cgccpp::SHT_RELA, 64, false>::reloc_size;
	  else
	    gold_unreachable();
	}
  ...
  if (add_debug && !parameters->options().shared())
    {
      ...
      odyn->add_constant(cgccpp::DT_DEBUG, 0);
    }
}
			       ...
			       const Symbol_table* symtab)
{
  if (!this->script_options_->saw_phdrs_clause()
    ...
    {
      Output_segment* oseg = this->make_output_segment(cgccpp::PT_DYNAMIC,
						       (cgccpp::PF_R
							| cgccpp::PF_W));
      oseg->add_output_section_to_nonload(this->dynamic_section_,
					  cgccpp::PF_R | cgccpp::PF_W);
    }
  ...
  for (Input_objects::Dynobj_iterator p = input_objects->dynobj_begin();
      ...
      if (!(*p)->is_needed() && (*p)->as_needed())
	{
      ...
      odyn->add_string(cgccpp::DT_NEEDED, (*p)->soname());
    }
  ...
  if (parameters->options().shared())
    {
      ...
      if (soname != NULL)
	odyn->add_string(cgccpp::DT_SONAME, soname);
    }

  Symbol* sym = symtab->lookup(parameters->options().init());
  if (sym != NULL && sym->is_defined() && !sym->is_from_dynobj())
    odyn->add_symbol(cgccpp::DT_INIT, sym);

  sym = symtab->lookup(parameters->options().fini());
  if (sym != NULL && sym->is_defined() && !sym->is_from_dynobj())
    odyn->add_symbol(cgccpp::DT_FINI, sym);
  ...
  for(Layout::Section_list::const_iterator p = this->section_list_.begin();
    ...
    switch((*p)->type())
      {
      case cgccpp::SHT_FINI_ARRAY:
	odyn->add_section_address(cgccpp::DT_FINI_ARRAY, *p);
	odyn->add_section_size(cgccpp::DT_FINI_ARRAYSZ, *p);
	break;
      case cgccpp::SHT_INIT_ARRAY:
	odyn->add_section_address(cgccpp::DT_INIT_ARRAY, *p);
	odyn->add_section_size(cgccpp::DT_INIT_ARRAYSZ, *p);
	break;
      case cgccpp::SHT_PREINIT_ARRAY:
	odyn->add_section_address(cgccpp::DT_PREINIT_ARRAY, *p);
	odyn->add_section_size(cgccpp::DT_PREINIT_ARRAYSZ, *p);
	break;
      default:
	break;
      }
  ...
  if (!rpath.empty())
	...
	odyn->add_string(cgccpp::DT_RPATH, rpath_val);
      else
	odyn->add_string(cgccpp::DT_RUNPATH, rpath_val);
    }
  ...
  if (parameters->options().filter() != NULL)
    odyn->add_string(cgccpp::DT_FILTER, parameters->options().filter());
  if (parameters->options().any_auxiliary())
    {
      for (options::String_set::const_iterator p =
	   ...
	   ++p)
	odyn->add_string(cgccpp::DT_AUXILIARY, *p);
    }
  ...
  if (have_textrel)
    {
      ...
      odyn->add_constant(cgccpp::DT_TEXTREL, 0);
      flags |= cgccpp::DF_TEXTREL;
  ...
  if (parameters->options().shared() && this->has_static_tls())
    flags |= cgccpp::DF_STATIC_TLS;
  if (parameters->options().origin())
    flags |= cgccpp::DF_ORIGIN;
  if (parameters->options().Bsymbolic())
    {
      flags |= cgccpp::DF_SYMBOLIC;
      // Add DT_SYMBOLIC for compatibility with older loaders.
      odyn->add_constant(cgccpp::DT_SYMBOLIC, 0);
    }
  if (parameters->options().now())
    flags |= cgccpp::DF_BIND_NOW;
  if (flags != 0)
    odyn->add_constant(cgccpp::DT_FLAGS, flags);

  flags = 0;
  if (parameters->options().initfirst())
    flags |= cgccpp::DF_1_INITFIRST;
  if (parameters->options().interpose())
    flags |= cgccpp::DF_1_INTERPOSE;
  if (parameters->options().loadfltr())
    flags |= cgccpp::DF_1_LOADFLTR;
  if (parameters->options().nodefaultlib())
    flags |= cgccpp::DF_1_NODEFLIB;
  if (parameters->options().nodelete())
    flags |= cgccpp::DF_1_NODELETE;
  if (parameters->options().nodlopen())
    flags |= cgccpp::DF_1_NOOPEN;
  if (parameters->options().nodump())
    flags |= cgccpp::DF_1_NODUMP;
  if (!parameters->options().shared())
    flags &= ~(cgccpp::DF_1_INITFIRST
	       | cgccpp::DF_1_NODELETE
	       | cgccpp::DF_1_NOOPEN);
  if (parameters->options().origin())
    flags |= cgccpp::DF_1_ORIGIN;
  if (parameters->options().now())
    flags |= cgccpp::DF_1_NOW;
  if (parameters->options().Bgroup())
    flags |= cgccpp::DF_1_GROUP;
  if (flags != 0)
    odyn->add_constant(cgccpp::DT_FLAGS_1, flags);
}
...
// based on the GNU linker default CGC linker script.
...
Layout::get_executable_sections(Section_list* section_list) const
{
  for (Section_list::const_iterator p = this->section_list_.begin();
       p != this->section_list_.end();
       ++p)
    if (((*p)->flags() & (cgccpp::SHF_ALLOC | cgccpp::SHF_EXECINSTR))
...
Output_segment*
Layout::make_output_segment(cgccpp::Cgc_Word type, cgccpp::Cgc_Word flags)
{
  gold_assert(!parameters->options().relocatable());
  Output_segment* oseg = new Output_segment(type, flags);
  this->segment_list_.push_back(oseg);

  if (type == cgccpp::PT_TLS)
    this->tls_segment_ = oseg;
  else if (type == cgccpp::PT_GNU_RELRO)
    this->relro_segment_ = oseg;
  else if (type == cgccpp::PT_INTERP)
...
// CGC code.  We simply walk through the segments, read them from
...
Layout::write_binary(Output_file* in) const
{
  ...
  for (Segment_list::const_iterator p = this->segment_list_.begin();
       ...
       ++p)
    {
      if ((*p)->type() == cgccpp::PT_LOAD && (*p)->filesz() > 0)
	{
...
Layout::layout<32, false>(Sized_relobj_file<32, false>* object,
			  unsigned int shndx,
			  const char* name,
			  const cgccpp::Shdr<32, false>& shdr,
...
Layout::layout<32, true>(Sized_relobj_file<32, true>* object,
			 unsigned int shndx,
			 const char* name,
			 const cgccpp::Shdr<32, true>& shdr,
			 unsigned int, unsigned int, off_t*);
#endif
...
Layout::layout<64, false>(Sized_relobj_file<64, false>* object,
			  unsigned int shndx,
			  const char* name,
			  const cgccpp::Shdr<64, false>& shdr,
			  unsigned int, unsigned int, off_t*);
#endif
...
Layout::layout<64, true>(Sized_relobj_file<64, true>* object,
			 unsigned int shndx,
			 const char* name,
			 const cgccpp::Shdr<64, true>& shdr,
			 unsigned int, unsigned int, off_t*);
#endif
