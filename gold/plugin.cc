...
Plugin_manager::claim_file(Input_file* input_file, off_t offset,
                           off_t filesize, Object* cgc_object)
{
  ...
  if (cgc_object != NULL)
    this->objects_.push_back(cgc_object);
...
Plugin_manager::make_plugin_object(unsigned int handle)
{
  ...
  // If the cgc object for this file was pushed into the objects_ vector, delete
...
// Get the cgc object corresponding to the handle. Return NULL if we
// found a Pluginobj instead.

Object*
Plugin_manager::get_cgc_object(const void* handle)
{
  ...
  for (int i = 0; i < this->nsyms_; ++i)
    {
      ...
      switch (isym->def)
        ...
        case LDPK_WEAKDEF:
        case LDPK_WEAKUNDEF:
          bind = cgccpp::STB_WEAK;
          break;
        case LDPK_DEF:
        case LDPK_UNDEF:
        case LDPK_COMMON:
        default:
          bind = cgccpp::STB_GLOBAL;
          break;
        }
      ...
      switch (isym->visibility)
        {
        case LDPV_PROTECTED:
          vis = cgccpp::STV_PROTECTED;
          break;
        case LDPV_INTERNAL:
          vis = cgccpp::STV_INTERNAL;
          break;
        case LDPV_HIDDEN:
          vis = cgccpp::STV_HIDDEN;
          break;
        case LDPV_DEFAULT:
        default:
          vis = cgccpp::STV_DEFAULT;
          break;
        }
      ...
      osym.put_st_name(0);
      osym.put_st_value(0);
      osym.put_st_size(static_cast<Cgc_size_type>(isym->size));
      osym.put_st_info(bind, cgccpp::STT_NOTYPE);
...
get_input_section_count(const void* handle, unsigned int* count)
{
  ...
  Object* obj = parameters->options().plugins()->get_cgc_object(handle);
                       ...
                       unsigned int* type)
{
  ...
  Object* obj
    = parameters->options().plugins()->get_cgc_object(section.handle); 
		     ...
		     unsigned int num_sections)
{
  ...
  for (unsigned int i = 0; i < num_sections; ++i)
    {
      Object* obj = parameters->options().plugins()->get_cgc_object(
...
// SECTION_LIST to a unique segment.  CGC segments do not have names
