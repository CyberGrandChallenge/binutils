...
# endif
  ...
  do_recognize_nacl_file(Input_file* input_file, off_t offset)
  {
    ...
    cgccpp::Cgc_file<size, big_endian, Sniff_file> cgc_file(&file);
    const unsigned int shnum = cgc_file.shnum();
    for (unsigned int shndx = 1; shndx < shnum; ++shndx)
      {
        if (cgc_file.section_type(shndx) == cgccpp::SHT_NOTE)
          {
            Sniff_file::Location loc = cgc_file.section_contents(shndx);
            if (loc.size() < (3 * 4
            ...
            if ((cgccpp::Swap<32, big_endian>::readval(note_data + 0)
                 == sizeof "NaCl")
                && (cgccpp::Swap<32, big_endian>::readval(note_data + 4)
                    == nacl_abi_name_.size() + 1)
                && (cgccpp::Swap<32, big_endian>::readval(note_data + 8)
                    == cgccpp::NT_VERSION))
              {
