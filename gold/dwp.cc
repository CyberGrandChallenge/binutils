...
class Dwo_file
{
 ...
 private:
  ...
  // and record the target info.  P is a pointer to the CGC header
  // in memory.
  Relobj*
  ...
  // The filename.
  const char* name_;
  // The CGC file, represented as a gold Relobj instance.
  Relobj* obj_;
...
class Sized_relobj_dwo : public Sized_relobj<size, big_endian>
{
 ...
 protected:
  ...
  { return this->cgc_file_.section_type(shndx); }

  // Get the name of a section.
  std::string
  do_section_name(unsigned int shndx)
  { return this->cgc_file_.section_name(shndx); }

  // Get the size of a section.
  uint64_t
  do_section_size(unsigned int shndx)
  { return this->cgc_file_.section_size(shndx); }
 ...
 private:
  // General access to the CGC file.
  cgccpp::Cgc_file<size, big_endian, Object> cgc_file_;
};

// The output file.
// This class is responsible for collecting the debug index information
// and writing the .dwp file in CGC format.
...
Sized_relobj_dwo<size, big_endian>::do_section_contents(
    ...
    bool cache)
{
  Object::Location loc(this->cgc_file_.section_contents(shndx));
...
Dwo_file::read_executable(File_list* files)
{
  ...
  for (unsigned int i = 1; i < shnum; i++)
    {
      if (this->section_type(i) != cgccpp::SHT_PROGBITS)
	continue;
...
Dwo_file::make_object(Dwp_output_file* output_file)
{
  ...
  // Check that it's an CGC file.
  off_t filesize = input_file->file().filesize();
  int hdrsize = cgccpp::Cgc_recognizer::max_header_size;
  if (filesize < hdrsize)
    hdrsize = filesize;
  const unsigned char* cgc_header =
      input_file->file().get_view(0, 0, hdrsize, true, false);
  if (!cgccpp::Cgc_recognizer::is_cgc_file(cgc_header, hdrsize))
    gold_fatal(_("%s: not an CGC object file"), this->name_);
  ...
  bool big_endian;
  std::string error;
  if (!cgccpp::Cgc_recognizer::is_valid_header(cgc_header, hdrsize, &size,
					       &big_endian, &error))
  ...
  if (size == 32)
	...
	return this->sized_make_object<32, true>(cgc_header, input_file,
						 output_file);
#else
	gold_unreachable();
#endif
	...
	return this->sized_make_object<32, false>(cgc_header, input_file,
						  output_file);
#else
	gold_unreachable();
#endif
	...
	return this->sized_make_object<64, true>(cgc_header, input_file,
						 output_file);
#else
	gold_unreachable();
#endif
	...
	return this->sized_make_object<64, false>(cgc_header, input_file,
						  output_file);
#else
	gold_unreachable();
#endif
...
// P is a pointer to the CGC header in memory.
...
Dwo_file::sized_make_object(const unsigned char* p, Input_file* input_file,
			    Dwp_output_file* output_file)
{
  cgccpp::Ehdr<size, big_endian> ehdr(p);
				    ...
				    Dwp_output_file* output_file)
{
  ...
  unsigned int version =
      cgccpp::Swap_unaligned<32, big_endian>::readval(contents);
  if (version != 1)
  ...
  for (unsigned int i = 0; i < nslots; ++i)
    {
      uint64_t dwo_id =
          cgccpp::Swap_unaligned<64, big_endian>::readval(phash);
      if (dwo_id != 0)
	{
	  unsigned int index =
	      cgccpp::Swap_unaligned<32, big_endian>::readval(pindex);
	  ...
	  for (;;)
		...
		gold_fatal(_("%s: .debug_cu_index is corrupt"),
			   this->name_);
	      unsigned int shndx =
		  cgccpp::Swap_unaligned<32, big_endian>::readval(shndx_list);
	      if (shndx == 0)
	        break;
	      if (shndx > this->shnum())
				  ...
				  section_size_type len)
{
  ...
  while (len > 0)
    {
      unsigned int val = cgccpp::Swap_unaligned<32, big_endian>::readval(p);
      val = this->remap_str_offset(val);
      cgccpp::Swap_unaligned<32, big_endian>::writeval(q, val);
      len -= 4;
      p += 4;
      q += 4;
    }
...
// set the CGC header values for the output file.  On subsequent
				    ...
				    int osabi, int abiversion)
{
  ...
  if (size == 32)
    this->next_file_offset_ = cgccpp::Cgc_sizes<32>::ehdr_size;
  else if (size == 64)
    this->next_file_offset_ = cgccpp::Cgc_sizes<64>::ehdr_size;
  else
    gold_unreachable();
  ...
  // Write zeroes for the CGC header initially.  We'll write
  // the actual header during finalize().
  static const char buf[cgccpp::Cgc_sizes<64>::ehdr_size] = { 0 };
...
Dwp_output_file::finalize()
{
  ...
  if (this->shnum_ >= cgccpp::SHN_LORESERVE)
    sh0_size = this->shnum_;
  if (this->shstrndx_ >= cgccpp::SHN_LORESERVE)
  ...
  for (unsigned int i = 0; i < this->sections_.size(); ++i)
    {
      ...
      this->write_shdr(sect.name, cgccpp::SHT_PROGBITS, 0, 0, sect.offset,
		       sect.size, 0, 0, sect.align, 0);
    }
  this->write_shdr(shstrtab_name, cgccpp::SHT_STRTAB, 0, 0,
...
Dwp_output_file::write_index(const char* sect_name, const Dwp_index& index)
{
  ...
  cgccpp::Swap_unaligned<32, big_endian>::writeval(p, 1);
  p += sizeof(uint32_t);
  cgccpp::Swap_unaligned<32, big_endian>::writeval(p, 0);
  p += sizeof(uint32_t);
  cgccpp::Swap_unaligned<32, big_endian>::writeval(p, nused);
  p += sizeof(uint32_t);
  cgccpp::Swap_unaligned<32, big_endian>::writeval(p, nslots);
  p += sizeof(uint32_t);

  // Write the hash table.
  for (unsigned int i = 0; i < nslots; ++i)
    {
      cgccpp::Swap_unaligned<64, big_endian>::writeval(p, index.hash_table(i));
      p += sizeof(uint64_t);
    }
  ...
  for (unsigned int i = 0; i < nslots; ++i)
    {
      cgccpp::Swap_unaligned<32, big_endian>::writeval(p, index.index_table(i));
      p += sizeof(uint32_t);
    }
  ...
  for (unsigned int i = 0; i < npool; ++i)
    {
      ...
      cgccpp::Swap_unaligned<32, big_endian>::writeval(p, *pool);
      p += sizeof(uint32_t);
      ++pool;
    }
...
{
  const unsigned int ehdr_size = cgccpp::Cgc_sizes<size>::ehdr_size;
  unsigned char buf[ehdr_size];
  cgccpp::Ehdr_write<size, big_endian> ehdr(buf);
  ...
  ehdr.put_e_type(cgccpp::ET_REL);
  ehdr.put_e_machine(this->machine_);
  ehdr.put_e_version(cgccpp::EV_CURRENT);
  ehdr.put_e_entry(0);
  ehdr.put_e_phoff(0);
  ehdr.put_e_shoff(this->shoff_);
  ehdr.put_e_flags(0);
  ehdr.put_e_ehsize(cgccpp::Cgc_sizes<size>::ehdr_size);
  ehdr.put_e_phentsize(0);
  ehdr.put_e_phnum(0);
  ehdr.put_e_shentsize(cgccpp::Cgc_sizes<size>::shdr_size);
  ehdr.put_e_shnum(this->shnum_ < cgccpp::SHN_LORESERVE ? this->shnum_ : 0);
  ehdr.put_e_shstrndx(this->shstrndx_ < cgccpp::SHN_LORESERVE
		      ? this->shstrndx_
		      : static_cast<unsigned int>(cgccpp::SHN_XINDEX));

  ::fseek(this->fd_, 0, 0);
  if (::fwrite(buf, 1, ehdr_size, this->fd_) < ehdr_size)
    gold_fatal(_("%s: error writing CGC header"), this->name_);
}
				  ...
				  unsigned int align, unsigned int ent_size)
{
  ...
  cgccpp::Shdr_write<size, big_endian> shdr(buf);
...
{
  const char* dwo_name = die->string_attribute(cgccpp::DW_AT_GNU_dwo_name);
  if (dwo_name != NULL)
...
{
  this->dwo_id_ = die->uint_attribute(cgccpp::DW_AT_GNU_dwo_id);
  if (this->dwo_id_ != 0)
