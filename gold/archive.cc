				    ...
				    size_t* tmpbuflen)
{
  ...
  *symp = sym;
  ...
  // symbol has actually been defined on the command line.
    ...
    return Library_base::SHOULD_INCLUDE_NO;
  else if (sym->binding() == cgccpp::STB_WEAK)
    return Library_base::SHOULD_INCLUDE_UNKNOWN;
...
Archive::read_armap(off_t start, section_size_type size)
{
  ...
  const cgccpp::Cgc_Word* pword = reinterpret_cast<const cgccpp::Cgc_Word*>(p);
  unsigned int nsyms = cgccpp::Swap<32, true>::readval(pword);
  ++pword;

  // Note that the addition is in units of sizeof(cgccpp::Cgc_Word).
  ...
  for (unsigned int i = 0; i < nsyms; ++i)
    {
      ...
      this->armap_[i].file_offset = cgccpp::Swap<32, true>::readval(pword);
...
// Return an CGC object for the member at offset OFF.  If
// PUNCONFIGURED is not NULL, then if the CGC object has an
// unsupported target type, set *PUNCONFIGURED to true and return
// NULL.

Object*
Archive::get_cgc_object_for_member(off_t off, bool* punconfigured)
{
  ...
  int read_size;
  Object *obj = NULL;
  bool is_cgc_obj = false;

  if (is_cgc_object(input_file, memoff, &ehdr, &read_size))
    {
      obj = make_cgc_object((std::string(this->input_file_->filename())
			     + "(" + member_name + ")"),
  ...
  if (!is_cgc_obj)
    {
      gold_error(_("%s: member at %zu is not an CGC object"),
		 this->name().c_str(), static_cast<size_t>(off));
      return NULL;
    }
...
{
  Object* obj = this->get_cgc_object_for_member(off, NULL);
  if (obj == NULL)
    return;
			...
			Mapfile* mapfile, Symbol* sym, const char* why)
{
  ...
  Object* obj = this->get_cgc_object_for_member(off, punconfigured);
  if (obj == NULL)
    {
