...
class Output_file_header : public Output_data
{
 public:
  ...
  // Add information about the section headers.  We lay out the CGC
...
// separate class.  The class is templatized on either cgccpp::SHT_REL
// or cgccpp::SHT_RELA, and also on whether this is a dynamic
...
class Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>
{
 public:
  typedef typename cgccpp::Cgc_types<size>::Cgc_Addr Address;
  typedef typename cgccpp::Cgc_types<size>::Cgc_Addr Addend;
  ...
  bool
  sort_before(const Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>&
	      r2) const
...
template<bool dynamic, int size, bool big_endian>
class Output_reloc<cgccpp::SHT_RELA, dynamic, size, big_endian>
{
 public:
  typedef typename cgccpp::Cgc_types<size>::Cgc_Addr Address;
  typedef typename cgccpp::Cgc_types<size>::Cgc_Addr Addend;
  ...
  // when sorting dynamic relocs.
  bool
  sort_before(const Output_reloc<cgccpp::SHT_RELA, dynamic, size, big_endian>&
	      r2) const
  {
 ...
 private:
  // The basic reloc.
  Output_reloc<cgccpp::SHT_REL, dynamic, size, big_endian> rel_;
  // The addend.
  Addend addend_;
};
...
// SH_TYPE is either cgccpp::SHT_REL or cgccpp::SHT_RELA.  DYNAMIC
...
template<bool dynamic, int size, bool big_endian>
class Output_data_reloc<cgccpp::SHT_REL, dynamic, size, big_endian>
  : public Output_data_reloc_base<cgccpp::SHT_REL, dynamic, size, big_endian>
{
 private:
  typedef Output_data_reloc_base<cgccpp::SHT_REL, dynamic, size,
				 big_endian> Base;
 ...
 public:
  ...
  Output_data_reloc(bool sr)
    : Output_data_reloc_base<cgccpp::SHT_REL, dynamic, size, big_endian>(sr)
  { }
...
template<bool dynamic, int size, bool big_endian>
class Output_data_reloc<cgccpp::SHT_RELA, dynamic, size, big_endian>
  : public Output_data_reloc_base<cgccpp::SHT_RELA, dynamic, size, big_endian>
{
 private:
  typedef Output_data_reloc_base<cgccpp::SHT_RELA, dynamic, size,
				 big_endian> Base;
 ...
 public:
  ...
  Output_data_reloc(bool sr)
    : Output_data_reloc_base<cgccpp::SHT_RELA, dynamic, size, big_endian>(sr)
  { }
...
{
 public:
  typedef typename cgccpp::Cgc_types<got_size>::Cgc_Addr Valtype;
 ...
 private:
  ...
  class Dynamic_entry
  {
   public:
    ...
    Dynamic_entry(cgccpp::DT tag, unsigned int val)
      : tag_(tag), offset_(DYNAMIC_NUMBER)
    { this->u_.val = val; }

    // Create an entry with the size or address of a section.
    Dynamic_entry(cgccpp::DT tag, const Output_data* od, bool section_size)
      : tag_(tag),
    ...
    Dynamic_entry(cgccpp::DT tag, const Output_data* od, const Output_data* od2)
      : tag_(tag),
    ...
    Dynamic_entry(cgccpp::DT tag, const Output_data* od, unsigned int offset)
      : tag_(tag),
	offset_(offset)
    { this->u_.od = od; }

    // Create an entry with the address of a symbol.
    Dynamic_entry(cgccpp::DT tag, const Symbol* sym)
      : tag_(tag), offset_(DYNAMIC_SYMBOL)
    { this->u_.sym = sym; }

    // Create an entry with a string.
    Dynamic_entry(cgccpp::DT tag, const char* str)
...
class Output_section : public Output_data
{
 public:
  ...
  Output_section(const char* name, cgccpp::Cgc_Word, cgccpp::Cgc_Xword);
  virtual ~Output_section();
  ...
  void
  update_flags_for_input_section(cgccpp::Cgc_Xword flags);
  ...
  { return this->is_large_section_ && this->type_ != cgccpp::SHT_NOBITS; }
  ...
  {
    gold_assert((this->flags_ & cgccpp::SHF_ALLOC) == 0);
  ...
  write_header(const Layout*, const Stringpool*,
	       cgccpp::Shdr_write<size, big_endian>*) const;
 ...
 protected:
  ...
  bool
  do_is_section_type(cgccpp::Cgc_Word type) const
  { return this->type_ == type; }

  // Return whether the specified section flag is set.
  bool
  do_is_section_flag_set(cgccpp::Cgc_Xword flag) const
 ...
 private:
  ...
  {
   public:
    Checkpoint_output_section(uint64_t addralign, cgccpp::Cgc_Xword flags,
...
class Output_segment
{
 public:
  ...
  Output_segment(cgccpp::Cgc_Word, cgccpp::Cgc_Word);
  ...
  // segment.
  void
  update_flags_for_output_section(cgccpp::Cgc_Xword flags)
  {
    // The CGC ABI specifies that a PT_TLS segment should always have
    // PF_R as the flags.
    if (this->type() != cgccpp::PT_TLS)
      this->flags_ |= flags;
  }
  ...
  void
  set_flags(cgccpp::Cgc_Word flags)
  { this->flags_ = flags; }
  ...
  template<int size, bool big_endian>
  void
  write_header(cgccpp::Phdr_write<size, big_endian>*);
