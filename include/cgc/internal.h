...
/* This file is part of CGC support for BFD, and contains the portions
   that describe how CGC is represented internally in the BFD library.
   I.E. it describes the in-memory representation of CGC.  It requires
   the cgc-common.h file which contains the portions that are common to
...
#ifndef _CGC_INTERNAL_H
#define _CGC_INTERNAL_H
...
typedef struct cgc_internal_ehdr {
  unsigned char		e_ident[EI_NIDENT]; /* CGC "magic number" */
  ...
  unsigned int		e_ehsize;	/* CGC header size in bytes */
...
typedef struct cgc_internal_phdr Cgc_Internal_Phdr;

/* Section header */

typedef struct cgc_internal_shdr {
...
typedef struct cgc_internal_sym Cgc_Internal_Sym;

/* Note segments */

typedef struct cgc_internal_note {
...
typedef struct cgc_internal_rela {
...
typedef struct cgc_internal_dyn {
  /* This needs to support 64-bit values in cgc64.  */
...
typedef struct cgc_internal_verdef {
  ...
  struct cgc_internal_verdef  *vd_nextdef;	/* vd_next as pointer.  */
  struct cgc_internal_verdaux *vd_auxptr;	/* vd_aux as pointer.  */
  unsigned int		       vd_exp_refno;	/* Used by the linker.  */
} Cgc_Internal_Verdef;

/* This structure appears in a SHT_GNU_verdef section.  */

typedef struct cgc_internal_verdaux {
  ...
  struct cgc_internal_verdaux *vda_nextptr;	/* vda_next as pointer.  */
} Cgc_Internal_Verdaux;

/* This structure appears in a SHT_GNU_verneed section.  */

typedef struct cgc_internal_verneed {
  ...
  struct cgc_internal_vernaux *vn_auxptr;	/* vn_aux as pointer.  */
  struct cgc_internal_verneed *vn_nextref;	/* vn_nextref as pointer.  */
} Cgc_Internal_Verneed;

/* This structure appears in a SHT_GNU_verneed section.  */

typedef struct cgc_internal_vernaux {
  ...
  struct cgc_internal_vernaux *vna_nextptr;	/* vna_next as pointer.  */
} Cgc_Internal_Vernaux;

/* This structure appears in a SHT_GNU_versym section.  This is not a
   standard CGC structure; CGC just uses Cgc32_Half.  */

typedef struct cgc_internal_versym {
...
#define CGC_TBSS_SPECIAL(sec_hdr, segment)			\
  (((sec_hdr)->sh_flags & SHF_TLS) != 0				\
   && (sec_hdr)->sh_type == SHT_NOBITS				\
   && (segment)->p_type != PT_TLS)

#define CGC_SECTION_SIZE(sec_hdr, segment)			\
  (CGC_TBSS_SPECIAL(sec_hdr, segment) ? 0 : (sec_hdr)->sh_size)
...
#define CGC_SECTION_IN_SEGMENT_1(sec_hdr, segment, check_vma, strict)	\
  ((/* Only PT_LOAD, PT_GNU_RELRO and PT_TLS segments can contain	\
   ...
   && ((sec_hdr)->sh_type == SHT_NOBITS					\
	   ...
	   && (!(strict)						\
		   ...
		   <= (segment)->p_filesz - 1))				\
	   && (((sec_hdr)->sh_offset - (segment)->p_offset		\
		+ CGC_SECTION_SIZE(sec_hdr, segment))			\
	       <= (segment)->p_filesz)))				\
...
#define CGC_SECTION_IN_SEGMENT(sec_hdr, segment)			\
  (CGC_SECTION_IN_SEGMENT_1 (sec_hdr, segment, 1, 0))

#define CGC_SECTION_IN_SEGMENT_STRICT(sec_hdr, segment)			\
  (CGC_SECTION_IN_SEGMENT_1 (sec_hdr, segment, 1, 1))

#endif /* _CGC_INTERNAL_H */
