/* readcgc.c -- display contents of an CGC format file
   ...
   02110-1301, USA.  */

/* The difference between readcgc and objdump:

  Both programs are capable of displaying the contents of CGC format files,
  so why does the binutils project have two file dumpers ?

  The reason is that objdump sees an CGC file through a BFD filter of the
  ...
  This is why the readcgc program does not link against the BFD library - it
  exists as an independent program to help verify the correct working of BFD.

  There is also the case that readcgc can provide more information about an
  CGC file than is provided by objdump.  In particular it can display DWARF
...
#if __GNUC__ >= 2
/* Define BFD64 here, even if our default architecture is 32 bit CGC
   as this will allow us to read in and parse 64bit and 32bit CGC files.
...
/* The following headers use the cgc/reloc-macros.h file to
   automatically generate relocation recognition functions
   such as cgc_mips_reloc_type()  */
...
#include "cgc/ip2k.h"
#include "cgc/lm32.h"
#include "cgc/iq2000.h"
#include "cgc/m32c.h"
#include "cgc/m32r.h"
#include "cgc/m68k.h"
#include "cgc/m68hc11.h"
#include "cgc/mcore.h"
#include "cgc/mep.h"
#include "cgc/metag.h"
#include "cgc/microblaze.h"
#include "cgc/mips.h"
#include "cgc/mmix.h"
#include "cgc/mn10200.h"
#include "cgc/mn10300.h"
#include "cgc/moxie.h"
#include "cgc/mt.h"
#include "cgc/msp430.h"
#include "cgc/nios2.h"
#include "cgc/or32.h"
#include "cgc/pj.h"
#include "cgc/ppc.h"
...
#include "cgc/spu.h"
#include "cgc/tic6x.h"
#include "cgc/tilegx.h"
#include "cgc/tilepro.h"
#include "cgc/v850.h"
#include "cgc/vax.h"
...
static Cgc_Internal_Sym * dynamic_symbols;
static Cgc_Internal_Syminfo * dynamic_syminfo;
...
static Cgc_Internal_Ehdr cgc_header;
static Cgc_Internal_Shdr * section_headers;
static Cgc_Internal_Phdr * program_headers;
static Cgc_Internal_Dyn *  dynamic_section;
static Cgc_Internal_Shdr * symtab_shndx_hdr;
...
#define GET_CGC_SYMBOLS(file, section, sym_count)			\
  (is_32bit_cgc ? get_32bit_cgc_symbols (file, section, sym_count)	\
   : get_64bit_cgc_symbols (file, section, sym_count))
...
#define REMOVE_ARCH_BITS(ADDR)			\
  do						\
    {						\
      if (cgc_header.e_machine == EM_ARM)	\
	(ADDR) &= ~1;				\
    }						\
  while (0)

...
static Cgc_Internal_Shdr *
find_section (const char * name)
{
  unsigned int i;

  for (i = 0; i < cgc_header.e_shnum; i++)
...
find_section_by_address (bfd_vma addr)
{
  ...
  for (i = 0; i < cgc_header.e_shnum; i++)
    {
      Cgc_Internal_Shdr *sec = section_headers + i;
...
/* Return true if the current file is for IA-64 machine and OpenVMS ABI.
   This OS has so many departures from the CGC standard that we test it at
   many places.  */

static inline int
is_ia64_vms (void)
{
  return cgc_header.e_machine == EM_IA_64
    && cgc_header.e_ident[EI_OSABI] == CGCOSABI_OPENVMS;
}
...
slurp_rela_relocs (FILE * file,
		   ...
		   Cgc_Internal_Rela ** relasp,
		   unsigned long * nrelasp)
{
  Cgc_Internal_Rela * relas;
  unsigned long nrelas;
  unsigned int i;

  if (is_32bit_cgc)
    {
      Cgc32_External_Rela * erelas;

      erelas = (Cgc32_External_Rela *) get_data (NULL, file, rel_offset, 1,
                                                 rel_size, _("32-bit relocation data"));
      if (!erelas)
	return 0;

      nrelas = rel_size / sizeof (Cgc32_External_Rela);

      relas = (Cgc_Internal_Rela *) cmalloc (nrelas,
    ...
    {
      Cgc64_External_Rela * erelas;

      erelas = (Cgc64_External_Rela *) get_data (NULL, file, rel_offset, 1,
                                                 rel_size, _("64-bit relocation data"));
      if (!erelas)
	return 0;

      nrelas = rel_size / sizeof (Cgc64_External_Rela);
...
#ifdef BFD64
	  if (cgc_header.e_machine == EM_MIPS
	      && cgc_header.e_ident[EI_DATA] != CGCDATA2MSB)
	    {
...
slurp_rel_relocs (FILE * file,
		  unsigned long rel_offset,
		  unsigned long rel_size,
		  Cgc_Internal_Rela ** relsp,
		  unsigned long * nrelsp)
{
  Cgc_Internal_Rela * rels;
  unsigned long nrels;
  unsigned int i;

  if (is_32bit_cgc)
    {
      Cgc32_External_Rel * erels;

      erels = (Cgc32_External_Rel *) get_data (NULL, file, rel_offset, 1,
                                               rel_size, _("32-bit relocation data"));
      if (!erels)
	return 0;

      nrels = rel_size / sizeof (Cgc32_External_Rel);

      rels = (Cgc_Internal_Rela *) cmalloc (nrels, sizeof (Cgc_Internal_Rela));
    ...
    {
      Cgc64_External_Rel * erels;

      erels = (Cgc64_External_Rel *) get_data (NULL, file, rel_offset, 1,
                                               rel_size, _("64-bit relocation data"));
      if (!erels)
	return 0;

      nrels = rel_size / sizeof (Cgc64_External_Rel);
...
{
  if (is_32bit_cgc)
    return CGC32_R_TYPE (reloc_info);

  switch (cgc_header.e_machine)
    {
    case EM_MIPS:
      /* Note: We assume that reloc_info has already been adjusted for us.  */
      return CGC64_MIPS_R_TYPE (reloc_info);

    case EM_SPARCV9:
      return CGC64_R_TYPE_ID (reloc_info);

    default:
      return CGC64_R_TYPE (reloc_info);
    }
}
...
{
  return is_32bit_cgc ? CGC32_R_SYM (reloc_info) : CGC64_R_SYM (reloc_info);
}
...
{
  return
    cgc_header.e_machine == EM_MSP430 /* Paranoia.  */
    /* GCC uses osabi == CGCOSBI_STANDALONE.  */
    && (((cgc_header.e_flags & EF_MSP430_MACH) == E_MSP430_MACH_MSP430X)
	/* TI compiler uses CGCOSABI_NONE.  */
	|| (cgc_header.e_ident[EI_OSABI] == CGCOSABI_NONE));
}
		  ...
		  int is_rela)
{
  ...
  if (is_rela == UNKNOWN)
    is_rela = guess_is_rela (cgc_header.e_machine);
  ...
  for (i = 0; i < rel_size; i++)
	...
	case EM_AARCH64:
	  rtype = cgc_aarch64_reloc_type (type);
	  break;

	case EM_M32R:
	case EM_CYGNUS_M32R:
	  rtype = cgc_m32r_reloc_type (type);
	  break;

	case EM_386:
	case EM_486:
	  rtype = cgc_i386_reloc_type (type);
	  break;

	case EM_68HC11:
	case EM_68HC12:
	  rtype = cgc_m68hc11_reloc_type (type);
	  break;

	case EM_68K:
	  rtype = cgc_m68k_reloc_type (type);
	  break;

	case EM_960:
	  rtype = cgc_i960_reloc_type (type);
	  break;

	case EM_AVR:
	case EM_AVR_OLD:
	  rtype = cgc_avr_reloc_type (type);
	  break;
	...
	case EM_SPARCV9:
	case EM_SPARC:
	  rtype = cgc_sparc_reloc_type (type);
	  break;

	case EM_SPU:
	  rtype = cgc_spu_reloc_type (type);
	  break;
	...
	case EM_D10V:
	case EM_CYGNUS_D10V:
	  rtype = cgc_d10v_reloc_type (type);
	  break;

	case EM_D30V:
	case EM_CYGNUS_D30V:
	  rtype = cgc_d30v_reloc_type (type);
	  break;

	case EM_DLX:
	  rtype = cgc_dlx_reloc_type (type);
	  break;

	case EM_SH:
	  rtype = cgc_sh_reloc_type (type);
	  break;

	case EM_MN10300:
	case EM_CYGNUS_MN10300:
	  rtype = cgc_mn10300_reloc_type (type);
	  break;

	case EM_MN10200:
	case EM_CYGNUS_MN10200:
	  rtype = cgc_mn10200_reloc_type (type);
	  break;

	case EM_FR30:
	case EM_CYGNUS_FR30:
	  rtype = cgc_fr30_reloc_type (type);
	  break;

	case EM_CYGNUS_FRV:
	  rtype = cgc_frv_reloc_type (type);
	  break;

	case EM_MCORE:
	  rtype = cgc_mcore_reloc_type (type);
	  break;

	case EM_MMIX:
	  rtype = cgc_mmix_reloc_type (type);
	  break;

	case EM_MOXIE:
	  rtype = cgc_moxie_reloc_type (type);
	  break;

	case EM_MSP430:
	  if (uses_msp430x_relocs ())
	    {
	      rtype = cgc_msp430x_reloc_type (type);
	      break;
	    }
	case EM_MSP430_OLD:
	  rtype = cgc_msp430_reloc_type (type);
	  break;

	case EM_PPC:
	  rtype = cgc_ppc_reloc_type (type);
	  break;

	case EM_PPC64:
	  rtype = cgc_ppc64_reloc_type (type);
	  break;

	case EM_MIPS:
	case EM_MIPS_RS3_LE:
	  rtype = cgc_mips_reloc_type (type);
	  break;

	case EM_ALPHA:
	  rtype = cgc_alpha_reloc_type (type);
	  break;

	case EM_ARM:
	  rtype = cgc_arm_reloc_type (type);
	  break;

	case EM_ARC:
	  rtype = cgc_arc_reloc_type (type);
	  break;

	case EM_PARISC:
	  rtype = cgc_hppa_reloc_type (type);
	  break;

	case EM_H8_300:
	case EM_H8_300H:
	case EM_H8S:
	  rtype = cgc_h8_reloc_type (type);
	  break;

	case EM_OPENRISC:
	case EM_OR32:
	  rtype = cgc_or32_reloc_type (type);
	  break;

	case EM_PJ:
	case EM_PJ_OLD:
	  rtype = cgc_pj_reloc_type (type);
	  break;
	case EM_IA_64:
	  rtype = cgc_ia64_reloc_type (type);
	  break;

	case EM_CRIS:
	  rtype = cgc_cris_reloc_type (type);
	  break;

	case EM_860:
	  rtype = cgc_i860_reloc_type (type);
	  break;

	case EM_X86_64:
	case EM_L1OM:
	case EM_K1OM:
	  rtype = cgc_x86_64_reloc_type (type);
	  break;
	...
	case EM_S390_OLD:
	case EM_S390:
	  rtype = cgc_s390_reloc_type (type);
	  break;

	case EM_SCORE:
	  rtype = cgc_score_reloc_type (type);
	  break;

	case EM_XSTORMY16:
	  rtype = cgc_xstormy16_reloc_type (type);
	  break;

	case EM_CRX:
	  rtype = cgc_crx_reloc_type (type);
	  break;

	case EM_VAX:
	  rtype = cgc_vax_reloc_type (type);
	  break;

	case EM_ADAPTEVA_EPIPHANY:
	  rtype = cgc_epiphany_reloc_type (type);
	  break;

	case EM_IP2K:
	case EM_IP2K_OLD:
	  rtype = cgc_ip2k_reloc_type (type);
	  break;

	case EM_IQ2000:
	  rtype = cgc_iq2000_reloc_type (type);
	  break;

	case EM_XTENSA_OLD:
	case EM_XTENSA:
	  rtype = cgc_xtensa_reloc_type (type);
	  break;

	case EM_LATTICEMICO32:
	  rtype = cgc_lm32_reloc_type (type);
	  break;

	case EM_M32C_OLD:
	case EM_M32C:
	  rtype = cgc_m32c_reloc_type (type);
	  break;

	case EM_MT:
	  rtype = cgc_mt_reloc_type (type);
	  break;

	case EM_BLACKFIN:
	  rtype = cgc_bfin_reloc_type (type);
	  break;

	case EM_CYGNUS_MEP:
	  rtype = cgc_mep_reloc_type (type);
	  break;

	case EM_CR16:
	  rtype = cgc_cr16_reloc_type (type);
	  break;

	case EM_MICROBLAZE:
	case EM_MICROBLAZE_OLD:
	  rtype = cgc_microblaze_reloc_type (type);
	  break;

	case EM_RL78:
	  rtype = cgc_rl78_reloc_type (type);
	  break;

	case EM_RX:
	  rtype = cgc_rx_reloc_type (type);
	  break;

	case EM_METAG:
	  rtype = cgc_metag_reloc_type (type);
	  break;

	case EM_XC16X:
	case EM_C166:
	  rtype = cgc_xc16x_reloc_type (type);
	  break;

	case EM_TI_C6000:
	  rtype = cgc_tic6x_reloc_type (type);
	  break;

	case EM_TILEGX:
	  rtype = cgc_tilegx_reloc_type (type);
	  break;

	case EM_TILEPRO:
	  rtype = cgc_tilepro_reloc_type (type);
	  break;

	case EM_XGATE:
	  rtype = cgc_xgate_reloc_type (type);
	  break;

	case EM_ALTERA_NIOS2:
	  rtype = cgc_nios2_reloc_type (type);
	  break;
	}
	...
	printf (do_wide ? "%-22.22s" : "%-17.17s", rtype);
      ...
      if (cgc_header.e_machine == EM_ALPHA
	  && rtype != NULL
      ...
      else if (symtab_index)
	{
	  if (symtab == NULL || symtab_index >= nsyms)
	    ...
	    {
	      Cgc_Internal_Sym * psym;

	      psym = symtab + symtab_index;

	      printf (" ");

	      if (CGC_ST_TYPE (psym->st_info) == STT_GNU_IFUNC)
		{
		  const char * name;
		  unsigned int len;
		  unsigned int width = is_32bit_cgc ? 8 : 14;
	      ...
	      if (psym->st_name == 0)
		{
		  ...
		  if (CGC_ST_TYPE (psym->st_info) == STT_SECTION)
		    {
		      if (psym->st_shndx < cgc_header.e_shnum)
			sec_name
			...
			sec_name = "COMMON";
		      else if ((cgc_header.e_machine == EM_MIPS
				&& psym->st_shndx == SHN_MIPS_SCOMMON)
			       || (cgc_header.e_machine == EM_TI_C6000
				   && psym->st_shndx == SHN_TIC6X_SCOMMON))
			sec_name = "SCOMMON";
		      else if (cgc_header.e_machine == EM_MIPS
			       && psym->st_shndx == SHN_MIPS_SUNDEFINED)
			sec_name = "SUNDEF";
		      else if ((cgc_header.e_machine == EM_X86_64
				|| cgc_header.e_machine == EM_L1OM
				|| cgc_header.e_machine == EM_K1OM)
			       && psym->st_shndx == SHN_X86_64_LCOMMON)
			sec_name = "LARGE_COMMON";
		      else if (cgc_header.e_machine == EM_IA_64
			       && cgc_header.e_ident[EI_OSABI] == CGCOSABI_HPUX
      ...
      else if (is_rela)
	{
      ...
      if (cgc_header.e_machine == EM_SPARCV9
	  && rtype != NULL
	  && streq (rtype, "R_SPARC_OLO10"))
	printf (" + %lx", (unsigned long) CGC64_R_TYPE_DATA (inf));

      putchar ('\n');

#ifdef BFD64
      if (! is_32bit_cgc && cgc_header.e_machine == EM_MIPS)
	{
	  bfd_vma type2 = CGC64_MIPS_R_TYPE2 (inf);
	  bfd_vma type3 = CGC64_MIPS_R_TYPE3 (inf);
	  const char * rtype2 = cgc_mips_reloc_type (type2);
	  const char * rtype3 = cgc_mips_reloc_type (type3);
  ...
  switch (type)
      ...
      if ((type >= DT_LOPROC) && (type <= DT_HIPROC))
	{
	  ...
	  snprintf (buff, sizeof (buff), _("Processor Specific: %lx"), type);
	       ...
	       || (cgc_header.e_machine == EM_PARISC
  ...
  if (e_flags)
	...
	case EM_MIPS_RS3_LE:
	  ...
	  switch ((e_flags & EF_MIPS_MACH))
	    ...
	    /* We simply ignore the field in this case to avoid confusion:
	       MIPS CGC does not specify EF_MIPS_MACH, it is a GNU
	       extension.  */
	      break;
	  ...
	  switch ((e_flags & EF_MIPS_ABI))
	    ...
	    case 0:
	    /* We simply ignore the field in this case to avoid confusion:
	       MIPS CGC does not specify EF_MIPS_ABI, it is a GNU extension.
	...
	case EM_IA_64:
	  ...
	  if ((e_flags & EF_IA_64_ABSOLUTE))
	    strcat (buf, ", absolute");
          if (cgc_header.e_ident[EI_OSABI] == CGCOSABI_OPENVMS)
            {
...
get_osabi_name (unsigned int osabi)
{
  ...
  switch (osabi)
    {
    case CGCOSABI_NONE:		return "UNIX - System V";
    case CGCOSABI_HPUX:		return "UNIX - HP-UX";
    case CGCOSABI_NETBSD:	return "UNIX - NetBSD";
    case CGCOSABI_GNU:		return "UNIX - GNU";
    case CGCOSABI_SOLARIS:	return "UNIX - Solaris";
    case CGCOSABI_AIX:		return "UNIX - AIX";
    case CGCOSABI_IRIX:		return "UNIX - IRIX";
    case CGCOSABI_FREEBSD:	return "UNIX - FreeBSD";
    case CGCOSABI_TRU64:	return "UNIX - TRU64";
    case CGCOSABI_MODESTO:	return "Novell - Modesto";
    case CGCOSABI_OPENBSD:	return "UNIX - OpenBSD";
    case CGCOSABI_OPENVMS:	return "VMS - OpenVMS";
    case CGCOSABI_NSK:		return "HP - Non-Stop Kernel";
    case CGCOSABI_AROS:		return "AROS";
    case CGCOSABI_FENIXOS:	return "FenixOS";
    default:
      if (osabi >= 64)
	switch (cgc_header.e_machine)
	  {
	  ...
	  case EM_MSP430_OLD:
	    switch (osabi)
	      {
	      case CGCOSABI_STANDALONE:	return _("Standalone App");
	      default:
		break;
	      }
	    break;
	  ...
	  case EM_TI_C6000:
	    switch (osabi)
	      {
	      case CGCOSABI_C6000_CGCABI:	return _("Bare-metal C6000");
	      case CGCOSABI_C6000_LINUX:	return "Linux C6000";
	      default:
		break;
	      }
	    break;
...
usage (FILE * stream)
{
  fprintf (stream, _("Usage: readcgc <option(s)> cgc-file(s)\n"));
  fprintf (stream, _(" Display information about the contents of CGC format files\n"));
  fprintf (stream, _(" Options are:\n\
  -a --all               Equivalent to: -h -l -S -s -r -d -V -A -I\n\
  -h --file-header       Display the CGC file header\n\
  ...
  -v --version           Display the version number of readcgc\n"));
...
static const char *
get_cgc_class (unsigned int cgc_class)
{
  static char buff[32];

  switch (cgc_class)
    {
    case CGCCLASSNONE: return _("none");
    case CGCCLASS32:   return "CGC32";
    case CGCCLASS64:   return "CGC64";
    default:
      snprintf (buff, sizeof (buff), _("<unknown: %x>"), cgc_class);
      return buff;
    }
}
...
get_data_encoding (unsigned int encoding)
{
  ...
  switch (encoding)
    {
    case CGCDATANONE: return _("none");
    case CGCDATA2LSB: return _("2's complement, little endian");
    case CGCDATA2MSB: return _("2's complement, big endian");
    default:
...
/* Decode the data held in 'cgc_header'.  */

static int
process_file_header (void)
{
  if (   cgc_header.e_ident[EI_MAG0] != CGCMAG0
      || cgc_header.e_ident[EI_MAG1] != CGCMAG1
      || cgc_header.e_ident[EI_MAG2] != CGCMAG2
      || cgc_header.e_ident[EI_MAG3] != CGCMAG3)
    {
      error
	(_("Not an CGC file - it has the wrong magic bytes at the start\n"));
      return 0;
    }

  init_dwarf_regnames (cgc_header.e_machine);
  ...
  if (do_header)
	...
	printf ("%2.2x ", cgc_header.e_ident[i]);
      printf ("\n");
      printf (_("  Class:                             %s\n"),
	      get_cgc_class (cgc_header.e_ident[EI_CLASS]));
      printf (_("  Data:                              %s\n"),
	      get_data_encoding (cgc_header.e_ident[EI_DATA]));
      printf (_("  Version:                           %d %s\n"),
	      cgc_header.e_ident[EI_VERSION],
	      (cgc_header.e_ident[EI_VERSION] == EV_CURRENT
	       ? "(current)"
	       : (cgc_header.e_ident[EI_VERSION] != EV_NONE
		  ? _("<unknown: %lx>")
		  : "")));
      printf (_("  OS/ABI:                            %s\n"),
	      get_osabi_name (cgc_header.e_ident[EI_OSABI]));
      printf (_("  ABI Version:                       %d\n"),
	      cgc_header.e_ident[EI_ABIVERSION]);
      printf (_("  Type:                              %s\n"),
	      get_file_type (cgc_header.e_type));
      printf (_("  Machine:                           %s\n"),
	      get_machine_name (cgc_header.e_machine));
      printf (_("  Version:                           0x%lx\n"),
	      (unsigned long) cgc_header.e_version);

      printf (_("  Entry point address:               "));
      print_vma ((bfd_vma) cgc_header.e_entry, PREFIX_HEX);
      printf (_("\n  Start of program headers:          "));
      print_vma ((bfd_vma) cgc_header.e_phoff, DEC);
      printf (_(" (bytes into file)\n  Start of section headers:          "));
      print_vma ((bfd_vma) cgc_header.e_shoff, DEC);
      printf (_(" (bytes into file)\n"));

      printf (_("  Flags:                             0x%lx%s\n"),
	      (unsigned long) cgc_header.e_flags,
	      get_machine_flags (cgc_header.e_flags, cgc_header.e_machine));
      printf (_("  Size of this header:               %ld (bytes)\n"),
	      (long) cgc_header.e_ehsize);
      printf (_("  Size of program headers:           %ld (bytes)\n"),
	      (long) cgc_header.e_phentsize);
      printf (_("  Number of program headers:         %ld"),
	      (long) cgc_header.e_phnum);
      if (section_headers != NULL
	  && cgc_header.e_phnum == PN_XNUM
	  && section_headers[0].sh_info != 0)
	printf (" (%ld)", (long) section_headers[0].sh_info);
      putc ('\n', stdout);
      printf (_("  Size of section headers:           %ld (bytes)\n"),
	      (long) cgc_header.e_shentsize);
      printf (_("  Number of section headers:         %ld"),
	      (long) cgc_header.e_shnum);
      if (section_headers != NULL && cgc_header.e_shnum == SHN_UNDEF)
	printf (" (%ld)", (long) section_headers[0].sh_size);
      putc ('\n', stdout);
      printf (_("  Section header string table index: %ld"),
	      (long) cgc_header.e_shstrndx);
      if (section_headers != NULL
	  && cgc_header.e_shstrndx == (SHN_XINDEX & 0xffff))
	printf (" (%u)", section_headers[0].sh_link);
      else if (cgc_header.e_shstrndx != SHN_UNDEF
	       && cgc_header.e_shstrndx >= cgc_header.e_shnum)
  ...
  if (section_headers != NULL)
    {
      if (cgc_header.e_phnum == PN_XNUM
	  && section_headers[0].sh_info != 0)
	cgc_header.e_phnum = section_headers[0].sh_info;
      if (cgc_header.e_shnum == SHN_UNDEF)
	cgc_header.e_shnum = section_headers[0].sh_size;
      if (cgc_header.e_shstrndx == (SHN_XINDEX & 0xffff))
	cgc_header.e_shstrndx = section_headers[0].sh_link;
      else if (cgc_header.e_shstrndx >= cgc_header.e_shnum)
	cgc_header.e_shstrndx = SHN_UNDEF;
...
static int
get_32bit_program_headers (FILE * file, Cgc_Internal_Phdr * pheaders)
{
  Cgc32_External_Phdr * phdrs;
  Cgc32_External_Phdr * external;
  Cgc_Internal_Phdr *   internal;
  unsigned int i;

  phdrs = (Cgc32_External_Phdr *) get_data (NULL, file, cgc_header.e_phoff,
                                            cgc_header.e_phentsize,
                                            cgc_header.e_phnum,
                                            _("program headers"));
  if (!phdrs)
    return 0;
...
static int
get_64bit_program_headers (FILE * file, Cgc_Internal_Phdr * pheaders)
{
  Cgc64_External_Phdr * phdrs;
  Cgc64_External_Phdr * external;
  Cgc_Internal_Phdr *   internal;
  unsigned int i;

  phdrs = (Cgc64_External_Phdr *) get_data (NULL, file, cgc_header.e_phoff,
                                            cgc_header.e_phentsize,
                                            cgc_header.e_phnum,
                                            _("program headers"));
  if (!phdrs)
...
get_program_headers (FILE * file)
{
  ...
  phdrs = (Cgc_Internal_Phdr *) cmalloc (cgc_header.e_phnum,
...
process_program_headers (FILE * file)
{
  ...
  if (cgc_header.e_phnum == 0)
    {
      /* PR binutils/12467.  */
      if (cgc_header.e_phoff != 0)
	warn (_("possibly corrupt CGC header - it has a non-zero program"
  ...
  if (do_segments && section_headers != NULL && string_table != NULL)
    {
      ...
      for (i = 0; i < cgc_header.e_phnum; i++)
	{
	  unsigned int j;
	  Cgc_Internal_Shdr * section;
	  ...
	  for (j = 1; j < cgc_header.e_shnum; j++, section++)
	    {
	      if (!CGC_TBSS_SPECIAL (section, segment)
		  && CGC_SECTION_IN_SEGMENT_STRICT (section, segment))
...
offset_from_vma (FILE * file, bfd_vma vma, bfd_size_type size)
{
  ...
  for (seg = program_headers;
       seg < program_headers + cgc_header.e_phnum;
       ++seg)
    {
...
{
  Cgc32_External_Shdr * shdrs;
  Cgc_Internal_Shdr *   internal;
  unsigned int i;

  shdrs = (Cgc32_External_Shdr *) get_data (NULL, file, cgc_header.e_shoff,
                                            cgc_header.e_shentsize, num,
                                            _("section headers"));
  if (!shdrs)
    return 0;

  section_headers = (Cgc_Internal_Shdr *) cmalloc (num,
...
{
  Cgc64_External_Shdr * shdrs;
  Cgc_Internal_Shdr *   internal;
  unsigned int i;

  shdrs = (Cgc64_External_Shdr *) get_data (NULL, file, cgc_header.e_shoff,
                                            cgc_header.e_shentsize, num,
                                            _("section headers"));
  if (!shdrs)
    return 0;
...
get_32bit_cgc_symbols (FILE * file,
		       Cgc_Internal_Shdr * section,
		       unsigned long * num_syms_return)
{
  unsigned long number = 0;
  Cgc32_External_Sym * esyms = NULL;
  Cgc_External_Sym_Shndx * shndx = NULL;
  Cgc_Internal_Sym * isyms = NULL;
  Cgc_Internal_Sym * psym;
  unsigned int j;
  ...
  if (number * sizeof (Cgc32_External_Sym) > section->sh_size + 1)
    {
      error (_("Invalid sh_entsize\n"));
      goto exit_point;
    }

  esyms = (Cgc32_External_Sym *) get_data (NULL, file, section->sh_offset, 1,
  ...
  if (symtab_shndx_hdr != NULL
    ...
    {
      shndx = (Cgc_External_Sym_Shndx *) get_data (NULL, file,
  ...
  isyms = (Cgc_Internal_Sym *) cmalloc (number, sizeof (Cgc_Internal_Sym));
...
static Cgc_Internal_Sym *
get_64bit_cgc_symbols (FILE * file,
		       Cgc_Internal_Shdr * section,
		       unsigned long * num_syms_return)
{
  unsigned long number = 0;
  Cgc64_External_Sym * esyms = NULL;
  Cgc_External_Sym_Shndx * shndx = NULL;
  Cgc_Internal_Sym * isyms = NULL;
  Cgc_Internal_Sym * psym;
  unsigned int j;
  ...
  if (number * sizeof (Cgc64_External_Sym) > section->sh_size + 1)
    {
      error (_("Invalid sh_entsize\n"));
      goto exit_point;
    }

  esyms = (Cgc64_External_Sym *) get_data (NULL, file, section->sh_offset, 1,
                                           section->sh_size, _("symbols"));
  if (!esyms)
    goto exit_point;
...
static const char *
get_cgc_section_flags (bfd_vma sh_flags)
{
  static char buff[1024];
  char * p = buff;
  int field_size = is_32bit_cgc ? 8 : 16;
  int sindex;
  ...
  while (sh_flags)
	  ...
	  switch (flag)
		...
		case EM_IA_64:
		  if (flag == SHF_IA_64_SHORT)
		    ...
		    sindex = 11;
#ifdef BFD64
		  else if (cgc_header.e_ident[EI_OSABI] == CGCOSABI_OPENVMS)
		    switch (flag)
		      {
      ...
      else
	{
	  switch (flag)
	    ...
	    default:
	      if ((cgc_header.e_machine == EM_X86_64
		   || cgc_header.e_machine == EM_L1OM
		   || cgc_header.e_machine == EM_K1OM)
...
process_section_headers (FILE * file)
{
  ...
  if (cgc_header.e_shnum == 0)
    {
      /* PR binutils/12467.  */
      if (cgc_header.e_shoff != 0)
	warn (_("possibly corrupt CGC file header - it has a non-zero"
  ...
  if (do_sections && !do_header)
    printf (_("There are %d section headers, starting at offset 0x%lx:\n"),
	    cgc_header.e_shnum, (unsigned long) cgc_header.e_shoff);

  if (is_32bit_cgc)
    {
      if (! get_32bit_section_headers (file, cgc_header.e_shnum))
	return 0;
    }
  else if (! get_64bit_section_headers (file, cgc_header.e_shnum))
    return 0;

  /* Read in the string table, so that we have names to display.  */
  if (cgc_header.e_shstrndx != SHN_UNDEF
       && cgc_header.e_shstrndx < cgc_header.e_shnum)
    {
      section = section_headers + cgc_header.e_shstrndx;
  ...
  eh_addr_size = is_32bit_cgc ? 4 : 8;
  switch (cgc_header.e_machine)
    {
    case EM_MIPS:
    case EM_MIPS_RS3_LE:
      /* The 64-bit MIPS EABI uses a combination of 32-bit CGC and 64-bit
	 ...
	 the official LP64 form is being used.  */
      if ((cgc_header.e_flags & EF_MIPS_ABI) == E_MIPS_ABI_EABI64
    ...
    case EM_H8_300:
    case EM_H8_300H:
      switch (cgc_header.e_flags & EF_H8_MACH)
	{
    ...
    case EM_M32C_OLD:
    case EM_M32C:
      switch (cgc_header.e_flags & EF_M32C_CPU_MASK)
	{
...
#define CHECK_ENTSIZE_VALUES(section, i, size32, size64) \
  do									    \
    {									    \
      bfd_size_type expected_entsize = is_32bit_cgc ? size32 : size64;	    \
...
#define CHECK_ENTSIZE(section, i, type)					\
  CHECK_ENTSIZE_VALUES (section, i, sizeof (Cgc32_External_##type),	    \
			sizeof (Cgc64_External_##type))

  for (i = 0, section = section_headers;
       i < cgc_header.e_shnum;
       i++, section++)
    {
      ...
      if (section->sh_type == SHT_DYNSYM)
	{
	  ...
	  dynamic_symbols = GET_CGC_SYMBOLS (file, section, & num_dynamic_syms);
	}
  ...
  for (i = 0, section = section_headers;
      ...
      if (is_32bit_cgc)
	{
	  ...
	  if (do_section_details)
	    ...
	    printf (" %3s ", get_cgc_section_flags (section->sh_flags));

	  if (section->sh_link >= cgc_header.e_shnum)
	    {
	      link_too_big = "";
      ...
      else
	{
      ...
      if (do_section_details)
	printf ("       %s\n", get_cgc_section_flags (section->sh_flags));
    }
...
process_section_groups (FILE * file)
{
  ...
  unsigned int i;
  struct group * group;
  Cgc_Internal_Shdr * symtab_sec;
  Cgc_Internal_Shdr * strtab_sec;
  Cgc_Internal_Sym * symtab;
  ...
  if (section_headers == NULL)
    {
      ...
      /* PR 13622: This can happen with a corrupt CGC header.  */
      return 0;
    }

  section_headers_groups = (struct group **) calloc (cgc_header.e_shnum,
                                                     sizeof (struct group *));
  ...
  for (i = 0, section = section_headers, group = section_groups;
      ...
      if (section->sh_type == SHT_GROUP)
	{
	  ...
	  if (symtab_sec != sec)
	    {
	      ...
	      if (symtab)
		free (symtab);
	      symtab = GET_CGC_SYMBOLS (file, symtab_sec, & num_syms);
	    }
	  ...
	  if (CGC_ST_TYPE (sym->st_info) == STT_SECTION)
	    {
	      if (sym->st_shndx == 0
		  || sym->st_shndx >= cgc_header.e_shnum)
		{
		  error (_("Bad sh_info in group section `%s'\n"), name);
		  continue;
		}
	  ...
	  else
	    {
	      ...
	      if (symtab_sec->sh_link >= cgc_header.e_shnum)
		{
		  strtab_sec = NULL;
		  if (strtab)
		    free (strtab);
	  ...
	  for (j = 0; j < size; j++)
	    {
	      ...
	      if (entry >= cgc_header.e_shnum)
		{
		  error (_("section [%5u] in group section [%5u] > maximum section [%5u]\n"),
			 entry, i, cgc_header.e_shnum - 1);
		  continue;
		}
...
{
  Cgc64_External_VMS_IMAGE_FIXUP *imfs;
  long i;
  const char *lib_name;
...
{
  Cgc64_External_VMS_IMAGE_RELA *imrs;
  long i;
	  ...
	  if (rel_size)
		...
		printf ("'%s'", SECTION_NAME (section));
		 ...
		 rel_offset, (unsigned long) (rel_size / section->sh_entsize));
		  ...
		  && section->sh_link < cgc_header.e_shnum)
		{
		  Cgc_Internal_Shdr * symsec;
		  Cgc_Internal_Sym *  symtab;
		  unsigned long nsyms;
		  ...
		  symtab = GET_CGC_SYMBOLS (file, symsec, & nsyms);

		  if (symtab == NULL)
		    continue;

		  if (symsec->sh_link != 0
		      && symsec->sh_link < cgc_header.e_shnum)
		    {
...
struct ia64_unw_aux_info
  {
    ...
    Cgc_Internal_Sym * symtab;	/* The symbol table.  */
...
static void
find_symbol_for_address (Cgc_Internal_Sym * symtab,
			 unsigned long nsyms,
			 const char * strtab,
  ...
  for (i = 0, sym = symtab; i < nsyms; ++i, ++sym)
    {
      ...
      if (CGC_ST_TYPE (sym->st_info) == STT_FUNC
	  && sym->st_name != 0
			 ...
			 Cgc_Internal_Shdr * sec)
{
  ...
  Cgc_Internal_Shdr * relsec;
  Cgc_Internal_Rela * rela;
  Cgc_Internal_Rela * rp;
  unsigned char * table;
  unsigned char * tp;
  Cgc_Internal_Sym * sym;
  const char * relname;
  ...
  for (relsec = section_headers;
       relsec < section_headers + cgc_header.e_shnum;
       ++relsec)
    {
      if (relsec->sh_type != SHT_RELA
	  || relsec->sh_info >= cgc_header.e_shnum
	...
	{
	  relname = cgc_ia64_reloc_type (get_reloc_type (rp->r_info));
...
ia64_process_unwind (FILE * file)
{
  ...
  for (i = 0, sec = section_headers; i < cgc_header.e_shnum; ++i, ++sec)
    {
      if (sec->sh_type == SHT_SYMTAB
	  && sec->sh_link < cgc_header.e_shnum)
	{
	  aux.symtab = GET_CGC_SYMBOLS (file, sec, & aux.nsyms);
  ...
  while (unwcount-- > 0)
	...
	if (sec->sh_type == SHT_IA_64_UNWIND)
      ...
      unwstart = i + 1;
      len = sizeof (CGC_STRING_ia64_unwind_once) - 1;
      ...
      if ((unwsec->sh_flags & SHF_GROUP) != 0)
	{
	  ...
	  for (; g != NULL; g = g->next)
	    {
	      ...
	      if (streq (SECTION_NAME (sec), CGC_STRING_ia64_unwind_info))
		break;
	    }
      ...
      else if (strneq (SECTION_NAME (unwsec), CGC_STRING_ia64_unwind_once, len))
	{
	  /* .gnu.linkonce.ia64unw.FOO -> .gnu.linkonce.ia64unwi.FOO.  */
	  len2 = sizeof (CGC_STRING_ia64_unwind_info_once) - 1;
	  suffix = SECTION_NAME (unwsec) + len;
	  for (i = 0, sec = section_headers; i < cgc_header.e_shnum;
	       ++i, ++sec)
	    if (strneq (SECTION_NAME (sec), CGC_STRING_ia64_unwind_info_once, len2)
      ...
      else
	{
	  ...
	  len = sizeof (CGC_STRING_ia64_unwind) - 1;
	  len2 = sizeof (CGC_STRING_ia64_unwind_info) - 1;
	  suffix = "";
	  if (strneq (SECTION_NAME (unwsec), CGC_STRING_ia64_unwind, len))
	    suffix = SECTION_NAME (unwsec) + len;
	  for (i = 0, sec = section_headers; i < cgc_header.e_shnum;
	       ++i, ++sec)
	    if (strneq (SECTION_NAME (sec), CGC_STRING_ia64_unwind_info, len2)
		&& streq (SECTION_NAME (sec) + len2, suffix))
	      break;
	}

      if (i == cgc_header.e_shnum)
	{
			 ...
			 Cgc_Internal_Shdr * sec)
{
  ...
  for (relsec = section_headers;
	...
	return 0;
      ...
      for (rp = rela; rp < rela + nrelas; ++rp)
	{
	  relname = cgc_hppa_reloc_type (get_reloc_type (rp->r_info));
	  sym = aux->symtab + get_reloc_symindex (rp->r_info);
...
struct arm_section
{
  ...
  Cgc_Internal_Shdr *  sec;		/* The cached unwind section header.  */
  Cgc_Internal_Rela *  rela;		/* The cached relocations for this section.  */
  unsigned long        nrelas;		/* The number of relocations.  */
  unsigned int         rel_type;	/* REL or RELA ?  */
  Cgc_Internal_Rela *  next_rela;	/* Cyclic pointer to the next reloc to process.  */
};

struct arm_unw_aux_info
{
  FILE *              file;		/* The file containing the unwind sections.  */
  Cgc_Internal_Sym *  symtab;		/* The file's symbol table.  */
  unsigned long       nsyms;		/* Number of symbols.  */
			 ...
			 bfd_vma *		    sym_name)
{
  ...
  if (sec != arm_sec->sec)
    {
      ...
      for (relsec = section_headers;
	   ...
	   ++relsec)
	{
	  if (relsec->sh_info >= cgc_header.e_shnum
	      || section_headers + relsec->sh_info != sec
  ...
  for (rp = arm_sec->next_rela; rp != arm_sec->rela + arm_sec->nrelas; rp++)
	...
	continue;
      ...
      sym = aux->symtab + CGC32_R_SYM (rp->r_info);
	...
	abort ();
      ...
      if (cgc_header.e_machine == EM_ARM)
	{
	  relname = cgc_arm_reloc_type (CGC32_R_TYPE (rp->r_info));
      ...
      else if (cgc_header.e_machine == EM_TI_C6000)
	{
	  relname = cgc_tic6x_reloc_type (CGC32_R_TYPE (rp->r_info));
...
decode_arm_unwind_bytecode (struct arm_unw_aux_info *aux,
			    ...
			    bfd_vma data_offset, Cgc_Internal_Shdr *data_sec,
...
arm_expand_prel31 (bfd_vma word, bfd_vma where)
{
  ...
  if (cgc_header.e_machine == EM_TI_C6000)
    offset <<= 1;
...
static void
dump_arm_unwind (struct arm_unw_aux_info *aux, Cgc_Internal_Shdr *exidx_sec)
{
  ...
  for (i = 0; i < exidx_len; i++)
      ...
      else
	{
	  ...
	  if (entry_addr.section != SHN_UNDEF
	      && entry_addr.section < cgc_header.e_shnum)
	    {
...
arm_process_unwind (FILE *file)
{
  ...
  for (i = 0, sec = section_headers; i < cgc_header.e_shnum; ++i, ++sec)
    {
      if (sec->sh_type == SHT_SYMTAB && sec->sh_link < cgc_header.e_shnum)
	{
	  aux.symtab = GET_CGC_SYMBOLS (file, sec, & aux.nsyms);
...
process_unwind (FILE * file)
{
  ...
  for (i = 0; handlers[i].handler != NULL; i++)
    if (cgc_header.e_machine == handlers[i].machtype)
      {
...
static void
dynamic_section_mips_val (Cgc_Internal_Dyn * entry)
{
  switch (entry->d_tag)
    {
    case DT_MIPS_FLAGS:
...
static void
dynamic_section_parisc_val (Cgc_Internal_Dyn * entry)
{
  switch (entry->d_tag)
    {
...
static void
dynamic_section_ia64_val (Cgc_Internal_Dyn * entry)
{
  switch (entry->d_tag)
    {
...
{
  Cgc32_External_Dyn * edyn;
  Cgc32_External_Dyn * ext;
  Cgc_Internal_Dyn * entry;

  edyn = (Cgc32_External_Dyn *) get_data (NULL, file, dynamic_addr, 1,
                                          dynamic_size, _("dynamic section"));
  if (!edyn)
    return 0;

/* SGI's CGC has more than one section in the DYNAMIC segment, and we
  ...
  dynamic_section = (Cgc_Internal_Dyn *) cmalloc (dynamic_nent,
                                                  sizeof (* entry));
...
{
  Cgc64_External_Dyn * edyn;
  Cgc64_External_Dyn * ext;
  Cgc_Internal_Dyn * entry;

  edyn = (Cgc64_External_Dyn *) get_data (NULL, file, dynamic_addr, 1,
                                          dynamic_size, _("dynamic section"));
  if (!edyn)
    return 0;
...
process_dynamic_section (FILE * file)
{
  ...
  if (dynamic_symbols == NULL)
	   ...
	   ++entry)
	{
	  ...
	  if (is_32bit_cgc)
	    section.sh_entsize = sizeof (Cgc32_External_Sym);
	  else
	    section.sh_entsize = sizeof (Cgc64_External_Sym);
  ...
  if (dynamic_syminfo == NULL)
	  ...
	  if (entry->d_tag == DT_SYMINENT)
	    {
		 ...
		 error from the SunOS4 C compiler.  */
	      assert (sizeof (Cgc_External_Syminfo) == entry->d_un.d_val);
	    }
	...
	{
	  Cgc_External_Syminfo * extsyminfo;
	  Cgc_External_Syminfo * extsym;
	  Cgc_Internal_Syminfo * syminfo;

	  /* There is a syminfo section.  Read the data.  */
	  extsyminfo = (Cgc_External_Syminfo *)
              get_data (NULL, file, dynamic_syminfo_offset, 1, syminsz,
                        _("symbol information"));
	  if (!extsyminfo)
	    return 0;

	  dynamic_syminfo = (Cgc_Internal_Syminfo *) malloc (syminsz);
	  if (dynamic_syminfo == NULL)
	    {
	      error (_("Out of memory\n"));
	      return 0;
	  ...
	  dynamic_syminfo_nent = syminsz / sizeof (Cgc_External_Syminfo);
...
process_version_sections (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
	...
	case SHT_GNU_verdef:
	  {
	    ...
	    for (idx = cnt = 0; cnt < section->sh_info; ++cnt)
	      {
		char * vstart;
		Cgc_External_Verdef * edef;
		Cgc_Internal_Verdef ent;
		Cgc_External_Verdaux * eaux;
		Cgc_Internal_Verdaux aux;
		int j;
		int isum;
		...
		edef = (Cgc_External_Verdef *) vstart;
		...
		eaux = (Cgc_External_Verdaux *) vstart;
	...
	case SHT_GNU_verneed:
	  {
	    ...
	    eneed = (Cgc_External_Verneed *) get_data (NULL, file,
                                                       section->sh_offset, 1,
                                                       section->sh_size,
	    ...
	    for (idx = cnt = 0; cnt < section->sh_info; ++cnt)
	      {
		Cgc_External_Verneed * entry;
		Cgc_Internal_Verneed ent;
		int j;
		int isum;
		char * vstart;
		...
		entry = (Cgc_External_Verneed *) vstart;
		...
		for (j = 0, isum = idx + ent.vn_aux; j < ent.vn_cnt; ++j)
		  {
		    Cgc_External_Vernaux * eaux;
		    Cgc_Internal_Vernaux aux;

		    if (vstart + sizeof (*eaux) > endbuf)
		      break;
		    eaux = (Cgc_External_Vernaux *) vstart;
	...
	case SHT_GNU_versym:
	  {
	    Cgc_Internal_Shdr * link_section;
	    int total;
	    int cnt;
	    unsigned char * edata;
	    unsigned short * data;
	    char * strtab;
	    Cgc_Internal_Sym * symbols;
	    Cgc_Internal_Shdr * string_sec;
	    unsigned long num_syms;
	    long off;
	    ...
	    total = section->sh_size / sizeof (Cgc_External_Versym);

	    if (link_section->sh_link >= cgc_header.e_shnum)
	      break;

	    found = 1;

	    symbols = GET_CGC_SYMBOLS (file, link_section, & num_syms);
	    if (symbols == NULL)
	      break;
		...
		for (j = 0; (j < 4) && (cnt + j) < total; ++j)
		  switch (data[cnt + j])
		    ...
		    default:
		      ...
		      check_def = 1;
		      check_need = 1;
		      if (symbols[cnt + j].st_shndx >= cgc_header.e_shnum
			...
			{
			  Cgc_Internal_Verneed ivn;
			  unsigned long offset;

			  offset = offset_from_vma
			    (file, version_info[DT_VERSIONTAGIDX (DT_VERNEED)],
			     sizeof (Cgc_External_Verneed));

			  do
			    {
			      Cgc_Internal_Vernaux ivna;
			      Cgc_External_Verneed evn;
			      Cgc_External_Vernaux evna;
			      unsigned long a_off;
			...
			{
			  Cgc_Internal_Verdef ivd;
			  Cgc_External_Verdef evd;
			  unsigned long offset;
			  ...
			  if (ivd.vd_ndx == (data[cnt + j] & VERSYM_VERSION))
			    {
			      Cgc_External_Verdaux evda;
			      Cgc_Internal_Verdaux ivda;
  ...
  switch (binding)
      ...
      else if (binding >= STB_LOOS && binding <= STB_HIOS)
	{
	  if (binding == STB_GNU_UNIQUE
	      && (cgc_header.e_ident[EI_OSABI] == CGCOSABI_GNU
...
get_symbol_type (unsigned int type)
{
  ...
  switch (type)
      ...
      if (type >= STT_LOPROC && type <= STT_HIPROC)
	{
	  ...
	  if (cgc_header.e_machine == EM_SPARCV9 && type == STT_REGISTER)
	    return "REGISTER";

	  if (cgc_header.e_machine == EM_PARISC && type == STT_PARISC_MILLI)
	    return "PARISC_MILLI";
	...
	{
	  if (cgc_header.e_machine == EM_PARISC)
	    {
	  ...
	  if (type == STT_GNU_IFUNC
	      && (cgc_header.e_ident[EI_OSABI] == CGCOSABI_GNU
		  || cgc_header.e_ident[EI_OSABI] == CGCOSABI_FREEBSD
		  /* GNU is still using the default value 0.  */
		  || cgc_header.e_ident[EI_OSABI] == CGCOSABI_NONE))
	    return "IFUNC";
...
get_symbol_index_type (unsigned int type)
{
  ...
  switch (type)
	...
	sprintf (buff, "RSV[0x%04x]", type & 0xffff);
      else if (type >= cgc_header.e_shnum)
...
print_dynamic_symbol (bfd_vma si, unsigned long hn)
{
  ...
  printf (" %-7s", get_symbol_type (CGC_ST_TYPE (psym->st_info)));
  printf (" %-6s",  get_symbol_binding (CGC_ST_BIND (psym->st_info)));
  printf (" %-7s",  get_symbol_visibility (CGC_ST_VISIBILITY (psym->st_other)));
  /* Check to see if any other bits in the st_other field are set.
     Note - displaying this information disrupts the layout of the
     table being generated, but for the moment this case is very
     rare.  */
  if (psym->st_other ^ CGC_ST_VISIBILITY (psym->st_other))
    printf (" [%s] ", get_symbol_other (psym->st_other ^ CGC_ST_VISIBILITY (psym->st_other)));
...
process_symbol_table (FILE * file)
{
  ...
  if (dynamic_info[DT_HASH]
	      ...
	      && dynamic_strings != NULL)))
    {
      ...
      if ((cgc_header.e_machine == EM_ALPHA
	   || cgc_header.e_machine == EM_S390
	   || cgc_header.e_machine == EM_S390_OLD)
	  && cgc_header.e_ident[EI_CLASS] == CGCCLASS64)
	hash_ent_size = 8;
  ...
  if ((dynamic_info[DT_HASH] || dynamic_info_DT_GNU_HASH)
	   ...
	   i++, section++)
	{
	  ...
	  symtab = GET_CGC_SYMBOLS (file, section, & num_syms);
	  if (symtab == NULL)
	    continue;

	  if (section->sh_link == cgc_header.e_shstrndx)
	    {
	      strtab = string_table;
	      strtab_size = string_table_length;
	    }
	  else if (section->sh_link < cgc_header.e_shnum)
	    {
	      Cgc_Internal_Shdr * string_sec;
	  ...
	  for (si = 0, psym = symtab; si < num_syms; si++, psym++)
		  ...
		  && version_info[DT_VERSIONTAGIDX (DT_VERSYM)] != 0)
		{
		  ...
		  is_nobits = (psym->st_shndx < cgc_header.e_shnum
...
static bfd_boolean
target_specific_reloc_handling (Cgc_Internal_Rela * reloc,
				unsigned char *     start,
				Cgc_Internal_Sym *  symtab)
{
  ...
  switch (cgc_header.e_machine)
    ...
    case EM_MSP430_OLD:
      {
	static Cgc_Internal_Sym * saved_sym = NULL;
...
/* Returns TRUE iff RELOC_TYPE is a 32-bit absolute RELA relocation used in
   ...
   we have already done with <cgc/h8.h>) because this would become very
		   ...
		   unsigned char * start)
{
  ...
  for (relsec = section_headers;
       ...
       ++relsec)
    {
	  ...
	  || relsec->sh_size == 0
	  || relsec->sh_link >= cgc_header.e_shnum)
	continue;
      ...
      else
	{
      ...
      if (cgc_header.e_machine == EM_SH)
	is_rela = FALSE;

      symsec = section_headers + relsec->sh_link;
      symtab = GET_CGC_SYMBOLS ((FILE *) file, symsec, & num_syms);
      ...
      for (rp = relocs; rp < relocs + num_relocs; ++rp)
	{
	  ...
	  if (sym != symtab
	      && CGC_ST_TYPE (sym->st_info) > STT_SECTION)
	    {
	      warn (_("skipping unexpected symbol type %s in %ld'th relocation in section %s\n"),
		    get_symbol_type (CGC_ST_TYPE (sym->st_info)),
		    (long int)(rp - relocs),
		    SECTION_NAME (relsec));
	      continue;
	    }
	  ...
	  /* R_XTENSA_32, R_PJ_DATA_DIR32 and R_D30V_32_NORMAL are
	     partial_inplace.  */
	  if (!is_rela
	      || (cgc_header.e_machine == EM_XTENSA
		  && reloc_type == 1)
	      || ((cgc_header.e_machine == EM_PJ
		   || cgc_header.e_machine == EM_PJ_OLD)
		  && reloc_type == 1)
	      || ((cgc_header.e_machine == EM_D30V
		   || cgc_header.e_machine == EM_CYGNUS_D30V)
		  && reloc_type == 12))
...
#ifdef SUPPORT_DISASSEMBLY
static int
disassemble_section (Cgc_Internal_Shdr * section, FILE * file)
{
...
static char *
get_section_contents (Cgc_Internal_Shdr * section, FILE * file)
{
...
static void
dump_section_as_strings (Cgc_Internal_Shdr * section, FILE * file)
{
  Cgc_Internal_Shdr * relsec;
  bfd_size_type num_bytes;
  char * data;
...
static void
dump_section_as_bytes (Cgc_Internal_Shdr * section,
		       FILE * file,
...
load_specific_debug_section (enum dwarf_section_display_enum debug,
			     Cgc_Internal_Shdr * sec, void * file)
{
...
static int
display_debug_section (int shndx, Cgc_Internal_Shdr * section, FILE * file)
{
  char * name = SECTION_NAME (section);
  ...
  for (cur = dump_sects_byname; cur; cur = cur->next)
    {
      ...
      for (i = 0, any = 0; i < cgc_header.e_shnum; i++)
...
process_section_contents (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
       i < cgc_header.e_shnum && i < num_dump_sects;
       i++, section++)
    {
#ifdef SUPPORT_DISASSEMBLY
...
display_sparc_hwcaps (int mask)
{
  if (mask)
    {
      int first = 1;
      if (mask & CGC_SPARC_HWCAP_MUL32)
	fputs ("mul32", stdout), first = 0;
      if (mask & CGC_SPARC_HWCAP_DIV32)
	printf ("%sdiv32", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_FSMULD)
	printf ("%sfsmuld", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_V8PLUS)
	printf ("%sv8plus", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_POPC)
	printf ("%spopc", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_VIS)
	printf ("%svis", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_VIS2)
	printf ("%svis2", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_ASI_BLK_INIT)
	printf ("%sASIBlkInit", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_FMAF)
	printf ("%sfmaf", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_VIS3)
	printf ("%svis3", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_HPC)
	printf ("%shpc", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_RANDOM)
	printf ("%srandom", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_TRANS)
	printf ("%strans", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_FJFMAU)
	printf ("%sfjfmau", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_IMA)
	printf ("%sima", first ? "" : "|"), first = 0;
      if (mask & CGC_SPARC_HWCAP_ASI_CACHE_SPARING)
...
print_mips_got_entry (unsigned char * data, bfd_vma pltgot, bfd_vma addr)
{
  ...
  printf (" ");
  if (data == NULL)
    printf ("%*s", is_32bit_cgc ? 8 : 16, _("<unknown>"));
  else
    {
      bfd_vma entry;

      entry = byte_get (data + addr - pltgot, is_32bit_cgc ? 4 : 8);
      print_vma (entry, LONG_HEX);
    }
  return addr + (is_32bit_cgc ? 4 : 8);
}
...
process_mips_specific (FILE * file)
{
  ...
  for (entry = dynamic_section; entry->d_tag != DT_NULL; ++entry)
	...
	liblist_offset
	  = offset_from_vma (file, entry->d_un.d_val,
			     liblistno * sizeof (Cgc32_External_Lib));
	break;
      case DT_MIPS_LIBLISTNO:
	...
	conflicts_offset
	  = offset_from_vma (file, entry->d_un.d_val,
			     conflictsno * sizeof (Cgc32_External_Conflict));
	break;
      case DT_MIPS_CONFLICTNO:
  ...
  if (liblist_offset != 0 && liblistno != 0 && do_dynamic)
    {
      Cgc32_External_Lib * elib;
      size_t cnt;

      elib = (Cgc32_External_Lib *) get_data (NULL, file, liblist_offset,
                                              liblistno,
                                              sizeof (Cgc32_External_Lib),
  ...
  if (options_offset != 0)
    {
      Cgc_External_Options * eopt;
      Cgc_Internal_Shdr * sect = section_headers;
      Cgc_Internal_Options * iopt;
      Cgc_Internal_Options * option;
      size_t offset;
      int cnt;
	...
	++sect;
      ...
      eopt = (Cgc_External_Options *) get_data (NULL, file, options_offset, 1,
                                                sect->sh_size, _("options"));
      if (eopt)
	{
	  iopt = (Cgc_Internal_Options *)
	  ...
	  while (offset < sect->sh_size)
	    {
	      Cgc_External_Options * eoption;

	      eoption = (Cgc_External_Options *) ((char *) eopt + offset);
	  ...
	  while (cnt-- > 0)
		...
		case ODK_REGINFO:
		  ...
		  if (cgc_header.e_machine == EM_MIPS)
		    {
		      /* 32bit form.  */
		      Cgc32_External_RegInfo * ereg;
		      Cgc32_RegInfo reginfo;

		      ereg = (Cgc32_External_RegInfo *) (option + 1);
		    ...
		    {
		      /* 64 bit form.  */
		      Cgc64_External_RegInfo * ereg;
		      Cgc64_Internal_RegInfo reginfo;

		      ereg = (Cgc64_External_RegInfo *) (option + 1);
		      reginfo.ri_gprmask    = BYTE_GET (ereg->ri_gprmask);
		      reginfo.ri_cprmask[0] = BYTE_GET (ereg->ri_cprmask[0]);
		      reginfo.ri_cprmask[1] = BYTE_GET (ereg->ri_cprmask[1]);
		      reginfo.ri_cprmask[2] = BYTE_GET (ereg->ri_cprmask[2]);
  ...
  if (conflicts_offset != 0 && conflictsno != 0)
      ...
      if (dynamic_symbols == NULL)
	{
      ...
      iconf = (Cgc32_Conflict *) cmalloc (conflictsno, sizeof (* iconf));
      if (iconf == NULL)
	{
	  error (_("Out of memory\n"));
	  return 0;
      ...
      if (is_32bit_cgc)
	{
	  Cgc32_External_Conflict * econf32;

	  econf32 = (Cgc32_External_Conflict *)
	...
	{
	  Cgc64_External_Conflict * econf64;

	  econf64 = (Cgc64_External_Conflict *)
              get_data (NULL, file, conflicts_offset, conflictsno,
      ...
      for (cnt = 0; cnt < conflictsno; ++cnt)
	{
	  Cgc_Internal_Sym * psym = & dynamic_symbols[iconf[cnt]];
  ...
  if (pltgot != 0 && local_gotno != 0)
      ...
      if (gotsym < symtabno)
	{
	  ...
	  sym_width = (is_32bit_cgc ? 80 : 160) - 28 - addr_size * 6 - 1;
	  for (i = gotsym; i < symtabno; i++)
	    {
	      ...
	      print_vma (psym->st_value, LONG_HEX);
	      printf (" %-7s %3s ",
		      get_symbol_type (CGC_ST_TYPE (psym->st_info)),
  ...
  if (mips_pltgot != 0 && jmprel != 0 && pltrel != 0 && pltrelsz != 0)
	...
	return 0;
      ...
      sym_width = (is_32bit_cgc ? 80 : 160) - 17 - addr_size * 6 - 1;
      for (i = 0; i < count; i++)
	{
	  Cgc_Internal_Sym * psym;
...
process_gnu_liblist (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
	...
	case SHT_GNU_LIBLIST:
	  ...
	  if (strtab == NULL
	      || section->sh_entsize != sizeof (Cgc32_External_Lib))
	    {
	      free (elib);
	      free (strtab);
	      break;
	  ...
	  printf (_("\nLibrary list section '%s' contains %lu entries:\n"),
		  SECTION_NAME (section),
		  (unsigned long) (section->sh_size / sizeof (Cgc32_External_Lib)));

	  puts (_("     Library              Time Stamp          Checksum   Version Flags"));

	  for (cnt = 0; cnt < section->sh_size / sizeof (Cgc32_External_Lib);
	       ++cnt)
	    {
	      Cgc32_Lib liblist;
	      time_t atime;
...
static int
print_core_note (Cgc_Internal_Note *pnote)
{
  unsigned int addr_size = is_32bit_cgc ? 4 : 8;
...
static const char *
get_gnu_cgc_note_type (unsigned e_type)
{
  static char buff[64];
...
static int
print_gnu_note (Cgc_Internal_Note *pnote)
{
  switch (pnote->type)
    {
    case NT_GNU_BUILD_ID:
...
static const char *
get_netbsd_cgccore_note_type (unsigned e_type)
{
  static char buff[64];
...
static int
print_stapsdt_note (Cgc_Internal_Note *pnote)
{
  int addr_size = is_32bit_cgc ? 4 : 8;
...
static int
print_ia64_vms_note (Cgc_Internal_Note * pnote)
{
  switch (pnote->type)
    {
    case NT_VMS_MHD:
...
/* Note that by the CGC standard, the name field is already null byte
   terminated, and namesz includes the terminating null byte.
   I.E. the value of namesz for the name "FSF" is 4.

   If the value of namesz is zero, there is no name present.  */
static int
process_note (Cgc_Internal_Note * pnote)
{
  ...
  if (pnote->namesz == 0)
    ...
    nt = get_gnu_cgc_note_type (pnote->type);

  else if (const_strneq (pnote->namedata, "NetBSD-CORE"))
    /* NetBSD-specific core file notes.  */
    nt = get_netbsd_cgccore_note_type (pnote->type);
...
{
  Cgc_External_Note * pnotes;
  Cgc_External_Note * external;
  int res = 1;

  if (length <= 0)
    return 0;

  pnotes = (Cgc_External_Note *) get_data (NULL, file, offset, 1, length,
					   _("notes"));
  if (pnotes == NULL)
    return 0;
  ...
  while ((char *) external < (char *) pnotes + length)
      ...
      if (!is_ia64_vms ())
	{
	  ...
	  min_notesz = offsetof (Cgc_External_Note, name);
	...
	{
	  Cgc64_External_VMS_Note *vms_external;

	  /* PR binutils/15191
	     Make sure that there is enough data to read.  */
	  min_notesz = offsetof (Cgc64_External_VMS_Note, name);
	  if (data_remaining < min_notesz)
	    {
	      warn (_("Corrupt note: only %d bytes remain, not enough for a full note\n"),
		    (int) data_remaining);
	  ...
	  vms_external = (Cgc64_External_VMS_Note *) external;
	  ...
	  || data_remaining < (size_t)(next - (char *) external))
	{
      ...
      external = (Cgc_External_Note *) next;

      /* Verify that name is null terminated.  It appears that at least
	 one version of Linux (RedHat 6.0) generates corefiles that don't
	 comply with the CGC spec by failing to include the null byte in
	 namesz.  */
...
process_note_sections (FILE * file)
{
  ...
  for (i = 0, section = section_headers;
       i < cgc_header.e_shnum && section != NULL;
       i++, section++)
...
get_file_header (FILE * file)
{
  ...
  if (fread (cgc_header.e_ident, EI_NIDENT, 1, file) != 1)
    return 0;

  /* Determine how to read the rest of the header.  */
  switch (cgc_header.e_ident[EI_DATA])
    {
    default: /* fall through */
    case CGCDATANONE: /* fall through */
    case CGCDATA2LSB:
  ...
  /* For now we only support 32 bit and 64 bit CGC files.  */
  is_32bit_cgc = (cgc_header.e_ident[EI_CLASS] != CGCCLASS64);

  /* Read in the rest of the header.  */
  if (is_32bit_cgc)
    {
      Cgc32_External_Ehdr ehdr32;

      if (fread (ehdr32.e_type, sizeof (ehdr32) - EI_NIDENT, 1, file) != 1)
	return 0;

      cgc_header.e_type      = BYTE_GET (ehdr32.e_type);
      cgc_header.e_machine   = BYTE_GET (ehdr32.e_machine);
      cgc_header.e_version   = BYTE_GET (ehdr32.e_version);
      cgc_header.e_entry     = BYTE_GET (ehdr32.e_entry);
      cgc_header.e_phoff     = BYTE_GET (ehdr32.e_phoff);
      cgc_header.e_shoff     = BYTE_GET (ehdr32.e_shoff);
      cgc_header.e_flags     = BYTE_GET (ehdr32.e_flags);
      cgc_header.e_ehsize    = BYTE_GET (ehdr32.e_ehsize);
      cgc_header.e_phentsize = BYTE_GET (ehdr32.e_phentsize);
      cgc_header.e_phnum     = BYTE_GET (ehdr32.e_phnum);
      cgc_header.e_shentsize = BYTE_GET (ehdr32.e_shentsize);
      cgc_header.e_shnum     = BYTE_GET (ehdr32.e_shnum);
      cgc_header.e_shstrndx  = BYTE_GET (ehdr32.e_shstrndx);
    }
  else
    {
      Cgc64_External_Ehdr ehdr64;

      /* If we have been compiled with sizeof (bfd_vma) == 4, then
	 we will not be able to cope with the 64bit data found in
	 64 CGC files.  Detect this now and abort before we start
	 overwriting things.  */
      if (sizeof (bfd_vma) < 8)
	{
	  error (_("This instance of readcgc has been built without support for a\n\
64 bit data type and so it cannot read 64 bit CGC files.\n"));
	  return 0;
	}
	...
	return 0;
      ...
      cgc_header.e_type      = BYTE_GET (ehdr64.e_type);
      cgc_header.e_machine   = BYTE_GET (ehdr64.e_machine);
      cgc_header.e_version   = BYTE_GET (ehdr64.e_version);
      cgc_header.e_entry     = BYTE_GET (ehdr64.e_entry);
      cgc_header.e_phoff     = BYTE_GET (ehdr64.e_phoff);
      cgc_header.e_shoff     = BYTE_GET (ehdr64.e_shoff);
      cgc_header.e_flags     = BYTE_GET (ehdr64.e_flags);
      cgc_header.e_ehsize    = BYTE_GET (ehdr64.e_ehsize);
      cgc_header.e_phentsize = BYTE_GET (ehdr64.e_phentsize);
      cgc_header.e_phnum     = BYTE_GET (ehdr64.e_phnum);
      cgc_header.e_shentsize = BYTE_GET (ehdr64.e_shentsize);
      cgc_header.e_shnum     = BYTE_GET (ehdr64.e_shnum);
      cgc_header.e_shstrndx  = BYTE_GET (ehdr64.e_shstrndx);
    }

  if (cgc_header.e_shoff)
    {
...
/* Process one CGC object file according to the command line options.
   This file may actually be stored in an archive.  The file is
   positioned at the start of the CGC object.  */
