...
#define SYM_SIZE(sym) \
  (sym->cgcinfo ? sym->cgcinfo->internal_cgc_sym.st_size: sym->ssize)
...
/* Sort the symbols by size.  CGC provides a size but for other formats
		      ...
		      struct size_sym **symsizesp)
{
  ...
  *symsizesp = symsizes;
  ...
  for (; from < fromend; from += size)
	...
	sz = ((cgc_symbol_type *) sym)->internal_cgc_sym.st_size;
...
print_symbol (bfd *abfd, asymbol *sym, bfd_vma ssize, bfd *archive_bfd)
{
  ...
  info.ssize = ssize;
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour)
    info.cgcinfo = (cgc_symbol_type *) sym;
  else
    info.cgcinfo = NULL;
...
print_symbol_info_sysv (struct extended_symbol_info *info, bfd *abfd)
{
  ...
  if (SYM_TYPE (info) == '-')
  ...
  else
    {
      ...
      if (info->cgcinfo)
	printf ("%18s|",
		get_symbol_type (CGC_ST_TYPE (info->cgcinfo->internal_cgc_sym.st_info)));
      else
	printf ("                  |");
      ...
      else
	{
      ...
      if (info->cgcinfo)
	printf("|     |%s", info->cgcinfo->symbol.section->name);
      else
	printf("|     |");
    }
}
