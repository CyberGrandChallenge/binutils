...
copy_usage (FILE *stream, int exit_status)
{
  ...
  -K --keep-symbol <name>          Do not strip symbol <name>\n\
     ...
     --localize-hidden             Turn all CGC hidden symbols into locals\n\
...
is_hidden_symbol (asymbol *sym)
{
  ...
  cgc_sym = cgc_symbol_from (sym->the_bfd, sym);
  if (cgc_sym != NULL)
    switch (CGC_ST_VISIBILITY (cgc_sym->internal_cgc_sym.st_other))
      {
      case STV_HIDDEN:
      case STV_INTERNAL:
	return TRUE;
  ...
  if (use_alt_mach_code != 0)
      ...
      if (! bfd_alt_mach_code (obfd, use_alt_mach_code))
	{
	  non_fatal (_("this target does not support %lu alternative machine codes"),
		     use_alt_mach_code);
	  if (bfd_get_flavour (obfd) == bfd_target_cgc_flavour)
	    {
	      non_fatal (_("treating that number as an absolute e_machine value instead"));
	      cgc_cgcheader (obfd)->e_machine = use_alt_mach_code;
	    }
	  else
...
setup_section (bfd *ibfd, sec_ptr isection, void *obfdarg)
{
  ...
  if (p != NULL)
    flags = p->flags | (flags & (SEC_HAS_CONTENTS | SEC_RELOC));
	   ...
	   && !(ibfd->xvec->flavour == bfd_target_cgc_flavour
		&& cgc_section_type (isection) == SHT_NOTE))
    {
      flags &= ~(SEC_HAS_CONTENTS | SEC_LOAD | SEC_GROUP);
      if (obfd->xvec->flavour == bfd_target_cgc_flavour)
	{
	  make_nobits = TRUE;

	  /* Twiddle the input section flags so that it seems to
	     cgc.c:copy_private_bfd_data that section flags have not
  ...
  if (make_nobits)
    cgc_section_type (osection) = SHT_NOBITS;
