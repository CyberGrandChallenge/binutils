...
#ifdef DLLTOOL_MCORE_CGC
static void mcore_cgc_cache_filename (const char *);
static void mcore_cgc_gen_out_file (void);
#endif
...
#ifdef DLLTOOL_DEFAULT_MCORE_CGC
static const char * mname = "mcore-cgc";
static char * mcore_cgc_out_file = NULL;
static char * mcore_cgc_linker   = NULL;
static char * mcore_cgc_linker_flags = NULL;

#define DRECTVE_SECTION_NAME ((machine == MMCORE_CGC || machine == MMCORE_CGC_LE) ? ".exports" : ".drectve")
#endif
...
#define MMCORE_LE 6
  ...
  {
#define MMCORE_CGC 7
    "mcore-cgc-be", ".byte", ".short", ".long", ".asciz", "//",
    "lrw r1,[1f]\n\tld.w r1,(r1,0)\n\tjmp r1\n\tnop\n1:.long",
    ".global", ".space", ".align\t2",".align\t4", "",
    "cgc32-mcore-big", bfd_arch_mcore,
    mcore_be_jtab, sizeof (mcore_be_jtab), 8,
    0, 0, 0, 0, 0, 0
  }
  ,
  {
#define MMCORE_CGC_LE 8
    "mcore-cgc-le", ".byte", ".short", ".long", ".asciz", "//",
    "lrw r1,[1f]\n\tld.w r1,(r1,0)\n\tjmp r1\n\tnop\n1:.long",
    ".global", ".space", ".align\t2",".align\t4", "-EL",
    "cgc32-mcore-little", bfd_arch_mcore,
    mcore_le_jtab, sizeof (mcore_le_jtab), 8,
    0, 0, 0, 0, 0, 0
  }
  ,
...
#ifdef DLLTOOL_MCORE_CGC
      if (mcore_cgc_out_file)
	inform (_("Cannot produce mcore-cgc dll from archive file: %s"), filename);
#endif
    }
...
#ifdef DLLTOOL_MCORE_CGC
      if (mcore_cgc_out_file)
	mcore_cgc_cache_filename (filename);
#endif
    }
...
usage (FILE *file, int status)
{
  ...
  fprintf (file, _("        possible <machine>: arm[_interwork], i386, mcore[-cgc]{-le|-be}, ppc, thumb\n"));
...
#ifdef DLLTOOL_MCORE_CGC
  fprintf (file, _("   -M --mcore-cgc <outname>  Process mcore-cgc object files into <outname>.\n"));
...
static const struct option long_options[] =
{
  ...
  {"mcore-cgc", required_argument, NULL, 'M'},
...
#ifdef DLLTOOL_MCORE_CGC
	case 'M':
	  mcore_cgc_out_file = optarg;
	  break;
	case 'L':
	  mcore_cgc_linker = optarg;
	  break;
	case 'F':
	  mcore_cgc_linker_flags = optarg;
	  break;
#endif
	case 'C':
...
static void
mcore_cgc_cache_filename (const char * filename)
{
  fname_cache * ptr;
...
#define MCORE_CGC_TMP_OBJ "mcorecgc.o"
#define MCORE_CGC_TMP_EXP "mcorecgc.exp"
#define MCORE_CGC_TMP_LIB "mcorecgc.lib"

static void
mcore_cgc_gen_out_file (void)
{
  fname_cache * ptr;
  dyn_string_t ds;
  ...
  if (mcore_cgc_linker_flags != NULL)
    dyn_string_append_cstr (ds, mcore_cgc_linker_flags);
  ...
  dyn_string_append_cstr (ds, MCORE_CGC_TMP_OBJ);

  if (mcore_cgc_linker == NULL)
    mcore_cgc_linker = deduce_name ("ld");

  run (mcore_cgc_linker, ds->s);
  ...
  dyn_string_append_cstr (ds, MCORE_CGC_TMP_EXP);
  dyn_string_append_cstr (ds, " -l ");
  dyn_string_append_cstr (ds, MCORE_CGC_TMP_LIB);
  dyn_string_append_cstr (ds, " " );
  dyn_string_append_cstr (ds, MCORE_CGC_TMP_OBJ);
  ...
  dyn_string_append_cstr (ds, MCORE_CGC_TMP_OBJ);
  dyn_string_append_cstr (ds, " -o ");
  dyn_string_append_cstr (ds, mcore_cgc_out_file);
  ...
  if (dontdeltemps == 0)
    unlink (MCORE_CGC_TMP_EXP);

  if (dontdeltemps < 2)
    unlink (MCORE_CGC_TMP_OBJ);
}
#endif /* DLLTOOL_MCORE_CGC */
