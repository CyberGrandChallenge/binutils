...
/* Internal headers for the CGC .stab-dump code - sorry.  */
		     ...
		     void *ignored ATTRIBUTE_UNUSED)
{
  ...
  /* Ignore linker created section.  See cgcNN_ia64_object_p in
     bfd/cgcxx-ia64.c.  */
...
#else
  /* With BFD64, non-CGC returns -1 and wants always 64 bit addresses.  */
...
disassemble_section (bfd *abfd, asection *section, void *inf)
{
  ...
  if (bfd_get_flavour (abfd) == bfd_target_cgc_flavour
      && (bed = get_cgc_backend_data (abfd)) != NULL
      && bed->sign_extend_vma)
		     ...
		     unsigned *string_offset_ptr)
{
  ...
  /* Loop through all symbols and print them.
     ...
     the front of stabs-in-{coff,cgc} sections that supplies sizes.  */
  ...
  for (p = relpp; relcount && *p != NULL; p++, relcount--)
      ...
      else if (q->howto->name)
	{
	  ...
	  /* R_SPARC_OLO10 relocations contain two addends.
	     ...
	     store them both, the 64-bit CGC Sparc backend
	  ...
	  if (abfd->xvec->flavour == bfd_target_cgc_flavour
	      && cgc_tdata(abfd)->cgc_header->e_machine == EM_SPARCV9
	      && relcount > 1
