...
dnl Check whether weak refs work like the CGC ones.
...
AC_DEFUN([GCC_CHECK_CGC_STYLE_WEAKREF], [
  AC_CACHE_CHECK([whether weak refs work like CGC],
                  ac_cv_have_cgc_style_weakref, [
  weakref_m4_saved_CFLAGS="$CFLAGS"
  case "${host}" in
...
]])], ac_cv_have_cgc_style_weakref=yes, ac_cv_have_cgc_style_weakref=no, [
case "${host}" in
  *-apple-darwin[[89]]*) ac_cv_have_cgc_style_weakref=no ;;
  *) ac_cv_have_cgc_style_weakref=yes;;
esac])CFLAGS="$weakref_m4_saved_CFLAGS"])
if test x"$ac_cv_have_cgc_style_weakref" = xyes; then
  AC_DEFINE(HAVE_CGC_STYLE_WEAKREF, 1, [Define to 1 if target has a weakref that works like the CGC one.])
fi])
