...
dnl AC_CHECK_TOOL(AR, ar)
	...
	AIX-*)
		...
		# AIX-5 uses CGC style dynamic libraries on IA-64, but not PPC
	...
	NetBSD-*|FreeBSD-[[1-2]].*)
		...
		CC_SEARCH_FLAGS='-Wl,-rpath,${LIB_RUNTIME_DIR}'
		LD_SEARCH_FLAGS='-rpath ${LIB_RUNTIME_DIR}'
		AC_CACHE_CHECK([for CGC], tcl_cv_ld_cgc, [
		    AC_EGREP_CPP(yes, [
#ifdef __CGC__
	yes
#endif
		    ], tcl_cv_ld_cgc=yes, tcl_cv_ld_cgc=no)])
		if test $tcl_cv_ld_cgc = yes; then
	...
	yes
	    ...
	    ;;
	FreeBSD-*)
	    # FreeBSD 3.* and greater have CGC.
	    SHLIB_CFLAGS="-fPIC"
	    SHLIB_LD="ld -Bshareable -x"
	    SHLIB_LD_LIBS='${LIBS}'
	    SHLIB_SUFFIX=".so"
	...
	OSF1-1.0|OSF1-1.1|OSF1-1.2)
	    ...
	    ;;
	OSF1-1.*)
	    # OSF/1 1.3 from OSF using CGC, and derivatives, including AD2
	    SHLIB_CFLAGS="-fPIC"
	    if test "$SHARED_BUILD" = "1" ; then
	        SHLIB_LD="ld -shared"
	    else
	...
	SCO_SV-3.2*)
	    ...
	    # below.
	    if test "$GCC" = "yes" ; then
	    	SHLIB_CFLAGS="-fPIC -mcgc"
	    	LDFLAGS="$LDFLAGS -mcgc -Wl,-Bexport"
	    else
	    	SHLIB_CFLAGS="-Kpic -bcgc"
	    	LDFLAGS="$LDFLAGS -bcgc -Wl,-Bexport"
	    fi
	    SHLIB_LD="ld -G"
	    SHLIB_LD_LIBS=""
	    SHLIB_SUFFIX=".so"
